...
name: db-dump-cf
description: >
  Этот скилл MUST быть вызван когда пользователь просит выгрузить конфигурацию в CF, сохранить конфигурацию, сделать бэкап CF.
  SHOULD также вызывать перед крупными изменениями как контрольная точка.
  Do NOT использовать для выгрузки в XML-файлы — используй db-dump-xml; для загрузки CF — используй db-load-cf.
