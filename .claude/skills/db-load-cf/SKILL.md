...
name: db-load-cf
description: >
  Этот скилл MUST быть вызван когда пользователь просит загрузить конфигурацию из CF, восстановить из бэкапа CF.
  SHOULD также вызывать после получения нового CF-файла для обновления базы.
  Do NOT использовать для загрузки из XML — используй db-load-xml; для выгрузки CF — используй db-dump-cf.
