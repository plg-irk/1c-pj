...
name: web-publish
description: >
  Этот скилл MUST быть вызван когда пользователь просит опубликовать базу, настроить веб-доступ, веб-клиент, открыть в браузере через Apache.
  SHOULD также вызывать после создания или обновления базы для обеспечения доступа через браузер.
  Do NOT использовать для проверки статуса — используй web-info; для автоматизации браузера — используй web-test или 1c-web-session.
