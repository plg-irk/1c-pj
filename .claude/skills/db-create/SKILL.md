...
name: db-create
description: >
  Этот скилл MUST быть вызван когда пользователь просит создать базу, новую ИБ, пустую информационную базу 1С.
  SHOULD также вызывать при старте работы над новым проектом.
  Do NOT использовать для управления реестром баз — используй db-list; для запуска базы — используй db-run.
