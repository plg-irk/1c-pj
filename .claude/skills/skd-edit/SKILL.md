...
name: skd-edit
description: >
  Этот скилл MUST быть вызван когда нужно модифицировать существующую СКД: добавить поля, итоги, фильтры, параметры, изменить текст запроса.
  SHOULD также вызывать для инкрементальных изменений готовых отчётов.
  Do NOT использовать для создания СКД с нуля — используй skd-compile; для анализа СКД — используй skd-info.
