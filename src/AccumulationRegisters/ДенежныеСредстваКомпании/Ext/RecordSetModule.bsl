...
Функция ВалютаСтруктурнойЕдиницыКорректна()
	Если ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.КассыКомпании") Тогда
		Если НЕ СтруктурнаяЕдиница.МноговалютнаяКасса И СтруктурнаяЕдиница.ВалютаДенежныхСредств <> Валюта  Тогда
			ВывестиСообщение(НСтр("ru = 'Не соответствует валюта денежных средств .'"),ДокументОбъект,,,Истина);
			Возврат Ложь;
		КонецЕсли; 
	ИначеЕсли ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		Если СтруктурнаяЕдиница.ВалютаДенежныхСредств <> Валюта  Тогда
			ВывестиСообщение(НСтр("ru = 'Не соответствует валюта расчетного счета .'"),ДокументОбъект,,,Истина);
			Возврат Ложь;
		КонецЕсли; 
	ИначеЕсли ТипЗнч(СтруктурнаяЕдиница) = Тип("СправочникСсылка.ЦенныеБумаги") Тогда
		Если СтруктурнаяЕдиница.ВалютаДенежныхСредств <> Валюта  Тогда
			ВывестиСообщение(НСтр("ru = 'Не соответствует валюта ценной бумаги .'"),ДокументОбъект,,,Истина);
			Возврат Ложь;
		КонецЕсли; 
	КонецЕсли; 
	Возврат Истина;
...
Функция Приход() Экспорт
	...
	Если (НЕ ЗначениеЗаполнено(Валюта)) И (НЕ ЗначениеЗаполнено(СтруктурнаяЕдиница)) Тогда
		ВывестиСообщение(НСтр("ru = 'Не достаточно параметров для проведения по регистру ""ДенежныеСредстваКомпании"" .'"),ДокументОбъект,,,Истина);
...
Функция Расход() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтруктурнаяЕдиница) Тогда
		ВывестиСообщение(НСтр("ru = 'Не достаточно параметров для проведения по регистру ""ДенежныеСредстваКомпании"" .'"), ДокументОбъект);
		// Убиваем циклическую ссылку
		Валюта			= Неопределено;
		ДокументОбъект	= Неопределено;
		Возврат Результат;
	...
	Если (СуммаОстатка-Сумма) < 0 Тогда
		...
		Если 	  (ТипСтруктурнойЕдиницы=Тип("СправочникСсылка.КассыКомпании")) ИЛИ (ТипСтруктурнойЕдиницы=Тип("СправочникСсылка.КассыККМ")) Тогда
			Результат = ПравоПользователя("РазрешитьОтрицательныеОстаткиПоКассе",ДокументОбъект);
		ИначеЕсли (ТипСтруктурнойЕдиницы=Тип("СправочникСсылка.БанковскиеСчета")) ИЛИ (ТипСтруктурнойЕдиницы=Тип("СправочникСсылка.ЦенныеБумаги")) Тогда
			Результат = ПравоПользователя("РазрешитьОтрицательныеОстаткиПоБанку",ДокументОбъект);
		Иначе
			Результат = Ложь;
		КонецЕсли;
		Если Результат Тогда
			ВывестиСообщение(НСтр("ru = 'Получены отрицательные суммовые остатки по <%1>'"), ДокументОбъект,,,, СокрЛП(СтруктурнаяЕдиница));
		Иначе
			ВывестиСообщение(НСтр("ru = 'Нет прав на отрицательные суммовые остатки по <%1>'"), ДокументОбъект,,,, СокрЛП(СтруктурнаяЕдиница));
			Возврат Результат;
		КонецЕсли;
	Иначе
		Результат = Истина;	
...
Функция СписаниеКурсовыхРазниц() Экспорт
	...
	|	ЕСТЬNULL(ДенежныеСредстваКомпанииОстатки.СуммаРегОстаток,0) КАК СуммаРег,
	|	ВЫБОР
