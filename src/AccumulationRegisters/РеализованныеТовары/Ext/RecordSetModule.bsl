...
Функция ВозвратТоваров()
	...
	Если РезультатЗапросаПоПартиям = Неопределено Тогда
		...
		Иначе
			ВывестиСообщение(НСтр("ru = 'Документ <%1> не имеет информации о движениях по партиям.'"), ДокументОбъект,,,, СокрЛП(ДокументОбъект));
		КонецЕсли; 
	...
	ТаблицаВозвращенныхТоваров.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, Партия, СтатусПартии", "Количество");
	...
	Для Каждого СтрокаТовары Из ТаблицаВозвращенныхТоваров Цикл
		...
		Если НадоВернуть > 0 Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТовары.ХарактеристикаНоменклатуры) Тогда
				ВывестиСообщение(НСтр("ru = '[%1] Товар ""%2"" вернуть не удалось, так как товар в указанном количестве не был взят на комиссию .'"), ДокументОбъект,,, Истина, ЗначениеКолонкиКода, СокрЛП(СтрокаТовары.Номенклатура));
			Иначе
				ВывестиСообщение(НСтр("ru = '[%1] Товар ""%2"" с характеристикой ""%3"" вернуть не удалось, так как товар в указанном количестве не был взят на комиссию .'"), ДокументОбъект,,, Истина, ЗначениеКолонкиКода, СокрЛП(СтрокаТовары.Номенклатура), СокрЛП(СтрокаТовары.ХарактеристикаНоменклатуры));
			КонецЕсли; 
			ВсеОК = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция Приход() Экспорт
	...
	КонецЕсли;
	ТаблицаПартий.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, Партия, СтатусПартии", "Количество,Сумма,СуммаНДС,СуммаБезНДС,СуммаУпр,СуммаНДСУпр,СуммаБезНДСУпр");
...
Функция Расход() Экспорт
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Для Сч = 0 По МассивНайденныхСтрок.ВГраница() Цикл
			...
			НоваяЗапись.СуммаНДСУпр 				= Окр(?(ТекСтрока.СуммаНДСУпр = NULL, 0, ?(ТекСтрока.Количество <= НадоСписать, ТекСтрока.СуммаНДСУпр, ТекСтрока.СуммаНДСУпр / ТекСтрока.Количество * НадоСписать)), 2);
			НоваяЗапись.СуммаНДС 					= Окр(?(ТекСтрока.СуммаНДС = NULL, 0, ?(ТекСтрока.Количество <= НадоСписать, ТекСтрока.СуммаНДС, ТекСтрока.СуммаНДС / ТекСтрока.Количество * НадоСписать)), 2);
			НоваяЗапись.СуммаБезНДС					= НоваяЗапись.СуммаРегл - НоваяЗапись.СуммаНДС;
			НоваяЗапись.СуммаБезНДСУпр				= НоваяЗапись.СуммаУпр - НоваяЗапись.СуммаНДСУпр;
		...
		Если НадоСписать > 0 Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаТовары.ХарактеристикаНоменклатуры) Тогда
				ВывестиСообщение(НСтр("ru = '[%1] Товар ""%2"" в количестве %3 %4 не брался на комиссию.'"), ДокументОбъект,,, Истина, ЗначениеКолонкиКода, СокрЛП(ВыборкаТовары.Номенклатура), Формат(ВыборкаТовары.Количество,"ЧДЦ=3; ЧН=0,00"), СокрЛП(ВыборкаТовары.Номенклатура.ОсновнаяЕдиницаИзмерения));
			Иначе
				ВывестиСообщение(НСтр("ru = '[%1""] Товар ""%2"" с характеристикой ""%3"" в количестве %4 %5 не брался на комиссию.'"), ДокументОбъект,,, Истина, ЗначениеКолонкиКода, СокрЛП(ВыборкаТовары.Номенклатура), СокрЛП(ВыборкаТовары.ХарактеристикаНоменклатуры), Формат(ВыборкаТовары.Количество,"ЧДЦ=3; ЧН=0,00"), СокрЛП(ВыборкаТовары.Номенклатура.ОсновнаяЕдиницаИзмерения));
			КонецЕсли;
			ВсеОК = Ложь;
		КонецЕсли;
