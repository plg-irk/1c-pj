// Модуль формы элемента справочника "Типы оплат"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Предопределенный Тогда
		...
		Если (Объект.Ссылка = Справочники.ТипыОплат.Наличные И Объект.ТипОплатыККТ = Перечисления.ТипыОплатыККТ.Наличные
			ИЛИ Объект.Ссылка = Справочники.ТипыОплат.ЗачетАванса И Объект.ТипОплатыККТ= Перечисления.ТипыОплатыККТ.Предоплата
			ИЛИ Объект.Ссылка = Справочники.ТипыОплат.ОплатаВРассрочку И Объект.ТипОплатыККТ = Перечисления.ТипыОплатыККТ.Постоплата
			ИЛИ Объект.Ссылка = Справочники.ТипыОплат.Безналичные И Объект.ТипОплатыККТ = Перечисления.ТипыОплатыККТ.Электронно
			ИЛИ Объект.Ссылка = Справочники.ТипыОплат.Талон И Объект.ТипОплатыККТ = Перечисления.ТипыОплатыККТ.Предоплата)
			И ЗначениеЗаполнено(Объект.ТипОплатыККТ) Тогда
			
			Элементы.ТипОплатыККТ.ТолькоПросмотр = Истина;
			
		КонецЕсли;
		
		Если (Объект.Ссылка = Справочники.ТипыОплат.ЗачетАванса И Объект.ВидОплаты = Перечисления.ВидыОплат.ЗаСчетЗаведения
			ИЛИ Объект.Ссылка = Справочники.ТипыОплат.ОплатаВРассрочку И Объект.ВидОплаты = Перечисления.ВидыОплат.ЗаСчетЗаведения)
			И ЗначениеЗаполнено(Объект.ВидОплаты) Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.Предопределенный Тогда
		Если Объект.Ссылка = Справочники.ТипыОплат.Наличные И Объект.ТипОплатыККТ <> Перечисления.ТипыОплатыККТ.Наличные Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Наличными"" может быть установлено значение типа оплат ККТ только ""Наличная оплата""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.ЗачетАванса И Объект.ТипОплатыККТ <> Перечисления.ТипыОплатыККТ.Предоплата Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Зачет аванса"" может быть установлено значение типа оплат ККТ только ""Предоплата""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.ЗачетАванса И Объект.ВидОплаты <> Перечисления.ВидыОплат.ЗаСчетЗаведения Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Зачет аванса"" может быть установлено значение ""Объект"" только ""Передача без внесения денежных средств""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.ОплатаВРассрочку И Объект.ТипОплатыККТ <> Перечисления.ТипыОплатыККТ.Постоплата Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Оплата в рассрочку"" может быть установлено значение типа оплат ККТ только ""Постоплата (кредит)""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.ОплатаВРассрочку И Объект.ВидОплаты <> Перечисления.ВидыОплат.ЗаСчетЗаведения Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Оплата в рассрочку"" может быть установлено значение ""Объект"" только ""Передача без внесения денежных средств""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.Безналичные И Объект.ТипОплатыККТ <> Перечисления.ТипыОплатыККТ.Электронно Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Плат. картой"" может быть установлено значение типа оплат ККТ только ""Электронные средства""'"),,,,Отказ);
		ИначеЕсли Объект.Ссылка = Справочники.ТипыОплат.Талон И Объект.ТипОплатыККТ <> Перечисления.ТипыОплатыККТ.Предоплата Тогда
			ВывестиСообщение(НСтр("ru='Для предопределенного значения типа оплат ""Талон"" может быть установлено значение типа оплат ККТ только ""Предоплата""'"),,,,Отказ);
		КонецЕсли;
	КонецЕсли;
