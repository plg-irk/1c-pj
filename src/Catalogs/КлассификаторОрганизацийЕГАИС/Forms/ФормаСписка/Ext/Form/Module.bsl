...
Функция ПолучитьРеквизитыОрганизации(Ссылка)
	...
	Если Выборка.Следующий() Тогда
		Результат.ИНН = СокрЛП(Выборка.ИНН);
		Результат.Код = СокрЛП(Выборка.Код);
	КонецЕсли;
...
Процедура ОбработкаВводаПараметраЗапроса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ВывестиСообщение(НСтр("ru = 'Не заполнен %1 организации'"), ЭтотОбъект, "Объект.ИНН",,, ДополнительныеПараметры.ИмяПараметра);
	КонецЕсли;
...
Процедура ЗапроситьОрганизациюПоИНН(Команда)
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		РеквизитыОрганизации = ПолучитьРеквизитыОрганизации(ТекущаяСтрока);
	...
	Если ПустаяСтрока(ИНН) Тогда
		...
		ДополнительныеПараметры.Вставить("ВидСообщения", ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросОрганизацииВерсия2"));
	...
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросОрганизацииВерсия2"), "ИНН", ИНН);
...
Процедура ЗапроситьОрганизациюПоКодуФСРАР(Команда)
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		РеквизитыОрганизации = ПолучитьРеквизитыОрганизации(ТекущаяСтрока);
		СИО = РеквизитыОрганизации.Код;
	КонецЕсли;
	
	Если ПустаяСтрока(СИО) Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИмяПараметра", "СИО");
		ДополнительныеПараметры.Вставить("ВидСообщения", ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросОрганизацииВерсия2"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаВводаПараметраЗапроса", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВводЗначения(ОписаниеОповещения, СИО, "Введите СИО производителя, импортера или собственной организации", Тип("Строка"));
		Возврат;
	КонецЕсли;
	
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросОрганизацииВерсия2"), "СИО", СИО);
...
Процедура ЗапроситьАлкогольнуюПродукцию(Команда)
	...
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросАлкогольнойПродукцииВерсия2"), "ИНН", ИНН);
