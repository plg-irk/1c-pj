...
Процедура УправлениеДиалогомНаСервере()
	...
	Если Объект.Ссылка = Справочники.ТипыСкидок.СкидкаНаШтрихкодТовара Тогда
		Элементы.ГруппаЗначенияСкидок.Видимость = Истина;
	Иначе
		Элементы.ГруппаЗначенияСкидок.Видимость = Ложь;
	КонецЕсли;
...
Процедура БлокироватьЭлементыФормы()
	...
	НеобходимостьБлокировки = ОбработкаСобытийОбъектаСервер.ПроверитьИспользованиеСсылки(Объект.Ссылка,"РегистрСведений.СкидкиСтроки","Скидка") И ОбработкаСобытийОбъектаСервер.ПроверитьИспользованиеСсылки(Объект.Ссылка,"РегистрСведений.СкидкиШапки","Скидка");
	
	// Составим список элементов для блокировки
	СписокРеквизитов = Новый СписокЗначений();
	СписокРеквизитов.Добавить("ФлагВытеснения","ФлагВытеснения",НеобходимостьБлокировки);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "РучнаяСкидка".
//
// Параметры:
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "СпособВычисления".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура СпособВычисленияПриИзменении(Элемент)
	...
	СформироватьНаименованиеПоУмолчанию();
	СпособВычисленияПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура СпособВычисленияПриИзмененииНаСервере()
	
	УправлениеДиалогомНаСервере();
	
КонецПроцедуры

// Обработчик события возникающего на клиенте при изменении данных реквизита "ФлагВытеснения".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
