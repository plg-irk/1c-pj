...
Процедура ЗаполнитьТаблицуВидовТелефонов()
	...
	НоваяСтрока.Наименование	= НСтр("ru='Внутренний номер'");
	НоваяСтрока.Псевдоним		= НСтр("ru='Внутренний'");
	...
	НоваяСтрока.Наименование	= НСтр("ru='Рабочий номер'");
	НоваяСтрока.Псевдоним		= НСтр("ru='Рабочий'");
	...
	НоваяСтрока.Пометка			= Истина;
	НоваяСтрока.Наименование	= НСтр("ru='Мобильный номер'");
	НоваяСтрока.Псевдоним		= НСтр("ru='Мобильный'");
	НоваяСтрока.Вид				= ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ПустаяСсылка");
	НоваяСтрока.Тип				= 0;
	// ТелефонПользователя
	НоваяСтрока = Объект.ВидыТелефонов.Добавить();
	...
	НоваяСтрока.Наименование	= НСтр("ru='Домашний номер'");
	НоваяСтрока.Псевдоним		= НСтр("ru='Домашний'");
	...
	НоваяСтрока.Наименование	= НСтр("ru='Дополнительный номер'");
	НоваяСтрока.Псевдоним		= НСтр("ru='Дополнительный'");
	НоваяСтрока.Вид				= ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ПустаяСсылка");
	НоваяСтрока.Тип				= 0;
	// Не определен
	НоваяСтрока = Объект.ВидыТелефонов.Добавить();
...
Процедура ПроверитьЗаполнениеТаблицыВидовТелефонов()
	Для Каждого СтрокаВида Из Объект.ВидыТелефонов Цикл
		...
		ИначеЕсли СтрНайти(СтрокаВида.Вид, НСтр("ru='<Объект не найден>'")) = 1 Тогда
...
Процедура ЗаполнитьДеревоПользователей()
	...
	Для Каждого СтрокаВида Из Объект.ВидыТелефонов Цикл
		Если СтрокаВида.Наименование = НСтр("ru='Внутренний номер'") Тогда
...
Процедура ДобавитьСтрокуСтруктурыВТелефоннуюКнигу(ЭлементМассива, СтрокиДерева, Родитель)
	Если СтрНайти(ЭлементМассива.Объект, НСтр("ru='<Объект не найден>'")) = 1 Тогда Возврат; КонецЕсли;
	Если ЗначениеЗаполнено(ЭлементМассива.Объект) Тогда	
		Если ЭлементМассива.Картинка = 2 Тогда
			Если ТипЗнч(ЭлементМассива.Объект) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекущийВнутреннийНомер	= сфпСофтФонПроСервер.сфпПолучитьЗначениеРеквизита(ЭлементМассива.Объект, "сфпТекущийВнутреннийНомер");
...
Процедура ДобавитьСтрокуВТелефоннуюКнигу(СтрокаКонтакта, СтрокаКниги)
	...
	Если НЕ СтрокаНайдена Тогда
		...
		Если СтрокаКонтакта.Картинка = 2 Тогда
			...
			Иначе
				НайденныеСтроки = Объект.ВидыТелефонов.НайтиСтроки(Новый Структура("Вид", НСтр("ru='Внутренний номер'")));
			КонецЕсли;	
...
Функция ПроверитьЗаполнениеТелефоннойКниги()
	...
	Если ПустаяСтрока(Объект.Наименование) Тогда	
		ЕстьОшибки = Истина;
		ВывестиСообщение(НСтр("ru='Не заполнено наименование телефонной книги'"));
	КонецЕсли;	
	НайденныеСтроки = Объект.ВидыТелефонов.НайтиСтроки(Новый Структура("Пометка", Истина));
	Если НайденныеСтроки.Количество() = 0 Тогда
		ЕстьОшибки = Истина;
		ВывестиСообщение(НСтр("ru='Не выбран ни один вид телефонного номера'"));
	КонецЕсли;
	Если ДеревоТелефоннойКниги.ПолучитьЭлементы().Количество() = 0 Тогда
		ЕстьОшибки = Истина;
		ВывестиСообщение(НСтр("ru='Не заполнена телефонная книга'"));
	КонецЕсли;	
...
Процедура ДобавитьНовуюГруппуВТелефоннуюКнигу(Строка, ДополнительныеПараметры) Экспорт
	...
	НоваяСтрокаКниги.Объект				= ПредопределенноеЗначение("Справочник.ПодразделенияКомпании.ПустаяСсылка");
	ИДСтроки = НоваяСтрокаКниги.ПолучитьИдентификатор(); 
	Элементы.ТелефоннаяКнига.Развернуть(ИДСтроки, Ложь);
	Элементы.ТелефоннаяКнига.ТекущаяСтрока = ИДСтроки;
...
Процедура ВидыТелефоновПриАктивизацииСтроки(Элемент)
	...
	Если ТД = Неопределено Тогда
		Возврат;
	ИначеЕсли ТД.Наименование = НСтр("ru='Внутренний номер'") Тогда
...
Процедура ВидыТелефоновПометкаПриИзменении(Элемент)
	...
	Если ТД.Наименование = НСтр("ru='Внутренний номер'") Тогда
		ЗаполнитьДеревоПользователей();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если НЕ сфпСофтФонПроСервер.сфпРолиДоступны("ПолныеПрава, сфпУправлениеМаршрутизацией") Тогда
		...
		Возврат;
	КонецЕсли;
	Если НЕ (ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей") = Истина) Тогда
		ЭтотОбъект.Элементы.ВариантОтображенияПользователей.СписокВыбора.Удалить(1);
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если НЕ ПустаяСтрока(ЭтотОбъект.АдресКнигиВоВременномХранилище) Тогда
		МассивТелефоннойКниги = ПолучитьИзВременногоХранилища(ЭтотОбъект.АдресКнигиВоВременномХранилище); 
