...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.ВнесеныИзменения Тогда
			...
			НастройкиКД = ПолучитьИзВременногоХранилища(СтрокаОтчеты.АдресНастроек);
			// -РАРУС
			
			// [1] Запись настроек.
			// +РАРУС
			//ОбъектСтрокаОтчеты.Настройки = Новый ХранилищеЗначения(ПользовательскиеНастройки, Новый СжатиеДанных(9));
			ОбъектСтрокаОтчеты.Настройки = Новый ХранилищеЗначения(НастройкиКД, Новый СжатиеДанных(9));
			// -РАРУС
		...
		Иначе
			...
			Иначе
				...
				НастройкиКД = ОбъектСтрокаОтчеты.Настройки.Получить();
				// -РАРУС
			КонецЕсли;
		...
		ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, Истина, НастройкиКД, Ложь);
		НастройкиОтчета = ?(СтрокаОтчеты.СКД, ПараметрыОтчета.КомпоновщикНастроекКД, НастройкиКД);
		// -РАРУС
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			// +РАРУС
			ОбъектСтрокаОтчеты.ВидПериода    = СтрокаОтчеты.ВидПериода;
			ОбъектСтрокаОтчеты.ПолеОтбора    = СтрокаОтчеты.ПолеОтбора;
			ОбъектСтрокаОтчеты.НачалоПериода = СтрокаОтчеты.НачалоПериода;
			ОбъектСтрокаОтчеты.КонецПериода  = СтрокаОтчеты.КонецПериода;
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	ПараметрыОтчета.Вставить("ПолноеИмя",            СтрокаОтчеты.ИмяОтчета);
	ПараметрыОтчета.Вставить("КлючВарианта",         СтрокаОтчеты.КлючВарианта);
	// +РАРУС
	ПараметрыОтчета.Вставить("ВидПериода",           ВидПериода);
	ПараметрыОтчета.Вставить("ПолеОтбора",           ПолеОтбора);
	...
	Если СтрокаОтчеты.СКД Тогда
		...
		//ПараметрыОтчета.Настройки = КомпоновщикНастроекКД.Настройки;
		// -РАРУС.
	Иначе
...
Процедура ПредварительныйПросмотрОтчетаПродолжение(РезультатВыбора, ПараметрыОтчета) Экспорт
	...
	ИначеЕсли НЕ ТипЗнч(ПользовательскиеНастройкиКД) = Тип("НастройкиКомпоновкиДанных") Тогда // РАРУС
	...
	ПараметрыФормы = Новый Структура;
	// +РАРУС
	Если ТипЗнч(ПользовательскиеНастройкиКД) = Тип("НастройкиКомпоновкиДанных") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ПараметрыФормы, ПараметрыОтчета, Истина);
		Если ПользовательскиеНастройкиКД.Отбор.Элементы.Количество() > 0 Тогда
			Отбор = Новый Структура;
			Отбор.Вставить("КонецПериода",  ПараметрыОтчета.КонецПериода);
			Отбор.Вставить("НачалоПериода", ПараметрыОтчета.НачалоПериода);
			Отбор.Вставить("ВидПериода",    ПараметрыОтчета.ВидПериода);
	...
	// +РАРУС
	Если ПерсонализированныеНастройки <> Неопределено Тогда
		Для Каждого ПользовательскаяНастройкаКД Из ПерсонализированныеНастройки Цикл
			Если ТипЗнч(ПользовательскаяНастройкаКД) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
				ПользовательскаяНастройкаКД.ПравоеЗначение = "[Получатель]";
			ИначеЕсли ТипЗнч(ПользовательскаяНастройкаКД) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если СКД Тогда
		...
		СтрокаНастроек = КомпоновщикНастроекКД.Настройки.ПолучитьОбъектПоИдентификатору(ИдентификаторНастройки);
		// -РАРУС
		Если СтрокаНастроек = Неопределено Тогда
...
Процедура ПереключитьСтраницу(Элементы, ИмяНабораСтраниц, СуффиксТекущейСтраницы)
	ГруппаСтраниц = Элементы[ИмяНабораСтраниц];
	ТекущаяСтраница = Элементы[ИмяНабораСтраниц + СуффиксТекущейСтраницы];
	ГруппаСтраниц.ТекущаяСтраница = ТекущаяСтраница;
	// Для платформы (в некоторых случаях страница не переключается с поля HTML документа).
...
&НаСервере
Функция ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, ДобавитьДежурнуюФразу = Истина, Знач НастройкиКД = Неопределено)
	...
	Если НастройкиКД = Неопределено Тогда
		...
		Иначе
			...
			Если ОбъектСтрокаОтчеты = Неопределено Тогда
				НастройкиКД                = Неопределено;
				СтрокаОтчеты.НачалоПериода = НачалоМесяца(ТекущаяДатаСеанса());
				СтрокаОтчеты.КонецПериода  = ТекущаяДатаСеанса();
			Иначе
				НастройкиКД = ОбъектСтрокаОтчеты.Настройки.Получить();
				СтрокаОтчеты.ВидПериода = ОбъектСтрокаОтчеты.ВидПериода;
				СтрокаОтчеты.ПолеОтбора = ОбъектСтрокаОтчеты.ПолеОтбора;
				// Даты для произвольного периода берем из настроек отчета
				Если СтрокаОтчеты.ВидПериода = "Произвольный" Тогда
					СтрокаОтчеты.НачалоПериода = ОбъектСтрокаОтчеты.НачалоПериода;
					СтрокаОтчеты.КонецПериода  = ОбъектСтрокаОтчеты.КонецПериода;
				Иначе
					СтрокаОтчеты.НачалоПериода = НачалоМесяца(ТекущаяДатаСеанса());
					СтрокаОтчеты.КонецПериода  = ТекущаяДатаСеанса();
				КонецЕсли;
	...
	ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, НастройкиКД);
	// -РАРУС
...
Процедура ПроверитьДобавленныеСтрокиОтчетов(СтруктураВыбора)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	ВариантыОтчетов.Ссылка В(&Отчеты)";
	Запрос.УстановитьПараметр("Отчеты", Объект.Отчеты.Выгрузить(,"Отчет"));	
	ВыборкаВариантов = Запрос.Выполнить().Выгрузить();  // -Рарус
	Для ОбратныйИндекс = 1 По СтруктураВыбора.Выбрано.Количество Цикл
		...
		Если ВидПериода = "Произвольный" Тогда
			ВыбранныйВариант = ВыборкаВариантов.Найти(СтрокаОтчеты.Отчет);
			СтрокаОтчеты.ВидПериода = "Произвольный";
			Если ЗначениеЗаполнено(ВыбранныйВариант.НачалоПериода) Тогда
				СтрокаОтчеты.НачалоПериода = ВыбранныйВариант.НачалоПериода;
			КонецЕсли;
			Если ЗначениеЗаполнено(ВыбранныйВариант.КонецПериода) Тогда
				СтрокаОтчеты.КонецПериода = ВыбранныйВариант.КонецПериода;
			КонецЕсли;
		КонецЕсли; // -Рарус
...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция = СтандартныеПодсистемыСервер.ИнструкцияУсловногоОформления();
	Инструкция.Поля = "Отчеты, ФорматыОтчетов";
	Инструкция.Отборы.Вставить("Объект.Отчеты.Доступность", Ложь);
	Инструкция.Оформление.Вставить("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
...
Процедура ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторСтроки)
	...
	ИначеЕсли СтрокаОтчеты.СКД Тогда
		...
		СтрокаОтчеты.ВидПериода = ВидПериода;
		СтрокаОтчеты.ПолеОтбора = ПолеОтбора;
		Если СтрокаОтчеты.ВидПериода = "Произвольный" Тогда
			Если ТипЗнч(НачалоПериода) = Тип("Дата") Тогда
				СтрокаОтчеты.НачалоПериода = НачалоПериода;
				СтрокаОтчеты.КонецПериода  = КонецПериода;
			ИначеЕсли ТипЗнч(НачалоПериода) = Тип("СтандартнаяДатаНачала") И НачалоПериода.Вариант = ВариантСтандартнойДатыНачала.ПроизвольнаяДата Тогда
				СтрокаОтчеты.НачалоПериода = НачалоПериода.Дата;
				СтрокаОтчеты.КонецПериода  = КонецПериода.Дата;
			Иначе
				СтрокаОтчеты.НачалоПериода = ОтчетыПлатформаКлиентСервер.СоответствиеСтандартнойДатыНачала(НачалоПериода);
				СтрокаОтчеты.КонецПериода  = ОтчетыПлатформаКлиентСервер.СоответствиеСтандартнойДатыНачала(КонецПериода);
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, НастройкиКД, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	...
	ПараметрыОтчета = Новый Структура("Отчет, Настройки", СтрокаОтчеты.Отчет, НастройкиКД);
	ПараметрыОтчета.Вставить("ВидПериода",    СтрокаОтчеты.ВидПериода);
	ПараметрыОтчета.Вставить("ПолеОтбора",    СтрокаОтчеты.ПолеОтбора);
	ПараметрыОтчета.Вставить("НачалоПериода", СтрокаОтчеты.НачалоПериода);
	ПараметрыОтчета.Вставить("КонецПериода",  СтрокаОтчеты.КонецПериода);
	// -РАРУС
	...
	Если ПараметрыОтчета.Инициализирован Тогда
		...
		СтрокаОтчеты.ИмяОтчета       = ПараметрыОтчета.ПолноеИмя;
	...
	Если СтрокаОтчеты.СКД Тогда
		...
		// +РАРУС
		СтрокаОтчеты.КлючВарианта = СтрокаОтчеты.Отчет.КлючВарианта;
		СтрокаОтчеты.ИмяОтчета    = ПараметрыОтчета.Метаданные.ПолноеИмя();
		
		ВидПериода = ПараметрыОтчета.ВидПериода;
		Если НЕ ЗначениеЗаполнено(ВидПериода) Тогда
			ВидПериода = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьВидПериода(СтрокаОтчеты.ИмяОтчета, СтрокаОтчеты.КлючВарианта);
		КонецЕсли;
		
		Если ТипЗнч(ПараметрыОтчета.НачалоПериода) = Тип("Дата") Тогда
			НачалоПериода = Новый СтандартнаяДатаНачала(ПараметрыОтчета.НачалоПериода); 
			КонецПериода =Новый СтандартнаяДатаНачала(ПараметрыОтчета.КонецПериода);
		Иначе
			Попытка
				НачалоПериода = Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[ПараметрыОтчета.НачалоПериода]); 
				КонецПериода =Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[ПараметрыОтчета.КонецПериода]);
			Исключение
			КонецПопытки;
		КонецЕсли;
		
		ЕстьНачалоПериода     = ПараметрыОтчета.ЕстьНачалоПериода;
		ЕстьКонецПериода      = ПараметрыОтчета.ЕстьКонецПериода;
		ПолеОтбора            = ПараметрыОтчета.ПолеОтбора;// -Рарус
		...
		СписокВыбораПоляОтбора.Очистить();
		
		Для Каждого ТекЭлемент Из ПараметрыОтчета.СписокВыбораПоляОтбора Цикл
			СписокВыбораПоляОтбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
		КонецЦикла;
		
		СписокВыбора = Элементы.ПолеОтбора.СписокВыбора;
		СписокВыбора.Очистить();
		Если ТипЗнч(ТипПолучателейРассылки) = Тип("ОписаниеТипов") И ТипПолучателейРассылки.Типы().Количество()>0 Тогда
			
			Если ТипПолучателейРассылки.Типы()[0] = Тип("СправочникСсылка.Контрагенты") Тогда
			...
			Для Каждого ТекЭлемент Из СписокВыбораПоляОтбора Цикл
				...
				СписокВыбора.Добавить(ТекЭлемент.Значение, Представление);
	...
	Иначе
		...
		// +РАРУС
		Если ТипЗнч(НастройкиКД) = Тип("ТаблицаЗначений") Тогда
			НастройкиТекущегоОтчета.Загрузить(НастройкиКД);
		// -РАУРС
		Иначе
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		НастройкиКД = СтрокаОтчетыПараметры.Настройки;
		
		Если СтрокаОтчетыПараметры.Свойство("ВидПериода") Тогда
			СтрокаОтчеты.НачалоПериода = СтрокаОтчетыПараметры.НачалоПериода;
			СтрокаОтчеты.КонецПериода  = СтрокаОтчетыПараметры.КонецПериода;
			СтрокаОтчеты.ВидПериода    = СтрокаОтчетыПараметры.ВидПериода;
		КонецЕсли;
		// -РАРУС
...
&НаСервере
Функция ПолучитьПредставлениеГруппировки(ТекущаяГруппировка, Уровень = 0)
	
	УровеньПодчиненных   = Уровень + 1;
	ТипНастройки         = ТипЗнч(ТекущаяГруппировка);
	ЗаголовокГруппировки = "";
	
	Если ТипНастройки = Тип("ГруппировкаТаблицыКомпоновкиДанных") ИЛИ ТипНастройки = Тип("ГруппировкаДиаграммыКомпоновкиДанных") ИЛИ ТипНастройки = Тип("ГруппировкаКомпоновкиДанных") Тогда
		
		// Переход на уровень ниже
		ТекущаяСтруктура = ОтчетыПлатформаСервер.ПолучитьСтруктуруОтчета(ТекущаяГруппировка.Структура, РежимЭксперт, Уровень);
		Если ТекущаяСтруктура = Неопределено Тогда
			ЗаголовокВложеннойГруппировки = "";
		Иначе
			ЗаголовокВложеннойГруппировки = ПолучитьПредставлениеГруппировки(ТекущаяСтруктура, УровеньПодчиненных);
		КонецЕсли;
		...
		Для Каждого ПолеГруппировки Из ТекущаяГруппировка.ПоляГруппировки.Элементы Цикл
			...
			ДоступноеПоле = КомпоновщикНастроекКД.Настройки.ДоступныеПоляГруппировок.НайтиПоле(ПолеГруппировки.Поле);
		...
		ЗаголовокГруппировки = ?(ЗаголовокГруппировки = "", "Детальные записи", ЗаголовокГруппировки);
		ЗаголовокГруппировки = ЗаголовокГруппировки + ?(ЗаголовокВложеннойГруппировки = "", "", ", " + ЗаголовокВложеннойГруппировки);
...
Процедура ОбновитьОписаниеГруппировок()
	...
	// Отображаем настройки только первой таблицы
	СтруктураКД = КомпоновщикНастроекКД.Настройки.Структура;
	
	РежимЭксперт = Ложь;
	ТекущаяСтруктура = ОтчетыПлатформаСервер.ПолучитьСтруктуруОтчета(СтруктураКД, РежимЭксперт, 0);
	...
	Если ТипСтруктурыОтчета = Тип("ТаблицаКомпоновкиДанных") Тогда
		...
		Для Каждого ТекущаяСтрока Из ТекущаяСтруктура.Строки Цикл
			ОписаниеСтрок = ПолучитьПредставлениеГруппировки(ТекущаяСтрока);
		КонецЦикла;
		
		Для Каждого ТекущаяКолонка Из ТекущаяСтруктура.Колонки Цикл
			ОписаниеКолонок = ПолучитьПредставлениеГруппировки(ТекущаяКолонка);
		КонецЦикла;
		
	ИначеЕсли ТипСтруктурыОтчета = Тип("ДиаграммаКомпоновкиДанных") Тогда
		
		Для Каждого ТекущаяСтрока Из ТекущаяСтруктура.Серии Цикл
			ОписаниеСтрок = ПолучитьПредставлениеГруппировки(ТекущаяСтрока);
		КонецЦикла;
		
		Для Каждого ТекущаяКолонка Из ТекущаяСтруктура.Точки Цикл
			ОписаниеКолонок = ПолучитьПредставлениеГруппировки(ТекущаяКолонка);
		КонецЦикла;
		
		ЭтоДиаграмма = Истина;
	Иначе
		ОписаниеСтрок = ПолучитьПредставлениеГруппировки(ТекущаяСтруктура);
	КонецЕсли;
	
	Если ЭтоДиаграмма Тогда
		Элементы.ОписаниеСтрок.Заголовок   = "Серии";
		Элементы.ОписаниеКолонок.Заголовок = "Точки";
	Иначе
		Элементы.ОписаниеСтрок.Заголовок   = "Строки";
		Элементы.ОписаниеКолонок.Заголовок = "Колонки";
	КонецЕсли;
...
Процедура ВидимостьПериода()
	...
	Если (НЕ ЕстьКонецПериода) И (НЕ ЕстьНачалоПериода) Тогда
	...
	Иначе
		...
		Если ЕстьНачалоПериода Тогда
			Элементы.ОсновныеДействияПериод.Заголовок = "Период с";
			Элементы.КонецПериода.Заголовок = "по";
		Иначе
			Элементы.ОсновныеДействияПериод.Заголовок = "Период";
		...
		Элементы.НачалоПериода.Видимость  = ЕстьНачалоПериода И (Произвольный);
		Элементы.КонецПериода.Видимость   = ЕстьКонецПериода И (Произвольный);
	КонецЕсли;
...
Процедура ОткрытьНастройкиОтчета(Команда)
	...
	СтрокаОтчеты = Элементы.Отчеты.ТекущиеДанные;
	Если СтрокаОтчеты = Неопределено ИЛИ (НЕ СтрокаОтчеты.СКД) Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураПараметров = Новый Структура;
	//СтруктураПараметров.Вставить("КлючОбъекта",               СтрокаОтчеты.ИмяОтчета);
	СтруктураПараметров.Вставить("КлючВарианта",              СтрокаОтчеты.КлючВарианта);
	СтруктураПараметров.Вставить("ЗаголовокОтчета",           Заголовок);
	СтруктураПараметров.Вставить("Вариант",                   КомпоновщикНастроекКД.Настройки);
	...
	СтруктураПараметров.Вставить("ПользовательскиеНастройки", КомпоновщикНастроекКД.ПользовательскиеНастройки);
	
	//СтруктураПараметров.Вставить("СхемаКомпоновки", СхемаКомпоновки);
	
	ОписаниеОповещенияИзмененияНастроек = Новый ОписаниеОповещения("ОповещениеИзмененияНастроек", ЭтотОбъект);
	
	Если РежимЭксперт Тогда
		ОткрытьФорму(СтрокаОтчеты.ИмяОтчета+".ФормаВарианта", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		СтруктураПараметров.Вставить("ОтсекатьОтключенныеГруппировки", Истина);
		ОткрытьФорму(СтрокаОтчеты.ИмяОтчета+".ФормаНастроек", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьСсылкуВариантОтчета(ОтчетПолноеИмя, КлючВарианта)
	
	ОтчетИнформация = ВариантыОтчетов.СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя);
	ВариантСсылка = ВариантыОтчетов.ПолучитьСсылку(ОтчетИнформация.Отчет, КлючВарианта);
...
Процедура ОповещениеИзмененияНастроек(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	КомпоновщикНастроекКД.ЗагрузитьНастройки(?(РезультатЗакрытия.Свойство("Вариант"), РезультатЗакрытия.Вариант, РезультатЗакрытия.НастройкиКД));
	
	Если НЕ СтрокаОтчеты.КлючВарианта = РезультатЗакрытия.КлючВарианта Тогда
		
		ВариантСсылка = ПолучитьСсылкуВариантОтчета(СтрокаОтчеты.ИмяОтчета, РезультатЗакрытия.КлючВарианта);
		
		СтрокаОтчеты.КлючВарианта = РезультатЗакрытия.КлючВарианта;
		СтрокаОтчеты.Отчет        = ВариантСсылка;
	...
	Для Каждого ТекНастройка Из ПользовательскиеНастройки.Элементы Цикл
		Если ЕстьРеквизитНаКлиенте(ТекНастройка, "Параметр") И ТекНастройка.Параметр = Новый ПараметрКомпоновкиДанных("Период") Тогда
			ВидПериода = ?(ТекНастройка.Значение.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод, "Произвольный", "");
			НачалоПериода = ТекНастройка.Значение.ДатаНачала;
			КонецПериода = ТекНастройка.Значение.ДатаОкончания;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура КомпоновщикНастроекКДНастройкиОтборПриИзменении(Элемент)
