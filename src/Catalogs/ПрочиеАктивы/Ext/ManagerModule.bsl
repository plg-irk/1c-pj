...
Функция ПолучитьОбязательныеРеквизиты(Объект) Экспорт
	...
	Если НЕ Объект.ЭтоГруппа Тогда
		...
		Если НЕ(Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Инструменты) Тогда
...
Функция ПолучитьУникальныеРеквизиты(Объект) Экспорт
	...
	Если (НЕ Объект.ЭтоГруппа) И НЕ(Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ Объект.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Инструменты) Тогда
...
Процедура ТипНоменклатурыПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	Если ПараметрыДействия = "ПодставитьСрокЭксплуатацииПоУмолчаниюИзАГ" Тогда
	ИначеЕсли Объект.ЭтоГруппа Тогда
...
Функция ПечатьОС6(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		Если НЕ Выборка.Следующий() Тогда
			ВывестиСообщение(НСтр("ru='На момент формирования отчета прочий актив не введен в эксплуатацию. %1 Нельзя сформировать инвентарную карточку объекта.'"),,,,, Символы.ПС);
			Продолжить;
		КонецЕсли;
		...
		Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		        ...
		        |ГДЕ
		        |	СведенияОСотрудникахСрезПоследних.ПодразделениеКомпании = &Подразделение
