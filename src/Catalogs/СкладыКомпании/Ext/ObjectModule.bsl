// Модуль объекта справочника "Склады компании"
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ЭтотОбъект.ЭтоГруппа Тогда
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ВидСклада) Тогда
			ЭтотОбъект.ВидСклада = Перечисления.ВидыСкладов.Обычный;
			Справочники.СкладыКомпании.ВидСкладаПриИзменении(ЭтотОбъект);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ЕдиницаРазмерности) Тогда
			ЭтотОбъект.ЕдиницаРазмерности = Константы.ОсновнаяЕдиницаРазмерности.Получить();
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ПодразделениеКомпании) Тогда
			ЭтотОбъект.ПодразделениеКомпании = ПараметрыСеанса.ТекущийПользователь.ПодразделениеКомпании;
		...
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.Организация) Тогда
			ЭтотОбъект.Организация = ПараметрыСеанса.ТекущийПользователь.Организация;
			Справочники.СкладыКомпании.ОрганизацияПриИзменении(ЭтотОбъект);
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.МОЛ) Тогда
			ЭтотОбъект.МОЛ = ПараметрыСеанса.ТекущийПользователь.Сотрудник;
			Справочники.СкладыКомпании.МОЛПриИзменении(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
