// Модуль формы элемента справочника "Подписанты".
...
Процедура УстановитьДоступностьЭлементов()
	...
	Если ЗначениеЗаполнено(Объект.Подписант) Тогда
		...
		ЭтоСотрудник                               = (ТипЗнч(Объект.Подписант) = Тип("СправочникСсылка.Сотрудники"));
		Элементы.Организация.Доступность           = ЭтоСотрудник;
		Элементы.ПодразделениеКомпании.Доступность = ЭтоСотрудник;
		Элементы.Контрагент.Доступность            = Не ЭтоСотрудник;
		
		Если ЭтоСотрудник Тогда
			
			ЗаполнитьОрганизацияПодразделение();
	...
	Иначе
		...
		Элементы.ПодразделениеКомпании.Доступность = Истина;
...
&НаСервере
Процедура ЗаполнитьОрганизацияПодразделение()
	
	Объект.Организация           = Объект.Подписант.Организация;
	Объект.ПодразделениеКомпании = Объект.Подписант.ПодразделениеКомпании;
	Объект.Контрагент            = Неопределено;
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьПараметрыВыбора(Элемент, Значение)
	
	Если Значение=Тип("СправочникСсылка.Контрагенты") Тогда
	...
	ИначеЕсли Значение=Тип("СправочникСсылка.Сотрудники") Тогда
...
// Процедура - обработчик события "ОбработкаВыбора" элемента формы "Подписант".
//
&НаКлиенте
Процедура ПодписантОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	УстановитьПараметрыВыбора(Элемент, ВыбранноеЗначение);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Подписант".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьПараметрыВыбора(Элементы.Подписант, ТипЗнч(Объект.Подписант));
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.Актуален Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Отказ = Истина;
			ВывестиСообщение(НСтр("ru = 'Не может быть более одного актуального подписанта'"), Объект, Объект.Актуален,, Отказ);
