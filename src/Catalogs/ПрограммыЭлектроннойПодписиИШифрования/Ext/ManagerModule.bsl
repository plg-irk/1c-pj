...
Процедура ЗаполнитьНачальныеНастройкиВОблачномСервисе() Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		// РАРУС +
		// В любом случае обработаем элемент
		Программа = ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
		Программа.ИмяПрограммы = Программа.Наименование;
		Программа.ТипПрограммы = 999999999;
		Программа.ОбменДанными.Загрузка = Истина;
		Программа.Записать();
		// РАРУС -
	КонецЦикла;
