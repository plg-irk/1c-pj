// Модуль формы элемента справочника "Шаблон иерархического справочника"
...
Процедура НаименованиеОткрытие(Элемент, СтандартнаяОбработка)
	...
	НижняяГраница	= Формат(Объект.Минимум, "ЧДЦ=2; ЧРГ=' '; ЧН=0; ЧГ=3,0");
	ВерхняяГраница	= Формат(Объект.Максимум, "ЧДЦ=2; ЧРГ=' '; ЧН=0; ЧГ=3,0");
	ЕдИзмСтрока		= ?(ЗначениеЗаполнено(Объект.ЕдиницаИзмерения), " " + Строка(Объект.ЕдиницаИзмерения), ".");
	
	Если ЗначениеЗаполнено(Объект.Минимум) 
		И ЗначениеЗаполнено(Объект.Максимум) Тогда 
		Объект.Наименование = НижняяГраница + " - " + ВерхняяГраница + ЕдИзмСтрока; 
	ИначеЕсли ЗначениеЗаполнено(Объект.Минимум) Тогда 
		Объект.Наименование = "свыше " + НижняяГраница + ЕдИзмСтрока;
	ИначеЕсли ЗначениеЗаполнено(Объект.Максимум) Тогда 
		Объект.Наименование = "до " + ВерхняяГраница + ЕдИзмСтрока;
	Иначе
		Объект.Наименование = "Пустой интервал";		
	КонецЕсли;
...
Процедура ОбновитьОтображениеПолейФормы()
	...
	НижняяГраница	= "От " + Формат(Объект.Минимум, "ЧДЦ=2; ЧРГ=' '; ЧН=0; ЧГ=3,0");
	НижняяГраница	= НижняяГраница + ?(Объект.ВключаяМинимум, " (включительно)", "");
	ВерхняяГраница	= "до " + Формат(Объект.Максимум, "ЧДЦ=2; ЧРГ=' '; ЧН=0; ЧГ=3,0");
	ВерхняяГраница	= ВерхняяГраница + ?(Объект.ВключаяМаксимум, " (включительно)", "");
	ЕдИзмСтрока		= ?(ЗначениеЗаполнено(Объект.ЕдиницаИзмерения), " " + Строка(Объект.ЕдиницаИзмерения), ".");
	Пояснение		= НижняяГраница + " " + ВерхняяГраница + ЕдИзмСтрока; 
	
КонецПроцедуры

&НаКлиенте
Процедура МинимумПриИзменении(Элемент)
...
&НаКлиенте
Процедура МаксимумПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВключаяМаксимумПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВключаяМинимумПриИзменении(Элемент)
