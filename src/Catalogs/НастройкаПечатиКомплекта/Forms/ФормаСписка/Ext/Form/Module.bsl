// Модуль формы списка справочника "Настройка печати комплекта".
...
//
&НаКлиенте
Процедура ОбработкаСозданияНовогоКомплекта(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	
	Элементы.Список.Обновить();
	
КонецПроцедуры // ОбработкаСозданияНовогоКомплекта()
...
Процедура КомандаСоздать(Команда)
	...
	ОткрытьФорму("Справочник.НастройкаПечатиКомплекта.ФормаОбъекта",Новый Структура("ВыбранныйДокумент",ВыбранныйДокумент),ЭтотОбъект,,,,Новый ОписаниеОповещения("ОбработкаСозданияНовогоКомплекта",ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца)
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Документ") Тогда
		...
		ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("СписокПечатныхФорм.ТипДокумента");
		ЭлементОтбора.Использование  = ИСТИНА;
		ЭлементОтбора.ВидСравнения   = ВидСравненияКомпоновкиДанных.Содержит;
		ЭлементОтбора.ПравоеЗначение = Параметры.Документ.Метаданные().ПолноеИмя();
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если НЕ УправлениеСпискомСправочникаКлиент.ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник, ПараметрыДействия) Тогда
		Если ИмяСобытия="ЗаписьПечатиКомплекта" Тогда
			ИмяСобытия = "";
			ВывестиСообщение(НСтр("ru='Настройки комплекта были изменены. Для корректного отображения подменю печати, откройте формы документов входящих в комплект еще раз.'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
