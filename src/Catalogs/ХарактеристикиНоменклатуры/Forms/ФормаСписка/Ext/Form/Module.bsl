...
// проверка на возможность ввода характеристик для текущего владельца
&НаСервереБезКонтекста
Функция НельзяВводитьХарактеристики(Владелец)
	
	// проверим заполнение владельца
	Если Не ЗначениеЗаполнено(Владелец) Тогда 
		ВывестиСообщение(НСтр("ru = 'Не определен владелец, невозможно определить тип учета по характеристикам.'"));
		Возврат Истина;
	Иначе
		...
		Если ТипЗнч(Владелец)=Тип("СправочникСсылка.Номенклатура") Тогда
			Если Владелец.ЭтоГруппа Тогда
				ВывестиСообщение(НСтр("ru = 'Владелец не может быть группой.'"));
				Возврат Истина;
			КонецЕсли;
		...
		Если ТипИспользованиеХарактеристик=3 Тогда
			...
			ВывестиСообщение(НСтр("ru = 'Для номенклатуры типа: """+СокрЛП(ТипНоменклатуры)+"""
...
КонецФункции // НельзяВводитьХарактеристики()
