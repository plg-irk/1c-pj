// Модуль формы списка справочника "Пользователи ВЕТИС"
...
// Обработчик события возникающего на клиенте при выполнении команды "Загрузить пользователей".
//
// Параметры:
...
Процедура ЗагрузитьПользователей(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗагрузитьПользователейПродолжение", ЭтотОбъект, Новый Структура("ФормаСписка", ЭтотОбъект));
...
Процедура ЗагрузитьПользователейНаСервере()
	...
	//ПользовательХозСубъекта = ХозяйствующийСубъект.Пользователь;
	//
	//Если НЕ ЗначениеЗаполнено(ПользовательХозСубъекта) Тогда
	//	
	//	//Пользователь = ПараметрыСеанса.Пользователь;
	//	//Если ЗначениеЗаполнено(Пользователь) Тогда
	//	//	Если ПустаяСтрока(Пользователь.МеркурийЛогин) Тогда
	//	//		Логин = "";
	//	//		ВвестиЗначение(Логин, "Введите логин пользователя хоз. субъекта", Новый ОписаниеТипов("Строка",Новый КвалификаторыСтроки(50)));
	//	//		Если обЗначениеНеЗаполнено(Логин) Тогда
	//	//			Сообщить("Логин пользователя не введен!");
	//	//			Возврат;
	//	//		КонецЕсли;
	//	//	Иначе
	//	//		Логин = СокрЛП(Пользователь.МеркурийЛогин);
	//	//	КонецЕсли;
	//	//КонецЕсли;
	//	
	//Иначе
	//	Логин = СокрЛП(ПользовательХозСубъекта.Логин);
	//КонецЕсли;
	//
	////ДанныеДляВыгрузки = Новый Структура;
	////ДанныеДляВыгрузки.Вставить("Логин", Логин);
	//
	////ОтправитьЗаявку(ХозяйствующийСубъект, ПараметрыЗаявок.getBusinessEntityUserListRequest, ДанныеДляВыгрузки);
	//
	//
	//ИнтеграцияВЕТИС.ЗагрузитьПользователей(ХозяйствующийСубъект, Логин);
	//Элементы.Список.Обновить();
	
КонецПроцедуры // ЗагрузитьПользователейНаСервере()
...
//
&НаКлиенте
Процедура ЗагрузитьПользователейПродолжение(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
...
КонецПроцедуры // ЗагрузитьПользователейПродолжение()
