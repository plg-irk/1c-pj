...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ВРег(ИмяСобытия) = ВРег("ОбновитьИсходящиеЭД") Тогда
		...
		// +Рарус
		ПодключитьОбработчикОжидания("ОпределитьИспользованиеКаталогаОбработчикОжидания", 0.1, Истина); // -РАРУС
...
Процедура ДоговорКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("РежимВыбора", Истина);// -РАРУС	
	
	Попытка
		СтандартнаяОбработка = Ложь;
		ОбработкаПродолжения = Новый ОписаниеОповещения("ДоговорКонтрагентаПродолжениеВыбора", ЭтотОбъект);
		ОткрытьФорму("Справочник.ДоговорыВзаиморасчетов.ФормаВыбора", ПараметрыОткрытия,,,,,
...
// +Рарус  Убран серверный вызов.
&НаКлиенте
Процедура ОпределитьИспользованиеКаталогаОбработчикОжидания()
