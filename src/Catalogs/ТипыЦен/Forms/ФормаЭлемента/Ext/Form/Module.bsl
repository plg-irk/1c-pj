...
Процедура ПроцентыСкидкиНаценкиИндикация()
	...
	ЭтотОбъект.СписокПроцентыСкидкиНаценки.Очистить();
	Для Каждого СкидкаНаценка Из Объект.ПроцентыСкидкиНаценки Цикл
			ЭтотОбъект.СписокПроцентыСкидкиНаценки.Добавить(,СокрЛП(СкидкаНаценка.ЦеноваяГруппа)
				+" / "+СокрЛП(СкидкаНаценка.БазовыйТипЦен)+" "+Формат(СкидкаНаценка.ПроцентСкидкиНаценки,"ЧДЦ=2; ЧН=0,00"));
	КонецЦикла;
	
КонецПроцедуры // ПроцентыСкидкиНаценкиИндикация()

// Проверяет наличие закольцованных ссылок.
&НаСервере
...
Процедура УправлениеДиалогомНаСервере()
	...
	Элементы.СписокПроцентыСкидкиНаценки.Доступность  = Объект.Рассчитывается;
	
	Элементы.РегистрироватьЦеныПоПриходу.Доступность  = НЕ Объект.Рассчитывается;
...
Процедура БлокироватьЭлементыФормы()
	...
	НеобходимостьБлокировки = ОбработкаСобытийОбъектаСервер.ПроверитьИспользованиеСсылки(Объект.Ссылка,"РегистрСведений.Цены","ТипЦен");
	
	// Составим список элементов для блокировки
	СписокРеквизитов = Новый СписокЗначений();
	СписокРеквизитов.Добавить("Рассчитывается",  "Рассчитывается",  НеобходимостьБлокировки);
	СписокРеквизитов.Добавить("БазовыйТипЦен",   "БазовыйТипЦен",   НеобходимостьБлокировки);
	СписокРеквизитов.Добавить("ВалютаЦены",      "ВалютаЦены",      НеобходимостьБлокировки);
	СписокРеквизитов.Добавить("ВВалютеУчета",    "ВВалютеУчета",    НеобходимостьБлокировки);
	СписокРеквизитов.Добавить("ЦенаВключаетНДС", "ЦенаВключаетНДС", НеобходимостьБлокировки);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Расчет цен" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
КонецПроцедуры // РасчетЦенПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "Расчет цен".
//
// Параметры:
//  Элемент - ПолеПереключателя - Элемент управления, в котором возникло данное событие.
//
&НаКлиенте
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Базовый тип цен" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Базовый тип цен".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
// Обработчик события возникающего на клиенте при начале выбора данных реквизита "Проценты скидки и наценки"
// в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
...
КонецПроцедуры // СписокПроцентыСкидкиНаценкиПриИзменении()
...
Процедура СписокПроцентыСкидкиНаценкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("Владелец",ЭтотОбъект.Объект);
...
КонецПроцедуры // СписокПроцентыСкидкиНаценкиНачалоВыбора()

// Обработчик события возникающего на клиенте при изменении данных реквизита "В валюте учета" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
КонецПроцедуры // ВВалютеУчетаПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "В валюте учета".
//
// Параметры:
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Закупка" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
КонецПроцедуры // ЗакупкаПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "Закупка".
//
// Параметры:
//  Элемент - ПолеФлажка - Элемент управления, в котором возникло данное событие.
//
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка ИЗ Справочник.ПодразделенияКомпании ГДЕ ТипЦенРекомендованнойНаценки=&ТипЦен");
	Запрос.УстановитьПараметр("ТипЦен", Объект.Ссылка);
	тзПодразделений = Запрос.Выполнить().Выгрузить();
	СписокПодразделений.ЗагрузитьЗначения(тзПодразделений.ВыгрузитьКолонку("Ссылка"));
