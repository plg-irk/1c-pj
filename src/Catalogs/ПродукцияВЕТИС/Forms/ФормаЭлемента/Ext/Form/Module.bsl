// Модуль формы элемента справочника "Продукция ВЕТИС"
...
//
&НаКлиенте
Процедура ЗавершитьОбновитьПродукцию(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(РезультатОповещения) Тогда
		ХозяйствующийСубъект = РезультатОповещения;
		ОбновитьПродукциюНаСервере();
	КонецЕсли;
	
КонецПроцедуры // ЗавершитьОбновитьПродукцию()
...
// Обработчик события возникающего на клиенте при выполнении команды "Обновить продукцию".
//
// Параметры:
//   Команда   (КомандаФормы)     Команда, которая была вызвана.
//
&НаСервере
Процедура ОбновитьПродукциюНаСервере()
	
	ИнтеграцияВЕТИС.ОбновитьПродукцию(Объект, ХозяйствующийСубъект);
	
КонецПроцедуры // ОбновитьПродукциюНаСервере()
...
Процедура ОбновитьПродукцию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗавершитьОбновитьПродукцию", ЭтотОбъект);
...
КонецПроцедуры // ОбновитьПродукцию()
