// Модуль формы выбора группы "Продукция ВЕТИС"
...
//
&НаКлиенте
Процедура ЗагрузитьПродукциюИзФормыСпискаПродолжение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	Иначе
		ХозяйствующийСубъект = Результат;
		НачальноеЗначениеВыбора = Новый Структура;
		НачальноеЗначениеВыбора.Вставить("ХозяйствующийСубъект", Результат);
		НачальноеЗначениеВыбора.Вставить("ТипПродукции", ДополнительныеПараметры.ТипПродукции);
		
		ПараметрыОткрытия = Новый Структура; 
		ПараметрыОткрытия.Вставить("НачальноеЗначениеВыбора", НачальноеЗначениеВыбора);
		
		ОткрытьФорму("Обработка.МодульОбменаВЕТИС.Форма.ФормаЗагрузкиПродукции", ПараметрыОткрытия, ДополнительныеПараметры.ФормаСписка,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры //ЗагрузитьПродукциюИзФормыСпискаПродолжение()

&НаСервере
Функция ПолучитьСобственныйХозСубъект()
	
	ХозяйствующийСубъект = ИнтеграцияВЕТИСВызовСервера.ПолучитьСобственныйХозСубъект();
...
Процедура Подбор(Команда)
	...
	ТипПродукции = ОтборРодитель;
	
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		ХозяйствующийСубъект = ПолучитьСобственныйХозСубъект();
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		ДополнительныеПараметры.Вставить("ФормаСписка", ЭтотОбъект);
		ДополнительныеПараметры.Вставить("ТипПродукции", ТипПродукции);
		
		Оповещение = Новый ОписаниеОповещения("ЗагрузитьПродукциюИзФормыСпискаПродолжение", ЭтотОбъект,ДополнительныеПараметры);
		ОткрытьФорму("Справочник.ХозяйствующиеСубъектыВЕТИС.ФормаВыбора",ПараметрыФормы,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	Иначе
		
		НачальноеЗначениеВыбора = Новый Структура;
		НачальноеЗначениеВыбора.Вставить("ХозяйствующийСубъект", ХозяйствующийСубъект);
		НачальноеЗначениеВыбора.Вставить("ТипПродукции", ТипПродукции);
		
	 	ПараметрыОткрытия = Новый Структура; 
		ПараметрыОткрытия.Вставить("НачальноеЗначениеВыбора", НачальноеЗначениеВыбора);
		
		ОткрытьФорму("Обработка.МодульОбменаВЕТИС.Форма.ФормаЗагрузкиПродукции", ПараметрыОткрытия, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Параметры.РежимВыбора И ТекущиеДанные.УровеньВложенности = УровеньВложенности Тогда
		
		СтандартнаяОбработка = Истина;
		
	ИначеЕсли ТекущиеДанные.УровеньВложенности < УровеньВложенности Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УровеньВложенности = ПолучитьЗначениеПараметраСтруктуры(Параметры.Отбор, "УровеньВложенности", 10);
	
	Родитель = ПолучитьЗначениеПараметраСтруктуры(Параметры.Отбор, "Родитель", Неопределено);
	Если НЕ Родитель = Неопределено Тогда
		Параметры.Отбор.Удалить("Родитель");
	КонецЕсли;
	
	Если Родитель = Неопределено Тогда
		Родитель = ПолучитьЗначениеПараметраСтруктуры(Параметры.Отбор, "ТипПродукции", Неопределено);
		Если НЕ Родитель = Неопределено Тогда
			Параметры.Отбор.Удалить("ТипПродукции");
		КонецЕсли;
	КонецЕсли;
	
	Если Родитель = Неопределено Тогда
		Родитель = ПолучитьЗначениеПараметраСтруктуры(Параметры.Отбор, "Продукция", Неопределено);
		Если НЕ Родитель = Неопределено Тогда
			Параметры.Отбор.Удалить("Продукция");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ОтборРодитель) Тогда
		Элементы.Список.Отображение = ОтображениеТаблицы.Список;
	Иначе
		Элементы.Список.Отображение = ОтображениеТаблицы.Дерево;
	КонецЕсли;
