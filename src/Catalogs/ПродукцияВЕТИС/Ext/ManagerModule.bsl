// Модуль менеджера справочника "Продукция ВЕТИС"
...
Процедура ВыполнитьНачальноеЗаполнение() Экспорт
	...
	|ГДЕ
	|	ПродукцияВЕТИС.ЭтоГруппа = ИСТИНА И 
	|	ПродукцияВЕТИС.Предопределенный = ИСТИНА И 
	|	ПродукцияВЕТИС.ПометкаУдаления = ЛОЖЬ И 
	|	(НЕ ПродукцияВЕТИС.УровеньВложенности = 1)
	|";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПродукцияОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ПродукцияОбъект.УровеньВложенности = 1;
...
Функция ПолучитьУникальныеРеквизиты(Объект) Экспорт
	...
	// у типов продукции нет идентификаторов
	Если Объект.Уровень() > 0 Тогда
		УникальныеРеквизиты.Вставить("ИдентификаторЗаписи");
	КонецЕсли;
