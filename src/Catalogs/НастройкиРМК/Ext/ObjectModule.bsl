// Модуль объекта справочника "Настройки РМК"
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	ЭтотОбъект.РабочееМесто = МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(РабочееМесто) Тогда
		...
		Если Выборка.Следующий() Тогда
			ВывестиСообщение(НСтр("ru='Для рабочего места %1 уже существует настройка РМК %2'"),,,,Отказ, РабочееМесто, Выборка.Ссылка);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	ЭтотОбъект.Наименование = Строка(ЭтотОбъект.РабочееМесто);
