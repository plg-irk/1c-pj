...
Процедура ПолучитьИдентификаторУчастникаОбменаЭДЧерез1СЭДО()
	...
	Если Не ВозниклиОшибки Тогда
		ОповещениеПослеПроверки = Новый ОписаниеОповещения("ЗапуститьПолучениеНовогоИдентификатора1СЭДО", ЭтотОбъект);
		ВыполнитьПроверкиПередПолучениемИдентификатора(ОповещениеПослеПроверки);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗапуститьПолучениеНовогоИдентификатора1СЭДО(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
