// Модуль формы элемента справочника "Конкуренты"
...
Процедура ЗаполнитьПодразделения()
	...
	|ГДЕ
	|	Ссылка.ГруппаПоставки=&ГруппаПоставки
	|	И Конкурент=&Конкурент");
	Запрос.УстановитьПараметр("ГруппаПоставки", Объект.ГруппаПоставки);
	Запрос.УстановитьПараметр("Конкурент", Объект.Ссылка);
	Рез = Запрос.Выполнить().Выгрузить();
	
	ЗначениеВРеквизитФормы(Рез, "Подразделения");
