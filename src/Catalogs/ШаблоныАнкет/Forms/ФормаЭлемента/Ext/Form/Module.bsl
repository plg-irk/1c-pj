...
Процедура ОткрытьФормуПростыхВопросов(ТекущиеДанные)
	...
	// +РАРУС Для определения заполнены ли обязательные реквизиты строки или нет подключим обработчик оповещения.
	ОбработчикОповещения = Новый ОписаниеОповещения("Подключаемый_ОбработкаРедактированияСтрокиАнкеты", ЭтотОбъект, Новый Структура("ТекущиеДанные", ТекущиеДанные));
	
	ОткрытьФорму("Справочник.ШаблоныАнкет.Форма.ФормаПростыхВопросов",СтруктураПараметры,ЭтотОбъект,,,, ОбработчикОповещения);
	// -РАРУС
...
Процедура ОткрытьФормуМастераТабличныхВопросов(ТекущиеДанные)
	...
	ОткрытьФорму("Справочник.ШаблоныАнкет.Форма.ФормаМастераТабличныхВопросов",СтруктураПараметры,ЭтотОбъект,,,, ОбработчикОповещения);
	// -РАРУС
...
Процедура ПеретащитьЭлементДерева(СтрокаНазначение,СтрокаПеретаскивание,ИспользоватьРодителяСтрокиНазначения = ЛОЖЬ,УдалятьПослеДобавления = Истина);
	...
	Если СтрокаПеретаскивание.ТипВопроса = ПредопределенноеЗначение("Перечисление.ТипыВопросовШаблонаАнкеты.Комплексный") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СтрокаПеретаскивание.СоставТабличногоВопроса,НоваяСтрока.СоставКомплексногоВопроса);
	КонецЕсли;
...
// +РАРУС
&НаКлиенте
Функция УдалитьСтрокуДереваАнкеты(Дерево, ТекущиеДанные)
	
	ИндексСтроки = Дерево.ПолучитьЭлементы().Индекс(ТекущиеДанные);
	Если ИндексСтроки = -1 Тогда
		Для Каждого ТекущаяСтрока Из Дерево.ПолучитьЭлементы() Цикл
			Если УдалитьСтрокуДереваАнкеты(ТекущаяСтрока, ТекущиеДанные) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
	Иначе
		Дерево.ПолучитьЭлементы().Удалить(ИндексСтроки);
		Возврат Истина;
	КонецЕсли;
	
КонецФункции // УдалитьСтрокуДереваАнкеты() // -РАРУС

// +РАРУС
&НаКлиенте
Процедура Подключаемый_ОбработкаРедактированияСтрокиАнкеты(РезультатОповещения, ДополнительныеПараметры) Экспорт
	
	ТекущиеДанные = ПолучитьЗначениеПараметраСтруктуры(ДополнительныеПараметры, "ТекущиеДанные", Неопределено);
	
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Данные не заполнены - удалим пустую строку
	Если НЕ ЗначениеЗаполнено(ТекущиеДанные.Формулировка) Тогда
		УдалитьСтрокуДереваАнкеты(ДеревоАнкеты, ТекущиеДанные); //ДеревоАнкеты.ПолучитьЭлементы().Индекс(ТекущиеДанные);
	КонецЕсли;
	
КонецПроцедуры // Подключаемый_ОбработкаРедактированияСтрокиАнкеты() // -РАРУС
