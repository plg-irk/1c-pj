...
Процедура ПеренестиВШаблон(Команда)
	...
	Если Не ЗначениеЗаполнено(Формулировка) Тогда
		Отказ = Истина;
		ТекстСообщения = ?(ТипСтрокиДерева = "Раздел", НСтр("ru = 'Не заполнено имя раздела'"), НСтр("ru = 'Не заполнена формулировка'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Формулировка");
	КонецЕсли;
