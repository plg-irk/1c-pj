// Модуль формы списка справочника "Прайс листы контрагентов"
...
Процедура ОбработкаРезультатаОповещенияЗаполнитьЦены(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если РезультатОповещения = КодВозвратаДиалога.Да Тогда
		...
		ПараметрыФормы.Вставить("Контрагент", ТекущиеДанные.Владелец);
		ПараметрыФормы.Вставить("ПрайсЛистКонтрагента", ТекущиеДанные.Ссылка);
...
Процедура КомандаЗагрузить(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ПараметрыФормыЗагрузки.Вставить("Контрагент", ТекущиеДанные.Владелец);
		...
		ОткрытьФорму("Обработка.ЗагрузкаДанныхИзТабличногоДокумента.Форма.ФормаПомощник", ПараметрыФормыЗагрузки, ЭтотОбъект,,,, , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура КомандаОчиститьНаСервере(ПараметрыДействия = Неопределено)
	...
	// Получим текущее значение объекта
	ЭлементОбъект=Элементы.Список.ТекущаяСтрока.ПолучитьОбъект();
	
	// Вызываем обработчик изменения данных объекта
	Справочники.ПрайсЛистыКонтрагентов.ОчиститьПрайсЛист(ЭлементОбъект, ПараметрыДействия);
...
Процедура КомандаЗагрузкаЦен(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ПоказатьВопрос(ОбработкаОповещения, НСтр("ru = 'Заполнить цены по прайс-листу """ + СокрЛП(ТекущиеДанные.Ссылка) + """?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
