// Модуль формы списка справочника "Регионы ВЕТИС"
...
//
&НаКлиенте
Процедура ПодборПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе		
		...
		ОткрытьФорму("Обработка.МодульОбменаВЕТИС.Форма.ФормаЗагрузкиРегионов", ПараметрыОткрытия, ДополнительныеПараметры.ФормаСписка,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
	
КонецПроцедуры // ПодборПродолжение()
...
Процедура Подбор(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ПодборПродолжение", ЭтотОбъект,Новый Структура("ФормаСписка",ЭтотОбъект));
	...
	Иначе
		...
		ОткрытьФорму("Обработка.МодульОбменаВЕТИС.Форма.ФормаЗагрузкиРегионов", ПараметрыОткрытия, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
