...
Процедура КалендарьПриВыводеПериода(Элемент, ОформлениеПериода)
	...
	Для Каждого СтрокаОформленияПериода Из ОформлениеПериода.Даты Цикл
		...
		КонецЕсли;
		// +Рарус
		Если ВидыДней.Получить(СтрокаОформленияПериода.Дата) = ПредопределенноеЗначение("Перечисление.ВидыДнейПроизводственногоКалендаря.Рабочий") Тогда
...
Процедура ЗаполнитьПредставлениеПереносов(Форма)
	...
	// +Рарус
	Форма.Элементы.СписокПереносов.Видимость = Форма.СписокПереносов.Количество() > 0;
	// -Рарус
