// Модуль формы списка справочника "Заболевания ВЕТИС"
...
//
&НаКлиенте
Процедура ЗавершитьЗагрузитьЗаболевания(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(РезультатОповещения) Тогда
		ХозяйствующийСубъект = РезультатОповещения;
		ЗагрузитьЗаболеванияНаСервере();
	КонецЕсли;
...
// Обработчик события возникающего на клиенте при выполнении команды "Загрузить заболевания".
//
// Параметры:
//   Команда   (КомандаФормы)     Команда, которая была вызвана.
//
&НаСервере
Процедура ЗагрузитьЗаболеванияНаСервере()
	
	НастройкаПодключения = ИнтеграцияВЕТИС.ПолучитьНастройкуПодключения(ХозяйствующийСубъект);
	
	ИнтеграцияВЕТИС.ЗагрузитьЗаболевания(НастройкаПодключения);
	Элементы.Список.Обновить();
	
КонецПроцедуры // ЗагрузитьЗаболеванияНаСервере()
...
//
&НаКлиенте
Процедура ЗагрузитьЗаболевания(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗавершитьЗагрузитьЗаболевания", ЭтотОбъект, Новый Структура("ФормаСписка", ЭтотОбъект));
