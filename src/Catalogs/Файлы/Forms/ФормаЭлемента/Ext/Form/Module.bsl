...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ФайлРедактировался И Не ПолученОтветНаВопросФайлЗанят Тогда
		...
		Если ПараметрыРаботыКлиента.АвторизованныйПользователь = Объект.Редактирует Тогда
			...
			|
			|Закрыть карточку?'");
			ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(ЭтотОбъект, Отказ, ЗавершениеРаботы, ТекстВопроса, "ПолученОтветНаВопросФайлЗанят");
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	ПредложитьОткрытьФайлДляРедактирования = РежимСоздания = "ИзШаблона" И НЕ Объект.ПодписанЭП 
		И НовыйФайл И НовыйФайлЗаписан И (Не ФайлРедактировался) И Не ПолученОтветНаВопросОткрытьДляРедактирования;
	Если ПредложитьОткрытьФайлДляРедактирования Тогда
		Отказ = Истина;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ЗаполнитьСписокШифрования()
	...
	Если Объект.Зашифрован Тогда
		МассивСертификатовШифрования = РаботаСФайламиСлужебныйВызовСервера.СертификатыШифрования(Объект.Ссылка);
