...
Процедура ПередЗаписью(Отказ)
	...
	Если ИнтерфейсУпрощенный Тогда
		...
		|		ПО ГруппыДоступа.Ссылка = ГруппыДоступаПрофили.Ссылка
		|ГДЕ
		|	ГруппыДоступаПрофили.Профиль = &Профиль
		// -РАРУС
...
Процедура ПриЗаписи(Отказ)
	...
	// +РАРУС
	Если ЛОЖЬ И ПометкаУдаления И СтараяПометкаУдаления = Ложь Тогда
	// -РАРУС
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Профиль", Ссылка);
		Запрос.Текст =
	...
	|	И (НЕ ГруппыДоступа.Ссылка.ЭтоГруппа)";
	// -РАРУС
...
Функция ОбновитьРолиПользователейПриИзмененииРолейПрофиля()
	...
	Если НЕ ДополнительныеСвойства.Свойство("НеОбновлятьРолиПользователей")
	   И НЕ РезультатыЗапросов[1].Пустой() Тогда
		...
		|ПОМЕСТИТЬ ГруппыДоступаПрофили
		|ИЗ
		...
		|ПОМЕСТИТЬ ГруппыДоступаПользователи
		|ИЗ
		|	Справочник.ГруппыДоступа.Пользователи КАК ГруппыДоступаПользователи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГруппыДоступаПрофили КАК ГруппыДоступаПрофили
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГруппыДоступаПользователи КАК ГруппыДоступаПользователи
