...
Процедура ОбновитьПоставляемыеПрофили(ЕстьИзменения = Неопределено) Экспорт
	...
	|			И (ГруппыДоступаПрофили.Профиль В (&Профили))";
	// -РАРУС
...
Функция ПоставляемыеПрофили()
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Заданы недопустимые значения в процедуре ПриЗаполненииПоставляемыхПрофилейГруппДоступа
		           |общего модуля УправлениеДоступомПереопределяемый.'") + Символы.ПС + Символы.ПС;
...
// +РАРУС Создает структуру описания профиля.
//
// Параметры:
//  Нет
//
// Возвращаемое значение:
//   Структура   - Структура описания профиля.
//
Функция НовыйСоставПрофиля()
	
	СоставПрофиля = Новый Структура;
	СоставПрофиля.Вставить("Роли",            Новый Массив);
	СоставПрофиля.Вставить("ВидыДоступа",     Новый СписокЗначений);
	СоставПрофиля.Вставить("ЗначенияДоступа", Новый СписокЗначений);
	
	Возврат СоставПрофиля;
	
КонецФункции // НовыйСоставПрофиля() // -РАРУС

// +РАРУС Заполняет список предопределенных профилей из макета
//
// Параметры:
//  Нет
//
// Возвращаемое значение:
//   СписокЗначений   - Список описаний профилей из макета.
//
Функция ЗаполнитьСписокПрофилейИзМакета() Экспорт
	
	// Получим макет предопределенных профилей.
	МакетПрофилей = Справочники.ПрофилиГруппДоступа.ПолучитьМакет("ПрофилиГруппДоступаПредопределенные");
	КоличествоЗаписейМакета = МакетПрофилей.ВысотаТаблицы;
	
	// Получим области-колонки.
	ОбластьНаименование = МакетПрофилей.Области.Наименование;
	ОбластьРоли         = МакетПрофилей.Области.Роли;
	ОбластьВидыДоступа  = МакетПрофилей.Области.ВидыДоступа;
	
	// Инициируем временные переменные.
	ПрофилиИзМакета          = Новый СписокЗначений;
	ИмяТекПрофиля            = "";
	ТекСоставПрофиляИзМакета = НовыйСоставПрофиля();
	
	// Обходим макет предопределенных профилей.
	Сч = 1;
	Пока Сч <= КоличествоЗаписейМакета Цикл
		
		// Получим значения ячеек.
		ИмяПрофиля     = СокрЛП(МакетПрофилей.Область(Сч, ОбластьНаименование.Лево, Сч, ОбластьНаименование.Право).Текст);
		ИмяРоли        = СокрЛП(МакетПрофилей.Область(Сч, ОбластьРоли.Лево,         Сч, ОбластьРоли.Право).Текст);
		ИмяВидаДоступа = СокрЛП(МакетПрофилей.Область(Сч, ОбластьВидыДоступа.Лево,  Сч, ОбластьВидыДоступа.Право).Текст);
		
		// Получаем признак окончания описания текущего профиля.
		ЭтоПоследняяСтрокаПрофиля = Сч = КоличествоЗаписейМакета ИЛИ ЗначениеЗаполнено(МакетПрофилей.Область(Сч + 1, ОбластьНаименование.Лево, Сч + 1, ОбластьНаименование.Право).Текст);
		
		// Проверяем, относится текущая строка к имени или к составу профиля.
		Если ЗначениеЗаполнено(ИмяПрофиля) Тогда
			
			// Текущая строка - имя профиля.
			ИмяТекПрофиля = ИмяПрофиля;
			
		Иначе
			
			// Текущая строка - роли и виды доступа профиля.
			Если ЗначениеЗаполнено(ИмяРоли) Тогда
				ТекСоставПрофиляИзМакета.Роли.Добавить(ИмяРоли);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ИмяВидаДоступа) Тогда
				ТекСоставПрофиляИзМакета.ВидыДоступа.Добавить("", ИмяВидаДоступа);
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоПоследняяСтрокаПрофиля Тогда
			
			// Добавляем в список профилей состав и имя профиля из макета.
			ПрофилиИзМакета.Добавить(ТекСоставПрофиляИзМакета, ИмяТекПрофиля);
	...
	Возврат ПрофилиИзМакета;
	
КонецФункции // ЗаполнитьСписокПрофилейИзМакета() // -РАРУС

// +РАРУС Заполняет подменю кнопками с командами из списка.
//
// Параметры:
//  ТекФорма      - УправляемаяФорма - Форма, в которой нужно заполнить подменю.
//  СписокКоманд  - СписокЗначений   - Список, по представлениям которого заполняются имена команд и кнопок.
//  ИмяПодменю    - Строка           - Имя заполняемого подменю (должно быть создано в командной панели).
//
Процедура ЗаполнитьПодменюКнопками(ТекФорма, СписокКоманд, ИмяПодменю) Экспорт
	
	// Заполняем подменю доступных профилей из макета.
	Для каждого ЭлементСписка Из СписокКоманд Цикл
		
		ИмяПрофиля = СтрЗаменить(ТРег(ЭлементСписка.Представление), " ", "");
		
		КомандаДобавитьПрофиль = ТекФорма.Команды.Добавить(ИмяПрофиля);
		КомандаДобавитьПрофиль.Заголовок                 = ЭлементСписка.Представление;
		КомандаДобавитьПрофиль.Действие                  = "ЗаполнитьПрофильПоШаблону";
		КнопкаДобавитьПрофиль = ТекФорма.Элементы.Добавить(ИмяПрофиля, Тип("КнопкаФормы"), ТекФорма.Элементы[ИмяПодменю]);
		КнопкаДобавитьПрофиль.ИмяКоманды = ИмяПрофиля;
	КонецЦикла;
	
КонецПроцедуры // ЗаполнитьПодменюКнопками() // -РАРУС

// +РАРУС Находит в списке выбранный профиль и возвращает его описание.
//
// Параметры:
//  СписокОписанийПрофилей  - СписокЗначений - Список описаний профилей, в котором производится поиск.
//  ИмяКоманды              - Строка         - Имя команды, вызванной из подменю, по которому будет найден профиль.
//
// Возвращаемое значение:
//  Структура - Описание найденного профиля.
//
Функция ЗаполнитьОписаниеПрофиля(СписокОписанийПрофилей, ИмяКоманды) Экспорт
	
	// Инициализируем переменные.
	СоставПрофиля = Неопределено;
	НовоеОписание = Новый Структура();
	
	СоставРоли            = Новый Массив;
	СоставВидыДоступа     = Новый СписокЗначений;
	СоставЗначенияДоступа = Новый СписокЗначений;
	...
	НовоеОписание.Вставить("Роли",            СоставРоли);
	НовоеОписание.Вставить("ВидыДоступа",     СоставВидыДоступа);
	НовоеОписание.Вставить("ЗначенияДоступа", СоставЗначенияДоступа);
	
	// Найдем по имени команды выбранный профиль.
	Для каждого ТекОписание Из СписокОписанийПрофилей Цикл
		Если СтрЗаменить(ТРег(ТекОписание.Представление), " ", "") = ИмяКоманды Тогда
			СоставПрофиля = ТекОписание.Значение;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	// Проверяем состав профиля и получаем из него значения коллекций.
	Если НЕ СоставПрофиля = Неопределено И СоставПрофиля.Свойство("Роли", СоставРоли) И СоставПрофиля.Свойство("ВидыДоступа", СоставВидыДоступа) И СоставПрофиля.Свойство("ЗначенияДоступа", СоставЗначенияДоступа) Тогда
		
		// Переформатируем элемент списка значений в структуру для передачи в процедуру создания профиля.
		НовоеОписание.Наименование      = ТекОписание.Представление;
		НовоеОписание.Роли              = СоставРоли;
		НовоеОписание.ВидыДоступа       = СоставВидыДоступа;
		НовоеОписание.ЗначенияДоступа   = СоставЗначенияДоступа;
	КонецЕсли;
