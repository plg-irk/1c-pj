// Модуль формы списка справочника "ГТД"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СопоставленнаяАлкогольнаяПродукция = Неопределено;
	Если Параметры.Свойство("СопоставленнаяАлкогольнаяПродукция") И ЗначениеЗаполнено(Параметры.СопоставленнаяАлкогольнаяПродукция) Тогда
		СопоставленнаяАлкогольнаяПродукция = Параметры.СопоставленнаяАлкогольнаяПродукция;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СопоставленнаяАлкогольнаяПродукция) Тогда 
		ОбщегоНаЗначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Ссылка", СопоставленнаяАлкогольнаяПродукция,ВидСравненияКомпоновкиДанных.ВСписке,,Истина,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
	Иначе
		ОбщегоНаЗначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Ссылка", Справочники.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка(),,,Ложь,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаВводаПараметраЗапроса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		ВывестиСообщение(НСтр("ru = 'Не введен параметр %1'"), ЭтотОбъект,,,, ДополнительныеПараметры.ИмяПараметра);
		Возврат;
	КонецЕсли;
	
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ДополнительныеПараметры.ВидСообщения, ДополнительныеПараметры.ИмяПараметра, СокрЛП(Результат));
...
Процедура ЗапроситьАлкогольнуюПродукциюПоИНН(Команда)
	...
	ДополнительныеПараметры.Вставить("ИмяПараметра", "ИНН");
	ДополнительныеПараметры.Вставить("ВидСообщения", ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросАлкогольнойПродукцииВерсия2"));
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаВводаПараметраЗапроса", ЭтотОбъект, ДополнительныеПараметры);
	ПоказатьВводЗначения(ОписаниеОповещения, ИНН, "Введите ИНН производителя, импортера или собственной организации", Тип("Строка"));
	
КонецПроцедуры

&НаКлиенте
Процедура ЗапроситьАлкогольнуюПродукциюПоКоду(Команда)
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		КодАП = ПолучитьКодАП(ТекущаяСтрока);
	КонецЕсли;
	
	Если ПустаяСтрока(КодАП) Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ИмяПараметра", "КОД");
		ДополнительныеПараметры.Вставить("ВидСообщения", ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросАлкогольнойПродукцииПоАлкокодуВерсия2"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаВводаПараметраЗапроса", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВводЗначения(ОписаниеОповещения, КодАП, "Введите код алкогольной продукции", Тип("Строка"));
		Возврат;
	КонецЕсли;
	
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросАлкогольнойПродукцииПоАлкокодуВерсия2"), "КОД", КодАП);
