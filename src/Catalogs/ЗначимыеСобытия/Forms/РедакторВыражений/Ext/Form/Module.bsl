...
Функция ПроверитьВыражениеСервер(Выражение, Источник, ПроверятьНаличиеОперандов, СтрокаОшибки, КолонкаОшибки)
	...
	Если ОбъектДляПроверки=Неопределено Тогда
		ВывестиСообщение(НСтр("ru='Не удалось создать экземпляр объекта-источника.'"));
	КонецЕсли;
	...
	Попытка
		...
		ПараметрыПроверки.Добавить(ТекУсловие);
		ПараметрыПроверки.Добавить(Объект);
		ПараметрыПроверки.Добавить(Результат);
		
		РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации("Справочники.ЗначимыеСобытия.ПроверитьВыражение", ПараметрыПроверки);
	...
	Если Результат Тогда
		ВывестиСообщение(НСтр("ru='Ошибок не обнаружено'"));
	Иначе
		ВывестиСообщение(НСтр("ru='%1'"),,,,,ТекстОшибки)
	КонецЕсли;
