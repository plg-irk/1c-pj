...
Процедура ВыполнитьНачальноеЗаполнение() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ Выборка.ПредставлениеИсточника = "" И (НЕ Метаданные.НайтиПоПолномуИмени(Выборка.ПредставлениеИсточника) = Неопределено ИЛИ СтрНайти(Выборка.ПредставлениеИсточника,".")=0) Тогда
			...
			Попытка
				...
				ВывестиСообщение(НСтр("ru='Справочник ""Значимые события"". %1 элемент ""%2""'"),,,,,"Обновлен",Выборка.Ссылка);
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при записи элемента справочника ""Значимые события""'",
...
Процедура ПроверитьВыражение(ТекУсловие, Объект, Результат) Экспорт
