...
// Формирует диалог для настройки расписания.
//
&НаКлиенте
...
// Продолжает диалог для настройки расписания.
//
&НаКлиенте
...
Процедура РедакторВыражений(Команда)
	...
	ОткрытьФорму("Справочник.ДействияНаЗначимыеСобытия.Форма.РедакторВыражений", ПараметрыОткрытия,Элементы.СодержаниеПроизвольныйКод,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПолучателиНапоминанияЗначениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущиеДанные.ВидПравила=ПредопределенноеЗначение("Перечисление.ВидыПравил.РеквизитОбъектаИсточника") Тогда
		...
		Если НЕ ВыбранноеЗначение.ТипМетаданного.СодержитТип(Тип("СправочникСсылка.Пользователи")) Тогда
			ВывестиСообщение(НСтр("ru='Выбранный реквизит не является ссылкой на элемент справочника ""Пользователи"".'"));
			Возврат;
		КонецЕсли;
...
Процедура ДатаНапоминанияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если НЕ ВыбранноеЗначение.ТипМетаданного.СодержитТип(Тип("Дата")) Тогда
		ВывестиСообщение(НСтр("ru='Выбранный реквизит не является датой.'"));
		Возврат;
	КонецЕсли;
...
// Обработчик события возникающего на клиенте при окончании изменения данных реквизита "Содержание произвольный код".
//
// Параметры:
//  Элемент              - ПолеФормы - Элемент управления, в котором возникло данное событие.
//  ВыбранноеЗначение    - Выбранное значение.
...
//
&НаКлиенте
Процедура СодержаниеПроизвольныйКодОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Объект.СодержаниеПроизвольныйКод = ВыбранноеЗначение;
	
КонецПроцедуры  //СодержаниеПроизвольныйКодОбработкаВыбора()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПолучателиНапоминанияВидПравила.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВидыПравил.ТочноеЗначение"));
	Если СсылочныйТипИсточника Тогда
		Элементы.ПолучателиНапоминанияВидПравила.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВидыПравил.РеквизитОбъектаИсточника"));	
	КонецЕсли;
	Элементы.ПолучателиНапоминанияВидПравила.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВидыПравил.ПроизвольныйКод"));
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ВУказанноеВремя 
		 ...
		 ИЛИ ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета 	Тогда
		ТекущийОбъект.ВремяСобытия    = ТекущийОбъект.ВремяСобытия;
