// Модуль формы элемента справочника "Прайс лист"
...
Процедура НоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Справочники.ПрайсЛист.НоменклатураПриИзменении(Объект, ПараметрыДействия);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.Владелец) И ТекущийОбъект.Владелец.ФлагУникальности Тогда
		СтрокаУникальность = ?(ТекущийОбъект.ЭтоГруппа,"Наименование","Номенклатура");
		Запрос=Новый Запрос("
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ГДЕ
		|ПрайсЛист."+СтрокаУникальность+" = &ИсходнаяНоменклатура
		|И ПрайсЛист.Ссылка <> &ИсходныйПрайсЛист
		|И ПрайсЛист.Владелец = &ИсходныйВладелец
		|");
		Запрос.УстановитьПараметр("ИсходнаяНоменклатура",?(ТекущийОбъект.ЭтоГруппа,ТекущийОбъект.Наименование,ТекущийОбъект.Номенклатура));
		Запрос.УстановитьПараметр("ИсходныйПрайсЛист",ТекущийОбъект.Ссылка);
		Запрос.УстановитьПараметр("ИсходныйВладелец",ТекущийОбъект.Владелец);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Количество()<>0 Тогда
			ВывестиСообщение(НСтр("ru='Товар не уникален.'"),,,,Отказ);
		Иначе
		КонецЕсли;
	КонецЕсли;
