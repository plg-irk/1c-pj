...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Для Каждого ДополнительныйОтчетИлиОбработка Из ВыделенныеСтроки Цикл
		ДанныеСтроки = Элементы.Список.ДанныеСтроки(ДополнительныйОтчетИлиОбработка);
		Если ДанныеСтроки.ЭтоГруппа Тогда
	...
	ИзменениеПубликации(ВариантПубликации, Результат);
...
&НаСервере
Процедура ИзменениеПубликации(ВариантПубликации, Результат = Истина)
	ВыделенныеСтроки = Элементы.Список.ВыделенныеСтроки;
	...
	Попытка
		...
		Для Каждого ДополнительныйОтчетИлиОбработка Из ВыделенныеСтроки Цикл
			...
			// +РАРУС Иначе при попытке изменить публикацию для группы - падает.
			Если ДополнительныйОтчетИлиОбработка.ЭтоГруппа Тогда
				ВывестиСообщение(НСтр("ru='Команда не доступна для группы %1'"),,,,,ДополнительныйОтчетИлиОбработка);
				Продолжить; 
			КонецЕсли;
			// -РАРУС
		...
		Для Каждого ДополнительныйОтчетИлиОбработка Из ВыделенныеСтроки Цикл
			...
			Если Не Объект.ПроверитьЗаполнение() Тогда
				...
				Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
					ВывестиСообщение(ПредставлениеОшибки);
					Результат = Ложь;
					Возврат;
				КонецЕсли;
			КонецЕсли;
