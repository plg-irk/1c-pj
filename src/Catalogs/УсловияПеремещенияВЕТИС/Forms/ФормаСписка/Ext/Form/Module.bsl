// Модуль формы списка справочника "Условия перемещения ВЕТИС"
...
//
&НаКлиенте
Процедура ЗавершитьЗагрузитьДанные(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(РезультатОповещения) Тогда
		
		Если Тип("СправочникСсылка.ХозяйствующиеСубъектыВЕТИС") = ТипЗнч(РезультатОповещения) Тогда
			ХозяйствующийСубъект = РезультатОповещения;
		КонецЕсли;
		
		Если ДополнительныеПараметры.Свойство("ПоЗаболеванию") Тогда
			ЗагрузитьДанныеНаСервере(РезультатОповещения);
		Иначе 
			ЗагрузитьДанные(ДополнительныеПараметры.Команда);
		КонецЕсли;
		
	КонецЕсли;
	
КонецПроцедуры // ЗавершитьЗагрузитьДанные()
...
// Обработчик события возникающего на клиенте при выполнении команды "Загрузить".
//
// Параметры:
//   Команда   (КомандаФормы)     Команда, которая была вызвана.
//
&НаСервере
Процедура ЗагрузитьДанныеНаСервере(Заболевание = Неопределено)
	
	НастройкаПодключения = ИнтеграцияВЕТИС.ПолучитьНастройкуПодключения(ХозяйствующийСубъект);
	
	ИнтеграцияВЕТИС.ЗагрузитьУсловияРегионализации(НастройкаПодключения, Заболевание);
...
Процедура ЗагрузитьДанные(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ХозяйствующийСубъект) Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ЗавершитьЗагрузитьДанные", ЭтотОбъект,Новый Структура("Команда",Команда));
		ОткрытьФорму("Справочник.ХозяйствующиеСубъектыВЕТИС.ФормаВыбора",ДополнительныеПараметры,ЭтотОбъект,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
	ИначеЕсли СтрНайти(Команда.Имя,"ПоЗаболеванию") Тогда
		ВыбратьЗаболевание();
	Иначе
...
// Вызывает форму выбора заболевания
&НаКлиенте
Процедура ВыбратьЗаболевание()
	
	Оповещение = Новый ОписаниеОповещения("ЗавершитьЗагрузитьДанные", ЭтотОбъект, Новый Структура("ПоЗаболеванию"));
	ОткрытьФорму("Справочник.ЗаболеванияВЕТИС.ФормаВыбора",,ЭтотОбъект,,,,Оповещение);
	
КонецПроцедуры // ВыбратьЗаболевание()
