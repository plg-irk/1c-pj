// Модуль формы элемента справочника "Виды прайс листов"

////////////////////////////////////////////////////////////////////////////////
// СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ

// Обработчик результата оповещения при копировании содержимого прайс листа.
//
// Параметры:
...
//
&НаКлиенте
Процедура ОбработкаРезультатаОповещенияКопироватьСодержимоеПрайса(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если РезультатОповещения = КодВозвратаДиалога.Да Тогда
		КопироватьСодержимое = Истина;
	КонецЕсли;
	
КонецПроцедуры // ОбработкаРезультатаОповещенияКопироватьСодержимоеПрайса()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Параметры.ЗначениеКопирования) Тогда
		КопируемыйОбъект  = ЭтотОбъект.Параметры.ЗначениеКопирования.Ссылка;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(КопируемыйОбъект) Тогда
		// Формируем описание обработчика перехвата закрытия формы
		ОбработчикВопроса = Новый ОписаниеОповещения("ОбработкаРезультатаОповещенияКопироватьСодержимоеПрайса", ЭтотОбъект);
		
		// Формируем текст вопроса
		ТекстВопроса = НСтр("ru = 'Скопировать содержимое прайса'") + " """+Строка(КопируемыйОбъект)
			+ """ " + НСтр("ru = 'в новый прайс-лист?'");
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КопироватьСодержимое Тогда
		ПараметрыЗаполнения = Новый Структура();
		ПараметрыЗаполнения.Вставить("ВидПрайсЛиста",КопируемыйОбъект);
		Справочники.ПрайсЛист.СоздатьЭлемент();
		Справочники.ПрайсЛист.ЗаполнитьПрайсЛист(Объект.Ссылка,3,ПараметрыЗаполнения);
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если КопироватьСодержимое Тогда
		...
		// Обновим выпадающее меню в открытых прайсах
		Оповестить("ОбновитьСписокВидовПрайсов");
	КонецЕсли;
