// Модуль формы списка справочника "Транспортные модули ВЕТИС"
...
Процедура СинхронизироватьНаСервере()
	...
	ХозяйствующийСубъект = Элементы.Список.ТекущаяСтрока;
	...
	ДанныеЗаполнения.Вставить("guid", ХозяйствующийСубъект.Код);
	ДанныеЗаполнения.Вставить("uuid", ХозяйствующийСубъект.ИдентификаторЗаписи);
...
&НаСервереБезКонтекста
Процедура НайтиПоИНННаСервере(СтрИНН)
	...
	Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.Ссылка) Тогда
		...
		Пока Шаг>0 Цикл
			...
			НастройкаПодключения = ИнтеграцияВЕТИС.ПолучитьНастройкуПодключения(Выборка.Ссылка);
			
			Результат = ИнтеграцияВЕТИС.ЗапросНаПолучениеСпискаХС(Выборка.Ссылка, СтрИНН, Смещение, Шаг);
			
			Если Результат = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Для Каждого ТекущаяЗапись Из Результат.BusinessEntity Цикл
				ИнтеграцияВЕТИС.ЗагрузитьХозяйствующийСубъект(НастройкаПодключения,, ТекущаяЗапись);
			КонецЦикла;
...
&НаКлиенте
Процедура ОбработатьИНН(ИНН, ДопПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(ИНН) Тогда
		ВывестиСообщение("ИНН не введен!");
		Возврат;
	КонецЕсли;
	
	НайтиПоИНННаСервере(ИНН);
...
Процедура НайтиПоИНН(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьИНН", ЭтотОбъект);
	ПоказатьВводСтроки(ОписаниеОповещения, ,"Введите ИНН", 12);
