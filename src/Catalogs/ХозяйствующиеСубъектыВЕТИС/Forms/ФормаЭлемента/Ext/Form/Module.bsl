// Модуль формы элемента справочника "Хозяйствующие субъектыВЕТИС"
...
Процедура НастроитьПараметрыВыбораЭлементовФормы()
	...
	Элементы.Огранизация.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Организации");
...
// Обработчик события возникающего на клиенте при выполнении команды "Синхронизировать".
//
// Параметры:
...
Процедура СинхронизироватьНаСервере()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) ТОгда
		ВывестиСообщение(НСтр("ru='Загрузка списка предприятий возможна только для записанного хозяйствующего субъекта.'"));
		Возврат;
	КонецЕсли;
	...
	//Иначе
	//	ВывестиСообщение(НСтр("ru='Хозяйствующий субъект не имеет связи с транспортным модулем.'"));
	//	Возврат;
	//КонецЕсли;
	
	НастройкаПодключения = ИнтеграцияВЕТИС.ПолучитьНастройкуПодключения(Объект.Ссылка);
	
	Предприятия = ИнтеграцияВЕТИС.ЗагрузитьПредприятия(НастройкаПодключения, Объект.Код);
	НачатьТранзакцию();
	
	Попытка
		
		ЗагруженныеПредприятия = Новый Массив;
		Для Каждого Строка Из Предприятия Цикл
			ПредприятиеСсылка = ИнтеграцияВЕТИС.СоздатьПредприятие(Объект.Ссылка,Строка);
			Если Объект.Предприятия.НайтиСтроки(Новый Структура("Предприятие",ПредприятиеСсылка)).Количество() = 0 Тогда
				НоваяСтрока = Объект.Предприятия.Добавить();
				НоваяСтрока.Предприятие = ПредприятиеСсылка;
			КонецЕсли;  
			ЗагруженныеПредприятия.Добавить(ПредприятиеСсылка);	
		КонецЦикла;
				
		СтрокиТЧКУдалению = Новый Массив;
		Для Каждого СтрокаТЧ Из Объект.Предприятия Цикл
			Если ЗагруженныеПредприятия.Найти(СтрокаТЧ.Предприятие) = Неопределено Тогда
...
КонецПроцедуры // СинхронизироватьНаСервере()
...
Процедура ПриОткрытии(Отказ)
	...
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтотОбъект, "Предприятия");
