...
//
Функция СохранитьПараметрыУстройства(Идентификатор, Знач Параметры) Экспорт
	...
	Попытка
		...
		Если ТипЗнч(Параметры) = Тип("Структура") Тогда
			Если Параметры.Свойство("ВыгружатьТовары") Тогда
				ОбъектСправочника.ВыгружатьТовары = Параметры.ВыгружатьТовары;
				Параметры.Удалить("ВыгружатьТовары");
			КонецЕсли;
			
			Если Параметры.Свойство("ВыгружатьСкидки") Тогда
				ОбъектСправочника.ВыгружатьСкидки = Параметры.ВыгружатьСкидки;
				Параметры.Удалить("ВыгружатьСкидки");
			КонецЕсли;
			
			Если Параметры.Свойство("ВыгружатьПерсонал") Тогда
				ОбъектСправочника.ВыгружатьПерсонал = Параметры.ВыгружатьПерсонал;
				Параметры.Удалить("ВыгружатьПерсонал");
			КонецЕсли;
			
			Если Параметры.Свойство("ЗагружатьПродажи") Тогда
				ОбъектСправочника.ЗагружатьПродажи = Параметры.ЗагружатьПродажи;
				Параметры.Удалить("ЗагружатьПродажи");
			КонецЕсли;
			
			Если Параметры.Свойство("ДатаНачалаЗагрузкиЧековИКассовыхСмен") Тогда
				ОбъектСправочника.ДатаНачалаЗагрузкиЧековИКассовыхСмен = Параметры.ДатаНачалаЗагрузкиЧековИКассовыхСмен;
				Параметры.Удалить("ДатаНачалаЗагрузкиЧековИКассовыхСмен");
			КонецЕсли;
			
			Если Параметры.Свойство("ВыполнятьПоРасписанию") Тогда
				ОбъектСправочника.ВыполнятьПоРасписанию = Параметры.ВыполнятьПоРасписанию;
				Параметры.Удалить("ВыполнятьПоРасписанию");
			КонецЕсли;
			
			Если Параметры.Свойство("РегламентноеЗадание") Тогда
				ОбъектСправочника.РегламентноеЗадание = Параметры.РегламентноеЗадание;
				Параметры.Удалить("РегламентноеЗадание");
			КонецЕсли;
			
			Если Параметры.Свойство("ПериодичностьРасписания") Тогда
				ОбъектСправочника.ПериодичностьРасписания = Параметры.ПериодичностьРасписания;
				Параметры.Удалить("ПериодичностьРасписания");
			КонецЕсли;
			
			Если Параметры.Свойство("ИндексМагазина") Тогда
				ОбъектСправочника.ИндексМагазина = Параметры.ИндексМагазина;
				Параметры.Удалить("ИндексМагазина");
			КонецЕсли;
			
			Если Параметры.Свойство("Расписание") Тогда
				ОбъектСправочника.ДополнительныеСвойства.Вставить("Расписание", Параметры.Расписание);
				Параметры.Удалить("Расписание");
			КонецЕсли;
			
			Если Параметры.Свойство("ИмяПользователя") Тогда
				ОбъектСправочника.ДополнительныеСвойства.Вставить("ИмяПользователя", Параметры.ИмяПользователя);
				Параметры.Удалить("ИмяПользователя");
			КонецЕсли;
			
			Если Параметры.Свойство("Кассы") Тогда
				Для Каждого ТекущаяКасса Из Параметры.Кассы Цикл
					Справочники.КассыККМ.ПолучитьКассуККМОборудования(Идентификатор, ТекущаяКасса.НомерPOS, ТекущаяКасса.НаименованиеPOS);
				КонецЦикла;
			КонецЕсли;
...
Функция ПолучитьДанныеУстройства(Идентификатор) Экспорт
	...
	// тк + 
	|	ПодключаемоеОборудование.ВыгружатьТовары,
	|	ПодключаемоеОборудование.ВыгружатьСкидки,
	|	ПодключаемоеОборудование.ВыгружатьПерсонал,
	|	ПодключаемоеОборудование.ЗагружатьПродажи,
	|	ПодключаемоеОборудование.ДатаНачалаЗагрузкиЧековИКассовыхСмен,	
	|	ПодключаемоеОборудование.ВыполнятьПоРасписанию,
	|	ПодключаемоеОборудование.РегламентноеЗадание,
	|	ПодключаемоеОборудование.ПериодичностьРасписания,
	|	ПодключаемоеОборудование.ИндексМагазина,
	|	ПодключаемоеОборудование.ГруппаТоваровОборудования,
	|	ПодключаемоеОборудование.ОсновнойОтдел,
	|	ПодключаемоеОборудование.ОсновнойОтдел.НомерОтделаДляККМ КАК НомерОтделаДляККМ
	// тк - 
	|ИЗ
	|	Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудование
	...
	Если Выборка.Следующий() Тогда
		...
		// тк + 
		ДанныеУстройства.Вставить("ВыгружатьТовары",         Выборка.ВыгружатьТовары);
		ДанныеУстройства.Вставить("ВыгружатьСкидки",         Выборка.ВыгружатьСкидки);
		ДанныеУстройства.Вставить("ВыгружатьПерсонал",       Выборка.ВыгружатьПерсонал);
		ДанныеУстройства.Вставить("ЗагружатьПродажи",       Выборка.ЗагружатьПродажи);
		ДанныеУстройства.Вставить("ДатаНачалаЗагрузкиЧековИКассовыхСмен",       Выборка.ДатаНачалаЗагрузкиЧековИКассовыхСмен);
		ДанныеУстройства.Вставить("ВыполнятьПоРасписанию",   Выборка.ВыполнятьПоРасписанию);
		ДанныеУстройства.Вставить("РегламентноеЗадание",     Выборка.РегламентноеЗадание);
		ДанныеУстройства.Вставить("ПериодичностьРасписания", Выборка.ПериодичностьРасписания);
		ДанныеУстройства.Вставить("ИндексМагазина",          Выборка.ИндексМагазина);
		ДанныеУстройства.Вставить("ГруппаТоваровОборудования", Выборка.ГруппаТоваровОборудования);
		ДанныеУстройства.Вставить("ОсновнойОтдел",             Выборка.ОсновнойОтдел);
		ДанныеУстройства.Вставить("НомерОтделаДляККМ",         Выборка.НомерОтделаДляККМ);
...
Функция ОборудованиеПоПараметрам(ТипыПО = Неопределено, Идентификатор = Неопределено, РабочееМесто = Неопределено) Экспорт
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПодключаемоеОборудование.Ссылка КАК Ссылка,
	|   ПодключаемоеОборудование.ДатаНачалаЗагрузкиЧековИКассовыхСмен КАК ДатаНачалаЗагрузкиЧековИКассовыхСмен,
	|	ПодключаемоеОборудование.ИдентификаторУстройства КАК ИдентификаторУстройства,
	|	ПодключаемоеОборудование.Наименование КАК Наименование,
	|	ПодключаемоеОборудование.ТипОборудования КАК ТипОборудования,
	|	ПодключаемоеОборудование.ДрайверОборудования КАК ДрайверОборудования,
	...
	Если Идентификатор = Неопределено Тогда
		...
		Если ТипыПО <> Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ТипыПО) = Тип("ПеречислениеСсылка.ТипыПодключаемогоОборудования") Тогда
				МассивТиповПО.Добавить(ТипыПО);
			Иначе
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(ДанныеУстройства.Параметры) = Тип("Структура") Тогда
			ДанныеУстройства.Параметры.Вставить("Идентификатор",  Выборка.Ссылка);
			ДанныеУстройства.Параметры.Вставить("ДатаНачалаЗагрузкиЧековИКассовыхСмен",  Выборка.ДатаНачалаЗагрузкиЧековИКассовыхСмен);
			ДанныеУстройства.Параметры.Вставить("ИндексМагазина", Выборка.ИндексМагазина);
			ДанныеУстройства.Параметры.Вставить("ГруппаТоваровОборудования", Выборка.ГруппаТоваровОборудования);
			ДанныеУстройства.Параметры.Вставить("ОсновнойОтдел",             Выборка.ОсновнойОтдел);
			ДанныеУстройства.Параметры.Вставить("НомерОтделаДляККМ",         Выборка.НомерОтделаДляККМ);
		КонецЕсли;
