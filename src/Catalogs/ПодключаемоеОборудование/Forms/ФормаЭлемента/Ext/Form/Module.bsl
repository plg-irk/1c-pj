...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//// тк + 
	//ТипPOSFrontol = Справочники.ДрайверыОборудования.ДрайверPOSFrontol;
	Для каждого СтрокаСписка Из СписокДрайверов Цикл
	//	Если Объект.Ссылка.Пустая() И СтрокаСписка.Значение = ТипPOSFrontol Тогда
	//		Продолжить;
	//	КонецЕсли;
	//// тк - 
...
Процедура НастроитьТипОфлайнОборудования()
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПодсистемаОфлайнОборудования() Тогда
		...
		Элементы.ТипРарусОборудования.ТолькоПросмотр = ЗапретИзмененияДрайвера;
		...
		Если Объект.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМОфлайн Тогда
			...
			Элементы.ТипРарусОборудования.Видимость = Ложь;
			Элементы.ОсновнойОтдел.Видимость = Ложь;
			Элементы.ГруппаТоваровОборудования.Видимость = Ложь;
			ЗаполнитьСписокТиповОфлайнОборудования();
		ИначеЕсли Объект.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.Рарус или 
			Объект.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.УКМ5 Тогда
			Элементы.ТипРарусОборудования.Видимость = Истина;
			Элементы.ОсновнойОтдел.Видимость = Истина;
			Элементы.ГруппаТоваровОборудования.Видимость = Истина;
			Элементы.ТипОфлайнОборудования.Видимость = Ложь;
			
			Если НЕ ЗначениеЗаполнено(Объект.ТипРарусОборудования) Тогда
				ТипPOS = Перечисления.ТипыРарусОборудования.POS;
				Объект.ТипОфлайнОборудования = ТипPOS;
			КонецЕсли;
			
			ЗаполнитьСписокТиповРарусОборудования();
			
		ИначеЕсли Объект.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток Тогда
			Элементы.ТипОфлайнОборудования.Видимость = Ложь;
			Элементы.ТипРарусОборудования.Видимость = Ложь;
			Элементы.ОсновнойОтдел.Видимость = Истина;
			Элементы.ГруппаТоваровОборудования.Видимость = Истина;
		ИначеЕсли Объект.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ТерминалСбораДанных Тогда
			Элементы.ТипОфлайнОборудования.Видимость = Ложь;
			Элементы.ТипРарусОборудования.Видимость = Ложь;
			Элементы.ОсновнойОтдел.Видимость = Истина;
			Элементы.ГруппаТоваровОборудования.Видимость = Истина;
...
Процедура НастроитьДрайвераОфлайнОборудования()
	...
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПодсистемаОфлайнОборудования() Тогда
		...
		//// тк + 
		//ТипPOSFrontol = Перечисления.ТипыОфлайнОборудования.POSFrontol;
		//ДрайверPOSFrontol = Элементы.ДрайверОборудования.СписокВыбора.НайтиПоЗначению(Справочники.ДрайверыОборудования.ДрайверPOSFrontol);
		//Если НЕ ДрайверPOSFrontol = Неопределено Тогда
		//	Элементы.ДрайверОборудования.СписокВыбора.Удалить(ДрайверPOSFrontol);
		//КонецЕсли;
		//// тк - 
		...
		//// тк + 
		//ИначеЕсли Объект.ТипОфлайнОборудования = ТипPOSFrontol Тогда
		//	Элементы.ДрайверОборудования.СписокВыбора.Очистить();
		//	Элементы.ДрайверОборудования.СписокВыбора.Добавить(Справочники.ДрайверыОборудования.ДрайверPOSFrontol);
		//// тк - 
		Иначе
			Элементы.ДрайверОборудования.СписокВыбора.Очистить();
		КонецЕсли;
...
Процедура НачатьЗапускПриложенияЗавершение(Результат, Параметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Выполнен переход на сайт ИТС.'")); 
...
Процедура ПерейтиНаСайтИнструкцииНажатие(Элемент)
	...
	НачатьЗапускПриложения(Оповещения, АдресЗагрузки, , Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ТипРарусОборудованияПриИзменении(Элемент)
...
&НаСервере
Процедура ЗаполнитьСписокТиповРарусОборудования()
	
	Если МенеджерОборудованияВызовСервера.ИспользуетсяПодсистемаОфлайнОборудования() Тогда
		
		ДоступныеТипыОборудования = МенеджерОборудованияРарусСервер.ПолучитьДоступныеТипыРарусОборудования();
		
		Элементы.ТипРарусОборудования.СписокВыбора.Очистить();
		
		Для Каждого СтрокаСписка Из ДоступныеТипыОборудования Цикл
			Элементы.ТипРарусОборудования.СписокВыбора.Добавить(СтрокаСписка);
		КонецЦикла;
...
&НаКлиенте
Процедура ОсновнойОтделПриИзменении(Элемент)
	Объект.ГруппаТоваровОборудования = ПолучитьГруппуТоваровОборудования(Объект.ОсновнойОтдел, Объект.ТипОборудования);
КонецПроцедуры

&НаСервере
Функция ПолучитьГруппуТоваровОборудования(ОсновнойОтдел, ТипОборудования)
	
	ГруппаТоваровОборудования = НЕОПРЕДЕЛЕНО;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ФорматыКатегорийТипыОборудованияГруппыТоваров.ГруппаТоваров КАК ГруппаТоваров
	|ИЗ
	|	Справочник.ФорматыКатегорий.ТипыОборудованияГруппыТоваров КАК ФорматыКатегорийТипыОборудованияГруппыТоваров
	|ГДЕ
	|	ФорматыКатегорийТипыОборудованияГруппыТоваров.Ссылка = &Ссылка
	|	И ФорматыКатегорийТипыОборудованияГруппыТоваров.ТипОборудования = &ТипОборудования";
	
	Запрос.УстановитьПараметр("Ссылка", ОсновнойОтдел.ПодразделениеКомпании.ФорматыАссортимента);
	Запрос.УстановитьПараметр("ТипОборудования", ТипОборудования);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ГруппаТоваровОборудования = ВыборкаДетальныеЗаписи.ГруппаТоваров;
	КонецЕсли;
		
	Возврат ГруппаТоваровОборудования;
