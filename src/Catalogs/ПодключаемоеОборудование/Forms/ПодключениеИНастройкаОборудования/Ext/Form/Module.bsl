...
Процедура ВсеПодразделенияПриИзменении(Элемент)
	...
	ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОсновнойОтдел.ПодразделениеКомпании");
	ОтборГруппаТоваров = Неопределено;
	Для Каждого ТекущийОтбор Из СписокУстройств.Отбор.Элементы Цикл
...
// +Рарус
&НаКлиенте
Процедура ТестированиеПодключаемогоОборудования(Команда)
	
	Результат = МенеджерОборудованияКлиент.ОбновитьРабочееМестоКлиента();
	Если Результат Тогда
		ОткрытьФорму("Обработка.ТестированиеПодключаемогоОборудования.Форма");
	Иначе
...
Процедура ОбновитьПользовательскийИнтерфейс()
	...
	Для Каждого ТекущийОтбор Из СписокУстройств.Отбор.Элементы Цикл
		Если ТекущийОтбор.ЛевоеЗначение = ЛевоеЗначение Тогда
			ОтборПодразделение = ТекущийОтбор;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ОтборПодразделение = Неопределено Тогда
		ОтборПодразделение = СписокУстройств.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборПодразделение.ЛевоеЗначение = ЛевоеЗначение;
	КонецЕсли;

	ОтборПодразделение.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборПодразделение.ПравоеЗначение = ТекущееПодразделение;
...
&НаКлиенте
Процедура ФильтрПоПодразделениюКомпанииПриИзменении(Элемент)
	Если Не СписокУстройств.Группировка.Элементы.Количество() > 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если ФильтрПоПодразделениюКомпании = Истина Тогда
