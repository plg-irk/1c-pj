...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		...
		КонецЕсли;
		// +РАРУС
		Элементы.ФормаСведенияОПользователях.Видимость = Ложь;
		// -РАРУС
	КонецЕсли;
	...
	Если Параметры.РежимВыбора Тогда
		...
		Элементы.ОбработкаПерсональныеНастройкиПерсональныеНастройкиПанельФормы.Видимость = Ложь;
...
Процедура Скопировать(Команда)
	...
	Если ТекущийЭлемент.Имя = "ПользователиСписок" Тогда
		Если Элементы.ПользователиСписок.ТекущиеДанные <> Неопределено Тогда
			Элементы.ПользователиСписок.СкопироватьСтроку();
		КонецЕсли;
	ИначеЕсли ТекущийЭлемент.Имя = "ГруппыПользователей" Тогда
		Если Элементы.ГруппыПользователей.ТекущиеДанные <> Неопределено Тогда
			Элементы.ГруппыПользователей.СкопироватьСтроку();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолучитьКартинкиИЗаполнитьСписокВыбранных(МассивВыбранныхЭлементов)
	...
	Если ТипЗнч(МассивВыбранныхЭлементов) <> Тип("Массив") Тогда
		МассивЭлементов = Новый Массив;
		МассивЭлементов.Добавить(МассивВыбранныхЭлементов);
		МассивВыбранныхЭлементов = МассивЭлементов;
	КонецЕсли;
...
Процедура ИзменитьПараметрыРасширеннойФормыПодбора()
	...
	Если Параметры.Свойство("ПараметрыРасширеннойФормыПодбора") Тогда 
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГруппыПользователей") Тогда
		...
		Если Параметры.Свойство("ПараметрыРасширеннойФормыПодбора") Тогда
			...
			КонецЕсли;
		Иначе
			Элементы.ВыбратьГруппу.Видимость                     = Истина;
		КонецЕсли;
		Элементы.ГруппаГруппы.Видимость                          = Истина;
		Элементы.ОтменитьВыборПользователя.Видимость             = Ложь;
		Элементы.ОчиститьСписокВыбранных.Видимость               = Ложь;
	Иначе
		...
		Элементы.ВыбратьГруппу.Видимость                         = Ложь;
		Элементы.ПользователиСписок.ПоложениеЗаголовка           = ПоложениеЗаголовкаЭлементаФормы.Нет;
	КонецЕсли;
...
Процедура НастроитьФормуПоИспользованиюГруппПользователей(ИзменилосьИспользованиеГрупп = Ложь)
	...
	ВыборГруппПользователей = ХранимыеПараметры.ВыборГруппПользователей
		...
		Если Параметры.РежимВыбора Тогда
			...
			Если (НЕ Параметры.Свойство("ЗакрыватьПриВыборе")) 
					ИЛИ (Параметры.Свойство("ЗакрыватьПриВыборе")И Параметры.ЗакрыватьПриВыборе = Ложь) Тогда
				// Режим подбора.
