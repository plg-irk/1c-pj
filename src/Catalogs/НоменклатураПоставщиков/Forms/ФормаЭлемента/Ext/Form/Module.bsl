...
Процедура НастроитьПараметрыВыбораЭлементовФормы()
	...
	Если Не Элементы.Найти("Характеристика")=Неопределено Тогда
		Если (ЗначениеЗаполнено(Объект.Номенклатура) И Объект.Номенклатура.ТипНоменклатуры.ИспользованиеХарактеристик = 2) Тогда
			УправлениеДиалогомСервер.ОбновитьПараметрВыбора(ЭтотОбъект.Элементы.Характеристика.ПараметрыВыбора, "Отбор.Владелец", Объект.Номенклатура);
		ИначеЕсли (ЗначениеЗаполнено(Объект.Номенклатура) И Объект.Номенклатура.ТипНоменклатуры.ИспользованиеХарактеристик = 1) Тогда
			УправлениеДиалогомСервер.ОбновитьПараметрВыбора(ЭтотОбъект.Элементы.Характеристика.ПараметрыВыбора, "Отбор.Владелец", Объект.Номенклатура.ТипНоменклатуры);
		КонецЕсли;
	КонецЕсли;
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Артикул".
//
// Параметры:
...
КонецПроцедуры //АртикулПриИзменении()

// Обработчик события возникающего при изменении данных реквизитов "Номенклатуры" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнении операции.
//
