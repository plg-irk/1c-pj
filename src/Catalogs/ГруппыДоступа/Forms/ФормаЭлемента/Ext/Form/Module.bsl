...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьПрофилиДоступа(?(ЗначениеЗаполнено(Параметры.ЗначениеКопирования), Параметры.ЗначениеКопирования.Ссылка, Объект.Ссылка));
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОтветНаВопросПриОткрытииФормы = "УстановитьТолькоПросмотр" Тогда
		// +Рарус ОтветНаВопросПриОткрытииФормы = "";
		ТолькоПросмотр = Истина;
	КонецЕсли;
	...
	Если ОтветНаВопросПриОткрытииФормы = "УстановитьПрофильАдминистратор" Тогда
		...
		// +РАРУС
		Объект.ПрофилиДоступа.Добавить().Профиль = ПредопределенноеЗначение("Справочник.ПрофилиГруппДоступа.Администратор");
		ПараметрыОтбора = Новый Структура("Профиль",ПредопределенноеЗначение("Справочник.ПрофилиГруппДоступа.Администратор"));
		ПрофильАдминистратор = ПрофилиДоступа.НайтиСтроки(ПараметрыОтбора);
		ПрофильАдминистратор[0].ПометкаИспользования = Истина;
		// -РАРУС
		Модифицированность = Истина;
		
	ИначеЕсли Объект.Ссылка = ПредопределенноеЗначение("Справочник.ГруппыДоступа.Администраторы")
		// +РАРУС
		    И Объект.ПрофилиДоступа.НайтиСтроки(Новый Структура("Профиль", ПредопределенноеЗначение("Справочник.ПрофилиГруппДоступа.Администратор"))).Количество()=0 
			И НЕ ОтветНаВопросПриОткрытииФормы = "УстановитьТолькоПросмотр" Тогда
		// -РАРУС
	...
	Иначе
		...
		ИначеЕсли НЕ ТолькоПросмотр
			...
			Если ЗначениеЗаполнено(ВладелецФормы.Родитель.Параметры.Профиль) Тогда
				// +РАРУС
				Объект.ПрофилиДоступа.Добавить().Профиль = ВладелецФормы.Родитель.Параметры.Профиль;
				// -РАРУС
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// +РАРУС
	Запрос.УстановитьПараметр("Родитель", Объект.ПрофилиДоступа.Выгрузить().ВыгрузитьКолонку("Профиль"));
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ГДЕ
	|	ПрофилиГруппДоступаНазначение.Ссылка В (&Родитель)
	// -РАРУС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
&НаКлиенте
Процедура ПользовательОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ПользовательАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ПрофилиДоступа

// +РАРУС
&НаКлиенте
Процедура ПрофилиДоступаПометкаИспользованияПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ПрофилиДоступа.ТекущиеДанные;
	
	Если ТекущиеДанные <> Неопределено Тогда
		
		Если ТекущиеДанные.ПометкаИспользования Тогда
			
			Объект.ПрофилиДоступа.Добавить().Профиль = ТекущиеДанные.Профиль;
		Иначе
			
			УдаляемыеСтроки = Объект.ПрофилиДоступа.НайтиСтроки(Новый Структура("Профиль", ТекущиеДанные.Профиль));
			
			Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
				Объект.ПрофилиДоступа.Удалить(УдаляемаяСтрока);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗначенияДоступаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Элементы.ВидыДоступа.ТекущиеДанные <> Неопределено Тогда
		Элемент.ТекущиеДанные.Профиль = Элементы.ВидыДоступа.ТекущиеДанные.Профиль;
	КонецЕсли;
	// -РАРУС
...
Процедура ОбновитьНазначение()
	...
	// +РАРУС
	НазначениеПрофилей = Новый Массив();
	Для каждого ТекущийПрофиль Из Объект.ПрофилиДоступа Цикл
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(НазначениеПрофилей, ТекущийПрофиль.Профиль.Назначение.ВыгрузитьКолонку("ТипПользователей"), ИСТИНА);
	КонецЦикла;
	Для Каждого Участник_ТипПользователей Из НазначениеПрофилей Цикл
		Если Участник_ТипПользователей <> Неопределено Тогда
			Назначение.Добавить(Участник_ТипПользователей);
			ПредставлениеТипа = Участник_ТипПользователей.Метаданные().Синоним;
	// -РАРУС
...
Процедура ОбновитьСоставВидовДоступа(Знач ПриЧтенииНаСервере = Ложь)
	...
	// +РАРУС
	"ВЫБРАТЬ
	|	ВидыДоступаПрофиля.Ссылка КАК Профиль,
	...
	|ГДЕ
	|	ВидыДоступаПрофиля.Ссылка В (&Ссылка)
	|	И НЕ ВидыДоступаПрофиля.Предустановленный";
	
	Запрос.УстановитьПараметр("Ссылка", Объект.ПрофилиДоступа.Выгрузить().ВыгрузитьКолонку("Профиль"));
	// -РАРУС
	...
	Пока Индекс >= 0 Цикл
		...
		// +РАРУС
		Отбор.Вставить("Профиль", Объект.ВидыДоступа[Индекс].Профиль);
		// -РАРУС
	...
	Если ПриЧтенииНаСервере
	     ...
	     И Пользователи.ЭтоПолноправныйПользователь() Тогда // Только администратор может обновлять виды доступа.
	// +РАРУС
		
		СтарыеПрофили = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка, "ПрофилиДоступа").ПрофилиДоступа.Выгрузить();
		
		Если Объект.ПрофилиДоступа.Количество() <> СтарыеПрофили.Количество() Тогда
			ИзменилсяСоставВидовДоступаПриЧтении = ЛОЖЬ;
		Иначе
			ИзменилсяСоставВидовДоступаПриЧтении = ИСТИНА;
			Для Каждого СтрокаПрофиль Из Объект.ПрофилиДоступа Цикл
				Если СтарыеПрофили.Найти(СтрокаПрофиль.Профиль, "Профиль") = Неопределено Тогда
					ИзменилсяСоставВидовДоступаПриЧтении = ЛОЖЬ;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Процедура ВосстановитьОбъектБезУчастниковГруппы(ТекущийОбъект)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ГруппыДоступаПрофили.Профиль
	|ИЗ
	|	Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступаПрофили
	|ГДЕ
	|	ГруппыДоступаПрофили.Ссылка = &Ссылка");
	// -РАРУС
	...
	// +РАРУС
	ТекущийОбъект.ПрофилиДоступа.Загрузить(РезультатыЗапросов[3].Выгрузить());
	// -РАРУС
...
// +РАРУС
&НаСервере
Процедура ЗаполнитьПрофилиДоступа(ТекГруппаДоступа)
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПрофилиГруппДоступа.Ссылка КАК Профиль,
	|	ВЫБОР
	|		КОГДА ГруппыДоступаПрофилиДоступа.Профиль ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПометкаИспользования
	|ИЗ
	|	Справочник.ПрофилиГруппДоступа КАК ПрофилиГруппДоступа
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступаПрофилиДоступа
	|		ПО ПрофилиГруппДоступа.Ссылка = ГруппыДоступаПрофилиДоступа.Профиль
	|			И (ГруппыДоступаПрофилиДоступа.Ссылка = &Ссылка)
	|ГДЕ
	|	НЕ ПрофилиГруппДоступа.ЭтоГруппа
	|	И НЕ ПрофилиГруппДоступа.ПометкаУдаления");
	
	Запрос.УстановитьПараметр("Ссылка", ТекГруппаДоступа);
	
	ПрофилиДоступа.Загрузить(Запрос.Выполнить().Выгрузить());
	
	ПрофилиБылиИзменены = Ложь;
	
КонецПроцедуры // ЗаполнитьПрофилиДоступа() // -РАРУС

// +РАРУС
&НаКлиенте
Процедура ПрофилиДоступаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ОткрытьФорму("Справочник.ПрофилиГруппДоступа.ФормаОбъекта",Новый Структура("Ключ",Элемент.ТекущиеДанные.Профиль));
