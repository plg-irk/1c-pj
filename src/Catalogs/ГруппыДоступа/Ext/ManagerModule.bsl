...
Процедура ПометитьНаУдалениеГруппыДоступаПомеченныхПрофилей(ЕстьИзменения = Неопределено) Экспорт
	...
	|	И НЕ ГруппыДоступа.Ссылка.Предопределенный";
...
Функция ОбновитьГруппыДоступаПрофиля(Профиль, ОбновлятьГруппыДоступаСУстаревшимиНастройками = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// +РАРУС
			И ГруппаДоступа.ПрофилиДоступа.Найти(Справочники.ПрофилиГруппДоступа.Администратор)=Неопределено Тогда
			// Установка профиля Администратор, если не задан.
			ГруппаДоступа.ПрофилиДоступа.Очистить();
			ГруппаДоступа.ПрофилиДоступа.Добавить().Профиль = Справочники.ПрофилиГруппДоступа.Администратор;
		// -РАРУС
		КонецЕсли;
		...
		Иначе
			Для каждого СтрокаВидаДоступа Из ГруппаДоступа.ВидыДоступа Цикл
				Если ВидыДоступаПрофиля.НайтиСтроки(Новый Структура("ВидДоступа, Предустановленный", СтрокаВидаДоступа.ВидДоступа, Ложь)).Количество() = 0 Тогда
					...
					КонецЕсли;
				// +РАРУС 
				ИначеЕсли НЕ СтрокаВидаДоступа.ВсеРазрешены = ВидыДоступаПрофиля.НайтиСтроки(Новый Структура("ВидДоступа, Предустановленный", СтрокаВидаДоступа.ВидДоступа, Ложь))[0].ВсеРазрешены Тогда
					СоставВидовДоступаИзменен = Истина;
				// -РАРУС
				КонецЕсли;
			КонецЦикла;
			...
			Для каждого СтрокаВидаДоступа Из ВидыДоступаПрофиля Цикл
				Если НЕ СтрокаВидаДоступа.Предустановленный 
				   И ГруппаДоступа.ВидыДоступа.Найти(СтрокаВидаДоступа.ВидДоступа, "ВидДоступа") = Неопределено Тогда
					...
					// +РАРУС
					НоваяСтрока.Профиль      = СтрокаВидаДоступа.Ссылка;
					// -РАРУС
					НоваяСтрока.ВсеРазрешены = СтрокаВидаДоступа.ВсеРазрешены;
				КонецЕсли;
			КонецЦикла;
			// +РАРУС 3. Изменение вида доступа
			Для Каждого Строка Из ГруппаДоступа.ВидыДоступа Цикл
				Если ВидыДоступаПрофиля.НайтиСтроки(Новый Структура("ВидДоступа, Предустановленный,ВсеРазрешены", Строка.ВидДоступа, Ложь,Строка.ВсеРазрешены)).Количество() = 0 Тогда
					Строка.ВсеРазрешены = НЕ Строка.ВсеРазрешены;
				КонецЕсли;
			КонецЦикла;
			// -РАРУС
		КонецЕсли;
...
Процедура ЗаполнитьПрофильГруппыДоступаАдминистраторы() Экспорт
	...
	// +РАРУС
	Если Объект.ПрофилиДоступа.Найти(Справочники.ПрофилиГруппДоступа.Администратор)=Неопределено Тогда
		Объект.ПрофилиДоступа.Очистить();
		Объект.ПрофилиДоступа.Добавить().Профиль = Справочники.ПрофилиГруппДоступа.Администратор;
	// -РАРУС
