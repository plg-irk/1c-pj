// Модуль формы списка справочника "Группы товаров оборудования"
...
&НаСервере
Процедура ОбновитьОтображенияГруппТоваров()
	...
	ДоступныеГруппыТоваров = Новый СписокЗначений;
	ДоступныеГруппыТоваров.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ГруппаТоваровОборудования"));
	
	ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка");
	ОтборГруппаТоваров = Неопределено;
	Для Каждого ТекущийОтбор Из Список.Отбор.Элементы Цикл
		Если ТекущийОтбор.ЛевоеЗначение = ЛевоеЗначение Тогда
			ОтборГруппаТоваров = ТекущийОтбор;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ОтборГруппаТоваров = Неопределено Тогда
		ОтборГруппаТоваров = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборГруппаТоваров.ЛевоеЗначение = ЛевоеЗначение;
	КонецЕсли;
	
	ОтборГруппаТоваров.ВидСравнения   = ВидСравненияКомпоновкиДанных.ВСписке;
	ОтборГруппаТоваров.ПравоеЗначение = ДоступныеГруппыТоваров;
	ОтборГруппаТоваров.Использование  = Истина;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьОтображенияГруппТоваров();
