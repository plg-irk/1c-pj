// Модуль формы элемента справочника "Группы товаров оборудования"


////////////////////////////////////////////////////////////////////////////////
// СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ

// Обработчик результата оповещения выбора значения режима заполнения.
//
// Параметры:
...
Процедура УправлениеДиалогомНаСервере()
	...
	Если Объект.РежимЗаполнения=3 Тогда
		...
		Элементы.ОграниченияЗаполненияГруппыТипГруппаНоменклатуры.Заголовок= НСтр("ru = 'Типы номенклатуры'");
	Иначе
		Элементы.ОграниченияЗаполненияГруппы.Доступность = Истина;
		Элементы.ГруппаОграниченияЗаполненияГруппы.Доступность = Истина;
		Элементы.ГруппаОграниченияЗаполненияГруппы.Заголовок   = НСтр("ru = 'Группы номенклатуры'");
		Элементы.ОграниченияЗаполненияГруппыТипГруппаНоменклатуры.Заголовок=НСтр("ru = 'Группы номенклатуры'");
	КонецЕсли;
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Режим заполнения" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
КонецПроцедуры // РежимЗаполненияПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "Режим заполнения".
//
// Параметры:
...
// Обработчик события возникающего на клиенте при обработке выбора данных реквизита "Режим заполнения".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//  ВыбранноеЗначение - Произвольный - Выбранное значение, которое будет установлено как значения 
//                                     поля ввода или массив выбранных значений в случае, если в подчиненной 
//                                     форме свойство МножественныйВыбор имеет значение Истина. В параметре 
//                                     можно вернуть другое значение, которое и будет помещено в элемент управления.
//
&НаКлиенте
Процедура РежимЗаполненияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.РежимЗаполнения <> ВыбранноеЗначение И Объект.РежимЗаполнения = 3 Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	// Произведем настройку параметров отображения кнопок командной панели списка Ограничения заполнения группы.
...
Процедура КаталогАрхиваНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	...
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		Объект.КаталогАрхива = ВыбранныеФайлы[0];
	КонецЕсли;
...
Процедура КаталогАрхиваНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, Объект.КаталогАрхива, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
	
КонецПроцедуры

&НаСервере
Процедура ФиксироватьСобытияПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ФиксироватьСобытияПриИзменении(Элемент)
	
	ФиксироватьСобытияПриИзмененииНаСервере();
