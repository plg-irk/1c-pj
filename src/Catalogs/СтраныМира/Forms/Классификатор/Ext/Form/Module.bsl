...
Процедура КлассификаторВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	Если ТипЗнч(Значение)=Тип("Массив") Тогда
		ОповещатьОВыборе = ?(Значение.Количество()=1, Истина, Ложь);
		Для Каждого Строка Из Значение Цикл
			ОповеститьОВыбореЭлементаКлассификатора(Строка, ОповещатьОВыборе);
		КонецЦикла;
	Иначе
...
&НаКлиенте
Процедура ОповеститьОВыбореЭлементаКлассификатора(ИдентификаторСтрокиВыбора, ОповещатьОВыборе=Истина)
	...
	Если ВсеДанныеСтроки <> Неопределено Тогда
		...
		ОповеститьОВыборе(?(ОповещатьОВыборе, ДанныеВыбора.Ссылка, Неопределено));
	КонецЕсли;
КонецПроцедуры
