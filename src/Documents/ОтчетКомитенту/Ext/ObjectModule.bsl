	...
	Для Каждого СтрокаТовар Из Товары Цикл
		...
		Если ЗначениеЗаполнено(ДокументДляПроверки) Тогда			
			Если ДоговорДокументаПоступления <> ДоговорВзаиморасчетов Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Строка <%1>: Договор документов <%2> <%3>  должен быть равен договору в шапке документа.'"), СтрокаТовар.НомерСтроки, ТекстТипаДокумента, СокрЛП(ДоговорДокументаПоступления.Наименование));
				ВывестиСообщение(ТекстОшибки,ЭтотОбъект,,СтрокаТовар.НомерСтроки,Отказ);
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЭтоОтчетАгента Тогда
		ТаблицаТоваров.Свернуть("ДокументРеализации,АвтоЗакрытиеСделок","СуммаВсего,Вознаграждение");
	Иначе	
		ТаблицаТоваров.Свернуть("ДокументПоступления,АвтоЗакрытиеСделок","СуммаВсего,Вознаграждение");
	КонецЕсли;
	Для каждого СтрокаВзаиморасчетов Из ТаблицаТоваров Цикл
		...
		НаборЗаписейВзаиморасчеты.АвтоЗакрытиеСделок		= СтрокаВзаиморасчетов.АвтоЗакрытиеСделок;
		...
		Если БалансВедетсяПоПодразделениям Тогда
			Если ЭтоОтчетАгента Тогда
				СтрокаСуммовыхРазниц.Подразделение = ДокументПоступленияРеализации.ДоговорКомитента.ПодразделениеКомпании;
			Иначе
				СтрокаСуммовыхРазниц.Подразделение = ДокументПоступленияРеализации.ДоговорВзаиморасчетов.ПодразделениеКомпании;
			КонецЕсли;
		Иначе
