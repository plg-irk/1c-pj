...
Процедура ДеревоПодразделенийПометкаПриИзменении(Элемент)
	...
	//	
	//	Если Не ЗначениеЗаполнено(Объект.итФормат) Тогда		
	//		Объект.итФормат 	= ПолучитьФорматПодразделения(ТекущиеДанные.ПодразделениеКомпании);
	//	Иначе
	//		ФорматПодразделения = ПолучитьФорматПодразделения(ТекущиеДанные.ПодразделениеКомпании);
	//		Если Объект.итФормат <>  ФорматПодразделения Тогда
	//			ТекущиеДанные.Пометка = Ложь;
	//			ПоказатьПредупреждение(,"Выбранный формат не соответствует формату подразделения");
	//			Возврат;
	//		КонецЕсли;			
	//	КонецЕсли;
	//	
...
Функция ПолучитьФорматПодразделения(Подразделение)
	Возврат Подразделение.ФорматыАссортимента;
КонецФункции
//Мальцев А.Ю. ---
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//	// Получим данные текущей строки табличной части
		ПараметрыДействия = Новый Структура;
		ткОбработкаТабличнойЧастиТовары.ЗаполнитьРеквизитРекомендуемыйПроцентНаценки(Объект, Объект.ТипЦен, Неопределено, "РекомендуемыйПроцентНаценки", "Товары", ПараметрыДействия);
	КонецЕсли;
	...
	//Мальцев А.Ю. +++
	Если ЗначениеЗаполнено(Объект.итФормат) Тогда
		ПравоИзменения = ИТ_СпециальныйМодульСервер.ПроверитьПравоНаСервере("итИзмененияДокументовАвтоВводаВАссортимент");
		Если ПравоИзменения = Ложь Тогда
