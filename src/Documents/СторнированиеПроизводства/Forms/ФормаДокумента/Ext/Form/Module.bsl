// Модуль основной формы документа "Сторнирование производства"
...
Процедура НастроитьПараметрыВыбораЭлементовФормы()
	...
	НаборПараметровВыбора = ?(Объект.ХозОперация = Справочники.ХозОперации.СторнированиеПроизводства ,
							Элементы.ПродукцияНоменклатура.ПараметрыВыбора,
							Элементы.ПродукцияКонсолидированнаяНоменклатура.ПараметрыВыбора); 
	
	УправлениеДиалогомСервер.ОбновитьПараметрВыбора(НаборПараметровВыбора,"Отбор.ВидНоменклатуры", ВидыНоменклатуры);
...
Процедура УправлениеДиалогомНаСервере()
	...
	Если Объект.ХозОперация = Справочники.ХозОперации.СторнированиеПроизводства Тогда
		...
		Элементы.СтраницаПродукцияКонсолидированная.Видимость = Ложь;
		Элементы.ДокументОснование.Видимость = Истина;
	Иначе
		Элементы.СтраницаПродукция.Видимость = Ложь;
		Элементы.СтраницаПродукцияКонсолидированная.Видимость = Истина;
...
Процедура ПодборНоменклатуры(Команда)
	Если Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаПродукцияКонсолидированная" Тогда
		ИмяТабличнойЧасти = "ПродукцияКонсолидированная"
	Иначе
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.СторнированиеПроизводства.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура СкладКомпанииПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.СкладКомпанииПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДокументОснованиеПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ДокументОснованиеПриИзменении(Объект, ПараметрыДействия);
...
Процедура ПродукцияНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ПродукцияНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ПродукцияКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ПродукцияКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
//
&НаСервере
Процедура ПродукцияНомерСтрокиДокументаОснованияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Продукция.НайтиПоИдентификатору(Элементы.Продукция.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ПродукцияНомерСтрокиДокументаОснованияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
//
&НаКлиенте
Процедура ПродукцияНомерСтрокиДокументаОснованияПриИзменении(Элемент)
	
	ОчиститьСообщения();
	// Обработаем событие в контексте сервера
	ПродукцияНомерСтрокиДокументаОснованияПриИзмененииНаСервере();
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ПОЛЕЙ ТАБЛИЦЫ ФОРМЫ "ПРОДУКЦИЯ КОНСОЛИДИРОВАННАЯ"
...
//
&НаСервере
Процедура ПродукцияКонсолидированнаяНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.ПродукцияКонсолидированная.НайтиПоИдентификатору(Элементы.ПродукцияКонсолидированная.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.СторнированиеПроизводства.ПродукцияКонсолидированнаяНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
&НаКлиенте
Процедура ПродукцияКонсолидированнаяНоменклатураПриИзменении(Элемент)
	  // Обработаем событие в контексте сервера
	ПродукцияКонсолидированнаяНоменклатураПриИзмененииНаСервере();
