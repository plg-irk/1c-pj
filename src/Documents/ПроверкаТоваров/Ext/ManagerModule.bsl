// Модуль менеджера документа "Проверка товаров"
...
Функция ПолучитьОбязательныеРеквизиты(Объект) Экспорт
	...
	ОбязательныеРеквизиты.Добавить("МоментПроведения");
	...
	ОбязательныеРеквизиты.Добавить("Товары.Результат");
...
Процедура ПодразделениеПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	Для Каждого Строка Из Объект.Товары Цикл
		ТоварыНоменклатураПриИзменении(Объект, Строка);
	КонецЦикла;
...
//  ПараметрыДействия - Структура            - Набор параметров, использующихся при выполнения операции.
//
Процедура МоментПроведенияПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
...
КонецПроцедуры //МоментПроведенияПриИзменении(
...
Процедура ТоварыНоменклатураПриИзменении(Объект, Строка, ПараметрыДействия=Неопределено) Экспорт
	...
	Если Строка <> Неопределено Тогда
		Если  ЗначениеЗаполнено(Строка.Номенклатура) И ЗначениеЗаполнено(Объект.Подразделение) Тогда
			Если Объект.Подразделение.ПолучатьЗакупочныеЦеныИзПротоколов Тогда
				ОбработкаТабличнойЧастиТовары.УстановитьСогласованнуюЦенуНоменклатуры(Объект, Строка, ПараметрыДействия);
			Иначе
				ОбработкаТабличнойЧастиТовары.УстановитьЦенуНоменклатуры(Объект, Строка, ПараметрыДействия, "Товары");
			КонецЕсли;
			
			Запрос = Новый Запрос();
			Запрос.УстановитьПараметр("Момент",Объект.МоментПроведения);
			Запрос.УстановитьПараметр("Подразделение",Объект.Подразделение);
			Запрос.УстановитьПараметр("Номенклатура",Строка.Номенклатура);
			
			ХозОперацииПриход = Новый Массив;
			ХозОперацииПриход.Добавить(Справочники.ХозОперации.ПоступлениеТоваров);
			ХозОперацииПриход.Добавить(Справочники.ХозОперации.ПоступлениеБезвозмездное);
			ХозОперацииПриход.Добавить(Справочники.ХозОперации.ПеремещениеТоваровИзФилиала);
			ХозОперацииПриход.Добавить(Справочники.ХозОперации.ПоступлениеТоваровКомиссия);
			Запрос.УстановитьПараметр("ХозОперацииПриход",ХозОперацииПриход);
			
			ХозОперацииПродажа = Новый Массив;
			ХозОперацииПродажа.Добавить(Справочники.ХозОперации.ЗакрытиеСмены);
			Запрос.УстановитьПараметр("ХозОперацииПродажа",ХозОперацииПродажа);
			
			
			
			Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ СУММА(ОстаткиТоваровКомпанииОстатки.КоличествоОстаток) КАК КоличествоОстаток
                         |ИЗ  РегистрНакопления.ОстаткиТоваровКомпании.Остатки(&Момент, СкладКомпании.ПодразделениеКомпании = &Подразделение И Номенклатура = &Номенклатура) КАК ОстаткиТоваровКомпанииОстатки
                         |СГРУППИРОВАТЬ ПО ОстаткиТоваровКомпанииОстатки.Номенклатура";
			Рез = Запрос.Выполнить().Выбрать();
			Строка.Остаток =?(Рез.Следующий(),Рез.КоличествоОстаток,0);			
			...
			Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ
			             ...
			             |			&Момент,
			             |			ЗаказПоставщику.ПодразделениеКомпании = &Подразделение
			             |				И Номенклатура = &Номенклатура) КАК ЗаказыПоставщикамОстатки
			             |
			             |СГРУППИРОВАТЬ ПО
			             |	ЗаказыПоставщикамОстатки.Номенклатура";
			Рез = Запрос.Выполнить().Выбрать();
			Строка.ТоварыВПути =?(Рез.Следующий(),Рез.ЗаказаноОстаток,0);			
			...
			Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			             ...
			             |ГДЕ
			             |	ОстаткиТоваровКомпании.ХозОперация В(&ХозОперацииПриход)
			             |	И ОстаткиТоваровКомпании.СкладКомпании.ПодразделениеКомпании = &Подразделение
			             |	И ОстаткиТоваровКомпании.Период < &Момент
			             |	И ОстаткиТоваровКомпании.Номенклатура = &Номенклатура
			             |
			             |УПОРЯДОЧИТЬ ПО
			             |	Период УБЫВ";
			Рез = Запрос.Выполнить().Выбрать();
			Строка.ПоследнееПоступление =?(Рез.Следующий(),Рез.Период,Дата("00010101"));
			...
			Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
			             ...
			             |	РегистрНакопления.ОстаткиТоваровКомпании КАК ОстаткиТоваровКомпании
			             |ГДЕ
			             |	ОстаткиТоваровКомпании.ХозОперация В(&ХозОперацииПродажа)
			             |	И ОстаткиТоваровКомпании.СкладКомпании.ПодразделениеКомпании = &Подразделение
			             |	И ОстаткиТоваровКомпании.Период < &Момент
			             |	И ОстаткиТоваровКомпании.Номенклатура = &Номенклатура
			             |
			             ...
			             |	Период УБЫВ";
			Рез = Запрос.Выполнить().Выбрать();
			Строка.ПоследняяПродажа =?(Рез.Следующий(),Рез.Период,Дата("00010101"));
		...
		Иначе
			Строка.Остаток=0;
			Строка.ТоварыВПути=0;
			Строка.ПоследнееПоступление=Дата("00010101");
			Строка.ПоследняяПродажа=Дата("00010101");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	УправлениеПечатьюПлатформа.ДобавитьКоманду(КомандыПечати,
		"Документ.ПроверкаТоваров",
		"ПФ_MXL_ПроверкаТоваров",
		НСтр("ru = 'Проверка товаров'"),
		СсылкаНаДокумент);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ПроверкаТоваров") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
															"ПФ_MXL_ПроверкаТоваров",
															НСтр("ru = 'Проверка товаров'"),
															ПечатьПроверкаТоваров(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
	
КонецПроцедуры // Печать()

// Формирует печатную форму "Проверка товаров".
//
Функция ПечатьПроверкаТоваров(МассивОбъектов, ОбъектыПечати)
	ТабличныйДокумент = Новый ТабличныйДокумент;
	
	// Добавим установку параметров печати
	ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПроверкаТоваров_ПроверкаТоваров";
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПроверкаТоваров.ПФ_MXL_ПроверкаТоваров");
		...
		// Заголовок счета
		ОбластьЗаголовокСчета.Параметры.БанкПолучателя						= Символы.НПП+Документ.Организация.ОсновнойБанковскийСчет.Банк;
		ОбластьЗаголовокСчета.Параметры.БанкПолучателяПредставление			= Документ.Организация.ОсновнойБанковскийСчет.Банк;
		ОбластьЗаголовокСчета.Параметры.БИКБанкаПолучателя					= Символы.НПП+СокрЛП(Документ.Организация.ОсновнойБанковскийСчет.Банк.Код);
		ОбластьЗаголовокСчета.Параметры.СчетБанкаПолучателя					= Символы.НПП+СокрЛП(Документ.Организация.ОсновнойБанковскийСчет.Банк.КоррСчет);
		...
		ОбластьЗаголовокСчета.Параметры.СчетПолучателя						= Символы.НПП+СокрЛП(Документ.Организация.ОсновнойБанковскийСчет.НомерСчета);
		ТабличныйДокумент.Вывести(ОбластьЗаголовокСчета);
		
		// Вывод заголовка документа.
		ОбластьЗаголовок.Параметры.Заполнить(Документ);
		ТекстЗаголовка = УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ, НСтр("ru = 'Проверка товаров'"));
...
КонецФункции //ПечатьПроверкаТоваров()
