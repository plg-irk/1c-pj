// Модуль основной формы документа "Перемещение в торговый зал ЕГАИС"
...
Процедура БлокироватьЭлементыФормы()
	...
	Сформирован = НЕ (Документы.ПеремещениеВТорговыйЗалЕГАИС.ВидыСтатусовДвижений().Найти(СтатусОбработки) = Неопределено);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.ПеремещениеВТорговыйЗалЕГАИС.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ПеремещениеВТорговыйЗалЕГАИС.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ПеремещениеВТорговыйЗалЕГАИС.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыАлкогольнаяПродукцияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ПеремещениеВТорговыйЗалЕГАИС.ТоварыАлкогольнаяПродукцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ОбработкаОповещенияНаСервере(ИмяСобытия, ПараметрыДействия=Неопределено)
	...
	//	Или (ТипЗнч(Параметр) = Тип("Структура") И Параметр.ОбновлятьСтатусОбработкиВФормахДокументов)) Тогда
	//	
	//	ОбновитьПредставленияНаФорме(Истина);
	//	
	//КонецЕсли;
...
&НаКлиенте
Процедура ПолученияОстатков(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ТолькоПартионныйУчет", Ложь);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработакаРезультатВыборки", ЭтотОбъект);
	ОткрытьФорму("Обработка.МодульОбменаЕГАИС.Форма.ФормаПолученияОстатков", ПараметрыОткрытия, ,,,,ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработакаРезультатВыборки(РезультатОповещения, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Строка Из РезультатОповещения Цикл
		...
		НоваяСтрокаТовары.АлкогольнаяПродукция = Строка.АлкогольнаяПродукция;
		НоваяСтрокаТовары.Количество           = Строка.КоличествоОстаток;
		НоваяСтрокаТовары.Справка2             = Строка.Справка2;
...
Процедура ТоварыСправка2ПриИзмененииНаСервере()
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ПеремещениеВТорговыйЗалЕГАИС.ТоварыСправка2ПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
