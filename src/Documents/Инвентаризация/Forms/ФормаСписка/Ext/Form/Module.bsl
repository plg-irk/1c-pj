// Модуль формы списка документа "Инвентаризация"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Мальцев А.Ю. +++
	ОтображатьВнеРабПодразделений   = ПравоПользователя("итОтображатьИнвентаризацииВнеГруппыРабочие");
	Если Не ОтображатьВнеРабПодразделений Тогда
		ЕРПМастерКлючРабПодразделения = "000003458";
		РабПодразделение = Справочники.ПодразделенияКомпании.НайтиПоРеквизиту("ЕРПМастерКлюч", ЕРПМастерКлючРабПодразделения);
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "ПодразделениеКомпании", РабПодразделение, ВидСравненияКомпоновкиДанных.ВИерархии);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбъединениеДокументов(Команда)
...
&НаКлиенте
Процедура итИзменениеДатЗапрета(Команда)
	ОткрытьФорму("РегистрСведений.ДатыЗапретаИзменения.Форма.итДатыЗапретаИзменения");
КонецПроцедуры
