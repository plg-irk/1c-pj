...
Процедура КонтрагентПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	Если НЕ Объект.Контрагент.Пустая() Тогда
		Если (Объект.РасчетныйСчетКонтрагента.Пустая()) ИЛИ (НЕ Объект.РасчетныйСчетКонтрагента.Владелец = Объект.Контрагент) Тогда
			Объект.РасчетныйСчетКонтрагента = Объект.Контрагент.ОсновнойБанковскийСчет;	
		КонецЕсли;
	Иначе
...
Функция ПечатьСчетОтПоставщика(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ОбластьЗаголовокСчета.Параметры.БИКБанкаПолучателя			= Символы.НПП+СокрЛП(Документ.РасчетныйСчетКонтрагента.Банк.Код);
		ОбластьЗаголовокСчета.Параметры.СчетБанкаПолучателя			= Символы.НПП+СокрЛП(Документ.РасчетныйСчетКонтрагента.Банк.КоррСчет);	
		ОбластьЗаголовокСчета.Параметры.ИНН							= Символы.НПП+УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Контрагент,Новый Структура("ИНН"));
		ОбластьЗаголовокСчета.Параметры.КПП							= Символы.НПП+УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Контрагент,Новый Структура("КПП"),,,ДатаДокумента);
		ОбластьЗаголовокСчета.Параметры.Получатель					= Документ.Контрагент;
		ОбластьЗаголовокСчета.Параметры.ПолучательПредставление		= УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Контрагент,Новый Структура("Наименование"),,,ДатаДокумента);
		...
		ТекстЗаголовка = УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ, НСтр("ru = 'Счет от поставщика'"));
		...
		ОбластьЗаголовок.Параметры.ПредставлениеПоставщика = УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Контрагент,СтруктураПредставления,,,ДатаДокумента);
		...
		Иначе
			ОбластьЗаголовок.Параметры.ПредставлениеПокупателя = УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Организация,СтруктураПредставления,,,ДатаДокумента,ДополнительныеПараметры);
		КонецЕсли;
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьПоДокументуОснование",, "Товары", НСтр("ru = 'Заполнить по документу-основание'"),,,,, Ложь);
