...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ГруппаРеквизитыАвтора.ЗаголовокСвернутогоОтображения = "Автор " + Объект.Автор;
	// Конец СтандартныеПодсистемы.Свойства
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ВыполняетсяОтправка Тогда
		...
		Если Объект.СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Черновик") И ЗначениеЗаполнено(Объект.ДатаКогдаОтправить) Тогда
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// +Рарус LISMAK
	// статус письма в момент при записи = черновик, поэтому вложения в коде ниже не записываются и не отправляются -> тут возврат возможно лишний.
...
&НаКлиенте
Процедура СписокПолучателейОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, Параметры, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СписокПолучателейАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
...
Процедура СписокПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Элемент.ТекущиеДанные.Адрес = Адрес;
...
Процедура СписокПолучателейПредставлениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ДанныеВыбора = ПолучитьДанныеВыбораДляЭлектронногоПисьма(Текст, 0);
...
&НаКлиенте
Процедура СписокПолучателейПослеУдаления(Элемент)
...
Процедура ОтправитьПереслатьВыполнить(Команда)
	...
	Если Команда.Имя = "Отправить" И Объект.СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Черновик") Тогда
		Объект.СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Исходящее");
	КонецЕсли;
...
Процедура ОпределитьВидимостьДоступностьЭлементовВЗависимостиОтСтатусаПисьма()
	...
	Иначе
		...
		Элементы.СписокПолучателейКому.ТолькоПросмотр = Истина;
...
Процедура ОбработатьПереданныеПараметры(ПереданныеПараметры)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если ТипЗнч(ПереданныеПараметры.Тело) = Тип("Структура") Тогда
			ТекстHTML = СтрЗаменить(ПереданныеПараметры.Тело.ТекстHTML, "cid:", "_");
			ТекстПисьмаФорматированныйДокумент.УстановитьHTML(ТекстHTML, ПереданныеПараметры.Тело.СтруктураВложений);
...
Процедура ЗаполнитьПодобранныхПолучателейПослеВыбора(ВыбранноеЗначение)
	...
	Перем ДляПодбора, СоответствиеТабличныхЧастей;
