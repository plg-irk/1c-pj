...
Процедура УправлениеДиалогомНаСервере()
	...
	ВидимостьСуммы = (Объект.ПричинаСписания = Перечисления.ПричиныСписанияЕГАИС.Реализация);
	
	Элементы.ТоварыЦенаРозничная.Видимость  = ВидимостьСуммы;
	Элементы.ТоварыСуммаРозничная.Видимость = ВидимостьСуммы;
	
	Если НЕ Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.Найти("ФормаСоздатьНаОсновании") = Неопределено 
		И НЕ Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.Найти("ФормаДокументПеремещениеВТорговыйЗалЕГАИССоздатьНаОсновании") = Неопределено 
		И Объект.ХозОперация = Справочники.ХозОперации.АктСписанияСТорговогоЗалаЕГАИС Тогда
		Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.ФормаДокументПеремещениеВТорговыйЗалЕГАИССоздатьНаОсновании.Заголовок = "Перемещение в торговый зал ЕГАИС";
	ИначеЕсли НЕ Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.Найти("ФормаСоздатьНаОсновании") = Неопределено 
		И НЕ Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.Найти("ФормаДокументПеремещениеВТорговыйЗалЕГАИССоздатьНаОсновании") = Неопределено 
		И Объект.ХозОперация = Справочники.ХозОперации.АктСписанияЕГАИС Тогда
		Элементы.ГлобальныеКоманды.ПодчиненныеЭлементы.ФормаСоздатьНаОсновании.ПодчиненныеЭлементы.ФормаДокументПеремещениеВТорговыйЗалЕГАИССоздатьНаОсновании.Заголовок = "Перемещение из торгового зала ЕГАИС";
	КонецЕсли;
...
Процедура БлокироватьЭлементыФормы()
	...
	Сформирован = НЕ (Документы.АктСписанияЕГАИС.ВидыСтатусовДвижений().Найти(СтатусОбработки) = Неопределено);
...
Процедура Подключаемый_ОбработкаРезультатаОповещения(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если ДополнительныеПараметры = "КомандаЗаполненияТоварыЗаполнитьАлкогольнойПродукциейПоРезультатамПродаж" И 
		ПолучитьЗначениеПараметраСтруктуры(РезультатОповещения, "ИзмениласьХозОперация", Ложь) Тогда
		ХозОперацияПриИзменении(ЭтотОбъект.Команды.ХозОперацияАктСписанияСТорговогоЗалаЕГАИС);
	КонецЕсли;
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.АктСписанияЕГАИС.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ХозОперацияПриИзменении(Команда)
	...
	ХозОперацияВидимостьЭлементовПриИзмененииСервер();
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ДатаПриИзменении(Объект, ПараметрыДействия);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Причина списания" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура ПричинаСписанияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	УправлениеДиалогомНаСервере();
	
КонецПроцедуры

// Обработчик события возникающего на клиенте при изменении данных реквизита "Причина списания".
//
// Параметры:
//  Команда - КомандаФормы - Команда, в которой возникло данное событие.
//
...
Процедура ПричинаСписанияПриИзменении(Элемент)
	ПричинаСписанияПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ОрганизацияЕГАИСПриИзмененииНаСервере()
	...
	Документы.АктСписанияЕГАИС.ОрганизацияЕГАИСПриИзменении(Объект);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыАлкогольнаяПродукцияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыАлкогольнаяПродукцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЦенаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыЦенаРозничнаяПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыСуммаРозничнаяПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСправка2ПриИзмененииНаСервере()
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.ТоварыСправка2ПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Элементы.Товары.ТекущиеДанные.ПредставлениеМарок  = "Марки не указаны";
	КонецЕсли;
...
Процедура МаркиКодМаркиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктСписанияЕГАИС.МаркиКодМаркиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСправка2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Отбор.Вставить("Грузополучатель",      Объект.ОрганизацияЕГАИС);
	Отбор.Вставить("АлкогольнаяПродукция", ТекущиеДанные.АлкогольнаяПродукция);
...
&НаСервере
Процедура ХозОперацияВидимостьЭлементовПриИзмененииСервер()
