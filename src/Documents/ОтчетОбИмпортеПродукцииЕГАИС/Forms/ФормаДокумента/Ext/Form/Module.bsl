...
&НаСервере
Функция ЕстьНеПривязанныеМарки()
	
	Результат = Ложь;
	
	ТаблицаМарок = Объект.Марки.Выгрузить(,"ИдентификаторСтроки");
	ТаблицаМарок.Свернуть("ИдентификаторСтроки");
	
	Запрос = Новый Запрос;
	Запрос.Текст = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаМарок.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	&Ссылка КАК Ссылка
	|ПОМЕСТИТЬ
	|	ТаблицаМарок
	|ИЗ
	|	Документ.ОтчетОбИмпортеПродукцииЕГАИС.Марки КАК ТаблицаМарок
	|ГДЕ
	|	ТаблицаМарок.Ссылка = &Ссылка
	|;
	|
	|ВЫБРАТЬ
	|	ТаблицаМарок.ИдентификаторСтроки
	|ИЗ
	|	ТаблицаМарок КАК ТаблицаМарок
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	Документ.ОтчетОбИмпортеПродукцииЕГАИС.Товары КАК ТаблицаТовары
	|ПО
	|	ТаблицаМарок.ИдентификаторСтроки = ТаблицаТовары.ИдентификаторСтроки И 
	|	ТаблицаМарок.Ссылка = ТаблицаТовары.Ссылка
	|ГДЕ
	|	ТаблицаТовары.ИдентификаторСтроки ЕСТЬ NULL
	|;
	|";
	...
	Если Выборка.Следующий() И (НЕ Выборка.ИдентификаторСтроки = Null) Тогда
		Результат = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтправитьОтчетОбИмпортеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОтправитьОтчетОбИмпорте()
	
	ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ОтправитьОтчетОбИмпортеЗавершение", ЭтотОбъект);
	
	Если ЕстьНеПривязанныеМарки() Тогда
		
		СодержимоеВопроса = Новый Массив;
		СодержимоеВопроса.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'ВНИМАНИЕ!!!'"),, ЦветОшибки));
		СодержимоеВопроса.Добавить(Символы.ПС);
		СодержимоеВопроса.Добавить(НСтр("ru = 'Есть марки не привязанные к товарам. Продолжить?'"));
		
		ТекстВопроса = Новый ФорматированнаяСтрока(СодержимоеВопроса);
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,,НСтр("ru = 'Передача отчета об импорте'"));
		
	Иначе
		
		ТекстВопроса = НСтр("ru = 'Отправить отчет об импорте ТТН?'");
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,,НСтр("ru = 'Передача отчета об импорте'"));
...
Процедура БлокироватьЭлементыФормы()
	...
	Сформирован = НЕ (Документы.ОтчетОбИмпортеПродукцииЕГАИС.ВидыСтатусовДвижений().Найти(СтатусОбработки) = Неопределено);
	ЭлементыФомы = ";СтраницаДополнительно, СтраницаШапка, Шапка";
...
Процедура ОбработкаРезультатаОповещенияНаСервере(РезультатОповещения, ДополнительныеПараметры=Неопределено)
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Строка") И ДополнительныеПараметры = "КомандаЗаполненияМаркиЗагрузкаИзФайла" Тогда
		...
		|	ТаблицаМарок
		|ИЗ
		|	НачальнаяТаблицаМарок КАК ТаблицаМарок
		|;
		|
		|УНИЧТОЖИТЬ
		|	НачальнаяТаблицаМарок
		...
		//|	ЗНАЧЕНИЕ(КлассификаторАП.Ссылка, ЗНАЧЕНИЕ(Справочник.КлассификаторАлкогольнойПродукцииЕГАИС.ПустаяСсылка)) КАК АлкогольнаяПродукция
		|ПОМЕСТИТЬ
		|	ТаблицаАП
		|ИЗ
		|	ТаблицаМарок КАК ТаблицаМарок
		...
		|ПО
		|	ТаблицаМарок.ИдентификаторСтроки = КлассификаторАП.Код
		|;
		|
		|ВЫБРАТЬ
		...
		|
		|ВЫБРАТЬ
		|	ТаблицаСопоставленныхТоваров.НомерСтроки-1 КАК ИндексСтроки,
		|	ТаблицаАП.АлкогольнаяПродукция,
		|	ТаблицаАП.ИдентификаторСтроки
		|ИЗ
		|	ТаблицаСопоставленныхТоваров КАК ТаблицаСопоставленныхТоваров
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|	ТаблицаАП КАК ТаблицаАП
		|ПО
		|	ТаблицаСопоставленныхТоваров.АлкогольнаяПродукция = ТаблицаАП.АлкогольнаяПродукция
		|";
		Запрос.УстановитьПараметр("ТаблицаМарок",   Объект.Марки.Выгрузить());
		...
		Пока Выборка.Следующий() Цикл
			Если НЕ ОбработанныеТовары.Найти(Выборка.ИдентификаторСтроки) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ОбработанныеТовары.Добавить(Выборка.ИдентификаторСтроки);
			
			ТекущаяСтрока = Объект.Товары[Выборка.ИндексСтроки];
			ТекущаяСтрока.ИдентификаторСтроки  = Выборка.ИдентификаторСтроки;
		...
		//		Продолжить;
		//	КонецЕсли;
		//	КодАП = ТекущаяСтрока.АлкогольнаяПродукция.Код;
		//	
		//	ТекущаяСтрока.ИдентификаторСтроки = КодАП;
		//КонецЦикла;
		...
		Для Каждого ТекущиеДанные Из Объект.Марки Цикл
			...
			Документы.ОтчетОбИмпортеПродукцииЕГАИС.МаркиКодМаркиПриИзменении(Объект, ТекущиеДанные);
...
&НаСервере
Функция ПолучитьПараметрыЗапросаАП()
	...
	|ГДЕ
	|	КлассификаторАлкогольнойПродукцииЕГАИС.Ссылка В (&Ссылки)
	|";
	
	Запрос.УстановитьПараметр("Ссылки", Объект.Товары.Выгрузить().ВыгрузитьКолонку("АлкогольнаяПродукция"));
	КодыАП = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Код");
	
	ТранспортныйМодуль = ИнтеграцияЕГАИСВызовСервера.ПолучитьТранспортныйМодульПоОрганизацииЕГАИС(Объект.Импортер);
	
	Результат = Новый Структура();
	Результат.Вставить("КодыАП",             КодыАП);
...
Процедура ЗапроситьАлкогольнуюПродукцию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Импортер) Тогда
		ВывестиСообщение(НСтр("ru = 'Не заполнен Импортер'"), Объект, "Импортер");
		Возврат;
	КонецЕсли;
	
	ТранспортныйМодуль = ИнтеграцияЕГАИСВызовСервера.ПолучитьТранспортныйМодульПоОрганизацииЕГАИС(Объект.Импортер);
	
	Результат = ПолучитьПараметрыЗапросаАП();
	
	ИнтеграцияЕГАИСКлиент.ОтправитьЗапросВЕГАИС(ПредопределенноеЗначение("Перечисление.ВидыОбъектовЕГАИС.ЗапросАлкогольнойПродукцииПоАлкокодуВерсия2"), "КОД", Результат.КодыАП, Результат.ТранспортныйМодуль);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ТоварыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыАлкогольнаяПродукцияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.ТоварыАлкогольнаяПродукцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
//
&НаСервере
Процедура ТоварыПослеУдаленияНаСервере(ПараметрыДействия=Неопределено, УдаляемыеСтроки=Неопределено)
...
Процедура МаркиКодМаркиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетОбИмпортеПродукцииЕГАИС.МаркиКодМаркиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
