...
Функция ОбработкаЗаполнения_ПриходныйКассовыйОрдер(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
	...
	СуммаНал 						= РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(ДанныеЗаполнения.СуммаДокумента,ДанныеЗаполнения.ВалютаДокумента,ДанныеЗаполнения.КурсДокумента,ВалютаДокумента,КурсДокумента);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ХозОперация=Справочники.ХозОперации.ВнесениеВКассуККМ Тогда
		Для Каждого СтрокаОплат Из Оплаты Цикл 
			Если СтрокаОплат.Сумма>0 И СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Наличные Тогда
				Если ЗначениеЗаполнено(Инкассатор) Тогда
	...
	ИначеЕсли ХозОперация=Справочники.ХозОперации.ИзъятиеИзКассыККМ Тогда
		Для Каждого СтрокаОплат Из Оплаты Цикл 
			...
			Если СтрокаОплат.Сумма>0 И СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Безналичные Тогда
			...
			Если СтрокаОплат.СуммаВозврат > 0 И СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Безналичные Тогда
			...
			Если СтрокаОплат.Сумма > 0 И (СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.КлубнаяКарта
									  ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Кредит
									  ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Талон
									  ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.ЗаСчетЗаведения)Тогда
				// извлечем деньги из кассы ККМ по безналу на расход
				НаборЗаписейКассыККМ = Движения.КассыККМ;
				НаборЗаписейКассыККМ.ДокументОбъект  = ЭтотОбъект;
				НаборЗаписейКассыККМ.РежимПроведения = РежимПроведения;
			...
			Если СтрокаОплат.СуммаВозврат > 0 И (СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.КлубнаяКарта
									  		ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Кредит
									  		ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.Талон
									  		ИЛИ СтрокаОплат.ТипОплаты.ВидОплаты = Перечисления.ВидыОплат.ЗаСчетЗаведения) Тогда
				// извлечем деньги из кассы ККМ по безналу на расход
