...
Процедура СформироватьГрафикПоставок()
	...
	ОткрытьФорму("Документ.ЗаказВнутренний.Форма.ФормаГрафикПоставок",ПараметрыОткрытия,Элементы.ТоварыГрафикПоставки, ,ВариантОткрытияОкна.ОтдельноеОкно,,Новый ОписаниеОповещения("ОбработкаРезультатаОповещенияСформироватьГрафикПоставок", ЭтотОбъект),РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
КонецФункции // РаспределениеЗаказаПриОкончанииРедактированияНаСервере()
