...
Процедура СохранитьИзмененияНаСервере()
	...
	Для Каждого СтрокаГрафика Из ГрафикПоставок Цикл
		...
		ПредставлениеПоставки = ПредставлениеПоставки + ?(ПредставлениеПоставки = "", "", "; ") + Строка(КоличествоЗаказа) 
									+ " " + ЕдиницаИзмерения + "," + Символы.НПП + НСтр("ru = 'дата поставки:'") + Символы.НПП 
									+ Формат(СтрокаГрафика.ДатаПоставки, "ДФ=dd.MM.yy");
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Количество = 0 Тогда
		Отказ = Истина;
		ВывестиСообщение(НСтр("ru='Не задано количество'"));
	КонецЕсли;
	...
	|		Номенклатура = &Номенклатура" + ?(ЕстьХарактеристика, "
