...
Процедура ЗаполнитьТабличнуюЧастьПоОснованию(ТекущийДокумент,ДокументОснование, ТолькоПроведенные = Истина)Экспорт
	...
	Если НЕ ДокументОснование.Метаданные().ТабличныеЧасти.Найти("Товары") = Неопределено И НЕ ТекущийДокумент.Метаданные().ТабличныеЧасти.Найти("Товары") = Неопределено Тогда
		...
		Для Каждого ТекДокумент Из МассивПодчиненныхДокументов Цикл
			Если НЕ ТекДокумент.Документ.Метаданные().ТабличныеЧасти.Найти("Товары") = Неопределено И ТекДокумент.Документ.Проведен Тогда
...
Функция ВычитаниеТаблиц(Знач ОсновнаяТаблица, МассивВычитаемыхТаблиц, СтруктураОсновнойТаблицы = Неопределено, МассивСтруктурВычитаемыхТаблиц = Неопределено)
	...
	Если НЕ ВычитаниеТаблицПроверкаЗаполнения(ОсновнаяТаблица, МассивВычитаемыхТаблиц, СтруктураОсновнойТаблицы, МассивСтруктурВычитаемыхТаблиц) Тогда
		#Если Клиент Тогда
			ВывестиСообщение(НСтр("ru = 'Вычитание таблиц не было выполнено, т.к. были обнаружены ошибки в параметрах'"));
		#КонецЕсли	
...
Функция ПечатьРасходныйСкладскойОрдер(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ТекстЗаголовка = УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ, НСтр("ru = 'Расходный складской ордер'"));
		...
		Если ЗначениеЗаполнено(Документ.ВладелецТовара) Тогда
			Если ТипЗнч(Документ.ВладелецТовара) = Тип("СправочникСсылка.Контрагенты") Тогда
				ОбластьЗаголовок.Параметры.ПредставлениеВладельца	= УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.ВладелецТовара,,,,ДатаДокумента);
			Иначе
				ОбластьЗаголовок.Параметры.ПредставлениеВладельца	= УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.ВладелецТовара);
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Документ.ДокументОснование) Тогда
			ОбластьЗаголовок.Параметры.Заполнить(Документ.ДокументОснование);
			ОбластьЗаголовок.Параметры.ПредставлениеДокументаОснования	= УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ.ДокументОснование);
			Если ЕстьРеквизит(Документ.ДокументОснование,"Контрагент") Тогда
				ОбластьЗаголовок.Параметры.ПредставлениеКонтрагента		= УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.ДокументОснование.Контрагент,,,,ДатаДокумента);
			КонецЕсли;
...
Функция ПечатьПодборочныйЛист(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		КонецЕсли;
		ОбластьЗаголовок.Параметры.ПредставлениеПокупателя = УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.ВладелецТовара,,,, ?(ТипЗнч(Документ.ВладелецТовара) = Тип("СправочникСсылка.Контрагенты"), ДатаДокумента, Неопределено));
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьСкладскимиОстаткамиПоВыбраннойЯчейке",, "Товары", НСтр("ru = 'Заполнить складскими остатками по ячейке'"), ПараметрыКоманды);
	
	ПараметрыКоманды = Новый Структура();
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьЯчейкиУВыбранныхТоваров", "ТоварыЗаполнитьЯчейкиУВыбранныхТоваров", "Товары", НСтр("ru = 'Заполнить ячейки у выбранных товаров'"), ПараметрыКоманды,,,, ЛОЖЬ, ИСТИНА);
...
//
Функция ТоварыЗаполнитьСкладскимиОстаткамиПоВыбраннойЯчейке(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
...
//
Функция ТоварыЗаполнитьЯчейкиУВыбранныхТоваров(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
