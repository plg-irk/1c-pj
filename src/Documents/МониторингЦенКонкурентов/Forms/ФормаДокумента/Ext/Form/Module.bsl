// Модуль основной формы документа "Мониторинг цен конкурентов"
&НаКлиенте
...
Процедура ЗаполнитьРасчетныеРеквизиты()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Документы.МониторингЦенКонкурентов.РассчитатьДополнительныеЦеныИРеквизиты(Объект);
	КонецЕсли;	
...
Процедура ВыборОбъектаСравненияЗавершение(ЗначениеВыбора, ДополнительныеПараметры) Экспорт
	...
	Документы.МониторингЦенКонкурентов.ТоварыНоменклатураПриИзменении(Объект, НоваяСтрока);
...
Процедура УправлениеДиалогомНаСервере()
	...
	Элементы.СсылкаНаФото.Заголовок = КаталогФайлов;
	
	Элементы.ГруппаКартинки.Видимость = ОтобразитьВыводИзображений;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Представление = "ТоварыНоменклатура";
	ПолеЭлемента      = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("ТоварыНоменклатура");
		
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.ФлагСортировки");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
		
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветФона", Новый Цвет(240, 128, 128));
	//----------
...
//
&НаСервере
Процедура КонкурентПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.КонкурентПриИзменении(Объект, ПараметрыДействия);
...
Процедура КонкурентПриИзменении(Элемент)
	...
	// Обработаем событие в контексте сервера
	ПараметрыДействия = Новый Структура;
	КонкурентПриИзмененииНаСервере(ПараметрыДействия);

КонецПроцедуры // ДействителенДоПриИзменении()

&НаСервере
Процедура КонкурентНачалоВыбораНаСервере()
	
	МассивКонкурентов = Документы.МониторингЦенКонкурентов.ПолучитьДоступныхКонкурентов(Объект);
	Если ТипЗнч(МассивКонкурентов)=Тип("Массив") Тогда
		УправлениеДиалогомСервер.ОбновитьПараметрВыбора(Элементы.Конкурент.ПараметрыВыбора, "Отбор.Ссылка", МассивКонкурентов);
	Иначе
		УправлениеДиалогомСервер.УдалитьПараметрВыбора(Элементы.Конкурент.ПараметрыВыбора, "Отбор.Ссылка");
	КонецЕсли;
	
КонецПроцедуры //КонкурентНачалоВыбораНаСервере()

&НаКлиенте
Процедура КонкурентНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	//СтандартнаяОбработка = Ложь;
	
	// Обработаем событие в контексте сервера
	КонкурентНачалоВыбораНаСервере();
	
	//ПараметрыФормы = Новый Структура();
	//ПараметрыФормы.Вставить("РежимВыбора", Истина);
	//ПараметрыФормы.Вставить("Отбор", Новый Структура("Ссылка", ПолучитьДоступныхКонкурентов()));
	//ОткрытьФорму("Справочник.Конкуренты.ФормаСписка", ПараметрыФормы, Элемент);
	
КонецПроцедуры //КонкурентНачалоВыбора()
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
// Обработчик события возникающего при изменении данных реквизита "ЦенаАкция" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура ТоварыЦенаАкцияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыЦенаАкцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЦенаАкцияПриИзменении(Элемент)
	...
	// Обработаем событие в контексте сервера
	ТоварыЦенаАкцияПриИзмененииНаСервере();
...
Процедура ТоварыНоменклатураНачалоВыбораНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыНоменклатураНачалоВыбора(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыНоменклатураОчисткаНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.МониторингЦенКонкурентов.ТоварыНоменклатураОчистка(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура СледующаяКартинка(Команда)
	...
	Если ФайлыКартинок.Количество()>0 Тогда
		Если НайтиФайлы(КаталогФайлов+"\Обработанные").Количество()=0 Тогда
			СоздатьКаталог(КаталогФайлов + "\Обработанные");
		КонецЕсли;
		
		ПереместитьФайл(КаталогФайлов+"\"+ФайлыКартинок[0].Значение, КаталогФайлов+"\Обработанные\"+ФайлыКартинок[0].Значение);
		ФайлыКартинок.Удалить(0);
	КонецЕсли;
	
	Если ФайлыКартинок.Количество()>0 Тогда
		ПолноеИмяФайла = КаталогФайлов + "\" + ФайлыКартинок[0].Значение;
		Элементы.СсылкаНаФото.Заголовок = ПолноеИмяФайла;
		СсылкаНаФото = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ПолноеИмяФайла), УникальныйИдентификатор);
	Иначе
		СсылкаНаФото = "";
		Элементы.СсылкаНаФото.Заголовок = КаталогФайлов;
		Элементы.СсылкаНаФото.ТекстНевыбраннойКартинки = "Нет файлов в указанном каталоге, для выбора нажмите F7.";
	КонецЕсли;
	
КонецПроцедуры //СледующаяКартинка()

&НаКлиенте
Процедура ПропуститьКартинку(Команда)
	
	Если ФайлыКартинок.Количество()>0 Тогда
		ФайлыКартинок.Сдвинуть(0, ФайлыКартинок.Количество()-1);
...
КонецПроцедуры //ПропуститьКартинку()
...
Процедура СсылкаНаФотоНажатие(Элемент, СтандартнаяОбработка)
	...
	ДиалогВыбора.Заголовок	= "Укажите расположение файлов...";
	...
	Если КаталогНаДиске.Существует() Тогда
		...
		НайденныеФайлы = НайтиФайлы(КаталогФайлов, "*.jpg");
		Для Индекс=0 По НайденныеФайлы.ВГраница() Цикл
			ИмяФайла = НайденныеФайлы[Индекс].Имя;
			ФайлыКартинок.Добавить(ИмяФайла);
		КонецЦикла;
		...
		КонецЕсли;
	Иначе
        Сообщить("Нет доступа к каталогу " + КаталогФайлов);
	КонецЕсли;
	
КонецПроцедуры //СсылкаНаФотоНажатие()
...
КонецПроцедуры //ОткрытьИзображение()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаМониторинга) Тогда
		Объект.ДатаМониторинга = Объект.Дата;
	КонецЕсли;
...
Процедура ПоказатьИзображения(Команда)
	...
	ОтобразитьВыводИзображений = НЕ ОтобразитьВыводИзображений;
	Элементы.Товары.КоманднаяПанель.ПодчиненныеЭлементы.ТоварыПоказатьИзображение.Пометка = ОтобразитьВыводИзображений;
