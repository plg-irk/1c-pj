...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		...
		КонецЕсли;
	Иначе
		Если Объект.Ссылка.Пустая() И Объект.РежимАнкетирования <> Перечисления.РежимыАнкетирования.Интервью Тогда
			ТекстСообщения = НСтр("ru = 'Документ ""Анкета"" может быть создан только в режиме ""Интервью"" или внешним пользователем.'");
	...
	ЗащищенныеФункцииСервер.ДобавитьКоманду(ЭтотОбъект, "НастроитьПараметрыДокумента", "КоманднаяПанельШапка", БиблиотекаКартинок.ПараметрыДокумента, НСтр("ru = 'Параметры'"), НСтр("ru = 'Расширенное редактирование параметров документа'"), ЛОЖЬ,,, ОтображениеКнопки.КартинкаИТекст);
	// -Рарус
	...
	Если Объект.РежимАнкетирования = Перечисления.РежимыАнкетирования.Интервью Тогда
		...
		// +РАРУС
		ИначеЕсли НЕ Объект.Опрос.Пустая() Тогда
...
Функция ОкончаниеРедактированиеФормыЗаполнения(РежимЗаписи)
	...
	// +Рарус Иначе ссылка пустая и документ рассылка невозможно сохранить анкету
...
Процедура ЗаполнитьОтветыТабличныйВопросСоставной(СтрокаДерева,Таблица)
	...
	Для ИндексКолонки = 0 По СтрокаДерева.СоставТабличногоВопроса.Количество()-1 Цикл
		...
		Для ИндексСтроки = 0 По Таблица.Количество() - 1 Цикл
			...
			// +РАРУС
			// Будем записывать в ТЧ Состав все вопросы вне зависимости от заполнения ответа.
...
Процедура УстановитьОбработчикСобытияПриИзмененииДляКомплексныхВопросов(СтрокаТаблицы, ИмяВопроса)
	...
	Для каждого СтрокаКомплексногоВопроса Из СтрокаТаблицы.СоставКомплексногоВопроса Цикл
		...
		Иначе
			...
			Элементы[ИмяРеквизитаВопроса].УстановитьДействие("ПриИзменении","Подключаемый_ПриИзмененииВопроса");
			Если СтрокаКомплексногоВопроса.ТребуетсяКомментарий Тогда
				ИмяРеквизитаКомментария = ИмяВопроса + "_Комментарий" + Формат(СтрокаКомплексногоВопроса.НомерСтроки, "ЧГ=");
				Элементы[ИмяРеквизитаКомментария].УстановитьДействие("ПриИзменении","Подключаемый_ПриИзмененииВопроса");
			КонецЕсли;
		КонецЕсли;
...
Процедура Подключаемый_ВыполнитьКоманду(Команда)
	ПодключаемыеКомандыКлиент.ВыполнитьКоманду(ЭтотОбъект, Команда, Объект, Истина);
КонецПроцедуры
