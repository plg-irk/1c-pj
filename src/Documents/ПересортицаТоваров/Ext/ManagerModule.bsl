...
Функция ПолучитьСтрокуПоПриходу(ТекущаяСтрока)
	...
	Исключение
		СтруктураСтроки.Вставить("ВладелецХарактеристики",     "");
		СтруктураСтроки.Вставить("ВладелецЕдиницыИзмерения",   "");
		СтруктураСтроки.Вставить("ОстатокНаСкладе",            "");		
	КонецПопытки;
...
Функция ПечатьПереоценка(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ЕстьКод = ПравоПользователя("ВыводитьКодВПечатныхФормах",Документ);
		...
		Если ЕстьКод Тогда
			...
			ОбластьШапкаТаблицы.Параметры.ИмяКолонкиКод = КолонкаКода;
		КонецЕсли;
		...
		Для каждого СтрокаТЧ Из ВыборкаТабличнойЧасти Цикл
			...
			ТоварНаименование = ТоварНаименование + ", " + УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТЧ.ХарактеристикаНоменклатуры);
			...
			Если ЭтоРозничный Тогда
				...
				ПроцентНаценки = ?(СтрокаТЧ.ЦенаРозничная = 0,0,100*(СтрокаТЧ.ЦенаРозничнаяПриход-СтрокаТЧ.ЦенаРозничная)/СтрокаТЧ.ЦенаРозничная);
...
Функция ПечатьПересортицаТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		Если Не ЭтоРозничный И Не ПравоПользователя("ПоказыватьСебестоимостьПроведения",Документ) Тогда
			ОбластьТовар = Макет.Область("ТоварРасход");
		...
		Если ПоказыватьСебестоимость И НЕ ЭтоРозничный Тогда
			...
			Если Документ.ВалютаДокумента = ВалютаРегл Тогда
				ИмяРесурсаСуммы = "Сумма"; ИмяРесурсаСуммыБезНДС = "СуммаБезНДС"; ТребуетсяПересчет = Ложь;
			ИначеЕсли Документ.ВалютаДокумента = ВалютаУпр Тогда
				ИмяРесурсаСуммы = "СуммаУпр"; ИмяРесурсаСуммыБезНДС = "СуммаБезНДСУпр"; ТребуетсяПересчет = Ложь;
			Иначе
				ИмяРесурсаСуммы = "СуммаУпр"; ИмяРесурсаСуммыБезНДС = "СуммаБезНДСУпр"; ТребуетсяПересчет = Истина;
			КонецЕсли;
		...
		// Вывод заголовка документа.
		ОбластьЗаголовок.Параметры.Заполнить(Документ);
		ТекстЗаголовка = УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ, НСтр("ru = 'Пересортица товаров'"));
		ОбластьЗаголовок.Параметры.ТекстЗаголовка = ТекстЗаголовка;
		Если ЗначениеЗаполнено(Документ.ПодразделениеКомпании) И ЗначениеЗаполнено(Документ.ПодразделениеКомпании.КПП) Тогда
			СтрокаПредставления = УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(Документ.Организация,,,,ДатаДокумента,ДополнительныеПараметры);
			ОбластьЗаголовок.Параметры.ПредставлениеОрганизации	= СтрЗаменить(СтрокаПредставления, ОбщегоНазначения.ПолучитьКППНаДату(Документ.Организация, ДатаДокумента), ОбщегоНазначения.ПолучитьКППНаДату(Документ.ПодразделениеКомпании, ДатаДокумента));
		...
		// Готовим шапку
		НомерСтраницы = 2; НомерСтраницыПред = 2;
		СтруктураИтоговПоСтранице = Новый Структура("ВалютаДокумента,СуммаБезНДС,СуммаРозничнаяРасход,СуммаРозничнаяПриход",Документ.ВалютаДокумента,0,0);
		ОбластьШапкаТаблицы.Параметры.ТекстЗаголовка = ТекстЗаголовка;
		ОбластьШапкаТаблицы.Параметры.НомерСтраницы  = "Страница: " + НомерСтраницы;
		...
		Для Каждого СтрокаТЧ Из ВыборкаТабличнойЧасти Цикл
			...
			// Характеристику выводим в строке с наименованием.
			ТоварНаименованиеРасход = ТоварНаименованиеРасход + ", " +  УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТЧ.ХарактеристикаНоменклатуры);
			ТоварНаименованиеПриход = ТоварНаименованиеПриход + ", " +  УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТЧ.ХарактеристикаНоменклатурыПриход);		
			// Заполняем структуру строки.
			СтруктураСтроки.Вставить("НомерСтроки",СтрокаТЧ.НомерСтроки);
			...
			Иначе
				...
				Если ТребуетсяПересчет Тогда
					СуммаСписания = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(СуммаСписания, ВалютаУпр, КурсВалютыУпр, Документ.ВалютаДокумента, Документ.КурсДокумента);
...
Функция ПечатьИНВ19 (МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ОбластьЗаголовок.Параметры.ОрганизацияПредставление		= УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(ОбластьЗаголовок.Параметры.Организация,СтруктураПредставления,,,ДатаДокумента,ДополнительныеПараметры);
		...
		ОбластьЗаголовок.Параметры.МОЛ				= УправлениеСвойствамиПлатформа.ПолучитьЗначениеСвойства("МОЛ", ЗначенияСвойствОбъекта,Документ.СкладКомпании.МОЛ);
		...
		Для каждого СтрокаТовар Из ВыборкаСтрокТовары Цикл
			...
			ТребуетсяПересчетИзУпр = (Документ.ВалютаДокумента = Константы.ВалютаУправленческогоУчетаКомпании.Получить());
			ТребуетсяПересчетИзРегл = (Документ.ВалютаДокумента = Константы.ВалютаРегламентированногоУчетаОрганизаций.Получить());
			...
			Иначе
				...
				Если ТребуетсяПересчетИзРегл Тогда
					НедостачаСумма = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(НедостачаСумма, Константы.ВалютаРегламентированногоУчетаОрганизаций.Получить(), Документ.Дата, Документ.ВалютаДокумента, Документ.КурсДокумента);
					ИзлишекСумма = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(ИзлишекСумма, Константы.ВалютаРегламентированногоУчетаОрганизаций.Получить(), Документ.Дата, Документ.ВалютаДокумента, Документ.КурсДокумента);
				ИначеЕсли ТребуетсяПересчетИзУпр Тогда
					НедостачаСумма = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(НедостачаСумма, Константы.ВалютаУправленческогоУчетаКомпании.Получить(), Документ.Дата, Документ.ВалютаДокумента, Документ.КурсДокумента);
					ИзлишекСумма = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(ИзлишекСумма, Константы.ВалютаУправленческогоУчетаКомпании.Получить(), Документ.Дата, Документ.ВалютаДокумента, Документ.КурсДокумента);
				КонецЕсли;
			КонецЕсли;
			...
			ОбластьСтрокаТаблицы.Параметры.ТоварНаименование			= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТовар.Номенклатура) + ?(Характеристика = "", "",", " + Характеристика);
			ОбластьСтрокаТаблицы.Параметры.ТоварКод					 	= УправлениеПечатьюПлатформа.ПолучитьЗначениеКолонкиКода(СтрокаТовар.Номенклатура);
			...
			ОбластьСтрокаТаблицы.Параметры.СписаниеНедостачКолонка1Колво = Формат((НедостачаКолво - ПересортКолво),ФорматВыводаКоличества);
			ОбластьСтрокаТаблицы.Параметры.СписаниеНедостачКолонка1Сумма = Формат((НедостачаСумма - ПересортСумма),ФорматВыводаСуммы);
			...
			НомерСтраницы = Обработки.ПечатьИНВ.ВывестиГоризонтальнуюОбластьИНВ19(ТабличныйДокумент, ОбластьСтрокаТаблицы, Области, Документ, НомерСтраницы, СтруктураИтогов, мсвДопОбластиПодвала);
			...
			ОбластьСтрокаТаблицы.Параметры.ТоварНаименование			= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТовар.НоменклатураПриход) + ?(Характеристика = "", "",", " + Характеристика);
			ОбластьСтрокаТаблицы.Параметры.ТоварКод					 	= УправлениеПечатьюПлатформа.ПолучитьЗначениеКолонкиКода(СтрокаТовар.НоменклатураПриход);
			ОбластьСтрокаТаблицы.Параметры.ЕдиницаИзмеренияНаименование = СтрокаТовар.НоменклатураПриход.БазоваяЕдиницаИзмерения;
			ОбластьСтрокаТаблицы.Параметры.ЕдиницаИзмеренияКодПоОКЕИ    = СтрокаТовар.НоменклатураПриход.БазоваяЕдиницаИзмерения.Код;
		...
		Если НомерСтраницы > 2 Тогда
			Обработки.ПечатьИНВ.ВывестиИтогиПоСтраницеИНВ19(ТабличныйДокумент,ОбластьИтогиПоСтранице,СтруктураИтогов,Документ);
		...
		ОбластьИтогоТаблицы.Параметры.ИтогоПересортНедостачиКолво   	 = Формат(ИтогоПересортНедостачиКолво,ФорматВыводаКоличества);
		...
		ОбластьПодвал.Параметры.МОЛ					= УправлениеСвойствамиПлатформа.ПолучитьЗначениеСвойства("МОЛ", ЗначенияСвойствОбъекта,Документ.СкладКомпании.МОЛ);
...
Функция ТоварыЗаполнитьСкладскимиОстатками(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	...
	Если ТипЗнч(Результат)=Тип("ТаблицаЗначений") Тогда
		...
		Для Каждого СтрокаТЧ Из Результат Цикл
			...
			// если строка была удалена (для номенклатуры типа набор)
			Если НоваяСтрока.НомерСтроки <> 0 Тогда
				НоваяСтрока.НоменклатураПриход               = СтрокаТЧ.Номенклатура;
				ТоварыНоменклатураПриходПриИзменении(Объект, НоваяСтрока, ПараметрыДействия);
				НоваяСтрока.КоличествоПриход                 = СтрокаТЧ.Количество;
				ТоварыКоличествоПриходПриИзменении(Объект, НоваяСтрока, ПараметрыДействия);
				НоваяСтрока.ХарактеристикаНоменклатурыПриход = СтрокаТЧ.ХарактеристикаНоменклатуры;
				ТоварыХарактеристикаНоменклатурыПриходПриИзменении(Объект, НоваяСтрока, ПараметрыДействия);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
