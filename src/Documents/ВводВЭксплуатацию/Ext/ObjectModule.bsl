...
Функция ОбработкаЗаполнения_ПоступлениеТоваров(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
	...
	Для каждого ТекСтрока Из Товары Цикл
		...
		Если НЕ ТекСтрока.Актив.Пустая() И КоличествоОстаток > 0 Тогда
			...
			Если НЕ (ТекСтрока.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ ТекСтрока.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ ТекСтрока.Актив.ВидПрочегоАктива = Перечисления.ВидыПрочихАктивов.Инструменты) Тогда
...
//  СтандартнаяОбработка - Булево       - В данный параметр передается признак выполнения системной обработки события.
//
Функция ОбработкаЗаполнения_ПрочиеАктивы(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
...
Функция ПровестиПоПартиям(Режим,ДокументСсылка) Экспорт
	...
	Иначе
		...
		ИтогБалансоваяСтоимостьУпр = ?(ВалютаДокумента = ВалютаУпр, ИтогоБалансоваяСтоимость,
				РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(ИтогоБалансоваяСтоимость, ВалютаДокумента, КурсДокумента, ВалютаУпр,
				?(НЕ ЗначениеЗаполнено(КурсВалютыУпр), МоментВремени(), КурсВалютыУпр)));
