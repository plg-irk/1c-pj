// Модуль формы списка документа "Задание на мониторинг"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивДнейНедели = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("Понедельник,Вторник,Среда,Четверг,Пятница,Суббота,Воскресенье");
	Для Каждого ДеньНедели Из МассивДнейНедели Цикл
		...
		НовыйОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ДеньНедели);
		НовыйОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		НовыйОтбор.ПравоеЗначение = Истина;
		НовыйОтбор.Использование = Ложь;
	КонецЦикла;
	
	МассивНедели = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("КаждаяНеделя,ЧетнаяНеделя,НеЧетнаяНеделя,ПерваяНеделя,ВтораяНеделя,ТретьяНеделя,ЧетвертаяНеделя");
	Для Каждого Неделя Из МассивНедели Цикл
		НовыйОтбор = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		НовыйОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(Неделя);
		НовыйОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		НовыйОтбор.ПравоеЗначение = Истина;
		НовыйОтбор.Использование = Ложь;
	КонецЦикла;
...
Процедура ДниНеделиПриИзменении(Элемент)
	МассивДнейНедели = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("Понедельник,Вторник,Среда,Четверг,Пятница,Суббота,Воскресенье");
	Для Каждого ДеньНедели Из МассивДнейНедели Цикл
		ЭтотОбъект[ДеньНедели] = (ДеньНедели=Элемент.Имя) И ЭтотОбъект[ДеньНедели];
	КонецЦикла;
	
	//установим отбор
	Для Каждого ДеньНедели Из МассивДнейНедели Цикл
		ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ДеньНедели);
		Для Каждого ТекущийОтбор Из Список.Отбор.Элементы Цикл
			Если ТекущийОтбор.ЛевоеЗначение=ЛевоеЗначение Тогда
				ТекущийОтбор.Использование = ЭтотОбъект[ДеньНедели];
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура НеделяПриИзменении(Элемент)
	...
	МассивНедели = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("КаждаяНеделя,ЧетнаяНеделя,НеЧетнаяНеделя,ПерваяНеделя,ВтораяНеделя,ТретьяНеделя,ЧетвертаяНеделя");
	Для Каждого Неделя Из МассивНедели Цикл
		ЭтотОбъект[Неделя] = (Неделя=Элемент.Имя) И ЭтотОбъект[Неделя];
	КонецЦикла;
	

	//установим отбор
	Для Каждого Неделя Из МассивНедели Цикл
		ЛевоеЗначение = Новый ПолеКомпоновкиДанных(Неделя);
		Для Каждого ТекущийОтбор Из Список.Отбор.Элементы Цикл
			Если ТекущийОтбор.ЛевоеЗначение=ЛевоеЗначение Тогда
				ТекущийОтбор.Использование = ЭтотОбъект[Неделя];
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
