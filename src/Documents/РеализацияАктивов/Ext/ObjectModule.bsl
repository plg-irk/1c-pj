...
//  Возвращает результат запроса.
//
Функция ПолучитьРезультатЗапросаПоПартиям()
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
//  СтандартнаяОбработка - Булево       - В данный параметр передается признак выполнения системной обработки события.
//
Функция ОбработкаЗаполнения_ВводОстатковПрочихАктивов(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Иначе							  
		ИтогДоходПоРеализованнымАктивам = РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(РезультатЗапросаПоАктивам.Итог("Сумма"), ВалютаДокумента, КурсДокумента, Константы.ВалютаУправленческогоУчетаКомпании.Получить(), ?(НЕ ЗначениеЗаполнено(КурсВалютыУпр), Дата, КурсВалютыУпр));
	КонецЕсли; 
	...
	Если НЕ Отказ Тогда
		...
		Если РезультатЗапросаПоТоварам.Количество()>0 Тогда
			...
			РезультатЗапросаПоПартиям = ПолучитьРезультатЗапросаПоПартиям();
