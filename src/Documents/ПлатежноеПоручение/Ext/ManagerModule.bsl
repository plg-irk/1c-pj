...
//
Функция СтатусыОтправителя(Объект) Экспорт
	...
	Список.Добавить("03", НСтр("ru = '03 - сборщик налогов и сборов'"));
	...
	Если Объект.Дата >= '20050101' Тогда
		...
		Список.Добавить("13", НСтр("ru = '13 - налогоплательщик (плательщик сборов) - иное физическое лицо - клиент банка (владелец счета)'"));
		...
		Список.Добавить("15", НСтр("ru = '15 - кредитная организация, оформившая расчетный документ на общую сумму на перечисление налогов, сборов и иных платежей в бюджетную систему Российской Федерации, уплачиваемых физическими лицами без открытия банковского счета'"));
...
//
Функция ОснованияПлатежа(Объект) Экспорт
...
//  СписокЗначений - в котором создаются элементы со статусами отправителя.
//
Функция ПоказателиТипа(Объект) Экспорт
...
Процедура СформироватьНазначениеПлатежа(Объект) Экспорт
	...
	Если ПустаяСтрока(ТекстНазначение) ИЛИ ТекстНазначение = НСтр("ru = 'Оплата по:'") + Символы.НПП Тогда
		Если ЗначениеЗаполнено(Объект.ДоговорВзаиморасчетов) Тогда
			ТекстНазначение = НСтр("ru = 'Оплата по договору'") + Символы.НПП + Строка(Объект.ДоговорВзаиморасчетов);
		КонецЕсли;
	КонецЕсли; 
	
	ТекстСумма= НСтр("ru = 'Сумма'") + Символы.НПП + Формат(Объект.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ=")
					+ Символы.ПС;
...
Процедура ДоговорВзаиморасчетовПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.СчетОрганизации) И Объект.ДоговорВзаиморасчетов.ВалютаВзаиморасчетов <> Объект.СчетОрганизации.ВалютаДенежныхСредств Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо выбрать другой договор взаиморасчетов. Валюта договора должна совпадать с валютой счета организации'");
...
Функция ПечатьПлатежноеПоручение(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		Если Документ.Организация.Пустая() Тогда
			ВывестиСообщение(НСтр("ru = 'Не указана организация.'"));
			Возврат Неопределено;
		КонецЕсли;
	
		Если Документ.Контрагент.Пустая() Тогда
		...
		Если Прав(НомерПечать,3)="000" Тогда
			ВывестиСообщение(НСтр("ru = 'Номер платежного поручения не может оканчиваться на ""000"".'"));
			Возврат Неопределено;
		КонецЕсли;
		...
		ОбластьЗаголовок.Параметры.ПлательщикИНН		= "ИНН " + ?(ПустаяСтрока(Документ.ИННПлательщика), Документ.Организация.ИНН, СокрЛП(Документ.ИННПлательщика));
		ОбластьЗаголовок.Параметры.ПлательщикКПП		= "КПП " + ?(ПустаяСтрока(Документ.КПППлательщика),?(Документ.ПеречислениеНалога,"0",""),СокрЛП(Документ.КПППлательщика));
		
		Если Документ.СчетОрганизации.БанкДляРасчетов.Пустая() Тогда
			СтрКорреспондент = "";
		Иначе	
			СтрКорреспондент = " р/с " + Документ.СчетОрганизации.НомерСчета+ " в " + Документ.СчетОрганизации.Банк + " " + Документ.СчетОрганизации.Банк.Город;	
		КонецЕсли;
		
		НаименованиеПолноеОрганизации = СокрЛП(ОбщегоНазначения.ПолучитьНаименованияНаДату(Документ.Организация, Документ.Дата, "НаименованиеДляПечатныхФорм"));
		НаименованиеПолноеКонтрагента = СокрЛП(ОбщегоНазначения.ПолучитьНаименованияНаДату(Документ.Контрагент, Документ.Дата, "НаименованиеДляПечатныхФорм"));
		...
		ОбластьЗаголовок.Параметры.ПолучательИНН			= "ИНН " + ?(ПустаяСтрока(Документ.ИННПолучателя), Документ.Контрагент.ИНН, СокрЛП(Документ.ИННПолучателя));
		ОбластьЗаголовок.Параметры.ПолучательКПП			= "КПП " + ?(ПустаяСтрока(Документ.КПППолучателя),?(Документ.ПеречислениеНалога,"0",""),СокрЛП(Документ.КПППолучателя));
		
		ОбластьЗаголовок.Параметры.Получатель			= ?(ПустаяСтрока(Документ.ТекстПолучателя),НаименованиеПолноеКонтрагента,СокрЛП(Документ.ТекстПолучателя));
		...
		Если Документ.ПеречислениеНалога Тогда
			...
			ОбластьЗаголовок.Параметры.СтатусСоставителя		= ?(ПустаяСтрока(Документ.СтатусСоставителя),"0",СокрЛП(Документ.СтатусСоставителя));
			...
			ОбластьЗаголовок.Параметры.ПоказательОснования	= ?(ПустаяСтрока(Документ.ПоказательОснования),"0",СокрЛП(Документ.ПоказательОснования));
			ОбластьЗаголовок.Параметры.ПоказательНомера		= ?(ПустаяСтрока(Документ.ПоказательНомера),"0",СокрЛП(Документ.ПоказательНомера));
			ОбластьЗаголовок.Параметры.ПоказательДаты		= ?(Документ.ПоказательДаты = '00010101000000',"0",Формат(Документ.ПоказательДаты,"ДЛФ=D"));
			ОбластьЗаголовок.Параметры.ПоказательТипа		= ?(ПустаяСтрока(Документ.ПоказательТипа),"0",СокрЛП(Документ.ПоказательТипа));
...
Функция Проверить(Документ)
	...
	Если ЗначениеЗаполнено(Документ.КодУИН) И СтрДлина(Документ.КодУИН) <> 20 И Документ.КодУИН <> "0" Тогда
	...
	Для Ном = 0 По Ошибка.Количество()-1 Цикл
		ВывестиСообщение(Ошибка.Получить(Ном));
	КонецЦикла;
