...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли СтрНайти("МС,КВ,ПЛ,ГД,Д1,Д2,Д3", ПериодичностьНалога) > 0 Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ДатаПоказателя");
...
Процедура Выбрать(Команда)
	...
	Если ПроверитьЗаполнение() Тогда
		...
		ОповеститьОВыборе(ПоказательПериода);
...
Процедура УправлениеЭлементамиФормы()
	...
	Если Не ПустаяСтрока(ПериодичностьНалога)
	   И СтрНайти("МС,КВ,ПЛ,ГД,Д1,Д2,Д3", ПериодичностьНалога) > 0 Тогда
		...
		ИначеЕсли (ПериодичностьНалога = "Д1") ИЛИ (ПериодичностьНалога = "Д2") ИЛИ (ПериодичностьНалога = "Д3") Тогда
			Если НомерПериода < 1 ИЛИ НомерПериода > 12 Тогда
				НомерПериода = 1;
			КонецЕсли;
			Элементы.НомерПериода.МинимальноеЗначение = 1;
