...
//
&НаСервере
Функция СформироватьСписокВыбораПлатежа(Объект)
	
	МассивСписковВидовПлатежей = Новый Массив;
	
	МассивСписковВидовПлатежей.Добавить(Документы.ПлатежноеПоручение.СтатусыОтправителя(Объект));
	МассивСписковВидовПлатежей.Добавить(Документы.ПлатежноеПоручение.ОснованияПлатежа(Объект));
	МассивСписковВидовПлатежей.Добавить(Документы.ПлатежноеПоручение.ПоказателиТипа(Объект));
...
Процедура Выгрузить(Команда)
	...
	Объект = ЭтотОбъект.ВладелецФормы.Объект;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ТипДокумента") ИЛИ НЕ Параметры.ТипДокумента = "ПлатежноеПоручение" Тогда
		ВывестиСообщение(НСтр("ru = 'Эта форма предназначена для открытия только из формы платежного поручения.'"));
		Отказ=Истина;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	МассивСписковВыбора = СформироватьСписокВыбораПлатежа(ЭтотОбъект.ВладелецФормы.Объект.Ссылка);
	...
	Если ЭтотОбъект.ВладелецФормы.Объект.Дата >= '20050101' Тогда
	...
	П101 = ЭтотОбъект.ВладелецФормы.Объект.СтатусСоставителя;
	П104 = ЭтотОбъект.ВладелецФормы.Объект.КодБК;
	П105 = ЭтотОбъект.ВладелецФормы.Объект.КодОКТМО;
	П106 = ЭтотОбъект.ВладелецФормы.Объект.ПоказательОснования;
	П107 = ЭтотОбъект.ВладелецФормы.Объект.ПоказательПериода;
	П108 = ЭтотОбъект.ВладелецФормы.Объект.ПоказательНомера;
	П109 = Строка(Формат(ЭтотОбъект.ВладелецФормы.Объект.ПоказательДаты,"ДЛФ=D"));
	П110 = ЭтотОбъект.ВладелецФормы.Объект.ПоказательТипа;
