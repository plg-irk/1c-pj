...
// Обработчик события возникающего при изменении данных реквизита "Подтверждение" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
...
Процедура ТоварыНоменклатураПоставщикаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.КорректировкаПоступления.ТоварыНоменклатураПоставщикаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Подтверждение".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Мальцев А.Ю. +++
	Если Объект.итЗаявкаНаВозврат Тогда
		Элементы.ГруппаАкцепт.Видимость = Истина;
		ПравоИзменения = ИТ_СпециальныйМодульСервер.ПроверитьПравоНаСервере("итРазрешитьРедактированикКорректировокПоступленийЗаявкиНаВозврат");
		Если ПравоИзменения = Ложь Тогда
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ФормаАкцептовать.Видимость = Истина;

		Если  Объект.итСтатусАкцептования = Перечисления.ИТ_СтатусыАкцептования.ОжидаетАкцепта Тогда
			Элементы.ФормаАкцептовать.Доступность = Истина;
		Иначе
			Элементы.ФормаАкцептовать.Доступность = Ложь;
		КонецЕсли;		
	КонецЕсли;
	//Мальцев А.Ю. ---
...
&НаКлиенте
Процедура Акцевтовать(Команда)
	Объект.итСтатусАкцептования = ПредопределенноеЗначение("Перечисление.ИТ_СтатусыАкцептования.Акцептован");
	Элементы.ФормаАкцептовать.Доступность = Ложь;
