...
&НаСервере
Процедура ПерезаполнитьТаблицуТоваровПоРецептуре()
	
	// Определим структуру параметров обработки текущего события
	ПараметрыДействия = Новый Структура;
	
	КоличествоРецептура = Объект.Рецептура.Количество;
	Если КоличествоРецептура = 0 Тогда
		КоличествоРецептура = 1;
	КонецЕсли;
	
	Объект.Товары.Очистить();
	Для Каждого СтрокаРецептуры Из Объект.Рецептура.Товары Цикл
		СтрокаТоваров = Объект.Товары.Добавить();
		СтрокаТоваров.Номенклатура             = СтрокаРецептуры.Номенклатура;
		Документы.Разделка.ТоварыНоменклатураПриИзменении(Объект, СтрокаТоваров, ПараметрыДействия);
		
		СтрокаТоваров.ЕдиницаИзмерения         = СтрокаРецептуры.ЕдиницаИзмерения;
		СтрокаТоваров.КоэффициентСебестоимости = СтрокаРецептуры.КоэффициентСебестоимости;
		Документы.Разделка.ТоварыЕдиницаИзмеренияПриИзменении(Объект, СтрокаТоваров, ПараметрыДействия);
		
		СтрокаТоваров.КоличествоНорма          = СтрокаРецептуры.КоличествоНетто*Объект.Количество/КоличествоРецептура;
		СтрокаТоваров.Количество               = СтрокаРецептуры.Количество*Объект.Количество/КоличествоРецептура;
		Документы.Разделка.ТоварыКоличествоПриИзменении(Объект, СтрокаТоваров, ПараметрыДействия);
	КонецЦикла;
	
КонецПроцедуры // ОбработкаРезультатаОтветаНаВопросТребуетсяЗаполнитьНаОснованииРецептуры()

// Служебная процедура. Используется для перехвата ответа пользователя на вопрос о перезаполнении товарного состава рецептуры.
//
// Параметры:
...
//
&НаКлиенте
Процедура ОбработкаРезультатаОтветаНаВопросТребуетсяЗаполнитьНаОснованииРецептуры(КодОтветаДа, ПараметрыВопросов) Экспорт
	
	// В случае если пользователь хочет полностью перезаполнить документ, то другие изменения будут не актуальны.
	Если КодОтветаДа Тогда
		ПерезаполнитьТаблицуТоваровПоРецептуре();
	КонецЕсли;
...
Процедура УправлениеДиалогомНаСервере()
	...
	РозничныйСклад = Объект.СкладКомпании.Розничный;
	
	Элементы.СуммаРозничная.Доступность = РозничныйСклад;
	Элементы.ЦенаРозничная.Доступность  = РозничныйСклад;
	Элементы.ТоварыСуммаРозничная.Видимость = РозничныйСклад;
	Элементы.ТоварыЦенаРозничная.Видимость  = РозничныйСклад;
...
Процедура Подключаемый_ОбработкаРезультатаОповещения(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если ТипЗнч(РезультатОповещения) = Тип("Структура") И РезультатОповещения.Свойство("ТребуетсяЗаполнитьНаОснованииРецептуры") Тогда
		РезультатОповещения.ТребуетсяЗаполнитьНаОснованииРецептуры = Ложь;
	КонецЕсли;
...
Процедура ОбработкаРезультатаВыполненияДействия(ПараметрыДействия)
	...
	Если ПолучитьЗначениеПараметраСтруктуры(ПараметрыДействия, "ТребуетсяЗаполнитьНаОснованииРецептуры", Ложь) Тогда
		
		ПараметрыДействия.ТребуетсяЗаполнитьНаОснованииРецептуры = Ложь;
		...
		Иначе
			...
			ТребуетсяЗаполнитьНаОснованииРецептуры = 
			Новый ОписаниеОповещения("ОбработкаРезультатаОтветаНаВопросТребуетсяЗаполнитьНаОснованииРецептуры", 
			ЭтотОбъект, ПараметрыВопросов);
			ПоследовательныеОперации.ДобавитьВопросДаНет(КоллекцияОпераций, "ТребуетсяЗаполнитьНаОснованииРецептуры", 
			НСтр("ru='Изменилась рецептура.'")+Символы.ПС+НСтр("ru = 'Выполнить перезаполнение таблицы товаров?'"),,,
			ТребуетсяЗаполнитьНаОснованииРецептуры);
		КонецЕсли;
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.Разделка.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура НоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.НоменклатураПриИзменении(Объект, ПараметрыДействия);
	
	ЗаполнитьСлужебныеРеквизиты();
	
	// Проверим наличие заметок по выбранному объекту.
	ЗаметкиПользователяПлатформа.ПроверитьЗаметкиПоПредмету(Объект.Номенклатура, ПараметрыДействия);
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	предКоличество = Объект.Количество;
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Рецептура" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура РецептураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.РецептураПриИзменении(Объект, ПараметрыДействия);
	
	// Обновляем отображение элементов формы
	УправлениеДиалогомНаСервере();
	
КонецПроцедуры // РецептураПриИзмененииНаСервере()
...
//
&НаКлиенте
Процедура РецептураПриИзменении(Элемент)
	
	// Определим структуру параметров обработки текущего события
	ПараметрыДействия = Новый Структура;
	
	// Обработаем событие в контексте сервера
	РецептураПриИзмененииНаСервере(ПараметрыДействия);
...
Процедура ЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.Разделка.ЕдиницаИзмеренияПриИзменении(Объект, ПараметрыДействия);
...
//
&НаСервере
Процедура КоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	Документы.Разделка.КоличествоПриИзменении(Объект, ПараметрыДействия);
	
КонецПроцедуры // КоличествоПриИзмененииНаСервере()
...
Процедура КоличествоПриИзменении(Элемент)
	...
	КоэффициентИзменения = ?(предКоличество=0, Объект.Количество, Объект.Количество/предКоличество);
	
	ПараметрыДействия = Новый Структура;
	ПараметрыДействия.Вставить("КоэффициентИзменения", КоэффициентИзменения);
	
	КоличествоПриИзмененииНаСервере(ПараметрыДействия);
	
	предКоличество = Объект.Количество;
	
КонецПроцедуры // КоличествоПриИзменении()

// Обработчик события возникающего на клиенте при изменении данных реквизита "Цена розничная" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура ЦенаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ЦенаРозничнаяПриИзменении(Объект, ПараметрыДействия);
	
	// Обновляем отображение элементов формы
	УправлениеДиалогомНаСервере();
	
КонецПроцедуры // ЦенаРозничнаяПриИзмененииНаСервере()
...
Процедура ЦенаРозничнаяПриИзменении(Элемент)
	...
	// Обработаем событие в контексте сервера
	ЦенаРозничнаяПриИзмененииНаСервере(ПараметрыДействия);
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Сумма розничная" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура СуммаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.СуммаРозничнаяПриИзменении(Объект, ПараметрыДействия);
	
	// Обновляем отображение элементов формы
	УправлениеДиалогомНаСервере();
	
КонецПроцедуры // СуммаРозничнаяПриИзмененииНаСервере()
...
//
&НаКлиенте
Процедура СуммаРозничнаяПриИзменении(Элемент)
	
	// Определим структуру параметров обработки текущего события
	ПараметрыДействия = Новый Структура;
	
	// Обработаем событие в контексте сервера
	СуммаРозничнаяПриИзмененииНаСервере(ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
// Обработчик события возникающего при изменении данных реквизита "Количество норма" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура ТоварыКоличествоНормаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыКоличествоНормаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры // ТоварыКоличествоНормаНормаПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "Количество норма".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
&НаКлиенте
Процедура ТоварыКоличествоНормаПриИзменении(Элемент)
	
	// Обработаем событие в контексте сервера
	ТоварыКоличествоНормаПриИзмененииНаСервере();
...
Процедура ТоварыЦенаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыЦенаРозничнаяПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаРозничнаяПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.Разделка.ТоварыСуммаРозничнаяПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Номенклатура) И (НЕ ЗначениеЗаполнено(Объект.ЕдиницаИзмерения)) Тогда
