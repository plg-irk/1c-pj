...
Процедура ПересчитатьСтроку(Объект, Строка=Неопределено) Экспорт
	...
	ГраницаЦен = Новый Граница(?(ЗначениеЗаполнено(Объект.Ссылка),Объект.Ссылка.МоментВремени(),КонецДня(Объект.Дата)),ВидГраницы.Исключая);
	Запрос.УстановитьПараметр("МоментВремени",				ГраницаЦен);
	...
	Если Строка = Неопределено Тогда
		...
		Для Каждого ТекСтрокаТЧ Из Объект.Товары Цикл
			...
			ТекСтрокаТЧ.Количество	= ?(МассивНайденныхСтрок.Количество() = 0,0,МассивНайденныхСтрок[0].Количество) / ТекСтрокаТЧ.Коэффициент;
	...
	Иначе 
		...
		Если Строка.Коэффициент=0 Тогда
			Коэффициент = 1;
			ВывестиСообщение(НСтр("ru = 'Для номенклатура'") + Символы.НПП + СокрЛП(Строка.Номенклатура) 
					+ НСтр("ru = 'не заполнена единица измерения.'"),
		...
		МассивНайденныхСтрок	= ТЗ.НайтиСтроки(СтруктураОтбора);
		Строка.Количество	= ?(МассивНайденныхСтрок.Количество() = 0,0,МассивНайденныхСтрок[0].Количество) / Коэффициент;
		Строка.ЦенаСтарая	= ?(МассивНайденныхСтрок.Количество() = 0,0,МассивНайденныхСтрок[0].ЦенаСтарая);
		Строка.ЦенаСтарая	= РаботаСКурсамиВалютПлатформа.ПересчетПоВалюте(Строка.ЦенаСтарая,ВалютаИз,КурсИз,Объект.ВалютаДокумента,Объект.КурсДокумента) * Коэффициент;
...
Функция ПолучитьОбязательныеРеквизиты(Объект) Экспорт
	...
	Если Объект.ХозОперация <> Справочники.ХозОперации.ПереоценкаТоваровВРознице
		И Объект.ХозОперация <> Справочники.ХозОперации.ПереоценкаТоваровОтданныхНаКомиссию Тогда
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьСкладскимиОстаткамиПоКомиссии",, "Товары", НСтр("ru = 'Заполнить остатками по складу по комиссии'"), ПараметрыКоманды);
	
	ПараметрыКоманды = Новый Структура();
	ПоследовательныеОперации.ДобавитьВыборСсылки(ПараметрыКоманды, "СкладКомпании", "Справочник.СкладыКомпании.ФормаСписка",,, ИСТИНА, НСтр("ru = 'Необходимо выбрать склад компании.'"));
	
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьСкладскимиОстаткамиПоРознице",, "Товары", НСтр("ru = 'Заполнить остатками по складу по рознице'"), ПараметрыКоманды);
	
	ПараметрыКоманды = Новый Структура();
	ПоследовательныеОперации.ДобавитьВыборСсылки(ПараметрыКоманды, "СкладКомпании", "Справочник.СкладыКомпании.ФормаСписка",,, ИСТИНА, НСтр("ru = 'Необходимо выбрать склад компании.'"));
	
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыЗаполнитьНеПереоцененными",, "Товары", НСтр("ru = 'Заполнить не переоцененными товарами'"), ПараметрыКоманды);
...
//
Функция ТоварыЗаполнитьСкладскимиОстаткамиПоКомиссии(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	...
	МоментВремени=?(Объект.Ссылка.Пустая(),Новый МоментВремени(КонецДня(Объект.Дата)), Новый МоментВремени(Объект.Дата));
...
//
Функция ТоварыЗаполнитьСкладскимиОстаткамиПоРознице(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
...
//
Функция ТоварыЗаполнитьНеПереоцененными(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	...
	Если Объект.СкладКомпании.Розничный Тогда
		...
		Для Каждого СтрокаТЗ Из ВременнаяТЗ Цикл
			...
			Если (СтрокаТЗ.Цена<>0) И (НЕ СтрокаТЗ.Цена = СтрокаТЗ.ЦенаСтарая) Тогда
		...
		КонецЦикла;
	Иначе
		ВывестиСообщение(НСтр("ru='Не указан розничный склад.'"), Объект);
	КонецЕсли;
