...
Функция ОбработкаЗаполнения_ИзменениеЦен(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
	...
	ИтоговыеДанныеЗаполнения = ДанныеЗаполнения;
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("Ссылка") Тогда
		ИтоговыеДанныеЗаполнения = ДанныеЗаполнения.Ссылка;
	КонецЕсли;
	
	// Вызываем общий обработчик заполнения
	Если НЕ ОбработкаСобытийДокументаСервер.ОбработкаЗаполненияНаОсновании(ЭтотОбъект, ИтоговыеДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Тогда
		Возврат ИСТИНА;
	КонецЕсли;
	...
	Иначе
		ТипЦенЗаполнения = ?(ИтоговыеДанныеЗаполнения.ПараметрыРасчета.Количество() > 0, ИтоговыеДанныеЗаполнения.ПараметрыРасчета[0].ТипЦен, Неопределено);
	КонецЕсли;
	Товары.Очистить();
	ВыполнитьОбщиеДействияПриЗаполнении(ИтоговыеДанныеЗаполнения, НСтр("ru='ИзменениеЦен'"), ТипЦенЗаполнения);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА)
	...
	Если НЕ (ЗаполненНаОснованииДокумента = "ПеремещениеТоваров" 
		...
		ИЛИ ЗаполненНаОснованииДокумента = "Инвентаризация"
		ИЛИ ЗаполненНаОснованииДокумента = "ИзменениеЦен")
		...
		Иначе
			ВыполнитьОбщиеДействияПриЗаполнении(ДанныеЗаполнения, ЗаполненНаОснованииДокумента);
		КонецЕсли;
	КонецЕсли;
