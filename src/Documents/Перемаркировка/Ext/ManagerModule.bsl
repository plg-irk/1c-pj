...
Функция ПолучитьОбязательныеРеквизиты(Объект) Экспорт
	...
	Если Объект.Статус <> Перечисления.СтатусыДокументовМаркировки.Новый Тогда
	...
	Если НЕ Объект.ОсобенностьУчета = Справочники.ОсобенностиУчетаНоменклатуры.Шины Тогда
		ОбязательныеРеквизиты.Добавить("Товары.СтранаПроизводства");
	КонецЕсли;
	
	Если Объект.ОсобенностьУчета = Справочники.ОсобенностиУчетаНоменклатуры.ОбувьМаркированная Тогда
...
Функция ПолучитьУникальныеРеквизиты(Объект) Экспорт
	...
	//Если Объект.ПричинаПеремаркировки = Перечисления.ПричиныПеремаркировки.ИспорченаМаркировка
	//	ИЛИ Объект.ПричинаПеремаркировки = Перечисления.ПричиныПеремаркировки.ОшибкаВОписании Тогда
	//	УникальныеТовары.Добавить("ХарактеристикаНоменклатуры");
	//КонецЕсли;
	////Если Объект.Статус <> Перечисления.СтатусыДокументовМаркировки.Новый Тогда
	//УникальныеТовары.Добавить("КодМаркировкиНовый");
	////КонецЕсли;
	//УникальныеТовары.Добавить("ХарактеристикаНоменклатурыНовая");
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ЗаполнениеДокументовСервер.ДобавитьКоманду(
		КомандыЗаполнения,
...
Функция ЗаполнитьКодыМаркировкиИзЗаказов(Объект, ПараметрыЗаполнения, ПараметрыДействия = Неопределено) Экспорт
	...
	|ГДЕ
	|	СостоянияКодовМаркировки.ДокументОснование В
	|				(ВЫБРАТЬ
...
Функция ДанныеПеремаркировкиКодов(Объект, Сообщение) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Перемаркировка.ОсобенностьУчета КАК ОсобенностьУчета
	               |ИЗ
	...
	ЭтоШина = (ШапкаДокумента.ОсобенностьУчета = Справочники.ОсобенностиУчетаНоменклатуры.Шины);
	ЭтоОбувь = (ШапкаДокумента.ОсобенностьУчета = Справочники.ОсобенностиУчетаНоменклатуры.ОбувьМаркированная);
...
Процедура ОбработатьРезультатОтвета(ДокументСсылка, ПараметрыКоманды) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Перемаркировка.ХозОперация КАК ХозОперация,
	|	Перемаркировка.ПричинаПеремаркировки КАК ПричинаПеремаркировки,
	|	Перемаркировка.Организация КАК Организация,
	|	Перемаркировка.Товары.(
	...
	ПричинаПеремаркировки = Выборка.ПричинаПеремаркировки;
	...
	Если ТаблицаМаркировок.Количество() > 0 Тогда
		Состояние = Перечисления.СостоянияКодовМаркировки.ВыведенИзОборотаПриПеремаркировке;
		РегистрыСведений.СостоянияКодовМаркировки.СостояниеКодовМаркировки(ДокументСсылка, Организация, Состояние, ТаблицаМаркировок);
		
		ОчищатьЗаписи = Ложь;
	КонецЕсли;
	
	Состояние = Перечисления.СостоянияКодовМаркировки.ВведенВОборот;
	РегистрыСведений.СостоянияКодовМаркировки.СостояниеКодовМаркировки(ДокументСсылка, Организация, Состояние, ТаблицаНовыхМаркировок,,, ОчищатьЗаписи);
