// Модуль основной формы документа "ВСД ВетИС"
...
Процедура БлокироватьЭлементыФормы()
	...
	СписокРеквизитов.Добавить("ЭтотОбъект" ,"ЭтотОбъект" , Объект.Статус = Перечисления.СтатусыВСДВЕТИС.UTILIZED);
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ "Упаковки"
...
Процедура УпаковкиПриАктивизацииСтроки(Элемент)
	...
	Элементы.Маркировка.ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторСтроки", ТекущиеДанные.ИдентификаторСтроки);
	Элементы.Маркировка.Обновить();
	
КонецПроцедуры // УпаковкиПриАктивизацииСтроки()
...
Процедура УпаковкиПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ И (НЕ ТекущиеДанные = Неопределено) Тогда
		МассивМаркировка = Объект.Маркировка.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ТекущиеДанные.ИдентификаторСтрокиПартий));
		Для Каждого ТекущаяСтрока Из МассивМаркировка Цикл
			Объект.Маркировка.Удалить(ТекущаяСтрока);
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры // УпаковкиПередУдалением()
...
Процедура УпаковкиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если (НЕ ТекущиеДанные = Неопределено) И НЕ ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтроки) Тогда
		ТекущиеДанные.ИдентификаторСтроки = Новый  УникальныйИдентификатор;
	КонецЕсли;
	УпаковкиПриАктивизацииСтроки(Неопределено);
	
КонецПроцедуры // УпаковкиПриНачалеРедактирования()

///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ "Маркировка"
...
//
&НаКлиенте
Процедура МаркировкаПередОкончаниемРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ТекущиеДанные      = Элементы.Упаковки.ТекущиеДанные;
	ТекущиеДанныеМаркировка = Объект.Маркировка.НайтиПоИдентификатору(Элементы.Маркировка.ТекущаяСтрока);
	Если (НЕ ТекущиеДанные = Неопределено) И (НЕ ТекущиеДанныеМаркировка = Неопределено) И НЕ ЗначениеЗаполнено(ТекущиеДанныеМаркировка.ИдентификаторСтроки) Тогда
		ТекущиеДанныеМаркировка.ИдентификаторСтроки = ТекущиеДанные.ИдентификаторСтроки;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ТекущиеДанныеМаркировка.ИдентификаторСтроки) Тогда
		ТекущиеДанныеМаркировка.ИдентификаторСтроки = Новый УникальныйИдентификатор;
	КонецЕсли;
	
КонецПроцедуры // МаркировкаПередОкончаниемРедактирования()
...
Процедура ПриОткрытии(Отказ)
	...
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтотОбъект, "Маркировка");
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтотОбъект, "ПунктыПерегрузки");
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтотОбъект, "Упаковки");
	...
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтотОбъект, "ПроизводственныеПартии");
...
//&НаСервере
//Процедура ПечатьВСДНаСервере()
//	
//	АдресСервера = ИнтеграцияВЕТИСВызовСервера.АдресСервера();
//	
//	Если ЭтоТестовыйСервер(АдресСервера) Тогда
//		Путь = "https://t2-mercury.vetrf.ru/pub/operatorui?_action=printVetDocumentByUuid&uuid=" + СокрЛП(Объект.Идентификатор);
//	Иначе
//		Путь = "https://mercury.vetrf.ru/pub/operatorui?_action=printVetDocumentByUuid&uuid=" + СокрЛП(Объект.Идентификатор);
//	КонецЕсли;
//	
//	ПутьФайла = КаталогВременныхФайлов() + Объект.Идентификатор + ".pdf";
//	
//	Попытка
