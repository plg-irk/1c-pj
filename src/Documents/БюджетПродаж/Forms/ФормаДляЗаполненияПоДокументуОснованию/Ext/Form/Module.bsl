...
Процедура Перечитать(Команда)
	...
	ПоказатьВопрос(ОбработчикОповещения, НСтр("ru = 'Перезаполнение таблицы приведет к потере сделанных вами изменений.
...
Процедура РаспределитьНаСервере()
	...
	ВывестиСообщение(НСтр("ru = 'Распределение завершено.'"), ЭтотОбъект);
...
Процедура НоменклатураОбработкаВыбораНаСервере(ВыбранноеЗначение)
	...
	Если НайденнаяСтрока = Неопределено Тогда
		ВывестиСообщение(НСтр("ru = 'Тип выбранной номенклатуры отсутствует в таблице.'"), ЭтотОбъект);
		Возврат;
	КонецЕсли;
