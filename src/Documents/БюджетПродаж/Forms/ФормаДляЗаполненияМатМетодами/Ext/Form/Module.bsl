...
Процедура Сформировать()
	...
	Если НеУчитыватьПериодыБезДанных Тогда
		Если МаксКоличествоПериодов < 2 Тогда
			ВывестиСообщение(НСтр("ru = 'Максимально допустимое количество периодов для анализа по продажам для текущего планируемого периода: %1. %2Построение математической модели невозможно. Увеличьте количество анализируемых периодов.'"), ЭтотОбъект,,,, СокрЛП(МаксКоличествоПериодов), Символы.ПС);
			Возврат;
		КонецЕсли;
		Если МодельПрогнозирования = Перечисления.МоделиПрогнозирования.Полиномиальный Тогда
			Если МаксКоличествоПериодов < Параметр1 * Параметр1 Тогда
				ВывестиСообщение(НСтр("ru = 'Максимально допустимое количество периодов для анализа по продажам для текущего планируемого периода: %1. %2Полином степени %3 не может быть построен. Необходимо увеличить количество анализируемых периодов.'"), ЭтотОбъект,,,, СокрЛП(МаксКоличествоПериодов), Символы.ПС, СокрЛП(Параметр1));
				Возврат;
			КонецЕсли; 
		КонецЕсли;
	ИначеЕсли (((КоличествоСезонов - 1) * 12) + (КоличествоПериодов)) > МаксКоличествоПериодов Тогда
		ВывестиСообщение(НСтр("ru = 'Максимально допустимое количество периодов для анализа по продажам для текущего планируемого периода: %1 %2.'"), ЭтотОбъект,,,, СокрЛП(МаксКоличествоПериодов), ПланированиеСервер.ВернутьСтрокуКоличествоПериодовДляАнализа(МаксКоличествоПериодов, Периодичность));
