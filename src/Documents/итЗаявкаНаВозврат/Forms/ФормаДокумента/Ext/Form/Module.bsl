...
Процедура УправлениеДиалогомНаСервере()
	...
	// покажем/скроем контрагента, договор, склад в зависимости от типа Хоз. операции
	...
	Элементы.ТоварыПартия.Видимость              = (ПравоПользователя("ВыборочноеСписаниеПартий",Объект) И Объект.ХозОперация<>Справочники.ХозОперации.СписаниеТоваровОтданныхНаКомиссию);;
	...
	//Если Объект.ДокументОснование.СкладКомпании <> Объект.СкладКомпании Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		Объект.Статус = Перечисления.итСтатусыЗаявокНаВозврат.Сформирован;
	КонецЕсли;
	//Мальцев А.Ю. ---	
	
	// Конов запрет редаутирования списаний, выгруженных в ЕРП, просьба Шмаковой П.  
	ПравоРазрешитьРедактированиеСписанийВыгруженныхВЕРП         = ПравоПользователя("РазрешитьРедактированиеСписанийВыгруженныхВЕРП", Объект);
	Если ЗначениеЗаполнено(Объект.НомерЕРП) и НЕ ПравоРазрешитьРедактированиеСписанийВыгруженныхВЕРП и Объект.Проведен ТОгда
		ЭтаФорма.ТолькоПросмотр = Истина;
		ТекстСообщения = НСтр("ru = 'Запрещено редактирвоание списаний, отправленных на завод!'");
		Сообщить(ТекстСообщения, СтатусСообщения.Важное);
	КонецЕсли;
	// Конов запрет редаутирования списаний, выгруженных в ЕРП 
	...
	Если Объект.Статус = Перечисления.итСтатусыЗаявокНаВозврат.ОбработанПоставщиком Тогда
		ЭтаФорма.ТолькоПросмотр = Истина;
	КонецЕсли;
	Если Объект.Статус = Перечисления.итСтатусыЗаявокНаВозврат.ОбработанПоставщиком Тогда
		Элементы.Отправить.Доступность = Ложь;
		Элементы.ТоварыЗаполнитьПоОснованию.Доступность = Ложь;
	КонецЕсли;
	
	ПричинаВозвратаПриИзмененииНаСервере();
	КлассификацияПриИзмененииНаСервере();
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ДокументОснование.АвтоОтметкаНезаполненного = Истина;
	Элементы.ПричинаВозврата.АвтоОтметкаНезаполненного   = Истина;
	Элементы.ПодразделениеЗатрат.Видимость               = Ложь;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// Установим имя регистра на основании которого будет рассчитана сумма списания
	Если Объект.ХозОперация = Справочники.ХозОперации.СписаниеТоваровОтданныхНаКомиссию Тогда
		ПараметрыДействия.Вставить("ИмяРегистра", "ПартииТоваровОтданные");
	КонецЕсли;
	
	// Вызываем общий обработчик события
	Если НЕ УправлениеДиалогомДокументаСервер.ПриЧтенииНаСервере(ЭтотОбъект, ТекущийОбъект, ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.Классификация = ПредопределенноеЗначение("Перечисление.итКлассификацияПаспортовНаВозврат.КоричневыйПаспорт") И Не ЗначениеЗаполнено(Объект.СтатьяСписанияТМЦ) Тогда		
		Сообщить("Не указана статья списания в выбранной причине возврата");
		Отказ = Истина;		
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Установим имя регистра на основании которого будет рассчитана сумма списания
	Если Объект.ХозОперация = Справочники.ХозОперации.СписаниеТоваровОтданныхНаКомиссию Тогда
		ПараметрыЗаписи.Вставить("ИмяРегистра", "ПартииТоваровОтданные");
	КонецЕсли;
	...
	// у нас в модуле двойная запись, надо перезаполнить служебные реквизиты формы
...
Процедура ПринятоНаСервере()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	итСостоянияВыполненияСписанийТоваров.Списание КАК Списание,
	|	итСостоянияВыполненияСписанийТоваров.Состояние КАК Состояние
	|ИЗ
	|	РегистрСведений.итСостоянияВыполненияСписанийТоваров.СрезПоследних(, Списание = &Ссылка) КАК итСостоянияВыполненияСписанийТоваров";
...
Процедура ЗаполнитьПоОснованиюНаСервере()
	...
	Запрос.Текст = 
		...
		|	ПоступлениеТоваровТовары.НоменклатураПоставщика КАК НоменклатураПоставщика,
		...
		|	ПоступлениеТоваровТовары.ИдентификаторУпаковки КАК ИдентификаторУпаковки,
		|	ПоступлениеТоваровТовары.Производитель КАК Производитель,
		|	ПоступлениеТоваровТовары.ЦенаДок КАК ЦенаДок,
		|	ПоступлениеТоваровТовары.СуммаДок КАК СуммаДок,
		|	ПоступлениеТоваровТовары.КоличествоДок КАК КоличествоДок,
		|	ПоступлениеТоваровТовары.ДатаИзготовления КАК ДатаИзготовления,
		|	ПоступлениеТоваровТовары.СрокГодности КАК СрокГодности,
		|	ПоступлениеТоваровТовары.ИТ_ДатаВыработки КАК ИТ_ДатаВыработки,
		|	ПоступлениеТоваровТовары.ИТ_ДатаВыработки2 КАК ИТ_ДатаВыработки2
		|ИЗ
		|	Документ.ПоступлениеТоваров.Товары КАК ПоступлениеТоваровТовары
		|ГДЕ
		|	ПоступлениеТоваровТовары.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", Об.ДокументОснование);
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если Объект.Товары.Количество() Тогда	
		ОповещениеОчисткиТЧ = Новый ОписаниеОповещения("ПродолжитьЗаполнитьПоОснованию", ЭтотОбъект);			
		ПоказатьВопрос(ОповещениеОчисткиТЧ, "Перед заполнением, будет очищена табличная часть ""Товары"". Продолжить?",РежимДиалогаВопрос.ДаНетОтмена,0,  КодВозвратаДиалога.Да,);
	Иначе
...
&НаКлиенте
Процедура ПродолжитьЗаполнитьПоОснованию(Результат, Параметры) Экспорт 
...
&НаСервере
Процедура ПричинаВозвратаПриИзмененииНаСервере()
	
	Если ЗначениеЗаполнено(Объект.ПричинаВозврата) Тогда
		Элементы.ДекорацияЦветПаспорта.ЦветФона = Обработки.бит_ПреобразованияЦветов.HexToColor(Объект.ПричинаВозврата.Цвет);
	КонецЕсли;
	//Элементы.СтатьяСписанияТМЦ.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый массив);
	//
	//Если ЗначениеЗаполнено(Объект.Классификация) Тогда
	//	
	//    
	//	НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.итПричинаВозврата", "Объект.ПричинаВозврата");
	//    НовыйМассив = Новый Массив();
	//    НовыйМассив.Добавить(НоваяСвязь);

	//    
	//    	Элементы.СтатьяСписанияТМЦ.СвязиПараметровВыбора = Новый ФиксированныйМассив(НовыйМассив);	
	//	
	//КонецЕсли;// Вставить содержимое обработчика.
	
	Объект.СтатьяСписанияТМЦ = Объект.ПричинаВозврата.СтатьяСписания;
...
&НаСервере
Процедура КлассификацияПриИзмененииНаСервере()
	
	Элементы.ПричинаВозврата.СвязиПараметровВыбора = Новый ФиксированныйМассив(Новый массив);
	
	Если ЗначениеЗаполнено(Объект.Классификация) Тогда
		
	    
		НоваяСвязь = Новый СвязьПараметраВыбора("Отбор.Классификация", "Объект.Классификация");
	    НовыйМассив = Новый Массив();
	    НовыйМассив.Добавить(НоваяСвязь);

	    
   	 	Элементы.ПричинаВозврата.СвязиПараметровВыбора = Новый ФиксированныйМассив(НовыйМассив);	
		
	КонецЕсли;// Вставить содержимое обработчика.
	
	Если ЗначениеЗаполнено(Объект.ПричинаВозврата) И Объект.ПричинаВозврата.Классификация <> Объект.Классификация Тогда		
		Объект.ПричинаВозврата = Справочники.mr_ПричиныВозвратов.ПустаяСсылка();
	КонецЕсли;
	
	Если Объект.Классификация = Перечисления.итКлассификацияПаспортовНаВозврат.КоричневыйПаспорт Тогда
		Элементы.ДокументОснование.Видимость = Ложь;
		//Элементы.СтатьяСписанияТМЦ.Видимость = Истина;
	Иначе
		Элементы.ДокументОснование.Видимость = Истина;
		//Элементы.СтатьяСписанияТМЦ.Видимость = Ложь;
		
	КонецЕсли;
	
	Объект.ПричинаВозврата   = Справочники.mr_ПричиныВозвратов.НайтиПоРеквизиту("Классификация", Объект.Классификация);
	Объект.СтатьяСписанияТМЦ = Объект.ПричинаВозврата.СтатьяСписания;
...
Процедура Отправить(Команда)
	...
	Если Объект.Проведен = Ложь Тогда
		Сообщить("Перед отправкой требуется провести документ");
		Возврат;
	КонецЕсли;
	Если Объект.Товары.Количество() = 0 Тогда
		Сообщить("Перед отправкой требуется заполнить табличную часть Товары");
		Возврат;
	КонецЕсли;		
	
	Сутки = 86400;
	МинимальнаяДата = НачалоДня(Объект.Дата) + Сутки;
	Если НачалоДня(Объект.ДатаЗаявки) < НачалоДня(Объект.Дата) + Сутки Тогда 
		Сообщить("Минимальная дата возврата может быть " + Формат(МинимальнаяДата,"ДФ=dd.MM.yy"));
		Возврат;
	КонецЕсли;		
		
	Объект.Статус = ПредопределенноеЗначение("Перечисление.итСтатусыЗаявокНаВозврат.КОтправке");
	Записать();
	ОтправитьНаСервере();
