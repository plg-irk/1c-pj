// Модуль основной формы документа "Отчет комиссионера"

&НаКлиенте
Перем УдаляемыеСтроки; // Для удаления строк ТЧ
&НаКлиенте
Перем СуммаДоИзменения, СуммаНДСДоИзменения, ЭтоУдалениеСтроки, ИдСтрокиДоИзменения;
&НаКлиенте
...
Процедура УправлениеДиалогомНаСервере()
	...
	УправлениеДиалогомДокументаСервер.УстановитьДоступностьФлагаАвтоЗакрытиеСделок(ЭтотОбъект,Объект,"Товары");
...
Процедура Подключаемый_ОбработкаКомандыФормы(Команда) Экспорт
	...
	// Проверим если ли смысл открывать заполнение ТЧ
	Если Команда.Имя = "КомандаЗаполненияТоварыЗаполнитьПоДоговоруКомиссии" Тогда
		Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
			ВывестиСообщение(НСтр("ru = 'Поле ""Контрагент"" не заполнено.'"), "Объект", "Контрагент");	
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПересчетСуммПокупателей(ИдСтроки, НоваяСумма, НоваяСуммаНДС, тч="Покупатели")
	МассивСтрок = Объект[тч].НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдСтроки));
	...
	РазницаСумм = НоваяСумма - СуммаДоИзменения;
	РазницаНДС  = НоваяСуммаНДС - СуммаНДСДоИзменения;
	
	СуммаДоИзменения    = НоваяСумма; 
	СуммаНДСДоИзменения = НоваяСуммаНДС;
	
	ТекущиеДанные.СуммаВсего = ТекущиеДанные.СуммаВсего + РазницаСумм;
	ТекущиеДанные.СуммаНДС   = ТекущиеДанные.СуммаНДС + РазницаНДС;
КонецПроцедуры //ПересчетСуммПокупателей()
...
Процедура ПодборПоПартиям(Команда)
	...
	// Определим структуру параметров обработки текущего события
	ТекущиеДанныеПокупатели	= Элементы.Покупатели.ТекущиеДанные; идСтроки = 0;
	Если ТекущиеДанныеПокупатели<>Неопределено Тогда
		идСтроки = ТекущиеДанныеПокупатели.ИдентификаторСтроки;
	КонецЕсли;
	
	ПараметрыПодбора = Новый Структура;
	ПараметрыПодбора.Вставить("ИдентификаторСтроки", идСтроки);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.ОтчетКомиссионера.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура КонтрагентПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.КонтрагентПриИзменении(Объект, ПараметрыДействия);
	
	// настроить параметры выбора
	НастроитьПараметрыВыбораЭлементовФормы();
...
Процедура ДоговорВзаиморасчетовПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ДоговорВзаиморасчетовПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаВсегоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыСуммаВсегоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСтавкаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыСтавкаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыСуммаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыДокументПередачиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыДокументПередачиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыГТДПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ТоварыГТДПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ТекущиеДанные=Неопределено Тогда
		Отказ = Истина;
		ВывестиСообщение(НСтр("ru = 'Не выбран покупатель.'"));	
	ИначеЕсли НЕ ЗначениеЗаполнено(ТекущиеДанные.Контрагент) Тогда
		Отказ = Истина;
		ВывестиСообщение(НСтр("ru = 'Не заполнен покупатель.'"));
	Иначе 
		СуммаДоИзменения    = 0;
		СуммаНДСДоИзменения = 0;
		ИдСтрокиДоИзменения	= ТекущиеДанные.ИдентификаторСтроки;
	КонецЕсли;
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	СуммаДоИзменения    = Элементы.Товары.ТекущиеДанные.СуммаВсего;
	СуммаНДСДоИзменения = Элементы.Товары.ТекущиеДанные.СуммаНДС;
	ИдСтрокиДоИзменения	= Элементы.Товары.ТекущиеДанные.ИдентификаторСтроки
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	//пересчитаем суммы по покупателю
	ПересчетСуммПокупателей(ИдСтрокиДоИзменения, 0, 0);
	//
	СуммаДоИзменения    = 0;
	СуммаНДСДоИзменения = 0;
	ИдСтрокиДоИзменения			= 0;
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если ТекущиеДанныеПокупатели<>Неопределено Тогда
			ТекущиеДанныеТовары.ИдентификаторСтроки = ТекущиеДанныеПокупатели.ИдентификаторСтроки;
		КонецЕсли;
		СуммаДоИзменения	= 0;
		СуммаНДСДоИзменения	= 0;
	Иначе
		СуммаДоИзменения 	= ТекущиеДанныеТовары.Сумма;
		СуммаНДСДоИзменения	= ТекущиеДанныеТовары.СуммаНДС;
	КонецЕсли;
	
	ИдСтрокиДоИзменения = ТекущиеДанныеТовары.ИдентификаторСтроки;
...
Процедура ТоварыПриИзменении(Элемент)
	...
	Если СуммаДоИзменения=Неопределено Тогда
 		СуммаДоИзменения = 0;
	КонецЕсли;
	
	Если СуммаНДСДоИзменения=Неопределено Тогда
		СуммаНДСДоИзменения = 0;
    КонецЕсли;
    ...
    Если ИдСтрокиДоИзменения=Неопределено Тогда
        Если ТекущиеДанныеПокупатели=Неопределено Тогда
		    ИдСтрокиДоИзменения = 0;
        Иначе
		    ИдСтрокиДоИзменения = ТекущиеДанныеПокупатели.ИдентификаторСтроки;
        КонецЕсли; 
	КонецЕсли;
	
	Если ТекущиеДанныеТовары<>Неопределено Тогда
		ПересчетСуммПокупателей(ИдСтрокиДоИзменения, ТекущиеДанныеТовары.СуммаВсего, ТекущиеДанныеТовары.СуммаНДС);
		
	ИначеЕсли ТекущиеДанныеПокупатели<>Неопределено Тогда
		ПересчетСуммПокупателей(ТекущиеДанныеПокупатели.ИдентификаторСтроки, 0, 0);
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если НоваяСтрока И ОтменаРедактирования Тогда
		...
		ПересчетСуммПокупателей(ТекущиеДанные.ИдентификаторСтроки, -ТекущиеДанные.СуммаВсего, -ТекущиеДанные.СуммаНДС);
	КонецЕсли;
КонецПроцедуры //ТоварыПередОкончаниемРедактирования()
...
Процедура ПокупателиПриАктивизацииСтроки(Элемент)
	...
	Иначе
		Элементы.Товары.ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторСтроки", 0);
	КонецЕсли;
...
Процедура ПокупателиПередУдалением(Элемент, Отказ)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		СтрокаТЧ = Объект.Покупатели.НайтиПоИдентификатору(ВыделеннаяСтрока);
...
//
&НаСервере
Процедура ПокупателиПослеУдаленияНаСервере(ПараметрыДействия = Неопределено)
	Если ТипЗнч(ПараметрыДействия.УдаляемыеСтроки)=Тип("Массив") Тогда
		Для Каждого ИдентификаторСтроки Из ПараметрыДействия.УдаляемыеСтроки Цикл
			НайденныеТовары = Объект.Товары.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки));
			Для Каждого ТекущийТовар Из НайденныеТовары Цикл
				Объект.Товары.Удалить(ТекущийТовар);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры // ПокупателиПослеУдаленияНаСервере()
...
Процедура ПокупателиПослеУдаления(Элемент)
	// Обработаем событие в контексте сервера
	ПокупателиПослеУдаленияНаСервере(Новый Структура("УдаляемыеСтроки", УдаляемыеСтроки));
КонецПроцедуры //ПокупателиПослеУдаления()
...
Процедура ВозвратыНоменклатураПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыКоличествоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыСуммаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыСуммаВсегоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыСуммаВсегоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыСтавкаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыСтавкаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ВозвратыСуммаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыСуммаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
//
&НаСервере
Процедура ВозвратыДокументПродажиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Возвраты.НайтиПоИдентификатору(Элементы.Возвраты.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыДокументПродажиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры // ВозвратыДокументПродажиПриИзмененииНаСервере()
...
//
&НаКлиенте
Процедура ВозвратыДокументПродажиПриИзменении(Элемент)
	
	// Обработаем событие в контексте сервера
	ВозвратыДокументПродажиПриИзмененииНаСервере();

КонецПроцедуры // ВозвратыДокументПродажиПриИзменении()
...
//
&НаСервере
Процедура ВозвратыГТДПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Возвраты.НайтиПоИдентификатору(Элементы.Возвраты.ТекущаяСтрока);
	
	// Вызываем обработчик изменения данных объекта
	Документы.ОтчетКомиссионера.ВозвратыГТДПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры // ВозвратыГТДПриИзмененииНаСервере()
...
//
&НаКлиенте
Процедура ВозвратыГТДПриИзменении(Элемент)
	
	// Обработаем событие в контексте сервера
	ВозвратыГТДПриИзмененииНаСервере();
...
Процедура ВозвратыПередУдалением(Элемент, Отказ)
	...
	СуммаДоИзменения    = Элементы.Возвраты.ТекущиеДанные.СуммаВсего;
	СуммаНДСДоИзменения = Элементы.Возвраты.ТекущиеДанные.СуммаНДС;
	ИдСтрокиДоИзменения	= Элементы.Возвраты.ТекущиеДанные.ИдентификаторСтроки
...
Процедура ВозвратыПослеУдаления(Элемент)
	...
	//пересчитаем суммы по покупателю
	ПересчетСуммПокупателей(ИдСтрокиДоИзменения, 0, 0, "ПокупателиВозвраты");
	//
	СуммаДоИзменения    = 0;
	СуммаНДСДоИзменения = 0;
	ИдСтрокиДоИзменения	= 0;
...
КонецПроцедуры // ВозвратыПроверкаПеретаскивания()
...
Процедура ВозвратыПриИзменении(Элемент)
	...
	ТекущиеДанныеВозвраты	= Элементы.Возвраты.ТекущиеДанные;
	...
	Если ТекущиеДанныеВозвраты<>Неопределено Тогда
		ПересчетСуммПокупателей(ИдСтрокиДоИзменения, ТекущиеДанныеВозвраты.СуммаВсего, ТекущиеДанныеВозвраты.СуммаНДС, "ПокупателиВозвраты");
		
	ИначеЕсли ТекущиеДанныеПокупатели<>Неопределено Тогда
		ПересчетСуммПокупателей(ТекущиеДанныеПокупатели.ИдентификаторСтроки, 0, 0, "ПокупателиВозвраты");
		
	КонецЕсли;
КонецПроцедуры //ВозвратыПриИзменении()
...
Процедура ВозвратыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		Если ТекущиеДанныеПокупатели<>Неопределено Тогда
			ТекущиеДанныеВозвраты.ИдентификаторСтроки = ТекущиеДанныеПокупатели.ИдентификаторСтроки;
		КонецЕсли;
		СуммаДоИзменения	= 0;
		СуммаНДСДоИзменения	= 0;
	Иначе
		СуммаДоИзменения 	= ТекущиеДанныеВозвраты.Сумма;
		СуммаНДСДоИзменения	= ТекущиеДанныеВозвраты.СуммаНДС;
	КонецЕсли;
	
	ИдСтрокиДоИзменения = ТекущиеДанныеВозвраты.ИдентификаторСтроки;
КонецПроцедуры //ВозвратыПриНачалеРедактирования()

&НаКлиенте
Процедура ВозвратыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если НоваяСтрока И ОтменаРедактирования Тогда
		ТекущиеДанные = Элементы.Возвраты.ТекущиеДанные;
		ПересчетСуммПокупателей(ТекущиеДанные.ИдентификаторСтроки, -ТекущиеДанные.СуммаВсего, -ТекущиеДанные.СуммаНДС, "ПокупателиВозвраты");
	КонецЕсли;
КонецПроцедуры //ВозвратыПередОкончаниемРедактирования()
...
Процедура ПокупателиВозвратыПриАктивизацииСтроки(Элемент)
	ТекущиеДанные = Элементы.ПокупателиВозвраты.ТекущиеДанные;
	Если ТекущиеДанные<>Неопределено Тогда
		Элементы.Возвраты.ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторСтроки", ТекущиеДанные.ИдентификаторСтроки);
	Иначе
		Элементы.Возвраты.ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторСтроки", 0);
	КонецЕсли;
КонецПроцедуры //ПокупателиВозвратыПриАктивизацииСтроки()
...
Процедура ПокупателиВозвратыПередУдалением(Элемент, Отказ)
	ВыделенныеСтроки = Элементы.ПокупателиВозвраты.ВыделенныеСтроки;
	
	УдаляемыеСтроки = Новый Массив;
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		СтрокаТЧ = Объект.ПокупателиВозвраты.НайтиПоИдентификатору(ВыделеннаяСтрока);
		УдаляемыеСтроки.Добавить(СтрокаТЧ.ИдентификаторСтроки);
	КонецЦикла;	
КонецПроцедуры //ПокупателиВозвратыПередУдалением()
...
Процедура ПокупателиВозвратыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		...
		Элементы.Возвраты.ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторСтроки", ТекущиеДанные.ИдентификаторСтроки);
	КонецЕсли;
КонецПроцедуры //ПокупателиВозвратыПриНачалеРедактирования()
...
//
&НаСервере
Процедура ПокупателиВозвратыПослеУдаленияНаСервере(ПараметрыДействия = Неопределено)
	Если ТипЗнч(ПараметрыДействия.УдаляемыеСтроки)=Тип("Массив") Тогда
		Для Каждого ИдентификаторСтроки Из ПараметрыДействия.УдаляемыеСтроки Цикл
			НайденныеТовары = Объект.Возвраты.НайтиСтроки(Новый Структура("ИдентификаторСтроки", ИдентификаторСтроки));
			Для Каждого ТекущийТовар Из НайденныеТовары Цикл
				Объект.Возвраты.Удалить(ТекущийТовар);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры // ПокупателиВозвратыПослеУдаленияНаСервере()
...
Процедура ПокупателиВозвратыПослеУдаления(Элемент)
	// Обработаем событие в контексте сервера
	ПокупателиВозвратыПослеУдаленияНаСервере(Новый Структура("УдаляемыеСтроки", УдаляемыеСтроки));
КонецПроцедуры //ПокупателиВозвратыПослеУдаления()
...
Процедура ПриОткрытии(Отказ)
	...
	//для старых документов, обеспечим видимость
	Если Объект.Товары.Количество()>0 И Объект.Покупатели.Количество()=0 И ЗначениеЗаполнено(Объект.Контрагент) Тогда
		НоваяСтрока = Объект.Покупатели.Добавить();
		НоваяСтрока.Контрагент = Объект.Контрагент;
		НоваяСтрока.СуммаВсего = ОбЪект.Товары.Итог("СуммаВсего");
		НоваяСтрока.СуммаНДС = ОбЪект.Товары.Итог("СуммаНДС");
		НоваяСтрока.ИдентификаторСтроки = Новый УникальныйИдентификатор();
		
		Для Каждого СтрокаТоваров Из Объект.Товары Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТоваров.ИдентификаторСтроки) Тогда
				СтрокаТоваров.ИдентификаторСтроки = НоваяСтрока.ИдентификаторСтроки;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
