...
Процедура ВсеПолученныеСправки(Команда)
	...
	Если СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().РазделениеВключено Тогда
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьВебСтраницу(
			АдресСтраницыЛичныйКабинетВсеСправкиВМоделиСервиса(),
...
&НаСервереБезКонтекста
Функция АдресСтраницыЛичныйКабинетВсеСправкиВМоделиСервиса()
	
	Если Не ПравоДоступа("Просмотр", Метаданные.Документы.СправкиСПАРКРиски) Тогда
		ВызватьИсключение НСтр("ru = 'Недостаточно прав.'");
	КонецЕсли;
	
	Результат = ИнтернетПоддержкаПользователейКлиентСервер.URLСтраницыПорталаПоддержки(
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		РезультатПолученияТикета = МодульИнтернетПоддержкаПользователейВМоделиСервиса.ТикетАутентификацииНаПорталеПоддержки(Результат);
