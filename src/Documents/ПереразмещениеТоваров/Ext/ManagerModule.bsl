...
Функция ПечатьПереразмещениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ТекстЗаголовка = УправлениеПечатьюПлатформа.ПолучитьПредставлениеДокумента(Документ, НСтр("ru = 'Переразмещение товаров'"));
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ПараметрыКоманды = Новый Структура();
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыОптимизация", "ТоварыОптимизация", "Товары", НСтр("ru = 'Оптимизация'"), ПараметрыКоманды,,,, ИСТИНА);
	
	ПараметрыКоманды = Новый Структура();
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "ТоварыПогашениеМинусов", "ТоварыПогашениеМинусов", "Товары", НСтр("ru = 'Погашение минусов'"), ПараметрыКоманды,,,, ИСТИНА);
...
Функция ТоварыЗаполнитьСкладскимиОстатками(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.ИмяКоманды = "ТоварыЗаполнитьСкладскимиОстатками" ИЛИ ПараметрыЗаполнения.ИмяКоманды = "ТоварыЗаполнитьСкладскимиОстаткамиПоГруппе" Тогда
		// Если по себестоимости - используем приватное заполнение.
	...
	ИначеЕсли ПараметрыЗаполнения.ИмяКоманды = "ТоварыЗаполнитьСкладскимиОстаткамиПоВыбраннойЯчейке" Тогда
...
//
Функция ТоварыОптимизация(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	
	// Проверим склады компании на соответствие одному складу
	Если Объект.СкладКомпании <> Объект.СкладПолучатель Тогда
		
		ВывестиСообщение(НСтр("ru='Оптимизация возможна только в пределах одного склада.'"));
...
// Обработчик свертывания однотипных товаров
//
Функция ТоварыПогашениеМинусов(Объект, ПараметрыЗаполнения, ПараметрыДействия=Неопределено) Экспорт
	
	// Проверим склады компании на соответствие одному складу
	Если Объект.СкладКомпании <> Объект.СкладПолучатель Тогда
		
		ВывестиСообщение(НСтр("ru='Погашение минусов возможно только в пределах одного склада.'"));
