...
Процедура УправлениеДиалогомНаСервере()
	...
	АктПостановкиНаСклад = Объект.ХозОперация = Справочники.ХозОперации.АктПостановкиНаБалансЕГАИС;
	
	Элементы.СтраницаМарки.Видимость                           = АктПостановкиНаСклад;
	Элементы.ТоварыПредставлениеМарок.Видимость                = АктПостановкиНаСклад;
	Элементы.ТоварыСправка1.Видимость            = АктПостановкиНаСклад;
	Элементы.ТоварыНомерТТН.Видимость            = АктПостановкиНаСклад;
	Элементы.ТоварыДатаТТН.Видимость             = АктПостановкиНаСклад;
	Элементы.ТоварыДатаРозлива.Видимость         = АктПостановкиНаСклад;
	Элементы.ТоварыКоличествоПродукции.Видимость = АктПостановкиНаСклад;
	Элементы.ТоварыНомерФиксации.Видимость       = АктПостановкиНаСклад;
	Элементы.ТоварыДатаФиксации.Видимость        = АктПостановкиНаСклад;
	...
	//ЭлементыФормы.ДействияФормы.Кнопки.ОтменитьПроведениеАктаВЕГАИС.Доступность = Ложь;
	//Если ЕстьОтменаПроведенияАктаЕГАИС И   
	//	ДокументОбъект.ПометкаУдаления = Ложь И ДокументОбъект.Статус = Справочники.СтатусыОбработкиЕГАИС.ЕГАИСЗарегистрировалаДокумент И 
	//	ДокументОбъект.Состояние = Перечисления.СостоянияДокументовЕГАИС.Выполнен Тогда
	//	
	//	ЭлементыФормы.ДействияФормы.Кнопки.ОтменитьПроведениеАктаВЕГАИС.Доступность = Истина;
	//КонецЕсли;
	
	//Попытка
	//	Если НЕ ПустаяСтрока(ДокументОбъект.РегистрационныйНомер) Тогда
	//		ЭлементыФормы.НомерАктаСписания.ТолькоПросмотр = Истина;
	//	ИначеЕсли ДокументОбъект.ПричинаПостановкиНаБаланс = "Пересортица" Тогда
	//		ЭлементыФормы.НомерАктаСписания.ТолькоПросмотр = Ложь;
	//	Иначе
	//		ЭлементыФормы.НомерАктаСписания.ТолькоПросмотр = Истина;
	//	КонецЕсли;
	//Исключение
	...
	//Для Каждого Кнопка Из ЭлементыФормы.КоманднаяПанельТовары.Кнопки.Заполнение.Кнопки Цикл
	//			Если Кнопка.Имя = "ДополнитьОтрицательнымиОстаткамиЕГАИС" Тогда  
	//		Кнопка.Доступность = ДокументОбъект.Хозоперация = Справочники.ХозОперации.АктПостановкиНаБалансВТорговыйЗалЕГАИС;    
	//		КонецЕсли;
	//КонецЦикла;
...
Процедура БлокироватьЭлементыФормы()
	...
	Сформирован = НЕ (Документы.АктПостановкиНаБалансЕГАИС.ВидыСтатусовДвижений().Найти(СтатусОбработки) = Неопределено);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.АктПостановкиНаБалансЕГАИС.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ТоварыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЕдиницаИзмеренияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ТоварыЕдиницаИзмеренияПриИзменении(Объект, ТекущиеДанные);
...
Процедура ТоварыКоличествоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ТоварыКоличествоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыАлкогольнаяПродукцияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.ТоварыАлкогольнаяПродукцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура МаркиКодМаркиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.АктПостановкиНаБалансЕГАИС.МаркиКодМаркиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
//Процедура АктПостановкиНаБалансНомерАктаСписанияНачалоВыбора(ДокументОбъект, ФормаДокумента, Элемент, СтандартнаяОбработка) Экспорт
//	
//	СтандартнаяОбработка = Ложь;

//	ФормаВыбора = Документы.СписаниеТоваров.ПолучитьФормуВыбора("ФормаСписка", ФормаДокумента);
//	ТекОтбор = ФормаВыбора.Отбор.Найти("РегистрационныйНомер");
//	Если ТекОтбор = Неопределено Тогда
//		ТекОтбор = ФормаВыбора.Отбор.Добавить("РегистрационныйНомер");
//	КонецЕсли;
//	
...
//	ДокументОбъект.НомерАктаСписания = Результат.РегистрационныйНомер;
