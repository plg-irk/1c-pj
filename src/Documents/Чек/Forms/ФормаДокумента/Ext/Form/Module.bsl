...
Процедура БлокироватьЭлементыФормы()
	...
	СписокРеквизитов.Добавить("ЭтотОбъект"	, "ЭтотОбъект"	, Пробит ИЛИ Объект.Архивный);
	СписокРеквизитов.Добавить("Архивный"	,				, НЕ (Пробит ИЛИ Объект.Архивный));
	СписокРеквизитов.Добавить("Оплаты"		, "Оплаты"		, Пробит ИЛИ Объект.Архивный);
	СписокРеквизитов.Добавить("Товары"		, "Товары"		, Пробит ИЛИ Объект.Архивный);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение И ТекущийОбъект.Товары.Итог("СуммаВсего") > (ТекущийОбъект.Оплаты.Итог("Сумма")-ТекущийОбъект.Оплаты.Итог("Сдача")) И ПравоПользователя("ПогашатьЧекЦеликом") Тогда
		...
		Если ТаблицаОплат.Количество()=0 Тогда
			...
			НоваяСтрока.Сумма     = ТекущийОбъект.Товары.Итог("СуммаВсего");
		...
		Иначе
			...
			Для каждого СтрокаТабличнойЧасти Из ТекущийОбъект.Оплаты Цикл
				...
				Если ЗначениеЗаполнено(СтрокаНаличных) Тогда
					СтрокаНаличных.Сумма = СтрокаНаличных.Сумма + (ТекущийОбъект.Товары.Итог("СуммаВсего") - (ТекущийОбъект.Оплаты.Итог("Сумма")-ТекущийОбъект.Оплаты.Итог("Сдача")));
				КонецЕсли;
