	...
	Если ЗаполнятьСтатьиВыручки Тогда
		...
		Если СпособРасчетаВыручки = 0 Тогда
			Если КоличествоПериодовВыручка <= 0 Тогда
				ВывестиСообщение(НСтр("ru = 'Количество периодов для анализа должно быть больше нуля.'"), ЭтотОбъект);
				Возврат;
			КонецЕсли;
		Иначе
			Если НЕ ЗначениеЗаполнено(ДокументОснованиеВыручка) Тогда
				ВывестиСообщение(НСтр("ru = 'Документ-основание для заполнения статьями доходов от основной деятельности не выбран.'"), ЭтотОбъект);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗаполнитьСтатьиСебестоимость Тогда
		...
		Иначе
			Если НЕ ЗначениеЗаполнено(ДокументОснованиеСебестоимость) Тогда
				ВывестиСообщение(НСтр("ru = 'Документ-основание для заполнения статьями расходов от основной деятельности не выбран.'"), ЭтотОбъект);
				Возврат;
			КонецЕсли;
		КонецЕсли; 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.СценарийПланирования) ИЛИ НЕ ЗначениеЗаполнено(Параметры.СценарийПланирования.Периодичность) Тогда
		ВывестиСообщение(НСтр("ru='Не выбран сценарий планирования, либо у выбранного сценария планирования не выбрана периодичность.'"), ЭтотОбъект);
		Отказ = Истина;
	КонецЕсли;
