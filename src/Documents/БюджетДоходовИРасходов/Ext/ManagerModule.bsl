...
Процедура ЗаполнитьСтатьямиВыручки(Объект, РезультатАвтозаполнения)
	...
	Иначе
		ВывестиСообщение(НСтр("ru ='Заполнение статьями доходов от основной деятельности не выполнено. Сумма выручки равна 0.'"));
	КонецЕсли;
...
Процедура ЗаполнитьСтатьямиСебестоимости(Объект, РезультатАвтозаполнения)
	...
	Иначе
		ВывестиСообщение(НСтр("ru = 'Заполнение статьями расходов от основной деятельности не выполнено. Сумма себестоимости равна 0.'"));
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ ИЗМЕНЕНИЯ ДАННЫХ ТАБЛИЦЫ "РАСХОДЫ И ДОХОДЫ"
...
Процедура ДобавитьКомандыЗаполнения(КомандыЗаполнения) Экспорт
	...
	ПоследовательныеОперации.ДобавитьВыборСсылки(ПараметрыКоманды, "Автозаполнение", "Документ.БюджетДоходовИРасходов.Форма.ФормаАвтозаполнения", ПараметрыФормы,, ИСТИНА);
	ЗаполнениеДокументовСервер.ДобавитьКоманду(КомандыЗаполнения, "РасходыИДоходыАвтозаполнение", "РасходыИДоходыАвтозаполнение", "РасходыИДоходы", НСтр("ru = 'Автозаполнение'"), ПараметрыКоманды,,,, ЛОЖЬ);
...
//  Объект - ДанныеФормыСтруктура - Объект, для которого выполняется обработка события.
//
Функция РасходыИДоходыАвтозаполнение(Объект, ПараметрыЗаполнения=Неопределено, ПараметрыДействия=Неопределено, РежимПолученияЦеныПоСебестоимости=ЛОЖЬ) Экспорт
