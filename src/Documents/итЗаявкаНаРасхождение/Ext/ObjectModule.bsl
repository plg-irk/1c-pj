...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//	Статус = Перечисления.итСтатусыЗаявокНаВозврат.КОтправке;	
	//КонецЕсли;
	////Мальцев А.Ю. ---
	СуммаДокумента = Товары.Итог("СуммаРозничная");
	//
	
	//НайденнаяЗаявкаНаРасхождение = Документы.итЗаявкаНаРасхождение.ПроверитьНаличиеЗаявкиНаРасхождение(ДокументОснование, Ссылка);
	//Если ЗначениеЗаполнено(НайденнаяЗаявкаНаРасхождение) Тогда
	//	Отказ = Истина;
	//	Сообщить("По поступлению " + Строка(ДокументОснование) + " уже есть документ " + Строка(НайденнаяЗаявкаНаРасхождение));
	//КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	//ВыполнитьКонтрольСоответствияОснованию(Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// - если заявка на расхождение оформлена на полный недовоз и она принята поставщиком, то накладную отменяем
	Если ЭтотОбъект.Статус = Перечисления.итСтатусыЗаявокНаВозврат.ОбработанПоставщиком И 
		Товары.Итог("Количество") = 0 И 
		ДокументОснование.Проведен Тогда
		
		НакладнаяОбъект = ДокументОснование.ПолучитьОбъект();
...
Процедура ВыполнитьКонтрольСоответствияОснованию(Отказ)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	итЗаявкаНаВозвратТовары.Номенклатура КАК Номенклатура,
	|	СУММА(итЗаявкаНаВозвратТовары.Количество) КАК КоличествоЗаявка
	|ПОМЕСТИТЬ втЗаявка
	|ИЗ
	|	Документ.итЗаявкаНаВозврат.Товары КАК итЗаявкаНаВозвратТовары
	|ГДЕ
	|	итЗаявкаНаВозвратТовары.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	итЗаявкаНаВозвратТовары.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	втЗаявка.КоличествоЗаявка КАК КоличествоЗаявка,
	|	ЕСТЬNULL(втПоступление.Количество, 0) КАК Количество
	|ИЗ
	|	втЗаявка КАК втЗаявка
	|		ЛЕВОЕ СОЕДИНЕНИЕ втПоступление КАК втПоступление
	|		ПО втЗаявка.Номенклатура = втПоступление.Номенклатура
	|ГДЕ
	|	втЗаявка.КоличествоЗаявка > ЕСТЬNULL(втПоступление.Количество, 0)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Сообщить("По номенклатуре " + Строка(ВыборкаДетальныеЗаписи.Номенклатура) + " возвращается (" + Строка(ВыборкаДетальныеЗаписи.КоличествоЗаявка) + ") больше чем поступило (" +  Строка(ВыборкаДетальныеЗаписи.Количество) + ")");  
		Отказ = Истина;
	КонецЦикла;
