// Модуль формы списка документа "Списание товаров"
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ УправлениеСпискомДокументаСервер.ПриСозданииНаСервере(ЭтотОбъект, Параметры, Отказ, СтандартнаяОбработка) Тогда
		Возврат;
	КонецЕсли;
	//ЗащищенныеФункцииСервер.ДобавитьКоманду(ЭтаФорма, "ОтчетВыполнениеЗаявокНаВозврат", "ДополнительныеДействия", БиблиотекаКартинок.Отчет, НСтр("ru = 'Отчет план/факт заявок на возврат'"), НСтр("ru = 'Отчет план/факт заявок на возврат'"), ЛОЖЬ,,, ОтображениеКнопки.КартинкаИТекст);
...
&НаКлиенте
Процедура ОтчетВыполнениеЗаявокНаВозврат(Команда)
	
	ИмяОбр = ПолучитьИмяВнешнегоОтчета();
	
	Если ИмяОбр = "" Тогда 
		Возврат;
	КонецЕсли;
	//
	ФормаСканирования = ОткрытьФорму("ВнешнийОтчет." + ИмяОбр + ".Форма");
	ФормаСканирования.Активизировать();             
...
Функция ПолучитьИмяВнешнегоОтчета()
	...
	УИДОтчета = "c73cb722-d3a0-11ed-80e0-005056b00073";
	ВнешняяОбработка = Справочники.ДополнительныеОтчетыИОбработки.ПолучитьСсылку(Новый УникальныйИдентификатор(УИДОтчета));
