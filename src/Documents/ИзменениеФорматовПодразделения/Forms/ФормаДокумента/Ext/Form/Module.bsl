// Модуль основной формы документа "Изменение форматов подразделения"
&НаКлиенте
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.ИзменениеФорматовПодразделения.ДатаПриИзменении(Объект, ПараметрыДействия);
	// Обновляем отображение элементов формы
	УправлениеДиалогомНаСервере();
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ИзменениеФорматовПодразделения.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	// Вызываем обработчик изменения данных объекта
	Документы.ИзменениеФорматовПодразделения.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
&НаСервере
Процедура ТоварыФорматПриИзмененииНаСервере()
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	
	Если ЗначениеЗаполнено(ТекущиеДанные.Формат) Тогда
		Для Каждого Эл Из Элементы Цикл
			Если Лев(Эл.Имя,3)="ФКП" Тогда
				Попытка
					ТекущиеДанные[Эл.Имя] = Эл.Имя = "ФКП" + ТекущиеДанные.Формат.КраткоеНаименование;
				Исключение
				КонецПопытки;	
			КонецЕсли;
		КонецЦикла;	
...
&НаКлиенте
Процедура ТоварыФорматПриИзменении(Элемент)
	ТоварыФорматПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	ОбновитьКолонкиФорматовНаСервере();
...
&НаСервере
Процедура ОбновитьКолонкиФорматовНаСервере()
	МассивРеквизитов = Новый Массив;
		
	//МассивЭлементов = Новый Массив;
	Для Каждого Эл Из Элементы Цикл
		Имя = Эл.Имя;
		Если Лев(Имя,3)="ФКП" Тогда
			//МассивЭлементов.Добавить(Эл);
			Если Найти(Имя,"КонтекстноеМеню")>0 Или Найти(Имя,"РасширеннаяПодсказка")>0 Тогда
				Продолжить;
			КонецЕсли;
			МассивРеквизитов.Добавить("Объект.Товары."+Имя); //Новый РеквизитФормы(Эл.Имя, Новый ОписаниеТипов("Булево"),"Объект.Товары",, Ложь));	
		КонецЕсли;
	КонецЦикла;	
	
	Для Каждого Реквизит Из МассивРеквизитов Цикл
		//Элементы.Удалить(Элементы[Реквизит]);
		Элементы.Удалить(Элементы[СтрЗаменить(Реквизит,"Объект.Товары.","")]);
	КонецЦикла;	
	...
	Если МассивРеквизитов.Количество()=0 И НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента) Тогда
		Возврат;
	КонецЕсли;
	
	
	Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента) ИЛИ НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента.ФорматыАссортимента) Тогда
		//ВывестиСообщение("Для формирования колонок форматов укажите подразделение и ветку форматов у него");
		//Возврат;
		//Формат = Справочники.ФорматыКатегорий.Выбрать();
		ЗапросФорматов = Новый Запрос("
		|ВЫБРАТЬ
		|	Наименование,
		|	КраткоеНаименование
		|ИЗ
		|	Справочник.ФорматыКатегорий
		|ГДЕ
		|	НЕ НеУчаствуетВДвижениях
		|УПОРЯДОЧИТЬ ПО
		|	Порядок ИЕРАРХИЯ
		|АВТОУПОРЯДОЧИВАНИЕ
		|");
		Формат = ЗапросФорматов.Выполнить().Выбрать();
	Иначе
		//Формат = Справочники.ФорматыКатегорий.Выбрать(Объект.ПодразделениеАссортимента.ФорматыАссортимента);
		ЗапросФорматов = Новый Запрос("
		|ВЫБРАТЬ
		|	Наименование,
		|	КраткоеНаименование
		...
		|АВТОУПОРЯДОЧИВАНИЕ
		|");
		ЗапросФорматов.УстановитьПараметр("Родитель", Объект.ПодразделениеАссортимента.ФорматыАссортимента);
		Формат = ЗапросФорматов.Выполнить().Выбрать();
	КонецЕсли;
	...
	Попытка
		...
		Пока Формат.Следующий() Цикл
			//Если Формат.НеУчаствуетВДвижениях Тогда
			//	Продолжить;
			//КонецЕсли;
			ИмяРеквизита = "ФКП" + Формат.КраткоеНаименование;
			МассивРеквизитов.Добавить(Новый РеквизитФормы(ИмяРеквизита, Новый ОписаниеТипов("Булево"),"Объект.Товары", Формат.Наименование, Ложь));	
		КонецЦикла;
		ИзменитьРеквизиты(МассивРеквизитов);		
	Исключение
		Ощ = ОписаниеОшибки();
		ИзменитьРеквизиты();
		ВывестиСообщение("Не удалось сформировать колонки форматов, проверьте заполнение справочника форматов");
	КонецПопытки;
	
	Для Каждого Реквизит Из МассивРеквизитов Цикл
		Элт = Элементы.Добавить(Реквизит.Имя, Тип("ПолеФормы"), Элементы.Товары);
		Элт.Вид = ВидПоляФормы.ПолеФлажка;
		Элт.ПутьКДанным = "Объект.Товары." + Реквизит.Имя;
		Элт.УстановитьДействие(
			"ПриИзменении", 		    //Имя события
			"ПриИзмененииФлагаФормата"); //Имя процедуры обработчика
		//Элт.Ширина = 10;
	...
	Для Каждого Стр Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(Стр.Формат) Тогда
			Попытка
				Стр["ФКП" + Стр.Формат.КраткоеНаименование] = Истина;
			Исключение
			КонецПопытки;
		КонецЕсли;	
	КонецЦикла;	
...
&НаКлиенте
Процедура ПриИзмененииФлагаФормата(Элемент)
	Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента) Тогда
		Возврат;
	КонецЕсли;
	Имя = Элемент.Имя;
	Значение = Элементы.Товары.ТекущиеДанные[Элемент.Имя];
	Строки = Элементы.Товары.ВыделенныеСтроки;
 	ПриИзмененииФлагаФорматаНаСервере(Имя,Строки,Значение); //Элементы.Товары.ТекущаяСтрока
...
&НаСервере
Процедура ПриИзмененииФлагаФорматаНаСервере(ИмяФормата,Строки, Значение)
	
	
	//Значение = Строка[Имя];
	Если НЕ Значение Тогда
		Формат = Справочники.ФорматыКатегорий.ПустаяСсылка();
	Иначе	
		ФорматКраткое = Сред(ИмяФормата,4);
		Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента) ИЛИ НЕ ЗначениеЗаполнено(Объект.ПодразделениеАссортимента.ФорматыАссортимента) Тогда
			Формат = Справочники.ФорматыКатегорий.НайтиПоРеквизиту("КраткоеНаименование",ФорматКраткое);
		Иначе
			Формат = Справочники.ФорматыКатегорий.НайтиПоРеквизиту("КраткоеНаименование",ФорматКраткое,Объект.ПодразделениеАссортимента.ФорматыАссортимента);
		КонецЕсли;
	...
	Для Каждого НомерСтроки Из Строки Цикл
		...
		Строка.Формат = Формат;
		Для Каждого Эл Из МассивЭлементов Цикл
			Если Эл = ИмяФормата Тогда
				Строка[Эл] = Значение;
			Иначе
				Строка[Эл] = Ложь;
			КонецЕсли;			
...
&НаКлиенте
Процедура ОбновитьКолонкиФорматов(Команда)
	ОбновитьКолонкиФорматовНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ПодразделениеАссортиментаПриИзменении(Элемент)
	ОбновитьКолонкиФорматовНаСервере();
КонецПроцедуры
