...
Функция ВремяОграниченияЗаписиЗаказаПоставщику(ДатаСравнения, ВидЗаявкиЗаказа, Контрагент, Подразделение) Экспорт
	
	ВремяОграничения = Дата(1,1,1,0,0,0);
	
	Запрос       = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               |	ГрафикиЗаказовСрезПоследних.ВремяОграниченияЗаписи КАК ВремяОграниченияЗаписи
	               |ИЗ
	               |	РегистрСведений.ГрафикиЗаказов.СрезПоследних(
	               |			&Дата,
	               |			СегментНоменклатуры В
	               |					(ВЫБРАТЬ ПЕРВЫЕ 1
	               |						ит_СоответствиеВидаЗаявкиВидуНоменклатуры.СегментНоменклатуры
	               |					ИЗ
	               ...
	               |				И Контрагент = &Контрагент) КАК ГрафикиЗаказовСрезПоследних";
	
	Запрос.УстановитьПараметр("Дата", ДатаСравнения);
	Запрос.УстановитьПараметр("ВидЗаказа", ВидЗаявкиЗаказа);
	Запрос.УстановитьПараметр("Подразделение", Подразделение);
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
	
	УстановитьПривилегированныйРежим(ИСТИНА);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() И Выборка.ВремяОграниченияЗаписи > Дата(1,1,1,0,0,0) Тогда
		ВремяОграничения = Выборка.ВремяОграниченияЗаписи;
	КонецЕсли;
	УстановитьПривилегированныйРежим(ЛОЖЬ);
	
	Возврат ВремяОграничения;
	
КонецФункции

Функция КоличествоСекундОграниченияЗаписиЗаказаПоставщику(ДатаСравнения, ВидЗаявкиЗаказа, Контрагент, Подразделение) Экспорт
	
	ВремяОграничения = ВремяОграниченияЗаписиЗаказаПоставщику(ДатаСравнения, ВидЗаявкиЗаказа, Контрагент, Подразделение);
	
	Возврат ВремяОграничения - НачалоДня(ВремяОграничения);
