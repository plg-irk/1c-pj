...
&НаСервере
// Процедура предназначена для установления отбора по не распределенным и полураспределенные заказам контрагета.
//
Процедура УстановитьОтборПоКонтрагенту(Контрагент, МоментВремени)
	
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		Возврат;
	КонецЕсли;
	
	ТекстПериода = ?(ЗначениеЗаполнено(МоментВремени), "&Период", "");
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыПоставщикам.Остатки(" + ТекстПериода +", Контрагент = &Контрагент) КАК Заказы
	|ГДЕ
	|	НЕ Заказы.ЗаказаноОстаток = 0
	|";
	
	Запрос.УстановитьПараметр("Контрагент", Контрагент);
	Если ТипЗнч(МоментВремени) = Тип("Дата") ИЛИ ТипЗнч(МоментВремени) = Тип("Граница") Тогда
		Запрос.УстановитьПараметр("Период", МоментВремени);
	Иначе
		Запрос.УстановитьПараметр("Период", МоментВремени.МоментВремени());
	КонецЕсли;
	...
	НовыйОтбор.ПравоеЗначение = СписокЗаказов;
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Список.Отбор, "Контрагент", Контрагент, ВидСравненияКомпоновкиДанных.Равно,,Истина);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Контрагент") Тогда
		ОтборХозяин = Параметры.Контрагент;
		
		Если ТипЗнч(Параметры.Момент) = Тип("Дата") Тогда
			Момент = Параметры.Момент;
		Иначе
			Момент = Новый Граница(Новый МоментВремени(Параметры.Момент.Дата, Параметры.Момент), ВидГраницы.Исключая);
		КонецЕсли;
		
		УстановитьОтборПоКонтрагенту(Параметры.Контрагент, Момент);
	КонецЕсли;
	// ТК - 
...
&НаКлиенте
Процедура МодульОбменаКонтур(Команда)
	ОткрытьФорму("Обработка.итМодульОбменаКонтур.Форма.Форма");
КонецПроцедуры
