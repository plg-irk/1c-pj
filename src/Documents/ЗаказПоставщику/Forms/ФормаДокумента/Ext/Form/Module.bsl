...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ВыполнитьКонтрольОсновнойЕдИзмерения(ТекущийОбъект, Отказ);
...
&НаСервере
Процедура ВыполнитьКонтрольОсновнойЕдИзмерения(ТекущийОбъект, Отказ)
	...
	|	втТоварыВыборка КАК втТоварыВыборка
	|ГДЕ
	|	втТоварыВыборка.Номенклатура.ОсновнаяЕдиницаИзмерения = ЗНАЧЕНИЕ(Справочник.ЕдиницыИзмерения.ПустаяСсылка)";
	Запрос.УстановитьПараметр("втТовары", тчТовары);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Сообщить("В строке - " + ВыборкаДетальныеЗаписи.НомерСтроки + " для номенклатуры " + Строка(ВыборкаДетальныеЗаписи.Номенклатура)  + " не выбрана основная ед. измерения");
		Отказ = Истина;
	КонецЦикла;
