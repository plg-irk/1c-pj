...
Процедура СоздатьРегламентированноеЗадание(ИмяВесов)
	...
	Если ВыполнятьПоРасписанию Тогда
		...
		Если Задание = Неопределено Тогда
			...
			Задание.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Выгрузка в Весы с печатью этикеток DIGI %1'"), СокрЛП(ПодразделениеКомпании));
			Задание.Записать();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	времКаталогШаблонов         = Неопределено;
	...
	Параметры.ПараметрыОборудования.Свойство("КаталогШаблонов",          времКаталогШаблонов);
	...
	КаталогШаблонов          = ?(времКаталогШаблонов = Неопределено, "", времКаталогШаблонов);
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	ИмяВесов = "";
	СтрIP = ВремНастройки.ПараметрыОборудования.P_IP;
	...
	ВремНастройки.ПараметрыОборудования.Вставить("КаталогШаблонов",         КаталогШаблонов);
...
&НаКлиенте
Процедура КаталогШаблоновНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		КаталогШаблонов = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КаталогШаблоновНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Оповещение = Новый ОписаниеОповещения("КаталогШаблоновНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, КаталогШаблонов, "ВыборКаталога");
