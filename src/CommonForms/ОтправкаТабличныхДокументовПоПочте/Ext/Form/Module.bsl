...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаданныйФормат = Неопределено; //lf ghjcnzn vtyz///
	Попытка
		ИмяПервого = ТабличныеДокументы[0].Представление;
		ПозицияТочки = СтрНайти(ИмяПервого,".",НаправлениеПоиска.СКонца);
		Если ПозицияТочки > 0 Тогда
			Расширение = Сред(ИмяПервого,ПозицияТочки+1);
			ЗаданныйФормат = ТипФайлаТабличногоДокумента[Расширение];
		КонецЕсли;
	Исключение
		
	КонецПопытки;
	
	Если НЕ ЗаданныйФормат = Неопределено Тогда
		ФорматыСохранения.НайтиПоЗначению(ТипФайлаТабличногоДокумента["xls"]).Пометка = Истина;
	Иначе
...
&НаКлиенте
Процедура ПрикрепитьАвто() Экспорт
	Прикрепить(Неопределено);
КонецПроцедуры

&НаКлиенте
Процедура Прикрепить(Команда = Неопределено)
...
Процедура ПриОткрытии(Отказ)
	Если НЕ ЗаданныйФормат = Неопределено Тогда
		ПодключитьОбработчикОжидания("ПрикрепитьАвто",0.1,Истина);
		//Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
