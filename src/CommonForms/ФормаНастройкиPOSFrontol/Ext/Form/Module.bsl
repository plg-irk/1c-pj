...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	времВыгружатьОстатки         = Неопределено;
	...
	Параметры.ПараметрыОборудования.Свойство("ВыгружатьОстатки",  		 времВыгружатьОстатки);
	...
	ВыгружатьОстатки         = ?(времВыгружатьОстатки = Неопределено, Ложь, времВыгружатьОстатки);
	...
	Если времКассы <> Неопределено Тогда
		Для Каждого СтрокаБазы Из времКассы Цикл
			...
			СтрокаТаблицы.ИзШапки = СтрокаБазы.ИзШапки;
			СтрокаТаблицы.КаталогВыгрузки = СтрокаБазы.КаталогВыгрузки;
			СтрокаТаблицы.КаталогЗагрузки = СтрокаБазы.КаталогЗагрузки;
			СтрокаТаблицы.КаталогОперативнойСводки = СтрокаБазы.КаталогОперативнойСводки;
			СтрокаТаблицы.КаталогОбмена = СтрокаБазы.КаталогОбмена;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Для Каждого СтрокаТаблицы Из Кассы Цикл
		Если СтрокаТаблицы.ИзШапки = Истина Тогда
	...
	Для Каждого СтрокаТаблицы Из Кассы Цикл
		Если СтрокаТаблицы.ИзШапки = Ложь И (Не ЗначениеЗаполнено(СтрокаТаблицы.КаталогВыгрузки)) Тогда
			Результат = Ложь;
			ТекстОшибкиОбщий = ТекстОшибкиОбщий + ?(ПустаяСтрока(ТекстОшибкиОбщий), "", Символы.ПС);
			ТекстОшибкиОбщий =ТекстОшибкиОбщий + НСтр("ru='Не заполнены значения полей табличной части'");
		КонецЕсли;
	КонецЦикла;
	...
	Если ПустаяСтрока(ТекстОшибкиОбщий) Тогда
		...
		НовыеЗначениеПараметров.Вставить("ВыгружатьОстатки",         ВыгружатьОстатки);
		...
		Для Каждого СтрокаТаблицы Из Кассы Цикл
			НоваяСтрока = Новый Структура("Активна, НомерPOS, НаименованиеPOS, ИзШапки", СтрокаТаблицы.Активна, СтрокаТаблицы.НомерPOS, СтрокаТаблицы.НаименованиеPOS, СтрокаТаблицы.ИзШапки);
			Если СтрокаТаблицы.ИзШапки Тогда
				НоваяСтрока.Вставить("КаталогВыгрузки",          КаталогВыгрузки);
				НоваяСтрока.Вставить("КаталогЗагрузки",          КаталогЗагрузки);
				НоваяСтрока.Вставить("КаталогОперативнойСводки", КаталогОперативнойСводки);
				НоваяСтрока.Вставить("КаталогОбмена",            КаталогОбмена);
			Иначе
				НоваяСтрока.Вставить("КаталогВыгрузки",          СтрокаТаблицы.КаталогВыгрузки);
				НоваяСтрока.Вставить("КаталогЗагрузки",          СтрокаТаблицы.КаталогЗагрузки);
				НоваяСтрока.Вставить("КаталогОперативнойСводки", СтрокаТаблицы.КаталогОперативнойСводки);
				НоваяСтрока.Вставить("КаталогОбмена",            СтрокаТаблицы.КаталогОбмена);
			КонецЕсли;
...
Процедура ТестУстройства(Команда)
	...
	времПараметрыУстройства.Вставить("ВыгружатьОстатки",         ВыгружатьОстатки);
	
	времКассы = Новый Массив();
	Для Каждого СтрокаТаблицы Из Кассы Цикл
		Если СтрокаТаблицы.ИзШапки = Ложь Тогда
			
			НоваяСтрока = Новый Структура("Активна,НомерPOS, НаименованиеPOS,ИзШапки, КаталогВыгрузки, КаталогЗагрузки, КаталогОперативнойСводки, КаталогОбмена",
			СтрокаТаблицы.Активна, СтрокаТаблицы.НомерPOS, СтрокаТаблицы.НаименованиеPOS, СтрокаТаблицы.ИзШапки, СтрокаТаблицы.КаталогВыгрузки, СтрокаТаблицы.КаталогЗагрузки,СтрокаТаблицы.КаталогОперативнойСводки, СтрокаТаблицы.КаталогОбмена, );
			
		Иначе
			НоваяСтрока = Новый Структура("Активна,НомерPOS, НаименованиеPOS, ИзШапки, КаталогВыгрузки, КаталогЗагрузки, КаталогОперативнойСводки, КаталогОбмена",
			СтрокаТаблицы.Активна, СтрокаТаблицы.НомерPOS, СтрокаТаблицы.НаименованиеPOS,СтрокаТаблицы.ИзШапки, ФорматОбмена, КаталогВыгрузки, КаталогЗагрузки, КаталогОперативнойСводки, КаталогОбмена);
...
Процедура КассыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока и Не Копирование Тогда
		...
		Элемент.ТекущиеДанные.ИзШапки = Истина;
	КонецЕсли;
...
&НаКлиенте
Процедура КаталогОперативнойСводкиНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		КаталогОперативнойСводки = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КаталогОперативнойСводкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("КаталогОперативнойСводкиНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, КаталогОперативнойСводки, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура КаталогОбменаНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		КаталогОбмена = ВыбранныеФайлы[0];
	КонецЕсли;
...
&НаКлиенте
Процедура КассыКаталогВыгрузкиНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		Элементы.Кассы.ТекущиеДанные.КаталогВыгрузки = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("КассыКаталогВыгрузкиНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, Элемент, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогЗагрузкиНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		Элементы.Кассы.ТекущиеДанные.КаталогЗагрузки = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("КассыКаталогЗагрузкиНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, Элемент, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогОперативнойСводкиНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		Элементы.Кассы.ТекущиеДанные.КаталогОперативнойСводки = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогОперативнойСводкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("КассыКаталогОперативнойСводкиНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, Элемент, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура КассыКаталогОбменаНачалоВыбораЗавершение(ВыбранныеФайлы, Параметры) Экспорт
	
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") И ВыбранныеФайлы.Количество() > 0 Тогда
		Элементы.Кассы.ТекущиеДанные.КаталогОбмена = ВыбранныеФайлы[0];
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КассыКаталогОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Оповещение = Новый ОписаниеОповещения("КассыКаталогОбменаНачалоВыбораЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыборФайла(Оповещение, Элемент, "ВыборКаталога");
	СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура КассыИзШапкиПриИзменении(Элемент)
	...
	Если НЕ ТекущиеДанные.ИзШапки Тогда
		
		Если ПустаяСтрока(ТекущиеДанные.КаталогВыгрузки) И ЗначениеЗаполнено(КаталогВыгрузки) Тогда
			ТекущиеДанные.КаталогВыгрузки = КаталогВыгрузки;
		ИначеЕсли ПустаяСтрока(ТекущиеДанные.КаталогВыгрузки) Тогда
			ТекущиеДанные.КаталогВыгрузки = "Не указан";
		КонецЕсли;
		
		Если ПустаяСтрока(ТекущиеДанные.КаталогЗагрузки) И ЗначениеЗаполнено(КаталогЗагрузки) Тогда
			ТекущиеДанные.КаталогЗагрузки = КаталогЗагрузки;
		ИначеЕсли ПустаяСтрока(ТекущиеДанные.КаталогЗагрузки) Тогда
			ТекущиеДанные.КаталогЗагрузки = "Не указан";
		КонецЕсли;
		
		Если ПустаяСтрока(ТекущиеДанные.КаталогОперативнойСводки) И ЗначениеЗаполнено(КаталогОперативнойСводки) Тогда
			ТекущиеДанные.КаталогОперативнойСводки = КаталогОперативнойСводки;
		ИначеЕсли ПустаяСтрока(ТекущиеДанные.КаталогОперативнойСводки) Тогда
			ТекущиеДанные.КаталогОперативнойСводки = "Не указан";
		КонецЕсли;
		
		Если ПустаяСтрока(ТекущиеДанные.КаталогОбмена) И ЗначениеЗаполнено(КаталогОбмена) Тогда
			ТекущиеДанные.КаталогОбмена = КаталогОбмена;
		ИначеЕсли ПустаяСтрока(ТекущиеДанные.КаталогОбмена) Тогда
			ТекущиеДанные.КаталогОбмена = "Не указан";
		КонецЕсли;
