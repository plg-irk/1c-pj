...
Процедура ЗапуститьТест(Команда)
	...
	Для Каждого СтрокаКомпьютеры Из Компьютеры Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаКомпьютеры.Компьютер) Тогда
			...
			Сообщение.Текст = НСтр("ru = 'Необходимо выбрать компьютер'");
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Сценарий) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Необходимо выбрать сценарий'");
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Тест) Тогда
			Предупреждение(НСтр("ru = 'В данный момент уже выполняется тест,'") + Символы.ПС + НСтр("ru = 'следует дождаться его окончания.'"), 30, "Ошибка");
			Возврат;
		КонецЕсли;
