...
Процедура ПриВыполненииТестирования()
	...
	Попытка
		...
		Если СостояниеТеста.ПаузаПослеЗапускаВРМ И СостояниеТеста.ВРМЗапущены Тогда
			...
			Предупреждение(НСтр("ru = 'ВРМ запущены. Нажмите ""ОК"", для продолжения выполнения теста.'"));
...
Процедура Инициализировать()
	...
	Если ТестИСценарий = Неопределено Тогда
		Предупреждение(НСтр("ru = 'Текущий тест не обнаружен'"), , НСтр("ru = 'Ошибка'"));
		Закрыть();
	КонецЕсли;
...
Процедура Стоп(Команда)
	...
	Текст = НСтр("ru = 'Прервать выполняющийся тест?'");
