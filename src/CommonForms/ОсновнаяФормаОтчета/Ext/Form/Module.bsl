...
Функция ПолучитьКартинкуДиаграммы(ИмяТипаДиаграммы)
...
Процедура Подключаемый_ОповещениеВыбораТипаДиаграммы(ВыбранныйТипДиаграммы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйТипДиаграммы <> Неопределено Тогда
		...
		Элементы.ДекорацияТипДиаграммы.Картинка = ПолучитьКартинкуДиаграммы(ВыбранныйТипДиаграммы);
...
Процедура НастроитьВидимостьЭлементовФормы()
	...
	Элементы.Редактирование.Видимость = ЕстьРазрешенныйВывод И ЕстьРазрешенноеРедактирование
										И НЕ ПравоПользователя("ЗащитаПечатныхФорм");
...
Процедура СформироватьПриНеобходимости()
	...
	Если НастройкиОтчета.Параметры.СформироватьПриОткрытии Тогда
		...
		КлючеваяОперация = "ФормированиеОтчета" + СтрЗаменить(ЭтотОбъект.КлючОбъекта,"Отчет.","");
		...
		ОчиститьСообщения();
		Если ЭтотОбъект.КлючОбъекта = "Отчет.ПечатьРеестра" Тогда
			Если СформироватьОтчетВФоне() Тогда 
...
Процедура ОбновитьВидТаблицы()
	...
	Если ВидОтчета = 0 Тогда
		Элементы.ДействияФормыВидОтчета.Картинка = БиблиотекаКартинок.Таблица
	ИначеЕсли ВидОтчета = 1 Тогда
...
Процедура ОбновитьТипДиаграммы()
	...
	Элементы.ГруппаТипДиаграммы.Картинка = ПолучитьКартинкуДиаграммы(ТипДиаграммыОтчета);
	Элементы.ДекорацияТипДиаграммы.Картинка = Элементы.ГруппаТипДиаграммы.Картинка;
...
Процедура Подключаемый_ФоновоеЗаданиеПроверитьНаКлиенте()
	...
	Иначе
		...
		ПодключитьОбработчикОжидания("Подключаемый_ФоновоеЗаданиеПроверитьНаКлиенте", ПараметрыОбработчика.ТекущийИнтервал, Истина);
	КонецЕсли;
...
Процедура ОбновитьБыстрыеНастройки(Вариант = Неопределено)
	...
	Для Каждого СтрокаДерева Из ДеревоОтбор Цикл
		Если НЕ Настройки.Отбор.ДоступныеПоляОтбора.Элементы.Найти(СтрокаДерева.ЛевоеЗначение) = Неопределено Тогда		
			СтрокаДерева.ДоступныеВидыСравнения = Настройки.Отбор.ДоступныеПоляОтбора.Элементы.Найти(СтрокаДерева.ЛевоеЗначение).ДоступныеВидыСравнения;	
		КонецЕсли;
	КонецЦикла;
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	ИмяФайла = ?(ПолучитьЗначениеПараметраСтруктуры(НастройкиСохранения, "ИмяФайла")=Неопределено, ЭтотОбъект.Заголовок, НастройкиСохранения.ИмяФайла);
	Если ЕстьНачалоПериода И ЕстьКонецПериода Тогда
		ИмяФайла = ИмяФайла+" ("+Формат(ЭтотОбъект.НачалоПериода,"ДФ=ddMMyy")+"-"+Формат(ЭтотОбъект.КонецПериода,"ДФ=ddMMyy")+")";
	ИначеЕсли ЕстьНачалоПериода Тогда
		ИмяФайла = ИмяФайла+" ("+Формат(ЭтотОбъект.КонецПериода,"ДФ=ddMMyy")+")";
	Иначе
		ИмяФайла = ИмяФайла;
	КонецЕсли;
...
Процедура Подключаемый_Команда(Команда)
	...
	Результат = Ложь;
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РассылкаОтчетов") И СтрНачинаетсяС(Команда.Имя, "РассылкаОтчетов") Тогда
	...
	Иначе
		ОтображениеСостояния = Элементы.ОтчетТабличныйДокумент.ОтображениеСостояния;
		Если ОтображениеСостояния.Видимость = Ложь ИЛИ ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.НеИспользовать Тогда
			ОбновитьНастройкиСКД(,, Истина);
			ИнтеграцияЕГАИСКлиент.ОбработчикКоманды(ЭтотОбъект, Команда, Результат);
		Иначе
			ВывестиСообщение(НСтр("ru = 'Отчет не сформирован!'"), ЭтотОбъект);
		КонецЕсли;
...
Процедура ВыбратьВариантНаСервере(ИмяВарианта)
	...
	ОтборСКД = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы;
	ОтборСКД.Очистить();
	...
	ВидПериода = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьВидПериода(КлючОбъекта, КлючТекущегоВарианта);
	
	ТиповойВариантНастроек = СхемаКомпоновкиДанных.ВариантыНастроек.Найти(ЭтотОбъект.КлючТекущегоВарианта);
	Если ТиповойВариантНастроек = Неопределено Тогда
		// Пользовательский вариант настроек
		Настройки = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьНастройки(КлючОбъекта, КлючТекущегоВарианта);
	...
	СтруктураПериода = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьПериод(КлючОбъекта, КлючТекущегоВарианта);
	...
	НастройкиПриемник = Отчет.КомпоновщикНастроек.ПолучитьНастройки();
	
	//Для Каждого НастройкиПриемник.Отбор.
	
	//НастройкиПриемник.Отбор.Элементы.Очистить();
	//СкопироватьОтборКомпоновкиДанных(Отчет.КомпоновщикНастроек.Настройки, ТекНастройки);
	//КомпоновкаДанныхКлиентСервер.СкопироватьОтборКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиПриемник, ТекНастройки, Истина);
	КомпоновкаДанныхКлиентСервер.ОбъединитьОтборКомпоновкиДанных(СхемаКомпоновкиДанных, НастройкиПриемник, ТекНастройки);
	
	Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиПриемник);
...
Процедура ОткрытьНастройкиОтчета(Команда)
	...
	СтруктураПараметров.Вставить("КлючВарианта",          СокрЛП(ЭтотОбъект.КлючТекущегоВарианта));
	СтруктураПараметров.Вставить("ПредставлениеВарианта", СокрЛП(ЭтотОбъект.ПредставлениеТекущегоВарианта));
...
Процедура КомандаТипДиаграммы(Команда)
	...
	Элементы.ДекорацияТипДиаграммы.Картинка = ПолучитьКартинкуДиаграммы(ТипДиаграммыОтчета);
...
Функция ПечатьНаСервере(Знач ВременныйТабличныйДокумент)
	...
	Пока НЕ КонецШапки Цикл
		Строка = Строка+1;
		КонецШапки = (ОтчетТабличныйДокумент.Область("R" + Формат(Строка,"ЧГ=0") + "C2").Текст = "" И ОтчетТабличныйДокумент.Область("R" + Формат(Строка,"ЧГ=0") + "C1").Текст = "");
	КонецЦикла;
	...
	ЭкземпляровНаСтранице = ПравоПользователя("КоличествоСтраницНаЛисте");
	
	Если ЭкземпляровНаСтранице<>0 И ЭкземпляровНаСтранице<>1 И ЭкземпляровНаСтранице<>2 Тогда
		ЭкземпляровНаСтранице=0;
	КонецЕсли;
	
	ТекстСлева = Символы.ПС + НСтр("ru='Отчет %1 сформирован'") + " [&Дата]-[&Время], " + ТекущийПользователь.Наименование;
	ДокументДляПечати.ВерхнийКолонтитул.ТекстСлева = СтрШаблон(ТекстСлева, ЭтотОбъект.Заголовок);
	ДокументДляПечати.ВерхнийКолонтитул.Шрифт      = Новый Шрифт(ДокументДляПечати.ВерхнийКолонтитул.Шрифт,,6);
	ДокументДляПечати.ВерхнийКолонтитул.Выводить   = ПравоПользователя("ПодписьПриПечатиОтчетов");
	ДокументДляПечати.НижнийКолонтитул.ТекстСправа = "[&НомерСтраницы]";
	ДокументДляПечати.НижнийКолонтитул.Выводить    = ПравоПользователя("НумерацияСтраницОтчетов");
	ДокументДляПечати.НижнийКолонтитул.Шрифт       = Новый Шрифт(ДокументДляПечати.ВерхнийКолонтитул.Шрифт,,6);
	
	ДокументДляПечати.ЭкземпляровНаСтранице     = ЭкземпляровНаСтранице;
...
// Обработчик события возникающего на клиенте при выполнении команды "Заполнить фильтр номенклатуры из буфера обмена".
//
// Параметры:
//  Команда - КомандаФормы - Команда, в которой возникло данное событие.
//
&НаКлиенте
Процедура ЗаполнитьФильтрПоНоменклатуреИзБуфераОбмена(Команда)
	
	ТекущиеДанные = Элементы.Отбор.ТекущиеДанные;
	ЭлементыОтбора = ОтчетыПлатформаКлиент.ПолучитьЭлементыДляДерева(ТекущиеДанные, Отбор);
	ЭлементОтбораНоменклатура = Неопределено;
	Для Каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		Если ЭлементОтбора.Заголовок = "Номенклатура" Тогда
			ЭлементОтбораНоменклатура = ЭлементОтбора;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	                  
	Если НЕ ЭлементОтбораНоменклатура=Неопределено Тогда
		
		#Если Клиент Тогда
	     	ОбъектШТМЛ = Новый COMОбъект("htmlfile");
		 	Буфер = ОбъектШТМЛ.ParentWindow.ClipboardData.Getdata("Text");
		#Иначе
			Буфер=Неопределено;
		#КонецЕсли
		...
		//ВывестиСообщение("Буфер: ("+СтрДлина(Буфер)+")"+буфер+"КонецБуфера");
		МассивСообщений=Новый Массив;
		СписокНоменклатуры = ПолучитьСписокНоменклатурыИзСтроки(Буфер,МассивСообщений);
		Если СписокНоменклатуры.Количество()>0 Тогда
			ЭлементОтбораНоменклатура.Использование = Истина;
			ЭлементОтбораНоменклатура.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии;
			ЭлементОтбораНоменклатура.ПравоеЗначение = СписокНоменклатуры;
			Элементы.Отбор.ТекущаяСтрока = ЭлементОтбораНоменклатура.ПолучитьИдентификатор();
		КонецЕсли;
	КонецЕсли;
	
КонецПроцедуры //ДобавитьГруппуОтборов()
&НаСервере
Функция ПолучитьСписокНоменклатурыИзСтроки(Строка,МассивСообщений=Неопределено)
	...
	СписокНоменклатуры = Новый СписокЗначений;
		...
		//с.ЗагрузитьЗначения(МассивНоменклатуры);
	
	Строка = СтрЗаменить(Строка,Символы.Таб,Символы.ПС);
	Строка = СтрЗаменить(Строка,Символы.ВТаб,Символы.ПС);
	...
	Для ч = 1 по Текст.КоличествоСтрок() Цикл
		...
		КонецЕсли;	
		Если НЕ ЗначениеЗаполнено(Товар) Тогда
			Товар = Справочники.Номенклатура.НайтиПоНаименованию(Значение);
		КонецЕсли;	
		...
		СписокНоменклатуры.Добавить(Товар);
...
//
&НаСервере
Функция УстановитьСтандартныеПараметрыНаСервере() Экспорт
	
	УправлениеПечатьюПлатформа.УстановитьСтандартныеПараметрыПечати(ОтчетТабличныйДокумент.ИмяПараметровПечати,ОтчетТабличныйДокумент);
	
КонецФункции //УстановитьСтандартныеПараметрыНаСервере()
...
Процедура РассчитатьПоказатели(ТекущаяКоманда = "РассчитатьСумму")
	// Расчет показателей.
	ПараметрыРасчета = ПараметрыРасчетаПоказателейЯчеек(ОтчетТабличныйДокумент);
	РасчетныеПоказатели = РасчетныеПоказателиЯчеекИсполняемая(
	...
	Элементы.ГруппаВидПоказателя.Заголовок = Элементы[ТекущаяКоманда].Заголовок;
	
	
	// Вывод основного показателя.
	Если НЕ ТекущаяКоманда = Неопределено Тогда
		ТекущийПоказатель = РасчетныеПоказатели[СтрЗаменить(ТекущаяКоманда,"Рассчитать","")];
		Показатель = ТекущийПоказатель;
		РазрядностьПоказателя = Мин(СтрДлина(Макс(ТекущийПоказатель, -ТекущийПоказатель) % 1) - 2, 4);
		Элементы.ОтображаемыйПоказатель.ФорматРедактирования = "ЧДЦ=" + РазрядностьПоказателя + "; ЧРГ=' '; ЧН=0";
	КонецЕсли;
КонецПроцедуры
...
Функция КомандыПоказателей()
	...
	КомандыПоказателей.Вставить("РассчитатьСумму", "Сумму");
...
&НаКлиенте
Функция РасчетныеПоказателиЯчеекИсполняемая(Знач ТабличныйДокумент, ВыделенныеОбласти)
	...
	РасчетныеПоказатели.Вставить("Сумму", 0);
			...
			Для НомерСтроки = ВыделеннаяОбластьВерх По ВыделеннаяОбластьНиз Цикл
				...
				Если Ячейка.Видимость = Истина Тогда
					...
					Если ТипЗнч(Число) = Тип("Число") Тогда
						...
						РасчетныеПоказатели.Сумму = РасчетныеПоказатели.Сумму + Число;
	...
	Если РасчетныеПоказатели.КоличествоЧисловых > 0 Тогда
		РасчетныеПоказатели.Среднее = РасчетныеПоказатели.Сумму / РасчетныеПоказатели.КоличествоЧисловых;
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ОТБОРЫ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ СТРОКИ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ПОКАЗАТЕЛИ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ КОЛОНКИ
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОтчетТабличныйДокумент.ОтображатьЗаголовки = Истина;
	...
	Если Не РежимВариантаОтчета Тогда
		...
		КлючВариантаПоУмолчанию = ХранилищаНастроек.ХранилищеВариантовОтчетов.ПолучитьКлючВариантаОтчета(КлючОбъекта);
		
		Если Не КлючВариантаПоУмолчанию = Неопределено Тогда
			ЭтотОбъект.КлючТекущегоВарианта = КлючВариантаПоУмолчанию;
		КонецЕсли;
	...
	Иначе
		...
		// !!!_ ни в одном отчете не прописаны исключения
		Попытка
		...
		Если НастройкиОтчета.Свойство("ОтменитьОткрытие") Тогда
			ВывестиСообщение(НастройкиОтчета.ОтменитьОткрытие, ЭтотОбъект,,, Отказ);
			Возврат;
		КонецЕсли;
		...
		Попытка
			...
			Если СтруктураОграничений.Свойство("Группировки") Тогда
				СтруктураОграничений.Вставить("Строки");
				СтруктураОграничений.Вставить("Колонки");
			КонецЕсли;
			
			Для Каждого СтрокаОграничения Из СтруктураОграничений Цикл
				Если НастройкиОтчета.Видимость.Свойство(СтрокаОграничения.Ключ) Тогда
					НастройкиОтчета.Видимость.Удалить(СтрокаОграничения.Ключ);
				КонецЕсли;
			КонецЦикла;
	...
	Для Каждого ТекущийТипДиаграммы Из ТипДиаграммы Цикл
		...
		НовоеОформление.Картинка   = ПолучитьКартинкуДиаграммы(ИмяТипаДиаграммы);
		
		СписокВыбора.Добавить(ИмяТипаДиаграммы, СокрЛП(ТекущийТипДиаграммы),, НовоеОформление.Картинка);
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(Настройки)
	...
	ХранилищаНастроек.ХранилищеВариантовОтчетов.СохранитьКлючВариантаОтчета(КлючОбъекта, КлючТекущегоВарианта);
...
&НаСервере
Процедура ПараметрыЗначениеПриИзмененииНаСервере()
	
	ТекущиеДанные = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.ПолучитьОбъектПоИдентификатору(Элементы.Параметры.ТекущаяСтрока);
	
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	Попытка
		ОтчетОбъект.ПриИзмененииЗначенияПараметра(ЭтотОбъект, ТекущиеДанные);
	Исключение
	КонецПопытки;
...
Процедура ПараметрыЗначениеПриИзменении(Элемент)
	...
	ПараметрыЗначениеПриИзмененииНаСервере();
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	ПолучитьИзСписка = ПолучитьЗначениеРасшифровки(Расшифровка);
	
	Если НЕ ПолучитьИзСписка = Неопределено Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыОтбора = Новый Структура("ТекущаяСтрока", ПолучитьИзСписка.Ссылка);
		ОткрытьФорму(ПолучитьИзСписка.Форма, ПараметрыОтбора, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Функция ПолучитьЗначениеРасшифровки(Расшифровка)
	...
	КолекцияРасшифровки = ПолучитьИзВременногоХранилища(ДанныеРасшифровки);
	КоллекцияПолей = КолекцияРасшифровки.Элементы[Расшифровка].ПолучитьПоля();
	Если КоллекцияПолей.Количество()>0 Тогда
		ТипЗначения = ТипЗнч(КоллекцияПолей[0].Значение);
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначения);
		Если НЕ ОбъектМетаданных = Неопределено И ОбъектМетаданных = Метаданные.Справочники.Номенклатура Тогда
			Результат = Новый Структура;
			Результат.Вставить("Форма", "Справочник."+ОбъектМетаданных.Имя+".ФормаСписка");
			Результат.Вставить("Ссылка", КоллекцияПолей[0].Значение);
		КонецЕсли;
		Возврат Результат;
	КонецЕсли;
...
Процедура ОтборВидСравненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Строка = Элементы.Отбор.ТекущиеДанные;
	Если Строка = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ОтборВидСравнения.РежимВыбораИзСписка = (Строка.ДоступныеВидыСравнения <> Неопределено);
	Если Строка.ДоступныеВидыСравнения <> Неопределено Тогда 
		Список = Элементы.ОтборВидСравнения.СписокВыбора;
		Список.Очистить();
		
		Для Каждого ВидыСравнения Из Строка.ДоступныеВидыСравнения Цикл 
			ЗаполнитьЗначенияСвойств(Список.Добавить(), ВидыСравнения);
		КонецЦикла;
	ИначеЕсли Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы().Количество() = 1 и Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы()[0] = Тип("Булево") Тогда
		Элементы.ОтборВидСравнения.РежимВыбораИзСписка = Истина;
		Список = Элементы.ОтборВидСравнения.СписокВыбора;
		Список.Очистить();
		Список.Добавить(ВидСравненияКомпоновкиДанных.Равно);
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеРавно);
	ИначеЕсли Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы().Количество() = 1 и Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы()[0] = Тип("Число") Тогда
		Элементы.ОтборВидСравнения.РежимВыбораИзСписка = Истина;
		Список = Элементы.ОтборВидСравнения.СписокВыбора;
		Список.Очистить();
		Список.Добавить(ВидСравненияКомпоновкиДанных.Равно);
	...
	ИначеЕсли Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы().Количество() = 1 и Элементы.ОтборПравоеЗначение.ОграничениеТипа.Типы()[0] = Тип("Строка") Тогда
		Элементы.ОтборВидСравнения.РежимВыбораИзСписка = Истина;
		Список = Элементы.ОтборВидСравнения.СписокВыбора;
		Список.Очистить();
		Список.Добавить(ВидСравненияКомпоновкиДанных.Равно);
