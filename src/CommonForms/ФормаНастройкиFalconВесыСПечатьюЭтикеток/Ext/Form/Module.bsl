...
Процедура ОбновитьНастраиваемыйИнтерфейс(ОписаниеИнтерфейса, ДополнительныеДействия, ПервыйЗапуск)
	...
	ИндексГруппы = 0;
	КоличествоСтраниц = 0;
	ТекущаяСтраница = Элементы.Вставить("ОсновнаяСтраница", Тип("ГруппаФормы"), Элементы.Страницы, Элементы.СтраницаДоступККартинкамПоFTP);
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если ЧтениеXML.Имя = "Page" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				Если КоличествоСтраниц > 1 Тогда
					...
					ТекущаяСтраница = Элементы.Вставить("Страница" + КоличествоСтраниц, Тип("ГруппаФормы"), Элементы.Страницы, Элементы.СтраницаДоступККартинкамПоFTP);
					БазоваяГруппа   = Неопределено;
				КонецЕсли;
	...
	Если НЕ БазоваяГруппа = Неопределено И БазоваяГруппа.ПодчиненныеЭлементы.Количество()>0 Тогда
		...
		Элемент = Элементы.Добавить("ДлинаКраткогоНаименования", Тип("ПолеФормы"), БазоваяГруппа);
		Элемент.Вид = ВидПоляФормы.ПолеВвода;
		Элемент.АвтоМаксимальнаяШирина = Ложь;
		Элемент.ПутьКДанным    = "ДлинаКраткогоНаименования";
		Элемент.Подсказка      = "Длина краткого наименования";
		Элемент.ТолькоПросмотр = ТолькоЧтение;
		МенеджерОборудованияВызовСервера.ПодготовитьЭлементУправления(Элемент);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.ПараметрыОборудования.Свойство("FTPКаталогКартинокГруппТоваров", FTPКаталогКартинокГруппТоваров);
	Параметры.ПараметрыОборудования.Свойство("FTPКаталогКартинокТоваров",      FTPКаталогКартинокТоваров);
	Параметры.ПараметрыОборудования.Свойство("АдресСервераFTP",                АдресСервераFTP);
	Параметры.ПараметрыОборудования.Свойство("ПарольFTP",                      ПарольFTP);
	Параметры.ПараметрыОборудования.Свойство("ПассивноеСоединениеFTP",         ПассивноеСоединениеFTP);
	Параметры.ПараметрыОборудования.Свойство("ПользовательFTP",                ПользовательFTP);
	Параметры.ПараметрыОборудования.Свойство("ПортFTP",                        ПортFTP);
	Параметры.ПараметрыОборудования.Свойство("ДлинаКраткогоНаименования",      ДлинаКраткогоНаименования);
	...
	Если НЕ ЗначениеЗаполнено(ПользовательFTP) Тогда
		ПользовательFTP = "admin";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПарольFTP) Тогда
		ПарольFTP = "admin";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(FTPКаталогКартинокГруппТоваров) Тогда
		FTPКаталогКартинокГруппТоваров = "/opt/pcscale/files/img/common";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(FTPКаталогКартинокТоваров) Тогда
		FTPКаталогКартинокТоваров = "/opt/pcscale/files/img/plu";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПортFTP) Тогда
		ПортFTP = 21;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДлинаКраткогоНаименования) Тогда
		ДлинаКраткогоНаименования = 32;
	КонецЕсли;
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	ВремНастройки.ПараметрыОборудования.Вставить("FTPКаталогКартинокГруппТоваров", FTPКаталогКартинокГруппТоваров);
	ВремНастройки.ПараметрыОборудования.Вставить("FTPКаталогКартинокТоваров",      FTPКаталогКартинокТоваров);
	ВремНастройки.ПараметрыОборудования.Вставить("АдресСервераFTP",                АдресСервераFTP);
	ВремНастройки.ПараметрыОборудования.Вставить("ПарольFTP",                      ПарольFTP);
	ВремНастройки.ПараметрыОборудования.Вставить("ПассивноеСоединениеFTP",         ПассивноеСоединениеFTP);
	ВремНастройки.ПараметрыОборудования.Вставить("ПользовательFTP",                ПользовательFTP);
	ВремНастройки.ПараметрыОборудования.Вставить("ПортFTP",                        ПортFTP);
	ВремНастройки.ПараметрыОборудования.Вставить("ДлинаКраткогоНаименования",      ДлинаКраткогоНаименования);
...
&НаКлиенте
Процедура ВыгрузитьМагазин(Команда)
	
	ОчиститьСообщения();
	
	СтрокаТаблицы = Новый Структура;
	СтрокаТаблицы.Вставить("Код", 1);
	СтрокаТаблицы.Вставить("Наименование", ПодключаемоеОборудованиеOfflineВызовСервера.ПолучитьИмяМагазинаДляЭтикетки(Идентификатор));
	
	ВходныеПараметры = Новый Массив;
	ВходныеПараметры.Добавить(СтрокаТаблицы);
	//ВыходныеПараметры   = Неопределено;
	//ПараметрыУстройства = ПолучитьНастройки().ПараметрыОборудования;
	
	Оповещение = Новый ОписаниеОповещения("ВыгрузитьМагазинЗавершение", ЭтотОбъект);
	МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(Оповещение, Идентификатор, "ВыгрузитьНазваниеМагазина", ВходныеПараметры);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьМагазинЗавершение(РезультатВыполнения, Параметры) Экспорт
...
&НаКлиенте
Процедура ТестПодключенияКFTP(Команда)
	
	Отказ = Ложь;
	
	Если Не ЗначениеЗаполнено(АдресСервераFTP) Тогда
		//АдресСервераFTP = АдресСервераFTP
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(FTPКаталогКартинокТоваров) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Каталог для картинок товаров'")),
		,
		"FTPКаталогКартинокТоваров",
		,
		Отказ);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(FTPКаталогКартинокГруппТоваров) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Поле", "Заполнение", НСтр("ru = 'Каталог для картинок групп товаров'")),
		,
		"FTPКаталогКартинокГруппТоваров",
		,
		Отказ);
	КонецЕсли;
	...
	Отказ = Ложь;
	Состояние(НСтр("ru = 'Проверка связи с весами по FTP.'"),
					,
					НСтр("ru = 'Выполняется тестирование FTP. Подождите...'"));
				ТестСвязиОбменаЧерезFTPНаСервере(Отказ);
...
Процедура ТестСвязиОбменаЧерезFTPНаСервере(Отказ)
	...
	Попытка
		ОбменСКонтрагентамиСлужебный.ПодготовитьПутьFTP(FTPКаталогКартинокТоваров);
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Тест подключения к FTP пройден");
...
&НаКлиенте
Процедура ИзIP(Команда)
	
	АдресСервераFTP = ЭтотОбъект.P_IPAddress;
