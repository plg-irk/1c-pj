...
Процедура ОбновитьПанельОтчетовНаСервере(Знач Событие = "")
	...
	// +РАРУС
	Если ЗначениеЗаполнено(СтрокаПоиска) И ИскатьВоВсехРазделах = 1 Тогда
		ЗаголовокПодсистемы = НСтр("ru = 'Отчеты всех разделов'");
	Иначе
		ЗаголовокПодсистемы = СписокЗаголовковСистемы.НайтиПоЗначению(ТекущийРазделСсылка);
		ЗаголовокПодсистемы = ?(ЗаголовокПодсистемы = "", Заголовок, ЗаголовокПодсистемы.Представление);
	КонецЕсли;
	
	Если РежимНастройки Тогда
		Заголовок = ЗаголовокПодсистемы + СуффиксРежимаНастройки;
	Иначе
		Заголовок = ЗаголовокПодсистемы;
	КонецЕсли;
	// -РАРУС
...
Процедура ЗаполнитьИнформациюОПодсистемахИУстановитьЗаголовок()
	...
	// +РАРУС
	СписокВыбораРазделов = Элементы.ТекущийРазделСсылка.СписокВыбора;
	СписокЗаголовковСистемы.Очистить();
	// -РАРУС
	Для Каждого СтрокаРаздел Из ВсеРазделы Цикл
		...
		// +РАРУС
		СписокВыбораРазделов.Добавить(СтрокаРаздел.РазделСсылка, СтрокаРаздел.Представление);
		// -РАРУС
		...
		// +РАРУС
		СписокЗаголовковСистемы.Добавить(СтрокаРаздел.РазделСсылка, СтрокаРаздел.ПолноеПредставление);
		// -РАРУС
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения)
	...
	// +РАРУС
	|	И (ВариантыРазмещение.Ссылка.Автор = &ТекущийПользователь
	|			ИЛИ ВариантыРазмещение.Ссылка.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.БезОграничения)
	|			ИЛИ (ВариантыРазмещение.Ссылка.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоПользователю) И ВариантыРазмещение.Ссылка.ОбъектДоступа = &ТекущийПользователь)
	|			ИЛИ (ВариантыРазмещение.Ссылка.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоПодразделениюКомпании) И ВариантыРазмещение.Ссылка.ОбъектДоступа = &ПодразделениеКомпании)
	|			ИЛИ (ВариантыРазмещение.Ссылка.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоГруппеПользователей) И ВариантыРазмещение.Ссылка.ОбъектДоступа В 
	|		(ВЫБРАТЬ
	|			ГруппыПользователейСостав.Ссылка
	|		ИЗ
	|			Справочник.ГруппыПользователей.Состав КАК ГруппыПользователейСостав
	|		ГДЕ
	|			ГруппыПользователейСостав.Ссылка = &ТекущийПользователь)))
	// -РАРУС
	...
	|ГДЕ
	|	НЕ(НЕ втВарианты.Использование = ИСТИНА
	|				И НЕ втВарианты.Использование ЕСТЬ NULL )
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Для Каждого ТекущаяСтрока Из ТаблицаРезультат Цикл
		...
		//Мальцев +++
		//Если ИмяОтчета = ТекущаяСтрока.ИмяОтчета Тогда
		//	ТекущаяСтрока.Наименование = ТекущаяСтрока.НаименованиеОтчета + ": "+ТекущаяСтрока.Наименование;
		//Иначе
		//	ТекущаяСтрока.Наименование = ТекущаяСтрока.НаименованиеОтчета;
		//	ИмяОтчета = ТекущаяСтрока.ИмяОтчета;
		//КонецЕсли;
	...
	ТаблицаРезультат.Сортировать("ПодсистемаПриоритет, Наименование");
	// -РАРУС
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Иначе
		...
		ГруппаОтступаИмя   = "ГруппаОтступаВарианта_" + Формат(ВыведеноЭлементовВариантов, "ЧГ=0");
		КартинкаОтступаИмя = "КартинкаОтступаВарианта_" + Формат(ВыведеноЭлементовВариантов, "ЧГ=0");
		
		ГруппаВарианта = Элементы.Вставить(ГруппаОтступаИмя, Тип("ГруппаФормы"), ВГруппу);
		ГруппаВарианта.Вид                      = ВидГруппыФормы.ОбычнаяГруппа;
		ГруппаВарианта.Группировка              = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
		...
		// Картинка.
		КартинкаОтступа = Элементы.Вставить(КартинкаОтступаИмя, Тип("ДекорацияФормы"), ГруппаВарианта);
		ЗаполнитьЗначенияСвойств(КартинкаОтступа, ПараметрыЗаполнения.Шаблоны.КартинкаОтступа);
		КартинкаОтступа.Ширина = 1;
	...
	Иначе
		// +РАРУС
		Надпись.Подсказка = НСтр("ru = 'Открыть отчет:'") + " <" + Вариант.НаименованиеОтчета 
			+ ">, " + НСтр("ru = 'с вариантом настроек:'") + " <" + Вариант.НаименованиеВарианта + ">";
		// -РАРУС
	КонецЕсли;
...
Процедура ОпределитьСоставПодсказкиВарианта(ПараметрыЗаполнения, Вариант, СоставПодсказки, Надпись)
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		Если ПараметрыПодсветки <> Неопределено Тогда
			// +РАРУС
			Если ПараметрыПодсветки.НаименованиеВарианта.КоличествоНайденныхСлов > 0 ИЛИ ПараметрыПодсветки.НаименованиеОтчета.КоличествоНайденныхСлов > 0 Тогда
				Содержимое = Новый Массив;
				
				ЗаголовокОтчета = СформироватьСтрокуСПодсветкой(ПараметрыПодсветки.НаименованиеОтчета);
				Содержимое.Добавить(ЗаголовокОтчета);
				Содержимое.Добавить(": ");
				ЗаголовокВарианта = СформироватьСтрокуСПодсветкой(ПараметрыПодсветки.НаименованиеВарианта);
				Содержимое.Добавить(ЗаголовокВарианта);
				
				Надпись.Заголовок = Новый ФорматированнаяСтрока(Содержимое);
...
// +РАРУС
&НаКлиенте
Процедура ТекущийРазделСсылкаПриИзменении(Элемент)
