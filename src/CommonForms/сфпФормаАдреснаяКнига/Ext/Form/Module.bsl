...
Функция ПолучитьМассивКонтактов(Справочник, Владелец = Неопределено)
	...
	УстановитьПривилегированныйРежим(Истина); 	
	Если (Справочник = "Контрагенты")  И ЗначениеЗаполнено(Владелец) Тогда
		Запрос.Текст = Запрос.Текст + "
				   |	И Получатели.ВедущийКонтрагент = &Владелец";
	КонецЕсли;	
	УстановитьПривилегированныйРежим(Ложь); 	
	Запрос.Текст = Запрос.Текст + "
	               |
...
Функция ПолучитьВладельца(Справочник)
	...
	Если Справочник = "Контрагенты" Тогда
		ВладелецСправочника = Справочник;
	КонецЕсли;
...
Процедура ДобавитьВнутреннийНомер(Ссылка)
	Если ПустаяСтрока(Ссылка.сфпТекущийВнутреннийНомер) Тогда Возврат; КонецЕсли;
	Попытка
	...
	ОтборНомера.Вставить("Представление", Ссылка.сфпТекущийВнутреннийНомер);
	...
	НоваяСтрока.Вид 			= Справочники.ВидыКонтактнойИнформации.CRM_ТелефонПользователяРабочий;
	НоваяСтрока.Представление	= Ссылка.сфпТекущийВнутреннийНомер;
...
Процедура ОбновитьСостоянияЛиний(МассивЛиний)
	Для Каждого ЭлементМассива Из МассивЛиний Цикл
		...
		Для Каждого НайденнаяСтрока Из МассивСтрок Цикл
			Если ЭлементМассива.LineState = 1 Тогда
				НайденнаяСтрока.Картинка = 0;
			ИначеЕсли ЭлементМассива.LineState = 2 Тогда	
...
Процедура ВладелецПриИзменении(Элемент)
	...
	Для Каждого ЭлементМассива Из МассивКонтактов Цикл
		...
		Если ТекущийСправочник = "Пользователи" Тогда
			НоваяСтрока.ВнутреннийНомер = сфпСофтФонПроСервер.сфпПолучитьЗначениеРеквизита(ЭлементМассива, "сфпТекущийВнутреннийНомер");
		КонецЕсли;
	КонецЦикла;	
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "OnAllLinesInfo" Тогда
		Если ТекущийСправочник = "Пользователи" Тогда
