...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	времПользователь             = Неопределено;
	времПароль                   = Неопределено;
	времБазаИмпорт               = Неопределено;
	времБазаЭкспорт              = Неопределено;
	времБазаТерминала            = Неопределено;
	времПрайсЛистИД              = Неопределено;
	времВыгружатьГруппыПоКоду    = Неопределено;
	времКассы                    = Неопределено;
	времЗагружатьПродажи         = Неопределено;
	времДатаНачалаЗагрузкиЧековИКассовыхСмен = Неопределено;
	...
	Параметры.ПараметрыОборудования.Свойство("Пользователь",             времПользователь);
	Параметры.ПараметрыОборудования.Свойство("Пароль",                   времПароль);
	Параметры.ПараметрыОборудования.Свойство("БазаИмпорт",               времБазаИмпорт);
	Параметры.ПараметрыОборудования.Свойство("БазаЭкспорт",              времБазаЭкспорт);
	Параметры.ПараметрыОборудования.Свойство("БазаТерминала",            времБазаТерминала);
	Параметры.ПараметрыОборудования.Свойство("ПрайсЛистИД",              времПрайсЛистИД);
	Параметры.ПараметрыОборудования.Свойство("ВыгружатьГруппыПоКоду",    времВыгружатьГруппыПоКоду);
	...
	Параметры.ПараметрыОборудования.Свойство("ДатаНачалаЗагрузкиЧековИКассовыхСмен",         времДатаНачалаЗагрузкиЧековИКассовыхСмен);	
	...
	Пользователь             = ?(времПользователь = Неопределено, "", времПользователь);
	Пароль                   = ?(времПароль = Неопределено, "", времПароль);
	БазаИмпорт               = ?(времБазаИмпорт = Неопределено, "import", времБазаИмпорт);
	БазаЭкспорт              = ?(времБазаЭкспорт = Неопределено, "export", времБазаЭкспорт);
	БазаТерминала            = ?(времБазаТерминала = Неопределено, "server", времБазаТерминала);
	ПрайсЛистИД              = ?(времФорматОбмена = Неопределено, 1,     времПрайсЛистИД);
	ВыгружатьГруппыПоКоду    = ?(времФорматОбмена = Неопределено, Истина,     времВыгружатьГруппыПоКоду);
	...
	ДатаНачалаЗагрузкиЧековИКассовыхСмен         = ?(времДатаНачалаЗагрузкиЧековИКассовыхСмен = Неопределено, "", времДатаНачалаЗагрузкиЧековИКассовыхСмен);
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если ПустаяСтрока(ТекстОшибкиОбщий) Тогда
		...
		НовыеЗначениеПараметров.Вставить("Пользователь",             Пользователь);
		НовыеЗначениеПараметров.Вставить("Пароль",                   Пароль);
		НовыеЗначениеПараметров.Вставить("БазаИмпорт",               БазаИмпорт);
		НовыеЗначениеПараметров.Вставить("БазаЭкспорт",              БазаЭкспорт);
		НовыеЗначениеПараметров.Вставить("БазаТерминала",            БазаТерминала);
		НовыеЗначениеПараметров.Вставить("ПрайсЛистИД",              ПрайсЛистИД);
		НовыеЗначениеПараметров.Вставить("ВыгружатьГруппыПоКоду",    ВыгружатьГруппыПоКоду);
		...
		НовыеЗначениеПараметров.Вставить("ДатаНачалаЗагрузкиЧековИКассовыхСмен",         ДатаНачалаЗагрузкиЧековИКассовыхСмен);
...
Процедура ТестУстройства(Команда)
	...
	времПараметрыУстройства.Вставить("Сервер",           Сервер);
	времПараметрыУстройства.Вставить("Пользователь",     Пользователь);
	времПараметрыУстройства.Вставить("Пароль",           Пароль);
	времПараметрыУстройства.Вставить("БазаИмпорт",       БазаИмпорт);
	времПараметрыУстройства.Вставить("БазаЭкспорт",      БазаЭкспорт);
	времПараметрыУстройства.Вставить("БазаТерминала",    БазаТерминала);
	времПараметрыУстройства.Вставить("ЗагружатьПродажи", ЗагружатьПродажи);
	времПараметрыУстройства.Вставить("ДатаНачалаЗагрузкиЧековИКассовыхСмен", ДатаНачалаЗагрузкиЧековИКассовыхСмен);
	времПараметрыУстройства.Вставить("ВыгружатьТовары",  ВыгружатьТовары);
	времПараметрыУстройства.Вставить("ВыгружатьСкидки",  ВыгружатьСкидки);
	
	времКассы = Новый Массив();
	Для Каждого СтрокаТаблицы Из Кассы Цикл
		
		НоваяСтрока = Новый Структура("Активна, НомерPOS, НаименованиеPOS",
		СтрокаТаблицы.Активна, СтрокаТаблицы.НомерPOS, СтрокаТаблицы.НаименованиеPOS);
...
&НаКлиенте
Процедура СопоставлениеСкидок(Команда)
	
	Если ПустаяСтрока(БазаТерминала) Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнено поле База терминала.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"БазаТерминала");
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Идентификатор", Идентификатор);
	ПараметрыФормы.Вставить("БазаТерминала", БазаТерминала);
	ОткрытьФорму("Обработка.СопоставлениеСкидокСуперМаг.Форма.Форма", ПараметрыФормы, ЭтотОбъект);
