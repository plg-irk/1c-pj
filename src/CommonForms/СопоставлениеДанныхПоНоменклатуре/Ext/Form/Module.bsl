...
&НаКлиенте
Процедура НоваяНоменклатураКонтрагентаНоменклатураПриИзменении(Элемент)
	
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Список.НайтиПоИдентификатору(Элементы.Список.ТекущаяСтрока);
	
	Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		НоваяНоменклатураКонтрагентаНоменклатураПриИзмененииНаСервере();
	КонецЕсли;
	
	ВидимостьХарактеристики = (НЕ ТекущиеДанные.ВладелецХарактеристики=Неопределено);
	
	// Видимость поля Характеристика можно изменять только в сторону ИСТИНА
	Если ВидимостьХарактеристики Тогда
		Элементы.НоваяНоменклатураКонтрагентаХарактеристикаНоменклатуры.Видимость = ИСТИНА;
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура НоваяНоменклатураКонтрагентаНоменклатураПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ХарактеристикаНоменклатуры) Тогда
		Если (ИспользованиеХарактеристик = 1 И ТекущиеДанные.ХарактеристикаНоменклатуры.Владелец <> ТекущиеДанные.Номенклатура.ТипНоменклатуры)
			ИЛИ (ИспользованиеХарактеристик = 2 И ТекущиеДанные.ХарактеристикаНоменклатуры.Владелец <> ТекущиеДанные.Номенклатура)
	...
	Если ИспользованиеХарактеристик = 1 Тогда
		ТекущиеДанные.ВладелецХарактеристики = ТекущиеДанные.Номенклатура.ТипНоменклатуры;
	ИначеЕсли ИспользованиеХарактеристик = 2 Тогда
		ТекущиеДанные.ВладелецХарактеристики = ТекущиеДанные.Номенклатура;
	Иначе
		ТекущиеДанные.ВладелецХарактеристики = Неопределено;
	КонецЕсли;	
...
Процедура УстановитьУсловноеОформление()
	...
	УправлениеДиалогомДокументаСервер.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтотОбъект,"НоваяНоменклатураКонтрагентаХарактеристикаНоменклатуры", "Список.ВладелецХарактеристики");
