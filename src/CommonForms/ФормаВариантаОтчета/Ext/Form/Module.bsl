	...
	Если Параметры.Свойство("НачалоПериода") И Параметры.Свойство("КонецПериода") Тогда
		...
		Для Каждого ТекНастройка Из ПользовательскиеНастройки.Элементы Цикл
			Если ЕстьРеквизитНаКлиенте(ТекНастройка, "Параметр") И ТекНастройка.Параметр = Новый ПараметрКомпоновкиДанных("Период") Тогда
				ЗначениеНастройки = ТекНастройка.Значение;
				ЗначениеНастройки.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
				ЗначениеНастройки.ДатаНачала = Параметры.НачалоПериода;
				ЗначениеНастройки.ДатаОкончания = Параметры.КонецПериода;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗавершитьРедактирование(Команда)
	...
	Иначе
		...
		РезультатВыбора.Вставить("ПользовательскиеНастройки", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки);
		...
		РезультатВыбора.Вставить("КлючВарианта", ЭтаФорма.КлючТекущегоВарианта);
...
Процедура КомпоновщикНастроекНастройкиПараметрыДанныхПриИзменении(Элемент)
	ТекДанные = Элементы.КомпоновщикНастроекНастройкиПараметрыДанных.ТекущиеДанные;
	
	Если ТекДанные.Параметр = "Период" Тогда
		ПользовательскиеНастройки = Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;
		
		Для Каждого ТекНастройка Из ПользовательскиеНастройки.Элементы Цикл
			Если ЕстьРеквизитНаКлиенте(ТекНастройка, "Параметр") И ТекНастройка.Параметр = Новый ПараметрКомпоновкиДанных("Период") Тогда
				ТекНастройка.Значение.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
				ТекНастройка.Значение.ДатаНачала = ТекДанные.ДатаНачала;
				ТекНастройка.Значение.ДатаОкончания = ТекДанные.ДатаОкончания;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
