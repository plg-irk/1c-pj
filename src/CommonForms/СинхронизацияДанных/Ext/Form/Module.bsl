...
Процедура ИзменитьУзелИнформационнойБазы(Команда)
	...
	ПараметрыДействия = Новый Структура("Ключ", УзелОбмена);
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ИзменитьУзелИнформационнойБазыЗавершение", ЭтотОбъект);
	//ОткрытьФорму("ПланОбмена."+УзелОбмена.Метаданные().Имя+".ФормаОбъекта",ПараметрыДействия,,,,,ОповещениеПриЗавершении);
	ПоказатьЗначение(ОповещениеПриЗавершении, УзелОбмена); // -РАРУС
	
КонецПроцедуры

// +рарус
&НаКлиенте
Процедура ИзменитьУзелИнформационнойБазыЗавершение(Ответ=Неопределено, ДополнительныеПараметры=Неопределено) Экспорт
	
	ИзменитьУзелИнформационнойБазыЗавершениеНаСервере();
		
КонецПроцедуры 

&НаСервере
Процедура ИзменитьУзелИнформационнойБазыЗавершениеНаСервере() 
