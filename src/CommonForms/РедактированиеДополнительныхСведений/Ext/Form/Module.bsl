...
Процедура ТаблицаЗначенийСвойствПередНачаломИзменения(Элемент, Отказ)
	...
	// +Рарус
	Элементы.ТаблицаЗначенийСвойствзначение.КнопкаВыбора = Строка.Свойство = ПредопределенноеЗначение("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ДополнительныйРеквизитЧека"); // -Рарус
...
// +Рарус
&НаКлиенте
Процедура ТаблицаЗначенийСвойствЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	//ПараметрыФормы = Новый Структура("РежимВыбора, ЗакрыватьПриВыборе, ВладелецСведений", Истина, Истина,СсылкаНаОбъект);
	//ОткрытьФорму("РегистрСведений.ФискальныеОперации.ФормаСписка",ПараметрыФормы,Элемент,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		
КонецПроцедуры // ТаблицаЗначенийСвойствЗначениеНачалоВыбора() -Рарус
	...
	Для Каждого Строка Из Таблица Цикл
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			РазрешенныеСвойства = МодульУправлениеДоступомСлужебный.РазрешенныеЗначенияДляДинамическогоСписка(
				...
				ЗначениеДоступа,
				ПроверяемыеСвойства);
			Если НЕ ЗначениеЗаполнено(РазрешенныеСвойства) Тогда
				Продолжить;
			КонецЕсли;
