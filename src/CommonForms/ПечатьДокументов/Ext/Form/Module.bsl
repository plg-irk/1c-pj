...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПараметрыПечати.Свойство("ЗаголовокФормы") И ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда
		ПараметрыПечати.Вставить("ЗаголовокФормы", Параметры.ИсточникДанных.Наименование);
	КонецЕсли;
	...
	Если Параметры.Свойство("ВыполняемаяКоманда") Тогда
		ПараметрыВывода.Вставить("ВыполняемаяКоманда", Параметры.ВыполняемаяКоманда);
	КонецЕсли;
	...
	УстановитьПринтерВПечатныхФормах();
	...
	// +РАРУС Убрать общие кнопки для печати этикеток
	ВключитьВидимость = НЕ (ЭтотОбъект.Параметры.ИмяМенеджераПечати = "Обработка.ПечатьЭтикетокИЦенников");
	ЭтотОбъект.Элементы.КнопкаПерейтиКДокументу.Видимость    = ВключитьВидимость;
	ЭтотОбъект.Элементы.ВычислитьСумму.Видимость             = ВключитьВидимость;
	//ЭтотОбъект.Элементы.Сумма.Видимость                    = ВключитьВидимость;
	ЭтотОбъект.Элементы.КнопкаОтправить.Видимость            = ЭтотОбъект.Элементы.КнопкаОтправить.Видимость И ВключитьВидимость;
	ЭтотОбъект.Элементы.КнопкаСохранить.Видимость            = ЭтотОбъект.Элементы.КнопкаСохранить.Видимость И ВключитьВидимость;
	ЭтотОбъект.Элементы.КнопкаОтправитьВсеДействия.Видимость = ВключитьВидимость;
	ЭтотОбъект.Элементы.КнопкаСохранитьВсеДействия.Видимость = ВключитьВидимость;
	// -РАРУС
...
&НаКлиенте
Процедура ТекущаяПечатнаяФормаПриИзменении(Элемент)
	НастройкаПечатнойФормы = ТекущаяНастройкаПечатнойФормы();
	Если ВыбранныйПринтер <> ТекущаяПечатнаяФорма.ИмяПринтера Тогда
		ВыбранныйПринтер = ТекущаяПечатнаяФорма.ИмяПринтера;
		УстановитьИмяПринтераВПодсказкеКнопкиПечать(ВыбранныйПринтер);
	КонецЕсли;
КонецПроцедуры
...
Процедура НастройкиПечатныхФормПриИзменении(Элемент)
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		МожноСохранять = МожноСохранять Или НастройкаПечатнойФормы.Печатать И ПечатнаяФорма.ВысотаТаблицы > 0
			И (ПроверитьПравоРедактирования() ИЛИ (ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Разрешить И Не ПолеТабличногоДокумента.Защита));
	КонецЦикла;
...
Процедура Печать(Команда)
	...
	// +РАРУС
		ТабличныеДокументы.Количество() > 1, ?(НастройкиПечатныхФорм.Количество() > 1, Копий, 1), ИмяПринтера);
	// -РАРУС
...
Функция УстановитьСтандартныеПараметрыНаСервере() Экспорт
	...
	УправлениеПечатьюПлатформа.УстановитьСтандартныеПараметрыПечати(ТекущаяПечатнаяФорма.ИмяПараметровПечати,ТекущаяПечатнаяФорма);
...
Функция СохранитьТекущиеПараметрыКакСтандартныеНаСервере()
	...
	УправлениеПечатьюПлатформа.СохранитьТекущиеПараметрыКакСтандартные(ТекущаяПечатнаяФорма.ИмяПараметровПечати,ТекущаяПечатнаяФорма);
...
Процедура УстановитьУсловноеОформление()
	...
	// +Рарус
	Элементы.ФормаСохранитьКакСтандартныеПараметры.Видимость = ПравоПользователя("СохранениеСтандартныхПараметровПечати") И ПравоДоступа("Вывод", Метаданные);
	Элементы.ФормаСохранитьКакСтандартныеПараметры1.Видимость = ПравоПользователя("СохранениеСтандартныхПараметровПечати") И ПравоДоступа("Вывод", Метаданные);
	
	Элементы.ФормаУстановитьСтандартныеПараметры.Доступность = ПравоДоступа("Вывод",Метаданные);
	Элементы.ФормаУстановитьСтандартныеПараметры1.Доступность = ПравоДоступа("Вывод",Метаданные);
	// -Рарус
...
// +РАРУС
&НаКлиенте
Процедура СоздатьКаталогДляПользователя(ИмяКаталога)
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	Элементы.КнопкаСохранить.Видимость = ЕстьДанныеДляПечати И (ПроверитьПравоРедактирования() ИЛИ (ЕстьРазрешенныйВывод И ЕстьРазрешенноеРедактирование));
	...
	Если НЕ ПроверитьПравоРедактирования() Тогда
		Элементы.КнопкаРедактирование.Видимость = ЕстьРазрешенныйВывод И ЕстьДанныеДляПечати И ЕстьРазрешенноеРедактирование И НЕ ПравоПользователя("ЗащитаПечатныхФорм");
	КонецЕсли;	
	...
	// +РАРУС
	Элементы.НастройкиПечатныхФорм.ПоложениеКоманднойПанели = ?(ДоступнаНастройкаКомплекта, ПоложениеКоманднойПанелиЭлементаФормы.Авто, ПоложениеКоманднойПанелиЭлементаФормы.Нет);
	// -РАРУС
...
&НаСервере
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать(ИмяПринтера = Неопределено)
	Если НастройкиПечатныхФорм.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПустаяСтрока(ИмяПринтера) Тогда
		ИмяПринтера = НСтр("ru = 'Принтер по умолчанию'");
	КонецЕсли;
	
	ПодсказкаКнопкиПечать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Напечатать на принтере (%1)'"),  ИмяПринтера);
	ЭтотОбъект.Команды["Печать"].Подсказка = ПодсказкаКнопкиПечать;
КонецПроцедуры

&НаСервере
Процедура УстановитьПринтерВПечатныхФормах()
	Если НастройкиПечатныхФорм.Количество() > 0 Тогда
		ВыбранныйПринтер = ЭтотОбъект[НастройкиПечатныхФорм[0].ИмяРеквизита].ИмяПринтера;
		Для Индекс = 1 По НастройкиПечатныхФорм.Количество() - 1 Цикл
			НастройкаПечатнойФормы = НастройкиПечатныхФорм[Индекс];
			ЭтотОбъект[НастройкаПечатнойФормы.ИмяРеквизита].ИмяПринтера = ВыбранныйПринтер;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьТекущийТабличныйДокумент(ИмяРеквизита)
	...
	ТекущаяПечатнаяФорма.ИмяПринтера = ВыбранныйПринтер;
КонецПроцедуры
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
				...
				Если СоответствиеОбъектаПечатнойФорме.Ключ <> "ОбъектыПечатиНеЗаданы" Тогда
					...
					КонецЕсли;
					ИмяФайла = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ПредставлениеФайла);
					// -РАРУС
				Иначе
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	// +РАРУС
	Попытка
		СоздатьКаталогДляПользователя(Папка);
	Исключение
		Состояние(НСтр("ru = 'Ошибка записи в текущий каталог.'"), , НСтр("ru = 'Недостаточно прав для записи файла.'"));
		Возврат;
	КонецПопытки;
	// -РАРУС
...
Процедура ПереключитьРедактированиеТекущейПечатнойФормы()
	...
	Если НастройкаПечатнойФормы <> Неопределено Тогда
		...
		Элементы.ТекущаяПечатнаяФорма.Редактирование = ПолеТабличногоДокумента.Редактирование И Элементы.КнопкаРедактирование.Видимость;
...
Процедура ПереключитьПометкуКнопкиРедактирование()
	...
	Если НастройкаПечатнойФормы <> Неопределено Тогда
		ПолеТабличногоДокумента = Элементы[НастройкаПечатнойФормы.ИмяРеквизита];
		РедактированиеВозможно = ПроверитьПравоРедактирования() ИЛИ (НЕ ПечатнаяФормаДоступна И Не ПолеТабличногоДокумента.Защита);
...
&НаСервере
Функция ПроверитьПравоРедактирования()
	
	ИсключениеРедактирования = Ложь;
	Если ЭтотОбъект.ОбъектыПечати = Неопределено ИЛИ ЭтотОбъект.ОбъектыПечати.Количество() = 0 Тогда 
		Возврат ИсключениеРедактирования;
	КонецЕсли;
	
	ИсключениеРедактирования = ?(ПравоПользователя("ИсключениеЗапретаРедактирования", ЭтотОбъект.ОбъектыПечати[0].Значение) = Неопределено, Ложь, ПравоПользователя("ИсключениеЗапретаРедактирования", ЭтотОбъект.ОбъектыПечати[0].Значение));
	Возврат ИсключениеРедактирования;	
КонецФункции
...
Функция ТабличныеДокументыДляПечати()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если Элементы[НастройкаПечатнойФормы.ИмяРеквизита].Вывод = ИспользованиеВывода.Разрешить И НастройкаПечатнойФормы.Печатать Тогда
			...
			ЗаполнитьЗначенияСвойств(ТабличныйДокумент, ПечатнаяФорма, "АвтоМасштаб,Вывод,ВысотаСтраницы,ДвусторонняяПечать,Защита,КодЯзыка,КоличествоЭкземпляров,МасштабПечати,ОриентацияСтраницы,ПолеСверху,ПолеСлева,ПолеСнизу,ПолеСправа,РазборПоКопиям,РазмерКолонтитулаСверху,РазмерКолонтитулаСнизу,РазмерСтраницы,ТочностьПечати,ЧерноБелаяПечать,ШиринаСтраницы,ЭкземпляровНаСтранице");
			ТабличныйДокумент.ИмяПринтера = ВыбранныйПринтер;
...
Процедура ЗаписатьСписокПринтеров()
	...
	Для Номер = 0 По Принтеры.Количество() - 1 Цикл
		СписокПринтеров.Добавить(Принтеры[Номер].Значение, Принтеры[Номер].Представление, Принтеры[Номер].Пометка);
	КонецЦикла;
...
Процедура ОбновитьСписокПринтеровДляПечати()
	...
	Счетчик = 0;
	Для Номер = 0 По Принтеры.Количество() - 1 Цикл
		ДобавитьПринтер(Счетчик, Принтеры[Номер].Значение, Принтеры[Номер].Пометка);
		Счетчик = Счетчик + 1;
	КонецЦикла;
...
Процедура ДобавитьПринтер(Счетчик, ИмяВремПринтера, Пометка = Ложь);
	...
	Если НоваяКоманда = Неопределено Тогда
		...
		НоваяКоманда.Действие = "ВыбратьПринтер";
	КонецЕсли;
	...
	// Добавим Вид сравнения
	НовыйПринтер = Элементы.Добавить(ИдентификаторПринтера+"Еще", Тип("КнопкаФормы"), Элементы.ПодменюПечатьВыборПринтера1);
	НовыйПринтер.ТолькоВоВсехДействиях = Истина;
	НовыйПринтер.ИмяКоманды = ИмяКоманды;
	НовыйПринтер.Заголовок  = ИмяВремПринтера;
	НовыйПринтер.Вид        = ВидКнопкиФормы.КнопкаКоманднойПанели;
	НовыйПринтер.Пометка    = Пометка;	
...
// +РАРУС Получает список принтеров установленных в системе и предлагает пользователю выбрать принтер для печати.
&НаКлиенте
Процедура ВыбратьПринтер(Команда)
	...
	Если КоличествоПринтеров>Число(Индекс) Тогда
		...
		КонецЦикла;
		Для Каждого ТекПринтер Из Элементы.ПодменюПечатьВыборПринтера1.ПодчиненныеЭлементы Цикл
			ТекПринтер.Пометка = (ТекПринтер.Имя = "Принтер"+Индекс);
		КонецЦикла;
