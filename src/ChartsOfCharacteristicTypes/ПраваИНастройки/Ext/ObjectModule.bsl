// Модуль объекта плана видов характеристик "Права и настройки"
...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		Если (ЭтотОбъект.ТипЗначения.Типы().Количество() <> 1) И (НЕ ЭтотОбъект.ЭтоГруппа) Тогда
			Отказ = Истина;
			ВывестиСообщение(НСтр("ru = 'Должен быть указан единственный тип возможных значений. Запись элемента <"+СокрЛП(ЭтотОбъект)+"> отменена.'"));
		КонецЕсли;
	КонецЕсли;
