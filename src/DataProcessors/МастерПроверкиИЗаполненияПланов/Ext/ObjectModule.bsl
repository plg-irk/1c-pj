...
Функция УстановкаСветофоровВидаПланов(СтрокаПлана) Экспорт
	...
	Если НайденноеПодразделение = Неопределено Тогда
		// Нет плана, нет факта.
		ВывестиСообщение(НСтр("ru = 'Вид плана'") + " "+СтрокаПлана+" " + НСтр("ru ='не рассчитан на работу с подразделением'")+" "
								+Подразделение+".");
		Возврат 19;
	КонецЕсли;
	Если ((ДатаНачала+(60*60*24*НайденноеПодразделение.ОтсрочкаПлана) > ДатаКонца) 
		ИЛИ (ДатаНачала+(60*60*24*НайденноеПодразделение.ОтсрочкаФакта) > ДатаКонца)) Тогда
...
Процедура НайтиСоздатьДокумент(ДатаНачала,ДатаКонца,Название,СтрокаВидаПлана)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	План.Проведен КАК ДокументПроведен
		|ИЗ
	...
	Если ВидДокумента="План" Тогда
		Для ПеременнаяЦикла=0 По ЗаполнениеПланов.Количество()-1 Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ДокументПлана) Тогда
				...
				ДокументПланФакт.Автор = ПараметрыСеанса.ТекущийПользователь;
