...
Перем МетаданныеОписание Экспорт; // Переменная описания метаданных.
Перем СсылочныеТипы Экспорт; // Переменная описания ссылочных типов.
Перем СоответствиеОбъектовМетаданныхИСсылочныхТипов;
Перем ОбработанныхКонстант Экспорт; // Переменная описания обработанных констант.
Перем ОбработанныхНаборовЗаписей Экспорт; // Переменная описания обработанных наборов записей.
			...
			Иначе
				...
				Попытка
					ТекстСообщения = НСтр("ru = 'При загрузке объекта %1(%2) возникла ошибка:
						|%3'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ЗаписанноеЗначение, ТипЗнч(ЗаписанноеЗначение), ТекстОшибки);
				Исключение
...
Функция СформироватьТаблицуОшибок(ОбъектыВыгруженныеСОшибками)
	...
	Иначе
		...
		Для Каждого СтрокаСоответствия Из ОбъектыВыгруженныеСОшибками Цикл
			...
			ПозицияНачалаОшибки = СтрНайти(ТекстСообщения, "ЗаписатьXML):");
...
Процедура ВыгрузитьДанныеМассиваСсылок(МассивСсылок, СтрокаИмяДляЗапроса, ЗаписьXML, ТолькоПроверкаНедопустимыхСимволов = Ложь, ОбъектыВыгруженныеСОшибками = Неопределено)
	...
	ЗапросИЗапись(РезультатЗапроса, ЗаписьXML, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, Истина);
...
Функция ПолучитьСтрокуОграниченияПоДатеДляЗапроса(Свойства, ИмяТипа) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаОкончания) Тогда
		...
		Иначе
			...
			|	И " + ИмяПоляОграничения + " <= &ДатаОкончания";
...
//
Процедура ЗапросИЗапись(РезультатЗапроса, ЗаписьXML,ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, 
	ЗапросВерхнегоУровня = Ложь)
	
	// универсальная процедура выгрузки ссылочных объектов процедура
	ОбработкаРезультатаЗапроса(РезультатЗапроса, ЗаписьXML,	ОбъектыВыгруженныеСОшибками, 
	ТолькоПроверкаНедопустимыхСимволов,Истина,ЗапросВерхнегоУровня);
...
//
Процедура ОбработкаРезультатаЗапроса(РезультатЗапроса, ЗаписьXML, ОбъектыВыгруженныеСОшибками,
	ТолькоПроверкаНедопустимыхСимволов, ЭтоЗапросПоОбъекту = Ложь,ЗапросВерхнегоУровня = Ложь)
			...
			Исключение
				...
				Иначе
					...
					ИтоговаяСтрокаСообщения = НСтр("ru = 'При выгрузке объекта %1(%2) возникла ошибка:
						|%3'");
					ИтоговаяСтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИтоговаяСтрокаСообщения, Объект, ТипЗнч(Объект), СтрокаОписанияОшибки);
...
Процедура ЗаписьКонстанты(ЗаписьXML, МД_Константа, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов)
	...
	Исключение
		...
		Иначе
			...
			|%2'");
			ИтоговаяСтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИтоговаяСтрокаСообщения, МД_Константа.Имя, СтрокаОписанияОшибки);
...
Процедура ЗаписьРегистра(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов, РегистрБухгалтерии = Ложь)
	...
	ПараметрыЗаписи.Вставить("ДляЗапроса",                         ИмяТаблицыДляЗапроса);
	ПараметрыЗаписи.Вставить("ИмяОбъекта",                         СтрокаДереваМетаданных.ОбъектМД.Имя);
	ПараметрыЗаписи.Вставить("ТолькоПроверкаНедопустимыхСимволов", ТолькоПроверкаНедопустимыхСимволов);
	ПараметрыЗаписи.Вставить("СтрокаДереваМетаданных",             СтрокаДереваМетаданных);
	ПараметрыЗаписи.Вставить("РегистрБухгалтерии",                 РегистрБухгалтерии);
	
	ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, ОбъектыВыгруженныеСОшибками, ПараметрыЗаписи);
...
Процедура ЗаписьПерерасчета(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов)
	...
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("ДляЗапроса",                         СтрокаДляЗапроса);
	ПараметрыЗаписи.Вставить("ИмяОбъекта",                         СтрокаДереваМетаданных.ОбъектМД.Имя);
	ПараметрыЗаписи.Вставить("ТолькоПроверкаНедопустимыхСимволов", ТолькоПроверкаНедопустимыхСимволов);
	ПараметрыЗаписи.Вставить("СтрокаДереваМетаданных",             СтрокаДереваМетаданных);
	ПараметрыЗаписи.Вставить("РегистрБухгалтерии",                 Ложь);
	ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерПерерасчета, ОбъектыВыгруженныеСОшибками, ПараметрыЗаписи);
...
Процедура ЗаписьПоследовательности(ЗаписьXML, СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, ТолькоПроверкаНедопустимыхСимволов)
	...
	ПараметрыЗаписи = Новый Структура;
	ПараметрыЗаписи.Вставить("ДляЗапроса",                         СтрокаДереваМетаданных.ЭлементОписания.ДляЗапроса);
	ПараметрыЗаписи.Вставить("ИмяОбъекта",                         СтрокаДереваМетаданных.ОбъектМД.Имя);
	ПараметрыЗаписи.Вставить("ТолькоПроверкаНедопустимыхСимволов", ТолькоПроверкаНедопустимыхСимволов);
	ПараметрыЗаписи.Вставить("СтрокаДереваМетаданных",             СтрокаДереваМетаданных);
	ПараметрыЗаписи.Вставить("РегистрБухгалтерии",                 Ложь);
...
//   СтрокаДереваМетаданных - строка дерева метаданных, соответствующая регистру.
//
Процедура ЗаписьЧерезНаборЗаписей(ЗаписьXML, МенеджерНабораЗаписей, ОбъектыВыгруженныеСОшибками, ПараметрыЗаписи)
	
	// Получить состав колонок записи регистра и проверить наличие хотя бы одной записи.
	Если ПараметрыЗаписи.ДляЗапроса = "РегистрБухгалтерии." Тогда
		ИмяТаблицыДляЗапроса = ПараметрыЗаписи.ДляЗапроса + ПараметрыЗаписи.ИмяОбъекта + ".ДвиженияССубконто";	
	Иначе
		ИмяТаблицыДляЗапроса = ПараметрыЗаписи.ДляЗапроса + ПараметрыЗаписи.ИмяОбъекта;	
	КонецЕсли;
	
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 * ИЗ " + ИмяТаблицыДляЗапроса + " КАК ТаблицаОбъекта_" + ПараметрыЗаписи.ИмяОбъекта);
	...
	МассивКолонок = ПолучитьМассивКолонокДвижения(ТаблицаДвижений, ПараметрыЗаписи.РегистрБухгалтерии);
	...
	ПостроительОтчета = ПодготовитьПостроительДляВыгрузки(ПараметрыЗаписи.СтрокаДереваМетаданных, СтрокаПолейОтбора); 
	ПостроительОтчета.Выполнить();
	...
	Пока ВыборкаИзРезультата.Следующий() Цикл
		...
		Если мНаличиеВыгрузкиПодчиненныхОбъектов Тогда
			...
			ВыгрузитьПодчиненныеЗначенияНабора(ЗаписьXML, НаборЗаписей, МассивКолонок, ОбъектыВыгруженныеСОшибками, ПараметрыЗаписи.ТолькоПроверкаНедопустимыхСимволов);
		...
		Попытка
			...
			ВыполнитьВспомогательныеДействияДляЗаписиXML(ВсегоОбработаноОбъектов, ЗаписьXML, ПараметрыЗаписи.ТолькоПроверкаНедопустимыхСимволов);
		...
		Исключение
			...
			// Не смогли записать в XML
			// возможно проблема с недопустимыми символами в XML.
			Если ПараметрыЗаписи.ТолькоПроверкаНедопустимыхСимволов Тогда
			...
			Иначе
				...
				ИтоговаяСтрокаСообщения = НСтр("ru = 'При выгрузке регистра %1%2 возникла ошибка:
					|%3'");
				ИтоговаяСтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИтоговаяСтрокаСообщения, ПараметрыЗаписи.ДляЗапроса, ПараметрыЗаписи.ИмяОбъекта, СтрокаОписанияОшибки);
...
Процедура УстановкаСостоянияВыгружатьПриНеобходимости(ЭлементДЗ)
	...
	Для Каждого СсылочныйТип Из ВсеТипы Цикл
		...
		Если ТипИОбъект = Неопределено Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстИсключения, СсылочныйТип);
...
Функция ПолучитьФабрикуСУказаниемТипов(Знач Типы)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось удалить файл'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
