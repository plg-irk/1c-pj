...
Процедура СформироватьТабличныйДокумент(Объект, ТабДокумент = Неопределено, НеПоказыватьТаблДокумент = Ложь) Экспорт
	...
	Для Каждого ТекРегистр Из Объект.Метаданные().Движения Цикл
		...
		ПоследняяКолонка = ?(ЕстьПериод, ?(ЕстьРеквизиты, 6, 5), ?(ЕстьРеквизиты, 5, 4));
		...
		ВысотаМакета = МАКС(ТекРегистр.Измерения.Количество(), ТекРегистр.Ресурсы.Количество(), ТекРегистр.Реквизиты.Количество());
