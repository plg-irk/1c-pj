	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			Если ИмяТипаОбъекта <> "РегистрСведений"
				...
				Отказ = ЗапретИзмененияДанныхПоДате(Объект);
			КонецЕсли;
...
Функция ЗапретИзмененияДанныхПоДате(ЭлементДанных)
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.ДатыЗапретаИзменения") Тогда
		Если НЕ Метаданные.Константы.Содержит(ЭлементДанных.Метаданные()) Тогда
			МодульОбщегоНазначения = Вычислить("ОбщегоНазначения");
			МодульДатыЗапретаИзменения = МодульОбщегоНазначения.ОбщийМодуль("ДатыЗапретаИзменения");
