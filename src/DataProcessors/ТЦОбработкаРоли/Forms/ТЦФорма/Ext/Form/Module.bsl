...
&НаКлиенте
Перем мИнструменты; // Служебные инструменты
&НаКлиенте
...
Функция ТЦИнициализировать() Экспорт
	...
	Попытка
		...
		Для Каждого СтрокаОперации Из ТЦОбъект.ИнтенсивностьОпераций Цикл
			...
			ПараметрыФормы.Вставить("ТЦСценарий", ФормаРоли.Параметры.ТЦСценарий);
			ПараметрыФормы.Вставить("ТЦРоль", СтрокаОперации.Роль);
			
			ФормаОбработки = ПолучитьФорму(ИмяФормыОбработки, ПараметрыФормы);
	...
	Исключение
		...
		ТЦОбщий.ЗаписатьВЖурнал(ОписаниеОшибки(), НСтр("ru = 'Ошибка в ТЦИнициализировать'"));
...
Функция ПолучитьСсылкуНаОбработку(Роль)
	...
	Исключение
		ТЦОбщий.ЗаписатьВЖурнал(ОписаниеОшибки(), НСтр("ru = 'Ошибка в ПолучитьСсылкуНаОбработку'"));
	КонецПопытки; 
...
Процедура ОжидатьЗавершенияВыполненияОперации()
	...
	Если ТекущаяФормаОбработки.ТЦКонтекст().ВозниклоИсключение ИЛИ РезультатВыполненияТекущейОперации = ТЦРезультатВыполненияОшибка() Тогда
		...
		Пояснение = ?(РезультатВыполненияТекущейОперации = ТЦРезультатВыполненияОшибка(), 
						НСтр("ru ='Функция вернула ошибку:'") + " ",НСтр("ru = 'Возникло исключение:'") + " ");
...
Функция ВыполнитьШагТеста()
	...
	Исключение
		...
		ТЦЗавершитьВыполнение(ТЦКонтекст().РезультатВыполнения.Неопределено,
								Истина,
								НСтр("ru ='Функция вернула ошибку:'") + " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ТЦВыполнитьДействиеТеста(Знач ИмяФункции)
	...
	Попытка
		...
		Если РезультатВыполнения = ТЦРезультатВыполненияОшибка() Тогда
			...
			ТЦКонтекстВыполнения.ТекстИсключенияИлиОшибки = НСтр("ru = 'Возникла ошибка при выполнении'") + " " + ИмяФункции + Символы.ПС
															+ НСтр("ru = 'Функция вернула ошибку.'") + " " 
															+ ТЦКонтекстВыполнения.ТекстИсключенияИлиОшибки;
		КонецЕсли;
...
&НаКлиенте
// Обработчик ПередОкончаниемРедактирования для табличной части ИнтенсивностьОпераций.
