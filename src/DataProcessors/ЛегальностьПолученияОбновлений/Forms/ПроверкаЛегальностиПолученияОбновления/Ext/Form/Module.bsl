...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Обработаем в зависимости от наличия макетов у обработки проверки легальности получения конфигурации.
	Если (НЕ ОбновлениеИнформационнойБазыСлужебный.РежимОбновленияДанных()="НачальноеЗаполнение") ИЛИ Метаданные.Обработки.ЛегальностьПолученияОбновлений.Макеты.Найти("УсловияРаспространенияПродукта")=Неопределено Тогда
		МакетДокумента = Обработки.ЛегальностьПолученияОбновлений.ПолучитьМакет("_УсловияРаспространенияОбновлений");
	Иначе
		МакетДокумента = Обработки.ЛегальностьПолученияОбновлений.ПолучитьМакет("УсловияРаспространенияПродукта");
	КонецЕсли;
	
	// -РАРУС
	
	ТекстПредупреждения = МакетДокумента.ПолучитьТекст();
	ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "[ЮридическоеЛицо]", Строка(Метаданные.Поставщик));
	ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "[НаименованиеТОРа]", Строка(Метаданные.Синоним));
