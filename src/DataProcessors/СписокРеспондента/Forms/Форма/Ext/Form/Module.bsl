...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +РАРУС Реализована возможность работы не только для внешних пользователей
	Если ЭтотОбъект.Параметры.Свойство("Респондент") И ЗначениеЗаполнено(ЭтотОбъект.Параметры.Респондент) Тогда
		Объект.Респондент = ЭтотОбъект.Параметры.Респондент;
		ПолучитьТаблицуАнкетРеспондента();
		
	ИначеЕсли ТипЗнч(ТекущийПользователь) <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда 
