...
Функция ЗначениеЯчейки(Колонка, ЗначениеЯчейки)
	...
	Для каждого ТипДанных Из Колонка.ТипКолонки.Типы() Цикл 
		// +РАРУС
		// Возможна ситуация, когда в составе типов реквизита имеются примитивные типы
		Если ТипДанных = Тип("Строка") ИЛИ ТипДанных = Тип("Булево") ИЛИ ТипДанных = Тип("Число")
			 ИЛИ ТипДанных = Тип("Дата") ИЛИ ТипДанных = Тип("УникальныйИдентификатор") Тогда
			Продолжить;
		КонецЕсли;
		// -РАРУС
	...
	// +РАРУС
	// Проверим, если до сих пор не найден элемент, но есть примитивные типы, то запишем данные ячейки
	Если НЕ ЗначениеЗаполнено(ДанныеЯчейки) Тогда
		Для каждого ТипДанных Из Колонка.ТипКолонки.Типы() Цикл 
			Если ТипДанных = Тип("Строка") ИЛИ ТипДанных = Тип("Булево") ИЛИ ТипДанных = Тип("Число")
				ИЛИ ТипДанных = Тип("Дата") ИЛИ ТипДанных = Тип("УникальныйИдентификатор") Тогда
				ДанныеЯчейки = ЗначениеЯчейки;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	// -РАРУС
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	// +РАРУС Добавим в общие реквизиты объекта
	МассивРеквизитов = Новый Массив;
	Для Каждого Реквизит Из Метаданные.ОбщиеРеквизиты Цикл
		ЗначениеСостава = Реквизит.Состав.Найти(МетаданныеСправочника);
