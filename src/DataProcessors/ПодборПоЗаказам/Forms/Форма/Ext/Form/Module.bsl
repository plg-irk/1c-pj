...
#Область ОсновныеОбработчикСобытийФормы

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	УстановитьУсловноеОформлениеПоУмолчанию();
	ПутьКСхемеКомпоновки = ПолучитьЗначениеПараметраСтруктуры(Параметры, "ПутьКСхемеКомпоновкиДанных", "СКД_ПоУмолчанию");
	...
	Объект.ИспользоватьЗаказыПокупателей =
		ПолучитьЗначениеПараметраСтруктуры(Параметры, "ИспользоватьЗаказыПокупателей", Истина);
	
	Объект.ИспользоватьЗаказыВнутренние =
		ПолучитьЗначениеПараметраСтруктуры(Параметры, "ИспользоватьЗаказыВнутренние", Истина);
		
	Объект.ВыводитьТолькоОстаткиПоЗаказам =
		ПолучитьЗначениеПараметраСтруктуры(Параметры, "ВыводитьТолькоОстаткиПоЗаказам", Ложь);
		
	Объект.Заказ =
		ПолучитьЗначениеПараметраСтруктуры(Параметры, "Заказ", Неопределено);
		
	Объект.ИмяКолонкиКоличество =
		ПолучитьЗначениеПараметраСтруктуры(Параметры, "ИмяКолонкиКоличество", "Заказано");
	
	Если Параметры.Свойство("Отбор") Тогда
		Для Каждого Отбор Из Параметры.Отбор Цикл
			ВидСравненияОтбора =
				?(НЕ (Отбор.Значение = Тип("СписокЗначений") ИЛИ Отбор.Значение = Тип("Массив")),
	...
	// Товар, уже добавленный в документ
	Если НЕ ПолучитьЗначениеПараметраСтруктуры(Параметры, "РаспределениеЗаказа") = Неопределено Тогда
		...
		КонецЦикла;
		Элементы.УчетЗаказанного.Видимость = Истина;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	// Установим дату документа для возможности отбора корректных данных
	ОбъектФормы = Неопределено;
	ПолучитьОсновнойОбъектФормы(ВладелецФормы, ОбъектФормы);
	Если НЕ ОбъектФормы = Неопределено Тогда
...
Процедура ПолучитьТаблицуЗаказов()
	...
	// Заполним количество уже заказанное в данном документе
	Если УчетЗаказанного Тогда
...
Процедура ЗаполнитьКолонкуЗаказаноВДокументе()
	...
	// Дальше будем работать, если передавались данные из документа
	...
	Для Каждого Строка Из СписокТоваров Цикл
		СтрокиВДокументе = ЗаказаноВДокументе.НайтиСтроки(Новый Структура("Номенклатура,ЗаказПокупателя", Строка.Номенклатура, Строка.Заказ));
		Если СтрокиВДокументе.Количество() > 0 Тогда
			Строка.ЗаказаноВДокументе = СтрокиВДокументе[0].Количество;
		КонецЕсли;
		СтрокиВПодобрано = Объект.ПодобранныеТовары.НайтиСтроки(Новый Структура("Номенклатура,Заказ", Строка.Номенклатура, Строка.Заказ));
		Если СтрокиВПодобрано.Количество() > 0 Тогда
			Строка.ЗаказаноВДокументе = Строка.ЗаказаноВДокументе + СтрокиВПодобрано[0].Количество;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьКоличествоПоСериям(Строка)
	...
	Если КоличествоБазовое > 1 И Строка.Номенклатура.ТипНоменклатуры.УникальностьСерийногоНомера Тогда
		...
		ТекстСообщения = НСтр("ru = 'Номенклатура ""%1"" учитывается по уникальным серийным номерам. Количество не может превышать 1 базовую единицу.'");
		
		ВывестиСообщение(ТекстСообщения,
			ЭтотОбъект,
			"Корзина[#].Номенклатура",
			Объект.ПодобранныеТовары.Индекс(Строка)+1,
			,
			Строка.Номенклатура);
	КонецЕсли;
...
Процедура НастроитьВидимостьКолонокФормы()
	...
	Элементы.ФильтрСтрокПоКлючевойКолонке.Доступность = ?(КоличествоВидимыхКолонокКоличества <= 1, Ложь, Истина);
...
Процедура СформироватьДеревоЗаказов()
	...
	Если Объект.РежимИерархии = 0 Тогда
		Для Каждого Строка Из Объект.СписокЗаказанныхТоваров Цикл
			Если Объект.ОтображатьВсеСтроки ИЛИ Строка[Объект.ИмяКолонкиКоличество] > 0 Тогда
	...
	Иначе
		...
		Для Каждого Строка Из Объект.СписокЗаказанныхТоваров Цикл
			Если (Объект.ОтображатьВсеСтроки ИЛИ Строка[Объект.ИмяКолонкиКоличество] > 0) И СверткаГруппировки.Найти(Строка[КолонкаГруппировки]) = Неопределено Тогда
...
Процедура ДеревоЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ДанныеСтроки.ВладелецХарактеристики = Неопределено ИЛИ НЕ ДанныеСтроки.ХарактеристикаНоменклатуры.Пустая() Тогда
		ДобавитьНоменклатуру(
			ДанныеСтроки.Номенклатура,
			ДанныеСтроки.ХарактеристикаНоменклатуры,
			ДанныеСтроки[Объект.ИмяКолонкиКоличество] - ?(УчетЗаказанного, ДанныеСтроки.ЗаказаноВДокументе, 0),
			ДанныеСтроки.Заказ,
	...
	Иначе
		...
		ХарактеристикиНоменклатуры.Параметры.УстановитьЗначениеПараметра("ВладелецХарактеристики", ДанныеСтроки.ВладелецХарактеристики);
		
		ВыбраннаяНоменклатура    = ДанныеСтроки.Номенклатура;
		ВыбранноеКоличество      = ДанныеСтроки[Объект.ИмяКолонкиКоличество] - ?(УчетЗаказанного, ДанныеСтроки.ЗаказаноВДокументе, 0);
