...
Функция ПечатьМ11Общая(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ОбластьЗаголовок.Параметры.ОрганизацияПредставление = УправлениеПечатьюПлатформа.ПолучитьПредставлениеСправочника(ДанныеДокумента.Организация,СтруктураПредставления,,,ДатаДокумента,ДополнительныеПараметры);
		...
		ОбластьЗаголовок.Параметры.СкладОтправитель			= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ДанныеДокумента.Грузоотправитель);
		ОбластьЗаголовок.Параметры.СкладПолучатель			= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ДанныеДокумента.Грузополучатель);
		...
		Для Каждого СтрокаТабличнойЧасти Из ДанныеДокумента.ВыборкаТабличнойЧасти Цикл
			...
			ОбластьСтрока.Параметры.ТоварНаименование				= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(СтрокаТабличнойЧасти.Номенклатура) + ?(Характеристика = "", "",", " + Характеристика);
			ОбластьСтрока.Параметры.НоменклатурныйНомер				= УправлениеПечатьюПлатформа.ПолучитьЗначениеКолонкиКода(СтрокаТабличнойЧасти.Номенклатура);
...
Функция ПечатьМ4Общая(МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого Документ Из МассивОбъектов Цикл
		...
		ОбластьЗаголовок.Параметры.ПодразделениеКомпании = УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ОбластьЗаголовок.Параметры.ПодразделениеКомпании);
		...
		ОбластьЗаголовокДокумента.Параметры.МестоПриемки			= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ДанныеДокумента.СкладКомпании);
		ОбластьЗаголовокДокумента.Параметры.ПоставщикНаименование	= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ДанныеДокумента.Контрагент);
		...
		ОбластьЗаголовокДокумента.Параметры.СтраховаяКомпанияПредставление	= УправлениеПечатьюПлатформа.ПолучитьНаименованиеСправочника(ДанныеДокумента.СтраховаяКомпания);
		...
		Для каждого СтрокаТабличнойЧасти Из ВыборкаСтрок Цикл
			...
			Если (ЦелаяСтраница = Цел(ЦелаяСтраница))
				ИЛИ ((ПереноситьПоследнююСтроку = 1) И (Ном = КоличествоСтрок)) Тогда
				ОбластьИтогиСтраницы.Параметры.ИтогоКоличествоПринято = Формат(ИтогоКоличествоПринятоПоСтранице,ФорматВыводаКоличества);
			...
			Иначе
				ОбластьСтрока.Параметры.ТоварКод						= УправлениеПечатьюПлатформа.ПолучитьЗначениеКолонкиКода(СтрокаТабличнойЧасти.Номенклатура);
			КонецЕсли;
			...
			Если ДанныеДокумента.Свойство("ОснованиеТовары") Тогда
				...
				МассивНайденныхСтрок = ДанныеДокумента.ОснованиеТовары.НайтиСтроки(Новый Структура("Номенклатура",СтрокаТабличнойЧасти.Номенклатура));
				КолвоИзОснования = 0;
