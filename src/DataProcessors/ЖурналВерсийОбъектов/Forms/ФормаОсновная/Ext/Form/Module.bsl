...
///////////////////////////////////////////////////////////////////////////////////////////
/////////////////////// ФОРМА ОБРАБОТКИ "ЖУРНАЛ ВЕРСИЙ ОБЪЕКТОВ" //////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////

// Модуль содержит процедуры и функции, необходимые для функционирования формы журнала версий
// объектов. Процедуры и функции вызываются и выполняются как на сервере, так и на клиенте.
...
Процедура ПостроитьДеревоИзмененийНаСервере()
	...
	Для Каждого ТекСтрока Из ТаблицаИзмененийРегистр Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтрока.ОбъектСжатый) Тогда
			...
			КонецЕсли; 
		ИначеЕсли НЕ ПустаяСтрока(ТекСтрока.ОбъектВXML) Тогда
			// Отобразим данные внешней БД.
			ТекСтрока.ОбъектВXML = ТекСтрока.ОбъектВXML;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ТекСтрока.ОбъектВXML) Тогда
			Продолжить;
		КонецЕсли;
	...
	Если НЕ Выборка.Количество() = 0 ИЛИ ЗначениеЗаполнено(ОбъектВXML) Тогда
		...
		ИначеЕсли Выборка.Следующий() Тогда
			...
			Иначе
				...
				Иначе
					ВывестиСообщение(НСтр("ru = 'Версия повреждена'"));
		...
		Таблица = ВерсионированиеОбъектовПлатформа.ОпределитьРазличиеВерсийОбъекта(СтрокаXML1, СтрокаXML2);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ, КОМАНД, ФОРМЫ
...
Процедура ДеревоИзмененийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ДеревоИзмененийНовоеЗначение" ИЛИ Поле.Имя = "ДеревоИзмененийСтароеЗначение" ИЛИ Поле.Имя = "ДеревоИзмененийПользователь" Тогда
		
		ТекЗначение = ?(Поле.Имя = "ДеревоИзмененийПользователь", ТекСтрока.Пользователь, ?(Поле.Имя = "ДеревоИзмененийСтароеЗначение", ТекСтрока.СтароеЗначение, ТекСтрока.НовоеЗначение));
...
Процедура СравнитьВерсии(Команда)
	...
	Если ВыделенныеСтроки.Количество() < 2 Тогда
		ВывестиСообщение(НСтр("ru = 'Для сравнения выберите две версии объекта.'"));
		Возврат;
	КонецЕсли;
	...
	Если ПерваяСтрока.Объект <> ВтораяСтрока.Объект Тогда
		ВывестиСообщение(НСтр("ru = 'Сравнивать версии можно только для одного и того же объекта.'"));
		Возврат;
	КонецЕсли;
