...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +РАРУС Формирование заполнения адреса в соответствии с основным адресом подразделения.
	ЗаполнитьПоУмолчаниюАдрес = ПолучитьЗначениеПараметраСтруктуры(Параметры, "ЗаполнитьАдресПоУмолчанию", Ложь);
	
	// Проверим наличие основного адреса заполнения у подразделения
	ОсновнойАдрес = ПараметрыСеанса.ПодразделениеКомпании.ОсновнойАдрес;
	
	Если ЗаполнитьПоУмолчаниюАдрес И ЗначениеЗаполнено(ОсновнойАдрес) Тогда
		Параметры.ЗначенияПолей = ОсновнойАдрес;
	КонецЕсли;
	// -РАРУС
	...
	// +РАРУС Установим признак модифицированности формы
	Если ЗаполнитьПоУмолчаниюАдрес И ЗначениеЗаполнено(НаселенныйПункт) Тогда
		Модифицированность = Истина;
	КонецЕсли;
	// -РАРУС
...
&НаКлиенте
Процедура СтранаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СтранаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
