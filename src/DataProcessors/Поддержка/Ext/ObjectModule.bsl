...
Функция ПолучитьИнформациюОбУстановленныхСоединениях(Результат)
	...
	Для каждого ТекСоединение Из МассивСоединений Цикл
		...
		Текст = Текст + " " + НСтр("ru = '- Соединение'") + " " + Сч + " " + НСтр("ru = '(Имя пользователя:'") + " " + ТекСоединение.Пользователь.ПолноеИмя + "; ";
		Текст = Текст + НСтр("ru = 'Имя компьютера:'") + " " + ТекСоединение.ИмяКомпьютера + "; " ;
		Текст = Текст + НСтр("ru = 'Режим:'") + " " + ?(ТекСоединение.ИмяПриложения="Designer", "Конфигуратор", "Предприятие") + ")";
		Сч = Сч + 1;
	КонецЦикла;
...
Функция ПолучитьТекстСИнформациейОЗащите()
	...
	Перем ВсегоПользователейЗаМесто,
		СвободноПользователейЗаМесто,
		ВсегоПользователейЗаСессию,
		СвободноПользователейЗаСессию,
		Маска,
		Счетчик1,
		Счетчик2,
		Счетчик3,
	...
	Текст = НСтр("ru = '*** Информация о защите:'") + " " + Символы.ПС;
	ИмяОбработки = ПолучитьИмяОбработкиПоИндексу(0);
	
	ИнформацияОЗащитеПолучена = ЛицензированиеСервер.ПолучитьПараметрыКлючаЗащиты(ИмяОбработки, ПараметрыКлючаЗащиты,ОписаниеОшибки, КодОшибки);


	Если ИнформацияОЗащитеПолучена Тогда
		
		Текст = Текст + НСтр("ru = 'Сервер лицензирования:'") + " " + ПараметрыСеанса.АдресСервераЛицензирования + Символы.ПС
			+ НСтр("ru = 'Название ключа:'") + " " + ПараметрыКлючаЗащиты.НазваниеКлюча + Символы.ПС
			+ НСтр("ru = 'Серийный номер:'") + " " + ПараметрыКлючаЗащиты.СерийныйНомер + Символы.ПС
			+ НСтр("ru = 'Аппаратный номер:'") + " " + ПараметрыКлючаЗащиты.АппаратныйНомер + Символы.ПС
			+ НСтр("ru = 'Дата окончания:'") + ПараметрыКлючаЗащиты.ДатаОкончания + Символы.ПС
			+ НСтр("ru = 'Тип ключа:'") + " " + ПараметрыКлючаЗащиты.ТипКлюча + Символы.ПС
			+ НСтр("ru = 'Пин-код:'") + " " + ПараметрыКлючаЗащиты.ПинКод + Символы.ПС
			+ НСтр("ru = 'Подключений за место:'") + " " + СтрокаКоличествоПодключений(ПараметрыКлючаЗащиты.ВсегоПользователейЗаМесто, ПараметрыКлючаЗащиты.СвободноПользователейЗаМесто) + Символы.ПС
			+ НСтр("ru = 'Подключений за сессию:'") + " " + СтрокаКоличествоПодключений(ПараметрыКлючаЗащиты.ВсегоПользователейЗаСессию, ПараметрыКлючаЗащиты.СвободноПользователейЗаСессию) + Символы.ПС
			+ НСтр("ru = 'Маска:'") + " " + ПараметрыКлючаЗащиты.Маска + Символы.ПС
			+ НСтр("ru = 'Маска 2:'") + " " + ПараметрыКлючаЗащиты.Счетчик2 + Символы.ПС
			+ НСтр("ru = 'Версия компоненты:'") + " " + ЛицензированиеСервер.ПолучитьЗащищеннуюОбработку(ИмяОбработки).Компонента.Версия + Символы.ПС;
	КонецЕсли;
	...
	Если Найти(МестоХраненияКомпоненты, "ОбщийМакет") Тогда
		...
		Текст = Текст + НСтр("ru = 'Источник компоненты: Внутреннее хранилище'") + Символы.ПС;
	Иначе
		
		Текст = Текст + НСтр("ru = 'Источник компоненты: Внешнее хранилище'") + Символы.ПС;
	КонецЕсли;
...
Функция ПолучитьТекстСИнформациейОЗапущенныхПроцессах()
	...
	Для каждого Process Из WinProcesses Цикл
		...
		User = "";
		Domain = "";
		Owner = Process.GetOwner(User, Domain);
