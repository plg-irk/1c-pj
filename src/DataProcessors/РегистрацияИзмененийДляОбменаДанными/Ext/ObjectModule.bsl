...
//
// Параметры:
//  ТекущийОбъект - Произвольное - переданный объект.
// 
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Не ВсеИзмерения Тогда
		...
		Для Каждого КолонкаРезультата Из ПустойРезультат.Колонки Цикл
			...
			ДобавлятьИзмерение = (МетаИзмерение <> Неопределено И МетаИзмерение.Ведущее);
			
			Если ДобавлятьИзмерение Или СтрНайти(НеУчитывать, ИмяКолонки + ",") = 0 Тогда
...
//
// Параметры:
//  КлючНастройки - Строка - имя настройки, значением содержит описанием ошибки.
//
