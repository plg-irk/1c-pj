...
Функция ОбщиеРеквизитыОбъектов(ТипыОбъектов) Экспорт
	...
	// +РАРУС Заполним общими реквизитами
	ОбщиеРеквизиты = Новый Массив();
	Для Каждого Реквизит Из Метаданные.ОбщиеРеквизиты Цикл
		ЗначениеСостава = Реквизит.Состав.Найти(ОбъектыМетаданных[0]);
		Если ЗначениеСостава=Неопределено ИЛИ (НЕ ЗначениеСостава.Использование=Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать) Тогда
			Продолжить;
		КонецЕсли;
		ОбщиеРеквизиты.Добавить(Реквизит);
	КонецЦикла;
	ОбщиеРеквизитыОбъекта = Новый Массив();
	Для Индекс = 1 По ОбъектыМетаданных.Количество() - 1 Цикл
		
		ОбщиеРеквизитыОбъекта.Очистить();
		Для Каждого Реквизит Из Метаданные.ОбщиеРеквизиты Цикл
			ЗначениеСостава = Реквизит.Состав.Найти(ОбъектыМетаданных[Индекс]);
			Если ЗначениеСостава=Неопределено ИЛИ (НЕ ЗначениеСостава.Использование=Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать) Тогда
				Продолжить;
			КонецЕсли;
			ОбщиеРеквизитыОбъекта.Добавить(Реквизит);
		КонецЦикла;
		
		ОбщиеРеквизиты = ПересечениеРеквизитов(ОбщиеРеквизиты, ОбщиеРеквизитыОбъекта);
	КонецЦикла;
