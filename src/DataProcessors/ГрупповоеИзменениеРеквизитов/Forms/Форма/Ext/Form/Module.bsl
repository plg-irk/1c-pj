...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Пока ПродолжитьОбработку Цикл
		...
		ТекстВопроса = НСтр("ru = 'При изменении элементов (группы элементов) возникли ошибки.'")
			+ Символы.ПС
			+ НСтр("ru = 'Прервать изменение элементов и перейти к просмотру ошибок?'")
			+ Символы.ПС;
...
Процедура ИнициализироватьКомпоновщикНастроек()
	...
	Если ВыбранныеОбъектыВКонтексте.Количество() > 0 Тогда
		Если Параметры.Свойство("СписокКомпоновщикНастроек") И ТипЗнч(Параметры.СписокКомпоновщикНастроек) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда
			НастройкиКомпоновщика = Параметры.СписокКомпоновщикНастроек.ПолучитьНастройки();
...
Функция НередактируемыеРеквизиты()
	...
	Для Каждого ВидИзменяемыхОбъектов Из СтрРазделить(ВидыИзменяемыхОбъектов, ",", Ложь) Цикл
		...
		// +РАРУС Добавим общие реквизиты
		Для Каждого ОписаниеРеквизита Из Метаданные.ОбщиеРеквизиты Цикл
			ЗначениеСостава = ОписаниеРеквизита.Состав.Найти(ОбъектМетаданных);
...
Процедура ЗаполнитьРеквизитыОбъекта(БлокируемыеРеквизиты, НеРедактируемые, ФильтруемыеРеквизиты, ДоступныеРеквизиты)
	...
	// +РАРУС Добавим в набор общие реквизиты объекта
	...
	Для Каждого Реквизит Из Метаданные.ОбщиеРеквизиты Цикл
		...
		Если ЗначениеСостава=Неопределено ИЛИ (НЕ ЗначениеСостава.Использование=Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать) Тогда
			Продолжить;
		КонецЕсли;
		МассивОбщихРеквизитов.Добавить(Реквизит);
	КонецЦикла;
	
	НаборыРеквизитов.Вставить("ОписанияРеквизитов", МассивОбщихРеквизитов);
	ДобавитьРеквизитыВНабор(НаборыРеквизитов, ОбъектМетаданных);
	// -РАРУС
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если МодульУправлениеСвойствами <> Неопределено Тогда
			...
			Если ИспользуютсяДопРеквизиты Или ИспользуютсяДопСведения Тогда
				ДобавитьДополнительныеРеквизитыИСведенияВНабор(НаборыРеквизитов); // +РАРУС Добавлен параметр
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьДополнительныеРеквизитыИСведенияВНабор(НаборыРеквизитов)// +РАРУС Добавлен параметр
	...
	Если КонтекстныйВызов Тогда
		Для Каждого ДанныеОбъекта Из ВыбранныеОбъекты(Истина).Строки Цикл
			...
			Для Каждого Свойство Из СписокСвойств Цикл
				...
				// +Рарус
				Если НеРедактируемые.Найти(Свойство.Ссылка.Наименование) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Если ФильтруемыеРеквизиты.Найти(Свойство.Ссылка.Наименование) <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				// -Рарус
	...
	Иначе
		Для Каждого Реквизит Из ОбщийСписокРеквизитов Цикл
			// +Рарус
			Если НеРедактируемые.Найти(Реквизит.Ссылка.Наименование) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если ФильтруемыеРеквизиты.Найти(Реквизит.Ссылка.Наименование) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			// -Рарус
