...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Склад") ИЛИ НЕ ЗначениеЗаполнено(Параметры.Склад) Тогда
		ВывестиСообщение(НСтр("ru='Данная обработка используется только объектами конфигурации.
...
Процедура СкладОбработкаВыбораНаСервере(ВыбранноеЗначение, СтандартнаяОбработка, ПараметрыДействия = Неопределено)
	...
	Если ВыбранноеЗначение.ВидСклада = Перечисления.ВидыСкладов.Ордерный ИЛИ ВыбранноеЗначение.ВидСклада = Перечисления.ВидыСкладов.Обычный Тогда
		...
		ВывестиСообщение(НСтр("ru='Для данного склада заполнение невозможно. Необходимо выбрать ячеистый склад.'"));
...
Процедура ВыполнитьЗаполнениеЯчеекНаСервере(ПараметрыЯчеек)
	...
	Если НЕ ЗначениеЗаполнено(Склад) Тогда
		ВывестиСообщение(НСтр("ru='Ошибка. Не выбран склад.'"));
		Возврат;
	КонецЕсли;
	...
	Если СтрокаПустыхПараметров <> "" Тогда
		...
		ВывестиСообщение(НСтр("ru='Автозаполнение склада ячейками невозможно.'"));
		ВывестиСообщение(НСтр("ru='Не введены параметры заполнения:  %1'"), , , , , Лев(СтрокаПустыхПараметров, СтрДлина(СтрокаПустыхПараметров) - 1));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗаполнениеЯчеек(Команда)
	...
	Оповестить("ОбработкаОповещения", , ЭтотОбъект.ВладелецФормы);
	Закрыть();
...
//
&НаКлиенте
Функция ПараметрыЯчеек() Экспорт
...
&НаСервере
Процедура СоздатьЯчейки(ПараметрыЯчеек) Экспорт
