...
Функция ПолучитьТекстЗапроса(ДокументКорень) Экспорт
	...
	Если ТипЗнч(ДокументКорень) = Тип("ДокументСсылка.итПриказОПроведенииИнвентаризации") Тогда
		ТекстЗапроса = ПолучитьТекстЗапросаДокументыИнвентаризации(ДокументКорень);
...
// Функция получает текст запроса
Функция ПолучитьТекстЗапросаДокументыИнвентаризации(Документ) Экспорт
	
	ТекстЗапроса = "ВЫБРАТЬ
	|	Инвентаризация.Ссылка КАК Документ,
	|	ВЫБОР
	|		КОГДА Инвентаризация.итЛокальнаяИнвентаризация
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК Порядок,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИТ_ДвиженияПакетаДокументовРезультатовИнвентаризации.ДокументРегистратор КАК Документ,
	|	2 КАК Порядок,
	|	ИТ_ДвиженияПакетаДокументовРезультатовИнвентаризации.ДокументРегистратор.Дата КАК ДатаДокумента
	|ПОМЕСТИТЬ втПакеты
	|ИЗ
	|	РегистрСведений.ИТ_ДвиженияПакетаДокументовРезультатовИнвентаризации КАК ИТ_ДвиженияПакетаДокументовРезультатовИнвентаризации
	|ГДЕ
	|	ИТ_ДвиженияПакетаДокументовРезультатовИнвентаризации.ДокументИнвентаризация В
	|			(ВЫБРАТЬ
	|				втИнвентаризации.Документ КАК Документ
	|			ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИТ_ФиксацияРезультатовИнвентаризацииПоПродавцам.Ссылка КАК Документ,
	|	3 КАК Порядок,
	|	ИТ_ФиксацияРезультатовИнвентаризацииПоПродавцам.Дата КАК ДатаДокумента
	|ПОМЕСТИТЬ втФиксации
	|ИЗ
	...
	|ВЫБРАТЬ
	|	втИнвентаризации.Документ КАК Документ,
	|	втИнвентаризации.Порядок КАК Порядок,
	|	втИнвентаризации.ДатаДокумента КАК ДатаДокумента
	|ИЗ
	|	втИнвентаризации КАК втИнвентаризации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	втПакеты.Документ,
	|	втПакеты.Порядок,
	|	втПакеты.ДатаДокумента
	|ИЗ
	|	втПакеты КАК втПакеты
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втФиксации.Документ,
	|	втФиксации.Порядок,
	|	втФиксации.ДатаДокумента
	|ИЗ
	|	втФиксации КАК втФиксации
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок";
