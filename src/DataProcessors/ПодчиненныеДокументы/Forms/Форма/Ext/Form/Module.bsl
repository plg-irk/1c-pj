...
Процедура СформироватьДеревоПодчиненныхДокументов()
	...
	НоваяСтрока.Картинка = ?(ДокументКорень.ПометкаУдаления, БиблиотекаКартинок.ДокументПомеченНаУдаление, ?(ДокументКорень.Проведен ИЛИ ДокументКорень.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить, БиблиотекаКартинок.ДокументПроведен, БиблиотекаКартинок.ДокументНеПроведен));
...
Процедура ДобавитьВДерево(Строка, ДокументКорень,ТаблицаКонтроляПовтора)
	...
	//Мальцев А.Ю. +++
	Если ТипЗнч(ДокументКорень) = Тип("ДокументСсылка.итПриказОПроведенииИнвентаризации") Тогда
		Запрос.УстановитьПараметр("ДатаНачала",   			  ДокументКорень.ДатаНачала);
		Запрос.УстановитьПараметр("ДатаОкончания",			  ДокументКорень.ДатаОкончания);
		Запрос.УстановитьПараметр("ПодразделениеКомпании",    ДокументКорень.ПодразделениеКомпании);
	КонецЕсли;	
	//Мальцев А.Ю. ---
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Картинка = ?(ТекущийДокумент.ПометкаУдаления, БиблиотекаКартинок.ДокументПомеченНаУдаление, ?(ТекущийДокумент.Проведен ИЛИ ТекущийДокумент.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить, БиблиотекаКартинок.ДокументПроведен, БиблиотекаКартинок.ДокументНеПроведен));
...
Процедура ВывестиДеревоДокументов()
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.СообщениеSMS") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.Встреча") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие")
		ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ТелефонныйЗвонок") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ЭлектронноеПисьмоИсходящее")
		//Мальцев А.Ю. +++
		ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.итПриказОПроведенииИнвентаризации") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ИТ_ПакетДокументовРезультатовИнвентаризации") ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ИТ_ФиксацияРезультатовИнвентаризацииПоПродавцам") Тогда
		//Мальцев А.Ю. ---
	...
	Область.Рисунки[0].Картинка       = ?(Документ.ПометкаУдаления, БиблиотекаКартинок.ДокументПомеченНаУдаление, ?(Документ.Проведен ИЛИ Документ.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить, БиблиотекаКартинок.ДокументПроведен, БиблиотекаКартинок.ДокументНеПроведен));
	ДеревоДокументов.Вывести(Область);
...
Процедура ВидТаблица(Команда)
	...
	Элементы.ИерархическийСписок.Видимость=Истина;
	Элементы.ДеревоДокументов.Видимость=Ложь;
	Элементы.ВидДерево.Пометка=Ложь;
	Элементы.Журнал.Пометка=Истина;
...
Процедура ВидДерево(Команда)
	...
	Элементы.ИерархическийСписок.Видимость=Ложь;
	Элементы.ДеревоДокументов.Видимость=Истина;
	Элементы.ВидДерево.Пометка=Истина;
	Элементы.Журнал.Пометка=Ложь;
