...
//
Процедура ВыполнитьВыражение(Выражение, ПользовательскиеДанные, Знач Ссылка, ТекущиеДанные=Неопределено, Результат=Неопределено)
...
Функция ЗагрузитьНастройки(НастройкаВыгрузки, УникальныйИдентификатор) Экспорт
	...
	Если Настройки=Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В выбранной настройке не выбрано ни одного поля для выгрузки.'"));
...
Функция ПолучитьДанныеДляВыгрузки(ОбработкаВладелец, АдресХранилищаПараметров) Экспорт
	...
	Для Каждого КолонкаДБФ Из н_ВыгружаемыеРеквизиты Цикл
		...
		Если КолонкаДБФ.Тип = "S" Тогда
			ОписаниеТипов = Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки (КолонкаДБФ.Длина, ДопустимаяДлина.Фиксированная));
		ИначеЕсли КолонкаДБФ.Тип = "N" Тогда
			ОписаниеТипов = Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла (КолонкаДБФ.Длина, КолонкаДБФ.Точность, ДопустимыйЗнак.Любой));
	...
	Если НЕ ПустаяСтрока(н_ПередВыгрузкойОбъекта) Тогда
		...
		Исключение
			...
			Описание = ?(ИнформацияОбОшибке.Причина<>Неопределено, ИнформацияОбОшибке.Причина.Описание, ИнформацияОбОшибке.Описание);
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= 'Ошибка при выполнении выражения ""Перед выгрузкой объекта""%1%2'"), Символы.ПС, Описание);
	...
	Для Каждого ТекущиеДанные Из ИсточникДанных Цикл
		...
		Для Каждого КолонкаДБФ Из н_ВыгружаемыеРеквизиты Цикл
			...
			Если КолонкаДБФ.ТипПоля = НСтр("ru = 'Поле объекта'") Тогда
				...
				Если ВРег(КолонкаДБФ.Представление)="ШТРИХКОД" Тогда
					...
					КонецЕсли;
				Иначе
					Выражение = "Результат = ТекущиеДанные."+КолонкаДБФ.Данные;
					Попытка
						РаботаВБезопасномРежиме.ВыполнитьВБезопасномРежиме(Выражение);
					Исключение 
			...
			Иначе //Выражение
				...
				Исключение
					ИнформацияОбОшибке = ИнформацияОбОшибке();
					Описание = ?(ИнформацияОбОшибке.Причина<>Неопределено, ИнформацияОбОшибке.Причина.Описание, ИнформацияОбОшибке.Описание);
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= 'Строка №%1 ошибка при выполнении выражения для колонки ""%2""%3%4'"), НомерСтроки, КолонкаДБФ.Представление, Символы.ПС, Описание);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
					Возврат Неопределено;
				КонецПопытки;
			КонецЕсли;
		...
		Если НЕ ПустаяСтрока(н_ПослеДобавленияСтроки) Тогда
			...
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				Описание = ?(ИнформацияОбОшибке.Причина<>Неопределено, ИнформацияОбОшибке.Причина.Описание, ИнформацияОбОшибке.Описание);
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= 'Строка №%1 ошибка при выполнении выражения ""После добавления строки""%2%3'"), НомерСтроки, Символы.ПС, Описание);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				Возврат Неопределено;
			КонецПопытки;
		КонецЕсли;
...
Функция ВыгрузитьДанныеВФайл(ДанныеДляВыгрузки, ИмяФайла, АдресХранилищаПараметров, АдресХранилища) Экспорт
	...
	Если ПустаяСтрока(ИмяФайла) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru= 'Не указан файл данных. Операция отменена.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого КолонкаДБФ Из н_ВыгружаемыеРеквизиты Цикл
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Описание = ?(ИнформацияОбОшибке.Причина<>Неопределено, ИнформацияОбОшибке.Причина.Описание, ИнформацияОбОшибке.Описание);
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= '%1Ошибка создания поля DBF %2 : %3'"), ТекстОшибки, КолонкаДБФ.ПолеДБ, Описание);
	...
	Если ВозниклиОшибки Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru= 'Операция отменена.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Описание = ?(ИнформацияОбОшибке.Причина<>Неопределено, ИнформацияОбОшибке.Причина.Описание, ИнформацияОбОшибке.Описание);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= 'Ошибка при создании файла ""%1"" : %2. Операция отменена'"), ИмяФайла, Описание);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		Возврат Ложь;
	КонецПопытки;
	
	Пока НЕ xDBF.Открыта() Цикл
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru= 'Возникла ошибка при открытии файла ""%1"". Возможно файл занят другим приложением'"), ИмяФайла);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		Возврат Ложь;
	КонецЦикла;
