...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ЦветГиперссылки = ЦветаСтиля.ЦветГиперссылкиБЭД;
				...
				Если ФайлУстановки.Существует() Тогда
					...
					Если ФайлУстановки.Размер() = ЭтотОбъект.РазмерДистрибутиваБайт Тогда
						...
						Если НЕ СовпадаетКонтрольнаяСумма(ЭтотОбъект.ИмяФайлаУстановки, ЭтотОбъект.КонтрольнаяСумма) Тогда
							...
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
								НСтр("ru='Файл дистрибутива поврежден (контрольная сумма загруженного дистрибутива отличается от указанной на сайте)'"));
						Иначе
							ОписаниеРезультата = Новый ФорматированнаяСтрока(
								ОписаниеРезультата,
								Символы.ПС,
					...
					Иначе
						...
						ОписаниеРезультата = Новый ФорматированнаяСтрока(
							ОписаниеРезультата,
							Символы.ПС,
							НСтр("ru='Файл дистрибутива поврежден (размер загруженного дистрибутива отличается от указанного на сайте)'"));
					КонецЕсли;
				Иначе
					ЕстьОшибки = Истина;
					ОписаниеРезультата = Новый ФорматированнаяСтрока(
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Активация Агента резервного копирования: успешно получены коды запуска, завершен первый запуск'"));
...
#Область Окончание
		...
		ОбновитьНадписиНаСервере(НСтр("ru='Все готово'"));
