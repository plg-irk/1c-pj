	...
	Если ЗначениеЗаполнено(ИскомыйОбъект) Тогда
		...
		Если ИскомыйОбъект<>Неопределено Тогда
			...
			ИначеЕсли Задачи.ТипВсеСсылки().СодержитТип(ТипЗнч(ИскомыйОбъект)) Тогда
				КоллекцияРеквизитов=ИскомыйОбъект.Метаданные().Реквизиты
			Иначе 
				ВывестиСообщение(НСтр("ru='Поиск по данному типу ссылки не определен.'"));
				Возврат;
			КонецЕсли;
	...
	Для Каждого ТекЭлемент Из МассивОбъектовДляЗамены Цикл
		Если ТекЭлемент.Родитель <> Неопределено Тогда
			...
			Если Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
				...
				Результат = ПоискЗаменаЗначенийСервер.ИзменитьДокумент(ТекЭлемент.Объект.ПолучитьОбъект(),Ссылка, ПравильныйЭлемент, ЭтотОбъект.ОтключитьКонтрольКорректностиПриЗаписи) И Результат;
				
			ИначеЕсли Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
				
				Результат = ПоискЗаменаЗначенийСервер.ИзменитьСправочник(ТекЭлемент.Объект.ПолучитьОбъект(),Ссылка, ПравильныйЭлемент, ЭтотОбъект.ОтключитьКонтрольКорректностиПриЗаписи) И Результат;
			...
			ИначеЕсли Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных)
				...
				Результат = ПоискЗаменаЗначенийСервер.ИзменитьДругиеОбъекты(ТекЭлемент.Объект.ПолучитьОбъект(),Ссылка,ПравильныйЭлемент, ЭтотОбъект.ОтключитьКонтрольКорректностиПриЗаписи) И Результат;
			...
			Иначе
				ВывестиСообщение(НСтр("ru='Ссылки типа %1 не заменяются.'"),,,,, ТекЭлемент.Ссылка);
			КонецЕсли;
		КонецЕсли;
		Если Не Результат Тогда
			ВывестиСообщение(НСтр("ru='Объект %1 не удалось записать.'"),,,,, ТекЭлемент.Ссылка);
		КонецЕсли;
		...
		Если (ОбъектДляУдаления <> Неопределено) И (Не Ссылка.ПометкаУдаления) Тогда
			...
			Если ОбщегоНазначения.ЭтоСправочник(ОбъектДляУдаления.Метаданные()) Или ОбщегоНазначения.ЭтоПланВидовХарактеристик(ОбъектДляУдаления.Метаданные())
				...
				Если ОбъектДляУдаления.Предопределенный Тогда
					ВывестиСообщение(НСтр("ru='Предопределенный элемент невозможно пометить на удаление.'"));
...
Процедура Поиск(Команда)
	Если ЗначениеЗаполнено(Найти) Тогда
		...
		НайтиВходящие(Найти);
	Иначе
		ВывестиСообщение(НСтр("ru='Не заданы параметры для поиска.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура Заменить(Команда)
	...
	Если Не ЗначениеЗаполнено(Найти) Тогда
		ВывестиСообщение(НСтр("ru='Не заполнено поле поиска.'"));
		Результат = Ложь;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Заменить) И Результат Тогда
		ВывестиСообщение(НСтр("ru='Для заменяемого объекта <%1> не указано значение замены.'"),,,,, Строка(Найти));
		Результат = Ложь;
	КонецЕсли;
	
	Если Заменить = Найти И Результат Тогда
		ВывестиСообщение(НСтр("ru='Для заменяемого объекта <%1> указано одинаковое значение замены.'"),,,,, Строка(Найти));
		Результат = Ложь;
	КонецЕсли;
	
	Если ПроверитьОтметкиВыбранныхОбъектов() И Результат Тогда
		ВывестиСообщение(НСтр("ru='Не выбрано ни одного объекта для замены.'"));
		Результат = Ложь;
	КонецЕсли;
