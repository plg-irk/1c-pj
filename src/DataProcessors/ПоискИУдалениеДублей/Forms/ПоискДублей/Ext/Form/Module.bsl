...
Процедура ПоказатьМестаИспользования(ДеревоИсточник)
	...
	Для Каждого ГруппаДублей Из ДеревоИсточник.ПолучитьЭлементы() Цикл
		Для Каждого СтрокаДерева Из ГруппаДублей.ПолучитьЭлементы() Цикл
			СписокСсылок.Добавить(СтрокаДерева.Ссылка);
		КонецЦикла;
	КонецЦикла;
	
	ПараметрыОтчета = Новый Структура;
	ПараметрыОтчета.Вставить("Отбор", Новый Структура("НаборСсылок", СписокСсылок));
...
Процедура ИнициализироватьКомпоновщикОтбораИПравила(НастройкиФормы)
	...
	// +РАРУС Добавим правила по общим реквизитам объекта
	...
	НаборДанных.Запрос = "ВЫБРАТЬ " + СтрСоединить(МассивИменаРеквизитов, ",") + " ИЗ " + ОбластьПоискаДублей;
	// -РАРУС
	...
	// +РАРУС
	ДобавитьПравилаМетаРеквизитов(ТаблицаПравил, ИгнорируемыеРеквизиты, ВсеВариантыСравнения, МассивОбщихРеквизитов, НечеткийПоиск);
	// -РАРУС
