...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Помощник не предназначен для использования в режиме веб-клиента.'"));
		Отказ = ИСТИНА;
		Возврат;
	#Иначе
...
//  Событие				 - Строка	 - События, указывающиеся в журнале регистрации
...
Функция ВстроитьТЦ() Экспорт
	...
	Если Объект.ОбновитьФайлыЦелевойКонфигурации И Объект.ПрименитьИзмененияКИБ Тогда
		...
		СтрокаЗапуска	= СтрокаЗапускаКонфигуратора + """ /DumpConfigToFiles """ + Объект.КаталогЦелевойКонфигурации + """ /Visible";
	...
	Если Объект.ПрименитьИзмененияКИБ Тогда
		...
		СтрокаЗапуска	= СтрокаЗапускаКонфигуратора + """ /LoadConfigFromFiles """ + Объект.КаталогЦелевойКонфигурации + """ /Visible";
		КодВозврата = ЗапуститьКлиента(СтрокаЗапуска, Объект.КаталогКонфигуратораИКлиента, Истина,
...
Функция УдалитьЭлементыТЦ(КаталогКонфигурации)
	...
	Событие = НСтр("ru = 'Удаление ТЦ перед встраиванием'");
	Префикс = "xmlns:";
...
Функция УдалитьСтрокиТЦИзМодулей(КаталогКонфигурации)
	...
	Событие = НСтр("ru = 'Удаление кода ТЦ перед встраиванием'");
...
Функция ВстроитьЭлементыТЦ(Знач КаталогКонфигурации, Знач КаталогТестЦентра) Экспорт
	...
	// Создаем подсистему ТестЦентр в configuration.xml целевой конфигурации.
	...
	// Включаем режимы использования модальности и синхронных вызовов.
	// Это необходимо для корректной работы Тест-центра
	//
	Модальность = ДокументDOMКонфигурации.ПолучитьЭлементыПоИмени("ModalityUseMode");
	Если Модальность.Количество() > 0 Тогда
		Модальность[0].ТекстовоеСодержимое = "Use";
	КонецЕсли;
	
	СинхронныеВызовы = ДокументDOMКонфигурации.ПолучитьЭлементыПоИмени("SynchronousExtensionAndAddInCallUseMode");
	Если СинхронныеВызовы.Количество() > 0 Тогда
		СинхронныеВызовы[0].ТекстовоеСодержимое = "Use";
	КонецЕсли;
...
Функция ПолучитьМассивПроцедурИФункций(ТекстМодуля, ПолучатьИсходныйКод = Истина)
	...
	СписокПроцедурИФункций = Новый Массив;
	...
	Пока ЭлементКода <> Неопределено Цикл
		...
		Если ТипОбласти <> СтарыйТипОбласти Тогда
			...
			Если ТипОбласти = "ЗаголовокПроцедуры" Тогда
				...
				СписокПроцедурИФункций.Добавить(ТекущаяПроцедураИлиФункция);
	...
	Возврат СписокПроцедурИФункций;
...
Функция СохранитьОбработкуВФайл(ИмяМакета)
	...
	Исключение
		...
		СообщитьПользователю(ТекстОшибки);
		ЗаписатьОшибкуВЖурнал(ТекстОшибки, НСтр("ru = 'Обработка выгрузка и загрузки данных'"));
		Возврат Неопределено;
	КонецПопытки;
...
Функция УстановитьЗначениеИзСоответствия(Знач ИмяНастройки, Соответствие, ПеременнаяНастройки)
	...
	Если Значение <> Неопределено Тогда
		...
		Исключение
			...
			СообщитьПользователю(ТекстОшибки);
			ЗаписатьОшибкуВЖурнал(ТекстОшибки, НСтр("ru = 'Параметры командной строки'"));
			Возврат Ложь;
...
Процедура ЗаписатьРезультатВФайл(ПутьКФайлуРезультата, РезультатТекст, НужноЗавершитьРаботу = Ложь)
	...
	Если Не ПустаяСтрока(ПутьКФайлуРезультата) Тогда
		...
		Исключение
			...
			ТекстОшибки	= ТекстОшибки + Символы.ПС + ОписаниеОшибки();
			ЗаписатьОшибкуВЖурнал(ТекстОшибки, НСтр("ru = 'Загрузка конфигурации из файлов'"));
		КонецПопытки;
