...
Процедура УстановитьВидимостьНастроекЗагрузки()
	Если ОтображениеНастроекЗагрузки Тогда
        ...
        ПодсветитьВыбраннуюКолонку("ТаблицаЗагружаемыхРеквизитов", ИзмененаРучнаяНумерацияКолонок, РазвернутТабличныйДокумент);
	Иначе
...
//
&НаСервере
Процедура ПодсветитьВыбраннуюКолонку(Знач ИмяТаблицыФормы, ИзмененаРучнаяНумерацияКолонок, РазвернутТабличныйДокумент)
	
	Если ИмяТаблицыФормы = Неопределено Тогда
		ТекТаблицаФормы = Неопределено;
	Иначе
		ТекТаблицаФормы = Элементы[ИмяТаблицыФормы];
	КонецЕсли;
	...
	Если Элементы.ТаблицаЗагружаемыхРеквизитов.ТекущаяСтрока = Неопределено Тогда
		ТекущиеДанные = Неопределено;
	Иначе
		ТекущиеДанные = ТаблицаЗагружаемыхРеквизитов.НайтиПоИдентификатору(Элементы.ТаблицаЗагружаемыхРеквизитов.ТекущаяСтрока);
	КонецЕсли;
	Если ТекТаблицаФормы = Неопределено И (ИзмененаРучнаяНумерацияКолонок ИЛИ РазвернутТабличныйДокумент) И ТекущиеДанные<>Неопределено Тогда
		РазвернутТабличныйДокумент = Ложь;
		ИзмененаРучнаяНумерацияКолонок = Ложь;
		ТекТаблицаФормы = Элементы.ТаблицаЗагружаемыхРеквизитов;
		ИмяТаблицыФормы = "ТаблицаЗагружаемыхРеквизитов";
	КонецЕсли;
	...
	Иначе
		Если ТекТаблицаФормы.ТекущаяСтрока = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ТекСтрока = ЭтотОбъект[ИмяТаблицыФормы].НайтиПоИдентификатору(ТекТаблицаФормы.ТекущаяСтрока);
...
Процедура ЗаполнитьМассивКнопок()
	...
	Для Каждого Кнопка Из МассивКнопокДляУдаления Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при удалении элемента формы:'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + " " + СокрЛП(Кнопка.Имя), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецЦикла;
...
Процедура ЗагрузитьДанныеВФорме(ТабличныйДокумент, ОписаниеОшибок ="", НастройкиЗагрузкиНоменклатурыИскатьПо = "")
	...
	Если Элементы.СохранитьСоответствияВПрайсЛистеПоставщика.Доступность И СохранитьСоответствияВПрайсЛистеПоставщика Тогда
		Если НЕ ЗначениеЗаполнено(ПрайсЛистКонтрагента) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан прайс-лист поставщика'")); 
			Возврат;
		КонецЕсли;
...
Процедура ПоказатьНастройки(Команда)
	...
	ПодсветитьВыбраннуюКолонку(Неопределено, ИзмененаРучнаяНумерацияКолонок, РазвернутТабличныйДокумент);
КонецПроцедуры
...
Процедура КомандаДалее(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница=Элементы.СтраницаВыборНастройки Тогда
		...
		Попытка
			...
			КонецЕсли;
		Исключение
			ТехнологическаяПлатформаВызовСервера.СделатьЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка получения данных из хранилища'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		...
		СтрокиНаименования = ТаблицаЗагружаемыхРеквизитов.НайтиСтроки(Новый Структура("ИмяРеквизита", "Наименование"));
		Если СтрокиНаименования.Количество()>0 Тогда
			НомерКолонкиНаименованиеТабличногоДокумента = СтрокиНаименования[0].НомерКолонки;
		КонецЕсли;
		...
		ДополнительныеПараметры.Вставить("ОписаниеОшибок",            ОписаниеОшибок);
		ДополнительныеПараметры.Вставить("ОтборСтрок",                ОтборСтрок);
		ДополнительныеПараметры.Вставить("ФайлНаДиске",               ФайлНаДиске);
		ДополнительныеПараметры.Вставить("Инкремент",                 Инкремент);
		...
		Если СтрокаНастройкиЗагрузкиНоменклатуры=Неопределено ИЛИ СтрокаНастройкиЗагрузкиНоменклатуры.РежимЗагрузки<>"Искать" Тогда
			...
			Попытка
				...
				Если Объект.ДействиеСЗагружаемымФайлом = 0 Тогда
					КаталогАрхиваПеремещаемыхФайлов = Объект.КаталогАрхиваЗагруженныхФайлов+"\"+Строка(Формат(ОбщегоНазначенияКлиент.ДатаСеанса(),"ДФ=""дд-ММ-гггг"""));
					СоздатьКаталог(КаталогАрхиваПеремещаемыхФайлов);
					ПереместитьФайл(ФайлДанных, КаталогАрхиваПеремещаемыхФайлов+"\"+Файл.Имя);
			...
			Исключение
				...
				Пока ИнформацияОбОшибке<>Неопределено Цикл
					СтрокаОшибки = СтрокаОшибки+"   "+ИнформацияОбОшибке.Описание+Символы.ПС;
					ИнформацияОбОшибке = ИнформацияОбОшибке.Причина;
				КонецЦикла;
				ПоказатьОповещениеПользователя(СтрокаОшибки); //Загрузку все равно произвели, можно закрывать форму
			КонецПопытки;
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница=Элементы.СтраницаСоответствияНоменклатуры Тогда
		...
		//Мальцев А.Ю. +++
		Оповестить("ПечатьЭтикетокЗагрузка", АдресТаблицаЗагрузкаИЗФайлаВХранилище);
		//Мальцев А.Ю. ---
...
Процедура КомандаДалееПомещениеФайлаВХранилищеНаСервере(АдресВоВременномХранилище, Расширение, ИмяБезРасширения)
	...
	Если Объект.ИспользоватьВнешнийМодульПреобразованияФайла И ЗначениеЗаполнено(Объект.ОбработкаПреобразования) Тогда
		ЗагрузкаИзЭлектронныхДокументовВызовСервера.ЗаполнитьТабличныйДокументИзXMLСервер(ФайлДанных, Объект.ОбработкаПреобразования, Объект.ТабличныйДокумент, АдресВоВременномХранилище);
	Иначе
		ЗагрузкаИзЭлектронныхДокументовВызовСервера.ПрочитатьТабличныйДокумент(Объект.ТабличныйДокумент, Расширение, ИмяБезРасширения, АдресВоВременномХранилище, Объект.Табуляция, Объект.Кодировка);
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ - ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ЗАГРУЖАЕМЫХ РЕКВИЗИТОВ
...
Процедура ТаблицаЗагружаемыхРеквизитовНомерКолонкиПриИзменении(Элемент)
	...
	Если Элементы.ТаблицаЗагружаемыхРеквизитов.ТекущиеДанные.ИмяРеквизита = "Наименование" Тогда
		НомерКолонкиНаименованиеТабличногоДокумента = Элементы.ТаблицаЗагружаемыхРеквизитов.ТекущиеДанные.НомерКолонки;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТаблицаЗагружаемыхРеквизитовПриАктивизацииСтроки()
...
Процедура ТаблицаЗагружаемыхРеквизитовПриАктивизацииСтроки(Элемент)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ТаблицаЗагружаемыхРеквизитовПриАктивизацииСтроки", 0.5, ИСТИНА);
...
&НаКлиенте
Процедура Подключаемый_ТабличныйДокументПриАктивизацииОбласти()
...
Процедура ТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ТабличныйДокументПриАктивизацииОбласти", 0.5, ИСТИНА);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.СсылкаИсточника.Пустая() Тогда
		Попытка
			ДокОбъект = ДанныеФормыВЗначение(Параметры.ОбъектИсточника.Объект,Тип("ДокументОбъект." + Параметры.СсылкаИсточника.Метаданные().Имя));
			ДокОбъект.ОбменДанными.Загрузка = Истина;
			ДокОбъект.Записать();
			
			Объект.СсылкаИсточника = ДокОбъект.Ссылка;
		Исключение
		КонецПопытки;
	КонецЕсли;	
