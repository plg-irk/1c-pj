...
Процедура ВыполнитьЗагрузкуНастроек(АдресВременногоХранилища)
	...
	Если Не ВсеОк Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Загружаемые настройки не являются настройками выбранного объекта.'"));
		Возврат;
	КонецЕсли;
	мОбновитьДанныеТабличногоДокумента(Объект.ТабличныйДокумент, Истина);
...
Процедура ТипОбъектаПриИзменении(Элемент)
	...
	Если Объект.РежимЗагрузки = 0 И ТипЗнч(Объект.СсылкаИсточника) = Тип("СправочникСсылка.Номенклатура") Тогда
		Объект.ПриЗаписиОбъекта = "
...
Процедура ЗагрузитьНастройкиИзФайла(Команда)
	...
	НачатьПомещениеФайла(Новый ОписаниеОповещения("ЗагрузитьНастройкиИзФайлаЗавершение", ЭтотОбъект, Новый Структура("АдресВоВременномХранилище", АдресВоВременномХранилище)), АдресВоВременномХранилище, ДиалогВыбораФайла.ПолноеИмяФайла, Ложь, Новый УникальныйИдентификатор);
...
Процедура ОткрытьФайл(Команда)
	...
	Если Объект.ИспользоватьВнешнийМодульПреобразованияФайла Тогда
		ЗаполнитьТабличныйДокументИзXML(Объект.ОбработкаПреобразования, КаталогЗагружаемогоФайла, ИндексТипаЗагружаемогоФайла);
	Иначе
		ЗаполнитьТабличныйДокументИзТабличногоФайла(КаталогЗагружаемогоФайла, ИндексТипаЗагружаемогоФайла, Объект.Табуляция, Объект.Кодировка);
	КонецЕсли;
...
//
&НаКлиенте
Процедура ЗаполнитьТабличныйДокументИзXML(ОбработкаПреобразования,КаталогЗагружаемогоФайла, ИндексТипаЗагружаемогоФайла) Экспорт
	...
	Если ДиалогВыбораФайла.Выбрать() Тогда
		...
		АдресВХранилище 			 = "";
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ЗаполнитьТабличныйДокументИзXMLЗавершение", ЭтотОбъект, Новый Структура("ДиалогВыбораФайла, ОбработкаПреобразования", ДиалогВыбораФайла, ОбработкаПреобразования)), АдресВХранилище, ДиалогВыбораФайла.ПолноеИмяФайла, Ложь, Новый УникальныйИдентификатор);
...
Процедура ЗаполнитьТабличныйДокументИзXMLЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
    ...
    ЗагрузкаИзЭлектронныхДокументовВызовСервера.ЗаполнитьТабличныйДокументИзXMLСервер(ДиалогВыбораФайла.ПолноеИмяФайла, ОбработкаПреобразования, Объект.ТабличныйДокумент, АдресВХранилище);
...
//
&НаКлиенте
Процедура ЗаполнитьТабличныйДокументИзТабличногоФайла(КаталогЗагружаемогоФайла, ИндексТипаЗагружаемогоФайла, Табуляция, Кодировка) Экспорт
	...
	ДиалогВыбораФайла.Заголовок = НСтр("ru = 'Прочитать табличный документ из файла'");
	ДиалогВыбораФайла.Фильтр    = НСтр("ru = 'dBase III (*.dbf)|*.dbf|Лист Excel (*.xls)|*.xls|Лист Excel 2007 (*.xlsx)|*.xlsx|Текстовый документ (*.txt)|*.txt|Текстовый документ (*.dat)|*.dat|Табличный документ (*.mxl)|*.mxl|Формат sst (*.sst)|*.sst|Формат mmo (*.mmo)|*.mmo|'");
	...
	Если ДиалогВыбораФайла.Выбрать() Тогда
		...
		НачатьПомещениеФайла(Новый ОписаниеОповещения("ЗаполнитьТабличныйДокументИзТабличногоФайлаЗавершение", ЭтотОбъект, ДополнительныеПараметры),
			АдресВоВременномХранилище,
...
&НаКлиенте
Процедура ЗаполнитьТабличныйДокументИзТабличногоФайлаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
    ...
    ЗагрузкаИзЭлектронныхДокументовВызовСервера.ПрочитатьТабличныйДокумент(Объект.ТабличныйДокумент, ФайлНаДиске.Расширение, ФайлНаДиске.ИмяБезРасширения, АдресВоВременномХранилище, Табуляция, Кодировка);
