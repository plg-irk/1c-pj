...
Процедура ТаблицаРасширенияПравДоступаЗначениеБулевоПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ТекущиеДанные.Значение = ?(ТекущиеДанные.ЗначениеБулево = 1,Истина,?(ТекущиеДанные.ЗначениеБулево = 0,Ложь,Неопределено));
	КонецЕсли;
...
Процедура ТаблицаРасширенияПравДоступаСинонимОбъектаПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если СтрокиСВыбраннымОбъектом.Количество() > 1 Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выбранный объект уже есть в таблице.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура Применить(Команда = Неопределено, Отказ = Ложь)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнен объект в строке %1'"), ТаблицаРасширенияПравДоступа.Индекс(СтрокиСПустымОбъектом[0]) + 1));
		Отказ = Истина;
	КонецЕсли;
