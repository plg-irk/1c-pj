...
Функция ТЦВыполнить() Экспорт
	...
	#Иначе
		...
		ВывестиСообщение(НСтр("ru = '" + ИДКлиента +"'"));
		...
		Пока ВремяСейчас < ВремяКонцаРаботы Цикл
			...
			Попытка
				...
				ФормаДокумента = ПолучитьФорму("Документ.ЗаказПокупателя.ФормаОбъекта");
				ДанныеФормы = ФормаДокумента.Объект;
				СформироватьДокументЗаказ(ДанныеФормы, "ЗаказПокупателя");
				ДанныеФормы.Комментарий = "Заказы" + ДанныеФормы.Автор + "Цепочка" + ПовторЦепочки;
				КопироватьДанныеФормы(ДанныеФормы, ФормаДокумента.Объект);
				ФормаДокумента.Открыть();
				
				// Запишем и закроем форму
				ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПроведениеЗаказПокупателя");
				ФормаДокумента.Записать(Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение));
				ЗаказПокупателяСсылка = ФормаДокумента.Объект.Ссылка;
				...
				// Откроем форму документа и заполним документ.
				ФормаДокумента = ПолучитьФорму("Документ.ЗаказВнутренний.ФормаОбъекта",Новый Структура("Основание",ЗаказПокупателяСсылка));
				ДанныеФормы = ФормаДокумента.Объект;
				СформироватьДокументЗаказВнутренний(ДанныеФормы,"ЗаказВнутренний");
				ДанныеФормы.Комментарий = "Заказы" + ДанныеФормы.Автор + "Цепочка" + ПовторЦепочки;
				КопироватьДанныеФормы(ДанныеФормы, ФормаДокумента.Объект);
				ФормаДокумента.Открыть();
				
				// Запишем и закроем форму.
				ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПроведениеЗаказВнутренний");
				ФормаДокумента.Записать(Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение));
				ЗаказВнутреннийСсылка = ФормаДокумента.Объект.Ссылка;
				ОценкаПроизводительностиКлиент.ЗакончитьЗамерВремениАвтоНеГлобальный();
				ФормаДокумента.Закрыть();
				...
				// Откроем форму документа и заполним документ.
				ФормаДокумента = ПолучитьФорму("Документ.ЗаказПоставщику.ФормаОбъекта",Новый Структура("Основание",ЗаказВнутреннийСсылка));
				ДанныеФормы = ФормаДокумента.Объект;
				ДанныеФормы.Комментарий = "Заказы" + ДанныеФормы.Автор + "Цепочка" + ПовторЦепочки;
				КопироватьДанныеФормы(ДанныеФормы, ФормаДокумента.Объект);
				ФормаДокумента.Открыть();
				...
				// Запишем и закроем форму
				ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("ПроведениеЗаказПоставщику");
				ФормаДокумента.Записать(Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение));
				ЗаказПоставщикуСсылка = ФормаДокумента.Объект.Ссылка;
				ОценкаПроизводительностиКлиент.ЗакончитьЗамерВремениАвтоНеГлобальный();
				ФормаДокумента.Закрыть();
				...
				// Поступление товаров
				// Откроем форму документа и заполним документ
				ФормаДокумента = ПолучитьФорму("Документ.ПоступлениеТоваров.ФормаОбъекта",Новый Структура("Основание",ЗаказПоставщикуСсылка));
				ДанныеФормы = ФормаДокумента.Объект;
				ЗафиксироватьПоступлениеТоваров(ДанныеФормы);
				ДанныеФормы.Комментарий = "Заказы" + ДанныеФормы.Автор + "Цепочка" + ПовторЦепочки;
				КопироватьДанныеФормы(ДанныеФормы, ФормаДокумента.Объект);
				ФормаДокумента.Открыть();
				...
				// Откроем форму документа и заполним документ.
				ФормаДокумента = ПолучитьФорму("Документ.ПеремещениеТоваров.ФормаОбъекта",Новый Структура("Основание",ЗаказВнутреннийСсылка));
				ДанныеФормы = ФормаДокумента.Объект;
				ПереместимЗаказ(ДанныеФормы);
				ДанныеФормы.Комментарий = "Заказы" + ДанныеФормы.Автор + "Цепочка" + ПовторЦепочки;
				КопироватьДанныеФормы(ДанныеФормы, ФормаДокумента.Объект);
				ФормаДокумента.Открыть();
				...
				//////////////////////////////////////////////////////////////////////////////////
				// ПКО или Чек на оплату или Выписка
...
///////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ЭМУЛЯЦИИ РАБОТЫ ЦЕПОЧКИ ЗАКАЗЫ
...
Процедура ЗаполнитьТаблицуНоменклатуры()
	...
	ВидыНоменклатуры = Справочники.ХозОперации.ПолучитьВидыНоменклатуры(Справочники.ХозОперации.ЗаказПокупателя);
...
&НаСервере
Процедура СформироватьДокументЗаказ(ДокументОбъект, ИмяДокумента)
	...
	Документы[ИмяДокумента].ХозОперацияПриИзменении(ДокументОбъект);
	
	ДокументОбъект.ПроцентПредоплаты = 0;
	Документы[ИмяДокумента].ПроцентПредоплатыПриИзменении(ДокументОбъект);
	...
	КоличествоСтрок = ?(КоличествоНоменклатуры < ТЦОбъект.КоличествоСтрокПрихода, КоличествоНоменклатуры, ТЦОбъект.КоличествоСтрокПрихода);
	...
	Пока Добавлено < КоличествоСтрок Цикл
		...
		КонецЕсли;
		НоваяСтрока.Количество = ГСЧ.СлучайноеЧисло(3, 50);
...
&НаСервере
Процедура ЗафиксироватьПоступлениеТоваров(ДокументОбъект)
	
	ДокументОбъект.СкладКомпании = ОбычныйСклад;
	Документы.ПоступлениеТоваров.СкладКомпанииПриИзменении(ДокументОбъект);
...
&НаСервере
Процедура СформироватьДокументЗаказВнутренний(ДокументОбъект, ИмяДокумента)
	
	ТекущаяДата = ТекущаяДата();
	Время = ТекущаяДата - НачалоДня(ТекущаяДата);
	ГСЧ = Новый ГенераторСлучайныхЧисел(ИДКлиента + Время);
	
	ДокументОбъект.ХозОперация = Справочники.ХозОперации.ЗаказВнутренний;
	Документы[ИмяДокумента].ХозОперацияПриИзменении(ДокументОбъект);
	
	// чтобы потом проще сделать перемещение
	ДокументОбъект.СкладКомпании =ОбычныйСклад;
...
КонецПроцедуры // СформироватьДокументЗаказВнутренний()

&НаСервере
Процедура СформироватьДокументРезервированиеЗаказов(ДокументОбъект, ИмяДокумента)
	
	ТекущаяДата = ТекущаяДата();
	Время = ТекущаяДата - НачалоДня(ТекущаяДата);
	ГСЧ = Новый ГенераторСлучайныхЧисел(ИДКлиента + Время);
	
	// Не можем резервировать больше чем есть на складе
	Для каждого Строка Из ДокументОбъект.Товары Цикл
		Если Строка.Количество > Строка.ОстатокНаСкладе Тогда
			Строка.Количество = ?(Строка.ОстатокНаСкладе<1, 0, Строка.ОстатокНаСкладе);
			Документы[ИмяДокумента].ТоварыКоличествоПриИзменении(ДокументОбъект,Строка);
		КонецЕсли;
	КонецЦикла;
	
	// Нет остатков
	УдаляемыеСтроки = ДокументОбъект.Товары.НайтиСтроки(Новый Структура("Количество",0));
...
КонецПроцедуры // СформироватьДокументРезервированиеЗаказов()

&НаСервере
Процедура ПереместимЗаказ(ДокументОбъект)
	
	ТекущаяДата = ТекущаяДата();
	Время = ТекущаяДата - НачалоДня(ТекущаяДата);
	ГСЧ = Новый ГенераторСлучайныхЧисел(ИДКлиента + Время);
	
	ДокументОбъект.СкладПолучатель = РозничныйСклад;
	Документы.ПеремещениеТоваров.СкладКомпанииПриИзменении(ДокументОбъект);
	
	// Заполним товары
	Для каждого Строка Из ДокументОбъект.Товары Цикл
		
		Если Строка.ОстатокНаСкладе < 1 Тогда
			ДокументОбъект.Товары.Удалить(Строка);
			Продолжить;
		КонецЕсли;
		...
		// Оприходуем товар
		МассивНоменклатуры = ТаблицаНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура,ХарактеристикаНоменклатуры,СкладКомпании",Строка.Номенклатура,Строка.ХарактеристикаНоменклатуры,ДокументОбъект.СкладПолучатель));
		Если МассивНоменклатуры.Количество() >0 Тогда
			МассивНоменклатуры[0].КоличествоОстаток = МассивНоменклатуры[0].КоличествоОстаток + Строка.Количество;
			МассивНоменклатуры[0].ЕстьОстаток = (МассивНоменклатуры[0].КоличествоОстаток>0);
		КонецЕсли;
...
Процедура СформироватьДокументРасход(ДокументОбъект, ИмяДокумента)
	...
	Если НЕ ЗначениеЗаполнено(ДокументОбъект.ПодразделениеКомпании) Тогда
		...
		Документы[ИмяДокумента].ПодразделениеКомпанииПриИзменении(ДокументОбъект);
	КонецЕсли;
	ДокументОбъект.СкладКомпании = РозничныйСклад;
	Документы[ИмяДокумента].СкладКомпанииПриИзменении(ДокументОбъект);
	...
	КоличествоСтрок = ?(КоличествоНоменклатуры < ТЦОбъект.КоличествоСтрокРасход, КоличествоНоменклатуры, ТЦОбъект.КоличествоСтрокРасход); 
	МассивДобавленныхСтрока = Новый Массив;
...
КонецПроцедуры // СформироватьДокументРасход()
