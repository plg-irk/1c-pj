...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	EmailРегистрацииСайта = Обработки.СоздатьСайт.ПолучитьКонтактнуюИнформацию(
		ТекПользователь, 
		"EmailПользователя");
		
	ИмяСправочникаВидыЦен = ОбменССайтомПовтИсп.ИмяПрикладногоСправочника("ВидыЦен");
		
	ВидЦен = Справочники[ИмяСправочникаВидыЦен].НайтиПоНаименованию("Розничная");
	...
	НадписьИмяСайтаДоступно = ?(ИмяСайта="","",ОбновитьНадписьИмяСайтаДоступно(ИмяСайтаПроверено, ИмяСайтаСвободно));
...
Процедура ПриОткрытии(Отказ)
	...
	Для нСтроки=1 По СтрДлина(СтрСимволы) Цикл
		РазрешенныеСимволыИмениСайта.Вставить(Сред(СтрСимволы, нСтроки, 1), Истина);
	КонецЦикла;
...
Процедура СоздатьСайт(Команда)
	...
	ОткрытьФорму("Обработка.СоздатьСайт.Форма.ДлительнаяОперация",,ЭтотОбъект,,,,ОписаниеОповещения);
