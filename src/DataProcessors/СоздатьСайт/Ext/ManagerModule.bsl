...
Функция СоздатьСайт(ВходящиеПараметры, ФоновоеЗаданиеАдресХранилища = "") Экспорт
	...
	ДанныеСайта = ПолучитьДанныеСозданияСайта(ВходящиеПараметры);
...
Функция ПолучитьДанныеСозданияСайта(ВходящиеПараметры) Экспорт
...
Процедура СоздатьУзелОбменаССайтом(ВходящиеПараметры, АдресСайта, ИДСайта, Пароль="")
	...
	ПланОбменаСайт.РежимВыгрузки				= 0;
	...
	ПланОбменаСайт.ОтборГруппыКатегорииНоменклатуры = ВходящиеПараметры.ОтборКатегорииГруппыПереключатель;
...
Процедура ЗаписатьКонтактнуюИнформацию(АдресСайта, ПарольСайта, ОсновнаяОрганизация, ТипСайта)
	...
	ФактАдресОрганизации = ПолучитьКонтактнуюИнформацию(ОсновнаяОрганизация, "ФактАдресОрганизации");
	ТелефонОрганизации = ПолучитьКонтактнуюИнформацию(ОсновнаяОрганизация, "ТелефонОрганизации");
	ФаксОрганизации = ПолучитьКонтактнуюИнформацию(ОсновнаяОрганизация, "ФаксОрганизации");
	EmailОрганизации = ПолучитьКонтактнуюИнформацию(ОсновнаяОрганизация, "EmailОрганизации");
	SkypeОрганизации = ПолучитьКонтактнуюИнформацию(ОсновнаяОрганизация, "Skype");
...
Функция ПолучитьКонтактнуюИнформацию(ОбъектКонтактнойИнформации, ВидИнформации) Экспорт
	...
	ИмяСправочникаКонтрагенты = ОбменССайтомПовтИсп.ИмяПрикладногоСправочника("Контрагенты");
	Если ТипЗнч(ОбъектКонтактнойИнформации) = Тип("СправочникСсылка.Организации") Тогда
		
		Возврат АдресОбъектаКонтактнойИнформации(ОбъектКонтактнойИнформации);
		
	ИначеЕсли ТипЗнч(ОбъектКонтактнойИнформации) = Тип("СправочникСсылка."+ИмяСправочникаКонтрагенты) Тогда
...
Функция АдресОбъектаКонтактнойИнформации(ОбъектКонтактнойИнформации)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		МетаданныеАдресЭлектроннойПочты = МетаданныеТипыКонтактнойИнформации.ЗначенияПеречисления.Найти("Адрес");
		
		Если Не (МетаданныеТипыКонтактнойИнформации = Неопределено
			ИЛИ МетаданныеАдресЭлектроннойПочты = Неопределено
			ИЛИ МодульКонтактнойИнформации = Неопределено) Тогда
			
			ТипыКонтактнойИнформации = Перечисления["ТипыКонтактнойИнформации"];
			Адрес = ТипыКонтактнойИнформации["Адрес"];
			
			ЗначениеАдрес = МодульКонтактнойИнформации.ЗначенияКонтактнойИнформацииОбъекта(ОбъектКонтактнойИнформации, Адрес);
			
			Для Каждого ТекСтрока Из ЗначениеАдрес Цикл
				Адрес.Добавить(ТекСтрока.Значение);
			КонецЦикла;
