...
Функция ПечатьКассоваяКнига (МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	НаименованиеПолноеОрганизации = СокрЛП(ОбщегоНазначения.ПолучитьНаименованияНаДату(Объект.Организация, Объект.Период.ДатаНачала, "НаименованиеДляПечатныхФорм"));
			...
			Пока ВыборкаГоды.Следующий() Цикл
				...
				Пока ВыборкаМесяца.Следующий() Цикл
					...
					Пока ВыборкаДни.Следующий() Цикл
						...
						КассоваяКнига.Присоединить(ОбластьМакетаОтчетКассира);
						...
						Если ВыборкаИтогиДни.Количество() > 0 Тогда
						...
						Пока ВыборкаДокументы.Следующий() Цикл
							...
							ОбластьМакетаСтрока.Параметры.Контрагент = Строка(ВыборкаДокументы.СтрокаДДС) + ВыборкаДокументы.Контрагент + ?(Объект.ВыводитьОснования," " + ВыборкаДокументы.Основание,"");
				...
				Если Объект.ПересчитыватьНомера Тогда
					Номера.ОбменДанными.Загрузка = Истина;					
					Номера.Записать();
				КонецЕсли;
		...
		Пока ВыборкаГоды.Следующий() Цикл
			...
			Пока ВыборкаМесяца.Следующий() Цикл
				...
				Пока ВыборкаДни.Следующий() Цикл
					...
					//Если ВыборкаИтогиДни.Строки.Количество() > 0 Тогда
					//	ВыборкаИтогиВалюты = ВыборкаИтогиДни.Строки[счИтоги].Строки;
					//	счИтоги = счИтоги + 1;
					ВыборкаИтогиВалюты = ВыборкаИтогиДни.Строки.Найти(ВыборкаДни.Период,"Период");
					Если НЕ ВыборкаИтогиВалюты = Неопределено Тогда
						ВыборкаИтогиВалюты = ВыборкаИтогиВалюты.Строки;
						Для Каждого СтрокаИтогиВалюты Из ВыборкаИтогиВалюты Цикл
							ОбластьМакетаОстатокНаНД.Параметры.НачальныйОстаток = СтрокаИтогиВалюты.НачальныйОстаток;
							Если Многовалютная Тогда
								ОбластьМакетаОстатокНаНД.Параметры.Валюта = " (" + СтрокаИтогиВалюты.Валюта + ")";
						...
						КонецЦикла;
					Иначе
						ВыборкаИтогиВалюты = Новый ТаблицаЗначений();
					КонецЕсли;
	...
	КассоваяКнига.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
	
	КассоваяКнига.АвтоМасштаб = Истина;
