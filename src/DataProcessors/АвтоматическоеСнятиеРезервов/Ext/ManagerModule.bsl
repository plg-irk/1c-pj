...
Процедура СоздатьНапоминание(НаборЗаписей, ТаблицаЗаказов, СнятиеРезервов=Неопределено, СрокДоРезервирования = 0) Экспорт
	...
	Пока ВыборкаПользователи.Следующий() Цикл
		...
		Пока ВыборкаЗаказы.Следующий() Цикл;
			...
			КонецЕсли;
		КонецЦикла;
		СтрЗаказы = ?(ВыборкаЗаказы.Количество()=1, НСтр("ru = 'по заказу'") + " " + СтрЗаказы, НСтр("ru = 'по заказам'") + " " + СтрЗаказы);
		
		Если СнятиеРезервов<>Неопределено Тогда
			Описание = НСтр("ru = 'Сформирован документ снятия резервов <'") + Строка(СнятиеРезервов) + "> " + СтрЗаказы;
		Иначе
			Если СрокДоРезервирования = 0 Тогда
				Описание = НСтр("ru = 'Истек срок снятия резерва'") + " " + СтрЗаказы + ".  " + НСтр("ru = 'Необходимо выполнить снятие резерва.'");
			Иначе
				Описание = НСтр("ru = 'Срок резерва'") + " " + СтрЗаказы + " истекает " 
...
Процедура СформироватьНапоминанияПроСнятиеРезервов() Экспорт
	...
	СрокНапоминанияСнятияРезервов = ПравоПользователя("СрокНапоминанияСнятияРезервов");
	...
	НаборЗаписейНапоминания.Отбор.Автор.Установить(ПараметрыСеанса.АвторизованныйПользователь);
	НаборЗаписейНапоминания.Прочитать();

	ТаблицаЗаказовИтоговая = Новый ТаблицаЗначений;
	ТаблицаЗаказовИтоговая.Колонки.Добавить("Заказ",Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));	
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось записать набор в регистр ""НапоминанияПользователя"".'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ВыполнитьСнятиеРезервов(ТаблицаЗаказовДляСписания, КэшЗаказы, СоздаватьНапоминание=Ложь, ФоновыйРежим=Ложь) Экспорт
	...
	КэшСнятиеРезервов.Колонки.Добавить("ЗаказПокупателя",Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
	...
	Если ТаблицаЗаказовДляСписания.Количество()=0 Тогда
		Если НЕ ФоновыйРежим Тогда
			ВывестиСообщение(НСтр("ru = 'Не выбрано заявок для формирования снятия резервов'"));
		КонецЕсли;
		...
		Если ТекущийКонтрагент <> ТекЗаказДляСписания.Контрагент ИЛИ ТекФлагКорректировки <> ТекЗаказДляСписания.Корректировка Тогда
			...
			Если НЕ ЗначениеЗаполнено(ТекущийДокумент.Ссылка) И ТекущийДокумент.Товары.Количество()>0 Тогда
				Попытка
					...
					Если НЕ ФоновыйРежим Тогда
						ВывестиСообщение(НСтр("ru = '""Сформирован и проведен документ ""%1""'"),,,,,Строка(ТекущийДокумент.Ссылка));
					КонецЕсли;
				...
				Исключение
					Если НЕ ФоновыйРежим Тогда
						ВывестиСообщение(НСтр("ru = '""Не удалось записать документ ""%1""'"),,,,,Строка(ТекущийДокумент));
					КонецЕсли;
				КонецПопытки;
	...
	Если СоздаватьНапоминание Тогда
		...
		Исключение 	
			Если НЕ ФоновыйРежим Тогда 
				ВывестиСообщение(НСтр("ru = '""Ошибка создания напоминаний о резервировании товаров: ""%1""'"),,,,,ОписаниеОшибки()); 
			КонецЕсли;
		КонецПопытки;
	КонецЕсли;
...
Процедура ВыполнитьАвтоматическоеСнятиеРезервов(СтруктураПараметров) Экспорт
	...
	ТаблицаЗаказовДляСписания.Колонки.Добавить("Заказ",Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
