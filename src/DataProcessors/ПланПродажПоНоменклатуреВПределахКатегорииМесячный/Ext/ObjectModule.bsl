...
Процедура ЗаполнитьПлан() Экспорт
	...
	Если ОграничиватьПоШиринеАссортимента Тогда
		Если ДокументОбъект.Показатели.Количество()<ШиринаАссортимента Тогда
			ВывестиСообщение(НСтр("ru = 'ВНИМАНИЕ! Заданная ширина ассортимента превосходит количество видов товаров, проданных за предыдущий период.'"));
		КонецЕсли;
	КонецЕсли;
	
	// Предотвращаем исключительные ситуации
	Если СуммаПродаж=0 Тогда
		ВывестиСообщение(НСтр("ru = 'Отсутствуют данные о суммах продаж. Автоматическое планирование невозможно...'"));
		Возврат;
	КонецЕсли;
