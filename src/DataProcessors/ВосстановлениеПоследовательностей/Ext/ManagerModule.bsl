...
Функция ПолучитьНеактуальныеДокументы(Границы, Параметры=Неопределено) Экспорт
	...
	Для Каждого Граница Из Границы Цикл
		...
		КонецЕсли;
		Где = Где + ?(ПустаяСтрока(Где), "ГДЕ", " И ")+ "
		|		"+СтрШаблон("(НЕ %1.Регистратор ССЫЛКА Документ.КорректировкаДвижений)", Граница.Имя);
...
Процедура Восстановить(ПараметрыВосстановления, АдресРезультата="") Экспорт
	...
	НеРегистрироватьДокументыВобмене = ПолучитьЗначениеПараметраСтруктуры(ПараметрыВосстановления, "НеРегистрироватьДокументыВобмене", Истина);
	...
	Если НЕ ПравоПользователя("РедактированиеПроведенныхДокументов") Тогда
	...
	Пока СледующийНеактуальныйДокумент(
		...
		ПерепровестиДокумент(ПерепроводимыйДокумент, НеРегистрироватьДокументыВобмене);
...
Функция ПолучитьМинимальнуюДатуГраниц(Границы, КВосстановлению)
...
Процедура ПерепровестиДокумент(Документ, НеРегистрироватьДокументыВобмене)
	
	Обработка = Метаданные.Обработки.ВосстановлениеПоследовательностей;
	
	// Если документ не был проведен или не проводится его проводить не нужно.
	Если НЕ Документ.Проведен И Документ.Движения.Количество()=0 Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		Если НеРегистрироватьДокументыВобмене Тогда
	...
	Исключение
		...
		ВывестиСообщениеВЛог(СобытияЛога().Ошибка);
		ЗаписьЖурналаРегистрации(
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Документ «%1» не удалось провести.'"), Документ.Ссылка);
...
Процедура СдвинутьГраницыДокумента(
	...
	Если ЗначениеЗаполнено(ЗатронутыеГраницы) Тогда
		...
		ВывестиСообщение(СтрШаблон(НСтр("ru = 'Изменены границы: %1;'"), СтрСоединить(ЗатронутыеГраницы, ", ")));
...
Процедура ВывестиСообщениеВЛог(ВидСобытия, Текст="",  Документ=Неопределено)
	...
	ВывестиСообщение(ЗначениеВJSON(ОписаниеСообщения));
