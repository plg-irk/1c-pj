...
Процедура Подключаемый_ОбработкаРезультатаОповещения(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	...
	ИначеЕсли ДополнительныеПараметры = "ПродолжитьЗагрузкуВыборФайла" 
		...
		АдресТаблицы = ПоместитьВоВременноеХранилище(Неопределено, ЭтотОбъект.УникальныйИдентификатор);
...
//
&НаКлиенте
Процедура ПродолжитьСохранитьФайлНастроекДляОбъектаВыборОбъекта(Результат = Неопределено) Экспорт
...
//
&НаКлиенте
Процедура ПродолжитьЗагрузитьФайлНастроекДляОбъектаВыборОбъекта(Результат = Неопределено) Экспорт
...
Процедура КомандаОК(Команда)
	...
	Если ПроверитьСохранить() Тогда
		ДанныеБылиМодифицированы = ЭтотОбъект.Модифицированность;
		Если НЕ ЭтотОбъект.ВладелецФормы = Неопределено Тогда
			ТекущаяСтрока = ЭтотОбъект.ВладелецФормы.элементы.Список.ТекущаяСтрока;
			ЭтотОбъект.ВладелецФормы.РасширеннаяИнформация = РасширеннаяИнформацияВызовСервера.СформироватьРасширеннуюИнформациюОбОбъекте(ТекущаяСтрока);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.Параметры.Свойство("ПолноеИмяОбъекта")  Тогда
		ПолноеИмяОбъекта = ЭтотОбъект.Параметры.ПолноеИмяОбъекта;
		
		// Передана ссылка
		Если ЭтотОбъект.Параметры.Свойство("СсылкаНаОбъект") Тогда
			СсылкаНаОбъект = ЭтотОбъект.Параметры.СсылкаНаОбъект;
			ПолноеИмяОбъекта = СсылкаНаОбъект.Метаданные().ПолноеИмя();
		КонецЕсли;
	Иначе
		Отказ = Истина;
		ВывестиСообщение(НСтр("ru='Служебная обработка. Вызывается из специального меню в формах списков. Интерактивная работа с обработкой не предполагается.'"));
		Возврат;
	КонецЕсли;
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЭтотОбъект.Параметры.ПолноеИмяОбъекта);
	Если ОбъектМетаданных<> Неопределено Тогда
		ЭтотОбъект.Заголовок = "" + ОбъектМетаданных.Синоним + ":" + НСтр("ru = 'настройка полей расширенной информации'");
	Иначе
		ЭтотОбъект.Заголовок = НСтр("ru = 'Настройка полей расширенной информации'");
	КонецЕсли;
