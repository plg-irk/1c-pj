...
Процедура ВыполнитьПересчетНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ВывестиСообщение(НСтр("ru='Обновлен документ ""%1"".'"),,,,, СокрЛП(ИзменяемыйДокумент.Ссылка));
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Конвертация цены. Не удалось обновить документ'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
