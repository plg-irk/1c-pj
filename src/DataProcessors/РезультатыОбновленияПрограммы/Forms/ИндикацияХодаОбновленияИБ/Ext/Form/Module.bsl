...
Процедура НачатьОбновлениеПараметровРаботыПрограммы()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗавершитьОбновлениеПараметровРаботыПрограммы", ЭтотОбъект);
...
Функция ЗагрузитьОбновитьПараметрыРаботыПрограммыВФоне()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("СтандартныеПодсистемыСервер.ЗагрузитьОбновитьПараметрыРаботыПрограммыВФоне", 
...
Процедура ЗавершитьОбновлениеПараметровРаботыПрограммы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		КраткоеСообщениеОбОшибке = НСтр("ru = 'Ошибка получения результата от фонового задания при обновлении параметров работы программы.'");
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		Если ТипЗнч(РезультатВыполнения) = Тип("Структура") Тогда
			...
			Если РезультатВыполнения.Свойство("ПараметрыКлиентаНаСервере") Тогда
				УстановитьПараметрыСеансаИзФоновогоЗадания(РезультатВыполнения.ПараметрыКлиентаНаСервере);
			КонецЕсли;
		...
		КонецЕсли;
	Иначе // ошибка
		КраткоеСообщениеОбОшибке   = Результат.КраткоеПредставлениеОшибки;
		ПодробноеСообщениеОбОшибке = Результат.ПодробноеПредставлениеОшибки;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОшибкаУстановкиМонопольногоРежима) И Не КлиентСервер Тогда
	...
	ОбновитьПараметрыРаботыПрограммы(ДополнительныеПараметры);
...
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммыПриОшибкеУстановкиМонопольногоРежимаЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	НачатьОбновлениеПараметровРаботыПрограммы();
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПараметрыРаботыПрограммы(ДополнительныеПараметры)
	...
	ИначеЕсли Результат.Статус = "Выполнено"  Тогда
		...
		Если ТипЗнч(РезультатОбновления) = Тип("Структура") Тогда
			Если РезультатОбновления.Свойство("КраткоеСообщениеОбОшибке")
				...
				Результат.КраткоеПредставлениеОшибки = РезультатОбновления.КраткоеСообщениеОбОшибке;
				Результат.ПодробноеПредставлениеОшибки = РезультатОбновления.ПодробноеСообщениеОбОшибке;
			Иначе
...
Процедура ЗарегистрироватьДанныеДляОтложенногоОбновления(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус <> "Выполнено"
		Или (ТипЗнч(РезультатОбновления) = Тип("Структура")
			И РезультатОбновления.Свойство("КраткоеСообщениеОбОшибке")
			И РезультатОбновления.Свойство("ПодробноеСообщениеОбОшибке")) Тогда
