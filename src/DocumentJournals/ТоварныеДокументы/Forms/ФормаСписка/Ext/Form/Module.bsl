// Модуль формы списка журнала документов "Складские документы"
...
Процедура Подключаемый_ОбработкаКомандыФормы(Команда) Экспорт
	...
	Если Команда.Имя = "ПоказатьСвязи" Тогда
		СвязиАктивны = ?(Не Элементы.Найти("Связи") = Неопределено И Элементы.ПоказатьСвязи.Пометка, Истина, Ложь);
		
		Связи.Очистить();
		
		Список.КомпоновщикНастроек.Настройки.ПараметрыВывода.УстановитьЗначениеПараметра("Заголовок", СвязиАктивны);
	КонецЕсли;
...
Процедура Подключаемый_СписокПриАктивизацииСтроки()
	...
	Если Не Элементы.Найти("Связи") = Неопределено И Элементы.ПоказатьСвязи.Пометка Тогда
		МассивСвязанныхДокументов = ПолучитьСвязанныеДокументы(Элементы.Список.ТекущаяСтрока);
		
		Если МассивСвязанныхДокументов.Количество() > 0 Тогда
			// заполняем
			Для Каждого ТекДокумент Из МассивСвязанныхДокументов Цикл
				Связи.Добавить(ТекДокумент, ТекДокумент);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	Для Каждого ТекСтрока Из Строки Цикл
		Заголовок = Настройки.ПараметрыВывода.Элементы.Найти("Заголовок");
		Если Настройки.ПараметрыВывода.Элементы.Найти("Заголовок") <> Неопределено И Настройки.ПараметрыВывода.Элементы.Найти("Заголовок").Значение = Истина Тогда
			МассивДокументов = ПолучитьСвязанныеДокументы(ТекСтрока.Ключ);
			
			Если МассивДокументов.Количество() > 0 Тогда
				// красим ....
				Для Каждого ТекЯчейка Из ТекСтрока.Значение.Оформление Цикл
					ЦветФона = ТекЯчейка.Значение.Элементы.Найти("ЦветФона");
					ЦветФона.Значение = WebЦвета.БледноБирюзовый;
		...
		Иначе
			Для Каждого ТекЯчейка Из ТекСтрока.Значение.Оформление Цикл
				ЦветФона = ТекЯчейка.Значение.Элементы.Найти("ЦветФона");
				ЦветФона.Значение = Неопределено;
				ЦветФона.Использование = Истина;
			КонецЦикла;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьСвязанныеДокументы(ТекЭлемент)
	МассивДокументов = КритерииОтбора.ПодчиненныеДокументы.Найти(ТекЭлемент);
	
	Если МассивДокументов = Неопределено Тогда
		МассивДокументов = Новый Массив;
	КонецЕсли;
	
	Если ЕстьРеквизит(ТекЭлемент, "ДокументОснование") И ЗначениеЗаполнено(ТекЭлемент.ДокументОснование) Тогда
		МассивДокументов.Добавить(ТекЭлемент.ДокументОснование);
	КонецЕсли;
...
Процедура СвязиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	УправлениеСпискомДокументаКлиент.СвязиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка, Связи);
КонецПроцедуры
