...
//      //
//      // Параметры:
//      //   Настройки - Коллекция - Передается "как есть" из 
...
//   Отчет - ОбъектМетаданных, СправочникСсылка.ИдентификаторыОбъектовМетаданных -
...
//               	// Код процедуры.
//               КонецПроцедуры
//       Прочие реквизиты см. в описании функции ДеревоПредопределенных().
//
...
//   ВыводитьОтчетыВместоВариантов - Булево, Строка - Режим вывода в панели отчетов:
//       - Истина, "ПООТЧЕТАМ" - По отчетам (варианты скрыты, а отчет включен и видим).
...
//
// Пример:
//  1. Вызов из собственной формы отчета, использующего СКД.
...
//  2. Вызов из обработчика ПриКомпоновкеРезультата отчета, использующего СКД.
//	ИспользуемыеТаблицы = ВариантыОтчетов.ИспользуемыеТаблицы(ЭтотОбъект.СхемаКомпоновкиДанных);
//	ВариантыОтчетов.ПроверитьИспользуемыеТаблицы(ИспользуемыеТаблицы);
//  3. Вызов из обработчика ПриКомпоновкеРезультата отчета, использующего запрос.
...
//
// Параметры:
//   Объект - Массив, СхемаКомпоновкиДанных, Строка - Имена таблиц, используемые отчетом,
//          											схема отчета или текст запроса.
...
//    Места использования <МодульКонфигурации>.ПриДобавленииОбработчиковОбновления().
//    См. также ПодсистемыКонфигурацииПереопределяемый.ПриДобавленииПодсистем().
//
// Параметры:
...
//       необходимо полностью обновить индекс поиска. Рекомендуется указывать последнюю функциональную версию,
//       при обновлении на которую были внесены изменения в представления объектов метаданных или их реквизитов,
//       которые могут выводиться в отчетах. Поднимается при необходимости.
//
// Пример:
...
//       * ЗначениеПараметраНастроек  - Булево - (Ложь) Сбрасывать настройку 
//                                                      "ЗначениеПараметраНастроекКомпоновкиДанных".
//       * ВыбранныеПоля              - Булево - (берется из ключа Прочие) Сбрасывать настройку
//                                                                         "ВыбранныеПоляКомпоновкиДанных".
//       * Порядок                    - Булево - (берется из ключа Прочие) Сбрасывать настройку
//                                                                         "ПорядокКомпоновкиДанных".
//       * ЭлементУсловногоОформления - Булево - (берется из ключа Прочие) Сбрасывать настройку
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	КлючиВариантов = Новый Массив; // Конечный список очищаемых ключей.
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		// +РАРУС
		ВариантОбъект.ТипДоступа = Перечисления.ТипыДоступаКВариантуОтчета.ПоПользователю;
		// -РАРУС
		...
		ВариантОбъект.ОбъектДоступа = СтрокаТаблицы.Автор;
...
Процедура ПриДобавленииСлужебныхСобытий(КлиентскиеСобытия, СерверныеСобытия) Экспорт
	...
	// Определяет настройки вариантов отчетов.
	//
	// Синтаксис:
	//   // См. ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов().
	...
	//
	// Вспомогательные методы:
	//   1. Функции ОписаниеОтчета и ОписаниеВарианта формируют описание настроек отчета
	//       и варианта для последующего изменения:
	...
	//       Подробнее - см. "свойства для изменения" в комментарии к
	//        ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов().
	...
	//                                                            Метаданные.Подсистемы.<ИмяПодсистемы>, Истина/Ложь);
	...
	//
	СерверныеСобытия.Добавить("СтандартныеПодсистемы.ВариантыОтчетов\ПриНастройкеВариантовОтчетов");
	
	// Регистрация изменений в именах вариантов отчетов.
	//   Используется в обработчиках обновления в целях ссылочной целостности,
	//   в том числе для сохранения пользовательских настроек и настроек рассылок отчетов.
	//
	// Синтаксис:
	//   // См. ВариантыОтчетовПереопределяемый.ЗарегистрироватьИзмененияКлючейВариантовОтчетов().
	//   Процедура ПриРегистрацииИзмененийИменВариантовОтчетов(Изменения) Экспорт
	//   	// Пример:
	...
	//   КонецПроцедуры
	//
	СерверныеСобытия.Добавить("СтандартныеПодсистемы.ВариантыОтчетов\ПриРегистрацииИзмененийИменВариантовОтчетов");
...
//       * ИдентификаторФормы - Неопределено, УникальныйИдентификатор - Идентификатор формы,
//                                                                       из которой подключается отчет.
//
// Возвращаемое значение:
...
Процедура ПользовательскиеВариантыОтчетов(ОтчетМетаданные, ПользовательИнформационнойБазы, ТаблицаВариантовОтчетов, СтандартнаяОбработка) Экспорт
	...
	Для Каждого ВариантОтчета Из ВсеВариантыОтчета Цикл
		...
		Если ЭлементСправочника <> Неопределено
			// +РАРУС
			И ЭлементСправочника.ТипДоступа = Перечисления.ТипыДоступаКВариантуОтчета.ПоПользователю Тогда
			// -РАРУС
...
Функция ДоступностьОтчетов(СсылкаИлиМассивСсылок) Экспорт
	// Поддержка передачи 1 ссылки, а не массива.
	Если ТипЗнч(СсылкаИлиМассивСсылок) <> Тип("Массив") Тогда
		Массив = Новый Массив;
		Массив.Добавить(СсылкаИлиМассивСсылок);
		Доступность = ДоступностьОтчетов(Массив);
		Возврат Доступность[0];
	КонецЕсли;
	...
	УстановитьПривилегированныйРежим(Истина);
	Для Каждого Ссылка Из СсылкаИлиМассивСсылок Цикл
...
//
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний")
...
//
Процедура ЗаполнитьОписаниеСтрокиВарианта(СтрокаВариант, СтрокаОтчет = Неопределено)
...
Функция СброситьНастройкиВариантаОтчета(ВариантОбъект) Экспорт
	...
	// +РАРУС
	ВариантОбъект.ТипДоступа = Перечисления.ТипыДоступаКВариантуОтчета.БезОграничения;
	ВариантОбъект.ВидПериода = "Месяц";
...
Функция НайтиСсылки(ПараметрыПоиска) Экспорт
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если ЕстьСтрокаПоиска Тогда
			...
			ПоискВариантовИПодсистем = "("
				...
				+ "				Или "
				// +РАРУС
				+ СтрЗаменить(ШаблонПоиска, "<ИмяТаблицы.ИмяПоля>", "ВариантыОтчетов.НаименованиеОтчета")
				+ Символы.ПС
				+ "				Или "
				// -РАРУС
		...
		Если Не ПолучатьИтоговуюТаблицу Тогда
			...
			// +РАРУС
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ВариантыОтчетов.Отчет КАК#", "ВариантыОтчетов.Отчет КАК");
			// -РАРУС
		КонецЕсли;
	...
	// +РАРУС
	Вариант.Вставить("НаименованиеОтчета",                   Новый Структура(ШаблонОбластиПоиска));
	// -РАРУС
	...
	Для Индекс = 1 По Количество Цикл
		...
		КонецЕсли;
		// +РАРУС
		Если Не ЗначениеЗаполнено(Вариант.НаименованиеОтчета.Значение) И ЗначениеЗаполнено(СтрокаТаблицы.НаименованиеОтчета) Тогда
			Вариант.НаименованиеОтчета.Значение = СтрокаТаблицы.НаименованиеОтчета;
		КонецЕсли;
		// -РАРУС
		...
		Если Индекс = Количество Или СтрокаТаблицы.Ссылка <> Вариант.Ссылка Тогда
			...
			Для Каждого Слово Из МассивСлов Цикл
				...
				// +РАРУС
				Если ОтметитьСлово(Вариант.НаименованиеОтчета, Слово) Тогда
					СловоНайдено = Истина;
				КонецЕсли;
				// -РАРУС
...
// 
Функция ЗначенияРеквизитовОбъектов(МассивСсылок, ИменаРеквизитов, УчитыватьПрава = Неопределено)
	
	ЗначенияРеквизитов = Новый Соответствие;
	Если МассивСсылок.Количество() = 0 Тогда
		Возврат ЗначенияРеквизитов;
	КонецЕсли;
	Если УчитыватьПрава = Неопределено Тогда
		УчитыватьПрава = Не ПривилегированныйРежим();
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ" + ?(УчитыватьПрава, " РАЗРЕШЕННЫЕ", "") + "
...
Процедура ПриОпределенииДоступностиОтчетов(СсылкиОтчетов, Результат)
	ЗначенияРеквизитовОтчетов = ЗначенияРеквизитовОбъектов(СсылкиОтчетов, "Имя");
	Для Каждого Отчет Из СсылкиОтчетов Цикл
		ЗначенияРеквизитов = ЗначенияРеквизитовОтчетов[Отчет];
		...
		Если ЗначенияРеквизитов = Неопределено Тогда
			ДоступенПоRLS = Ложь;
		Иначе
			ИмяОтчета = ЗначенияРеквизитов.Имя;
