...
Процедура ПриИзменении(Форма, Элемент, ЭтоТабличнаяЧасть = Ложь) Экспорт
	...
	// +РАРУС
	Если СтрНайти(Элемент.Имя, "ОсновнойДляСвязи") > 0 Тогда
		ОсновнойДляСвязиНажатие(Форма, Элемент);
		Возврат;
	КонецЕсли;
	// +РАРУС
	...
	// +Рарус
	Результат = Новый Структура("Представление,Вид", Текст, ДанныеСтроки.Вид);
	
	НайденнаяСтрока = Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Результат);
	Если НайденнаяСтрока.Количество()>0 Тогда
		ДанныеСтроки.ЗначенияПолей = НайденнаяСтрока[0].ЗначенияПолей;
	Иначе
	...
	ОбновитьКонтактнуюИнформациюФормы(Форма, Новый Структура("ИмяРеквизита", Элемент.Имя));
	
КонецПроцедуры

// Обработчик события НачалоВыбора поля формы контактной информации.
// Вызывается из подключаемых действий при внедрении подсистемы "Контактная информация". +РАРУС
//
// Параметры:
//     Форма                     - УправляемаяФорма - Форма владельца контактной информации.
//     Элемент                   - ПолеФормы        - Элемент формы, содержащий представление контактной информации.
...
//     ЗаполнитьАдресПоУмолчанию - Булево - флаг заполнения КИ по умолчанию только с типом "Адрес".
//
Процедура НачалоВыбора(Форма, Элемент, Модифицированность = Истина, СтандартнаяОбработка = Ложь, ПараметрыОткрытия = Неопределено, ЗаполнитьАдресПоУмолчанию = Ложь) Экспорт
	...
	Если ДанныеСтроки.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") И НЕ ЗначениеЗаполнено(ДанныеСтроки.ЗначенияПолей) Тогда
		ПредставлениеТелефона = Элемент.ТекстРедактирования;
		ДанныеСтроки.ЗначенияПолей = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияXMLПоПредставлению(ПредставлениеТелефона, ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"));
		ДанныеСтроки.Представление = ПредставлениеТелефона;
	КонецЕсли;
	
	// +РАРУС Начальное заполнение КИ по умолчанию только с типом "Адрес".
	ЗаполнитьАдресПоУмолчанию = ЗаполнитьАдресПоУмолчанию И ДанныеСтроки.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Адрес") И НЕ ЗначениеЗаполнено(ДанныеСтроки.ЗначенияПолей);
	// -РАРУС
	...
	Если Элемент.Вид = ВидПоляФормы.ПолеВвода Тогда
		Если ДанныеЗаполнения[Элемент.Имя] <> Элемент.ТекстРедактирования Тогда
			...
			// +РАРУС Выполним проверку необходимости заполнять адрес КИ по умолчанию.
			ЗаполнитьАдресПоУмолчанию = ЗаполнитьАдресПоУмолчанию И НЕ ЗначениеЗаполнено(Элемент.ТекстРедактирования);
			// -РАРУС
	...
	// +РАРУС
	ПараметрыОткрытияФормы.Вставить("ЗаполнитьАдресПоУмолчанию", ЗаполнитьАдресПоУмолчанию);
	// -РАРУС
...
Процедура ВыполнитьКоманду(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Адрес") Тогда
		// +РАРУС
		ОтобразитьНаКарте(Форма,ИмяКоманды);
		// -РАРУС
...
//                    Структура - Используется для определения типа, если его невозможно вычислить
//                                по полю ЗначенияПолей.
//    ИсточникКонтактнойИнформации - ЛюбаяСсылка - Ссылка на источник являющийся владельцем.
//
...
//
Процедура ОсновнойДляСвязиНажатие(Форма, Элемент)
	...
	НайденныеСтроки[0].ОсновнойДляСвязи = (НЕ НайденныеСтроки[0].ОсновнойДляСвязи);
	Элемент.Картинка = ?(НайденныеСтроки[0].ОсновнойДляСвязи, БиблиотекаКартинок.КонтактнаяИнформацияОсновная, БиблиотекаКартинок.КонтактнаяИнформацияВторичная);
	
	Форма.ТекущийЭлемент = Форма.Элементы[ИмяРеквизита];
	
	Если НЕ НайденныеСтроки[0].ОсновнойДляСвязи Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТабличнойЧасти Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Новый Структура("Тип", НайденныеСтроки[0].Тип)) Цикл
		...
		СтрокаТабличнойЧасти.ОсновнойДляСвязи = ЛОЖЬ;
		Форма.Элементы["ОсновнойДляСвязи" + СтрокаТабличнойЧасти.ИмяРеквизита].Картинка = БиблиотекаКартинок.КонтактнаяИнформацияВторичная;
...
// Событие Нажатие в поле "Основной для связи" таблицы контактной информации
//
Процедура ОтобразитьНаКарте(Форма, ИмяКоманды)
	
	ИмяРеквизита = СтрЗаменить(ИмяКоманды, "Команда", "");
	Результат = Новый Структура("ИмяРеквизита", ИмяРеквизита);
	НайденнаяСтрока = Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Результат)[0];
	СтрокаПоиска = СтрЗаменить(НайденнаяСтрока.Представление," ","%20");
	ЗапуститьПриложение("https://maps.yandex.ru/?text= " + СтрокаПоиска);
	
КонецПроцедуры // ОтобразитьНаКарте()
...
Функция ЛогическоеПобитовоеИ(БинарныйМассив1, БинарныйМассив2) // +Рарус Перетянуто с бсп 3.0.2.238
	Результат = Новый Массив;
...
Функция ЛогическоеПобитовоеИли(БинарныйМассив1, БинарныйМассив2) // +Рарус Перетянуто с бсп 3.0.2.238
	Результат = Новый Массив;
	Для Индекс = 0 По БинарныйМассив1.Количество()-1 Цикл
