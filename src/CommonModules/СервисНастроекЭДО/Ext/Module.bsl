...
Функция НайтиОператоровЭДОПоИдентификаторам(ИдентификаторыЭДО) Экспорт
	...
	СвойстваАбонентов = Новый ТаблицаЗначений;
	СвойстваАбонентов.Колонки.Добавить("ИдентификаторЭДО", ОбщегоНазначения.ОписаниеТипаСтрока(70));
	СвойстваАбонентов.Колонки.Добавить("ИдентификаторОператора", ОбщегоНазначения.ОписаниеТипаСтрока(10));
	
	Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
		СвойстваАбонента = СвойстваАбонентов.Добавить();
		СвойстваАбонента.ИдентификаторЭДО = ИдентификаторЭДО;
		СвойстваАбонента.ИдентификаторОператора = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(ИдентификаторЭДО);
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СвойстваАбонентов.ИдентификаторЭДО КАК ИдентификаторЭДО,
		|	СвойстваАбонентов.ИдентификаторОператора КАК ИдентификаторОператора
		|ПОМЕСТИТЬ СвойстваАбонентов
		|ИЗ
		|	&СвойстваАбонентов КАК СвойстваАбонентов
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	ОператорыЭДО.Представление КАК ПредставлениеОператора
		|ИЗ
		|	СвойстваАбонентов КАК СвойстваАбонентов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОператорыЭДО КАК ОператорыЭДО
		|		ПО СвойстваАбонентов.ИдентификаторОператора = ОператорыЭДО.ИдентификаторОператора";
	
	Запрос.УстановитьПараметр("СвойстваАбонентов", СвойстваАбонентов);
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	Результат.Индексы.Добавить("ИдентификаторЭДО");
...
Функция ДанныеАбонентовИзСервисаНастроек(Отбор, Отказ, КонтекстДиагностики = Неопределено) Экспорт
	...
	НаборСведений = Новый Массив; // Массив из Структура
	...
	Пока КоличествоПолучено = РазмерСтраницы Цикл
		...
		Для Каждого Сведения Из ДанныеАбонентовПорции.identifierInfos Цикл
			
			Если УникальныеЗначения[Сведения.id] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			УникальныеЗначения.Вставить(Сведения.id, Истина);
			НаборСведений.Добавить(Сведения);
		...
		Если КоличествоПолучено = 0 Тогда
			Прервать;
		КонецЕсли;
	...
	ДанныеАбонентов.identifierInfos = НаборСведений;
...
#Область СопоставлениеКлассификатораОграниченийСОбъектамиБЭД

// Получить настройки для сопоставления классификатора ограничений полномочий МЧД с объектами БЭД.
// 
// Параметры:
//  КонтекстДиагностики см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ДатаИзмененияФайла - Дата
...
//   				  - ДвоичныеДанные
//   * ДатаПоследнегоИзменения - Дата
Функция НастройкиСоответствиеКлассификатораОграниченийСОбъектамиБЭД(КонтекстДиагностики, ДатаИзмененияФайла) Экспорт 
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение настроек соответствия классификатора ограничений полномочий с объектами БЭД'"));
		
	ИмяФайлаДанныхНастроек = "MatchingConstraintClassifierSettingsWithObjectsLED.json";
		
	Путь = "/classifier/" + ИмяФайлаДанныхНастроек;
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
	...
	Результат = Новый Структура;
	Результат.Вставить("ДвоичныеДанные", РезультатПолученияДанныхФайла.ДанныеФайла);
	Результат.Вставить("ДатаПоследнегоИзменения", РезультатПолученияДанныхФайла.ДатаИзмененияФайла);
...
#Область КлассификаторыОграниченийМЧД

// Получить классификатор ограничений МЧД.
// 
// Параметры:
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  ДатаИзмененияФайла - Дата
...
//					- ДвоичныеДанные
Функция ПолучитьКлассификаторОграниченийМЧД(КонтекстДиагностики, ДатаИзмененияФайла) Экспорт

	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение данных классификатора ограничений (МЧД 003)'"));
	
	Путь  = "/mchd/rights_classifier/POWER_PWR_RESTR.zip";
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
	...
	Если РезультатПолученияДанныхФайла.ДанныеНеИзменялись И ЗначениеЗаполнено(ДатаИзмененияФайла) Тогда
		ИзменитьСостояниеСинхронизации(
			Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификаторовОграниченийМЧД,
			ДатаИзмененияФайла);
		Возврат Неопределено;
	КонецЕсли;
	...
	Результат = Новый Структура;
	Результат.Вставить("ДатаПоследнегоИзменения", РезультатПолученияДанныхФайла.ДатаИзмененияФайла);
	Результат.Вставить("ДанныеФайла", РезультатПолученияДанныхФайла.ДанныеФайла);
...
// Тип документа ЭДО по соответствующему значению электронного документа из сервиса.
// Используется для определения соответствия типов документов при загрузке данных электронного документа, в том числе и 
// через интеграционные сервисы: 1С:Бизнес-сеть, 1С:Share.
//
// Параметры:
//  Значение - Строка - значение типа документа электронного документа.
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыДокументовЭДО - тип документа электронного документа соответствующего перечисления.
Функция ТипДокументаПоЗначениюСервиса(Значение) Экспорт
	...
	ТипыДокументов.Вставить("УниверсальноеСообщение", Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение);

	// Для интеграции с сервисом 1С:Share
...
// Возвращаемое значение:
//  См. ДанныеФайлаОблачногоХранилищаНастроек
Функция ДатыНачалаДействияУниверсальныхСообщений() Экспорт
	
	ПараметрыОперации = Новый Структура("Наименование",
		НСтр("ru = 'Получение дат начала действия Универсальных сообщений'"));
	Путь = "/universal_message/UniversalMessageDates.json";
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
...
Функция ДанныеФайлаОблачногоХранилищаНастроек(ПутьКФайлу, ВидОперации = "", ПараметрыПолучения = Неопределено,
	...
	Если Не ЗначениеЗаполнено(ДатаИзмененияФайлаRFC1123) Тогда 
		ДатаИзмененияФайлаRFC1123 = РезультатПолученияФайла.Заголовки["last-modified"];
	КонецЕсли;
...
Функция ОбработкаРезультатаПолученияДанныхАбонентов(Знач ПолученныеДанные, Отказ)
	...
	Для Каждого Абонент Из ПолученныеДанные.identifierInfos Цикл
		...
		НоваяСтрока.ОператорЭДО = РаботаСАбонентамиЭДОКлиентСервер.ОператорАбонента(Абонент.id);
