...
Функция КолонкиДереваДокумента()
	...
	СписокКолонок.Добавить("МинимальноеДопустимоеЗначение");
		...
		Для Сч = НомерУровня По КолУровней Цикл
			...
			Если ТЗ.Колонки.Найти(ИдТекУровня) <> Неопределено И ЗначениеЗаполнено(СтрокаТЗ[ИдТекУровня]) Тогда
				...
				ИначеЕсли НомерУровня = Сч Тогда
					...
					СтрокаТекУровня.МинимальноеДопустимоеЗначение = МинимальноеДопустимоеЗначение(СтрокаТЗ.КвалификаторыЧисла);
...
//                           Также допускается указание минимального допустимого значения, например, описание
//                           "15, 3, Неотрицательный, 0" означает, что 0 (ноль) является валидным значением типа.
//
// Возвращаемое значение:
...
// На основании текстового описания типа возвращает его минимальное допустимое значение.
//
// Параметры:
//  ОписаниеЧисла - Строка - см. КвалификаторыСтрокиПоОписанию().
// 
// Возвращаемое значение:
//  Число, Неопределено - минимальное допустимое значение или Неопределено, если оно не задано.
//
Функция МинимальноеДопустимоеЗначение(ОписаниеЧисла)

	Результат = Неопределено;
	
	Если ЗначениеЗаполнено(ОписаниеЧисла) Тогда
		ПараметрыОписания = СтрРазделить(ОписаниеЧисла, ",", Истина);
		Если ПараметрыОписания.Количество() > 3 Тогда
			МинимальноеДопустимоеЗначение = СокрЛП(ПараметрыОписания[3]);
			Результат = Число(МинимальноеДопустимоеЗначение);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеВСтрокеДереваСоответствуетТипу(СтрокаДерева, ДатаПоФорматуФНС = Ложь)
	...
	Возврат СтрокаДерева.ТипЗначения.ПривестиЗначение(СтрокаДерева.Значение) = СтрокаДерева.Значение;

КонецФункции

Функция ЗначениеДатыВСтрокеДереваСоответствуетТипуДатыФНС(СтрокаДерева)
	
	Если ТипЗнч(СтрокаДерева.Значение) = Тип("Дата") Тогда
		
		ДатаСодержитТолькоВремя = НЕ ЗначениеЗаполнено(НачалоДня(СтрокаДерева.Значение));
		
		Если НЕ ДатаСодержитТолькоВремя
			И (СтрокаДерева.Значение < Дата(1900, 1, 1) Или СтрокаДерева.Значение >= Дата(2100, 1, 1)) Тогда
			Возврат Ложь;
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеРеквизитаДереваДанных(СтрокаДерева, ПроверятьОбязательностьПараметровОшибки, ДатаПоФорматуФНС, ТекстОшибки)
	...
	ЗначениеСтрокиДереваЗаполнено = ЗначениеЗаполнено(СтрокаДерева.Значение);
	Если Не ЗначениеСтрокиДереваЗаполнено
		И СтрокаДерева.Значение <> Неопределено
		И СтрокаДерева.Значение <> ""
		И СтрокаДерева.ТипЗначения <> Неопределено
		И СтрокаДерева.ТипЗначения.СодержитТип(Тип("Число"))
		И СтрокаДерева.МинимальноеДопустимоеЗначение <> Неопределено Тогда
		ЗначениеСтрокиДереваЗаполнено =
			(СтрокаДерева.Значение >= СтрокаДерева.МинимальноеДопустимоеЗначение);
	КонецЕсли;
	
	// Проверим строку на заполненность, если реквизит обязательный.
	Если СтрокаДерева.Обязательное = "Да" И Не ЗначениеСтрокиДереваЗаполнено Тогда
		
		Если Не ЗначениеСтрокиДереваЗаполнено Тогда 
			ТекстОшибки = ТекстОшибкиНеЗаполненногоЗначенияДерева();
			Возврат;
		КонецЕсли;
		
	// Проверим строку на соответствие типу
	ИначеЕсли ЗначениеСтрокиДереваЗаполнено И СтрокаДерева.ТипЗначения <> Неопределено Тогда
		
		Если Не ЗначениеВСтрокеДереваСоответствуетТипу(СтрокаДерева) Тогда 
		...
		Если ДатаПоФорматуФНС И Не ЗначениеДатыВСтрокеДереваСоответствуетТипуДатыФНС(СтрокаДерева) Тогда
			
			ШаблонОшибки = НСтр("ru = 'неверное значение даты ""%1"" (интервал значений: 01.01.1900 - 31.12.2099)'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, СтрокаДерева.Значение);
			Возврат;
			...
			Для Каждого Элемент Из МассивРеквизитов Цикл
				...
				Если СтрокаДерева <> Неопределено Тогда
					...
					ИначеЕсли СтрокаДерева.Признак = "Выбор" Тогда
						...
						Если МассивРеквизитов.Количество() > (ТекущийИндекс + 1) Тогда
