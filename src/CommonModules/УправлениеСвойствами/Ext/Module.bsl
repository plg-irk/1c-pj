...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	// +РАРУС
	Контекст.Вставить("ИспользоватьСвойства",       Неопределено);
	Контекст.Вставить("ИспользоватьДопРеквизиты",   Неопределено);
	// -РАРУС
	...
	// +РАРУС
	ИспользоватьПодсистему = Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьДополнительныеРеквизитыИСведения");
	Если Контекст.Объект=Неопределено Тогда
		Контекст.Объект = Форма.Объект;
	КонецЕсли;
	Если Контекст.ИспользоватьСвойства=Неопределено Тогда
		Контекст.ИспользоватьСвойства     = ИспользоватьПодсистему И ?(Контекст.ПроизвольныйОбъект, ЛОЖЬ, УправлениеСвойствами.ИспользоватьДопСведения(Контекст.Объект.Ссылка));
	КонецЕсли;
	Если Контекст.ИспользоватьДопРеквизиты=Неопределено Тогда
		Контекст.ИспользоватьДопРеквизиты = ИспользоватьПодсистему И ?(Контекст.ПроизвольныйОбъект, ЛОЖЬ, УправлениеСвойствами.ИспользоватьДопРеквизиты(Контекст.Объект));
	КонецЕсли;
	// -РАРУС
	...
	Иначе
		...
		КонецЕсли;
		// +РАРУС
		СоздатьОписаниеДополнительныхРеквизитов = Контекст.ИспользоватьДопРеквизиты;
		// -РАРУС
	...
	// +РАРУС
	Если НЕ(Контекст.ИспользоватьСвойства ИЛИ Контекст.ИспользоватьДопРеквизиты) Тогда
...
Процедура ПередЗаписьюВидаОбъекта(ВидОбъекта,
	...
	// +РАРУС
	НаборСвойствОбъект.Наименование    = ВидОбъекта.Наименование + ?(ИмяРеквизитаНабораСвойств="НаборСвойствХарактеристик", 
...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		// +РАРУС
		Если ОписаниеСвойства.Удалено И Форма.Свойства_СкрытьУдаленные ИЛИ ОписаниеСвойства.ТолькоДляПечати И (НЕ ОписаниеСвойства.ЗаполнятьОбязательно) Тогда
			Продолжить;
		КонецЕсли;
		// -РАРУС
		...
		Иначе
			...
			Иначе
				...
				// +РАРУС Добавлено, так как в дополнительных реквизитах была возможность выбрать группу.
		...
		Элемент.Заголовок	= ОписаниеСвойства.СвойствоЗаголовок;
		Элемент.Подсказка   = ОписаниеСвойства.СвойствоПодсказка;
		...
		// +РАРУС Отбор Доверенностей по контрагенту
		Если ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Доверенность И ЕстьРеквизит(ОписаниеОбъекта, "Контрагент") Тогда
		...
		Если ОписаниеСвойства.СвойствоМногострочноеПолеВвода > 0 Тогда
			Если НЕ ПоляНадписей Тогда
				Элемент.МногострочныйРежим = Истина;
			КонецЕсли;
			Элемент.Высота = ОписаниеСвойства.СвойствоМногострочноеПолеВвода;
		КонецЕсли;
		...
		Если ОписаниеСвойства.ТипЗначения=Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1024)) И ОписаниеСвойства.СвойствоМногострочноеПолеВвода=0 Тогда //для возможности обработки события начало ввода
...
Функция ИспользоватьДопРеквизиты(ВладелецСвойств) Экспорт
	...
	Если Метаданные.НайтиПоТипу(ТипЗнч(ВладелецСвойств.Ссылка))=Метаданные.Справочники.НаборыДополнительныхРеквизитовИСведений Тогда
		Возврат ЛОЖЬ;
	КонецЕсли;
	
	Возврат ЕстьРеквизит(ВладелецСвойств,, "ДополнительныеРеквизиты");
...
Функция ИспользоватьДопСведения(ВладелецСвойств) Экспорт
	...
	Возврат Метаданные.ОпределяемыеТипы.ВладелецДополнительныхСведений.Тип.СодержитТип(ТипЗнч(ВладелецСвойств));
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	// +РАРУС
	ОпцияИспользоватьСвойства = (Контекст.ИспользоватьСвойства ИЛИ Контекст.ИспользоватьДопРеквизиты);
	// -РАРУС
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			// Добавление команды формы, если установлена роль "ДобавлениеИзменениеДополнительныхРеквизитовИСведений"
			// или это полноправный пользователь.
			Если Пользователи.РолиДоступны("ДобавлениеИзменениеДополнительныхРеквизитовИСведений") Тогда
				...
				// +РАРУС
				Команда.Действие = "Подключаемый_ОбработкаКомандыФормы";
				// -РАРУС
	...
	Если ОпцияИспользоватьСвойства
		...
		Декорация = Форма.Элементы.Добавить("Свойства_ПустаяДекорация", Тип("ДекорацияФормы"), ГруппаФормы);
...
Процедура ОтключитьДополнительныеРеквизитыНаФорме(Форма, ДополнительныеПараметры)
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		// +РАРУС Ошибка БСП
		Если ДополнительныеПараметры.Свойство("ИмяЭлементаДляРазмещения") И ЗначениеЗаполнено(ДополнительныеПараметры.ИмяЭлементаДляРазмещения) И Форма.Элементы.Найти(ДополнительныеПараметры.ИмяЭлементаДляРазмещения)<>Неопределено Тогда
		// -РАРУС
