...
//
Процедура ПриСозданииВерсииОбъекта(Объект, РежимЗаписи) Экспорт // РАРУС
...
Процедура СоздатьВерсиюОбъектаПоОбменуДанными(Объект, СведенияОВерсииОбъекта, СсылкаСуществует, Отправитель) Экспорт
	...
	Если Не ЗначениеЗаполнено(СведенияОВерсииОбъекта.АвторВерсии) Тогда
		СведенияОВерсииОбъекта.АвторВерсии = Справочники.Пользователи.ПустаяСсылка();
	КонецЕсли;
...
//
Функция ВариантВерсионированияОбъекта(ТипОбъекта) Экспорт //*РАРУС
...
//
Функция ОбъектВерсионируется(Знач Источник, НомерПоследнейВерсии, РежимЗаписиПроведение = Ложь) Экспорт //*РАРУС
...
Процедура СоздатьВерсиюОбъекта(Объект, СведенияОВерсииОбъекта, ЗаписьОбычнойВерсии = Истина)
	...
	Если ЗаписьОбычнойВерсии Тогда
		...
		ЭтоНовый = ?(Объект.ОбменДанными.Загрузка,Объект.Ссылка.Пустая(), Объект.ЭтоНовый());
		
		// Запись данных предыдущей версии.
		Если Не ЭтоНовый И (ИзмененаПроведенность И СведенияОВерсииОбъекта.НомерВерсии > 1 Или ВерсияОтличаетсяОтРанееЗаписанной(Объект)) Тогда
			...
			Если СведенияОВерсииОбъекта.НомерВерсии = 1 Тогда
				Если ОбъектВерсионируется(Объект.Ссылка, Ложь) Тогда
					...
					// +Рарус
					ПараметрыВерсии.Вставить("Компьютер", СведенияОВерсииОбъекта.Компьютер);
					ПараметрыВерсии.Вставить("Событие"  , СведенияОВерсииОбъекта.Событие);
					// -Рарус
	...
	// +Рарус
	СведенияОВерсииОбъекта.Свойство("Компьютер", МенеджерЗаписи.Компьютер);
	СведенияОВерсииОбъекта.Свойство("Событие",   МенеджерЗаписи.Событие);
	// -Рарус
