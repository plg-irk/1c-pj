...
Процедура КонтрольНеразделенныхДанныхПриОбновлении() Экспорт
	...
	|	ИдентификаторыОбъектовРасширений.ЗначениеПустойСсылки,
	|	ИдентификаторыОбъектовРасширений.КлючОбъектаМетаданных
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		МетаданныеРасширения = Метаданные.НайтиПоПолномуИмени(Выборка.ПолноеИмя);
		Если НЕ МетаданныеРасширения = Неопределено Тогда
			Исключения.Добавить(МетаданныеРасширения);
		КонецЕсли;
	КонецЦикла;
