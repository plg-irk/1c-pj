...
//	Булево - результат подключения к сайту.
//
...
//	ТаблицаИнформации - таблица - лог обмена.
//
...
Функция ВыгрузитьДанныеНаСайт(Параметры, СтрокаТаблицыИнформации, ТипСоединения = Неопределено)
	...
	Успешно = ВыгрузитьНаСайт(Параметры, ОписаниеОшибки, МассивПодкаталогов, Истина, ТипСоединения);
...
Процедура ЗаписатьДанныеВФайл(Параметры, КоммерческаяИнформацияXDTO, СтрокаТаблицыИнформации, ФайлЗаписан)
	...
	Исключение
		...
		ШаблонОшибки = НСтр("ru = 'Не удалось записать XML-файл изменений предложений на диск: %1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, ИмяФайлаПрайса);
		ОписаниеИсключительнойОшибки = ОписаниеИсключительнойОшибки(ТекстОшибки + Символы.ПС + ОписаниеОшибки());
...
Процедура ЗаполнитьДанныеВладельцаКаталога(Параметры, ДанныеОрганизацииВладельцаКаталога)
	...
	СправочникОрганизации = ОбменССайтомПовтИсп.ИмяПрикладногоСправочника("Организации");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&СправочникОрганизации", "Справочник." + СправочникОрганизации);
...
Процедура ЗаполнитьИзмененияПакета(СтрокаТаблицыКаталога, ТаблицаОбновленияПакета, КоммерческаяИнформацияXDTO)
	...
	Для Каждого ТекСтрока Из ТаблицаОбновленияПакета Цикл
		...
		Для Каждого СтрокаТипЦены Из ТекСтрока.Цены Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТипЦены.ТипЦеныВалютаКод) Тогда
				ЦенаXDTO.Валюта = СтрокаТипЦены.ТипЦеныВалютаКод;
			КонецЕсли;
			Если СтрДлина(СтрокаТипЦены.ТипЦеныЕдиницаЦеныНаименованиеСокращенное) >= 3 
				И СтрДлина(СтрокаТипЦены.ТипЦеныЕдиницаЦеныНаименованиеСокращенное) <= 4 Тогда
...
Функция ЗагрузитьЗаказы(ДанныеЗаказов, Параметры, ОписаниеОшибки)
	...
	МассивЗаказовXDTO = ОбъектыСпискаXDTO(ЗаказыXDTO.Документ);
...
Функция СоздатьОбновитьЗаказы(ДокументыXDTO, РанееЗагруженныеЗаказы, Параметры, ОписаниеОшибки)
	...
	Для Каждого ДокументXDTO Из ДокументыXDTO Цикл
		...
		Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "ЗначенияРеквизитов")
			...
			МассивЗначенийРеквизитовXDTO = ОбъектыСпискаXDTO(ДокументXDTO.ЗначенияРеквизитов.ЗначениеРеквизита);
...
Процедура ЗаполнитьДанныеКонтрагента(ДанныеКонтрагента, ДокументXDTO)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "РасчетныеСчета")
		...
		МассивРасчетныхСчетовXDTO = ОбъектыСпискаXDTO(РасчетныеСчетаXDTO);
...
//			Наименование - Строка, наименованиеСклада.
//
Процедура ИдентифицироватьСклад(ДанныеСклада, Склад, ОписаниеОшибки)
	...
	Если Не ЗначениеЗаполнено(ДанныеСклада.Ид) 
		...
		ШаблонСообщения = НСтр("ru = 'Не заполнены поля идентификации точки самовывоза: %1 и %2'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, "Ид", "Наименование");
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru = 'Точка самовывоза не найдена по уникальному идентификатору: %1'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ДанныеСклада.Ид);
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ТекстОшибки);
...
Функция УточненноеПредставлениеАдреса(АдресXDTO)
	...
	Если ОбъектXDTOСодержитСвойство(АдресXDTO, "АдресноеПоле")
		...
		Для Каждого ТипАдреса Из АдресноеПоле Цикл
			...
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Почтовый индекс'")) Тогда
				ПочтовыйИндекс = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Страна'")) Тогда
				Страна = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Регион'")) Тогда
				Регион = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Район'")) Тогда
				Район = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Населенный пункт'")) Тогда
				НаселенныйПункт = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Город'")) Тогда
				Город = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Улица'")) Тогда
				Улица = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Дом'")) Тогда
				Дом = ТипАдреса.Значение;
			КонецЕсли;

			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Корпус'")) Тогда
				Корпус = ТипАдреса.Значение;
			КонецЕсли;
			
			Если НРег(ТипАдреса.Тип) = НРег(НСтр("ru = 'Квартира'")) Тогда
				Квартира = ТипАдреса.Значение;
			КонецЕсли;
...
Процедура ЗаполнитьДанныеКонтрагентаЗаказа(КонтрагентXDTO, ИнформацияПоКонтрагенту)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Контакты") И КонтрагентXDTO.Контакты <> Неопределено Тогда
		...
		МассивКонтактовXDTO = ОбъектыСпискаXDTO(КонтактыXDTO);
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "Представители") И КонтрагентXDTO.Представители <> Неопределено Тогда
		...
		МассивПредставителейXDTO = ОбъектыСпискаXDTO(ПредставителиXDTO);
...
Функция ИдентифицироватьНоменклатуру(РеквизитыЗаказа, ДокументXDTO, Параметры, ОписаниеОшибки)
	...
	Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "Налоги")
		...
		МассивНалоговXDTO = ОбъектыСпискаXDTO(НалогиXDTO);
	...
	МассивТоваровXDTO = ОбъектыСпискаXDTO(ТоварыXDTO);
	...
	Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
		...
		Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "ЗначенияРеквизитов")
			...
			МассивЗначенийРеквизитовXDTO = ОбъектыСпискаXDTO(ЗначенияРеквизитовXDTO);
		...
		Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "СтавкиНалогов")
			...
			МассивСтавокXDTO = ОбъектыСпискаXDTO(СтавкиНалогов);
			
			Для каждого СтавкаНалога Из МассивСтавокXDTO Цикл
				
				Если НРег(СтавкаНалога.Наименование) = НРег("Ндс") Тогда
		...
		Если ОбъектXDTOСодержитСвойство(ТоварXDTO, "Скидки")
			...
			МассивСкидокXDTO = ОбъектыСпискаXDTO(ТоварXDTO.Скидки.Скидка);
	...
	Если ОбъектXDTOСодержитСвойство(ДокументXDTO, "Скидки")
		...
		МассивСкидокXDTO = ОбъектыСпискаXDTO(ДокументXDTO.Скидки.Скидка);
		Для каждого СкидкаXDTO Из МассивСкидокXDTO Цикл
...
// В том случае если в табличной части документа не указана упаковка, в которой сделан заказ, то
// заполняем единицу измерения табличной части документа Заказ единицей измерения номенклатуры.
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ОписаниеОшибки, ПараметрыЗапроса="", Заголовки="")
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка удаления временного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ПолучитьДанныеССервера(Соединение, ОписаниеОшибки, ПараметрыЗапроса = "", Заголовки = "")
	...
	Попытка
		...
		Если ContentType <> Неопределено Тогда
			Если Найти(ContentType, "charset=") > 0 Тогда
				Кодировка = Прав(ContentType, СтрДлина(ContentType) - (Найти(ContentType, "charset=")+7));
			КонецЕсли;
		КонецЕсли;
...
Функция ТипЗначенияСправочник(Тип)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка преобразования типа'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
//
Функция ОбъектыСпискаXDTO(СписокXDTO)
