...
Функция ДоступныеВерсииФорматаПоПараметрамЭД(ИсходящийДокумент, СпособОбменаЭД, ИспользоватьУПД, ИспользоватьУКД) Экспорт 
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав") Тогда
		Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
			ФорматыДляУдаления.Добавить("ФНС 5.01 (УПД:Первичный документ)");
		КонецЕсли;	
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
			ФорматыДляУдаления.Добавить("ФНС 5.01 (УКД:Первичный документ)");

		КонецЕсли;
	КонецЕсли;
	
	// +Рарус Устаревший формат
	ФорматыДляУдаления.Добавить("CML 4.02");
	// -Рарус
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклНачало(Контекст)
	...
	Если ЭтоАдресВременногоХранилища(Контекст.Подпись.АдресЭД) Тогда
		ДвоичныеДанныеФайлаЭД = ПолучитьИзВременногоХранилища(Контекст.Подпись.АдресЭД);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДвоичныеДанныеФайлаЭД) Тогда
		ПрочитатьСертификатыПодписейВнутреннийЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
	
	Контекст.МенеджерКриптографии.НачатьПроверкуПодписи(Новый ОписаниеОповещения(
			"ПрочитатьСертификатыПодписейВнутреннийЦиклПослеПроверкиПодписи", ЭтотОбъект, Контекст,
			"ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиПроверкиПодписи", ЭтотОбъект),
		ДвоичныеДанныеФайлаЭД, ДвоичныеДанныеФайлаЭП);
	
КонецПроцедуры

// Продолжение процедуры ПрочитатьСертификатыПодписейВнутреннийЦиклНачало.
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиПроверкиПодписи(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	
	СтандартнаяОбработка = Ложь;
	
	ШаблонСообщения = НСтр("ru = 'Проверка подписи: %1'");
...
// Продолжение процедуры ПрочитатьСертификатыПодписейВнутреннийЦиклНачало.
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеПроверкиПодписи(Сертификат, Контекст) Экспорт
	
	Если Сертификат = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Сертификат не найден в данных подписи.'");
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", ТекстСообщения);
		ПрочитатьСертификатыПодписейВнутреннийЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьДанныеЗашифрованногоМаркера(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		...
		// В переменной Параметры получаем структуру (СоотвСоглашенийИМассиваСертификатовАвторизации,
		//                                            СоотвСертификатовИИхСтруктур,
		//                                            ВозврСоответствие).
		// ВозврСоответствие - параметр, который передается в метод,
		//                               указанный в обработке оповещения (ДополнительныеПараметры.ОбработкаОповещения).
...
Процедура РасшифроватьМаркер(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		...
		// В переменной Параметры получаем структуру(СоотвСоглашенийИМассиваСертификатовАвторизации,
		//                                           СоотвСертификатовИИхСтруктур, ВозврСоответствие).
		// ВозврСоответствие - параметр, который передается в метод, указанный
		// в обработке оповещения (ДополнительныеПараметры.ОбработкаОповещения).
		ВозврСоответствие = Неопределено;
		Параметры = ПараметрыПриложения["ЭлектронноеВзаимодействие." + ИД_Параметров];
		Если НЕ (ТипЗнч(Параметры) = Тип("Структура")
...
Функция НастройкиРегламентаЭДО(ВидЭД, ВерсияФормата, СпособОбменаЭДО) Экспорт
	...
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		...
		// АПК:1297-выкл
		Если
			ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УПД:Счет-фактура и первичный документ)")
			ИЛИ ВРег(ВерсияФормата) = "ON_NSCHFDOPPR_1_997_01_05_01"
		Тогда
