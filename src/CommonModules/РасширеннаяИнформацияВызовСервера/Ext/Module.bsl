...
Процедура ПриСозданииНаСервере(Форма) Экспорт
	...
	Если Форма.Элементы.Список.РежимВыбора Тогда
		Форма.Элементы.РасширеннаяИнформация.Видимость       = ЛОЖЬ;
...
Функция СформироватьРасширеннуюИнформациюОбОбъекте(СсылкаИлиДеревоНастроекИлиОбъект, ОграниченияТипов = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СсылкаИлиДеревоНастроекИлиОбъект = Неопределено ИЛИ ТипЗнч(СсылкаИлиДеревоНастроекИлиОбъект)=Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		...
		Для Каждого СтрокаНастройки Из ДеревоНастроек.Строки Цикл
			...
			Если Не СтрокаНастройки.ЭтоТабличнаяЧасть И СтрокаНастройки.Строки.Количество() > 0 Тогда        
				Для Каждого ТекСтрока Из СтрокаНастройки.Строки Цикл
					...
					Если ВывестиПримерЗаполнения Тогда
						ПримерПредставлениеРеквизита = ?(ЗначениеЗаполнено(ТекСтрока.ПредставлениеРеквизита), ТекСтрока.ПредставлениеРеквизита, ?(ЗначениеЗаполнено(ТекСтрока.ОригинальноеПредставлениеРеквизита), ТекСтрока.ОригинальноеПредставлениеРеквизита, НСтр("ru = 'Реквизит'")));
						...
						мОписание = мОписание + ?(ЗначениеЗаполнено(ТекСтрока.ПредставлениеРеквизита), "<%ЗаголовокОткрывающийТег%>" + ТекСтрока.ПредставлениеРеквизита + ": " + "<%ЗаголовокЗакрывающийТег%>", "") + ?(ТекСтрока.ТипЗначенияРеквизита = "Строка", "  {" + ПримерПредставлениеРеквизита + "}", "</B>" + "<a href="""">" + "  {" + ПримерПредставлениеРеквизита + "}</a>");
					Иначе
					...
					Если ЗначениеЗаполнено(мОписание) Тогда
						Попытка    Размещение = ТекСтрока.Размещение;
			...
			ИначеЕсли СтрокаНастройки.ЭтоТабличнаяЧасть Тогда
				...
				Если НЕ ДеревоНастроек.Колонки.Найти("ПредставлениеСписком")=Неопределено И СтрокаНастройки.ПредставлениеСписком Тогда
...
Функция НастройкиПолейРасширеннойИнформацииПолучитьНастройкиПоУмолчанию(ИмяОбъекта)
	...
	Исключение
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при загрузке из файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Функция НастройкиПолейРасширеннойИнформацииПолучитьHTMLОписаниеТабличнойЧастиСписком(СсылкаНаОбъект, Реквизиты, СтрокаНастройки)
		...
		Для Каждого Строка Из ТаблицаРеквизитов Цикл
			...
			Для Каждого колонка Из  ТаблицаРеквизитов.Колонки Цикл
				...
				Если Реквизит <> Неопределено Тогда
					...
					ОписаниеСтроки = ОписаниеСтроки + ОписаниеРеквизита+ ?(ТаблицаРеквизитов.Колонки.Количество()>1,?(ОписаниеСтроки = "",": ",?(ТаблицаРеквизитов.Колонки.Количество()=ТекущаяКолонка + 1,"",", ")),?(ТаблицаРеквизитов.Колонки.Количество()=ТекущаяКолонка + 1,"",", "));
				КонецЕсли;
				ТекущаяКолонка = ТекущаяКолонка + 1;
			КонецЦикла;
	...
	Возврат  "<%ЗаголовокТаблицыОткрывающийТег%>" + СтрокаНастройки.ПредставлениеРеквизита + "<%ЗаголовокТаблицыЗакрывающийТег%></BR>" +  Описание;
...
//	
Функция НастройкиПолейРасширеннойИнформацииПолучитьОписаниеОбъектаHTML(СсылкаНаОбъект, Представление, БезПереноса = Ложь, ГиперСсылка = Истина, ТегиЭлемента = Неопределено)
...
Функция НастройкиПолейРасширеннойИнформацииСохранитьВФайл(ИмяФайла,Объект) Экспорт
	...
	Попытка
		...
		// Для записи общего макета - убрать комментарий
		//ТаблицаНастроек[0].Объект = "ВсеОбъекты";
