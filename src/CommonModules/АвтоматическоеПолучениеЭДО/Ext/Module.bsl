...
Процедура ПриСозданииУчетнойЗаписиЭДО(ОписаниеУчетнойЗаписи) Экспорт
	...
	Если ОписаниеУчетнойЗаписи.ИспользоватьЦентрЭДО Тогда
...
Процедура ПриУдаленииУчетнойЗаписи(ИдентификаторЭДО, СпособОбмена, ИспользоватьЦентрЭДО) Экспорт
	
	Если СпособОбмена <> Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО
		И Не ИспользоватьЦентрЭДО Тогда
		Возврат;
	КонецЕсли;
...
Функция Использовать() Экспорт
	...
	Возврат АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеПоКлючуДоступа()
		...
		Или НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО();
...
Функция ВключитьПоУчетнымЗаписямЭДО(ИдентификаторыЭДО, РасшифрованныеМаркеры) Экспорт
	...
	Если ЗначениеЗаполнено(ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентраЭДО)
		...
		РасшифрованныеМаркерыДляЦентраЭДО = ВыбратьРасшифрованныеМаркеры(РасшифрованныеМаркеры, ИдентификаторыЭДО);
		
		ОбщийМодульАвтоматическоеПолучениеЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль(
			"АвтоматическоеПолучениеЭДОИнтеграцияЦентраЭДО");
			
		РезультатВключенияВЦентреЭДО = ОбщийМодульАвтоматическоеПолучениеЭДОИнтеграцияЦентраЭДО.ВключитьПоУчетнымЗаписямЭДО(
			ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентраЭДО, РасшифрованныеМаркерыДляЦентраЭДО);
		
		ДополнитьРезультатВключения(РезультатВключения, РезультатВключенияВЦентреЭДО);
	КонецЕсли;
...
Функция ВидОшибкиПроверкаПодписейНедоступна() Экспорт
	...
	ВидОшибки.ОписаниеРешения = НСтр("ru = '<a href = ""%1"">Включите</a> проверку электронных подписей на сервере
		|и повторите операцию'");
	ВидОшибки.ОбработчикиНажатия.Вставить("Включите",
	...
	ВидОшибки.ПараметрыОбработчиков.Вставить("Включите", ДополнительныеПараметры);
...
Функция ЕстьУчетныеЗаписиДляПолучения()
	Возврат НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО()
			И УчетныеЗаписиЭДО.ЕстьУчетныеЗаписиЧерезЦентрЭДО()
...
Функция Получить(ВидОперации, РасшифрованныеМаркеры = Неопределено)
	...
	Попытка
		...
		Если ИспользоватьИнтеграциюЦентраЭДО Тогда
			...
			РезультатПоЦентруЭДО = ПолучитьПоУчетнымЗаписямЦентровЭДО();
			ДополнитьРезультатПолучения(РезультатВариантовПолучения, РезультатПоЦентруЭДО);
...
Функция ПолучитьПоУчетнымЗаписямЦентровЭДО()
	...
	ИдентификаторыЭДОПоУчетнымЗаписямЦентровЭДО =
		МодульИнтеграцияЦентраЭДО.ИдентификаторыЭДОПоУчетнымЗаписямЦентровЭДО();
	...
	Для Каждого ИдентификаторыЭДОПоУчетнойЗаписиЦентраЭДО Из ИдентификаторыЭДОПоУчетнымЗаписямЦентровЭДО Цикл
		...
		РезультатВыполнения = МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ВыполнитьДействияЭДО(
			ПараметрыВыполнения, КонтекстДиагностики);
		РезультатДействийЭДО = РезультатВыполнения.РезультатыДействий[УчетнаяЗаписьЦентраЭДО];
		
		ЭлектронныеДокументыЭДОКлиентСервер.ДополнитьИтогВыполненияДействийПоЭДО(
			ИтогДействийПоЭДО, РезультатДействийЭДО.Итог);
		
		ОбновитьДатуПолучения();
		ЕстьКритичнаяОшибка = Ложь;
		Ошибки = ОбработкаНеисправностейБЭДКлиентСервер.ПолучитьОшибки(КонтекстДиагностики);
		Для Каждого Ошибка Из Ошибки Цикл
			ЕстьКритичнаяОшибка = ОбработкаНеисправностейБЭДКлиентСервер.ЭтоОшибкаДанногоВида(Ошибка,
				МодульИнтеграцияЦентраЭДО.ВидОшибкиОтсутствуетОбластьЦентраЭДО()) Или ЕстьКритичнаяОшибка;
		КонецЦикла;

		Для Каждого ИдентификаторЭДО Из ИдентификаторыЭДО Цикл
			СостояниеПолучения = РегистрыСведений.СостоянияАвтоматическогоПолученияЭДО.Записать(
				ИдентификаторЭДО, ЕстьКритичнаяОшибка, ДатаНачала);
			СостояниеПоИдентификаторамЭДО.Вставить(ИдентификаторЭДО, СостояниеПолучения);
		КонецЦикла;
...
Процедура ДополнитьРезультатПолучения(Приемник, Источник)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
		Приемник.СостояниеПоИдентификаторамЭДО, Источник.СостояниеПоИдентификаторамЭДО, Истина);
