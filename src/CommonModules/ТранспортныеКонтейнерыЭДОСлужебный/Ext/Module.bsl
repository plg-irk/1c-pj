...
Процедура ПодготовитьДанныеПоКарточке(ОписаниеОбъекта, ДеревоКарточки)
	...
	Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ОписаниеОбъекта.ТипЭлементаРегламента) Тогда
		СвойстваУниверсальногоСообщенияЭДО = Документы.СообщениеЭДО.СвойстваУниверсальногоСообщенияЭДО(ОписаниеОбъекта.Объект);
		Если ЗначениеЗаполнено(СвойстваУниверсальногоСообщенияЭДО) Тогда
			НоваяСтрока = ДополнительныеДанные.Добавить();
			НоваяСтрока.Имя = "ФайлНазначения";
			НоваяСтрока.Значение = СвойстваУниверсальногоСообщенияЭДО.ИмяФайлаНазначения;
		КонецЕсли;
	КонецЕсли;
...
Функция ТаблицаТиповЭлементовРегламента()
	...
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица, 
		...
		Перечисления.ТипыЭлементовРегламентаЭДО.УС_ИОП);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица, 
		"UniversalMessageCorrectionNotice",
		Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ);
	ДобавитьВТаблицуТипЭлементаРегламента(Таблица, 
...
Функция ЭтоИзвещениеОПолучении(ТипЭлементаРегламента)
	...
	Возврат ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИОП
		Или ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ИОП
