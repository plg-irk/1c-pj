...
Функция ОтложеннаяОтправкаЭлектронныхДокументов() Экспорт
	...
	Результат = Константы.ИспользоватьОтложеннуюОтправкуЭлектронныхДокументов.Получить();
...
// Определяет, используется ли просмотр PDF документов ЭДО.
// 
// Возвращаемое значение:
...
Функция ИспользуетсяПросмотрPDFДокументов() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.ИспользоватьПросмотрPDFДокументовЭДО.Получить();
	УстановитьПривилегированныйРежим(Ложь);
...
// Параметры:
//  ИспользоватьПросмотр - Булево
Процедура УстановитьИспользованиеПросмотраPDFДокументовЭДО(ИспользоватьПросмотр) Экспорт
	
	ИспользуетсяПросмотрPDFДокументов = ИспользуетсяПросмотрPDFДокументов();
	Если ИспользуетсяПросмотрPDFДокументов <> ИспользоватьПросмотр Тогда
		Константы.ИспользоватьПросмотрPDFДокументовЭДО.Установить(ИспользоватьПросмотр);
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ИспользоватьИнтеграциюЛокальногоЭДО() Экспорт
...
Функция ИспользоватьИнтеграциюЦентраЭДО() Экспорт
	Возврат ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияЦентраЭДО")
		И ПолучитьФункциональнуюОпцию("ИспользоватьИнтеграциюЦентраЭДО");
КонецФункции

// Возвращаемое значение:
// 	Булево - Описание
Функция ПриниматьПриглашенияЭДОАвтоматически() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.ПриниматьПриглашенияЭДОАвтоматически.Получить();
	УстановитьПривилегированныйРежим(Ложь);
...
//  Неопределено - В случае подписания, будет использоваться настройка БСП по типам подписи.
//
...
// Определяет, используются ли черновики электронных документов.
// 
// Возвращаемое значение:
//  Булево
Функция ИспользуютсяЧерновикиЭлектронныхДокументов() Экспорт
	
	Возврат ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьЧерновикиЭлектронныхДокументов");
	
КонецФункции

// Определяет, используется ли механизм проверки поддерживаемых абонентами форматов электронных документов.
// 
// Возвращаемое значение:
//  Булево
Функция ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО() Экспорт
	
	Возврат ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО");
	
КонецФункции

// Возвращаемое значение:
// 	Булево
Функция ИспользуетсяЗаполнениеРеквизитовВходящегоНеформализованногоДокументаПоНаименованиюФайла() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Результат = Константы.ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла.Получить();
	УстановитьПривилегированныйРежим(Ложь);
...
// Возвращаемое значение:
//  см. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
Функция ЗапросНастройкиОтправкиЭлектронныхДокументовПоВидамПоОписаниюОбъектовУчета(ОписанияОбъектовУчета,
...
//
Процедура ОбновитьФорматПоУмолчаниюВНастройках(ПараметрыПроцедуры, АдресРезультата = "") Экспорт
	
	ВидОперации = НСтр("ru = 'Обновление форматов в настройках отправки ЭДО'");
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ втДействующиеФорматы КАК ДействующиеФорматы
	|	ПО (НастройкиОтправки.ВерсияФормата = ДействующиеФорматы.Формат)
	|ГДЕ
	|	НЕ НастройкиОтправки.ВерсияФорматаУстановленаВручную
	|	ИЛИ ДействующиеФорматы.Формат ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	втНастройкиДляОбновления.Формат КАК ТекущаяВерсияФормата,
	|	ЕСТЬNULL(втНастройкиКонтрагентов.Формат, втНастройкиПоУмолчанию.Формат) КАК НоваяВерсияФормата,
	|	ЕСТЬNULL(НастройкиЗаполненияДополнительныхПолей.Настройка, НЕОПРЕДЕЛЕНО) КАК НастройкиЗаполненияДополнительныхПолей,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиЗаполненияДополнительныхПолей КАК НастройкиЗаполненияДополнительныхПолей
	|		ПО втНастройкиДляОбновления.Отправитель = НастройкиЗаполненияДополнительныхПолей.Отправитель
	|		И втНастройкиДляОбновления.Получатель = НастройкиЗаполненияДополнительныхПолей.Получатель
	|		И втНастройкиДляОбновления.Договор = НастройкиЗаполненияДополнительныхПолей.Договор
	|		И втНастройкиДляОбновления.ВидЭД = НастройкиЗаполненияДополнительныхПолей.ВидЭлектронногоДокумента 
	|		И втНастройкиДляОбновления.Формат = НастройкиЗаполненияДополнительныхПолей.Формат
	|ГДЕ
	...
	ИндексЗапросаСДублямиФормата = РезультатЗапроса.Количество() - 1;
	
	ИндексЗапросаСНастройкамиДляОбновления = ИндексЗапросаСДублямиФормата - 1;
	
	Шаблон = НСтр("ru = 'Для вида ЭД ''%1'' указан одинаковый приоритет у %2 форматов'");
	
	Выборка = РезультатЗапроса[ИндексЗапросаСДублямиФормата].Выбрать();
	...
	Выборка = РезультатЗапроса[ИндексЗапросаСНастройкамиДляОбновления].Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		ПереходНаНовыйФорматВыполнен = Ложь;
		...
		Попытка
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				Запись.ВерсияФормата = Выборка.НоваяВерсияФормата;
			КонецЦикла;
			
			НаборЗаписей.Записать();
			УстановитьПривилегированныйРежим(Ложь);
			
			ПереходНаНовыйФорматВыполнен = Истина;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ВидОперации,
				...
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())); //АПК:157 Для параметра "ИмяСобытия" метода "ЗаписьЖурналаРегистрации()" не задан основной язык конфигурации (157)
		КонецПопытки;
		
		Если ПереходНаНовыйФорматВыполнен И Выборка.НастройкиЗаполненияДополнительныхПолей <> Неопределено Тогда
			ДанныеДляПереносаНастроек = НастройкиОтправкиЭДО.НовыеДанныеДляПереносаНастроекЗаполненияДополнительныхПолей();
			ЗаполнитьЗначенияСвойств(ДанныеДляПереносаНастроек, Выборка,
				"Отправитель, Получатель, Договор, ВидДокумента");
			ДанныеДляПереносаНастроек.СтарыйФормат = Выборка.ТекущаяВерсияФормата;
			ДанныеДляПереносаНастроек.НовыйФормат = Выборка.НоваяВерсияФормата;
			ДанныеДляПереносаНастроек.Настройка = Выборка.НастройкиЗаполненияДополнительныхПолей;
			НастройкиОтправкиЭДО.ПеренестиНастройкиЗаполненияДополнительныхПолей(ДанныеДляПереносаНастроек);
		КонецЕсли;
...
// Создает настройки отправки при наличии отправленного приглашения на идентификатор
// 
// Параметры:
// 	КлючНастроекОтправки - Неопределено - если не удалось определить ключ настроек отправки.
//  					 - См. НастройкиЭДОКлиентСервер.НовыйКлючНастроекОтправки
// 	
// Возвращаемое значение:
// 	См. НастройкиОтправкиЭДО.НастройкиОтправки
Функция СоздатьНастройкиПриНаличииПриглашения(КлючНастроекОтправки) Экспорт
	...
	Отбор.Статус = "&СтатусПриглашения";
	Отбор.Тип = "&ТипПриглашения";
	...
	Запрос.Текст = 
		...
		|		И (Приглашения.Тип = &ТипПриглашения)
		|		И (Приглашения.Статус В (&СтатусПриглашения))";
	
	ИтоговыйЗапрос = ОбщегоНазначенияБЭД.СоединитьЗапросы(Запрос, Запросы);
	ИтоговыйЗапрос.УстановитьПараметр("Контрагент", Контрагент);
	ИтоговыйЗапрос.УстановитьПараметр("Организация", Организация);
	ИтоговыйЗапрос.УстановитьПараметр("СтатусПриглашения", Перечисления.СтатусыПриглашений.ОжидаемСогласия);
	ИтоговыйЗапрос.УстановитьПараметр("ТипПриглашения", Перечисления.ТипыПриглашений.НаИдентификатор);
	
	РезультатЗапроса = ИтоговыйЗапрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	
	НастройкиСозданы = Ложь;
	Пока Выборка.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборка.ИдентификаторОрганизации) 
			Или Не ЗначениеЗаполнено(Выборка.ИдентификаторКонтрагента) Тогда
				Продолжить;
		КонецЕсли;
		
		Отказ = Ложь;
		НастройкиОтправкиЭДО.СоздатьНастройкиОтправки(Организация, Контрагент, ,
			Выборка.ИдентификаторОрганизации, Выборка.ИдентификаторКонтрагента, Отказ);
		
		Если Отказ Тогда
			НастройкиСозданы = Ложь;
		Иначе
			НастройкиСозданы = Истина;
			СоздатьНастройкиОтраженияВУчете(Организация, Контрагент, Выборка.ИдентификаторОрганизации,
				Выборка.ИдентификаторКонтрагента);
		КонецЕсли;
		
	КонецЦикла;
	
	Если НастройкиСозданы Тогда
		Настройки = НастройкиОтправки(КлючНастроекОтправки);
	КонецЕсли;
...
Функция ПредупрежденияКомпонента(КлючНастроекОтправки, ФорматДокумента) Экспорт
	
	Возврат НастройкиЭДОСлужебный.ПредупрежденияПоНастройкеОбменаНаДату(КлючНастроекОтправки,, ФорматДокумента);
...
//  ИмяВременнойТаблицы - Строка
//  Отбор - Неопределено
//        - См. СинхронизацияЭДО.НовыйОтборПоддерживаемыхФорматов
// 
// Возвращаемое значение:
//  См. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
Функция ЗапросАктуальныхПоддерживаемыхФорматов(ИмяВременнойТаблицы, Отбор = Неопределено)
	
	Если Не ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО() Тогда
		Возврат ЗапросАктуальныхФорматов(ИмяВременнойТаблицы);
	КонецЕсли;
...
// Возвращаемое значение:
//  См. ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса
Функция ЗапросАктуальныхФорматов(ИмяВременнойТаблицы)
	...
	Запрос.Текст =
		...
		|	АктуальныеФорматы.ВидДокумента КАК ВидДокумента,
		|	"""" КАК ИдентификаторАбонента
		|ПОМЕСТИТЬ ИмяВременнойТаблицы
		|ИЗ
		|	АктуальныеФорматы КАК АктуальныеФорматы";
	
	ИтоговыйЗапрос = ОбщегоНазначенияБЭД.СоединитьЗапросы(Запрос, Запросы);
	
	ОписаниеЗапроса = ОбщегоНазначенияБЭД.НовоеОписаниеЗапроса();
	ОписаниеЗапроса.Текст = ОбщегоНазначенияБЭД.ТекстЗапросаИзШаблона(ИтоговыйЗапрос.Текст,
		ИмяВременнойТаблицы, "", "");
	Для Каждого Параметр Из ИтоговыйЗапрос.Параметры Цикл
		ОписаниеЗапроса.СлужебныеПараметры.Вставить(Параметр.Ключ, Параметр.Значение);
	КонецЦикла;
