...
Функция ОсновнойЭлектронныйДокументОбъектаУчета(Знач ОбъектУчета) Экспорт
	...
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент.Организация КАК Организация,
	// ТК + 
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент.Дата КАК ЭлектронныйДокументДата,
	|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент.Номер КАК ЭлектронныйДокументНомер
	// ТК - 
	...
	ПоляТаблицы = 
		...
		// ТК + 
		|	ДанныеДокументовОбъектаУчета.ЭлектронныйДокументДата КАК ЭлектронныйДокументДата,
		|	ДанныеДокументовОбъектаУчета.ЭлектронныйДокументНомер КАК ЭлектронныйДокументНомер";
		// ТК - 
	ИмяТаблицы = 
	...
	// ТК + 
	Выборка.Сортировать("ЭлектронныйДокументДата УБЫВ, ЭлектронныйДокументНомер УБЫВ");
	// ТК - 
...
Функция ОписаниеОбъектаУчета(ИсточникДанных) Экспорт
	...
	ИначеЕсли ЭтоЭПД = Истина Тогда
		...
		СтрокаОписания.Договор      = ПараметрыИсточника.ДоговорКонтрагента;
	// Рарус + 
	ИначеЕсли ТипИсточникаДанных = Тип("СправочникСсылка.ДоговорыВзаиморасчетов")
		Или ТипИсточникаДанных = Тип("СправочникОбъект.ДоговорыВзаиморасчетов") Тогда
		
		СтрокаОписания = Описание.Добавить();
		СтрокаОписания.ОбъектУчета  = ИсточникДанныхСсылка;
		СтрокаОписания.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент;
		СтрокаОписания.Направление  = Перечисления.НаправленияЭДО.Исходящий;
...
Функция ЗначенияРеквизитовОбъектаУчета(Знач ОбъектУчета, Знач Реквизиты) Экспорт
	...
	Для каждого Реквизит Из НаборРеквизитов Цикл
		...
		// Рарус + 
		ИначеЕсли ЕстьРеквизит(ОбъектУчета, Реквизит) Тогда
		// Рарус - 
...
// Параметры:
//  КлючевыеРеквизитыКонтрагента - См. КлючевыеРеквизитыКонтрагента
// 
// Возвращаемое значение:
//  Неопределено,ОпределяемыйТип.КонтрагентБЭД
Функция КонтрагентПоКлючевымРеквизитам(КлючевыеРеквизитыКонтрагента) Экспорт
	Возврат СсылкаНаОбъектПоИННКПП("Контрагенты", КлючевыеРеквизитыКонтрагента.ИНН, КлючевыеРеквизитыКонтрагента.КПП);
КонецФункции
...
Функция ДанныеСостоянияЭДОДляФормыОбъектаУчета(ОбъектУчета) Экспорт
	...
	//RADV
	Если НЕ Пользователи.РолиДоступны("ЧтениеНастроекОбменаСКонтрагентами") Тогда
...
// Формирует контекст, который может помочь пользователю при сопоставлении номенклатуры.
// Организация и договор контрагента заполняются только в случае единственного значения по переданному массиву ЭД.
// 
// Параметры:
...
//
Функция КонтекстСопоставленияНоменклатуры(ЭлектронныеДокументы) Экспорт
	
	КонтекстСопоставления = Новый Структура;
	КонтекстСопоставления.Вставить("Организация", Неопределено);
	КонтекстСопоставления.Вставить("ДоговорКонтрагента", Неопределено);
	...
	|	ЭлектронныйДокументВходящийЭДО.ДоговорКонтрагента
	|ИЗ
	...
	Если Выборка.Количество() = 1 Тогда
		Выборка.Следующий();
		ЗаполнитьЗначенияСвойств(КонтекстСопоставления, Выборка);
	КонецЕсли;
	
	Возврат КонтекстСопоставления;
...
// Возвращаемое значение:
//  См. ОбменСКонтрагентамиКлиентСервер.НовыйНастройкиПодключаемыхКомандЭДО
Функция НастройкиПодключаемыхКомандЭДО() Экспорт
	
	НастройкиКоманд = ОбменСКонтрагентамиКлиентСервер.НовыйНастройкиПодключаемыхКомандЭДО();
	ОбменСКонтрагентамипереопределяемый.ПриОпределенииНастроекПодключаемыхКомандЭДО(НастройкиКоманд);
...
//                                             еще не подписан - внесение изменений в данные электронного, 
...
Функция НовоеОписаниеОснованияЭлектронногоДокумента()
	...
	Описание.Вставить("КодВалюты", "");
	Описание.Вставить("ЦенаВключаетНДС", Ложь);
	Описание.Вставить("СуммаНДСДокумента", 0);
