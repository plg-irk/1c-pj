...
Процедура ДополнитьКонтекстДиагностики(Приемник, Источник) Экспорт
	...
	// ТК + 
	Попытка
		ДатаОкончанияОперации = ПолучитьЗначениеВложенногоПараметраСтруктуры(Источник, "ДатаОкончанияОперации", Приемник.ДатаОкончанияОперации);
		Приемник.ДатаОкончанияОперации = ДатаОкончанияОперации;
	Исключение
	КонецПопытки;
	// ТК - 
...
// Возвращает Массив Из КонтекстДиагностики.Диагностика.Ошибки.ПодробноеПредставлениеОшибки
// Параметры:
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  Массив из Строка - массив подробных ошибок из контекста диагностики
Функция ПодробныеПредставленияОшибок(КонтекстДиагностики) Экспорт
	Ошибки = ПолучитьОшибки(КонтекстДиагностики);
	Результат = Новый Массив;
	Для Каждого Ошибка Из Ошибки Цикл
		Результат.Добавить(Ошибка.ПодробноеПредставлениеОшибки);
	КонецЦикла;
	Возврат Результат;
КонецФункции

// Возвращает Массив Из КонтекстДиагностики.Диагностика.Ошибки.КраткиеПредставленияОшибок
// Параметры:
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  Массив из Строка - массив подробных ошибок из контекста диагностики
Функция КраткиеПредставленияОшибок(КонтекстДиагностики) Экспорт
	Ошибки = ПолучитьОшибки(КонтекстДиагностики);
	Результат = Новый Массив;
	Для Каждого Ошибка Из Ошибки Цикл
		Результат.Добавить(Ошибка.КраткоеПредставлениеОшибки);
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
//  * ОбъектыДействий - Массив из см. ВыполнитьДействияЭДО.ПараметрыВыполнения
...
//  Структура:
// * Ошибка - см. НоваяОшибка
// * СообщениеДляПользователя - Строка
//
Функция ДобавитьОшибку(КонтекстДиагностики, Ошибка, ЗаписыватьВЖурналРегистрации, Подсистема, СообщатьОбОшибке) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации Тогда
		ВидОперации = ?(ПустаяСтрока(КонтекстДиагностики.ЗаголовокОперации), Ошибка.ВидОперации,
			КонтекстДиагностики.ЗаголовокОперации + ". " + Ошибка.ВидОперации);
