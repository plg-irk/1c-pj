...
//  Объект               - ДанныеФормыСтруктура - Будет использован объект, переданный через параметр.
//
Процедура РазрешитьРедактированиеРеквизитовОбъекта(Знач Форма, ОбработкаПродолжения = Неопределено, Объект = Неопределено) Экспорт // -РАРУС
	...
	// +РАРУС Принято решение отказаться от проверки ссылок на объект, так как разработана такая схема блокировки,
	// при которой неиспользуемые объекты не будут блокироваться.
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("Форма", Форма);
	ПараметрыОповещения.Вставить("ЗаблокированныеРеквизиты", ЗаблокированныеРеквизиты);
	ПараметрыОповещения.Вставить("ОбработкаПродолжения", ОбработкаПродолжения);
	ПараметрыОповещения.Вставить("МассивСсылок", Новый Массив);
	...
	ИначеЕсли СинонимыРеквизитов[0] = НСтр("ru = 'Этот объект'") Тогда
			ТекстВопроса = НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе, все реквизиты не доступны для редактирования.
			|Разрешить редактирование?'");
	Иначе
	...
	ПараметрыОповещения.Вставить("ОбработкаПродолжения", Новый ОписаниеОповещения("РазрешитьРедактированиеРеквизитовОбъектаПослеПроверкиСсылок",ЗапретРедактированияРеквизитовОбъектовСлужебныйКлиент, Параметры));
	ПоказатьВопрос(Новый ОписаниеОповещения("ПроверитьСсылкиНаОбъектПослеПодтвержденияПроверки",ЗапретРедактированияРеквизитовОбъектовСлужебныйКлиент, ПараметрыОповещения),ТекстВопроса,РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, ЗаголовокДиалога);
	Возврат;
	// -РАРУС
...
Процедура УстановитьДоступностьЭлементовФормы(Знач Форма, Знач Реквизиты = Неопределено) Экспорт
	...
	Для Каждого ОписаниеБлокируемогоРеквизита Из Форма.ПараметрыЗапретаРедактированияРеквизитов Цикл
		Если ОписаниеБлокируемогоРеквизита.РедактированиеРазрешено Тогда
			...
			// +РАРУС Возможно нужно разблокировать всю форму
			Если ОписаниеБлокируемогоРеквизита.ИмяРеквизита = "ЭтотОбъект" И Форма.ТолькоПросмотр Тогда
				Форма.ТолькоПросмотр = Ложь;
				Прервать;
			КонецЕсли;
			
			// Или форму параметров
				...
				Если ЭлементФормы <> Неопределено Тогда
					Если ТипЗнч(ЭлементФормы) = Тип("ПолеФормы")
					 ...
					 Или ТипЗнч(ЭлементФормы) = Тип("ТаблицаФормы") Тогда
						...
						// +РАРУС Разблокируем добавленные программно кнопки, если т/ч заблокирована.
						Если НЕ Форма.Элементы.Найти(""+БлокируемыйЭлементФормы + "КоманднаяПанель") = Неопределено Тогда
							Форма.Элементы.Найти(""+БлокируемыйЭлементФормы + "КоманднаяПанель").Доступность = Истина;
						КонецЕсли;
						// -РАРУС
					Иначе
	...
	Для Каждого ОписаниеБлокируемогоРеквизита Из Форма.ПараметрыЗапретаРедактированияРеквизитов Цикл
		...
		Если ОписаниеБлокируемогоРеквизита.ПравоРедактирования
		   И (    ОписаниеБлокируемогоРеквизита.РедактированиеРазрешено = Ложь
			...
			// Или форму параметров документа
			Если ОписаниеБлокируемогоРеквизита.ИмяРеквизита = "ПараметрыДокумента" Тогда
				ДобавитьРеквизит = Истина;
			КонецЕсли;
			// -РАРУС
