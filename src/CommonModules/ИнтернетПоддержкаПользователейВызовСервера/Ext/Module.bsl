...
Функция ПодключениеКСервисуИППССервера1СПредприятия(СетевыеПараметрыИПП = Неопределено) Экспорт
	...
	Если Не ОбращениеССервера Тогда
		...
		СетевыеПараметрыИПП.Вставить("ТаймаутСети",
			ИнтернетПоддержкаПользователейКлиентСервер.НастройкиСоединенияССерверами().ТаймаутПодключения);
	КонецЕсли;
...
Функция ЗначениеНастройкиНеНапоминатьОбАвторизацииДо() Экспорт
	...
	Возврат ЗначениеНастройкиИППНеНапоминатьОбАвторизацииДо();
...
Процедура УстановитьНастройкуНеНапоминатьОбАвторизацииДо(УстанавливатьНастройку) Экспорт
	...
	УстановитьНастройкуИППНеНапоминатьОбАвторизацииДо(УстанавливатьНастройку);
...
//                             (см. ОблачныйАрхивКлиентСервер.ПолучитьСписокВсехСобытийЖурналаРегистрации).
...
Функция ДопПараметрыОтправкиСообщения(Знач ДопПараметры) Экспорт
	...
	Результат = Новый Структура;
	ИнтернетПоддержкаПользователей.ЗаполнитьДопПараметрыОтправкиСообщенияВТехПоддержку(Результат, ДопПараметры);
	Возврат Результат;
...
// Возвращает параметры получения тикета.
//
Функция ПараметрыПолученияТикета() Экспорт
	
	Возврат ИнтернетПоддержкаПользователей.ПараметрыПолученияТикетаДляОткрытияВебСтраницы();
