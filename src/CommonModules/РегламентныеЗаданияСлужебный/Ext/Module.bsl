...
Процедура ПроверитьВозможностьРаботыСВнешнимиРесурсами(РегламентноеЗадание, ЗависимостиЗаданий = Неопределено) Экспорт
	...
	Иначе
		...
		//ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменилась строка соединения
		// информационной базы. |Возможно информационная база была перемещена.
...
Функция РегламентныеЗаданияЗависимыеОтФункциональныхОпций() Экспорт
	...
	// +Рарус
	ТехнологическаяПлатформаВызовСервера.ПриОпределенииНастроекРегламентныхЗаданий(Зависимости);
	// -Рарус
...
Функция ОбновленнаяТаблицаРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для каждого Задание Из ТекущиеЗадания Цикл
			...
			// Проверим наличие отдельной формы настройки регламентного задания.
			Обновляемое.ЕстьФормаНастройки = НЕ Метаданные.Обработки.РегламентныеИФоновыеЗадания.Формы.Найти("НастройкаРегламентногоЗадания" + ИмяЗадания) = Неопределено;
			Обновляемое.Имя                = ИмяЗадания;
