...
// Процедура пытается получить логин и пароль из кеша, если не находит то пытается получить из безопасного хранилища.
//
// Параметры:
//  ОповещениеВозврата - ОписаниеОповещения - обработчик, вызываемый при завершении процедуры со следующими параметрами:
...
//
Процедура ПолучитьДанныеАутентификации(ОповещениеВозврата, НастройкаОбмена) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеАвторизации) Тогда
		...
		ВыполнитьОбработкуОповещения(ОповещениеВозврата, ДанныеАвторизации);
		
	Иначе
		
		// Если данные не получены из ПараметрыПриложения, то возможно он хранятся в безопасном хранилище, 
		//  если ТребуетсяSMSАвторизация установлено в Истина
		ПрочитатьПарольИзБезопасногоХранилища(НастройкаОбмена, ОповещениеВозврата);
		
	КонецЕсли;
	
КонецПроцедуры
	
// Добавляет пароль в кэш на время сеанса.
//
// Параметры:
//  ОбъектПривязки - СправочникСсылка.НастройкиОбменСБанками - ссылка на объект, к которому относится пароль.
//  ДанныеАвторизации - Структура - с ключами Логин и Пароль
//
Процедура СохранитьПарольНаВремяСеанса(НастройкаОбмена, ДанныеАвторизации) Экспорт
	...
	Соответствие.Вставить(НастройкаОбмена, ДанныеАвторизации);
...
// Сохраняет пароль в безопасное хранилище на длительный срок.
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на объект, к которому относится пароль.
//  ДанныеАвторизации - Структура - с ключами Логин и Пароль
//	
Процедура СохранитьПарольВБезопасноеХранилище(НастройкаОбмена, ДанныеАвторизации) Экспорт
	
	СохранитьПарольНаВремяСеанса(НастройкаОбмена, ДанныеАвторизации);
	
	ПараметрыОперации = Новый Структура;
	ПараметрыОперации.Вставить("ИдентификаторМетода", "Сохранить");
	ПараметрыОперации.Вставить("НастройкаОбмена", НастройкаОбмена);
	
	ПарольСтрокой = Строка(ДанныеАвторизации.Пароль);
	ДвоичныеДанныеИзСтроки = ПолучитьДвоичныеДанныеИзСтроки(ПарольСтрокой);
	Пароль = Base64Строка(ДвоичныеДанныеИзСтроки); 
	
	ДанныеАвторизацииСохранение = ДанныеАвторизации(ДанныеАвторизации.Логин, Пароль);
	ПараметрыОперации.Вставить("ДанныеАвторизации", ДанныеАвторизацииСохранение);
		
	НачатьОперациюСПаролемВБезопасномХранилище(ПараметрыОперации);
...
Процедура СинхронизироватьСБанком(
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
		// Только получение статусов, т.к. при отправке не формируется пакет
		ОбменСБанкамиSberAPIСлужебныйКлиент.ПолучитьДокументы(Параметры);
		...
		Если Соответствие.Количество() > 0 Тогда
			...
			Иначе
				Оповещение = Новый ОписаниеОповещения(
					...
					Параметры);
				ПолучитьПарольКСертификату(Оповещение, Соответствие, ВидОперации, , , НастройкаОбмена);
				Возврат;
			КонецЕсли;
		Иначе
			...
			Если НЕ ПарольУстановлен Тогда
				...
				Иначе
					...
					ПолучитьПарольКСертификату(
						ОООЗ,
						Соответствие,
						НСтр("ru = 'Аутентификация на ресурсе банка'"),
						,
						,
						Параметры.НастройкаОбмена);
					Возврат
...
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками, Неопределено - используемая настройка обмена с банком.
//
Процедура ПолучитьПарольКСертификату(
	...
	Если ТипЗнч(СоотвСертификатовИИхСтруктур) = Тип("Соответствие")
		...
		SberAPI = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI");
		Если СоотвСертификатовИИхСтруктур.Количество() = 1
			Или ПрограммаБанка = СбербанкОнлайн
			Или ПрограммаБанка = SberAPI Тогда
			
			Для Каждого Элемент Из СоотвСертификатовИИхСтруктур Цикл
				Если Элемент.Значение.ПарольПолучен = Истина Или ПрограммаБанка = СбербанкОнлайн Или ПрограммаБанка = SberAPI Тогда
...
Процедура ПровестиТестНастройки(Обработчик, НастройкаОбмена, ВыводитьОкноОжидания=Ложь) Экспорт
	...
	ИначеЕсли ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ПровестиТестНастройки(Обработчик, НастройкаОбмена, ПараметрыОбмена, ВыводитьОкноОжидания);
...
// Произвольный - значение параметра.
//
Функция ЗначениеИзКеша(НазваниеПараметра) Экспорт
...
//
Процедура ПолучитьДанныеАутентификацииЗавершение(Результат, Контекст)
	
	Если Результат <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(Контекст.ОповещениеПолученияДанныхАутентификации, Результат);
	Иначе
		
		СрокХраненияПароляИстек = Неопределено;
		Контекст.Свойство("СрокХраненияПароляИстек", СрокХраненияПароляИстек);
		ИзменилисьПараметрыОкружения = Неопределено;
		Контекст.Свойство("ИзменилисьПараметрыОкружения", ИзменилисьПараметрыОкружения);
		
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("ВидОперации", НСтр("ru = 'Аутентификация на сервере банка'"));
		ПараметрыФормы.Вставить("НастройкаОбмена", Контекст.НастройкаОбмена);
		
		// Если была попытка получения зашифрованного пароля,
		//  но он истек или изменились параметры окружения, то проверим на это
		Если СрокХраненияПароляИстек = Истина Тогда
			ПараметрыФормы.Вставить("СрокХраненияПароляИстек", Истина);
		КонецЕсли;
		Если ИзменилисьПараметрыОкружения = Истина Тогда
			ПараметрыФормы.Вставить("ИзменилисьПараметрыОкружения", Истина);
		КонецЕсли;
		
		// Очистим пароль в безопасном хранилище, если он там есть
		ОбменСБанкамиСлужебныйВызовСервера.УдалитьДанныеАвторизацииОбменСБанками(Контекст.НастройкаОбмена, "ДляПользователя");
		
		ОткрытьФорму("Обработка.ОбменСБанками.Форма.ЗапросПароляКСертификату", ПараметрыФормы,
			, , , , Контекст.ОповещениеПолученияДанныхАутентификации, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	КонецЕсли;
...
Процедура ПроверитьВерсиюВКСбербанк(Оповещение, ИмяВнешнегоМодуля)
	...
	Если ЭтоWindows7() = Истина Тогда
		ВыполнитьОбработкуОповещения(Оповещение, Истина);
	Иначе
		ИнформацияОВК = ОбменСБанкамиСлужебныйВызовСервера.ИнформацияОКомпоненте(ИмяВнешнегоМодуля);
...
Процедура ОпределитьНеобходимостьПовторнойАутентификацииНаТокенеПослеПолученияСпискаБизнесСистемСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Результат.Успех Тогда
		РеквизитыНастройкиОбмена = Новый Структура("ИмяВнешнегоМодуля,ПрограммаБанка");
		ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(
...
Процедура НайтиБизнесСистемуПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НомерБизнесСистемы = Неопределено Тогда
		...
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещенияПослеОпределенияНомераБизнесСистемы,
			Неопределено);
		Возврат;
	КонецЕсли;
...
Функция ДоступнаРаботаНаМобильномКлиенте(ПрограммаБанка) Экспорт
	...
	Если ЭтоМобильныйКлиент И
		(ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI")
		Или ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн")) Тогда
...
// Читает пароль из безопасного хранилища, и помещает его в кэш сеанса.
//
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - ссылка на объект, к которому относится пароль.
//  ОповещениеВозврата - ОписаниеОповещения - описание вызова процедуры, которая будет вызвана при окончании. 
//	
Процедура ПрочитатьПарольИзБезопасногоХранилища(НастройкаОбмена, ОповещениеВозврата)
	
	ПараметрыОперации = Новый Структура;
	ПараметрыОперации.Вставить("ИдентификаторМетода", "Прочитать");
	ПараметрыОперации.Вставить("НастройкаОбмена", НастройкаОбмена);
	ПараметрыОперации.Вставить("ОповещениеПолученияДанныхАутентификации", ОповещениеВозврата);
	
	// Проверим ТребуетсяSMSАвторизация
	ТребуетсяSMSАвторизация = ОбменСБанкамиСлужебныйВызовСервера.ТребуетсяSMSАвторизация(НастройкаОбмена);
	
	Если ТребуетсяSMSАвторизация = ПредопределенноеЗначение("Перечисление.SMSАвторизацияОбменСБанками.Требуется") Тогда
		
		НачатьОперациюСПаролемВБезопасномХранилище(ПараметрыОперации);
		
	Иначе
		
		ПолучитьДанныеАутентификацииЗавершение(Неопределено, ПараметрыОперации);
...
Процедура НачатьОперациюСПаролемВБезопасномХранилище(ПараметрыОперации)
	
	#Если МобильныйКлиент Тогда
		
		ПродолжитьОперациюСПаролемВБезопасномХранилище(Истина, ПараметрыОперации);
		
	#Иначе
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ОповещениеПослеПолученияФрода", 
			Новый ОписаниеОповещения("ПродолжитьОперациюСПаролемВБезопасномХранилище", 
		...
		НачатьПодключениеРасширенияПолученияИнформацииОКомпьютере(ОписаниеОповещения);
		
	#КонецЕсли
		
КонецПроцедуры

Процедура ПродолжитьОперациюСПаролемВБезопасномХранилище(Успешно, Контекст) Экспорт
	
	КлючевыеПараметры = НовыйКлючевыеПараметрыКомпьютера();
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ЗаполнитьЗначенияСвойств(КлючевыеПараметры, СистемнаяИнформация);
	
	#Если НЕ ВебКлиент Тогда
		КлючевыеПараметры.ИмяКомпьютера = ИмяКомпьютера();
	#КонецЕсли
	...
	Если Успешно Тогда
		...
		Если ТипЗнч(ПараметрыПодсистемыОбменСБанками) = Тип("Соответствие") Тогда
			...
			Фрод = ПараметрыПодсистемыОбменСБанками.Получить("Фрод");
			Если Фрод <> Неопределено И Фрод.Свойство("MAC") Тогда
				
				Для Сч = 0 По Фрод.MAC.ВГраница() Цикл
					
					КлючевыеПараметры.MACАдреса = КлючевыеПараметры.MACАдреса + Фрод.MAC[Сч];
					
				КонецЦикла;
				
			КонецЕсли;
			
			Если Фрод <> Неопределено И Фрод.Свойство("ИменаАдаптеров") Тогда
				
				Для Сч = 0 По Фрод.ИменаАдаптеров.ВГраница() Цикл
					
					КлючевыеПараметры.ИменаАдаптеров = КлючевыеПараметры.ИменаАдаптеров + Фрод.ИменаАдаптеров[Сч];
	...
	ПараметрыОперации = Новый Структура;
	ПараметрыОперации.Вставить("КлючевыеПараметры", КлючевыеПараметры);
	ПараметрыОперации.Вставить("НастройкаОбмена", Контекст.НастройкаОбмена);
	ПараметрыОперации.Вставить("ИдентификаторМетода", Контекст.ИдентификаторМетода);
	
	Если Контекст.ИдентификаторМетода = "Сохранить" Тогда
		ПараметрыОперации.Вставить("ДанныеАвторизации", Контекст.ДанныеАвторизации);
	КонецЕсли;
	
	ОбменСБанкамиСлужебныйВызовСервера.НачатьОперациюСПаролемВБезопасномХранилище(ПараметрыОперации);
	
	Если Контекст.ИдентификаторМетода = "Прочитать" Тогда
		
		ПарольПолучен = Ложь;
		
		// Т.к. ДанныеАвторизации могут быть пустые, то параметры вставляем только в случае наличия ДанныеАвторизации
		Если ПараметрыОперации.ДанныеАвторизации <> Неопределено Тогда
			
			СохранитьПарольНаВремяСеанса(Контекст.НастройкаОбмена, ПараметрыОперации.ДанныеАвторизации);
			ПараметрыОперации.ДанныеАвторизации.Вставить("ЗапомнитьПарольНаДлительныйСрок", Истина);
			ПарольПолучен = Истина;
			
		КонецЕсли;
		
		// Параметр добавляется только в случае реального окончания срока хранения пароля, в других случаях параметр не нужен
		Если ПараметрыОперации.Свойство("СрокХраненияПароляИстек") Тогда
			
			Контекст.Вставить("СрокХраненияПароляИстек", ПараметрыОперации.СрокХраненияПароляИстек);
			ПарольПолучен = Ложь;
			
		КонецЕсли;
		
		// Параметр добавляется только в случае изменения окружения, в других случаях параметр не нужен
		Если ПараметрыОперации.Свойство("ИзменилисьПараметрыОкружения") Тогда
			
			Контекст.Вставить("ИзменилисьПараметрыОкружения", ПараметрыОперации.ИзменилисьПараметрыОкружения);
			ПарольПолучен = Ложь;
		...
		Если ПарольПолучен Тогда
			...
			// Передадим признак, что пароль получен из долгосрочного хранилища
			ПараметрыОперации.ДанныеАвторизации.Вставить("ПарольПолученИзХранилища", Истина);
			
		КонецЕсли;
		
		ПолучитьДанныеАутентификацииЗавершение(ПараметрыОперации.ДанныеАвторизации, Контекст);
		
	КонецЕсли;
	
КонецПроцедуры

Функция НовыйКлючевыеПараметрыКомпьютера()

	Результат = Новый Структура;
	
	Результат.Вставить("MACАдреса", "");
	Результат.Вставить("ИменаАдаптеров", "");
	Результат.Вставить("ИмяКомпьютера", "");

	Результат.Вставить("ИдентификаторКлиента", "");	
	Результат.Вставить("ОперативнаяПамять", 0);
...
Процедура НачатьОтправкуСообщенийВБанкСАвторизациейЛогинПароль(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОбменСБанкамиСлужебныйКлиентСервер.ДополнитьПараметрыДаннымиСессииОбменаСБанком(Параметры, Результат);
...
Процедура ОтправкаСообщенийОбмена(Параметры) Экспорт
	...
	ИначеЕсли СтруктураКОтправке.ЧерезТокенСбербанка.Количество() Тогда
		...
		ОтправитьДокументыВСбербанк(Параметры);
	ИначеЕсли СтруктураКОтправке.ЧерезТокенSberAPI.Количество() > 0 Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ОтправитьПлатежныеДокументы(СтруктураКОтправке.ЧерезТокенSberAPI, Параметры);
	ИначеЕсли СтруктураКОтправке.ЧерезЛогинПарольSberAPI.Количество() > 0 Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ОтправитьПлатежныеДокументы(СтруктураКОтправке.ЧерезЛогинПарольSberAPI, Параметры);
	...
	ИначеЕсли СтруктураКОтправке.ЧерезСервисFintech.Количество() > 0 Тогда
...
Процедура ЗакрытиеПовторнойАутентификации(ДанныеАутентификации, ДополнительныеПараметры) Экспорт
	
	Если ДанныеАутентификации <> Неопределено Тогда
		
		ПолучитьМаркерБанкаПоЛогинуИПаролю(
			ДополнительныеПараметры.Обработчик,
			ДополнительныеПараметры.АдресСервера,
			ДополнительныеПараметры.ИдентификаторОрганизации,
			ДанныеАутентификации,
			ДополнительныеПараметры.ВерсияAPI,
			ДополнительныеПараметры.НастройкаОбмена,
			ДополнительныеПараметры.ПробнаяОперация);
		
	Иначе
		
		УдалитьПарольИзСеанса(ДополнительныеПараметры.НастройкаОбмена);
...
//    ТребуетсяSMSАвторизация - ПеречислениеСсылка.SMSАвторизацияОбменСБанками - значение параметра ТребуетсяSMSАвторизация.
//
Процедура ПолучитьМаркерБанкаПоЛогинуИПаролю(
	...
	Если Результат.НеверныеДанныеАутентификации Тогда
		...
		ПомощникСозданияНастройкиОбмена = Ложь;
		ДанныеАутентификации.Свойство("ПомощникСозданияНастройкиОбмена", ПомощникСозданияНастройкиОбмена);
		Если ПомощникСозданияНастройкиОбмена = Истина И ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
		...
		Иначе
			...
			ПараметрыФормы.Вставить("ВидОперации", ВидОперации);
			ПараметрыФормы.Вставить("НастройкаОбмена", НастройкаОбмена);
			Если ДанныеАутентификации.Свойство("ПарольПолученИзХранилища", ПарольПолученИзХранилища) Тогда
				
				СообщениеПользователю = НСтр("ru = 'Ранее сохраненный пароль более не действует.'")
					+ Символы.ПС
					+ НСтр("ru = 'Введите новый пароль.'");
			...
			ДополнительныеПараметры.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
			ДополнительныеПараметры.Вставить("ДанныеАутентификации", ДанныеАутентификации);
			ДополнительныеПараметры.Вставить("ВерсияAPI", ВерсияAPI);
			ДополнительныеПараметры.Вставить("НастройкаОбмена", НастройкаОбмена);
			ДополнительныеПараметры.Вставить("ПробнаяОперация", ПробнаяОперация);

			ЗакрытиеПовторнойАутентификации = Новый ОписаниеОповещения("ЗакрытиеПовторнойАутентификации",
				ЭтотОбъект, ДополнительныеПараметры);
			ОткрытьФорму("Обработка.ОбменСБанками.Форма.ЗапросПароляКСертификату",
				ПараметрыФормы, , , , , ЗакрытиеПовторнойАутентификации, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	...
	Если Результат.ТребуетсяSMSАвторизация Тогда
		...
		ТребуетсяSMSАвторизация = ПредопределенноеЗначение("Перечисление.SMSАвторизацияОбменСБанками.Требуется");
		ДанныеАутентификации.Вставить("ТребуетсяSMSАвторизация", ТребуетсяSMSАвторизация);
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Результат.ТекстОшибки) Тогда
			...
			Если НЕ ЗначениеЗаполнено(ТипОшибкиИнтернетСоединения) Тогда
				...
				Если ПарольПолученИзХранилища <> Истина Тогда
		...
		Иначе
			...
			// Если мы попали сюда, значит подключение прошло успешно и SMSАвторизация не требуется.
			// Поэтому нужно очистить пароль в безопасном хранилище
			Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
				
				ОбменСБанкамиСлужебныйВызовСервера.УдалитьДанныеАвторизацииОбменСБанками(
					НастройкаОбмена, 
					"ДляПользователя");
				ОбменСБанкамиСлужебныйВызовСервера.УстановитьТребуетсяSMSАвторизация(
					НастройкаОбмена, 
					ПредопределенноеЗначение("Перечисление.SMSАвторизацияОбменСБанками.НеТребуется"));
...
Процедура ЗавершитьОтправку(Результат, Параметры) Экспорт
	...
	ИначеЕсли ОбменСБанкамиКлиентСервер.ЕстьДействие(Параметры.Действия, "Подписать") Тогда
		...
		Если Параметры.Свойство("НастройкиОбменаСДаннымиСессии", НастройкиОбменаСДаннымиСессии) Тогда
			ОжиданиеЗавершениеОбработкиОтправленныхДокументов(НастройкиОбменаСДаннымиСессии);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОкноОповещенияОбменСБанками(ДополнительныеПараметры) Экспорт
...
Процедура НачатьПодписаниеБанковскихЭД(Структура)
	...
	Если Структура.Свойство("СоотвСертификатовИИхСтруктур", СоотвСертификатовИИхСтруктур)
		...
		ПолучитьПарольКСертификату(
			...
			Соответствие,
			ВидОперации,
			МассивСообщенийОбменаКПодписи,
			Структура.ПрограммаБанка,
			Структура.НастройкаОбмена);
	...
	Если ТипЗнч(Результат) = Тип("Структура") И Результат.Свойство("ВыбранныйСертификат", ВыбранныйСертификат)
		...
		Если Параметры.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.СбербанкОнлайн") Тогда
			...
			Для Каждого ЭлементСоответствия Из ДанныеСпецОбработки Цикл
				...
				Возврат;
			КонецЦикла;
		ИначеЕсли Параметры.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
			Параметры.Вставить("МассивОбработанныхНастроекОбмена", Новый Массив);
			Параметры.Вставить("РезультатВыбораСертификатаБанка", Результат);
			ДанныеСпецОбработки = Параметры.ДанныеДляСпецОбработки;
			Для Каждого ЭлементСоответствия Из ДанныеСпецОбработки Цикл
				ТекущиеДанные = ЭлементСоответствия.Значение;
				Параметры.ДанныеДляСпецОбработки.Удалить(ЭлементСоответствия.Ключ);
				ОбменСБанкамиSberAPIСлужебныйКлиент.НачатьПодписаниеЭДСбербанка(ТекущиеДанные, Параметры);
				Возврат;
			КонецЦикла;
...
Процедура ПоказатьОшибкуСетевогоДоступа(КоллекцияОшибок, ОбработкаОповещения, ПараметрОповещенияЗакрытия = Неопределено) Экспорт
	...
	Иначе // выполнено
		...
		Если ЭтоАсинхронныйОбмен Тогда
			...
			Если ДополнительныеПараметрыПодписаниеПоSMS.ВозможноПодписаниеПоSMS Тогда
				...
				ОбменСБанкамиСлужебныйКлиентСервер.ДополнитьПараметрыДаннымиСессииОбменаСБанком(
					ДополнительныеПараметрыОповещение, РезультатОперации);
...
Процедура ОбработатьЭД(
	...
	Если ЗначениеЗаполнено(СообщениеОбмена) Тогда
		РеквизитыДокумента = ОбменСБанкамиСлужебныйВызовСервера.РеквизитыЭлектронногоДокумента(СообщениеОбмена);
		Если ЗначениеЗаполнено(РеквизитыДокумента)
			И НЕ ДоступнаРаботаНаМобильномКлиенте(РеквизитыДокумента.ПрограммаБанка) Тогда
			
			Банк = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыДокумента, "Банк");
			ТекстСообщения = ОбменСБанкамиСлужебныйКлиентСервер.ТекстПредупрежденияМобильныйКлиентНедоступнаРабота(
				РеквизитыДокумента.ПрограммаБанка, Банк);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
			Возврат;
...
Процедура ПродолжитьОбработкуДокументовПослеПереопределения(МассивСсылокНаОбъект, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Фрод) Тогда
		ДополнительныеПараметры.Вставить("МассивСсылокНаОбъект", МассивСсылокНаОбъект);
...
Процедура ПослеОбработкиДокументов(РезультатЗадания, Параметры) Экспорт
	...
	Иначе // выполнено
		...
		Если Результат <> Неопределено Тогда
			...
			Если Результат.ОтправленныеДокументы.Количество() > 0 Тогда
				Оповестить("ОтправленоDirectBank", Результат.ОтправленныеДокументы);
			КонецЕсли;
			...
			Если Результат.СоотвСертификатовИИхСтруктур.Количество() > 0
				И Результат.СертификатыСообщенийОбмена.Количество() > 0 Тогда
...
Процедура ПолучитьФрод(Оповещение, Обязательно = Ложь)
	...
	#Если МобильныйКлиент Тогда
		...
		ПослеПопыткиПодключенияРасширенияПолученияИнформацииОКомпьютере(Истина, ДополнительныеПараметры);
...
Процедура ПослеПодключенияРасширенияПолученияИнформацииОКомпьютере(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		#Если МобильныйКлиент Тогда
			...
			ИнформацияОСетевыхАдаптерах = Новый Массив;
			ПослеПолученияИнформацииОСетевыхАдаптерах(ИнформацияОСетевыхАдаптерах, ДополнительныеПараметры);
...
Процедура ПослеПолученияИнформацииОСетевыхАдаптерах(ИнформацияОСетевыхАдаптерах, ДополнительныеПараметры) Экспорт
	...
	МассивИменАдаптеров = Новый Массив;
	Для Каждого Элемент Из ИнформацияОСетевыхАдаптерах Цикл
		...
		МассивИменАдаптеров.Добавить(Элемент.ИмяАдаптера);
	КонецЦикла;
	
	Фрод = Новый Структура();
	Фрод.Вставить("MAC", МассивMAC);
	Фрод.Вставить("IP", МассивIP);
	Фрод.Вставить("ИменаАдаптеров", МассивИменАдаптеров);
	
	ДобавитьЗначениеВКеш("Фрод", Фрод);
...
Процедура ПолучитьПарольАутентификацииПослеОпределенияСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПолучитьПарольКСертификату(
			Оповещение,
			ДополнительныеПараметры.ДоступныеСертификаты,
			ВидОперации,
			,
			,
			ДополнительныеПараметры.НастройкаОбмена);
...
Процедура ПослеПолученияДанныхСертификатовСКлючаВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПараметрыФормы.Вставить("НастройкаОбмена", ДополнительныеПараметры.НастройкаОбмена);
		ПараметрыФормы.Вставить("МассивСертификатов", МассивСертификатов);
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("АутентификацияПослеПолученияПароляЧерезВК",
			ЭтотОбъект, ДополнительныеПараметры);
		ОткрытьФорму("Обработка.ОбменСБанками.Форма.ЗапросПароляКСертификату",
...
Процедура ПослеОтправкиПакетаЧерезВК(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	Исключение
		...
		ТекстОшибкиВЖурнале = НСтр("ru = 'При чтении ответа банка возникла ошибка:
										|%1'");
		ТекстОшибкиВЖурнале = СтрШаблон(ТекстОшибкиВЖурнале, ТекстИсключения);
...
Процедура ВывестиСообщения(СообщенияПользователю, ИдентификаторНазначения) Экспорт
...
// Создает структуру с данными авторизации. Если передан Логин или Пароль, то сразу заполняет их
//
// Параметры:
//  Логин - Строка - логин пользователя, может быть пустым
//  Пароль - Строка - пароль пользователя, может быть пустым
//
// Возвращаемое значение:
//  Структура - ДанныеАвторизации. Содержит поля:
//   * Логин - Строка - логин пользователя
//   * Пароль - Строка - пароль пользователя
Функция ДанныеАвторизации(Логин = Неопределено, Пароль = Неопределено) Экспорт
...
Процедура УстановитьФормуВладельцаЧерезАктивноеОкно(ПараметрыОжидания) Экспорт
	
	Окно = АктивноеОкно();
	Если ТипЗнч(Окно) = Тип("ОкноКлиентскогоПриложения")
		И Окно.Содержимое.Количество() = 1
		И Окно.Содержимое[0].Открыта() Тогда
		ПараметрыОжидания.ФормаВладелец = Окно.Содержимое[0]; // требуется в БСП для вывода сообщений из фонового задания
	КонецЕсли;
...
// Запускает фоновое задание ожидания завершения обработки отправленных в банк документов.
//
// Параметры:
//  НастройкиОбменаСДаннымиСессии - Соответствие - содержит настройки обмена, по которым есть открытые сессии обмена с банком, где:
//   * Ключ - СправочникСсылка.НастройкиОбменСБанками - ссылка на настройку обмена с банком.
//   * Значение - Структура: см. НовыеСвойстваОткрытойСессииОбменаСБанком.
//
Процедура ОжиданиеЗавершениеОбработкиОтправленныхДокументов(НастройкиОбменаСДаннымиСессии) 
	
	ПараметрыПриложенияНастройкиСДаннымиСессии = ЗначениеИзКеша("НастройкиОбменаСДаннымиСессии");
	Если ПараметрыПриложенияНастройкиСДаннымиСессии = Неопределено Тогда
		ПараметрыПриложенияНастройкиСДаннымиСессии = Новый Соответствие;
		ДобавитьЗначениеВКеш("НастройкиОбменаСДаннымиСессии", ПараметрыПриложенияНастройкиСДаннымиСессии);
	КонецЕсли;
	
	ЭтоФайловаяБаза = ОбщегоНазначенияКлиент.ИнформационнаяБазаФайловая();
	
	Для каждого КлючИЗначение Из НастройкиОбменаСДаннымиСессии Цикл
		
		НастройкаОбмена = КлючИЗначение.Ключ;
		ТекущиеДанныеСессии = КлючИЗначение.Значение;
		ИдентификаторСессии = ТекущиеДанныеСессии.ИдентификаторСессии;
		УниверсальнаяДатаОбновленияСессии = ТекущиеДанныеСессии.УниверсальнаяДатаОбновленияСессии;
		ВремяЖизниСессии = ТекущиеДанныеСессии.ВремяЖизниСессии;
		ТолькоОбменДепозитами = ТекущиеДанныеСессии.ТолькоОбменДепозитами;
		...
		Если ВремяЖизниСессии = 0 
			И Не ТекущиеДанныеСессии.ТолькоОбменДепозитами Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ПараметрыПриложенияДанныеСессии) Тогда
			ПараметрыПриложенияДанныеСессии = НовыеДанныеСессииДляПараметровПриложения();
			ПараметрыПриложенияНастройкиСДаннымиСессии.Вставить(НастройкаОбмена, ПараметрыПриложенияДанныеСессии);
		Иначе
			ИдентификаторЗаданияОжидания = ПараметрыПриложенияДанныеСессии.ИдентификаторЗаданияОжидания;
		КонецЕсли;
		
		РезультатЗапускаЗаданияПроверкиСтатуса = Неопределено;
		
		// В файловой базе ожидание завершения обработки отправленных документов
		// используется только при обмене депозитами.
		//
		Если Не ЭтоФайловаяБаза Тогда
			...
			Если Не ЗначениеЗаполнено(РезультатЗапускаЗаданияПроверкиСтатуса) Тогда
				...
				ПараметрыПриложенияНастройкиСДаннымиСессии.Удалить(НастройкаОбмена);
				Продолжить;
		...
		ПараметрыПриложенияДанныеСессии.ИдентификаторСессии = ИдентификаторСессии;
		ПараметрыПриложенияДанныеСессии.УниверсальнаяДатаОбновленияСессии = УниверсальнаяДатаОбновленияСессии;
		ПараметрыПриложенияДанныеСессии.ВремяЖизниСессии = ВремяЖизниСессии;
		ПараметрыПриложенияДанныеСессии.ТолькоОбменДепозитами = ТолькоОбменДепозитами;
		
		Если ЗначениеЗаполнено(РезультатЗапускаЗаданияПроверкиСтатуса) Тогда
			
			ПараметрыПриложенияДанныеСессии.ИдентификаторЗаданияОжидания =
				РезультатЗапускаЗаданияПроверкиСтатуса.ИдентификаторЗадания;
			ОповещениеОЗавершении = Новый ОписаниеОповещения("ПроверкаЗавершенияОбработкиОтправленныхДокументов",
			ЭтотОбъект);
...
//
Процедура ПроверкаЗавершенияОбработкиОтправленныхДокументов(Результат, ДополнительныеПараметры) Экспорт
	
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		Оповестить("ЗавершеноЗаданиеОбновленияСессииОбменСБанками");
		Возврат;
	КонецЕсли;
	...
	Если Результат.КоличествоПолученныхПакетов > 0 Тогда
		Оповестить("ОбновитьСостояниеОбменСБанками");
	КонецЕсли;
...
// Функция создает новую структуру свойств данных открытой сессии обмена с банками
// для хранения в ПараметрыПриложения
// 
// Возвращаемое значение:
//  Структура:
...
//   * ИдентификаторЗаданияОжидания - Строка - идентификатор задания ожидания завершения обработки документов.
...
//
Функция НовыеДанныеСессииДляПараметровПриложения()
	
	Результат = Новый Структура();
	Результат.Вставить("ИдентификаторСессии", "");
	Результат.Вставить("УниверсальнаяДатаОбновленияСессии", 0);
	Результат.Вставить("ИдентификаторЗаданияОжидания", "");
...
// Заполняет необходимые параметры для запуска процесса подписания документов кодом из SMS
//
// Параметры:
// 	НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
...
Процедура ОтправитьЗапросКодаИзSMSВБанкПослеПолученияМаркера(Маркер, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Маркер) Тогда
		...
		ВывестиОшибкуОкноОповещенияОбОперациях(ДополнительныеПараметры);
		Возврат;
...
// Запускает каскад процедур для подписания платежных документов с помощью кода из SMS
//
// Параметры:
...
Процедура ОбработатьРезультатОтправкиЗапросовСтатусовЭД(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияПолученияИзвещенийОСостоянийЭД(
			ДополнительныеПараметры, УникальныйИдентификаторЗадания);  
...
Процедура ВывестиОшибкуОкноОповещенияОбОперациях(ДополнительныеПараметры)
...
Процедура ПослеПолученияИзвещенийОСостоянияхЭД(Результат, ДополнительныеПараметры) Экспорт
	...
	// Дополним массив документами, которые ожидают изменения статуса, но не вернулись из фонового задания.
