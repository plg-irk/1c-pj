...
// 
// +РАРУС
Процедура ВключитьПользователяВГруппуДоступа(Пользователь, ПоставляемыйПрофиль, ПерсональнаяГруппаДоступа = Ложь) Экспорт
	
	ОбработатьСвязьПользователяСГруппойДоступа(Пользователь, ПоставляемыйПрофиль, Истина, ПерсональнаяГруппаДоступа); // -РАРУС
...
Процедура УстановкаПараметровСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если Не Константы.ОграничиватьДоступНаУровнеЗаписей.Получить() Тогда
		...
		ПараметрыСеанса.ОграничениеДоступаНаУровнеЗаписейВключено  = Ложь;
...
Процедура УстановитьПравоОткрытияВнешнихОтчетовИОбработок(ОткрытиеРазрешено) Экспорт
	...
	Если ОткрытиеРазрешено Тогда
		...
		// +РАРУС
		ПользователиАдмины = Новый Массив;
		Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
			Если ПользовательИБ.Роли.Содержит(Метаданные.Роли.АдминистраторСистемы) Тогда
				...
				Если Пользователь = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				ПользователиАдмины.Добавить(Пользователь);
			КонецЕсли;
		КонецЦикла;
		...
		|ИЗ
		|	Справочник.ГруппыДоступа.Пользователи КАК ГруппыДоступаПользователи
		|		ЛЕВОЕ СОЕДИНЕНИЕ ИсключаемыеГруппы КАК ИсключаемыеГруппы
		|		ПО ГруппыДоступаПользователи.Ссылка = ИсключаемыеГруппы.Ссылка
		|ГДЕ
		|	ГруппыДоступаПользователи.Пользователь В(&Пользователи)
		|	И ИсключаемыеГруппы.Ссылка ЕСТЬ NULL
		|	И НЕ ГруппыДоступаПользователи.Ссылка.ПометкаУдаления";
		Запрос = Новый Запрос(ТекстЗапроса);
		Запрос.УстановитьПараметр("Пользователи", ПользователиАдмины);
		Запрос.УстановитьПараметр("Профиль", СвойстваПрофиля.Ссылка);
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ГруппаДоступаОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ГруппаДоступаОбъект.ПрофилиДоступа.Добавить().Профиль = СвойстваПрофиля.Ссылка;
			ГруппаДоступаОбъект.Записать();
		КонецЦикла;
		// -РАРУС
	...
	Иначе
		...
		|ИЗ
		// +РАРУС
		|	Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступа
		// -РАРУС
		|ГДЕ
		...
		Для Каждого ГруппаДоступа Из МассивГрупп Цикл
			...
			// +РАРУС
			ГруппаДоступаОбъект.ПрофилиДоступа.Удалить(ГруппаДоступаОбъект.ПрофилиДоступа.Найти(СвойстваПрофиля.Ссылка, "Профиль"));
			// -РАРУС
			// При обновлении ИБ важно, чтобы изменения уехали в подчиненный узел.
			ГруппаДоступаОбъект.Записать();
		КонецЦикла;
	...
	Для каждого ОписаниеПользователя Из ИдентификаторыПользователейИБ Цикл
		...
		Если ПользовательИБ <> Неопределено И ЗначениеЗаполнено(ПользовательИБ.Имя) Тогда
			...
			Если Администраторы[ТекущийПользователь] = Неопределено Тогда
				ПользовательАдминистратор = Ложь;
			...
			Иначе // Администратор.
				...
				ПользовательАдминистратор = Истина;
				
				Для каждого Роль Из ПользовательИБ.Роли Цикл
					ИмяРоли = Роль.Имя;
					СтарыеРоли.Вставить(ИмяРоли, Истина);
					Если РолиТекущегоАдминистратора[ИмяРоли] = Истина Тогда
						Продолжить;
					КонецЕсли;
					Если НовыеРоли.Найти(ИмяРоли, "Роль") = Неопределено Тогда
						РолиДляУдаления.Вставить(ИмяРоли, Истина);
			...
			Для каждого Строка Из НовыеРоли Цикл
				...
				Если СтарыеРоли = Неопределено Тогда
				// ИЛИ Администраторы[ТекущийПользователь] <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				Если НЕ СтарыеРоли[Строка.Роль] = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				Если ПользовательАдминистратор И (НЕ РолиТекущегоАдминистратора[Строка.Роль] = Неопределено) Тогда
					Продолжить;
				КонецЕсли;
...
// Процедура является обработчиком одноименного события, возникающего при обмене
...
// Процедура является обработчиком одноименного события, возникающего при обмене данными
...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.ОграничиватьДоступНаУровнеЗаписей")
	      ИЛИ ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.ПараметрыОграниченияДоступа")
	      ...
	      ИЛИ ТипЗнч(ЭлементДанных) = Тип("ПланВидовХарактеристикОбъект.УдалитьВидыДоступа")
	      ...
	      ИЛИ ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ПраваРолей")
	      ...
	      ИЛИ ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.УдалитьПраваПоЗначениямДоступа") Тогда
...
//   1. Функции ОписаниеОтчета и ОписаниеВарианта формируют описание настроек отчета и варианта
...
//       Подробнее - см. "свойства для изменения" в комментарии
//                   к ВариантыОтчетовПереопределяемый.НастроитьВариантыОтчетов().
//   2. Процедура УстановитьРежимВыводаВПанеляхОтчетов позволяет настроить режим группировки вариантов
//       в панелях отчетов:
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, НастройкиОтчета, Истина/Ложь);
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, Метаданные.Отчеты.<ИмяОтчета>, Истина/Ложь);
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки,
//                                                            Метаданные.Подсистемы.<ИмяПодсистемы>,
//                                                            Истина/Ложь);
...
Процедура ОбновитьГруппыЗначенийДоступа(Знач Объект, Отказ) Экспорт
	...
	// +РАРУС Получим признак обработки события в упрощенном режиме, когда проверка корректности не производится.
	Если Объект.ОбменДанными.Загрузка ИЛИ ОбработкаСобытийОбъектаСервер.РежимУпрощеннойЗагрузкиОбъекта(Объект) Тогда
	// -РАРУС
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнениеДанныхДляОграниченияДоступа(КоличествоДанных = 0, ТолькоКэшРеквизиты = Ложь, ЕстьИзменения = Неопределено) Экспорт
	...
	Если КоличествоДанных < 10000 Тогда
		...
		Для каждого КлючИЗначение Из ТипыЗначенийДоступа Цикл
			// +РАРУС
			Если ЭтоПримитивныйТип(КлючИЗначение.Ключ) Тогда
				ТаблицаТиповЗначений.Добавить().ТипЗначений = ПустоеЗначениеПримитивногоТипа(КлючИЗначение.Ключ);
			Иначе
		...
		Для каждого КлючИЗначение Из ТипыЗначенийДоступаСГруппами Цикл
			// +РАРУС
			Если ЭтоПримитивныйТип(КлючИЗначение.Ключ) Тогда
				ТаблицаТиповЗначенийСГруппами.Добавить().ТипЗначений = ПустоеЗначениеПримитивногоТипа(КлючИЗначение.Ключ);
			Иначе
...
Процедура ПриСозданииНаСервереФормыРедактированияРазрешенныхЗначений(Форма, ЭтоПрофиль = Ложь, ИмяРеквизитаХранилищаТаблиц = "Объект") Экспорт
	...
	Для каждого СвойстваВидаДоступа Из СвойстваВидаДоступа() Цикл
		Для каждого Тип Из СвойстваВидаДоступа.ТипыВыбираемыхЗначений Цикл
			...
			ИначеЕсли // -РАРУС
				Метаданные.Перечисления.Найти(МетаданныеТипа.Имя) = МетаданныеТипа Тогда
...
Процедура ОбновитьОтображениеНеиспользуемыхВидовДоступа(Форма, ПриСозданииНаСервере = Ложь) Экспорт
	...
	// +Рарус
	Если НЕ Элементы.Найти("ПоказыватьНеиспользуемыеВидыДоступа") = Неопределено И Не Элементы.ПоказыватьНеиспользуемыеВидыДоступа.Пометка Тогда // -РАРУС
	...
	Для каждого Строка Из Форма.ВсеВидыДоступа Цикл
		...
		// +Рарус 
		Если НЕ Элементы.Найти("ПоказыватьНеиспользуемыеВидыДоступа") = Неопределено И Не Элементы.ПоказыватьНеиспользуемыеВидыДоступа.Пометка
		   И Не Строка.Используется Тогда // -РАРУС
...
// +РАРУС Определяет, является ли переданный тип примитивным типом: строка, число, дата, булево.
//
// Параметры:
//  ТипОбъекта- Тип
//
...
// +РАРУС Получает Пустое значение примитивного типа по типу
//
// Параметры:
//  ТипОбъекта - Тип - Тип
// 
...
//   0, 1.1.0001, "", Ложь.
//
Функция ПустоеЗначениеПримитивногоТипа(ТипОбъекта)
...
Процедура ПреобразоватьИменаРолейВИдентификаторы() Экспорт
	...
	|ГДЕ
	|	НЕ(Роли.Роль <> ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовМетаданных.ПустаяСсылка) И 
	|	Роли.Роль <> ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовРасширений.ПустаяСсылка)
	|				И Роли.УдалитьРоль = """")";
...
Процедура ЗаменитьУдаленныеРолиВПрофиляхГруппДоступа() Экспорт
	...
	|ИЗ
	|	Справочник.ПрофилиГруппДоступа.Роли КАК РолиПрофилей
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ИдентификаторыОбъектовРасширений КАК ИдентификаторыУдаленныхРолей
	|		ПО РолиПрофилей.Роль = ИдентификаторыУдаленныхРолей.Ссылка
	|		И ИдентификаторыУдаленныхРолей.Имя В (&МассивУдаленныхРолей)
	|ИТОГИ ПО
	|	Профиль";
...
Функция ГруппаДоступаОткрытиеВнешнихОтчетовИОбработок(СвойстваПрофиля)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ГруппыДоступа.Ссылка.ПометкаУдаления";
	// -РАРУС
	Запрос.УстановитьПараметр("Профиль", СвойстваПрофиля.Ссылка);
	Выборка = Запрос.Выполнить().Выбрать();
...
Процедура ВключитьОтключитьПрофильПользователя(Пользователь, Профиль, Включить)
	...
	Иначе
		// +РАРУС
		УсловиеОтбора = "ГруппыДоступа.Ссылка.Пользователь = &Пользователь";
		// -РАРУС
	...
	Попытка
		...
		Пока Истина Цикл
			...
			ИначеЕсли ТекущийПрофиль <> Неопределено Тогда
				...
				// +РАРУС
				ГруппаДоступаОбъект.ПрофилиДоступа.Добавить().Профиль = ТекущийПрофиль;
				// -РАРУС
...
// +РАРУС
Функция ОбработатьСвязьПользователяСГруппойДоступа(Пользователь, ПоставляемыйПрофиль, Включить = Неопределено, ПерсональнаяГруппаДоступа = Ложь) // -РАРУС
	...
	// +РАРУС
	Если УпрощенныйИнтерфейсНастройкиПравДоступа() ИЛИ ПерсональнаяГруппаДоступа Тогда
	// -РАРУС
		...
		Если ГруппаДоступа = Неопределено Тогда
			...
			Иначе
				...
				// +РАРУС
				ГруппаДоступа.Наименование = СвойстваПрофиля.Наименование + " [" + Пользователь.Наименование+ "]";
				ГруппаДоступа.ПрофилиДоступа.Добавить().Профиль = ПоставляемыйПрофиль;
				// -РАРУС
	...
	Иначе
		...
		"ВЫБРАТЬ
		|	ГруппыДоступа.Ссылка КАК Ссылка,
		|	ГруппыДоступа.Ссылка.ОсновнаяГруппаДоступаПоставляемогоПрофиля
		|ИЗ
		|	Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступа
		|ГДЕ
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ГруппыДоступа.Ссылка.ОсновнаяГруппаДоступаПоставляемогоПрофиля УБЫВ";
		// -РАРУС
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ГруппаДоступа = Выборка.Ссылка;
...
Функция ТекущиеСвойстваПользователей(МассивПользователей)
	...
	|			ВЫБОР
	|				КОГДА ГруппыДоступаПользователи.Ссылка.Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|					ИЛИ ГруппыДоступаПользователи.Ссылка.Пользователь = НЕОПРЕДЕЛЕНО
	|					ИЛИ ГруппыДоступаПользователи.Ссылка.Пользователь ЕСТЬ NULL 
	|				ТОГДА ГруппыДоступаПользователи.Пользователь
	|				ИНАЧЕ ГруппыДоступаПользователи.Ссылка.Пользователь
	|			КОНЕЦ КАК Пользователь
	|		ИЗ
	|			Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступаПрофилиДоступа
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ГруппыДоступа.Пользователи КАК ГруппыДоступаПользователи
	|				ПО ГруппыДоступаПрофилиДоступа.Ссылка = ГруппыДоступаПользователи.Ссылка
	|				И (НЕ ГруппыДоступаПрофилиДоступа.Ссылка.ПометкаУдаления)
	|				И (НЕ ГруппыДоступаПрофилиДоступа.Профиль.ПометкаУдаления)) КАК ВложенныйЗапрос
	|		ПО СоставыГруппПользователей.ГруппаПользователей = ВложенныйЗапрос.Пользователь
	// -РАРУС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ПрофилиПользователяСРолью(ТекущийПользователь, Роль)
	...
	|		ПО (ГруппыДоступаПользователи.Ссылка = ГруппыДоступаПрофили.Ссылка)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПрофилиГруппДоступа.Роли КАК Роли
	 ...
	 ИЛИ (  КоличествоДляЧтения < 100000
		...
		Для каждого Строка Из КлючиЗаписей Цикл
			...
			Иначе
				...
				// +РАРУС
				ИначеЕсли НайденныеЗаписи.Количество() > 1 Тогда
					
					НоваяЗапись = НайденныеЗаписи[0];
					Если НЕ НоваяЗапись.Изменение Тогда
						
						// Попробуем найти запись с разрешенным изменением.
						Для каждого ТекЗапись Из НайденныеЗаписи Цикл
							
							Если ТекЗапись.Изменение Тогда
								
								НоваяЗапись = ТекЗапись;
								Прервать;
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
