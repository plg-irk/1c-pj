	...
	Если ТребуетсяПереносНастроекЗаполненияДополнительныхПолей Тогда
		...
		Пока НастройкиЗаполненияДополнительныхПолей.Следующий() Цикл
			...
			Если ДанныеНастройки = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			НовыеДанныеНастройки = РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.НовыеДанныеНастройкиЗаполненияПолей();
			ЗаполнитьЗначенияСвойств(НовыеДанныеНастройки, ПолучаемаяНастройка, "Отправитель, Получатель, Договор");
			НовыеДанныеНастройки.ВидЭлектронногоДокумента = ПолучаемаяНастройка.ВидДокумента;
			НовыеДанныеНастройки.Формат = ПоддерживаемыеФорматы.УПД_5_03.ИнформацияПродавца;
			НовыеДанныеНастройки.Настройка = ДанныеНастройки;
			РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.ЗаписатьНастройкуЗаполненияПолей(
				НовыеДанныеНастройки);
		КонецЦикла;
	КонецЕсли;
