// @strict-types

#Область СлужебныйПрограммныйИнтерфейс

#Область ПроверкаПодписи

// См. МашиночитаемыеДоверенности.ЗаписатьРезультатПроверкиПолномочий
Функция ЗаписатьРезультатПроверкиПолномочий(Знач ДанныеПроверки) Экспорт;
	Возврат МашиночитаемыеДоверенности.ЗаписатьРезультатПроверкиПолномочий(ДанныеПроверки);
КонецФункции

// См. МашиночитаемыеДоверенности.ДобавитьВОчередьЗапросовВРеестрМЧД
Процедура ДобавитьВОчередьЗапросовВРеестрМЧД(Доверенность, ВидЗапроса) Экспорт
	МашиночитаемыеДоверенности.ДобавитьВОчередьЗапросовВРеестрМЧД(Доверенность, ВидЗапроса);
КонецПроцедуры

// См. МашиночитаемыеДоверенности.УдалитьИзОчередиНаРегистрацию
Процедура УдалитьИзОчередиНаРегистрацию(Доверенность) Экспорт;
	МашиночитаемыеДоверенности.УдалитьИзОчередиНаРегистрацию(Доверенность);
КонецПроцедуры

// См. МашиночитаемыеДоверенности.СохранитьНастройкиРучнойПроверкиПодписиПоМЧД
Процедура СохранитьНастройкиРучнойПроверкиПодписиПоМЧД(Настройки, Знач ЭлектронныйДокумент) Экспорт;
	МашиночитаемыеДоверенности.СохранитьНастройкиРучнойПроверкиПодписиПоМЧД(Настройки, ЭлектронныйДокумент);
КонецПроцедуры	

// См. МашиночитаемыеДоверенности.ЗаписатьРезультатРучнойПроверкиПодписи
Процедура ЗаписатьРезультатРучнойПроверкиПодписи(ДанныеПодписи) Экспорт
	МашиночитаемыеДоверенности.ЗаписатьРезультатРучнойПроверкиПодписи(ДанныеПодписи);
КонецПроцедуры

// Параметры:
//  Доверенности - Массив Из ОпределяемыйТип.МашиночитаемаяДоверенность
//
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение:
//  * Ключ - ОпределяемыйТип.МашиночитаемаяДоверенность
//  * Значение - Булево
Функция ТребуетсяПроверкаДоверенностейНаКлиенте(Доверенности) Экспорт
	ТребуетсяПроверкаДоверенностейНаКлиенте = Новый Соответствие;
	Для Каждого Доверенность Из Доверенности Цикл
		ТребуетсяПроверкаДоверенностейНаКлиенте.Вставить(Доверенность, Не МашиночитаемыеДоверенности.ПроверенаВручную(Доверенность));
	КонецЦикла;
	Возврат ТребуетсяПроверкаДоверенностейНаКлиенте;
КонецФункции

#КонецОбласти

// Выгрузить данные доверенности в строку XML.
// 
// Параметры:
//  СправочникСсылка - СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  				 - СправочникСсылка.МЧД003
// 
// Возвращаемое значение:
//  См. МашиночитаемыеДоверенности.ВыгрузитьXML
Функция ВыгрузитьXML(Знач СправочникСсылка) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ВыгрузитьXML(СправочникСсылка);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ЗагрузитьМЧДИзФайла
Функция ЗагрузитьМЧД(Знач ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, Знач ОбновлятьСуществующий = Ложь,
	Знач ДополнительныеСведения = Неопределено) Экспорт

	Возврат МашиночитаемыеДоверенности.ЗагрузитьМЧДИзФайла(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте,
		ОбновлятьСуществующий, ДополнительныеСведения);

КонецФункции

// См. МашиночитаемыеДоверенности.ЗагрузитьРодительскиеДоверенности
Процедура ЗагрузитьРодительскиеДоверенности(МЧД) Экспорт

	МашиночитаемыеДоверенности.ЗагрузитьРодительскиеДоверенности(МЧД);

КонецПроцедуры

// См. МашиночитаемыеДоверенности.ВыгрузитьЗаявлениеНаОтменуМЧД
Функция ВыгрузитьЗаявлениеНаОтменуМЧД(Знач НомерДоверенности, Знач ПричинаОтмены) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ВыгрузитьЗаявлениеНаОтменуМЧД(НомерДоверенности, ПричинаОтмены);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ПолучитьНомерМЧД
Функция ПолучитьНомерМЧД(Знач ТокенДоступа = "") Экспорт
	
	Возврат МашиночитаемыеДоверенности.ПолучитьНомерМЧД(ТокенДоступа);
	
КонецФункции

// Регистрирует МЧД на сервере МЧД.
// 
// Параметры:
//  СсылкаНаДоверенность - СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций
//  					 - СправочникСсылка.МЧД003
//  ДанныеВыгрузки - ДвоичныеДанные - Данные выгрузки
//  ДанныеПодписи - ДвоичныеДанные - Данные подписи
//  ТокенДоступа - Строка - Токен доступа
//  НомерДоверенности - Строка - Номер доверенности
// 
// Возвращаемое значение:
//  См. МашиночитаемыеДоверенности.ЗарегистрироватьМЧД
Функция ЗарегистрироватьМЧД(Знач СсылкаНаДоверенность, ДанныеВыгрузки, Знач ДанныеПодписи, Знач ТокенДоступа = "",
	Знач НомерДоверенности = "") Экспорт
	
	Возврат МашиночитаемыеДоверенности.ЗарегистрироватьМЧД(ДанныеВыгрузки, ДанныеПодписи,
		ТокенДоступа, НомерДоверенности, СсылкаНаДоверенность);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ПолучитьСтатусТранзакцииМЧД
Функция ПолучитьСтатусТранзакцииМЧД(Знач ИдентификаторТранзакции,
	Знач ТокенДоступа = "", Знач НомерДоверенности = "") Экспорт
	
	Возврат МашиночитаемыеДоверенности.ПолучитьСтатусТранзакцииМЧД(ИдентификаторТранзакции, ТокенДоступа,
		НомерДоверенности);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ПолучитьСведенияДоверенностиНаСервереМЧД
Функция ПолучитьСведенияДоверенностиНаСервереМЧД(Знач НомерДоверенности,
	Знач ИННДоверителя, Знач ИННПредставителя, Знач ТокенДоступа = "") Экспорт
	
	Возврат МашиночитаемыеДоверенности.ПолучитьСведенияДоверенностиНаСервереМЧД(
		НомерДоверенности, ИННДоверителя, ИННПредставителя, ТокенДоступа);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ПолучитьЧастичныеДанныеДоверенностиНаСервереМЧД
Функция ПолучитьЧастичныеДанныеДоверенностиНаСервереМЧД(Знач НомерДоверенности, Знач ТокенДоступа = "") Экспорт
	
	Возврат МашиночитаемыеДоверенности.ПолучитьЧастичныеДанныеДоверенностиНаСервереМЧД(НомерДоверенности, ТокенДоступа);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ПолучитьНомераДоверенностей
Функция ПолучитьНомераДоверенностей(Знач Доверенности) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ПолучитьНомераДоверенностей(Доверенности);
	
КонецФункции

// См. Справочники.МашиночитаемыеДоверенностиКонтрагентов.ЗагрузитьМЧДИзФайла
Функция ЗагрузитьМЧДКонтрагентаИзФайла(Знач ДанныеФайла) Экспорт
	
	Возврат Справочники.МашиночитаемыеДоверенностиКонтрагентов.ЗагрузитьМЧДИзФайла(ДанныеФайла);
	
КонецФункции

// См. Справочники.МашиночитаемыеДоверенностиОрганизаций.ЗагрузитьМЧДИзФайла
Функция ЗагрузитьМЧДОрганизацииИзФайла(Знач ДанныеФайла) Экспорт
	
	Возврат Справочники.МашиночитаемыеДоверенностиОрганизаций.ЗагрузитьМЧДИзФайла(ДанныеФайла);
	
КонецФункции

// См. МашиночитаемыеДоверенности.ЗагрузитьМЧДИзФайла
Функция ЗагрузитьМЧДИзФайла(Знач АдресХранилища) Экспорт
	Возврат МашиночитаемыеДоверенности.ЗагрузитьМЧДИзФайла(АдресХранилища);
КонецФункции

// См. МашиночитаемыеДоверенности.ОтразитьРезультатПроверкиМЧД
Процедура ОтразитьРезультатПроверкиМЧД(Знач МЧД, Знач Результат, Знач ТекстОшибки, Знач ОписаниеОшибкиРегистрации = "", 
	Знач РезультатПроверкиПодписиМЧД = Неопределено, Знач СнятьОтметкуРучнойПроверки = Ложь) Экспорт
	
	МашиночитаемыеДоверенности.ОтразитьРезультатПроверкиМЧД(
		МЧД, Результат, ТекстОшибки, ОписаниеОшибкиРегистрации, РезультатПроверкиПодписиМЧД, СнятьОтметкуРучнойПроверки);
	
КонецПроцедуры

// См. МашиночитаемыеДоверенности.ДанныеФайлаДоверенностиИПодписи
Функция ДанныеФайлаДоверенностиИПодписи(Знач МЧД) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ДанныеФайлаДоверенностиИПодписи(МЧД);
	
КонецФункции

// Получает доверенность из журнала
//
// Параметры:
//  НомерДоверенности - Строка - Номер доверенности
//
// Возвращаемое значение:
//  ОпределяемыйТип.МашиночитаемаяДоверенность - Доверенность
Функция ПолучитьДоверенностьИзЖурналаПоНомеру(НомерДоверенности) Экспорт
	Возврат МашиночитаемыеДоверенности.ПолучитьДоверенностьИзЖурналаПоНомеру(НомерДоверенности);
КонецФункции

// Получает совокупный список из 10 первых элементов данных выбора по нескольким типам справочников.
// Например, общий список по справочникам Организации и Контрагенты.
// Приоритет ссылок определяется порядком указания типа в параметре ТипыСубъектов
// 
// Параметры:
//  СтрокаПоиска - Строка - Строка поиска
//  ТипыСубъектов - СписокЗначений Из Тип - Типы ссылок, для запроса данных выбора
// 
// Возвращаемое значение:
//  СписокЗначений Из ЛюбаяСсылка - представление элемента имеет вид "<Представление ссылки> [<Синоним справочника>]"
Функция ДанныеВыбораСубъекта(Знач СтрокаПоиска, Знач ТипыСубъектов) Экспорт
	Возврат МашиночитаемыеДоверенности.ДанныеВыбораСубъекта(СтрокаПоиска, ТипыСубъектов);
КонецФункции

// См. МашиночитаемыеДоверенности.ЦветаФона 
Функция ЦветаФона() Экспорт
	
	Возврат МашиночитаемыеДоверенности.ЦветаФона();
	
КонецФункции

// См. МашиночитаемыеДоверенности.ТипПоддержанОператором 
Функция ТипПоддержанОператором(Оператор, ТипДоверенности) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ТипПоддержанОператором(Оператор, ТипДоверенности);
	
КонецФункции

#КонецОбласти

#Область СлужебныеПроцедурыИФункции

// См. МашиночитаемыеДоверенности.РезультатПроверкиДоверенности
Функция РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи, КонтекстДиагностики) Экспорт
	
	Возврат МашиночитаемыеДоверенности.РезультатПроверкиДоверенности(ДанныеДоверенности, РезультатПроверкиПодписи,
		КонтекстДиагностики);
	
КонецФункции

#Область Подписание

// Параметры:
//  Доверенности - Массив из СправочникСсылка.МашиночитаемыеДоверенностиОрганизаций,СправочникСсылка.МЧД003
//  ОтпечаткиСертификатов - см. КриптографияБЭДКлиентСервер.НовыеРезультатыПолученияОтпечатков
//  УчитыватьСНИЛСДоверителя - Булево
//  ЭтоЗаявлениеНаОтмену - Булево
//  ИдентификаторФормы - Неопределено,УникальныйИдентификатор
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция ДанныеДляПодписанияВФоне(Знач Доверенности, Знач ОтпечаткиСертификатов, Знач УчитыватьСНИЛСДоверителя,
	Знач ЭтоЗаявлениеНаОтмену = Ложь, Знач ИдентификаторФормы = Неопределено, Знач КонтекстДиагностики = Неопределено) Экспорт

	Если ИдентификаторФормы = Неопределено Тогда 
		ИдентификаторФормы = Новый УникальныйИдентификатор;
	КонецЕсли;

	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
		
	// См. МашиночитаемыеДоверенности.ДанныеДляПодписанияДляКлиента
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.ДанныеДляПодписанияДляКлиента",
		Доверенности, ОтпечаткиСертификатов, УчитыватьСНИЛСДоверителя, ЭтоЗаявлениеНаОтмену, КонтекстДиагностики);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

// Начать проверку подписей документов по МЧД.
// 
// Параметры:
//  ПараметрыПроверки - см. МашиночитаемыеДоверенностиКлиент.НовыйПараметрыПроверкиПодписиМЧД
// 
// Возвращаемое значение:
//  см. ДлительныеОперации.ВыполнитьФункцию
// 
Функция НачатьПроверкуПодписейДокументовПоМЧД(ПараметрыПроверки) Экспорт 
	
	Возврат МашиночитаемыеДоверенности.НачатьПроверкуПодписейДокументовПоМЧД(ПараметрыПроверки);
	
КонецФункции

#КонецОбласти

#Область ПодписаниеИРегистрация

// Параметры:
//  Доверенность - СправочникСсылка.МЧД003
//  СвойстваПодписи - см. ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи
//  ЗарегистрироватьВРеестреФНС - Булево
//  ИдентификаторФормы - Неопределено,УникальныйИдентификатор
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция ПодписатьЗарегистрироватьМЧДВФоне(Знач Доверенность, Знач СвойстваПодписи, Знач ЗарегистрироватьВРеестреФНС, Знач ИдентификаторФормы = Неопределено) Экспорт
	
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
	
	// См. МашиночитаемыеДоверенности.ПодписатьЗарегистрироватьМЧД
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.ПодписатьЗарегистрироватьМЧД",
		Доверенность, СвойстваПодписи, ЗарегистрироватьВРеестреФНС);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

#КонецОбласти

#Область Регистрация

// Параметры:
//  Доверенность - СправочникСсылка.МЧД003
//  ИдентификаторФормы - УникальныйИдентификатор
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция ЗарегистрироватьВРеестреФНСВФоне(Знач Доверенность, Знач ИдентификаторФормы = Неопределено) Экспорт
	
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
	
	// См. МашиночитаемыеДоверенности.ЗарегистрироватьВРеестреФНС
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.ЗарегистрироватьВРеестреФНС",
		Доверенность);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

#КонецОбласти

#Область Отмена

// см. МашиночитаемыеДоверенности.ОтменитьМЧД
Функция ОтменитьМЧД(Знач ИмяФайлаВыгрузки, Знач ДанныеВыгрузки, Знач ДанныеПодписи, Знач ДанныеСертификата,
		Знач ТокенДоступа = "", Знач НомерДоверенности = "", Знач СсылкаНаДоверенность = Неопределено) Экспорт
	
	Возврат МашиночитаемыеДоверенности.ОтменитьМЧД(ИмяФайлаВыгрузки, ДанныеВыгрузки,
		ДанныеПодписи, ДанныеСертификата, ТокенДоступа, НомерДоверенности, СсылкаНаДоверенность);
		
КонецФункции

// Параметры:
//  Доверенность - СправочникСсылка.МЧД003
//  СвойстваПодписи - Строка
//                  - см. ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи
//  ИдентификаторФормы - Неопределено,УникальныйИдентификатор
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция ПодписатьОтменитьМЧДВФоне(Знач Доверенность, Знач СвойстваПодписи, Знач ИдентификаторФормы = Неопределено) Экспорт
	
	Если ТипЗнч(СвойстваПодписи) = Тип("Строка") Тогда
		ЗначенияСвойствПодписи = ПолучитьИзВременногоХранилища(СвойстваПодписи); // См. ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи
		УдалитьИзВременногоХранилища(СвойстваПодписи);
	Иначе
		ЗначенияСвойствПодписи = СвойстваПодписи;
	КонецЕсли;
	
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
	
	// См. МашиночитаемыеДоверенности.ПодписатьОтменитьМЧД
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.ПодписатьОтменитьМЧД",
		Доверенность, ЗначенияСвойствПодписи);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

#КонецОбласти

#Область ПометитьВерной

// Параметры:
//  Доверенность - ОпределяемыйТип.МашиночитаемаяДоверенность
//  Значение - Булево
//  ИдентификаторФормы - Неопределено,УникальныйИдентификатор
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция УстановитьПометкуВернаВФоне(Знач Доверенность, Знач Значение, Знач ИдентификаторФормы = Неопределено) Экспорт
	
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
	
	// См. МашиночитаемыеДоверенности.УстановитьПометкуВернаПоМестуОбработкиЭДО
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.УстановитьПометкуВернаПоМестуОбработкиЭДО",
		Доверенность, Значение);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

#КонецОбласти

#Область ПометитьОтозванной

// Параметры:
//  Доверенность - ОпределяемыйТип.МашиночитаемаяДоверенность
//  Значение - Булево
//  ДатаОтзыва - Дата
//  ИдентификаторФормы - Неопределено,УникальныйИдентификатор
// 
// Возвращаемое значение:
//  См. ДлительныеОперации.ВыполнитьФункцию
Функция УстановитьПометкуОтозванаВФоне(Знач Доверенность, Знач Значение, Знач ДатаОтзыва, Знач ИдентификаторФормы = Неопределено) Экспорт
	
	ПараметрыВыполненияВФоне = ДлительныеОперации.ПараметрыВыполненияФункции(ИдентификаторФормы);
	
	// См. МашиночитаемыеДоверенности.УстановитьПометкуОтозванаПоМестуОбработкиЭДО
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"МашиночитаемыеДоверенности.УстановитьПометкуОтозванаПоМестуОбработкиЭДО",
		Доверенность, Значение, ДатаОтзыва);
	
	Возврат ДлительнаяОперация;
	
КонецФункции

#КонецОбласти

#КонецОбласти
