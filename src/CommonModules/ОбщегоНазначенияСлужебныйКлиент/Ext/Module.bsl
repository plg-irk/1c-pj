		...
		Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту на клиенте
				           ...
				           |en = 'Cannot connect the external component on the client 
				           |%1
				           |due to:
				           |You should specify either ID or ИдентификаторыСозданияОбъектов IDs'"), 
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на клиенте
			           ...
			           |en = 'Cannot connect the ""%1"" external component on the client
			           |%2
			           |due to:
			           |You cannot connect components from the specified location.'"), 
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось установить внешнюю компоненту ""%1"" на клиенте
			           ...
			           |en = 'Cannot set the ""%1"" external component on the client
			           |%2
			           |due to:
			           |You cannot set components from the specified location.'"), 
			Контекст.Идентификатор,
			Контекст.Местоположение);
	КонецЕсли;
		...
		Иначе 
			ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось подключить внешнюю компоненту ""%1"" на клиенте
				           ...
				           |en = 'Cannot connect the ""%1"" external component on client
				           |%2
				           |due to:
				           |The НачатьПодключениеВнешнейКомпоненты method returned False.'"),
				Контекст.Идентификатор,
		...
		Попытка
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				ВызватьИсключение НСтр("ru = 'Оператор Новый вернул Неопределено';
										|en = 'Operator New returned Undefined'");
			КонецЕсли;
		Исключение
			ПодключаемыйМодуль = Неопределено;
			ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		...
		Если ПодключаемыйМодуль = Неопределено Тогда 
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось создать объект внешней компоненты ""%1"", подключенной на клиенте
				           ...
				           |по причине:
				           |%3';
				           |en = 'Cannot create object of the ""%1"" external component connected on the client 
				           |%2
				           |due to:
				           |%3'"),
				Контекст.Идентификатор,
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось создать объект ""%1"" внешней компоненты ""%2"", подключенной на клиенте
					           ...
					           |по причине:
					           |%4';
					           |en = 'Cannot create the ""%1"" object of the ""%2"" external component connected on the client
					           |%3,
					           |due to:
					           |%4'"),
...
Функция ПредставлениеМестоположенияКомпоненты(Местоположение)
	...
	Если СтрНачинаетсяС(Местоположение, "e1cib/") Тогда
		Возврат НСтр("ru = 'из хранилища внешних компонент';
					|en = 'from external component storage'");
	Иначе 
		...
		Если ВыполнитьПерезагрузкуСеанса Тогда
			...
			ТекстВопроса = 
				НСтр("ru = 'Для завершения перерегистрации компоненты comcntr необходимо перезапустить программу.
				           ...
				           |en = 'To end reregistration of component comcntr, restart the application.
				           |Restart now?'");
	...
	Иначе 
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при регистрации компоненты comcntr.
			           ...
			           |en = 'An error occurred while registering the comcntr component.
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
			НСтр("ru = 'Регистрация компоненты comcntr';
				|en = 'comcntr component registration'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), 
			"Ошибка", 
			ТекстСообщения,,
			Истина);
...
Процедура ПроверитьРасширениеРаботыСФайламиПодключеноЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстСообщения) Тогда
		ТекстСообщения = НСтр("ru = 'Действие недоступно, так как не установлено расширение для веб-клиента 1С:Предприятие.';
								|en = 'The action is not available as an extension for 1C:Enterprise web client is not installed. '")
	КонецЕсли;
