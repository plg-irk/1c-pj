...
Процедура ПроверитьКоличествоУникальныхНомеров(Объект, Отказ=ЛОЖЬ, ИмяТабличнойЧасти="Активы", ИмяПоляАктива="ПрочийАктив") Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		...
		Если ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Инструменты Тогда
			// Ограничений не имеется
		Иначе
			Если СтрокаТабличнойЧасти.Количество > 1 Тогда
				СтрокаТабличнойЧасти.Количество = 1;
				ВывестиСообщение(НСтр("ru = 'Прочий актив ""%1"" учитывается по уникальным номерам. Количество не может превышать 1 единицу. Заменена на 1.'"), Объект, ИмяТабличнойЧасти+"[#].Количество", СтрокаТабличнойЧасти.НомерСтроки,, ПрочийАктив);
			КонецЕсли;
		КонецЕсли;
...
Процедура АктивПриИзменении(Объект, Строка, ПараметрыДействия = Неопределено, ИмяТабличнойЧасти="Активы") Экспорт
	...
	Если ЕстьРеквизит(Объект, "Количество", ИмяТабличнойЧасти) И ЗначениеЗаполнено(Строка.ПрочийАктив) Тогда
		...
		УникальныйНомер = НЕ(Строка.ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Спецодежда ИЛИ Строка.ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Спецоснастка ИЛИ Строка.ПрочийАктив.ВидПрочегоАктива=Перечисления.ВидыПрочихАктивов.Инструменты);
		...
		ИначеЕсли Строка.Количество > 1 И УникальныйНомер Тогда
			Строка.Количество = 1;
			ВывестиСообщение(НСтр("ru = 'Актив ""%1"" учитывается по уникальным номерам. Количество не может превышать 1 единицу.'"), Объект, ИмяТабличнойЧасти+"[#].ПрочийАктив", Строка.НомерСтроки,, Строка.ПрочийАктив);
		КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизиты(Объект, ТекущаяСтрока = Неопределено, ПараметрыДействия = Неопределено, ИмяТабличнойЧасти="Активы") Экспорт
	...
	ВыводитьСуммаСписания       = ЕстьРеквизит(Объект, "СуммаСписания",       ИмяТабличнойЧасти) И ПолучитьФункциональнуюОпцию("ИспользоватьИндикациюСуммыСписания");
	...
	Для Каждого Строка Из Объект.Активы Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Для Каждого СтрМассивСумм Из НайденныеСтроки Цикл
				Строка.СуммаСписания = Строка.СуммаСписания + СтрМассивСумм[ИмяРесурса];
			КонецЦикла;
