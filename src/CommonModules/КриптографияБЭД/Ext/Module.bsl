...
// Возвращает только действующие сертификаты по их отпечаткам.
// 
// Параметры:
//  Отпечатки - Массив из Строка
// 
...
Функция ПриПроверкеПодписиВозниклиОшибки(Подпись)
	...
	СертификатОтозванИлиИстек = Ложь;
	РежимСтрогойПроверкиДоверенностейОтключен = Ложь;
	ПроверкаПолномочийВыполненаУспешно = Ложь;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		
		МодульМашиночитаемыеДоверенности = 
				ОбщегоНазначения.ОбщийМодуль("МашиночитаемыеДоверенности");
				
		РежимСтрогойПроверкиДоверенностейОтключен = 
				МодульМашиночитаемыеДоверенности.РежимСтрогойПроверкиДоверенностейОтключен();
			
		ПротоколПроверкиПодписиМЧД = 
			МодульМашиночитаемыеДоверенности.ПротоколПроверкиПодписиМЧД(Подпись.СвойстваДоверенности);
	
		Если Подпись.РезультатПроверкиПоМЧД <> Неопределено
			И Подпись.РезультатПроверкиПоМЧД.Свойство("ПротоколПроверки")
			И Подпись.РезультатПроверкиПоМЧД.ПротоколПроверки <> Неопределено Тогда
				
			ПроверкаПолномочийВыполненаУспешно = 
				МодульМашиночитаемыеДоверенности.ПроверкаПолномочийВыполненаУспешно(
					Подпись.РезультатПроверкиПоМЧД.ПротоколПроверки.ПроверкаМЧД)
		КонецЕсли;
			
		Если ПротоколПроверкиПодписиМЧД <> Неопределено Тогда
			
			МодульМашиночитаемыеДоверенностиКлиентСервер = 
				ОбщегоНазначения.ОбщийМодуль("МашиночитаемыеДоверенностиКлиентСервер");
			
			СертификатОтозванИлиИстек = 
				МодульМашиночитаемыеДоверенностиКлиентСервер.СертификатОтозванИлиИстек(ПротоколПроверкиПодписиМЧД);
	...
	Если Результат И РежимСтрогойПроверкиДоверенностейОтключен Тогда
		Если СертификатОтозванИлиИстек Или Не ПроверкаПолномочийВыполненаУспешно Тогда
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
...
//  					 - см. МашиночитаемыеДоверенности.НовыеСвойстваДоверенности
//
// Возвращаемое значение:
//  Строка
Функция ПредставлениеРезультатаАвтоматическойПроверкиМЧД(ДоверенностьДействует, ПротоколПроверкиМЧД = Неопределено, 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами") Тогда
		...
		Результат = 
			МодульМашиночитаемыеДоверенности.ПредставлениеРезультатаАвтоматическойПроверкиМЧД(
				ДоверенностьДействует, ПротоколПроверкиМЧД, СвойстваДоверенности);
				...
				Если ДоверенностьОпределена Тогда
					...
					Если Не ПроверкаДоверенностиВыполненаВручную Тогда
						ТекстОтметки = ТекстОтметки 
							+ Символы.ПС
							+ ПредставлениеРезультатаАвтоматическойПроверкиМЧД(
								ДоверенностьДействует, 
								Подпись.РезультатПроверкиПоМЧД.ПротоколПроверки.ПроверкаМЧД,
								Подпись.СвойстваДоверенности);
					Иначе
						ТекстОтметки = ТекстОтметки 
							+ Символы.ПС 
...
Функция ТекстШтампаЭП(ДанныеДляФормирования, Подписи, ТекстШтампа)
	...
	Для Каждого Подпись Из Подписи Цикл
		...
		ТекстОтметки = "";
		ДопустимоеКоличествоСимволовВСтрокеМакета = 20;
