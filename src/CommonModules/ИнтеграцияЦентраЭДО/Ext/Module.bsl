...
Функция ВидОшибкиОтсутствуетОбластьЦентраЭДО(АдресЦентраЭДО = "") Экспорт
	...
	Если ЗначениеЗаполнено(АдресЦентраЭДО) Тогда
		ВидОшибки.ОписаниеПроблемы = СтрШаблон(НСтр("ru = 'Центр ЭДО по адресу %1 больше не доступен.'"), АдресЦентраЭДО);
	Иначе
...
//  ТекстОшибки - Строка
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЦентровЭДО
Процедура ДобавитьНеизвестнуюОшибкуИлиИсключение(КонтекстДиагностики, ВидОперации, ТекстОшибки, УчетнаяЗапись) Экспорт
