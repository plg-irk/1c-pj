...
//  Идентификатор         - Строка          - имя проверяемого макета.
//
...
Процедура ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, ИмяМакета, СинонимМакета, ТабличныйДокумент,
	...
	Если ОписаниеПечатнойФормы <> Неопределено Тогда
		...
		// +РАРУС Настроим доступность для редактирования в зависимости от роли.
		ОписаниеПечатнойФормы.ТабличныйДокумент.Защита = НЕ ПраваИНастройкиПользователяПовтИсп.ПроверитьРольПользователя("РедактированиеПечатныхФорм");
		// -РАРУС
...
//  СписокОбъектов - Массив - список объектов метаданных.
//
// Возвращаемое значение:
...
Функция КомандыПечатиФормы(Форма, СписокОбъектов = Неопределено) Экспорт
	...
	Если СтандартнаяОбработка Тогда
		...
		Иначе
			...
			Иначе
				...
				// +РАРУС
				УправлениеПечатьюПлатформа.ПриПолученииКомандПечати(КомандыПечати, ОбъектМетаданных.ПолноеИмя());
				// -РАРУС
	...
	Если ОбъектМетаданных <> Неопределено Тогда
		НастройкиРасширений = ПодключаемыеКоманды.НастройкиРасширенийПодключенныхКОбъектуМетаданных(СсылкаОМ);
		РасширенияПечати = НастройкиРасширений.НайтиСтроки(Новый Структура("ДобавитьКомандыПечати", Истина));
		Для Каждого Расширение Из РасширенияПечати Цикл
			Расширение.Менеджер.ДобавитьКомандыПечати(КомандыПечати);
			...
			Для Каждого Команда Из ДобавленныеКоманды Цикл
				Команда.МенеджерПечати = Расширение.ПолноеИмя;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
//											в модуле основной формы отчета или обработки.
...
//												//
//												&НаКлиенте
//													Функция <ИмяПроцедуры>(ПараметрыПечати) Экспорт
//												// Обработчик печати.
//												КонецФункции
//
...
//
//											При выборе команды печати сразу открывается сформированный в формате pdf документ.
//
...
Функция СоздатьКоллекциюКомандПечати() Экспорт
	...
	// проверка проведения
	Результат.Колонки.Добавить("ПроверкаИсключенияЗапретаРедактирования", Новый ОписаниеТипов("Булево"));
	...
	Результат.Колонки.Добавить("ВидГруппыПодменю",   Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ЭлементФормы",       Новый ОписаниеТипов("Строка"));
	Результат.Колонки.Добавить("НастройкаВидимости", Новый ОписаниеТипов("ХранилищеЗначения"));
	// -РАРУС
...
Процедура ПриОпределенииСтруктурыНастроекПодключаемыхОбъектов(СтруктураНастроек) Экспорт
	СтруктураНастроек.Вставить("ДобавитьКомандыПечати", Новый ОписаниеТипов("Булево"));
КонецПроцедуры
...
Процедура ПриОпределенииИспользуемыхПодменю(Форма, ИспользуемыеПодменю) Экспорт
	Если Форма.Команды.Найти("АдресКомандПечатиВоВременномХранилище") <> Неопределено Тогда
		Возврат; // Команды печати уже выведены при помощи механики печати.
	КонецЕсли;
	Если Форма.Элементы.Найти("ПодменюПечать") <> Неопределено Тогда
		ИспользуемыеПодменю.Добавить("ПодменюПечать");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, НастройкиОбъекта, НастройкиРасширений, Команды) Экспорт
	Если НастройкиФормы.ИспользуемыеПодменю.Найти("ПодменюПечать") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	КомандыПечати = СоздатьКоллекциюКомандПечати();
	
	СтандартнаяОбработка = НастройкиОбъекта.Менеджер <> Неопределено;
	...
	Если СтандартнаяОбработка Тогда
		НастройкиСписка = Новый Структура;
		НастройкиСписка.Вставить("МенеджерКомандПечати", НастройкиОбъекта.Менеджер);
		НастройкиСписка.Вставить("АвтоматическоеЗаполнение", НастройкиОбъекта.ЭтоЖурналДокументов);
		Если НастройкиСписка.АвтоматическоеЗаполнение Тогда
			УправлениеПечатьюПереопределяемый.ПриПолученииНастроекСпискаКомандПечати(НастройкиСписка);
		КонецЕсли;
		Если НастройкиСписка.АвтоматическоеЗаполнение Тогда
			ЗаполнитьКомандыПечатиДляСпискаОбъектов(НастройкиОбъекта.Метаданные.РегистрируемыеДокументы, КомандыПечати);
		Иначе
			НастройкиОбъекта.Менеджер.ДобавитьКомандыПечати(КомандыПечати);
			ДобавленныеКоманды = КомандыПечати.НайтиСтроки(Новый Структура("МенеджерПечати", Неопределено));
			Для Каждого КомандаПечати Из ДобавленныеКоманды Цикл
				КомандаПечати.МенеджерПечати = НастройкиОбъекта.ПолноеИмя;
			КонецЦикла;
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
				МодульДополнительныеОтчетыИОбработки = ОбщегоНазначения.ОбщийМодуль("ДополнительныеОтчетыИОбработки");
				МодульДополнительныеОтчетыИОбработки.ПриПолученииКомандПечати(КомандыПечати, НастройкиОбъекта.ПолноеИмя);
			КонецЕсли;
			// +РАРУС
			УправлениеПечатьюПлатформа.ПриПолученииКомандПечати(КомандыПечати, НастройкиОбъекта.ПолноеИмя);
			// -РАРУС
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Расширение Из РасширенияПечати Цикл
		...
		ДобавленныеКоманды = КомандыПечати.НайтиСтроки(Новый Структура("МенеджерПечати", Неопределено));
		Для Каждого КомандаПечати Из ДобавленныеКоманды Цикл
			КомандаПечати.МенеджерПечати = Расширение.ПолноеИмя;
		КонецЦикла;
	КонецЦикла;
	...
	ОтключенныеКомандыПечати = ОтключенныеКомандыПечати(НастройкиОбъекта.Ссылка);
	ЕстьФильтрПоОтключенным = ОтключенныеКомандыПечати.Количество() > 0;
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		Если ЕстьФильтрПоОтключенным Тогда
			...
			Если ОтключенныеКомандыПечати.Найти(КомандаПечати.УникальныйИдентификатор) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		КомандаПечати.ДополнительныеПараметры.Вставить("ДополнитьКомплектВнешнимиПечатнымиФормами", КомандаПечати.ДополнитьКомплектВнешнимиПечатнымиФормами);
		...
		Если Команда.Порядок = 0 Тогда
			Команда.Порядок = 50;
		КонецЕсли;
		Команда.Группа = КомандаПечати.МестоРазмещения;
		Если ПустаяСтрока(Команда.Группа) Тогда
			Команда.Группа = "ПодменюПечать";
		КонецЕсли;
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Если ТипЗнч(ИменаМакетов) = Тип("Строка") Тогда
		ИменаМакетов = ?(ЗначениеЗаполнено(ИменаМакетов), СтрРазделить(ИменаМакетов, ","), Новый Массив);
	Иначе // Тип("Массив")
	...
	Если ТипЗнч(ПараметрыПечати) = Тип("Структура") 
		...
		Для Каждого Идентификатор Из ИдентификаторыВнешнихПечатныхФорм Цикл
			// +Рарус Добавлен учет использования внешних форм
...
Функция СписокКомандЭкземпляраФормы(АдресКомандПечатиВоВременномХранилище) Экспорт
	Команды = ПолучитьИзВременногоХранилища(АдресКомандПечатиВоВременномХранилище);
	Если Команды.Колонки.Найти("ИмяКомандыНаФорме") = Неопределено Тогда
		КомандыПечати = СоздатьКоллекциюКомандПечати();
		Для Каждого Команда Из Команды Цикл
			КомандаПечати = КомандыПечати.Добавить();
			ЗаполнитьЗначенияСвойств(КомандаПечати, Команда, , "Обработчик");
			ЗаполнитьЗначенияСвойств(КомандаПечати, Команда.ДополнительныеПараметры);
			КомандаПечати.МестоРазмещения = Команда.Группа;
			Если ТипЗнч(Команда.ТипПараметра) = Тип("ОписаниеТипов") Тогда
				КомандаПечати.ТипыОбъектовПечати = Команда.ТипПараметра.Типы();
			КонецЕсли;
			КомандаПечати.СписокФорм = Команда.ВидимостьВФормах;
			Если Команда.РежимЗаписи = "БезЗаписи" Тогда
				КомандаПечати.НеВыполнятьЗаписьВФорме = Истина;
			Иначе
				КомандаПечати.НеВыполнятьЗаписьВФорме = Ложь;
				Если Команда.РежимЗаписи = "Проведение" Тогда
					КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
				Иначе
					КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
				КонецЕсли;
			КонецЕсли;
			КомандаПечати.ТребуетсяРасширениеРаботыСФайлами = Команда.ТребуетсяРаботаСФайлами;
			КомандаПечати.ИмяКомандыНаФорме = Команда.ИмяВФорме;
		КонецЦикла;
	Иначе
		КомандыПечати = Команды;
	КонецЕсли;
...
Функция ПакетДокументов(ТабличныеДокументы, ОбъектыПечати, ПечататьКомплектами, КоличествоЭкземпляров = 1) Экспорт
	...
	Если КоллекцияПечатныхФорм.Количество() > 0 Тогда
		ПакетКомплектов.ИмяПринтера = КоллекцияПечатныхФорм[0].ИмяПринтера;
	КонецЕсли;
...
Процедура УстановитьНастройкиКомандПечати(КомандыПечати, Владелец)
	...
	|ВЫБРАТЬ
	|	НастройкиКомандПечати.УникальныйИдентификатор КАК УникальныйИдентификатор,
	|	НастройкиКомандПечати.НастройкаВидимости КАК НастройкаВидимости
	|ИЗ
	|	РегистрСведений.НастройкиКомандПечати КАК НастройкиКомандПечати
	|ГДЕ
	|	НастройкиКомандПечати.Владелец = &Владелец
	|	И НастройкиКомандПечати.Видимость = ИСТИНА";
	// -РАРУС
	...
	СписокОтключенных = РезультатЗапроса[0].Выгрузить().ВыгрузитьКолонку("УникальныйИдентификатор");
	
	// Получим настройки видимости команд печати.
	СписокНастроекВидимости = РезультатЗапроса[1].Выгрузить();
	// -РАРУС
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		// +РАРУС
		НайденныеСтроки = СписокНастроекВидимости.НайтиСтроки(Новый Структура("УникальныйИдентификатор", КомандаПечати.УникальныйИдентификатор));
		Для Каждого ТекущаяСтрока Из НайденныеСтроки Цикл
			КомандаПечати.НастройкаВидимости = ТекущаяСтрока.НастройкаВидимости;
			Прервать;
		КонецЦикла;
		// -РАРУС
	КонецЦикла;
...
Функция КомандыПечатиОбъекта(ОбъектМетаданных) Экспорт
	...
	// +РАРУС
	УправлениеПечатьюПлатформа.ПриПолученииКомандПечати(КомандыПечати, ИмяОбъекта);
	// -РАРУС
