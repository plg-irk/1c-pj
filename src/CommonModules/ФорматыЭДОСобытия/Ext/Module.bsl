...
Процедура ЗаполнитьДанные_ИзвещениеОПолучении(Данные, ДанныеДляФормирования, Отказ) Экспорт
	...
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, "ВерсияФормата", ВерсияФорматаКвитанций()); 
	...
	Иначе
		...
		Путь = "Получатель.ИдУчастникаЭДО";
		ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь, Абонент.Идентификатор);
		ТипУчастникаЭДО = ИнтеграцияЭДО.ТипУчастникаЭДО(Абонент.Ссылка);
		ЭтоИностранноеЛицо = ТипУчастникаЭДО.ЭтоИностранноеЛицо;
		ЭтоФизЛицоИлиИП = ТипУчастникаЭДО.ЭтоФизЛицо Или ТипУчастникаЭДО.ЭтоИП;
		ДанныеЮрФизЛица = ОбщегоНазначенияБЭД.ДанныеЮрФизЛица(Абонент.Ссылка);
		ПолноеНаименованиеИмя = ?(ЭтоФизЛицоИлиИП, "ПолноеНаименование", "ОфициальноеНаименование");
		ПолноеНаименование = "";
		...
		Если ЭтоИностранноеЛицо Тогда
			
			Если НЕ ЗначениеЗаполнено(ДанныеЮрФизЛица.КодСтраныНерезидента) Тогда
				ДанныеЮрФизЛица.КодСтраныНерезидента = ТрансграничныйЭДО.КодСтраныАбонента(Абонент.Идентификатор);
			КонецЕсли;
			
			Путь = "Получатель.ТипУчастника.СвИнУчастНеУч";
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь + ".ИдСтат", "ЮЛ");
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные,
				Путь + ".СтрРег", ДанныеЮрФизЛица.КодСтраныНерезидента);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь + ".Наим", ПолноеНаименование);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные,
				Путь + ".КодНПРег", ДанныеЮрФизЛица.ИдентификаторЮридическогоЛица);
			
		Иначе
			
			Если ЭтоФизЛицоИлиИП Тогда
	...
	Иначе
		...
		Если ЭтоИностранноеЛицо Тогда
			...
			Если НЕ ЗначениеЗаполнено(ДанныеЮрФизЛица.КодСтраныНерезидента) Тогда
				ДанныеЮрФизЛица.КодСтраныНерезидента = ТрансграничныйЭДО.КодСтраныАбонента(Абонент.Идентификатор);
			КонецЕсли;
			НаименованиеСтраныНерезидента = ТрансграничныйЭДО.НаименованиеСтраныПоКоду(ДанныеЮрФизЛица.КодСтраныНерезидента);
			
			Путь = "Получатель.ТипУчастника.СвИнУчастНеУч";
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь + ".ИдСтат", "ИО");
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".КодСтр", ДанныеЮрФизЛица.КодСтраныНерезидента);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".НаимСтран", НаименованиеСтраныНерезидента);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(Данные, Путь + ".Наим", ПолноеНаименование);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				Данные, Путь + ".Идентиф", ДанныеЮрФизЛица.ИдентификаторЮридическогоЛица);
...
Процедура ЗаполнитьДанные_УведомлениеОбУточнении(Данные, ДанныеДляФормирования, Отказ) Экспорт
	...
	Иначе
		...
		ЭтоИностранноеЛицо = ИнтеграцияЭДО.ТипУчастникаЭДО(Абонент.Ссылка).ЭтоИностранноеЛицо;
			...
			Если ДанныеСтрокиЗапроса <> Неопределено 
				...
				Если Не НайденаСопоставленнаяНоменклатура Тогда //Зарегистрируем сопоставление
					...
					СопоставлениеНоменклатурыКонтрагентов.СоздатьОбновитьНоменклатуруКонтрагента(
						ОписаниеНоменклатурыКонтрагента,
						ОписаниеНоменклатурыИБ,
						Отказ);
...
Функция СтавкаНДСИзФорматаBNCommerceOffering(Знач ЗначениеСтавки, Знач ЭтоУСН = Ложь)
	...
	ИначеЕсли ЗначениеСтавки = "VAT_22" Тогда
		ПредставлениеСтавкиНДС = "22";
	КонецЕсли;
