...
Функция ИспользуетсяОтправкаПолучениеСОтборомПоОрганизации() Экспорт
	...
	Возврат ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить(
		ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО(),
...
Процедура ОбработкаПолученияФормыЭлектронногоДокумента(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация,
	...
	ЭтоСуществующийЛокальныйДокумент = ЭтоСуществующийДокумент И Не ЭлектронныеДокументыЭДО.ЭтоДокументЦентраЭДО(
		ЭлектронныйДокумент);
	ЭтоСозданиеНовогоЛокальногоДокумента = Не ЭтоСуществующийДокумент
		И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ИспользоватьЦентрЭДО",
		ИспользоватьИнтеграциюЦентраЭДО);
	
	ВариантыИнтерфейса = ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО();
	
	Если Не (ЭтоСуществующийЛокальныйДокумент Или ЭтоСозданиеНовогоЛокальногоДокумента) Тогда
...
// 
// Возвращаемое значение:
//  См. ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации
Функция РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации(Знач ДлительнаяОперация) Экспорт
	Возврат ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации(
		ДлительнаяОперация);
КонецФункции
...
// 
// Возвращаемое значение:
//  См. ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации
Функция РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации(Знач ДлительнаяОперация) Экспорт
	Возврат ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.РезультатПодготовкиДанныхПросмотраПоФайлуXMLПослеДлительнойОперации(
		ДлительнаяОперация);
КонецФункции

#Область НастройкаИнтерфейса

// См. ИнтерфейсДокументовЭДО.ВариантИнтерфейсаДокументовЭДО
Функция ВариантИнтерфейсаДокументовЭДО() Экспорт
	
	Возврат ИнтерфейсДокументовЭДО.ВариантИнтерфейсаДокументовЭДО();

КонецФункции

// См. ИнтерфейсДокументовЭДО.СохранитьНастройкуИспользованияЛегкогоВариантаИнтерфейсаДокументовЭДО
//
Процедура СохранитьНастройкуИспользованияЛегкогоВариантаИнтерфейсаДокументовЭДО(ИспользоватьЛегкий = Ложь) Экспорт
	
	ИнтерфейсДокументовЭДО.СохранитьНастройкуИспользованияЛегкогоВариантаИнтерфейсаДокументовЭДО(ИспользоватьЛегкий);
...
Функция ТребуетсяВыборФайлаПередСозданиемНовогоПроизвольногоДокумента() Экспорт
	...
	Если ВариантИнтерфейса = ВариантыИнтерфейса.Легкий
		Или ВариантИнтерфейса = ВариантыИнтерфейса.ЛегкийКлиентЭДО
		Или НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
// Параметры:
//  РедактированиеВключено - Булево
Процедура ВключитьРедактированиеРеквизитовВходящихНеформализованныхДокументов(РедактированиеВключено) Экспорт
	
	КлючОбъектаНастроек = ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО();
	КлючНастройки = "ИспользуетсяРедактированиеРеквизитовВходящихНеформализованныхДокументов";
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаНастроек, КлючНастройки, РедактированиеВключено);
...
// Возвращает номенклатуру и контекст сопоставления.
// 
// Параметры:
...
//   * НоменклатураДляСопоставления - см. ИнтеграцияЭДО.НоменклатураКонтрагентовНаКонтроле
//
Функция НоменклатураИКонтекстСопоставленияПоЭД(Знач ЭлектронныеДокументы) Экспорт
	
	НоменклатураИКонтекст = ИнтеграцияЭДО.КонтекстСопоставленияНоменклатуры(ЭлектронныеДокументы);
	Отбор = ИнтеграцияЭДО.ОтборНоменклатурыКонтрагентовНаКонтроле(ЭлектронныеДокументы);
	НоменклатураИКонтекст.Вставить("НоменклатураДляСопоставления", 
		ИнтеграцияЭДО.НоменклатураКонтрагентовНаКонтроле(Отбор));
	
	Возврат НоменклатураИКонтекст;
