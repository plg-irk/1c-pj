...
Процедура СоздатьНастройкуОбмена(СтруктураПараметров, АдресХранилища) Экспорт
	...
	НастройкаОбменаСсылка = ОбменСБанками.НастройкаОбмена(
		СтруктураПараметров.Организация, СтруктураПараметров.Банк, Ложь);
...
Процедура ПрочитатьВыпискуСбербанкXDTO(ЭД, ДеревоРазбора, НовыйЭД, Ошибка)
	...
	Если НЕ ЭД.InfoForStamp = Неопределено Тогда
		...
		ПараметрыШтампа.Вставить("НазваниеБанка", ЭД.InfoForStamp.BankName);
		ПараметрыШтампа.Вставить("ГородБанка", ЭД.InfoForStamp.BranchName);
		ПараметрыШтампа.Вставить("Офис", ЭД.InfoForStamp.SubBranchName);
		ПараметрыШтампа.Вставить("ОтделениеБанка", ЭД.InfoForStamp.SubBranchNum);
		НайденныйТипВДереве = ЭлектронноеВзаимодействиеСлужебный.НайтиСоздатьТипОбъектаВДеревеРазбора(
			ДеревоРазбора, "Штамп");
		НайденнаяСтрока = НайтиСоздатьСтрокуВДеревеРазбора(
