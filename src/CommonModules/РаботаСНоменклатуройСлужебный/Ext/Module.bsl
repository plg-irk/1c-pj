...
Функция ОписаниеПараметровЗапросаКатегории() Экспорт
	...
	Классификаторы = Новый ТаблицаЗначений; // CategoryRequest.search.classifiers
	Классификаторы.Колонки.Добавить("Код",          Типы().Строка);	// code
	Классификаторы.Колонки.Добавить("Наименование", Типы().Строка);	// name
	...
	Результат.Вставить("ДополнительныеРеквизиты", ДополнительныеРеквизиты); // search.attributes.
	Результат.Вставить("Классификаторы",          Классификаторы); 			// search.classifiers
...
Функция ПараметрыЗапросаКатегории(ПараметрыЗапроса) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗапроса.Идентификаторы) Тогда
		...
		Для Каждого ЭлементИдентификатор Из ПараметрыЗапроса.Идентификаторы Цикл
			Объект.ids.Добавить(СокрЛП(ЭлементИдентификатор));
		КонецЦикла;
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЗапроса.ПоисковаяСтрока)
				...
				Или ЗначениеЗаполнено(ПараметрыЗапроса.ДополнительныеРеквизиты)
				Или ЗначениеЗаполнено(ПараметрыЗапроса.Классификаторы) Тогда
		...
		// Заполнение реквизитов поиска по кодам классификаторов
		Если ЗначениеЗаполнено(ПараметрыЗапроса.Классификаторы) Тогда
			Для Каждого СтрокаКлассификатора Из ПараметрыЗапроса.Классификаторы Цикл
				Если ЗначениеЗаполнено(СтрокаКлассификатора.Код)
						И ЗначениеЗаполнено(СтрокаКлассификатора.Наименование) Тогда
					ОбъектКлассификатора = ФабрикаXDTO.Создать(ТипОбъектаXDTO("CategoryRequest.search.classifiers"));
					ОбъектКлассификатора.code = СтрокаКлассификатора.Код;
					ОбъектКлассификатора.name = СтрокаКлассификатора.Наименование;
					ОбъектПоиска.classifiers.Добавить(ОбъектКлассификатора);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ОписаниеТаблицыНоменклатуры() Экспорт
	...
	Результат.Колонки.Добавить("ЗапрошенныйШтрихкод", Типы().Строка, ИмяСлужебногоПоляЗапрошенныйШтрихкод());
	Результат.Колонки.Добавить("Ошибка", ОбщегоНазначения.ОписаниеТипаСтрока(50), ИмяСлужебногоПоляОшибка());
...
Процедура ОбновитьСостоянияВыгрузкиНоменклатуры(НастройкаВыгрузки) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		ОтветаСервиса.Колонки.Добавить("signed",           Новый ОписаниеТипов("Булево"));
		...
		Пока ВыборкаСостав.Следующий() Цикл
			...
			Для Каждого СтрокаОтвета Из СтрокиОтвета Цикл
				...
				Подписан  = Макс(Подписан, СтрокаОтвета.signed);
			...
			ИначеЕсли Состояние = 1 Тогда
				...
				Иначе
					Состояние = Перечисления.СостоянияВыгрузкиНоменклатуры.ОжидаетПодписания;
				КонецЕсли;
...
// Определяет статистику выгрузки номенклатуры.
//
// Параметры:
//   Организация - ОпределяемыйТип.Организация - организация.
//
// Возвращаемое значение:
//   Структура - статистика по состояниям:
//   * ОжидаетВыгрузки        - Число - количество товарных позиций по организации, ожидающих выгрузки.
//   * ПроверяетсяМодератором - Число - количество товарных позиций по организации на проверке модератором.
//   * ОжидаетПодписания      - Число - количество товарных позиций по организации, ожидающих подписания.
//   * Принята                - Число - количество принятых товарных позиций по организации.
//   * Отклонена              - Число - количество отклоненных товарных позиций по организации.
//
Функция СтатистикаРезультатовВыгрузки(Организация) Экспорт 
	...
	СтруктураРезультата.Вставить("ОжидаетПодписания",      0);
	...
	Запрос.Текст = 
		...
		|	ГДЕ
		|		СостоянияВыгрузкиНоменклатуры.Организация = &Организация
		|		И СостоянияВыгрузкиНоменклатуры.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияВыгрузкиНоменклатуры.ОжидаетПодписания)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ ПЕРВЫЕ 1000
		...
		|	ГДЕ
		|		СостоянияВыгрузкиНоменклатуры.Организация = &Организация
		|		И СостоянияВыгрузкиНоменклатуры.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияВыгрузкиНоменклатуры.Отклонена)) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, 
		"1000", 
		Формат(РаботаСНоменклатуройСлужебныйКлиентСервер.РазмерПорции(), "ЧГ=0"));
	
	Запрос.УстановитьПараметр("Организация", Организация);
	
	ВыборкаДанных = Запрос.Выполнить().Выбрать();
	Пока ВыборкаДанных.Следующий() Цикл
		СтруктураРезультата.Вставить(ВыборкаДанных.Состояние, ВыборкаДанных.Количество);
	КонецЦикла;
...
//  ТаблицаЗначений:
//   * Штрихкод - Строка
//   * Номенклатура - Неопределено,ОпределяемыйТип.НоменклатураРаботаСНоменклатурой
//   * Характеристика - Неопределено,ОпределяемыйТип.ХарактеристикаРаботаСНоменклатурой
//   * ЕдиницаИзмерения - Неопределено
//
Функция НовыеДанныеНоменклатурыПоШтрихкодам() Экспорт
	
	ДанныеНоменклатуры = Новый ТаблицаЗначений;
	ДанныеНоменклатуры.Колонки.Добавить("Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200));
	ДанныеНоменклатуры.Колонки.Добавить("Номенклатура", 
		Метаданные.ОпределяемыеТипы.НоменклатураРаботаСНоменклатурой.Тип);
	ДанныеНоменклатуры.Колонки.Добавить("Характеристика", 
		Метаданные.ОпределяемыеТипы.ХарактеристикаРаботаСНоменклатурой.Тип);
	ДанныеНоменклатуры.Колонки.Добавить("ЕдиницаИзмерения");
...
// Возвращаемое значение:
//  Структура:
//   * Результат - см. РаботаСНоменклатуройСлужебныйКлиентСервер.ШаблонДанныхНоменклатуры.
//   * Отказ - Булево - принимает значение Истина, если не удалось получить Результат
//   * Ошибка - Строка - тип ошибки, значения: см. РаботаСНоменклатурой.ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам.
//
Функция НовоеОписаниеНоменклатурыПоШтрихкоду() Экспорт
	
	ОписаниеНоменклатуры = Новый Структура;
	ОписаниеНоменклатуры.Вставить("Результат", РаботаСНоменклатуройСлужебныйКлиентСервер.ШаблонДанныхНоменклатуры());
	ОписаниеНоменклатуры.Вставить("Отказ", Ложь);
	ОписаниеНоменклатуры.Вставить("Ошибка", "");
...
// Возвращает данные Национального каталога, найденные по штрихкодам
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  - см. РаботаСНоменклатурой.ДанныеНоменклатурыСервиса - данные по найденной номенклатуре
//  - Неопределено - если данные по номенклатуре не найдены
//
Функция ДанныеНоменклатурыНациональногоКаталогаПоШтрихкодам(Штрихкоды, ПараметрыЗагрузки) Экспорт
	
	КонтекстПроверки = "РаботаСНоменклатуройСлужебный.ДанныеНоменклатурыНациональногоКаталогаПоШтрихкодам";
	
	ПроверкаПараметров = РаботаСНоменклатуройКлиентСервер.НовыеПараметрыПолученияНоменклатурыСервисаПоШтрихкодам();
	ПроверкаПараметров.ТокенГИСМТ = Тип("Строка");
	...
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(КонтекстПроверки, "ПараметрыЗагрузки", ПараметрыЗагрузки, 
		Тип("Структура"), ПроверкаПараметров);
	ОбщегоНазначенияКлиентСервер.Проверить(Не ПустаяСтрока(ПараметрыЗагрузки.ТокенГИСМТ),
		НСтр("ru = 'Не указан токен ГИСМТ'"), КонтекстПроверки);
	
	ПараметрШтрихкоды = РаботаСНоменклатурой.КлючиКоллекцииКакМассив(Штрихкоды, Истина);
	
	Если ПараметрШтрихкоды.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Результат      = ОписаниеТаблицыНоменклатуры();
	СервисДоступен = СервисПолученияДанныхПоШтрихкодамДоступен(КонтекстПроверки);
	ДобавитьДополнительныеКолонкиДанныхПоНоменклатуре(Результат);
	Если СервисДоступен Тогда
		ПолучитьДанныеНоменклатурыНациональногоКаталогаПорциями(Результат, ПараметрШтрихкоды, ПараметрыЗагрузки);
		Если Результат.Найти("", "Ошибка") <> Неопределено Тогда
			Отказ = Ложь;
			РаботаСНоменклатурой.ЗаполнитьДополнительныеДанныеНоменклатурыСервиса(Результат, Отказ);
			РаботаСНоменклатурой.УдалитьЛишниеКолонкиДанныхНоменклатурыСервиса(Результат);
		КонецЕсли;
	Иначе
		ЗаполнитьОшибку(Результат, ПараметрШтрихкоды, 
			РаботаСНоменклатурой.ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().СервисНедоступен);
	КонецЕсли;
	
	Для Каждого СтрокаРезультата Из Результат Цикл
		СтрокаРезультата.ДополнительныеПараметры = Штрихкоды[СтрокаРезультата.ЗапрошенныйШтрихкод];
	КонецЦикла;
...
// Формирует запрос выборки товарных позиций к подписанию в Национальном каталоге.
//
// Параметры:
//   ОбходРезультатов - Структура - 
//     * ВыборкаДанных - Неопределено, Число - индекс основной выборки в пакете запросов.
//     * Штрихкоды     - Неопределено, Число - индекс выборки штрихкодов товарных позиций в пакете запросов.
//
// Возвращаемое значение:
//   Запрос - запроса выборки.
//
Функция ИнициализацияЗапросаТоваровНациональногоКаталогаКПодписанию(ОбходРезультатов) Экспорт
	
	СхемаЗапроса = СхемаЗапросаВыгружаемаяНоменклатураСХарактеристиками();	// СхемаЗапроса
	
	ОсновнойЗапрос = СхемаЗапроса.ПакетЗапросов[СхемаЗапроса.ПакетЗапросов.Количество() - 1];
	ОсновнойЗапрос.Операторы[0].КоличествоПолучаемыхЗаписей = 
		РаботаСНоменклатуройСлужебныйКлиентСервер.ПолучитьЛимитЗапросовНациональногоКаталога();
	
	ОсновнойЗапрос.Операторы[0].ВыбираемыеПоля.Добавить("СостоянияВыгрузкиНоменклатуры.ХарактеристикаНоменклатуры");
	ОсновнойЗапрос.Колонки[ОсновнойЗапрос.Колонки.Количество() - 1].Псевдоним = "ХарактеристикаБезПреобразования";
	ОсновнойЗапрос.Операторы[0].ВыбираемыеПоля.Добавить("СостоянияВыгрузкиНоменклатуры.ИдентификаторПакетаДанных");
	ОсновнойЗапрос.Колонки[ОсновнойЗапрос.Колонки.Количество() - 1].Псевдоним = "ИдентификаторПакетаДанных";
	ОсновнойЗапрос.Операторы[0].ВыбираемыеПоля.Добавить("СостоянияВыгрузкиНоменклатуры.Состояние");
	ОсновнойЗапрос.Колонки[ОсновнойЗапрос.Колонки.Количество() - 1].Псевдоним = "Состояние";
	ОсновнойЗапрос.Операторы[0].ВыбираемыеПоля.Добавить("СостоянияВыгрузкиНоменклатуры.Хеш");
	ОсновнойЗапрос.Колонки[ОсновнойЗапрос.Колонки.Количество() - 1].Псевдоним = "Хеш";
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТоварыКПодписанию.Номенклатура КАК Номенклатура,
		|	ТоварыКПодписанию.ХарактеристикаНоменклатуры КАК ХарактеристикаНоменклатуры
		|ПОМЕСТИТЬ ТоварыКПодписанию
		|ИЗ
		|	&ТоварыКПодписанию КАК ТоварыКПодписанию
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		...
		|	ОтборНоменклатуры.Номенклатура КАК Номенклатура,
		|	ОтборНоменклатуры.ХарактеристикаБезПреобразования КАК Характеристика,
		|	ОтборНоменклатуры.ИдентификаторПакетаДанных КАК ИдентификаторПакетаДанных,
		|	ОтборНоменклатуры.Состояние КАК Состояние,
		|	ОтборНоменклатуры.Хеш КАК Хеш
		|ИЗ
		|	ОтборНоменклатуры КАК ОтборНоменклатуры";
	
	Если Не ИнициализацияЗапросаВыборкиДанныхДляВыгрузки(Запрос) Тогда
		ВызватьИсключение НСтр("ru = 'Невозможно получить карточки товаров Национального каталога к последующему подписанию. Не заполнен текст запроса.'");
	КонецЕсли;
	
	ОбходРезультатов.Вставить("ВыборкаДанных", Неопределено);
	
	// Исключить из запроса лишние таблицы, дополнить условием по товарам к подписанию
	СхемаЗапроса = Новый СхемаЗапроса;
	СхемаЗапроса.УстановитьТекстЗапроса(Запрос.Текст);
	
	КоличествоЗапросов = СхемаЗапроса.ПакетЗапросов.Количество();
	Для ОбратныйИндекс = 1 По КоличествоЗапросов Цикл
		ЗапросПакета = СхемаЗапроса.ПакетЗапросов[КоличествоЗапросов - ОбратныйИндекс];
		
		Если ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
				И Не ЗапросПакета.ТаблицаДляПомещения = "ТоварыКПодписанию"
				И Не ЗапросПакета.ТаблицаДляПомещения = "ОтборНоменклатуры"
				И Не ЗапросПакета.ТаблицаДляПомещения = ""
				И Не ОбходРезультатов.Свойство(ЗапросПакета.ТаблицаДляПомещения) Тогда
		...
		ИначеЕсли ТипЗнч(ЗапросПакета) = Тип("ЗапросУничтоженияТаблицыСхемыЗапроса")
					И Не ЗапросПакета.ИмяТаблицы = "ТоварыКПодписанию"
					И Не ЗапросПакета.ИмяТаблицы = "ОтборНоменклатуры" Тогда
			СхемаЗапроса.ПакетЗапросов.Удалить(КоличествоЗапросов - ОбратныйИндекс);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого ЗапросПакета Из СхемаЗапроса.ПакетЗапросов Цикл
		Если ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
				...
				И ОбходРезультатов.ВыборкаДанных = Неопределено Тогда
			ОбходРезультатов.Вставить("ВыборкаДанных", СхемаЗапроса.ПакетЗапросов.Индекс(ЗапросПакета));
			
		ИначеЕсли ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
					И ЗапросПакета.ТаблицаДляПомещения = "ОтборНоменклатуры" Тогда
			УсловиеКПодписанию = 
				"ИСТИНА В
				|	(ВЫБРАТЬ ПЕРВЫЕ 1
				|		ИСТИНА
				...
				|		ТоварыКПодписанию КАК ТоварыКПодписанию
				|	ГДЕ
				|		ТоварыКПодписанию.Номенклатура = СостоянияВыгрузкиНоменклатуры.Номенклатура
				|		И ТоварыКПодписанию.ХарактеристикаНоменклатуры = СостоянияВыгрузкиНоменклатуры.ХарактеристикаНоменклатуры)";
			ЗапросПакета.Операторы[0].Отбор.Добавить(УсловиеКПодписанию);
				
		ИначеЕсли ТипЗнч(ЗапросПакета) = Тип("ЗапросВыбораСхемыЗапроса")
					И Не ЗапросПакета.ТаблицаДляПомещения = ""
					И ОбходРезультатов.Свойство(ЗапросПакета.ТаблицаДляПомещения) Тогда
			ТекущаяТаблица = ЗапросПакета.ТаблицаДляПомещения;
			ОбходРезультатов[ТекущаяТаблица] = СхемаЗапроса.ПакетЗапросов.Индекс(ЗапросПакета);
			...
			Если КолонкаХарактеристика <> Неопределено Тогда
				ТекущийИндекс = ЗапросПакета.Колонки.Индекс(КолонкаХарактеристика);
				ШаблонПроверкиХарактеристики = "ЕСТЬNULL(%1, &ПустоеЗначениеХарактеристики)";
				ВыражениеХарактеристика = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ШаблонПроверкиХарактеристики, 
					Строка(ЗапросПакета.Операторы[0].ВыбираемыеПоля[ТекущийИндекс]));
				ЗапросПакета.Операторы[0].ВыбираемыеПоля[ТекущийИндекс] = Новый ВыражениеСхемыЗапроса(ВыражениеХарактеристика);
...
//
// Параметры:
//   Настройки             - см. РаботаСНоменклатуройСлужебный.НастройкаВыгрузкиНоменклатуры.
//   ИдентификаторыТоваров - Массив из Число - идентификаторы товаров в Национальном каталоге (goodIds).
//   Штрихкоды             - Массив из Строка - штрихкоды товаров в Национальном каталоге (gtins).
//   Отказ                 - Булево - признак ошибки при работе с сервисом.
//
// Возвращаемое значение:
//   Произвольный - возвращаемый результат обработки.
//
Функция ПолучитьТоварыНациональногоКаталогаКПодписанию(Настройки, ИдентификаторыТоваров = Неопределено, 
			Штрихкоды = Неопределено, Отказ = Ложь) Экспорт
	
	Результат        = НовыйРезультатДействия();
	ПараметрыКоманды = ПараметрыКомандыПолучитьТоварыНациональногоКаталогаКПодписанию(Настройки);
	
	Соединение = Неопределено;
	Настройки.Свойство("Соединение", Соединение);
	...
	Если Соединение = Неопределено Тогда
		...
		Если ПараметрыСоединения = Неопределено Тогда
			ПараметрыСоединения = ПараметрыСоединенияСНациональнымКаталогом(Настройки.ТестовыйРежим);
		КонецЕсли;
		...
		Исключение
			ТекстОшибки = НСтр("ru = 'Отсутствует соединение с сервисом Национального каталога.'");
			Отказ = Истина;
			
			Результат.ТекстСообщения = ТекстОшибки + " " + НСтр("ru = 'Подробности см. в журнале регистрации.'");
			ЗаписьЖурналаРегистрации(ПараметрыКоманды.Наименование,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ТекстОшибки
					+ Символы.ПС
					+ ПолучитьСтрокуURLЗапроса(Соединение, ПараметрыКоманды.Адрес)
					+ Символы.ПС
	...
	Данные = Новый Структура;
	Данные.Вставить("publicationAgreement", Настройки.СогласиеНаПубликациюТоваров);
	Если ЗначениеЗаполнено(ИдентификаторыТоваров) Тогда
		Данные.Вставить("goodIds", ИдентификаторыТоваров);
	КонецЕсли;
	Если ЗначениеЗаполнено(Штрихкоды) Тогда
		Данные.Вставить("gtins", Штрихкоды);
	КонецЕсли;
	
	ПараметрыКоманды.Вставить("СоединениеССервисом", Соединение);
	ПараметрыКоманды.Данные = ЗначениеВФорматJSON(Данные);
	
	Результат = ВыполнитьДействиеВПовторе("ЗапросКНациональномуКаталогу", ПараметрыКоманды);
	
	Возврат ОбработатьОтветСервиса(ПараметрыКоманды, 
...
// Получает информацию о собственных товарах из Национального каталога.
//
// Параметры:
//   Настройки             - см. РаботаСНоменклатуройСлужебный.НастройкаВыгрузкиНоменклатуры.
//   ИдентификаторыТоваров - Массив из Число - идентификаторы товаров в Национальном каталоге (goodIds).
...
//   Произвольный - возвращаемый результат обработки.
//
Функция ПолучитьСобственныеТоварыНациональногоКаталога(Настройки, ИдентификаторыТоваров = Неопределено, 
			Штрихкоды = Неопределено, Отказ = Ложь) Экспорт
	
	Результат        = НовыйРезультатДействия();
	ПараметрыКоманды = ПараметрыКомандыПолучитьСобственныеТоварыНациональногоКаталога(Настройки);
	...
	Если ЗначениеЗаполнено(ИдентификаторыТоваров) Тогда
		Если ИдентификаторыТоваров.Количество() = 1 Тогда
			ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "good_id", ИдентификаторыТоваров[0]);
		Иначе
			ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "good_ids", СтрЗаменить(СтрСоединить(ИдентификаторыТоваров, ";"), Символы.НПП, ""));
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Штрихкоды) Тогда
		Если Штрихкоды.Количество() = 1 Тогда
			ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "gtin", Штрихкоды[0]);
		Иначе
			ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "gtins", СтрЗаменить(СтрСоединить(Штрихкоды, ";"), Символы.НПП, ""));
		КонецЕсли;
	КонецЕсли;
...
// Параметры:
//   Настройки           - см. РаботаСНоменклатуройСлужебный.НастройкаВыгрузкиНоменклатуры.
//   ДанныеДляПодписания - Соответствие из КлючИЗначение - карточки товаров к подписанию:
//     * Ключ              - Число - идентификатор товара в Национальном каталоге.
//     * Значение          - Структура - данные для подписания:
...
//   Произвольный - возвращаемый результат обработки.
//
Функция ПодписатьТоварыНациональногоКаталога(Настройки, ДанныеДляПодписания, Отказ = Ложь) Экспорт
	
	Результат        = НовыйРезультатДействия();
	ПараметрыКоманды = ПараметрыКомандыПодписатьТоварыНациональногоКаталога(Настройки);
	...
	Для Каждого КлючИЗначение Из ДанныеДляПодписания Цикл
		...
		Товар.Вставить("goodId",    КлючИЗначение.Ключ);
		Товар.Вставить("base64Xml", КлючИЗначение.Значение.КарточкаТовара);
		Товар.Вставить("signature", КлючИЗначение.Значение.Подпись);
...
//   * ДопустимАнонимныйЗапрос - Булево - допустим ли анонимный запрос.
...
Функция ОписаниеПараметровКомандыСервиса() Экспорт
	...
	ПараметрыКоманды.Вставить("Заголовки", Новый Соответствие);
...
Функция ПреобразоватьЗначениеСервиса(Значение, Правило, КодСостояния = 200)
	...
	ИначеЕсли Правило = "CategoryResponse.categories.classifiers" Тогда
		...
		Результат = КоллекцияМассивовСтруктурВКоллекциюТаблицЗначений(Значение, ОписаниеТаблицыКлассификатора);
		Возврат Результат;
...
Функция ИмяПодсистемы()
	...
	Возврат ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().РаботаСНоменклатурой;
...
Функция НоваяНастройкаВыгрузкиНоменклатуры(Организация)
	...
	СтруктураНастройки.Вставить("ПодписыватьЭлектроннойПодписью", Ложь);
	СтруктураНастройки.Вставить("СогласиеНаПубликациюТоваров",    Ложь);
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|	Свойства.ТекстоваяСтрока КАК ТекстоваяСтрока,
...
Функция ИсключитьРеквизитыИзЗапроса(Знач ТекстЗапроса, Знач ИсключенныеРеквизиты)
	...
	Для ОбратныйИндекс = 1 По КоличествоЗапросов Цикл
		...
		Иначе
			Для Каждого ИсключенныйРеквизит Из ИсключенныеРеквизиты Цикл
				Если ЗапросПакета.ТаблицаДляПомещения = ИсключенныйРеквизит Тогда
					СхемаЗапроса.ПакетЗапросов.Удалить(КоличествоЗапросов - ОбратныйИндекс);
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Функция ПеречислениеСтавкиНДС()
	...
	ПеречислениеСтавкиНДС.Добавить("22", "НДС22");
...
Функция ВыгрузитьНоменклатуру(Запрос, Параметры)
	...
	ИндексыОбходаРезультатов.Вставить("СоставНаборов");
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаТаблицыТовары Из ОбходРезультатов.ТоварыСлужебный Цикл
			...
			ЭлементНоменклатура.Вставить("vat", "22");
			...
			Если ЗначениеЗаполнено(ОбходРезультатов.СвойстваСлужебный) Тогда
				...
				Для каждого СтрокаДанных Из ТаблицаДанных Цикл
					...
					Если ТипЗнч(ЗначениеСвойства) = Тип("Строка")
							И ЗначениеЗаполнено(СтрокаДанных.ТекстоваяСтрока) Тогда
						ЗначениеСвойства = СтрокаДанных.ТекстоваяСтрока;
					КонецЕсли;
			...
			// Состав наборов
			Если ЗначениеЗаполнено(ОбходРезультатов.СоставНаборов) Тогда
				ДанныеПакета = Новый СписокЗначений;
				ДанныеПакета.Добавить("gtin", "Штрихкод");
				ДанныеПакета.Добавить("quantity", "Количество");
				
				СтрокиВложеннойТаблицы = ОбходРезультатов.СоставНаборов.НайтиСтроки(ПоискСХарактеристикой);
				КоллекцияДанных        = КоллекцияДанныхВыгрузки(СтрокиВложеннойТаблицы, ДанныеПакета);
				Если КоллекцияДанных.Количество() > 0 Тогда
					ЭлементНоменклатура.Вставить("set", КоллекцияДанных);
				КонецЕсли;
			КонецЕсли;
...
Функция ВыполнитьДействиеВПовторе(Действие, ПараметрыДействия)
	...
	ИначеЕсли Действие = "ЗапросКНациональномуКаталогу" Тогда
		Результат = ЗапросКНациональномуКаталогу(ПараметрыДействия, ВыполнятьПовтор);
	Иначе 
		Результат = НовыйРезультатДействия();
	КонецЕсли;
...
Функция ТокенАутентификацииСервиса(Соединение, ПараметрыКоманды)
	...
	Если ОтветСервиса.Результат.КодСостояния <> КодСостоянияВыполнено() Тогда
		ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось получить токен аутентификации в сервисе %1.
				|Код состояния: %2;
				|Тело ответа: %3'"),
			Соединение.Сервер,
			ОтветСервиса.Результат.КодСостояния,
			ОтветСервиса.Результат.ПолучитьТелоКакСтроку());
		ВыполнитьЗаписьСобытияВЖурналРегистрации(ИнформацияОбОшибке, УровеньЖурналаРегистрации.Ошибка);
		
		ОтветСервиса.Результат      = Неопределено;
		ОтветСервиса.ТекстСообщения = НСтр("ru = 'Ошибка аутентификации в сервисе.
...
// Параметры:
//  ПараметрыКоманды - см. ОписаниеПараметровКомандыСервиса
//
Функция ВыполнитьЗапрос(ПараметрыКоманды)
	
	ПараметрыСоединения = ПараметрыКоманды.ПараметрыСоединенияССервисом;
	...
	Попытка
		Соединение = Новый HTTPСоединение(ПараметрыСоединения.ИмяСервера, ПараметрыСоединения.Порт,,,
			ПараметрыСоединения.Прокси, ПараметрыСоединения.Таймаут, ПараметрыСоединения.ЗащищенноеСоединение);
	Исключение
		ПодробноеОписаниеОшибки = СтрШаблон(НСтр("ru='Ошибка соединения с сервером %1, порт %2.
			|Подробно:
			|%3'"),
	...
	Запрос = Новый HTTPЗапрос(ПараметрыКоманды.Адрес, ПараметрыКоманды.Заголовки);
	
	Если ТипЗнч(ПараметрыКоманды.Данные) = Тип("ДвоичныеДанные") Тогда
		Запрос.УстановитьТелоИзДвоичныхДанных(ПараметрыКоманды.Данные);
	ИначеЕсли ТипЗнч(ПараметрыКоманды.Данные) = Тип("Строка") И Не ПустаяСтрока(ПараметрыКоманды.Данные) Тогда 
		Запрос.УстановитьТелоИзСтроки(ПараметрыКоманды.Данные);
	КонецЕсли;
	
	Возврат ОтветСервиса(Соединение, Запрос, ПараметрыКоманды);
...
Функция АдресСервисаПолученияДанныхПоШтрихкодам()
	
	Возврат "catalog-integration.1c.ru";
	
КонецФункции

Функция ПрефиксАдресаРесурсаСервисаПолученияДанныхПоШтрихкодам()
	
	Возврат "nc-1c-prod01/api/nc";
	
КонецФункции

Функция КодСостоянияНайдено()
	
	Возврат 302;
	
КонецФункции

Функция КодСостоянияВнутренняяОшибкаСервера()
	
	Возврат 500;
	
КонецФункции

Функция ИмяСлужебногоПоляЗапрошенныйШтрихкод()
	
	Возврат "requestedGTIN";
	
КонецФункции

Функция ИмяСлужебногоПоляОшибка()
	
	Возврат "error";
	
КонецФункции

Функция РазмерПорцииДанныхНациональногоКаталога()
...
//  Число
//
Функция ИнтервалПовторногоОбращенияПоУмолчанию()
...
Функция ПараметрыСоединенияССервисомПолученияДанныхПоШтрихкодам()

	ПараметрыСоединения = Новый Структура;
	
	ПараметрыСоединения.Вставить("ИмяСервера", АдресСервисаПолученияДанныхПоШтрихкодам());
	ПараметрыСоединения.Вставить("Порт",       443);
	ПараметрыСоединения.Вставить("Таймаут",    30);
	ПараметрыСоединения.Вставить("Прокси",     ПолучениеФайловИзИнтернета.ПолучитьПрокси("https"));
	ПараметрыСоединения.Вставить("ЗащищенноеСоединение",
...
Функция ПараметрыКомандыСоздатьЗаданиеВСервисеНаПолучениеДанныхПоШтрихкодам()
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Адрес = СтрШаблон("%1/product", ПрефиксАдресаРесурсаСервисаПолученияДанныхПоШтрихкодам());
	ПараметрыКоманды.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыКоманды.ПараметрыСоединенияССервисом = ПараметрыСоединенияССервисомПолученияДанныхПоШтрихкодам();
	ПараметрыКоманды.Наименование = НСтр("ru = 'Создание задания на получение данных по штрихкодам'");
	ПараметрыКоманды.Таймаут = 60;
...
Функция ПараметрыКомандыПерейтиПоАдресуПеренаправленияСервисеНаПолучениеДанныхПоШтрихкодам()
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	ПараметрыКоманды.ПараметрыСоединенияССервисом = ПараметрыСоединенияССервисомПолученияДанныхПоШтрихкодам();
	ПараметрыКоманды.Наименование = НСтр("ru = 'Переход по адресу перенаправления'");
	ПараметрыКоманды.Таймаут = 60;
...
Функция ПараметрыКомандыСервисПолученияДанныхПоШтрихкодамДоступен()
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Адрес = ПрефиксАдресаРесурсаСервисаПолученияДанныхПоШтрихкодам() + "/ping";
	ПараметрыКоманды.Метод = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	ПараметрыКоманды.ПараметрыСоединенияССервисом = ПараметрыСоединенияССервисомПолученияДанныхПоШтрихкодам();
	ПараметрыКоманды.Наименование = НСтр("ru = 'Проверка доступности сервиса'");
	ПараметрыКоманды.Таймаут = 5;
...
Функция СервисПолученияДанныхПоШтрихкодамДоступен(КонтекстПроверки)
	
	СервисДоступен   = Ложь;
	ПараметрыКоманды = ПараметрыКомандыСервисПолученияДанныхПоШтрихкодамДоступен();
	ОтветСервиса     = ВыполнитьЗапрос(ПараметрыКоманды);
	
	Если ОтветСервиса = Неопределено Тогда
		ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Не удалось проверить доступность сервиса'"), 
			КонтекстПроверки);
	ИначеЕсли ОтветСервиса.КодСостояния <> КодСостоянияВыполнено() Тогда
		ЗаписатьВЖурналРегистрации(
			СтрШаблон(НСтр("ru = 'Сервис недоступен. Код ответа %1 отличается от ожидаемого %2'"), 
				ОтветСервиса.КодСостояния,
				КодСостоянияВыполнено()), 
			КонтекстПроверки);
	Иначе
...
Функция АдресПеренаправленияЗаполнен(Знач АдресПеренаправления)
	
	Возврат ТипЗнч(АдресПеренаправления) = Тип("Строка") И Не ПустаяСтрока(АдресПеренаправления);
	
КонецФункции

// Параметры:
//  Штрихкоды - Массив из Строка
//  ПараметрыЗагрузки - см. РаботаСНоменклатуройКлиентСервер.НовыеПараметрыПолученияНоменклатурыСервисаПоШтрихкодам
// 
// Возвращаемое значение:
//  - Неопределено
...
//    * ИнтервалПовторногоОбращения - Число
//
Функция СоздатьЗаданиеВСервисеНаПолучениеДанныхПоШтрихкодам(Штрихкоды, ПараметрыЗагрузки)
	
	КонтекстПроверки = "РаботаСНоменклатуройСлужебный.СоздатьЗаданиеВСервисеНаПолучениеДанныхПоШтрихкодам";
	ПараметрыКоманды = ПараметрыКомандыСоздатьЗаданиеВСервисеНаПолучениеДанныхПоШтрихкодам();
	ПараметрыКоманды.Данные = ЗначениеВФорматJSON(Штрихкоды);
	ПараметрыКоманды.Заголовки.Вставить("TokenGISMT", ПараметрыЗагрузки.ТокенГИСМТ);
	
	ОтветСервиса = ВыполнитьЗапрос(ПараметрыКоманды);
	
	Если ОтветСервиса = Неопределено Тогда
		ЗаписатьВЖурналРегистрации(
			НСтр("ru = 'Не удалось создать задание на получение данных по штрихкодам в сервисе'"), 
			КонтекстПроверки);
		Возврат Неопределено;
	ИначеЕсли ОтветСервиса.КодСостояния <> КодСостоянияОжидание() Тогда
		ЗаписатьВЖурналРегистрации(
			СтрШаблон(НСтр("ru = 'Код ответа сервиса %1 отличается от ожидаемого %2'"), 
				ОтветСервиса.КодСостояния,
	...
	Результат = Новый Структура;
	Результат.Вставить("АдресПеренаправления", ИнтернетСоединениеБЭД.АдресПеренаправления(ОтветСервиса));
	Результат.Вставить("ИнтервалПовторногоОбращения",
		ИнтернетСоединениеБЭД.ИнтервалПовторногоОбращения(ОтветСервиса, ИнтервалПовторногоОбращенияПоУмолчанию()));
	Результат.Вставить("Штрихкоды", ОбщегоНазначения.СкопироватьРекурсивно(Штрихкоды));
	
	Возврат Результат;
	
КонецФункции

Процедура ЗаписатьВЖурналРегистрации(Комментарий, Контекст = "", Уровень = Неопределено)
	
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(Комментарий + Символы.ПС + Контекст, ИмяПодсистемы(), Уровень);
	
КонецПроцедуры

// Параметры:
//  ПараметрыЗадания - см. ПараметрыЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам
//  ГраничноеВремя - Дата,Неопределено
//
// Возвращаемое значение:
//  Строка,Неопределено
//
Функция АдресРезультатаЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам(ПараметрыЗадания, ГраничноеВремя = Неопределено)
	
	Результат        = Неопределено;
	Интервал         = ПараметрыЗадания.ИнтервалПовторногоОбращения;
	КонтекстПроверки = "РаботаСНоменклатуройСлужебный.АдресРезультатаЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам";
	ПараметрыКоманды = ПараметрыКомандыПерейтиПоАдресуПеренаправленияСервисеНаПолучениеДанныхПоШтрихкодам();
	ПараметрыКоманды.Адрес = ПараметрыЗадания.АдресПеренаправления;
	
	Если ГраничноеВремя = Неопределено Тогда
		ГраничноеВремя = ТекущаяДатаСеанса() + МаксимальнаяДлительностьВыполненияЗапроса();
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		Ответ = ВыполнитьЗапрос(ПараметрыКоманды);
		
		Если Ответ.КодСостояния = КодСостоянияОжидание() Тогда
			Интервал   = ИнтернетСоединениеБЭД.ИнтервалПовторногоОбращения(Ответ, Интервал);
			НовыйАдрес = ИнтернетСоединениеБЭД.АдресПеренаправления(Ответ);
			Если ЗначениеЗаполнено(НовыйАдрес) И НовыйАдрес <> ПараметрыКоманды.Адрес Тогда
				ПараметрыКоманды.Адрес = НовыйАдрес;
			КонецЕсли;
		ИначеЕсли Ответ.КодСостояния = КодСостоянияНайдено() Тогда
			Результат = ИнтернетСоединениеБЭД.АдресПеренаправления(Ответ);
			Прервать;
		ИначеЕсли Ответ.КодСостояния = КодСостоянияВнутренняяОшибкаСервера() Тогда
			ЗаписатьВЖурналРегистрации(СтрШаблон(НСтр("ru='Внутренняя ошибка сервера (500). Подробно:%1'"),
				Символы.ПС + Ответ.ПолучитьТелоКакСтроку()), КонтекстПроверки);
			Прервать;
		Иначе
			ЗаписатьВЖурналРегистрации(
				СтрШаблон(
					НСтр("ru='Необрабатываемый ответ сервера. Подробно:
						|Код состояния: %1
						|Тело ответа: %2'"),
					Ответ.КодСостояния, 
		...
		Если ТекущаяДатаСеанса() >= ГраничноеВремя Тогда
			ЗаписатьВЖурналРегистрации(НСтр("ru='Превышено время ожидания ответа сервиса конвертации'"),
				КонтекстПроверки);
			Прервать;
		КонецЕсли;
...
Функция ПредставлениеОшибкиСервиса(Знач Ошибка)
	
	Если ПустаяСтрока(Ошибка) Тогда
		Возврат "";
	КонецЕсли;
	
	Ошибки = РаботаСНоменклатурой.ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам();
	
	ОбрабатываемыеОшибкиСервиса = Новый Соответствие;
	ОбрабатываемыеОшибкиСервиса.Вставить("НеНайдено", Ошибки.НеНайдено);
	ОбрабатываемыеОшибкиСервиса.Вставить("ОшибкаАвторизацииНК", Ошибки.ОшибкаАвторизацииНК);
	ОбрабатываемыеОшибкиСервиса.Вставить("НеизвестнаяОшибкаНК", Ошибки.НеизвестнаяОшибкаНК);
	ОбрабатываемыеОшибкиСервиса.Вставить("ОшибкаКонвертации", Ошибки.ОшибкаКонвертации);
	
	Представление = ОбрабатываемыеОшибкиСервиса[Ошибка];
	Если Представление = Неопределено Тогда
		Представление = Ошибки.НеизвестнаяОшибкаСервиса;
	КонецЕсли;
...
//  Массив из Структура
//
Функция РезультатЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам(АдресРезультата)
	
	ДанныеКакИзСервиса = Новый Массив();
	КонтекстПроверки   = "РаботаСНоменклатуройСлужебный.РезультатЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам";
	
	Если Не АдресПеренаправленияЗаполнен(АдресРезультата) Тогда
		ЗаписатьВЖурналРегистрации(НСтр("ru='Не заполнен адрес перенаправления'"), КонтекстПроверки);
		Возврат ДанныеКакИзСервиса;
	КонецЕсли;
	
	ПараметрыКоманды       = ПараметрыКомандыПерейтиПоАдресуПеренаправленияСервисеНаПолучениеДанныхПоШтрихкодам();
	ПараметрыКоманды.Адрес = АдресРезультата;
	
	ОтветСервиса = ВыполнитьЗапрос(ПараметрыКоманды);
	КодСостояния = ОтветСервиса.КодСостояния;
	ДанныеОтвета = ЗначениеИзСтрокиJSON(ОтветСервиса.ПолучитьТелоКакСтроку());
	
	Если КодСостояния = КодСостоянияВыполнено() 
		И ТипЗнч(ДанныеОтвета) = Тип("Массив") Тогда
		Для Каждого ЭлементОтвета Из ДанныеОтвета Цикл
			Если ТипЗнч(ЭлементОтвета) <> Тип("Структура") Тогда
				Продолжить;
			КонецЕсли;
			Если ТипЗнч(ЭлементОтвета.data) <> Тип("Структура") Тогда
				ЭлементОтвета.data = Новый Структура();
			КонецЕсли;
			ЭлементОтвета.data.Вставить(ИмяСлужебногоПоляОшибка(), ПредставлениеОшибкиСервиса(ЭлементОтвета.error));
			ЭлементОтвета.data.Вставить(ИмяСлужебногоПоляЗапрошенныйШтрихкод(), ЭлементОтвета.barcode);
			ДанныеКакИзСервиса.Добавить(ЭлементОтвета.data);
		КонецЦикла;
	КонецЕсли;
...
Процедура ДобавитьДополнительныеКолонкиДанныхПоНоменклатуре(ДанныеПоНоменклатуре)
...
Процедура ЗаполнитьОшибку(Результат, Штрихкоды, Ошибка)
	
	Для Каждого Штрихкод Из Штрихкоды Цикл
		ЗаполнитьЗначенияСвойств(Результат.Добавить(), Новый Структура("ЗапрошенныйШтрихкод, Ошибка", Штрихкод, Ошибка));
	КонецЦикла;
	
КонецПроцедуры

Процедура ПолучитьДанныеНоменклатурыНациональногоКаталогаПорциями(Результат, ПараметрШтрихкоды, ПараметрыЗагрузки)
	
	РазмерПорции     = РазмерПорцииДанныхНациональногоКаталога();
	Порция           = Новый Массив;
	Остаток          = ПараметрШтрихкоды.Количество();
	ПараметрыЗаданий = Новый Массив;
	Ошибка           = РаботаСНоменклатурой.ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().НеизвестнаяОшибкаСервиса;
	Для Каждого Штрихкод Из ПараметрШтрихкоды Цикл
		Порция.Добавить(Штрихкод);
		
		Если Порция.Количество() = Мин(РазмерПорции, Остаток) Тогда
			ПараметрыЗадания = СоздатьЗаданиеВСервисеНаПолучениеДанныхПоШтрихкодам(Порция, ПараметрыЗагрузки);
			Если ПараметрыЗадания = Неопределено Тогда
				ЗаполнитьОшибку(Результат, Порция, Ошибка);
			Иначе
				ПараметрыЗаданий.Добавить(ПараметрыЗадания);
			КонецЕсли;
			Порция.Очистить();
		КонецЕсли;
	...
	Обработка = ОписаниеТаблицыНоменклатуры();
	
	Для Каждого ПараметрыЗадания Из ПараметрыЗаданий Цикл
		АдресРезультата  = АдресРезультатаЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам(ПараметрыЗадания);
		Если АдресРезультата = Неопределено Тогда
			ЗаполнитьОшибку(Результат, ПараметрыЗадания.Штрихкоды, Ошибка);
			Продолжить;
		КонецЕсли;
		РезультатЗадания = РезультатЗаданияВСервисеНаПолучениеДанныхПоШтрихкодам(АдресРезультата);
		Если РезультатЗадания.Количество() = 0 Тогда
			ЗаполнитьОшибку(Результат, ПараметрыЗадания.Штрихкоды, Ошибка);
			Продолжить;
		КонецЕсли;
		ТекущийРезультат = МассивСтруктурВТаблицуЗначений(РезультатЗадания, Обработка);
		Для Каждого СтрокаРезультата Из ТекущийРезультат Цикл
			ЗаполнитьЗначенияСвойств(Результат.Добавить(), СтрокаРезультата);
		КонецЦикла;
	КонецЦикла;
...
Функция ПараметрыСоединенияСНациональнымКаталогом(ТестовыйРежим = Ложь)
	
	ПараметрыСоединения = Новый Структура;
	Если ТестовыйРежим Тогда
		ПараметрыСоединения.Вставить("ИмяСервера", "api.integrators.nk.crptech.ru");
	Иначе
		ПараметрыСоединения.Вставить("ИмяСервера", "апи.национальный-каталог.рф");
	КонецЕсли;
	ПараметрыСоединения.Вставить("Порт",    443);
	ПараметрыСоединения.Вставить("Таймаут", 15);
	ПараметрыСоединения.Вставить("Прокси",  ПолучениеФайловИзИнтернета.ПолучитьПрокси("https"));
	ПараметрыСоединения.Вставить("ЗащищенноеСоединение",
		ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(, Новый СертификатыУдостоверяющихЦентровОС));
...
Процедура ОшибкиНациональногоКаталога(ПараметрыКоманды)
	
	ОшибкиСервиса = Новый Соответствие;
	ОшибкиСервиса.Вставить(400, НСтр("ru = 'Ошибка в параметрах запроса.'"));
	ОшибкиСервиса.Вставить(401, НСтр("ru = 'Не авторизован, в запросе отсутствует apikey/token.'"));
	ОшибкиСервиса.Вставить(403, НСтр("ru = 'Нет доступа к запрашиваемой информации.'"));
	ОшибкиСервиса.Вставить(404, НСтр("ru = 'Запрашиваемая информация не найдена.'"));
	ОшибкиСервиса.Вставить(413, НСтр("ru = 'Размер запроса превышает лимит.'"));
	ОшибкиСервиса.Вставить(429, НСтр("ru = 'Превышен лимит запросов к api.'"));
	ОшибкиСервиса.Вставить(500, НСтр("ru = 'Внутренняя ошибка сервера.'"));
	ОшибкиСервиса.Вставить(501, НСтр("ru = 'Метод не существует.'"));
	ОшибкиСервиса.Вставить(503, НСтр("ru = 'Сервис недоступен, техническое обслуживание, повторите запрос позже..'"));
...
Функция ЗапросКНациональномуКаталогу(ПараметрыКоманды, ВыполнятьПовтор)
	...
	Если ЗначениеЗаполнено(ПараметрыКоманды.Данные) Тогда
		Если ТипЗнч(ПараметрыКоманды.Данные) = Тип("ДвоичныеДанные") Тогда
			Данные = ПараметрыКоманды.Данные;	// ДвоичныеДанные
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка выполнения запроса к сервису Национального каталога.'");
		Ответ = Неопределено;
...
Функция ПараметрыКомандыПолучитьТоварыНациональногоКаталогаКПодписанию(Настройки)
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Наименование                 = НСтр("ru = 'Национальный каталог. Получить XML для последующей подписи карточки'");
	ПараметрыКоманды.Адрес                        = "/v3/feed-product-document";
	ПараметрыКоманды.Метод                        = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыКоманды.Таймаут                      = 5;
	ПараметрыКоманды.ДопустимАнонимныйЗапрос      = Ложь;
	ПараметрыКоманды.Аутентификация               = Ложь;
	ПараметрыКоманды.ТипАутентификации            = "";
	ПараметрыКоманды.Результат                    = "result";
	ПараметрыКоманды.ДопустимоПустоеТелоОтвета    = Ложь;
	ПараметрыКоманды.ПараметрыСоединенияССервисом = ПараметрыСоединенияСНациональнымКаталогом(Настройки.ТестовыйРежим);
	
	ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "apikey", Настройки.apikey);
	ДобавитьПараметрАдресаHTML(ПараметрыКоманды.Адрес, "format", "json");
	
	ОшибкиНациональногоКаталога(ПараметрыКоманды);
	
	Возврат ПараметрыКоманды;
	
КонецФункции

Функция ПараметрыКомандыПолучитьСобственныеТоварыНациональногоКаталога(Настройки)
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Наименование                 = НСтр("ru = 'Национальный каталог. Получить информацию о собственной карточке'");
	ПараметрыКоманды.Адрес                        = "/v3/feed-product";
	ПараметрыКоманды.Метод                        = ИнтернетСоединениеБЭД.HTTPМетоды().GET;
	ПараметрыКоманды.Таймаут                      = 5;
	ПараметрыКоманды.ДопустимАнонимныйЗапрос      = Ложь;
	ПараметрыКоманды.Аутентификация               = Ложь;
...
Функция ПараметрыКомандыПодписатьТоварыНациональногоКаталога(Настройки)
	
	ПараметрыКоманды = ОписаниеПараметровКомандыСервиса();
	ПараметрыКоманды.Наименование                 = НСтр("ru = 'Национальный каталог. Подписать карточку с использованием открепленной подписи'");
	ПараметрыКоманды.Адрес                        = "/v3/feed-product-sign-pkcs";
	ПараметрыКоманды.Метод                        = ИнтернетСоединениеБЭД.HTTPМетоды().POST;
	ПараметрыКоманды.Таймаут                      = 5;
	ПараметрыКоманды.ДопустимАнонимныйЗапрос      = Ложь;
	ПараметрыКоманды.Аутентификация               = Ложь;
