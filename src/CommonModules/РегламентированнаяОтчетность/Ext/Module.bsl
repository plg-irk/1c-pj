...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ОбязательностьЭДООрганизаций");
...
#Область ОбменПоСЭДО
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ЖурналСобытийОбменаСЭДО");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.СоответствиеСообщенийСЭДОЗапросам");
...
#КонецОбласти
	...
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.АктуальностьСпискаЗадачБухгалтера");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ВариантыПримененияТребованийЗаконодательства");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.ЗадачиБухгалтера");
	ВставитьОбъектМетаданныхПриЕгоНаличииВСпискиСОграничениемДоступа(Списки,
		"РегистрСведений.НалогиОтчеты");
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.АктуальностьСпискаЗадачБухгалтера");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.ВариантыПримененияТребованийЗаконодательства");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.ЗадачиБухгалтера");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.НалогиОтчеты");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
...
#Область ОбменПоСЭДО
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.ЖурналСобытийОбменаСЭДО");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени("РегистрСведений.СоответствиеСообщенийСЭДОЗапросам");
	Если НЕ ОбъектМетаданных = Неопределено Тогда
...
Процедура ДобавитьВДеревоРегламентированныхОтчетовОтчетыИзТаблицы(ДеревоОтчетов, ТаблицаОписанияОтчетов) Экспорт
	...
	Для Каждого СтрокаТаблицыОписанияОтчетов Из ТаблицаОписанияОтчетов Цикл
		...
		Если МетаОбъект <> Неопределено И ЗначениеЗаполнено(СтрокаТаблицыОписанияОтчетов.ГруппаВДереве) Тогда
			...
			ЗначенияКолонок.Вставить("ВидОО", СтрокаТаблицыОписанияОтчетов.ВидЭлектронногоПредставления);
...
Процедура ДобавитьКнопкуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчет") > 0
			...
			Если НЕ Форма.Элементы.Найти("ГруппаКнопокДляПрисоединенныхФайлов") = Неопределено Тогда		
			
				Группа = Форма.Элементы.Вставить("ГруппаПрисоединенныеФайлы",
					Тип("ГруппаФормы"), Форма.Элементы.ГруппаКнопокДляПрисоединенныхФайлов);
					
			ИначеЕсли НЕ Форма.Элементы.Найти("ФормаКоманднаяПанель") = Неопределено Тогда
...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	...
	ЭтоОтчетМинобороны = Реквизиты.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ"
		...
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма1д"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма2д"
		Или Реквизиты.ИсточникОтчета = "РегламентированныйОтчетГОЗФорма3д";
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	Если СтрЧислоВхождений(ОрганизацияОтчета, НСтр("ru = '<Объект не найден>'")) > 0 Тогда
		ОрганизацияОтчета = "";
	КонецЕсли;
	...
	ПараметрыОтчета.Вставить("СтатусОтправки",           ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусВРаботеСтрокой());
...
Функция ДоступнаПодсистема85() Экспорт 
	Возврат Ложь;
КонецФункции
...
Функция СформироватьСтруктуруОбязательныхРеквизитовСтатистическогоОтчета(Организация = Неопределено) Экспорт 
	...
	СтруктураРеквизитовФормы.Вставить("ДоступнаПодсистема85", ДоступнаПодсистема85());
...
Функция ВидКонтролирующегоОргана(Знач ИмяОтчета) Экспорт
	...
	Иначе
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ИсполнениеКонтрактовГОЗ") > 0 
			...
			Или СтрНайти(ИмяОтчета, "ЦенаПродукцииГОЗ") > 0 
			Или СтрНайти(ИмяОтчета, "ГОЗФорма1д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма2д") > 0
			Или СтрНайти(ИмяОтчета, "ГОЗФорма3д") > 0 Тогда
...
Процедура ЗаполнитьДанныеВШапкеОбособленныхПодразделений(Форма, ИмяТД = "ПолеТабличногоДокументаФормаОтчета") Экспорт
	...
	Если ТабДок.Области.Найти("ОбособленноеПодразделение") <> Неопределено Тогда
		...
		Если ИсключитьДанныеОбособленныхПодразделений Тогда
			...
			ОблОбособленноеПодразделение.ЦветФона = Цвета.ЦветЗаполняетсяАвтоматически;
		Иначе
...
#Область УправлениеВидимостьюКнопкиЗаполнить

// Конструктор настроек видимости элемента формы.
//
// Возвращаемое значение:
//   Структура - Новая структура настроек видимости элементов формы:
//     * ПостояннаяВидимость - Булево - признак необходимости вывода кнопки независимо от поддержки заполнения в потребителе;
//     * ТекстПредупреждения - Строка - сообщение, выводимое пользователю при нажатии кнопки, если не заполнено, выводится сообщение по умолчанию;
//     * ПоказыватьВФорме - Булево - признак вывода сообщения пользователю в специальной форме;
//     * ОбщаяФормаЗаголовок - Строка - заголовок формы с выводимым сообщением;
//     * ОбщаяФормаСсылка - Строка - гиперссылка для перехода из формы с выводимым сообщением.
//
Функция НоваяСтруктураНастроекВидимостиЭлементаФормы() Экспорт
	
	НастройкаВидимости = Новый Структура;
	НастройкаВидимости.Вставить("ПостояннаяВидимость", Ложь);
	НастройкаВидимости.Вставить("ТекстПредупреждения", "");
	НастройкаВидимости.Вставить("ПоказыватьВФорме", Ложь);
	НастройкаВидимости.Вставить("ОбщаяФормаЗаголовок", "");
	НастройкаВидимости.Вставить("ОбщаяФормаСсылка", "");
...
Функция НастройкиВидимостиЭлементовФормы(Форма, НастройкиВидимости = Неопределено) Экспорт
	...
	РазложениеПолногоИмениФормы = СтрРазделить(Форма.ИмяФормы, ".");
	ИмяОтчета = РазложениеПолногоИмениФормы[1];
	ИмяФормы = РазложениеПолногоИмениФормы[3];
	
	НастройкиВидимости = НоваяСтруктураНастроекВидимостиЭлементаФормы();
	РегламентированнаяОтчетностьПереопределяемый.ПолучитьНастройкиВидимостиЭлементовФормы(
		ИмяОтчета, ИмяФормы, НастройкиВидимости);
...
Процедура ПриИнициализации(Форма, Объект) Экспорт
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		Ссылка = ?(ОбщегоНазначения.ЗначениеСсылочногоТипа(Объект), Объект, Объект.Ссылка);
...
Процедура УправлениеДоступностьюРеквизитов(Форма, БлокируемыеЭлементы, ТолькоПросмотр) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		...
		Если НЕ Форма.Элементы.Найти("ФормаНастроитьЗаполнение") = Неопределено Тогда
			БлокируемыеЭлементы.Добавить("ФормаНастроитьЗаполнение");
		КонецЕсли;
...
Процедура ПриЧтенииНаСервере(Форма, Объект)
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		Если ТипЗнч(Объект) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			ГруппаДляКнопкиПроцесс = ОпределитьГруппуКнопкиПОД(Форма);
		Иначе
...
Функция ОпределитьГруппуКнопкиПОД(Форма)
	Если Форма.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Нет Или Форма.КоманднаяПанель.Видимость = Ложь Тогда
		ГруппаКнопки = Форма.Элементы.Найти("ГруппаКнопки");
		Если ТипЗнч(ГруппаКнопки) = Тип("ГруппаФормы") Тогда 
			Возврат ГруппаКнопки;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапретРедактированияОткрытогоОтчета(Форма, МассивНедоступных)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗапретРедактированияРеквизитовОбъектов") Тогда
		...
		МодульЗапретРедактированияРеквизитовОбъектов.ЗаблокироватьРеквизиты(Форма,,
			НСтр("ru = '<Не используется>'"), Форма.СтруктураРеквизитовФормы.мСохраненныйДок);
	КонецЕсли;
...
Процедура БлокировкаИлиЗапретРедактированияОтчета(Форма, СписокНедоступных = Неопределено) Экспорт 
	Попытка
		Если ПравоДоступа("Изменение", Метаданные.Документы.РегламентированныйОтчет) Тогда 
		...
		Иначе
			НеобходимаБлокировка = Истина;
			ОписаниеОшибки = "Отсутствуют права на запись, отчет открыт в режиме только просмотр";
		КонецЕсли;
	Исключение
		НеобходимаБлокировка = Истина;
...
Процедура РазрешитьРедактированиеРеквизитовОтчета(Форма) Экспорт
	...
	Форма.СтруктураРеквизитовФормы.Вставить("ЗаблокированоВДругомСеансе", Ложь);
КонецПроцедуры
...
Процедура ДополнитьДеревоОтчетовЭлементамиПатчей(ДеревоВсехИспользуемыхОтчетов) Экспорт 
	Попытка
		...
		Для Каждого Стр Из ТаблицаОписанияОтчетов Цикл
			...
			ЗначенияКолонок.Вставить("ВидОО", Стр.ВидЭлектронногоПредставления);
...
#Область ПредложениеКонвертацииОтчетаПриОткрытии

Процедура ФормированиеДанныхПоАктуальностиФормы(Форма, КонтролирующийОрган = "ФНС") Экспорт 
	СвойстваФормы = Новый Структура("СтруктураРеквизитовФормы");
	ЗаполнитьЗначенияСвойств(СвойстваФормы, Форма);
	...
	Форма.СтруктураРеквизитовФормы.КонвертацияИзФормыОнлайнБлокировок = 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.Параметры, "КонвертацияИзФормыОнлайнБлокировок", Ложь);
	
	СохраненныйДок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СвойстваФормы.СтруктураРеквизитовФормы, "мСохраненныйДок");
	Если ТипЗнч(СохраненныйДок) <> Тип("ДокументСсылка.РегламентированныйОтчет") Или НЕ ЗначениеЗаполнено(СохраненныйДок)
		Или ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СвойстваФормы.СтруктураРеквизитовФормы, "ЗаблокированоВДругомСеансе", Ложь) Тогда
		Возврат;
	КонецЕсли;
	
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СохраненныйДок,
		"Организация,ДатаНачала,ДатаОкончания,ВыбраннаяФорма,ИсточникОтчета");
	СтатусОтправки = Неопределено;
	СостояниеСдачиОтчетности = Неопределено;
	ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОпределитьСтатусИСостояниеСдачиОтчетности(
		СохраненныйДок, РеквизитыДокумента.Организация, КонтролирующийОрган, СостояниеСдачиОтчетности, СтатусОтправки);
		
	СтатусОтправкиВЖурнале = СохраненныйСтатусОтправкиРеглОтчета(СохраненныйДок);
	Если СтатусОтправки <> ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусВРаботеСтрокой() 
		Или СостояниеСдачиОтчетности <> Перечисления.СостояниеСдачиОтчетности.ДокументооборотНеНачат Тогда 
		Возврат;
	КонецЕсли;
	
	МенеджерОтчета = ?(СтрНачинаетсяС(НРег(Форма.ИмяФормы), "внешний"),
		РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(Форма.ИмяФормы), Отчеты[РеквизитыДокумента.ИсточникОтчета]);
	...
	Для Каждого Стр Из ТаблицаФормОтчета.НайтиСтроки(Новый Структура("ФормаОтчета", РеквизитыДокумента.ВыбраннаяФорма)) Цикл 
		Если НачалоДня(Стр.ДатаНачалоДействия) <= РеквизитыДокумента.ДатаНачала 
			...
			ФормаАктуальна = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ФормаАктуальна Тогда 
		Возврат;
	КонецЕсли;
	
	ФормыСМеханизмомКонвертации = МенеджерОтчета.СписокФормСМеханизмомКонвертации();
	АктуальныеФормы = Новый Массив;
	Для Каждого Стр Из ТаблицаФормОтчета Цикл 
		Если НачалоДня(Стр.ДатаНачалоДействия) <= РеквизитыДокумента.ДатаНачала 
			И (КонецДня(Стр.ДатаКонецДействия) >= РеквизитыДокумента.ДатаОкончания Или Не ЗначениеЗаполнено(Стр.ДатаКонецДействия)) Тогда 
			
			Форма.СтруктураРеквизитовФормы.АктуальныеФормы.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Стр));
			АктуальныеФормы.Добавить(Стр.ФормаОтчета);
			Если ФормыСМеханизмомКонвертации.Найти(Стр.ФормаОтчета) <> Неопределено Тогда 
				Форма.СтруктураРеквизитовФормы.ДоступныеДляКонвертацииФормы.Добавить(ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Стр))
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.ДоступныеДляКонвертацииФормы) 
		И Не ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.АктуальныеФормы) Тогда 
		Возврат;
	КонецЕсли;
	
	Форма.СтруктураРеквизитовФормы.ЕстьАктуальныеФормы = НаличиеАктуальныхФорм(РеквизитыДокумента, АктуальныеФормы);
	Форма.СтруктураРеквизитовФормы.ПредлагатьКонвертацию = ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.ДоступныеДляКонвертацииФормы);
КонецПроцедуры

Функция ЗапросАктуальныхФорм(РеквизитыДокумента, АктуальныеФормы) Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РегламентированныйОтчет.Ссылка КАК Ссылка
	|ИЗ
	...
	|	И РегламентированныйОтчет.ДатаОкончания = &ДатаОкончания
	|	И РегламентированныйОтчет.ИсточникОтчета = &ИсточникОтчета
	|	И РегламентированныйОтчет.ВыбраннаяФорма В(&АктуальныеФормы)";
	
	Запрос.УстановитьПараметр("Организация", РеквизитыДокумента.Организация);
	Запрос.УстановитьПараметр("ДатаНачала", РеквизитыДокумента.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", РеквизитыДокумента.ДатаОкончания);
	Запрос.УстановитьПараметр("ИсточникОтчета", РеквизитыДокумента.ИсточникОтчета);
	Запрос.УстановитьПараметр("АктуальныеФормы", АктуальныеФормы);
	Возврат Запрос;
КонецФункции

Функция НаличиеАктуальныхФорм(РеквизитыДокумента, АктуальныеФормы)
	Запрос = ЗапросАктуальныхФорм(РеквизитыДокумента, АктуальныеФормы);
	Выборка = Запрос.Выполнить().Выбрать();
	Возврат Выборка.Следующий();
КонецФункции

Процедура СозданиеСтандартныхРеквизитовОсновнойФормы(Форма) Экспорт 
	ОписаниеТиповБулево = Новый ОписаниеТипов("Булево");
	
	ДобавляемыеРеквизиты = Новый Массив;
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("КонвертацияИзФормыОнлайнБлокировок", ОписаниеТиповБулево));
	Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Форма["КонвертацияИзФормыОнлайнБлокировок"] = 
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.Параметры, "КонвертацияИзФормыОнлайнБлокировок", Ложь);
КонецПроцедуры
