...
Функция ТребуетсяПроверитьЛегальностьПолученияОбновления() Экспорт
	...
	// +РАРУС Обработаем в зависимости от наличия макетов у обработки проверки легальности получения конфигурации.
	Если Метаданные.Обработки.ЛегальностьПолученияОбновлений.Макеты.Найти("УсловияРаспространенияПродукта")=Неопределено Тогда
		Если Метаданные.Обработки.ЛегальностьПолученияОбновлений.Макеты.Найти("УсловияРаспространенияОбновлений")=Неопределено Тогда
			Возврат ЛОЖЬ;
		КонецЕсли;
	Иначе
		Возврат (РежимОбновленияДанных()="НачальноеЗаполнение");
	КонецЕсли;
	// -РАРУС
...
Функция ВозможноОперативноеОбновление(ИтерацииОбновления = Неопределено) Экспорт
		...
		Для каждого ФлагРазделенности Из ФильтрыРазделенностиОбработчиков Цикл
			...
			Если ДеревоОбработчиков.Строки.Количество() > 1 
				...
				Для Каждого СтрокаВерсия Из ДеревоОбработчиков.Строки Цикл
					...
					КонецЕсли;
					Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
						СписокОбработчиков = СписокОбработчиков
							+ ?(Не ЗначениеЗаполнено(СписокОбработчиков), "", Символы.ПС)
							+ Обработчик.Процедура;
					КонецЦикла;
				КонецЦикла;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(СписокОбработчиков) Тогда
		ТекстСообщения = НСтр("ru = 'Следующие обработчики не поддерживают обновление без установки монопольного режима:
			|
			|%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, СписокОбработчиков);
...
Процедура ПроверитьСвойстваОбработчиков(ИтерацияОбновления)
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		ИначеЕсли Обработчик.Версия <> "*" Тогда
			...
			// +РАРУС
			Если НЕ НулеваяВерсия И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Обработчик.Версия, ИтерацияОбновления.Версия) > 0 Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Требуется обновить сведения о версии подсистемы ""%1"" в процедуре ""%2"".'"),
					ИтерацияОбновления.Подсистема, ИтерацияОбновления.ИмяОсновногоСерверногоМодуля+".ПриДобавленииПодсистемы");
...
Функция НеобходимоСохранитьНевыполненныйОтложенныйОбработчик(Библиотека, Обработчик)
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(Обработчик.НомерВерсии, Библиотека.ПредыдущаяВерсия) <= 0 Тогда
		// Версия проверяемого обработчика ниже текущей версии библиотеки и он не будет добавлен заново
...
Процедура ЗаполнитьДанныеДляПараллельногоОтложенногоОбновления(СведенияОбОбновлении, Параметры)
	...
	Если Параметры.ПриЗапускеКлиентскогоПриложения
		...
		Коробка       = Не ОбщегоНазначенияПовтИсп.РазделениеВключено();
...
Функция ЗаполнениеДанныхОтложенногоОбработчикаВФоне(ПараметрыОбработчикаСтруктура, АдресРезультата)
	...
	Попытка
		...
		РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации(ОписаниеПроцедуры.ПроцедураЗаполнения, ПараметрыОбработчика);
	Исключение
		ЗаписатьОшибку(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При вызове процедуры заполнения данных
					   |""%1""
