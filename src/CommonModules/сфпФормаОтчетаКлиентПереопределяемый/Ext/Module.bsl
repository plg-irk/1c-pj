...
// Вызывается для отчета "сфпЕженедельныйОтчет"
//
Процедура сфпОбработатьРасшифровкуЕженедельныйОтчет(ЭтаФорма, Расшифровка, СтандартнаяОбработка)
	...
	сфпФормаОтчетаВызовСервера.сфпЕженедельныйОтчетОбработкаРасшифровки(КомпоновщикНастроек, ЭтаФорма.ДанныеРасшифровки, Расшифровка, СтандартнаяОбработка, СтруктураДляОткрытияОтчета);
...
// Вызывается для отчета "сфпОтчетПоВсемЗвонкам"
//
Процедура сфпОбработатьРасшифровкуОтчетПоВсемЗвонкам(ЭтаФорма, Расшифровка, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	СтруктураДляОткрытияОтчета = Новый Структура();
	КомпоновщикНастроек = ЭтаФорма.Отчет.КомпоновщикНастроек;	
	сфпФормаОтчетаВызовСервера.сфпОтчетПоВсемЗвонкамОбработкаРасшифровки(КомпоновщикНастроек, ЭтаФорма.ДанныеРасшифровки, Расшифровка, СтандартнаяОбработка, СтруктураДляОткрытияОтчета);
	Если СтруктураДляОткрытияОтчета.Количество() > 0 Тогда	
