...
Процедура ПроверитьПодписьЗавершение(Результат, Контекст) Экспорт
	...
	Контекст.РезультатПроверки.СвойстваПодписи.Протокол = РезультатКЭП.Протокол;
	Контекст.РезультатПроверки.СвойстваПодписи.ДатаПроверкиПодписи = ОбщегоНазначенияКлиент.ДатаСеанса();
...
Процедура ОткрытьФормуРасширенногоПредставленияОшибки(КонтекстДиагностики, 
	...
	ДанныеДляОбработкиПослеЗакрытияФормы = Новый Структура("ДанныеСертификата, ДанныеПодписи");
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения(
			"ОчиститьВременноеХранилищеПослеЗакрытияФормыРасширенногоПредставленияОшибки",
			ЭтотОбъект, ДанныеДляОбработкиПослеЗакрытияФормы);
	
	ШаблонТекстаДиагностики = НСтр("ru='<a href = ""%1"">Выполнить диагностику</a> электронного документооборота.'");
	ТекстДиагностики = СтрШаблон(ШаблонТекстаДиагностики, "ВыполнитьДиагностику");
	ПараметрыОшибки.ТекстДополнительнойСсылки = ТекстДиагностики;
	ПараметрыОшибки.ОбработчикДополнительнойСсылки = 
		"КриптографияБЭДСлужебныйКлиент.ОбработатьНавигационнуюСсылкуИзФормыРасширенногоПредставленияОшибки";
	ПараметрыОшибки.ПараметрОбработчикаДополнительнойСсылки = 
		Новый Структура("КонтекстДиагностики", КонтекстДиагностики);
	
	ЭлектроннаяПодписьКлиент.ОткрытьФормуРасширенногоПредставленияОшибки(ПараметрыОшибки,, ОповещениеОЗавершении);

КонецПроцедуры

// Очищает данные временного хранилища после закрытия формы "РасширенноеПредставлениеОшибки"
// Параметры:
...
Процедура ОчиститьВременноеХранилищеПослеЗакрытияФормыРасширенногоПредставленияОшибки(
...
// Выполняет обработчик нажатия навигационной ссылки на форме "РасширенноеПредставлениеОшибки"
// Параметры:
//  ПараметрыСсылки - Структура:
//  * НавигационнаяСсылка - Строка - Действие переданное при нажатии на ссылку.
//  * ПараметрыВыполнения - Структура:
//  ** КонтекстДиагностики - См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//  ДополнительныеПараметры - Неопределено
Процедура ОбработатьНавигационнуюСсылкуИзФормыРасширенногоПредставленияОшибки(ПараметрыСсылки, ДополнительныеПараметры) Экспорт
	
	ПараметрыВыполнения = ПараметрыСсылки.ПараметрОбработчикаДополнительнойСсылки;
	НавигационнаяСсылка = ПараметрыСсылки.НавигационнаяСсылка;
	
	Если НавигационнаяСсылка = "ВыполнитьДиагностику" Тогда
		КонтекстДиагностики = ПараметрыВыполнения.КонтекстДиагностики;
		ПараметрыОбработкиОшибок = ОбработкаНеисправностейБЭДКлиент.НовыеПараметрыОбработкиОшибок();
