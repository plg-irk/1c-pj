...
// Результат формирования url-ссылки на публикуемый электронный документ в сервисе 1С:Share.
//
// Если в дополнительных файлах есть недопустимые файлы, то такие файлы игнорируются и в сервис не отправляются.
// Запрещенные файлы см ИнтеграцияShareКлиентСервер.ЗапрещенныеРасширенияФайла.
// Если сформированный пакет с электронным документом и прикрепленными файлами превысил допустимый размер,
// установленный на сервисе 1С:Share - ссылка сформирована не будет.
// При наличии ошибок выполняется запись в журнал регистрации и вывод сообщения пользователю.
//
// Параметры:
//  ЭлектронныйДокумент - ДокументСсылка, СправочникСсылка - ссылка на прикладной документ учета.
//  ДополнительныеФайлы - Массив из Структура - дополнительные файлы к электронному документу:
...
//  НастройкиПубликации - Неопределено - настройки отсутствуют, используются настройки по умолчанию,
//                                       см Обработка.СервисShare.Макет.ТранспортныйКонтейнер.
//                      - Структура - настройки публикации документа в сервисе, если свойство остается пустым,
//                                    то в этом случае будет заполнено по умолчанию,
//                                    см Обработка.СервисShare.Макет.ТранспортныйКонтейнер:
//   * СхемаПубликации - Строка - см. ИнтеграцияShareКлиентСервер.СхемыПубликацииДокумента.
//   * ДлительностьСрокаЖизни - Число - срок жизни доступа к электронному документу в минутах.
//                                      Допускается значение до 30 дней (43200 минут).
//                            - Дата - содержит дату и время в часом поясе UTC длительности срока жизни доступа к электронному документу.
//                                     Допускается значение до 30 дней.
//   * ИнициаторПубликации - Строка - механизм инициириующий публикацию документа в сервис 1С:Share. При интеграции с другими сервиса
//      рекомендуется указывать наименование подсистемы или функциональности, например: "ОнлайнЗаказы", "ГрупповаяОбработка" и т.д.
//      По умолчанию значение "СервисShare".
//   * ДоступноРучноеУправление - Булево - признак доступности ручного управления ссылкой на документ в пользовательских интерфейсах.
//                                 Например, разрешается удаление ссылки до завершения срока жизни. По умолчанию значение Истина.
//                                 Если ссылка размещается в другом сервисе и необходимо сохранить целостность ссылки,
//                                 то необходимо установить значение Ложь.
//  ДополнительныеПараметры - Неопределено, Структура - произвольные дополнительные параметры для передачи в переопределяемые методы.
//   Параметры передаются в метод ИнтеграцияShareПереопределяемый.ПриОпределенииВизуализацииПубликуемогоДокумента.
//
// Возвращаемое значение:
//  Структура - результат формирования ссылки:
//   * URLСсылка - Строка - ссылка на опубликованный электронный документ.
Функция РезультатФормированияURLСсылкиНаЭлектронныйДокумент(Знач ЭлектронныйДокумент, Знач ДополнительныеФайлы,
	НастройкиПубликации, ДополнительныеПараметры) Экспорт
	
	Отказ = Ложь;
	
	Результат = Новый Структура;
	Результат.Вставить("URLСсылка", "");
	
	ПараметрыДокумента = ИнтеграцияShareКлиентСервер.НовыеПараметрыДокументаКПубликации();
	ПараметрыДокумента.ЭлектронныйДокумент = ЭлектронныйДокумент;
	ПараметрыДокумента.ДополнительныеПараметры = ДополнительныеПараметры;
	
	Если НастройкиПубликации <> Неопределено Тогда
		
		СрокЖизниВМинутах = 0;
		ДлительностьСрокаЖизни = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиПубликации, "ДлительностьСрокаЖизни");
		Если ТипЗнч(ДлительностьСрокаЖизни) = Тип("Дата") Тогда
			СрокЖизниВМинутах = ДатаUTCВМинуты(ДлительностьСрокаЖизни);
		ИначеЕсли ТипЗнч(ДлительностьСрокаЖизни) = Тип("Число") Тогда
			СрокЖизниВМинутах = ДлительностьСрокаЖизни;
		КонецЕсли;
		
		ПараметрыДокумента.НастройкиПубликации.ДоступноРучноеУправление = НастройкиПубликации.ДоступноРучноеУправление;
		ПараметрыДокумента.НастройкиПубликации.ИнициаторПубликации = НастройкиПубликации.ИнициаторПубликации;
		ПараметрыДокумента.НастройкиПубликации.СрокЖизниВМинутах = СрокЖизниВМинутах;
		ПараметрыДокумента.НастройкиПубликации.СхемаПубликации = НастройкиПубликации.СхемаПубликации;
	КонецЕсли;
	
	Для Каждого СтрокаФайла Из ДополнительныеФайлы Цикл
		
		СтруктураИмениФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(СтрокаФайла.ИмяФайла);
		РасширениеФайла = ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(СтруктураИмениФайла.Расширение);
		
		Если ПустаяСтрока(РасширениеФайла)
			Или Не РасширениеФайлаДоступно(РасширениеФайла, СтрокаФайла.ДвоичныеДанные) Тогда
			Продолжить;
		КонецЕсли;
		
		ФайлДокумента = ИнтеграцияShareКлиентСервер.НовыеДанныеФайлаДляВыбораКПубликации();
		ФайлДокумента.ДвоичныеДанныеФайла = СтрокаФайла.ДвоичныеДанные;
		ФайлДокумента.Наименование = СтрокаФайла.Представление;
		ФайлДокумента.ИмяФайла = СтрокаФайла.ИмяФайла;
		ФайлДокумента.Размер = СтрокаФайла.ДвоичныеДанные.Размер();
		ФайлДокумента.Расширение = РасширениеФайла;
		
		ПараметрыДокумента.РезультатПроверкиПакета.ДанныеФайловКПубликации.Добавить(ФайлДокумента);
	КонецЦикла;
	...
	ПревышенРазмерПакета = ПараметрыДокумента.РезультатПроверкиПакета.ПревышенРазмерПакета;
	Если ПревышенРазмерПакета Тогда
		
		МаксимальныйРазмерПакетаВМегабайтах = ИнтеграцияShareКлиентСервер.РазмерФайлаИзБайтовВМегабайты(
			ПараметрыДокумента.РезультатПроверкиПакета.МаксимальныйРазмерПакета);
		
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось сформировать ссылку на электронный документ ""%1"" по причине:
			|объем передаваемых данных превышает максимальный размер %2 МБ'", ОбщегоНазначения.КодОсновногоЯзыка()),
				Строка(ЭлектронныйДокумент),
				МаксимальныйРазмерПакетаВМегабайтах);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
		Возврат Результат;
	КонецЕсли;
	
	СсылкаДляСкачиванияДокумента = ПараметрыДокумента.ДанныеОпубликованногоДокумента.СсылкаДляСкачиванияДокумента;
	
	Результат.URLСсылка = СсылкаДляСкачиванияДокумента;
	
	Возврат Результат;
	
КонецФункции

// Деактивирует url-ссылку опубликованного электронного документа в сервисе 1С:Share.
//
// Параметры:
//  URLСсылка - Строка - ссылка на опубликованный электронный документ.
...
//                  При наличии ошибок выполняется запись в журнал регистрации и вывод сообщения пользователю.
//
Процедура ДеактивироватьURLСсылкуНаЭлектронныйДокумент(Знач URLСсылка, Отказ) Экспорт
	
	Если ПустаяСтрока(URLСсылка) Тогда
		Возврат;
	КонецЕсли;
	
	ЗапросСервиса = ЗапросСервисаДеактивацииURLСсылки(URLСсылка);
	ВыполнитьЗапрос(ЗапросСервиса, Отказ);
...
//   * ДвоичныеДанныеДокумента - Неопределено, ДвоичныеДанные - данные публикуемого документа
//   * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО
//   * СвойстваПредставления - Структура - свойства представления, которые при необходимости можно переопределить при
//      использовании стандартной обработки:
...
//   * ДополнительныеПараметры - Неопределено, Структура - произвольные дополнительные параметры. Могут быть переданы
//    при отправке с клиента для передачи в переопределяемые методы. Параметры передаются в метод
//    ИнтеграцияShareПереопределяемый.ПриОпределенииВизуализацииПубликуемогоДокумента.
//
Функция НовыеПараметрыПубликуемогоДокументаДляВизуализации() Экспорт
	
	ПараметрыДокумента = Новый Структура;
	ПараметрыДокумента.Вставить("ОбъектУчета", Неопределено);
	ПараметрыДокумента.Вставить("ДвоичныеДанныеДокумента", Неопределено);
	ПараметрыДокумента.Вставить("ТипДокумента", Перечисления.ТипыДокументовЭДО.ПустаяСсылка());
...
//   * ПараметрыОтраженияВУчете - См. НовыеПараметрыОтраженияВУчете
//   * ИспользоватьФормуЗагрузкиДокументаПоУмолчанию - Булево - признак использования типовой формы загрузки документа по ссылке на этапе загрузки документа.
//       см. Обработки.СервисShare.ЗагрузитьДокументПоСсылке. При необходимости использования переопределенной формы требуется установить Ложь.
Функция НовыеНастройкиЗагрузкиДокумента() Экспорт
	...
	Результат.Вставить("ИспользоватьФормуЗагрузкиДокументаПоУмолчанию", Истина);
...
//      по данным электронного документа. Например, может передаваться поддерживаемый идентификатор объекта метаданных
...
//   * ДанныеФайловДокумента - Массив из Структура - сведения о файлах документа:
//     ** ИдентификаторФайла - Строка - идентификатор файла.
//     ** ПредставлениеФайла - Строка - представление файла для отображения пользователю.
//     ** РасширениеФайла - Строка - расширение файла.
//     ** ВидФайла - Строка - вид файла, см. ИнтеграцияShareКлиентСервер.ВидыФайловДокумента
//     ** СсылкаСкачиванияИсходногоФайла - Строка - http-ссылка для скачивания исходного файла из сервиса 1С:Share.
//     ** СсылкаСкачиванияФайлаВизуализации - Строка - http-ссылка для скачивания визуализации исходного файла из сервиса 1С:Share.
//
Функция НовоеКраткоеОписаниеЭлектронногоДокумента() Экспорт
	...
	Результат.Вставить("ДанныеФайловДокумента", Новый Массив);
...
//                                                                       Заполняется при работе с файлами через БСП.
//   * Наименование - Строка - наименование файла без расширения.
//   * Расширение   - Строка - расширение файла без точки.
//   * ВидФайла - Строка - способ хранения дополнительных файлов к документу в прикладном решение.
//                  См. ИнтеграцияShareКлиентСервер.ВидыХраненияФайловВБазе. По умолчанию вариант "ПрисоединенныеФайлы".
//   * ИдентификаторФайла - Строка - идентификатор файла. При заполнении вида файла "ИнтеграцияС1СДокументооборотом"
...
Функция НовыеДанныеПрисоединенногоФайла() Экспорт
	...
	ДанныеФайла.Вставить("ВидФайла", ИнтеграцияShareКлиентСервер.ВидыХраненияФайловВБазе().ПрисоединенныеФайлы);
...
// Гиперссылка на веб-страницу с инструкцией по работе с сервисом 1С:Share в учетной системе.
//
// Возвращаемое значение:
//  Строка
...
//
// Возвращаемое значение:
//  Неопределено - Ошибка публикации документа. Для анализа причин следует пользователя направлять в ЖР.
//  См. ИнтеграцияShareКлиентСервер.НовыеПараметрыДокументаКПубликации
Функция ОпубликованныеДанныеДокумента(Знач ПараметрыДокумента) Экспорт
...
//  ПараметрыДокумента - См. ИнтеграцияShareКлиентСервер.НовыеПараметрыДокументаКПубликации
//  Отказ              - Булево - признак наличия ошибок при выполнении операции.
//                                Описание ошибок записывается в ЖР и выводится пользователю в сообщение.
//
Процедура ОпубликоватьИПолучитьСсылкуНаСкачиваниеДокумента(ПараметрыДокумента, Отказ) Экспорт
	...
	ДополнительныеПараметры = ПараметрыДокумента.ДополнительныеПараметры;
	
	ДополнительныеФайлы = ДополнительныеФайлыДляПубликации(СсылкаНаДокумент, РезультатПроверкиПакета.ДанныеФайловКПубликации);
	
	ДанныеДокумента = ДанныеТранспортногоКонтейнераПакетаЭДО(СсылкаНаДокумент, ДополнительныеФайлы, ДополнительныеПараметры);
	...
	ДанныеПубликации = ДанныеДокументаДляПубликации(ДанныеДокумента, ПараметрыДокумента);
	...
	ДанныеОпубликованногоДокумента.ТипДокумента                 = ДанныеДокумента.ТипДокумента;
	ДанныеОпубликованногоДокумента.Организация                  = ДанныеДокумента.Организация;
	ДанныеОпубликованногоДокумента.ФорматЭлектронногоДокумента  = ДанныеДокумента.ФорматЭлектронногоДокумента;
	ДанныеОпубликованногоДокумента.ЭтоФорматФНС                 = ДанныеДокумента.ЭтоФорматФНС;
	
	ПараметрыДокумента.ДанныеОпубликованногоДокумента = ДанныеОпубликованногоДокумента;
	
	ТекстСообщения = СтрШаблон(НСтр("ru='Сформирована ссылка %1 на электронный документ %2'",
		ОбщегоНазначения.КодОсновногоЯзыка()), ДанныеОпубликованногоДокумента.СсылкаДляСкачиванияДокумента,
		Строка(СсылкаНаДокумент));
		
	ПараметрыЗаписи = ОбщегоНазначенияБЭД.НовыеПараметрыЗаписиВЖурналРегистрации();
	ПараметрыЗаписи.ОбъектМетаданных = СсылкаНаДокумент.Метаданные();
	ПараметрыЗаписи.Данные = СсылкаНаДокумент;
	
	ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(ТекстСообщения, ИнтеграцияShareКлиентСервер.ИмяПодсистемы(),
		УровеньЖурналаРегистрации.Информация, ПараметрыЗаписи);
	
КонецПроцедуры

// Параметры объекта учета к публикации электронного документа.
// Примечание: нельзя использовать в транзакции, т.к. на нижних уровнях используется попытка при поиске файлов через БИД.
//
// Параметры:
...
// Данные электронного документа по идентификатору из сервисе 1С:Share.
//
// Параметры:
...
Функция ДанныеЭлектронногоДокументаПоИдентификатору(Знач ОписаниеЭлектронногоДокумента) Экспорт
...
// Выполняет загрузку электронного документа в прикладной объект.
// Параметры:
//  ОписаниеЭлектронногоДокумента - см. НовоеОписаниеДанныхЭлектронногоДокумента
//
// Возвращаемое значение:
//  Неопределено - возвращается в случае ошибки при загрузке документа.
//  См. НовыеДанныеЭлектронногоДокументаДляЗагрузки
Функция ЗагрузитьЭлектронныйДокументВОбъектУчета(Знач ОписаниеЭлектронногоДокумента) Экспорт
	...
	Возврат СоздатьВУчетеЭлектронныйДокумент(ДанныеДокумента);
	
КонецФункции

// Формирует в прикладной базе учетный документ на основе данных из сервиса, и отмечает его загрузку в базу
// в сервисе 1С:Share.
//
// Параметры:
//  ДанныеДокумента - См. НовыеДанныеЭлектронногоДокументаДляЗагрузки
//
// Возвращаемое значение:
//  Неопределено - возвращается в случае ошибки при загрузке документа.
//  См. НовыеДанныеЭлектронногоДокументаДляЗагрузки
Функция СоздатьВУчетеЭлектронныйДокумент(Знач ДанныеДокумента) Экспорт
	...
	ПараметрыОтраженияВУчете.Направление  = Перечисления.НаправленияЭДО.Входящий;
	...
	ПараметрыОтраженияВУчете.СпособОбмена = СпособОбмена();
	...
	ОтражениеВУчетеЭДО.ОтразитьДанныеЭлектронногоДокумента(ПараметрыОтраженияВУчете,
		ДанныеДокумента.СпособОтраженияВУчете, МассивОбъектов);

	Если Не ЗначениеЗаполнено(ДанныеДокумента.ДокументУчета) И МассивОбъектов.Количество() = 1 Тогда
	...
	ТипЗначенияДокумента = ТипЗнч(ДанныеДокумента.ДокументУчета);
	МетаданныеОбъекта    = Метаданные.НайтиПоТипу(ТипЗначенияДокумента);
	
	ОбщегоНазначенияКлиентСервер.Проверить(
		МетаданныеОбъекта <> Неопределено, СтрШаблон(НСтр(
		"ru = 'Не удалось определить метаданные объекта типа ""%1""'"),
		ТипЗначенияДокумента));
		
	ПараметрыДокумента.ТипМетаданныхДокументаВладельца = МетаданныеОбъекта.ПолноеИмя();
	
	ПараметрыДокумента.ИдентификаторЭлектронногоДокумента = ДанныеДокумента.ИдентификаторЭлектронногоДокумента;
	ОтметитьВСервисеЗагрузкуДокументаВБазу(ПараметрыДокумента, Ложь);
...
Функция РезультатПоискаДокументаПоПубличнойСсылке(Знач СсылкаДляСкачивания, Отказ = Ложь, ТекстОшибки = "") Экспорт
	...
	Если Не ЕстьПравоИспользованияСервиса() Тогда
		ТекстОшибки = НСтр("ru = 'Недостаточно прав для поиска документа по ссылке. Обратитесь к администратору.'");
		Отказ = Истина;
		Возврат Неопределено;
	КонецЕсли;
	...
	Результат.НастройкиЗагрузки = НастройкиЗагрузкиДокументаПоСсылке(ОписаниеЭлектронногоДокумента, Отказ, ТекстОшибки);
...
//  ДополнительныеПараметры - Структура - дополнительные параметры глобального поиска, свойства могут отсутствовать:
//   * ЭтоСсылкаОнлайнЗаказа - Булево - ссылка сервиса 1С:Онлайн-Заказы.
//
// Возвращаемое значение:
...
Функция РезультатГлобальногоПоискаДокументаПоПубличнойСсылке(СсылкаДляСкачивания, ДополнительныеПараметры) Экспорт
	
	ТекстОшибки = "";
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ЭтоСсылкаОнлайнЗаказа", Ложь) Тогда
		
		СсылкаДляСкачивания = СсылкаДляСкачиванияВСервисеОнлайнЗаказы(СсылкаДляСкачивания, ТекстОшибки);
	...
	Отказ = Ложь;
	РезультатПоискаДокумента = РезультатПоискаДокументаПоПубличнойСсылке(СсылкаДляСкачивания, Отказ, ТекстОшибки);
	
	ДоступенТолькоПросмотрДокумента = Ложь;
	Если РезультатПоискаДокумента = Неопределено И Отказ Тогда
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр(
				"ru = 'Не удалось получить документ по ссылке %1 по причине:
				|%2'"), СсылкаДляСкачивания, ТекстОшибки));
		КонецЕсли;
		Возврат Неопределено;
	ИначеЕсли Отказ Тогда
		ДоступенТолькоПросмотрДокумента = Истина;
	КонецЕсли;
	...
	Если ОписаниеЭлектронногоДокумента.ДокументНеНайден Тогда
		Возврат РезультатГлобальногоПоиска;
	ИначеЕсли ОписаниеЭлектронногоДокумента.ДоступКДокументуЗакрыт Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Ссылка на документ удалена или доступ к нему закрыт.
			|Обратитесь к отправителю для получения новой ссылки на документ.'", ОбщегоНазначения.КодОсновногоЯзыка()));
		Возврат РезультатГлобальногоПоиска;
	КонецЕсли;
	
	Если ДоступенТолькоПросмотрДокумента Тогда
		
		Если РезультатПоискаДокумента.НастройкиЗагрузки.ВыводитьРезультатПоискаПредпросмотрВВебе Тогда
			ТекстПоиска = СтрШаблон(НСтр("ru = 'Посмотреть ""%1 от %2"" (%3)'"),
		...
		ТекстСообщения = СтрШаблон(НСтр(
				"ru = 'Загрузка документа по ссылке %1 недоступна'"), СсылкаДляСкачивания);
		
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = '%1 по причине:
				|%2'"),
	...
	ТекстПоиска = СтрШаблон(НСтр("ru = 'Посмотреть ""%1 от %2""'"),
			ОписаниеЭлектронногоДокумента.ПредставлениеДокументаОтправителя,
			ОписаниеЭлектронногоДокумента.НаименованиеОтправителя);
			
	НастройкиЗагрузки = Новый Структура;
	НастройкиЗагрузки.Вставить("ИспользоватьФормуЗагрузкиДокументаПоУмолчанию",
		РезультатПоискаДокумента.НастройкиЗагрузки.ИспользоватьФормуЗагрузкиДокументаПоУмолчанию);
			
	ДанныеДокумента = Новый Структура;
	ДанныеДокумента.Вставить("НастройкиЗагрузки", НастройкиЗагрузки);
	ДанныеДокумента.Вставить("ОписаниеЭлектронногоДокумента", ОписаниеЭлектронногоДокумента);
	ДанныеДокумента.Вставить("СсылкаДляСкачивания", СсылкаДляСкачивания);
	
	Результат = НовыйРезультатГлобальногоПоиска();
	Результат.Значение         = ДанныеДокумента;
	Результат.Представление    = ТекстПоиска;
	Результат.Картинка         = БиблиотекаКартинок.Документ;
	Результат.ОписаниеДействия = ИнтеграцияShareКлиентСервер.ИмяРезультатаГлобальногоПоискаОткрытьФормуЗагрузкиДокумента();
	РезультатГлобальногоПоиска.Добавить(Результат);
	...
	Если ПараметрыОтраженияВУчете.Количество() = 0 Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Загрузка документа с типом документа ""%1"" не поддерживается.'"),
...
// Возвращаемое значение:
//  Структура - описание данных электронного документа для загрузки в учетную базу:
//   * ДанныеОрганизации - см. НовыйУчастникОбмена
//   * ДанныеКонтрагента - см. НовыйУчастникОбмена
//   * ИдентификаторЭлектронногоДокумента - Строка
//   * ПредставлениеДокумента - Строка
//   * ТипДокумента - Строка
//   * СпособОтраженияВУчете - Строка
//   * Контрагент - СправочникСсылка
//   * Организация - СправочникСсылка
//   * НоменклатураСопоставления - Булево
//   * ТребуетсяСопоставлениеНоменклатуры - Булево
//   * ДанныеОсновногоФайла - См. РаботаСФайламиБЭД.НовоеОписаниеФайла
//   * ДанныеФайлаДопДанных - См. РаботаСФайламиБЭД.НовоеОписаниеФайла
//   * НаправлениеЭД - ПеречислениеСсылка.НаправленияЭДО
//   * ДокументУчета - Неопределено
//                   - ДокументСсылка
//                   - СправочникСсылка
//   * ИдентификаторДокумента - Строка
//   * ДополнительныеФайлыДокумента - Неопределено
//                                  - См. НовоеОписаниеДополнительныхФайлов
//   * ФайлыВизуализации - Массив из см. НовыйФайлВизуализации() 
//   * ЕстьДополнительныеФайлы - Булево
Функция НовыеДанныеЭлектронногоДокументаДляЗагрузки() Экспорт
	...
	Результат.Вставить("ФайлыВизуализации", Новый Массив);
	Результат.Вставить("ЕстьДополнительныеФайлы", Ложь);
...
// Конструктор описания данных о загруженном документе в прикладное решение.
//
// Возвращаемое значение:
...
Функция НовыйУчастникОбмена()
	...
	Результат.Вставить("КодСтраны", "643"); // По умолчанию Россия.
...
// Преобразует дату в формате UTC в минуты.
//
// Параметры:
//  ИсходнаяДата - Дата - исходная дата в формате UTC.
//
// Возвращаемое значение:
//  Число - результат преобразования в минуты.
//
Функция ДатаUTCВМинуты(ИсходнаяДата)
	
	Если Не ЗначениеЗаполнено(ИсходнаяДата) Тогда
		Возврат 0;
	КонецЕсли;
	
	Возврат Окр((ИсходнаяДата - НачалоМинуты(ТекущаяУниверсальнаяДата()))/60);
...
Функция ЗапросСервисаОтправитьДокумент(Знач ДанныеПубликации, Отказ = Ложь)
	...
	ПараметрыЗапроса = СтрШаблон("?publicationScheme=%1", СтроковыеФункции.СтрокаЛатиницей(СхемаПубликации));
	...
	Если ЗначениеЗаполнено(НастройкиПубликации.ИнициаторПубликации) Тогда
		ЗапросСервиса.Заголовки.Вставить("x-vnd-1cshare-initiator",
			СтроковыеФункции.СтрокаЛатиницей(НастройкиПубликации.ИнициаторПубликации));
	КонецЕсли;
	
	Если Не НастройкиПубликации.ДоступноРучноеУправление Тогда
		ЗапросСервиса.Заголовки.Вставить("x-vnd-1cshare-manual-control",
			XMLСтрока(НастройкиПубликации.ДоступноРучноеУправление));
	КонецЕсли;
		
	Если ЗначениеЗаполнено(НастройкиПубликации.СрокЖизниВМинутах) Тогда
		ЗапросСервиса.Заголовки.Вставить("x-vnd-1cshare-ttl", XMLСтрока(НастройкиПубликации.СрокЖизниВМинутах));
	КонецЕсли;
...
Функция ЗапросСервисаПолучитьКраткиеДанныеДокументаПоПубличнойСсылке(Знач СсылкаДляСкачивания)
	...
	ЗапросСервиса.ТипВозвращаемогоКонтента = "application/vnd-1cshare-shortLink-doc.v3+json";
...
Функция ОписаниеДокументаПоПубличнойСсылкеВМоделиСервиса(Знач РезультатЗапроса)
	...
	ЗаполнитьДанныеФайловДокументаВМоделиСервиса(Результат, РезультатЗапроса);
...
Функция ЗапросСервисаДеактивацииURLСсылки(Знач URLСсылкаНаДокумент)
	
	ОписаниеСсылки = ОбщегоНазначенияКлиентСервер.СтруктураURI(URLСсылкаНаДокумент);
	
	ЗапросСервиса = НовыйЗапросСервиса();
	ЗапросСервиса.АдресСервиса  = ОписаниеСсылки.ИмяСервера;
	ЗапросСервиса.СтрокаЗапроса = ОписаниеСсылки.ПутьНаСервере;
	ЗапросСервиса.Метод         = "DELETE";
	ЗапросСервиса.ДопустимыеКодыСостояния.Добавить(204);
	ЗапросСервиса.ДопустимыеКодыСостояния.Добавить(КодСостоянияУдален());
	ЗапросСервиса.Наименование  =
		НСтр("ru = 'Деактивация ссылки на документ в сервисе 1С:Share'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ВыполнитьЗапрос(Знач ЗапросСервиса, Отказ, ТекстОшибки = Неопределено)
	...
	Если РезультатВыполнения.КодСостояния = КодСостоянияРедиректаЗапроса() Тогда
		...
		РезультатВыполнения = ВыполнитьЗапрос(ЗапросСервисаРедиректа, Отказ, ТекстОшибки);
	КонецЕсли;
...
// Возвращаемое значение:
//  Соответствие из КлючИЗначение - коды ошибок при выполнении HTTP-запросов:
//   * Ключ - Число - код ошибки.
//   * Значение - Строка - описание кода ошибки.
...
Функция СсылкаНаИнструкциюСервисаВБЭД()
	...
	Возврат "https://its.1c.ru/db/bed1915doc#content:1953:hdoc";
...
Функция ЗначениеСвойстваКоллекцииВРекурсии(Знач Значение, Знач СвойстваПути, ЕстьОшибки = Ложь)
	...
	Если Не ЗначениеЗаполнено(СвойстваПути) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ДанныеДокументаДляПубликации(ДанныеДокумента, ПараметрыДокумента)
	...
	НастройкиПубликации = НастройкиПубликацииДокумента(ИтоговыйТранспортныйКонтейнер, ПараметрыДокумента);
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'При формировании возникли следующие ошибки:
			|%1'"), ТекстОшибки);
...
Функция ЗаполнитьДанныеОбУчастникеОбмена(Знач ДанныеУчастникаОбмена)
	...
	Результат.Вставить("КодСтраны"   , ДанныеУчастникаОбмена.КодСтраны);
	Возврат Результат;
...
Функция СформироватьТранспортныйКонтейнерCML(ДеревоДанных, Ошибки, ЭлектронныйДокумент = Неопределено)
	...
	Попытка
		...
		ПараметрыЗаполнения = РаботаСФайламиБЭД.НовыеПараметрыЗаполненияXDTO();
		ПараметрыЗаполнения.ОбязательныеСвойства = "Name";
		ПараметрыЗаполнения.ПропускаемыеНезаполненныеСвойства = "TIN";
		
		РаботаСФайламиБЭД.ЗаполнитьСвойстваXDTOИзДерева(Получатель, ЗначенияСвойствXDTO, ДеревоДанных, Ошибки, ПараметрыЗаполнения);
		
		Если ПустаяСтрока(РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Получатель, "TIN")) Тогда
			РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Получатель, "TIN", "-");
		КонецЕсли;
...
Функция НастройкиПубликацииДокумента(Знач ДеревоДанных, ПараметрыДокумента)
	
	НастройкиПубликацииДерева =  ДеревоЭлектронногоДокументаБЭД.ДанныеЭлементаДереваЭлектронногоДокумента(
		ДеревоДанных, "КарточкаПередаваемыхДанных.НастройкиПубликации");
		
	НастройкиЭлектронногоДокумента = ПараметрыДокумента.НастройкиПубликации;
		
	НастройкиПубликацииДерева.СрокЖизниВМинутах = НастройкиЭлектронногоДокумента.СрокЖизниВМинутах;
	НастройкиПубликацииДерева.СхемаПубликации = НастройкиЭлектронногоДокумента.СхемаПубликации;
	НастройкиПубликацииДерева.ИнициаторПубликации = НастройкиЭлектронногоДокумента.ИнициаторПубликации;
	НастройкиПубликацииДерева.ДоступноРучноеУправление = НастройкиЭлектронногоДокумента.ДоступноРучноеУправление;
	
	Возврат НастройкиПубликацииДерева;
...
Процедура ПрочитатьДанныеЭлектронногоДокумента(ДанныеДокумента, Знач ОписаниеЭлектронногоДокумента)
	...
	НайденныеСтроки = ФайлыТранспортногоКонтейнера.НайтиСтроки(
	...
	ДанныеГлавногоФайл = НайденныеСтроки[0];
...
Процедура ДополнитьДанныеДокументаИзОписанияПакетаДокументовЭДО(ДанныеДокумента, Знач ДанныеГлавногоФайл)
	...
	ДанныеДокумента.ЕстьДополнительныеФайлы = ДополнительныеФайлы.Количество() > 0;
...
Функция НовыеДанныеТранспортногоКонтейнераПакетаЭДО()
	...
	Результат.Вставить("ФорматЭлектронногоДокумента");
	Результат.Вставить("ЭтоФорматФНС");
...
Функция НовыеНастройкиФормированияДокумента()
	...
	Результат.Вставить("ЭтоФорматФНС", Ложь);
	
	Возврат Результат;
	
КонецФункции

Функция ДанныеТранспортногоКонтейнераПакетаЭДО(Знач СсылкаНаДокумент, Знач ДополнительныеФайлы, Знач ДополнительныеПараметры)
	...
	Если Не ЗначениеЗаполнено(НастройкиФормированияДокумента.ТипДокумента) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ПараметрыДокументаДляВизуализации.ДополнительныеПараметры = ДополнительныеПараметры;
	...
	ОписаниеПредставленияДокумента = РаботаСФайламиБЭД.НовоеОписаниеФайла();
	...
	Результат.ФорматЭлектронногоДокумента         = НастройкиФормированияДокумента.Формат;
	Результат.ЭтоФорматФНС                        = НастройкиФормированияДокумента.ЭтоФорматФНС;
...
Функция ДанныеУчастникаОбмена(Знач УчастникОбмена)
	...
	Если СведенияУчастника.Свойство("ИНННерезидента")
		И ЗначениеЗаполнено(СведенияУчастника.ИНННерезидента) Тогда
		Результат.ИНН = СведенияУчастника.ИНННерезидента;
	Иначе
	...
	Если СведенияУчастника.Свойство("КодСтраныНерезидента")
		И ЗначениеЗаполнено(СведенияУчастника.КодСтраныНерезидента) Тогда
		Результат.КодСтраны = СведенияУчастника.КодСтраныНерезидента;
	КонецЕсли;
...
Процедура ПрочитатьТранспортныйКонтейнер(ДанныеДокумента, Знач РаспакованныеФайлы)
	...
	Если ДанныеДокумента.ВерсияФормата <> АктуальнаяВерсияФорматаТранспортногоКонтейнера() Тогда
		ТекстВерсии = ?(ЗначениеЗаполнено(ДанныеДокумента.ВерсияФормата),
			СтрШаблон(НСтр("ru = 'не поддержана версия формата ""%1""'"), ДанныеДокумента.ВерсияФормата),
			НСтр("ru = 'не заполнена версия формата'"));
	...
	Если ЗначениеЗаполнено(ДанныеКонтактовXDTO) Тогда
		ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных,
			"КарточкаПередаваемыхДанных.Отправитель.Контакты.Наименование", РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
...
//
// Возвращаемое значение:
//  См. НовыеНастройкиЗагрузкиДокумента
Функция НастройкиЗагрузкиДокументаПоСсылке(Знач ОписаниеЭлектронногоДокумента, Отказ = Ложь, ТекстОшибки = "") Экспорт
	
	НастройкиЗагрузки = НовыеНастройкиЗагрузкиДокумента();
	...
	Если ЗначениеЗаполнено(ОбъектУчета) Тогда
		...
		ПараметрОтраженияВУчете.ПредставлениеРезультатаПоиска = СтрШаблон(НСтр("ru = 'Открыть ""%1""'"),
					ПараметрОтраженияВУчете.ПредставлениеДокумента);
	...
	ИнтеграцияShareПереопределяемый.ПриОпределенииНастроекЗагрузкиДокумента(НастройкиЗагрузки,
...
Процедура ЗаполнитьСсылкуНаОбъектУчетаЭлектронногоДокумента(Знач ДанныеЗагрузкиДокумента)
	...
	ТипМетаданныхОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(
		ДанныеЗагрузкиДокумента.ТипМетаданныхВладельца);
		
	УстановитьПривилегированныйРежим(Истина);
	
	СсылкаНаОбъектУчета = ТипМетаданныхОбъекта.ПолучитьСсылку(
		Новый УникальныйИдентификатор(ДанныеЗагрузкиДокумента.ИдентификаторДокумента));
...
Процедура ЗаполнитьФайлыДокументаДляВыбораКПубликации(ДанныеФайлов, Знач Документ)
	...
	ДобавитьФайлыИзПрисоединенныхФайловВБазе(ДанныеФайлов, Документ);
	ДобавитьПрисоединенныеФайлыИзИнтеграцииС1СДокументооборот(ДанныеФайлов, Документ);
	
КонецПроцедуры

Процедура ДобавитьФайлыИзПрисоединенныхФайловВБазе(ДанныеФайлов, Знач Документ)
...
Процедура ДобавитьПрисоединенныеФайлыИзИнтеграцииС1СДокументооборот(ДанныеФайлов, Знач Документ)
	...
	Попытка
		ПрисоединенныеФайлыДокумента = МодульИнтеграцияС1СДокументооборотБазоваяФункциональность.ДанныеФайловОбъектаИС(
		...
		Для Каждого ФайлДокумента Из ПрисоединенныеФайлыДокумента Цикл
			...
			ДанныеФайла.Наименование = ФайлДокумента.Наименование;
			ДанныеФайла.Расширение   = ФайлДокумента.Расширение;
			ДанныеФайла.ИмяФайла     = ФайлДокумента.ИмяФайла;
			
			ДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(ФайлДокумента.СсылкаНаДвоичныеДанныеФайла);
			Если ДвоичныеДанныеФайла = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ДанныеФайла.ДвоичныеДанныеФайла            = ДвоичныеДанныеФайла;
			ДанныеФайла.Размер                         = ФайлДокумента.Размер;
			ДанныеФайла.ВидФайла                       =
				ИнтеграцияShareКлиентСервер.ВидыХраненияФайловВБазе().ИнтеграцияС1СДокументооборот;
			ДанныеФайла.ИдентификаторФайла             = ФайлДокумента.ID;
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось получить информацию о файлах из БИД (Интеграция с 1С:Документооборотом).
...
Процедура ЗаполнитьДанныеФайловДокументаВМоделиСервиса(Результат, Знач РезультатЗапроса)
	
	ДанныеФайловДокументаВМоделиСервиса = ЗначениеКоллекцииПоАбсолютномуПути(РезультатЗапроса, "dataFilesVisualization");
	Если ДанныеФайловДокументаВМоделиСервиса = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеФайловДокумента = Новый Массив;
	Для Каждого ДанныеФайла Из ДанныеФайловДокументаВМоделиСервиса Цикл
		
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("ИдентификаторФайла", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла, "id"));
		ОписаниеФайла.Вставить("ПредставлениеФайла", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла, "name"));
		ОписаниеФайла.Вставить("РасширениеФайла", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла, "extension"));
		ОписаниеФайла.Вставить("ВидФайла", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла, "type"));
		ОписаниеФайла.Вставить("СсылкаСкачиванияИсходногоФайла", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла,
			"linkDownloadОriginalFile"));
		ОписаниеФайла.Вставить("СсылкаСкачиванияФайлаВизуализации", ЗначениеКоллекцииПоАбсолютномуПути(ДанныеФайла,
			"linkDownloadVisualizationFile"));
		
		ДанныеФайловДокумента.Добавить(ОписаниеФайла);
	КонецЦикла;
	
	Результат.ДанныеФайловДокумента = ДанныеФайловДокумента;
	
КонецПроцедуры

// Параметры:
//  СсылкаНаФайл - Строка - внешний адрес URL-ссылки для скачивания файла.
//  ИмяФайла - Строка - полное имя файла с расширением.
//
// Возвращаемое значение:
//  ДвоичныеДанные - файл полученный по URL-ссылке
Функция ДвоичныеДанныеФайлаПоВнешнейСсылке(Знач СсылкаНаФайл, Знач ИмяФайла) Экспорт
	
	Если ПустаяСтрока(СсылкаНаФайл) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ПараметрыФайла = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
	ПараметрыФайла.Таймаут = 60;
	
	РезультатПолученияФайла = ПолучениеФайловИзИнтернета.СкачатьФайлВоВременноеХранилище(СсылкаНаФайл, ПараметрыФайла, Истина);
	
	Если Не РезультатПолученияФайла.Статус Тогда
		ОбщегоНазначения.СообщитьПользователю(РезультатПолученияФайла.СообщениеОбОшибке);
		Возврат Неопределено;
	КонецЕсли;
	
	АдресХраненияФайла = РезультатПолученияФайла.Путь;
	ДанныеФайла = ПолучитьИзВременногоХранилища(АдресХраненияФайла);
	УдалитьИзВременногоХранилища(АдресХраненияФайла);
...
// Параметры:
//  URLСсылка - Строка - ссылка сервиса онлайн-заказы.
//  ТекстОшибки - Строка - текст ошибки.
//
// Возвращаемое значение:
//  Неопределено - ссылка не найдена или произошла ошибка.
//  Строка - найденная ссылка для скачивания сервиса 1С:Share.
//
Функция СсылкаДляСкачиванияВСервисеОнлайнЗаказы(Знач URLСсылка, ТекстОшибки) Экспорт
	
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ОнлайнЗаказы") Тогда
		Возврат Неопределено;
	КонецЕсли;
		
	МодульОнлайнЗаказы = ОбщегоНазначения.ОбщийМодуль("ОнлайнЗаказы");
	Результат = МодульОнлайнЗаказы.ПубличныеДанныеЗаказа(URLСсылка);
		
	Если Не ПустаяСтрока(Результат.ПараметрыShare.ИнформацияОбОшибке) Тогда
		ТекстОшибки = СтрШаблон(НСтр(
				"ru = 'Не удалось получить документ по ссылке %1 по причине:
				|%2'"), URLСсылка, Результат.ПараметрыShare.ИнформацияОбОшибке);
		Возврат Неопределено;
	КонецЕсли;
	
	СсылкаДляСкачивания = Результат.ПараметрыShare.URL;
	Если ПустаяСтрока(СсылкаДляСкачивания)
		Или Не ИнтеграцияShareКлиентСервер.ЭтоДоменСервисаКороткихСсылокShare(СсылкаДляСкачивания) Тогда
		// Ссылка на документ отсутствует в сервисе 1С:Онлайн-Заказы.
		Возврат Неопределено;
	КонецЕсли;
