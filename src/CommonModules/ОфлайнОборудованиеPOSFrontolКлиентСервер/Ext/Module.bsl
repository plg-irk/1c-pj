...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	КаталогОперативнойСводки = Неопределено;
	...
	Параметры.Свойство("КаталогОперативнойСводки", КаталогОперативнойСводки);

	Если КаталогВыгрузки  = Неопределено
	 Или КаталогОбмена   = Неопределено
	 Или КаталогЗагрузки   = Неопределено
	 Или КаталогОперативнойСводки = Неопределено Тогда
...
Функция НачатьВыполнениеКоманды(ОповещениеПриЗавершении = Неопределено, Команда, ВходныеПараметры = Неопределено, ОбъектДрайвера, Параметры, ПараметрыПодключения) Экспорт
	...
	ИначеЕсли Команда = "ЗагрузитьДанные" Тогда
		...
		РезультатВыполнения = НачатьЗагрузкуРасширенногоОтчета(Параметры, ВыходныеПараметры,Параметры.Идентификатор);
...
Функция НачатьВыгрузкуДанных(ОповещениеПриЗавершении, Параметры, ПолнаяВыгрузка, ВыгружатьТовары, ВыгружатьСкидки, ВыгружатьПерсонал, ВыходныеПараметры)
	...
	//ИмяФайла      = Параметры.КаталогВыгрузки;
	//ИмяФайлаФлага = Параметры.КаталогОбмена;
	ФорматОбмена  = Параметры.ФорматОбмена;
	ИмяВременногоФайла = КаталогВременныхФайлов()+"POS.spr";
	Если Параметры.Свойство("ВыгружатьОстатки") Тогда
		ВыгружатьОстатки = Параметры.ВыгружатьОстатки;
	Иначе
	...
	Если ВыгружатьТовары Тогда
		...
		Для каждого Касса из Параметры.Кассы Цикл
			...
			стрНомерКассы = Прав("00"+Формат(Касса.НомерPOS,"ЧГ=0"),2);
			КаталогОбмена = Касса.КаталогВыгрузки+?(Прав(Касса.КаталогВыгрузки,1)<>"\","\","");
			// проверим каталог обмена
			// проверим, а не лежит ли еще незагруженный
			ФС = Новый Файл(КаталогОбмена+"Pos"+стрНомерКассы+".spr");
			Если ФС.Существует() Тогда
				// файл есть, надо проверить, загружен он или нет
				Попытка
					Файл = Новый Файл(КаталогОбмена+"POS"+стрНомерКассы+".flz");
					Если Файл.Существует() Тогда
	 					ВызватьИсключение("");
					КонецЕсли;
					ТД = Новый ТекстовыйДокумент();
					ТД.Прочитать(КаталогОбмена+"Pos"+стрНомерКассы+".spr",КодировкаТекста.ANSI);
					// проверим вторую строку, там признак прочитанности
					Если ТД.ПолучитьСтроку(2)<>"@" Тогда
						ВызватьИсключение("");
					КонецЕсли;
					
					// удаляем
					УдалитьФайлы(КаталогОбмена,"Pos"+стрНомерКассы+".*");
				Исключение
					Продолжить;
				КонецПопытки;
			КонецЕсли;
		...
		Для каждого СтруктураМомента из МоментыПоследнейВыгрузки Цикл
			#Если Клиент Тогда
				Товары = МенеджерОборудованияРарусСервер.ПолучитьТаблицуТоваров(Параметры.Идентификатор, ПолнаяВыгрузка, Истина, СтруктураМомента.МоментПоследнейВыгрузки,,ВыгружатьОстатки);
			#Иначе
				Товары = МенеджерОборудованияРарусСервер.ПолучитьТаблицуТоваров(Параметры.Идентификатор, ПолнаяВыгрузка, Ложь,   СтруктураМомента.МоментПоследнейВыгрузки,,ВыгружатьОстатки);
			#КонецЕсли
			...
			Результат = СформироватьФайлТоваров(Файл, ПолнаяВыгрузка, ФорматОбмена, Разделитель, Товары, ВыходныеПараметры);
			Если ПолнаяВыгрузка Тогда
				СформироватьСтавкиНДС(Файл, ФорматОбмена);
			КонецЕсли;
			Попытка
				Файл.Записать(ИмяВременногоФайла, "windows-1251");
			Исключение
				т = ОписаниеОшибки();
				к = ИнформацияОбОшибке();
				ВыходныеПараметры.Добавить(999);
				ОписаниеОшибки = НСтр("ru='Не удалось записать файл товаров по адресу: %Адрес%'");
				ВыходныеПараметры.Добавить(СтрЗаменить(ОписаниеОшибки, "%Адрес%", ИмяВременногоФайла));
				Результат = Ложь;
			КонецПопытки;
			
			Если Результат Тогда
				Кассы = СтруктураМомента.Кассы;
				Для каждого Касса из Кассы Цикл
					...
					Попытка
						...
						КопироватьФайл(ИмяВременногоФайла,КаталогОбмена+"Pos"+стрНомерКассы+".spr");
						// и флажок положим
						ТД = Новый ТекстовыйДокумент();
						ТД.Записать(КаталогОбмена+"Pos"+стрНомерКассы+".flz",КодировкаТекста.ANSI);
					Исключение
						т = ОписаниеОшибки();
						к = ИнформацияОбОшибке();
						ВыходныеПараметры.Добавить(999);
...
Функция ВыгрузкаТоваровЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Результат = СформироватьФайлТоваров(Файл, НЕ ЧастичнаяВыгрузка, ФорматОбмена, Разделитель, Товары, ВыходныеПараметры);
...
Функция НачатьЗагрузкуРасширенногоОтчета(Параметры, ВыходныеПараметры,ОфлайнОборудование)
	
	
	Для Каждого Касса Из Параметры.Кассы Цикл
		КаталогОбмена = Касса.КаталогЗагрузки+?(Прав(Касса.КаталогЗагрузки,1)<>"\","\","");
		// проверим существование файла выгрузки
		ФайлыКаталога = НайтиФайлы(КаталогОбмена,"pos*.rep");
			
		Для Каждого ФайлКаталога Из ФайлыКаталога Цикл
			ИмяФайла = ФайлКаталога.ПолноеИмя;
			Файл = Новый Файл(ИмяФайла);
			Если Файл.Существует() Тогда
				...
				РезультатВыполнения = ЗагрузкаРасширенногоОтчетаЗавершение(РезультатЧтенияФайлов, ДополнительныеПараметры,ОфлайнОборудование);
				Если РезультатВыполнения.Результат Тогда
					УдалитьФайлы(Файл.Путь, Файл.Имя);
					УдалитьФайлы(Файл.Путь, "*.txt");
				КонецЕсли;
			Иначе
				ВыходныеПараметры.Добавить(999);
	...
	Если РезультатВыполнения=Неопределено тогда
		...
		ОписаниеОшибки = НСтр("ru='Нет данных для обработки'");
	...
	//ИмяФайла = Параметры.КаталогЗагрузки;
	//
	//Файл = Новый Файл(ИмяФайла);
	//Если Файл.Существует() Тогда
	//	
...
Функция ЗагрузкаРасширенногоОтчетаЗавершение(Результат, ДополнительныеПараметры,ОфлайнОборудование) Экспорт
	...
	Иначе
		...
		Результат = ОфлайнОборудованиеPOSFrontolВызовСервера.ОбработатьЗагружаемыеДанныеИзККМ(Файл, ФорматОбмена, ВыходныеПараметры,ОфлайнОборудование);
...
Функция СформироватьФайлТоваров(Файл, ПолнаяВыгрузка, ФорматОбмена, Разделитель, Товары, ВыходныеПараметры)
...
Процедура ДополнитьФайл(Результат, Файл, ВыходныеПараметры, Параметры, Знач ДанныеДляЗаполнения = Неопределено)
	...
	Если Уровень = "Номенклатура" Тогда
		...
		Если НЕ Источник.ЭтоГруппа Тогда
			...
			ПризнакПредметаРасчета = "1";// товар кроме подакцизного
			ПризнакСпособаРасчета = "2"; // полный расчет
			АлкогольМаркируемый = Ложь;
			ЕстьАлкогольныеРеквизиты = ЗначениеЗаполнено(Источник.ЭтоАлкоголь) И Источник.ЭтоАлкоголь;
			ДанныеДляЗаполнения.Вставить("ТипНоменклатуры", "0");
			ДанныеДляЗаполнения.Вставить("Маркируемый", "");
			...
			Если НЕ ЗначениеЗаполнено(Источник.ОсобенностьУчета) Тогда
				// возьмем признак предмета расчета из типа номенклатуры
				//Признак предмета расчёта во фронтоле:
				//0 – не используется;
				...
				//6 – иной товар;
				//7 – аванс, предоплата
				Если Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Товар") Тогда
					ПризнакПредметаРасчета = "1";
				ИначеЕсли Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ПодакцизныйТовар") Тогда
					ПризнакПредметаРасчета = "2";
				ИначеЕсли Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Работа") Тогда
					ПризнакПредметаРасчета = "3";
				ИначеЕсли Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.Услуга") Тогда
					ПризнакПредметаРасчета = "4";
				ИначеЕсли Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.СоставнойПредметРасчета") Тогда
					ПризнакПредметаРасчета = "5";
				ИначеЕсли Источник.ПризнакПредметаРасчета = ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ПлатежВыплата") Тогда
					ПризнакПредметаРасчета = "7";
					ПризнакСпособаРасчета = "1";
				ИначеЕсли Источник.ПризнакПредметаРасчета <> ПредопределенноеЗначение("Перечисление.ПризнакиПредметаРасчета.ПустаяСсылка") Тогда
					ПризнакПредметаРасчета = "6";
				КонецЕсли;
			ИначеЕсли Источник.ОсобенностьУчета = ПредопределенноеЗначение("Справочник.ОсобенностиУчетаНоменклатуры.АлкогольМаркированный") Тогда
				ДанныеДляЗаполнения.ТипНоменклатуры = "1";
				ДанныеДляЗаполнения.Маркируемый = "0";
				ПризнакПредметаРасчета = "2";
			ИначеЕсли Источник.ОсобенностьУчета = ПредопределенноеЗначение("Справочник.ОсобенностиУчетаНоменклатуры.АлкогольНеМаркированный") Тогда
				ДанныеДляЗаполнения.ТипНоменклатуры = "1";
				ДанныеДляЗаполнения.Маркируемый = "1";
				ПризнакПредметаРасчета = "2";
			ИначеЕсли Источник.ОсобенностьУчета = ПредопределенноеЗначение("Справочник.ОсобенностиУчетаНоменклатуры.ТабакМаркированный") Тогда
				ДанныеДляЗаполнения.ТипНоменклатуры = "2";
				ПризнакПредметаРасчета = "2";
			ИначеЕсли Источник.ОсобенностьУчета = ПредопределенноеЗначение("Справочник.ОсобенностиУчетаНоменклатуры.ТабакНеМаркированный") Тогда
				ПризнакПредметаРасчета = "2";
			ИначеЕсли Источник.ОсобенностьУчета = ПредопределенноеЗначение("Справочник.ОсобенностиУчетаНоменклатуры.Аванс") Тогда
				ПризнакПредметаРасчета = "7";
				ПризнакСпособаРасчета = "1";
			КонецЕсли;
			ДанныеДляЗаполнения.Вставить("ПризнакПредметаРасчета", ПризнакПредметаРасчета);
			ДанныеДляЗаполнения.Вставить("ПризнакСпособаРасчета", ПризнакСпособаРасчета);
			
			Если ЕстьАлкогольныеРеквизиты Тогда
			...
			ИначеЕсли ТипЗнч(Источник.Штрихкоды) = Тип("Массив") Тогда
				...
				Для Каждого ДанныеШтрихкода Из Источник.Штрихкоды Цикл
					Штрихкод = Штрихкод + Запятая + ?(ТипЗнч(ДанныеШтрихкода)=Тип("Структура"),ДанныеШтрихкода.ШтрихКод,ДанныеШтрихкода); //laza+skud иначе ШК не грузятся, т.к. ДанныеШтрихкода -  это структура...
					Запятая = ",";
				КонецЦикла;
			Иначе
				Штрихкод = "";
			...
			ДанныеДляЗаполнения.Вставить("Остаток", Формат(Источник.Остаток,"ЧРД=.; ЧГ=0"));
		КонецЕсли;
...
Функция СпискиНалоговИКомбинацийНалогов()
	...
	СтавкаНалога20.Вставить("Текст", "НДС 20%");
	СтавкаНалога20.Вставить("Значение", 20);
	СтавкаНалога20.Вставить("НомерНалогаККМ", 3);
...
Процедура ДобавитьСтрокуВФайлСтарый(Файл, Разделитель, ДанныеДляЗаполнения, ФорматОбмена, ЭтоГруппа = Ложь)  //skud 19-09-23 заменил только на формат для Фронтол6
...
Процедура ДобавитьСтрокуВФайл(Файл, Разделитель, ДанныеДляЗаполнения, ФорматОбмена, ЭтоГруппа = Ложь)
	...
	Иначе
		...
		Строка =
			...
			+ "" + Разделитель																					// Файл *.bmp с картинкой (31)
			+ "" + Разделитель																					// Описание (32)
			+ "" + Разделитель																								// Кратность количества (33)
			+ Разделитель																				// Код ценника (34)
			+ Разделитель																				// Тип вознаграждения (35)
			+ Разделитель																				// Значение вознаграждения (36)
			+ Разделитель																				// Номер секции ККМ (37)
			...
			+ ДанныеДляЗаполнения.КодВидаАлкогольнойПродукции + Разделитель								// Код вида алкогольной или спиртосодержащей продукции (53)
			+ ДанныеДляЗаполнения.ЕмкостьТары + Разделитель												// Емкость тары в литрах (54)
			+ ДанныеДляЗаполнения.ТипНоменклатуры + Разделитель											// Тип номенклатуры (55)
			+ ДанныеДляЗаполнения.Маркируемый + Разделитель												// Маркировка алкогольной продукции (56)
			+ ДанныеДляЗаполнения.Крепость + Разделитель												// Крепость алкогольной продукции (57)
			+ ДанныеДляЗаполнения.ПризнакСпособаРасчета + Разделитель;									// Признак способа расчёта (58)
