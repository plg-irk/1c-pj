...
Процедура ПоказатьВыборСпособаОбработкиСозданияОбъектовУчетаИзСпискаТекущихДел(Форма) Экспорт
	...
	Если Форма.СписокСоздания.Количество() > 1 Тогда
		Форма.ПоказатьВыборИзМеню(ОбработкаВыбора, Форма.СписокСоздания,
			Форма.Элементы.РеквизитыДокументаИОтражениеВУчетеHTML);
	Иначе
		ВыполнитьОбработкуОповещения(ОбработкаВыбора, Форма.СписокСоздания[0]);
	КонецЕсли;
...
Процедура ПоказатьВыборСпособаОбработкиПодбораОбъектаУчетаИзСпискаТекущихДел(Форма, Номер) Экспорт
	...
	Если ТекущийСписокПодбора.Количество() > 1 Тогда
		...
		Форма.ПоказатьВыборИзМеню(ОбработкаВыбора, ТекущийСписокПодбора,
			Форма.Элементы.РеквизитыДокументаИОтражениеВУчетеHTML);
...
//  * Объект - ДанныеФормыСтруктура:
//   ** Ссылка - ДокументСсылка.ЭлектронныйДокументИсходящийЭДО,ДокументСсылка.ЭлектронныйДокументВходящийЭДО
Асинх Процедура ВыполнитьДействиеОтправитьСообщение(Форма) Экспорт
	...
	ОсновноеДействие = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.СформироватьДополнительнуюИнформацию");
	...
	ДополнительныеПараметрыДействия.Комментарий = Форма.ТекстСообщения;
	
	ПараметрыВыполнения.ДополнительныеПараметрыДействий.Вставить(ОсновноеДействие, ДополнительныеПараметрыДействия);
...
//  * ИдентификаторПакета - УникальныйИдентификатор
//  ЭтоПакетнаяОбработка - Булево
Асинх Процедура ВыполнитьДействиеЗапроситьУточнениеИзКарточкиДокумента(Форма, ЭтоПакетнаяОбработка = Ложь) Экспорт
	
	ВидВыполняемогоДействия = ВидыВыполняемыхДействийСДокументом().ЗапроситьУточнение;
	Если Не ВыполнениеДействийПоЭДОДоступноДляПакета(Форма.СостоянияПакетаОднородно, ЭтоПакетнаяОбработка) Тогда
		ТекстВопроса = ТекстВопросаОВыполненииДействияДляДокументаПакета(ВидВыполняемогоДействия);
		Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНет); // КодВозвратаДиалога
		Если Ответ = КодВозвратаДиалога.Да Тогда
	...
	ОсновноеДействие = ПредопределенноеЗначение("Перечисление.ДействияПоЭДО.ЗапроситьУточнение");
	...
	ПараметрыВводаСтроки = ПараметрыВводаСтрокиДляЗапросаУточнения();
	ПоказатьВводСтрокиДляВыполненияДействийПоЭДО(ПараметрыВводаСтроки, ОбработчикЗавершения);
...
Функция ПараметрыВводаСтрокиДляЗапросаУточнения()
	
	ПараметрыВводаСтроки = ОбщегоНазначенияБЭДКлиент.ПараметрыВводаСтроки();
	ПараметрыВводаСтроки.ЗаголовокФормы = НСтр("ru = 'Укажите причины запроса уточнения документа'");
	ПараметрыВводаСтроки.НазваниеКнопкиПоУмолчанию = НСтр("ru = 'Запросить уточнение'");
	ПараметрыВводаСтроки.Строка = НСтр("ru = 'Причина уточнения: '");
	ПараметрыВводаСтроки.КомментарийОбязательностиВвода =
		НСтр("ru = 'Для запроса уточнения по документу необходимо указать причину.'");
...
//  * ИдентификаторПакета - УникальныйИдентификатор
//  ЭтоПакетнаяОбработка - Булево
Асинх Процедура ВыполнитьДействиеОтказатьсяОтПодписанияИзКарточкиДокумента(Форма, ЭтоПакетнаяОбработка = Ложь) Экспорт
	
	ВидВыполняемогоДействия = ВидыВыполняемыхДействийСДокументом().ОтказатьсяОтПодписания;
	Если Не ВыполнениеДействийПоЭДОДоступноДляПакета(Форма.СостоянияПакетаОднородно, ЭтоПакетнаяОбработка) Тогда
		ТекстВопроса = ТекстВопросаОВыполненииДействияДляДокументаПакета(ВидВыполняемогоДействия);
		Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНет); // КодВозвратаДиалога
		Если Ответ = КодВозвратаДиалога.Да Тогда
	...
	ПараметрыВводаСтроки = ПараметрыВводаСтрокиДляОтказаОтПодписанияДокумента();
	ПоказатьВводСтрокиДляВыполненияДействийПоЭДО(ПараметрыВводаСтроки, ОбработчикЗавершения);
...
// Возвращаемое значение:
//  См. ОбщегоНазначенияБЭДКлиент.ПараметрыВводаСтроки
Функция ПараметрыВводаСтрокиДляОтказаОтПодписанияДокумента()
	
	ПараметрыВводаСтроки = ОбщегоНазначенияБЭДКлиент.ПараметрыВводаСтроки();
	ПараметрыВводаСтроки.ЗаголовокФормы = НСтр("ru = 'Укажите причины отказа от подписания документа'");
	ПараметрыВводаСтроки.НазваниеКнопкиПоУмолчанию = НСтр("ru = 'Отказать'");
	ПараметрыВводаСтроки.Строка = НСтр("ru = 'Причина отказа: '");
	ПараметрыВводаСтроки.КомментарийОбязательностиВвода =
...
Функция ВидыВыполняемыхДействийСДокументом() Экспорт
	...
	ВидыВыполняемыхДействий.Вставить("ЗапроситьУточнение", НСтр("ru='Запросить уточнение'"));
	ВидыВыполняемыхДействий.Вставить("ОтказатьсяОтПодписания", НСтр("ru='Отказаться от подписания'"));
	ВидыВыполняемыхДействий.Вставить("ОтправитьСообщение", НСтр("ru='Отправить сообщение'"));
...
Процедура ОткрытьФайлИнформацииОтправителяДокумента(ЭлектронныйДокумент, ИспользоватьЦентрЭДО, ИдентификаторФормы) Экспорт
	
	ДлительнаяОперация = ЛегкийИнтерфейсДокументовЭДОВызовСервера.ДанныеФайлаИнформацииОтправителяДокумента(
		ЭлектронныйДокумент, ИдентификаторФормы, ИспользоватьЦентрЭДО);
...
Процедура СоздатьДокументыПакетаПоПредварительнымДаннымИзКарточкиДокумента(Форма,
	...
	Для Каждого ДанныеПредварительногоДокумента Из Форма.ПредварительныйПакетДокументов Цикл
		...
		Если ДанныеПредварительногоДокумента.ИдентификаторДокумента = Форма.ТекущийИдентификаторДокумента
			И Не Форма.Объект.ИспользоватьЦентрЭДО Тогда
			Продолжить;
		КонецЕсли;
	...
	ПараметрыОповещенияПослеФормированияПоПредварительнымДанным.Вставить("ИспользоватьЦентрЭДО",
		Форма.Объект.ИспользоватьЦентрЭДО);
...
Функция НовыеПараметрыСозданияОбъектовУчета()
	...
	ПараметрыОтраженияВУчете.Вставить("ЭтоДокументЦентраЭДО", Ложь);
...
Процедура СформироватьПакетЭДИзФайловСДискаИОбъектаУчета_ДобавлениеВПакетФайловСКомпьютера(
	...
	ПараметрыДобавленияВПакет.ИспользоватьЦентрЭДО = РеквизитыЭлектронногоДокумента.ИспользоватьЦентрЭДО;
...
Процедура ОбъединитьДокументыВПакетПослеФормированияПоПредварительнымДанным(Результат, ПараметрыОповещения) Экспорт
	...
	Если Не ПараметрыОповещения.ИспользоватьЦентрЭДО Тогда
