...
Функция ДанныеЗаполненияНастройкиОтправкиПриглашений(ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.ЗаполнитьОператоровЭДО Тогда
		ОписаниеМетодаПолученияСпискаОператоровЭДО =
			ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса("СписокОператоровЭДО");
	...
	Если ЗаполнитьПараметрыСвязей Тогда
		...
		ОписаниеМетодаПолученияСвязиИдентификаторовЭДО =  ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса(
	...
	Для Каждого УчетнаяЗаписьЦентраЭДО Из УчетныеЗаписиЦентровЭДО Цикл
		...
		РезультатыПакета = ИнтеграцияЦентраЭДО.ВыполнитьПакетМетодовСервиса(ПараметрыВыполнения, ПакетМетодов,
			КонтекстДиагностики);
		СписокОператоровЭДОВФорматеСервиса = РезультатыПакета[ИдентификаторМетодаПолученияСпискаОператоровЭДО]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.СписокОператоровЭДО
		ПараметрыСвязиВФорматеСервиса = РезультатыПакета[ИдентификаторМетодаПолученияСвязиИдентификаторовЭДО]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.НаборПараметровСвязиИдентификаторовАбонентовЭДО
...
Функция ПараметрыСвязиИдентификаторов(ПараметрыПолучения, КонтекстДиагностики = Неопределено) Экспорт
	...
	Для Каждого УчетнаяЗаписьЦентраЭДО Из УчетныеЗаписиЦентровЭДО Цикл
		...
		ПараметрыСвязиВФорматеСервиса = ИнтеграцияЦентраЭДО.ВыполнитьМетодСервиса(ПараметрыВыполнения,
			ОписаниеМетода, КонтекстДиагностики); // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.НаборПараметровСвязиИдентификаторовАбонентовЭДО
...
Функция ОтправитьПриглашения(Приглашения, РасшифрованныеМаркеры) Экспорт
	...
	// См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ОтправитьПриглашенияЭДО
...
Процедура ДобавитьОписаниеМетодаДанныеПриглашенийЭДОСОшибками(ПакетМетодов, ПараметрыОперации) Экспорт
	...
	ИдентификаторыПриглашенийЭДО = РегистрыСведений.СостоянияСинхронизацииОбъектовСЦентромЭДО.ОбъектыСОшибками(
...
//  * Количество - Число
//  * КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ЗагрузитьПриглашенияВЦентреЭДО(УчетнаяЗаписьЦентраЭДО, РасшифрованныеМаркеры) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Количество", 0);
	Результат.Вставить("КонтекстДиагностики", ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики());
	
	ВидОперации = НСтр("ru = 'Загрузка приглашений в центре ЭДО'");
	...
	МаркерыАвторизации = УчетныеЗаписиЭДОИнтеграцияЦентраЭДО.МаркерыАвторизацииИзРасшифрованныхМаркеров(
		РасшифрованныеМаркеры);
	
	ОписаниеМетода = ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса(
		"ЗагрузитьПриглашенияИзСервисаЭДО", МаркерыАвторизации);
	
	РезультатЗагрузкиВСервисе = ИнтеграцияЦентраЭДО.ВыполнитьМетодСервиса(ПараметрыВыполнения, ОписаниеМетода,
		Результат.КонтекстДиагностики); // См. ПрограммныйИнтерфейсЦентраЭДОВерсия2.ЗагрузитьПриглашенияИзСервисаЭДО
	
	Если Не ЗначениеЗаполнено(РезультатЗагрузкиВСервисе) Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.Количество = РезультатЗагрузкиВСервисе.Количество;
	
	Если ЗначениеЗаполнено(РезультатЗагрузкиВСервисе.Ошибка) Тогда
		ИнтеграцияЦентраЭДО.ДобавитьНеизвестнуюОшибку(Результат.КонтекстДиагностики, ВидОперации,
			РезультатЗагрузкиВСервисе.Ошибка);
	КонецЕсли;
...
Функция ВыполнитьДействиеОтправитьПриглашенияВСервисе(ДанныеПриглашений, РасшифрованныеМаркеры, КонтекстДиагностики)
	...
	ПараметрыДействияПоУчетнымЗаписям = ПараметрыДействияСПриглашениямиПоУчетнымЗаписямЦентраЭДО(
	...
	Для Каждого ПараметрыДействияПоУчетнойЗаписи Из ПараметрыДействияПоУчетнымЗаписям Цикл
		...
		УчетнаяЗаписьЦентраЭДО = ПараметрыДействияПоУчетнойЗаписи.Ключ;
		...
		Если ЕстьНовыеПриглашения Тогда
			ОписаниеМетодаФормированияПриглашений = ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса(
				ИмяМетодаФормированияПриглашений, ПараметрыДействия.ДанныеПриглашений,
				ПараметрыДействия.МаркерыАвторизации);
			ИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаВПакет(ПакетМетодов, ОписаниеМетодаФормированияПриглашений,
				ИмяМетодаФормированияПриглашений);
		КонецЕсли;
		
		ОписаниеМетодаОтправкиПриглашений = ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса(ИмяМетодаОтправкиПриглашений,
			ПараметрыДействия.ДанныеПриглашений, ПараметрыДействия.МаркерыАвторизации);
		ИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаВПакет(ПакетМетодов, ОписаниеМетодаОтправкиПриглашений,
...
Функция ВыполнитьДействиеСПриглашениямиВСервисе(ИмяМетода, ДанныеПриглашений, РасшифрованныеМаркеры, КонтекстДиагностики)
	...
	Для Каждого ПараметрыДействияПоУчетнойЗаписи Из ПараметрыДействияПоУчетнымЗаписям Цикл
		...
		ОписаниеМетода = ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса(ИмяМетода,
			ПараметрыДействия.ДанныеПриглашений, ПараметрыДействия.МаркерыАвторизации);
		
		РезультатыДействийСПриглашениямиПоУчетнойЗаписи = ИнтеграцияЦентраЭДО.ВыполнитьМетодСервиса(
...
// 
// Возвращаемое значение:
//  См. ПрограммныйИнтерфейсЦентраЭДОВерсия2.НаборПараметровСвязиИдентификаторовАбонентовЭДО.ПараметрыАбонентовЭДО
...
// Параметры:
//  СписокОператоровЭДОВФорматеСервиса - См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.СписокОператоровЭДО
// 
// Возвращаемое значение:
...
// Параметры:
//  ПараметрыСвязиВФорматеСервиса - См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.НаборПараметровСвязиИдентификаторовАбонентовЭДО
//  ДанныеКонтрагентов - См. ДанныеКонтрагентовДляПриглашения
// 
// Возвращаемое значение:
//  См. ПриглашенияЭДО.НовыеПараметрыСвязиИдентификаторов
Функция ПараметрыСвязиИзФорматаСервиса(ПараметрыСвязиВФорматеСервиса, ДанныеКонтрагентов)
	...
	Для Каждого ЭлементСвязиВФорматеСервиса Из ПараметрыСвязиВФорматеСервиса Цикл
		...
		СтрокаПараметров.ИспользоватьЦентрЭДО = Истина;
...
Функция ПараметрыДействияСПриглашениямиПоУчетнымЗаписямЦентраЭДО(ДанныеПриглашений, РасшифрованныеМаркеры, ВидОперации, КонтекстДиагностики)
	...
	УчетныеЗаписиЦентровЭДО = РегистрыСведений.НастройкиИнтеграцииЦентраЭДО.УчетныеЗаписиПоИдентификаторамЭДО(ИдентификаторыЭДО);
	...
	Для Каждого ДанныеПриглашения Из ДанныеПриглашений Цикл
		...
		УчетнаяЗаписьЦентраЭДО = УчетныеЗаписиЦентровЭДО[ИдентификаторОрганизации];
		Если УчетнаяЗаписьЦентраЭДО = Неопределено Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Отсутствует учетная запись центра ЭДО для идентификатора организации %1'"),
		...
		ПараметрыДействия = ПараметрыДействияПоУчетнымЗаписям[УчетнаяЗаписьЦентраЭДО];
		Если ПараметрыДействия = Неопределено Тогда
			ПараметрыДействия = НовыеПараметрыДействияСПриглашениями();
			ПараметрыДействияПоУчетнымЗаписям.Вставить(УчетнаяЗаписьЦентраЭДО, ПараметрыДействия);
		КонецЕсли;
...
// Параметры:
//  ИдентификаторыПриглашенийЭДО - см. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ДанныеСинхронизацииПриглашенийЭДО.ИдентификаторыПриглашенийЭДО
// 
// Возвращаемое значение:
...
// Параметры:
//  ДанныеПриглашенийВФорматеСервиса - См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ДанныеСинхронизацииПриглашенийЭДО
//  РезультатДействий - см. ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.НовыйРезультатДействийЭДО
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ОбработатьРезультатМетодаДанныеПриглашенийЭДО(ДанныеПриглашенийВФорматеСервиса,
	...
	ОбновитьПриглашенияЭДОПоДаннымСинхронизацииПриЗагрузке(ДанныеПриглашенийВФорматеСервиса, РезультатДействий,
		КонтекстДиагностики);
	
КонецПроцедуры

// Параметры:
//  МеткиВремениПоИдентификаторамЭДО - см. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ПорцияДанныхСинхронизацииПриглашенийЭДОПоМеткамВремени.МеткиВремениПоИдентификаторамЭДО
// 
// Возвращаемое значение:
//  См. ИнтеграцияЦентраЭДО.ОписаниеМетодаСервиса
...
// Параметры:
//  ПорцияДанных - см. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ПорцияДанныхСинхронизацииПриглашенийЭДОПоМеткамВремени
...
Процедура ОбработатьРезультатМетодаПорцияДанныхПриглашенийЭДОПоМеткамВремени(
	...
	РезультатОбновления = ОбновитьПриглашенияЭДОПоДаннымСинхронизацииПриЗагрузке(
		ПорцияДанных.НаборДанных, РезультатДействий, КонтекстДиагностики);
...
// Параметры:
//  ДанныеСинхронизацииВФорматеСервиса - Массив из см. НовыеДанныеСинхронизацииПриглашенияЭДО
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
// 
// Возвращаемое значение:
//  Структура:
...
//    ** Значение - Булево
Функция ОбновитьПриглашенияЭДОПоДаннымСинхронизации(ДанныеСинхронизацииВФорматеСервиса, КонтекстДиагностики) Экспорт
	...
	ДанныеСинхронизацииПриглашенийЭДО = ДанныеСинхронизацииПриглашенийЭДОИзФорматаСервиса(
