...
Процедура ДобавитьКолонкиКодаВГруппировке(ТипПоиска, Группировка, ДополнительныеКолонки)
	
	Результат        = Неопределено;
	ДобавленаКолонка = Ложь;
	Элементы = Группировка.ПоляГруппировки.Элементы;
	ДоступныеПоля = Группировка.ПоляГруппировки.ДоступныеПоляПолейГруппировок;
	Для Каждого ТекГруппировка Из Элементы Цикл
		Если Не ТекГруппировка.Использование Тогда
			Продолжить;
		КонецЕсли;
		ДоступноеПоле = ДоступныеПоля.НайтиПоле(ТекГруппировка.Поле);
		Если ДоступноеПоле = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если НЕ ДоступноеПоле.ТипЗначения.СодержитТип(ТипПоиска) Тогда
			Продолжить;
		КонецЕсли;
		
		Для Каждого ДополнительнаяКолонка Из ДополнительныеКолонки Цикл
			ПутьКПолю = Строка(ТекГруппировка.Поле)+"."+ДополнительнаяКолонка;
			ПолеКомпоновки = Новый ПолеКомпоновкиДанных(ПутьКПолю);
			Если ДоступноеПоле.Элементы.Найти(ПолеКомпоновки) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ДобавленаКолонка = Истина;
			НайденноеПолеВыбора = Неопределено;
			Для Каждого ПолеВыборка Из Группировка.Выбор.Элементы Цикл
				//Если НЕ ПолеВыборка.Использование Тогда
				//	Продолжить;
				//КонецЕсли;
				Если НЕ ТипЗнч(ПолеВыборка) = Тип("ВыбранноеПолеКомпоновкиДанных") Тогда
					Продолжить;
				КонецЕсли;
				Если ПолеВыборка.Поле = ПолеКомпоновки Тогда
					НайденноеПолеВыбора = ПолеВыборка;
				КонецЕсли;
			КонецЦикла;
			
			Если НайденноеПолеВыбора = Неопределено Тогда
				НайденноеПолеВыбора = Группировка.Выбор.Элементы.Вставить(0, Тип("ВыбранноеПолеКомпоновкиДанных"));
				НайденноеПолеВыбора.Поле = ПолеКомпоновки;
			КонецЕсли;
			
			НайденноеПолеВыбора.Использование = Истина;
			
		КонецЦикла;
		
	КонецЦикла;
	
	Если ДобавленаКолонка Тогда
		ПараметрОформления = Группировка.ПараметрыВывода.Элементы.Найти("РасположениеРеквизитов");
		Если (НЕ ПараметрОформления = Неопределено) Тогда
			ПараметрОформления.Использование = Истина;
			ПараметрОформления.Значение = РасположениеРеквизитовКомпоновкиДанных.ВСпециальнойПозиции;
		КонецЕсли;
	КонецЕсли;
	
	//Результат        = Неопределено;
	//ДобавитьПоКолонкам = Новый Массив;
	//Элементы = Группировка.ПоляГруппировки.Элементы;
	//ДоступныеПоля = Группировка.ПоляГруппировки.ДоступныеПоляПолейГруппировок;
	//Для Каждого ТекГруппировка Из Элементы Цикл
	//	Если Не ТекГруппировка.Использование Тогда
	...
	//			Продолжить;
	//		КонецЕсли;
	//		ДобавитьПоКолонкам.Добавить(ПолеКомпоновки);
	//	КонецЦикла;
	//КонецЦикла;
	//
	//ДобавленаКолонка = Ложь;
	//Для Каждого ПолеКода Из ДобавитьПоКолонкам Цикл
	//	
	//	НайденноеГруппировкаКода = Неопределено;
	//	Для Каждого ТекГруппировка Из Элементы Цикл
	//		Если НЕ ТипЗнч(ТекГруппировка) = Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
	//			Продолжить;
	//		КонецЕсли;
	//		Если ТекГруппировка.Поле = ПолеКода Тогда
	//			НайденноеГруппировкаКода = ТекГруппировка;
	//		КонецЕсли;
	//	КонецЦикла;
	//	
	//	Если НайденноеГруппировкаКода = Неопределено Тогда
	//		НайденноеГруппировкаКода = Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
	//		НайденноеГруппировкаКода.Поле = ПолеКода;
	//	КонецЕсли;
	//	
	//	НайденноеГруппировкаКода.Использование = Истина;
	//	
	//	ДобавленаКолонка = Истина;
	//	
	//КонецЦикла;
...
КонецПроцедуры // ДобавитьКолонкиКодаВГруппировке()

Процедура ДобавитьКолонкиКода(ТипПоиска, СтруктураОтчета, ДополнительныеКолонки) Экспорт
	...
	Для Каждого ТекЭлемент Из СтруктураОтчета Цикл
		...
		Если ТипЭлемента = ТипСтруктурыТаблицы Тогда
			ДобавитьКолонкиКода(ТипПоиска, ТекЭлемент.Строки, ДополнительныеКолонки);
			ДобавитьКолонкиКода(ТипПоиска, ТекЭлемент.Колонки, ДополнительныеКолонки);
		ИначеЕсли ТипЭлемента = ТипСтруктурыДиаграммы Тогда
			ДобавитьКолонкиКода(ТипПоиска, ТекЭлемент.Серии, ДополнительныеКолонки);
			ДобавитьКолонкиКода(ТипПоиска, ТекЭлемент.Точки, ДополнительныеКолонки);
		Иначе
			ДобавитьКолонкиКодаВГруппировке(ТипПоиска, ТекЭлемент, ДополнительныеКолонки);
			ДобавитьКолонкиКода(ТипПоиска, ТекЭлемент.Структура, ДополнительныеКолонки);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры // ДобавитьКолонкиКода()
...
// Параметры:
// ЭлементыОтбора      - Список отборов настройки компоновщика данных
...
Процедура ПроверитьКорректностьПоследовательностейОтчета(Форма, Отчет)
	...
	Если НЕ ПравоПользователя("ПредупреждатьОНарушенииПоследовательностей") Тогда
	...
	ВывестиСообщение(СтрСоединить(СообщенияПоПоследовательностям, Символы.ПС));
...
Процедура ВывестиОтчет(ОтчетОбъект, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, ВнешниеНаборыДанных = Неопределено, СхемаКомпоновкиДанных = Неопределено, Настройки = Неопределено, ИспользоватьВнешниеФункции = Ложь) Экспорт
	...
	Если ПравоПользователя("ВыводитьКодВОтчетах") Тогда
		РежимВыводаКода = УправлениеПечатьюПлатформа.ПолучитьПараметрыРежимаВыводаКодаВДокументах().Имя;
		ТипНоменклатуры = Тип("СправочникСсылка.Номенклатура");
		ДополнительныеКолонки = Новый Массив;
		Если СтрНайти(РежимВыводаКода, "Код") > 0 Тогда
			ДополнительныеКолонки.Добавить("Код");
		КонецЕсли;
		Если СтрНайти(РежимВыводаКода, "Артикул") > 0 Тогда
			ДополнительныеКолонки.Добавить("Артикул");
		КонецЕсли;
		//Настройки.РазвернутьАвтоПоля();
		ДобавитьКолонкиКода(ТипНоменклатуры, Настройки.Структура, ДополнительныеКолонки);
	КонецЕсли;
	...
	Пока НЕ КонецШапки Цикл
		Строка = Строка+1;
		КонецШапки = (ВременныйТабличныйДокумент.Область("R" + Формат(Строка,"ЧГ=0") + "C2").Текст = "" И ВременныйТабличныйДокумент.Область("R" + Формат(Строка,"ЧГ=0") + "C1").Текст = "");
	КонецЦикла;
	
	Строка = Формат(Строка,"ЧГ=0");
	
	ОбластьШапка = ВременныйТабличныйДокумент.ПолучитьОбласть("R2:R" + Строка);
	ДокументРезультат.НачатьГруппуСтрок(,Ложь);
	ДокументРезультат.Вывести(ОбластьШапка);
	ДокументРезультат.ЗакончитьГруппуСтрок();
	ВременныйТабличныйДокумент.УдалитьОбласть(ВременныйТабличныйДокумент.Область("R1:R" + Строка),ТипСмещенияТабличногоДокумента.ПоВертикали);
	...
	Если ТипЗнч(СтруктураСвойств) = Тип("Структура") Тогда
		...
		Для Каждого ТекНабор Из НаборыДанных Цикл
			...
			Если ДоступныеПоля.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
// !!!_Перевести на новый механизм работы с доп. реквизитами и свойствами
			...
			//			Если ТекПоле.Значение.СодержитТип(ТипНазначения) 
			//				И СоответствиеНазначений[СвойстваВыборка.Назначение] = Неопределено Тогда
			//				
			...
			//				ИмяПВХ =
			//					ПланыВидовХарактеристик.НазначенияСвойствОбъектов.ПолучитьИмяПредопределенного(СвойстваВыборка.Назначение);
			//				
			//			КонецЕсли;
			//		КонецЦикла;
			...
			//		|		""Свойства."" + НазначенияСвойств.Свойство.Наименование КАК Наименование,
			...
			//		|		И ВЫБОР
			//		|				КОГДА НазначенияСвойств.Ссылка.БазовоеНазначение =
			//		|						ЗНАЧЕНИЕ(ПланВидовХарактеристик.НазначенияСвойствОбъектов.ПустаяСсылка)
			...
			//КонецЦикла;
			//
			//Если НЕ ТекстХарактеристик = "" Тогда
...
Процедура ЗаполнитьОтборОрганизацияПодразделение(ДеревоОтбор, ДоступныеОтборы) Экспорт
	...
	ОтборПоОрганизации       = ПравоПользователя("УстанавливатьОтборОрганизацияВОтчетах");
	ОтборПоПодразделению     = ПравоПользователя("УстанавливатьОтборПодразделениеВОтчетах");
	ЕстьОтборПоОрганизации   = Ложь;
	ЕстьОтборПоПодразделению = Ложь;
	ТекущийПользователь      = Пользователи.ТекущийПользователь();
	
	ОтборПоОрганизацииСнимаемый       = ПравоПользователя("РазрешитьИзменениеОтбораПоОрганизации");
	ОтборПоПодразделениюСнимаемый     = ПравоПользователя("РазрешитьИзменениеОтбораПоПодразделению");
...
Процедура УстановитьУсловноеОформлениеОсновнаяФорма(ФормаОтчета) Экспорт
	...
	Если НЕ ФормаОтчета.Элементы.Найти("ФормаСохранитьКакСтандартныеПараметры") = Неопределено Тогда
		ФормаОтчета.Элементы.ФормаСохранитьКакСтандартныеПараметры.Видимость = ПравоПользователя("СохранениеСтандартныхПараметровПечати") И ПравоДоступа("Вывод", Метаданные);;
	КонецЕсли;
...
Процедура СформироватьОтчет(ОтчетОбъект, ДокументРезультат, Расшифровка, ЗаголовокОтчета,ИмяОформлениеТаблицы, ИнформацияОбОшибке = "", КлючВарианта = "") Экспорт
...
Процедура СформироватьОтчетВФоне(ПараметрыФормированияОтчета, АдресХранилища) Экспорт
	...
	Заголовок       = ПолучитьЗначениеПараметраСтруктуры(ПараметрыФормированияОтчета, "Заголовок", "");
	КлючОбъекта     = ПолучитьЗначениеПараметраСтруктуры(ПараметрыФормированияОтчета, "КлючОбъекта", "");
	КлючВарианта    = ПолучитьЗначениеПараметраСтруктуры(ПараметрыФормированияОтчета, "КлючВарианта", "");
