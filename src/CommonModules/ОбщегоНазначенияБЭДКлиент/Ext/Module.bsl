...
// * КоличествоСтрокДополнительногоОписания - Число - Определяет высоту поля ДополнительноеОписание в строках.
...
Функция ПараметрыВводаСтроки() Экспорт
	...
	Параметры.Вставить("КоличествоСтрокДополнительногоОписания", 5);
...
//  ВидОперации - Строка
//  ПодсистемаБЭД - Строка - См. ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД
//  
// Возвращаемое значение:
//  Произвольный
//
Функция РезультатДлительнойОперации(ДлительнаяОперация, КонтекстДиагностики, ВидОперации, ПодсистемаБЭД = "") Экспорт
	...
	ИначеЕсли ДлительнаяОперация.Статус = "Выполнено" Тогда
		Результат = ОбщегоНазначенияБЭДВызовСервера.ПолучитьУдалитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата);
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		...
		ОбработкаНеисправностейБЭДКлиент.ДобавитьОшибку(КонтекстДиагностики, Ошибка, ПодсистемаОшибки);
	КонецЕсли;
