...
//				объекты одного типа!
//  ИменаРеквизитов - Строка - Строка, имена реквизитов перечисленные через запятую,
...
//  ИмяРеквизита - Строка - имя реквизита, например, "Код".
// 
// Возвращаемое значение:
		...
		Если ДокументОбъект.ПроверитьЗаполнение() Тогда
			...
			Попытка
				...
				Если НЕ ДокументОбъект.Проведен Тогда
					НепроведенныеДокументы.Добавить(Новый Структура("Ссылка,ОписаниеОшибки", ДокументСсылка, "Не удалось провести документ."));
				КонецЕсли;
...
//
//  ДругиеИсключения - Массив - Массив полных имен объектов метаданных, которые также
...
//			Истина - По умолчанию. Проверять каждую пару "дубль-оригинал"
...
//
//		* ПривилегированнаяЗапись - Булево - Необязательный. Требуется ли устанавливать привилегированный режим.
...
//				"НеизвестныеДанные" - при обработке были найдены данные, которые не планировались к анализу,
//				                      замена не реализована.
...
//
// Параметры:
//  Значение - Перечисление - значение перечисления для которого необходимо получить имя перечисления.
//
// Возвращаемое значение:
...
//	МассивНепроверяемыхРеквизитов - Массив - массив строк с именами реквизитов объекта, не требующих проверки.
//
...
// 
// Параметры:
//  СтрокаТаблицыЗначений - СтрокаТаблицыЗначений - исходная строка таблицы значений.
//
...
// 
// Параметры:
//	МенеджерЗаписи - РегистрСведенийМенеджерЗаписи - менеджер записи,
//	МетаданныеРегистра - Метаданные - метаданные регистра сведений.
//
...
//
// Параметры:
//	КоллекцияСтрок - Коллекция - коллекция, для которой доступен обход посредством 
//		оператора Для каждого … Из … Цикл.
//	ИмяКолонки - Строка - строка с именем поля коллекции, значения которого нужно выгрузить.
...
//
// Параметры:
//  XML     - Строка - текст в формате XML или ЧтениеXML.
//						   Схема XML
...
//            Строка, Число, Булево -  сложные типы для сравнения и простые типы, которые можно сравнивать на равно.
//
...
//
// Параметры:
//  РаспределяемаяСумма       - Число - распределяемая сумма;
//  КоэффициентыРаспределения - Массив - массив коэффициентов распределения;
...
//
// Параметры:
//  СтрокаXML - Строка - строка представления значения в сериализованном виде.
//
// Возвращаемое значение:
//  ЛюбоеЗначение - Значение, полученное из переданной XML-строки.
//
...
//
// Параметры:
//	ПереданныеПараметры					- Структура -	настройки подключения
//  													(необходимо использовать функцию ПараметрыПодключенияWSПрокси):
...
//		* ИспользоватьАутентификациюОС	- Булево	-	(необязательный) включает использование авторизации 
...
//		* ОтключитьИдентификаторыОбъектовМетаданных 		- Булево - отключает заполнение справочников
//																	ИдентификаторыОбъектовМетаданных и ИдентификаторыОбъектовРасширений, процедуру выгрузки и загрузки
//																	в узлах РИБ. Для частичного встраивания отдельных функций библиотеки
//																	в конфигурации без постановки на поддержку.
//
...
//
// Параметры:
//	Настройки - Структура - Структура со свойствами:
...
//
// Параметры:
//  ДлинаСтроки - Число - длина строки описания типов.
//
// ВозвращаемоеЗначение:
//  ОписаниеТипов.
...
// (См. СтандартноеХранилищеНастроекМенеджер.Сохранить) в синтакс-помощнике.
//
// Параметры:
...
// (См. СтандартноеХранилищеНастроекМенеджер.Загрузить) в синтакс-помощнике.
//
// Параметры:
//   КлючОбъекта  - Строка - Ключ объекта настройки.
//   КлючНастроек - Строка - Ключ сохраняемых настроек.
...
//             Владелец = "СтандартныеПодсистемы.УправлениеДоступом.<Уточнение>";, если нужно более 1 хранилища
//                                                                                 на подсистему БСП.
...
Функция ИзмененныеОбъектыПриЗаменеВОбъекте(ПараметрыВыполнения, МестоИспользования, ОбрабатываемыеСтроки)
	...
	// +РАРУС
	// Общие реквизиты
	Для Каждого КлючЗначение Из Описание.ОбщиеРеквизиты Цикл
		Имя = КлючЗначение.Ключ;
		ЦелеваяСсылка = ПарыЗамен[ Объект[Имя] ];
		Если ЦелеваяСсылка <> Неопределено Тогда
			Объект[Имя] = ЦелеваяСсылка;
...
Функция ОписаниеОбъекта(Знач Мета)
	...
	// +РАРУС
	// Добавим для рассмотрения общие реквизиты
	Кандидаты = Новый Структура("Реквизиты, СтандартныеРеквизиты, ОбщиеРеквизиты, ТабличныеЧасти, СтандартныеТабличныеЧасти");
	ЗаполнитьЗначенияСвойств(Кандидаты, Мета);
	
	// Заполним общими реквизитами
	МассивОбщихРеквизитов = Новый Массив;
	Для Каждого ТекущийРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
		ЗначениеСостава = ТекущийРеквизит.Состав.Найти(Мета);
		Если ЗначениеСостава=Неопределено ИЛИ (НЕ ЗначениеСостава.Использование=Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать) Тогда
			Продолжить;
		КонецЕсли;
		МассивОбщихРеквизитов.Добавить(ТекущийРеквизит);
	КонецЦикла;
	Кандидаты.ОбщиеРеквизиты = МассивОбщихРеквизитов;
	// -РАРУС
	...
	ОписаниеОбъекта.Вставить("ОбщиеРеквизиты", Новый Структура);
	Если Кандидаты.ОбщиеРеквизиты <> Неопределено Тогда
		Исключаемые = Новый Структура("Ссылка");
		
		Для Каждого МетаРеквизит Из Кандидаты.ОбщиеРеквизиты Цикл
			Имя = МетаРеквизит.Имя;
			Если Не Исключаемые.Свойство(Имя) И ОписанияТиповПересекаются(МетаРеквизит.Тип, ТипВсеСсылки) Тогда
				ОписаниеОбъекта.Реквизиты.Вставить(МетаРеквизит.Имя);
			КонецЕсли;
...
Функция ПолучитьКППНаДату(Объект, НаДату) Экспорт
	...
	Если ТипОбъекта = Тип("СправочникСсылка.Контрагенты") Тогда
		КПП = Справочники.Контрагенты.КППНаДату(Объект, НаДату);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Организации") Тогда
		КПП = Справочники.Организации.КППНаДату(Объект, НаДату);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ПодразделенияКомпании") Тогда
		КПП = Справочники.ПодразделенияКомпании.КППНаДату(Объект, НаДату);
	КонецЕсли;
...
Функция ПолучитьНаименованияНаДату(Объект, НаДату, ТипНаименования = "") Экспорт
	СтруктураНаименований = Новый Структура("СокращенноеНаименование, НаименованиеПолное, НаименованиеДляПечатныхФорм", Объект.Наименование, Объект.НаименованиеПолное, Объект.НаименованиеПолное);
