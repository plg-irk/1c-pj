...
Функция ПодсистемыБЭД() Экспорт
	...
	Подсистемы.Вставить("ИнтеграцияЦентраЭДО", "ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияЦентраЭДО");
...
#Область ДатаВМиллисекундах

// Параметры:
//  ДатаВМиллисекундах - Число
// 
// Возвращаемое значение:
//  Строка - метка времени в формате "дд.ММММ.гггг ЧЧ:мм:сс.ССС".
Функция ПредставлениеДатыВМиллисекундах(ДатаВМиллисекундах) Экспорт
	
	Если ДатаВМиллисекундах = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	Возврат Формат('00010101' + Цел(ДатаВМиллисекундах / 1000), НСтр("ru = 'ДФ=dd.MM.yyyy HH:mm:ss;'"))
		+ "." + Формат(ДатаВМиллисекундах % 1000, "ЧЦ=3; ЧН=000; ЧВН=; ЧГ=0");
	
КонецФункции

// Параметры:
//  Значение - см. ПредставлениеДатыВМиллисекундах
// 
// Возвращаемое значение:
//  Неопределено,Число
Функция СтрокаВМиллисекунды(Значение) Экспорт
	
	Если ПустаяСтрока(Значение) Тогда
		Возврат 0;
	КонецЕсли;
	
	ЧислоСимволовМиллисекунд = 3;
	МиллисекундыЗначенияСтрокой = Прав(Значение, ЧислоСимволовМиллисекунд);
	
	ДатаВремяЗначенияСтрокой = Лев(Значение, СтрДлина(Значение) - ЧислоСимволовМиллисекунд - 1);
	ДатаВремяЗначения = СтроковыеФункцииКлиентСервер.СтрокаВДату(ДатаВремяЗначенияСтрокой, ЧастиДаты.ДатаВремя);
	Если Не ЗначениеЗаполнено(ДатаВремяЗначения) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ДатаВМиллисекундах(ДатаВремяЗначения)
		+ СтроковыеФункцииКлиентСервер.СтрокаВЧисло(МиллисекундыЗначенияСтрокой);
...
Функция ДатаВМиллисекундах(ДатаВремя)
	...
	Возврат (ДатаВремя - '00010101') * 1000;
