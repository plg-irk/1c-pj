...
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	// тк + 
	Если ТипЗнч(ДанныеУстройства) = Тип("Структура") Тогда
		Если ДанныеУстройства.Свойство("ВыгружатьТовары") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ВыгружатьТовары",         ДанныеУстройства.ВыгружатьТовары);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ВыгружатьСкидки") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ВыгружатьСкидки",         ДанныеУстройства.ВыгружатьСкидки);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ВыгружатьПерсонал") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ВыгружатьПерсонал",       ДанныеУстройства.ВыгружатьПерсонал);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ЗагружатьПродажи") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ЗагружатьПродажи",        ДанныеУстройства.ЗагружатьПродажи);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ДатаНачалаЗагрузкиЧековИКассовыхСмен") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ДатаНачалаЗагрузкиЧековИКассовыхСмен",        ДанныеУстройства.ДатаНачалаЗагрузкиЧековИКассовыхСмен);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ВыполнятьПоРасписанию") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ВыполнятьПоРасписанию",   ДанныеУстройства.ВыполнятьПоРасписанию);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("РегламентноеЗадание") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("РегламентноеЗадание",     ДанныеУстройства.РегламентноеЗадание);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ПериодичностьРасписания") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ПериодичностьРасписания", ДанныеУстройства.ПериодичностьРасписания);
		КонецЕсли;
		
		Если ДанныеУстройства.Свойство("ИндексМагазина") Тогда
			ПараметрыФормы.ПараметрыОборудования.Вставить("ИндексМагазина",          ДанныеУстройства.ИндексМагазина);
		КонецЕсли;
	КонецЕсли;
	// тк - 
...
Процедура НачатьВыборФайлаРасширенияЗавершение(Установлено, ДополнительныеПараметры) Экспорт
	...
	Если Установлено Тогда
		...
		Если Диалог.Режим = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
			Диалог.Каталог = ДополнительныеПараметры.ИмяФайла;
		КонецЕсли;
...
//
Функция СостояниеПроцесса(ТекстСообщения, Прогресс = Неопределено, Пояснение = Неопределено) Экспорт
...
// Начать выгрузку шаблонов в весы с печатью этикеток.
//
Процедура НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ВесыСПечатьюЭтикеток",
			НСтр("ru='Выберите весы с печатью этикеток'"), 
			НСтр("ru='Весы с печатью этикеток не подключены.'"),
			НСтр("ru='Весы с печатью этикеток не выбраны.'"), 
			Истина);
	Иначе
		НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
	
КонецПроцедуры

Процедура НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Параметры.Вставить("ИдентификаторУстройства", ИдентификаторУстройства);
	ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ПодключениеЗавершение", ЭтотОбъект, Параметры);
	НачатьПодключениеОборудованиеПоИдентификатору(ОписаниеОповещения, Параметры.УникальныйИдентификатор, ИдентификаторУстройства);
	
КонецПроцедуры

Процедура НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	
	ТекстСообщения = НСтр("ru='Ошибок нет.'");
	
	Если РезультатПодключения.Результат Тогда
		ВходныеПараметры  = Неопределено;
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ВыполнитьКомандуЗавершение", ЭтотОбъект, Параметры);
		НачатьВыполнениеКоманды(ОписаниеОповещения, Параметры.ИдентификаторУстройства, "ВыгрузитьШаблоны", ВходныеПараметры);
	Иначе
		ОписаниеОшибки = НСтр("ru='При подключении устройства произошла ошибка. |%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьВыгрузкуШаблоновВВесыСПечатьюЭтикеток_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ru='Ошибок нет.'");
	Иначе
		ОписаниеОшибки = НСтр("ru='При выгрузке шаблонов в оборудование произошла ошибка. |%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
