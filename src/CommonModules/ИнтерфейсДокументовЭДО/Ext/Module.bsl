...
Процедура ПриОпределенииСпискаКомандЭДО(Знач СоставКоманд, Знач ПолноеИмя, Знач НаправлениеЭД, КомандыЭДО) Экспорт
	...
	НастройкиКоманд = ИнтеграцияЭДО.НастройкиПодключаемыхКомандЭДО();
	СкрыватьПереходКТекущиеДелаЭДО = НастройкиКоманд.СкрыватьПереходКТекущиеДелаЭДО;
	...
	Если СоставКоманд.Исходящие.Найти(ПолноеИмя) <> Неопределено 
		...
		Если ИспользоватьОбменЭД Тогда
			...
			КомандаЭДО.РежимИспользованияПараметра = РежимИспользованияПараметраКоманды.Одиночный;
			КомандаЭДО.МестоРазмещения = "КомандыЭДОДополнительное";
			КомандаЭДО.Недоступна    = Не ЕстьПравоПросмотраЭД Или СкрыватьПереходКТекущиеДелаЭДО;
...
//  * Элементы - ВсеЭлементыФормы:
//  ** СтраницаСообщения - ГруппаФормы
//  ** ОтправитьСообщение - Кнопка
//  ** ТекстСообщения - ПолеВвода
//  ЭлементТитулаСхемы - ДанныеФормыЭлементДерева
Процедура НастроитьСтраницуСообщений(Форма, ЭлементТитулаСхемы) Экспорт
	
	Если Не Форма.ИспользуютсяУниверсальныеСообщения Тогда
		Форма.Элементы.СтраницаСообщения.Видимость = Ложь;
		Возврат;
	КонецЕсли;

	ДоступнаОтправкаДопСообщений = ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(Форма.Объект.Ссылка) 
		Или (ЗначениеЗаполнено(Форма.Объект.Ссылка) 
			И ЭлектронныеДокументыЭДО.СообщениеОтправлено(ЭлементТитулаСхемы.Статус));
		
	Форма.Элементы.ОтправитьСообщение.Доступность = ДоступнаОтправкаДопСообщений;
	Форма.Элементы.ТекстСообщения.Доступность = ДоступнаОтправкаДопСообщений;
	
	СписокСообщений = РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.СписокСообщений(Форма.Объект.Ссылка);
	ПредставлениеЧата = РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.ПредставлениеЧата(СписокСообщений);
...
Функция ПодготовитьДанныеПредварительногоПросмотра(ОписанияДокументовОбъектаУчета, НастройкиФормирования,
	...
	ЭлектронныеДокументыЭДО.ПроверитьПравоСозданияДокументов();
	
	МенеджерДанных = МенеджерДанныхИнтеграцииЭДО(НастройкиФормирования.ИспользоватьЦентрЭДО);
...
//  ДоступноИзменениеВидаДокумента - Булево
//
Процедура НастроитьКомандуРедактированияРеквизитовВходящихНеформализованныхДокументов(ЭлементКомандыФормы,
	ЕстьПравоОбработкиДокументов, ДоступноИзменениеВидаДокумента) Экспорт
	...
	ЭлементКомандыФормы.Видимость = ДоступноИзменениеВидаДокумента;
	ЭлементКомандыФормы.Пометка = ИспользуетсяРедактированиеРеквизитовВходящихНеформализованныхДокументов();
	
КонецПроцедуры

// Возвращаемое значение:
//  Булево
Функция ИспользуетсяРедактированиеРеквизитовВходящихНеформализованныхДокументов() Экспорт
	
	КлючОбъектаНастроек = ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО();
	КлючНастройки = "ИспользуетсяРедактированиеРеквизитовВходящихНеформализованныхДокументов";
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъектаНастроек, КлючНастройки, Ложь);
	
КонецФункции

// Проверяет возможность использования учетной записи центра ЭДО, если выключена настройка использования интеграции
// центра ЭДО - вызывает исключение.
// 
// Параметры:
//  ИдентификаторЭДО - Строка
Процедура ПроверитьВозможностьИспользованияУчетнойЗаписиЦентраЭДО(ИдентификаторЭДО) Экспорт

	Если Не НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		ВызватьИсключение УчетныеЗаписиЭДОИнтеграцияЦентраЭДО.ТекстОшибкиОтключенаНастройкаИспользоватьЦентрЭДО(
			ИдентификаторЭДО);
	КонецЕсли;
...
Функция ПодготовитьДанныеПредварительногоПросмотраПоФайлуXML(ВыбранныйФайл) Экспорт
	...
	МенеджерДанных = МенеджерДанныхИнтеграцииЭДО(НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО());
...
Функция СформироватьФайлыВыгрузкиЭДДокументооборотЦеликом(Знач ЭлектронныеДокументы, Знач АдресДанныхВХранилище) Экспорт
	...
	ПолныеИменаФайлов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		ПрисоединенныеФайлыЭлектронныхДокументов.ВыгрузитьКолонку("ПрисоединенныйФайл"), 
		"ПолноеИмяФайла");
	...
	Для каждого ЭлектронныйДокумент Из ЭлектронныеДокументы Цикл
		...
		Для каждого ФайлДокумента Из ФайлыДокумента Цикл
			...
			ИмяФайла = КаталогСообщения + ПолныеИменаФайлов[ПрисоединенныйФайл];
	...
	Если ДанныеДляПолученияПредставления.ЭтоПодписьПоДоверенности Тогда
		...
		Если ДоверенностьОпределена Тогда 
			...
			Иначе
				...
				ПроверкаДоверенности = Неопределено;
				...
				Если ДоверенностьОтозвана И ЗначениеЗаполнено(СвойстваДоверенности.ДатаОтзыва) Тогда
					ДоверенностьОтозванаНаМоментПодписи = СвойстваДоверенности.ДатаОтзыва < ДатаПодписи;
					ДоверенностьДействует = 
						ДоверенностьДействует И Не ДоверенностьОтозванаНаМоментПодписи;
				КонецЕсли;
				
				Результат.ПодписьВернаПредставление = Результат.ПодписьВернаПредставление
					+ МашиночитаемыеДоверенности.ПредставлениеРезультатаАвтоматическойПроверкиМЧД(
						ДоверенностьДействует, ПроверкаДоверенности, СвойстваДоверенности)
					+ Символы.ПС;
...
Функция НовыеДанныеЭлементовСхемы() Экспорт
	...
	ДанныеЭлементов.Колонки.Добавить("РодительскоеСообщение", Новый ОписаниеТипов("ДокументСсылка.СообщениеЭДО"));
...
Функция ВариантИнтерфейсаДокументовЭДО() Экспорт
	...
	ПриОпределенииВариантаИнтерфейсаДокументовЭДО(ВариантИнтерфейса);
	
	Возврат ВариантИнтерфейса;

КонецФункции

// Сохраняет выбранный пользователем вариант интерфейса документов ЭДО.
// 
// Параметры:
//  ИспользоватьЛегкий - Булево - для пользователя будет использоваться легкий вариант интерфейса документов ЭДО.
Процедура СохранитьНастройкуИспользованияЛегкогоВариантаИнтерфейсаДокументовЭДО(ИспользоватьЛегкий) Экспорт
	
	КлючОбъектаНастроек = ИнтерфейсДокументовЭДОКлиентСервер.КлючОбъектаНастроекИнтерфейсаЭДО();
	КлючНастройки = "ИспользуетсяЛегкийВариантИнтерфейса";
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(КлючОбъектаНастроек, КлючНастройки, ИспользоватьЛегкий);
...
Функция ВыбранЛегкийВариантИнтерфейса() Экспорт
	...
	Иначе
		...
		ОсновнойВариантИнтерфейса = ВариантыИнтерфейса.Легкий;
...
Функция ТекстСообщенияОНедоступностиОткрытияФормыТекущихДелЭДО(ИмяОткрываемойФормы, ВыбранныйВариантИнтерфейса) Экспорт
	...
	Возврат СтрШаблон(НСтр("ru = 'Использование формы ""%1"" недоступно при выбранном варианте интерфейса ""%2"".
		|Измените варианта интерфейса в настройках формы Текущих дел ЭДО'"),
...
Процедура ЗаполнитьНаименованиеВидаДокумента(ВидДокументаОбъект, ПредставлениеКомандыПечати = "") Экспорт
	...
	Иначе
		...
		ВидДокументаОбъект.КраткоеНаименование = ЭлектронныеДокументыЭДО.КраткоеНаименованиеТипаДокумента(
...
Функция НовыеПараметрыСвязиИдентификаторов() Экспорт
	...
	ПараметрыСвязи.Колонки.Добавить("ИспользоватьЦентрЭДО", ОписаниеТипаБулево);
...
Функция ПараметрыСвязиИдентификаторов(ПараметрыПолучения, КонтекстДиагностики = Неопределено, ОбновитьДанныеАбонентов = Ложь) Экспорт	
	...
	ПараметрыСвязиИдентификаторовЦентровЭДО = Неопределено;
	ПараметрыСвязиИдентификаторовВстроенногоЭДО = Неопределено;
	
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО()
		И ЗначениеЗаполнено(ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентровЭДО) Тогда
		ПараметрыПолученияДляЦентраЭДО = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыПолучения);
		ПараметрыПолученияДляЦентраЭДО.ИдентификаторыОрганизации =
			ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентровЭДО;
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль("ПриглашенияЭДОИнтеграцияЦентраЭДО");
		ПараметрыСвязиИдентификаторовЦентровЭДО =
			ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДО.ПараметрыСвязиИдентификаторов(
				ПараметрыПолученияДляЦентраЭДО, КонтекстДиагностики);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыСвязиИдентификаторовЦентровЭДО) Тогда
		ПараметрыСвязиИдентификаторов = ПараметрыСвязиИдентификаторовЦентровЭДО;
...
Функция ДанныеЗаполненияНастройкиОтправкиПриглашений(ПараметрыЗаполнения) Экспорт
	...
	ДанныеЗаполненияНастройкиЦентраЭДО = Неопределено;
	ДанныеЗаполненияНастройкиВстроенногоЭДО = Неопределено;
	
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		ПараметрыЗаполненияДляЦентраЭДО = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыЗаполнения);
		Если ИдентификаторыПоИнтеграциямЭДО <> Неопределено Тогда
			ПараметрыЗаполненияДляЦентраЭДО.ПараметрыПолученияСвязиИдентификаторов.ИдентификаторыОрганизации =
				ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентровЭДО;
		КонецЕсли;
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль("ПриглашенияЭДОИнтеграцияЦентраЭДО");
		ДанныеЗаполненияНастройкиЦентраЭДО = 
			ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДО.ДанныеЗаполненияНастройкиОтправкиПриглашений(
				ПараметрыЗаполненияДляЦентраЭДО);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеЗаполненияНастройкиЦентраЭДО) Тогда
		ДанныеЗаполненияНастройки = ДанныеЗаполненияНастройкиЦентраЭДО;
...
Функция ОбновитьДанныеДляНастройкиОтправкиПриглашений() Экспорт
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль("УчетныеЗаписиЭДОИнтеграцияЦентраЭДО");
		КонтекстДиагностики = ОбщийМодульУчетныеЗаписиЭДОИнтеграцияЦентраЭДО.ОбновитьУчетныеЗаписиЭДО();
	КонецЕсли;
...
Функция ПараметрыПросмотраЭлектронногоДокументаПоОбъектуУчета(ОбъектУчета, ПараметрыОткрытия) Экспорт
	...
	ВариантИнтерфейса = ВариантИнтерфейсаДокументовЭДО();
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО()
		Или ВариантИнтерфейса = ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО().Легкий Тогда
...
Функция НастройкиФормированияЭлектронногоДокументаОбъектаУчета(ОписаниеОбъектаУчета, ВидДокумента = Неопределено)
	...
	ИначеЕсли ОписаниеОбъектаУчета.Направление = Перечисления.НаправленияЭДО.Интеркампани Тогда
		...
		Если ЗначениеЗаполнено(Настройки) Тогда
			...
			НастройкиФормирования.ИспользоватьЦентрЭДО = УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(
	...
	Иначе		
		...
		Если ЗначениеЗаполнено(Настройки) Тогда
			...
			ИспользоватьЦентрЭДО = УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(
				Настройки.ИдентификаторОтправителя);
			Если ИспользоватьЦентрЭДО Тогда
				ПроверитьВозможностьИспользованияУчетнойЗаписиЦентраЭДО(Настройки.ИдентификаторОтправителя);
			КонецЕсли;
				
			НастройкиФормирования.ИспользоватьЦентрЭДО = ИспользоватьЦентрЭДО;
		КонецЕсли;
...
Функция НаименованиеЭлементаСхемыРегламента(ТипЭлементаРегламента);
	...
	Если ЗначениеЗаполнено(ТипСлужебногоДокумента) 
		И Не РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипЭлементаРегламента) Тогда
...
Процедура ЗаполнитьПараметрыПереформированияОсновногоДокумента(Параметры, СвойстваДокумента)
	...
	Если СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяУточнение 
		Или СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
		Параметры.ТекстВопроса = НСтр("ru = 'Документ будет переформирован. Продолжить?'");
...
Функция ДанныеДляЗаполненияСтатусовЭлектронныхДокументов(ПараметрыОбъектовУчета, УчитыватьВсеАктуальные)
	...
	Запрос.Текст =
		...
		|	АктуальныеДокументы.ЭлектронныйДокумент.ИспользоватьЦентрЭДО КАК ИспользоватьЦентрЭДО,
...
Процедура ЗаполнитьСтатусыПоАктуальнымДокументам(СтатусыЭлектронныхДокументов, АктуальныеДокументы)
	...
	Для Каждого  АктуальныйДокумент Из АктуальныеДокументы Цикл
		...
		СвойстваДокумента.Вставить("ИспользоватьЦентрЭДО", АктуальныйДокумент.ИспользоватьЦентрЭДО);
...
Функция ПараметрыВыполненияДействийПриОтправкеПолученииДокументовЭДО()
	...
	ИспользоватьИнтеграциюЦентраЭДО = НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО();
	...
	Если ИспользоватьИнтеграциюВстроенногоЭДО
		И Не ИспользоватьИнтеграциюЦентраЭДО Тогда
		ОтборУчетныхЗаписей.ИспользоватьЦентрЭДО = Ложь;
	ИначеЕсли Не ИспользоватьИнтеграциюВстроенногоЭДО
		И ИспользоватьИнтеграциюЦентраЭДО Тогда
		ОтборУчетныхЗаписей.ИспользоватьЦентрЭДО = Истина;
	КонецЕсли;
...
Функция ДоступныеДействияПоЭДО(СостояниеДокумента, СвойстваДокумента)
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправкеОтказаВПодписи Тогда
		...
		ЗаполнитьДействияПоСостояниюТребуетсяПодготовкаКОтправкеОтказаВПодписи(Действия, СвойстваДокумента);
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи Тогда
		...
		ЗаполнитьДействияПоСостояниюТребуетсяОтправкаОтказаВПодписи(Действия, СвойстваДокумента);
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту
		Или СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
		
		ЗаполнитьДействияПоСостояниюОтказаноВПодписи(Действия, СвойстваДокумента);
...
//  Действия - см. ДоступныеДействияПоЭДО
//  СвойстваДокумента - см. ДействияПоСостояниюДокумента.СвойстваДокумента
Процедура ЗаполнитьДействияПоСостояниюТребуетсяПодготовкаКОтправкеОтказаВПодписи(Действия, СвойстваДокумента)
...
//  Действия - см. ДоступныеДействияПоЭДО
//  СвойстваДокумента - см. ДействияПоСостояниюДокумента.СвойстваДокумента
Процедура ЗаполнитьДействияПоСостояниюТребуетсяОтправкаОтказаВПодписи(Действия, СвойстваДокумента)
...
//  Действия - см. ДоступныеДействияПоЭДО
//  СвойстваДокумента - см. ДействияПоСостояниюДокумента.СвойстваДокумента
Процедура ЗаполнитьДействияПоСостояниюОтказаноВПодписи(Действия, СвойстваДокумента)
	
	Если Не ЭтоВходящийЭДО(СвойстваДокумента.Ссылка) Тогда
		Действия.Вставить(Перечисления.ДействияПоЭДО.СформироватьИсправление, Истина);
	КонецЕсли;
	...
	Для Каждого ЭлементИсточника Из КоллекцияЭлементовИсточника Цикл
		...
		Если ЗначениеЗаполнено(ЭлементПриемника.Сообщение) 
			...
			СвойстваДокумента = ЭлектронныеДокументыЭДО.СвойстваДокумента(ЭлементПриемника.ЭлектронныйДокумент,
				"Ссылка, НаОзнакомлении, СпособОбмена, ВидДокумента, ИспользоватьЦентрЭДО");
...
// Доступны для выбора варианты Расширенный и Легкий, ЛегкийКлиентЭДО доступен для выбора только в Клиенте ЭДО
// в рамках проекта БизКуб.
// 
// Параметры:
//  ВариантИнтерфейса - Строка - См. ИнтерфейсДокументовЭДОКлиентСервер.ВариантыИнтерфейсаДокументовЭДО
Процедура ПриОпределенииВариантаИнтерфейсаДокументовЭДО(ВариантИнтерфейса)
