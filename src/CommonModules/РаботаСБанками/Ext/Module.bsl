...
Процедура ПолучитьДанныеРБК(ПараметрыЗагрузкиКлассификатора, АдресХранилища = "") Экспорт
	...
	Если ПараметрыПолученияФайловРБК["ЕстьОбновлениеНаСервере"] Тогда
		...
		Исключение
			ТекстСообщения = НСтр("ru ='Возникли проблемы с файлом классификатора банков, полученным из Интернета.'");
...
Процедура ПолучитьДанныеРБКИзИнтернета(ПараметрыПолученияФайловРБК)
	...
	Если ИспользоватьАльтернативныйСервер Тогда
		ФайлРБКНаВебСервере = "http://cbrates.rbc.ru/bnk/bnk.zip";
	Иначе
		ФайлРБКНаВебСервере = "https://bankregister.1c.ru/bankregister/v1/bnk.zip";
	КонецЕсли;
	
	ВременныйФайлРБК = ПараметрыПолученияФайловРБК["ВременныйКаталог"]+ "\bnk.zip";
