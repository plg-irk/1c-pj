...
Процедура ЗапуститьПрограммуНачатьЗапуск(Контекст)
	...
	Иначе
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() И ЗначениеЗаполнено(КодировкаИсполнения) Тогда
...
Процедура ЗапуститьПрограммуСНаивысшимиПравамиWindows(Контекст)
	...
	ТекстовыйДокумент = ОбменСДаннымиТККлиентСервер.НовыйФайлЗапускаКомандыWindows(
