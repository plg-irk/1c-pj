...
//
// Параметры:
//  РезультатПолученияДокументов - см. НовыйРезультатПолученияДокументов
//  КлючиСинхронизации - см. СинхронизацияЭДОКлиентСервер.НовыеКлючиСинхронизации
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  РезультатыОтправкиПолучения - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеРезультатыОтправкиПолучения
//
Процедура ПолучитьНовыеЭлектронныеДокументыЧерезОператораЭДО(
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		ОписанияДокументов = ПолучитьОписаниеДокументов(КлючСинхронизации, ДатаПоследнегоЗапроса,
			ДатаПоследнегоЗапросаЭПД, КонтекстДиагностики);
			
		НеЗагруженныеВБазуДокументы = ОписанияДокументов.НеЗагруженныеВБазуДокументы;
		...
		УспешнаяЗагрузкаОдиночныхДокументов = ЗагрузитьОдиночныеДокументы(НеЗагруженныеВБазуДокументы,
			РезультатПолученияДокументов, РезультатыОтправкиПолучения,
			ПараметрыПолучения, КонтекстДиагностики);
		
		Если Не УспешнаяЗагрузкаОдиночныхДокументов
			И ТребуетсяПовторноеВыполнениеОперации(КлючСинхронизации) Тогда
			Прервать;
		КонецЕсли;
		
		УспешнаяЗагрузкаПакетовДокументов = ЗагрузитьПакетыДокументов(НеЗагруженныеВБазуДокументы,
			РезультатПолученияДокументов, РезультатыОтправкиПолучения,
			ПараметрыПолучения, КонтекстДиагностики);
		
		УспешноеПолучениеТранспортныхКонтейнеров = УспешнаяЗагрузкаОдиночныхДокументов
			И УспешнаяЗагрузкаПакетовДокументов;
		
		ВсеОписанияДокументов = ОписанияДокументов.ВсеДокументы;
		
		Если УспешноеПолучениеТранспортныхКонтейнеров И ПараметрыПолучения.СдвигатьДату
			И ЗначениеЗаполнено(ДатаПоследнегоЗапроса) И ВсеОписанияДокументов.Количество() > 0 Тогда
			ДатаПолученияЭД = ДатаПоследнегоЗапроса;
			СинхронизацияЭДО.ИзменитьСостояниеОбмена(ИдентификаторОрганизации, "ДатаПолученияДокументов",
				ДатаПолученияЭД);
		КонецЕсли;
		
		Если ПараметрыПолучения.СдвигатьДату И ЗначениеЗаполнено(ДатаПоследнегоЗапросаЭПД)
			И ВсеОписанияДокументов.Количество() > 0 Тогда
		...
		Если Не ТребуетсяПовторноеВыполнениеОперации(КлючСинхронизации) Тогда
			ДатаУспешногоПолученияЭД = ?(ЗначениеЗаполнено(ДатаПолученияЭД), ДатаПолученияЭД,
				ДатаПолученияЭДИзНастройки);
		...
		Если КлючСинхронизации.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
			РезультатПолученияДокументов.УчетныеЗаписи1СЭДОПолученыВсеДокументы.Добавить(
...
// Параметры:
//  НеЗагруженныеВБазуДокументы - см. СервисЭДО.НоваяТаблицаКонтейнеров
//  РезультатПолученияДокументов - см. НовыйРезультатПолученияДокументов
//  РезультатыОтправкиПолучения - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеРезультатыОтправкиПолучения
...
// 
// Возвращаемое значение:
//  Булево - документы успешно загружены.
Функция ЗагрузитьОдиночныеДокументы(НеЗагруженныеВБазуДокументы, РезультатПолученияДокументов,
	РезультатыОтправкиПолучения, ПараметрыПолучения, КонтекстДиагностики)

	Отбор = Новый Структура;
	Отбор.Вставить("ИДПакета", "");
	ДокументыВнеПакетов = НеЗагруженныеВБазуДокументы.Скопировать(Отбор);
	
	Возврат ПолучитьДокументы(ДокументыВнеПакетов, КонтекстДиагностики, РезультатПолученияДокументов,
		РезультатыОтправкиПолучения, ПараметрыПолучения);
...
// Возвращаемое значение:
//  Булево - документы успешно загружены.
Функция ЗагрузитьПакетыДокументов(НеЗагруженныеВБазуДокументы, РезультатПолученияДокументов,
	РезультатыОтправкиПолучения, ПараметрыПолучения, КонтекстДиагностики)

	УспешноеПолучениеТранспортныхКонтейнеров = Истина;
	
	ИдентификаторыПакетов = ОбщегоНазначения.ВыгрузитьКолонку(НеЗагруженныеВБазуДокументы, "ИДПакета", Истина);
	...
	Для Каждого ИдентификаторПакета Из ИдентификаторыПакетов Цикл
		...
		ЗагруженыВсеКонтейнерыПакета = Истина;
		
		Отбор = Новый Структура;
		Отбор.Вставить("ИДПакета", ИдентификаторПакета);
		ТранспортныеКонтейнерыПакета = НеЗагруженныеВБазуДокументы.Скопировать(Отбор);
		
		Для Каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнерыПакета Цикл
			ДвоичныеДанныеТранспортногоКонтейнера = СервисЭДО.ПолучитьДокумент(
				ПараметрыПолучения.КлючСинхронизации, ТранспортныйКонтейнер, КонтекстДиагностики);
			...
			Если Не ЗначениеЗаполнено(ДвоичныеДанныеТранспортногоКонтейнера) Тогда
				ЗагруженыВсеКонтейнерыПакета = Ложь;
			КонецЕсли;
		КонецЦикла;
		
		Если Не ЗагруженыВсеКонтейнерыПакета Тогда
			Продолжить;
		КонецЕсли;
		
		НачатьТранзакцию();
		Попытка
			УспешноеПолучениеТранспортныхКонтейнеров = ПолучитьДокументы(ТранспортныеКонтейнерыПакета,
				КонтекстДиагностики, РезультатПолученияДокументов, РезультатыОтправкиПолучения,
				ПараметрыПолучения, Истина);
		...
		Если Не УспешноеПолучениеТранспортныхКонтейнеров 
			И ТребуетсяПовторноеВыполнениеОперации(ПараметрыПолучения.КлючСинхронизации) Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат УспешноеПолучениеТранспортныхКонтейнеров;
...
Функция СформироватьИОтправитьТранспортныеКонтейнеры(КонтекстОтправки, КонтекстДиагностики,
	...
	Для Каждого Отправитель Из СписокОтправителейДляОбновленияИнформацииОбАктивности Цикл
		...
		// Список отправителей заведомо меньше чем обновление информации при отправке каждого контейнера
...
#Область РегистрыСведений_СертификатыУчетныхЗаписейЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами:  заполнение должностей сотрудников использующих сертификаты.'");
	
	Читаемые = Новый Массив;
	Читаемые.Добавить(Метаданные.РегистрыСведений.СертификатыУчетныхЗаписейЭДО.ПолноеИмя());
	Читаемые.Добавить(Метаданные.Справочники.СертификатыКлючейЭлектроннойПодписиИШифрования.ПолноеИмя());
