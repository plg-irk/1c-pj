...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
	...
	//Мальцев А.Ю +++ Общая дата запрета по разделам	
	Если Не НайденЗапретПоРазделам Тогда
		...
		Если Объекты  <> Неопределено И Адресаты <> Неопределено Тогда
		
			// Поиск для любого раздела и любого объекта (общая дата).
			ДатаЗапрета = НайтиДатуЗапрета(Адресаты, РазделЗапрета, ОбъектЗапрета, ДополнительныеПараметры);

			Если ДатаЗапрета <> Неопределено И Данные <> НЕОПРЕДЕЛЕНО И ДатаЗапрета >= Данные.Дата Тогда
