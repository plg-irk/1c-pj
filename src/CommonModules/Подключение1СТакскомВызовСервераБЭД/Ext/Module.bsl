...
// ОбщийМодуль.Подключение1СТакскомВызовСервераБЭД.
//
////////////////////////////////////////////////////////////////////////////////
...
Функция URLДляПереходаНаСтраницуИнтегрированногоСайта(URLСтраницыСайта) Экспорт
	...
	Возврат ИнтернетПоддержкаПользователей.URLДляПереходаНаСтраницуИнтегрированногоСайта(URLСтраницыСайта);
...
//		Подключение1СТакскомБЭД.НовыйОписаниеСервисаИПП();
...
Функция НовыйКонтекстВзаимодействия(
	...
	СтруктураЗапуска = Подключение1СТакскомБЭД.ОпределитьВозможностьЗапускаПоМестуИПараметрам(
		МестоЗапуска,
		ПараметрыИПП);
	...
	ОсновныеПараметры.ИмяWSОпределения      = Подключение1СТакскомБЭД.АдресWSDLСервисаБизнесПроцессов();
	ОсновныеПараметры.URIСервиса            = Подключение1СТакскомБЭД.URIСервисаСервисаБизнесПроцессов();
...
Функция КонфигурацияЗарегистрированаВСервисеИПП(ОшибкаОбращенияКВебСервису = Ложь) Экспорт
	...
	Возврат Подключение1СТакскомБЭД.КонфигурацияЗарегистрированаВСервисеИПП(
...
Процедура ОбработкаКомандСервиса(
	...
	Если НЕ Продолжение Тогда
		...
		Если СоздаваемыйКонтекстВзаимодействия <> Неопределено Тогда
			Подключение1СТакскомБЭД.ДобавитьДополнительныеСтартовыеПараметрыЗапроса(
				КСКонтекст,
	...
	Попытка
		...
		Пока КонтекстОбработчика.Команды.Количество() > 0
			...
			ТипКоманды = Подключение1СТакскомКлиентСерверБЭД.ТипКоманды(
				ТекущаяКоманда,
...
Процедура ДобавитьКомандыСервиса(
	...
	Попытка
		...
		Подключение1СТакскомБЭД.ДобавитьКомандыСервиса(
			ОписаниеСервисаИППНаСервере,
			ДополнительныеПараметры,
			КонтекстОбработчика,
			ОсновныеПараметры,
...
Процедура ВыполнитьКомандуСервиса(КСКонтекст, СтруктураКоманды, КонтекстОбработчика)
	...
	ИначеЕсли ИмяКоманды = "store.putorganizations" Тогда
		Подключение1СТакскомБЭД.ЗаписатьСписокОрганизаций(КСКонтекст, СтруктураКоманды);
		
	ИначеЕсли ИмяКоманды = "store.putadressclassifier" Тогда
		Подключение1СТакскомБЭД.ЗаписатьАдресныйКлассификатор(КСКонтекст, СтруктураКоманды);
	...
	ИначеЕсли ИмяКоманды = "setcodesregion" Тогда
		Подключение1СТакскомКлиентСерверБЭД.СохранитьВПараметрахКодыРегионов(КСКонтекст, СтруктураКоманды);
...
Процедура ВыполнениеКомандыПрочитатьПараметры(
	...
	Для Каждого Параметр Из МассивПараметров Цикл
		...
		ЗначениеПараметра = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(
			КСКонтекст,
			Параметр.Имя);
...
Процедура ВыполнениеКомандыЗаписьПараметров(КСКонтекст, СтруктураКоманды, КонтекстОбработчика)
	...
	Для каждого Параметр Из ЗаписываемыеПараметры Цикл
		...
		Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(
			КСКонтекст,
			Параметр.Имя,
			Параметр.Значение,
...
Процедура ЗаписатьСтартовыеПараметрыКонтекста(Знач СтартовыеПараметрыСтруктура, КСКонтекст)
	...
	Если СтартовыеПараметрыСтруктура <> Неопределено Тогда
		...
		Для каждого ПараметрВСтруктуре Из СтартовыеПараметрыСтруктура Цикл
			...
			Параметр = Подключение1СТакскомКлиентСерверБЭД.НовыйПараметрИПП(
...
Процедура ПодготовитьСертификатЭПКОтправке(КСКонтекст, КонтекстОбработчика)
	...
	СертификатЭП = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(
		КСКонтекст,
		"IDCertificateED");
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Организация = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(КСКонтекст,
		"IDOrganizationED");
	...
	СтруктураДанныхОбОрганизации = Неопределено;
	...
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "postindexED", Индекс);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addressregionED", Регион);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addresscoderegionED", "");
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст,
		"addresstownshipED",
		Район);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addresscityED", Город);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст,
		"addresslocalityED",
		НаселенныйПункт);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addressstreetED", Улица);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addressbuildingED", Дом);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст,
		"addresshousingED",
		Корпус);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст,
		"addressapartmentED",
		Квартира);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "addressphoneED", Телефон);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "agencyED", Наименование);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "orgindED", ЮрФизЛицо);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "innED", ИНН);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "kppED", КПП);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "ogrnED", ОГРН);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "codeimnsED", КодИМНС);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "lastnameED", Фамилия);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "firstnameED", Имя);
	Подключение1СТакскомКлиентСерверБЭД.ЗаписатьПараметрКонтекста(КСКонтекст, "middlenameED", Отчество);
...
Процедура ПодготовитьДанныеСертификатаПоОтпечатку(КСКонтекст, КонтекстОбработчика)
	...
	ОтпечатокСертификата = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(
		КСКонтекст,
...
Процедура ЗаписатьОбщиеПараметрыИППСлужебная(Знач ОбщиеПараметры)
	...
	ИдентификаторПодсистемы = Подключение1СТакскомБЭД.ИдентификаторПодсистемы();
	...
	Если Логин <> Неопределено И Пароль <> Неопределено Тогда
		Подключение1СТакскомБЭД.СлужебнаяСохранитьДанныеАутентификации(
...
Процедура УдалитьОбщиеПараметрыИППСлужебная(Знач УдаляемыеИзРС)
	...
	ИДПодсистемы = Подключение1СТакскомБЭД.ИдентификаторПодсистемы();
	НачатьТранзакцию();
	Попытка
	...
	Если УдаленыДанныеАутентификации Тогда
		Подключение1СТакскомБЭД.СлужебнаяСохранитьДанныеАутентификации(Неопределено);
	КонецЕсли;
...
Процедура ДобавитьЗначенияСтартовыхПараметров(ПараметрыЗапроса, ИменаСтартовыхПараметров, ИмяWSОпределения, КСКонтекст)
	...
	ДанныеИПП = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
		Подключение1СТакскомБЭД.ИдентификаторПодсистемы(),
		ИменаСтартовыхПараметров);
	...
	Если СтрНайти(ИменаСтартовыхПараметров, ",") > 0 Тогда
		...
		Для Каждого КлючЗначение Из ДанныеИПП Цикл
			...
			Если НРег(СокрЛП(КлючЗначение.Ключ)) <> "password" Тогда
				СессионныйПараметр = Подключение1СТакскомКлиентСерверБЭД.НовыйПараметрИПП(
					КлючЗначение.Ключ,
					КлючЗначение.Значение,
					"startup");
...
Процедура ДобавитьСессионныеПараметрыКЗапросу(
	...
	Если ИменаСессионныхПараметров <> Неопределено Тогда
		...
		Для каждого ИмяПараметра Из ИменаСессионныхПараметров Цикл
			...
			ЗначениеПараметра = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(КСКонтекст, ИмяПараметра);
	...
	Если НЕ ИдентификаторСеансаДобавлен Тогда
		...
		ИдентификаторСеансаИПП = Подключение1СТакскомБЭД.ЗначениеСессионногоПараметра(КСКонтекст, "session_id");
