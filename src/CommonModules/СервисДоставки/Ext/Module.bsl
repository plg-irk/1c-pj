...
Процедура ПараметрыСервисаДоставки(ПараметрыФормы) Экспорт
	...
	ДанныеОрганизаций = СервисДоставкиПовтИсп.ДоступныеПодключенныеОрганизации();
...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено, ПараметрыОповещения = Неопределено) Экспорт
	...
	Если ЭлементУправления <> Неопределено Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "Ссылка")
			...
			ЗаказыПоОснованиям = РегистрыСведений.ДанныеЗаказовСервисДоставки.КоличествоЗаказовПоОснованиям(Форма.Объект.Ссылка);
			
			КоличествоДокументов = 0;
			Если ЗаказыПоОснованиям[Форма.Объект.Ссылка] <> Неопределено Тогда
				КоличествоДокументов = ЗаказыПоОснованиям[Форма.Объект.Ссылка];
			КонецЕсли;
...
Функция СоздатьИзменитьОтправитьЗаказыНаДоставку(Знач ПараметрыВыполнения, Знач АдресРезультата) Экспорт
	...
	ГрупповаяОбработка = ПараметрыВыполнения.ГрупповаяОбработка;
	...
	Для Каждого ДанныеЗаказа Из МассивЗаказов Цикл
		...
		Если ПустаяСтрока(ДанныеЗаказа.Идентификатор) Тогда
			...
			Если ДанныеЗаказа.ДокументыОснования.Количество() > 0 Тогда
				ДанныеЗаказа.ПунктВыдачиКлиента.Идентификатор = 
					ПолучитьПунктВыдачиКлиента(Организация, ТипГрузоперевозки,
...
Функция НайтиЗаказыНаДоставкуПоОснованию(Ссылка, ИсключатьИзмененные = Ложь, ДопустимыеСостояния = Неопределено) Экспорт
	...
	СписокОснований = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ссылка);
	
	СервисДоставкиПереопределяемый.ПриОпределенииОснованийЗаказаНаДоставку(СписокОснований);
	...
	|ГДЕ
	|	ТаблицаРегистра.ДокументОснование В (&СписокОснований)
	|	И ВЫБОР
...
Процедура ЗаполнитьСервисыДоставки(СервисыДоставки) Экспорт
	...
	СервисДоставкиПереопределяемый.ПриЗаполненииСервисовДоставки(ДоступныеСервисы, СтандартнаяОбработка);
	
	Если СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
	
	МаксИндекс = СервисыДоставки.Количество()-1;
	
	Для Инд = 0 По МаксИндекс Цикл
		Строка = СервисыДоставки[МаксИндекс - Инд];
		Если ДоступныеСервисы.Найти(Строка.ТипГрузоперевозки) = Неопределено Тогда
			СервисыДоставки.Удалить(Строка);
		КонецЕсли;
	КонецЦикла;
...
//
// Возвращаемое значение:
//  Строка - идентификатор пункта выдачи клиента
Функция ПолучитьПунктВыдачиКлиента(Организация, ТипГрузоперевозки, Склад = Неопределено) Экспорт
	...
	|	И ТаблицаСоответствий.ТипОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовСервисовДоставки.ПунктВыдачи)
	|
	|УПОРЯДОЧИТЬ ПО
...
// Изменить использование задания синхронизация с сервисом интеграции.
// 
// Параметры:
//  ТипСинхронизации - ПеречислениеСсылка.ТипыСинхронизацииСервисДоставки
...
// Отключить использование регламентного задания.
// 
// Параметры:
//  РегламентноеЗадание - РегламентноеЗадание
//  ПараметрыПоиска - Структура
//   * Метаданные - РегламентныеЗадания
//   * Использование - Булево
...
#Область РегистрыСведений_ДанныеЗаказовСервисДоставки_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Версия = "1.9.12.27"; // Версия БЭД
	...
	Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, НСтр("ru = 'Заполняет новый регистр сведений'"),
	...
	Изменяемые.Добавить(Метаданные.РегистрыСведений.ДанныеЗаказовСервисДоставки.ПолноеИмя());
	...
	Обработчик.ПроцедураПроверки = ИмяПроцедурыПроверки;
	Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, НСтр("ru = 'Заполняет данные регистра сведений'"),
...
#КонецОбласти
	...
	// Очистка регистра СоответствиеОбъектовСервисовДоставки
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = 
		"РегистрыСведений.СоответствиеОбъектовСервисовДоставки.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "1.9.15.4";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами = Истина;
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("487f5b2d-32bd-4a78-9756-39d005380113");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = 
		"РегистрыСведений.СоответствиеОбъектовСервисовДоставки.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = ИмяПроцедурыПроверки;
	Обработчик.Комментарий = СтрШаблон(ШаблонКомментария, НСтр("ru = 'Удаляет данные регистра сведений'"),
		Метаданные.РегистрыСведений.СоответствиеОбъектовСервисовДоставки.Синоним);
	ПолноеИмяОбъекта = Метаданные.РегистрыСведений.СоответствиеОбъектовСервисовДоставки.ПолноеИмя();
...
// * ОрганизацияБизнесСетиСсылка - ОпределяемыйТип.ОрганизацияСервисДоставки - Необязательный параметр. Если указан, будет добавлен во все параметры методов, входящих в Очередь.
...
Процедура ВыполнитьЗагрузкуДанныхПоНаложеннымПлатежам(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Иначе
		...
		ТаблицаОрганизацииБизнесСети = СервисДоставкиПовтИсп.ПодключенныеОрганизации();
...
Функция ОрганизацияПоУмолчанию(ТипГрузоперевозки) Экспорт
	...
	ДоступныеПодключенныеОрганизации = СервисДоставкиПовтИсп.ДоступныеПодключенныеОрганизации();
	Если ЗначениеЗаполнено(ОрганизацияБизнесСетиСсылка) Тогда
		Если ДоступныеПодключенныеОрганизации.Найти(ОрганизацияБизнесСетиСсылка, "Организация") = Неопределено Тогда
			ОрганизацияБизнесСетиСсылка = Неопределено;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОрганизацияБизнесСетиСсылка) Тогда
		Если ДоступныеПодключенныеОрганизации.Количество() > 0 Тогда
			ОрганизацияБизнесСетиСсылка = ДоступныеПодключенныеОрганизации[0].Организация;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьПараметрыЗапросаСервиса(ЗапросСервиса, ПараметрыЗапроса, ТипГрузоперевозки, ИмяМетода = "")
	...
	ИначеЕсли ИмяМетода = СервисДоставкиКлиентСервер.ИмяПроцедурыОформитьЗаказНаДоставку() Тогда
		...
		Если СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) Тогда
			ЗапросСервиса.Контент = ТипДанныхКонтентаОбработкаЗаказНаДоставку_Версия2();
		Иначе
	...
	ИначеЕсли ИмяМетода = СервисДоставкиКлиентСервер.ИмяПроцедурыДобавитьЗаказНаДоставкуВМультизаказ()
		...
		ЗапросСервиса.Заголовки.Вставить("Accept", ТипДанныхКонтентаОбработкаЗаказНаДоставку_Версия2());	
	КонецЕсли;
...
Функция ТипДанныхКонтентаОбработкаЗаказНаДоставку_Версия2()
