...
Функция ДанныеОФорматахОтправителяИПолучателя(ВидЭлектронногоДокумента, ВерсияФормата, ИдентификаторПолучателя, ДатаПроверки = Неопределено)
	...
	Если Не НастройкиЭДО.ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО() Тогда
		Возврат Результат;
	КонецЕсли;
...
#Область НачальноеЗаполнение
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "Константы.ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("38271f36-72d3-4a3e-bf1b-d9134c92b070");
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "ЭлектронноеВзаимодействие.ЗаполнениеДанныхОбновления";
	Обработчик.ОчередьОтложеннойОбработки = 1;
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЧитаемыеОбъекты = "Константа.ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла";
	Обработчик.ИзменяемыеОбъекты = "Константа.ИспользоватьЗаполнениеРеквизитовВходящегоДокументаПоНаименованиюФайла";
	Обработчик.БлокируемыеОбъекты = "";
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: Включает заполнение реквизитов входящего неформализованного документа по наименованию файла при загрузке.'");
