...
Процедура ПечатьПоВнешнемуИсточнику(ДополнительнаяОбработкаСсылка, ПараметрыИсточника, КоллекцияПечатныхФорм,
	...
	ВнешняяОбработкаОбъект = ОбъектВнешнейОбработки(ДополнительнаяОбработкаСсылка);
...
//           
//           ** ПоказыватьОповещение - Булево - если Истина, то при запуске команды 
//                                              выводится оповещение "Команда выполняется...".
...
Функция СведенияОВнешнейОбработке(ВерсияБСП = "") Экспорт
	...
	ТаблицаКоманд.Колонки.Добавить("ПроверкаПроведенияПередПечатью");
	ТаблицаКоманд.Колонки.Добавить("ПроверкаИсключенияЗапретаРедактирования");
...
Процедура ВыполнитьНазначаемуюКомандуНаСервере(Форма, ИмяЭлемента, РезультатВыполнения = Неопределено) Экспорт
	...
	ВнешнийОбъект = ОбъектВнешнейОбработки(ОписаниеКоманды.Ссылка);
...
//
// Возвращаемое значение: 
//   Массив - массив объектов метаданных Подсистема - метаданные разделов,
//   в которые выведен список команд дополнительных отчетов.
//
...
// Возвращаемое значение: 
//   Массив - массив объектов метаданных Подсистема - метаданные разделов,
//   в которые выведен список команд дополнительных обработок.
//
...
//
// Параметры:
//  ЭлементДанных - объекты базы данных, наборы записей регистров, последовательностей или перерасчетов - содержит
//          																элемент данных, прочитанный из сообщения обмена данными.
//  ПолучениеЭлемента - ПолучениеЭлементаДанных - определяет поведение системы при получении элементов данных.
//
...
Процедура ПриДобавленииСлужебныхСобытий(КлиентскиеСобытия, СерверныеСобытия) Экспорт
	...
	//
	// Синтаксис:
	// Процедура ПриПроверкеВозможностиЗагрузкиОбработкиИзФайла(Знач ДополнительнаяОбработка, Результат,
	...
	//
	// Синтаксис:
	// Процедура ПриПроверкеВозможностиВыгрузкиОбработкиВФайл(Знач ДополнительнаяОбработка, Результат,
	//                                                        СтандартнаяОбработка) Экспорт
	//
	...
	//
	// Синтаксис:
	// Процедура ПриПроверкеНеобходимостиОтображенияРасширеннойИнформации(Знач ДополнительнаяОбработка, Результат,
	//                                                                    СтандартнаяОбработка) Экспорт
	//
	...
	//
	// Параметры:
	//   Объект - СправочникОбъект.ДополнительныеОтчетыИОбработки - Объект дополнительной обработки.
	//   РегистрационныеДанные - Структура - См. описание функции 
	//                                       ДополнительныеОтчетыИОбработки.СведенияОВнешнейОбработке().
	//   СтандартнаяОбработка - Булево - Признак продолжения стандартной обработки этого события.
	//
	// Синтаксис:
	...
	//
	// Синтаксис:
	// Процедура ПриПолученииРазрешенийСессииБезопасногоРежима(Знач КлючСессии, ОписанияРазрешений,
	//                                                         СтандартнаяОбработка) Экспорт
	//
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ЕстьКомандаОтчетов = (Метаданные.ОбщиеКоманды.Найти("ЗаполнениеОбъекта") <> Неопределено);
	ЕстьКомандаЗаполнения = (Метаданные.ОбщиеКоманды.Найти("ОтчетыОбъекта") <> Неопределено);
	
	Если УстанавливатьПараметрыФО Или ЕстьКомандаОтчетов Или ЕстьКомандаЗаполнения Тогда
	...
	Если ТипыЗаполнениеОбъекта.Количество() > 0 Тогда
		Если ЕстьКомандаЗаполнения И ИОМы.Количество() = 1 Тогда
	...
	Если ТипыОтчетов.Количество() > 0 Тогда
		Если ЕстьКомандаОтчетов И ИОМы.Количество() = 1 Тогда
			НастройкиФормы.ФункциональныеОпции.Вставить("ДополнительныеОтчетыИОбработкиОбъектНазначения", ИОМы[0]);
		Иначе
			Команда = Команды.Добавить();
...
Процедура ПриОпределенииТипаИСсылкиЕслиОтчетДополнительный(ОтчетИнформация) Экспорт
	...
	Если ОтчетИнформация.ПоУмолчаниюВсеПодключеныКХранилищу Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Таблица.ИспользуетХранилищеВариантов = ИСТИНА", "");
	КонецЕсли;
...
Процедура ПриПолученииКомандПечати(КомандыПечати, ИмяОбъекта) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКоманд Цикл
		...
		// вывод заголовка
		КомандаПечати.ЗаголовокФормы = Строка(СтрокаТаблицы.Ссылка);
...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ЗаполнениеОбъекта
		...
		ИЛИ Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		ИЛИ Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.СозданиеСвязанныхОбъектов Тогда
		МассивТиповИлиМетаданных = ДобавитьВсеОбъектыДляСсылочногоТипаМетаданных();
	КонецЕсли;
	// -РАРУС
...
Функция НовыйЗапросПоДоступнымКомандам(ВидОбработок, Размещение, ЭтоФормаОбъекта = Неопределено, ТипыКоманд = Неопределено, ТолькоВключенные = Истина) Экспорт
	...
	ЗапросАдаптированКМоделиСервиса = ИспользуетсяТеснаяИнтеграцияСПодсистемойВМоделиСервиса()
		И АдаптироватьЗапросПоДоступнымКомандамКМоделиСервиса(Запрос);
	Если Не ЗапросАдаптированКМоделиСервиса Тогда
...
// +РАРУС Возвращает массив объектов метаданных
Функция ДобавитьВсеОбъектыДляСсылочногоТипаМетаданных()
	МассивМетаданных = Новый Массив;
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.Справочники);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.Документы);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.ПланыВидовХарактеристик);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.ПланыСчетов);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.ПланыВидовРасчета);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.БизнесПроцессы);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.Задачи);
	ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, Метаданные.ПланыОбмена);
	Возврат МассивМетаданных;
КонецФункции // -РАРУС

// +РАРУС
Процедура ЗаполнитьМассивОбъектовМетаданных(МассивМетаданных, КоллекцияОбъектовМетаданных)
...
Функция ПолучитьВидОбработкиПоСтроковомуПредставлениюВида(СтроковоеПредставление) Экспорт
	...
	ИначеЕсли СтроковоеПредставление = ДополнительныеОтчетыИОбработкиКлиентСервер.ВидОбработкиПроверкаЗаполнения() Тогда
		Возврат Перечисления.ВидыДополнительныхОтчетовИОбработок.ПроверкаЗаполнения;
	КонецЕсли;
...
// Определяет используется-ли в текущем сеансе фильтр подключенных ДОИО по версии модели сервиса.
Функция ИспользуетсяТеснаяИнтеграцияСПодсистемойВМоделиСервиса()
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	|	И ДополнительныеОтчетыИОбработкиНазначение.Ссылка.Вид = ЗНАЧЕНИЕ(Перечисление.ВидыДополнительныхОтчетовИОбработок.ПроверкаЗаполнения)
	//|	И ДополнительныеОтчетыИОбработкиНазначение.Ссылка.ИспользоватьДляФормыОбъекта = ЛОЖЬ
	//|	И ДополнительныеОтчетыИОбработкиНазначение.Ссылка.ИспользоватьДляФормыСписка = ЛОЖЬ
	...
	|ПО
	|	втСсылки.Ссылка = ДополнительныеОтчетыИОбработкиКоманды.Ссылка И 
	|	ДополнительныеОтчетыИОбработкиКоманды.ВариантЗапуска = ЗНАЧЕНИЕ(Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода)
	|ГДЕ
	|	ДополнительныеОтчетыИОбработкиКоманды.ВариантЗапуска = ЗНАЧЕНИЕ(Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода)
	|УПОРЯДОЧИТЬ ПО
	|	Представление";
	
	ОбъектНазначения = СтандартныеПодсистемыСервер.ИдентификаторОбъектаМетаданных(Объект.Ссылка.Метаданные());
	...
	Пока Выборка.Следующий() Цикл
		...
		ВнешнийОбъект = ОбъектВнешнейОбработки(Выборка.Ссылка);
		ИдентификаторКоманды = Выборка.Идентификатор;
		...
		ПараметрыКоманды.Вставить("ПроверяемыеРеквизиты", ПроверяемыеРеквизиты);
		ПараметрыКоманды.Вставить("ОбъектыНазначения",    ОбъектыНазначения);
		
		РезультатВыполнения = ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, Неопределено);
		
		Отказ                = ПараметрыКоманды.Отказ;
		ПроверяемыеРеквизиты = ПараметрыКоманды.ПроверяемыеРеквизиты;
...
Процедура СформироватьПодменюКомандЗаполнения(Форма, Параметры)
	...
	Для НомерЭлемента = 0 По ТаблицаКоманд.Количество() - 1 Цикл
		...
		ИмяЭлемента = "КомандаДополнительнойОбработкиЗаполнения" + Формат(НомерЭлемента, "ЧГ=");
		ОписаниеКоманды.ИмяЭлемента = ИмяЭлемента;
		
		// +РАРУС  Внесены правки, так как нужно использовать другое действие команды.
		ЗащищенныеФункцииСервер.ДобавитьКоманду(Форма, ИмяЭлемента, Подменю.Имя,, ОписаниеКоманды.Представление,, ЛОЖЬ,,,ОтображениеКнопки.Картинка);
		// -РАРУС
		
		СчетчикКоманд = СчетчикКоманд + 1;
	КонецЦикла;
	
	ГраницаТаблицыКоманд = ТаблицаКоманд.Количество() - 1;
	
	Если НЕ Элементы.Найти("ТоварыПодменюЗаполнения") = Неопределено Тогда
		Для НомерЭлемента = 0 По ГраницаТаблицыКоманд Цикл
			ОписаниеКоманды = ТаблицаКоманд[НомерЭлемента];
			
			НовоеОписаниеКоманды = ТаблицаКоманд.Добавить();
			ЗаполнитьЗначенияСвойств(НовоеОписаниеКоманды, ОписаниеКоманды);
			
			ИмяЭлемента = "КомандаДополнительнойОбработкиЗаполнения" + Формат(СчетчикКоманд, "ЧГ=");
			НовоеОписаниеКоманды.ИмяЭлемента = ИмяЭлемента;
			
			// +РАРУС  Внесены правки, так как нужно использовать другое действие команды.
			ЗащищенныеФункцииСервер.ДобавитьКоманду(Форма, ИмяЭлемента, "ТоварыПодменюЗаполнения",, НовоеОписаниеКоманды.Представление,, ЛОЖЬ,,,ОтображениеКнопки.Картинка);
			// -РАРУС
	...
	Если НЕ Элементы.Найти("ПродукцияПодменюЗаполнения") = Неопределено Тогда
		Для НомерЭлемента = 0 По ГраницаТаблицыКоманд Цикл
			ОписаниеКоманды = ТаблицаКоманд[НомерЭлемента];
			...
			// +РАРУС Внесены правки, так как нужно использовать другое действие команды.
			ЗащищенныеФункцииСервер.ДобавитьКоманду(Форма, ИмяЭлемента, "ПродукцияПодменюЗаполнения",, НовоеОписаниеКоманды.Представление,, ЛОЖЬ,,,ОтображениеКнопки.Картинка);
			// -РАРУС
...
// Для внутреннего использования.
Процедура ВыполнитьКомандуПроверкиЗаполнения(ВнешнийОбъект, Знач ИдентификаторКоманды, ПараметрыКоманды, ОбъектыНазначения, Знач СценарийВБезопасномРежиме = Ложь)
...
Функция ВыполнитьКомандуВнешнегоОбъекта(ВнешнийОбъект, ИдентификаторКоманды, ПараметрыКоманды, АдресРезультата)
	...
	ИначеЕсли ВидОбработки = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПроверкаЗаполнения Тогда
		
		ВыполнитьКомандуПроверкиЗаполнения(
			ВнешнийОбъект,
			ИдентификаторКоманды,
			?(ПередаватьПараметры, ПараметрыКоманды, Неопределено),
			ПараметрыКоманды.ОбъектыНазначения,
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если ЕстьРеквизит(КомандаОписание,"ПроверкаПроведенияПередПечатью") Тогда
			ЗаполнитьЗначенияСвойств(Команда, КомандаОписание,, "ПроверкаПроведенияПередПечатью");
			Если КомандаОписание.ПроверкаПроведенияПередПечатью = Неопределено Тогда
				Команда.ПроверкаПроведенияПередПечатью = Истина;
			Иначе
				Команда.ПроверкаПроведенияПередПечатью = КомандаОписание.ПроверкаПроведенияПередПечатью;
			КонецЕсли;
		Иначе
		...
		Если ЕстьРеквизит(КомандаОписание,"ПроверкаИсключенияЗапретаРедактирования") Тогда
			ЗаполнитьЗначенияСвойств(Команда, КомандаОписание,, "ПроверкаИсключенияЗапретаРедактирования");
			Если КомандаОписание.ПроверкаИсключенияЗапретаРедактирования = Неопределено Тогда
				Команда.ПроверкаИсключенияЗапретаРедактирования = Ложь;
			Иначе
				Команда.ПроверкаИсключенияЗапретаРедактирования = КомандаОписание.ПроверкаИсключенияЗапретаРедактирования;
			КонецЕсли;
		Иначе
			ЗаполнитьЗначенияСвойств(Команда, КомандаОписание);
			Команда.ПроверкаИсключенияЗапретаРедактирования = Ложь;
