...
Функция ОписанияДатЗапрета() Экспорт
	...
	Список.Вставить("ТекущаяДата",           НСтр("ru = 'Текущая дата'"));
...
Процедура ОбщаяДатаЗапретаСОписаниемПриИзменении(Знач Контекст, РассчитатьДатуЗапрета = Истина) Экспорт
	...
	Иначе
		...
		РасчетныеДатыЗапрета = РасчетДатыЗапрета(
			Контекст.ОписаниеДатыЗапрета, Контекст.НачалоДня, Контекст.КоличествоДнейРазрешения);
		...
		Если Контекст.РазрешитьИзменениеДанныхДоДатыЗапрета Тогда
			...
			Если Контекст.ОписаниеДатыЗапрета = "ТекущаяДата" Тогда
			...
			Если Контекст.ОписаниеДатыЗапрета = "ТекущаяДата" Тогда
				...
				| от текущей даты с отсрочкой %1 дней.'"),
				Формат(Контекст.КоличествоДнейРазрешения, "ЧН=0; ЧГ="));
			...
			ИначеЕсли Контекст.НачалоДня > СрокРазрешения Тогда
				ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Запрещен ввод и редактирование данных за все прошлые периоды 
				|по %1 включительно (%2).
...
Функция РасчетДатыЗапрета(Знач ВариантДатыЗапрета, Знач ТекущаяДатаНаСервере, Знач КоличествоДнейРазрешения)
	...
	ИначеЕсли ВариантДатыЗапрета = "ТекущаяДата" Тогда
		ТекущаяДатаЗапрета    = НачалоДня(ТекущаяДатаНаСервере) - КоличествоДнейРазрешения*Сутки;
