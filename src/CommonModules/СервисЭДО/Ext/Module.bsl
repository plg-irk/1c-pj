...
Функция ВыполнитьОперацию(ПараметрыОперации, КонтекстДиагностики, КлючСинхронизации = Неопределено) Экспорт
	...
	Если Не ИспользоватьТокенПриВыполненииОперации(ПараметрыОперации)
		И ПараметрыОперации.ТребуетсяМаркер И МаркерПросрочен(КлючСинхронизации) Тогда
...
Функция Авторизоваться(КлючСинхронизации, КонтекстДиагностики,
	...
	Если ДанныеСертификата = Неопределено Тогда
		ДанныеСертификата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КлючСинхронизации.ВыбранныйСертификат,
			"ДанныеСертификата");
	КонецЕсли;
	...
	СертификатДляПолученияМаркера = КлючСинхронизации.ВыбранныйСертификат;
	Если Не ЗначениеЗаполнено(СертификатДляПолученияМаркера) Тогда
		СертификатДляПолученияМаркера = ЭлектроннаяПодпись.СсылкаНаСертификат(ДвоичныеДанныеСертификата);
	КонецЕсли;
	
	КэшированиеМаркераПоддерживается = КэшированиеМаркераПоддерживается(КлючСинхронизации.СпособОбмена);
	МаркерИзБезопасногоХранилища = Неопределено;
	
	Если КэшированиеМаркераПоддерживается Тогда
		МаркерИзБезопасногоХранилища = ЗашифрованныйМаркерИзБезопасногоХранилища(
			КлючСинхронизации.ИдентификаторУчетнойЗаписи, СертификатДляПолученияМаркера);
	КонецЕсли;
	
	Если МаркерИзБезопасногоХранилища <> Неопределено Тогда
		КлючСинхронизации.МаркерВалиден = Истина;
		КлючСинхронизации.МаркерПросрочен = Ложь;
		КлючСинхронизации.ДатаОкончанияДействияМаркера = МаркерИзБезопасногоХранилища.ДатаОкончанияДействияМаркера;
		Возврат МаркерИзБезопасногоХранилища.ЗашифрованныйМаркер;
	КонецЕсли;
	...
	ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "Gateway",
		ОповещенияОСобытияхЭДО.АдресДляУведомленийЭДО(КлючСинхронизации.СпособОбмена));
	...
	ДатаОкончанияДействияМаркера = Результат.Ответ.Заголовки["Expires"];
	Если КэшированиеМаркераПоддерживается И ДатаОкончанияДействияМаркера <> Неопределено Тогда
		КлючСинхронизации.ДатаОкончанияДействияМаркера = 
			ОбщегоНазначенияКлиентСервер.ДатаRFC1123(ДатаОкончанияДействияМаркера);
	КонецЕсли;
	
	Маркер = Результат.Ответ.ПолучитьТелоКакДвоичныеДанные();
	
	Если КэшированиеМаркераПоддерживается Тогда
		ДанныеМаркера = НовыеДанныеЗашифрованногоМаркера();
		ДанныеМаркера.ЗашифрованныйМаркер = Маркер;
		ДанныеМаркера.ДатаОкончанияДействияМаркера = КлючСинхронизации.ДатаОкончанияДействияМаркера;
		ЗаписатьЗашифрованныйМаркерВБезопасноеХранилище(КлючСинхронизации.ИдентификаторУчетнойЗаписи,
			СертификатДляПолученияМаркера, ДанныеМаркера);
	КонецЕсли;
...
Функция ПриоритетРаспаковкиКонтейнера(КодТранзакции, ТипДокументаСтрокой = "") Экспорт
	...
	ИначеЕсли КодТранзакции = "UniversalMessageReceiveNotice" Тогда
	...
	ИначеЕсли КодТранзакции = "CancellationOfferReject" 
		Или КодТранзакции = "CorrectionNotice" Тогда
		
		Приоритет = 8;
		
	ИначеЕсли КодТранзакции = "UniversalMessageRejectionNotice" 
		Или КодТранзакции = "UniversalMessageCorrectionNotice" Тогда
...
Функция ПолучитьТокенАвторизации(ИдентификаторУчетнойЗаписи, ДанныеМаркера, КонтекстДиагностики) Экспорт
	...
	ПараметрыВыполнения.Заголовки.Вставить("x-app-name", Метаданные.Имя);
	...
	Если ЗначениеЗаполнено(СписокЭД_XML) Тогда
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.ЛокальноеИмя = "ContainerDescription" Тогда
				...
				Результат.ПолученыВсеДокументы = НРег(ЧтениеXML.ПолучитьАтрибут("IsLast")) = "true";
				Продолжить;
			КонецЕсли;
...
Функция НовыйРезультатПолученияСпискаДокументов()
	...
	Результат.Вставить("ПолученыВсеДокументы", Истина);
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора, Ошибки)
	...
	Если ДанныеОрганизацииПереданыВПараметрах Тогда
		ДанныеОрганизации = ПараметрыРегистрации;
	Иначе
		ДанныеОрганизации = ДанныеОрганизацииДляРегистрационногоПакета(ПараметрыРегистрации.Организация);
	КонецЕсли;
	УдалитьНедопустимыеСимволыИзНаименования(ДанныеОрганизации.НаименованиеОрганизации);
	НаименованиеОрганизации = ДанныеОрганизации.НаименованиеОрганизации;
	ИННОрганизации = СокрЛП(ДанныеОрганизации.ИННОрганизации);
	КППОрганизации = СокрЛП(ДанныеОрганизации.КППОрганизации);
	ОГРНОрганизации = СокрЛП(ДанныеОрганизации.ОГРНОрганизации);
	...
	Для каждого Сертификат Из ПараметрыРегистрации.СертификатыПодписейОрганизации Цикл
		...
		ПараметрыСертификата = ПараметрыСертификатов[Сертификат];
		Если ПараметрыСертификата.ДействителенДо < ТекущаяДата
			Или ПараметрыСертификата.Отозван Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Сертификат = ПараметрыРегистрации.СертификатПодписанта Тогда
			ЭтоПодписаниеПоМЧД = УчетныеЗаписиЭДОСлужебный.ТребуетсяДоверенностьДляРегистрацииУчетнойЗаписиПоИНН(
				ИННОрганизации, Сертификат, ДанныеОператора.СпособОбмена);
		КонецЕсли;
...
//  * КППОрганизации - Строка
//  * ОГРНОрганизации - Строка
Функция ДанныеОрганизацииДляРегистрационногоПакета(Организация)
	
	Результат = Новый Структура;
	Результат.Вставить("НаименованиеОрганизации", "");
	Результат.Вставить("ИННОрганизации", "");
	Результат.Вставить("КППОрганизации", "");
	Результат.Вставить("ОГРНОрганизации", "");
	...
	ИмяРеквизитаОГРНОрганизации = ИнтеграцияЭДО.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ОГРНОрганизации");
		
	ИмяРеквизитаНаименованиеОрганизации = ИнтеграцияЭДО.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"НаименованиеОрганизации");
	...
	Результат.НаименованиеОрганизации = ПараметрыОрганизации[ИмяРеквизитаНаименованиеОрганизации];
	Результат.ИННОрганизации = ПараметрыОрганизации[ИмяРеквизитаИННОрганизации];
	Результат.КППОрганизации = ПараметрыОрганизации[ИмяРеквизитаКППОрганизации];
	Результат.ОГРНОрганизации = ПараметрыОрганизации[ИмяРеквизитаОГРНОрганизации];
...
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, АдресРесурса, КонтекстДиагностики, ВидСервисаЭДО = Неопределено,
	...
	ПараметрыВыполненияОперации.Заголовки.Вставить("UM_Supported", "true");
	...
	РезультатПолученияСпискаДокументов = СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатОперации,
		ДатаПоследнегоЗапроса, ВидСервисаЭДО);
	ЗаполнитьЗначенияСвойств(Результат, РезультатПолученияСпискаДокументов);
	
	Пока Не РезультатПолученияСпискаДокументов.ПолученыВсеДокументы Цикл
		Если РезультатПолученияСпискаДокументов.ЭлектронныеДокументы.Количество() = 0 Тогда
			Результат.ПолученыВсеДокументы = Истина;
			Прервать;
		КонецЕсли;
		
		УниверсальнаяДатаСтрокой = СтрокаИзУниверсальнойДатыВМиллисекундах(
			РезультатПолученияСпискаДокументов.ДатаПоследнегоЗапроса, КлючСинхронизации.СпособОбмена);
		ДобавитьПараметрЗапроса(ПараметрыВыполненияОперации, "date", УниверсальнаяДатаСтрокой);
		...
		Результат.ДатаПоследнегоЗапроса = РезультатПолученияСпискаДокументов.ДатаПоследнегоЗапроса;
		Результат.ПолученыВсеДокументы = РезультатПолученияСпискаДокументов.ПолученыВсеДокументы;
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатПолученияСпискаДокументов.ЭлектронныеДокументы,
...
// Возвращаемое значение:
//  Булево
Функция КэшированиеМаркераПоддерживается(СпособОбмена)
...
// Возвращаемое значение:
//  Булево
Функция МаркерПросрочен(КлючСинхронизации)
	
	Если КлючСинхронизации = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не КэшированиеМаркераПоддерживается(КлючСинхронизации.СпособОбмена) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДатаОкончанияДействияМаркера = КлючСинхронизации.ДатаОкончанияДействияМаркера;
	
	Если Не ЗначениеЗаполнено(ДатаОкончанияДействияМаркера) Тогда
		ДанныеМаркера = ЗашифрованныйМаркерИзБезопасногоХранилища(КлючСинхронизации.ИдентификаторУчетнойЗаписи,
			КлючСинхронизации.ВыбранныйСертификат);
		Если ДанныеМаркера = Неопределено Тогда
			Возврат Истина;
		КонецЕсли;
		
		ДатаОкончанияДействияМаркера = ДанныеМаркера.ДатаОкончанияДействияМаркера;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДатаОкончанияДействияМаркера)
		И ДатаОкончанияДействияМаркера <= ТекущаяУниверсальнаяДата() Тогда
		Возврат Истина;
	КонецЕсли;
...
// Возвращаемое значение:
//  - Неопределено
//  - См. НовыеДанныеЗашифрованногоМаркера
Функция ЗашифрованныйМаркерИзБезопасногоХранилища(ИдентификаторУчетнойЗаписи, Сертификат)

	УстановитьПривилегированныйРежим(Истина);
	ДанныеМаркеровПоСертификатам = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
		ИдентификаторУчетнойЗаписи, КлючЗашифрованногоМаркераВБезопасномХранилище());
		
	Если ДанныеМаркеровПоСертификатам = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
		
	АктуализироватьЗашифрованныеМаркерыВБезопасномХранилищеПоСрокуДействия(ИдентификаторУчетнойЗаписи,
		ДанныеМаркеровПоСертификатам);
	
	Возврат ДанныеМаркеровПоСертификатам.Получить(Сертификат);
		
КонецФункции

// Параметры:
//  ИдентификаторУчетнойЗаписи - Строка
//  ДанныеМаркеровПоСертификатам - Соответствие Из КлючИЗначение:
//   * Ключ - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//   * Значение - См. НовыеДанныеЗашифрованногоМаркера
Процедура АктуализироватьЗашифрованныеМаркерыВБезопасномХранилищеПоСрокуДействия(ИдентификаторУчетнойЗаписи,
	ДанныеМаркеровПоСертификатам)
	
	ТекущаяУниверсальнаяДата = ТекущаяУниверсальнаяДата();
	СертификатыСПросроченнымиМаркерами = Новый Массив; // Массив Из СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
	ЕстьПросроченныеМаркеры = Ложь;
	
	Для Каждого ДанныеМаркераПоСертификату Из ДанныеМаркеровПоСертификатам Цикл
		СертификатМаркера = ДанныеМаркераПоСертификату.Ключ;
		ДанныеМаркера = ДанныеМаркераПоСертификату.Значение;
		Если Не ЗначениеЗаполнено(ДанныеМаркера.ДатаОкончанияДействияМаркера)
			Или ДанныеМаркера.ДатаОкончанияДействияМаркера <= ТекущаяУниверсальнаяДата Тогда
			ЕстьПросроченныеМаркеры = Истина;
			СертификатыСПросроченнымиМаркерами.Добавить(СертификатМаркера);
		КонецЕсли;
	КонецЦикла;

	Если Не ЕстьПросроченныеМаркеры Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СертификатСПросроченнымМаркером Из СертификатыСПросроченнымиМаркерами Цикл
		ДанныеМаркеровПоСертификатам.Удалить(СертификатСПросроченнымМаркером);
	КонецЦикла;
	
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
		ИдентификаторУчетнойЗаписи, ДанныеМаркеровПоСертификатам, КлючЗашифрованногоМаркераВБезопасномХранилище());
...
//  ИдентификаторУчетнойЗаписи - Строка
//  Сертификат - СправочникСсылка.СертификатыКлючейЭлектроннойПодписиИШифрования
//  ДанныеМаркера - См. НовыеДанныеЗашифрованногоМаркера
//
Процедура ЗаписатьЗашифрованныйМаркерВБезопасноеХранилище(ИдентификаторУчетнойЗаписи, Сертификат, ДанныеМаркера)
	...
	Если ДанныеМаркеровПоСертификатам = Неопределено Тогда
		ДанныеМаркеровПоСертификатам = Новый Соответствие;
	КонецЕсли;
	
	ДанныеМаркеровПоСертификатам.Вставить(Сертификат, ДанныеМаркера);
...
// Возвращаемое значение:
//  Структура:
//  * ЗашифрованныйМаркер - Неопределено,ДвоичныеДанные
//  * ДатаОкончанияДействияМаркера - Дата
Функция НовыеДанныеЗашифрованногоМаркера()
	
	Результат = Новый Структура;
	Результат.Вставить("ЗашифрованныйМаркер", Неопределено);
	Результат.Вставить("ДатаОкончанияДействияМаркера", Дата(1, 1, 1));
	Возврат Результат;
...
// Возвращаемое значение:
//  Строка
Функция КлючЗашифрованногоМаркераВБезопасномХранилище()
	
	Возврат "ЗашифрованныйМаркер";
	
КонецФункции

#КонецОбласти // Маркеры
