...
Процедура ОткрытьНастройкуОтраженияВУчете(ПараметрыФормы, ПараметрыОткрытияФормы = Неопределено) Экспорт
	...
	ОбменСКонтрагентамиКлиентПереопределяемый.ОткрытьНастройкуОтраженияВУчете(ПараметрыФормы, ПараметрыОткрытияФормы, 
...
//
Процедура ОтразитьДанныеЭлектронногоДокумента(ОповещениеОЗавершении, ДанныеЭлектронногоДокумента, ОбъектыУчета, СпособОбработки = Неопределено) Экспорт
...
// Параметры:
//  ОповещениеОЗавершении - ОписаниеОповещения - результат см. НовыйРезультатОтраженияЭлектронныхДокументов
...
Процедура ОтразитьЭлектронныеДокументы(ОповещениеОЗавершении, СпособыОбработкиДокументов) Экспорт
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеОЗавершении", ОповещениеОЗавершении);
	Контекст.Вставить("СпособыОбработкиДокументов", СпособыОбработкиДокументов);
	Контекст.Вставить("СопоставлениеВыполнялось", Ложь);
	
	ОтразитьЭлектронныеДокументыПослеСозданияКонтекста(Контекст);
...
Функция НовыйРезультатОтраженияЭлектронныхДокументов()
	Результат = Новый Структура;
	Результат.Вставить("ОбъектыУчетаДокументов", Новый Соответствие);
...
// Параметры:
//  Контекст - Структура:
//  * ОповещениеОЗавершении - См. ОтразитьЭлектронныеДокументы.ОповещениеОЗавершении
//  * СпособыОбработкиДокументов - См. ОтразитьЭлектронныеДокументы.СпособыОбработкиДокументов
//  * СопоставлениеВыполнялось - Булево
Процедура ОтразитьЭлектронныеДокументыПослеСозданияКонтекста(Контекст)
	
	ДлительнаяОперация = ОтражениеВУчетеЭДОВызовСервера.ОтразитьЭлектронныеДокументыВФоне(
		Контекст.СпособыОбработкиДокументов);
	
	Оповещение = Новый ОписаниеОповещения("ОтразитьЭлектронныеДокументыПослеДлительнойОперации",
		ОтражениеВУчетеЭДОКлиент, Контекст);
...
//  * ОповещениеОЗавершении - ОписаниеОповещения
//  * СопоставлениеВыполнялось - Булево
Процедура ОтразитьЭлектронныеДокументыПослеДлительнойОперации(ДлительнаяОперация, Контекст) Экспорт
	
	КонтекстДиагностики = ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики();
	РезультатОтражения = ОбщегоНазначенияБЭДКлиент.РезультатВыполненияФоновогоЗадания(
		НСтр("ru = 'Отражение электронных документов'"), ДлительнаяОперация, КонтекстДиагностики); // См. ОтражениеВУчетеЭДО.ОтразитьЭлектронныеДокументы
	Если РезультатОтражения = Неопределено Тогда
		ОбработкаНеисправностейБЭДКлиент.ОбработатьОшибки(КонтекстДиагностики);
		ОтразитьЭлектронныеДокументыЗавершение(Контекст);
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(РезультатОтражения.НоменклатураДляСопоставления)
		Или Контекст.СопоставлениеВыполнялось Тогда
		
		ОтразитьЭлектронныеДокументыЗавершение(Контекст, РезультатОтражения);
		
	Иначе
		Контекст.СопоставлениеВыполнялось = Истина;
		Оповещение = Новый ОписаниеОповещения("ОтразитьЭлектронныеДокументыПослеСопоставления",
			ОтражениеВУчетеЭДОКлиент, Контекст);
		Настройки = СопоставлениеНоменклатурыКонтрагентовКлиент.НовыеНастройкиСопоставленияНоменклатуры();
		Настройки.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
		СопоставлениеНоменклатурыКонтрагентовКлиент.ОткрытьСопоставлениеНоменклатуры(
			РезультатОтражения.НоменклатураДляСопоставления, Настройки, Оповещение);
	КонецЕсли;
	
КонецПроцедуры

// Параметры:
//  Результат - Неопределено,Массив из Структура:
//   * НоменклатураКонтрагента - См. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагента
//   * НоменклатураИБ          - См. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураИнформационнойБазы
//  Контекст - См. ОтразитьЭлектронныеДокументыПослеСозданияКонтекста.Контекст
Процедура ОтразитьЭлектронныеДокументыПослеСопоставления(Результат, Контекст) Экспорт
...
// Параметры:
//  Контекст - См. ОтразитьЭлектронныеДокументыПослеСозданияКонтекста.Контекст
//  РезультатОтражения - Неопределено
//                     - См. ОтражениеВУчетеЭДО.ОтразитьЭлектронныеДокументы
Процедура ОтразитьЭлектронныеДокументыЗавершение(Контекст, РезультатОтражения = Неопределено)
	
	Результат = НовыйРезультатОтраженияЭлектронныхДокументов();
	...
	ЗаполнитьЗначенияСвойств(Результат, РезультатОтражения);
	
	Если ЗначениеЗаполнено(РезультатОтражения.НоменклатураДляСопоставления) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Документ учета не создан, т.к. не сопоставлена номенклатура'"));
	КонецЕсли;
