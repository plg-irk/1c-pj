...
// Добавление признака ДанныеЭлектронногоАктированияЕИС для участника для перевода типа адреса
//
// Параметры:
//  ДанныеПриложения - ДеревоЗначений - данные учатсника
//  Путь - Строка - полный путь в дереве значений.
//
Процедура ДополнитьПризнакДанныеЭлектронногоАктированияЕИСДляУчастника(
	Данные,
	Путь) Экспорт
	
	// Для отправок в ЕИС адрес всегда должен быть в структурированном формате.
	Если ИспользоватьЭлектронноеАктированиеВЕИС()
		И ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Данные, "ДанныеЭлектронногоАктированияЕИС", Ложь) = Истина Тогда
...
// Изменение типа адреса для участника
//
// Параметры:
//  АдресСоответствуетСтруктурированномуФорматуФНС - Булево - данные учатсника
//  ДеревоДанных - ДеревоЗначений - для определения отправки ЕИС.
//
Процедура АдресСоответствуетСтруктурированномуФорматуФНС(
	АдресСоответствуетСтруктурированномуФорматуФНС,
	ДеревоДанных) Экспорт
	
	// Для отправок в ЕИС адрес всегда должен быть в структурированном формате.
	Если ИспользоватьЭлектронноеАктированиеВЕИС()
		И ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(ДеревоДанных,
			"ДанныеЭлектронногоАктированияЕИС", Ложь) = Истина Тогда
			
		АдресСоответствуетСтруктурированномуФорматуФНС = Истина;
	КонецЕсли;
	
КонецПроцедуры

Процедура ИнформацияУчастникаПокупателя(ТаблицаПокупателей, ИмяУчастника) Экспорт
	
	ИнформацияДляУчастника = НачальныйИндексИнформацииУчастникаПокупателя();
...
// Проверка и дополнение реквизитов основания отгрузки товаров для версии 5.02.
// Контроль РДИК_0035 требует заполнения атрибутов узла ОснПер в
// соответствии с данными контракта. 
// Проверяются значения атрибутов @РеквНаимДок/@РеквНомерДок/@РеквДатаДок/@РеквДопСведДок.
//
// Параметры:
//  ДеревоДанных - ДеревоЗначений - Дерево данных электронного документа.
...
// Проверка на вхождение продавца в список грузополучателей для формата 5.02 и 5.03.
// Для прохождения "РДИК_0507. В списке грузополучателей указаны дублирующиеся грузополучатели"
// требуется чтобы в списке грузополучателей не было ИНН покупателей.
// 
// Параметры:
...
Процедура ПриФормированииДвоичныхДанныхЭД(ВидЭД, ОбъектФайла, ДеревоДанных) Экспорт
	...
	УбратьКоличествоПриТекстовомВыраженииОбъема(ВидЭД, ОбъектФайла, ДеревоДанных);
...
Функция ТекстЗапросаНастроекОтправкиОбъектовУчета() Экспорт
	...
	|	ЛОЖЬ КАК ВерсияФорматаУстановленаВручную,
	...
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(НастройкиОтправкиСДоговором.ВерсияФорматаУстановленаВручную, НастройкиОтправкиБезДоговора.ВерсияФорматаУстановленаВручную)
	|		ИНАЧЕ НастройкиОтправкиЕИС.ВерсияФорматаУстановленаВручную
	|	КОНЕЦ КАК ВерсияФорматаУстановленаВручную,
	|	ВЫБОР
	|		КОГДА НастройкиОтправкиЕИС.СпособОбмена ЕСТЬ NULL
...
Функция НовыеСведенияОКонтрагенте() Экспорт
	...
	Результат.Вставить("ИНН", "");
	Результат.Вставить("КПП", "");
	Результат.Вставить("ПолноеНаименованиеОбособленногоПодразделенияПоставщика", "");
	Результат.Вставить("КППОбособленногоПодразделенияПоставщика", "");
...
Функция РазобратьФайлКонтракта(ПутьККонтрактуИлиДвоичныеДанныеИлиОбъектXDTO,
	...
	Если ЕстьСвойство(ДанныеКонтракта, "foundation")
		И ЕстьСвойство(ДанныеКонтракта.foundation, "fcsOrder")
		И ЕстьСвойство(ДанныеКонтракта.foundation.fcsOrder, "order") Тогда
		
		ДанныеИзвещения = ДанныеКонтракта.foundation.fcsOrder.order;
		
		Результат.НомерИзвещенияОПроведении = ЗначениеНеобязательногоСвойства(ДанныеИзвещения, "notificationNumber");
...
Функция ИзвлечьСведенияОбОбъектахЗакупки(ДанныеКонтракта) Экспорт
	...
	Для каждого Данные Из ЗначениеВКоллекцию(ДанныеОбъектов.product) Цикл
		...
		// НДС в новом виде
		Если ОбъектЗакупки.СтавкаНДС = Неопределено Тогда
			СведенияОСтавкеНДС = ЗначениеНеобязательногоСвойства(Данные, "VATRateInfo");
			Если СведенияОСтавкеНДС = Неопределено Тогда
				КодСтавкиНДС = "RN";
			Иначе	
				КодСтавкиНДС = ЗначениеНеобязательногоСвойства(СведенияОСтавкеНДС, "VATCode");
			КонецЕсли;
			ОбъектЗакупки.СтавкаНДС = СтрЗаменить(КодСтавкиНДС, "R", "");
		КонецЕсли;
...
Функция НовыеДанныеКонтракта()
	...
	// notificationNumber, Строка19.
	Результат.Вставить("НомерИзвещенияОПроведении");
...
Функция НовыеСведенияОПоставщике()
	...
	Результат.Вставить("КПП");
	// supplierInfo/legalEntityRF/legalEntityRFSubdivision/EGRULInfo/fullName, Строка 2000.
	Результат.Вставить("ПолноеНаименованиеОбособленногоПодразделенияПоставщика", "");
	// supplierInfo/legalEntityRF/legalEntityRFSubdivision/EGRULInfo/KPP, Строка 9.
	Результат.Вставить("КППОбособленногоПодразделенияПоставщика");
	...
	Для Каждого УзелПоставщика Из ЗначениеВКоллекцию(УзелПоставщики.supplierInfo) Цикл
		...
		Если ЕстьСвойство(УзелПоставщика, "legalEntityRF") Тогда
			...
			Поставщик.КПП = ЗначениеНеобязательногоСвойства(УзелИнформации,
				"KPP");
			УзелЮЛ = УзелПоставщика[ИмяУзлаДанных];
			Если ЕстьСвойство(УзелЮЛ, "legalEntityRFSubdivision") Тогда
				ИмяУзлаДанных = "legalEntityRFSubdivision";
				УзелИнформации = УзелЮЛ[ИмяУзлаДанных].EGRULInfo;
				Поставщик.ПолноеНаименованиеОбособленногоПодразделенияПоставщика = ЗначениеНеобязательногоСвойства(УзелИнформации,
					"fullName");
				Поставщик.КППОбособленногоПодразделенияПоставщика = ЗначениеНеобязательногоСвойства(УзелИнформации,
					"KPP");
			КонецЕсли;
...
Функция ИзвлечьИнформациюОЦенах(ДанныеКонтракта)
	...
	Если ДанныеОЦенах.Свойства().Получить("currency") = Неопределено Тогда
		ИнформацияОЦенах.КодВалютыКонтракта = "RUB";
		ИнформацияОЦенах.ИмяВалютыКонтракта = "РОССИЙСКИЙ РУБЛЬ";
		
		ИнформацияОЦенах.ЦенаВРублях = ЗначениеНеобязательногоСвойства(ДанныеОЦенах, "price", "Число");
...
Функция СерверыОбменаЕИС()
	...
	Префикс = "https://int.zakupki.gov.ru/eis-integration/elact";
	...
	Если ДанныеПриложения.МестаПоставки.Количество() > 0 Тогда
		...
		Для каждого Место Из ДанныеПриложения.МестаПоставки Цикл
			...
			Если ЗначениеЗаполнено(ДанныеГосударственногоКонтракта.ДатаФормированияЗакупки)
				...
				АдресМестаПоставкиПоГАР = Истина;
			Иначе
				ДатаИзвещения = ПолучитьДатуИзвещенияИзОткрытойЧастиЕИСНаСервере(ДанныеГосударственногоКонтракта.НомерИзвещенияОПроведении);
				Если ЗначениеЗаполнено(ДатаИзвещения) И ДатаИзвещения >= Дата(2025, 01, 01) Тогда
					АдресМестаПоставкиПоГАР = Истина;
				КонецЕсли;
			КонецЕсли;
				
			Если АдресМестаПоставкиПоГАР Тогда
...
// Текст приложения к титулу продавца УПД дял формата 5.03.
// 
// Параметры:
//  ДанныеПриложения - Структура - см. НовыеДанныеПриложенияКТитулуПродавцаУПД
// 
...
Функция НовыеНедетализированныеСведенияТРУУПДЗаказчика()
	...
	// подписанному после выхода версии ЕИС 13.0. В других случаях игнорируется.
	Результат.Вставить("ИдентификаторПозиции", "");
	Тип = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(32, ДопустимаяДлина.Фиксированная));
...
Функция ПространствоИменЕИСПоИмениФайла(Знач ИмяФайла, ВерсияФормата = Неопределено)
	...
	ИначеЕсли ВерсияФормата = "5.03" Тогда 
		СтруктураИмен.Вставить("ON_NSCHFDOPPR", "ON_NSCHFDOPPR_1_997_01_05_03_01_EIS");
	ИначеЕсли ВерсияФормата = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура УбратьКоличествоПриТекстовомВыраженииОбъема(ВидДокумента, Файл, ДеревоДанных)
...
Функция ОпределитьКодировку(Знач ДвоичныеДанныеИлиАдрес)
	...
	Если СтрНайти(BOM, "EFBBBF") > 0 Тогда 
		КодировкаПоBOM = КодировкаUtf8;
		Стр = Сред(Стр, 7);
	ИначеЕсли СтрНайти(BOM, "FEFF") > 0 Тогда // BE
		КодировкаПоBOM = КодировкаUtf16;
		Стр = Сред(Стр, 5);
	...
	Строка1251 = "31323531";
	ДлинаВыборкиXML = 150;
	Если КодировкаПоBOM = КодировкаUtf16 Тогда
	...
	КириллицыAnsi = 0; //"windows-1251"
	КириллицыOem = 0; //"oem-866"
	КириллицыUtf8 = 0; //"UTF-8"
	КириллицыUtf16 = 0; //"UTF-16"
	Служебных = 0;
	...
	Если (Процент16 > 0 Или Процент00 > 30) Тогда
		Возврат КодировкаUtf16;
	КонецЕсли;
	
	Если (Процент8 > ПроцентAnsi И Процент8 > ПроцентOem)
		Или (Процент8 >= ПроцентAnsi И Процент8 >= ПроцентOem
		И (КодировкаПоBOM = КодировкаUtf8 Или КодировкаПоXML = КодировкаUtf8)) Тогда
		КодировкаПоЧастоте = КодировкаUtf8;
	КонецЕсли;
	...
	Если КодировкаПоЧастоте = Неопределено И ПроцентOem > Процент8 И ПроцентOem >= ПроцентAnsi Тогда
		КодировкаПоЧастоте = КодировкаOem;
	КонецЕсли;
...
Функция ПолучитьДатуИзвещенияИзОткрытойЧастиЕИСНаСервере(НомерИзвещения)
	
	ДатаИзвещения = Неопределено;
	
	Если НЕ ЗначениеЗаполнено(НомерИзвещения) Тогда
		Возврат ДатаИзвещения;
	КонецЕсли;
	
	СсылкаДляЗагрузки = "https://zakupki.gov.ru/epz/order/notice/printForm/"
		+ "viewXml.html?regNumber=" + НомерИзвещения;
	РезультатЗагрузки = ПолучениеФайловИзИнтернета.СкачатьФайлВоВременноеХранилище(
		СсылкаДляЗагрузки);
	Если НЕ РезультатЗагрузки.Статус Тогда
		ОписаниеОшибки = НСтр("ru = 'Электронный документооборот с ЕИС. При загрузке извещения с сервера ЕИС возникла ошибка:'", ОбщегоНазначения.КодОсновногоЯзыка())
	...
	АдресДанныхИзвещения = РезультатЗагрузки.Путь;
	
	Если ЭтоАдресВременногоХранилища(АдресДанныхИзвещения) Тогда
		Данные = ПолучитьИзВременногоХранилища(АдресДанныхИзвещения);
		ДанныеИзвещения = РазобратьФайлИзвещения(Данные);
		ДатаИзвещения = ДанныеИзвещения.ДатаРазмещенияИзвещенияВЕИС;
	КонецЕсли;
...
Функция НовыеДанныеИзвещения()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ДатаРазмещенияИзвещенияВЕИС");
	
	Возврат Результат;
	
КонецФункции

// Имя узла данных извещения.
// 
// Возвращаемое значение:
//  Строка
Функция ИмяУзлаДанныхИзвещения() Экспорт
	
	Возврат "commonInfo";
	
КонецФункции

// Разобрать файл извещения.
// 
// Параметры:
//  ПутьККонтрактуИлиДвоичныеДанныеИлиОбъектXDTO - Строка,
//  	ДвоичныеДанные,
//  	ОбъектXDTO
// Возвращаемое значение:
//  Структура - Разобрать файл извещения, см. НовыеДанныеИзвещения()
Функция РазобратьФайлИзвещения(ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO) Экспорт
	
	Результат = НовыеДанныеИзвещения();
	
	Если ТипЗнч(ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO) = Тип("ОбъектXDTO") Тогда
		Данные = ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO;
	Иначе
		Чтение = Новый ЧтениеXML;
		Если ТипЗнч(ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO) = Тип("Строка") Тогда
			Чтение.ОткрытьФайл(ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO);
		Иначе
			СтрокаДанных = ПолучитьСтрокуИзДвоичныхДанных(
				ПутьКИзвещениюИлиДвоичныеДанныеИлиОбъектXDTO, "UTF-8");
			Чтение.УстановитьСтроку(СтрокаДанных);
		КонецЕсли;
		Данные = ФабрикаXDTO.ПрочитатьXML(Чтение);
	КонецЕсли;
	...
	ИмяУзлаВерсииСхемы = "schemeVersion";
	Если ЕстьСвойство(Данные, ИмяУзлаДанныхИзвещения()) Тогда 
		ДанныеИзвещения = Данные[ИмяУзлаДанныхИзвещения()];
	Иначе
		Если НЕ ЕстьСвойство(Данные, ИмяУзлаВерсииСхемы) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	
	Результат.ДатаРазмещенияИзвещенияВЕИС	= ЗначениеНеобязательногоСвойства(ДанныеИзвещения, "publishDTInEIS", "Дата");
