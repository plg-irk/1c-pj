...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Попытка
		...
		ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(НаборСсылок, ПараметрыОтметкиВыполнения);
		ОбъектовОбработано = НаборСсылок.Количество() + 1;
	...
	Исключение
		...
		Для Каждого СсылкаНаОбъект Из НаборСсылок Цикл
			...
			ШаблонСообщения = НСтр("ru = 'Не удалось сконвертировать правила проверки полномочий для МЧД: %1 по причине:'");
...
Функция ПроверитьПодписиМЧД() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ПроверенаВручную = СтрНайти(Выборка.ТекстОшибкиПроверкиМЧД, НСтр("ru='Проверена вручную:'")) > 0;
...
Процедура ОбновитьКлассификаторПолномочий_МЧД003(ДанныеКлассификатора) Экспорт
	...
	Попытка
		...
		|	ТаблицаНовыхДанных.ДатаИзменения КАК ДатаИзменения,
		|	ТаблицаНовыхДанных.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ТаблицаНовыхДанных_1
		|ИЗ
		|	&ТаблицаНовыхДанныхГрупп КАК ТаблицаНовыхДанных
		|;
		|
		|ВЫБРАТЬ
		|	ТаблицаНовыхДанныхСрез.УИДРодителя КАК УИДРодителя,
		|	ТаблицаНовыхДанных.Родитель КАК Родитель,
		|	ТаблицаНовыхДанных.Ссылка КАК Ссылка
		...
		|	(ВЫБРАТЬ
		|		ТаблицаНовыхДанных.УИДРодителя КАК УИДРодителя,
		|		МАКСИМУМ(ТаблицаНовыхДанных.ДатаИзменения) КАК ДатаИзменения
		|ИЗ
		|		ТаблицаНовыхДанных_1 КАК ТаблицаНовыхДанных
		|	СГРУППИРОВАТЬ ПО
		|		ТаблицаНовыхДанных.УИДРодителя) КАК ТаблицаНовыхДанныхСрез
		|ВНУТРЕННЕЕ СОЕДИНЕНИЕ
		|	ТаблицаНовыхДанных_1 КАК ТаблицаНовыхДанных
		|ПО
		|	ТаблицаНовыхДанныхСрез.УИДРодителя = ТаблицаНовыхДанных.УИДРодителя
		|	И ТаблицаНовыхДанныхСрез.ДатаИзменения = ТаблицаНовыхДанных.ДатаИзменения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		КолонкиГруппировок = "Родитель, УИДРодителя, ДатаИзменения";
...
Функция ПолучитьИтоговыйТекстПоясненияПроверкиПравил(НомераПравилИРезультатыПроверки, ТекстЗаголовкаСообщения)
	...
	Для Каждого ТекущаяСтрока Из НомераПравилИРезультатыПроверки Цикл
		...
		ТекстДополнения = НСтр("ru = 'Правило №'") + ТекущаяСтрока.НомерПравила + ":";
...
Функция ПолучитьПредставлениеПодписи(ПодписьВерна)
	...
	Возврат ?(ПодписьВерна, НСтр("ru = 'Верна'"), НСтр("ru = 'Не верна'"));
