...
//  ПротоколПочты - Строка - если указано значение "IMAP", то письмо будет передано по протоколу IMAP.
//                           в другом случае по протоколу SMTP.
//
...
Функция ОтправитьПочтовоеСообщение(Знач УчетнаяЗапись,
	...
	ИдентификаторСообщения = РаботаСПочтовымиСообщениямиСлужебный.ОтправитьСообщение(УчетнаяЗапись, ПараметрыПисьма, Соединение, ПротоколПочты);
...
Функция ДоступныеУчетныеЗаписи(Знач ДляОтправки = Неопределено,
	...
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|	И УчетныеЗаписиЭлектроннойПочты.ПользовательСистемы = &ПользовательСистемы
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ИспользоватьДляПолучения = &ДляПолучения
	|		КОНЕЦ
	|	И &ВключатьСистемнуюУчетнуюЗапись
	|	И УчетныеЗаписиЭлектроннойПочты.АдресЭлектроннойПочты <> """"
	|	И ВЫБОР
	|			КОГДА УчетныеЗаписиЭлектроннойПочты.ИспользоватьДляПолучения
	|				ТОГДА УчетныеЗаписиЭлектроннойПочты.СерверВходящейПочты <> """"
	...
	Запрос.Параметры.Вставить("ПользовательСистемы", ПараметрыСеанса.ТекущийПользователь);
