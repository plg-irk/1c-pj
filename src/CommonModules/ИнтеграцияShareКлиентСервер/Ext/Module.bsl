...
//   * ДополнительныеПараметры - Неопределено, Структура - произвольные дополнительные параметры.
//   * НастройкиПубликации - Структура - настройки публикации электронного документа в сервисе:
//      ** СхемаПубликации - Строка - см. СхемыПубликацииДокумента
//      ** ИнициаторПубликации - Строка - инициатор публикации документа, в роли которого может выступать сервис или
//                                        какая-то функциональность. При интеграции с другими сервиса, функционалами
//                                        рекомендуется указывать наименование подсистемы, где лежит сервис, например
//                                        "ОнлайнЗаказы" или механизм "ГрупповаяОбработка" и т.д.
//                                        По умолчанию заполняется значением "СервисShare".
//      ** СрокЖизниВМинутах - Число - срок жизни ссылки на документ в минутах, после которого закрывается доступ.
//                                     Доступно устанавливать до 30 дней.
...
//                                             Например, разрешается удаление ссылки до завершения срока жизни.
//                                             По умолчанию значение Истина. Если ссылка размещается в другом сервисе и
//                                             необходимо сохранить целостность ссылки, то необходимо установить значение Ложь.
//
Функция НовыеПараметрыДокументаКПубликации() Экспорт
	...
	НастройкиПубликации.Вставить("СхемаПубликации", СхемыПубликацииДокумента().Стандартная);
	НастройкиПубликации.Вставить("ИнициаторПубликации", "СервисShare");
	НастройкиПубликации.Вставить("СрокЖизниВМинутах", 0);
	НастройкиПубликации.Вставить("ДоступноРучноеУправление", Истина);
	
	ПараметрыДокумента.Вставить("НастройкиПубликации", НастройкиПубликации);
...
//   * ТипДокумента                 - Неопределено, ПеречислениеСсылка.ТипыДокументовЭДО - тип документа электронного документооборота.
//   * Организация                  - Неопределено, ОпределяемыйТип.Организация - организация-отправитель документа.
//   * ФорматЭлектронногоДокумента  - Строка - формат электронного документа.
...
Функция НовыеДанныеОпубликованногоДокумента() Экспорт
	...
	ДанныеДокумента.Вставить("ФорматЭлектронногоДокумента", "");
	ДанныеДокумента.Вставить("ЭтоФорматФНС", Ложь);
...
//   * Наименование - Строка - представление файла.
...
//   * ДвоичныеДанныеФайла - Строка - адрес хранилища двоичных данных файла.
//   * ФайлВыбран - Булево - признак выбранного файла в пользовательском интерфейсе из формы выбора.
//   * ВидФайла - Строка - способ хранения дополнительных файлов к документу в прикладном решение.
//                         См. ВидыХраненияФайловВБазе. По умолчанию вариант "ПрисоединенныеФайлы".
//   * ИдентификаторФайла - Строка - идентификатор файла. При заполнении вида файла "ИнтеграцияС1СДокументооборот"
...
Функция НовыеДанныеФайлаДляВыбораКПубликации() Экспорт
	...
	ДанныеФайла.Вставить("ВидФайла", ВидыХраненияФайловВБазе().ПрисоединенныеФайлы);
...
//   * ДополнительныеФайлыДокумента - ХранилищеЗначения из См. ИнтеграцияShare.НовоеОписаниеДополнительныхФайлов - 
//                                             дополнительные файлы переданные вместе с электронным документом.
//   * ФайлыВизуализации - Массив из См. ИнтеграцияShare.НовыйФайлВизуализации - файлы представления электронного документа.
//   * ЕстьДополнительныеФайлы - Булево - признак наличия дополнительных файлов к электронному документу.
Функция НовыеДанныеЗагруженногоДокумента() Экспорт
	...
	ДанныеДокумента.Вставить("ЕстьДополнительныеФайлы", Ложь);
...
//   * ФайлВизуализации - Строка - файл визуализации главного файла.
//   * ГлавныйФайл - Строка - пакет со всеми данными электронного документа.
//   * ДополнительныйФайл - Строка - произвольный файл, может быть прикреплен к главному документу.
//
Функция ВидыФайловДокумента() Экспорт
	...
	ВидыФайлов.Вставить("ДополнительныйФайл", "ДополнительныйФайл");
...
//   * Стандартная - Строка - выполняется публикация документа с возможностью последующей загрузки в базу 1С,
//                            либо просмотром и скачиванием на веб-странице.
//   * ТолькоПубликация - Строка - выполняется публикация документа с возможностью просмотра документа на веб-странице.
//
...
// Возвращает виды хранения файлов в прикладной базе.
//
// Возвращаемое значение:
//  Структура - виды хранения файлов:
//   * ПрисоединенныеФайлы - Строка - присоединенные файлы для работы через подсистему БСП.
//   * ИнтеграцияС1СДокументооборот - Строка - присоединенные файлы хранящиеся в интеграционной базе 1С:Документооборот
//                                               для работы через подсистему БИД.
//
Функция ВидыХраненияФайловВБазе() Экспорт

	Результат = Новый Структура;
	Результат.Вставить("ПрисоединенныеФайлы", "ПрисоединенныеФайлы");
	Результат.Вставить("ИнтеграцияС1СДокументооборот", "ИнтеграцияС1СДокументооборот");
...
// Возвращаемое значение:
//  Строка - идентификатор результат глобального поиска Загрузить документ.
...
// Возвращаемое значение:
//  Строка - идентификатор результат глобального поиска Открыть форму загрузки.
Функция ИмяРезультатаГлобальногоПоискаОткрытьФормуЗагрузкиДокумента() Экспорт
	
	Возврат "ОткрытьФормуЗагрузкиСервисShare";
...
Функция ЭтоДоменСервисаКороткихСсылокShare(КороткаяСсылкаНаДокумент) Экспорт
	...
	Возврат ИмяСервераКороткойСсылки = АдресСервиса();
