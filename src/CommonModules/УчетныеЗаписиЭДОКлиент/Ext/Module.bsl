...
Функция НовыеПараметрыАвторизацииВСервисеЭДО() Экспорт
	...
	Параметры.Вставить("ИдентификаторыЭДО", Неопределено);
	...
	Параметры.Вставить("ВыбранныеСертификаты", Новый Массив);
...
Процедура АвторизоватьсяВСервисеЭДО(ОповещениеОЗавершении, ПараметрыАвторизации) Экспорт
	
	КонтекстАвторизации = УчетныеЗаписиЭДОСлужебныйКлиент.НовыйКонтекстАвторизацииВСервисеЭДО();
...
Функция НовыйРезультатАвторизацииВСервисеЭДО() Экспорт
	...
	Результат.Вставить("РасшифрованныеМаркеры", КриптографияБЭДКлиентСервер.НовыйНаборРасшифрованныхДанных());
...
Процедура СоздатьУчетнуюЗапись(ПараметрыСоздания = Неопределено) Экспорт
	...
	ИспользоватьЛегкийИнтерфейс = Ложь;
	ОбменСКонтрагентамиКлиентПереопределяемый.ПриОпределенииИнтерфейсаПомощникаПодключения(ИспользоватьЛегкийИнтерфейс);
	Если ИспользоватьЛегкийИнтерфейс Тогда
		УчетныеЗаписиЭДОСлужебныйКлиент.СоздатьУчетнуюЗаписьЛегкийИнтерфейс(ПараметрыСоздания);
	Иначе
...
Процедура НачатьПолучениеНовогоИдентификатораТакском(ОписаниеОповещения, ОбработчикСтатусаЗаявки,
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
		...
		МодульПодключение1СТакскомКлиент.ПолучитьУникальныйИдентификаторАбонента(СертификатКриптографии, Организация,
				ОписаниеОповещения, ПараметрыОбработки);
	Иначе
		Подключение1СТакскомКлиентБЭД.ПолучитьУникальныйИдентификаторАбонента(СертификатКриптографии, Организация,
				ОписаниеОповещения, ПараметрыОбработки);
	КонецЕсли;
...
// Возвращает имя события вызываемого после успешного создания учетной записи ЭДО
// Возвращаемое значение:
//  Строка
Функция ИмяСобытияСозданаУчетнаяЗаписьЭДО() Экспорт
	Возврат "СозданПрофиль1СЭДО";
КонецФункции
