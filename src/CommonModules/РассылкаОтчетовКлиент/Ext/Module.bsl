...
Процедура ОткрытьРассылкуИзФормыОтчета(Форма, Ссылка = Неопределено)
	...
	Если Форма.РежимВариантаОтчета Тогда
		...
		СтрокаОтчетыПараметры.Вставить("Настройки",      Форма.Отчет.КомпоновщикНастроек.Настройки);
		Попытка
			СтрокаОтчетыПараметры.Вставить("НачалоПериода",  Форма.НачалоПериода);
			СтрокаОтчетыПараметры.Вставить("КонецПериода",   Форма.КонецПериода);
			СтрокаОтчетыПараметры.Вставить("ВидПериода",     Форма.ВидПериода);
		Исключение
		КонецПопытки;
		// -РАРУС
	КонецЕсли;
