...
// Вызывающий код должен устанавливать привилегированный режим перед
// вызовом метода.
//
...
// См. процедуру ОбщегоНазначенияПереопределяемый.ПараметрыРаботыКлиента.
//
...
// См. процедуру РаботаВБезопасномРежимеПереопределяемый.ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам.
//
...
// См. процедуру ПользователиПереопределяемый.ПриОпределенииНазначенияРолей.
//
...
// См. процедуру ТекущиеДелаПереопределяемый.ПриОпределенииОбработчиковТекущихДел.
//
...
// См. процедуру ТекущиеДелаПереопределяемый.ПриОпределенииПорядкаРазделовКомандногоИнтерфейса.
//
...
// Смотреть описание этой же процедуры в общем модуле
...
//
// Параметры:
//  Шаблоны - Массив - Массив строк. В параметр следует добавить имена предопределенных
...
// См. процедуру ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки.
//
...
// Устарела. Будет удалена в следующей редакции библиотеки.
// Необходимо использовать функцию ПоставщикУслугПортал1СИТСПриФормированииСпискаУслуг.
//
// Возвращает описание поставщика "Портал 1С:ИТС" для заполнения списка услуг
// в методе ТарификацияПереопределяемый.ПриФормированииСпискаУслуг.
// Поставщик добавляется в список поставщиков.
...
//		описание параметра ПоставщикиУслуг.
//
Функция УдалитьПоставщикУслугПортал1СИТСПриФормированииСпискаУслуг(ПоставщикиУслуг) Экспорт
	
	Возврат ПоставщикУслугПортал1СИТСПриФормированииСпискаУслуг(ПоставщикиУслуг);
...
Процедура ЗаписатьСообщениеВЖурналРегистрации(
	...
	// или другим способом (для других подсистем).
...
//                             (см. [МодульПодсистемыКлиентСервер].ПолучитьСписокВсехСобытийЖурналаРегистрации).
...
Процедура ЗаполнитьДопПараметрыОтправкиСообщенияВТехПоддержку(Результат, ДопПараметры) Экспорт
...
Функция ПараметрыПолученияТикетаДляОткрытияВебСтраницы() Экспорт
	...
	// Возвращается только для пользователя с правами администрирования
	// при работе в локальном режиме для получения тикета на клиенте.
	УстановитьПривилегированныйРежим(Истина);
...
Функция ПроверитьПравильностьВнедрения()
	...
	// Проверка минимально необходимой версии БСП.
	ВерсияБСП = СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
	РелизБСП = Лев(ВерсияБСП, 6);
	Если РелизБСП = "2.3.3." Тогда
		МинимальнаяВерсияБСП = "2.3.3.69";
	ИначеЕсли РелизБСП = "2.3.4." Тогда
		МинимальнаяВерсияБСП = "2.3.4.5";
	Иначе
		МинимальнаяВерсияБСП = "";
	КонецЕсли;
	
	Если Не ПустаяСтрока(МинимальнаяВерсияБСП) Тогда
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБСП, МинимальнаяВерсияБСП) < 0 Тогда
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка внедрения библиотеки Интернет-поддержки пользователей.
					|Необходима версия библиотеки стандартных подсистем не ниже версии %1'"),
				МинимальнаяВерсияБСП);
