...
Процедура ОбработатьОчередьЗапросовВРеестрМЧД() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.ОбработкаОчередиЗапросовВРеестрМЧДЭДО);
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		...
		ОбработанныеДоверенности = МодульМашиночитаемыеДоверенностиИнтеграцияЦентраЭДО.ОбработатьОчередьЗапросовВРеестрМЧД();
	Иначе
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ОбработанныеДоверенности[МЧД] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ВыборкаДетальныеЗаписи.ВидЗапроса = 
			...
			ИначеЕсли МашиночитаемыеДоверенностиКлиентСервер.ДоверенностьУжеИстекла(
				...
				РезультатОбновленияСтатуса = 
					МашиночитаемыеДоверенности.ПолучитьСтатусМЧДИзРеестраФНС(ДанныеДляПроверкиВРеестреФНС);
