...
//  ОписаниеОповещенияОЗакрытии - ОписаниеОповещения - описание процедуры, которая будет вызвана.
//
...
// Обработчик для события формы ПроверкаПеретаскивания. Вызывается для списка предметов
//  при перетаскивании в него взаимодействий.
//
// Параметры:
...
Процедура СоздатьВзаимодействиеИлиПредмет(ИмяФормыОбъекта, Основание, Источник) Экспорт
	...
	ИначеЕсли (ТипЗнч(Основание) = Тип("ДокументСсылка.СообщениеSMS") 
		// +Рарус Исправлена ошибка БСП. условия поменяны местами
		 И Источник.Элементы.Найти("Адресаты") <> Неопределено И Источник.Элементы.Адресаты.ТекущиеДанные <> Неопределено) Тогда
		// -Рарус
