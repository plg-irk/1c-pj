...
// Извлекает данные транспортных контейнеров.
//
// Параметры:
...
Функция ДанныеКонтейнеров(ТранспортныеКонтейнеры, КонтекстДиагностики) Экспорт
	
	Возврат СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики);
...
Функция СохранитьКонтейнер(ОписаниеКонтейнера, КлючСинхронизации, КонтекстДиагностики, ИмяБезРасширения = "") Экспорт
	...
	РезультатСохранения.ТранспортныйКонтейнер = СохранитьКонтейнерПоПараметрам(
		ПараметрыКонтейнера, ОписаниеКонтейнера.ДвоичныеДанные, КонтекстДиагностики);
...
// 
// Возвращаемое значение:
//  ДокументСсылка.ТранспортныйКонтейнерЭДО
Функция СохранитьКонтейнерПоПараметрам(ПараметрыКонтейнера, ДвоичныеДанныеФайла, КонтекстДиагностики) Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ТранспортныйКонтейнер) Тогда
			ДобавитьПрисоединенныйФайлКонтейнера(ТранспортныйКонтейнер, ДвоичныеДанныеФайла,
				ПараметрыКонтейнера.Представление);
		КонецЕсли;
	...
	Попытка
		...
		ЗаполнитьПараметрыКонтейнера(ПараметрыКонтейнера, ДвоичныеДанныеФайла,
...
// 	Представление - Строка - например, имя без расширения.
// 	
// Возвращаемое значение:
// 	Структура:
...
// * Объект - Неопределено,Произвольный
// * Статус - ПеречислениеСсылка.СтатусыТранспортныхСообщенийБЭД
...
Функция ПараметрыКонтейнера(ИдентификаторДокументооборота, ИдентификаторСообщения, ИдентификаторПакета,
	СпособОбмена, ИдентификаторУчетнойЗаписи, ВидСервисаЭДО = Неопределено, Представление = "") Экспорт
	...
	ПараметрыКонтейнера.Вставить("Представление", ?(ЗначениеЗаполнено(Представление), Представление,
		"EDI_" + ИдентификаторСообщения));
...
Функция ПрочитатьКарточкуЭлектронногоДокумента(ИмяФайла)
	...
	Карточка.Вставить("ФайлНазначения", "");
	...
	Если ДополнительныеПараметры.Свойство("ФайлНазначения") Тогда
		Карточка.ФайлНазначения = ДополнительныеПараметры.ФайлНазначения;
	КонецЕсли;
