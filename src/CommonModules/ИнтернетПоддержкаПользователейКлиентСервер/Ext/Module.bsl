...
Функция ОписаниеРесурсаИнтернетПоддержки() Экспорт
	...
	Результат.Вставить("Описание", НСтр("ru = 'Интернет-поддержка пользователей'"));
	Возврат Результат;
...
Функция ВерсияAPIВзаимодействия() Экспорт
	...
	Возврат ВерсияAPIСервисаБизнесПроцессов();
...
Функция ИмяWSОпределения() Экспорт
	...
	Возврат АдресWSDLСервисаБизнесПроцессов();
...
Функция ИмяURIСервиса() Экспорт
	...
	Возврат URIСервисаСервисаБизнесПроцессов();
...
Функция ЗагрузитьСодержимоеИзИнтернет(
	...
	МаксКоличествоПеренаправлений   = 7;
	...
	Ответ = Неопределено;
	Пока КоличествоПеренаправлений < МаксКоличествоПеренаправлений Цикл
		...
		Если Ответ.КодСостояния = 301 // 301 Moved Permanently
			...
			Если КоличествоПеренаправлений > МаксКоличествоПеренаправлений Тогда
				УстановитьОписаниеОшибки(
					Результат,
					"ServerError",
...
//	Код ответа 200;
//	Таймаут ответа не превышает 3 сек.
//
...
//  СокращатьРазрядностьВерсии - Булево - разрешить сокращать версию (завершающуюся на 0000/000/00 или 9999/999/99)
//  в следующих случаях:
//                    А.00.000.0000 = А,
//                    А.Б.000.0000 = А.Б,
//                    А.Б.В.0000 = А.Б.В,
...
//
// Возвращаемое значение:
//  Список значений - если пустой, значит ошибок нет, в противном случае
//                    в него будут возвращены все неправильные символы.
//
...
//  ФабрикаХДТО - ФабрикаXDTO - на каком основании создавать тип
//                             (может быть по-умолчанию - из конфигурации, и из веб-сервиса).
//
// Возвращаемое значение:
...
//   Структура - Структура с определенными ключами, см. код.
//
Функция СтруктураШагаВыполнения() Экспорт
...
//
// Параметры:
//  КонтекстВыполнения - Структура - см. возврат
//									 ИнтернетПоддержкаПользователейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций;
...
Функция ЗавершитьРегистрациюРезультатаВыполненияОперации(КонтекстВыполнения, КодРезультата, ОписаниеРезультата, ВложенныйКонтекстВыполнения = Неопределено) Экспорт
	...
	// Если не было зарегистрировано начало шага выполнения,
	// то впоследствии могут быть ошибки при записи лога в журнал регистрации.
...
//
// Параметры:
//  КонтекстВыполнения - Структура - Структура с ключами, описанными 
//                                   в Интернет.НоваяЗаписьРезультатовВыполненияОпераций();
			...
			Результат = Результат
				...
				+ СтрШаблон(
					ШаблонШага,
						...
						ИнтернетПоддержкаПользователейКлиентСервер.ВставитьОтступВМногострочнуюСтроку("(" + ТекущийШаг.КодРезультата + "), " + ТекущийШаг.ОписаниеРезультата, Отступ, Ложь))
				+ "#";
		КонецЕсли;
...
Функция СлужебнаяТикетАутентификации(
	...
	Если Не ЗначениеЗаполнено(Логин) Тогда
		ПараметрыПолученияТикета = ИнтернетПоддержкаПользователейВызовСервера.ПараметрыПолученияТикета();
	...
	Если НЕ ПустаяСтрока(ПроксиСервиса.КодОшибки) Тогда
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ru = 'Ошибка при подключении к сервису аутентификации.'")
		...
		Результат.СообщениеОбОшибке = НСтр("ru = 'Ошибка при подключении к сервису.'")
			+ Символы.ПС
	...
	Иначе
		...
		Результат.ИнформацияОбОшибке =
			НСтр("ru = 'Ошибка при получении тикета аутентификации.'")
...
Функция СервисАутентификации_getTicket(
	ПроксиСервиса,
	Логин,
	Пароль,
	ИмяСервиса,
	...
	ОписаниеОтвета = ОтправитьЗапросSOAP(
		ТекстКонверта,
		ПроксиСервиса,
		ТаймаутПодключения,
		НастройкиПрокси);
	...
	Если Не ПустаяСтрока(Результат.КодОшибки) Тогда
		...
		Если СтрНайти(Результат.ИнформацияОбОшибке, "IncorrectLoginOrPasswordApiException") > 0 Тогда
			Результат.КодОшибки         = "НеверныйЛогинИлиПароль";
			Результат.ИнформацияОбОшибке = НСтр("ru = 'Неверный логин или пароль (IncorrectLoginOrPasswordApiException).'");
			Результат.СообщениеОбОшибке  = НСтр("ru = 'Неверный логин или пароль.'");

		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TooManyFailedAttemptsApiException") > 0 Тогда
			Результат.КодОшибки = "ПревышеноКоличествоПопыток";
			Результат.ИнформацияОбОшибке =
				НСтр("ru = 'Превышено количество попыток аутентификации (TooManyFailedAttemptsApiException).'");
		...
		ИначеЕсли СтрНайти(Результат.ИнформацияОбОшибке, "TicketApiException") > 0 Тогда
			...
			Результат.ИнформацияОбОшибке = НСтр("ru = 'Неизвестная ошибка сервиса аутентификации (TicketApiException).'");
	...
	Иначе
		...
		Если ОписаниеОтветаSOAP.Ошибка Тогда
			...
			Результат.СообщениеОбОшибке = НСтр("ru = 'Ошибка сервиса.'")
				+ Символы.ПС + ОписаниеОтветаSOAP.СообщениеОбОшибке;
			Результат.ИнформацияОбОшибке =
				НСтр("ru = 'Ошибка вызова операции getTicket() сервиса аутентификации.'") + " "
				+ ОписаниеОтветаSOAP.ИнформацияОбОшибке
				+ Символы.ПС + НСтр("ru = 'Тело ответа:'") + " " + Лев(ОписаниеОтвета.ТелоОтвета, 1024 * 5);
			Возврат Результат;
		КонецЕсли;
...
Функция ОтправитьДанныеСообщенияВТехПоддержку(
	...
	ДопПараметрыОтправкиСообщения =
		ИнтернетПоддержкаПользователейВызовСервера.ДопПараметрыОтправкиСообщения(
			ДопПараметры);
