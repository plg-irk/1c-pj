...
// Следующие процедуры и функции предназначены для интеграции подсистемы ЦентрЭДО.
...
// Возвращает результаты проверки подписей по данным контейнеров.
// 
// Параметры:
//  ДанныеКонтейнеров - см. СинхронизацияЭДО.НовыеДанныеОбъектов
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  МенеджерКриптографии - Неопределено,МенеджерКриптографии
// 
// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДОСлужебный.ПроверитьПодписиПоОбъектамКонтейнеров
Функция ПроверитьПодписиПоДаннымКонтейнеров(ДанныеКонтейнеров, КонтекстДиагностики, МенеджерКриптографии = Неопределено) Экспорт
	
	Возврат ЭлектронныеДокументыЭДОСлужебный.ПроверитьПодписиПоОбъектамКонтейнеров(
		ДанныеКонтейнеров, МенеджерКриптографии, КонтекстДиагностики)
	
КонецФункции

// Возвращает ссылку на электронный документ по идентификатору документооборота;
//
// Параметры:
...
// 
// Возвращаемое значение:
//  Неопределено,ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
Функция НайтиПоИдентификаторуДокументооборота(ИдентификаторДокументооборота, ЭтоВходящийЭДО) Экспорт
	...
	Если Выборка.Следующий() Тогда
		ЭлектронныйДокумент = Выборка.Ссылка; // ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
...
Функция ЕстьПравоОбработкиДокументов(КонтекстДиагностики = Неопределено) Экспорт
	...
	Если Не ЕстьПраво И КонтекстДиагностики <> Неопределено Тогда
		...
		Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(НСтр("ru = 'Выполнение действий по электронным документам'"),
			ЭлектронныеДокументыЭДОКлиентСервер.ВидОшибкиНетПравНаОбработкуДокументов(), ТекстСообщения, ТекстСообщения);
...
Процедура ПроверитьПравоСозданияДокументов() Экспорт
...
Функция ТипСлужебногоДокумента(ТипЭлементаРегламента) Экспорт
	...
	ИначеЕсли РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипЭлементаРегламента) Тогда
...
Функция КраткоеНаименованиеТипаДокумента(ТипДокумента) Экспорт
	...
	Возврат Перечисления.ТипыДокументовЭДО.КраткоеНаименование(ТипДокумента);
...
// 
// Возвращаемое значение:
//  См. НовыеНастройкиОтправкиОбъектовУчетаСМножественнымиОписаниями
Функция НастройкиОтправкиОбъектовУчетаСМножественнымиОписаниями(ОписанияОбъектовУчета) Экспорт
	...
	НастройкиОбъектов = НовыеНастройкиОтправкиОбъектовУчетаСМножественнымиОписаниями();
	Если РезультатЗапроса.Пустой() Тогда
...
//  * Формировать - Булево
//  * ЗаполнениеКодаТовара - Строка
Функция НовыеНастройкиОтправкиОбъектовУчетаСМножественнымиОписаниями() Экспорт
	
	МетаданныеОпределяемыеТипы = Метаданные.ОпределяемыеТипы;
	
	НастройкиОбъектов = Новый ТаблицаЗначений;
	НастройкиОбъектов.Колонки.Добавить("Ключ", ОбщегоНазначения.ОписаниеТипаЧисло(10));
	НастройкиОбъектов.Колонки.Добавить("ОбъектУчета", МетаданныеОпределяемыеТипы.ОснованияЭлектронныхДокументовЭДО.Тип);
	НастройкиОбъектов.Колонки.Добавить("Отправитель", МетаданныеОпределяемыеТипы.Организация.Тип);
	НастройкиОбъектов.Колонки.Добавить("Получатель", МетаданныеОпределяемыеТипы.УчастникЭДО.Тип);
	НастройкиОбъектов.Колонки.Добавить("Направление", Новый ОписаниеТипов("ПеречислениеСсылка.НаправленияЭДО"));
	НастройкиОбъектов.Колонки.Добавить("ТипДокумента",Новый ОписаниеТипов("ПеречислениеСсылка.ТипыДокументовЭДО"));
	НастройкиОбъектов.Колонки.Добавить("Договор", МетаданныеОпределяемыеТипы.ДоговорСКонтрагентомЭДО.Тип);
	НастройкиОбъектов.Колонки.Добавить("ВидДокумента", Новый ОписаниеТипов("СправочникСсылка.ВидыДокументовЭДО"));
	НастройкиОбъектов.Колонки.Добавить("Формат", Новый ОписаниеТипов("Строка"));
	НастройкиОбъектов.Колонки.Добавить("МаршрутПодписания",Новый ОписаниеТипов("СправочникСсылка.МаршрутыПодписания"));
	НастройкиОбъектов.Колонки.Добавить("СпособОбмена", Новый ОписаниеТипов("ПеречислениеСсылка.СпособыОбменаЭД"));
	НастройкиОбъектов.Колонки.Добавить("ИдентификаторОтправителя", Новый ОписаниеТипов("Строка"));
	НастройкиОбъектов.Колонки.Добавить("ИдентификаторПолучателя", Новый ОписаниеТипов("Строка"));
	НастройкиОбъектов.Колонки.Добавить("ТребуетсяОтветнаяПодпись", Новый ОписаниеТипов("Булево"));
	НастройкиОбъектов.Колонки.Добавить("ТребуетсяИзвещениеОПолучении", Новый ОписаниеТипов("Булево"));
	НастройкиОбъектов.Колонки.Добавить("ВыгружатьДополнительныеСведения", Новый ОписаниеТипов("Булево"));
	НастройкиОбъектов.Колонки.Добавить("ОбменБезПодписи", Новый ОписаниеТипов("Булево"));
	НастройкиОбъектов.Колонки.Добавить("Формировать", Новый ОписаниеТипов("Булево"));
	НастройкиОбъектов.Колонки.Добавить("ЗаполнениеКодаТовара", Новый ОписаниеТипов("Строка"));
...
Функция СтатусДокумента(ЭлектронныйДокумент) Экспорт
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.Аннулирован
		...
		ИЛИ СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ЗакрытСОтклонениемПриглашения 
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОжидаетсяПодтверждениеАннулирования
		...
		ИЛИ СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль(
			"ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ОбновитьПолныеДанныеДокументовЭДО(
...
Функция ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль(
			"ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ОбновитьПолныеДанныеДокументовЭДО(ЭлектронныеДокументы, КонтекстДиагностики);
	КонецЕсли;
...
Функция СвойстваДокументовДляОписанияПакетов(ЭлектронныеДокументы) Экспорт
	...
	Запрос.Текст =
		...
		|	ЭлектронныйДокументВходящийЭДО.ИспользоватьЦентрЭДО КАК ИспользоватьЦентрЭДО
		|ИЗ
		|	Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
		|ГДЕ
		|	ЭлектронныйДокументВходящийЭДО.Ссылка В (&ЭлектронныеДокументы)
		...
		|	ЭлектронныйДокументИсходящийЭДО.ИспользоватьЦентрЭДО КАК ИспользоватьЦентрЭДО
		|ИЗ
		|	Документ.ЭлектронныйДокументИсходящийЭДО КАК ЭлектронныйДокументИсходящийЭДО
		|ГДЕ
		|	ЭлектронныйДокументИсходящийЭДО.Ссылка В (&ЭлектронныеДокументы)";
...
// Возвращаемое значение:
//  Булево
Функция ЭтоДокументЦентраЭДО(ЭлектронныйДокумент) Экспорт
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "ИспользоватьЦентрЭДО");
КонецФункции
...
Функция ДанныеНоменклатурыДокументов(ЭлектронныеДокументы, КонтекстДиагностики) Экспорт
	...
	Если Не ЕстьИнтеграцияЦентраЭДО() Тогда
		Возврат ДанныеНоменклатурыДокументов;
	КонецЕсли;
	
	ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль(
		"ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
	ДанныеНоменклатурыДокументовИнтеграцияЦентраЭДО =
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ДанныеНоменклатурыДокументов(
			ЭлектронныеДокументы, КонтекстДиагностики);
	
	Если Не ЗначениеЗаполнено(ДанныеНоменклатурыДокументов) Тогда
		Возврат ДанныеНоменклатурыДокументовИнтеграцияЦентраЭДО;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
		ДанныеНоменклатурыДокументов, ДанныеНоменклатурыДокументовИнтеграцияЦентраЭДО);
...
Процедура ЗаблокироватьДанныеДокументаДляИзменения(ЭлектронныйДокумент, ЭтоВходящийЭДО) Экспорт
	...
	ЭлектронныеДокументыЭДОСлужебный.ЗаблокироватьДанныеДокументаДляИзменения(ЭлектронныйДокумент, ЭтоВходящийЭДО)
...
Функция ПроверитьПодписиДокумента(ЭлектронныйДокумент, КонтекстДиагностики) Экспорт
	...
	Если ЭтоДокументЦентраЭДО(ЭлектронныйДокумент) Тогда
		Возврат ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ПроверитьПодписиДокумента(
...
Функция ВыполнитьДействияПоИнтеграциямЭДО(ПараметрыВыполненияДействий, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = 
			ОбщегоНазначения.ОбщийМодуль("ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		РезультатДействийВЦентреЭДО = МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ВыполнитьДействияЭДО(
			ПараметрыВыполненияДействий);
		
		ИдентификаторМенеджераИнтеграцииЦентраЭДО = МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ИдентификаторМенеджераДействийЭДО();
		РезультатыПоИнтеграциямЭДО.Вставить(ИдентификаторМенеджераИнтеграцииЦентраЭДО, РезультатДействийВЦентреЭДО);
		
		ПараметрыВыполненияДляЛокальногоЭДО = ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО();
		ЗаполнитьЗначенияСвойств(ПараметрыВыполненияДляЛокальногоЭДО, ПараметрыВыполненияДействий);
		ПараметрыВыполненияДляЛокальногоЭДО.ОбъектыДействий = ИсключитьОбработанныеОбъектыДействийЭДО(
			ПараметрыВыполненияДействий.ОбъектыДействий, РезультатДействийВЦентреЭДО.ОбработанныеОбъекты);
		РезультатДействийВЦентреЭДО.ОбработанныеОбъекты = Неопределено;
	Иначе
		ПараметрыВыполненияДляЛокальногоЭДО = ПараметрыВыполненияДействий;
	КонецЕсли;
	
	РезультатДействийЭДО = ЭлектронныеДокументыЭДОСлужебный.ВыполнитьДействияЭДО(ПараметрыВыполненияДляЛокальногоЭДО);
	РезультатыПоИнтеграциямЭДО.Вставить(ЭлектронныеДокументыЭДОКлиентСервер.ИдентификаторМенеджераДействийЭДО(),
		РезультатДействийЭДО);
...
Функция НовыеСвойстваДокументаДляОпределенияДоступныхДействийПоЭДО() Экспорт
	...
	Параметры.Вставить("ИспользоватьЦентрЭДО", Ложь);
	Возврат Параметры;
КонецФункции
...
Функция ЕстьИнтеграцияЦентраЭДО() Экспорт
	Возврат ОбщегоНазначения.ПодсистемаСуществует(
		"ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ИнтеграцияЦентраЭДО");
КонецФункции
...
//  Строка
//
Функция ВидОперацииПоДействиюЭДО(Действие) Экспорт
	
	Возврат ЭлектронныеДокументыЭДОСлужебный.ВидОперацииПриДобавленииОшибки(НРег(Действие));
...
Процедура ПриЗаписиНовогоДокумента(ДокументОбъект, ОписанияСообщений, КонтекстДиагностики, ОбъектыУчета = Неопределено) Экспорт
	...
	СостоянияЭлементовРегламента = ЭлектронныеДокументыЭДОСлужебный.НовыйСостоянияСообщений();
...
Функция НовыеНастройкиОтправкиДокумента() Экспорт
	...
	НастройкиОтправки.Вставить("ИспользоватьЦентрЭДО", Ложь);
...
// * ОшибкаПодготовкиДанныхВИнтеграцииЦентраЭДО - Булево
...
Функция НовоеОписаниеОшибкиФормирования() Экспорт
	...
	ОписаниеОшибки.Вставить("ОшибкаПодготовкиДанныхВИнтеграцииЦентраЭДО", Ложь);
...
Функция ТекстЗапросаДанныхЭлементовСхемыРегламента(ЭтоВходящийЭДО) Экспорт
	ТекстЗапроса = 
		...
		|	ПрисоединенныеФайлы.Ссылка КАК ПрисоединенныйФайл,
		|	ПрисоединенныеФайлы.ПолноеИмяФайла КАК ПолноеИмяФайла,
		|	СообщениеЭДО.РодительскоеСообщение
		|ИЗ
		|	Документ.СообщениеЭДО КАК СообщениеЭДО
...
Процедура ПередУдалениемДокумента(Объект, Отказ) Экспорт
	...
	Если Объект.ИспользоватьЦентрЭДО Тогда
		МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль(
			"ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ПередУдалениемДокумента(Объект, Отказ);
	КонецЕсли;
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.РегистрыСведений.ТекстовыеСообщенияДокументовЭДО, Истина);
...
Функция ТипыСводныхСостоянийЭДОУниверсальногоФормата(ВерсияФормата, СостояниеЭДО) Экспорт
	...
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодписаниеОтклонения
		...
		ИЛИ СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправкеОтказаВПодписи
...
// 
// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДОСлужебный.СформироватьМаршрутПодписания
Функция СформироватьМаршрутПодписания(СвойстваСообщения, МаршрутПодписания = Неопределено, СписокПодписантов = Неопределено) Экспорт
	...
	Возврат ЭлектронныеДокументыЭДОСлужебный.СформироватьМаршрутПодписания(СвойстваСообщения, МаршрутПодписания,
		СписокПодписантов);
	
КонецФункции

#КонецОбласти // РаботаСМаршрутамиПодписания

#Область ОбработкаНеисправностей

// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДОСлужебный.ВидОшибкиДокументыНедоступныДляПодписания
Функция ВидОшибкиДокументыНедоступныДляПодписания() Экспорт
	
	Возврат ЭлектронныеДокументыЭДОСлужебный.ВидОшибкиДокументыНедоступныДляПодписания();
	
КонецФункции

// Возвращаемое значение:
//  Строка
Функция ИдентификаторОшибкиДокументыНедоступныеДляПодписания() Экспорт
	
	Возврат "ДокументыНедоступныеДляПодписания";
	
КонецФункции

// Вид ошибки документы недоступны для подписания на сервере по доверенности (без решения).
// 
// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиДокументыНедоступныДляПодписанияНаСервереПоДоверенностиБезРешения() Экспорт
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = ИдентификаторОшибкиДокументыНедоступныеДляПодписанияНаСервереПоДоверенности();
...
// Возвращаемое значение:
//  Строка
Функция ИдентификаторОшибкиДокументыНедоступныеДляПодписанияНаСервереПоДоверенности() Экспорт
	
	Возврат "ДокументыНедоступныеДляПодписанияНаСервереПоДоверенности";
...
Функция ВидОшибкиДокументыВФорматеНеПоддержанномОператором(ИспользоватьОбработчик = Ложь) Экспорт
	...
	Возврат ЭлектронныеДокументыЭДОСлужебный.ВидОшибкиДокументыВФорматеНеПоддержанномОператором(ИспользоватьОбработчик);
...
#КонецОбласти // ОбработкаНеисправностей
...
Процедура ПриОпределенииМенеджеровДействийЭДО(МенеджерыДействийЭДО)
	...
	Если ЕстьИнтеграцияЦентраЭДО() Тогда
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = 
			ОбщегоНазначения.ОбщийМодуль("ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		ОбщийМодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ПриОпределенииМенеджеровДействийЭДО(МенеджерыДействийЭДО);
	КонецЕсли;
...
Функция ПродолжитьВыполнениеДействийПоИнтеграциямЭДОПослеОбработкиРезультатов(ПараметрыПродолженияДействийПоИнтеграциямЭДО, ПараметрыВыполненияДействий) Экспорт
	...
	Если НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
		МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО = 
			ОбщегоНазначения.ОбщийМодуль("ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО");
		ИдентификаторМенеджера = МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ИдентификаторМенеджераДействийЭДО();
		ПараметрыПродолжения = ПараметрыПродолженияДействийПоИнтеграциямЭДО[ИдентификаторМенеджера];
		РезультатДействийВЦентреЭДО = МодульЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ПродолжитьВыполнениеДействийЭДОПослеОбработкиРезультатов(
			ПараметрыПродолжения, ПараметрыВыполненияДействий);
		РезультатыПоИнтеграциямЭДО.Вставить(ИдентификаторМенеджера, РезультатДействийВЦентреЭДО);
	...
	ИдентификаторМенеджера = ЭлектронныеДокументыЭДОКлиентСервер.ИдентификаторМенеджераДействийЭДО();
		ПараметрыПродолжения = ПараметрыПродолженияДействийПоИнтеграциямЭДО[ИдентификаторМенеджера];
	РезультатДействий = ЭлектронныеДокументыЭДОСлужебный.ПродолжитьВыполнениеДействийЭДОПослеОбработкиРезультатов(
		ПараметрыПродолжения, ПараметрыВыполненияДляЛокальногоЭДО);
...
// Параметры:
//  ИсходныеОбъектыДействий - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеОбъектыДействийПоЭДО
//  ОбработанныеОбъектыДействий - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеОбъектыДействийПоЭДО
// 
// Возвращаемое значение:
//  См. ЭлектронныеДокументыЭДОКлиентСервер.НовыеОбъектыДействийПоЭДО
Функция ИсключитьОбработанныеОбъектыДействийЭДО(ИсходныеОбъектыДействий, ОбработанныеОбъектыДействий)
	
	Если Не ЗначениеЗаполнено(ОбработанныеОбъектыДействий) Тогда
		Возврат ИсходныеОбъектыДействий;
	КонецЕсли;
	
	ОбъектыДействий = ЭлектронныеДокументыЭДОКлиентСервер.НовыеОбъектыДействийПоЭДО();
	
	Для Каждого ОбработанныеОбъектыПоТипу Из ОбработанныеОбъектыДействий Цикл
		ТипОбъектов = ОбработанныеОбъектыПоТипу.Ключ;
		Если ЗначениеЗаполнено(ОбработанныеОбъектыПоТипу.Значение) Тогда
			ОбъектыДействий[ТипОбъектов] = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
				ИсходныеОбъектыДействий[ТипОбъектов], ОбработанныеОбъектыПоТипу.Значение);
		Иначе
			ОбъектыДействий[ТипОбъектов] = ИсходныеОбъектыДействий[ТипОбъектов];
		КонецЕсли;
	КонецЦикла;
...
Функция ТипСостоянияЭлектронногоДокументаУниверсальногоФормата_1_10(СостояниеЭДО)
	...
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправке
		...
		Или СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи
		Или СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяПовторнаяОтправка
...
Функция ТипСостоянияЭлектронногоДокументаУниверсальногоФормата_1_15(СостояниеЭДО)
	...
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи Тогда
		ТипСостояния = "ТребуетсяОтправкаОтказаВПодписи";
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаПриглашения Тогда
	...
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправкеОтказаВПодписи Тогда
		ТипСостояния = "ТребуетсяПодготовкаКОтправкеОтказаВПодписи";
	...
	ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту Тогда
		ТипСостояния = "ОтказаноВПодписиКонтрагенту";
		ИначеЕсли СостояниеЭДО = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
		ТипСостояния = "КонтрагентОтказалВПодписи";
	Иначе
		ТипСостояния = "";
	КонецЕсли;
...
Функция СостояниеЭДОПоЗначениюУниверсальногоФормата_1_15(ТипСостояния)
	...
	ИначеЕсли ТипСостояния = "ТребуетсяОтправкаОтказаВПодписи" Тогда
		Состояние = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи;
	...
	ИначеЕсли ТипСостояния = "ТребуетсяПодготовкаКОтправкеОтказаВПодписи" Тогда
		Состояние = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправкеОтказаВПодписи;
	...
	ИначеЕсли ТипСостояния = "ОтказаноВПодписиКонтрагенту" Тогда
		Состояние = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту;
	ИначеЕсли ТипСостояния = "КонтрагентОтказалВПодписи" Тогда
		Состояние = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи;
	Иначе
		Состояние = Перечисления.СостоянияДокументовЭДО.ПустаяСсылка();
	КонецЕсли;
