...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	// +РАРУС
	МассивТиповНастроек.Добавить(Тип("НастройкиКомпоновкиДанных"));
	// -РАРУС
	...
	// +РАРУС
	ТаблицаОтчетов.Колонки.Добавить("ПолеОтбора", Новый ОписаниеТипов("Строка"));
	ТаблицаОтчетов.Колонки.Добавить("ВидПериода", Новый ОписаниеТипов("Строка"));
	
	Массив = Новый Массив;
	Массив.Добавить(Тип("СтандартнаяДатаНачала"));
	Массив.Добавить(Тип("Дата"));
	ТаблицаОтчетов.Колонки.Добавить("НачалоПериода", Новый ОписаниеТипов(Массив));
	ТаблицаОтчетов.Колонки.Добавить("КонецПериода", Новый ОписаниеТипов(Массив));
	// -РАРУС
	...
	Для Каждого СтрокаОтчет Из Рассылка.Отчеты Цикл
		...
		// +РАРУС
		Стр.ПолеОтбора = СтрокаОтчет.ПолеОтбора;
		
		Если ТипЗнч(СтрокаОтчет.НачалоПериода) = Тип("Строка") Тогда
			Стр.НачалоПериода = Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[СтрокаОтчет.НачалоПериода]);
			Стр.КонецПериода = Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[СтрокаОтчет.КонецПериода]);
		Иначе
			Стр.НачалоПериода = СтрокаОтчет.НачалоПериода;
			Стр.КонецПериода = СтрокаОтчет.КонецПериода;
		КонецЕсли;
		Стр.ВидПериода = СтрокаОтчет.ВидПериода;
		// -РАРУС
...
//           - НастройкиКомпоновкиДанных - Табличный документ сформируется механизмами СКД.
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		...
		// +РАРУС
		ПараметрыОтчета = Новый Структура("Отчет, Настройки, Форматы, ОтправлятьЕслиПустой, НачалоПериода, КонецПериода");
		Если ПараметрыДоставки.Персонализирована Тогда
			ПараметрыОтчета.Вставить("ПолеОтбора");
		КонецЕсли;
		// -РАРУС
		...
		// +РАРУС
		Если ПараметрыДоставки.Персонализирована Тогда
			ПараметрыОтчета.Вставить("Персонализирован", ЗначениеЗаполнено(ПараметрыОтчета.ПолеОтбора));
		Иначе
...
//
// Параметры:
//   СписокФорматов - Произвольное - Передается из параметров процедуры "как есть".
...
Процедура АктуализироватьСписокРегламентныхЗаданий(Параметры) Экспорт
	...
	Пока РассылкиОтчетов.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать регламентное задание рассылки отчетов: %1 по причине: %2'"),
			РассылкиОтчетов.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//       * Настройки - Неопределено, НастройкиКомпоновкиДанных, ТаблицаЗначений -
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	// +РАРУС
	ПараметрыОтчета.Вставить("ЕстьНачалоПериода", Ложь);
	ПараметрыОтчета.Вставить("ЕстьКонецПериода",  Ложь);
	ПараметрыОтчета.Вставить("СписокВыбораПоляОтбора", Новый СписокЗначений);
	// -РАРУС
	...
	// +РАРУС
	Если ТипЗнч(ПараметрыОтчета.Настройки) = Тип("НастройкиКомпоновкиДанных") Тогда
		ПараметрыПодключения.Вставить("НастройкиКД", ПараметрыОтчета.Настройки);
	КонецЕсли;
	// -РАРУС
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		//	ПользовательскиеНастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.Настройки;
		//	// -РАРУС
		...
		ОбъектНастройки = ПараметрыОтчета.КомпоновщикНастроекКД.Настройки.ПараметрыДанных.Элементы;
		Для Каждого ЭлементПараметров Из ОбъектНастройки Цикл
			...
			Если ИмяПараметра = "НачалоПериода" Тогда
				ПараметрыОтчета.ЕстьНачалоПериода = Истина;
				Если НЕ ПараметрыОтчета.Свойство("НачалоПериода") Тогда
					ПараметрыОтчета.Вставить("НачалоПериода", НачалоМесяца(ТекущаяДатаСеанса()));
				КонецЕсли;
				Продолжить;
			КонецЕсли;
			
			Если ИмяПараметра = "КонецПериода" ИЛИ ИмяПараметра = "Период" ИЛИ ИмяПараметра = "ПериодГраница" Тогда
				ПараметрыОтчета.ЕстьКонецПериода = Истина;
				Если ИмяПараметра = "Период" И ТипЗнч(ЭлементПараметров.Значение) = Тип("СтандартныйПериод") Тогда
					ПараметрыОтчета.ЕстьНачалоПериода = Истина;
					Если НЕ ПараметрыОтчета.Свойство("НачалоПериода") Тогда
						ПараметрыОтчета.Вставить("НачалоПериода", НачалоМесяца(ТекущаяДатаСеанса()));
					КонецЕсли;
				ИначеЕсли ИмяПараметра = "Период" И НЕ ТипЗнч(ЭлементПараметров.Значение) = Тип("СтандартныйПериод") Тогда
					ПараметрыОтчета.ЕстьНачалоПериода = Ложь;
				КонецЕсли;
				Если НЕ ПараметрыОтчета.Свойство("КонецПериода") Тогда
					ПараметрыОтчета.Вставить("КонецПериода", ТекущаяДатаСеанса());
				КонецЕсли;
				Продолжить;
			КонецЕсли;
		...
		Для Каждого ТекОтбор Из ПараметрыОтчета.КомпоновщикНастроекКД.Настройки.ДоступныеПоляОтбора.Элементы Цикл
			
			Если ТекОтбор.Тип.СодержитТип(ТипКонтрагенты) Тогда
				Представление = ТекОтбор.Заголовок + "_Истина";
				ПараметрыОтчета.СписокВыбораПоляОтбора.Добавить(СокрЛП(ТекОтбор.Поле), Представление);
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Получатель <> Неопределено И ПараметрыОтчета.Свойство("ПолеОтбора") Тогда
	// -РАРУС
		Если ПараметрыОтчета.СКД Тогда
			// +РАРУС
			//ПользовательскиеНастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки;
			...
			ПолеОтбора = Новый ПолеКомпоновкиДанных(ПараметрыОтчета.ПолеОтбора);
			
			ДоступноеПолеОтбора = ПараметрыОтчета.КомпоновщикНастроекКД.Отбор.ДоступныеПоляОтбора.НайтиПоле(ПолеОтбора);
			Если ДоступноеПолеОтбора = Неопределено ИЛИ (НЕ ДоступноеПолеОтбора.ТипЗначения.СодержитТип(ТипЗнч(Получатель))) Тогда
				Возврат Результат;
			Иначе
				Отбор = ПараметрыОтчета.КомпоновщикНастроекКД.Отбор.Элементы;
				ЭлементОтбора = Неопределено;
				Для Каждого ТекЭлемент Из Отбор Цикл
					Если ТекЭлемент.ЛевоеЗначение = ПолеОтбора Тогда
	...
	// +РАРУС
	ОтчетыПлатформаСервер.ЗаполнитьФорматПараметров(ПараметрыОтчета.Объект.СхемаКомпоновкиДанных);
	
	Если ТипЗнч(ПараметрыОтчета.КомпоновщикНастроекКД) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда
		ПараметрыОтчета.КомпоновщикНастроекКД = ПараметрыОтчета.КомпоновщикНастроекКД.Настройки;
	КонецЕсли;
	ВыводитьЗаголовок = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьЗаголовок");
	Если (НЕ ВыводитьЗаголовок.Использование) И ПараметрыОтчета.Свойство("Метаданные") Тогда
		
		ЗаголовокОтчета = ПараметрыОтчета.Метаданные.Синоним;
		
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьЗаголовок").Значение        = ТипВыводаТекстаКомпоновкиДанных.Выводить;
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьЗаголовок").Использование   = Истина;
		
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("Заголовок").Значение                = ЗаголовокОтчета;
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("Заголовок").Использование           = Истина;
	КонецЕсли;
	
	ВыводитьОтбор = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор");
	Если НЕ ВыводитьОтбор.Использование Тогда
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор").Значение            = ТипВыводаТекстаКомпоновкиДанных.Выводить;
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьОтбор").Использование       = Истина;
	КонецЕсли;
	
	ВыводитьПараметры = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьПараметрыДанных");
	Если НЕ ВыводитьПараметры.Использование Тогда
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьПараметрыДанных").Значение      = ТипВыводаТекстаКомпоновкиДанных.Выводить;
		ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыВывода.Элементы.Найти("ВыводитьПараметрыДанных").Использование = Истина;
	КонецЕсли;
	
	Если ПараметрыОтчета.ЕстьНачалоПериода Тогда
		ДоступныйПараметр = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("НачалоПериода"));
		Если НЕ ДоступныйПараметр = Неопределено Тогда
			ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.УстановитьЗначениеПараметра("НачалоПериода", ПараметрыОтчета.НачалоПериода);
		КонецЕсли;
	КонецЕсли;
	
	Если ПараметрыОтчета.ЕстьКонецПериода Тогда
		// Конец периода
		ЗначениеПериода = ?(ПараметрыОтчета.КонецПериода = Дата(1, 1, 1), КонецДня(ТекущаяДатаСеанса()), КонецДня(ПараметрыОтчета.КонецПериода));
		
		ДоступныйПараметр = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("КонецПериода"));
		Если НЕ ДоступныйПараметр = Неопределено Тогда
			ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.УстановитьЗначениеПараметра("КонецПериода", ЗначениеПериода);
		КонецЕсли;
		
		// Конец периода с границей
		ДоступныйПараметр = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("ПериодГраница"));
		Если НЕ ДоступныйПараметр = Неопределено Тогда
			ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.УстановитьЗначениеПараметра("ПериодГраница", Новый Граница(ЗначениеПериода, ВидГраницы.Включая));
		КонецЕсли;
		
		// Период
		ДоступныйПараметр = ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("Период"));
		Если НЕ ДоступныйПараметр = Неопределено И НЕ ПараметрыОтчета.ЕстьНачалоПериода Тогда
			ПараметрыОтчета.КомпоновщикНастроекКД.ПараметрыДанных.УстановитьЗначениеПараметра("Период", ЗначениеПериода);
		КонецЕсли;
	КонецЕсли;
	// -РАРУС
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.Архивировать Тогда
		...
		ПараметрыДоставки.ИмяАрхива = ПривестиИмяФайла(ИмяАрхива);
...
Процедура АрхивацияВложений(Вложения, ПараметрыДоставки, КаталогВременныхФайлов)
	...
	Для Каждого Вложение Из Вложения Цикл
		// При скачивании zip с почты, в названиях отчетов иероглифы (проблема кодировки) → делаем
		// транслитерацию названий отчетов.
		НовоеИмяФайла = КаталогВременныхФайлов() + СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(Вложение.Настройки.ИмяФайла);
		ПереместитьФайл(Вложение.Значение, НовоеИмяФайла);
		ЗаписьZipФайла.Добавить(НовоеИмяФайла, РежимСохранения, РежимОбработки);
...
Функция СоздатьДеревоОтчетов()
	...
	//   Ключ      - Имя файла.
	//   Значение  - Полный путь к файлу.
	//   Настройки - ФайлСКаталогом, ИмяФайла, ПолноеИмяФайла, ИмяКаталога,
	//               ПолноеИмяКаталога, Формат, Имя, Расширение, ТипФайла.
