...
Функция АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	// +Рарус Хотят чтобы при старте пустой базы создавался администратор
		//Если ПустаяСтрока(ТекущийПользовательИБ.Имя) Тогда
		//ТекущийПользовательИБ = Неопределено
	...
	Если ПользовательПоИдентификаторуСуществует(
		...
		Если ПриНачалеРаботыСистемы
		   И Пользователи.ЭтоПолноправныйПользователь(ТекущийПользовательИБ, ОбщегоНазначенияПовтИсп.РежимРаботыПрограммы().Локальный, Ложь) Тогда
			...
			УправлениеДоступомСлужебный.ПриАвторизацииАдминистратораПриНачалеРаботыСистемы(НайденныйПользователь);
		КонецЕсли;
	...
	Если ТребуетсяСоздатьАдминистратора Тогда
		...
		Если РольДоступна(Метаданные.Роли.ПолныеПрава)
			...
			УправлениеДоступомСлужебный.ПослеЗаписиАдминистратораПриАвторизации(Комментарий);
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1
			           ...
			           |Следует очистить минимальную длину и требование сложности пароля в конфигураторе
			           |и задать настройки входа пользователей в настройках программы.'"),
			ТекстОшибки);
	КонецЕсли;
...
Функция СоздатьНеуказанногоПользователя() Экспорт
	...
	Иначе
		...
		// +РАРУС Установим идентификационные реквизиты пользователя
...
Процедура ПреобразоватьИменаРолейВИдентификаторы() Экспорт
	...
	|	И Роли.Роль <> ЗНАЧЕНИЕ(Справочник.ИдентификаторыОбъектовРасширений.ПустаяСсылка)
	|				И Роли.УдалитьРоль = """")";
...
Процедура ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей() Экспорт
	...
	// +СофтФон
	ПараметрыВида = МодульУправлениеКонтактнойИнформацией.ПараметрыВидаКонтактнойИнформации("Телефон");
	ПараметрыВида.Вид = "CRM_ТелефонПользователяРабочий";
	ПараметрыВида.МожноИзменятьСпособРедактирования = Истина;
	ПараметрыВида.РазрешитьВводНесколькихЗначений = Истина;
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь, РегистрироватьВЖурнале = Истина)
	...
	Попытка
		...
		КонецЕсли;
		Записать = Ложь;
		Если СведенияОПользователе.ДатаПоследнейАктивности <> ТекущаяДатаСеанса() Тогда
			СведенияОПользователе.ДатаПоследнейАктивности = ТекущаяДатаСеанса();
			Записать = Истина;
		КонецЕсли;
...
Процедура ЗаполнитьРоли(Параметры)
	...
	Для Каждого ОписаниеРоли Из ПрочитанныеРоли Цикл
		...
		ИначеЕсли ТипЗнч(Роль) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
			ИмяРоли = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Роль, "Имя");
			ИмяРоли = ?(ИмяРоли = Неопределено, "(" + Роль.УникальныйИдентификатор() + ")", ИмяРоли);
			ИмяРоли = ?(Лев(ИмяРоли, 1) = "?", Сред(ИмяРоли, 2), ИмяРоли);
			КоллекцияРолей.Добавить().Роль = СокрЛП(ИмяРоли);
