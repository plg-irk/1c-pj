...
Процедура ИнициализироватьСлужебныеРеквизиты(Форма)
	...
	СлужебныеРеквизиты.Вставить("КлючОбъекта",            СлужебныеРеквизиты.ПолноеИмяОбъекта+?(ПолучитьЗначениеПараметраСтруктуры(Форма.Параметры, "РежимВыбора", ЛОЖЬ), ".РежимВыбора", ""));
	СлужебныеРеквизиты.Вставить("ЭтоЖурналДокументов",    ЛЕВ(Форма.Список.ОсновнаяТаблица, 16)="ЖурналДокументов");
	СлужебныеРеквизиты.Вставить("ИмяДополнительнойФормы", ?(СлужебныеРеквизиты.ЭтоЖурналДокументов, "", УправлениеДиалогомВызовСервера.ПолучитьИмяДополнительнойФормыОбъекта(СлужебныеРеквизиты.ПолноеИмяОбъекта)));
...
Процедура ИнициализироватьКомандыПанелиДействий(Форма, Объект)
	...
	Если НЕ Форма.Элементы.Найти("Связи") = Неопределено Тогда
		ЗащищенныеФункцииСервер.ДобавитьКоманду(Форма, "ПоказатьСвязи", ИмяКоманднойПанелиФормы, БиблиотекаКартинок.Связь, НСтр("ru = 'Связи документов'"), НСтр("ru = 'Показать/скрыть поле связей документов'"), ЛОЖЬ,,, ОтображениеКнопки.КартинкаИТекст);
	...
	Если НЕ Форма.Элементы.Найти("КомандыЭДО") = Неопределено Тогда
		...
		ОбменСКонтрагентами.ПриСозданииНаСервере_ФормаСписка(Ложь,Истина,ПараметрыПриСозданииНаСервере);
	КонецЕсли;
	...
	Если НазначитьДополнительныеДействия Тогда
		...
		Если ПравоПользователя("ПроверкаЗаполненияСправочниковИДокументов") И Форма.Элементы.Список.РежимВыделения = РежимВыделенияТаблицы.Множественный Тогда
			ЗащищенныеФункцииСервер.ДобавитьКоманду(Форма, "ИзменитьВыделенные", "ДополнительныеДействия", БиблиотекаКартинок.ДействиеИзменить, НСтр("ru = 'Изменить выделенные объекты'"), НСтр("ru = 'Изменить выделенные объекты'"), Истина);
		КонецЕсли;
...
// Производит установку расширенного заголовка формы списка документов.
//
// Параметры
//  Форма - УправляемаяФорма - форма переданного документа.
//
Процедура УстановитьЗаголовокСпискаДокументов(Форма)
...
КонецПроцедуры // УстановитьЗаголовокСпискаДокументов()
...
Функция ОбработкаКомандыФормы(Форма, КомандаИмя, ПараметрыДействия=Неопределено) Экспорт
	...
	ИначеЕсли Лев(КомандаИмя, 21)="УтверждениеДокументов" Тогда
		...
		УтверждениеДокументовСервер.ОбработкаКомандыФормы(Форма, КомандаИмя, Форма.Элементы.Список.ТекущаяСтрока, ПараметрыДействия);
...
Функция ПриСозданииНаСервере(Форма, Параметры, Отказ=ЛОЖЬ, СтандартнаяОбработка=ИСТИНА, Объект = Неопределено) Экспорт
	...
	Если (НЕ ПолеФормыХозОперация=Неопределено) И ПолеФормыХозОперация.Видимость Тогда
		...
		Если Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
			...
			СписокХозОпераций = Справочники.ХозОперации.ПолучитьСписокХозОпераций(МенеджерОбъекта);
	...
	КонтролироватьПодразделение = ПравоПользователя("КонтролироватьПодразделение");
	Если КонтролироватьПодразделение <> Перечисления.ВидыКонтроля.НеКонтролировать Тогда
		РежимОтображения = ?(КонтролироватьПодразделение=Перечисления.ВидыКонтроля.Запрещать
				,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный
				,РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ);
		// Установим отбор по подразделению пользователя
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				...
				РежимОтображения);	
	КонецЕсли;
	Если НЕ Форма.Элементы.Список.ПодчиненныеЭлементы.Найти("Контрагент")=Неопределено Тогда
		СписокКонтрагентов = РегистрыСведений.СегментацияКлиентов.ПолучитьСписокКонтрагентовПользователя();
		Если НЕ СписокКонтрагентов=Неопределено Тогда
			// Установим отбор по сегментам
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
					Форма.Список,
					"Контрагент",
					СписокКонтрагентов,
	...
	УстановитьЗаголовокСпискаДокументов(Форма);
...
Процедура СписокПриАктивизацииСтроки(Форма, Элемент) Экспорт
	...
	ЗащищенныеФункцииСервер.УстановитьПараметрыКомандПечати(Форма, Элемент.ТекущаяСтрока, Форма.ЭтоЖурналДокументов);
...
//
// Параметры:
//   Форма     - Управляемая форма, форма переданного списка документов.
