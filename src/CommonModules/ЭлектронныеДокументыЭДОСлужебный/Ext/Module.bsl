...
Функция ВыполнитьДействияЭДО(ПараметрыВыполнения, КонтекстДиагностики = Неопределено) Экспорт
	...
	Если ЭлектронныеДокументыЭДОКлиентСервер.ОтсутствуютОбъектыДействий(
		ПараметрыВыполнения.ОбъектыДействий) Тогда
...
Функция ПродолжитьВыполнениеДействийЭДОПослеОбработкиРезультатов(ПараметрыПродолжения, ПараметрыВыполнения) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатОбработкиПодписания) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПараметрыВыполнения.ВыбранныеСертификаты,
	...
	Если ЗначениеЗаполнено(НаборДействийДляПродолжения) Тогда
		...
		ЗаполнитьЗначенияСвойств(ПараметрыВыполненияПослеОбработки, ПараметрыВыполнения,,"НаборДействий");
...
Процедура ЗаполнитьОбъектыДействийПоПакетамДокументовЭДО(ОбъектыДействий, ПакетыДокументов)
	...
	ОбъектыДействий.ПакетыДокументов = ПакетыДокументовЭДО.ОтобратьПакетыДокументовПоПризнакуЦентраЭДО(
...
Процедура ЗаполнитьОбъектыДействийПоДокументамЭДО(ОбъектыДействий, ЭлектронныеДокументы)
	...
	РеквизитыПоДокументам = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ЭлектронныеДокументы, "ИспользоватьЦентрЭДО");
	
	Для Каждого РеквизитыПоДокументу Из РеквизитыПоДокументам Цикл
		Если РеквизитыПоДокументу.Значение.ИспользоватьЦентрЭДО Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьОбъектыДействийПоСообщениямЭДО(ОбъектыДействий, СообщенияЭДО)
	...
	Для Каждого СвойстваСообщения Из СвойстваСообщений Цикл
		Если СвойстваСообщения.Значение.ИспользоватьЦентрЭДО Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПродолжитьВыполнениеДействийПоЭДО(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.СформироватьДополнительнуюИнформацию) Тогда
		...
		ВыполнитьДействиеСформироватьДополнительнуюИнформацию(ПараметрыВыполнения, РезультатДействий);
	...
	Если ЕстьДействие(НаборДействий, Перечисления.ДействияПоЭДО.ЗапроситьУточнение) Тогда
		...
		ВыполнитьДействиеЗапроситьУточнение(ПараметрыВыполнения, РезультатДействий);
...
// Возвращаемое значение:
//  Структура:
//  * Владелец - ДокументСсылка.СообщениеЭДО
...
Функция СвойстваФайлаОснованияСлужебногоДокумента()
	...
	Свойства.Вставить("Владелец", Документы.СообщениеЭДО.ПустаяСсылка());
...
//  ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО,ДокументСсылка.ЭлектронныйДокументИсходящийЭДО
//  ЭтоВходящийЭДО - Булево
Процедура ЗаблокироватьДанныеДокументаДляИзменения(ЭлектронныйДокумент, ЭтоВходящийЭДО) Экспорт
...
//  СписокПодписантов - Неопределено,Массив из СправочникСсылка.Пользователи
// 
// Возвращаемое значение:
...
Процедура ВыполнитьДействиеСформироватьПоОбъектамУчета(ПараметрыВыполнения, РезультатДействий)
	...
	Для Каждого ПараметрыФормирования Из НаборПараметровФормирования Цикл
		...
		Если ИдентификаторыПоИнтеграциямЭДО.ИдентификаторыДляЦентраЭДО.Найти(ИдентификаторОрганизации) <> Неопределено
			И Не НастройкиЭДО.ИспользоватьИнтеграциюЦентраЭДО() Тогда
			Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
				УчетныеЗаписиЭДОИнтеграцияЦентраЭДО.ВидОшибкиОтключенаНастройкаИспользоватьЦентрЭДО(ИдентификаторОрганизации),
...
//
// Возвращаемое значение:
//  См. НовыйРезультатСозданияДокументаПоФайлу
Функция СоздатьДокументПоФайлу(ПараметрыСоздания, ОписаниеФайла) Экспорт
	...
	ОписаниеФайлаДокумента = ЗаполнитьДанныеУчастниковЭДО(ОписаниеФайла, НастройкиОтправки, Содержание);
	...
	РезультатПроверкиОграничения = ЭлектронныеДокументыЭДО.ПроверкаПревышенияМаксимальногоРазмераФайла(
		...
		ОписаниеФайлаДокумента.ДвоичныеДанные);
	...
	КонецЕсли;
	НастройкиДокумента.ИспользоватьЦентрЭДО = УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(
	...
	ОписаниеСообщения.Данные.Документ = ОписаниеФайлаДокумента;
...
//  
// Возвращаемое значение:
//  См. ФорматыЭДО.ЗаполнитьДанныеУчастниковЭДО
Функция ЗаполнитьДанныеУчастниковЭДО(ОписаниеФайла, НастройкиОтправки, Содержание)
	
	Если Не ЗначениеЗаполнено(Содержание)
		Или Не ЗначениеЗаполнено(Содержание.Формат)
		Или Не ФорматыЭДО.ЗаполнениеДанныхУчастниковЭДОДоступно(Содержание.Формат) Тогда
		Возврат ОписаниеФайла;
	КонецЕсли;
	
	ДанныеУчастниковЭДО = ФорматыЭДО.ПрочитатьДанныеУчастниковЭДО(ОписаниеФайла, Содержание.Формат);
	
	Если ДанныеУчастниковЭДО.ИдентификаторОтправителя = НастройкиОтправки.ИдентификаторОтправителя
		И ДанныеУчастниковЭДО.ИдентификаторПолучателя = НастройкиОтправки.ИдентификаторПолучателя Тогда
		Возврат ОписаниеФайла;
	КонецЕсли;
	
	ДанныеУчастниковЭДО.ИдентификаторОтправителя = НастройкиОтправки.ИдентификаторОтправителя;
	ДанныеУчастниковЭДО.ИдентификаторПолучателя = НастройкиОтправки.ИдентификаторПолучателя;
	
	ДанныеОператора = СинхронизацияЭДО.СведенияОбОператоре(НастройкиОтправки.ИдентификаторОтправителя);
	...
	Возврат ФорматыЭДО.ЗаполнитьДанныеУчастниковЭДО(ОписаниеФайла, ДанныеУчастниковЭДО);
...
Процедура ДобавитьПараметрыФормированияИсходящегоДокумента(ПараметрыВыполнения, ВыборкаНастроек, НаборПараметровФормирования, ТаблицаСвязанныхОбъектовУчета, РезультатДействий)
	...
	Если НастройкиОбъекта <> Неопределено Тогда
		Если НастройкиОтправки.ВидДокумента = НастройкиОбъекта.ВидДокумента Тогда
...
Процедура ЗаполнитьИсходящийДокументОбъект(ДокументОбъект, НастройкиОтправки, ОписаниеСообщения,
	...
	КонецЕсли;
	ДокументОбъект.ИспользоватьЦентрЭДО = НастройкиОтправки.ИспользоватьЦентрЭДО;
...
Функция СоздатьСообщение(ОписаниеСообщения, ЭлектронныйДокумент, НастройкиДокумента) Экспорт
	...
	СообщениеОбъект.РодительскоеСообщение = ОписаниеСообщения.РодительскоеСообщение;
	...
	Если ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя
		И ОписаниеСообщения.Направление = Перечисления.НаправленияЭДО.Входящий И Содержание.ТипРегламента
		= Перечисления.ТипыРегламентовЭДО.АктОРасхождениях Тогда
		...
		Если СвойстваДокументовПоФорматам.ФормируютсяИтогиПриемки
			И (СвойстваДокументовПоФорматам.ВариантФормированияИтоговПриемки = ВариантыФормирования.ПродавцомФормируютсяДопСведенияОСогласииНесогласииСРезультатамиПриемки
			...
			ДополнительноеСообщениеОбъект.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ДопСведенияПоРезультатамПриемки;
		...
		КонецЕсли;
	ИначеЕсли РегламентыЭДО.ЭтоУниверсальноеСообщение(ОписаниеСообщения.ТипЭлементаРегламента) 
		И ЗначениеЗаполнено(ОписаниеСообщения.ДополнительнаяИнформация) Тогда
			
			РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.ДобавитьСообщение(СообщениеОбъект.Ссылка, ОписаниеСообщения.ДополнительнаяИнформация);
...
Функция СформироватьИсправление(ЭлектронныйДокумент, ПараметрыВыполнения, РезультатДействий)
	...
	Попытка
		...
		СостоянияСообщений = НовыйСостоянияСообщений();
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатыЗапроса[1].Выгрузить(), СостоянияСообщений);
...
Процедура СформироватьИзвещение(ЭлектронныйДокумент, РезультатДействий)
	...
	Попытка
		...
		ТипыЭлементовРегламентаРодительскихСообщений = РегламентыЭДО.ТипыЭлементовРегламентаРодительскихСообщений(
			СостоянияСообщений.ВыгрузитьКолонку("Ссылка"), 
			СостоянияСообщений);
		Для Каждого СвойстваСообщения Из СостоянияСообщений Цикл
			Если РегламентыЭДО.ЭтоУниверсальноеСообщение(СвойстваСообщения.ТипЭлементаРегламента) Тогда
				СвойстваСообщения.ТипЭлементаРегламентаРодителя = ТипыЭлементовРегламентаРодительскихСообщений[СвойстваСообщения.Ссылка];
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого СвойстваСообщения Из СостоянияСообщений Цикл
			...
			СвойстваИзвещения = Неопределено;
			Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипИзвещения) Тогда
				ОтборИзвещений = Новый Структура("ТипЭлементаРегламента, ТипЭлементаРегламентаРодителя", 
					ТипИзвещения,
					СвойстваСообщения.ТипЭлементаРегламента);
					
				СвойстваИзвещений = СостоянияСообщений.НайтиСтроки(ОтборИзвещений);
						
				Если ЗначениеЗаполнено(СвойстваИзвещений) Тогда
					СвойстваИзвещения = СвойстваИзвещений[0];
				КонецЕсли;
			Иначе
		...
		Пока ВыборкаФайлов.Следующий() Цикл
			...
			Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипИзвещения) Тогда
				ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение;
			КонецЕсли;
			ОписаниеСообщения = ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, ВыборкаФайлов,
				ТипДокумента, ТипИзвещения);
			...
			Если Не ПараметрыДокумента.ОбменБезПодписи 
				И Не РегламентыЭДО.ЭтоУниверсальноеСообщение(ИзвещениеОбъект.ТипЭлементаРегламента) Тогда
...
Процедура ОбновитьДанныеСостоянияДокументаЭДОПриПереформировании(ДокументОбъект, СообщениеОбъект,
	...
	НовыеСостоянияСообщений = НовыйСостоянияСообщений();
...
// Возвращаемое значение:
//  Строка
Функция ТекстЗапросаФайловДокументов()
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ПрисоединенныеФайлы.Ссылка
		|ИЗ
...
Функция Отклонить(Сообщение, КонтекстДиагностики, Комментарий = "", ЭлементДляОтклонения = Неопределено)
	...
	Попытка
		...
		Если РегламентыЭДО.ИспользоватьУниверсальныеСообщения(ПараметрыОпределенияИспользованияУС) Тогда
			ТипЭлементаРегламентаУОУ = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи;
		КонецЕсли;
		
		Если Не РегламентыЭДО.ОтклонениеДоступно(СостоянияСообщений, ЭтоВходящийЭДО, ТипЭлементаРегламентаУОУ) Тогда
			ДобавитьОшибкуОтменыОперации(КонтекстВыполненияДействия, КонтекстДиагностики);
			Результат.Отказ = Истина;
			ОтменитьТранзакцию();
			Возврат Результат;
		...
		Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипЭлементаРегламентаУОУ) Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение;
		КонецЕсли;
		
		ОписаниеСообщения = ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, СвойстваОсновногоФайла,
			ТипДокумента, ТипЭлементаРегламентаУОУ, Комментарий);
		...
		ДополненияСостоянийЭДО = Неопределено;
		Если Не ПараметрыДокумента.ОбменБезПодписи 
			И Не РегламентыЭДО.ЭтоУниверсальноеСообщение(СообщениеОбъект.ТипЭлементаРегламента) Тогда
...
//  ПараметрыВыполнения - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
Процедура ВыполнитьДействиеЗапроситьУточнение(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ОбъектыДействий.ПакетыДокументов) Тогда
		...
		Для Каждого ПакетДокументов Из ПараметрыВыполнения.ОбъектыДействий.ПакетыДокументов Цикл
			ЗапроситьУточнениеПоДокументамПакета(ПакетДокументов, РезультатДействий, Комментарий);
		КонецЦикла;
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ОбъектыДействий.ЭлектронныеДокументы)
		...
		Отбор = НовыйОтборСообщенийДляОбработкиДействия();
		Отбор.ТипЭлементаРегламента.Добавить(Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя);
		ЭлементДляУточнения = ДополнительныйПараметрДействия(ПараметрыВыполнения, Действие, "ЭлементДляУточнения");
		Если ЗначениеЗаполнено(ЭлементДляУточнения) Тогда
			Отбор.ТипЭлементаРегламента.Добавить(ЭлементДляУточнения);
		КонецЕсли;
		ВыборкаСообщений = ВыборкаСообщенийДляОбработкиДействия(ПараметрыВыполнения.ОбъектыДействий, Отбор);
		Пока ВыборкаСообщений.Следующий() Цикл
			Если ТипЗнч(Комментарий) = Тип("Соответствие") Тогда
			...
			Иначе
				КомментарийКДокументу = Комментарий;
			КонецЕсли;
			ЗапроситьУточнениеПоДокументу(ВыборкаСообщений.Ссылка, РезультатДействий, КомментарийКДокументу, ЭлементДляУточнения);
		КонецЦикла;
...
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
//  Комментарий - Строка
//  ЭлементДляУточнения - см. ДополнительныйПараметрДействия
Процедура ЗапроситьУточнениеПоДокументу(Сообщение, РезультатДействий, Комментарий = "", ЭлементДляУточнения = Неопределено)
	
	Результат = ЗапроситьУточнение(Сообщение, РезультатДействий.КонтекстДиагностики, Комментарий, ЭлементДляУточнения);
	
	Если Результат.Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ПослеЗапросаУточнения(Результат, РезультатДействий);
...
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
//  Комментарий - Строка
Процедура ЗапроситьУточнениеПоДокументамПакета(ПакетДокументов, РезультатДействий, Комментарий)
	...
	Попытка
		...
		Для Каждого Сообщение Из СообщенияПакета Цикл
			Результат = ЗапроситьУточнение(Сообщение, КонтекстДиагностики, Комментарий);
			Если Результат.Отказ Тогда
				ДобавитьОшибкуОтменыОперации(КонтекстВыполненияДействия, КонтекстДиагностики);
				ОтменитьТранзакцию();
				Возврат;
	...
	Для Каждого РезультатОтклонения Из НаборРезультатов Цикл
		ПослеЗапросаУточнения(РезультатОтклонения, РезультатДействий);
	КонецЦикла;
...
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
//  Комментарий - Строка
//  ЭлементДляУточнения - Неопределено
//                       - см. ДополнительныйПараметрДействия
// 
// Возвращаемое значение:
//  Структура:
...
// * СообщениеОбъект - Неопределено - 
// * ТаблицаПодписания - Неопределено - 
Функция ЗапроситьУточнение(Сообщение, КонтекстДиагностики, Комментарий = "", ЭлементДляУточнения = Неопределено)
	...
	Попытка
		...
		Если СообщениеОбъект.ТипЭлементаРегламента <> Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя
			И (?(ЗначениеЗаполнено(ЭлементДляУточнения), СообщениеОбъект.ТипЭлементаРегламента <> ЭлементДляУточнения, Истина)) Тогда
			ДобавитьОшибкуОтменыОперации(КонтекстВыполненияДействия, КонтекстДиагностики);
			Результат.Отказ = Истина;
			ОтменитьТранзакцию();
			Возврат Результат;
		...
		ОписаниеСообщения = ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, СвойстваОсновногоФайла,
			Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение, Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ, Комментарий);
...
// Параметры:
//  РезультатУточнения - Структура:
//  * Отказ - Булево
//  * СообщениеОбъект - ДокументОбъект.СообщениеЭДО
//  * ТаблицаПодписания - см. МаршрутыПодписанияБЭД.СформироватьМаршрутПодписанияЭД
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
Процедура ПослеЗапросаУточнения(РезультатУточнения, РезультатДействий)
	
	ЗаполнитьИтогВыполненияДействияПоЭДО(РезультатДействий.Итог, Перечисления.ДействияПоЭДО.ЗапроситьУточнение,
		РезультатУточнения.СообщениеОбъект.ЭлектронныйДокумент);	
КонецПроцедуры
...
//  ПараметрыВыполнения - см. ЭлектронныеДокументыЭДОКлиентСервер.НовыеПараметрыВыполненияДействийПоЭДО
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
Процедура ВыполнитьДействиеСформироватьДополнительнуюИнформацию(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ОбъектыДействий.ПакетыДокументов) Тогда
		...
		Для Каждого ПакетДокументов Из ПараметрыВыполнения.ОбъектыДействий.ПакетыДокументов Цикл
			СформироватьДополнительнуюИнформациюПоДокументамПакета(ПакетДокументов, РезультатДействий, Комментарий);
		КонецЦикла;
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ОбъектыДействий.ЭлектронныеДокументы)
		...
		Пока ВыборкаСообщений.Следующий() Цикл
			...
			Иначе
				КомментарийКДокументу = Комментарий;
			КонецЕсли;
			СформироватьДополнительнуюИнформациюПоДокументу(ВыборкаСообщений.Ссылка, РезультатДействий, КомментарийКДокументу, ЭлементДляУточнения);
		КонецЦикла;
...
//  Комментарий - Строка
//  ЭлементДляУточнения - см. ДополнительныйПараметрДействия
Процедура СформироватьДополнительнуюИнформациюПоДокументу(Сообщение, РезультатДействий, Комментарий = "", ЭлементДляУточнения = Неопределено)
	
	Результат = СформироватьДополнительнуюИнформацию(Сообщение, РезультатДействий.КонтекстДиагностики, Комментарий, ЭлементДляУточнения);
	
	Если Результат.Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ПослеФормированияДополнительнойИнформации(Результат, РезультатДействий);
...
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
//  Комментарий - Строка
Процедура СформироватьДополнительнуюИнформациюПоДокументамПакета(ПакетДокументов, РезультатДействий, Комментарий)
	...
	Попытка
		...
		Для Каждого Сообщение Из СообщенияПакета Цикл
			Результат = СформироватьДополнительнуюИнформацию(Сообщение, КонтекстДиагностики, Комментарий);
			Если Результат.Отказ Тогда
				ДобавитьОшибкуОтменыОперации(КонтекстВыполненияДействия, КонтекстДиагностики);
				ОтменитьТранзакцию();
				Возврат;
	...
	Для Каждого РезультатОтклонения Из НаборРезультатов Цикл
		ПослеФормированияДополнительнойИнформации(РезультатОтклонения, РезультатДействий);
	КонецЦикла;
...
// * СообщениеОбъект - Неопределено - 
// * ТаблицаПодписания - Неопределено - 
Функция СформироватьДополнительнуюИнформацию(Сообщение, КонтекстДиагностики, Комментарий = "", ЭлементДляУточнения = Неопределено)
	...
	РеквизитыСообщения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сообщение, "Направление, ЭлектронныйДокумент, ОсновнойФайл");
	...
	Попытка
		...
		ТекстыЗапросов.Добавить(ТекстЗапросаПараметровОбновленияСостоянияДокумента(ЭтоВходящийЭДО));
		ТекстыЗапросов.Добавить(ТекстЗапросаСвойствОсновногоФайла());
		Если РеквизитыСообщения.Направление = Перечисления.НаправленияЭДО.Исходящий
			ИЛИ РеквизитыСообщения.Направление = Перечисления.НаправленияЭДО.Входящий Тогда
			ТекстыЗапросов.Добавить(ТекстЗапросаУчастниковЭДО(ЭтоВходящийЭДО));
		КонецЕсли;
		...
		Запрос.Текст = СтрСоединить(ТекстыЗапросов, ОбщегоНазначения.РазделительПакетаЗапросов());
		Запрос.УстановитьПараметр("ЭлектронныйДокумент", ЭлектронныйДокумент);
		Запрос.УстановитьПараметр("ОсновнойФайл", РеквизитыСообщения.ОсновнойФайл);
		РезультатыЗапроса = Запрос.ВыполнитьПакет();
		...
		ОписаниеСообщения = ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, СвойстваОсновногоФайла,
			Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение, 
			Перечисления.ТипыЭлементовРегламентаЭДО.УС_ДополнительнаяИнформация, 
			Комментарий);
		
		Если ОписаниеСообщения.Данные.ЕстьОшибки Тогда
			
			ТекстОшибки = НСтр("ru = 'Не удалось сформировать дополнительную информацию'");
			ДобавитьОшибкуВыполненияДействия(ТекстОшибки, КонтекстВыполненияДействия, КонтекстДиагностики);
		...
		Результат.СообщениеОбъект = СоздатьСообщение(ОписаниеСообщения, ПараметрыДокумента.Ссылка, ПараметрыДокумента);
		
		РегистрыСведений.ТекстовыеСообщенияДокументовЭДО.ДобавитьСообщение(Результат.СообщениеОбъект.Ссылка, Комментарий);
...
//  * ТаблицаПодписания - см. МаршрутыПодписанияБЭД.СформироватьМаршрутПодписанияЭД
//  РезультатДействий - см. НовыйРезультатДействийПоЭДО
Процедура ПослеФормированияДополнительнойИнформации(РезультатУточнения, РезультатДействий)
	
	ЗаполнитьИтогВыполненияДействияПоЭДО(РезультатДействий.Итог, Перечисления.ДействияПоЭДО.СформироватьДополнительнуюИнформацию,
		РезультатУточнения.СообщениеОбъект.ЭлектронныйДокумент);	
КонецПроцедуры
...
Функция ОтклонитьАннулирование(Сообщение, КонтекстДиагностики, Комментарий = "")
	...
	Попытка
		...
		ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА_УОУ;
		
		ПараметрыОпределенияИспользованияУС = РегламентыЭДО.НовыеПараметрыОпределенияИспользованияУС();
		ЗаполнитьЗначенияСвойств(ПараметрыОпределенияИспользованияУС, ПараметрыДокумента);
		Если РегламентыЭДО.ИспользоватьУниверсальныеСообщения(ПараметрыОпределенияИспользованияУС) Тогда
			ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи;
		КонецЕсли;
		
		ТипДокумента = Перечисления.ТипыДокументовЭДО.УведомлениеОбУточнении;
		Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ТипЭлементаРегламента) Тогда
			ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение;
		КонецЕсли;
...
Функция Утвердить(Сообщение, КонтекстДиагностики, НаборОбъектовУчета = Неопределено)
	...
	Попытка
		...
		Если (Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОбменЗавершен
			...
			ОтправитьНаОзнакомление(ПараметрыДокумента.Ссылка);
		КонецЕсли;
...
Функция ВыборкаДокументовДляФормированияОтвета(ОбъектыДействий) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектыДействий.ОбъектыУчета) Тогда
		...
		ТекстЗапроса = ИнтеграцияЭДО.ЗапросАктуальныхЭлектронныхДокументов(
			...
			|	И НЕ ЭлектронныйДокументВходящийЭДО.ИспользоватьЦентрЭДО";
...
Функция ЗапросДанныхСообщенийДляПодписания(ПараметрыВыполнения, КонтекстДиагностики)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СертификатыУчетныхЗаписей.Должность,  """") КАК Должность
		|ИЗ
...
Функция ОбработатьРезультатыЗапросаДанныхСообщенийДляПодписания(ПараметрыВыполнения, РезультатыЗапроса, КонтекстДиагностики)
	...
	ОбъектыДействий = ПараметрыВыполнения.ОбъектыДействий;
	...
	Для Каждого ПараметрыСообщенийПоСертификатам Из НаборыПараметровСообщенийПоСертификатам Цикл
		...
		Иначе
			...
			ОписаниеНабораДанных = ОписаниеНабораДанныхДляИнтерактивногоПодписания(
				ПараметрыСообщенийПоСертификатам, ДвоичныеДанныеФайлов, ОбъектыДействий);
...
Функция НайтиСоздатьНаборСертификатовДляПодписания(Выборка, НаборыСертификатов, СертификатыДляПодписания,
	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		...
		ДанныеСертификата.Вставить("Должность", СтрокаТаблицы.Должность);
...
Функция ПараметрыПодписанияСообщения(Выборка)
	...
	Параметры.Вставить("ИдентификаторОрганизации", Выборка.ИдентификаторОрганизации);
	Возврат Параметры;
КонецФункции
...
Функция ДанныеСообщенияДляИнтерактивногоПодписания(ПараметрыПодписания, ДвоичныеДанные)
	...
	ДанныеСообщения.Вставить("РежимСтрогойПроверкиДоверенностейОтключен", Ложь);
	...
	Для Каждого ПараметрыСообщения Из ПараметрыСообщенийПоСертификатам.Значение Цикл
		...
		Если СодержаниеДокумента <> Неопределено Тогда
			...
			Если Формат = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПродавца Тогда
				...
				ИдентификаторыФорматовДляОбновления = Новый Массив;
				ИдентификаторыФорматовДляОбновления.Добавить(ФорматыЭДО_ФНС.ПоддерживаемыеФорматы().УПД_5_03.ИнформацияПродавца);
				ИдентификаторыФорматовДляОбновления.Добавить("ПроизвольныйXML");
				КонвертацияЭДО.ОчиститьДатыОбновленияФорматовДляПреобразованияИПравилПреобразованияФорматов(
					ИдентификаторыФорматовДляОбновления);
		...
		Если ТребуетсяМЧД Тогда
			...
			Результат = МашиночитаемыеДоверенности.ПолучитьПодходящиеДоверенностиОрганизации(
				ОтборМЧД, ЭлектронныйДокумент);
			
			Если СодержаниеДокумента <> Неопределено 
				И Результат.Доверенности.Количество() > 0
				...
				ОписаниеФайла = ФорматыЭДО.ЗаполнитьДанныеДоверенностейДокумента(
					ОписаниеФайла, ПараметрыСообщения.Организация, Результат.Доверенности, СодержаниеДокумента);
		...
		Если СодержаниеДокумента <> Неопределено И НЕ ПараметрыСообщения.ПодписанЭП Тогда
			...
			Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				ПараметрыСообщения.Ссылка, "ЭлектронныйДокумент, ЭлектронныйДокумент.ИдентификаторОрганизации");
			ЭлектронныйДокумент = Реквизиты.ЭлектронныйДокумент;
			ИдентификаторОрганизации = Реквизиты.ЭлектронныйДокументИдентификаторОрганизации;
			...
			Если ФорматыЭДО.ЗаполнениеДанныхПодписантаДоступно(СодержаниеДокумента) Тогда
				ОписаниеФайла = ФорматыЭДО.ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, ПараметрыСообщения.Организация, 
					ДанныеСертификата.Ссылка, СодержаниеДокумента, ИдентификаторОрганизации, ВыбраннаяМЧД);
				ОбновитьДанные = Истина;
			КонецЕсли;
		...
		Если ТребуетсяМЧД Тогда
			...
			ДанныеСообщения.РежимСтрогойПроверкиДоверенностейОтключен = Результат.РежимСтрогойПроверкиДоверенностейОтключен;
		
			Если Результат.Доверенности.Количество() > 0 Тогда
				ДанныеСообщения.Доверенности.Вставить(ДанныеСертификата.Ссылка, Результат.Доверенности);
			Иначе
...
//        *** Должность -Строка
//        *** ИдентификаторОрганизации - Строка
//  * МассовоеПодписаниеДокументов - Булево
//
Функция ОписаниеНабораДанныхДляИнтерактивногоПодписания(ПараметрыСообщенийПоСертификатам, ДвоичныеДанныеФайлов, ОбъектыДействий) Экспорт
	...
	ИспользованиеБазовогоТипаПодписиДоступно = Истина;
	ИспользоватьБазовыйТипПодписи = Ложь;
	ДолжностиПоСертификатам = Новый Соответствие;
	МассовоеПодписаниеДокументов = 
		ОбъектыДействий.ОбъектыУчета.Количество() + ОбъектыДействий.ЭлектронныеДокументы.Количество() > 1;
	...
	Для Каждого ПараметрыСообщения Из ПараметрыСообщенийПоСертификатам.Значение Цикл
		...
		Если СообщениеНеПодписаноИСоответствуетРасширениям Тогда
			...
			Если СодержаниеДокумента <> Неопределено Тогда
				...
				Если ФорматыЭДО.ПодписаниеСИспользованиемБазовогоТипаПодписиДоступно(СодержаниеДокумента) Тогда
					ИспользоватьБазовыйТипПодписи = Истина;
				Иначе
					ИспользованиеБазовогоТипаПодписиДоступно = Ложь;
				КонецЕсли;
		...
		Если СообщениеНеПодписаноИСоответствуетРасширениям = Ложь Или СодержаниеДокумента = Неопределено Тогда
			ИспользованиеБазовогоТипаПодписиДоступно = Ложь;
		КонецЕсли;
		...
		Для Каждого ДанныеСертификата Из ПараметрыСообщенийПоСертификатам.Ключ.ДанныеСертификатов Цикл
			...
			Если ТребуетсяДоверенность Тогда
				...
				СертификатКлючаЭлектроннойПодписи = ДанныеСертификата.Ссылка;
				
				СертификатыТребующиеДоверенность.Вставить(СертификатКлючаЭлектроннойПодписи, Истина);
				
				СвойстваСертификата = СвойстваСертификатов[СертификатКлючаЭлектроннойПодписи];
				...
				Если Результат.Доверенности.Количество() > 0 Тогда
					ДанныеСообщения.Доверенности.Вставить(СертификатКлючаЭлектроннойПодписи, Результат.Доверенности);
				Иначе
					Если Результат.ОшибкиПроверкиПолномочий.Количество() > 0 Тогда
						ДанныеСообщения.ОшибкиПроверкиПолномочий.Вставить(
							СертификатКлючаЭлектроннойПодписи, Результат.ОшибкиПроверкиПолномочий);
					КонецЕсли;
				КонецЕсли;
			
			КонецЕсли;
			
			СведенияОДолжности = Новый Структура("Должность, ИдентификаторОрганизации");
			СведенияОДолжности.Должность = ДанныеСертификата.Должность;
			СведенияОДолжности.ИдентификаторОрганизации = ПараметрыСообщения.ИдентификаторОрганизации;
			ДолжностиПоСертификатам.Вставить(ДанныеСертификата.Ссылка, СведенияОДолжности);
	...
	ОписаниеНабораДанных.Вставить("ИспользоватьБазовыйТипПодписи", ИспользоватьБазовыйТипПодписи
		И ИспользованиеБазовогоТипаПодписиДоступно);
	ОписаниеНабораДанных.Вставить("ДолжностиПоСертификатам", ДолжностиПоСертификатам);
	ОписаниеНабораДанных.Вставить("МассовоеПодписаниеДокументов", МассовоеПодписаниеДокументов);
	Возврат ОписаниеНабораДанных;
...
// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиДокументыНедоступныДляПодписания() Экспорт
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = ЭлектронныеДокументыЭДО.ИдентификаторОшибкиДокументыНедоступныеДляПодписания();
...
Функция ВидОшибкиДокументыНедоступныДляПодписанияНаСервереПоДоверенности(Сертификаты)
	...
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор =
		ЭлектронныеДокументыЭДО.ИдентификаторОшибкиДокументыНедоступныеДляПодписанияНаСервереПоДоверенности();
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Документы недоступны для подписания'");
...
// Возвращаемое значение:
//  См. ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки
Функция ВидОшибкиДокументыВФорматеНеПоддержанномОператором(ИспользоватьОбработчик = Ложь) Экспорт
	
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор =
		ЭлектронныеДокументыЭДО.ИдентификаторОшибкиДокументыВФорматеНеПоддержанномОператором();
...
// Возвращаемое значение:
//  Строка
Функция ИдентификаторОшибкиДокументыВФорматеНеПоддержанномОператором() Экспорт
	
	Возврат "ДокументыВФорматеНеПоддержанномОператором";
...
Функция ЗаполнитьПодписантовИДоверенностиПоСертификату(ДанныеДляЗаполнения,
	...
	Пока Выборка.Следующий() Цикл
		...
		РезультатЗаполненияФайла.Вставить("Ошибка", Неопределено);
		...
		Если СодержаниеДокумента <> Неопределено Тогда
			...
			Если ПараметрыЗаполнения.ТребуетсяЗаполнитьПодписанта
				...
				ОписаниеФайла = ФорматыЭДО.ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла, Выборка.Организация,
					Сертификат, СодержаниеДокумента, ИдентификаторОрганизации, РезультатУточненияМЧД);
				
				ЭтоФайлПДФ_А_3 = Обработки.ФорматДоговорныйДокумент101.ЭтоФайлФормата(Выборка.ПолноеИмяФайла);
				Если ЭтоФайлПДФ_А_3 И Не РезультатЗаполненияФайла.Отказ Тогда
...
Процедура ПослеПодписания(СообщениеОбъект, РезультатПодписания, РезультатДействий)
	...
	Если ПолностьюПодписанТитулОтправителяИлиПолучателя Или ЧастичноПодписанВнутреннийДокумент Тогда
...
Процедура ВыполнитьДействиеПодготовитьКОтправке(ПараметрыВыполнения, РезультатДействий)
	...
	Если ЗначениеЗаполнено(ПараметрыВыполнения.ОбъектыДействий.ЭлектронныеДокументы)
		...
		Если ПараметрыВыполнения.ОбработатьТолькоИзвещения Тогда
			Отбор.ТипЭлементаРегламента = РегламентыЭДО.ТипыЭлементовРегламентаЭДОИзвещений();
		КонецЕсли;
...
Функция ПодготовитьКОтправкеДокумент(Сообщение, КонтекстДиагностики, СообщенияДляОбработки, ОбработанныеСообщения,
	...
	Попытка
		...
		СостоянияСообщений = НовыйСостоянияСообщений();
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатыЗапроса[4].Выгрузить(), СостоянияСообщений);
...
Процедура ВыполнитьДействиеОтправить(ПараметрыВыполнения, РезультатДействий)
	...
	Если РезультатОтправки.ТребуетсяОбработкаНаКлиенте
		И ПараметрыВыполнения.РасшифрованныеМаркеры = Неопределено
		Или ВРезультатеОтправкиЕстьМаркерыДляРасшифровки(РезультатОтправки) Тогда
...
Процедура ОбработатьОтправленноеСообщение(Сообщение, ОшибкаПередачи, КонтекстДиагностики, Отказ, ИтогДействийПоЭДО)
	...
	Попытка
		...
		Если СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОбменЗавершен 
			Или СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту 
			Или СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
			ОтправитьНаОзнакомление(СообщениеОбъект.ЭлектронныйДокумент);
		КонецЕсли;
...
// Параметры:
//  РезультатОтправки - См. ОтправитьСообщения
// 
// Возвращаемое значение:
//  Булево
Функция ВРезультатеОтправкиЕстьМаркерыДляРасшифровки(РезультатОтправки)
	
	Если РезультатОтправки.КонтекстОтправки = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	КлючиСинхронизации = Неопределено;
	Если Не РезультатОтправки.КонтекстОтправки.Свойство("КлючиСинхронизации", КлючиСинхронизации) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого ДанныеКлюча Из КлючиСинхронизации Цикл
		КлючСинхронизации = ДанныеКлюча.Значение;
		
		Если Не ЗначениеЗаполнено(КлючСинхронизации.МаркерРасшифрованный)
			И ЗначениеЗаполнено(КлючСинхронизации.МаркерЗашифрованный) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
//  * ИсключитьДиагностику - Булево
//
Функция ПараметрыПроверкиПодписейНаКлиенте(РезультатЗагрузки) Экспорт
...
Процедура ДополнитьДанныеОбъектаКомментариемКОтклонениюАннулированию(ДанныеОбъекта)
	...
	Если ДанныеОбъекта.ТипЭлементаРегламента <> Перечисления.ТипыЭлементовРегламентаЭДО.УОУ
		...
		И ДанныеОбъекта.ТипЭлементаРегламента <> Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи
		И ДанныеОбъекта.ТипЭлементаРегламента <> Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ
...
Функция ПроверитьПодписиПоОбъектамКонтейнеров(ДанныеОбъектов, МенеджерКриптографии, КонтекстДиагностики) Экспорт
...
Функция ЗагрузитьДанныеОбъектовКонтейнеровПослеПроверкиПодписей(ДанныеОбъектов, ПроверенныеПодписи, 
	...
	ДокументыДляПроверкиПодписейПоМЧД = Новый Массив;
	Для Каждого КлючЗначение Из РезультатЗагрузки.ИтогДействийПоЭДО.ОбработанныеДокументы Цикл
		Если КлючЗначение.Значение = Истина Тогда
			ДокументыДляПроверкиПодписейПоМЧД.Добавить(КлючЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаФайловДокументов();
	Запрос.УстановитьПараметр("ЭлектронныеДокументы", ДокументыДляПроверкиПодписейПоМЧД);
	СообщенияДляПроверки = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	ПроверитьПодписиПоМЧДПослеЗагрузкиДанных(СообщенияДляПроверки, КонтекстДиагностики);
...
Функция ЗагрузитьСообщениеПоДаннымОбъекта(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики, РезультатЗагрузкиДанных)
	...
	Если (Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОбменЗавершен
		ИЛИ Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОбменЗавершенСИсправлением
		ИЛИ Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.Аннулирован
		ИЛИ Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту
		ИЛИ Результат.СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи) Тогда
...
Функция ЗагрузитьСообщениеДокумента(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики)
	...
	ЭтоОтклонениеПОА = ЭтоОтклонениеАннулирования(ОписаниеСообщения, ПараметрыДокумента);
	УстановитьСостояниеХранение = УстанавливатьХранение(ОписаниеСообщения, ПараметрыЗагрузки, ЭтоОтклонениеПОА)
	...
	ИначеЕсли ЭтоОтклонениеПОА Тогда
...
// Возвращаемое значение:
//   Булево
Функция ЭтоОтклонениеАннулирования(ОписаниеСообщения, ПараметрыДокумента)
	
	ЭтоОтклонениеПОА = Ложь;
	Если ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА_УОУ Тогда
		ЭтоОтклонениеПОА = Истина;
	
	ИначеЕсли ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи Тогда
		
		Если ЗначениеЗаполнено(ПараметрыДокумента.СвойстваДокументовПоФорматам) 
			И ЗначениеЗаполнено(ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение) Тогда
			ИмяФайлаНазначения = ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение.ИмяФайлаНазначения;
			ЭтоОтклонениеПОА = СтрНайти(ИмяФайлаНазначения, "DP_PRANNUL") <> 0;
		КонецЕсли;
	КонецЕсли;
	
	Возврат ЭтоОтклонениеПОА;
КонецФункции

// Параметры:
//   ОписаниеСообщения - См. НовоеОписаниеСообщения
//   ПараметрыЗагрузки - См. ПараметрыЗагрузкиСообщения
//   ЭтоОтклонениеПОА - Булево
//
// Возвращаемое значение:
//   Булево
Функция УстанавливатьХранение(ОписаниеСообщения, ПараметрыЗагрузки, ЭтоОтклонениеПОА)
	Если ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ПОА Тогда
		Возврат Истина;
	КонецЕсли;
	
	ЕстьАктивноеАннулирование = РегламентыЭДО.ЕстьАктивноеАннулирование(ПараметрыЗагрузки.СостоянияСообщений);
	Если ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УОУ Тогда
		Возврат Не ЕстьАктивноеАннулирование;
	КонецЕсли;
	
	Если РегламентыЭДО.ВыполненПереходНаУниверсальныеСообщения() 
		И (ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_УОУ 
		Или ОписаниеСообщения.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.УС_ОтказВПодписи И Не ЭтоОтклонениеПОА) Тогда
...
// 	 * Статус - ПеречислениеСсылка.СтатусыСообщенийЭДО
// 	 * Направление - ПеречислениеСсылка.НаправленияЭДО
Функция НовыйСостоянияСообщений() Экспорт
	...
	ТаблицаЗначений.Колонки.Добавить("Состояние", Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияСообщенийЭДО"));
...
Функция ПараметрыЗагрузкиСообщения(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики) Экспорт
	...
	Если ЭтоДубль
		...
		Или РегламентыЭДО.ЭтоУниверсальноеСообщение(ДанныеОбъекта.ТипЭлементаРегламента))
	...
	Иначе
		...
		Если Результат.ЭтоНовыйДокумент Тогда
			ОбработатьЗагрузкуСообщенияБезДокумента(ДанныеОбъекта, КонтекстДиагностики);
			Возврат Результат;
		КонецЕсли;
		...
		Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ДанныеОбъекта.ТипЭлементаРегламента) Тогда
			
			СвойстваУниверсальногоСообщенияПоФайлу = Неопределено;
			Если ЗначениеЗаполнено(ДанныеОбъекта.ОписаниеДанных) 
				И ДанныеОбъекта.ОписаниеДанных.ДвоичныеДанные <> Неопределено Тогда
					
				СвойстваУниверсальногоСообщенияПоФайлу = Документы.СообщениеЭДО.СвойстваУниверсальногоСообщенияПоФайлу(ДанныеОбъекта.ОписаниеДанных.ДвоичныеДанные);

			КонецЕсли;
			
			Если ЗначениеЗаполнено(СвойстваУниверсальногоСообщенияПоФайлу) Тогда
				ЗаполнитьЗначенияСвойств(ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение, СвойстваУниверсальногоСообщенияПоФайлу);
			ИначеЕсли ЗначениеЗаполнено(ДанныеОбъекта.Карточка.ФайлНазначения) Тогда
				
				ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение.ИмяФайлаНазначения = ДанныеОбъекта.Карточка.ФайлНазначения;
			КонецЕсли;
			
			Если ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение.Статус = "4002" Тогда
				ОписаниеСообщения.ДополнительнаяИнформация = ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение.ТекстСообщения;
			КонецЕсли;
			
			СообщенияДокумента = Результат.СостоянияСообщений.ВыгрузитьКолонку("Ссылка");
			ОписаниеСообщения.РодительскоеСообщение = 
				СообщениеПоИмениФайла(СообщенияДокумента, 
					ПараметрыДокумента.СвойстваДокументовПоФорматам.УниверсальноеСообщение.ИмяФайлаНазначения,
					Ложь);
	...
	ЗаполнитьРеквизитыДокументаПоНаименованиюФайла(ДанныеОбъекта, Результат);
...
//  ДанныеОбъекта - СтрокаТаблицыЗначений: см. СинхронизацияЭДО.НовыеДанныеОбъектов
//  КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ОбработатьЗагрузкуСообщенияБезДокумента(ДанныеОбъекта, КонтекстДиагностики)
	
	СтандартнаяОбработка = Истина;
	
	Если ОбщегоНазначенияБЭД.ЭтоРежимЦентрЭДО() Тогда
		МодульТранспортныеКонтейнерыЭДОВЦентреЭДО = ОбщегоНазначения.ОбщийМодуль("ТранспортныеКонтейнерыЭДОЦентрЭДО");
		МодульТранспортныеКонтейнерыЭДОВЦентреЭДО.ПриЗагрузкеСообщенияБезДокумента(
			ДанныеОбъекта, КонтекстДиагностики, СтандартнаяОбработка);
	КонецЕсли;
...
Функция СоздатьИзвещениеОПолучении(Основание, ПараметрыЗагрузки, КонтекстДиагностики, КомментарийСостояния = "")
	...
	СвойстваОсновногоФайла.Владелец = Основание.Ссылка;
	
	ТипДокумента = Перечисления.ТипыДокументовЭДО.ИзвещениеОПолучении;
	Если РегламентыЭДО.ЭтоУниверсальноеСообщение(ПараметрыЗагрузки.ТипИзвещения) Тогда
		ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение;
	КонецЕсли;
	
	ИзвещениеОбъект = СоздатьСлужебноеСообщение(ПараметрыЗагрузки.ПараметрыДокумента, СвойстваОсновногоФайла,
		ПараметрыЗагрузки.ДанныеУчастниковЭДО, ТипДокумента, ПараметрыЗагрузки.ТипИзвещения);
	...
	ДополненияСостоянийЭДО = Неопределено;
	Если Не ПараметрыЗагрузки.ПараметрыДокумента.ОбменБезПодписи 
		И Не РегламентыЭДО.ЭтоУниверсальноеСообщение(ИзвещениеОбъект.ТипЭлементаРегламента) Тогда
...
Функция ЕстьСообщенияПоИмениФайла(СообщенияДокумента, ИмяФайла)
	...
	СообщениеПоИмениФайла = СообщениеПоИмениФайла(СообщенияДокумента, ИмяФайла);
	
	Возврат ЗначениеЗаполнено(СообщениеПоИмениФайла);
...
//  СообщенияДокумента - Массив из ДокументСсылка.СообщениеЭДО
//  ИмяФайла - Строка
//  СтрогоеСоответствие - Булево
// 
// Возвращаемое значение:
//  ДокументСсылка.СообщениеЭДО
Функция СообщениеПоИмениФайла(СообщенияДокумента, ИмяФайла, СтрогоеСоответствие = Истина)
	
	Результат = Документы.СообщениеЭДО.ПустаяСсылка();
...
#Область РаспознаваниеРеквизитовВНаименованииФайла

// Параметры:
//  ДанныеОбъекта - См. ПараметрыЗагрузкиСообщения.ДанныеОбъекта
//  Результат - См. ПараметрыЗагрузкиСообщения 
Процедура ЗаполнитьРеквизитыДокументаПоНаименованиюФайла(ДанныеОбъекта, Результат)
	
	Если Не Результат.ЭтоВходящийЭДО Или Не Результат.ЭтоНовыйДокумент
		Или ДанныеОбъекта.ТипРегламента <> Перечисления.ТипыРегламентовЭДО.Неформализованный Тогда
		Возврат;
	КонецЕсли;
	
	Если Не НастройкиЭДО.ИспользуетсяЗаполнениеРеквизитовВходящегоНеформализованногоДокументаПоНаименованиюФайла() Тогда
		Возврат;
	КонецЕсли;

	РеквизитыДокументаПоНаименованиюФайла = РеквизитыДокументаИзНаименованияФайла(
		ДанныеОбъекта.ОписаниеДанных.ИмяФайла,
		ДанныеОбъекта.ТипДокумента = Перечисления.ТипыДокументовЭДО.Прочее,
		Не ЗначениеЗаполнено(Результат.ОписаниеСообщения.Данные.Содержание.НомерДокумента));
		
	Если ЗначениеЗаполнено(РеквизитыДокументаПоНаименованиюФайла.ВидДокумента) Тогда
		Результат.ПараметрыДокумента.ВидДокумента = РеквизитыДокументаПоНаименованиюФайла.ВидДокумента;
		Результат.ОписаниеСообщения.ВидСообщения = РеквизитыДокументаПоНаименованиюФайла.ВидДокумента;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РеквизитыДокументаПоНаименованиюФайла.НомерДокумента) Тогда
		Результат.ОписаниеСообщения.Данные.Содержание.НомерДокумента =
			РеквизитыДокументаПоНаименованиюФайла.НомерДокумента;
	КонецЕсли;
		
	Если ЗначениеЗаполнено(РеквизитыДокументаПоНаименованиюФайла.ДатаДокумента) Тогда
		Результат.ОписаниеСообщения.Данные.Содержание.ДатаДокумента =
			РеквизитыДокументаПоНаименованиюФайла.ДатаДокумента;
	КонецЕсли;
...
// Параметры:
//  ИмяФайла - Строка
//  ОпределятьВид - Булево
//  ОпределятьНомер - Булево
//  ОпределятьДату - Булево
// 
// Возвращаемое значение:
//  Структура:
...
//                  - Дата
Функция РеквизитыДокументаИзНаименованияФайла(ИмяФайла, ОпределятьВид = Истина, ОпределятьНомер = Истина,
	ОпределятьДату = Истина)
	...
	Если ОпределятьВид Тогда
		Результат.ВидДокумента = ВидДокументаИзНаименованияФайла(ИмяФайла);
	КонецЕсли;
	Если ОпределятьНомер Тогда
		Результат.НомерДокумента = НомерДокументаИзНаименованияФайла(ИмяФайла);
	КонецЕсли;
	Если ОпределятьДату Тогда
		Результат.ДатаДокумента = ДатаДокументаИзНаименованияФайла(ИмяФайла);
	КонецЕсли;
...
// - Неопределено
// - СправочникСсылка.ВидыДокументовЭДО
Функция ВидДокументаИзНаименованияФайла(ИмяФайла)
	
	Результат = Неопределено;
	
	ТаблицаРегулярныхВыражений = ТаблицаРегулярныхВыраженийДляОпределенияТипаДокумента();
	
	Для Каждого СтрокаТаблицы Из ТаблицаРегулярныхВыражений Цикл
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ИмяФайла, СтрокаТаблицы.РегулярноеВыражение,,,, Истина);
...
//
Функция ТаблицаРегулярныхВыраженийДляОпределенияТипаДокумента() 
	
	ТаблицаРегулярныхВыражений = Новый ТаблицаЗначений;
	ТаблицаРегулярныхВыражений.Колонки.Добавить("ТипДокумента",
		Новый ОписаниеТипов("ПеречислениеСсылка.ТипыДокументовЭДО"));
	ТаблицаРегулярныхВыражений.Колонки.Добавить("РегулярноеВыражение", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВзаимозачета;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+взаимозачета|акт[\s_]+взаимо(?:зачета)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВзаимозачета;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+vzaimozacheta|akt[\s_]+vzaimo(?:zacheta)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+выполн(?:ен(?:ных)?)?[\s_]+работ";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+vypolnen(?:nyh|nih)?[\s_]+rabot|akt[\s_]+vypolnennyh[\s_]+rabot";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+об?[\s_]+оказании?[\s_]+услуг";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+ob?[\s_]+okazanii?[\s_]+uslug";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+(?:приемки-сдачи|приемки|сдачи)[-\s_]+(?:приемки|сдачи)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктВыполненныхРабот;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+(?:priemki-sdachi|priemki|sdachi)[-\s_]+(?:priemki|sdachi)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+на[\s_]+передачу[\s_]+прав";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктНаПередачуПрав;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+na[\s_]+peredachu[\s_]+prav";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+о[\s_]+расхождениях";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктОРасхождениях;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+o[\s_]+raskhozhdeniyah";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктСверки;
	НоваяСтрока.РегулярноеВыражение = "акт[\s_]+сверки|акт[\s_]+сверки[\s_]+взаиморасчетов";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктСверки;
	НоваяСтрока.РегулярноеВыражение = "a(k|c)t[\s_]+sverki|a(k|c)t[\s_]+sverki[\s_]+vzaimoraschetov";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктСверки;
	НоваяСтрока.РегулярноеВыражение = "сверка";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.АктСверки;
	НоваяСтрока.РегулярноеВыражение = "sverka";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Ведомость;
	НоваяСтрока.РегулярноеВыражение = "ведомость";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Ведомость;
	НоваяСтрока.РегулярноеВыражение = "vedomost";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ГарантийноеПисьмо;
	НоваяСтрока.РегулярноеВыражение = "гарантийное[\s_]+письмо";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ГарантийноеПисьмо;
	НоваяСтрока.РегулярноеВыражение = "garanti(j|y)noe[\s_]+pismo";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДополнительноеСоглашение;
	НоваяСтрока.РегулярноеВыражение = "дополнительное[\s_]+соглашение|доп\.?[\s_]*соглашение|дс\b";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДополнительноеСоглашение;
	НоваяСтрока.РегулярноеВыражение = "dopolnitelnoe[\s_]+soglashenie|dop\.?[\s_]*soglashenie|ds\b";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент;
	НоваяСтрока.РегулярноеВыражение = "приложение[\s_]+к[\s_]+договору";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ДоговорныйДокумент;
	НоваяСтрока.РегулярноеВыражение = "prilozhenie[\s_]+k[\s_]+dogovoru";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Договор;
	НоваяСтрока.РегулярноеВыражение = "договор";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Договор;
	НоваяСтрока.РегулярноеВыражение = "dogovor";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС11;
	НоваяСтрока.РегулярноеВыражение = "кс[-\s_]?11";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС11;
	НоваяСтрока.РегулярноеВыражение = "ks[-\s_]?11";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС2;
	НоваяСтрока.РегулярноеВыражение = "кс[-\s_]?2";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС2;
	НоваяСтрока.РегулярноеВыражение = "ks[-\s_]?2";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС3;
	НоваяСтрока.РегулярноеВыражение = "кс[-\s_]?3";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.КС3;
	НоваяСтрока.РегулярноеВыражение = "ks[-\s_]?3";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность;
	НоваяСтрока.РегулярноеВыражение = "машиночитаемая[\s_]+доверенность";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность;
	НоваяСтрока.РегулярноеВыражение = "mashinochitaemaya[\s_]+doverennost";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Отчет;
	НоваяСтрока.РегулярноеВыражение = "отч[её]т";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Отчет;
	НоваяСтрока.РегулярноеВыражение = "otchet";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ПлатежноеПоручение;
	НоваяСтрока.РегулярноеВыражение = "платежное[\s_]+поручение";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ПлатежноеПоручение;
	НоваяСтрока.РегулярноеВыражение = "platezhnoe[\s_]+poruchenie";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ПриложениеКАкту;
	НоваяСтрока.РегулярноеВыражение = "приложение[\s_]+к[\s_]+акту";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ПриложениеКАкту;
	НоваяСтрока.РегулярноеВыражение = "prilozhenie[\s_]+k[\s_]+aktu";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СоглашениеОбЭДО;
	НоваяСтрока.РегулярноеВыражение = "соглашение[\s_]+об[\s_]+эдо";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СоглашениеОбЭДО;
	НоваяСтрока.РегулярноеВыражение = "soglashenie[\s_]+ob[\s_]+edo";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Спецификация;
	НоваяСтрока.РегулярноеВыражение = "спецификация";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Спецификация;
	НоваяСтрока.РегулярноеВыражение = "specifikaciya";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату;
	НоваяСтрока.РегулярноеВыражение = "сч[ёе]т(?![-\s]*фактура)[\s_]*(?:на[\s_]+оплату)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.СчетНаОплату;
	НоваяСтрока.РегулярноеВыражение = "schet(?![-\s]*faktura)[\s_]*(?:na[\s_]+oplatu)?";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная;
	НоваяСтрока.РегулярноеВыражение = "товарная[\s_]+накладная|тн\b";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.ТоварнаяНакладная;
	НоваяСтрока.РегулярноеВыражение = "tovarnaya[\s_]+nakladnaya|tn\b";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Уведомление;
	НоваяСтрока.РегулярноеВыражение = "уведомление";
	
	НоваяСтрока = ТаблицаРегулярныхВыражений.Добавить();
	НоваяСтрока.ТипДокумента = Перечисления.ТипыДокументовЭДО.Уведомление;
	НоваяСтрока.РегулярноеВыражение = "uvedomlenie";
	
	Возврат ТаблицаРегулярныхВыражений;
...
// - Неопределено
// - Строка
Функция НомерДокументаИзНаименованияФайла(ИмяФайла)
	
	Результат = Неопределено;
	
	РегулярныеВыраженияДляПоискаНомераДокумента = Новый Массив;
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("№\s*(\d+-\d+-\d+)"); // № 1234-12345-12345
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("№\s*(\d{1,4}-\d{5,})"); // № 4-1234567
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("№\s*(\d{5,})"); // № 123456789123
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("№\s*([\w-]+?)(?=_)"); // № П-28149, № 40
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("N[o|°]?\s*([\w-]+)"); // No 1398, N° 1398
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("#\s*([\w-]+)"); // # 1398
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("_(\d+)"); // _12345
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("\b([A-ZА-Я]{1,4}-\d{2,})\b"); // АСУН-000422, МСК-00003344063
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("\b(\d{4}-\d{6}-\d{5})\b"); // 2504-429460-35575
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("\b(\d{1,4})\b"); // 1, 12, 123, 1234
	РегулярныеВыраженияДляПоискаНомераДокумента.Добавить("\b(\d{5,})\b"); // Длинные цифровые номера

	Для Каждого РегулярноеВыражение Из РегулярныеВыраженияДляПоискаНомераДокумента Цикл
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ИмяФайла, РегулярноеВыражение);
		Если РезультатПоиска.НачальнаяПозиция = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Результат = СокрЛП(СтрЗаменитьПоРегулярномуВыражению(РезультатПоиска.Значение, "[№#]\s*|[Nn][o°]|_\s*", ""));
		Прервать;
	КонецЦикла;
...
// - Неопределено
// - Дата
Функция ДатаДокументаИзНаименованияФайла(ИмяФайла)
	
	Результат = Неопределено;
	
	РегулярныеВыраженияДляПоискаДатыДокумента = Новый Массив;
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{2}\.\d{2}\.\d{4}"); // DD.MM.YYYY
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{2}\.\d{2}\.\d{2}"); // DD.MM.YY
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{2}\/\d{2}\/\d{4}"); // DD/MM/YYYY
	РегулярныеВыраженияДляПоискаДатыДокумента.Добавить("\d{1,2}[\s_]+\w+[\s_]+\d{4}"); // DD MM YYYY 22 апреля 2025, 22_апреля_2025

	Для Каждого РегулярноеВыражение Из РегулярныеВыраженияДляПоискаДатыДокумента Цикл
		РезультатПоиска = СтрНайтиПоРегулярномуВыражению(ИмяФайла, РегулярноеВыражение,,,, Истина);
		Если РезультатПоиска.НачальнаяПозиция = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Результат = ОбщегоНазначенияБЭД.ДатаПоСтроковомуПредставлению(РезультатПоиска.Значение);
		Прервать;
	КонецЦикла;
...
#КонецОбласти // РаспознаваниеРеквизитовВНаименованииФайла
...
Процедура ДобавитьОшибкуПодписиМЧД(ЭлектронныйДокумент, ПодписиСОшибкамиМЧД, Действие, КонтекстДиагностики)
	...
	Если МашиночитаемыеДоверенности.РежимСтрогойПроверкиДоверенностейОтключен() Тогда
		Возврат;
	КонецЕсли;
		...
		Если НРег(ДанныеФайла.Расширение) = "xml" Тогда
			...
			Если Выборка.ТипЭлементаРегламента = Перечисления.ТипыЭлементовРегламентаЭДО.ИнформацияОтправителя
				...
				ИнтеграцияЭДО.ЗаполнитьНоменклатуруИБВДеревеДокумента(Контрагент, ДанныеСообщения.НовыйЭД,
					ЭлектронныйДокумент, Истина, Отказ);
			КонецЕсли;
...
Процедура ОбновитьСвязанныеДокументы(ТипДокумента, ИдентификаторыОснований, Действие, КонтекстДиагностики, ЭтоВходящийЭДО = Ложь) Экспорт
	...
	Попытка
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			СостоянияЭлементовРегламента = НовыйСостоянияСообщений();
...
Функция НовыеСвойстваДокументовДляОписанияПакетов()
	...
	СвойстваДокументов.Колонки.Добавить("ИспользоватьЦентрЭДО", Новый ОписаниеТипов("Булево"));
...
Функция ДанныеНоменклатурыДокументов(ЭлектронныеДокументы, КонтекстДиагностики) Экспорт
	...
	Для Каждого ПараметрыПолученияДокумента Из ПараметрыПолученияДокументов Цикл
		...
		Если НЕ ЗначениеЗаполнено(ИнформацияОТоваре) Тогда
			Продолжить;
		КонецЕсли;
		НайденнаяИнформация = Результат[ЭлектронныйДокумент];
		Если НайденнаяИнформация = Неопределено Тогда
			Результат.Вставить(ЭлектронныйДокумент, ИнформацияОТоваре);
			Продолжить;
		КонецЕсли;
		Для Каждого СтрокаТаблицы Из ИнформацияОТоваре Цикл
			НоваяСтрока = НайденнаяИнформация.Добавить();
...
Функция ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы) Экспорт
	...
	Запрос.Текст =
		...
		|	ЭлектронныеДокументы.СпособОбмена
...
// у которых нельзя получить данные из файла автоматичесики.
// Изменение реквизитов входящих формализованных документов недопустимо.
// 
// Параметры:
// ЭлектронныйДокумент - ДокументСсылка.ЭлектронныйДокументВходящийЭДО
// ДанныеДляИзменения - См. НовыеДанныеВходящегоДокументаДляИзменения
// 
// Возвращаемое значение:
//  Структура:
//  * Успех - Булево
//  * Ошибка - Строка
// 
Функция ИзменитьДанныеВходящегоДокумента(ЭлектронныйДокумент, ДанныеДляИзменения) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Успех", Ложь);
	Результат.Вставить("Ошибка", "");
	
	Если ДанныеДляИзменения.ВидДокумента = Справочники.ВидыДокументовЭДО.ПустаяСсылка() Тогда
		Результат.Ошибка = НСтр("ru = 'Не указан вид документа.'");
		Возврат Результат;
	КонецЕсли;
	...
	Попытка
		...
		ЗаблокироватьДанныеДокументаДляИзменения(ЭлектронныйДокумент, Истина);
		
		ДокументОбъект = ЭлектронныйДокумент.ПолучитьОбъект();
			
		Если ДанныеДляИзменения.ВидДокумента <> Неопределено
			И ДанныеДляИзменения.ВидДокумента <> ДокументОбъект.ВидДокумента Тогда
			ИзмененВидДокумента = Истина;
			ТребуетсяЗапись = Истина;
			ДокументОбъект.ВидДокумента = ДанныеДляИзменения.ВидДокумента;
		КонецЕсли;
		
		Если ДанныеДляИзменения.НомерДокумента <> Неопределено
			И ДанныеДляИзменения.НомерДокумента <> ДокументОбъект.НомерДокумента Тогда
			ТребуетсяЗапись = Истина;
			ДокументОбъект.НомерДокумента = ДанныеДляИзменения.НомерДокумента;
		КонецЕсли;
		
		Если ДанныеДляИзменения.ДатаДокумента <> Неопределено
			И ДанныеДляИзменения.ДатаДокумента <> ДокументОбъект.ДатаДокумента Тогда
			ТребуетсяЗапись = Истина;
			ДокументОбъект.ДатаДокумента = ДанныеДляИзменения.ДатаДокумента;
		КонецЕсли;
		
		Если ДанныеДляИзменения.СуммаДокумента <> Неопределено
			И ДанныеДляИзменения.СуммаДокумента <> ДокументОбъект.СуммаДокумента Тогда
			ТребуетсяЗапись = Истина;
			ДокументОбъект.СуммаДокумента = ДанныеДляИзменения.СуммаДокумента;
		КонецЕсли;
	...
	Если ИзмененВидДокумента Тогда
		ЭлектронныеДокументыЭДО.ПриИзмененииВидаДокумента(ЭлектронныйДокумент, ДанныеДляИзменения.ВидДокумента, Ложь);
	КонецЕсли;
...
//
Функция НовыеДанныеВходящегоДокументаДляИзменения() Экспорт
...
Функция ДоступныеДействияПоЭДО(СостояниеДокумента, СвойстваДокумента) Экспорт
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодписание Тогда
		...
		Если ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(СвойстваДокумента.Ссылка) Тогда
			...
			Действия.Вставить(Перечисления.ДействияПоЭДО.ЗапроситьУточнение, Истина);
		КонецЕсли;
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяПодготовкаКОтправкеОтказаВПодписи Тогда
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ТребуетсяОтправкаОтказаВПодписи Тогда
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.ОтказаноВПодписиКонтрагенту Тогда
	...
	ИначеЕсли СостояниеДокумента = Перечисления.СостоянияДокументовЭДО.КонтрагентОтказалВПодписи Тогда
...
Функция ДействиеПодписатьДоступно(СвойстваДокумента)
	...
	Если ЭлектронныеДокументыЭДО.ЭтоВходящийЭДО(СвойстваДокумента.Ссылка)
		Или СвойстваДокумента.ИспользоватьЦентрЭДО Тогда
		Возврат Истина;
	КонецЕсли;
...
//  * РодительскоеСообщение - ДокументСсылка.СообщениеЭДО
Функция НовоеОписаниеСообщения() Экспорт
	...
	ОписаниеСообщения.Вставить("РодительскоеСообщение", Документы.СообщениеЭДО.ПустаяСсылка());
...
Функция ОписаниеСлужебногоСообщения(ДанныеУчастниковЭДО, СвойстваФайлаОснования, ТипДокумента, ТипЭлементаРегламента, Комментарий = "")
	...
	ДанныеДляФормирования.ТипЭлементаРегламента = ТипЭлементаРегламента;
	...
	ДанныеДляФормирования.Основание.Сообщение = СвойстваФайлаОснования.Владелец;
	...
	ОписаниеСообщения.Направление = Перечисления.НаправленияЭДО.Исходящий;
	ОписаниеСообщения.Данные = ДанныеСообщения;
	ОписаниеСообщения.РодительскоеСообщение = СвойстваФайлаОснования.Владелец;
...
Функция СвойстваСообщения(Сообщение) Экспорт
	...
	Результат.ИспользоватьЦентрЭДО = ЗначенияРеквизитов.ЭлектронныйДокументИспользоватьЦентрЭДО;
...
Функция СвойстваСообщений(Сообщения)
	...
	Запрос.Текст = 
		...
		|	СообщениеЭДО.ОсновнойФайл,
		|	СообщениеЭДО.ОсновнойФайл.ПолноеИмяФайла КАК ПолноеИмяФайла,
		|	СообщениеЭДО.ЭлектронныйДокумент.ИспользоватьЦентрЭДО КАК ИспользоватьЦентрЭДО
		|ИЗ
		|	Документ.СообщениеЭДО КАК СообщениеЭДО
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументИсходящийЭДО КАК ЭлектронныйДокументИсходящийЭДО
		|		ПО СообщениеЭДО.ЭлектронныйДокумент = ЭлектронныйДокументИсходящийЭДО.Ссылка
		...
		|	СообщениеЭДО.ЭлектронныйДокумент.ИспользоватьЦентрЭДО
		|ИЗ
		|	Документ.СообщениеЭДО КАК СообщениеЭДО
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭлектронныйДокументВходящийЭДО КАК ЭлектронныйДокументВходящийЭДО
		|		ПО СообщениеЭДО.ЭлектронныйДокумент = ЭлектронныйДокументВходящийЭДО.Ссылка
...
//  * ПолноеИмяФайла - Строка
//  * ТипЭлементаРегламента - ПеречислениеСсылка.ТипыЭлементовРегламентаЭДО
Функция ТипыРегламентаИФайлыСообщений(Сообщения) Экспорт
	...
	Запрос.Текст =
		...
		|	СообщениеЭДОПрисоединенныеФайлы.ВладелецФайла.ТипЭлементаРегламента КАК ТипЭлементаРегламента
		|ИЗ
		|	Справочник.СообщениеЭДОПрисоединенныеФайлы КАК СообщениеЭДОПрисоединенныеФайлы
		|ГДЕ
		|	СообщениеЭДОПрисоединенныеФайлы.ВладелецФайла В (&Сообщения)";
...
// 
// Параметры: 
//  ФайлыСообщенийДляПроверкиПодписейПоМЧД - Массив Из СправочникСсылка.СообщениеЭДОПрисоединенныеФайлы
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ПроверитьПодписиПоМЧДПослеЗагрузкиДанных(ФайлыСообщенийДляПроверкиПодписейПоМЧД, КонтекстДиагностики) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	СведенияОПодписяхДляПроверки = МашиночитаемыеДоверенности.СведенияОПодписяхДляПроверки(ФайлыСообщенийДляПроверкиПодписейПоМЧД); 
	
	ПроверитьПодписиФайловПоМЧДПослеЗагрузкиДанных(СведенияОПодписяхДляПроверки, КонтекстДиагностики);
	
	ЗапуститьПолнуюПроверкуПодписейПоМЧД();
	
КонецПроцедуры

// Запускает фоновую проверку всех подписей по МЧД где проверка не выполнена
Процедура ЗапуститьПолнуюПроверкуПодписейПоМЧД()
	
	КлючЗадания = КлючЗаданияФоновойПроверкиПодписейПоМЧД();
	...
	Попытка
		ФоновыеЗадания.Выполнить("ЭлектронныеДокументыЭДОСлужебный.ПроверитьНепроверенныеПодписиПоМЧДВФоне",,КлючЗадания);
	Исключение
		Подсистема = ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().ОбменСКонтрагентами;
		ТекстСообщения = НСтр("ru = 'Не удалось запустить полную проверку подписей по МЧД по причине:'")
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
// Проверяет все не проверенные подписи по МЧД 
// 
// Параметры: 
//  КонтекстДиагностики - Неопределено, См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ПроверитьНепроверенныеПодписиПоМЧДВФоне(КонтекстДиагностики = Неопределено) Экспорт
...
Процедура ПроверитьПодписиФайловПоМЧДПослеЗагрузкиДанных(СведенияОПодписяхПоМЧД, КонтекстДиагностики)
	...
	Для Каждого СведенияОПодписиПоМЧД Из СведенияОПодписяхПоМЧД Цикл
		...
		Исключение     
			ВидОперации = НСтр("ru = 'Проверка подписей по МЧД'");
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось проверить подпись документа по МЧД. %1'"), ФайлСообщения);			
			ПодробныйТекстОшибки = ТекстОшибки + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Ошибка = ОбработкаНеисправностейБЭД.НоваяОшибка(ВидОперации,
				ОбработкаНеисправностейБЭДКлиентСервер.ВидОшибкиНеизвестнаяОшибка(),
				ПодробныйТекстОшибки, ТекстОшибки);
...
Функция СведенияОПодписанныхОбъектах(ФайлыСообщений) Экспорт
	...
	|	Ссылка,
	//ТК + 
	|	ВЫРАЗИТЬ(СвойстваФайла.ВладелецФайла КАК Документ.СообщениеЭДО) КАК СообщениеЭДО,
	//ТК - 
	|	ПолноеИмяФайла
	...
	//ТК + 
	|	И СвойстваФайла.ВладелецФайла ССЫЛКА Документ.СообщениеЭДО
	//ТК - 
	|;
	|
	|ВЫБРАТЬ
...
// Возвращаемое значение:
//  Строка
Функция КлючЗаданияФоновойПроверкиПодписейПоМЧД() Экспорт
	Возврат "ФоноваяПроверкаПодписейПоМЧД";
КонецФункции
