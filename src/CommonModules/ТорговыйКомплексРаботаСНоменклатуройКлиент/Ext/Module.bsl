...
//
Процедура ПоискНоменклатурыПоШтрихкодуВСервисе(Оповещение, Штрихкод) Экспорт
	
	Если РаботаСНоменклатуройКлиент.ДоступнаФункциональностьПодсистемы() Тогда
		
		Если Оповещение.ДополнительныеПараметры.ОповещениеОРезультате.ДополнительныеПараметры <> Неопределено
			И Оповещение.ДополнительныеПараметры.ОповещениеОРезультате.ДополнительныеПараметры.Свойство("ДинамическийСписок") Тогда
			
			// Вызов происходит из формы списка.
			
			РаботаСНоменклатуройКлиент.НайтиНоменклатуруПоШтрихкодуВСервисе(Штрихкод, Неопределено, Оповещение);
		Иначе
			
			// Вызов происходит из документа.
			
			ПараметрыФормы = РаботаСНоменклатуройКлиент.ПараметрыФормыПодбораНоменклатурыПоШтрихкодам();
			ПараметрыФормы.НеизвестныеШтрихкоды.Добавить(Новый Структура("Штрихкод, Количество", Штрихкод, 1));
			
			РаботаСНоменклатуройКлиент.ОткрытьФормуПодбораНоменклатурыПоШтрихкодам(ПараметрыФормы, Неопределено, Оповещение);
		КонецЕсли;
...
Процедура СоздатьВидНоменклатурыИнтерактивно(ПараметрыФормы, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	ОткрытьФорму("Справочник.ТипыНоменклатуры.Форма.ФормаЭлемента", 
