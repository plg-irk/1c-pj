...
Функция ОбработатьРезультатНаивногоПрогнозирования(Параметры, СтруктураПолей, МенеджерДанныхРегистра) Экспорт
	...
	Для каждого стр Из Результат Цикл
			Если стр.Цена=0 Тогда
				...
				ВывестиСообщение(НСтр("ru = 'Для позиции: <""%1""> не установлена цена. Цена рассчитана как средняя цена за период'"),,,,, СокрЛП(стр.Представление));
			КонецЕсли;
...
Функция ТрендЛогарифмический(МассивРяд,КоличествоПрошлыхПериодов, ТочекПрогноза = 1) 
	...
	Для Сч=0 По МассивРяд.ВГраница() Цикл
		Х=Сч; У=МассивРяд[Сч];
		Если У=0 Тогда 
			ВывестиСообщение(НСтр("ru = 'При построении логарифмического тренда обнаружены точки с значениями = 0. Тренд не построен.'"));
			Возврат "ТрендНеПостроен"; 
		КонецЕсли; // при экспоненциальной зависимости нолей быть не должно
		Если У<0 Тогда 
			ВывестиСообщение(НСтр("ru = 'При построении логарифмического тренда обнаружены точки с отрицательными значениями 0. Тренд не построен.'"));
			Возврат "ТрендНеПостроен"; 
		КонецЕсли; 
	...
	Иначе
		...
		Для Сч=1 По ДеревоЗначений.Строки.Количество()-1 Цикл
			...
			Если НовСтрока.Цена = 0 Тогда
				ВывестиСообщение(НСтр("ru = 'Для позиции: <""%1""> не установлена цена. Цена рассчитана как средняя цена за период'"),,,,,  ТекСтрока.Представление);
