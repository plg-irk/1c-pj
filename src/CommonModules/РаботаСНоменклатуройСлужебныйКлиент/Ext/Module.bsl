...
//
// Параметры:
//   СтрокаПоискаКатегорий   - Строка - строка поиска.
//   УникальныйИдентификатор - УникальныйИдентификатор - уникальный идентификатор формы.
//   ИдентификаторЗадания    - УникальныйИдентификатор - идентификатор задания.
//   ПоискПоТНВЭД            - Булево - Истина, если выполнять поиск по классификатору ТНВЭД.
//
// Возвращаемое значение:
//   См. ДлительныеОперации.ВыполнитьВФоне.
//
Функция ПоискКатегорийПоСтроке(Знач СтрокаПоискаКатегорий, Знач УникальныйИдентификатор, ИдентификаторЗадания, 
			Знач ПоискПоТНВЭД = Ложь) Экспорт
	...
	ПараметрыПоиска.ПоискПоТНВЭД      = ПоискПоТНВЭД;
...
Процедура ОбновитьЗаголовкиКнопокСостояние(Форма) Экспорт 
	...
	ПодписыватьЭлектроннойПодписью = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, 
											"ПодписыватьЭлектроннойПодписью")
										И Форма.ПодписыватьЭлектроннойПодписью;
	...
	Для Каждого ПараметрыКнопки Из СтатистикаРезультатов Цикл
		...
		Если Не ПодписыватьЭлектроннойПодписью Тогда
			Если ПараметрыКнопки.Ключ = "ОжидаетПодписания" Тогда
				КоличествоПоКнопке = 0;
			ИначеЕсли ПараметрыКнопки.Ключ = "Принята" Тогда
				КоличествоПоКнопке = КоличествоПоКнопке 
					+ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтатистикаРезультатов, "ОжидаетПодписания", 0);
			КонецЕсли;
		КонецЕсли;
		...
		Если ПараметрыКнопки.Ключ = "Принята" Тогда
			Если ПодписыватьЭлектроннойПодписью Тогда
				СостояниеПредставление = НСтр("ru = 'Опубликована'");
			Иначе
				СостояниеПредставление = НСтр("ru = 'Принята'");
			КонецЕсли;
		КонецЕсли;
		
		Элементы[ИмяКоманды].Заголовок = СтрШаблон("%1 (%2)", СостояниеПредставление, ПредставлениеКоличества);
		Элементы[ИмяКоманды].Видимость = (КоличествоПоКнопке > 0);
	КонецЦикла;
