...
Функция ОбновитьКешиОператоровЭДОИФорматов(КонтекстДиагностики = Неопределено) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Попытка
			...
			ПараметрыСинхронизации = ПараметрыСинхронизацииДанныхИзСервисаНастроек();
			ПараметрыСинхронизации.ТипСинхронизации = ВыборкаДетальныеЗаписи.ТипСинхронизации;
			ПараметрыСинхронизации.ДатаПоследнегоИзменения = ВыборкаДетальныеЗаписи.ДатаПоследнегоИзменения;
			ПараметрыСинхронизации.КонтекстДиагностики = КонтекстДиагностики;
			
			СинхронизироватьКлассификаторыОграниченийМЧД(ПараметрыСинхронизации);
			СинхронизироватьСопоставленияКлассификатораОграниченийСОбъектамиБЭД(ПараметрыСинхронизации);
...
Процедура ОбновитьПоддерживаемыеФорматыАбонентовИзСервисаНастроек(ДатаПоследнегоОбновления,
	АдресРезультата = "") Экспорт
	
	ТипСинхронизации = Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеАбонентовЭДО;
	
	ВидОперации = НСтр("ru = 'Обновление данных абонентов ЭДО'");
	...
	СинхронизироватьАбонентовЭлектронногоДокументооборотаИзСервисаНастроек(ТипСинхронизации,
		ДатаПоследнегоОбновления, ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики());
...
// Возвращаемое значение:
//  Строка
Функция ИдентификаторОшибкиОтсутствуетСертификатДляПодписания() Экспорт
...
// Загружает сопоставления классификатора ограничений полномочий МЧД с объектами БЭД из сервиса настроек
// 
// Параметры:
//  ПараметрыОперации см. ПараметрыСинхронизацииДанныхИзСервисаНастроек
//  АдресРезультата - Неопределено
//
//Возвращаемое значение:
// Булево
Функция СинхронизироватьСопоставленияКлассификатораОграниченийСОбъектамиБЭД(ПараметрыОперации, 
		АдресРезультата = Неопределено) Экспорт
	
	Результат = Ложь;
	
	ТипСинхронизации = ПараметрыОперации.ТипСинхронизации;
	
	Если ТипСинхронизации <> 
		Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеСопоставленийКлассификатораОграниченийСОбъектамиБЭД Тогда
		Возврат Результат;
	КонецЕсли;
		
	ДанныеКлассификатора = СервисНастроекЭДО.НастройкиСоответствиеКлассификатораОграниченийСОбъектамиБЭД(
		ПараметрыОперации.КонтекстДиагностики, ПараметрыОперации.ДатаПоследнегоИзменения);
	...
	Попытка
		...
		МашиночитаемыеДоверенности.ОбновитьСопоставленияКлассификатораОграниченийСОбъектамиБЭД(
			ДанныеКлассификатора.ДвоичныеДанные);
		СервисНастроекЭДО.ИзменитьСостояниеСинхронизации(ПараметрыОперации.ТипСинхронизации,
			ДанныеКлассификатора.ДатаПоследнегоИзменения);
...
#Область СинхронизироватьКлассификаторыОграниченийМЧД

// Загружает классификаторы ограничений МЧД из сервиса настроек
// 
// Параметры:
//  ПараметрыОперации см. ПараметрыСинхронизацииДанныхИзСервисаНастроек
//  АдресРезультата - Строка
//                  - Неопределено
Процедура СинхронизироватьКлассификаторыОграниченийМЧД(ПараметрыОперации, АдресРезультата = Неопределено) Экспорт
	
	ТипСинхронизации = ПараметрыОперации.ТипСинхронизации;
	
	Если ТипСинхронизации <> 
		Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ОбновлениеКлассификаторовОграниченийМЧД Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеКлассификатора = СервисНастроекЭДО.ПолучитьКлассификаторОграниченийМЧД(ПараметрыОперации.КонтекстДиагностики, 
	...
	Попытка
		...
		РегистрыСведений.КлассификаторыОграниченийМЧД.ОбновитьКлассификаторы(ДанныеКлассификатора);
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Синхронизация классификатора ограничений МЧД из сервиса настроек'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , ,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
Функция ВидОшибкиОтсутствуетСертификатДляПодписания()
	...
	ВидОшибки = ОбработкаНеисправностейБЭДКлиентСервер.НовоеОписаниеВидаОшибки();
	ВидОшибки.Идентификатор = ИдентификаторОшибкиОтсутствуетСертификатДляПодписания();
...
// Параметры синхронизации данных из сервиса настроек.
// 
// Возвращаемое значение:
//  Структура:
// * ТипСинхронизации - ПеречислениеСсылка.ТипыСинхронизацииСервисаНастроекЭДО
// * ДатаПоследнегоИзменения - Дата
// * КонтекстДиагностики - см. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Функция ПараметрыСинхронизацииДанныхИзСервисаНастроек() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТипСинхронизации", 
		Перечисления.ТипыСинхронизацииСервисаНастроекЭДО.ПустаяСсылка());
	Результат.Вставить("ДатаПоследнегоИзменения", '00010101');
