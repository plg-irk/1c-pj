...
//    ** УниверсальноеСообщение - Строка - идентификатор формата универсального сообщений
...
Функция ПоддерживаемыеФорматы() Экспорт
    ...
    Форматы.ФНС.Вставить("УниверсальноеСообщение", ФорматыФНС.УниверсальноеСообщение);
...
Функция ЗаполнитьДанныеПодписантаДокумента(ОписаниеФайла,
	Организация, СертификатПодписи, СодержаниеДокумента, ИдентификаторОрганизации, ВыбраннаяМЧД = Неопределено) Экспорт
	...
	ОтборСертификата = УчетныеЗаписиЭДО.НовыйОтборСертификатовУчетныхЗаписей();
	ОтборСертификата.УчетныеЗаписи = "&ИдентификаторыУчетныхЗаписейЭДО";
	ОтборСертификата.Сертификат = "&Сертификат";
	ЗапросДолжностиВУчетнойЗаписи = 
		УчетныеЗаписиЭДО.ЗапросСертификатовУчетныхЗаписей("", ОтборСертификата);
	
	ИтоговыйЗапрос = Новый Запрос;
	ИтоговыйЗапрос.Текст = СтрЗаменить(ЗапросДолжностиВУчетнойЗаписи.Текст, "ПОМЕСТИТЬ", "");
	ИтоговыйЗапрос.УстановитьПараметр("ИдентификаторыУчетныхЗаписейЭДО", ИдентификаторОрганизации);
	ИтоговыйЗапрос.УстановитьПараметр("Сертификат", СертификатПодписи);	
...
Функция НовыеДанныеДляФормированияСлужебногоСообщения() Экспорт
	...
	Данные.Вставить("Основание", Новый Структура);
	Данные.Основание.Вставить("ИмяБезРасширения", "");
	Данные.Основание.Вставить("Сообщение", Документы.СообщениеЭДО.ПустаяСсылка());
...
Функция СформироватьДанныеСлужебногоСообщения(Знач ТипДокумента, Знач Данные) Экспорт
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение Тогда
		Формат = Форматы.ФНС.УниверсальноеСообщение;
	КонецЕсли;
...
//   * УниверсальноеСообщение - Структура:
...
Функция НовоеОписаниеСвойствДокументовПоФорматам() Экспорт
	...
	СвойстваУниверсальногоСообщения = Новый Структура;
	СвойстваУниверсальногоСообщения.Вставить("ИмяФайлаНазначения", "");
	СвойстваУниверсальногоСообщения.Вставить("ТекстСообщения", "");
	СвойстваУниверсальногоСообщения.Вставить("Статус", "");
	Результат.Вставить("УниверсальноеСообщение", СвойстваУниверсальногоСообщения);
...
Процедура ПроверитьРегистрационныеДанныеДляОператораЭДО(ДеревоДанных, Ошибки) Экспорт
	...
	Если ЭтоЮЛ Тогда
		...
		ЭтоФилиалИностраннойОрганизации = ОбщегоНазначенияБЭДКлиентСервер.ЭтоКППФилиалаИностраннойОрганизации(КПП);
	Иначе
...
Функция ТекстУточнения(ОписаниеФайла) Экспорт
	...
	ИначеЕсли СтрНайти(ОписаниеФайла.ИмяФайла, "DP_UNISOOBSCH") > 0 
		И ТипЗнч(Данные.Документ.СвСобытДок.ТекстУС) = Тип("Строка") Тогда
			
		ТекстУточнения = Данные.Документ.СвСобытДок.ТекстУС;	
	КонецЕсли;
...
Функция ДанныеСлужебнойКвитанции(Знач ТипДокумента, Знач ДанныеЭД)
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.УниверсальноеСообщение Тогда
		Данные = ДанныеСлужебнойКвитанцииУниверсальногоСообщения(ТипДокумента, ДанныеЭД);
	КонецЕсли;
...
//  ДанныеЭД - см. НовыеДанныеДляФормированияСлужебногоСообщения
// Возвращаемое значение:
//  Структура - см. НовыеДанныеДокументаПоУчету
Функция ДанныеСлужебнойКвитанцииУниверсальногоСообщения(Знач ТипДокумента, Знач ДанныеЭД)
	
	РезультатПолученияДанныхДляФормированияЭД = Обработки.ФорматУниверсальноеСообщениеЭДО.ДанныеСлужебнойКвитанции(ТипДокумента, ДанныеЭД);

	ДанныеУчета = НовыеДанныеДокументаПоУчету();
	ДанныеУчета.Данные = РезультатПолученияДанныхДляФормированияЭД.Данные;
...
Функция ТипРегламентаДляАктаОРасхождениях(СодержаниеДокумента)
	...
	Если СодержаниеДокумента.СвойстваДокументовПоФорматам.АктОРасхождениях.ФормируютсяИтогиПриемки Тогда
		Возврат ТипРегламента;
	КонецЕсли;
...
// Возвращает массив путей к информационным полям формата, предназначенным для размещения данных по МЧД.
// 
// Параметры:
...
// Параметры:
//  Доверенности - Массив Из ОпределяемыйТип.МашиночитаемаяДоверенность
//  ИнформационноеПоле - СписокXDTO - Элемент файла обмена, содержащий произвольные сведения в текстовом формате
//  ПространствоИмен - Строка - Пространство имен схемы документа
...
Функция МетаданныеОбработокФорматов()
	...
	МассивМетаданных.Добавить(Метаданные.Обработки.ФорматУниверсальноеСообщениеЭДО);
