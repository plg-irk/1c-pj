...
Процедура ЗарегистрироватьИзмененияРегистраТоварыВОборудовании(Источник, Отказ = Ложь, ТипЦен = Неопределено, ПодразделениеКомпании = Неопределено, ОсновнойОтдел = Неопределено) Экспорт
	
	ТипИсточника = ТипЗнч(Источник);
	
	Если (НЕ ТипИсточника = Тип("Массив")) И Источник.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстОтбора = "";
	Если ТипИсточника = Тип("СправочникОбъект.Номенклатура") 
		ИЛИ ТипИсточника = Тип("СправочникОбъект.ГруппыТоваровОборудования") Тогда
		ВидОбъекта=?(ТипЗнч(Источник)=Тип("СправочникОбъект.Номенклатура"),"Номенклатура","ГруппаТоваров");
		Ссылка = Источник.Ссылка;
		ТекстОтбора = "ТоварыВОборудовании."+ВидОбъекта+"=&Фильтр";
	ИначеЕсли ТипИсточника = Тип("СправочникОбъект.ЕдиницыИзмерения") И ТипЗнч(Источник.Владелец)=Тип("СправочникСсылка.Номенклатура") Тогда
		Ссылка = Источник.Владелец;
		ТекстОтбора = "ТоварыВОборудовании.Номенклатура=&Фильтр";
	ИначеЕсли ТипИсточника = Тип("Массив") Тогда
		Ссылка = Источник;
		ТекстОтбора = "ТоварыВОборудовании.Номенклатура В (&Фильтр)";
	Иначе
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТипЦен) И ЗначениеЗаполнено(ОсновнойОтдел) Тогда
		ТекстОтбора = ТекстОтбора + " И 
		|	ТоварыВОборудовании.ГруппаТоваров В (ВЫБРАТЬ
		|			ПодключаемоеОборудование.ГруппаТоваровОборудования
		|		ИЗ
		|			Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудование
		|		ГДЕ
		|			ПодключаемоеОборудование.ОсновнойОтдел = &ОсновнойОтдел)";
	ИначеЕсли ЗначениеЗаполнено(ТипЦен) И ЗначениеЗаполнено(ПодразделениеКомпании) Тогда
		ТекстОтбора = ТекстОтбора + " И 
		|	ТоварыВОборудовании.ГруппаТоваров В (ВЫБРАТЬ
		|			ПодключаемоеОборудование.ГруппаТоваровОборудования
		|		ИЗ
		|			Справочник.ПодключаемоеОборудование КАК ПодключаемоеОборудование
		|		ГДЕ ((ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.БазовыйТипЦен = &ТипЦен
		|				И ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.Рассчитывается) или
		|			ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли = &ТипЦен) И 
		|			ПодключаемоеОборудование.ОсновнойОтдел.ПодразделениеКомпании = &ПодразделениеКомпании)";
	ИначеЕсли ЗначениеЗаполнено(ТипЦен) Тогда
		ТекстОтбора = ТекстОтбора + " И 
		|	ТоварыВОборудовании.ГруппаТоваров В (ВЫБРАТЬ
		|			ПодключаемоеОборудование.ГруппаТоваровОборудования
		|		ИЗ
		...
		|		ГДЕ (ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.БазовыйТипЦен = &ТипЦен
		|				И ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.Рассчитывается) или
		|			ПодключаемоеОборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли = &ТипЦен)";
	...
	|	ТоварыВОборудовании.ВыполненаПечатьЦенников КАК ВыполненаПечатьЦенников,
	...
	Запрос.УстановитьПараметр("Фильтр", Ссылка);
	...
	Запрос.УстановитьПараметр("ОсновнойОтдел", ОсновнойОтдел);
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяЗапись.ВыполненаПечатьЦенников = Выборка.ВыполненаПечатьЦенников;
		НоваяЗапись.Записать(Истина);
		
		НоваяСтрока = Новый Структура;
		НоваяСтрока.Вставить("Номенклатура",    Выборка.Номенклатура);
		НоваяСтрока.Вставить("Штрихкод",        Выборка.Штрихкод);
		НоваяСтрока.Вставить("НомерЯчейки",     Выборка.НомерЯчейки);
		НоваяСтрока.Вставить("МоментИзменения", Выборка.МоментИзменения);
		
		Если ГруппаТоваров = Неопределено Тогда
			ГруппаТоваров = Выборка.ГруппаТоваров;
		КонецЕсли;
		
		Если НЕ Выборка.ГруппаТоваров = ГруппаТоваров Тогда
			...
			Комментарий = "Регистрация изменений товаров в оборудовании. Изменено: " + Формат(Данные.Количество(), "ЧН=0; ЧГ=0");
			
			МенеджерОборудованияРарусСервер.ЗаписатьСобытиеЖурналаОборудования(ГруппаТоваров, , "Регистрация изменений товаров в оборудовании", Комментарий, ДанныеСобытия);
	...
	Если НЕ ГруппаТоваров = Неопределено Тогда
		ДанныеСобытия = Новый Структура;
		ДанныеСобытия.Вставить("ТекущаяДатаСеанса", ТекущаяДатаСеанса());
		ДанныеСобытия.Вставить("Данные",            Данные);
...
Процедура ЗарегистрироватьИзменениеОбъекта(ИмяПланаОбмена, Объект, Отказ, ДополнительныеПараметры = Неопределено)
	...
	Попытка
		...
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
			...
			ЭтоРазделенныеДанные = ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
				ОбъектМетаданных.ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительОсновныхДанных());
			
			Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
				
				ЭтоСовместноРазделенныеДанные = ОбщегоНазначенияПовтИсп.ЭтоРазделенныйОбъектМетаданных(
					ОбъектМетаданных.ПолноеИмя(), ОбщегоНазначенияПовтИсп.РазделительВспомогательныхДанных());
