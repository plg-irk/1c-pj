...
// Загружает из каталогов 1С:Номенклатура и Национального каталога номенклатуру, найденную по штрихкодам (GTIN).
// Сначала создаются:
//  * Виды номенклатуры, соответствующие категориям сервиса, категории загружаются без иерархии.
//  * Дополнительные реквизиты, подчиненные виду номенклатуры.
//  * Значения дополнительных реквизитов. 
// 
// Параметры:
//  Штрихкоды - Соответствие из КлючИЗначение:
//  * Ключ - Строка - штрихкод.
//  * Значение - Произвольный - дополнительные параметры создания номенклатуры, передаваемые в прикладной контекст.
//  ПараметрыЗагрузки - см. РаботаСНоменклатуройКлиентСервер.НовыеПараметрыПолученияНоменклатурыСервисаПоШтрихкодам
//  
// Возвращаемое значение:
//  Структура:
//   * Отказ - Булево - принимает значение Истина при наличии общей ошибки.
//   * Ошибка - Строка - тип ошибки, для общей ошибки единственный вариант значения-
//                       см. ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().ОшибкаАутентификацииНаПорталеПоддержки.
//   * Результат - Соответствие из КлючИЗначение:
//    ** Ключ - Строка - штрихкод.
//    ** Значение - см. РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду.
//
Функция ЗагрузитьНоменклатуруСервисаПоШтрихкодам(Знач Штрихкоды, Знач ПараметрыЗагрузки) Экспорт
	
	Ответ            = Новый Структура("Отказ, Ошибка, Результат", Ложь, "", Новый Соответствие);
	КонтекстПроверки = "РаботаСНоменклатурой.ЗагрузитьНоменклатуруСервисаПоШтрихкодам";
	
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(КонтекстПроверки, "Штрихкоды", Штрихкоды, Тип("Соответствие"));
	
	ОжидаемыеТипыСвойств = РаботаСНоменклатуройКлиентСервер.НовыеПараметрыПолученияНоменклатурыСервисаПоШтрихкодам();
	ОжидаемыеТипыСвойств.ТокенГИСМТ = Тип("Строка");
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(КонтекстПроверки, "ПараметрыЗагрузки", ПараметрыЗагрузки, 
		Тип("Структура"), ОжидаемыеТипыСвойств);
	
	Если РаботаСНоменклатуройСлужебный.ТикетАутентификацииНаПорталеПоддержки().Результат = Неопределено Тогда
		Ответ.Ошибка = ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().ОшибкаАутентификацииНаПорталеПоддержки;
		Ответ.Отказ  = Истина;
		Возврат Ответ;
	КонецЕсли;
	
	// Защита от многократной передачи одного и того же штрихкода внутри соответствия с пробелами до или после
	СоответствиеШтрихкодов = Новый Соответствие;
	УникальныеШтрихкоды    = Новый Соответствие;
	Для Каждого ДанныеЗапроса Из Штрихкоды Цикл
		ШтрихкодБезПробелов = ШтрихкодДляПоиска(ДанныеЗапроса.Ключ);
		Если ПустаяСтрока(ШтрихкодБезПробелов)
			Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ШтрихкодБезПробелов) Тогда
			Продолжить;
		КонецЕсли;
		
		// Все ключи исходного соответствия записываются в массив, соответствующий ключу без пробелов
		СвернутыеДанные = СоответствиеШтрихкодов[ШтрихкодБезПробелов];
		Если СвернутыеДанные = Неопределено Тогда
			СоответствиеШтрихкодов.Вставить(ШтрихкодБезПробелов,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДанныеЗапроса.Ключ));
		Иначе
			СвернутыеДанные.Добавить(ДанныеЗапроса.Ключ);
		КонецЕсли;
		
		// Дополнительные параметры выбираются произвольно
		УникальныеШтрихкоды.Вставить(ШтрихкодБезПробелов, ДанныеЗапроса.Значение);
	КонецЦикла;
	
	Если УникальныеШтрихкоды.Количество() > 0 Тогда
		НайтиНоменклатуруПоШтрихкодамВИнформационнойБазе(УникальныеШтрихкоды, Ответ.Результат, КонтекстПроверки);
	КонецЕсли;
	Если УникальныеШтрихкоды.Количество() > 0 Тогда
		НайтиСоздатьНоменклатуруПоШтрихкодамВСервисе(УникальныеШтрихкоды, Ответ.Результат, КонтекстПроверки,
			ПараметрыЗагрузки);
	КонецЕсли;
	
	Ответ.Результат = РазвернутьРезультат(Ответ.Результат, СоответствиеШтрихкодов);
	
	Возврат Ответ;
	
КонецФункции

// Возвращает ошибки загрузки номенклатуры сервиса по штрихкодам
// 
// Возвращаемое значение:
//  Структура:
//   * ОшибкаАутентификацииНаПорталеПоддержки - Строка - не удалось получить тикет аутентификации пользователя на 
//                                                       портале поддержки.
//   * ОшибкаКонвертации - Строка - не выполнена конвертация данных Национального каталога в формат сервиса 
//                                  1С:Номенклатура.
//   * НеНайдено - Строка - номенклатура по штрихкоду не найдена.
//   * ОшибкаАвторизацииНК - Строка - ответ 401 от API Национального каталога, требуется обновить токен.
//   * НеизвестнаяОшибкаНК - Строка - необрабатываемая ошибка Национального каталога.
//   * НеизвестнаяОшибкаСервиса - Строка - необрабатываемая ошибка сервиса.
//   * СервисНедоступен - Строка - сервис недоступен.
//   * ОшибкаПокупкиКарточек - Строка - не удалось купить карточки сервиса 1С:Номенклатура.
//
Функция ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам() Экспорт
	
	Ошибки = Новый Структура();
	Ошибки.Вставить("ОшибкаАутентификацииНаПорталеПоддержки", "ОшибкаАутентификацииНаПорталеПоддержки");
	Ошибки.Вставить("ОшибкаКонвертации", "ОшибкаКонвертации");
	Ошибки.Вставить("НеНайдено", "НеНайдено");
	Ошибки.Вставить("ОшибкаАвторизацииНК", "ОшибкаАвторизацииНК");
	Ошибки.Вставить("НеизвестнаяОшибкаНК", "НеизвестнаяОшибкаНК");
	Ошибки.Вставить("НеизвестнаяОшибкаСервиса", "НеизвестнаяОшибкаСервиса");
	Ошибки.Вставить("СервисНедоступен", "СервисНедоступен");
	Ошибки.Вставить("ОшибкаПокупкиКарточек", "ОшибкаПокупкиКарточек");
...
//    * СтавкаНДС                     - Строка - ставка НДС. Возможны значения: "22", "20", "18", "10", "-".
...
//    * ЗапрошенныйШтрихкод        - Строка - заполняется, если данные номенклатуры найдены по штрихкоду
//    * Ошибка                     - Строка - тип ошибки, значения: см. ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам.
//
Функция ДанныеНоменклатурыСервиса(Знач ИдентификаторыНоменклатуры, ПолучатьТолькоОсновныеДанные = Ложь, ИсключатьНеКупленные = Истина) Экспорт
	...
	ДанныеПоНоменклатуре.Колонки.Добавить("ДополнительныеПараметры");
...
Процедура ЗаполнитьКарточкуВидаНоменклатуры(ТабличныйДокумент, ДанныеПоКатегории, ОбновитьПредставлениеВидовНоменклатуры = Ложь) Экспорт
	...
	ВывестиКлассификаторы(ДанныеПоКатегории.Классификаторы, ТабличныйДокумент, 
		Обработки.РаботаСНоменклатурой.ПолучитьМакет("ПФ_MXL_КарточкаНоменклатуры"), Истина);
...
#Область НациональныйКаталог

// Получает товары из Национального каталога, доступные к подписанию.
//
// Параметры:
//   Организация               - ОпределяемыйТип.Организация - организация.
//   ТоварыКПодписанию         - Массив из РегистрСведенийКлючЗаписи.СостоянияВыгрузкиНоменклатуры - выбранные товарные 
//                                 позиции к подписанию.
//                             - Строка - адрес временного хранилища таблицы значений с перечнем товарных позиций к 
//                                 подписанию со следующими обязательными колонками.
//                                 # Номенклатура               - ОпределяемыйТип.НоменклатураРаботаСНоменклатурой - номенклатура.
//                                 # ХарактеристикаНоменклатуры - ОпределяемыйТип.ХарактеристикаРаботаСНоменклатурой - характеристика.
//   АдресРезультата           - Строка - адрес временного хранилища, в которое должен быть помещен результат
//                                 работы процедуры - Структура:
//                                 # ТоварыКПодписанию     - ТаблицаЗначений - товары к подписанию:
//                                   ## Организация                - ОпределяемыйТип.Организация - организация.
//                                   ## Номенклатура               - ОпределяемыйТип.НоменклатураРаботаСНоменклатурой - номенклатура.
//                                   ## ХарактеристикаНоменклатуры - ОпределяемыйТип.ХарактеристикаРаботаСНоменклатурой - характеристика.
//                                   ## ИдентификаторПакетаДанных  - Строка - идентификатор пакета данных.
//                                   ## Состояние                  - ПеречислениеСсылка.СостоянияВыгрузкиНоменклатуры - состояние выгрузки.
//                                   ## ОписаниеОшибки             - Строка - описание ошибки.
//                                   ## Хеш                        - Строка - хеш выгружаемых данных.
//                                   ## Штрихкод                   - Строка - преобразованный штрихкод товарной позиции.
//                                   ## ШтрихкодБезПреобразования  - Строка - оригинальный штрихкод товарной позиции.
//                                   ## ИдентификаторТовара        - Число - идентификатор товара в Национальном каталоге.
//                                   ## КарточкаТовара             - Строка - xml карточки товара Национального каталога.
//                                   ## ОбновитьВРегистре          - Булево - обновлять ли данные в регистре сведений (служебное).
//                                 # СтатистикаРезультатов - Неопределено, 
//                                   см. РаботаСНоменклатуройСлужебный.СтатистикаРезультатовВыгрузки().
//   ПроверятьСостояниеТоваров - Булево - Истина, если при получении карточки товара из Национального каталога будет 
//                                 определено, что данные не готовы к подписанию, выполняется дополнительная проверка 
//                                 не были ли карточки товара подписаны независимо в личном кабинете.
//   ОбновитьСтатистику        - Булево - обновлять ли статистику результатов выгрузки по завершении метода.
//
Процедура ПолучитьТоварыНациональногоКаталогаКПодписанию(Организация, Знач ТоварыКПодписанию, 
			АдресРезультата = Неопределено, ПроверятьСостояниеТоваров = Ложь, ОбновитьСтатистику = Ложь) Экспорт
	
	Если ТипЗнч(ТоварыКПодписанию) = Тип("Строка")
			И ЭтоАдресВременногоХранилища(ТоварыКПодписанию) Тогда
		Товары = ПолучитьИзВременногоХранилища(ТоварыКПодписанию);
		
	Иначе
		Товары = Новый ТаблицаЗначений;
		Товары.Колонки.Добавить("Номенклатура",               Метаданные.ОпределяемыеТипы.НоменклатураРаботаСНоменклатурой.Тип);
		Товары.Колонки.Добавить("ХарактеристикаНоменклатуры", Метаданные.ОпределяемыеТипы.ХарактеристикаРаботаСНоменклатурой.Тип);
		
		Для Каждого ЭлементКоллекции Из ТоварыКПодписанию Цикл
			ЗаполнитьЗначенияСвойств(Товары.Добавить(), ЭлементКоллекции);
		КонецЦикла;
	КонецЕсли;
	...
	СписокСостояний.Добавить(Перечисления.СостоянияВыгрузкиНоменклатуры.ОжидаетПодписания);
	
	Настройки = РаботаСНоменклатуройСлужебный.НастройкаВыгрузкиНоменклатуры(Организация);
	Настройки.Вставить("Соединение", Неопределено);
	
	// Получить товары к подписанию
	ОбходРезультатов = Новый Структура;
	ОбходРезультатов.Вставить("ВыборкаДанных", Неопределено);
	ОбходРезультатов.Вставить("Штрихкоды",     Неопределено);
	
	Запрос = РаботаСНоменклатуройСлужебный.ИнициализацияЗапросаТоваровНациональногоКаталогаКПодписанию(ОбходРезультатов);
	
	Запрос.УстановитьПараметр("Организация",                  Организация);
	Запрос.УстановитьПараметр("ТоварыКПодписанию",            Товары);
	...
	Для Каждого ЭлементКоллекции Из ОбходРезультатов Цикл
		Если ЭлементКоллекции.Ключ = "ВыборкаДанных" Тогда
			Если Не ЗначениеЗаполнено(ЭлементКоллекции.Значение) 
					Или РезультатЗапроса[ЭлементКоллекции.Значение].Пустой() Тогда
				ВызватьИсключение НСтр("ru = 'Подписать можно только товарные позиции в состоянии ""Ожидает подписания"".'");
			Иначе
				ОбходРезультатов.Вставить(ЭлементКоллекции.Ключ, РезультатЗапроса[ЭлементКоллекции.Значение]);
			КонецЕсли;
		
		ИначеЕсли ЗначениеЗаполнено(ЭлементКоллекции.Значение) Тогда
			РезультатВыборки = РезультатЗапроса[ЭлементКоллекции.Значение].Выгрузить();
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатВыборки.Колонки, "Характеристика") Тогда
				РезультатВыборки.Индексы.Добавить("Номенклатура, Характеристика");
			Иначе 
				РезультатВыборки.Индексы.Добавить("Номенклатура");
			КонецЕсли;
			ОбходРезультатов.Вставить(ЭлементКоллекции.Ключ, РезультатВыборки);
		КонецЕсли;
	КонецЦикла;
	
	// Получить карточки товаров из Национального каталога
	Товары.Очистить();
	Товары.Колонки.Добавить("ИдентификаторПакетаДанных", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(36)));
	Товары.Колонки.Добавить("Состояние",                 Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияВыгрузкиНоменклатуры"));
	Товары.Колонки.Добавить("ОписаниеОшибки",            Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("Хеш",                       Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(32)));
	Товары.Колонки.Добавить("Штрихкод",                  Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(200)));
	Товары.Колонки.Добавить("ШтрихкодБезПреобразования", Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(200)));
	Товары.Колонки.Добавить("ИдентификаторТовара",       Новый ОписаниеТипов("Число"));
	Товары.Колонки.Добавить("КарточкаТовара",            Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("ОбновитьВРегистре",         Новый ОписаниеТипов("Булево"));
	Товары.Колонки.ХарактеристикаНоменклатуры.Имя = "Характеристика";
	
	ТоварыПорция       = Товары.Скопировать();
	Штрихкоды          = Новый Массив;
	КоличествоВЗапросе = 25;
	ОшибкаСервиса      = Ложь;
	
	РезультатЗапроса = ОбходРезультатов.ВыборкаДанных;	// РезультатЗапроса
	ВыборкаДанных    = РезультатЗапроса.Выбрать();
	
	Пока ВыборкаДанных.Следующий() Цикл
		НеУказанШтрихкод = Истина;
		
		Если ЗначениеЗаполнено(ОбходРезультатов.Штрихкоды) Тогда
			ЗаполнитьЗначенияСвойств(ПоискСХарактеристикой, ВыборкаДанных);
			НайденныеСтроки = ОбходРезультатов.Штрихкоды.НайтиСтроки(ПоискСХарактеристикой);
			
			Для Каждого ЭлементКоллекции Из НайденныеСтроки Цикл
				Штрихкод = ЭлементКоллекции.Штрихкод;
				Штрихкод = СтрЗаменить(Штрихкод, "Тест_", "");
				
				Если ЗначениеЗаполнено(Штрихкод) Тогда
					НеУказанШтрихкод = Ложь;
					
					СтрокаТаблицыЗначений = ТоварыПорция.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыЗначений, ВыборкаДанных);
					СтрокаТаблицыЗначений.Штрихкод = ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(
						Штрихкод);
					СтрокаТаблицыЗначений.ШтрихкодБезПреобразования = Штрихкод;
					
					Если ЭтоВалидныйШтрихкодНациональногоКаталога(Штрихкод) Тогда
						Штрихкоды.Добавить(Штрихкод);
					Иначе
						СтрокаТаблицыЗначений.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Указан некорректный gtin (%1) товарной позиции.'"),
							Штрихкод);
					КонецЕсли;
					
					Если Штрихкоды.Количество() >= КоличествоВЗапросе Тогда
						ОшибкаСервиса = Не ОбработатьТоварыНациональногоКаталогаКПодписанию(ТоварыПорция, 
							Штрихкоды, Настройки);
						ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТоварыПорция, Товары);
						ТоварыПорция.Очистить();
						Штрихкоды.Очистить();
						
						Если ОшибкаСервиса Тогда
							Прервать;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
		...
		Если НеУказанШтрихкод Тогда
			СтрокаТаблицыЗначений = ТоварыПорция.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыЗначений, ВыборкаДанных);
			СтрокаТаблицыЗначений.ОписаниеОшибки = НСтр("ru = 'Не указан штрихкод (gtin) товарной позиции.'");
		КонецЕсли;
	...
	Если Штрихкоды.Количество() > 0 Тогда
		ОбработатьТоварыНациональногоКаталогаКПодписанию(ТоварыПорция, Штрихкоды, Настройки);
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТоварыПорция, Товары);
	
	// Объединить строки по одинаковым товарным позициям
	КУдалению = Новый Массив;
	
	Для Каждого СтрокаТаблицыЗначений Из Товары Цикл
		Если Не ЗначениеЗаполнено(СтрокаТаблицыЗначений.КарточкаТовара)
				И Не ЗначениеЗаполнено(СтрокаТаблицыЗначений.ОписаниеОшибки) Тогда
			СтрокаТаблицыЗначений.ОписаниеОшибки = НСтр("ru = 'Не удалось получить товар по GTIN'");
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаблицыЗначений.ОписаниеОшибки) 
				И Прав(СтрокаТаблицыЗначений.ОписаниеОшибки, 1) <> "." Тогда
			СтрокаТаблицыЗначений.ОписаниеОшибки = СтрокаТаблицыЗначений.ОписаниеОшибки + ".";
		КонецЕсли;
		
		Индекс = СтрНайти(ВРег(СтрокаТаблицыЗначений.ОписаниеОшибки), ВРег(" gtin:"));
		Если Индекс > 0 Тогда
			СледующийПробел = СтрНайти(СтрокаТаблицыЗначений.ОписаниеОшибки, " ",, Индекс + СтрДлина(" gtin:") + 1);
			СтрокаТаблицыЗначений.ОписаниеОшибки = Сред(СтрокаТаблицыЗначений.ОписаниеОшибки, 1, Индекс - 1)
				+ " gtin ("
				+ Сред(СтрокаТаблицыЗначений.ОписаниеОшибки, Индекс + СтрДлина(" gtin:") + 1, СледующийПробел - СтрДлина(" gtin:") - Индекс - 1) 
				+ ")"
				+ Сред(СтрокаТаблицыЗначений.ОписаниеОшибки, СледующийПробел);
		Иначе
			Индекс = СтрНайти(ВРег(СтрокаТаблицыЗначений.ОписаниеОшибки), ВРег(" gtin"));
			Если Индекс > 0 
					И СтрНайти(ВРег(СтрокаТаблицыЗначений.ОписаниеОшибки), ВРег(" gtin (")) = 0 Тогда
				СтрокаТаблицыЗначений.ОписаниеОшибки = Сред(СтрокаТаблицыЗначений.ОписаниеОшибки, 1, Индекс - 1)
					+ " gtin"
					+ ?(ЗначениеЗаполнено(СтрокаТаблицыЗначений.ШтрихкодБезПреобразования),
						" (" + СтрокаТаблицыЗначений.ШтрихкодБезПреобразования + ")",
						"")
					+ Сред(СтрокаТаблицыЗначений.ОписаниеОшибки, Индекс + СтрДлина(" gtin"));
			КонецЕсли;
		КонецЕсли;
		
		ЭтаСтрока = Товары.Индекс(СтрокаТаблицыЗначений);
		
		Если КУдалению.Найти(СтрокаТаблицыЗначений) = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ПоискСХарактеристикой, СтрокаТаблицыЗначений);
			НайденныеСтроки = Товары.НайтиСтроки(ПоискСХарактеристикой);
			
			Если ЗначениеЗаполнено(СтрокаТаблицыЗначений.ИдентификаторТовара)
					И ЗначениеЗаполнено(СтрокаТаблицыЗначений.КарточкаТовара) Тогда
				СтрокаТаблицыЗначений.КарточкаТовара = СтрЗаменить(СтрокаТаблицыЗначений.КарточкаТовара, Символы.ПС, "");
				СтрокаТаблицыЗначений.КарточкаТовара = СтрЗаменить(СтрокаТаблицыЗначений.КарточкаТовара, Символы.ВК, "");
				СтрокаТаблицыЗначений.ОписаниеОшибки = "";
				
				Для Каждого ЭлементКоллекции Из НайденныеСтроки Цикл
					Если Товары.Индекс(ЭлементКоллекции) <> ЭтаСтрока Тогда
						КУдалению.Добавить(ЭлементКоллекции);
					КонецЕсли;
				КонецЦикла;
				
			ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицыЗначений.ОписаниеОшибки) Тогда
				СтрокаТаблицыЗначений.ОбновитьВРегистре = Истина;
				
				Для Каждого ЭлементКоллекции Из НайденныеСтроки Цикл
					Если ЗначениеЗаполнено(ЭлементКоллекции.ИдентификаторТовара)
							И ЗначениеЗаполнено(ЭлементКоллекции.КарточкаТовара) Тогда
						КУдалению.Добавить(СтрокаТаблицыЗначений);
						Прервать;
					КонецЕсли;
					
					Если Товары.Индекс(ЭлементКоллекции) <> ЭтаСтрока Тогда
						Если ЗначениеЗаполнено(ЭлементКоллекции.ОписаниеОшибки)
								И СтрНайти(СтрокаТаблицыЗначений.ОписаниеОшибки, ЭлементКоллекции.ОписаниеОшибки) = 0 Тогда
							СтрокаТаблицыЗначений.ОписаниеОшибки = СтрокаТаблицыЗначений.ОписаниеОшибки
								+ Символы.ПС 
								+ ЭлементКоллекции.ОписаниеОшибки;
						КонецЕсли;
	...
	// Некоторые товары уже могли быть подписаны в личном кабинете
	Если ПроверятьСостояниеТоваров Тогда
		Штрихкоды          = Новый Массив;
		КоличествоВЗапросе = 25;
		ОшибкаСервиса      = Ложь;
		
		Для Каждого СтрокаТаблицыЗначений Из Товары Цикл
			Если КУдалению.Найти(СтрокаТаблицыЗначений) = Неопределено
					И ЗначениеЗаполнено(СтрокаТаблицыЗначений.ОписаниеОшибки)
					И СтрНайти(ВРег(СтрокаТаблицыЗначений.ОписаниеОшибки), ВРег(НСтр("ru = 'не готов к подписанию'"))) > 0 Тогда
				Штрихкоды.Добавить(Штрихкод);
				
				Если Штрихкоды.Количество() >= КоличествоВЗапросе Тогда
					ОшибкаСервиса = Не ОбработатьСостояниеОпубликованТоваровНациональногоКаталога(Товары, Штрихкоды, Настройки);
					Штрихкоды.Очистить();
		...
		Если Штрихкоды.Количество() > 0 Тогда
			ОбработатьСостояниеОпубликованТоваровНациональногоКаталога(Товары, Штрихкоды, Настройки);
		КонецЕсли;
	КонецЕсли;
	
	Товары.Колонки.Добавить("Организация", Метаданные.ОпределяемыеТипы.Организация.Тип);
	Товары.ЗаполнитьЗначения(Организация, "Организация");
	Товары.Колонки.Характеристика.Имя = "ХарактеристикаНоменклатуры";
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.СостоянияВыгрузкиНоменклатуры");
		ЭлементБлокировкиДанных.ИсточникДанных = Товары;
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Организация", "Организация");
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("ХарактеристикаНоменклатуры", "ХарактеристикаНоменклатуры");
		БлокировкаДанных.Заблокировать();
		
		Для Каждого СтрокаТаблицыЗначений Из Товары Цикл
			Если КУдалению.Найти(СтрокаТаблицыЗначений) = Неопределено
					И СтрокаТаблицыЗначений.ОбновитьВРегистре Тогда
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Национальный каталог.Получить карточки товаров к подписанию'",
	...
	// Удалить дубли и строки с ошибками
	КУдалению = ОбщегоНазначенияКлиентСервер.СвернутьМассив(КУдалению);
	...
	Если ЗначениеЗаполнено(АдресРезультата) Тогда
		Результат = Новый Структура;
		Результат.Вставить("ТоварыКПодписанию",     Товары);
		Результат.Вставить("СтатистикаРезультатов", ?(ОбновитьСтатистику, 
			РаботаСНоменклатуройСлужебный.СтатистикаРезультатовВыгрузки(Организация), Неопределено));
...
// Подписывает карточки товаров Национального каталога с использованием открепленной подписи.
//
// Параметры:
//   Организация         - ОпределяемыйТип.Организация - организация.
//   ДанныеДляПодписания - Массив из Структура - карточки товаров к подписанию:
//     * Организация                - ОпределяемыйТип.Организация - организация.
//     * Номенклатура               - ОпределяемыйТип.НоменклатураРаботаСНоменклатурой - номенклатура.
//     * ХарактеристикаНоменклатуры - ОпределяемыйТип.ХарактеристикаРаботаСНоменклатурой - характеристика.
//     * ИдентификаторПакетаДанных  - Строка - идентификатор пакета данных.
...
//     * Хеш                        - Строка - хеш выгружаемых данных.
//     * ИдентификаторТовара        - Число - идентификатор товара в Национальном каталоге.
//     * КарточкаТовара             - Строка - двоичные данные xml карточки товара, закодированные по алгоритму base64.
//     * Подпись                    - Строка - двоичные данные подписи, закодированные по алгоритму base64.
//   АдресРезультата     - Строка - адрес временного хранилища, в которое должен быть помещен результат работы 
//                           процедуры - Структура:
//                           # Успешно               - Булево - Истина, если все переданные данные были подписаны.
//                           # СтатистикаРезультатов - Неопределено, 
//                             см. РаботаСНоменклатуройСлужебный.СтатистикаРезультатовВыгрузки().
//   ОбновитьСтатистику  - Булево - обновлять ли статистику результатов выгрузки по завершении метода.
//
Процедура ПодписатьТоварыНациональногоКаталога(Организация, Знач ДанныеДляПодписания, 
			АдресРезультата = Неопределено, ОбновитьСтатистику = Ложь) Экспорт
	
	Настройки = РаботаСНоменклатуройСлужебный.НастройкаВыгрузкиНоменклатуры(Организация);
	Настройки.Вставить("Соединение", Неопределено);
	
	Подписано = Новый ТаблицаЗначений;
	Подписано.Колонки.Добавить("Организация",                Метаданные.ОпределяемыеТипы.Организация.Тип);
	Подписано.Колонки.Добавить("Номенклатура",               Метаданные.ОпределяемыеТипы.НоменклатураРаботаСНоменклатурой.Тип);
	Подписано.Колонки.Добавить("ХарактеристикаНоменклатуры", Метаданные.ОпределяемыеТипы.ХарактеристикаРаботаСНоменклатурой.Тип);
	Подписано.Колонки.Добавить("ИдентификаторПакетаДанных",  Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(36)));
	Подписано.Колонки.Добавить("Состояние",                  Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияВыгрузкиНоменклатуры"));
	Подписано.Колонки.Добавить("ОписаниеОшибки",             Новый ОписаниеТипов("Строка"));
	Подписано.Колонки.Добавить("Хеш",                        Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(32)));
	Подписано.Колонки.Добавить("ИдентификаторТовара",        Новый ОписаниеТипов("Число"));
	
	// Подписать карточки товаров Национального каталога
	ТоварыПорция       = Новый Соответствие;
	КоличествоВЗапросе = 25;
	
	Для Каждого ЭлементКоллекции Из ДанныеДляПодписания Цикл
		ТоварыПорция.Вставить(ЭлементКоллекции.ИдентификаторТовара, ЭлементКоллекции);
		
		Если ТоварыПорция.Количество() >= КоличествоВЗапросе Тогда
			ПодписатьПорциюТоваровНациональногоКаталога(ТоварыПорция, Настройки);
			
			Для Каждого КлючИЗначение Из ТоварыПорция Цикл
				ЗаполнитьЗначенияСвойств(Подписано.Добавить(), КлючИЗначение.Значение);
			КонецЦикла;
			ТоварыПорция.Очистить();
		КонецЕсли;
	КонецЦикла;
	
	Если ТоварыПорция.Количество() > 0 Тогда
		ПодписатьПорциюТоваровНациональногоКаталога(ТоварыПорция, Настройки);
	...
	Попытка
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.СостоянияВыгрузкиНоменклатуры");
		ЭлементБлокировкиДанных.ИсточникДанных = Подписано;
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Организация", "Организация");
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("Номенклатура", "Номенклатура");
		ЭлементБлокировкиДанных.ИспользоватьИзИсточникаДанных("ХарактеристикаНоменклатуры", "ХарактеристикаНоменклатуры");
		БлокировкаДанных.Заблокировать();
		
		Для Каждого СтрокаТаблицыЗначений Из Подписано Цикл
			Запись = РегистрыСведений.СостоянияВыгрузкиНоменклатуры.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(Запись, СтрокаТаблицыЗначений);
			Запись.ДатаСостояния = ТекущаяДатаСеанса();
			Если ЗначениеЗаполнено(Запись.ОписаниеОшибки) 
					И Прав(Запись.ОписаниеОшибки, 1) <> "." Тогда
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Национальный каталог.Подписать карточки товаров'",
				ОбщегоНазначения.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
	...
	Если ЗначениеЗаполнено(АдресРезультата) Тогда
		...
		Результат.Вставить("Успешно", 
			(Подписано.Найти(Перечисления.СостоянияВыгрузкиНоменклатуры.ОжидаетПодписания, "Состояние") = Неопределено));
		Результат.Вставить("СтатистикаРезультатов", ?(ОбновитьСтатистику, 
			РаботаСНоменклатуройСлужебный.СтатистикаРезультатовВыгрузки(Организация), Неопределено));
...
Процедура СоздатьНоменклатуруИХарактеристики(Знач ДанныеТекущейНоменклатуры, ПараметрыТекущейНоменклатуры, СозданнаяНоменклатура, ТекущаяХарактеристика = Неопределено)
	...
	Попытка
		...
		Если ЭтоПустойИдентификатор(ШаблонДанныхНоменклатуры.ИдентификаторНоменклатуры) Тогда
			ШаблонДанныхНоменклатуры.ИдентификаторНоменклатуры = ДанныеТекущейНоменклатуры.ЗапрошенныйШтрихкод;
		КонецЕсли;
...
//  ТаблицаЗначений:
//   * Штрихкод - Строка
//   * НаименованиеНоменклатурыПолное - Строка
...
Функция РазвернутыеДанныеПоШтрихкодам(ДанныеПоШтрихкодам) Экспорт
	...
	ШтрихкодыСервиса.Колонки.Добавить("ИдентификаторНоменклатуры",      ОбщегоНазначения.ОписаниеТипаСтрока(36));
	ШтрихкодыСервиса.Колонки.Добавить("ИдентификаторХарактеристики",    ОбщегоНазначения.ОписаниеТипаСтрока(60));
...
Процедура ЗаполнитьДополнительныеДанныеНоменклатурыСервиса(ДанныеПоНоменклатуре, Отказ) Экспорт
...
Процедура УдалитьЛишниеКолонкиДанныхНоменклатурыСервиса(ДанныеПоНоменклатуре) Экспорт
...
Процедура ЗаполнитьСоответствиеРеквизитовИЗначенийНоменклатуры(СоответствиеРеквизитовИЗначений, 
	...
	Если ЗначениеЗаполнено(СоответствиеРеквизитовИЗначений)
		И ТипЗнч(ДанныеПоНоменклатуре.Категория) = Тип("Структура") Тогда
...
Процедура УстановитьСтатусыПроверкиНоменклатуры(ДанныеПоНоменклатуре, ИдентификаторыСПараметрами)
	...
	Для каждого СтрокаНоменклатуры Из ДанныеПоНоменклатуре Цикл
		...
		Если ТипЗнч(СтрокаНоменклатуры.Категория) = Тип("Структура")
			И СтрокаНоменклатуры.Категория.ВидыНоменклатурыИнформационнойБазы.Количество() = 0
		...
		Если ТипЗнч(СтрокаНоменклатуры.Категория) = Тип("Структура")
			И СтрокаНоменклатуры.Категория.ВидыНоменклатурыИнформационнойБазы.Количество() > 1 
			И Не ЗначениеЗаполнено(ДополнительныеПараметры.ВидНоменклатуры) Тогда
...
Функция КатегорииПоСтрокеПоиска(ПараметрыПоиска, Отказ)
	...
	КодКлассификатора = СтрЗаменить(ПоисковаяСтрока, " ", "");
	КодКлассификатора = СтрЗаменить(КодКлассификатора, Символы.НПП, "");
	Если ПараметрыПоиска.ПоискПоТНВЭД
			И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодКлассификатора)
			И СтрДлина(КодКлассификатора) = 10 Тогда
		КлассификаторТНВЭД = ПоисковыйЗапрос.Классификаторы.Добавить();
		КлассификаторТНВЭД.Код          = Сред(КодКлассификатора, 1, 4) + " " + Сред(КодКлассификатора, 5, 2) + " "
		 									+ Сред(КодКлассификатора, 7, 3) + " " + Сред(КодКлассификатора, 10, 1);
		КлассификаторТНВЭД.Наименование = "TNVED";
		
		ПоисковыйЗапрос.ТолькоКонечныеКатегории = Истина;
		ПоисковыйЗапрос.ПоисковаяСтрока         = "";
	КонецЕсли;
...
Функция КлючиКоллекцииКакМассив(ДанныеДляПреобразования, ТолькоНепустыеКлючи = Ложь) Экспорт
	
	Результат = Новый Массив;
	
	Для каждого ЭлементКоллекции Из ДанныеДляПреобразования Цикл
		Если ТолькоНепустыеКлючи И Не ЗначениеЗаполнено(ЭлементКоллекции.Ключ) Тогда
			Продолжить;
		КонецЕсли;
...
#Область ПоискИСозданиеНоменклатурыПоШтрихкодам

Функция ЗначениеПустогоИдентификатора()
	
	Возврат "0";
	
КонецФункции

Функция ЭтоПустойИдентификатор(Знач Идентификатор)
	
	Возврат ПустаяСтрока(Идентификатор) Или Идентификатор = ЗначениеПустогоИдентификатора();
	
КонецФункции

Процедура НайтиНоменклатуруПоШтрихкодамВИнформационнойБазе(Штрихкоды, Результат, КонтекстПроверки)
	
	Если Штрихкоды.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДопустимаяДлинаGTIN = Новый Массив();
	ДопустимаяДлинаGTIN.Добавить(8);
	ДопустимаяДлинаGTIN.Добавить(12);
	ДопустимаяДлинаGTIN.Добавить(13);
	ДопустимаяДлинаGTIN.Добавить(14);
	
	ШтрихкодыДляПоиска   = КлючиКоллекцииКакМассив(Штрихкоды, Истина);
	СоответствиеКлючей   = Новый Соответствие;
	КоличествоШтрихкодов = ШтрихкодыДляПоиска.Количество();
	Для ОбратныйИндекс = 1 По КоличествоШтрихкодов Цикл
		ТекущийШтрихкод = ШтрихкодыДляПоиска[КоличествоШтрихкодов - ОбратныйИндекс];
		Для Каждого ДлинаGTIN Из ДопустимаяДлинаGTIN Цикл
			ПроверитьДобавитьШтрихкодДляПоиска(ТекущийШтрихкод, ДлинаGTIN, ШтрихкодыДляПоиска, СоответствиеКлючей);
		КонецЦикла;
	КонецЦикла;
	НайденоВИнформационнойБазе = РаботаСНоменклатуройСлужебный.НовыеДанныеНоменклатурыПоШтрихкодам();
	РаботаСНоменклатуройПереопределяемый.ПолучитьНоменклатуруПоШтрихкодам(ШтрихкодыДляПоиска, 
		НайденоВИнформационнойБазе);
	ОбщегоНазначенияБЭД.СтруктураТаблицыСоответствуетЭталонной(НайденоВИнформационнойБазе, 
		РаботаСНоменклатуройСлужебный.НовыеДанныеНоменклатурыПоШтрихкодам(), 
		КонтекстПроверки);
	Для Каждого НайденнаяНоменклатура Из НайденоВИнформационнойБазе Цикл
		Описание = РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду();
		ЗаполнитьЗначенияСвойств(Описание.Результат, НайденнаяНоменклатура);
		Ключ = СоответствиеКлючей[НайденнаяНоменклатура.Штрихкод];
		Ключ = ?(Ключ = Неопределено, НайденнаяНоменклатура.Штрихкод, Ключ);
		Результат.Вставить(Ключ, Описание);
		Штрихкоды.Удалить(Ключ);
	КонецЦикла;
	
КонецПроцедуры

Процедура ПроверитьДобавитьШтрихкодДляПоиска(ПроверяемыйШтрихкод, Длина, ВсеШтрихкоды, СоответствиеШтрихкодов)
	
	Если СтрДлина(ПроверяемыйШтрихкод) < Длина Тогда
		ШтрихкодНужнойДлины = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ПроверяемыйШтрихкод, Длина);
		ВсеШтрихкоды.Добавить(ШтрихкодНужнойДлины);
		СоответствиеШтрихкодов.Вставить(ШтрихкодНужнойДлины, ПроверяемыйШтрихкод);
	КонецЕсли;
	
КонецПроцедуры

Процедура НайтиСоздатьНоменклатуруПоШтрихкодамВСервисе(Штрихкоды, Результат, КонтекстПроверки, ПараметрыЗагрузки)
	
	Если Штрихкоды.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СоответствиеИдентификаторовШтрихкодам = Новый Соответствие();
	ДанныеПоНоменклатуре                  = ДанныеНоменклатурыСервисаПоШтрихкодам(Штрихкоды, 
		СоответствиеИдентификаторовШтрихкодам, Результат);
	
	Если Штрихкоды.Количество() > 0 Тогда
		ДополнитьДанныеСервисаДаннымиНациональногоКаталога(ДанныеПоНоменклатуре, Штрихкоды, ПараметрыЗагрузки);
		Для Каждого Элемент Из Штрихкоды Цикл
			СоответствиеИдентификаторовШтрихкодам.Вставить(Элемент.Ключ, Элемент.Ключ);
		КонецЦикла;
	КонецЕсли;
	...
	НаЗагрузку = ДанныеПоНоменклатуре.Скопировать(ДанныеПоНоменклатуре.НайтиСтроки(Новый Структура("Ошибка", "")));
	
	Если НаЗагрузку.Количество() > 0 Тогда
		СоздатьНоменклатуруПоДаннымСервиса(НаЗагрузку, СоответствиеИдентификаторовШтрихкодам, Результат, 
			КонтекстПроверки);
	КонецЕсли;
	
	Для Каждого ЭлементДанных Из ДанныеПоНоменклатуре Цикл
		Если Не ПустаяСтрока(ЭлементДанных.Ошибка) Тогда
			Описание = РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду();
			Описание.Отказ = Истина;
			Описание.Ошибка = ЭлементДанных.Ошибка;
			Результат.Вставить(ЭлементДанных.ЗапрошенныйШтрихкод, Описание);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура СоздатьНоменклатуруПоДаннымСервиса(ДанныеПоНоменклатуре, СоответствиеИдентификаторовШтрихкодам, Результат,
	КонтекстПроверки)
	...
	ИдентификаторыСПараметрами   = Новый Соответствие;
	СозданнаяНоменклатура        = Новый Массив;
	КоличествоЭлементовКСозданию = 0;            // количество отработанных идентификаторов номенклатуры
	НоменклатураДляЗагрузки      = Новый Массив; // массив строк таблицы данных номенклатуры
	ПодготовитьПараметрыЗагрузкиНоменклатуры(ДанныеПоНоменклатуре.ВыгрузитьКолонку("Идентификатор"),
		ИдентификаторыСПараметрами, НоменклатураДляЗагрузки, ДанныеПоНоменклатуре, КонтекстПроверки);
	ЗагрузитьОтобраннуюНоменклатуру(ДанныеПоНоменклатуре, НоменклатураДляЗагрузки, 
		СозданнаяНоменклатура, ИдентификаторыСПараметрами, КоличествоЭлементовКСозданию);
	
	Для Каждого ОписаниеНоменклатуры Из СозданнаяНоменклатура Цикл
		Штрихкод = СоответствиеИдентификаторовШтрихкодам[ОписаниеНоменклатуры.ИдентификаторНоменклатуры];
		Если ЗначениеЗаполнено(Штрихкод) Тогда
			Описание = РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду();
			ЗаполнитьЗначенияСвойств(Описание.Результат, ОписаниеНоменклатуры);
			Результат.Вставить(Штрихкод, Описание);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

#Область ПоискИСозданиеПоШтрихкодамВСервисе
...
//
// Возвращаемое значение:
//  см. РазвернутыеДанныеПоШтрихкодам
//
Функция РезультатПоискаНоменклатурыВСервисеПоШтрихкодам(Штрихкоды, КонтекстПроверки = "")
	
	Результат = Неопределено;
	
	ШтрихкодыДляПоиска = КлючиКоллекцииКакМассив(Штрихкоды, Истина);
	
	Если ШтрихкодыДляПоиска.Количество() = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Отказ = Ложь;
	НоменклатураСервисаКратко = НоменклатураПоШтрихкодам(ШтрихкодыДляПоиска, Отказ);
	Если Не Отказ И ТипЗнч(НоменклатураСервисаКратко) = Тип("ТаблицаЗначений") Тогда 
		Результат = РазвернутыеДанныеПоШтрихкодам(НоменклатураСервисаКратко);
	КонецЕсли;
...
// Возвращает ДанныеНоменклатурыСервиса
// 
// Параметры:
//  Штрихкоды - Соответствие из КлючИЗначение:
//   * Ключ - Строка - штрихкод.
//   * Значение - Произвольный - дополнительные параметры создания номенклатуры, передаваемые в прикладной контекст.
//  СоответствиеИдентификаторовШтрихкодам - Соответствие из КлючИЗначение:
//   * Ключ - Строка - идентификатор номенклатуры.
//   * Значение - Строка - штрихкод номенклатуры.
//  Результат - Соответствие из КлючИЗначение:
//    * Ключ - Строка - штрихкод.
//    * Значение - см. РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду.
// 
// Возвращаемое значение:
//  см. ДанныеНоменклатурыСервиса
// 
Функция ДанныеНоменклатурыСервисаПоШтрихкодам(Штрихкоды, СоответствиеИдентификаторовШтрихкодам, Результат)
	
	ДанныеПоНоменклатуре = Неопределено;
	НоменклатураСервиса  = РезультатПоискаНоменклатурыВСервисеПоШтрихкодам(Штрихкоды);
	// нужно ли проверять соответствия по id, если найдены - дописывать ШК ?
	ИдентификаторыНоменклатуры = Новый Массив;
	Если НоменклатураСервиса <> Неопределено Тогда
		Для Каждого Элемент Из НоменклатураСервиса Цикл
			СоответствиеИдентификаторовШтрихкодам.Вставить(Элемент.ИдентификаторНоменклатуры, Элемент.Штрихкод);
			ИдентификаторыНоменклатуры.Добавить(Элемент.ИдентификаторНоменклатуры);
		КонецЦикла;
	КонецЕсли;
	...
	Отказ = Ложь;
	РезультатПокупки = ПокупкаКарточекНоменклатуры(ИдентификаторыНоменклатуры, Отказ); 
	КарточкиКуплены  = НЕ Отказ 
		И Не РезультатПокупки.ЕстьОшибки
		И ТипЗнч(РезультатПокупки) = Тип("Структура");
	
	Если КарточкиКуплены Тогда
		ДанныеПоНоменклатуре = ДанныеНоменклатурыСервиса(ИдентификаторыНоменклатуры);
	КонецЕсли;
	
	Если ДанныеПоНоменклатуре <> Неопределено Тогда
		Для Каждого СтрокаДанных Из ДанныеПоНоменклатуре Цикл
			ТекущийШтрихкод = СоответствиеИдентификаторовШтрихкодам[СтрокаДанных.Идентификатор];
			СтрокаДанных.ДополнительныеПараметры = Штрихкоды[ТекущийШтрихкод];
			Штрихкоды.Удалить(ТекущийШтрихкод);
		КонецЦикла;
	Иначе
		Ошибка = ?(КарточкиКуплены, ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().НеизвестнаяОшибкаСервиса, 
			ОшибкиЗагрузкиНоменклатурыСервисаПоШтрихкодам().ОшибкаПокупкиКарточек);
		Для Каждого Элемент Из СоответствиеИдентификаторовШтрихкодам Цикл
			ТекущийШтрихкод = Элемент.Значение;
			Описание        = РаботаСНоменклатуройСлужебный.НовоеОписаниеНоменклатурыПоШтрихкоду();
			Описание.Отказ  = Истина;
			Описание.Ошибка = Ошибка;
			Результат.Вставить(ТекущийШтрихкод, Описание);
			Штрихкоды.Удалить(ТекущийШтрихкод);
		КонецЦикла;
	КонецЕсли;
...
Функция ТекстЗапросаСуществующиеИдентификаторыКатегорий()
...
// Параметры:
//  ДанныеПоНоменклатуре - см. ДанныеНоменклатурыСервиса
//  КонтекстПроверки - Строка
//
Процедура НайтиЗагрузитьВидыНоменклатурыИДополнительныеРеквизиты(ДанныеПоНоменклатуре, КонтекстПроверки = "")
	
	ВсеИдентификаторы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(
		ДанныеПоНоменклатуре.ВыгрузитьКолонку("ИдентификаторКатегории"));
	
	Запрос = Новый Запрос(ТекстЗапросаСуществующиеИдентификаторыКатегорий());
	Запрос.УстановитьПараметр("ИдентификаторыКатегорий", ВсеИдентификаторы);
	
	СуществующиеИдентификаторы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ИдентификаторКатегории");
	НовыеИдентификаторы        = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ВсеИдентификаторы, 
		СуществующиеИдентификаторы);
	
	ДанныеКатегорий = ДанныеКатегорийСервиса(НовыеИдентификаторы);
	Если ДанныеКатегорий <> Неопределено Тогда 
		НовыеЭлементы = Новый Массив();
		ЗагрузитьКатегорииПоОтдельности(ДанныеКатегорий, 0, НовыеЭлементы);
	КонецЕсли;
	
	Если СуществующиеИдентификаторы.Количество() > 0 Тогда
		РаботаСНоменклатуройСлужебный.РазобратьИзмененияВидовНоменклатуры(СуществующиеИдентификаторы, 
			Неопределено,, Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПодготовитьПараметрыЗагрузкиНоменклатуры(ИдентификаторыНоменклатуры, ИдентификаторыСПараметрами,
	НоменклатураДляЗагрузки, ДанныеПоНоменклатуре, КонтекстПроверки)
	
	ИдентификаторыНоменклатурыДляЗагрузки = Новый Массив;
	НоменклатураДляЗагрузкиХарактеристик  = Новый Соответствие; 
	
	ИдентификаторыКратко   = Новый Массив;
	ИдентификаторыПодробно = Новый Массив;
	Для Каждого ИдентификаторНоменклатуры Из ИдентификаторыНоменклатуры Цикл
		Шаблон = РаботаСНоменклатуройСлужебныйКлиентСервер.ШаблонДанныхНоменклатуры();
		Шаблон.ИдентификаторНоменклатуры = ИдентификаторНоменклатуры;
		ИдентификаторыПодробно.Добавить(Шаблон);
	КонецЦикла;
	
	ПараметрыСозданияНоменклатуры = Новый Структура;
	ПараметрыСозданияНоменклатуры.Вставить("Идентификаторы", ИдентификаторыПодробно);
	ПараметрыСозданияНоменклатуры.Вставить("ДополнительныеПараметры", Новый Соответствие);
	ПодготовитьДанныеДляСозданияНоменклатуры(ПараметрыСозданияНоменклатуры, ИдентификаторыКратко, 
		ИдентификаторыСПараметрами, НоменклатураДляЗагрузкиХарактеристик);
		
	Если ВедетсяУчетВидовНоменклатуры() Тогда
		НайтиЗагрузитьВидыНоменклатурыИДополнительныеРеквизиты(ДанныеПоНоменклатуре, КонтекстПроверки);
		ПодготовитьИдентификаторыНоменклатурыДляЗагрузки(
			ДанныеПоНоменклатуре, ИдентификаторыСПараметрами, ИдентификаторыНоменклатурыДляЗагрузки);
	КонецЕсли;
...
// Выполняет поиск данных номенклатуры в Национальном каталоге по штрихкодам и добавляет результат в таблицу данных
// сервиса.
// 
// Параметры:
//  ДанныеПоНоменклатуре - см. ДанныеНоменклатурыСервиса
...
//  ПараметрыЗагрузки - см. РаботаСНоменклатуройКлиентСервер.НовыеПараметрыПолученияНоменклатурыСервисаПоШтрихкодам
//
Процедура ДополнитьДанныеСервисаДаннымиНациональногоКаталога(ДанныеПоНоменклатуре, Штрихкоды, ПараметрыЗагрузки)
	
	Если Штрихкоды.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеНациональногоКаталога = РаботаСНоменклатуройСлужебный.ДанныеНоменклатурыНациональногоКаталогаПоШтрихкодам(
		Штрихкоды,
		ПараметрыЗагрузки);
	
	Если ДанныеНациональногоКаталога = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ДанныеПоНоменклатуре = Неопределено Тогда
		ДанныеПоНоменклатуре = ДанныеНациональногоКаталога;
	Иначе 
		Для Каждого СтрокаДанных Из ДанныеНациональногоКаталога Цикл
			ЗаполнитьЗначенияСвойств(ДанныеПоНоменклатуре.Добавить(), СтрокаДанных);
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

// Возвращает штрихкод без пробелов и лидирующих нулей
// 
// Параметры:
//  Штрихкод - Строка
// 
...
//  Строка
// 
Функция ШтрихкодДляПоиска(Знач Штрихкод)
	
	ШтрихкодДляПоиска = СокрЛП(Штрихкод);
	ШтрихкодДляПоиска = СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(ШтрихкодДляПоиска, "0");
	
	Возврат ШтрихкодДляПоиска;
	
КонецФункции

Функция РазвернутьРезультат(Знач Результат, Знач СоответствиеШтрихкодов)
	
	РазвернутыйРезультат = Новый Соответствие();
	
	Для Каждого КлючИЗначение Из Результат Цикл
		ИсходныеКлючи = СоответствиеШтрихкодов[КлючИЗначение.Ключ];
		Если ИсходныеКлючи = Неопределено Тогда
			РазвернутыйРезультат.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
		Иначе
			Для Каждого ИсходныйКлюч Из ИсходныеКлючи Цикл
				РазвернутыйРезультат.Вставить(ИсходныйКлюч, КлючИЗначение.Значение);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Возврат РазвернутыйРезультат;
...
Функция ЭтоВалидныйШтрихкодНациональногоКаталога(Знач Штрихкод)
	...
	Если ДлинаШтрихкода < 6 
			Или ДлинаШтрихкода > 14
			Или ДлинаШтрихкода = 9 
			Или ДлинаШтрихкода = 10
	...
	// Исключение: 9600 - 9604 (были присвоены GS1 UK под номера GTIN-8).
...
//
// Параметры:
//   GTIN - Строка - Текстовая строка с GTIN.
// 
// Возвращаемое значение:
//   Число - Контрольный символ, рассчитанный по алгоритму для GTIN.
//
...
Функция ОбработатьТоварыНациональногоКаталогаКПодписанию(Товары, Штрихкоды, Настройки)
	
	Отказ = Ложь;
	Результат = РаботаСНоменклатуройСлужебный.ПолучитьТоварыНациональногоКаталогаКПодписанию(Настройки, 
		, 
		Штрихкоды, 
		Отказ);
	...
	Если Не Отказ 
		...
		Если Результат.Свойство("xmls") Тогда
			Для Каждого ЭлементКоллекции Из Результат.xmls Цикл
				Если ЭлементКоллекции.Свойство("gtin")
						И ЗначениеЗаполнено(ЭлементКоллекции.gtin) Тогда
					Отбор = Новый Структура("Штрихкод", 
						ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ЭлементКоллекции.gtin));
					НайденныеСтроки = Товары.НайтиСтроки(Отбор);
					
					Если НайденныеСтроки.Количество() > 0 Тогда
						ЭлементКоллекции.Свойство("goodId", НайденныеСтроки[0].ИдентификаторТовара);
						ЭлементКоллекции.Свойство("xml",    НайденныеСтроки[0].КарточкаТовара);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если Результат.Свойство("errors") Тогда
			Для Каждого ЭлементКоллекции Из Результат.errors Цикл
				Если ЭлементКоллекции.Свойство("gtin")
						И ЗначениеЗаполнено(ЭлементКоллекции.gtin) Тогда
					Отбор = Новый Структура("Штрихкод", 
						ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ЭлементКоллекции.gtin));
					...
					Если НайденныеСтроки.Количество() > 0 Тогда
						ЭлементКоллекции.Свойство("goodId",  НайденныеСтроки[0].ИдентификаторТовара);
						ЭлементКоллекции.Свойство("message", НайденныеСтроки[0].ОписаниеОшибки);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Иначе
		...
		Если ТипЗнч(Результат) = Тип("Структура") Тогда
			Результат.Свойство("ТекстСообщения", ТекстОшибки);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ТекстОшибки) Тогда
			ТекстОшибки = НСтр("ru = 'Ошибка выполнения запроса к сервису Национального каталога. Подробности см. в журнале регистрации.'");
		КонецЕсли;
		
		Для Каждого СтрокаТаблицыЗначений Из Товары Цикл
			Если Не ЗначениеЗаполнено(СтрокаТаблицыЗначений.ОписаниеОшибки) Тогда
				СтрокаТаблицыЗначений.ОписаниеОшибки = ТекстОшибки;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ОбработатьСостояниеОпубликованТоваровНациональногоКаталога(Товары, Штрихкоды, Настройки)
	
	Отказ = Ложь;
	Результат = РаботаСНоменклатуройСлужебный.ПолучитьСобственныеТоварыНациональногоКаталога(Настройки, 
		, 
		Штрихкоды, 
		Отказ);
	...
	Если Не Отказ 
		...
		Для Каждого Товар Из Результат Цикл
			Если Товар.Свойство("identified_by")
					И Товар.Свойство("good_status")
					И ВРег(Товар.good_status) = ВРег("published")
					И (Не Товар.Свойство("good_detailed_status")
						Или Товар.Свойство("good_detailed_status")
							И Товар.good_detailed_status.Количество() = 1
							И ВРег(Товар.good_detailed_status[0]) = ВРег("published")) Тогда
				Для Каждого ЭлементКоллекции Из Товар.identified_by Цикл
					Если ЭлементКоллекции.Свойство("value")
							И ЗначениеЗаполнено(ЭлементКоллекции.value) Тогда
						Отбор = Новый Структура("Штрихкод", 
							ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ЭлементКоллекции.value));
						НайденныеСтроки = Товары.НайтиСтроки(Отбор);
						
						Если НайденныеСтроки.Количество() > 0 Тогда
							НайденныеСтроки[0].Состояние         = Перечисления.СостоянияВыгрузкиНоменклатуры.Принята;
							НайденныеСтроки[0].ОписаниеОшибки    = "";
							НайденныеСтроки[0].ОбновитьВРегистре = Истина;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Функция ПодписатьПорциюТоваровНациональногоКаталога(ДанныеДляПодписания, Настройки)
	
	Отказ = Ложь;
	Результат = РаботаСНоменклатуройСлужебный.ПодписатьТоварыНациональногоКаталога(Настройки, 
		ДанныеДляПодписания, 
		Отказ);
	
	Если Не Отказ 
			И ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("signed") Тогда
			Для Каждого ЭлементКоллекции Из Результат.signed Цикл
				Товар = ДанныеДляПодписания.Получить(ЭлементКоллекции);
				Если Товар <> Неопределено Тогда
					Товар.Вставить("Состояние",      Перечисления.СостоянияВыгрузкиНоменклатуры.Принята);
					Товар.Вставить("ОписаниеОшибки", "");
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Если Результат.Свойство("errors") Тогда
			Для Каждого ЭлементКоллекции Из Результат.errors Цикл
				Если ЭлементКоллекции.Свойство("goodId")
						И ЗначениеЗаполнено(ЭлементКоллекции.goodId)
						И ЭлементКоллекции.Свойство("message") Тогда
					Товар = ДанныеДляПодписания.Получить(ЭлементКоллекции.goodId);
					Если Товар <> Неопределено Тогда
						Товар.Вставить("ОписаниеОшибки", ЭлементКоллекции.message);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Иначе
		...
		Для Каждого КлючИЗначение Из ДанныеДляПодписания Цикл
			КлючИЗначение.Значение.Вставить("ОписаниеОшибки", ТекстОшибки);
		КонецЦикла;
	КонецЕсли;
