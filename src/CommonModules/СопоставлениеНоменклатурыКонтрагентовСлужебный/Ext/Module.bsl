...
Функция ВариантыСопоставленияНоменклатуры(НаборНоменклатурыКонтрагентов, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура ВыполнитьДействияПриЗаписиНоменклатураКонтрагентовБЭД(Источник, Отказ, Замещение) Экспорт
	...
	Если Источник.Количество() = 0 Тогда
		// сюда попадаем только если это "настоящее" (единственное, намеренное) удаление
	...
	ИначеЕсли Источник.Количество() > 0 Тогда
		...
		ПометкаНаУдаление = ОбщегоНазначения.ЭтоУдалениеНабораЗаписей(Замещение);
	Иначе
		Возврат;
	КонецЕсли;
...
//
// Возвращаемое значение:
//  Массив из см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовоеСоответствиеНоменклатуры - соответствия номенклатуры, найденное по правилам поиска по части
//   наименования.
//
		...
		Для Каждого Правило Из ПравилаПоВладельцу Цикл
			...
			Для ОбратныйИндекс = 1 По ВсегоНоменклатуры Цикл
				...
				Если СтрНайти(ТекущаяНоменклатура.Наименование, Правило.ПоисковаяСтрока) > 0 Тогда
					...
					Соответствие = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовоеСоответствиеНоменклатуры(
...
Функция ХешиНатуральныхКлючейНоменклатуры(Знач Наименование, Знач Артикул, Знач КодНоменклатуры,
	...
	Если СтрокаЗаполнена(Наименование) Тогда
		Ключи.ХешНаименования = ХешПоСтроке(Наименование);
	КонецЕсли;
	
	Если СтрокаЗаполнена(Артикул) Тогда
		Ключи.ХешАртикула = ХешПоСтроке(Артикул);
	КонецЕсли;
	
	Если СтрокаЗаполнена(КодНоменклатуры) Тогда
		Ключи.ХешКодаНоменклатуры = ХешПоСтроке(КодНоменклатуры);
	КонецЕсли;
	
	Если СтрокаЗаполнена(Штрихкод) Тогда
...
Процедура НоменклатураПриИзменении(Форма, ИмяРеквизита, ЕдиницаИзмеренияПоУмолчанию = Неопределено) Экспорт
	
	Элементы = Форма.Элементы;
	ИсходныеДанныеСопоставления = Форма[ИмяРеквизита];
	
	НовыеДанныеСопоставления = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовыеДанныеСопоставления();
	ЗаполнитьЗначенияСвойств(НовыеДанныеСопоставления, ИсходныеДанныеСопоставления);
	НовыеДанныеСопоставления.ЕдиницаИзмеренияПоУмолчанию = ЕдиницаИзмеренияПоУмолчанию;
	
	ИменаПараметровСвязи = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовыеИменаПараметровСвязи();
	СопоставлениеНоменклатурыКонтрагентовПереопределяемый.ПриОпределенииСтруктурыНоменклатурыИнформационнойБазы(
		ИменаПараметровСвязи);
	ЗаполнитьЗначенияСвойств(НовыеДанныеСопоставления, ИменаПараметровСвязи);

	СопоставлениеНоменклатурыКонтрагентовПереопределяемый.ПриИзмененииНоменклатурыСопоставления_НоменклатураКонтрагентов(
		Форма, НовыеДанныеСопоставления);
	
	ЗаполнитьЗначенияСвойств(ИсходныеДанныеСопоставления, НовыеДанныеСопоставления,, "Номенклатура");
	
	ОбязательноеЗаполнениеХарактеристики = НовыеДанныеСопоставления.ИспользоватьХарактеристики 
		И НовыеДанныеСопоставления.ОбязательноеЗаполнениеХарактеристики;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Характеристика", "АвтоОтметкаНезаполненного", 
		ОбязательноеЗаполнениеХарактеристики);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Характеристика", "Доступность",
		НовыеДанныеСопоставления.ИспользоватьХарактеристики);
	
	ЕдиницаИзмеренияПоУмолчанию = НовыеДанныеСопоставления.ЕдиницаИзмеренияПоУмолчанию;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Упаковка", "ПодсказкаВвода",
		ЕдиницаИзмеренияПоУмолчанию);
	
	ПутьКДанным = СтрШаблон("%1.Номенклатура", ИмяРеквизита);
	УстановитьСвязиПараметровВыбора(Элементы, "Характеристика", НовыеДанныеСопоставления.ИмяПараметраСвязиХарактеристики,
		ПутьКДанным);
	УстановитьСвязиПараметровВыбора(Элементы, "Упаковка", НовыеДанныеСопоставления.ИмяПараметраСвязиУпаковки, 
		ПутьКДанным);
...
Функция ПредставленияНоменклатурыКонтрагентов() Экспорт
	...
	ОписаниеТиповНоменклатура = Новый ОписаниеТипов(
		СопоставлениеНоменклатурыКонтрагентовПовтИсп.ИспользуемыеТипыНоменклатурыБЭД());
	...
	Представления.Вставить("НоменклатураПредставлениеОбъекта", 
		ПредставлениеОбъектаПоТипу(ОписаниеТиповНоменклатура, "Номенклатура"));
...
Процедура РазобратьНаименованиеДанныхСопоставленияНоменклатурыНаОтдельныеСлова(Знач НаименованиеИБ, ТаблицаСлов = Неопределено,
	...
	НоваяСтрока.СловоЧисло      = ЭтоЧисло(НаименованиеИБ);
	...
	// При исключении незначимых слов, см. ИсключитьИзТаблицыСловНезначимые условия должны обрабатываться по ИЛИ, 
	//  а именно: в итоговый набор слов попадают все слова, для которых 
	// СловоЧисло = Истина Или СловоИзЛатиницы = Истина Или ДлинаСлова > 2.
	// Наличие более одного условия для строки избыточно, приводит к дублированию строк и последующей ошибки записи.
	НоваяСтрока.СловоЧисло = ?(НоваяСтрока.ДлинаСлова > 2, Ложь, НоваяСтрока.СловоЧисло);
...
Функция ТекстЗапросаИсключитьИзТаблицыСловНезначимые()
	...
	|			ИСТИНА
	|		ИЗ
	|			втИсключенияНезначимые КАК ИсключенияНезначимые
	|		ГДЕ
...
Функция ПодобратьВариантыСопоставленияНоменклатуры(Знач ТаблицаНоменклатурыКонтрагентов, ПараметрыСопоставления)
	...
	ПрименитьОграничениеТипаНоменклатуры(ТаблицаНоменклатурыКонтрагентов, 
		ПараметрыСопоставления.ОграничениеТипаНоменклатуры);
...
Процедура ПрименитьОграничениеТипаНоменклатуры(ТаблицаНоменклатурыКонтрагентов, ОграничениеТипаНоменклатуры)
	
	Если ТипЗнч(ОграничениеТипаНоменклатуры) <> Тип("ОписаниеТипов")
		Или Не ОграничениеТипаНоменклатурыПрименимо(ОграничениеТипаНоменклатуры) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого Элемент Из ТаблицаНоменклатурыКонтрагентов Цикл
		Если ЗначениеЗаполнено(Элемент.НоменклатураИБ) 
			И Не ОграничениеТипаНоменклатуры.СодержитТип(ТипЗнч(Элемент.НоменклатураИБ)) Тогда
			Элемент.НоменклатураИБ = ОграничениеТипаНоменклатуры.ПривестиЗначение();
			Элемент.УпаковкаИБ = Неопределено;
			Элемент.ХарактеристикаИБ = Неопределено;
		КонецЕсли;
		ВсегоВариантов = Элемент.ВариантыСопоставленияНоменклатурыБЭД.Количество();
		Для ОбратныйИндекс = 1 По ВсегоВариантов Цикл
			ТекущийВариант = Элемент.ВариантыСопоставленияНоменклатурыБЭД[ВсегоВариантов - ОбратныйИндекс];
			Если Не ЗначениеЗаполнено(ТекущийВариант.НоменклатураИБ) 
				Или Не ОграничениеТипаНоменклатуры.СодержитТип(ТипЗнч(ТекущийВариант.НоменклатураИБ)) Тогда
				Элемент.ВариантыСопоставленияНоменклатурыБЭД.Удалить(ВсегоВариантов - ОбратныйИндекс);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПодобратьВариантыСопоставленияПоИдентификаторамНоменклатурыХарактеристики(ТаблицаНоменклатурыКонтрагентов,
	...
	УсловиеПоТипу = УсловиеПоТипуНоменклатуры("НоменклатураКонтрагентовБЭД.Номенклатура", ОграничениеТипаНоменклатуры);
	Запрос.Текст  = СтрЗаменить(Запрос.Текст,
		"&УсловиеПоТипуНоменклатуры", УсловиеПоТипу);
...
Функция ТекстЗапросаПоискаПоИдентификаторамНоменклатурыХарактеристики()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|		ПО НесопоставленныеТовары.ИдентификаторНоменклатуры = НоменклатураКонтрагентовБЭД.ИдентификаторНоменклатуры
	|			И НесопоставленныеТовары.Владелец = НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры
	|			И НесопоставленныеТовары.ИдентификаторХарактеристики = НоменклатураКонтрагентовБЭД.ИдентификаторХарактеристики
	|			И НесопоставленныеТовары.Идентификатор <> НоменклатураКонтрагентовБЭД.Идентификатор
	...
	|ВЫБРАТЬ
	|	НесопоставленныеТовары.ИдентификаторНоменклатуры КАК ИдентификаторНоменклатуры,
	|	НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры КАК Владелец,
	|	НоменклатураКонтрагентовБЭД.Номенклатура КАК Номенклатура,
...
Процедура ПоискВариантовСопоставленияНоменклатурыСНоменклатуройКонтрагентов(Знач Запрос, ТаблицаНоменклатурыКонтрагентов, Знач ТаблицаНатуральныхКлючей,
	...
	УсловиеПоТипу = УсловиеПоТипуНоменклатуры("СловарьСопоставленияНоменклатурыБЭД.СсылкаНаОбъект",
		ПараметрыСопоставления.ОграничениеТипаНоменклатуры);
	Запрос.Текст  = СтрЗаменить(Запрос.Текст, "&УсловиеПоТипуНоменклатуры", УсловиеПоТипу);
...
Процедура ИнициализацияТекстаЗапросаПоискаВСловареСопоставленияПоНоменклатуры(ТекстЗапроса, ПараметрыСопоставления)
	...
	УсловиеПоТипу = УсловиеПоТипуНоменклатуры("СловарьСопоставленияНоменклатурыБЭД.СсылкаНаОбъект", 
		ПараметрыСопоставления.ОграничениеТипаНоменклатуры);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоТипуНоменклатуры", УсловиеПоТипу);
...
//  Строка
//
Функция УсловиеПоТипуНоменклатуры(ИмяПоля, ОграничениеТипаНоменклатуры)
	
	Если ТипЗнч(ОграничениеТипаНоменклатуры) = Тип("ОписаниеТипов") 
		И ОграничениеТипаНоменклатурыПрименимо(ОграничениеТипаНоменклатуры) Тогда
		ВсеУсловия = Новый Массив;
		Для Каждого Элемент Из ОграничениеТипаНоменклатуры.Типы() Цикл
			ТекущийТип = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Элемент));
			ВсеУсловия.Добавить(СтрШаблон("%1 ССЫЛКА %2", 
				ИмяПоля,
				ОбщегоНазначения.ИмяТаблицыПоСсылке(ТекущийТип.ПривестиЗначение())));
		КонецЦикла;
		УсловиеПоТипу = СтрШаблон("(%1)", СтрСоединить(ВсеУсловия, " ИЛИ "));
	Иначе
		УсловиеПоТипу = "ИСТИНА";
	КонецЕсли;
	
	Возврат УсловиеПоТипу;
	
КонецФункции

Функция ОграничениеТипаНоменклатурыПрименимо(ОграничениеТипаНоменклатуры)
	
	Возврат ТипЗнч(ОграничениеТипаНоменклатуры) = Тип("ОписаниеТипов")
		И ЗначениеЗаполнено(ОграничениеТипаНоменклатуры)
		И СопоставлениеНоменклатурыКонтрагентовПовтИсп.ИспользуемыеТипыНоменклатурыБЭД().Количество() > 1
...
Функция ТекстЗапросаПоискаПоНоменклатуреКонтрагента()
	...
	|		НоменклатураКонтрагентовБЭД.Номенклатура КАК Номенклатура,
	|		ВЫБОР
	|			КОГДА втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры
	|				ТОГДА &ВесНаименованияПоВладельцу
	|			ИНАЧЕ &ВесНаименованияБезВладельца
	|		КОНЕЦ КАК Вес
	|	ИЗ
	|		втДанныеНоменклатуры КАК втДанныеНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.ХешНаименования = НоменклатураКонтрагентовБЭД.ХешНаименования
	...
	|		ВЫБОР
	|			КОГДА втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры
	|				ТОГДА &ВесАртикулаПоВладельцу
	|			ИНАЧЕ &ВесАртикулаБезВладельца
	|		КОНЕЦ
	|	ИЗ
	|		втДанныеНоменклатуры КАК втДанныеНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.ХешАртикула = НоменклатураКонтрагентовБЭД.ХешАртикула
	|			И &УсловиеПроверкиПустойНоменклатуры
	|			И &УсловиеПоТипуНоменклатуры
	|	ГДЕ
	...
	|		втДанныеНоменклатуры КАК втДанныеНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры
	...
	|		ВЫБОР
	|			КОГДА втДанныеНоменклатуры.Владелец = НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры
	|				ТОГДА &ВесШтрихкодаПоВладельцу
	|			ИНАЧЕ &ВесШтрихкодаБезВладельца
	|		КОНЕЦ
	|	ИЗ
	|		втДанныеНоменклатуры КАК втДанныеНоменклатуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|			ПО втДанныеНоменклатуры.ХешШтрихкода = НоменклатураКонтрагентовБЭД.ХешШтрихкода
	|			И &УсловиеПроверкиПустойНоменклатуры
	|			И &УсловиеПоТипуНоменклатуры
	|	ГДЕ
	...
	|	МАКСИМУМ(втВарианты.Вес) КАК Вес
	|ПОМЕСТИТЬ втМаксимальныеВеса
	|ИЗ
	|	втВарианты КАК втВарианты
	|СГРУППИРОВАТЬ ПО
	|	втВарианты.Владелец,
	|	втВарианты.Идентификатор
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Владелец,
	|	Идентификатор,
	...
	|	втВарианты.Владелец КАК Владелец,
	|	втВарианты.Идентификатор КАК Идентификатор,
	|	МАКСИМУМ(втВарианты.Номенклатура) КАК Номенклатура
	|ИЗ
	|	втМаксимальныеВеса КАК втМаксимальныеВеса
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВарианты КАК втВарианты
	|		ПО втМаксимальныеВеса.Владелец = втВарианты.Владелец
	|		И втМаксимальныеВеса.Идентификатор = втВарианты.Идентификатор
	|		И втМаксимальныеВеса.Вес = втВарианты.Вес
	|СГРУППИРОВАТЬ ПО
	|	втВарианты.Владелец,
	|	втВарианты.Идентификатор";
...
Функция ВесаНатуральныхКлючей()
	
	Веса = Новый Структура;
	Веса.Вставить("ВесНаименованияПоВладельцу", 2);
	Веса.Вставить("ВесНаименованияБезВладельца", 1);
	Веса.Вставить("ВесАртикулаПоВладельцу", 4);
	Веса.Вставить("ВесАртикулаБезВладельца", 2);
	Веса.Вставить("ВесКодаПоВладельцу", 4);
	Веса.Вставить("ВесШтрихкодаПоВладельцу", 10);
	Веса.Вставить("ВесШтрихкодаБезВладельца", 5);
...
Функция РезультатПоискаИСравненияИзмененийНоменклатурыКонтрагентов(ТаблицаНоменклатурыКонтрагентов,
	...
	УсловиеПоТипу = УсловиеПоТипуНоменклатуры("НоменклатураКонтрагентов.Номенклатура", ОграничениеТипаНоменклатуры);
	Запрос.Текст  = СтрЗаменить(Запрос.Текст, "&УсловиеПоТипуНоменклатуры", УсловиеПоТипу);
...
// 
Функция ПравилаПоискаПоНоменклатуреКонтрагента(Знач НоменклатураКонтрагента, ОграничениеТипаНоменклатуры = Неопределено)
	
	ТекстЗапроса  = ТекстЗапросаПравилаПоискаПоНоменклатуреКонтрагента();
	УсловиеПоТипу = УсловиеПоТипуНоменклатуры("ПравилаПоискаПоЧастиНаименованияБЭД.Номенклатура",
		ОграничениеТипаНоменклатуры);
	ТекстЗапроса  = СтрЗаменить(ТекстЗапроса, "&УсловиеПоТипуНоменклатуры", УсловиеПоТипу);
	Запрос = Новый Запрос(ТекстЗапроса);
...
Процедура СохранитьСоответствиеНоменклатуры(СоответствиеНоменклатуры, ВсяНоменклатураИБ)
	...
	ДополнительныеПараметры = 
		СопоставлениеНоменклатурыКонтрагентов.НовыеДополнительныеПараметрыПриЗаписиНоменклатурыКонтрагентов();
	ДополнительныеПараметры.ТребуетсяЗаписьВРегистр = Истина;
	ДополнительныеПараметры.ТребуетсяПоискСсылки = Истина;
	ДополнительныеПараметры.ТребуетсяПоискЕдиницыИзмеренияПоОКЕИ = Истина;
	ДополнительныеПараметры.ДанныеРегистраНоменклатурыКонтрагентов = Ложь;
	...
	Для Каждого Элемент Из СоответствиеНоменклатуры Цикл
		...
		Отказ = Ложь;
		СопоставлениеНоменклатурыКонтрагентов.СоздатьОбновитьНоменклатуруКонтрагента(Элемент.НоменклатураКонтрагента,
...
Процедура ПодобратьВариантыСопоставленияПоПравиламПоискаПоЧастиНаименования(ТаблицаНоменклатурыКонтрагентов,
	...
	Правила   = ПравилаПоискаПоНоменклатуреКонтрагента(ТаблицаНоменклатурыКонтрагентов, ОграничениеТипаНоменклатуры);
	Владельцы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(Правила.ВыгрузитьКолонку("Владелец"));
...
Процедура УстановитьСвязиПараметровВыбора(ЭлементыФормы, ИмяЭлементаФормы, ИмяПараметраСвязи, ПутьКДанным)

	ВсеСвязи = Новый Массив();
	Если ЗначениеЗаполнено(ИмяПараметраСвязи) Тогда
		НоваяСвязь = Новый СвязьПараметраВыбора(ИмяПараметраСвязи, ПутьКДанным);
		ВсеСвязи.Добавить(НоваяСвязь);
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(ЭлементыФормы, "Упаковка", "СвязиПараметровВыбора",
		Новый ФиксированныйМассив(ВсеСвязи));

КонецПроцедуры

//-- Локализация

Функция СтрокаЗаполнена(ПроверяемаяСтрока)
	
	РегулярноеВыражение = "\p{L}|\d"; // проверяет, что строка содержит хотя бы один символ или цифру
	
	Возврат СтрНайтиПоРегулярномуВыражению(ПроверяемаяСтрока, РегулярноеВыражение).НачальнаяПозиция > 0;
...
//
Функция ПредставлениеОбъектаПоТипу(Знач ОписаниеТипа, Знач ПредставлениеПоУмолчанию = "") Экспорт
