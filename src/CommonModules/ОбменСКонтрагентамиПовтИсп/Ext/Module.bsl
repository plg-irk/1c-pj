...
// Сведения задаются в макете Обработки.ОбменСКонтрагентами.ВерсииФорматовЭД.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - состав и типы колонок указаны в макете Обработки.ОбменСКонтрагентами.ВерсииФорматовЭД.
//
Функция СведенияОФорматахЭлектронныхДокументов() Экспорт

	Результат = Новый ТаблицаЗначений;
	Макет = Обработки.ОбменСКонтрагентами.ПолучитьМакет("ВерсииФорматовЭД");
				...
				Если ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
					...
					Иначе
						...
						СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, 
							Метаданные.Обработки.ОбменСКонтрагентами.Макеты.ВерсииФорматовЭД.Синоним, ПолноеИмяОбъекта);
...
Функция СтавкаНДСЧислом(СтрокаСтавкаНДС) Экспорт
	...
	Если ПозицияСимвола > 0 Тогда
		СтавкаЧислом = Окр(РаботаВБезопасномРежиме.ВычислитьВБезопасномРежиме(СтавкаНДС) * 100, 4);
	Иначе
