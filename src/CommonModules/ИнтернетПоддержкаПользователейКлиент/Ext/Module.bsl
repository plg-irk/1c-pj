...
Процедура ПриНачалеРаботыСистемы() Экспорт
	...
	// тк + 
	Если ПараметрыРаботыКлиентаПриЗапуске.ДоступноИспользованиеРазделенныхДанных И ПараметрыРаботыКлиентаПриЗапуске.Свойство("ИнтернетПоддержкаПользователей") Тогда
	// тк - 
...
Процедура СтартоватьМеханизмИзМеню(СтартовыеПараметры = Неопределено) Экспорт
	...
	Если НЕ ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.МониторИнтернетПоддержки") Тогда
		...
		ВызватьИсключение НСтр("ru = 'Ошибка встраивания. Отсутствует подсистема ""Монитор Интернет-поддержки"".'");

	Иначе

		МодульМониторИПП = ОбщегоНазначенияКлиент.ОбщийМодуль("МониторИнтернетПоддержкиКлиент");
		МодульМониторИПП.ОткрытьМониторИнтернетПоддержки(СтартовыеПараметры);
...
Процедура ОткрытьВебСтраницуСДополнительнымиПараметрами(
	...
	Если НеобходимаАвторизации
		...
		Или ПараметрыОткрытия.РазделениеВключено) Тогда
		...
		РезультатПолученияТикета = ТикетАутентификации(
			ПараметрыОткрытия.Логин,
			ПараметрыОткрытия.Пароль,
			АдресСтраницы,
			ПараметрыОткрытия.НастройкиПрокси,
...
Процедура ОтправитьСообщениеВТехПоддержку(
	...
	ПараметрыСообщения.Вставить("ОтправкаНаСервере"    , Истина);
	...
	Иначе
		...
		ПараметрыСообщения.Вставить("НастройкиПрокси"              , НастройкиПрокси);
		ПараметрыСообщения.Вставить("ЭтоПолноправныйПользователь"  , ЭтоПолноправныйПользователь);
...
Функция ТикетАутентификации(
	...
	Знач НастройкиСоединения = Неопределено)
					...
					Для каждого ПФайл Из ФайлыВложений Цикл
						Если ПФайл.Хранение = ТекВложение.Адрес Тогда
							...
							ТекВложение.Вставить(
								"Текст",
								ИнтернетПоддержкаПользователейКлиентСервер.ТекстВФайле(ПФайл.ПолноеИмя));

							Попытка
								УдалитьФайлы(ПФайл.ПолноеИмя);
							Исключение
								ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
									ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
							КонецПопытки;
