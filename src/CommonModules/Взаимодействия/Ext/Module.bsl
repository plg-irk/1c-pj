...
Процедура ЗаполнитьРеквизитыПоУмолчанию(Объект, ДанныеЗаполнения) Экспорт
	...
	Если ЭтоВзаимодействие Тогда
		...
		// +Рарус Реквизит "Ответственный" является аналогом реквизита "Менеджер"
		Объект.Ответственный = Объект.Автор.Сотрудник;
		Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеОтветственного) Тогда
			Объект.ПодразделениеОтветственного = Объект.Ответственный.ПодразделениеКомпании;
		КонецЕсли;
	...
	ИначеЕсли ВзаимодействияКлиентСервер.ЯвляетсяПредметом(ДанныеЗаполнения) Тогда
		...
		// +РАРУС
		Контакты = УправлениеДиалогомСервер.ПолучитьКонтактыВзаимодействий(ДанныеЗаполнения);
		// -РАРУС
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предмет") И ЗначениеЗаполнено(ДанныеЗаполнения.Предмет) Тогда
			...
			Если НЕ (ДанныеЗаполнения.Свойство("Контакт") И ЗначениеЗаполнено(ДанныеЗаполнения.Контакт)) Тогда
				// +РАРУС
				Если ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(ДанныеЗаполнения.Предмет) Тогда
				Контакты = МенеджерОбъекта.ПолучитьКонтакты(ДанныеЗаполнения.Предмет);
				Иначе
					Контакты = УправлениеДиалогомСервер.ПолучитьКонтактыВзаимодействий(ДанныеЗаполнения.Предмет);
				КонецЕсли;
				// -РАРУС
			КонецЕсли;
		КонецЕсли;
...
// Параметры:
//  Предмет        - ПроизвольнаяСсылка - созданный предмет взаимодействий,
//  Взаимодействие - Ссылка             - взаимодействие, по которому создан предмет,
...
//  ИспользоватьВзаимодействиеОснование - Булево - флаг использования.
//
...
//   неразделенных регламентных заданий, которые должны использоваться в качестве шаблонов для заданий очереди.
//
...
//
Функция ПолучитьШаблонУсловияНаСсылкиКонтактовДляЗапроса() Экспорт //*РАРУС
...
// Изменено для того чтобы получать КИ в зависимости от переданного типа.
...
//
Функция ПолучитьАдресаЭлектроннойПочтыДляМассиваКонтактов(МассивКонтактов, ТипКИ, Группа = "") Экспорт //*РАРУС
...
Функция ПользовательЯвляетсяОтветственнымЗаВедениеПапок(УчетнаяЗапись) Экспорт
	...
	// +РАРУС
	Запрос.УстановитьПараметр("ОтветственныйЗаВедениеПапок", Пользователи.АвторизованныйПользователь().Сотрудник);
	// -РАРУС
