...
Функция ТарифныеОграничения(ПроверяемыеУсловия) Экспорт
	
	ТарифныеОграничения = ТарифныеОграниченияБизнесСтарт(ПроверяемыеУсловия);
	
	Возврат СериализаторXDTO.ЗаписатьXDTO(ТарифныеОграничения);
	
КонецФункции

Функция ИспользованиеМобильнойБухгалтерииОграниченоТарифом() Экспорт
	
	ТарифныеОграничения = ТарифныеОграниченияБизнесСтарт();
	
	//Ограничение на учет зарплаты не препятствует использовать мобильную бухгалтерию
	Если ТарифныеОграничения.Свойство("УчетЗарплаты") Тогда
		ТарифныеОграничения.Удалить("УчетЗарплаты");
	КонецЕсли;
	
	//Разрешаем подключаться, чтобы проинформировать о проблеме
	Если ТарифныеОграничения.Свойство("ПроведениеДокументов") Тогда
		ТарифныеОграничения.Удалить("ПроведениеДокументов");
	КонецЕсли;
	
	Возврат ЗначениеЗаполнено(ТарифныеОграничения);
	
КонецФункции

Функция ТарифныеОграниченияБизнесСтарт(Организация = Неопределено)
	
	ОграниченияТарифа = Новый Структура;
	
	Если Не ОбщегоНазначенияБП.ЭтоБизнесСтарт() Тогда
		Возврат ОграниченияТарифа;
	КонецЕсли;
	...
	Если НЕ ТарификацияБПВызовСервераПовтИсп.РазрешенУчетРегулярнойДеятельности() Тогда
		ОграниченияТарифа.Вставить("ВводДокументов");
	ИначеЕсли НЕ ТарификацияБПВызовСервераПовтИсп.РазрешенБухгалтерскийУчет() Тогда
		ОграниченияТарифа.Вставить("ВедениеУчета");
	ИначеЕсли Не ТарификацияБП.РазрешенДополнительныйФункционал() Тогда
		ОграниченияТарифа.Вставить("Функциональность");
	ИначеЕсли ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНедоступнаяСистемаНалогообложения()
		Или ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНедоступныйВидОрганизации() Тогда
		ОграниченияТарифа.Вставить("ПроведениеДокументов");
	ИначеЕсли ТарификацияБПВызовСервераПовтИсп.ИспользуетсяНедоступнаяФункциональность()
		И Не ТарификацияБП.ВозможнаВыборочнаяБлокировка() Тогда
		ОграниченияТарифа.Вставить("ПроведениеДокументов");
	КонецЕСли;
	
	Если ЗначениеЗаполнено(Организация) Тогда
		//Проверяем на превышение числа сотрудников
		ТекущееКоличествоСотрудников = 0;
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			...
			ТекущееКоличествоСотрудников = МодульРасчетЗарплатыДляНебольшихОрганизаций.КоличествоСотрудниковОрганизации(Организация);
		КонецЕсли;
		РазрешенноеКоличествоСотрудников = ТарификацияБП.РазрешенноеКоличествоСотрудников();
		
		ОписаниеТарифногоОграничения = НовыйОписаниеТарифногоОграничения();
		ОписаниеТарифногоОграничения.Предел = ?(РазрешенноеКоличествоСотрудников = -1, 99999, РазрешенноеКоличествоСотрудников);
		ОписаниеТарифногоОграничения.Значение = ТекущееКоличествоСотрудников;
		
		ОграниченияТарифа.Вставить("УчетЗарплаты", ОписаниеТарифногоОграничения);
...
Функция РезультатПроверкиАдресовПоКлассификатору(Параметры) Экспорт
	...
	Результат = Обработки.РасширенныйВводКонтактнойИнформации.ОшибкиЗаполненияАдреса(Параметры.Адрес, Параметры.ВидКонтактнойИнформации);
...
Функция РасширеннаяФункциональность(Организация, Дата, ПроверятьАктуальностьЗакрытиеМесяца) Экспорт
	...
	ИначеЕсли НЕ УчетнаяПолитика.ПрименяетсяУСНДоходы(Организация, Дата)
		...
		//Неподдерживаемый режим налогообложения
		
		ТребуетсяРасчетВБП = Истина;
		
	ИначеЕсли //Не поддерживаемые операции
	...
	ИначеЕсли ТекущаяДатаСеанса() >= Дата(2023,1,16)
		И УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Организация) Тогда
		//С 2023 года не поддерживаемый в мобильном режим расчет зарплаты и уплата налогов
		
		ТребуетсяРасчетВБП = Истина;
		
	ИначеЕсли УчетнаяПолитика.ПлательщикЕНП(Организация, Дата(2022,12,31)) Тогда
		//Для участников эксперимента с 01.07.22 - 31.12.2022 оставляем расчет в БП
		
		ТребуетсяРасчетВБП = Истина;
		
	ИначеЕсли УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, Дата) Тогда
...
Процедура ПроверитьАктуализироватьЗакрытиеМесяца(Организация, Дата) Экспорт
	...
	Если День(Дата) > 10 Тогда
		...
		Если РезультатПроверки.ТребуетсяАктуализация
			...
			Если НачалоМесяца(РезультатПроверки.ДатаАктуальности) < НачалоМесяца(Дата) Тогда
				...
				Исключение
					// Зарегистрируем факт того, что не получилось запустить актуализацию
					ЗаписьЖурналаРегистрации(
						НСтр("ru = 'МобильнаяБухгалтерия.ПроверитьАктуализироватьЗакрытиеМесяца'", ОбщегоНазначения.КодОсновногоЯзыка()),
						УровеньЖурналаРегистрации.Ошибка,
						,
						,
						СтрШаблон(НСтр("ru = 'Не удалось запланировать актуализацию для %1: %2'",ОбщегоНазначения.КодОсновногоЯзыка()), 
							Организация,
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
				КонецПопытки;
			КонецЕсли;
	...
	Если РезультатПроверки.СоответствуетТребованиям Тогда
		...
		Если РезультатПроверки.ЭтоЮрЛицо Тогда
			...
			Если ЗначениеЗаполнено(СведенияОКонтрагенте.ОписаниеОшибки) Тогда
				РеквизитыОрганизации.ОписаниеОшибки = СведенияОКонтрагенте.ОписаниеОшибки;
			ИначеЕсли ЗначениеЗаполнено(ЕГРЮЛ) Тогда
...
Функция СведенияОНалоговомОрганеПоАдресу(ЗначениеАдресаJSON)
	...
	СведенияОНалоговомОрганеПоАдресу = АдресныйКлассификатор.КодыАдреса(ЗначениеАдресаJSON, АдресныйКлассификаторБП.ИсточникПолученияСведенийОбАдресныхСведениях());
...
Функция ПолучитьСостояниеФоновогоЗадания(ИдентификаторФоновогоЗадания, СообщениеОбОшибке) Экспорт
	...
	Если ФоновоеЗадание <> Неопределено И СостояниеЗаданияСтрокой <> "Активно" Тогда
		СообщениеПользователю = ДлительныеОперации.ПрочитатьПрогресс(Новый УникальныйИдентификатор(ИдентификаторФоновогоЗадания));
		ПрефиксСлужебногоСообщения = СообщениеРезультатаФоновогоЗадания();
		Если СообщениеПользователю <> Неопределено
			И СообщениеПользователю.Свойство("Текст") Тогда
...
Функция ОтправитьДекларациюНаEmail(СсылкаНаОтчет, ОписаниеОшибки) Экспорт
	...
	Если НЕ ЭтоАдресВременногоХранилища(РезультатВыгрузки.АдресФайлаВыгрузки) Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'Неверный формат адреса хранилища при формировании файла выгрузки налоговой декларации.'");
...
Функция ОтправитьУведомлениеНаEmail(СсылкаНаОтчет, ОписаниеОшибки) Экспорт
	...
	Если Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УменьшениеНалогаНаСтраховыеВзносы Тогда
		Если Объект.ИмяФормы = "Форма2020_1" Тогда
			ИмяМакетаПечатнойФормы = "TIFF_2020_1";
		ИначеЕсли Объект.ИмяФормы = "Форма2021_1" Тогда
			ИмяМакетаПечатнойФормы = "TIFF_2021_1";
		КонецЕсли;
	ИначеЕсли Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ИсчисленныеСуммыНалогов Тогда
		Если Объект.ИмяФормы = "Форма2023_1" Тогда
			ИмяМакетаПечатнойФормы = "TIFF_2023_1";
		КонецЕсли;
	ИначеЕсли Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявлениеОЗачетеНалога Тогда
		Если Объект.ИмяФормы = "Форма2023_1" Тогда
			ИмяМакетаПечатнойФормы = "TIFF_2023_1";
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИмяМакетаПечатнойФормы) Тогда
	...
	Если НЕ ЭтоАдресВременногоХранилища(РезультатВыгрузки.АдресФайлаВыгрузки) Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'Неверный формат адреса хранилища при формировании файла выгрузки отчета.'");
	...
	ОтправитьМашиночитаемуюФормуНаEmail = РегламентированнаяОтчетностьВызовСервера.ОтправитьМашиночитаемуюФормуНаEmail(
		ПараметрыОтправки,
		ПараметрыСформированногоОтчета,
		СообщениеОбОшибке);
...
Функция ПреобразоватьДанныеРеглОтчетаМПДляБРО(ДанныеМП, ИмяФормыОтчета)
	...
	Если ИсточникОтчета = "РегламентированныйОтчетУСН" Тогда
		...
		Если ИмяФормыОтчета = "ФормаОтчета2015Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2021Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2023Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2024Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2025Кв1" Тогда
			
			Контейнер = Новый Структура;
			Контейнер.Вставить("Раздел21", Новый Структура);
			
			ЗаполнитьДекларациюУСН2015Кв1_2021кв1(ДанныеМП, Контейнер);
...
Функция ПреобразоватьДанныеРеглОтчетаБРОДляМП(ИсточникОтчета, ИмяФормыОтчета, ДанныеБРО)
	...
	Если ИсточникОтчета = "РегламентированныйОтчетУСН" Тогда
		Если ИмяФормыОтчета = "ФормаОтчета2015Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2021Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2023Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2024Кв1" Тогда
			Возврат ПолучитьДанныеДекларацииУСН2015кв1_2021кв1(ДанныеБРО);
		ИначеЕсли ИмяФормыОтчета = "ФормаОтчета2025Кв1" Тогда
			Возврат ПолучитьДанныеДекларацииУСН2025(ДанныеБРО);
		КонецЕсли;
	ИначеЕсли ИсточникОтчета = УчетЕНВД.ИдентификаторДекларации() Тогда
		Если ИмяФормыОтчета = "ФормаОтчета2016Кв1"
			Или ИмяФормыОтчета = "ФормаОтчета2017Кв1" Тогда
...
Функция ПреобразоватьДанныеУведомленияМБДляБРО(СтрокаЗаполнения)
	...
	Если СтрокаЗаполнения.ИмяОтчета = "РегламентированноеУведомлениеУменьшениеНалогаНаСтраховыеВзносы" Тогда
		...
		//Форма2020_1
		Если СтрокаЗаполнения.ИмяФормы = "Форма2020_1" Тогда
			
			Контейнер = Новый Структура;
			
			ЛистА = НовыеДанныеЛистаА_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2020_1();
			...
			ЛистБ = НовыеДанныеЛистаБ_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2020_1();
		...
		ИначеЕсли СтрокаЗаполнения.ИмяФормы = "Форма2021_1" Тогда
			
			Контейнер = Новый Структура;
			
			Титульная = НовыеДанныеЛистаТитульная_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1();
			Титульная.ГодДействПат = ДанныеУведомления.Титульная.ГодДействияПатента;
			
			Контейнер.Вставить("Титульная", Титульная);
			
			ЛистА = НовыеДанныеЛистаА_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1();
			Для каждого СведенияОПатенте Из ДанныеУведомления.ЛистА Цикл
				ДанныеЛистаА = ЛистА.Добавить();
				ДанныеЛистаА.НомерПат     = СведенияОПатенте.НомерПатента;
				ДанныеЛистаА.СумНалПатУм  = СведенияОПатенте.СуммаНалогаПоПатенту;
				ДанныеЛистаА.ДатаНачПат   = СведенияОПатенте.ДатаНачала;
				ДанныеЛистаА.ДатаКонПат   = СведенияОПатенте.ДатаОкончания;
				ДанныеЛистаА.ПрНП         = ?(СведенияОПатенте.ИспользуетсяТрудНаемныхРаботников, "1", "2");
				ДанныеЛистаА.СумСВУмНал   = СведенияОПатенте.СуммаУменьшения;
				ДанныеЛистаА.СумСВУменУчт = СведенияОПатенте.СуммаУменьшенияУчтеннаяРанее;
			КонецЦикла;
			
			Контейнер.Вставить("ЛистА", ЛистА);
			
			ЛистБ = НовыеДанныеЛистаБ_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1();
			
			ЛистБ.ОбщСумУплСВ     = ДанныеУведомления.ЛистБ.СуммаСтраховыхВзносов;
			ЛистБ.ОбщСумСВУмен    = ЛистА.Итог("СумСВУмНал");
			ЛистБ.ОбщСумСВУменУчт = ЛистА.Итог("СумСВУменУчт");
			ЛистБ.СумСВОст        = ЛистБ.ОбщСумУплСВ - ЛистБ.ОбщСумСВУмен - ЛистБ.ОбщСумСВУменУчт;
	...
	ИначеЕсли СтрокаЗаполнения.ИмяОтчета = "РегламентированноеУведомлениеИсчисленныеСуммыНалогов" Тогда
		...
		Если СтрокаЗаполнения.ИмяФормы = "Форма2023_1" Тогда
			
			Контейнер = Новый Структура;
			Контейнер.Вставить("СумИсчНалог", Новый СписокЗначений);
			Контейнер.Вставить("ДокументОснование", ДесериализоватьСсылку(ДанныеУведомления.ДокументОснование));
			
			Для Каждого СтрокаДокумента Из ДанныеУведомления.СумИсчНалог Цикл
				Контейнер.СумИсчНалог.Добавить(СтрокаДокумента);
			КонецЦикла;
...
Функция ПреобразоватьДанныеУведомленияБРОДляМБ(ИмяОтчета, ИмяФормыОтчета, ДанныеБРО)
	...
	Если ИмяОтчета = "РегламентированноеУведомлениеУменьшениеНалогаНаСтраховыеВзносы" Тогда
		...
		ИначеЕсли ИмяФормыОтчета = "Форма2021_1" Тогда
			
			Контейнер = Новый Структура;
			
			ДанныеУведомления = ДанныеБРО.ДанныеУведомления;
			
			Титульная = Новый Структура("ГодДействияПатента");
			Титульная.ГодДействияПатента = ДанныеУведомления.Титульная.ГодДействПат;
			
			Контейнер.Вставить("Титульная", Титульная);
			
			ЛистА = Новый Массив();
			ДанныеУведомленияЛистА = ДанныеБРО.ДанныеМногостраничныхРазделов.ЛистА;
			Для каждого ДанныеПатента Из ДанныеУведомленияЛистА Цикл
				
				СведенияОПатенте = ДанныеПатента.Значение;
				
				ОписаниеЛистА = Новый Структура();
				ОписаниеЛистА.Вставить("НомерПатента",                      СведенияОПатенте.НомерПат);
				ОписаниеЛистА.Вставить("СуммаНалогаПоПатенту",              СведенияОПатенте.СумНалПатУм);
				ОписаниеЛистА.Вставить("ДатаНачала",                        СведенияОПатенте.ДатаНачПат);
				ОписаниеЛистА.Вставить("ДатаОкончания",                     СведенияОПатенте.ДатаКонПат);
				ОписаниеЛистА.Вставить("ИспользуетсяТрудНаемныхРаботников", СведенияОПатенте.ПрНП = "1");
				ОписаниеЛистА.Вставить("СуммаУменьшения",                   СведенияОПатенте.СумСВУмНал);
				ОписаниеЛистА.Вставить("СуммаУменьшенияУчтеннаяРанее",      СведенияОПатенте.СумСВУменУчт);
			...
			ДанныеУведомленияЛистБ = ДанныеУведомления.ЛистБ;
			
			ЛистБ = Новый Структура("СуммаСтраховыхВзносов");
			ЛистБ.СуммаСтраховыхВзносов = ДанныеУведомления.ЛистБ.ОбщСумУплСВ;
		...
		КонецЕсли;
	ИначеЕсли ИмяОтчета = "РегламентированноеУведомлениеИсчисленныеСуммыНалогов" Тогда
		Если ИмяФормыОтчета = "Форма2023_1" Тогда
			
			Контейнер = Новый Структура;
			Контейнер.Вставить("СумИсчНалог", Новый СписокЗначений);
			Контейнер.Вставить("ДокументОснование", СериализоватьТипИСсылку(ДанныеБРО.ДокументОснование));
			
			Для Каждого СтрокаДокумента Из ДанныеБРО.ДанныеМногостраничныхРазделов.СумИсчНалог Цикл
				Контейнер.СумИсчНалог.Добавить(СтрокаДокумента.Значение);
			КонецЦикла;
...
Процедура СоздатьУзелОбмена(НастройкиУзлаБП, СообщениеОбОшибке) Экспорт
	...
	Если НастройкиУзлаБП.Свойство("ВерсияСхемы") Тогда
		СтруктураПараметров.ВерсияСхемы = НастройкиУзлаБП.ВерсияСхемы;
	КонецЕсли;
	
	Если НастройкиУзлаБП.Свойство("РасчетНалоговВБП") Тогда
		СтруктураПараметров.РасчетНалоговВБП = НастройкиУзлаБП.РасчетНалоговВБП;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДокСсылка) И ОбщегоНазначения.СсылкаСуществует(ДокСсылка) Тогда
		...
		Для Каждого ИмяПечатнойФормы Из МассивПечатныхФорм Цикл
			...
			Если ИмяПечатнойФормы = "СчетФактура"
				...
				Если ПлательщикНДС Или ПлательщикНДСПрименяющийУСН Или ИмяПечатнойФормы = "СчетФактура" Тогда
...
Функция ЗарегистрироватьИзмененияПоЗапросу(УзелОбменаКод, УзелОбменаИмя, СообщениеОбОшибке, ПараметрыОтбора) Экспорт
	...
	Если (СтрНайти(ВРЕГ(ПараметрыОтбора.ПолноеИмяОбъекта), "ДОКУМЕНТ") <> 0
		ИЛИ СтрНайти(ВРЕГ(ПараметрыОтбора.ПолноеИмяОбъекта), "СПРАВОЧНИК") <> 0)
		И ЗначениеЗаполнено(ПараметрыОтбора.СписокЗначений) Тогда
		...
		КонецЕсли;
		Отбор = Новый Массив();
		Для Каждого Значение из ПараметрыОтбора.СписокЗначений Цикл
			Идентификатор = Новый УникальныйИдентификатор(Значение);
			Отбор.Добавить(МенеджерОбъекта.ПолучитьСсылку(Идентификатор));
		КонецЦикла;
	Иначе
		Отбор = Новый Массив();
	...
	Попытка
		...
		Для Каждого ЭлементОтбора из Отбор Цикл
			ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена, ЭлементОтбора);
		КонецЦикла;
		Результат = Истина;
	Исключение
		СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ПодготовитьДанныеДляВыгрузки(Параметры, АдресРезультата) Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(,СтрСоединить(МассивСтрок));
...
Функция ПлатежнаяСсылка(СсылкаСчетНаОплату, ОтсутствуетДоступ) Экспорт
	...
	Возврат ОнлайнОплаты.ПлатежнаяСсылка(Ссылка, ОтсутствуетДоступ);

КонецФункции

Функция ОстаткиНоменклатуры(Параметры) Экспорт
	
	СтруктураПараметров = СериализаторXDTO.ПрочитатьXDTO(Параметры);
	
	Организация = ДесериализоватьСсылкуНаСправочник(СтруктураПараметров.Организация, "Организации");
	
	Возврат Новый ХранилищеЗначения(ОстаткиНоменклатурыОрганизации(Организация));
	
КонецФункции

Функция ОстаткиНоменклатурыОрганизации(Организация)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(, Счет В (&СчетаУчетаТоваров), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура), Организация = &Организация) КАК ХозрасчетныйОстатки
	|ГДЕ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = Новый Структура("Номенклатура,Остаток");
		НоваяСтрока.Номенклатура = СериализоватьСсылку(Выборка.Номенклатура);
...
Функция ПолучитьТарифныеОграничения(ПараметрыПроверкиОграничений) Экспорт
	
	Организация = Неопределено;
	
	Если ПараметрыПроверкиОграничений <> Неопределено Тогда
		ПараметрыПроверки = СериализаторXDTO.ПрочитатьXDTO(ПараметрыПроверкиОграничений);
		
		Если ПараметрыПроверки.Свойство("Организация") Тогда
			Организация = ДесериализоватьСсылкуНаСправочник(ПараметрыПроверки.Организация, "Организации");
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат СериализаторXDTO.ЗаписатьXDTO(ТарифныеОграниченияБизнесСтарт(Организация));
	
КонецФункции

Функция НовыйОписаниеТарифногоОграничения()
	
	НовыйОписаниеТарифногоОграничения = Новый Структура;
	НовыйОписаниеТарифногоОграничения.Вставить("Предел", 0);
	НовыйОписаниеТарифногоОграничения.Вставить("Значение", 0);
	
	Возврат НовыйОписаниеТарифногоОграничения;
	
КонецФункции

Процедура ЗагрузитьОперацииОнлайнОплат(СтруктураПараметров)
	
	Перем ВидОперацииОплатаПлатежнойКартой;
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьОнлайнОплаты")
		ИЛИ НЕ ЕстьНастройкаОнлайнОплатыДляОрганизации(СтруктураПараметров.Организация, ВидОперацииОплатаПлатежнойКартой) Тогда
	...
	ДатаНачалаВыборки = ?(ЗначениеЗаполнено(СтруктураПараметров.ДатаНачалаВыгрузки),
		СтруктураПараметров.ДатаНачалаВыгрузки,
		СтруктураПараметров.ДатаНачалаРаботы);
	
	ПериодЗагрузки.Вставить("ДатаНачала",    ДатаНачалаВыборки);
	ПериодЗагрузки.Вставить("ДатаОкончания", ТекущаяДатаСеанса());
	
	ОнлайнОплаты.ЗагрузитьОперацииОнлайнОплат(ПериодЗагрузки, СтруктураПараметров.Организация);
	
КонецПроцедуры

Функция ЕстьДоступКОнлайнОплатам(Организация, АдресСтраницыПредоставленияДоступа) Экспорт
	
	СсылкаОрганизация = ДесериализоватьСсылкуНаСправочник(Организация, "Организации");
	
	ЕстьДоступ = (ОнлайнОплаты.НастройкаИнтеграцииВыполнена(СсылкаОрганизация) = ИСТИНА);
	Если НЕ ЕстьДоступ Тогда
		АдресСтраницыПредоставленияДоступа = ОнлайнОплаты.АдресСтраницыПредоставленияДоступа();
	КонецЕсли; 
...
Процедура РазобратьПолученноеСообщение(СтруктураПараметров, СообщениеОбменаСтрокаXML, СообщениеОбОшибке)
	...
	Пока ЧтениеXML.URIПространстваИмен = URIПространстваИмен Цикл
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ИспользоватьИнтеграциюСЯндексКассой" Тогда
			...
			ЗагрузитьИспользоватьОнлайнОплаты(ОбъектXDTO, СтруктураПараметров.УзелОбмена);
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ИспользоватьНумерациюДоговоровСПокупателями" Тогда
			
			ЗагрузитьИспользоватьНумерациюДоговоровСПокупателями(ОбъектXDTO, СтруктураПараметров.УзелОбмена);
		// Справочники
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "НастройкаЯндексКассы" Тогда
			...
			ЗагрузитьНастройкиОнлайнОплат(ОбъектXDTO, СтруктураПараметров.УзелОбмена);
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "Доверенность" Тогда
			
			ЗагрузитьДоверенность(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов);
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "УведомлениеОбИсчисленныхСуммахНалогов" Тогда
			
			ЗагрузитьУведомлениеОбИсчисленныхСуммахНалогов(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов, РегистрацииВНалоговомОргане);
			
		ИначеЕсли ОбъектXDTO.Тип().Имя = "УведомлениеОбУменьшенииНалогаПоПатенту" Тогда
			
			ЗагрузитьУведомлениеОбУменьшенииНалогаПоПатенту(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов, РегистрацииВНалоговомОргане);
			
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ОперацияСПатентом" Тогда
			
			ЗагрузитьОперацияСПатентом(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов, РегистрацииВНалоговомОргане);
			
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ОперацияПоЕдиномуНалоговомуСчету" Тогда
			
			ЗагрузитьОперацияПоЕдиномуНалоговомуСчету(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов, РегистрацииВНалоговомОргане);
			
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ЗаявлениеОЗачетеВСчетПредстоящейОбязанности" Тогда
			
			ЗагрузитьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(ОбъектXDTO, СтруктураПараметров.УзелОбмена, ПроведениеДокументов, РегистрацииВНалоговомОргане);
		...
		ИначеЕсли ОбъектXDTO.Тип().Имя = "ЗадачиБухгалтераНалоговыеПлатежи" Тогда
			
			ЗагрузитьЗадачиБухгалтераНалоговыеПлатежи(ОбъектXDTO, СтруктураПараметров.УзелОбмена);
	...
	Для Каждого ДанныеРегистрацииВНалоговомОргане ИЗ РегистрацииВНалоговомОргане Цикл
		...
		Если ЗначениеЗаполнено(ДанныеРегистрацииВНалоговомОргане.Владелец) Тогда
			...
			Если ТипЗнч(ДанныеРегистрацииВНалоговомОргане.Владелец) = Тип("СправочникСсылка.Патенты") Тогда
				ЗаполняемыйОбъект.УдалитьНалоговыйОрган = РегистрацияВНО;
			ИначеЕсли ТипЗнч(ДанныеРегистрацииВНалоговомОргане.Владелец) = Тип("ДокументСсылка.ОперацияСПатентом") Тогда
	...
	Для Каждого Документ Из ПроведениеДокументов Цикл
		Попытка
			...
			Если НЕ Документ.ОжидаетПолученияНомера
				И НЕ Документ.ОжидаетЗаполнения Тогда
				ДокументОбъект.ОбменДанными.Отправитель = СтруктураПараметров.УзелОбмена;
			КонецЕсли;
			
			Если Документ.Провести Тогда
				//Принудительно установим признак Актуализировать, т.к. после загрузки документ в состоянии Актуален
				Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности")
					Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.УведомлениеОбИсчисленныхСуммахНалогов")
					Или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.УведомлениеОбУменьшенииНалогаПоПатенту") Тогда
					ДокументОбъект.ДополнительныеСвойства.Вставить("Актуализировать");
				КонецЕсли;
				
				Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОперацияСПатентом") Тогда
					Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоКБК(ДокументОбъект.КБК);
					Если Не ЗначениеЗаполнено(Налог)
						Или Налог = Справочники.ВидыНалоговИПлатежейВБюджет.ПрочиеНалогиИСборы Тогда
						ДокументОбъект.ДополнительныеСвойства.Вставить("ПоявиласьНоваяКатегорияПодчиненияПоВидуНалога");
					КонецЕсли;
				КонецЕсли;
...
Процедура ЗагрузитьВестиУчетПоДоговорам(Данные, УзелОбмена)
...
Процедура ЗагрузитьИспользоватьНумерациюДоговоровСПокупателями(Данные, УзелОбмена)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Менеджер = Константы.ИспользоватьНумерациюДоговоровСПокупателями.СоздатьМенеджерЗначения();
	Менеджер.Значение                 = Данные.Значение;
	Менеджер.ОбменДанными.Загрузка    = Истина;
	Менеджер.ОбменДанными.Отправитель = УзелОбмена;
	Менеджер.Записать();
...
Процедура ЗагрузитьИспользоватьОнлайнОплаты(Данные, УзелОбмена)
	
	Менеджер = Константы.ИспользоватьОнлайнОплаты.СоздатьМенеджерЗначения();
	Менеджер.Значение                 = Данные.Значение;
	Менеджер.ОбменДанными.Загрузка    = Истина;
	Менеджер.ОбменДанными.Отправитель = УзелОбмена;
	Менеджер.Записать();
...
Процедура ЗагрузитьБанковскийСчет(ОбъектXDTO, УзелОбмена)
	...
	Если ОбъектXDTO.ДатаОткрытия <> Неопределено Тогда
		Объект.ДатаОткрытия = ОбъектXDTO.ДатаОткрытия;
		Объект.ДатаЗакрытия = ОбъектXDTO.ДатаЗакрытия;
	КонецЕсли;
...
Процедура ЗагрузитьПатент(ОбъектXDTO, УзелОбмена, ДанныеРегистрацииВНалоговомОргане)
	...
	Объект.УдалитьНомерПатента  = ОбъектXDTO.НомерПатента;
	Объект.УдалитьДатаВыдачи    = ОбъектXDTO.ДатаВыдачи;
	Объект.УдалитьКБК           = ОбъектXDTO.КБК;
	Объект.УдалитьКодПоОКТМО    = ОбъектXDTO.КодПоОКТМО;
	Объект.УдалитьДатаНачала    = ОбъектXDTO.ДатаНачала;
	Объект.УдалитьДатаОкончания = ОбъектXDTO.ДатаОкончания;
	
	Если ОбъектXDTO.СуммаОсвобожденияОтНалога <> Неопределено Тогда
		Объект.УдалитьСуммаОсвобожденияОтНалога = ОбъектXDTO.СуммаОсвобожденияОтНалога;
	КонецЕсли;
	
	Объект.УдалитьСуммаПервогоПлатежа = ОбъектXDTO.СуммаПервогоПлатежа;
	Объект.УдалитьСуммаВторогоПлатежа = ОбъектXDTO.СуммаВторогоПлатежа;
	Объект.УдалитьДатаПервогоПлатежа  = ОбъектXDTO.ДатаПервогоПлатежа;
	Объект.УдалитьДатаВторогоПлатежа  = ОбъектXDTO.ДатаВторогоПлатежа;
	
	Объект.УдалитьПотенциальноВозможныйГодовойДоход = ОбъектXDTO.ПотенциальноВозможныйГодовойДоход;
	
	Если ЗначениеЗаполнено(ОбъектXDTO.КодНалоговогоОргана) Тогда
		Объект.УдалитьПостановкаНаУчетВНалоговомОргане  = Перечисления.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане;
		НовыеДанныеРегистрацииВНалоговомОргане = ДанныеРегистрацииВНалоговомОргане.Добавить();
		НовыеДанныеРегистрацииВНалоговомОргане.Владелец = Ссылка;
		НовыеДанныеРегистрацииВНалоговомОргане.КодНалоговогоОргана = ОбъектXDTO.КодНалоговогоОргана;
	Иначе
		Объект.УдалитьПостановкаНаУчетВНалоговомОргане  = Перечисления.ПостановкаНаУчетВНалоговомОргане.ПоМестуНахожденияОрганизации;
	КонецЕсли;
...
Процедура ЗагрузитьКонтрагента(ОбъектXDTO, УзелОбмена)
	...
	Если ОбъектXDTO.ИндивидуальныйПредприниматель <> Неопределено Тогда
		Объект.ИндивидуальныйПредприниматель = ОбъектXDTO.ИндивидуальныйПредприниматель;
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиСистемыНалогообложения(ОбъектXDTO, НастройкиУчетнойПолитики)
	...
	Если ОбъектXDTO.Записи.Количество()> 0 Тогда
		...
		Для каждого Запись Из ОбъектXDTO.Записи Цикл
			...
			НастройкиСистемыНалогообложения.Вставить("ПлательщикНДСПрименяющийУСН",             ПервоеНепустоеЗначение(Запись.ПлательщикНДСПрименяющийУСН, Ложь));
			...
			НастройкиСистемыНалогообложения.Вставить("ПрименяетсяАУСНДоходы",                   ПервоеНепустоеЗначение(Запись.ПрименяетсяАУСНДоходы, Ложь));
			НастройкиСистемыНалогообложения.Вставить("ПрименяетсяАУСНДоходыМинусРасходы",       ПервоеНепустоеЗначение(Запись.ПрименяетсяАУСНДоходыМинусРасходы, Ложь));
			НастройкиСистемыНалогообложения.Вставить("ПлательщикЕНП",                           ПервоеНепустоеЗначение(Запись.ПлательщикЕНП, Ложь));
...
Процедура ЗагрузитьНастройкиПродленияСроковНалоговОтчетов(ОбъектXDTO, УзелОбмена)
	...
	СохраняемыеДанные.Вставить("ПрименяетсяПостановление512", Ложь);
	СохраняемыеДанные.Вставить("ПрименяетсяПостановление776П1", Ложь);
	СохраняемыеДанные.Вставить("ПрименяетсяПостановление776П2", Ложь);
	...
	Для Каждого Запись ИЗ ОбъектXDTO.Записи Цикл
		...
		КонецЕсли;
			// Новые реквизиты были переданы в пакете
		Если Запись.ПрименяетсяПостановление512 <> Неопределено Тогда
			НоваяЗапись.ПрименяетсяПостановление512 = 
							Запись.ПрименяетсяПостановление512;
		КонецЕсли;
			// Новые реквизиты были переданы в пакете
		Если Запись.ПрименяетсяПостановление776П1 <> Неопределено Тогда
			НоваяЗапись.ПрименяетсяПостановление776П1 = 
							Запись.ПрименяетсяПостановление776П1;
		КонецЕсли;
			// Новые реквизиты были переданы в пакете
		Если Запись.ПрименяетсяПостановление776П2 <> Неопределено Тогда
			НоваяЗапись.ПрименяетсяПостановление776П2 = 
							Запись.ПрименяетсяПостановление776П2;
		КонецЕсли;
...
Процедура ЗагрузитьНастройкиУчетаУСН(ОбъектXDTO, НастройкиУчетнойПолитики)
	...
	Если НЕ НастройкиУчетнойПолитики.Свойство("НастройкиУчетаУСН", НастройкиУчетаУСН) Тогда
		...
		НастройкиУчетаУСН.Колонки.Добавить("ОснованиеЛьготнойСтавки", ОбщегоНазначения.ОписаниеТипаСтрока(12));
	...
	Для Каждого Запись ИЗ ОбъектXDTO.Записи Цикл
		...
		НоваяЗапись.ОснованиеЛьготнойСтавки = Запись.ОснованиеЛьготнойСтавки;
...
Процедура ЗагрузитьНоменклатуру(ОбъектXDTO, УзелОбмена)
	...
	ЭтоНовый = Ложь;
	Если Объект = Неопределено Тогда
		Если ОбъектXDTO.ЭтоГруппа = Истина Тогда
	...
	Если ОбъектXDTO.Родитель <> Неопределено Тогда
	...
	Если Не Объект.ЭтоГруппа Тогда
		...
		Если ЭтоНовый Или Объект.Услуга <> ОбъектXDTO.Услуга Тогда
			
			ВидНоменклатуры = Неопределено;
			
			Если ОбщегоНазначения.СсылкаСуществует(Объект.Родитель) Тогда
				РеквизитыГруппы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Родитель, 
					"ВидНоменклатуры, ВидНоменклатуры.Услуга");
				Если РеквизитыГруппы.ВидНоменклатурыУслуга = ОбъектXDTO.Услуга Тогда
					ВидНоменклатуры = РеквизитыГруппы.ВидНоменклатуры;
				КонецЕсли;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ВидНоменклатуры) Тогда
				ВидНоменклатуры = Справочники.ВидыНоменклатуры.ЭлементВидНоменклатурыПоУмолчанию(ОбъектXDTO.Услуга);
			КонецЕсли;
		...
		Если ОбъектXDTO.Артикул <> Неопределено Тогда
...
Процедура ЗагрузитьОрганизацию(ОбъектXDTO, УзелОбмена, Организация, ДанныеРегистрацииВНалоговомОргане, СсылкаНаФайлПодписьРуководителя)
	...
	// Для совместимости со старыми приложениями МБ
	...
	Если Объект.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо
		...
		Если НЕ ЗначениеЗаполнено(Объект.ИндивидуальныйПредприниматель) Тогда
			...
			Иначе
				...
				ДанныеФизическогоЛица.ИНН = Объект.ИНН;
...
Процедура ЗагрузитьЗадачиБухгалтераНалоговыеПлатежи(ОбъектXDTO, УзелОбмена)
	
	Организация        = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.ОрганизацияОтбор, "Организации");
	ПлатежноеПоручение = ДесериализоватьСсылку(ОбъектXDTO.ПлатежноеПоручениеОтбор);
	...
	Для Каждого Запись ИЗ ОбъектXDTO.Записи Цикл
		...
		ИдЗадачи  = Запись.ИдентификаторЗадачи;
		ИдПравила = Запись.ИдентификаторПравила;
		...
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Организация         = Организация;
		НоваяЗапись.ПлатежноеПоручение  = ПлатежноеПоручение;
		
		НоваяЗапись.Правило                     = Правило;
		НоваяЗапись.ПериодСобытия               = Запись.ПериодСобытия;
		НоваяЗапись.ОплатаЗадолженности         = Запись.ОплатаЗадолженности;
...
Процедура ЗагрузитьЧекиНПД(ОбъектXDTO, УзелОбмена) Экспорт
	...
	ИначеЕсли ОбъектXDTO.ТипДокументОснованиеОтбор = "ОплатаПлатежнойКартой" Тогда
		ДокументОснование = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.ДокументОснованиеОтбор, "ОплатаПлатежнойКартой");
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиОнлайнОплат(ОбъектXDTO, УзелОбмена)
	
	Ссылка = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Ссылка, "НастройкиОнлайнОплат");
	Объект = Ссылка.ПолучитьОбъект();
	Если Объект = Неопределено Тогда
		Объект = Справочники.НастройкиОнлайнОплат.СоздатьЭлемент();
		Объект.УстановитьСсылкуНового(Ссылка);
		
		Объект.Наименование             = УзелОбмена.Организация;
		Объект.ОтправкаЧековЧерезСервис = Ложь;
	...
	ОнлайнОплатыБП.ЗаполнитьНастройкуВидОплатыПоУмолчанию(Объект);
	
	Объект.ОбменДанными.Загрузка = Истина;
	Объект.ОбменДанными.Отправитель = УзелОбмена;
	Объект.Записать();
	
	РегистрыСведений.СтатусыОнлайнОплат.ОбновитьЗначениеРеквизитаОрганизация(Объект.Ссылка, Объект.Организация);
...
Процедура ЗагрузитьВводНачальныхОстатковРасчетыПоНалогамИСборам(ОбъектXDTO, Объект)
	...
	Для каждого ЗагружаемаяСтрокаТЧ Из ОбъектXDTO.РасчетыПоНалогамИСборам Цикл
		...
		ИначеЕсли ЗагружаемаяСтрокаТЧ.ВидНалога = "ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф" Тогда
			
			НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП; // 69.06.6
			НоваяСтрока.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
		...
		ИначеЕсли ЗагружаемаяСтрокаТЧ.ВидНалога = "СтраховыеВзносыЕдиныйТариф" Тогда
			
			НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф; //69.09
			НоваяСтрока.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
			
		ИначеЕсли ЗагружаемаяСтрокаТЧ.ВидНалога = "ЕдиныйНалоговыйПлатеж" Тогда
			
			НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет; //68.90
			НоваяСтрока.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
			
		ИначеЕсли ЗагружаемаяСтрокаТЧ.ВидНалога = "АУСН" Тогда
			
			НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.НалогПриАУСН; //68.15
			НоваяСтрока.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
...
Процедура ЗагрузитьДоверенность(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "Доверенность");
	Объект = Ссылка.ПолучитьОбъект();
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.Доверенность.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
	...
	Объект.ДатаДействия = ОбъектXDTO.ДатаДействия;
	...
	Объект.ФизЛицо           = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.ФизЛицо,  "ФизическиеЛица");
	
	Если Не ЭтоНовыйДокумент 
		И Объект.ПоДокументу <> ОбъектXDTO.ПоДокументу Тогда
		Объект.Сделка = Неопределено;
	КонецЕсли;
	
	Объект.НаПолучениеОт      = ОбъектXDTO.НаПолучениеОт;
	Объект.ПоДокументу        = ОбъектXDTO.ПоДокументу;
		
	Объект.Товары.Очистить();
	
	Для каждого СтрокаТЧ Из ОбъектXDTO.Товары Цикл
		НоваяСтрока = Объект.Товары.Добавить();
		НоваяСтрока.НаименованиеТовара = СтрокаТЧ.НаименованиеТовара;
		НоваяСтрока.ЕдиницаПоКлассификатору = ПолучитьСсылкуНаСправочникПоКлючевымСвойствам(СтрокаТЧ.ЕдиницаПоКлассификатору, "КлассификаторЕдиницИзмерения");
...
Процедура ЗагрузитьЗаявлениеАбонентаСпецоператораСвязи(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если НовыйДокумент Тогда
		...
		Объект.ТребуетсяВстречаСПартнером       = Истина;
		Объект.СпособПолученияСертификата       = Перечисления.СпособПолученияСертификата.ИздатьНовый;
		// В облаке доступны сертификаты только УЦАналитическийЦентр
		Объект.УдостоверяющийЦентр              = Перечисления.УдостоверяющиеЦентрыБРО.УЦАналитическийЦентр;
		Объект.МодельХраненияЗакрытогоКлюча     = Перечисления.МодельХраненияЗакрытогоКлюча.ВМоделиСервиса;
...
Процедура ЗагрузитьПлатежноеПоручение(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога Тогда
		...
		ИначеЕсли ОбъектXDTO.ВидНалога = "ФиксированныеВзносы_ПФР_СДоходов" Тогда
			Если Объект.Дата < ПлатежиВБюджетКлиентСерверПереопределяемый.ПериодИзмененияСтраховыхВзносов() Тогда
				Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть, Истина);
			Иначе
				Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела, Истина);
			КонецЕсли;
			Объект.ПоказательПериода = "ГД.00." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=yyyy");
		...
		ИначеЕсли ОбъектXDTO.ВидНалога = "ФиксированныеВзносы_ФФОМС" Тогда
			//По неактуальным налогам налог надо создавать, указывая год действия
			...
			Объект.ПоказательПериода = "ГД.00." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=yyyy");
		ИначеЕсли ОбъектXDTO.ВидНалога = "ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф" Тогда
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф, Истина);
			Объект.ПоказательПериода = "ГД.00." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=yyyy");
		...
		ИначеЕсли ОбъектXDTO.ВидНалога = "НДФЛ" Тогда
			...
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.НДФЛ, Истина, , КодВидаДоходов);
		...
		ИначеЕсли ОбъектXDTO.ВидНалога = "НДФЛ_ДоходыСвышеПредельнойВеличины" Тогда
			...
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(
				Перечисления.ВидыНалогов.НДФЛ, Истина, , КодВидаДоходов);
			Объект.ПоказательПериода = "МС." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=MM.yyyy");
		...
		ИначеЕсли ОбъектXDTO.ВидНалога = "СтраховыеВзносы_ФФОМС" Тогда
			...
			Объект.ПоказательПериода = "МС." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=MM.yyyy");
		ИначеЕсли ОбъектXDTO.ВидНалога = "СтраховыеВзносыЕдиныйТариф" Тогда
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.СтраховыеВзносыЕдиныйТариф, Истина);
			Объект.ПоказательПериода = "МС." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=MM.yyyy");
		ИначеЕсли ОбъектXDTO.ВидНалога = "АУСН" Тогда
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.АУСН, Истина);
			Объект.ПоказательПериода = "МС." + Формат(ОбъектXDTO.ПериодНачисления, "ДФ=MM.yyyy");
		ИначеЕсли ОбъектXDTO.ВидНалога = "ЕдиныйНалоговыйПлатеж" Тогда
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж, Истина);
			Объект.ПоказательПериода = "0";
		Иначе
...
Процедура ЗагрузитьПоступлениеНаРасчетныйСчет(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
		...
		Объект.Содержание_УСН = НСтр("ru = 'Прочий приход денежных средств.'");
	КонецЕсли;
	
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога Тогда
		Если ЗначениеЗаполнено(ОбъектXDTO.ВидНалога) Тогда
			ВидНалога = ЗначениеПеречисленияПоИмени(Перечисления.ВидыНалогов, ОбъектXDTO.ВидНалога);
			Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалога, Истина);
		КонецЕсли;
	...
	Суффикс = ?(Объект.ВалютаДокумента = ВалютаРегламентированногоУчета, "", "ВВалюте");
	
	Если ЭтоНовый Тогда
		Если ЗначениеЗаполнено(Объект.СчетОрганизации) Тогда
			Объект.СчетБанк = Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(Объект.СчетОрганизации);
		Иначе
			Объект.СчетБанк = ПланыСчетов.Хозрасчетный.ОсновнойСчет("БезналичныеДеньги" + Суффикс);
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.СчетУчетаРасчетовСКонтрагентом) Тогда
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
			И УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Объект.Организация, Объект.Дата) Тогда
			Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПереводыВПути" + Суффикс);
		ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
			Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ОсновнойСчет("Депозит" + Суффикс);
		КонецЕсли;
	КонецЕсли;
	
	ПараметрыКурсаДокумента = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Объект.ВалютаДокумента, Объект.Дата);
	
	ПараметрыКурсаВзаиморасчетов = РаботаСКурсамиВалют.ПолучитьКурсВалюты(
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДоговорКонтрагента, "ВалютаВзаиморасчетов"), 
		Объект.Дата);
	
	Объект.Патент = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Патент, "Патенты");
	
	Если ОбъектXDTO.РасшифровкаПлатежа <> Неопределено 
		И ОбъектXDTO.РасшифровкаПлатежа.Количество() > 0 Тогда
		
		// Если таблица расшировки в БП пустая - загружаем без дополнительных проверок
		Если Объект.РасшифровкаПлатежа.Количество() = 0 Тогда
			Для каждого СтрокаРасшифровки Из ОбъектXDTO.РасшифровкаПлатежа Цикл
				НоваяСтрока = Объект.РасшифровкаПлатежа.Добавить();
				
				НоваяСтрока.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.Автоматически;
				НоваяСтрока.СтавкаНДС                    = Перечисления.СтавкиНДС.БезНДС;
				НоваяСтрока.УслугаНПД                    = ДесериализоватьСсылкуНаСправочник(СтрокаРасшифровки.УслугаНПД, "Номенклатура");
				НоваяСтрока.СуммаПлатежа                 = СтрокаРасшифровки.Сумма;
			КонецЦикла
		Иначе
			ТаблицаДокумента   = Объект.РасшифровкаПлатежа.Выгрузить();
			ТаблицаРасшифровки = Объект.РасшифровкаПлатежа.ВыгрузитьКолонки("УслугаНПД, СуммаПлатежа");
			...
			// Сворачиваем таблицу товаров чтобы ключевые поля не повторялись
			ТаблицаРасшифровки.Свернуть("УслугаНПД", "СуммаПлатежа");
			
			ОтборКлючевыеПоля = Новый Структура;
			ОтборКлючевыеПоля.Вставить("ДоговорКонтрагента", Объект.ДоговорКонтрагента);
			
			Для каждого СтрокаРасшифровки Из ТаблицаРасшифровки Цикл
				ОтборКлючевыеПоля.Вставить("УслугаНПД", СтрокаРасшифровки.УслугаНПД);
				
				НайденныеСтроки = ТаблицаДокумента.Скопировать(ОтборКлючевыеПоля);
				
				Если НайденныеСтроки.Итог("СуммаПлатежа") = СтрокаРасшифровки.СуммаПлатежа Тогда
					// Если изменений нет - сохраняем строки табличной части
					ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(НайденныеСтроки, Объект.РасшифровкаПлатежа);
				Иначе
					НоваяСтрока = Объект.РасшифровкаПлатежа.Добавить();
					
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ОтборКлючевыеПоля);
					
					НоваяСтрока.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.Автоматически;
					НоваяСтрока.СтавкаНДС                    = Перечисления.СтавкиНДС.БезНДС;
					НоваяСтрока.СуммаПлатежа                 = СтрокаРасшифровки.СуммаПлатежа;
					НоваяСтрока.КурсВзаиморасчетов           = ПараметрыКурсаВзаиморасчетов.Курс;
					НоваяСтрока.КратностьВзаиморасчетов      = ПараметрыКурсаВзаиморасчетов.Кратность;
					НоваяСтрока.СуммаВзаиморасчетов          = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
						НоваяСтрока.СуммаПлатежа,
						ПараметрыКурсаДокумента,
						ПараметрыКурсаВзаиморасчетов);
					
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		// Если комиссия по документу не совпадает с комиссией по строкам - распределим ее заново
		Если Объект.СуммаУслуг <> Объект.РасшифровкаПлатежа.Итог("СуммаУслуг") Тогда
			ТаблицаДокумента = Объект.РасшифровкаПлатежа.Выгрузить();
			ОбщегоНазначенияБПВызовСервера.РаспределитьСуммуПоКолонкеТаблицы(Объект.СуммаУслуг, ТаблицаДокумента, "СуммаУслуг", "СуммаПлатежа");
			Объект.РасшифровкаПлатежа.Загрузить(ТаблицаДокумента);
		КонецЕсли;
	Иначе
		Если Объект.РасшифровкаПлатежа.Количество() = 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СтавкаНДС) Тогда
			...
			СтрокаРасшифровки.СтавкаНДС = ПоступлениеНаРасчетныйСчетФормы.СтавкаНДСРасшифровкиПлатежа(Объект.Дата,
				Объект.ВидОперации, СтрокаРасшифровки.ДоговорКонтрагента, ПлательщикНДС, ПлательщикНДСПриУСН, СтавкаНДСПриУСН);
	...
	Объект.ОтражениеВУСН = ?(ОбъектXDTO.ДоходУСН > 0, Перечисления.ОтражениеВУСН.Принимаются, Перечисления.ОтражениеВУСН.НеПринимаются);
	...
	Если НовыйДокумент Тогда
		...
		Если УчетнаяПолитика.Существует(Объект.Организация, Объект.Дата) Тогда
			Объект.НДСВключенВСтоимость = 
				НЕ УчетнаяПолитика.ПлательщикНДС(Объект.Организация, Объект.Дата)
				И НЕ УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Объект.Организация, Объект.Дата);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьПриходныйКассовыйОрдер(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если ЭтоНовый Тогда
		Объект.СчетКасса = ПланыСчетов.Хозрасчетный.ОсновнойСчет("НаличныеДеньги" + Суффикс);
	КонецЕсли;
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ПолучениеНаличныхВБанке Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.СчетУчетаРасчетовСКонтрагентом) Тогда
			Если УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Объект.Организация, Объект.Дата) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПереводыВПути" + Суффикс);
			ИначеЕсли ЗначениеЗаполнено(Объект.Контрагент) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом = Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(Объект.Контрагент);
			КонецЕсли;
		КонецЕсли;
	Иначе
		Объект.Контрагент = ДесериализоватьСсылкуНаСправочник(ОбъектXDTO.Контрагент, "Контрагенты");
	...
	// Услуги НПД приходят в отдельной ТЧ
	Если ОбъектXDTO.РасшифровкаПлатежа <> Неопределено
		И ОбъектXDTO.РасшифровкаПлатежа.Количество() > 0 Тогда
...
Процедура ЗагрузитьРасходныйКассовыйОрдер(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ПрочийРасход
		...
		Объект.Содержание_УСН = НСтр("ru = 'Прочий расход денежных средств.'");
	КонецЕсли;
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВзносНаличнымиВБанк
		...
		Если Не ЗначениеЗаполнено(Объект.СчетУчетаРасчетовСКонтрагентом) Тогда
			Если УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Объект.Организация, Объект.Дата) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПереводыВПути" + Суффикс);
			ИначеЕсли ЗначениеЗаполнено(Объект.СчетОрганизации) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом =
					Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(Объект.СчетОрганизации);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьСписаниеСРасчетногоСчета(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.КомиссияБанка
		И Не ЗначениеЗаполнено(Объект.Содержание_УСН) Тогда
		Объект.Содержание_УСН = НСтр("ru = 'Расходы, связанные с оплатой услуг, оказываемых кредитными организациями.'");
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.СчетУчетаРасчетовСКонтрагентом) Тогда
		Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет Тогда
			Если УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Объект.Организация, Объект.Дата) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ОсновнойСчет("ПереводыВПути" + Суффикс);
			ИначеЕсли ЗначениеЗаполнено(Объект.СчетКонтрагента) Тогда
				Объект.СчетУчетаРасчетовСКонтрагентом =
					Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(Объект.СчетКонтрагента);
			КонецЕсли;
	...
	Объект.ОтражениеВУСН = ?(ОбъектXDTO.РасходУСН > 0, Перечисления.ОтражениеВУСН.Принимаются, Перечисления.ОтражениеВУСН.НеПринимаются);
	...
	Если НЕ ЗначениеЗаполнено(СтавкаНДС) Тогда
		...
		ПлательщикНДСПриУСН = Ложь;
		СтавкаНДСПриУСН     = Перечисления.СтавкиНДС.ПустаяСсылка();
		СтрокаРасшифровки.СтавкаНДС = СписаниеСРасчетногоСчетаФормы.СтавкаНДСРасшифровкиПлатежа(Объект.Дата,
			Объект.ВидОперации,
...
Процедура ЗагрузитьСчетНаОплатуПокупателю(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Если Не ЗначениеЗаполнено(Объект.ВариантПечатиQRКода) Тогда
		Объект.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.ПлатежныеРеквизиты;
	КонецЕсли;
...
Процедура ЗагрузитьАктСверкиВзаиморасчетов(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	СверкаСогласована = Документы.АктСверкиВзаиморасчетов.СверкаСогласована(Объект.Ссылка, Объект.Организация);
	
	ИзмененПризнакСверкаСогласована = СверкаСогласована <> ОбъектXDTO.СверкаСогласована;
	СверкаСогласована = ОбъектXDTO.СверкаСогласована;
	...
	ИначеЕсли ИзмененПризнакСверкаСогласована Тогда
		Если СверкаСогласована И Объект.ПоДаннымКонтрагента.Количество() = 0 Тогда
		...
		ИначеЕсли НЕ СверкаСогласована Тогда
			Объект.ПоДаннымКонтрагента.Очистить();
			Объект.Расхождение = 0;
		КонецЕсли;
	КонецЕсли;
	...
	РегистрыСведений.СтатусыДокументов.УстановитьСтатусыДокумента(Объект.Ссылка, СтатусыДокумента);
...
Процедура ЗагрузитьОплатуПлатежнойКартой(ОбъектXDTO, УзелОбмена, ПроведениеДокументов)
	...
	Объект = Ссылка.ПолучитьОбъект();
	Если Объект = Неопределено Тогда
		// Новый документ ОплатаПлатежнойКартой из МП прийти не может
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьУведомлениеОСпецрежимахНалогообложения(ОбъектXDTO, УзелОбмена, ЗаполнениеУведомлений, ДанныеРегистрацииВНалоговомОргане)
	...
	Если ЗначениеЗаполнено(ОбъектXDTO.ВидУведомления) Тогда
		Объект.ВидУведомления = ЗначениеПеречисленияПоИмени(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения, ОбъектXDTO.ВидУведомления);
	Иначе
		Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УменьшениеНалогаНаСтраховыеВзносы;
	КонецЕсли;
	
	//Остальные уведомления перезаполняются при записи документа основания
	Если Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УменьшениеНалогаНаСтраховыеВзносы
		И Не ЗначениеЗаполнено(ОбъектXDTO.Основание) Тогда
		ПерезаполнитьОтчет = НЕ ОбъектXDTO.ПометкаУдаления;
	Иначе
		ПерезаполнитьОтчет = Ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ОбъектXDTO.Основание) Тогда
		Объект.Основание = ДесериализоватьСсылку(ОбъектXDTO.Основание);
	КонецЕсли;
...
Процедура ЗагрузитьУведомлениеОбИсчисленныхСуммахНалогов(ОбъектXDTO, УзелОбмена, ПроведениеДокументов, ДанныеРегистрацииВНалоговомОргане)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "УведомлениеОбИсчисленныхСуммахНалогов");
	Объект = Ссылка.ПолучитьОбъект();
	
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.УведомлениеОбИсчисленныхСуммахНалогов.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
	...
	Объект.СуммаДокумента = ОбъектXDTO.СуммаДокумента;
	Объект.Уведомление = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Уведомление, "УведомлениеОСпецрежимахНалогообложения");
	
	Объект.Налоги.Очистить();
	Объект.НалоговыйАгентНДС.Очистить();
	
	Для каждого СтрокаДокумента Из ОбъектXDTO.Налоги Цикл
		
		ОтражениеНалога = ОтражениеВУчетеПоВидуНалогаМобильного(СтрокаДокумента.ВидНалога, Объект.Дата);
		Налог = ОтражениеНалога.ВидНалога;
		СчетУчета = ОтражениеНалога.СчетУчета;
		...
		НоваяСтрока = Объект.Налоги.Добавить();
		НоваяСтрока.Налог     = Налог;
		НоваяСтрока.СчетУчета = СчетУчета;
		
		Если ЗначениеЗаполнено(СтрокаДокумента.КодНалоговогоОргана) Тогда
			НовыйДанныеРегистрацииВНалоговомОргане = ДанныеРегистрацииВНалоговомОргане.Добавить();
			НовыйДанныеРегистрацииВНалоговомОргане.КодНалоговогоОргана = СтрокаДокумента.КодНалоговогоОргана;
		КонецЕсли;

		РегистрацияВНалоговомОргане = Справочники.РегистрацииВНалоговомОргане.РегистрацияВНалоговомОргане(
				Объект.Организация, "", СтрокаДокумента.КодНалоговогоОргана);
				
		НоваяСтрока.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане.Ссылка;
		
		НоваяСтрока.КодБК = СтрокаДокумента.КодБК;
		НоваяСтрока.КодПоОКТМО = СтрокаДокумента.КодПоОКТМО;

		НоваяСтрока.Сумма = СтрокаДокумента.Сумма;
		
		НоваяСтрока.СрокУплаты = СтрокаДокумента.СрокУплаты;
		
		НоваяСтрока.НачалоПериода = СтрокаДокумента.НачалоПериода;
		НоваяСтрока.КонецПериода = СтрокаДокумента.КонецПериода;
		
		НоваяСтрока.КлючСтроки = НоваяСтрока.КлючСтроки;
...
Процедура ЗагрузитьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(ОбъектXDTO, УзелОбмена, ПроведениеДокументов, ДанныеРегистрацииВНалоговомОргане)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ЗаявлениеОЗачетеВСчетПредстоящейОбязанности");
	Объект = Ссылка.ПолучитьОбъект();
	
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
...
Процедура ЗагрузитьОперацияПоЕдиномуНалоговомуСчету(ОбъектXDTO, УзелОбмена, ПроведениеДокументов, ДанныеРегистрацииВНалоговомОргане)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ОперацияПоЕдиномуНалоговомуСчету");
	Объект = Ссылка.ПолучитьОбъект();
	
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.ОперацияПоЕдиномуНалоговомуСчету.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
	...
	Объект.ВидОперации = ЗначениеПеречисленияПоИмени(Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету, ОбъектXDTO.ВидОперации);
	Объект.СуммаДокумента = ОбъектXDTO.СуммаДокумента;
	...
	Для каждого СтрокаДокумента Из ОбъектXDTO.Налоги Цикл
		...
		Налог = ОтражениеНалога.ВидНалога;
		СчетУчета = ОтражениеНалога.СчетУчета;
		ВидПлатежаВГосБюджет = ОтражениеНалога.ВидПлатежаВБюджет;
		...
		НоваяСтрока.Налог = Налог;
		НоваяСтрока.СчетУчета = СчетУчета;
		НоваяСтрока.ВидПлатежаВГосБюджет = ВидПлатежаВГосБюджет;
		...
		НоваяСтрока.Сумма = СтрокаДокумента.Сумма;
		НоваяСтрока.СрокУплаты = СтрокаДокумента.СрокУплаты;
		НоваяСтрока.ПлатежныйДокумент = ДесериализоватьСсылку(СтрокаДокумента.ПлатежныйДокумент);
		
		НоваяСтрока.КорректировкаЕНС = СтрокаДокумента.КорректировкаЕНС;
		
		НоваяСтрока.КлючСтроки = НоваяСтрока.КлючСтроки;
		
		//Для начислениеНалога СчетЗатрат игнорируется
		
		//Перенос переплаты по налогам на 68.90
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов Тогда
			
			Если Не НоваяСтрока.КорректировкаЕНС Тогда
				НоваяСтрока.СчетЗатрат = НоваяСтрока.СчетУчета;
				...
				Если СвойстваСчета.ВидСубконто1 = ВидСубконто Тогда
					НоваяСтрока.Субконто1 = НоваяСтрока.ВидПлатежаВГосБюджет;
				ИначеЕсли СвойстваСчета.ВидСубконто2 = ВидСубконто Тогда
					НоваяСтрока.Субконто2 = НоваяСтрока.ВидПлатежаВГосБюджет;
				ИначеЕсли СвойстваСчета.ВидСубконто3 = ВидСубконто Тогда
					НоваяСтрока.Субконто3 = НоваяСтрока.ВидПлатежаВГосБюджет;
				КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ОбъектXDTO.Санкции Цикл
		
		НоваяСтрока = Объект.Санкции.Добавить();
		
		НоваяСтрока.ВидПлатежа = ЗначениеПеречисленияПоИмени(Перечисления.ВидыПлатежейВГосБюджет, СтрокаДокумента.ВидПлатежа);
		
		НоваяСтрока.Сумма = СтрокаДокумента.Сумма;
		
		НоваяСтрока.ПлатежныйДокумент = ДесериализоватьСсылку(СтрокаДокумента.ПлатежныйДокумент);
		
		Если Не СтрокаДокумента.КорректировкаЕНС Тогда
			
			//Заполнен Счет затрат - налоговым счетом
			Если ЗначениеЗаполнено(СтрокаДокумента.ВидНалога) Тогда
				
				ОтражениеНалога = ОтражениеВУчетеПоВидуНалогаМобильного(СтрокаДокумента.ВидНалога, Объект.Дата);
				
				НоваяСтрока.СчетЗатрат = ОтражениеНалога.СчетУчета;
				
				СвойстваСчета           = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(НоваяСтрока.СчетЗатрат);
				ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет;
				Если СвойстваСчета.ВидСубконто1 = ВидСубконто Тогда
					НоваяСтрока.Субконто1 = НоваяСтрока.ВидПлатежа;
				ИначеЕсли СвойстваСчета.ВидСубконто2 = ВидСубконто Тогда
					НоваяСтрока.Субконто2 = НоваяСтрока.ВидПлатежа;
				ИначеЕсли СвойстваСчета.ВидСубконто3 = ВидСубконто Тогда
					НоваяСтрока.Субконто3 = НоваяСтрока.ВидПлатежа;
				КонецЕсли;
	...
	//Перенос переплат, в т.ч. штрафов
	Для каждого СтрокаДокумента Из ОбъектXDTO.ЕдиныйСчет Цикл
		
		НоваяСтрока = Объект.ЕдиныйСчет.Добавить();
		
		НоваяСтрока.ВидДвижения = СтрокаДокумента.ВидДвижения;
		
		НоваяСтрока.Сумма = СтрокаДокумента.Сумма;
		
		НоваяСтрока.ПлатежныйДокумент = ДесериализоватьСсылку(СтрокаДокумента.ПлатежныйДокумент);
		
		//Пополнение ЕНС при переходе в 2023 - за счет переплат по налогам, взносам и штрафам
		//При пополнении платежный документ можно явно не указывать
		//Прочие пополнения ЕНС в мобильном передаются без аналитики
		
		//Списание ЕНС при переходе в 2023 - погашение задолженности одних налогов за счет переплат других налогов, штрафов
		//Переплаты уже перенесен на ЕНС
		//При списании важно указать документ переплаты
		...
		Если Не СтрокаДокумента.КорректировкаЕНС Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДокумента.ВидНалога) Тогда
				...
				Если НЕ ЗначениеЗаполнено(ОтражениеНалога.СчетУчета) Тогда
					Продолжить;
				КонецЕсли;
				...
				ИначеЕсли СвойстваСчета.ВидСубконто2 = ВидСубконто Тогда
					НоваяСтрока.Субконто2 = ВидПлатежаВГосБюджет;
				ИначеЕсли СвойстваСчета.ВидСубконто3 = ВидСубконто Тогда
					НоваяСтрока.Субконто3 = ВидПлатежаВГосБюджет;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗагрузитьУведомлениеОбУменьшенииНалогаПоПатенту(ОбъектXDTO, УзелОбмена, ПроведениеДокументов, ДанныеРегистрацииВНалоговомОргане)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "УведомлениеОбУменьшенииНалогаПоПатенту");
	Объект = Ссылка.ПолучитьОбъект();
	
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.УведомлениеОбУменьшенииНалогаПоПатенту.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
	...
	РегистрацияВНалоговомОргане = Справочники.РегистрацииВНалоговомОргане.РегистрацияВНалоговомОргане(
			Объект.Организация, "", ОбъектXDTO.КодНалоговогоОргана);
		
	Объект.НалоговыйОрган = РегистрацияВНалоговомОргане.Ссылка;
	
	Объект.СуммаРасходов = ОбъектXDTO.СуммаРасходов;
	Объект.Уведомление = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Уведомление, "УведомлениеОСпецрежимахНалогообложения");
	
	Объект.Патенты.Очистить();
	
	Для каждого СтрокаДокумента Из ОбъектXDTO.Патенты Цикл
		
		НоваяСтрока = Объект.Патенты.Добавить();
		НоваяСтрока.ДокументПатент      = ДесериализоватьСсылкуНаДокумент(СтрокаДокумента.ДокументПатент, "ОперацияСПатентом");
		НоваяСтрока.СуммаУменьшеноРанее = СтрокаДокумента.СуммаУменьшеноРанее;
		НоваяСтрока.Сумма               = СтрокаДокумента.Сумма;
...
Процедура ЗагрузитьОперацияСПатентом(ОбъектXDTO, УзелОбмена, ПроведениеДокументов, ДанныеРегистрацииВНалоговомОргане)
	
	Ссылка = ДесериализоватьСсылкуНаДокумент(ОбъектXDTO.Ссылка, "ОперацияСПатентом");
	Объект = Ссылка.ПолучитьОбъект();
	
	ЭтоНовыйДокумент = (Объект = Неопределено);
	Если ЭтоНовыйДокумент Тогда
		Объект = Документы.ОперацияСПатентом.СоздатьДокумент();
		Объект.УстановитьСсылкуНового(Ссылка);
	КонецЕсли;
	...
	Объект.НалоговаяСтавка = ОбъектXDTO.НалоговаяСтавка;
...
Функция ИмяМенеджераИзXDTOОбъекта(ОбъектXDTO)
	...
	ИначеЕсли ТипОбъекта = "ЗаявлениеОЗачетеВСчетПредстоящейОбязанности" Тогда
		Возврат "Документы.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности";
	ИначеЕсли ТипОбъекта = "УведомлениеОСпецрежимахНалогообложения" Тогда
		Возврат "Документы.УведомлениеОСпецрежимахНалогообложения";
	ИначеЕсли ТипОбъекта = "ОперацияПоЕдиномуНалоговомуСчету" Тогда
		Возврат "Документы.ОперацияПоЕдиномуНалоговомуСчету";
	ИначеЕсли ТипОбъекта = "ОперацияСПатентом" Тогда
		Возврат "Документы.ОперацияСПатентом";
	ИначеЕсли ТипОбъекта = "УведомлениеОбИсчисленныхСуммахНалогов" Тогда
		Возврат "Документы.УведомлениеОбИсчисленныхСуммахНалогов";
	ИначеЕсли ТипОбъекта = "УведомлениеОбУменьшенииНалогаПоПатенту" Тогда
		Возврат "Документы.УведомлениеОбУменьшенииНалогаПоПатенту";
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ВыгрузитьИспользоватьТипыЦенНоменклатуры(Данные)
	
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ИспользоватьТипыЦенНоменклатуры");
	ПередаваемыйОбъект.Значение = Данные.Значение;
	Возврат ПередаваемыйОбъект;
	
КонецФункции

Функция ВыгрузитьИспользоватьНумерациюДоговоровСПокупателями(Данные)
	
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ИспользоватьНумерациюДоговоровСПокупателями");
	ПередаваемыйОбъект.Значение = Данные.Значение;
	Возврат ПередаваемыйОбъект;
	
КонецФункции

Функция ВыгрузитьИспользоватьОнлайнОплаты(Данные)
...
Функция ПодготовитьСообщениеВыгрузки(СтруктураПараметров, Отбор = Неопределено, ПолноеИмяФайлаОбмена = "")
	...
	ЗагрузитьОперацииОнлайнОплат(СтруктураПараметров);
...
Функция ПолучитьОбъектXDTO(Данные, СтруктураПараметров)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("КонстантаМенеджерЗначения.ИспользоватьОнлайнОплаты") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьИспользоватьОнлайнОплаты(Данные);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("КонстантаМенеджерЗначения.ИспользоватьТипыЦенНоменклатуры") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьИспользоватьТипыЦенНоменклатуры(Данные);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("КонстантаМенеджерЗначения.ИспользоватьНумерациюДоговоровСПокупателями") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьИспользоватьНумерациюДоговоровСПокупателями(Данные);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("СправочникОбъект.НастройкиОнлайнОплат") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьНастройкиОнлайнОплаты(Данные, СтруктураПараметров);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.Доверенность") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьДоверенность(Данные, СтруктураПараметров);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.ОперацияПоЕдиномуНалоговомуСчету") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьОперацияПоЕдиномуНалоговомуСчету(Данные, СтруктураПараметров);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.УведомлениеОбИсчисленныхСуммахНалогов") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьУведомлениеОбИсчисленныхСуммахНалогов(Данные, СтруктураПараметров);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.УведомлениеОбУменьшенииНалогаПоПатенту") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьУведомлениеОбУменьшенииНалогаПоПатенту(Данные, СтруктураПараметров);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.ОперацияСПатентом") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьОперацияСПатентом(Данные, СтруктураПараметров);
		
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(Данные, СтруктураПараметров);
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("РегистрСведенийНаборЗаписей.ЗадачиБухгалтераНалоговыеПлатежи") Тогда
		
		ПередаваемыйОбъект = ВыгрузитьЗадачиБухгалтераНалоговыеПлатежи(Данные, СтруктураПараметров);
...
Функция ВыгрузитьБанковскиеСчета(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы >= 23 Тогда
		ПередаваемыйОбъект.ДатаОткрытия = Данные.ДатаОткрытия;
		ПередаваемыйОбъект.ДатаЗакрытия = Данные.ДатаЗакрытия;
	КонецЕсли;
	...
	Если СтруктураПараметров.ВерсияСхемы >= 5 Тогда
		...
		КонецЕсли;
		ПередаваемыйОбъект.ЗапрещеноРедактироватьВМобильном = Данные.Валютный ИЛИ НЕ ЭтоБанкРФ Или Данные.ЦифровойСчет;
...
Функция ВыгрузитьПатент(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект.НомерПатента        = Данные.УдалитьНомерПатента;
	ПередаваемыйОбъект.ДатаВыдачи          = Данные.УдалитьДатаВыдачи;
	ПередаваемыйОбъект.КодПоОКТМО          = Данные.УдалитьКодПоОКТМО;
	ПередаваемыйОбъект.КБК                 = Данные.УдалитьКБК;
	ПередаваемыйОбъект.ДатаНачала          = Данные.УдалитьДатаНачала;
	ПередаваемыйОбъект.ДатаОкончания       = Данные.УдалитьДатаОкончания;
	
	ПередаваемыйОбъект.ДатаПервогоПлатежа  = Данные.УдалитьДатаПервогоПлатежа;
	ПередаваемыйОбъект.СуммаПервогоПлатежа = Данные.УдалитьСуммаПервогоПлатежа;
	ПередаваемыйОбъект.ДатаВторогоПлатежа  = Данные.УдалитьДатаВторогоПлатежа;
	ПередаваемыйОбъект.СуммаВторогоПлатежа = Данные.УдалитьСуммаВторогоПлатежа;
	
	ПередаваемыйОбъект.ПотенциальноВозможныйГодовойДоход = Данные.УдалитьПотенциальноВозможныйГодовойДоход;
	
	Если СтруктураПараметров.ВерсияСхемы >= 15 Тогда
		ПередаваемыйОбъект.СуммаОсвобожденияОтНалога = Данные.УдалитьСуммаОсвобожденияОтНалога;
	КонецЕсли;
	
	Если Данные.УдалитьПостановкаНаУчетВНалоговомОргане = Перечисления.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане
		И ЗначениеЗаполнено(Данные.УдалитьНалоговыйОрган) Тогда
		ПередаваемыйОбъект.КодНалоговогоОргана = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Данные.УдалитьНалоговыйОрган, "Код");
	КонецЕсли;
...
Функция ВыгрузитьКонтрагенты(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы >=31 Тогда
		ПередаваемыйОбъект.ИндивидуальныйПредприниматель = Данные.ИндивидуальныйПредприниматель;
	КонецЕсли;
...
Функция ВыгрузитьНоменклатура(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы <18 Тогда
		Если Данные.ЭтоГруппа Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
	...
	Если СтруктураПараметров.ВерсияСхемы >= 2 Тогда
		...
		ИначеЕсли ТипЗнч(ОтчетСсылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если ОбщегоНазначения.СсылкаСуществует(ОтчетСсылка) Тогда
				ПараметрыОтчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОтчетСсылка, "ИсточникОтчета, ВыбраннаяФорма, ДатаОкончания");
				...
				Попытка
					Если ЗначениеЗаполнено(ПараметрыОтчета.ВыбраннаяФорма) Тогда
						ИмяФормыРеглОтчета = ПараметрыОтчета.ВыбраннаяФорма;
					Иначе
...
Функция ВыгрузитьНастройкиСистемыНалогообложения(Данные, СтруктураПараметров)
	...
	Для каждого ДанныеЗаписи Из Данные Цикл
		...
		Если СтруктураПараметров.ВерсияСхемы >=20 Тогда
			ПередаваемаяЗапись.ПлательщикЕНП                           = ДанныеЗаписи.ПлательщикЕНП;
			ПередаваемаяЗапись.ПрименяетсяАУСНДоходы                   = ДанныеЗаписи.ПрименяетсяАУСН
				И ДанныеЗаписи.ОбъектНалогообложенияУСН = Перечисления.ОбъектыНалогообложенияПоУСН.Доходы;
			ПередаваемаяЗапись.ПрименяетсяАУСНДоходыМинусРасходы       = ДанныеЗаписи.ПрименяетсяАУСН
				И ДанныеЗаписи.ОбъектНалогообложенияУСН = Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы;
		КонецЕсли;
		
		Если СтруктураПараметров.ВерсияСхемы >= 34 Тогда
			ПередаваемаяЗапись.ПлательщикНДСПрименяющийУСН             = ДанныеЗаписи.ПлательщикНДСПрименяющийУСН;
		КонецЕсли;
...
Функция ВыгрузитьНастройкиУчетаУСН(Данные, СтруктураПараметров)
	...
	Для каждого ДанныеЗаписи Из Данные Цикл
		...
		Если СтруктураПараметров.ВерсияСхемы >=19 Тогда
			ПередаваемаяЗапись.ОснованиеЛьготнойСтавки = ДанныеЗаписи.ОснованиеЛьготнойСтавки;
		КонецЕсли;
		ПередаваемыйОбъект.Записи.Добавить(ПередаваемаяЗапись);
	КонецЦикла;
...
Функция ВыгрузитьНастройкиПродленияСроковНалоговОтчетов(Данные, СтруктураПараметров)
	...
	Для каждого ДанныеЗаписи Из Данные Цикл
		...
		Если СтруктураПараметров.ВерсияСхемы >= 21 Тогда
			ПередаваемаяЗапись.ПрименяетсяПостановление512 =
							ДанныеЗаписи.ПрименяетсяПостановление512;
			ПередаваемаяЗапись.ПрименяетсяПостановление776П1 =
							ДанныеЗаписи.ПрименяетсяПостановление776П1;
			ПередаваемаяЗапись.ПрименяетсяПостановление776П2 =
							ДанныеЗаписи.ПрименяетсяПостановление776П2;
		КонецЕсли;
...
Функция ВыгрузитьЗадачиБухгалтераНалоговыеПлатежи(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ЗадачиБухгалтераНалоговыеПлатежи");
	
	ПередаваемыйОбъект.ОрганизацияОтбор            = СериализоватьСсылку(Данные.Отбор.Организация.Значение);
	ПередаваемыйОбъект.ПлатежноеПоручениеОтбор     = СериализоватьТипИСсылку(Данные.Отбор.ПлатежноеПоручение.Значение);
	
	Для каждого ДанныеЗаписи Из Данные Цикл
		
		ПередаваемаяЗапись = СоздатьОбъектXDTO("ЗадачиБухгалтераНалоговыеПлатежи.Запись");
		
		ПередаваемаяЗапись.Организация        = ПередаваемыйОбъект.ОрганизацияОтбор;
		ПередаваемаяЗапись.ПлатежноеПоручение = ПередаваемыйОбъект.ПлатежноеПоручениеОтбор;
		
		Если ТипЗнч(ДанныеЗаписи.Правило) = Тип("СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов") Тогда
		
			РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаписи.Правило, "Код, Владелец.Код");
			
			ПередаваемаяЗапись.ИдентификаторПравила = РеквизитыПравила.Код;
			ПередаваемаяЗапись.ИдентификаторЗадачи  = РеквизитыПравила.ВладелецКод;
			
		ИначеЕсли ТипЗнч(ДанныеЗаписи.Правило) = Тип("СправочникСсылка.Патенты") Тогда
		
			ПередаваемаяЗапись.ИдентификаторПравила = "ПАТЕНТ";
			ПередаваемаяЗапись.ИдентификаторЗадачи  = СериализоватьСсылку(ДанныеЗаписи.Правило);
		...
		ПередаваемаяЗапись.ОплатаЗадолженности         = ДанныеЗаписи.ОплатаЗадолженности;
...
Функция ВыгрузитьСписокЗадач(Данные, СтруктураПараметров)
	...
	Если ТипЗнч(Данные.Отбор.Правило.Значение) = Тип("СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов") Тогда
		...
		РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Данные.Отбор.Правило.Значение, "Код, Владелец.Код, Действие, ВыполняетсяЕдинымПомощником");
		...
		Если РеквизитыПравила.Действие <> Перечисления.ВидыДействийКалендаряБухгалтера.Отчет
			...
			И РеквизитыПравила.Действие <> Перечисления.ВидыДействийКалендаряБухгалтера.УплатаНалога
			И РеквизитыПравила.Действие <> Перечисления.ВидыДействийКалендаряБухгалтера.Уведомление Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		ВыполняетсяЕдинымПомощником = РеквизитыПравила.ВыполняетсяЕдинымПомощником;
	...
	Если Не ВыполняетсяЕдинымПомощником Тогда
		...
		Для каждого ДанныеЗаписи Из Данные Цикл
			...
			Если СтруктураПараметров.ВерсияСхемы >= 20 Тогда
				...
				ПередаваемаяЗапись.НачальныйСрок = ДанныеЗаписи.НачальныйСрок;
...
Функция ВыгрузитьОтветственныеЛицаОрганизаций(Данные, СтруктураПараметров)
	...
	ОтветственныеЛицаСрезПоследних = РегистрыСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(,Отбор);
	Если ОтветственныеЛицаСрезПоследних.Количество() = 0 Тогда
		//удаление последней записи
		Возврат ПередаваемыйОбъект;
		
	КонецЕсли;
	
	ДанныеЗаписи = ОтветственныеЛицаСрезПоследних[0];
...
Функция ВыгрузитьЧекиНПД(Данные, СтруктураПараметров)
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
		ТипДокумента = "ОплатаПлатежнойКартой";
	Иначе
...
Функция ВыгрузитьЖурналОтправокВКонтролирующиеОрганы(Данные, СтруктураПараметров)
	...
	Если ОбщегоНазначения.СсылкаСуществует(ОтчетСсылка) Тогда
		...
		РазрешенныеВиды = Новый Массив;
		Если СтруктураПараметров.ВерсияСхемы >= 25 Тогда
			РазрешенныеВиды.Добавить(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ИсчисленныеСуммыНалогов);
		КонецЕсли;
		Если СтруктураПараметров.ВерсияСхемы >= 28 Тогда
			РазрешенныеВиды.Добавить(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявлениеОЗачетеНалога);
		КонецЕсли;
		РазрешенныеВиды.Добавить(Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УменьшениеНалогаНаСтраховыеВзносы);
		
		Если НЕ ЗначениеЗаполнено(ПараметрыОтчета.ВидУведомления)
			ИЛИ РазрешенныеВиды.Найти(ПараметрыОтчета.ВидУведомления) = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ВыгрузитьНастройкиОнлайнОплаты(Данные, СтруктураПараметров)
...
Функция ВыгрузитьВводНачальныхОстатков(Данные, СтруктураПараметров)
	...
	Для каждого СтрокаДокумента Из Данные.РасчетыПоНалогамИСборам Цикл
		...
		ИначеЕсли СтрокаДокумента.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП
			И СтрокаДокумента.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
			
			ВидНалога = "ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф";
		...
		ИначеЕсли СтрокаДокумента.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф
			И СтрокаДокумента.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
			
			ВидНалога = "СтраховыеВзносыЕдиныйТариф";
			
		ИначеЕсли СтрокаДокумента.СчетУчета = ПланыСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет
			И СтрокаДокумента.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
			
			ВидНалога = "ЕдиныйНалоговыйПлатеж";
			
		ИначеЕсли СтрокаДокумента.СчетУчета = ПланыСчетов.Хозрасчетный.НалогПриАУСН
			И СтрокаДокумента.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
...
Функция ВыгрузитьПоступлениеНаРасчетныйСчет(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы >= 29 Тогда
		Если Данные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога Тогда
			ОписаниеВидаНалога = ОписаниеВидаНалога(Данные.Налог, Перечисления.ВидыПлатежейВГосБюджет.Налог, СтруктураПараметров.ВерсияСхемы);
			Если ЗначениеЗаполнено(ОписаниеВидаНалога.ВидНалога) Тогда
				ПередаваемыйОбъект.ВидНалога = ОписаниеВидаНалога.ВидНалога;
			Иначе
				ПередаваемыйОбъект.ВидНалога = "ПрочиеНалогиИСборы";
			...
			Если Данные.Налог <> Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж, Ложь) Тогда
				ЗапрещеноРедактироватьВМобильном = Истина;
			КонецЕсли;
	...
	ИначеЕсли УчетнаяПолитика.ПрименяетсяУСН(Данные.Организация, Данные.Дата)
		ИЛИ УчетнаяПолитика.ПрименяетсяАУСН(Данные.Организация, Данные.Дата) Тогда
	...
	Если СтруктураПараметров.ВерсияСхемы >= 5 Тогда
		...
		ПередаваемыйОбъект.ЗапрещеноРедактироватьВМобильном = 
			ЗапрещеноРедактироватьВМобильном Или ДокументЗапрещеноРедактироватьВМобильномПриложении(Данные);
	КонецЕсли;
	...
	Если СтруктураПараметров.ВерсияСхемы >= 6 И Данные.РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		Если СтруктураПараметров.ВерсияСхемы >= 17 Тогда
			РасшифровкаПлатежа = Данные.РасшифровкаПлатежа.Выгрузить(, "УслугаНПД, СуммаПлатежа");
			РасшифровкаПлатежа.Свернуть("УслугаНПД", "СуммаПлатежа");
			
			// Если есть услуга НПД в ТЧ,  то выгружаем табличную часть
			Если РасшифровкаПлатежа.Количество() > 1 
				ИЛИ ЗначениеЗаполнено(РасшифровкаПлатежа[0].УслугаНПД) Тогда
				Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
					ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ПоступлениеНаРасчетныйСчет.РасшифровкаПлатежа");
					
					ПередаваемаяСтрокаТЧ.УслугаНПД = СериализоватьСсылку(СтрокаРасшифровки.УслугаНПД);
					ПередаваемаяСтрокаТЧ.Сумма     = СтрокаРасшифровки.СуммаПлатежа;
					
					ПередаваемыйОбъект.РасшифровкаПлатежа.Добавить(ПередаваемаяСтрокаТЧ);
				КонецЦикла; 
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли;
...
Функция ВыгрузитьПоступлениеТоваровУслуг(Данные, СтруктураПараметров)
	...
	Если Данные.СкладВСписке Тогда
		Возврат ВыгрузитьУдалениеОбъекта(Данные, СтруктураПараметров);
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Данные.Товары Цикл
		...
		ЭтоРасчетнаяСтавкаНДС = Перечисления.СтавкиНДС.ЭтоРасчетнаяСтавкаНДС(СтрокаТЧ.СтавкаНДС);
...
Функция ВыгрузитьПриходныйКассовыйОрдер(Данные, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВерсияСхемы >= 6 И Данные.РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		Если СтруктураПараметров.ВерсияСхемы >= 17 Тогда
			...
			// Если есть услуша НПД в ТЧ,  то выгружаем табличную часть
			Если РасшифровкаПлатежа.Количество() > 1 
				ИЛИ ЗначениеЗаполнено(РасшифровкаПлатежа[0].УслугаНПД) Тогда
			
				Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
					ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ПриходныйКассовыйОрдер.РасшифровкаПлатежа");
...
Функция ВыгрузитьРеализацияТоваровУслуг(Данные, СтруктураПараметров)
	...
	Иначе
		Если УчетнаяПолитика.ПрименяетсяУСН(Данные.Организация, Данные.Дата)
			ИЛИ УчетнаяПолитика.ПрименяетсяАУСН(Данные.Организация, Данные.Дата) Тогда
...
Функция ВыгрузитьСчетНаОплатуПокупателю(Данные, СтруктураПараметров)
	...
	Если ЗначениеЗаполнено(Данные.ВидОперации)
		И Данные.ВидОперации <> Перечисления.ВидыОперацийСчетаПокупателю.ТоварыИУслуги Тогда
		Возврат ВыгрузитьУдалениеОбъекта(Данные, СтруктураПараметров);
	КонецЕсли;
...
Функция ВыгрузитьАктСверкиВзаиморасчетов(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект.СверкаСогласована = Документы.АктСверкиВзаиморасчетов.СверкаСогласована(Данные.Организация , Данные.Ссылка);
...
Функция ВыгрузитьДоверенность(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("Доверенность");
	ПередаваемыйОбъект.Ссылка          = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер           = Данные.Номер;
	ПередаваемыйОбъект.Дата            = Данные.Дата;
	ПередаваемыйОбъект.Проведен        = Данные.Проведен;
	...
	ПередаваемыйОбъект.БанковскийСчетОрганизации = СериализоватьСсылку(Данные.БанковскийСчетОрганизации);
	ПередаваемыйОбъект.ФизЛицо = СериализоватьСсылку(Данные.ФизЛицо);
	
	ПередаваемыйОбъект.ДатаДействия  = Данные.ДатаДействия;
	ПередаваемыйОбъект.НаПолучениеОт = Данные.НаПолучениеОт;
	ПередаваемыйОбъект.ПоДокументу   = Данные.ПоДокументу;
	
	Для каждого СтрокаДокумента Из Данные.Товары Цикл
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("Доверенность.Товары");
		ПередаваемаяСтрокаТЧ.НаименованиеТовара          = СтрокаДокумента.НаименованиеТовара;
		ПередаваемаяСтрокаТЧ.ЕдиницаПоКлассификатору     = СоздатьКлючевыеСвойстваПоСсылке(СтрокаДокумента.ЕдиницаПоКлассификатору, "ЕдиницаИзмерения");
		ПередаваемаяСтрокаТЧ.Количество        = СтрокаДокумента.Количество;
...
Функция ВыгрузитьОплатаПлатежнойКартой(Данные, СтруктураПараметров)
	Перем ВидОнлайнОплаты;
	
	Если Данные.ВидОперации <> Перечисления.ВидыОперацийОплатаПлатежнойКартой.РозничнаяВыручка 
		И СтруктураПараметров.Свойство("ВидОперацииОплатаПлатежнойКартой", ВидОнлайнОплаты) Тогда

		ВидОперацииВыгружается = ЗначениеЗаполнено(ВидОнлайнОплаты)
			И ВидОнлайнОплаты = Данные.ВидОплаты;
		
	Иначе
		ВидОперацииВыгружается = Ложь;
	КонецЕсли;
	
		
	Если Данные.Организация <> СтруктураПараметров.Организация
		ИЛИ Данные.Дата < СтруктураПараметров.ДатаНачалаВыгрузки
		ИЛИ НЕ ВидОперацииВыгружается Тогда
	...
	Если Данные.РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		Если СтруктураПараметров.ВерсияСхемы >= 17 Тогда
			...
			Если РасшифровкаПлатежа.Количество() > 1 
				...
				Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
					ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ОплатаПлатежнойКартой.РасшифровкаПлатежа");
...
Функция ВыгрузитьУведомлениеОбИсчисленныхСуммахНалогов(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("УведомлениеОбИсчисленныхСуммахНалогов");
	ПередаваемыйОбъект.Ссылка              = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер               = Данные.Номер;
	ПередаваемыйОбъект.Дата                = Данные.Дата;
	...
	ПередаваемыйОбъект.СуммаДокумента = Данные.СуммаДокумента;
	ПередаваемыйОбъект.Уведомление = СериализоватьСсылку(Данные.Уведомление);
	
	ЗапрещеноРедактироватьВМобильном = Ложь;
	Для каждого СтрокаДокумента Из Данные.Налоги Цикл
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("УведомлениеОбИсчисленныхСуммахНалогов.Налоги");
		
		ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Налог;
		ОписаниеВидаНалога = ОписаниеВидаНалогаВМобильном(СтрокаДокумента.Налог, ВидНалоговогоОбязательства, СтруктураПараметров.ВерсияСхемы);
		
		Если ЗначениеЗаполнено(ОписаниеВидаНалога.ВидНалога) Тогда
			ПередаваемаяСтрокаТЧ.ВидНалога = ОписаниеВидаНалога.ВидНалога;
		Иначе
			ПередаваемаяСтрокаТЧ.ВидНалога = "ПрочиеНалогиИСборы";
			ЗапрещеноРедактироватьВМобильном = Истина;
		КонецЕсли;
		
		КодИФНС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДокумента.РегистрацияВНалоговомОргане, "Код");
		ПередаваемаяСтрокаТЧ.КодНалоговогоОргана = КодИФНС;
		
		ПередаваемаяСтрокаТЧ.КодБК = СтрокаДокумента.КодБК;
		ПередаваемаяСтрокаТЧ.КодПоОКТМО = СтрокаДокумента.КодПоОКТМО;
		
		ПередаваемаяСтрокаТЧ.Сумма = СтрокаДокумента.Сумма;
		
		ПередаваемаяСтрокаТЧ.СрокУплаты = СтрокаДокумента.СрокУплаты;
		
		ПередаваемаяСтрокаТЧ.НачалоПериода = СтрокаДокумента.НачалоПериода;
		ПередаваемаяСтрокаТЧ.КонецПериода = СтрокаДокумента.КонецПериода;
		
		ПередаваемаяСтрокаТЧ.КлючСтроки = СтрокаДокумента.КлючСтроки;
		
		ПередаваемыйОбъект.Налоги.Добавить(ПередаваемаяСтрокаТЧ);
		
	КонецЦикла;
	
	ПередаваемыйОбъект.ЗапрещеноРедактироватьВМобильном = ЗапрещеноРедактироватьВМобильном
		Или Данные.НалоговыйАгентНДС.Количество() > 0;
	
	Возврат ПередаваемыйОбъект;
	
КонецФункции

Функция ВыгрузитьОперацияПоЕдиномуНалоговомуСчету(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ОперацияПоЕдиномуНалоговомуСчету");
	ПередаваемыйОбъект.Ссылка              = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер               = Данные.Номер;
	ПередаваемыйОбъект.Дата                = Данные.Дата;
	...
	Для каждого СтрокаДокумента Из Данные.Налоги Цикл
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ОперацияПоЕдиномуНалоговомуСчету.Налоги");
		
		Если ЗначениеЗаполнено(СтрокаДокумента.ВидПлатежаВГосБюджет) Тогда
			ВидНалоговогоОбязательства = СтрокаДокумента.ВидПлатежаВГосБюджет;
		Иначе
			ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Налог;
		КонецЕсли;
		...
		Если ТипЗнч(СтрокаДокумента.ПлатежныйДокумент) = Тип("ДокументСсылка.ОперацияБух") Тогда
			ЗапрещеноРедактироватьВМобильном = Истина;
		Иначе
			ПередаваемаяСтрокаТЧ.ПлатежныйДокумент = СериализоватьТипИСсылку(СтрокаДокумента.ПлатежныйДокумент);
		КонецЕсли;
		
		КорректировкаЕНС = СтрокаДокумента.КорректировкаЕНС;
		
		//В БП для операции НачисленияНалогов счет затрат игнорируется
		Если Данные.ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.УплатаНалогов Тогда
			//В БП для уплаты налога без указания счета затрат - движения только по ЕНС
			Если Не ЗначениеЗаполнено(СтрокаДокумента.СчетЗатрат) Тогда
				КорректировкаЕНС = Истина;
			КонецЕсли;
		КонецЕсли;
		
		ПередаваемаяСтрокаТЧ.КорректировкаЕНС = КорректировкаЕНС;
	...
	Для каждого СтрокаДокумента Из Данные.Санкции Цикл
		
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ОперацияПоЕдиномуНалоговомуСчету.Санкции");
		
		ПередаваемаяСтрокаТЧ.ВидПлатежа = ИмяЗначенияПеречисления(СтрокаДокумента.ВидПлатежа);
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.СчетЗатрат) Тогда
			//Получаем для ВидыПлатежейВГосБюджет.Налог, т.к. для ПФР с доходов нет штрафов
			ВидНалога = ВидНалогаВМобильномПоОтражениюВУчете(СтрокаДокумента.СчетЗатрат, Данные.Организация, Данные.Дата, Перечисления.ВидыПлатежейВГосБюджет.Налог);
		...
		Иначе
			ПередаваемаяСтрокаТЧ.КорректировкаЕНС = Истина;
		КонецЕсли;
		
		
		ПередаваемыйОбъект.Санкции.Добавить(ПередаваемаяСтрокаТЧ);
		
	КонецЦикла;
	
	//Движения по таблице ЕдиныйСчет не формируют движений по ЕНС_УЧетНалогов, но могу двигать бух.учет
	//При переходе на ЕНС отражается переплата по налогам и штрафам
	Для каждого СтрокаДокумента Из Данные.ЕдиныйСчет Цикл
		
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ОперацияПоЕдиномуНалоговомуСчету.ЕдиныйСчет");
		
		ПередаваемаяСтрокаТЧ.ВидДвижения = СтрокаДокумента.ВидДвижения;
		...
		ВидПлатежаВБюджет = Неопределено;
		
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаДокумента.СчетЗатрат);
		ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет;
		Если СвойстваСчета.ВидСубконто1 = ВидСубконто Тогда
			ВидПлатежаВБюджет = СтрокаДокумента.Субконто1;
		ИначеЕсли СвойстваСчета.ВидСубконто2 = ВидСубконто Тогда
			ВидПлатежаВБюджет = СтрокаДокумента.Субконто2;
		ИначеЕсли СвойстваСчета.ВидСубконто3 = ВидСубконто Тогда
			ВидПлатежаВБюджет = СтрокаДокумента.Субконто3;
		КонецЕсли;
		
		//Это налог или штраф
		Если ЗначениеЗаполнено(ВидПлатежаВБюджет) Тогда
			
			ВидНалога = ВидНалогаВМобильномПоОтражениюВУчете(СтрокаДокумента.СчетЗатрат, Данные.Организация, Данные.Дата, ВидПлатежаВБюджет);
			...
			ПередаваемаяСтрокаТЧ.ВидНалога = ВидНалога;
			ПередаваемаяСтрокаТЧ.ВидПлатежа = ИмяЗначенияПеречисления(ВидПлатежаВБюджет);

		КонецЕсли;
		
		ПередаваемыйОбъект.ЕдиныйСчет.Добавить(ПередаваемаяСтрокаТЧ);
...
Функция ВыгрузитьУведомлениеОСпецрежимахНалогообложения(Данные, СтруктураПараметров)
	...
	Если Данные.Организация <> СтруктураПараметров.Организация
		ИЛИ Данные.Дата < СтруктураПараметров.ДатаНачалаВыгрузки
		ИЛИ РазрешенныеВиды.Найти(Данные.ВидУведомления) = Неопределено Тогда
		Возврат ВыгрузитьУдалениеОбъекта(Данные, СтруктураПараметров);
	КонецЕсли;
	...
	Если СтруктураПараметров.ВерсияСхемы >= 25 Тогда
		ПередаваемыйОбъект.ВидУведомления = ИмяЗначенияПеречисления(Данные.ВидУведомления);
	КонецЕсли;
	
	Если СтруктураПараметров.ВерсияСхемы >= 33 Тогда
		ПередаваемыйОбъект.Основание = СериализоватьТипИСсылку(Данные.Основание);
	КонецЕсли;
	
	Если Данные.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.УменьшениеНалогаНаСтраховыеВзносы Тогда
...
Функция ВыгрузитьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ЗаявлениеОЗачетеВСчетПредстоящейОбязанности");
	ПередаваемыйОбъект.Ссылка              = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер               = Данные.Номер;
	ПередаваемыйОбъект.Дата                = Данные.Дата;
	...
	ЗапрещеноРедактироватьВМобильном = Ложь;
	Для каждого СтрокаДокумента Из Данные.Налоги Цикл
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("ЗаявлениеОЗачетеВСчетПредстоящейОбязанности.Налоги");
		...
		ЗапрещеноРедактироватьВМобильном = ЗапрещеноРедактироватьВМобильном Или СтрокаДокумента.НалоговыйАгент;
...
Функция ВыгрузитьУведомлениеОбУменьшенииНалогаПоПатенту(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("УведомлениеОбУменьшенииНалогаПоПатенту");
	ПередаваемыйОбъект.Ссылка              = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер               = Данные.Номер;
	ПередаваемыйОбъект.Дата                = Данные.Дата;
	...
	ПередаваемыйОбъект.СуммаРасходов = Данные.СуммаРасходов;
	ПередаваемыйОбъект.Уведомление = СериализоватьСсылку(Данные.Уведомление);
	
	КодИФНС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Данные.НалоговыйОрган, "Код");
	ПередаваемыйОбъект.КодНалоговогоОргана = КодИФНС;
	
	Для каждого СтрокаДокумента Из Данные.Патенты Цикл
		
		ПередаваемаяСтрокаТЧ = СоздатьОбъектXDTO("УведомлениеОбУменьшенииНалогаПоПатенту.Патенты");
		
		ПередаваемаяСтрокаТЧ.ДокументПатент = СериализоватьСсылку(СтрокаДокумента.ДокументПатент);
		
		ПередаваемаяСтрокаТЧ.СуммаУменьшеноРанее = СтрокаДокумента.СуммаУменьшеноРанее;
		ПередаваемаяСтрокаТЧ.Сумма               = СтрокаДокумента.Сумма;
		
		ПередаваемыйОбъект.Патенты.Добавить(ПередаваемаяСтрокаТЧ);
...
Функция ВыгрузитьОперацияСПатентом(Данные, СтруктураПараметров)
	...
	ПередаваемыйОбъект = СоздатьОбъектXDTO("ОперацияСПатентом");
	ПередаваемыйОбъект.Ссылка              = СериализоватьСсылку(Данные.Ссылка);
	ПередаваемыйОбъект.Номер               = Данные.Номер;
	ПередаваемыйОбъект.Дата                = Данные.Дата;
	...
	ПередаваемыйОбъект.НалоговаяСтавка = Данные.НалоговаяСтавка;
...
Функция ВыгрузитьУдалениеОбъекта(Данные, СтруктураПараметров)
	...
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("СправочникСсылка.НастройкиОнлайнОплат") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "НастройкаЯндексКассы";
	...
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "ОплатаПлатежнойКартой";
	...
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "УведомлениеОСпецрежимахНалогообложения";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.Доверенность") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "Доверенность";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "УведомлениеОбИсчисленныхСуммахНалогов";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.УведомлениеОбУменьшенииНалогаПоПатенту") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "УведомлениеОбУменьшенииНалогаПоПатенту";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ОперацияСПатентом") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "ОперацияСПатентом";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ОперацияПоЕдиномуНалоговомуСчету") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "ОперацияПоЕдиномуНалоговомуСчету";
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности") Тогда
		ПередаваемыйОбъект.ТипОбъекта = "ЗаявлениеОЗачетеВСчетПредстоящейОбязанности";
	Иначе
		ПередаваемыйОбъект = Неопределено;
	КонецЕсли;
...
Процедура ПриИзмененииВерсииСхемы(СтруктураПараметров)
	...
	Если НЕ СтруктураПараметров.ПерваяСинхронизация Тогда
		...
		Если ТекущаяВерсияСхемы < 15 И СтруктураПараметров.ВерсияСхемы >= 15 Тогда
			...
			|ГДЕ
			|	Патенты.УдалитьСуммаОсвобожденияОтНалога <> 0
		...
		Если ТекущаяВерсияСхемы < 16 И СтруктураПараметров.ВерсияСхемы >= 16 Тогда
			...
			Если УчетПСН.ВидыДеятельностиПоПатентуОрганизации(СтруктураПараметров.Организация, Дата(2021,1,1)).Количество() > 0 Тогда
		...
		Если ТекущаяВерсияСхемы < 17 И СтруктураПараметров.ВерсияСхемы >= 17 Тогда
			
			Запрос = Новый Запрос;
			
			ДатаНачалаПримененияНПД = Неопределено;
			ДатаНачалаРаботы = ?(ЗначениеЗаполнено(СтруктураПараметров.ДатаНачалаВыгрузки), СтруктураПараметров.ДатаНачалаВыгрузки, СтруктураПараметров.ДатаНачалаРаботы);
			
			Если УчетнаяПолитика.ПрименяетсяНалогНаПрофессиональныйДоход(СтруктураПараметров.Организация, ДатаНачалаРаботы) Тогда
				ДатаНачалаПримененияНПД = ДатаНачалаРаботы;
			Иначе
				// Даже если на начало работы он не был самозанятым, возможно он стал им в процессе
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	НастройкиСистемыНалогообложенияСрезПервых.Период КАК ДатаНачалаПримененияНПД
				|ИЗ
				|	РегистрСведений.НастройкиСистемыНалогообложения.СрезПервых(
				|			&ДатаНачалаРаботы,
				|			Организация = &Организация
				|				И ПрименяетсяНалогНаПрофессиональныйДоход = ИСТИНА) КАК НастройкиСистемыНалогообложенияСрезПервых";
				Запрос.УстановитьПараметр("Организация",        СтруктураПараметров.Организация);
				Запрос.УстановитьПараметр("ДатаНачалаРаботы",   ДатаНачалаРаботы);
				РезультатЗапроса = Запрос.Выполнить().Выбрать();
				Если РезультатЗапроса.Следующий() Тогда
					ДатаНачалаПримененияНПД = РезультатЗапроса.ДатаНачалаПримененияНПД;
				КонецЕсли; 
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(ДатаНачалаПримененияНПД) Тогда
				Запрос.Текст = 
				"ВЫБРАТЬ
				...
				|			И (ПриходныйКассовыйОрдерРасшифровкаПлатежа.УслугаНПД <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
				|ГДЕ
				|	ПриходныйКассовыйОрдер.Организация = &Организация
				|	И ПриходныйКассовыйОрдер.Дата >= &ДатаНачалаПримененияНПД
				|
				|СГРУППИРОВАТЬ ПО
				|	ПриходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка
				|
				|ИМЕЮЩИЕ
				|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПриходныйКассовыйОрдерРасшифровкаПлатежа.УслугаНПД) > 1
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				...
				|			И (ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.УслугаНПД <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
				|ГДЕ
				|	ПоступлениеНаРасчетныйСчет.Организация = &Организация
				|	И ПоступлениеНаРасчетныйСчет.Дата >= &ДатаНачалаПримененияНПД
				|
				|СГРУППИРОВАТЬ ПО
				|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Ссылка
				|
				|ИМЕЮЩИЕ
				|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.УслугаНПД) > 1";
			
				Запрос.УстановитьПараметр("Организация",             СтруктураПараметров.Организация);
				Запрос.УстановитьПараметр("ДатаНачалаПримененияНПД", ДатаНачалаПримененияНПД);
			...
			// Выгружаем все оплаты платежной картой, потому что добавился реквизит - вид операции
			ВидОнлайнОплаты = Неопределено;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьОнлайнОплаты")
				И ЕстьНастройкаОнлайнОплатыДляОрганизации(СтруктураПараметров.Организация, ВидОнлайнОплаты) Тогда
				...
				|ГДЕ
				|	ОплатаПлатежнойКартой.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОплатаПлатежнойКартой.РозничнаяВыручка)
				|	И ОплатаПлатежнойКартой.Организация = &Организация
				|	И ОплатаПлатежнойКартой.ВидОплаты = &ВидОплаты";
				
				Запрос.УстановитьПараметр("ВидОплаты",   ВидОнлайнОплаты);
				Запрос.УстановитьПараметр("Организация", СтруктураПараметров.Организация);
				...
				Пока Выборка.Следующий() Цикл
					...
					НаборЗаписей = РегистрыСведений.ЧекиНПД.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.ДокументОснование.Значение       = Выборка.Ссылка;
					НаборЗаписей.Отбор.ДокументОснование.Использование  = Истина;
					НаборЗаписей.Отбор.Организация.Значение             = СтруктураПараметров.Организация;
					НаборЗаписей.Отбор.Организация.Использование        = Истина;
					
					ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена, НаборЗаписей)
					
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		Если ТекущаяВерсияСхемы < 18 И СтруктураПараметров.ВерсияСхемы >= 18 Тогда
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	ЕСТЬNULL(КОЛИЧЕСТВО(*), 0) КАК КоличествоЭлементов
			|ИЗ
			...
			Если КоличествоЭлементов > 1000 Тогда
				Запрос = Новый Запрос;
				Запрос.Текст =
				"ВЫБРАТЬ
			...
			ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена,
				Метаданные.Константы.ИспользоватьТипыЦенНоменклатуры);
			КонецЕсли;
			
			Если ТекущаяВерсияСхемы < 20 И СтруктураПараметров.ВерсияСхемы >= 20 Тогда
				
				Если ПолучитьФункциональнуюОпцию("ИспользуетсяАУСН") Тогда
					ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена,
					Метаданные.РегистрыСведений.НастройкиСистемыНалогообложения);
				КонецЕсли;
			КонецЕсли;
			Если ТекущаяВерсияСхемы < 21 И СтруктураПараметров.ВерсияСхемы >= 21 Тогда
				
				ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена,
					Метаданные.РегистрыСведений.НастройкиПродленияСроковНалоговОтчетов);
				
			КонецЕсли;
			Если ТекущаяВерсияСхемы < 22 И СтруктураПараметров.ВерсияСхемы >= 22 Тогда
				
				ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена,
					Метаданные.Константы.ИспользоватьНумерациюДоговоровСПокупателями);
				
			КонецЕсли;
			
			Если ТекущаяВерсияСхемы < 23 И СтруктураПараметров.ВерсияСхемы >= 23 Тогда
				...
				|	И Доверенность.Дата >= &ДатаНачалаРаботы
				|	И Доверенность.ДатаДействия >= &ДатаДействия";
				
				Запрос.УстановитьПараметр("ДатаДействия", НачалоДня(ТекущаяДатаСеанса()));
				Запрос.УстановитьПараметр("Организация", СтруктураПараметров.Организация);
				Запрос.УстановитьПараметр("ДатаНачалаРаботы",   ДатаНачалаРаботы);
			...
			Если ТекущаяВерсияСхемы < 29 И СтруктураПараметров.ВерсияСхемы >= 29 Тогда
				...
				|	Документ.ПоступлениеНаРасчетныйСчет КАК ПоступлениеНаРасчетныйСчет
				|ГДЕ
				|	ПоступлениеНаРасчетныйСчет.ВидОперации = &ВидОперации
				|	И ПоступлениеНаРасчетныйСчет.Организация = &Организация";
				Запрос.УстановитьПараметр("Организация", СтруктураПараметров.Организация);
				Запрос.УстановитьПараметр("ВидОперации",
					Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога);
				Выборка = Запрос.Выполнить().Выбрать();
				Пока Выборка.Следующий() Цикл
					ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена, Выборка.Ссылка);
				КонецЦикла;
			...
			Если ТекущаяВерсияСхемы < 31 И СтруктураПараметров.ВерсияСхемы >= 31 Тогда
				...
				|	Справочник.Контрагенты КАК Контрагенты
				|ГДЕ
				|	Контрагенты.ИндивидуальныйПредприниматель";
				Выборка = Запрос.Выполнить().Выбрать();
				Пока Выборка.Следующий() Цикл
					ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена, Выборка.Ссылка);
				КонецЦикла;
			...
			Если ТекущаяВерсияСхемы < 33 И СтруктураПараметров.ВерсияСхемы >= 33 Тогда
				
				Документы.ОперацияСПатентом.СоздатьДокументыПолученияПатентаПоДаннымСправочника();
				...
				|ГДЕ
				|	УведомлениеОбУменьшенииНалогаПоПатенту.Организация = &Организация";
				Запрос.УстановитьПараметр("Организация", СтруктураПараметров.Организация);
				Выборка = Запрос.Выполнить().Выбрать();
				Пока Выборка.Следующий() Цикл
					ПланыОбмена.ЗарегистрироватьИзменения(СтруктураПараметров.УзелОбмена, Выборка.Ссылка);
...
Процедура УстановитьУчетнуюПолитику(Организация, НастройкиУчетнойПолитики)
	...
	Если НастройкиУчетнойПолитики.Свойство("НастройкиСистемыНалогообложения", НастройкиСистемыНалогообложения) Тогда
		...
		Для каждого НастройкаСистемыНалогообложения Из НастройкиСистемыНалогообложения Цикл
			...
			ДанныеЗаполнения.Вставить("ПлательщикНДСПрименяющийУСН",    НастройкаСистемыНалогообложения.ПлательщикНДСПрименяющийУСН);
			ДанныеЗаполнения.Вставить("ВариантБухгалтерскойОтчетности", Перечисления.ВариантыБухгалтерскойОтчетности.ДляМалыхПредприятий);
			...
			ИначеЕсли НастройкаСистемыНалогообложения.ПрименяетсяАУСНДоходы ИЛИ НастройкаСистемыНалогообложения.ПрименяетсяАУСНДоходыМинусРасходы Тогда
				ДанныеЗаполнения.Вставить("СистемаНалогообложения", Перечисления.СистемыНалогообложения.АУСН);
				ДанныеЗаполнения.Вставить("ПрименяетсяАУСН", Истина);
				Если НастройкаСистемыНалогообложения.ПрименяетсяАУСНДоходы Тогда
					ДанныеЗаполнения.Вставить("ОбъектНалогообложенияУСН",         Перечисления.ОбъектыНалогообложенияПоУСН.Доходы);
				ИначеЕсли НастройкаСистемыНалогообложения.ПрименяетсяАУСНДоходыМинусРасходы Тогда 
					ДанныеЗаполнения.Вставить("ОбъектНалогообложенияУСН",         Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы);
				КонецЕсли; 
	...
	Если НастройкиУчетнойПолитики.Свойство("НастройкиУчетаУСН", НастройкиУчетаУСН) Тогда
		...
		Для Каждого НастройкаУчетаУСН Из НастройкиУчетаУСН Цикл
			...
			ДанныеЗаполнения.Вставить("ОснованиеЛьготнойСтавки",          НастройкаУчетаУСН.ОснованиеЛьготнойСтавки);
...
//Устарела, следует использовать ОписаниеВидаНалогаВМобильном
Функция ОписаниеВидаНалога(ВидНалогаИПлатежаВБюджет, ВидНалоговогоОбязательства, ВерсияСхемы = 0)
	...
	Если ВидНалогаИПлатежаВБюджет = СпрМенеджер.ПрочиеНалогиИСборы Тогда
		
		ВидНалога = "ПрочиеНалогиИСборы";
		
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.ЕдиныйНалоговыйПлатеж Тогда
	...
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть, Ложь)
		И ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела
		Или ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(ВидыНалогов.ФиксированныеВзносы_СвышеПредела, Ложь) Тогда
	...
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(
		ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф, Ложь) Тогда
	...
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(
		ВидыНалогов.НДФЛ, Ложь, ,
		Справочники.ВидыНалоговИПлатежейВБюджет.КодВидаДоходаНДФЛАгент13()) Тогда
		
		ВидНалога = "НДФЛ";
		
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(
		ВидыНалогов.НДФЛ, Ложь, ,
		Справочники.ВидыНалоговИПлатежейВБюджет.КодВидаДоходаНДФЛАгент15()) Тогда
	...
	ИначеЕсли ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(ВидыНалогов.АУСН, Ложь) Тогда
...
// По ВидуНалога строкой из мобильного приложения, Счет учета и ВидПлатежаВБюджет
Функция ОтражениеВУчетеПоВидуНалогаМобильного(ВидНалога, Дата)
	
	Результат = Новый Структура("СчетУчета, ВидНалога, ВидПлатежаВБюджет");
	
	Если ВидНалога = "ФиксированныеВзносы_ПФР_СДоходов" Тогда
		
		Если Дата < ПлатежиВБюджетКлиентСерверПереопределяемый.ПериодИзмененияСтраховыхВзносов() Тогда
			ВидНалогаБП = Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть;
		Иначе
			ВидНалогаБП = Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела;
		КонецЕсли;
		
		Результат.ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалогаБП, Истина);
		Результат.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела;
		
	ИначеЕсли ВидНалога = "ФиксированныеВзносы_ПФР" Тогда
		
		ВидНалогаБП = Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть;
		
		Результат.ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалогаБП, Истина);
		Результат.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
		
	ИначеЕсли ВидНалога = "ФиксированныеВзносы_ФФОМС" Тогда
		
		ВидНалогаБП = Перечисления.ВидыНалогов.ФиксированныеВзносы_ФФОМС;
	...
	Иначе
		...
		ВидНалогаБП = ЗначениеПеречисленияПоИмени(Перечисления.ВидыНалогов, ВидНалога);
		
		Результат.ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалогаБП);
		Результат.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог;
	КонецЕсли;
	
	Результат.СчетУчета = Справочники.ВидыНалоговИПлатежейВБюджет.СчетУчета(Результат.ВидНалога, Дата);
	
	Возврат Результат;
	
КонецФункции

// По СправочникСсылка.ВидыНалоговИПлатежейВБюджет бухгалтерии получает строками ВидНалога и ВидПлатежаВБюджет строкой для мобильного
Функция ОписаниеВидаНалогаВМобильном(ВидНалогаИПлатежаВБюджет, ВидНалоговогоОбязательства, ВерсияСхемы = 0)
	...
	ФиксированныеВзносы_ПФР_СтраховаяЧасть = ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть;
	Если ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(ФиксированныеВзносы_ПФР_СтраховаяЧасть, Ложь)
		И ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела
		Или ВидНалогаИПлатежаВБюджет = СпрМенеджер.НалогПоВиду(ВидыНалогов.ФиксированныеВзносы_СвышеПредела, Ложь) Тогда
	...
	Иначе
		ВидНалогаСсылка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидНалогаИПлатежаВБюджет, "ВидНалога");
		Если ЗначениеЗаполнено(ВидНалогаСсылка) Тогда
			Если ВидНалогаСсылка = ФиксированныеВзносы_ПФР_СтраховаяЧасть Тогда
				ВидНалога = "ФиксированныеВзносы_ПФР";
			Иначе
				ВидНалога = ИмяЗначенияПеречисления(ВидНалогаСсылка);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// По счету учета и значению субконто счета ВидПлатежаВБюджет, определяет ВидНалога строкой для мобильного
Функция ВидНалогаВМобильномПоОтражениюВУчете(СчетУчета, Организация, Дата, ВидПлатежаВБюджет)
	
	ВидНалога = "";
	
	Если СчетУчета = ПланыСчетов.Хозрасчетный.ПФР_ОПС_ИП Тогда
		Если ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела Тогда
			ВидНалога = "ФиксированныеВзносы_ПФР_СДоходов";
		Иначе
			ВидНалога = "ФиксированныеВзносы_ПФР";
		КонецЕсли;
	//для отражения штрафов и пени
	ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.НалогПриПСН
		И ВидПлатежаВБюджет <> Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
		
		ВидНалога = "ПСН_ГородскойОкруг";
	Иначе
		
		ВидНалога = ИмяЗначенияПеречисления(РасчетыСБюджетом.ВидНалогаПоСчетуУчета(СчетУчета, Организация, Дата));
...
Процедура ЗаполнитьРегламентированныеОтчеты(СтруктураПараметров, ЗаполнениеОтчетности, СообщениеОбОшибке)
	...
	Для каждого СтрокаЗаполнения Из ЗаполнениеОтчетности Цикл
		...
		Если СтрокаЗаполнения.ВидОтчета = "РегламентированныйОтчетУСН" Тогда
			ДоступнаПечать = РегламентированнаяОтчетностьКлиентСервер.ОперацииСРегламентированнымОтчетом(
				СтрокаЗаполнения.ВидОтчета,
				ИмяФормыОтчета).ПечатьСоШтрихкодомPDF417НаСервере;
			Если Не ДоступнаПечать Или СтруктураПараметров.ВерсияСхемы < 35 Тогда
				//Получим форму предыдущую
				ИмяФормыОтчета = РегламентированнаяОтчетностьВызовСервера.ИмяФормыРеглОтчетаДействующейВОтчетномПериоде(
					СтрокаЗаполнения.ВидОтчета, ДобавитьМесяц(СтрокаЗаполнения.ДатаОкончания, -12));
			КонецЕсли;
		КонецЕсли;
...
Функция ДесериализоватьИПроверитьСсылкуНаОтчет(СсылкаНаОтчет, СообщениеОбОшибке)
	...
	Если НЕ ЗначениеЗаполнено(СохраненныйОтчет) ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(СохраненныйОтчет) Тогда
		...
		СообщениеОбОшибке = НСтр("ru = 'Отчет отсутствует в ИБ на сервере'");
...
Функция ПечатьДокументаРегламентированнойОтчетности(ВидДокумента, СерСсылка, СообщениеОбОшибке) Экспорт
	...
	ИначеЕсли ВидДокумента = "РегламентированноеУведомлениеУменьшениеНалогаНаСтраховыеВзносы"
		Или ВидДокумента = "УведомлениеИсчисленныеСуммыНалогов"
		Или ВидДокумента = "УведомлениеЗачетНалога" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(СерСсылка, "УведомлениеОСпецрежимахНалогообложения");
	ИначеЕсли ВидДокумента = "РегламентированноеУведомлениеИсчисленныеСуммыНалогов" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(СерСсылка, "УведомлениеОбИсчисленныхСуммахНалогов");
	ИначеЕсли ВидДокумента = "РегламентированноеУведомлениеЗачетНалога" Тогда
		Ссылка = ДесериализоватьСсылкуНаДокумент(СерСсылка, "ЗаявлениеОЗачетеВСчетПредстоящейОбязанности");
	Иначе
	...
	ИначеЕсли ВидДокумента = "РегламентированноеУведомлениеИсчисленныеСуммыНалогов"
		Или ВидДокумента = "РегламентированноеУведомлениеЗачетНалога" Тогда
		//Передаем ссылку на документ УведомлениеОбИсчисленныхСуммахНалогов
		Уведомление =  ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Уведомление");
...
Процедура ЗаполнитьУведомления(СтруктураПараметров, ЗаполнениеУведомлений, ОтложенныеСтатусыУведомлений, СообщениеОбОшибке)
	...
	Для каждого СтрокаЗаполнения Из ЗаполнениеУведомлений Цикл
		...
		Если ДанныеДляЗаполнения = Неопределено Тогда
			СообщениеОбОшибке = 
				НСтр("ru = 'Заполнение формы уведомления пока не поддерживается. Попробуйте позже'");
			Возврат;
		КонецЕслИ;
		...
		Попытка
			Если СтрокаЗаполнения.ИмяОтчета = "РегламентированноеУведомлениеУменьшениеНалогаНаСтраховыеВзносы" Тогда
				...
				ИначеЕсли СтрокаЗаполнения.ИмяФормы = "Форма2021_1" Тогда
					ЗаполненныйОтчет = МенеджерОтчета.СформироватьНовоеУведомлениеПоДаннымЗаполнения_2021(
						СтрокаЗаполнения.Организация,
						РегистрацияВНалоговомОргане,
						ДанныеДляЗаполнения,
						СтрокаЗаполнения.Ссылка);
...
//   Контейнер - Структура
//
Функция ПолучитьДанныеДекларацииУСН2015кв1_2021кв1(ДанныеОтчета)
...
//   Контейнер - Структура
//
Функция ПолучитьДанныеДекларацииУСН2025(ДанныеОтчета)
	...
	//Показатели 2025
	ДанныеМП.Вставить("ФиксированныеВзносыЕдиныйТариф", Раздел21.П002110015003);
	ДанныеМП.Вставить("ФиксированныеВзносыСДоходаВсего", Раздел21.П002110016003);
	ДанныеМП.Вставить("ФиксированныеВзносыСДоходаТекущийГод", Раздел21.П002110016103);
	ДанныеМП.Вставить("ФиксированныеВзносыСДоходаПредыдущийГод", Раздел21.П002110016203);
...
//   Контейнер - Структура
//
Процедура ЗаполнитьДекларациюУСН2015Кв1_2021кв1(ДанныеМП, Контейнер)
	
	Организация    = ДанныеМП.Организация;
	ОтчетныйПериод = ДанныеМП.ДатаНачала;
	...
	ПараметрыЛьготы = УчетУСН.ПараметрыПримененияЛьготнойСтавкиНалога(Организация, КонецГода(ОтчетныйПериод));
	
	Если ПараметрыЛьготы.ПрименяласьЛьготнаяСтавка Тогда
		Контейнер.Раздел21.Вставить("П002110012403", ПараметрыЛьготы.КодОснованияЛьготнойСтавки);
		Контейнер.Раздел21.Вставить("П002110012404", ПараметрыЛьготы.ОснованиеЛьготнойСтавки);
	КонецЕсли;
	...
	// Показатели 2025
	Если ДанныеОтчета.Свойство("ФиксированныеВзносыЕдиныйТариф") Тогда
		Контейнер.Раздел21.Вставить("П002110015003", ДанныеОтчета.ФиксированныеВзносыЕдиныйТариф);
		Контейнер.Раздел21.Вставить("П002110016003", ДанныеОтчета.ФиксированныеВзносыСДоходаВсего);
		Контейнер.Раздел21.Вставить("П002110016103", ДанныеОтчета.ФиксированныеВзносыСДоходаТекущийГод);
		Контейнер.Раздел21.Вставить("П002110016203", ДанныеОтчета.ФиксированныеВзносыСДоходаПредыдущийГод);
	КонецЕсли;
...
Процедура НайтиСоздатьУзлыОбмена(СтруктураПараметров)
	...
	Если СтруктураПараметров.УзелОбмена.Пустая() Тогда
		...
		НовыйУзел.ВерсияСхемы        = СтруктураПараметров.ВерсияСхемы;
		НовыйУзел.РасчетНалоговВБП   = СтруктураПараметров.РасчетНалоговВБП;
...
Процедура ЗаполнитьАктСверкиПоДаннымИБ(Объект)
	...
	ВалютаРегламентированногоУчета = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	СверкаСогласована = Документы.АктСверкиВзаиморасчетов.СверкаСогласована(Объект.Организация, Объект.Ссылка);
	...
	СтруктураПараметров.Вставить("ЗаполнятьДанныеКонтрагента",    СверкаСогласована);
		...
		Если ЭлементСоставаПланаОбмена.АвтоРегистрация = АвтоРегистрацияИзменений.Запретить Тогда
			...
			ИначеЕсли ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) Тогда // Регистры сведений
				...
				Если ОбъектМетаданных.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
					...
					Если ОбъектМетаданных = Метаданные.РегистрыСведений.СтатусыДокументов Тогда
...
Функция ДанныеПлатежаПоЗадачеБухгалтера(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	Версия = "";
	СтруктураПараметров.Свойство("Версия", Версия);
	
	Если Версия = "2023" Тогда
		ДанныеПлатежаПоЗадачеБухгалтера2023(СтруктураПараметров, СообщениеОбОшибке);
	Иначе
		ДанныеПлатежаПоЗадачеБухгалтера2022(СтруктураПараметров, СообщениеОбОшибке);
	КонецЕсли;
...
Функция ПараметрыРасчетаПатентаКУплате(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	Если СтруктураПараметров.Свойство("ДокументПатент") Тогда
		ДокументПатент      = Мобильнаябухгалтерия.ДесериализоватьСсылкуНаДокумент(СтруктураПараметров.ДокументПатент,"ОперацияСПатентом");
		ДатаПроверкиРасходов = РегистрыНакопления.РасходыУменьшающиеНалогПоОтдельнымРежимам.ДатаПроверкиРасходовПоПатентам(
			КонецМесяца(ПериодСобытия));
		ПоказателиРасходовСтраховыеВзносы = Обработки.ПомощникОплатыПатента.ПоказателиРасходовСтраховыеВзносы(Организация, ДокументПатент,
			НачалоГода(ПериодСобытия),
			ДатаПроверкиРасходов,
			ПериодСобытия);
		РасходыСтраховыеВзносы = ПоказателиРасходовСтраховыеВзносы.ВсегоРасходыСтраховыеВзносы;
	Иначе
		РасходыСтраховыеВзносы =
			РегистрыНакопления.РасходыУменьшающиеНалогПоОтдельнымРежимам.РасходыПСНЗаПериод(Организация,
				НачалоГода(ПериодСобытия),
				КонецГода(ПериодСобытия));
	КонецЕсли;
	
	МассивПравилУведомлений = Новый Массив;
	МассивПравилУведомлений.Добавить(Патент);
	...
	СписокУведомлений = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаУведомления);
	Для каждого СтрокаУведомления Из СписокУведомлений Цикл
		СтрокаУведомления.Уведомление = СериализоватьТипИСсылку(СтрокаУведомления.Уведомление);
	КонецЦикла;
	
	Результат = Новый Структура;
	Результат.Вставить("РасходыСтраховыеВзносы", РасходыСтраховыеВзносы);
	Результат.Вставить("Уплачено",               Уплачено);
	Результат.Вставить("Уведомления",            СписокУведомлений);
		
	Возврат СериализаторXDTO.ЗаписатьXDTO(Результат);
	
КонецФункции

Функция СведенияОЕдиномНалоговСчете(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	Если ИмяМетода = "ОСТАТОК" Тогда
		
		Период = СтруктураПараметров.Период;
		Результат.Вставить("Сумма", ЕдиныйНалоговыйСчет.ОстатокНаЕдиномНалоговомСчете(Организация, Период));
		
	ИначеЕсли ИмяМетода = "ОСТАТОК_РАСЧЕТОВ_БУ" Тогда
		
		ДатаДокумента = СтруктураПараметров.Период;
		ВидОперации = ЗначениеПеречисленияПоИмени(Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету, СтруктураПараметров.ВидОперации);
		ВводНачальныхДанных = СтруктураПараметров.ВводНачальныхДанных;
		
		СписокНалоговДляЗаполнения = Новый СписокЗначений;
		Для каждого ВидНалогаМБ Из СтруктураПараметров.СписокНалогов Цикл
			ОтражениеВУчете = ОтражениеВУчетеПоВидуНалогаМобильного(ВидНалогаМБ, ДатаДокумента);
			Если ЗначениеЗаполнено(ОтражениеВУчете.ВидНалога) Тогда
				СписокНалоговДляЗаполнения.Добавить(ОтражениеВУчете.ВидНалога);
			КонецЕсли;
		КонецЦикла;
		
		ПараметрыДокумента = ЕдиныйНалоговыйСчет.ПараметрыПолученияОстатковРасчетов();
		ПараметрыДокумента.Организация         = Организация;
		ПараметрыДокумента.Дата                = ДатаДокумента;
		ПараметрыДокумента.ВидОперации         = ВидОперации;
		ПараметрыДокумента.СписокНалогов       = СписокНалоговДляЗаполнения;
		ПараметрыДокумента.ВводНачальныхДанных = ВводНачальныхДанных;
		...
		Если ВводНачальныхДанных Тогда
			...
			ТаблицыОстатков = Обработки.ПомощникПереходаНаЕНС.ПолучитьОстаткиРасчетов(ПараметрыДокумента, Исключения);
		Иначе
			
			ТаблицыОстатков = ЕдиныйНалоговыйСчет.ПолучитьОстаткиРасчетовПоНалогам(ПараметрыДокумента);
		КонецЕсли;
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов Тогда
			...
			Результат.Вставить(ИмяТаблицы, Новый Массив);
			
			Для каждого СтрокаТаблицаДанных Из ТаблицыОстатков[ИмяТаблицы] Цикл
				
				ПередаваемаяСтрокаТЧ = Новый Структура();
				
				Если ЗначениеЗаполнено(СтрокаТаблицаДанных.ВидПлатежаВГосБюджет) Тогда
					ВидНалоговогоОбязательства = СтрокаТаблицаДанных.ВидПлатежаВГосБюджет;
				Иначе
					ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Налог;
				КонецЕсли;
			
				ОписаниеВидаНалога = ОписаниеВидаНалогаВМобильном(СтрокаТаблицаДанных.Налог, ВидНалоговогоОбязательства, ТекущаяВерсияСервиса());
				Если ЗначениеЗаполнено(ОписаниеВидаНалога.ВидНалога) Тогда
					ПередаваемаяСтрокаТЧ.Вставить("ВидНалога", ОписаниеВидаНалога.ВидНалога);
				Иначе
					Продолжить;
				КонецЕсли;
				
				КодИФНС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицаДанных.РегистрацияВНалоговомОргане, "Код");
				ПередаваемаяСтрокаТЧ.Вставить("КодНалоговогоОргана", КодИФНС);
				
				ПередаваемаяСтрокаТЧ.Вставить("КодБК",     СтрокаТаблицаДанных.КодБК);
				ПередаваемаяСтрокаТЧ.Вставить("КодПоОКТМО", СтрокаТаблицаДанных.КодПоОКТМО);
				ПередаваемаяСтрокаТЧ.Вставить("Сумма",      СтрокаТаблицаДанных.Сумма);
				
				ПередаваемаяСтрокаТЧ.Вставить("СрокУплаты", СтрокаТаблицаДанных.СрокУплаты);
				
				Результат[ИмяТаблицы].Добавить(ПередаваемаяСтрокаТЧ);
			КонецЦикла;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеПенейШтрафов Тогда
			...
			Для каждого СтрокаТаблицаДанных Из ТаблицыОстатков[ИмяТаблицы] Цикл
				...
				ВидНалога = "";
				Если ЗначениеЗаполнено(СтрокаТаблицаДанных.СчетЗатрат) Тогда
					//Получаем для ВидыПлатежейВГосБюджет.Налог, т.к. для ПФР с доходов нет штрафов
					ВидНалога = ВидНалогаВМобильномПоОтражениюВУчете(СтрокаТаблицаДанных.СчетЗатрат, Организация, ДатаДокумента, Перечисления.ВидыПлатежейВГосБюджет.Налог);
				...
				ПередаваемаяСтрокаТЧ.Вставить("ВидПлатежа", ИмяЗначенияПеречисления(СтрокаТаблицаДанных.ВидПлатежа));
				
				ПередаваемаяСтрокаТЧ.Вставить("ВидНалога", ВидНалога);
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.КорректировкаСчета Тогда
			...
			КонецЦикла
		Иначе
			СообщениеОбОшибке = "Заполнение остатков не поддерживается: " + ВидОперации;
		КонецЕсли;
		
	Иначе
		СообщениеОбОшибке = "Метод не поддерживается: " + ИмяМетода;
	КонецЕсли;
...
Процедура ДанныеПлатежаПоЗадачеБухгалтера2023(СтруктураПараметров, СообщениеОбОшибке)
	...
	ДанныеПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Правило, "Владелец,Периодичность");
	
	ПараметрыКоманды.Задача = ДанныеПравила.Владелец;
	Периодичность = ДанныеПравила.Периодичность;
	
	ДатаПолученияРеквизитов = ТекущаяДатаСеанса();
	...
	НачалоПериода = ИнтерфейсыВзаимодействияБРОКлиентСервер.НачалоПериода(Периодичность, СтруктураПараметров.ПериодСобытия);
	КонецПериода = ИнтерфейсыВзаимодействияБРОКлиентСервер.КонецПериода(Периодичность, СтруктураПараметров.ПериодСобытия);
	
	СрокУплатыНалогаПоПравилу = ВыполнениеЗадачБухгалтера.СрокВыполненияЗадачи(ОрганизацияСсылка, Правило, КонецПериода);
	
	ПараметрыНабораПравил = ВыполнениеЗадачБухгалтера.ПараметрыНабораПравил();
	ЗаполнитьЗначенияСвойств(ПараметрыНабораПравил, СтруктураПараметров);
	ПараметрыНабораПравил.Организация = ОрганизацияСсылка;
	ПараметрыНабораПравил.Правило = Правило;
	ВыполнениеЗадачБухгалтера.ОпределитьСвязанныеПравила(ПараметрыНабораПравил);
	ПравилоУведомления = ПараметрыНабораПравил.ПравилоУведомления;
	
	ПодаетсяУведомлениеПоНалогуЗаПериод = 
		СтруктураПараметров.ПериодСобытия >= НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж()
		И ЗначениеЗаполнено(ПравилоУведомления);
		
	СтруктураПараметров.Вставить("ПодаетсяУведомление", ПодаетсяУведомлениеПоНалогуЗаПериод);
	
	Если СтруктураПараметров.Свойство("Налоги")
		И СтруктураПараметров.Налоги.Количество() > 0  Тогда
		
		ОплатаЗадолженности = Ложь;
		ОстаткиПоНалогам = Неопределено;
		...
		Если ОписаниеДействия <> Неопределено И ОписаниеДействия.Свойство("ИмяФормы") Тогда
			...
			ОплатаЗадолженности = 
				(ОписаниеДействия.ИмяФормы = "Обработка.ПомощникУплатыНалоговВзносовПрошлыхЛет.Форма.ФормаОплатыЗаПрошлыеПериоды");
		Иначе
			ПомощникВыполненияЗадачи = Неопределено;
		КонецЕсли;
		
		Если ОплатаЗадолженности Тогда
			ОстаткиПоНалогам = Обработки.ПомощникУплатыНалоговВзносовПрошлыхЛет.НачальныеОстаткиПоНалогам(ОрганизацияСсылка, СтруктураПараметров.ПериодСобытия);
		КонецЕсли;
		...
		Для Каждого Налог Из СтруктураПараметров.Налоги Цикл
			...
			ВидНалоговИПлатежейВБюджет = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(ВидНалога);
			
			СвойстваКонтекста = ИсточникДанныхКонтекстаПлатежаВБюджет(СтруктураПараметров.ПериодСобытия,
				ОрганизацияСсылка,
				ВидНалоговИПлатежейВБюджет,
			...
			РеквизитыПлатежаВБюджет = Документы.ПлатежноеПоручение.РеквизитыПлатежногоПорученияВБюджетПоУмолчанию(
				ДатаПолученияРеквизитов, ОрганизацияСсылка,
				Истина,
				ВидНалоговИПлатежейВБюджет,
				ВидНалоговогоОбязательства,
				Контекст);
			...
			Если ПорядокУплаты <> Неопределено Тогда
				...
				КонецЕсли;
			Иначе
				СрокУплатыНалога = СрокУплатыНалогаПоПравилу;
			КонецЕсли;
			...
			Если Налог.НалогКод = "УСН_Доходы"
				...
				ИЛИ Налог.НалогКод = "ФиксированныеВзносы_ФСС"
				ИЛИ Налог.НалогКод = "ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф"
				ИЛИ Налог.НалогКод = "СтраховыеВзносыЕдиныйТариф"
				ИЛИ Налог.НалогКод = "ФиксированныеВзносы_СвышеПредела" Тогда
				...
				Если ОплатаЗадолженности Тогда
					Отбор = Новый Структура("ВидНалога, ВидПлатежаВБюджет", ВидНалога, ВидНалоговогоОбязательства);
					ДанныеОбОстатках = ОстаткиПоНалогам.НайтиСтроки(Отбор);
					Если ДанныеОбОстатках.Количество()=1 Тогда
						СуммаПлатежа = ДанныеОбОстатках[0].Задолженность;
					Иначе
						СуммаПлатежа = 0;
					КонецЕсли;
				Иначе
				...
				Если (ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть
					Или ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела)
					И ВидНалоговогоОбязательства =  Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела Тогда
					СрокУплатыНалога = Дата(Год(СрокУплатыНалога)+1, 7, 1);
					Обработки.РасчетСтраховыхВзносовИП.УточнитьСрокУплатыВзносовСвышеПредела(СрокУплатыНалога);
				КонецЕсли;
			...
			Налог.Вставить("СрокУплаты", СрокУплатыНалога);
			Налог.Вставить("НачалоПериода", НачалоПериода);
			Налог.Вставить("КонецПериода", КонецПериода);
			
		КонецЦикла;
		
		Если СтруктураПараметров.Свойство("Уведомления") Тогда

			МассивПравилУведомлений = Новый Массив;
			МассивПравилУведомлений.Добавить(Правило);
			МассивПравилУведомлений.Добавить(ВыполнениеЗадачБухгалтера.ПравилоУведомленияПоПравилуУплаты(Правило, ОрганизацияСсылка, СтруктураПараметров.ПериодСобытия));
			МассивПравилУведомлений.Добавить(ВыполнениеЗадачБухгалтера.ПравилоУведомленияПоПравилуОтчета(Правило, ОрганизацияСсылка, СтруктураПараметров.ПериодСобытия));
			МассивПравилУведомлений.Добавить(ВыполнениеЗадачБухгалтера.ПравилоУплатыПоПравилуОтчета(Правило, ОрганизацияСсылка, СтруктураПараметров.ПериодСобытия));
			МассивПравилУведомлений.Добавить(ВыполнениеЗадачБухгалтера.ПравилоОтчетаПоПравилуУплаты(Правило, ОрганизацияСсылка, СтруктураПараметров.ПериодСобытия));
			
			ТаблицаУведомления = ВыполнениеЗадачБухгалтера.ДанныеДляУведомлений(
				МассивПравилУведомлений,
				ОрганизацияСсылка,
				СтруктураПараметров.ПериодСобытия);
			...
			СтруктураПараметров.Уведомления = СписокУведомлений;
			
		КонецЕсли;
		
		Если СтруктураПараметров.Свойство("Платежи") Тогда

			ПлатежныеДокументы = Неопределено;
			Если ОписаниеДействия.ИмяФормы = "Обработка.РасчетСтраховыхВзносовИП.Форма" Тогда
				ПлатежныеДокументы =Обработки.РасчетСтраховыхВзносовИП.ПлатежныеДокументы(
					ОрганизацияСсылка,
					СтруктураПараметров.ПериодСобытия,
					Периодичность,
					Правило);
					
			ИначеЕсли ОписаниеДействия.ИмяФормы = "Обработка.ПомощникРасчетаНалогаУСН.Форма.Форма" Тогда
				ПлатежныеДокументы = Обработки.ПомощникРасчетаНалогаУСН.ДокументыУплатыНалогов(ОрганизацияСсылка,
						СтруктураПараметров.ПериодСобытия,
						Правило);
			ИначеЕсли ОплатаЗадолженности Тогда
				ПлатежныеДокументы = Обработки.ПомощникУплатыНалоговВзносовПрошлыхЛет.ДокументыУплаты(ОрганизацияСсылка,
						СтруктураПараметров.ПериодСобытия);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПлатежныеДокументы) Тогда
				
				СписокПлатежей = ОбщегоНазначения.ТаблицаЗначенийВМассив(
					ПлатежныеДокументы.скопировать(,"Ссылка, Оплачено,ПредставлениеДокумента,Сумма"));
				Для каждого СтрокаПлатежей Из СписокПлатежей Цикл
					СтрокаПлатежей.Ссылка = СериализоватьТипИСсылку(СтрокаПлатежей.Ссылка);
				КонецЦикла;
				
				СтруктураПараметров.Платежи = СписокПлатежей;
	...
	Иначе
		...
		ПараметрыЗадачи.ПериодОтчета  = ВыполнениеЗадачБухгалтера.ПериодОтчетаПоПравилуУплаты(ПараметрыЗадачи.ПравилоУплаты, ПараметрыЗадачи.ПериодСобытия);
		ПараметрыЗадачи.ПравилоОтчета = ВыполнениеЗадачБухгалтера.ПравилоОтчетаПоПравилуУплаты(ПараметрыЗадачи.ПравилоУплаты,
		...
		СведенияОРасчетеСуммы = РезультатВыполнения.СведенияОРасчетах;
		ТаблицаУведомлений = СведенияОРасчетеСуммы.ТаблицаУведомлений;
		ТаблицаРасчета = СведенияОРасчетеСуммы.ТаблицаРасчета;
		
		Если СтруктураПараметров.Свойство("Уведомления")
			И ТаблицаУведомлений <> Неопределено Тогда
			СписокУведомлений = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаУведомлений);
			Для каждого СтрокаУведомления Из СписокУведомлений Цикл
				СтрокаУведомления.Уведомление = СериализоватьТипИСсылку(СтрокаУведомления.Уведомление);
			КонецЦикла;
		...
		Если НЕ СведенияОРасчетеСуммы.РасчетСуммыВыполнен Или ТаблицаРасчета = Неопределено Тогда
			СообщениеОбОшибке = "Налог не расчитан или не заполнена декларация";
		Иначе
			
			Для Каждого СтрокаРасчета Из ТаблицаРасчета Цикл
				
				ВидНалога = СтрокаРасчета.СведенияОРасчете.Налог;
				
				Если Не ЗначениеЗаполнено(ВидНалога) Тогда
					Продолжить;
				КонецЕсли;
				
				ТаблицаПлатежей = СтрокаРасчета.СведенияОРасчете.ТаблицаПлатежей;
				
				ПодаетсяУведомлениеПоЗадаче = 
						ЗначениеЗаполнено(СтрокаРасчета.СведенияОРасчете.ПараметрыЗадачи.ПравилоУведомления);
				...
				Для Каждого СтрокаПлатежа Из ТаблицаПлатежей Цикл
					...
					РеквизитыПлатежаВБюджет = Документы.ПлатежноеПоручение.РеквизитыПлатежногоПорученияВБюджетПоУмолчанию(
						ДатаПолученияРеквизитов, ОрганизацияСсылка, Истина, ВидНалога, Перечисления.ВидыПлатежейВГосБюджет.Налог, Контекст);
					...
					СтруктураНалога.Вставить("НалогНаименование", СтрокаРасчета.ОписаниеНалога);
					...
					СтруктураНалога.Вставить("СрокУплаты", СрокУплатыНалога);
					СтруктураНалога.Вставить("НачалоПериода", НачалоПериода);
					СтруктураНалога.Вставить("КонецПериода", КонецПериода);
					
					СтруктураНалога.Вставить("ПодаетсяУведомление", ПодаетсяУведомлениеПоЗадаче);
					
					СтруктураНалога.Вставить("СуммаУведомления", СтрокаПлатежа.Сумма);
...
Процедура ДанныеПлатежаПоЗадачеБухгалтера2022(СтруктураПараметров, СообщениеОбОшибке)
	...
	Если СтруктураПараметров.Свойство("Налоги")
		...
		Для Каждого Налог Из СтруктураПараметров.Налоги Цикл
			...
			Если УчетнаяПолитика.ПлательщикЕНП(ОрганизацияСсылка, ДатаПолученияРеквизитов)
				И ЕдиныйНалоговыйСчетПовтИсп.ВидыНалоговУплачиваемыеОтдельно(ДатаПолученияРеквизитов).Найти(ВидНалога) = Неопределено Тогда
				
				ВидНалоговИПлатежейВБюджет = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(
			...
			Если Налог.НалогКод = "УСН_Доходы"
				...
				ИЛИ Налог.НалогКод = "ФиксированныеВзносы_ФСС"
				ИЛИ Налог.НалогКод = "ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф"
				ИЛИ Налог.НалогКод = "СтраховыеВзносыЕдиныйТариф" Тогда
...
Функция ЕстьНастройкаОнлайнОплатыДляОрганизации(Организация, ВидОнлайнОплаты)
	
	НастройкаОнлайнОплат = ОнлайнОплаты.НайтиНастройку(Новый Структура("Организация", Организация));
	
	Если ЗначениеЗаполнено(НастройкаОнлайнОплат) Тогда
		ВидОнлайнОплаты = ОнлайнОплатыБП.ВидОплатыИзНастройки(НастройкаОнлайнОплат);
	КонецЕсли; 
	
	Возврат ЗначениеЗаполнено(НастройкаОнлайнОплат);
...
Функция ВидыОперацийБанкКассаМобильноеПриложение()
	...
	СписокВидов.Добавить(Менеджер.ПереводНаДругойСчет);
	...
	СписокВидов.Добавить(Менеджер.ВозвратНалога);
	...
	Менеджер = Перечисления.ВидыОперацийОплатаПлатежнойКартой;
	СписокВидов.Добавить(Менеджер.ОплатаПокупателя);
...
Функция ДокументЗапрещеноРедактироватьВМобильномПриложении(ДанныеДокумента)
	...
	Если ТипЗнч(ДанныеДокумента) = Тип("ДокументОбъект.ПриходныйКассовыйОрдер")
		...
		ИЛИ ТипЗнч(ДанныеДокумента) = Тип("ДокументОбъект.ОплатаПлатежнойКартой")
		...
		Если (ТипЗнч(ДанныеДокумента) = Тип("ДокументОбъект.СписаниеСРасчетногоСчета")
			Или ТипЗнч(ДанныеДокумента) = Тип("ДокументОбъект.ПоступлениеНаРасчетныйСчет"))
			И ЗначениеЗаполнено(ДанныеДокумента.СчетОрганизации)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеДокумента.СчетОрганизации, "ЦифровойСчет") Тогда
			Возврат Истина;
		КонецЕсли;
		...
		Если ТипЗнч(ДанныеДокумента) <> Тип("ДокументОбъект.ПлатежноеПоручение")
			...
			Если НЕ УчетнаяПолитика.ПрименяетсяНалогНаПрофессиональныйДоход(ДанныеДокумента.Организация, ДанныеДокумента.Дата) Тогда
				Возврат Истина;
			Иначе
				ТаблицаДокумента = ДанныеДокумента.РасшифровкаПлатежа.Выгрузить(,"ДоговорКонтрагента");
				ТаблицаДокумента.Свернуть("ДоговорКонтрагента");
				
				Возврат ТаблицаДокумента.Количество() > 1;
			КонецЕсли; 
		КонецЕсли;
...
Процедура ИсправитьИнтерфейсБизнесСтарт() Экспорт
	
	Если ПолучитьФункциональнуюОпцию("ИнтерфейсТаксиБазовый")
		Или Не ОбщегоНазначенияБП.ЭтоБизнесСтарт()
		Или ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком()
		Или НЕ ОбменДаннымиВключен() Тогда
		Возврат;
	КонецЕсли;
	
	ОбщегоНазначенияБП.УстановитьКонстантуИнтерфейсТаксиБазовый();
...
#Область Форма2020_1
Функция НовыеДанныеЛистаА_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2020_1()
...
Функция НовыеДанныеЛистаБ_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2020_1()
...
#Область Форма2021_1
Функция НовыеДанныеЛистаТитульная_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1()
	
	Данные = Новый Структура;
	Данные.Вставить("ГодДействПат");
	
	Возврат Данные;
	
КонецФункции

Функция НовыеДанныеЛистаА_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1()
...
Функция НовыеДанныеЛистаБ_УведомлениеУменьшениеНалогаНаСтраховыеВзносы2021_1()
	
	Данные = Новый Структура;
	Данные.Вставить("ОбщСумУплСВ");
	Данные.Вставить("ОбщСумСВУмен");
	Данные.Вставить("ОбщСумСВУменУчт");
	Данные.Вставить("СумСВОст");
