...
//   Массив - массив имен установленных параметров сеанса.
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	//Мальцев А.Ю. +++
	Если ИменаПараметровСеанса.Найти("НомерСессииМаркировки") <> Неопределено Тогда
...
Процедура ПроверитьОбновлениеПараметровРаботыПрограммы(ИмяКонстанты, ИменаПараметров = "", Отказ = Неопределено) Экспорт
	...
	Если ИменаПараметров <> "" Тогда
		...
		Если ТребуетсяОбновление Тогда
			...
			ПодтвердитьОбновлениеПараметраРаботыПрограммы(ИмяКонстанты, ИменаПараметров);
...
Процедура ЗагрузитьОбновитьПараметрыРаботыПрограммы(ОшибкаУстановкиМонопольногоРежима = Неопределено, ВФоне = Ложь) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   И ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Возврат;
	Иначе // RadV временно, пока не обновим работу с расширениями из БСП
...
// Параметр Отправитель может быть не заполнен, например, при получении сообщения начального образа в АРМ.
// 
...
//	                   ОтборыИндексКартинкиДоступа, ОтборыЗаголовок";
...
Функция ДобавитьПараметрыРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	// +РАРУС
	ЛицензированиеСлужебный.ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры);
	// -РАРУС
...
Процедура ПередЗапускомПрограммы()
	...
	Если (Метаданные.ОсновныеРоли.Количество() <> 2 И Метаданные.ОсновныеРоли.Количество() <> 3 И Метаданные.ОсновныеРоли.Количество() <> 4)
