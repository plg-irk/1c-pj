...
Процедура ВыполнитьКомандуОборудования(ИмяКоманды, ИдентификаторУстройства, УникальныйИдентификатор, ОповещениеПриЗавершении, ПараметрыДанных = Неопределено) Экспорт
	
	Если ИмяКоманды = "ВыгрузитьДанные" Тогда
		ВыгрузитьДанныеВОборудование(ИдентификаторУстройства, УникальныйИдентификатор, ОповещениеПриЗавершении, ПараметрыДанных);
	ИначеЕсли ИмяКоманды = "ЗагрузитьДанные" Тогда
		НачатьЗагрузкуДанныхИзККМ(ИдентификаторУстройства, УникальныйИдентификатор, ОповещениеПриЗавершении, ПараметрыДанных);
	КонецЕсли;
...
Процедура ВыгрузитьДанныеВОборудование(ИдентификаторУстройства, УникальныйИдентификатор, ОповещениеПриЗавершении, ПараметрыДанных, ОтображатьСообщения = Истина)
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыгрузкуДанныхПослеВыбораУстройства", ЭтотОбъект, Контекст);
		МенеджерОборудованияКлиент.ПредложитьВыбратьУстройство(ОписаниеОповещения, "Рарус",
			НСтр("ru='Выберите оборудование'"), НСтр("ru='Оборудование не подключено'"));
	Иначе
		НачатьВыгрузкуДанныхПослеВыбораУстройства(ИдентификаторУстройства, Контекст);
	КонецЕсли;
	
КонецПроцедуры

Процедура НачатьЗагрузкуДанныхИзККМ(ИдентификаторУстройства, УникальныйИдентификатор, ОповещениеПриЗавершении, ПараметрыДанных, ОтображатьСообщения = Истина) Экспорт
	...
	Иначе
		НачатьЗагрузкуДанныхПослеВыбораУстройства(ИдентификаторУстройства, Контекст);
	КонецЕсли;
...
Процедура НачатьВыгрузкуДанныхПослеВыбораУстройства(ИдентификаторУстройства, Контекст) Экспорт // оповещение
	...
	Если Результат Тогда
		...
		Состояние(НСтр("ru='Выполняется выгрузка данных в Оборудование...'")); 
		//
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыгрузкуДанныхЗавершение", ЭтотОбъект, Контекст);
		МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОписаниеОповещения, ИдентификаторУстройства, Контекст.Команда, Контекст.ПараметрыДанных);
		//МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ИмяКоманды, Контекст.ПараметрыДанных, , ОписаниеОповещения);
	Иначе
...
Процедура НачатьВыгрузкуДанныхЗавершение(РезультатКоманды, Параметры) Экспорт // оповещение
	
	Если НЕ РезультатКоманды.Результат Тогда
		ТекстСообщения = РезультатКоманды.ВыходныеПараметры[1];
	Иначе
		МенеджерОборудованияРарусСервер.ПослеУспешнойВыгрузкиДанных(Параметры.ИдентификаторУстройства, Параметры.ПараметрыДанных);
...
Процедура НачатьЗагрузкуДанныхПослеВыбораУстройства(ИдентификаторУстройства, Контекст) Экспорт
	...
	Если Результат Тогда
		Состояние(НСтр("ru='Выполняется загрузка данных из оборудования'"));
		
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьЗагрузкуДанныхЗавершение", ЭтотОбъект, Контекст);
		МенеджерОборудованияКлиент.НачатьВыполнениеКоманды(ОписаниеОповещения, ИдентификаторУстройства, Контекст.Команда, Контекст.ПараметрыДанных);
...
Процедура НачатьЗагрузкуДанныхЗавершение(РезультатКоманды, Параметры) Экспорт
