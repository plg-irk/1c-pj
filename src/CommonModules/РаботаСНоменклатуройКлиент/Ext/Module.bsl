...
//   ТоварыКПодписанию            - Массив из РегистрСведенийКлючЗаписи.СостоянияВыгрузкиНоменклатуры - выбранные 
//                                    товарные позиции к подписанию.
...
//   ДекорацияДлительногоОжидания - Неопределено, ДекорацияФормы - декорация формы для индикации выполнения фонового 
//                                    задания.
//   ОповещениеОЗавершении        - Неопределено, ОписаниеОповещения - оповещение о завершении процедуры.
//   ОбновитьСтатистику           - Булево - обновлять ли статистику результатов выгрузки по завершении метода.
//
Процедура ПолучитьТоварыНациональногоКаталогаКПодписанию(Форма, Организация, Знач ТоварыКПодписанию, 
			ИдентификаторЗадания, ДекорацияДлительногоОжидания = Неопределено, ОповещениеОЗавершении = Неопределено, 
			ОбновитьСтатистику = Ложь) Экспорт
	
	ДлительнаяОперация = РаботаСНоменклатуройСлужебныйВызовСервера.ПолучитьТоварыНациональногоКаталогаКПодписаниюВФоне(
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		ВыполнитьОбработкуОповещения(ДлительнаяОперацияЗавершение, ДлительнаяОперация);
		Возврат;
	КонецЕсли;
	...
	ПараметрыОжидания.ВыводитьОкноОжидания = (ДекорацияДлительногоОжидания = Неопределено);
	ПараметрыОжидания.ВыводитьСообщения    = Истина;
	ПараметрыОжидания.ТекстСообщения       = НСтр("ru = 'Получение карточек товаров из Национального каталога для последующего подписания.'");
...
//   ОбновитьСтатистику           - Булево - обновлять ли статистику результатов выгрузки по завершении метода.
//
Процедура ПодписатьТоварыНациональногоКаталога(Форма, Организация, Знач ДанныеДляПодписания, 
			ИдентификаторЗадания, ДекорацияДлительногоОжидания = Неопределено, ОповещениеОЗавершении = Неопределено, 
			ОбновитьСтатистику = Ложь) Экспорт
	
	ДлительнаяОперация = РаботаСНоменклатуройСлужебныйВызовСервера.ПодписатьТоварыНациональногоКаталогаВФоне(
		Форма.УникальныйИдентификатор, 
		Организация,
		ДанныеДляПодписания,
		ИдентификаторЗадания,
	...
	ПараметрыОжидания.ВыводитьОкноОжидания = (ДекорацияДлительногоОжидания = Неопределено);
	ПараметрыОжидания.ВыводитьСообщения    = Истина;
	ПараметрыОжидания.ТекстСообщения       = НСтр("ru = 'Подписание карточек товаров Национального каталога.'");
