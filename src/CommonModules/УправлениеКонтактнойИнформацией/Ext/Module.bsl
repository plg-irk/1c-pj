...
//																		ПоложениеЗаголовкаЭлементаФормы.Верх или ПоложениеЗаголовкаЭлементаФормы.Лево (по умолчанию).
//	ИсключаемыеВиды - Массив - необязательный, содержит исключаемые виды.
//	ОтложеннаяИнициализация - Булево - необязательный, признак отложенной инициализации.
//
Процедура ПриСозданииНаСервере(Форма, Объект, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация", ПоложениеЗаголовкаКИ = "",
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
		...
		// +РАРУС
		|	ЕСТЬNULL(КонтактнаяИнформация.ОсновнойДляСвязи, ЛОЖЬ) КАК ОсновнойДляСвязи,
		// -РАРУС
...
//    Форма  - УправляемаяФорма - Форма объекта-владельца, предназначенная для вывода контактной информации.
//    Объект - Произвольный - Объект-владелец контактной информации.
//    Отказ - Булево - признак возможности дальнейшей обработки.
//
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Иначе
			...
			Если ВидИнформации <> ПредыдущийВид И ОбязательноеЗаполнение И ПустаяСтрока(Представление)
				...
				И НЕ ПравоПользователя("РазрешитьНеЗаполнятьОбязательныеВидыКИ") Тогда   // -Рарус добавлено право
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Если СтрокаТаблицы.ЭтоРеквизитТабличнойЧасти Тогда
			...
			Для Каждого СтрокаТабличнойЧастиФормы Из ТабличнаяЧастьФормы Цикл
				...
				// +РАРУС
				ЗаписатьКонтактнуюИнформацию(Объект, ЗначенияПолей, ВидИнформации, ТипИнформации, ИдентификаторСтроки,, СтрокаТаблицы.ОсновнойДляСвязи);
				// -РАРУС
		...
		Иначе
			...
			// +РАРУС
			ЗаписатьКонтактнуюИнформацию(Объект, СтрокаТаблицы.ЗначенияПолей, ВидИнформации, ТипИнформации,, ДействуетС, СтрокаТаблицы.ОсновнойДляСвязи);
			// -РАРУС
		КонецЕсли;
...
//      ОжидаемыйВид  - СправочникСсылка.ВидыКонтактнойИнформации, ПеречислениеСсылка.ТипыКонтактнойИнформации,
//                      Структура - Используется для определения типа, если его невозможно 
//                                   вычислить по полю ЗначенияПолей.
//
// Возвращаемое значение:
...
//																								из ЗначенияПолей.
//			* ВидКонтактнойИнформации 	- СправочникСсылка.ВидыКонтактнойИнформации,		- Используется в случае,
//										- ПеречислениеСсылка.ТипыКонтактнойИнформации,			если не удалось вычислить тип из ЗначенияПолей.
//										- Структура
//	
...
Функция КонтактнаяИнформацияОбъектов(МассивОбъектов, ТипыКИ = Неопределено, ВидыКИ = Неопределено, Дата = Неопределено) Экспорт
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	КонтактнаяИнформация.ОсновнойДляСвязи УБЫВ";
	// -РАРУС
...
//
// Параметры:
//  Представление - Строка - Строка  - представления. Используется, если невозможно определить представление
//                                      из параметра.
//                    ЗначенияПолей (отсутствие поля "Представление").
//  ЗначенияПолей - Строка, Структура, Соответствие, СписокЗначений - описание полей контактной информации.
//  ВидИнформации - СправочникСсылка.ВидыКонтактнойИнформации - Используется для определения типа,
//                                                               если его невозможно вычислить по полю ЗначенияПолей.
...
//       * НаименованиеВключаетСокращение - Булево - Если Истина, то поля содержат сокращения
//                                                    в наименованиях адресных объектов.
// Возвращаемое значение:
...
//        * ДополнительныеКоды  - Структура - Коды ОКТМО, ОКТМО, ОКАТО, КодИФНСФЛ, КодИФНСЮЛ,
//                                             КодУчасткаИФНСФЛ, КодУчасткаИФНСЮЛ.
...
Функция АдресВФорматеКЛАДР(Знач Данные) Экспорт
	...
	Если Результат.Свойство("Страна") 
		И СтрСравнить(Результат.Страна, Справочники.СтраныМира.Россия.Наименование) = 0 Тогда
...
Функция ЗначенияКонтактнойИнформацииОбъекта(Ссылка, ТипКонтактнойИнформации) Экспорт
	...
	"ВЫБРАТЬ
	// +РАРУС
	|	КонтактнаяИнформацияОбъекта.ОсновнойДляСвязи КАК ОсновнойДляСвязи,
	// -РАРУС
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	КонтактнаяИнформацияОбъекта.ОсновнойДляСвязи УБЫВ";
	// -РАРУС
...
//																		пользователем свойства вида контактной информации.
...
//															проверка по КЛАДР (только если АдресТолькоРоссийский = Истина и ПроверятьКорректность = Истина).
//				* ЗапрещатьВводНекорректного	- Булево - Устарел. Все переданные значения игнорируются. Для запрета пользователю
//															записывать некорректные адреса следует использовать параметр ПроверятьКорректность.
//				* СкрыватьНеактуальныеАдреса	- Булево - Истина, если не требуется показывать неактуальные адреса при вводе
//															(только если АдресТолькоРоссийский = Истина).
...
//																										идентификатор предопределенного элемента.
//		* Тип									- ПеречислениеСсылка.ТипыКонтактнойИнформации 		- Тип контактной информации или его идентификатор.
//		* Порядок								- Число, Неопределено								- Порядок вида контактной информации, положение в списке
...
//		* РедактированиеТолькоВДиалоге			- Булево											- Истина, если редактировать только в диалоге, Ложь - иначе.
//		* ОбязательноеЗаполнение				- Булево											- Истина, если требуется обязательное заполнение поля, Ложь - иначе.
//		* РазрешитьВводНесколькихЗначений		- Булево											- Признак возможности использования дополнительных полей
//																										ввода для данного вида.
//		* ЗапретитьРедактированиеПользователем	- Булево											- Признак недоступности редактирования пользователем
//																										свойства вида контактной информации.
...
//				* СкрыватьНеактуальныеАдреса	- Булево - Истина, если не требуется показывать неактуальные адреса при вводе
//															(только если АдресТолькоРоссийский = Истина).
//				* ВключатьСтрануВПредставление	- Булево - Истина, если требуется включать наименование страны
//															в представление адреса.
//			Для типа АдресЭлектроннойПочты		- Структура, содержащая поля:
//				* ПроверятьКорректность			- Булево - Истина, если требуется запрещать пользователю записывать
//															некорректный адрес электронной почты.
//			Для остальных типов или для задания настроек по умолчанию используется Неопределено.
...
//    Дата - Дата - Дата с которой действует запись контактной информации,
//                  используется при хранение истории изменения контактной информации.
//    ОсновнойДляСвязи - Булево - необязательный, признак основного значения контактной информации данного вида.
Процедура ЗаписатьКонтактнуюИнформацию(Объект, ЗначенияПолей, ВидИнформации, ТипИнформации, ИдентификаторСтроки = 0, Дата = Неопределено, ОсновнойДляСвязи=ЛОЖЬ) Экспорт //*РАРУС
	...
	// +РАРУС
	НоваяСтрока.ОсновнойДляСвязи = ОсновнойДляСвязи;
	// -РАРУС
...
Процедура ДобавитьСтрокуКонтактнойИнформации(Форма, Результат, ИмяЭлементаДляРазмещения, ЭтоНовыйВидКИ = Ложь, КоличествоАдресов = Неопределено, СледующаяСтрока = Неопределено)
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		Если ТипЗнч(ДобавляемыйВид)= Тип("СправочникСсылка.ВидыКонтактнойИнформации") Тогда
			// +Рарус
			ИнформацияОВидеКИ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДобавляемыйВид, "Тип, Наименование, РедактированиеТолькоВДиалоге, ВидПоляДругое, ХранитьИсториюИзменений");
			// -Рарус
		Иначе
	...
	// +РАРУС
	КорневаяГруппа         = Форма.Элементы[ИмяЭлементаДляРазмещения];
	ГруппаДанные           = Группа(ИмяЭлементаДляРазмещения+"Данные",           Форма, НСтр("ru = 'Контактная информация'"),         ИмяЭлементаДляРазмещения, КорневаяГруппа);
	ГруппаОсновнойДляСвязи = Группа(ИмяЭлементаДляРазмещения+"ОсновнойДляСвязи", Форма, НСтр("ru = 'Контактная информация (лево)'"),  ИмяЭлементаДляРазмещения, ГруппаДанные);
	ГруппаПоляВвода        = Группа(ИмяЭлементаДляРазмещения+"ПоляВвода",        Форма, НСтр("ru = 'Контактная информация (право)'"), ИмяЭлементаДляРазмещения, ГруппаДанные);
	
	КорневаяГруппа        .Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаДанные          .Группировка = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
	ГруппаОсновнойДляСвязи.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	ГруппаПоляВвода       .Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	ГруппаОсновнойДляСвязи.Объединенная = ЛОЖЬ;
	ГруппаПоляВвода       .Объединенная = ЛОЖЬ;
	
	ПараметрыКонтактнойИнформации = ПараметрыКонтактнойИнформацииФормы(Форма.ПараметрыКонтактнойИнформации, ИмяЭлементаДляРазмещения);
	ИмяЭлемента = "ОсновнойДляСвязи" + ИмяРеквизита;
	ОсновнойДляСвязи = Форма.Элементы.Добавить(ИмяЭлемента, Тип("ДекорацияФормы"), ГруппаОсновнойДляСвязи);
	ОсновнойДляСвязи.Заголовок          = НСтр("ru = 'Основной для связи'");
	ОсновнойДляСвязи.Вид                = ВидДекорацииФормы.Картинка;
	ОсновнойДляСвязи.Высота             = 1;
	ОсновнойДляСвязи.Ширина             = 2;
	ОсновнойДляСвязи.Подсказка          = НСтр("ru = 'Использовать как основной способ связи данного типа'");
	ОсновнойДляСвязи.ПропускатьПриВводе = ИСТИНА;
	ОсновнойДляСвязи.Гиперссылка        = ИСТИНА;
	ОсновнойДляСвязи.Картинка           = ?(ЕстьРеквизит(ИнформацияОВидеКИ, "ОсновнойДляСвязи") И ИнформацияОВидеКИ.ОсновнойДляСвязи, БиблиотекаКартинок.КонтактнаяИнформацияОсновная, БиблиотекаКартинок.КонтактнаяИнформацияВторичная);
	ОсновнойДляСвязи.УстановитьДействие("Нажатие", "Подключаемый_КонтактнаяИнформацияПриИзменении");
	ПараметрыКонтактнойИнформации.ДобавленныеЭлементы.Добавить(ИмяЭлемента, 2);
	
	// Отрисовка на форме
	ГруппаСтроки = Группа("Группа" + ИмяРеквизита, Форма, ДобавляемыйВид.Наименование, ИмяЭлементаДляРазмещения, ГруппаПоляВвода);
	
	Родитель = ГруппаПоляВвода;//Родитель(Форма, ИмяЭлементаДляРазмещения);
	// -РАРУС
	Если Не ЭтоПоследняяСтрока Тогда
		...
		// +РАРУС
		Форма.Элементы.Переместить(ОсновнойДляСвязи, ГруппаОсновнойДляСвязи, Форма.Элементы[СтрЗаменить(СледующаяГруппа.Имя, "ГруппаКонтактнаяИнформацияПоле", "ОсновнойДляСвязиКонтактнаяИнформацияПоле")]);
		// -РАРУС
	ИначеЕсли ДобавлениеНовогоЗначения Тогда
		СледующаяГруппа = Форма.Элементы[Результат.ИмяКоманды].Родитель;
		// +РАРУС
		Форма.Элементы.Переместить(ГруппаСтроки, Родитель);
		Форма.Элементы.Переместить(ОсновнойДляСвязи, ГруппаОсновнойДляСвязи);
		// -РАРУС
	КонецЕсли;
	...
	Если ДобавлениеНовогоЗначения Тогда
		...
		ИначеЕсли ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			// +Рарус mikolv
			ПредставлениеТелефона = Форма[ПолеВвода.Имя];
			
			Если НЕ ЗначениеЗаполнено(ПредставлениеТелефона) Тогда
				// заполняем по умолчанию
				КодСтраны = ПравоПользователя("КодСтраныТелефона");
				КодГорода = ПравоПользователя("КодГородаТелефона");
				
				Если ЗначениеЗаполнено(КодСтраны) ИЛИ ЗначениеЗаполнено(КодГорода) Тогда
					КодСтраны = СтрЗаменить(КодСтраны, "+", "");
					Форма[ПолеВвода.Имя] = "+" + КодСтраны + " (" + КодГорода + ") ";
					ОбновитьКонтактнуюИнформацию(Форма,, Новый Структура("ИмяРеквизита", ПолеВвода.Имя));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		// -Рарус mikolv
...
Функция ПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	// +Рарус добавлено право
	Если ОбязательноеЗаполнение И ЭтоНовыйВидКИ И Элемент.Вид = ВидПоляФормы.ПолеВвода И НЕ ПравоПользователя("РазрешитьНеЗаполнятьОбязательныеВидыКИ") Тогда // - Рарус
		Элемент.АвтоОтметкаНезаполненного = Истина;
	КонецЕсли;
...
Функция Группа(ИмяГруппы, Форма, Заголовок, ИмяЭлементаДляРазмещения, РодительГруппы=Неопределено) //*РАРУС
	
	Группа = Форма.Элементы.Найти(ИмяГруппы);
	
	Если Группа = Неопределено Тогда
		// +РАРУС
		Группа = Форма.Элементы.Добавить(ИмяГруппы, Тип("ГруппаФормы"), ?(РодительГруппы=Неопределено, Родитель(Форма, ИмяЭлементаДляРазмещения), РодительГруппы));
		// -РАРУС
...
Процедура ПроверитьНаличиеРеквизитовКонтактнойИнформации(Форма, МассивДобавляемыхРеквизитов)
	...
	Если СоздатьТаблицуКонтактнойИнформации Тогда
		...
		// +РАРУС
		МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы("ОсновнойДляСвязи", Новый ОписаниеТипов("Булево"), ИмяОписания));
		// -РАРУС
...
Процедура ДобавитьРеквизитВОписание(Форма, СтрокаКИ, ЭтоНовыйВидКИ, ЭтоРеквизитТабличнойЧасти = Ложь, ЗаполнятьЗначениеРеквизита = Истина, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	// +РАРУС
	НоваяСтрока.ОсновнойДляСвязи = СтрокаКИ.ОсновнойДляСвязи;
	// -РАРУС
	НоваяСтрока.ИмяЭлементаДляРазмещения  = ИмяЭлементаДляРазмещения;
...
Процедура УдалитьКомандыИЭлементыФормы(Форма, ИмяЭлементаДляРазмещения)
	...
	Для Каждого УдаляемыйЭлемент Из ДобавленныеЭлементы Цикл
		...
		Иначе
			// +РАРУС
			Если НЕ Форма.Элементы.Найти(УдаляемыйЭлемент.Значение) = Неопределено Тогда
...
Процедура СоздатьДействие(Форма, ВидКонтактнойИнформации, ИмяРеквизита, ГруппаДействий, КоличествоАдресов, ЕстьКомментарий = Ложь, ИмяЭлементаДляРазмещения = "ГруппаКонтактнаяИнформация")
	...
	Если СоздаватьДействиеДляТипа[Тип] = Истина Тогда
		...
		Если Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
			...
			ИмяКоманды = "КомандаКарта" + ИмяРеквизита;
			Элемент.Заголовок = НСтр("ru = 'Карта'");
			Команда.Подсказка = НСтр("ru = 'Отобразить на карте'");
			Команда.Картинка = БиблиотекаКартинок.КартаСЛупой;
			// -РАРУС
