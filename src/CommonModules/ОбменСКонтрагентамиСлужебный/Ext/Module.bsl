...
// с массивом сертификатов зарегистрированных в 1с (действующих и доступных текущему пользователю).В случае,
// если передан необязательный параметр НастройкаЭДО, то массив зарегистрированных в 1с сертификатов, дополнительно
// ограничивается условием вхождения в список зарегистрированных по данной настройке или профилю сертификатов.
//
// Параметры:
...
//    ЭтоТест - Булево - получение файла выполняется в рамках тестирования.
//
Процедура УдалитьФайлFTP(FTPСоединение, Путь, ЭтоТест = Ложь, РезультатТеста = Неопределено) Экспорт
...
//
// Параметры:
//  СсылкаНаФайлЭД - СправочникСсылка.ЭДПрисоединенныеФайлы - ссылка на электронный документ, текстовое представление
...
// 
// Возвращаемое значение:
//  ТаблицаЗначений - состав и типы колонок описаны в макете Обработка.ОбменСКонтрагентами.ВерсииФорматовЭД.
...
//              Обработка.ОбменСКонтрагентами.ВерсииФорматовЭД.
//
...
//
// Параметры:
//  ПрисоединенныйФайл - ДокументСсылка.ЭлектронныйДокументВходящий,
//                       ЭлектронныйДокументИсходящий - электронный документ.
...
//  СсылкаНаЭД - ДокументСсылка.ЭлектронныйДокументВходящий,
//               ЭлектронныйДокументИсходящий - ссылка на электронный документ.
// 
// Возвращаемое значение:
...
//  ЕстьОшибкиЗаполнения - Булево - результат проверки.
...
Функция БилетНаСайтПоддержки(ПоказыватьОшибки = Истина) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		...
		Если Не МодульТарификация.ЗарегистрированаЛицензияБезлимитнойУслуги(
			...
			ТекстЗаголовкаСообщения = НСтр("ru = 'При аутентификации на портале 1С:ИТС возникла ошибка:'");
...
Процедура СформироватьДокумент(ДеревоРазбора, СтрокаОбъекта, ДокументСсылка, Контрагент, Записывать, ТекстСообщения, Отказ) Экспорт
	...
	ИначеЕсли СтрокаОбъекта.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФУПД
		...
		// +Рарус
		//!!!_alalek Временно. Для проверки эдо по счет фактуре
		//	
	...
	// Добавлено для обработки корректировок по аналогии с БП
	ИначеЕсли СтрокаОбъекта.ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель
		...
		Если ТипЗнч(ДанныеВыбранногоКонтрагента) = Тип("Структура") Тогда
			...
			ДанныеВыбранногоКонтрагента.Вставить("ЭтоФизЛицо",ОбменСКонтрагентамиПереопределяемый.ЭтоФизЛицо(Контрагент));
...
// Возвращаемое значение:
// ТаблицаЗначений - пустая таблица с колонками "Контрагент", "ИНН", "КПП", "Дата", "Состояние",
//                    "ДополнительныеПараметры", "ОбластьДанныхВспомогательныеДанные"
//                    (если есть подсистема РаботаВМоделиСервиса).
...
// +Рарус необходимо для получения корректировки поступления.
...
//
// Параметры:
//  ВладелецФайла		 - ДокументСсылка.ЭлектронныйДокументВходящий,
//                         ДокументСсылка.ЭлектронныйДокументИсходящий - документ, для которого требуется найти
//  присоединенный файл
//  ТипЭлементаВерсииЭД	 - ПеречислениеСсылка.ТипыЭлементовВерсииЭД - тип присоединенного файла, если не указан -
//                                                                    то найдется основной присоединенный файл;
//  СообщатьОбОшибке	 - Булево - признак необходимости сообщить пользователю, если файл не будет найден.
// 
// Возвращаемое значение:
//  СправочникСсылка.ЭДПрисоединенныеФайлы, Неопределено - ссылка на присоединенный файл либо неопределено,
//                                                         если файл не найден.
//
...
// Параметры:
//  СсылкаНаЭД			 - ДокументСсылка.ЭлектронныйДокументВходящий,
//						   ДокументСсылка.ЭлектронныйДокументИсходящий - ссылка на отклоняемый документ
...
//
// Параметры:
//  СсылкаНаФайлЭД	 - СправочникСсылка.ЭДПрисоединенныеФайлы - основной присоединенный файл среди связанных,
//																с которым будет происходить поиск уведомления.
// 
// Возвращаемое значение:
...
//  ЭлектронныйДокумент	 - ДокументСсылка.ЭлектронныйДокументВходящий, 
//                         ДокументСсылка.ЭлектронныйДокументИсходящий - ссылка на электронный документ.
//
...
// 
// Возвращаемое значение:
//  СписокЗначений - список способов обработки с указанием представления и отметки
//                   в списке используемого по-умолчанию способа.
//
...
//  ДеревоЗначений	 - ДеревоЗначений - дерево, содержащее ссылки на справочник ЭДПрисоединенныеФайлы,
//                                      которые требуется вывести на форме.
//
...
//    * СсылкаНаДокумент - ДокументСсылка.ЭлектронныйДокументВходящий,
//                         ЭлектронныйДокументИсходящий - электронный документ.
...
// Возвращаемое значение:
//  Ссылка на электронный документ вида Торг-12 титул покупателя, Акт титул заказчика,
//  СоглашениеОбИзмененииСтоимостиПолучатель, если такой документ есть или неопределено, если документа нет.
//
