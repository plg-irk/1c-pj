...
Процедура ПриКопированииДокумента(ДокументОбъект, ОбъектКопирования) Экспорт
	...
	НастройкаКУДиР
		= Новый Структура("ОтражениеВУСН, Графа4_УСН, Графа5_УСН, Графа6_УСН, Графа7_УСН, НДС_УСН, ДоходыЕНВД_УСН");
	ЗаполнитьЗначенияСвойств(НастройкаКУДиР, ОбъектКопирования);
	
	СохранитьНастройкиУСН = УчетнаяПолитика.ПрименяетсяУСН(ДокументОбъект.Организация, ДокументОбъект.Дата)
		Или УчетнаяПолитика.ПрименяетсяАУСН(ДокументОбъект.Организация, ДокументОбъект.Дата);
	СохранитьНастройкиРасходовУСН = УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(ДокументОбъект.Организация, ДокументОбъект.Дата)
		Или УчетнаяПолитика.ПрименяетсяАУСНДоходыМинусРасходы(ДокументОбъект.Организация, ДокументОбъект.Дата);
	
	Если Не СохранитьНастройкиУСН Тогда
		НастройкаКУДиР.ОтражениеВУСН  = Перечисления.ОтражениеВУСН.НеПринимаются;
	...
	ИначеЕсли Не СохранитьНастройкиРасходовУСН Тогда
		НастройкаКУДиР.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
		НастройкаКУДиР.Графа6_УСН    = 0;
		НастройкаКУДиР.Графа7_УСН    = 0;
		НастройкаКУДиР.НДС_УСН       = 0;
...
Процедура ЗаполнитьОтражениеДокументаВУСН(ДокументОбъект, ПараметрыУСН) Экспорт
	...
	ПрименениеУСН = ПараметрыУСН.ПрименениеУСН Или ПараметрыУСН.ПрименяетсяАУСН;
	...
	// Доходы
	ОбъектДоходы  = ПараметрыУСН.ПрименениеУСНДоходы Или ПараметрыУСН.ПрименяетсяАУСН;
	
	// Доходы минус расходы
	ОбъектРасходы = (ПараметрыУСН.ПрименениеУСН И Не ПараметрыУСН.ПрименениеУСНДоходы)
		Или ПараметрыУСН.ПрименяетсяАУСН;
	...
	МожноРаспределятьРасходы = ОбъектРасходы И ПараметрыУСН.ПрименяетсяОсобыйНалоговыйРежим;
	
	//Поступление
	
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		И ПараметрыУСН.ПрименяетсяАУСН Тогда
		
		УчестьДоход = Не Документы.ПоступлениеНаРасчетныйСчет.ТребуетсяОформитьКассовыйЧек(
			ПараметрыУСН.Контрагент,
			ПараметрыУСН.СчетКонтрагента);
		
		Если УчестьДоход Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда 
		Графа4_УСН = СуммаДляКУДиР;
		Графа5_УСН = 0;
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
		...
		ПризнаватьДоходомПроцентыПоВкладам = ПризнаватьДоходомПроцентыПоВкладам(
			ДокументОбъект.Организация,
			ДокументОбъект.Дата,
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыУСН, "СубконтоКт1"));
			
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
			И Не ПризнаватьДоходомПроцентыПоВкладам Тогда
				Графа4_УСН = 0;
				Графа5_УСН = 0;
				ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
		Иначе
			...
			Если ПараметрыУСН.ПрименяетсяАУСН Тогда
				ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются;
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога
		И ПараметрыУСН.ПрименяетсяАУСН Тогда
		ОтражениеВУСН = Перечисления.ОтражениеВУСН.ВозвратРасхода;
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка
		...
		Если ПараметрыУСН.НТТНаЕНВД Тогда
			...
			Графа5_УСН = 0;
		Иначе
			СуммаНДС = ДокументОбъект.РасшифровкаПлатежа.Итог("СуммаНДС");
			ДоходыЕНВД_УСН = Ложь;
			Графа5_УСН = СуммаДляКУДиР - СуммаНДС;
		КонецЕсли;
	...
	//Списание
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику
		И ПараметрыУСН.ПрименяетсяАУСН Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратПокупателю
			И ОбъектДоходы И ПараметрыУСН.ПрименяетсяАУСН Тогда // Доходы
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочееСписание
		...
		Если ПараметрыУСН.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ПрочиеРасходы
			...
			Если ВидПрочихРасходов = Перечисления.ВидыПрочихДоходовИРасходов.РасходыНаУслугиБанков Тогда
				Графа7_УСН = СуммаДляКУДиР;
				ОтражениеВУСН = ?(МожноРаспределятьРасходы,
					Перечисления.ОтражениеВУСН.Распределяются, Перечисления.ОтражениеВУСН.Принимаются);
			КонецЕсли;
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога
		...
		Если ПараметрыУСН.ПрименяетсяАУСН И ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме") Тогда
			
			ВидНалога = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыУСН.Налог, "ВидНалога");
			...
			Если ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоНДФЛ(ВидНалога)
				И ВидыНалоговыхПлатежей.Найти(ПараметрыУСН.СубконтоДт1) <> Неопределено Тогда
				
				// При АУСН и внешнем учете зарплаты НДФЛ за сотрудников по умолчанию включается в расходы.
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.РасчетыПоКредитамИЗаймам
		...
		Графа7_УСН = ДокументОбъект.РасшифровкаПлатежа.Итог("РасходыУСН");
		Графа6_УСН = СуммаДляКУДиР;
	...
	НастройкаКУДиР.ОтражениеВУСН   = ОтражениеВУСН;
...
Процедура ЗаполнитьДоходыРасходыВсего(ДокументОбъект, ПараметрыУСН) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		...
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПолучениеЗайма
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПолучениеКредита
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал
		...
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПоступлениеОтПоставщика
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПрочееПоступление Тогда
...
Процедура ЗаполнитьОтражениеВУСНСтрокиРасшифровкиПлатежа(СтрокаТЧ, Параметры) Экспорт
	...
	ДанныеСтрокиТЧ = Новый Структура("ОтражениеВУСН, ДоходыУСН, РасходыУСН, НДСУСН, СчетУчетаРасчетовСКонтрагентом, ВидПлатежаПоКредитамЗаймам");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТЧ, СтрокаТЧ);
	
	МожноРаспределятьРасход = (Параметры.ПрименениеУСН И Не Параметры.ПрименениеУСНДоходы)
		И Параметры.ПрименяетсяОсобыйНалоговыйРежим;
	
	СуммаНУ = СтрокаТЧ.СуммаПлатежа;
	Если Параметры.ВалютаДокумента <> Параметры.ВалютаРегламентированногоУчета Тогда
	...
	Если Не Параметры.ПрименениеУСН И Не Параметры.ПрименяетсяАУСН Тогда
		...
		ИначеЕсли ЕстьРасчетыПоРасходамУСН Тогда
			...
			ДанныеСтрокиТЧ.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
		КонецЕсли;
	...
	Если ЕстьРасчетыПоРасходамУСН Тогда
		...
		Если НЕ Параметры.ПрименениеУСНДоходы И ЕстьЗаймыПолученные Тогда
			Если ЗначениеЗаполнено(ДанныеСтрокиТЧ.ВидПлатежаПоКредитамЗаймам) И
				...
				ДанныеСтрокиТЧ.ОтражениеВУСН = ?(МожноРаспределятьРасход,
					Перечисления.ОтражениеВУСН.Распределяются, Перечисления.ОтражениеВУСН.Принимаются);
			КонецЕсли;
	...
	// При АУСН и оплате поставщику без закрывающих документов считаем расходы принимаемыми по умолчанию. 
	Если (Параметры.ПрименяетсяАУСН
		И Параметры.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику)
		Или (Параметры.ПрименяетсяАУСНДоходы
		И Параметры.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратПокупателю) Тогда
		
		ДанныеСтрокиТЧ.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются;
		ДанныеСтрокиТЧ.РасходыУСН = СуммаНУ;
...
Процедура ЗаполнитьОтображаемоеСодержаниеКУДиР(ДокументОбъект, ПараметрыУСН) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникамПоДоговорамПодряда Тогда
		...
		Содержание_УСН = НСтр("ru = 'Перечислена оплата за работы по договорам ГПХ.'");
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда Тогда
		
		Содержание_УСН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"Перечислена оплата за работы по договору ГПХ сотруднику %1",
			НаименованиеФизлицаНаПечать(ПараметрыУСН.Контрагент, ПараметрыУСН.Дата));
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда Тогда
		
		Содержание_УСН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"Оплачены из кассы работы по договору ГПХ сотруднику %1",
			НаименованиеФизлицаНаПечать(ПараметрыУСН.Контрагент, ПараметрыУСН.Дата));
...
Функция СодержаниеОперацииДляКУДиР(ПараметрыУСН) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда
		Если ТипЗнч(ПараметрыУСН.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда 
			НаименованиеКонтрагента = НаименованиеКонтрагентаНаПечать(ПараметрыУСН.Контрагент, ПараметрыУСН.Дата);
		Иначе
			НаименованиеКонтрагента = НаименованиеФизлицаНаПечать(ПараметрыУСН.Контрагент, ПараметрыУСН.Дата);
		КонецЕсли;
		Содержание_УСН = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Взнос в уставный капитал от %1.'"), НаименованиеКонтрагента);
...
//  ДатаЗаписи				 - Дата - если нет - берем дату регистратора из регистра
//  ВзносыПодлежащиеУплате	 - Булево - признак, что формируются реквизиты для взноса, подлежащего уплате
// 
// Возвращаемое значение:
//  Строка - дата и номер документа в строке
//
Функция РеквизитыПервичногоДокумента(Документ, Организация = Неопределено, Знач ДатаЗаписи = Неопределено, ВзносыПодлежащиеУплате = Ложь) Экспорт
	...
	Если РеквизитыДокумента <> Неопределено Тогда
		...
		Если Не ВзносыПодлежащиеУплате Тогда
...
Процедура ЗаполнитьОтражениеВНУпоСписанию(Реквизиты, ТаблицаСписанныхПартий, ВидОперации) Экспорт
	...
	// По умолчанию все расходы считаем принимаемыми. Исключения определяются далее в алгоритме.
	...
	// Проверим простые сценарии, когда все расходы заведомо не принимаемые.
	
	Если ВидОперации = "ВозвратПоставщику" Или ВидОперации = "БезвозмезднаяПередача" Тогда
	...
	Если МетаданныеДокумента.ТабличныеЧасти.Количество() = 0
		И ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокументОтгрузки", МетаданныеДокумента) Тогда
		ДокументОтгрузки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ДокументОтгрузки");
		Если ЗначениеЗаполнено(ДокументОтгрузки) Тогда
			ДокументСсылка = ДокументОтгрузки;
			МетаданныеДокумента = ДокументСсылка.Метаданные();
		КонецЕсли;
	КонецЕсли;
	
	// Если операция в рамках деятельности на патенте - расходы не принимаются.
	...
	// Сложный сценарий: непринимаемые расходы определяются на основании данных в табличных частях.
	
	ВсеТабличныеЧасти = Новый Массив;
	ВсеТабличныеЧасти.Добавить("ИспользованныеМатериалы");
	ВсеТабличныеЧасти.Добавить("Материалы");
	ВсеТабличныеЧасти.Добавить("Товары");
	ВсеТабличныеЧасти.Добавить("Спецодежда");
	ВсеТабличныеЧасти.Добавить("Спецоснастка");
	ВсеТабличныеЧасти.Добавить("ИнвентарьИХозяйственныеПринадлежности");
	
	ТабличныеЧасти = Новый Массив;
	
	Для Каждого ПроверяемоеИмя Из ВсеТабличныеЧасти Цикл
		Если ОбщегоНазначенияБП.ЕстьТабЧастьДокумента(ПроверяемоеИмя, МетаданныеДокумента) Тогда
			ТабличныеЧасти.Добавить(ПроверяемоеИмя);
		КонецЕсли;
	КонецЦикла;
	...
	//В отчете комиссионера ОтражениеВУСН используется только для отражения списания (при реализации игнорируется)
	УчитыватьОтражениеВУСН = НЕ (ИмяВидаДокумента = "ОтчетКомиссионераОПродажах" И ВидОперации = "Реализация");
	
	Для Каждого ИмяТЧ Из ТабличныеЧасти Цикл
		//Если отражение в НУ указывается явно
		Если УчитыватьОтражениеВУСН
			И ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("ОтражениеВУСН", МетаданныеДокумента, ИмяТЧ) Тогда
		...
		ИначеЕсли ПрименениеСпецрежимов.ЕНВД
			И ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СчетДоходов", МетаданныеДокумента, ИмяТЧ) Тогда
			...
			Для каждого Строка Из ТаблицаРезультата Цикл
				Если БухгалтерскийУчетПовтИсп.СчетОтноситсяКДеятельностиЕНВД(Строка.СчетДоходов) Тогда
					СтруктураОтбора = Новый Структура;
					СтруктураОтбора.Вставить("Номенклатура", Строка.Номенклатура);
...
Функция КоэффициентРаспределенияРасходов(СтруктураШапкиДокумента)
	...
	Если СтруктураШапкиДокумента.Дата >= УчетЕНВДКлиентСервер.ДатаОтменыЕНВД() Тогда
		...
		Результат = УчетПСН.ДоляДоходовПатентнойСистемыНалогообложения(
	...
	Иначе
		...
		// До отмены ЕНВД распределение при совмещении с ПСН не поддерживалось.
		...
		Результат = ПолучитьКоэффРаспределенияЕНВД(СтруктураШапкиДокумента, Метод, База);
...
// и выручку по деятельности на ЕНВД и патенте.
//
Функция ДоляОсобыхРежимовИКомиссии(СтруктураШапкиДокумента, ТаблицаСписанныхПартий = Неопределено, ДвиженияРегистров = Неопределено, ВидОперации = "Оплата", ДоговорДляОтложенногоПроведения = Неопределено)
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокументОтгрузки", МетаданныеДокумента) Тогда
		Если ЗначениеЗаполнено(ДокументСсылка.ДокументОтгрузки) Тогда
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ЧастичнаяПродажа", МетаданныеДокумента) Тогда
				ЧастичнаяПродажа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ЧастичнаяПродажа");
			КонецЕсли;
...
Процедура СформироватьДвиженияУСН(ДокументОбъект, СтруктураДопПараметров = Неопределено, ЭтоОбъект = Истина, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета")
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
			Если ЭтоОбъект Тогда
				СтруктураШапкиДокумента.Вставить("РеестрВыплатСамозанятым", ДокументОбъект.РеестрВыплатСамозанятым);
			Иначе
				СтруктураШапкиДокумента.Вставить("РеестрВыплатСамозанятым", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ДокументСсылка, "РеестрВыплатСамозанятым"));
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
		...
		ОтразитьВУСН(ДокументСсылка, "ВозвратПоставщику", СтруктураШапкиДокумента, , СтруктураДопПараметров,
			ДоговорДляОтложенногоПроведения);
	...
	ИначеЕсли (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.НачислениеЗарплаты")
		...
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПерерасчетСтраховыхВзносов")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.Премия"))
		И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
			ВидОперацииСписания = "СписаниеУКомиссионера";
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
			Возврат;
		Иначе			
			ВидОперацииСписания = "РеализацияКомиссионером";
		КонецЕсли;
		...
		Если Не СтруктураШапкиДокумента.УдержатьВознаграждение Тогда
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
				...
				ОтразитьВУСН(ДокументСсылка, "УслугаКомиссионера", СтруктураШапкиДокумента, 1,
					СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			КонецЕсли;
			
			СтруктураДопПараметров.Вставить("ТаблицаРасчетов", СтруктураДопПараметров.ТаблицаРасчетовПоРеализации);
			ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента, 2,
				СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		Иначе
			...
			Если ВыделятьНДСУСН И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
				...
				ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента, 2,
					СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			Иначе
				ОтразитьВУСН(ДокументСсылка, ВидОперацииСписания, СтруктураШапкиДокумента, 1,
					СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			КонецЕсли;
		КонецЕсли;
		
		ОтразитьВУСН(ДокументСсылка, "ЗачетВыручкиКомиссионера", СтруктураШапкиДокумента, 3,
			СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		
		Если СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании
			И ОтражатьДоходыНаДатуОтчетаКомиссионера(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) Тогда
			ОтразитьВУСН(ДокументСсылка, "ОтражениеДоходаОтРеализацииКомиссионером", СтруктураШапкиДокумента, 4,
				СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РаспределениеНДС") И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
		
		ОтразитьВУСН(ДокументСсылка, "РаспределениеНДС", СтруктураШапкиДокумента, , СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеНМА") Тогда
			
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеНМА")
			Или СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Права Тогда
			
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
				ОтразитьВУСН(ДокументСсылка, "ПриобретениеПрав", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			КонецЕсли;
		...
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку Тогда
			...
			Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
				ОтразитьВУСН(ДокументСсылка, "ЗачетВыручкиКомиссионера", СтруктураШапкиДокумента, 2, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ВыкупТоваровКомиссионером Тогда
			ОтразитьВУСН(ДокументСсылка, "РеализацияКомиссионером", СтруктураШапкиДокумента,,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаТоваровСоСкладаКомиссионера Тогда
			ОтразитьВУСН(ДокументСсылка, "РеализацияКомиссионером", СтруктураШапкиДокумента,,СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РегламентнаяОперация") И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН 
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВидОперации") = Перечисления.ВидыРегламентныхОпераций.РасходМатериаловДляВыпускаПродукции Тогда
		
		ОтразитьВУСН(ДокументСсылка, "Списание", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РегламентнаяОперация") И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВидОперации") = Перечисления.ВидыРегламентныхОпераций.ЗачетАвансаПоЕдиномуНалоговомуСчету Тогда
		
		ОтразитьВУСН(ДокументСсылка, "ЗачетАвансаПоЕдиномуНалоговомуСчету", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		
	ИначеЕсли (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов")
			Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоЕдиномуНалоговомуСчету")
			Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности"))
		И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
		
		ОтразитьВУСН(ДокументСсылка, "ЗачетАвансаПоЕдиномуНалоговомуСчету", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СведенияОбУплатеНалогов")
			И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
		
		ОтразитьВУСН(ДокументСсылка, "ОтражениеУплатыНалогаУСН", СтруктураШапкиДокумента, 1, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения);
...
Процедура ОтразитьВУСН(ДокументСсылка, ВидОперации = Неопределено, Параметры = Неопределено, ЭтапПроведения = 0, СтруктураДопПараметров = Неопределено, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Перем КэшНаименованиеКонтрагента;
	...
	ДобавитьСвойстваПоНДС(СтруктураШапкиДокумента);
	
	Если СтруктураДопПараметров <> Неопределено Тогда
		СписанныеПартииНДС = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			СтруктураДопПараметров, "ТаблицаСписанныеПартииНДС", Неопределено);
	Иначе
		СписанныеПартииНДС = Неопределено;
	КонецЕсли;
	СтруктураШапкиДокумента.Вставить("СписанныеПартииНДС", СписанныеПартииНДС);
	...
	Если ВидОперации = "Поступление"
		...
		Или ВидОперации = "ПриобретениеПрав" Тогда
		...
		// Подготовим таблицу по способам учета НДС для оплачиваемых партий
		Если СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
			СпособыУчетаНДСПартий = СпособыУчетаНДСПартийПоДаннымПоступленияРасходов(ТаблицаПрихода);
			СтруктураШапкиДокумента.Вставить("СпособыУчетаНДСПартий", СпособыУчетаНДСПартий);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ВыплатыСамозанятым" Тогда
		...
		РасчетыСКонтрагентами_ВыплатыСамозанятымВУСН(ДокументСсылка, КУДиР, СтруктураДопПараметров, СтруктураШапкиДокумента, ДвиженияРегистров);
		
	ИначеЕсли ВидОперации = "ПеречислениеПоИсполнительномуЛисту" Тогда
		
		ТаблицаОплат = СтруктураДопПараметров.ТаблицаПеречислениеПоИсполнительномуЛисту;
		Если ТаблицаОплат.Количество() > 0 Тогда
			ОплатаПрочееУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, "Взыскатель");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "КорректировкаПоступления" Тогда
		...
		// Подготовим таблицу по способам учета НДС для корректировки партий
		Если СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
			СпособыУчетаНДСПартий = СпособыУчетаНДСПартийПоДаннымПоступленияРасходов(ТаблицаКорректировка, Истина);
			СтруктураШапкиДокумента.Вставить("СпособыУчетаНДСПартий", СпособыУчетаНДСПартий);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "РаспределениеНДС" Тогда
		
		ВключенныйВСтоимостьНДС = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураДопПараметров, "ТаблицаВключенныйВСтоимостьНДС");
		
		РаспределениеНДСВключениеВСтоимость(СтруктураШапкиДокумента, ВключенныйВСтоимостьНДС, ДвиженияРегистров);
	...
	ИначеЕсли (ВидОперации = "Списание")
		...
		ИЛИ (ВидОперации = "СписаниеУКомиссионера")
		...
		Если ВидОперации = "СписаниеУКомиссионера" Тогда
			СтатусПартии = Перечисления.СтатусыПартийУСН.НаРеализации;
		Иначе
			СтатусПартии = Перечисления.СтатусыПартийУСН.Купленные;
		КонецЕсли;
		...
		Если ВидОперации = "Реализация" Тогда
			Выручка 			 = ДоляОсобыхРежимовИКомиссии(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистровВх);
		...
		ИначеЕсли ВидОперации = "Списание"
			ИЛИ ВидОперации = "СписаниеУКомиссионера" Тогда
		...
		Если ВидОперации = "Реализация" Тогда
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("УдержатьВознаграждение", МетаданныеДокумента) И ДокументСсылка.УдержатьВознаграждение Тогда
				...
				Если СтруктураДопПараметров.Свойство("ТаблицаВознаграждение")
					...
					СуммаВознаграждения = СтруктураДопПараметров.ТаблицаВознаграждение.Итог("СуммаБезНДСРуб")
						+ СтруктураДопПараметров.ТаблицаВознаграждение.Итог("СуммаНДСРуб");
				КонецЕсли;
				...
				Выручка.Патент   = Окр(Выручка.Патент - Выручка.Патент * КоэффВознагр, 2, 1);
				Выручка.Комиссия = Окр(Выручка.Комиссия - Выручка.Комиссия * КоэффВознагр, 2, 1);
				Выручка.НДС      = Окр(Выручка.НДС - Выручка.НДС * КоэффВознагр, 2, 1);
			...
			ЭтоОтчетКомиссионераСПризнаниемДохода = ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
				И СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании
				И ОтражатьДоходыНаДатуОтчетаКомиссионера(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата);
			Если ЭтоОтчетКомиссионераСПризнаниемДохода Тогда
				СуммаВозвратов = 0;
				Если СтруктураДопПараметров.Свойство("ТаблицаВозвраты")
					И ЗначениеЗаполнено(СтруктураДопПараметров.ТаблицаВозвраты) Тогда
					СуммаВозвратов = СтруктураДопПараметров.ТаблицаВозвраты.Итог("СуммаРуб");
				КонецЕсли;
				
				КоэффициентВозвратов = ?(Выручка.Всего = 0, 0, СуммаВозвратов / Выручка.Всего);

				Выручка.Всего       = Выручка.Всего - СуммаВозвратов;
				Выручка.ЕНВД        = Окр(Выручка.ЕНВД - Выручка.ЕНВД * КоэффициентВозвратов, 2, 1);
				Выручка.Патент      = Окр(Выручка.Патент - Выручка.Патент * КоэффициентВозвратов, 2, 1);
				Выручка.Комиссия    = Окр(Выручка.Комиссия - Выручка.Комиссия * КоэффициентВозвратов, 2, 1);
				Выручка.НДС         = Окр(Выручка.НДС - Выручка.НДС * КоэффициентВозвратов, 2, 1);
			КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ОтражениеДоходаОтРеализацииКомиссионером" Тогда
		
		ТаблицаОплат = СтруктураДопПараметров.ТаблицаРасчетов.Скопировать();
		
		// Определим сумму зачитываемого аванса.
		// Может быть, что часть выручки будет зачтено авансом, который и так уже отражен в доходах.
		СуммаЗачтенногоАванса = 0;
		Для Каждого СтрокаОплаты Из ТаблицаОплат Цикл
			Если СтрокаОплаты.ДокументРасчетов <> СтруктураШапкиДокумента.Ссылка
				И (Не СтрокаОплаты.ЭтоВозврат Или (СтрокаОплаты.ЭтоВозврат И СтрокаОплаты.СчетАвансов <> СтрокаОплаты.СчетРасчетов)) Тогда
				СуммаЗачтенногоАванса = СуммаЗачтенногоАванса + СтрокаОплаты.СуммаВзаиморасчетов;
			КонецЕсли;
		КонецЦикла;
		
		СуммаДоходов = 0;
		СуммаНДС = 0;
		Если СтруктураДопПараметров.Свойство("ТаблицаСобственныеТоварыУслуги")
			И ЗначениеЗаполнено(СтруктураДопПараметров.ТаблицаСобственныеТоварыУслуги) Тогда
			СуммаРеализации = СтруктураДопПараметров.ТаблицаСобственныеТоварыУслуги.Итог("СуммаВзаиморасчетов");
			СуммаНДСРеализации = СтруктураДопПараметров.ТаблицаСобственныеТоварыУслуги.Итог("СуммаНДСРуб");
			
			СуммаДоходов = СуммаДоходов + СуммаРеализации - СуммаЗачтенногоАванса;
			
			КоэффициентРасчетовПоНДС = ?(СуммаРеализации = 0, 0, СуммаДоходов / СуммаРеализации);
			СуммаНДСДохода = Окр(СуммаНДСРеализации * КоэффициентРасчетовПоНДС, 2);

			СуммаДоходов = СуммаДоходов - СуммаНДСДохода;
			СуммаНДС = СуммаНДС + СуммаНДСДохода;
		КонецЕсли;
		
		ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность",
			ВалютаДокумента, СтруктураШапкиДокумента.КурсДокумента, СтруктураШапкиДокумента.КратностьДокумента);
		ПараметрыНовогоКурса = Новый Структура("Валюта, Курс, Кратность",
			ВалютаРегламентированногоУчета, 1, 1);
		
		СуммаДоходов = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
			СуммаДоходов, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
		
		СуммаНДС = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
			СуммаНДС, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
		...
		Если СтруктураДопПараметров.Свойство("ТаблицаВозвраты")
			...
			Для Каждого СтрокаВозврата Из СтруктураДопПараметров.ТаблицаВозвраты Цикл
				Если СтрокаВозврата.ЭтоКомиссия Тогда
					Продолжить;
				КонецЕсли;
				
				СуммаВозврат = СуммаВозврат + СтрокаВозврата.СуммаВзаиморасчетов;
			КонецЦикла;
			
			Если СтруктураДопПараметров.Свойство("ТаблицаСобственныеТоварыУслугиВозврат")
				И ЗначениеЗаполнено(СтруктураДопПараметров.ТаблицаСобственныеТоварыУслугиВозврат) Тогда
				СуммаВозвратов = СтруктураДопПараметров.ТаблицаСобственныеТоварыУслугиВозврат.Итог("СуммаВзаиморасчетов");
				СуммаНДСВозвратов = СтруктураДопПараметров.ТаблицаСобственныеТоварыУслугиВозврат.Итог("СуммаНДСРуб");
				
				КоэффициентРасчетовПоНДС = ?(СуммаВозвратов = 0, 0, СуммаВозврат / -СуммаВозвратов);
				СуммаНДСПоВозвратам = Окр(-СуммаНДСВозвратов * КоэффициентРасчетовПоНДС, 2);
				
				СуммаВозврат = СуммаВозврат - СуммаНДСПоВозвратам;
				СуммаНДСВозврат = СуммаНДСВозврат + СуммаНДСПоВозвратам;
			КонецЕсли;
			
			СуммаВозврат = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
				СуммаВозврат, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
			
			СуммаНДСВозврат = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
				СуммаНДСВозврат, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
		...
		СоставноеСодержанияКУДиР = НовыйСоставноеСодержаниеКУДиР();
		СоставноеСодержанияКУДиР.Событие = ОписаниеСобытияДляКУДиР(
			СтруктураШапкиДокумента, СтруктураШапкиДокумента.ВидОперацииУСН, МетаданныеДокумента);
		...
		РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(
			СтруктураШапкиДокумента.Ссылка, СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата);
		
		Если СуммаДоходов > 0 Тогда
			
			СоставноеСодержанияКУДиР.Пояснение = НСтр("ru = 'Оплаты от покупателей отражены в доходах'",
				ОбщегоНазначения.КодОсновногоЯзыка());

			СодержаниеОперации = СобратьСодержаниеКУДиР(СоставноеСодержанияКУДиР);
			
			// Если в параметрах присутствует таблица "ДенежныеСредства", то необходимо отражать доход по данной таблице
			Если СтруктураДопПараметров.Свойство("ДенежныеСредства")
				И ЗначениеЗаполнено(СтруктураДопПараметров.ДенежныеСредства) Тогда
				ТаблицаДенежныеСредства = СтруктураДопПараметров.ДенежныеСредства.СкопироватьКолонки(
					"ВидОтчетаПоПлатежам, ДатаСобытия, СуммаБезНДС, НДС");
				
				Для Каждого СтрокаДенежныеСредства Из СтруктураДопПараметров.ДенежныеСредства Цикл
					Если СтрокаДенежныеСредства.ВидОтчетаПоПлатежам = Перечисления.ВидыОтчетовПоПлатежамКомиссия.ЗачетАванса
						Или СтрокаДенежныеСредства.СуммаБезНДС <= 0 Тогда
						Продолжить;
					КонецЕсли;
					
					НоваяСтрокаДенежныеСредства = ТаблицаДенежныеСредства.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаДенежныеСредства, СтрокаДенежныеСредства);
				КонецЦикла;
				
				ТаблицаДенежныеСредства.Свернуть("ДатаСобытия", "СуммаБезНДС, НДС");
				
				Для Каждого СтрокаДохода Из ТаблицаДенежныеСредства Цикл
					НоваяСтрокаКУДиР = ТаблицаКУДиР.Добавить();
					НоваяСтрокаКУДиР.Активность        = Истина;
					НоваяСтрокаКУДиР.Период            = СтрокаДохода.ДатаСобытия;
					НоваяСтрокаКУДиР.Организация       = СтруктураШапкиДокумента.Организация;
					НоваяСтрокаКУДиР.Графа4            = СтрокаДохода.СуммаБезНДС + СтрокаДохода.НДС;
					НоваяСтрокаКУДиР.Графа5            = СтрокаДохода.СуммаБезНДС;
					НоваяСтрокаКУДиР.ЭтапПроведения    = ЭтапПроведения;
					НоваяСтрокаКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;
					
					Если СтрокаДохода.НДС > 0 Тогда
						ПояснениеНДС = СтрШаблон(ШаблонПояснениеНДС,
							ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтрокаДохода.НДС, , , " "));
						НоваяСтрокаКУДиР.Содержание = СодержаниеОперации + ПояснениеНДС;
					Иначе
						НоваяСтрокаКУДиР.Содержание = СодержаниеОперации;
					КонецЕсли;
				КонецЦикла;
				
				Если СуммаВозврат > 0 Тогда
					СоставноеСодержанияКУДиР.Пояснение = НСтр("ru = 'Отражен возврат покупателю'",
						ОбщегоНазначения.КодОсновногоЯзыка());
					...
					НоваяСтрокаКУДиР = ТаблицаКУДиР.Добавить();
					НоваяСтрокаКУДиР.Активность        = Истина;
					НоваяСтрокаКУДиР.Период            = СтруктураШапкиДокумента.Дата;
					НоваяСтрокаКУДиР.Организация       = СтруктураШапкиДокумента.Организация;
					НоваяСтрокаКУДиР.Содержание        = СодержаниеОперации;
					НоваяСтрокаКУДиР.Графа4            = -(СуммаВозврат + СуммаНДСВозврат);
					НоваяСтрокаКУДиР.Графа5            = -СуммаВозврат;
					НоваяСтрокаКУДиР.ЭтапПроведения    = ЭтапПроведения;
					НоваяСтрокаКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;
				КонецЕсли;
			Иначе
				Если СуммаДоходов - СуммаВозврат > 0 Тогда
					НоваяСтрокаКУДиР = ТаблицаКУДиР.Добавить();
					НоваяСтрокаКУДиР.Активность        = Истина;
					НоваяСтрокаКУДиР.Период            = СтруктураШапкиДокумента.Дата;
					НоваяСтрокаКУДиР.Организация       = СтруктураШапкиДокумента.Организация;
					НоваяСтрокаКУДиР.Графа4            = (СуммаДоходов - СуммаВозврат) + (СуммаНДС - СуммаНДСВозврат);
					НоваяСтрокаКУДиР.Графа5            = СуммаДоходов - СуммаВозврат;
					НоваяСтрокаКУДиР.ЭтапПроведения    = ЭтапПроведения;
					НоваяСтрокаКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;
					
					Если СуммаНДС - СуммаНДСВозврат > 0 Тогда
						ПояснениеНДС = СтрШаблон(ШаблонПояснениеНДС,
							ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаНДС - СуммаНДСВозврат, , , " "));
						НоваяСтрокаКУДиР.Содержание = СодержаниеОперации + ПояснениеНДС;
					Иначе
						НоваяСтрокаКУДиР.Содержание = СодержаниеОперации;
					КонецЕсли;
				ИначеЕсли СуммаДоходов - СуммаВозврат <> 0 Тогда
					Если СуммаДоходов > 0 Тогда
						НоваяСтрокаКУДиР = ТаблицаКУДиР.Добавить();
						НоваяСтрокаКУДиР.Активность        = Истина;
						НоваяСтрокаКУДиР.Период            = СтруктураШапкиДокумента.Дата;
						НоваяСтрокаКУДиР.Организация       = СтруктураШапкиДокумента.Организация;
						НоваяСтрокаКУДиР.Графа4            = СуммаДоходов + СуммаНДС;
						НоваяСтрокаКУДиР.Графа5            = СуммаДоходов;
						НоваяСтрокаКУДиР.ЭтапПроведения    = ЭтапПроведения;
						НоваяСтрокаКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;

						Если СуммаНДС > 0 Тогда
							ПояснениеНДС = СтрШаблон(ШаблонПояснениеНДС,
								ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаНДС, , , " "));
							НоваяСтрокаКУДиР.Содержание = СодержаниеОперации + ПояснениеНДС;
						Иначе
							НоваяСтрокаКУДиР.Содержание = СодержаниеОперации;
						КонецЕсли;
		...
		ИначеЕсли СуммаВозврат > 0 Тогда
		...
		Если ТаблицаКУДиР.Количество() > 0 Тогда
			Выручка = ДоляОсобыхРежимовИКомиссии(СтруктураШапкиДокумента);
			
			ОтражатьДоходПоПатенту = СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНПатент
				И ОбщегоНазначения.ЕстьРеквизитОбъекта("Патент", МетаданныеДокумента)
				И Выручка.Патент > 0;

			Если ОтражатьДоходПоПатенту Тогда
				ДоходПоПатенту = 0;
				ПатентДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Патент");
				Если ЗначениеЗаполнено(ПатентДокумента) Тогда
					Для Каждого СтрокаКУДиР Из ТаблицаКУДиР Цикл
						ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
						ЗаполнитьЗначенияСвойств(ЗаписьКУДиРПатент, СтрокаКУДиР);
						ЗаписьКУДиРПатент.Патент = ПатентДокумента;
						ДоходПоПатенту = ДоходПоПатенту + СтрокаКУДиР.Графа4;
					КонецЦикла;
				КонецЕсли;
				КУДиР.ДоходЕНВД = ДоходПоПатенту;
				КУДиР.Графа4 = ДоходПоПатенту;
			Иначе
				...
				Для Каждого СтрокаКУДиР Из ТаблицаКУДиР Цикл
					...
					ЗаполнитьЗначенияСвойств(ЗаписьКУДиР, СтрокаКУДиР);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ЗачетВознагражденияКомиссионера" Тогда
		...
		Выручка = ДоляОсобыхРежимовИКомиссии(СтруктураШапкиДокумента, ТаблицаПартий);
		...
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДокументСсылка, "СуммаВознаграждения, Товары, Услуги, ТоварыВозвращенные");
		
		СуммаНДСВознаграждения = РеквизитыДокумента.Товары.Выгрузить().Итог("СуммаНДСВознаграждения")
			+ РеквизитыДокумента.Услуги.Выгрузить().Итог("СуммаНДСВознаграждения")
			- РеквизитыДокумента.ТоварыВозвращенные.Выгрузить().Итог("СуммаНДСВознаграждения");
		...
		ВыделятьНДСУСН = СтруктураШапкиДокумента.ПорядокПризнанияРасходовПоНДС = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику
			Или СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС;
		Если ВыделятьНДСУСН И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
			СуммаВознаграждения = РеквизитыДокумента.СуммаВознаграждения - СуммаНДСВознаграждения;
		Иначе
			СуммаВознаграждения = РеквизитыДокумента.СуммаВознаграждения;
		КонецЕсли;
		
		СуммаДоходов = Мин(СуммаПринимаемойВыручки, РеквизитыДокумента.СуммаВознаграждения);
		
		// Исключим НДС из дохода
		СуммаИсключаемогоНДС = 0;
		Если Выручка.Всего > 0 И Выручка.НДС > 0 Тогда
			СуммаИсключаемогоНДС = СуммаДоходов * Выручка.НДС / Выручка.Всего;
			СуммаИсключаемогоНДС = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
				СуммаИсключаемогоНДС, ВалютаДокумента, ВалютаРегламентированногоУчета,
		...
		ОтражатьВознаграждениеПоПатенту = СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНПатент
			И ОбщегоНазначения.ЕстьРеквизитОбъекта("Патент", МетаданныеДокумента)
			И Выручка.Патент > 0
			И СуммаДоходов > 0;
		
		Если Не ОтражатьДоходыНаДатуОтчетаКомиссионера(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) Тогда
			КУДиР.Графа4 = СуммаДоходов;
			Если ОтражатьВознаграждениеПоПатенту Тогда
				ПатентДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Патент");
				Если ЗначениеЗаполнено(ПатентДокумента) Тогда
					КУДиР.ДоходЕНВД = СуммаДоходов - СуммаИсключаемогоНДС;

					ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
					ЗаписьКУДиРПатент.Патент = ПатентДокумента;
					ЗаписьКУДиРПатент.Содержание = ОписаниеСобытияДляКУДиР(
						СтруктураШапкиДокумента, СтруктураШапкиДокумента.ВидОперацииУСН, МетаданныеДокумента);
					ЗаписьКУДиРПатент.Графа4 = СуммаДоходов - СуммаИсключаемогоНДС;
				КонецЕсли;
			Иначе
				КУДиР.Графа5 = СуммаДоходов - СуммаИсключаемогоНДС;
				Если СуммаИсключаемогоНДС > 0 Тогда
					ДополнитьСодержаниеКУДиРПояснениемПоИсключениюНДС(КУДиР, СуммаИсключаемогоНДС);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН
			И Не ОтражатьВознаграждениеПоПатенту Тогда
			КУДиР.Графа7 = СуммаВзаиморасчетов;
			Если Не ВыделятьНДСУСН Тогда
				КУДиР.НДС = СуммаНДСВознаграждения;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ЗачетВознагражденияКомиссионераНДС" Тогда
		...
		Если РеквизитыДокумента.СуммаВознаграждения > 0 Тогда
			СуммаПринимаемойВыручки = СуммаПринимаемойВыручки * СуммаНДСВознаграждения / РеквизитыДокумента.СуммаВознаграждения;
		КонецЕсли;
		...
		СуммаДоходов = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
			...
			СтруктураШапкиДокумента.КурсДокумента, 1, СтруктураШапкиДокумента.КратностьДокумента, 1);
		СуммаВзаиморасчетов = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
			СуммаНДСВознаграждения, ВалютаДокумента, ВалютаРегламентированногоУчета,
			СтруктураШапкиДокумента.КурсДокумента, 1, СтруктураШапкиДокумента.КратностьДокумента, 1);
	...
	ИначеЕсли ВидОперации = "РеализацияУслуг" Тогда
		...
		Выручка 			 = ДоляОсобыхРежимовИКомиссии(СтруктураШапкиДокумента, , ,ВидОперации, ДоговорДляОтложенногоПроведения);
		Выручка.НДС			 = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(Выручка.НДС, ВалютаДокумента,
																СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
																СтруктураШапкиДокумента.КурсДокумента, КурсВзаиморасчетов,
																СтруктураШапкиДокумента.КратностьДокумента, КратностьВзаиморасчетов);
		...
		ИначеЕсли ОбщегоНазначенияБП.ЕстьТабЧастьДокумента("Товары", МетаданныеДокумента) Тогда
			Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СуммаВознаграждения", МетаданныеДокумента, "Товары") Тогда
				// Сумма доходов определяется без НДС
				ЕстьРеквизитСуммаВключаетНДС = ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаВключаетНДС", МетаданныеДокумента);
				ИменаРеквизитов = "УдержатьВознаграждение, СуммаВознаграждения, Товары, ДенежныеСредства";
				Если ЕстьРеквизитСуммаВключаетНДС Тогда
					ИменаРеквизитов = ИменаРеквизитов + ", СуммаВключаетНДС";
				КонецЕсли;
				
				РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, ИменаРеквизитов);
				
				СуммаДокументаВключаетНДС = Истина;
				Если ЕстьРеквизитСуммаВключаетНДС Тогда
					СуммаДокументаВключаетНДС = РеквизитыДокумента.СуммаВключаетНДС;
				КонецЕсли;
				
				ТаблицаТоварыДокумента = РеквизитыДокумента.Товары.Выгрузить();
				
				СуммаВознаграждения = ТаблицаТоварыДокумента.Итог("СуммаВознаграждения");
				СуммаНДСВознаграждения = ТаблицаТоварыДокумента.Итог("СуммаНДСВознаграждения");
				СуммаУслугПоДокументу =
					?(СуммаДокументаВключаетНДС, СуммаВознаграждения - СуммаНДСВознаграждения, СуммаВознаграждения);
					
				Если РеквизитыДокумента.УдержатьВознаграждение Тогда
					СуммаПолучено = РеквизитыДокумента.ДенежныеСредства.Выгрузить().Итог("Сумма");
					...
					КУДиР.Графа5 = КУДиР.Графа5 + РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
						ДоходПоУдержанномуВознаграждению, СтруктураШапкиДокумента.ВалютаВзаиморасчетов,
				...
				ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОЗакупках
					...
					Для Каждого СтрокаПлатеж Из СтруктураДопПараметров.ТаблицаРасчетов Цикл
						Если ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента)
							...
							Если СуммаВознаграждения > 0 И СуммаНДСВознаграждения > 0 Тогда
								ИсключаемыйИзДоходаНДС =
									ИсключаемыйИзДоходаНДС + СтрокаПлатеж.СуммаРуб * СуммаНДСВознаграждения / СуммаВознаграждения;
							КонецЕсли;
					...
					Если ДоходыПоВознаграждению > 0 Тогда
						...
						Если ИсключаемыйИзДоходаНДС > 0 Тогда
							ДополнитьСодержаниеКУДиРПояснениемПоИсключениюНДС(КУДиР, ИсключаемыйИзДоходаНДС);
						КонецЕсли;
						
						КУДиР.Графа5 = КУДиР.Графа5 + ДоходыПоВознаграждению - ИсключаемыйИзДоходаНДС;
					КонецЕсли;
				КонецЕсли;
			Конецесли;
	...
	ИначеЕсли ВидОперации = "КорректировкаРеализации" Тогда
		...
		ЭтоКомиссионер      = ТипЗнч(Параметры.Реквизиты[0].ДокументРеализацииСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
							  И Параметры.Реквизиты[0].ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером;	
		
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН 
			И НЕ ЭтоКомиссионер Тогда
		...
		ИначеЕсли СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН 
			И ЭтоКомиссионер Тогда
			
			Договор       = Параметры.Реквизиты[0].ДоговорКонтрагента;
			
			ТаблицаПартий = ПодготовитьТаблицуПартий(СтруктураДопПараметров.ТаблицаРеализация, "НаРеализацию");
			СтатусПартии  = Перечисления.СтатусыПартийУСН.Купленные;
			
			ПеремещениеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистровВх, Договор, СтатусПартии, "НаРеализацию");
			
			ТаблицаПартий = ПодготовитьТаблицуПартий(СтруктураДопПараметров.ТаблицаВозвратОтКомиссионера, "ВозвратСРеализации");
			СтатусПартии  = Перечисления.СтатусыПартийУСН.НаРеализации;
			
			ПеремещениеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистровВх, Договор, СтатусПартии, "ВозвратСРеализации");
	...
	ИначеЕсли ВидОперации = "ВыплатаЗП"
		Или ВидОперации = "ВыплатаЗПРаботнику" Тогда
		
		ТаблицаОплат = СтруктураДопПараметров.ТаблицаВыплатаЗарплаты;
		
		ВыплатаЗарплаты(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР);
	...
	ИначеЕсли ВидОперации = "ВыплатаДГПХ"
		Или ВидОперации = "ВыплатаДГПХРаботнику" Тогда
		
		ТаблицаОплат = СтруктураДопПараметров.ТаблицаВыплатаДоговоровПодряда;
		
		ВыплатаЗарплаты(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, Истина);
	...
	ИначеЕсли ВидОперации = "ОплатаОтПокупателя"
		...
		ТаблицаОплат = СтруктураДопПараметров.ТаблицаРасчетов;
		ТаблицаКомиссииБанка = Неопределено;
		Если ВидОперации = "ОплатаОтЭквайера" Тогда
			ПрименяетсяУСНДоходыМинусРасходы = УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(
				СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата);
			ВыделятьНДССКомиссииБанка = ПрименяетсяУСНДоходыМинусРасходы
				И УчетнаяПолитика.ВыделятьНДССКомиссииБанка(
					СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата);
			Если ВыделятьНДССКомиссииБанка Тогда
				ТаблицаКомиссииБанка = ТаблицаОплат.СкопироватьКолонки();
				Для Каждого СтрокаТаблицы Из ТаблицаОплат Цикл
					Если УчетВзаиморасчетов.ЭтоСчетРасчетовСПоставщиками(СтрокаТаблицы.КорСчет) Тогда
						СтрокаТаблицыКомиссия = ТаблицаКомиссииБанка.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыКомиссия, СтрокаТаблицы);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		...
		Если ТаблицаОплат.Количество() > 0 Тогда
			...
			РасчетыСКонтрагентамиУСН(
				СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, "Доход", ТаблицаНДСПродажи);
		Конецесли;
		
		Если ЗначениеЗаполнено(ТаблицаКомиссииБанка) Тогда
			РасчетыСКонтрагентамиУСН(
				СтруктураШапкиДокумента, ТаблицаКомиссииБанка, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, "Оплата");
		КонецЕсли;
	...
	ИначеЕсли (ВидОперации = "РаспределениеРасходов") Тогда
		РаспределениеРасходов(СтруктураШапкиДокумента, ДвиженияРегистров);
	...
	ИначеЕсли ВидОперации = "ПеречислениеНалога" ИЛИ ВидОперации = "УплатаНалогаПоКвитанции" Тогда
		...
		Иначе
			...
			Если СтруктураДопПараметров.Свойство("ТаблицаУплатыПоЕдиномуНалоговомуСчету")
				...
				Для Каждого СтрокаТаблицы Из СтруктураДопПараметров.ТаблицаУплатыПоЕдиномуНалоговомуСчету Цикл
			...
			ИначеЕсли СчетНалога <> ПланыСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет Тогда
	...
	ИначеЕсли ВидОперации = "ЗачетАвансаПоЕдиномуНалоговомуСчету" Тогда
		...
		Если СтруктураДопПараметров.Свойство("ТаблицаУплатыПоЕдиномуНалоговомуСчету")
			...
			НачисленияПрочиеРасчеты = НачисленияПрочиеРасчеты(СтруктураШапкиДокумента.Организация,
				Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Включая));
			...
			Для Каждого ПлатежныйДокумент Из ПлатежныеДокументы Цикл
				...
				Для Каждого СтрокаТаблицы Из ТаблицаПогашенийПоДокументу Цикл
					...
					Если ЗначениеЗаполнено(СтрокаТаблицы.Налог)
						И СчетНалога = ПланыСчетов.Хозрасчетный.НДФЛ Тогда
						КодБК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Налог, "КодБК");
						ЭтоНДФЛДивиденды = Справочники.ВидыНалоговИПлатежейВБюджет.ЭтоНДФЛНалоговыйАгентДивиденды13(КодБК)
							Или Справочники.ВидыНалоговИПлатежейВБюджет.ЭтоНДФЛНалоговыйАгентДивиденды15(КодБК);
						Если ЭтоНДФЛДивиденды Тогда
					...
					Если СчетНалога = ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате Тогда
						...
						СтатусыОплатыРасходовУСН.Добавить(Перечисления.СтатусыРасходовУСН.НеОплачено);
						СтатусыОплатыРасходовУСН.Добавить(Перечисления.СтатусыРасходовУСН.НеОплаченоНеОплаченоПокупателем);
						...
						Фильтр.ЗначениеПоля = СтатусыОплатыРасходовУСН;
						Фильтр.Условие      = "В";
						
						Фильтр = ТаблицаФильтров.Добавить();
						Фильтр.ИмяПоля      = "ВидРасхода";
						Фильтр.ЗначениеПоля = Перечисления.ВидыРасходовУСН.НДС_ТС;
						Фильтр.Условие      = "=";
						ТаблицаРасходов = ВыполнитьЗапросПоРасходам(Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Включая),
							СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
						Если ТаблицаРасходов.Количество() > 0 Тогда
							ДвиженияРегистров = Новый Структура("ВзаиморасчетыРасход, РасходыУСН, КУДиР",
									Новый ТаблицаЗначений, Новый ТаблицаЗначений, Новый ТаблицаЗначений);
							ДвиженияРегистров.РасходыУСН = ТаблицаРасходов.СкопироватьКолонки();
							Если ДвиженияРегистров.РасходыУСН.Колонки.Найти("ВидДвижения") = Неопределено Тогда
								ДвиженияРегистров.РасходыУСН.Колонки.Добавить("ВидДвижения");
							КонецЕсли;
							СуммаКРаспределению = СтрокаТаблицы.Сумма;
							ИтоговаяСтруктура = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, СуммаКРаспределению, "Оплата", , ,);
							ДвиженияРегистров = СобратьТаблицыДвижений(ИтоговаяСтруктура, , , ДвиженияРегистров);
						КонецЕсли;
					КонецЕсли;
					
					НачислениеПрочиеРасчеты = НачисленияПрочиеРасчеты.Найти(СчетНалога, "СчетУчета");
					СуммаДвижения = 0;
					Если НачислениеПрочиеРасчеты <> Неопределено Тогда
						СуммаДвижения = Мин(СтрокаТаблицы.Сумма, НачислениеПрочиеРасчеты.Сумма);
					КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ОтражениеУплатыНалогаУСН" Тогда
		...
		Если Не (СтруктураДопПараметров.Свойство("ТаблицаНалоги")
				И ЗначениеЗаполнено(СтруктураДопПараметров.ТаблицаНалоги)) Тогда
			Возврат;
		КонецЕсли;
		
		Для Каждого СтрокаНалоги Из СтруктураДопПараметров.ТаблицаНалоги Цикл
			НоваяСтрока = ТаблицаОплат.Добавить();
			НоваяСтрока.ДоговорКонтрагента = СтрокаНалоги.СчетУчета;
			НоваяСтрока.Сделка = Неопределено;
			НоваяСтрока.Сумма  = СтрокаНалоги.Сумма;
			
			Если СтрокаНалоги.СчетУчета = ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате Тогда
				//Формирование таблицы фильтров
				ТаблицаФильтров = Новый ТаблицаЗначений;
				ТаблицаФильтров.Колонки.Добавить("ИмяПоля");
				ТаблицаФильтров.Колонки.Добавить("ЗначениеПоля");
				...
				Фильтр.ЗначениеПоля = Перечисления.ВидыРасходовУСН.НДС_ТС;
				Фильтр.Условие      = "=";
				ТаблицаРасходов = ВыполнитьЗапросПоРасходам(Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Исключая),
					СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
				Если ТаблицаРасходов.Количество() > 0 Тогда
					ДвиженияРегистров = Новый Структура("ВзаиморасчетыРасход, РасходыУСН, КУДиР",
							Новый ТаблицаЗначений, Новый ТаблицаЗначений, Новый ТаблицаЗначений);
					...
					Если ДвиженияРегистров.РасходыУСН.Колонки.Найти("ВидДвижения") = Неопределено Тогда
						ДвиженияРегистров.РасходыУСН.Колонки.Добавить("ВидДвижения");
					КонецЕсли;
					СуммаКРаспределению = СтрокаНалоги.Сумма;
					ИтоговаяСтруктура = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, СуммаКРаспределению, "Оплата", , ,);
					ДвиженияРегистров = СобратьТаблицыДвижений(ИтоговаяСтруктура, , , ДвиженияРегистров);
					ИсключитьИзКУДиРНДС_ТС(ДвиженияРегистров);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если ТаблицаОплат.Количество() <> 0 Тогда
			Реквизиты = СтруктураДопПараметров.Реквизиты[0];
			СтруктураШапкиДокумента.Вставить("ДокументОплаты", Реквизиты.ПлатежныйДокумент);
			ОплатаПрочееУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ОписаниеПолучателя);
		КонецЕсли;
	...
	ИначеЕсли (ВидОперации = "ОплатаОтПокупателяБезЗакрывающихДокументов")
		...
		Если НЕ ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам")
			...
			И ВидОперации = "ОплатаОтПокупателяБезЗакрывающихДокументов" Тогда
			ПризнаватьДоходы = НЕ ((СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером)
				ИЛИ (ОбщегоНазначения.ЕстьРеквизитОбъекта("Контрагент", МетаданныеДокумента) И ОбменСМаркетплейс.ЭтоМаркетплейсКомиссионер(ДокументСсылка.Контрагент)));
		КонецЕсли;
			
		Если ПризнаватьДоходы Тогда
			...
			Для Каждого СтрокаТаблицы Из ДокументСсылка.РасшифровкаПлатежа Цикл
				...
				Если НЕ СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
					...
					Если СтрокаТаблицы.ПорядокОтраженияАванса = Перечисления.ПорядокОтраженияАвансов.ДоходУСН Тогда
						...
						Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН Тогда
							ИсключенныйНДС  = УчетНДСКлиентСервер.РассчитатьСуммуНДС(СуммаПлатежа, Истина, 
								УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтруктураШапкиДокумента.СтавкаНДСПриУСН));
								
							КУДиР.Графа5 = КУДиР.Графа5 - ИсключенныйНДС;
							НоваяСтрокаРасшифровкаПоДоговорам = КУДиР.РасшифровкаПоДоговорам.Добавить();
							НоваяСтрокаРасшифровкаПоДоговорам.ИсключенныйНДС = ИсключенныйНДС;
							НоваяСтрокаРасшифровкаПоДоговорам.Графа4 = СуммаПлатежа;
							НоваяСтрокаРасшифровкаПоДоговорам.Графа5 = СуммаПлатежа - ИсключенныйНДС;
						КонецЕсли;
	...
	ИначеЕсли (ВидОперации = "ВыручкаСНТТ") Тогда
		...
		Если УчетнаяПолитика.СпособОценкиТоваровВРознице(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) =
			...
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы
				И ОбщегоНазначения.ЕстьРеквизитОбъекта("ДеятельностьНаТорговомСборе", МетаданныеДокумента) Тогда
				РевизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
					ДокументСсылка, "ДеятельностьНаТорговомСборе, Графа5_УСН");
				Если РевизитыДокумента.ДеятельностьНаТорговомСборе Тогда
					КУДиР.ДоходТорговыйСбор = КУДиР.ДоходТорговыйСбор + РевизитыДокумента.Графа5_УСН;
				КонецЕсли;
			КонецЕсли;
			
			Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНПатент Тогда
				
				РасшифровкаПлатежаВыборка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "РасшифровкаПлатежа");
				РасшифровкаПлатежа = РасшифровкаПлатежаВыборка.Выгрузить();
				СуммаДокумента = РасшифровкаПлатежа.Итог("СуммаПлатежа");
				
				Если СуммаДокумента > 0
					И ОбщегоНазначения.ЕстьРеквизитОбъекта("Патент", МетаданныеДокумента) Тогда
					Патент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Патент");
					Если ЗначениеЗаполнено(Патент) Тогда
						ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
						ЗаписьКУДиРПатент.Патент = Патент;
		...
		Иначе
			...
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Контрагент, РасшифровкаПлатежа");
			...
			СчетаДоходовРасходов = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаВНТТ(СтруктураШапкиДокумента.Организация, РеквизитыДокумента.Контрагент, СтруктураШапкиДокумента.Дата,
			...
			РасшифровкаПлатежа = РеквизитыДокумента.РасшифровкаПлатежа.Выгрузить();
			...
			Иначе
				КУДиР.Графа5 = КУДиР.Графа5 + СуммаДокумента - СуммаНДС;
			КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ЗачетВыручкиКомиссионера" Тогда
		
		// Признание доходов по выручке отраженная отчетом комиссионера, зачитываемой в счет вознаграждения
		Если СтруктураДопПараметров.Свойство("ДолгПокупателя") Тогда
			ТаблицаОплат = СтруктураДопПараметров.ДолгПокупателя;
			ТаблицаНДСПродажи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураДопПараметров, "ТаблицаНДСПродажи");
			Если Не СтруктураДопПараметров.Свойство("ТаблицаНДСПродажи") Тогда
				СтруктураДопПараметров.Свойство("ТаблицаНДСПродажи", ТаблицаНДСПродажи);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ТаблицаОплат) Тогда
				...
				Если Параметры.ВидДокумента = "ОтчетКомиссионераОПродажах" Тогда
					// Признание расходов по оплате для поступления услуг "оплаченного" выручкой комиссионера 
					РасчетыСКонтрагентамиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, "Оплата");
					КУДиР.Графа6 = 0;
	...
	ИначеЕсли (ВидОперации = "ПроведениеВзаимозачетаДебитор") Тогда
		...
		Если СтруктураДопПараметров.Свойство("АвансПокупателя") Тогда
			...
			Если ТаблицаОплат.Количество() > 0 Тогда
				Для Каждого СтрокаОплаты Из ТаблицаОплат Цикл
					СуммаВзаиморасчетов = СтрокаОплаты.СуммаРуб;
					
					ДоходКомитента      = СтрокаОплаты.СуммаВзаиморасчетовКомитента;
					ДоходПатент         = СтрокаОплаты.СуммаВзаиморасчетовПатент;
					
					ДоходУСН = СуммаВзаиморасчетов - ДоходКомитента - ДоходПатент;
					
					Если ДоходУСН > 0 Тогда
						СуммаАвансаСНДС = 0;
						СуммаНДСАванса = 0;
						
						ОпределятьНДССАванса = СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
							И ЗначениеЗаполнено(СтрокаОплаты.ДокументРасчетов)
							И (ТипЗнч(СтрокаОплаты.ДокументРасчетов) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет")
								Или ТипЗнч(СтрокаОплаты.ДокументРасчетов) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер"));
						
						Если ОпределятьНДССАванса Тогда
							РасшифровкаПлатежаАванса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
								СтрокаОплаты.ДокументРасчетов, "РасшифровкаПлатежа");
							РасшифровкаАванса = РасшифровкаПлатежаАванса.Выгрузить();
							ПараметрыОтбора = Новый Структура("ДоговорКонтрагента", СтрокаОплаты.ДоговорКонтрагента);
							СтрокиРасшифровкиАвансаПоДоговору = РасшифровкаАванса.НайтиСтроки(ПараметрыОтбора);
							
							Для Каждого СтрокаАванса Из СтрокиРасшифровкиАвансаПоДоговору Цикл
								СпособПогашенияПоДокументу =
									СтрокаАванса.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.ПоДокументу
									И ЗначениеЗаполнено(СтрокаАванса.Сделка);
								АвансУСН = СтрокаАванса.ПорядокОтраженияАванса <> Перечисления.ПорядокОтраженияАвансов.ДоходКомитента
									И СтрокаАванса.ПорядокОтраженияАванса <> Перечисления.ПорядокОтраженияАвансов.ДоходЕНВД
									И ТипЗнч(СтрокаАванса.ПорядокОтраженияАванса) <> Тип("СправочникСсылка.Патенты");
								
								Если СпособПогашенияПоДокументу Или Не АвансУСН Тогда
									Продолжить;
								КонецЕсли;
								
								СуммаАвансаСНДС = СуммаАвансаСНДС + СтрокаАванса.СуммаПлатежа;
								СуммаНДСАванса = СуммаНДСАванса + СтрокаАванса.СуммаНДС;
							КонецЦикла;
						КонецЕсли;
						
						СуммаУменьшенияДоходаУСН = ДоходУСН;
						Для Каждого СтрокаРасшифровки Из КУДиР.РасшифровкаПоДоговорам Цикл
							КорректировкаДоходаУСН = Мин(СуммаУменьшенияДоходаУСН, СтрокаРасшифровки.Графа4);
							
							Если КорректировкаДоходаУСН <= 0 Тогда
								Продолжить;
							КонецЕсли;
							
							КорректировкаИсключенныйНДС =?(СтрокаРасшифровки.Графа4 = 0, 0,
								Окр(СтрокаРасшифровки.ИсключенныйНДС * КорректировкаДоходаУСН / СтрокаРасшифровки.Графа4, 2));
							
							ИсключенныйНДСАванс = ?(СуммаАвансаСНДС = 0, 0,
								Окр(СуммаНДСАванса * КорректировкаДоходаУСН / СуммаАвансаСНДС, 2));
							КорректировкаИсключенныйНДСАванс = ИсключенныйНДСАванс - КорректировкаИсключенныйНДС;
							
							СтрокаРасшифровки.Графа5 = СтрокаРасшифровки.Графа5
								- КорректировкаДоходаУСН
								+ КорректировкаИсключенныйНДС
								+ КорректировкаИсключенныйНДСАванс;
							СтрокаРасшифровки.Графа4 = СтрокаРасшифровки.Графа4 - КорректировкаДоходаУСН;
							
							КУДиР.Графа5 = КУДиР.Графа5
								- КорректировкаДоходаУСН
								+ КорректировкаИсключенныйНДС
								+ КорректировкаИсключенныйНДСАванс;
							КУДиР.Графа4 = КУДиР.Графа4 - КорректировкаДоходаУСН;
							
							СуммаУменьшенияДоходаУСН = СуммаУменьшенияДоходаУСН - КорректировкаДоходаУСН;
						КонецЦикла;
					КонецЕсли;
					
					Если ДоходПатент > 0 Тогда
						ДокументРасчетов = СтрокаОплаты.ДокументРасчетов;
						МетаданныеРасчетногоДокумента = ДокументРасчетов.Метаданные();
						ПатентРасчетногоДокумента = Неопределено;
						Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Патент", МетаданныеРасчетногоДокумента) Тогда
							ПатентРасчетногоДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРасчетов, "Патент");
						КонецЕсли;
						
						Если ЗначениеЗаполнено(ПатентРасчетногоДокумента) Тогда
							СуммаУменьшенияДохода = ДоходПатент;
							
							СтрокиПризнанныйДоходПоПатенту = ДвиженияКУДиРПатент.НайтиСтроки(
								Новый Структура("Патент", ПатентРасчетногоДокумента));
							Для Каждого СтрокаДоходаПоПатенту Из СтрокиПризнанныйДоходПоПатенту Цикл
								КорректировкаДоходаПоПатенту = Мин(СуммаУменьшенияДохода, СтрокаДоходаПоПатенту.Графа4);
								
								Если КорректировкаДоходаПоПатенту <= 0 Тогда
									Продолжить;
								КонецЕсли;
								СтрокаДоходаПоПатенту.Графа4 = СтрокаДоходаПоПатенту.Графа4 - КорректировкаДоходаПоПатенту;
								СуммаУменьшенияДохода = СуммаУменьшенияДохода - КорректировкаДоходаПоПатенту;
							КонецЦикла;
							
							Если СуммаУменьшенияДохода > 0 Тогда
								ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
								ЗаписьКУДиРПатент.Патент = ПатентРасчетногоДокумента;
								ЗаписьКУДиРПатент.Графа4 = -ДоходПатент;
								ЗаписьКУДиРПатент.Содержание = ОписаниеСобытияДляКУДиР(
									СтруктураШапкиДокумента,
									СтруктураШапкиДокумента.ВидОперацииУСН,
									МетаданныеРасчетногоДокумента,
									СтрокаОплаты.ДоговорКонтрагента);
								ЗаписьКУДиРПатент.Контрагент = СтрокаОплаты.Контрагент;
								ЗаписьКУДиРПатент.ДоговорКонтрагента = СтрокаОплаты.ДоговорКонтрагента;
							КонецЕсли;
							
							КоличествоЭлементовКоллекции = ДвиженияКУДиРПатент.Количество();
							Для Номер = 1 По КоличествоЭлементовКоллекции Цикл
								ЭлементКоллекции = ДвиженияКУДиРПатент[КоличествоЭлементовКоллекции - Номер];
								Если ЭлементКоллекции.Графа4 = 0 Тогда
									ДвиженияКУДиРПатент.Удалить(ЭлементКоллекции);
								КонецЕсли;
							КонецЦикла;
							
							КУДиР.Графа4 = КУДиР.Графа4 - ДоходПатент;
							КУДиР.ДоходЕНВД = КУДиР.ДоходЕНВД - ДоходПатент;
							НоваяСтрокаРасшифровкаПоДоговорам = КУДиР.РасшифровкаПоДоговорам.Добавить();
							НоваяСтрокаРасшифровкаПоДоговорам.Контрагент = СтрокаОплаты.Контрагент;
							НоваяСтрокаРасшифровкаПоДоговорам.ДоговорКонтрагента = СтрокаОплаты.ДоговорКонтрагента;
							НоваяСтрокаРасшифровкаПоДоговорам.Графа4 = -ДоходПатент;
							НоваяСтрокаРасшифровкаПоДоговорам.ДоходЕНВД = -ДоходПатент;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			Конецесли;
		...
		Если СтруктураДопПараметров.Свойство("ДолгПоставщику") Тогда
			...
			Если ТаблицаОплат.Количество() > 0 И
				...
				((ТаблицаОплат[0].ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.Прочее) Или
				(ТаблицаОплат[0].ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку)) Тогда
	...
	ИначеЕсли ВидОперации = "ИсключениеТуристическогоНалогаИзДоходов" Тогда
		
		СодержаниеОперации = НСтр("ru = 'Исключение туристического налога из доходов'",
			ОбщегоНазначения.КодОсновногоЯзыка());
		КУДиР.Графа5 = СтруктураДопПараметров.ИсключаемыйДоход;
	...
	Если ВидОперации = "НачислениеЗП" Тогда
		ДатаДокумента = СтруктураШапкиДокумента.Ссылка.МоментВремени().Дата;
		МоментДокумента = ДатаДокумента;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ПериодРегистрации", МетаданныеДокумента) Тогда
			ПериодРегистрацииДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				СтруктураШапкиДокумента.Ссылка, "ПериодРегистрации");
			Если КонецМесяца(ПериодРегистрацииДокумента) <> КонецМесяца(ДатаДокумента) Тогда
				МоментДокумента = КонецМесяца(ПериодРегистрацииДокумента);
			КонецЕсли;
		КонецЕсли;
	...
	НеВыполнятьКонтрольУдаляемых = Ложь;
	Если ТипЗнч(СтруктураШапкиДокумента.Объект) <> Тип("Структура") Тогда
		НеВыполнятьКонтрольУдаляемых = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			СтруктураШапкиДокумента.Объект.ДополнительныеСвойства, "НеВыполнятьКонтрольУдаляемых", Ложь);
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН ИЛИ СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы Тогда
		...
		Если ДвиженияРегистров <> Неопределено Тогда
			...
			Если ДвиженияРегистров.ВзаиморасчетыРасход.Количество() > 0 Тогда
				...
				Если НеВыполнятьКонтрольУдаляемых Тогда
					НаборДвижений.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
				КонецЕсли;
			...
			Если (ДвиженияРегистров.КУДиР.Количество() > 0)
				...
				КонецЕсли;
				Если ТаблицаРасходов.Колонки.Найти("ОплатаИзВыручкиКомиссионера") = Неопределено  Тогда
					ТаблицаРасходов.Колонки.Добавить("ОплатаИзВыручкиКомиссионера", Новый ОписаниеТипов("Булево"));
					ТаблицаРасходов.ЗаполнитьЗначения(Ложь, "ОплатаИзВыручкиКомиссионера");
				КонецЕсли;
				Если ТаблицаРасходов.Колонки.Найти("ИсключатьНДС") = Неопределено Тогда
					ТаблицаРасходов.Колонки.Добавить("ИсключатьНДС", Новый ОписаниеТипов("Булево"));
				КонецЕсли;
				...
				Если ВидОперации = "РаспределениеРасходов" Тогда
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, "", "Сумма, НДС");
				ИначеЕсли ВидОперации = "ГТД" ИЛИ ВидОперации = "СписаниеТаможенныхПлатежей" Тогда
					ИменаГруппировок = "ВидРасхода,ЭтоТаможенныйНДС";
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, ИменаГруппировок, "Сумма, НДС");
				...
				ИначеЕсли ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
					ИменаГруппировок =
						"ДокументОплаты, ВидРасхода, ВидСтраховогоВзноса, СтраховойПериод,
						| СчетУчета, ВзносыПодлежащиеУплате, ВидВзносаПодлежащегоУплате";
					Если ТаблицаРасходов.Колонки.Найти("ДополнительноеОписание") <> Неопределено Тогда
						ИменаГруппировок = ИменаГруппировок + ", ДополнительноеОписание";
					КонецЕсли;
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, ИменаГруппировок, "Сумма, НДС");
				ИначеЕсли ВидОперации = "Поступление" И СтруктураШапкиДокумента.ВидДокумента = "ПоступлениеТоваровУслуг" Тогда
					Для Каждого СтрокаРасходов Из ТаблицаРасходов Цикл
						СтрокаРасходов.ОплатаИзВыручкиКомиссионера = ТипЗнч(СтрокаРасходов.РеквизитыДокументаОплаты) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах");
					КонецЦикла; 
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, "ВидРасхода, ОплатаИзВыручкиКомиссионера", "Сумма, НДС");
				Иначе
					ДеревоРасходов = ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРасходов, "ВидРасхода", "Сумма, НДС");
				КонецЕсли;
				...
				ТаблицаОплатыОС = НоваяТаблицаОплатыВнеоборотногоАктива();
				ТаблицаОплатыНМА = НоваяТаблицаОплатыВнеоборотногоАктива();
				
				ОтражатьОплатыОСДляУСН  = ДокументОбъект.Движения.Найти("ОплатыОСДляУСН") <> Неопределено;
				ОтражатьОплатыНМАДляУСН = ДокументОбъект.Движения.Найти("ОплатыНМАДляУСН") <> Неопределено;
				...
				Для каждого Расход Из ДеревоРасходов Цикл
					...
					Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.ОС Тогда
						...
						Если ОтражатьОплатыОСДляУСН Тогда
							...
							Для Каждого СтрокаОС Из Расход.Строки Цикл
								...
								НоваяСтрокаОплатыОС.ВнеоборотныйАктив        = СтрокаОС.ЭлементРасхода;
								...
								Если СтрокаОС.ИсключатьНДС Тогда
									НоваяСтрокаОплатыОС.СуммаОплаты = СтрокаОС.Сумма - СтрокаОС.НДС;
								Иначе
					...
					Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.НМА Тогда
						
						Если ОтражатьОплатыНМАДляУСН Тогда
							
							Для Каждого СтрокаНМА Из Расход.Строки Цикл
							
								НоваяСтрокаОплатыНМА = ТаблицаОплатыНМА.Добавить();
								НоваяСтрокаОплатыНМА.ВнеоборотныйАктив        = СтрокаНМА.ЭлементРасхода;
								НоваяСтрокаОплатыНМА.Организация              = ДокументСсылка.Организация;
								НоваяСтрокаОплатыНМА.РеквизитыДокументаОплаты = СтрокаНМА.РеквизитыДокументаОплаты;
								
								Если СтрокаНМА.ИсключатьНДС Тогда
									НоваяСтрокаОплатыНМА.СуммаОплаты = СтрокаНМА.Сумма - СтрокаНМА.НДС;
								Иначе
									НоваяСтрокаОплатыНМА.СуммаОплаты = СтрокаНМА.Сумма;
								КонецЕсли;
					...
					Если ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
						ЗаписьКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(
							Расход.ДокументОплаты, ЗаписьКУДиР.Организация, ЗаписьКУДиР.Период, Расход.ВзносыПодлежащиеУплате);
						Если Расход.ВзносыПодлежащиеУплате Тогда
							ЗаписьКУДиР.ВзносыПодлежащиеУплате = Расход.Сумма;
							ЗаписьКУДиР.ВидВзносаПодлежащегоУплате = Расход.ВидВзносаПодлежащегоУплате;
						КонецЕсли;
					ИначеЕсли ВидОперации = "ВыплатыСамозанятым" Тогда
						ЗаписьКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(
							ДокументСсылка.РеестрВыплатСамозанятым, ЗаписьКУДиР.Организация, ЗаписьКУДиР.Период);
					Иначе
						ЗаписьКУДиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(
							ДокументСсылка, ЗаписьКУДиР.Организация, ЗаписьКУДиР.Период);
					КонецЕсли;
					
					// Для ОС и НМА графа 7 и НДС заполняется регламентными операциями
					// "Признание расходов на приобретение ОС для УСН" и "Признание расходов на приобретение НМА для УСН"
					Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.ОС
						Или Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.НМА Тогда
						Продолжить;
					КонецЕсли;
					...
					Если (ЗначениеЗаполнено(Расход.ВидРасхода)) Или (ВидОперации = "РаспределениеРасходов") Тогда
						...
						Если ОтражатьРазнымиПериодами Тогда
							Если ЗначениеЗаполнено(Расход.ДатаКорректировки) И Год(Расход.ДатаКорректировки) < Год(МоментДокумента) Тогда
								ПараметрыЗаписиРасшифровки = ?(РеквизитыКорректировки = Неопределено,
									СтруктураШапкиДокумента,
									ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(РеквизитыКорректировки));
								ДобавитьСвойстваПоНДС(ПараметрыЗаписиРасшифровки);
							КонецЕсли;
						КонецЕсли;
						СформироватьЗаписьРасшифровки(ПараметрыЗаписиРасшифровки, НаборЗаписейРасшифровки, Расход, ЗаписьКУДиР, ВидОперации, КэшНаименованиеКонтрагента);
					КонецЕсли;
				...
				Если ТаблицаОплатыОС.Количество() > 0 Тогда
					...
					Если НеВыполнятьКонтрольУдаляемых Тогда
						ОплатыОСДляУСН.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
					КонецЕсли;
					
					ТаблицаОплатыОСДляУСН = ПодготовитьТаблицуОплатыВнеоборотноогоАктиваДляУСН(ТаблицаОплатыОС);
					...
					Для Каждого СтрокаОплатыОС Из ТаблицаОплатыОСДляУСН Цикл
						...
						Движение.ОсновноеСредство = СтрокаОплатыОС.ВнеоборотныйАктив;
				...
				Если ТаблицаОплатыНМА.Количество() > 0 Тогда
					
					ОплатыНМАДляУСН = ДокументОбъект.Движения.ОплатыНМАДляУСН;
					Если НеВыполнятьКонтрольУдаляемых Тогда
						ОплатыНМАДляУСН.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
					КонецЕсли;
					
					ТаблицаОплатыНМАДляУСН = ПодготовитьТаблицуОплатыВнеоборотноогоАктиваДляУСН(ТаблицаОплатыНМА);
					
					Для Каждого СтрокаОплатыНМА Из ТаблицаОплатыНМАДляУСН Цикл
						
						Движение = ОплатыНМАДляУСН.Добавить();
						
						Движение.Период              = МоментДокумента;
						Движение.Регистратор         = ДокументСсылка;
						Движение.Активность          = Истина;
						Движение.НематериальныйАктив = СтрокаОплатыНМА.ВнеоборотныйАктив;
						
						ЗаполнитьЗначенияСвойств(Движение, СтрокаОплатыНМА);
						
					КонецЦикла;
					
					ОплатыНМАДляУСН.Записать(Истина);
	...
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНПатент И ДвиженияКУДиРПатент.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПатент Из ДвиженияКУДиРПатент Цикл
			...
			Если ЗначениеЗаполнено(СтрокаПатент.Патент)
				И Не УчетПСН.ДеятельностьПоПатентуДействуетНаДату(СтрокаПатент.Патент, МоментДокумента) Тогда
...
Процедура РасчетыСКонтрагентами_ВыплатыСамозанятымВУСН(ДокументСсылка, КУДиР, СтруктураДопПараметров, СтруктураШапкиДокумента, ДвиженияРегистров)
	
	ТаблицаВыплатыСамозанятым = СтруктураДопПараметров.ТаблицаВыплатаСамозанятым;
	ИтоговаяСтруктура = Новый Структура(
		"ТаблицаПриход, ТаблицаРасход, ТаблицаПринятых", Новый ТаблицаЗначений, Новый ТаблицаЗначений, Новый ТаблицаЗначений);
	
	Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
		НаборЗаписей = РегистрыНакопления.РасходыПриУСН.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(СтруктураШапкиДокумента.РеестрВыплатСамозанятым);
		НаборЗаписей.Прочитать();
		
		ТаблицаПрихода = НаборЗаписей.ВыгрузитьКолонки();

		Для Каждого Запись Из НаборЗаписей Цикл
			Если Запись.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеОплачено
		...
		ИтоговаяСтруктура = ИзменениеСтатусовРасходов(
			СтруктураШапкиДокумента,
			ТаблицаПрихода,
			ТаблицаВыплатыСамозанятым.Итог("Сумма"),
			"Оплата",
			,
			,
			ИтоговаяСтруктура);
	...
	// Записи в КУДиР
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураШапкиДокумента.РеестрВыплатСамозанятым, "Номер, Дата");
	КУДиР.Графа6 = ТаблицаВыплатыСамозанятым.Итог("Сумма");
	КУДиР.ЧастиСодержания.Событие = СтрШаблон(
		НСтр("ru='Оплата услуг самозанятых по реестру №%1 от %2'"),
...
Процедура СформироватьЗаписьРасшифровки(СтруктураШапкиДокумента, НаборЗаписейРасшифровки, Расход, ЗаписьКУДиР, ВидОперации, КэшНаименованиеКонтрагента)
	...
	ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Налоги Тогда
		...
		Если ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
			ИменаГруппировок = "ВидРасхода, СчетУчета, ОтражениеВУСН, РеквизитыДокументаОплаты,
				|ВидСтраховогоВзноса, СтраховойПериод, ВзносыПодлежащиеУплате, ВидВзносаПодлежащегоУплате";
		Иначе
			ИменаГруппировок = "ВидРасхода, СчетУчета, ОтражениеВУСН, РеквизитыДокументаОплаты";
		КонецЕсли;
	...
	Счетчик = 1;
	ЕстьТаможенныйНДС = Расход.Строки.Колонки.Найти("ЭтоТаможенныйНДС") <> Неопределено;
	...
	Для Каждого Строка Из Расход.Строки Цикл
		...
		Если Строка.ВидРасхода = Перечисления.ВидыРасходовУСН.Номенклатура Тогда
			...
			Если ТипЗнч(Строка.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				Описание = Описание + СтрШаблон(" Оплачено поставщику: ""%1"".",
					НаименованиеКонтрагентаНаПечатьИзКэша(Строка.ДоговорКонтрагента, ЗаписьКУДиР.Период, КэшНаименованиеКонтрагента));
		...
		ИначеЕсли Строка.ВидРасхода = Перечисления.ВидыРасходовУСН.РБП Тогда
			Описание = "Статья РБП: """ + Строка.ЭлементРасхода + """"
				+ ", общая сумма: " + Строка.ЭлементРасхода.Сумма
		...
		ИначеЕсли Строка.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
			Если ВидОперации = "ВыплатаДГПХ" Тогда
				Описание = "Оплата договора ГПХ сотрудника: """ + Строка.ЭлементРасхода + """.";
			Иначе
		...
		ИначеЕсли Строка.ВидРасхода = Перечисления.ВидыРасходовУСН.Налоги Тогда
			Если ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
				Описание = УчетСтраховыхВзносовИП.СодержаниеОперацииУплатыВзносов(Строка.СчетУчета, Строка.ВидСтраховогоВзноса,
					Строка.СтраховойПериод, СтруктураШапкиДокумента.Организация, Строка.ВзносыПодлежащиеУплате);
			Иначе
		...
		ИначеЕсли (Не ЗначениеЗаполнено(Расход.ВидРасхода)) И (ВидОперации = "РаспределениеРасходов") Тогда
		...
		Если (СтруктураШапкиДокумента.ПлательщикНДС
				Или СтруктураШапкиДокумента.ПорядокПризнанияРасходовПоНДС = Перечисления.ПорядокПризнанияРасходовПоНДС.ВключатьВСтоимость)
...
Процедура СписаниеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистров, Договор, СтатусПартии, ВидОперации = "Списание")
	...
	// Подготовим таблицу способов учета НДС по партиям
	Если ТаблицаДляСписания.Количество() > 0
		И СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
		
		СпособыУчетаНДСПартий = НоваяТаблицаСпособовУчетаНДСПартий();
		
		Если ВидОперации = "Реализация" Или ВидОперации = "Розница" Тогда
			
			ДвиженияДокумента = СтруктураШапкиДокумента.Объект.Движения;
			
			Если ДвиженияДокумента.Найти("НДСРаздельныйУчет") <> Неопределено Тогда
				СпособыУчетаНДСДвижения = ДвиженияДокумента.НДСРаздельныйУчет.Выгрузить();

				Для Каждого СтрокаСпособаУчетаНДС Из СпособыУчетаНДСДвижения Цикл
					
					// Дополнительные расходы не обрабатываем, т.к. они списываются отдельно
					ВидЦенности = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						СтрокаСпособаУчетаНДС.АналитикаУчетаНДС, "ВидЦенности");
					
					Если ВидЦенности = Перечисления.ВидыЦенностей.ПрочиеРаботыИУслуги Тогда
						Продолжить;
					КонецЕсли;
					
					РеквизитыАналитикиЗатрат = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
						СтрокаСпособаУчетаНДС.АналитикаУчетаЗатрат, "СчетЗатрат, Субконто1, Субконто2");
					
					НоваяСтрокаСпособУчетаНДС = СпособыУчетаНДСПартий.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаСпособУчетаНДС, СтрокаСпособаУчетаНДС);
					
					Если ЗначениеЗаполнено(СтрокаСпособаУчетаНДС.СпособУчетаНДСПриВыбытии) Тогда
						НоваяСтрокаСпособУчетаНДС.СпособУчетаНДС = СтрокаСпособаУчетаНДС.СпособУчетаНДСПриВыбытии;
					КонецЕсли;
					
					НоваяСтрокаСпособУчетаНДС.СчетУчета = РеквизитыАналитикиЗатрат.СчетЗатрат;
					
					Если ТипЗнч(РеквизитыАналитикиЗатрат.Субконто1) = Тип("СправочникСсылка.Номенклатура") Тогда
						НоваяСтрокаСпособУчетаНДС.Номенклатура = РеквизитыАналитикиЗатрат.Субконто1;
					ИначеЕсли ТипЗнч(РеквизитыАналитикиЗатрат.Субконто2) = Тип("СправочникСсылка.Номенклатура") Тогда
						НоваяСтрокаСпособУчетаНДС.Номенклатура = РеквизитыАналитикиЗатрат.Субконто2;
					Иначе
						НоваяСтрокаСпособУчетаНДС.Номенклатура = Справочники.Номенклатура.ПустаяСсылка();
					КонецЕсли;
					
				КонецЦикла;
				
				СпособыУчетаНДСПартий.Свернуть("Партия, Номенклатура, СчетУчета, СпособУчетаНДС", "НДС");
				
			КонецЕсли;
			
		Иначе
			
			ТаблицаДляСписанияПоНоменклатуре = ТаблицаДляСписания.Скопировать(
				Новый Структура("ВидРасхода", Перечисления.ВидыРасходовУСН.Номенклатура));
			Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоНоменклатуре, "РасчетныйДокумент", Истина);
		
			СпособыУчетаНДСПартий = СпособыУчетаНДСПартий(
				СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата, Партии);
	...
	ИначеЕсли ВидОперации = "КорректировкаРеализации" Тогда
		...
		КонецЕсли;
	Иначе
		Для Каждого СтрокаПриход Из ТаблицыДвижений.ТаблицаПриход Цикл
			Если СтрокаПриход.Сумма <> СтрокаПриход.НДС Тогда
				СтрокаПриход.СтатусыПартийУСН = Перечисления.СтатусыПартийУСН.Списанные;
				СтрокаПриход.Партия = СтруктураШапкиДокумента.Ссылка;
			Иначе
				СтрокаПриход.НДС = 0;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПеремещениеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистров, Договор, СтатусПартии, ВидОперации = "Списание")
	...
	СписанныеПартииНДС = СтруктураШапкиДокумента.СписанныеПартииНДС;
	ВидыРасходовУСНБезВыделенияНДС = УчетУСН.ВидыРасходовУСНБезВыделенияНДС();
	ЭтоРазукомплектация =
		СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКомплектацияНоменклатуры.Разукомплектация;
	
	Для Каждого Расход Из ТаблицаДляСписания Цикл
		
		Если Расход.СчетУчета <> Расход.СчетПриход Или ВидОперации <> "Перемещение" Тогда
		...
		// Обработаем изменение способа учета НДС
		ОбработатьИзменениеСпособаУчетаНДС = ЗначениеЗаполнено(СписанныеПартииНДС)
			И Расход.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеСписаноПринято
			И ВидыРасходовУСНБезВыделенияНДС.Найти(Расход.ВидРасхода) = Неопределено
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.ПустаяСсылка()
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.НеПринимаются;

		Если ОбработатьИзменениеСпособаУчетаНДС Тогда
			
			ПараметрыИзмененияСпособаУчетаНДС = Новый Структура;
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("СтрокаРасход", Расход);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ТаблицаКорректировкиПриход", ТаблицаКорректировкиПриход);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ТаблицаКорректировкиРасход", ТаблицаКорректировкиРасход);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ТаблицаПризнанныхРасходов", ТаблицаПризнанныхРасходов);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("СписанныеПартииНДС", СписанныеПартииНДС);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ЭтоРазукомплектация", ЭтоРазукомплектация);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ЭтоПеремещение", Истина);
			
			ОбработатьИзменениеСпособаУчетаНДСПартий(ПараметрыИзмененияСпособаУчетаНДС);
	...
	Для Каждого СтрокаПлатеж Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если Строка.Сумма > СуммаСписания Тогда
				...
				НоваяСтрока.Количество	= НоваяСтрока.Сумма / Строка.Сумма * Строка.Количество;
				НоваяСтрока.НДС			= НоваяСтрока.Сумма / Строка.Сумма * Строка.НДС;
			Иначе
...
Процедура СписаниеТЗР(СтруктураШапкиДокумента, ДвиженияРегистров)
	...
	МассивРасчетныхДокументов = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаРасходов, "РасчетныйДокумент", Истина);
	ОбщегоНазначенияБПКлиентСервер.УдалитьНеЗаполненныеЭлементыМассива(МассивРасчетныхДокументов);

	// Подготовим таблицу способов учета НДС партий
	СпособыУчетаНДСОбщая = Неопределено;
	СпособыУчетаНДСПартий = НоваяТаблицаСпособовУчетаНДСПартий();
	Если СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
		СпособыУчетаНДСОбщая = СпособыУчетаНДСПартийДополнительныхРасходовСписание(
			СтруктураШапкиДокумента.Организация, НачалоПериодаОперации, КонецПериодаОперации, МассивПартий, МассивРасчетныхДокументов);
	КонецЕсли;
	...
	Для Каждого СтрокаПартии Из ТаблицаПартий Цикл
		...
		СпособыУчетаНДСПартий.Очистить();
		...
		Для Каждого СтрокаРасчетныйДокумент Из НайденныеСтрокиРД Цикл
			...
			Если ЗначениеЗаполнено(СпособыУчетаНДСОбщая) Тогда
				СтруктураОтбораСпособовУчетаНДС = Новый Структура;
				СтруктураОтбораСпособовУчетаНДС.Вставить("Номенклатура", СтрокаПартии.Номенклатура);
				СтруктураОтбораСпособовУчетаНДС.Вставить("Партия", СтрокаРасчетныйДокумент.РасчетныйДокумент);
				НайденныеСтрокиСпособовУчетаНДС = СпособыУчетаНДСОбщая.НайтиСтроки(СтруктураОтбораСпособовУчетаНДС);
				Для Каждого СтрокаСпособаУчетаНДС Из НайденныеСтрокиСпособовУчетаНДС Цикл
					ЗаполнитьЗначенияСвойств(СпособыУчетаНДСПартий.Добавить(), СтрокаСпособаУчетаНДС);
				КонецЦикла;
			КонецЕсли;
...
Процедура СписаниеТаможенныхПлатежей(СтруктураШапкиДокумента, ДвиженияРегистров)
	...
	// Из таблицы расходов выбираем соответствующие партии и элементы расходов
	ГраницаЗапроса = Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Включая);
	ТаблицаРасходов = ВыполнитьЗапросПоРасходам(ГраницаЗапроса, СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
	...
	ТаблицаДляСписания.Колонки.Добавить("ИсключатьНДС", Новый ОписаниеТипов("Булево"));

	// Структура - хранилище итоговых таблиц движений по всем партиям.
	// Для соответствия имен элементов итоговую структуру таблиц формируем так же, как и промежуточные,
	// но по пустой таблице списания расходов.
	ТаблицыДвижений = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаДляСписания, , "Списание", Ложь);
	...
	Для Каждого СтрокаПартии Из ТаблицаПартий Цикл
		...
		Для Каждого СтрокаРасчетныйДокумент Из НайденныеСтрокиРД Цикл
			...
			Для Каждого Строка Из НайденныеСтроки Цикл
				...
				НоваяСтрока = ТаблицаДляСписания.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
				НоваяСтрока.ИсключатьНДС = СтрокаПартии.ИсключатьНДС;
...
Процедура ПрочееДДС(Документ, ДвиженияРегистров, КУДиР) Экспорт
	...
	ЭтоПоступлениеНаРасчетныйСчет = ТипЗнч(Документ) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет");
	Если ЭтоПоступлениеНаРасчетныйСчет Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Организация, Дата, СубконтоКт1", Истина);
		ПризнаватьДоход = ПризнаватьДоходомПроцентыПоВкладам(
			РеквизитыДокумента.Организация, РеквизитыДокумента.Дата, РеквизитыДокумента.СубконтоКт1);
		Если Не ПризнаватьДоход И Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком() Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", МетаданныеДокумента)
		...
		Для каждого СтрокаТЧ Из Документ.РасшифровкаПлатежа Цикл
			...
			Если СтрокаТЧ.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Распределяются Тогда
			...
			ИначеЕсли СтрокаТЧ.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются
				Или Не ЗначениеЗаполнено(СтрокаТЧ.ОтражениеВУСН) Тогда
				
				// Отражение может быть не заполнено для некоторых операций, если расходы признаются частично.
				
				КУДиР.Графа7 = КУДиР.Графа7 + СтрокаТЧ.РасходыУСН;
				КУДиР.НДС    = КУДиР.НДС    + ?(ЕстьНДС, СтрокаТЧ.НДСУСН, 0);
				
			Иначе
				// Принимаемых расходов нет - ничего не отражаем.
			КонецЕсли;
			
		КонецЦикла;
		
	Иначе
		// Расходы указаны в шапке.
		
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ОтражениеВУСН", МетаданныеДокумента) Тогда
			ОтражениеВУСН = Документ.ОтражениеВУСН;
		Иначе
			ОтражениеВУСН = Неопределено;
		КонецЕсли;
		
		Если ОтражениеВУСН = Перечисления.ОтражениеВУСН.Распределяются Тогда
		...
		ИначеЕсли ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются
			Или Не ЗначениеЗаполнено(ОтражениеВУСН) Тогда
			
			// Отражение может быть не заполнено для операции "Комиссия банка", если расходы признаются частично.
			
			КУДиР.Графа7 = Документ.Графа7_УСН;
			КУДиР.НДС    = Документ.НДС_УСН;
			
		Иначе // Расходы не принимаются
...
Процедура ОтразитьУдержанияИзЗарплаты(СтруктураШапкиДокумента, ТаблицаРасходов, ОстаткиВзаиморасчетов, Параметры)
	...
	РаспределитьУдержания = УдержанияДокумента.Скопировать(, "КорСубконто1, Субконто1, Субконто3, Сумма");
	РаспределитьУдержания.Колонки.КорСубконто1.Имя = "Сотрудник";
	РаспределитьУдержания.Колонки.Субконто1.Имя    = "Контрагент";
	РаспределитьУдержания.Колонки.Субконто3.Имя    = "ИсполнительныйЛист";
	
	РаспределитьУдержания.Свернуть("Сотрудник, Контрагент, ИсполнительныйЛист", "Сумма");
	...
	РасходыУдержания = ТаблицаРасходов.СкопироватьКолонки();
	СчетРасчетовПоИсполнительнымЛистам = ПланыСчетов.Хозрасчетный.РасчетыПоИсполнительнымДокументамРаботников;
	...
	Для каждого СтрокаУдержания Из РаспределитьУдержания Цикл
		...
		Если РаспределениеУдержаний.Количество() > 0 Тогда
			...
			Для каждого СтрокаРаспределения Из РаспределениеУдержаний Цикл
				...
				Если ЗначениеЗаполнено(СтрокаУдержания.ИсполнительныйЛист) Тогда
					НоваяСтрокаУдержаний = РасходыУдержания.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаУдержаний, СтрокаРаспределения.СтрокаТаблицыРасходов);
					НоваяСтрокаУдержаний.Сумма = ?(НоваяСуммаНачисления < 0, СтрокаРаспределения.Начислено, СтрокаРаспределения.Удержать);
					НоваяСтрокаУдержаний.РасчетныйДокумент = СтрокаУдержания.ИсполнительныйЛист;
					НоваяСтрокаУдержаний.Контрагент        = СтрокаУдержания.Контрагент;
					НоваяСтрокаУдержаний.СчетУчета         = СчетРасчетовПоИсполнительнымЛистам;
					НоваяСтрокаУдержаний.СчетУчетаРасчетов = СчетРасчетовПоИсполнительнымЛистам;
				КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РасходыУдержания, ТаблицаРасходов);
...
Процедура НачислениеЗП(СтруктураШапкиДокумента, ДвиженияРегистров, Параметры)
	...
	МассивСчетовНДФЛ = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСчетовНДФЛ,
	...
	Запрос.УстановитьПараметр("ПриниматьУдержанныйНДФЛ",
		ПриниматьУдержанныйНДФЛкРасходам(СтруктураШапкиДокумента.Дата));
	Запрос.УстановитьПараметр("СчетаНДФЛ", МассивСчетовНДФЛ);
	...
	|	И ВЫБОР
	|		КОГДА &ПриниматьУдержанныйНДФЛ
	|			ТОГДА НЕ Хозрасчетный.Ссылка В (&СчетаНДФЛ)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	И ВЫБОР
	|		КОГДА &ПриниматьУдержанныйНДФЛ
	|			ТОГДА НЕ ОтражениеЗарплатыВУчетеУСН.КорСчет В (&СчетаНДФЛ)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	ТаблицаРасходов.Сортировать("ВидРасхода, ЭлементРасхода, СчетУчета, РасчетныйДокумент, ОтражениеВУСН Убыв");
	ТаблицаРасходов.Колонки.Добавить("Валюта");
	ТаблицаРасходов.Колонки.Добавить("СтатусыОплатыРасходовУСН");
	ТаблицаРасходов.Колонки.Добавить("НДС");
	ТаблицаРасходов.Колонки.Добавить("ДокументОплаты");
	ТаблицаРасходов.Колонки.Добавить("Контрагент");
	...
	//Расчеты с взыскателями
	МассивКонтрагентов        = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаРасходов, "Контрагент", Истина);
	МассивРасчетныхДокументов = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаРасходов, "РасчетныйДокумент", Истина);
	...
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля			= "Контрагент";
	Фильтр.ЗначениеПоля		= МассивКонтрагентов;
	Фильтр.Условие			= "В";
	
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля			= "РасчетныйДокумент";
	Фильтр.ЗначениеПоля		= МассивРасчетныхДокументов;
	Фильтр.Условие			= "В";
	
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля			= "СчетУчета";
	Фильтр.ЗначениеПоля		= ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СчетРасчетовПоИсполнительнымЛистам);
	Фильтр.Условие			= "В";
	
	ТаблицаРасчетовСВзыскателями = ВыполнитьЗапросПоВзаиморасчетамПрочееУСН(Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Исключая), СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
	ТаблицаРасчетовСВзыскателями.Индексы.Добавить("Контрагент");
	ТаблицаРасчетовСВзыскателями.Индексы.Добавить("РасчетныйДокумент");
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		СтруктураОтбора = Новый Структура;
		Если Расход.СчетУчета = СчетРасчетовПоИсполнительнымЛистам Тогда
			СтруктураОтбора.Вставить("РасчетныйДокумент", Расход.РасчетныйДокумент);
			СтруктураОтбора.Вставить("Контрагент",        Расход.Контрагент);
			НайденныеСтроки = ТаблицаРасчетовСВзыскателями.НайтиСтроки(СтруктураОтбора);
		ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
			СтруктураОтбора.Вставить("Контрагент",  Расход.ЭлементРасхода);
			СтруктураОтбора.Вставить("СчетУчета",  Расход.СчетУчетаРасчетов);
		...
		Если СуммаОсталосьПогасить > 0 Тогда
			...
			Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
				Если ЗначениеЗаполнено(Расход.СчетУчетаРасчетов) Тогда
					Если ЗначениеЗаполнено(Расход.Контрагент) Тогда
						ЭлементРасхода = Расход.Контрагент;
					Иначе
						ЭлементРасхода = Расход.ЭлементРасхода;
					КонецЕсли;
					ДвижениеВзиморасчетовПрочееУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, ЭлементРасхода, РасчетныйДокумент, Расход.СчетУчетаРасчетов, СуммаОсталосьПогасить);
				Иначе
				...
				Если Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
					Если ЗначениеЗаполнено(Расход.СчетУчетаРасчетов) Тогда
						...
						Иначе
							ЭлементРасхода = Расход.ЭлементРасхода;
						КонецЕсли;
						ДвижениеВзиморасчетовПрочееУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, ЭлементРасхода, Расход.РасчетныйДокумент, Расход.СчетУчетаРасчетов, СуммаОсталосьПогасить);
					Иначе
...
Процедура НачислениеСтраховыхВзносовИП(СтруктураШапкиДокумента, ДвиженияРегистров, Параметры)
	...
	ТаблицаПоступленияРасходов.Колонки.Добавить("ВзносыПодлежащиеУплате");
	ТаблицаПоступленияРасходов.Колонки.Добавить("ВидВзносаПодлежащегоУплате");
...
Процедура СписаниеРБПУСН(СтруктураШапкиДокумента, ДвиженияРегистров)
	...
	Для Каждого Строка Из ТаблицаРасходов Цикл
		...
		ЭлементРасходаРБП = Строка.ЭлементРасхода;

		Если Не ЗначениеЗаполнено(ЭлементРасходаРБП) Тогда
			Продолжить;
		КонецЕсли;
		
		ОписаниеРБП = РасходыБудущихПериодов.ОписаниеРасходаБудущихПериодов(ЭлементРасходаРБП);
		
		Если Не ЗначениеЗаполнено(ОписаниеРБП.ДатаНачалаСписания) Или Не ЗначениеЗаполнено(ОписаниеРБП.ДатаОкончанияСписания) Тогда
			Продолжить;
		КонецЕсли;

		ВспомогательныеДанныеРасчета = Новый Структура("КоличествоМесяцевДней, КоличествоМесяцевДнейТекущегоПериода", 0, 0);
		ДатаНачала = ?(ОписаниеРБП.ДатаНачалаСписания > НачМесяца, ОписаниеРБП.ДатаНачалаСписания, НачМесяца);
		СуммаСписания = БухгалтерскийУчет.СуммаСписанияЗаМесяц(ОписаниеРБП, ДатаНачала, ОписаниеРБП.ДатаОкончанияСписания, Строка.Сумма, ВспомогательныеДанныеРасчета, НачМесяца, КонМесяца);
	...
	Если ТаблицаДляСписания.Количество() > 0
		И СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
		СпособыУчетаНДСПартий = СпособыУчетаНДСПартийПоДаннымТаблицыСписания(СтруктураШапкиДокумента, ТаблицаДляСписания);
	КонецЕсли;
	СтруктураШапкиДокумента.Вставить("СпособыУчетаНДСПартий", СпособыУчетаНДСПартий);
...
//
Процедура РаспределениеРасходов(СтруктураШапкиДокумента, ДвиженияРегистров)
	...
	//Разложим таблицу списываемых расходов на движения корректировки статуса и принятые расходы
	ТаблицыДвижений = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, , "РаспределениеРасходов", Ложь);
	
	Коэфф = КоэффициентРаспределенияРасходов(СтруктураШапкиДокумента);
	...
	Если НачалоГода(СтруктураШапкиДокумента.Дата) <> НачалоКвартала(СтруктураШапкиДокумента.Дата)
		И СтруктураШапкиДокумента.Дата < УчетЕНВДКлиентСервер.ДатаОтменыЕНВД()
		И СтруктураШапкиДокумента.МетодРаспределения <> Перечисления.МетодыРаспределенияРасходовУСНПоВидамДеятельности.ЗаКвартал Тогда
...
Процедура РасчетыСКонтрагентами_ЗачетАвансаОтПокупателяУСН(СтруктураШапкиДокумента, ТаблицаАвансов, ТаблицаДвижений, ДвиженияРегистровВх, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, Выручка)
	...
	Для Каждого СтрокаЗачета Из ТаблицаЗачетАвансаУСН Цикл
		...
		Если СтрокаЗачета.ИсключатьНДСИзДоходов Тогда
			СтрокаЗачета.ИсключитьНДС_УСН = Окр(СтрокаЗачета.ВыручкаНДС * ДоляСобственнойВыручкиУСН * КурсДокумента, 2);
		ИначеЕсли СтрокаЗачета.ПроверятьНДССАванса Тогда
			// НДС в доходах УСН
			СуммаКорректировкиНДС_УСН = Окр((СтрокаЗачета.АвансНДС - СтрокаЗачета.ВыручкаНДС) * КурсДокумента, 2);
			СтрокаЗачета.ИсключитьНДС_УСН = Макс(-СуммаКорректировкиНДС_УСН, 0);
			СтрокаЗачета.ВключитьНДС_УСН = Макс(СуммаКорректировкиНДС_УСН, 0);
		КонецЕсли;
	...
	Для Каждого СтрокаЗачета Из ТаблицаЗачетАвансаУСНПоДоговорам Цикл
		...
		ВключитьНДС_УСН  = СтрокаЗачета.ВключитьНДС_УСН;
		...
		СтрокаРасшифровки.Графа5            = КорректировкаУСН - ИсключитьНДС_УСН + ВключитьНДС_УСН;
		...
		Если ВключитьНДС_УСН > 0 Тогда
			ПояснениеОперации = ПояснениеОперации
				+ СтрШаблон(НСтр("ru = 'Исключенный с аванса НДС на сумму %1 руб. включен в доходы. '"),
					ОбщегоНазначенияБПВызовСервера.ФорматСумм(ВключитьНДС_УСН, , , " "));
		КонецЕсли;
...
Процедура РасчетыСКонтрагентамиУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, ВидОперации = "Оплата", ТаблицаНДСПродажи = Неопределено)
	...
	Если ВидОперации = "Доход" Тогда
		...
		Если СтруктураШапкиДокумента.Ссылка.Метаданные().Реквизиты.Найти("СуммаУслуг") <> Неопределено И
			...
			Если УчетВзаиморасчетов.ЭтоСчетРасчетовСПоставщиками(
				СтруктураШапкиДокумента.Ссылка.СчетУчетаРасчетовСКонтрагентом)
				И УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(
					СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) Тогда
				СуммаУслуг = 0;
			КонецЕсли;
	...
	ИмяРеквизитаПатент = "Патент";
	ЭтоПлательщикНДСПрименяющийУСН = СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН;
	
	ИмяРеквизитаРасшифровкаПлатежа = "РасшифровкаПлатежа";
	Документ = СтруктураШапкиДокумента.Ссылка;
	РасшифровкаПлатежа = Неопределено;
	ЕстьКолонкаПорядокОтраженияАванса = Ложь;
	Если Документ.Метаданные().ТабличныеЧасти.Найти(ИмяРеквизитаРасшифровкаПлатежа) <> Неопределено Тогда
		РасшифровкаПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, ИмяРеквизитаРасшифровкаПлатежа, Истина);
		Если РасшифровкаПлатежа <> Неопределено Тогда
			РасшифровкаПлатежа = РасшифровкаПлатежа.Выгрузить();
			КолонкиГруппировок =
				"ДоговорКонтрагента, КурсВзаиморасчетов, КратностьВзаиморасчетов, СпособПогашенияЗадолженности, Сделка";
			Если РасшифровкаПлатежа.Колонки.Найти("ПорядокОтраженияАванса") <> Неопределено Тогда
				КолонкиГруппировок = КолонкиГруппировок + ", ПорядокОтраженияАванса";
				ЕстьКолонкаПорядокОтраженияАванса = Истина;
			КонецЕсли;
			РасшифровкаПлатежа.Свернуть(КолонкиГруппировок, "СуммаПлатежа, СуммаНДС");
		КонецЕсли;
	КонецЕсли;
	
	РасчетныеДокументы = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДвиженияВзаиморасчетов, "РасчетныйДокумент", Истина);
	
	ТаблицаНДСВозвраты = Неопределено;
	Если ВидОперации = "ВозвратПокупателю" Тогда
		ТаблицаНДСВозвраты = ТаблицаНДСПоВозвратам(СтруктураШапкиДокумента, РасчетныеДокументы);
		Если ЗначениеЗаполнено(ТаблицаНДСВозвраты) Тогда
			ТаблицаНДСВозвраты.Индексы.Добавить("ДокументРасчетов");
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаПлатеж Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		ИначеЕсли ВидОперации = "ВозвратПокупателю" Тогда
			...
			ИсключаемаяСуммаНДС = 0;
			Если ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				СтрокаНДСВозвратПоДокументу = Неопределено;
				Если ЗначениеЗаполнено(ТаблицаНДСВозвраты) Тогда
					СтрокаНДСВозвратПоДокументу = ТаблицаНДСВозвраты.Найти(СтрокаПлатеж.РасчетныйДокумент, "ДокументРасчетов");
				КонецЕсли;
				
				Если ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
					ИсключаемаяСуммаНДС = ИсключаемаяСуммаНДС(
						СтруктураШапкиДокумента, СтрокаПлатеж.РасчетныйДокумент, СторноПринимаемыхДоходов);
				ИначеЕсли ЗначениеЗаполнено(СтрокаНДСВозвратПоДокументу) Тогда
					КоэффициентРасчетовПоНДСВозврат =
						?(СтрокаНДСВозвратПоДокументу.Сумма = 0, 0, СторноПринимаемыхДоходов / СтрокаНДСВозвратПоДокументу.Сумма);
					КоэффициентРасчетовПоНДСВозврат = Мин(1, КоэффициентРасчетовПоНДСВозврат);
					ИсключаемаяСуммаНДС = Окр(КоэффициентРасчетовПоНДСВозврат * СтрокаНДСВозвратПоДокументу.НДС, 2);
				Иначе
					ИсключаемаяСуммаНДС = ИсключаемаяСуммаНДСВозвраты(
						СтруктураШапкиДокумента, СтрокаПлатеж, СторноПринимаемыхДоходов);
				КонецЕсли;
			КонецЕсли;
			
			СторноПринимаемыхДоходов = СторноПринимаемыхДоходов - ИсключаемаяСуммаНДС;
			Если СторноДоходовТорговыйСбор > 0 Тогда
				СторноДоходовТорговыйСбор = СторноДоходовТорговыйСбор - ИсключаемаяСуммаНДС;
			КонецЕсли;
			...
			Если СторноДоходов - СторноПринимаемыхДоходов > 0 Тогда
				...
				КонецЕсли;
				Если ИсключаемаяСуммаНДС > 0 Тогда
					СодержаниеОперации = СодержаниеОперации + "НДС - " + ОбщегоНазначенияБПВызовСервера.ФорматСумм(ИсключаемаяСуммаНДС, , , " ")+ " руб.; ";
				КонецЕсли;
			...
			Если ДоходПатент > 0 И ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				// Подменим ссылку если документ - корректировка реализации
				ДокументСсылка = СтрокаПлатеж.РасчетныйДокумент;
				Если ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					ДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						СтрокаПлатеж.РасчетныйДокумент, "ДокументРеализации");
					ДокументРеализацииСсылка = УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументРеализации(
						ДокументРеализации, Истина);
					Если ЗначениеЗаполнено(ДокументРеализацииСсылка) Тогда
						ДокументСсылка = ДокументРеализацииСсылка;
					КонецЕсли;
				КонецЕсли;
				МетаданныеДокумента = ДокументСсылка.Метаданные();
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта(ИмяРеквизитаПатент, МетаданныеДокумента) И ЗначениеЗаполнено(ДокументСсылка.Патент) Тогда
		...
		ИначеЕсли ВидОперации = "Доход" Тогда
			...
			ВыручкаВсегоПоРеализацииПСН = 0;
			ВалютаРеализации            = ВалютаРеглУчета;
			ВалютаНДС                   = ВалютаРеглУчета;
			КурсРеализации              = 1;
			КратностьРеализации         = 1;
			ПатентДохода                = Неопределено; 
			ИсключитьНДСПатент          = Неопределено;
			...
			Если ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				...
				ДоходУчтенДоПереходаНаУСН = ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(СтруктураШапкиДокумента.Организация)
					И (ДатаПереходаНаУСН > СтрокаПлатеж.РасчетныйДокумент.Дата);
				
				// Не включаем в доходы суммы, которые были учтены в отчете комиссионера
				ДоходУчтенОтчетомКомиссионера = Ложь;
				Если ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
					ВидОперацииОтчетаКомиссионера =
						ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПлатеж.РасчетныйДокумент, "ВидОперации");
					Если ВидОперацииОтчетаКомиссионера <> Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
						ДоходУчтенОтчетомКомиссионера =
							ОтражатьДоходыНаДатуОтчетаКомиссионера(СтруктураШапкиДокумента.Организация, СтрокаПлатеж.ДатаДокументаРасчетов);
					КонецЕсли;
				КонецЕсли;
				
				СтрокаНДСПоДокументу = Неопределено;
				...
				Если ЗначениеЗаполнено(СтрокаНДСПоДокументу) Тогда
					СуммаНДСПоРеализации     = СтрокаНДСПоДокументу.НДС;                     // В валюте регламентированного учета
					ВыручкаВсегоПоРеализации = СтрокаНДСПоДокументу.ВалютнаяСуммаРеализации; // В валюте реализации
				...
				ИначеЕсли ЭтоПлательщикНДСПрименяющийУСН Тогда
					Если СтруктураШапкиДокумента.ВидОперацииУСН = "ОплатаОтЭквайера"
						И ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
						ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаПлатеж.РасчетныйДокумент,
							"РасшифровкаПлатежа, СуммаДокумента, ВалютаДокумента");
						РасшифровкаПлатежа = ЗначенияРеквизитов.РасшифровкаПлатежа.Выгрузить();
						СуммаНДСПоРеализации = РасшифровкаПлатежа.Итог("СуммаНДС");
						ВыручкаВсегоПоРеализации = ЗначенияРеквизитов.СуммаДокумента;
						ВалютаРеализации = ЗначенияРеквизитов.ВалютаДокумента;
						ВалютаНДС = ЗначенияРеквизитов.ВалютаДокумента;
						КурсРеализации = СтрокаПлатеж.Курс;
						КратностьРеализации = СтрокаПлатеж.Кратность;
					ИначеЕсли СтруктураШапкиДокумента.ВидОперацииУСН = "ОплатаОтЭквайера"
						И ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
						ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураШапкиДокумента.Ссылка,
							"РасшифровкаПлатежа, ВалютаДокумента");
						РасшифровкаПлатежа = ЗначенияРеквизитов.РасшифровкаПлатежа.Выгрузить();
						ВыручкаВсегоПоРеализации = РасшифровкаПлатежа.Итог("СуммаПлатежа");
						ВалютаРеализации = ЗначенияРеквизитов.ВалютаДокумента;
						ВалютаНДС = ЗначенияРеквизитов.ВалютаДокумента;
						СуммаНДСПоРеализации = ИсключаемаяСуммаНДС(СтруктураШапкиДокумента,
							СтрокаПлатеж.РасчетныйДокумент,
							ВыручкаВсегоПоРеализации);
						КурсРеализации = СтрокаПлатеж.Курс;
						КратностьРеализации = СтрокаПлатеж.Кратность;
					ИначеЕсли СтруктураШапкиДокумента.ВидОперацииУСН = "ЗачетВыручкиКомиссионера"
						И ТипЗнч(СтруктураШапкиДокумента.Ссылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
						И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
						РеквизитыОтчетаКомиссионера = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
							СтруктураШапкиДокумента.Ссылка, "Товары, ТоварыВозвращенные, СуммаДокумента");
						
						ТоварыРеализованные = РеквизитыОтчетаКомиссионера.Товары.Выгрузить();
						ТоварыВозвращенные = РеквизитыОтчетаКомиссионера.ТоварыВозвращенные.Выгрузить();
						
						СуммаНДСПоРеализации = ТоварыРеализованные.Итог("СуммаНДС") - ТоварыВозвращенные.Итог("СуммаНДС");
						ВыручкаВсегоПоРеализации = РеквизитыОтчетаКомиссионера.СуммаДокумента;
						ВалютаРеализации = СтруктураШапкиДокумента.ВалютаДокумента;
						ВалютаНДС = СтруктураШапкиДокумента.ВалютаДокумента;
						КурсРеализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураШапкиДокумента, "КурсДокумента", 1);
						КратностьРеализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураШапкиДокумента, "КратностьДокумента", 1);
					ИначеЕсли СтруктураШапкиДокумента.ВидОперацииУСН = "ОплатаОтПокупателя"
						И (СтрокаПлатеж.РасчетныйДокумент = СтруктураШапкиДокумента.Ссылка
							Или ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.КорректировкаДолга"))
						И РасшифровкаПлатежа <> Неопределено
						И РасшифровкаПлатежа.Количество() > 0 Тогда
						
						ДоговорКонтрагента = СтрокаПлатеж.ДоговорКонтрагента;
						ПараметрыОтбора = Новый Структура("ДоговорКонтрагента", ДоговорКонтрагента);
						РасшифровкиПлатежаСОтбором = РасшифровкаПлатежа.Скопировать(ПараметрыОтбора);
						Для Каждого СтрокаРасшифровки Из РасшифровкиПлатежаСОтбором Цикл
							СпособПогашенияПоДокументу = СтрокаРасшифровки.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.ПоДокументу
								И ЗначениеЗаполнено(СтрокаРасшифровки.Сделка)
								И РасчетныеДокументы.Найти(СтрокаРасшифровки.Сделка) <> Неопределено
								И ТипЗнч(СтрокаРасшифровки.Сделка) <> Тип("ДокументСсылка.КорректировкаДолга");
							НеАвансУСН = ЕстьКолонкаПорядокОтраженияАванса
								И (СтрокаРасшифровки.ПорядокОтраженияАванса = Перечисления.ПорядокОтраженияАвансов.ДоходКомитента
								Или СтрокаРасшифровки.ПорядокОтраженияАванса = Перечисления.ПорядокОтраженияАвансов.ДоходЕНВД);
							Если СпособПогашенияПоДокументу Или НеАвансУСН Тогда
								Продолжить;
							КонецЕсли;
							Если ТипЗнч(СтрокаРасшифровки.ПорядокОтраженияАванса) = Тип("СправочникСсылка.Патенты") Тогда
								ИсключитьНДСПатент = СтрокаРасшифровки.СуммаНДС;
								ВыручкаВсегоПоРеализацииПСН = ВыручкаВсегоПоРеализацииПСН + СтрокаРасшифровки.СуммаПлатежа;
							Иначе
								СуммаНДСПоРеализации = СуммаНДСПоРеализации + СтрокаРасшифровки.СуммаНДС;
								ВыручкаВсегоПоРеализации = ВыручкаВсегоПоРеализации + СтрокаРасшифровки.СуммаПлатежа;
							КонецЕсли;
						КонецЦикла;
						ВалютаРеализации = Документ.ВалютаДокумента;
						ВалютаНДС = Документ.ВалютаДокумента;
						КурсРеализации = ?(ДоговорКонтрагента.РасчетыВУсловныхЕдиницах, 1,
							РасшифровкиПлатежаСОтбором[0].КурсВзаиморасчетов);
						КратностьРеализации = РасшифровкиПлатежаСОтбором[0].КратностьВзаиморасчетов;
						
					ИначеЕсли СтруктураШапкиДокумента.ВидОперацииУСН = "ПроведениеВзаимозачетаДебитор"
						И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ЗачетЗадолженности
						И СтрокаПлатеж.РасчетныйДокумент <> СтруктураШапкиДокумента.Ссылка
						И ЗначениеЗаполнено(ТаблицаНДСПродажи) Тогда
						СтрокаНДСПоДокументу    = ТаблицаНДСПродажи.Найти(СтрокаПлатеж.РасчетныйДокумент, "ДокументРасчетов");
						Если СтрокаНДСПоДокументу <> Неопределено Тогда
							СуммаНДСПоРеализации     = СтрокаНДСПоДокументу.НДС;
							ВыручкаВсегоПоРеализации = СтрокаНДСПоДокументу.ВалютнаяСуммаРеализации;
							ВалютаРеализации         = СтрокаНДСПоДокументу.ВалютаРеализации;
							КурсРеализации           = СтрокаНДСПоДокументу.КурсРеализации;
							КратностьРеализации      = СтрокаНДСПоДокументу.КратностьРеализации;
						КонецЕсли;
			...
			Если Не ДоходУчтенОтчетомКомиссионера Тогда
				СуммаГрафа4 =
					Окр((СтрокаПлатеж.СуммаВзаиморасчетов - СтрокаПлатеж.СуммаУслуг) * Курс / Кратность, 2, РежимОкругления.Окр15как20);
			...
			ЭтоВидДоговораСКомитентомНаЗакупку = ТаблицаОплат.Колонки.Найти("ДоговорКонтрагента") <> Неопределено 
				И ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента)
				И БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомитентомНаЗакупку(СтрокаПлатеж.ДоговорКонтрагента.ВидДоговора);
			
			Если ЭтоВидДоговораСКомитентомНаЗакупку
				И ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент)
				И (ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) <> Тип("ДокументСсылка.ОтчетКомитентуОПродажах")) Тогда
			...
			МетаданныеДокументаОплаты = СтруктураШапкиДокумента.Ссылка.Метаданные();
			
			Если ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент) Тогда
				
				РасчетныйДокумент = СтрокаПлатеж.РасчетныйДокумент;
				
				ПатентДокументаОплаты = Неопределено;
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта(ИмяРеквизитаПатент, МетаданныеДокументаОплаты) Тогда
					ПатентДокументаОплаты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураШапкиДокумента.Ссылка, ИмяРеквизитаПатент);
				КонецЕсли;
					
				// Переквалифицируем доход с оплаты по сделке на патенте с истекшим сроком действия
				Если ДоходПатент > 0 Тогда
					ДокументСсылка      = РасчетныйДокумент;
					МетаданныеДокумента = ДокументСсылка.Метаданные();
					// Подменим ссылку если документ - реализация отгруженной продукции
					Если (МетаданныеДокумента.ТабличныеЧасти.Количество() = 0)
						И (Не МетаданныеДокумента.Реквизиты.Найти("ДокументОтгрузки") = Неопределено) Тогда
						ДокументОтгрузки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ДокументОтгрузки");
						Если ЗначениеЗаполнено(ДокументОтгрузки) Тогда
							ДокументСсылка      = ДокументОтгрузки;
							МетаданныеДокумента = ДокументСсылка.Метаданные();
					...
					ПатентРасчетногоДокумента = Неопределено;
					Если ОбщегоНазначения.ЕстьРеквизитОбъекта(ИмяРеквизитаПатент, МетаданныеДокумента) Тогда
						ПатентРасчетногоДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, ИмяРеквизитаПатент);
					КонецЕсли;
					
					Если ЗначениеЗаполнено(ПатентРасчетногоДокумента) Тогда
						Если РасчетныйДокумент <> СтруктураШапкиДокумента.Ссылка
							И Не УчетПСН.ДеятельностьПоПатентуДействуетНаДату(ПатентРасчетногоДокумента, СтруктураШапкиДокумента.Дата) Тогда
							Если ЗначениеЗаполнено(ПатентДокументаОплаты) Тогда
								ПатентДохода = ПатентДокументаОплаты;
							Иначе
								ДоходПатент = 0;
							КонецЕсли;
						Иначе
							ПатентДохода = ПатентРасчетногоДокумента;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				
				РасчетныйДокументДата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РасчетныйДокумент, "Дата");
				Если ЭтоОплатаЗаПериодБезПатентаПриПримененииТолькоПатента(
						СтруктураШапкиДокумента.Организация, РасчетныйДокументДата, СтруктураШапкиДокумента.Дата) Тогда
					ДоходПатент = Макс(СуммаВзаиморасчетов - ДоходКомитента - ДоходЕНВД, 0);
					ДоходЕНВД         = 0;
					ДоходТорговыйСбор = 0;
					
					Если Не ЗначениеЗаполнено(ПатентДохода) И ЗначениеЗаполнено(ПатентДокументаОплаты) Тогда
						ПатентДохода = ПатентДокументаОплаты;
					КонецЕсли;
				КонецЕсли;
			...
			Если Не ДоходУчтенДоПереходаНаУСН И Не ДоходУчтенОтчетомКомиссионера Тогда
				
				Если ЭтоПлательщикНДСПрименяющийУСН Тогда
					ДатаДокументаРасчетов = Документ.Дата;
				Иначе
					ДатаДокументаРасчетов = СтрокаПлатеж.ДатаДокументаРасчетов;
				КонецЕсли;
				...
				Если СтрокаПлатеж.ВалютаВзаиморасчетов <> ВалютаРеализации Тогда
					ПараметрыТекущегоКурса =
						Новый Структура("Валюта, Курс, Кратность", ВалютаРеализации, КурсРеализации, КратностьРеализации);
					ПараметрыНовогоКурса =
						РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаПлатеж.ВалютаВзаиморасчетов, ДатаДокументаРасчетов);
					ВыручкаВсегоПоРеализации = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
						ВыручкаВсегоПоРеализации, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
					ВыручкаВсегоПоРеализацииПСН = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
						ВыручкаВсегоПоРеализацииПСН, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
				КонецЕсли;
				
				// В таблице ТаблицаНДСПродажи сумма НДС всегда в валюте регламентированного учета, пересчет не требуется.
				// Сумму НДС, полученную из расшифровки платежа, необходимо привести к валюте регламентированного учета.
				Если ВалютаНДС <> ВалютаРеглУчета И Не СтрокаПлатеж.РасчетыВУсловныхЕдиницах Тогда
					
					ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность");
					ПараметрыТекущегоКурса.Валюта = ВалютаНДС;
					ПараметрыТекущегоКурса.Курс = КурсРеализации;
					ПараметрыТекущегоКурса.Кратность = КратностьРеализации;
					
					ПараметрыНовогоКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРеглУчета, ДатаДокументаРасчетов);
					
					СуммаНДСПоРеализации = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СуммаНДСПоРеализации,
						ПараметрыТекущегоКурса,
						ПараметрыНовогоКурса);
						
					Если ИсключитьНДСПатент <> Неопределено Тогда
						ИсключитьНДСПатент = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(ИсключитьНДСПатент,
							ПараметрыТекущегоКурса,
							ПараметрыНовогоКурса);
					КонецЕсли;
				КонецЕсли;
				...
				КоэффициентРасчетовПоНДС = ?(ВыручкаВсегоПоРеализации = 0, 0, СобственнаяВыручка / ВыручкаВсегоПоРеализации);
				КоэффициентРасчетовПоНДС_ПСН = ?(ВыручкаВсегоПоРеализацииПСН = 0, 0, СобственнаяВыручка / ВыручкаВсегоПоРеализацииПСН);
				Если ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
					РеквизитыМежценовойРазницы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаПлатеж.РасчетныйДокумент,
						"НДССМежценовойРазницы, СуммаДокумента");
					Если РеквизитыМежценовойРазницы.НДССМежценовойРазницы Тогда
						КоэффициентРасчетовПоНДС = ?(РеквизитыМежценовойРазницы.СуммаДокумента = 0,
							0, СобственнаяВыручка / РеквизитыМежценовойРазницы.СуммаДокумента);
					КонецЕсли;
				КонецЕсли;
				КоэффициентРасчетовПоНДС = Мин(1, КоэффициентРасчетовПоНДС);
				КоэффициентРасчетовПоНДС_ПСН = Мин(1, КоэффициентРасчетовПоНДС_ПСН);
				
				// Доходы принимаемые
				ДоходУСН            = СуммаВзаиморасчетов - ДоходЕНВД - ДоходКомитента - ДоходПатент;
				// Для отчета о розничных продажах сумму НДС рассчитаем отдельно,
				// т.к. НДС с полученной предоплаты и с продажи подарочного сертификата не учитывается по подсистеме НДС
				ЭтоРозничнаяПродажа = ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент)
					И ТипЗнч(СтрокаПлатеж.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах");
				Если ЭтоРозничнаяПродажа Тогда
					ИсключитьНДС = ИсключаемаяСуммаНДС(
						СтруктураШапкиДокумента, СтрокаПлатеж.РасчетныйДокумент, ДоходУСН);
				Иначе
					ИсключитьНДС = ?(СобственнаяВыручка = 0, 0,
				...
				Если ДоходТорговыйСбор > 0 Тогда
					Если ЭтоРозничнаяПродажа Тогда
						ИсключитьНДСТорговыйСбор = ИсключаемаяСуммаНДС(
							СтруктураШапкиДокумента, СтрокаПлатеж.РасчетныйДокумент, ДоходТорговыйСбор);
					Иначе
						ИсключитьНДСТорговыйСбор = ?(СобственнаяВыручка = 0,
							0, Окр(КоэффициентРасчетовПоНДС * СуммаНДСПоРеализации * ДоходТорговыйСбор / СобственнаяВыручка, 2));
					КонецЕсли;
				...
				Если ДоходЕНВД > 0 Тогда
					ИсключитьНДСЕНВД = ?(СобственнаяВыручка = 0, 0,
				...
				Если ДоходПатент > 0 Тогда
					Если ИсключитьНДСПатент = Неопределено Тогда
						ИсключитьНДСПатент = ?(СобственнаяВыручка = 0, 0,
							Окр(КоэффициентРасчетовПоНДС * СуммаНДСПоРеализации * ДоходПатент / СобственнаяВыручка, 2));
					Иначе
						ИсключитьНДСПатент = ?(СобственнаяВыручка = 0, 0,
							Окр(КоэффициентРасчетовПоНДС_ПСН * ИсключитьНДСПатент * ДоходПатент / СобственнаяВыручка, 2));
					КонецЕсли;
					...
					Если ЗначениеЗаполнено(ПатентДохода) Тогда
						ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
						ЗаписьКУДиРПатент.Патент = ПатентДохода;
						ЗаписьКУДиРПатент.Содержание = ОписаниеСобытияДляКУДиР(
							СтруктураШапкиДокумента,
							СтруктураШапкиДокумента.ВидОперацииУСН,
							МетаданныеДокументаОплаты,
	...
	Если НЕ ЗначениеЗаполнено(СтруктураШапкиДокумента.ДатаОплаты) Тогда
		МоментДокумента = СтруктураШапкиДокумента.Ссылка.МоментВремени();
	Иначе
		МоментДокумента = Новый МоментВремени(
			КонецДня(СтруктураШапкиДокумента.ДатаОплаты),
			СтруктураШапкиДокумента.Ссылка);
	КонецЕсли;
	ТаблицаРасходов = ВыполнитьЗапросПоРасходам(
		Новый Граница(МоментДокумента, ВидГраницы.Включая),
		СтруктураШапкиДокумента.Организация,
		ТаблицаФильтров);
	...
	Для Каждого СтрокаПартии Из ТаблицаДвиженияВзаиморасчетов Цикл
		...
		Если НайденныеСтроки.Количество() > 0
			И НайденныеСтроки[0].Валюта <> СтрокаПартии.ВалютаВзаиморасчетов Тогда
			СуммаОсталосьПогасить = СтрокаПартии.СуммаВзаиморасчетов * СтрокаПартии.Курс / СтрокаПартии.Кратность;
		Иначе
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если НЕ ВидОперации = "Доход" Тогда
				...
				Если ЗначениеЗаполнено(Строка.РасчетныйДокумент) Тогда
					ВыделятьНДСУСНДокумент = УчетнаяПолитика.ПорядокПризнанияРасходовПоНДС(СтруктураШапкиДокумента.Организация, Строка.РасчетныйДокумент.Дата) =
						Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику;
				Иначе
					ВыделятьНДСУСНДокумент = ВыделятьНДСУСН;
				КонецЕсли;

				Если ВыделятьНДСУСНДокумент И Не СтруктураШапкиДокумента.ПлательщикНДС
					И (Строка.ВидРасхода <> Перечисления.ВидыРасходовУСН.ОС)
					И (Строка.ВидРасхода <> Перечисления.ВидыРасходовУСН.НМА) Тогда
	...
	// Подготовим таблицу способов учета НДС по оплачиваемым партиям
	Если ТаблицаДляСписания.Количество() > 0
		И СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС И СтруктураШапкиДокумента.РаздельныйУчетНДС Тогда
		СпособыУчетаНДСПартий = СпособыУчетаНДСПартийПоДаннымТаблицыСписания(СтруктураШапкиДокумента, ТаблицаДляСписания);
		СтруктураШапкиДокумента.Вставить("СпособыУчетаНДСПартий", СпособыУчетаНДСПартий);
...
Процедура ОплатаПрочееУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, Получатель = "Налог", ВидДвижения = Неопределено)
	...
	ИначеЕсли Получатель = "Взыскатель" Тогда //Контрагент
		МассивФильтра = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаОплат, "Контрагент", Истина);
		Фильтр.ИмяПоля			= "Контрагент";
		Фильтр.ЗначениеПоля		= МассивФильтра;
		Фильтр.Условие			= "В";
		...
		Фильтр = ТаблицаФильтров.Добавить();
		Фильтр.ИмяПоля			= "СчетУчета";
		Фильтр.ЗначениеПоля		= ПланыСчетов.Хозрасчетный.РасчетыПоИсполнительнымДокументамРаботников;
		Фильтр.Условие			= "=";
		
		Если УчетЗарплаты.ДетальныйУчетИсполнительныхЛистов(СтруктураШапкиДокумента.Дата) Тогда
			МассивФильтра = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаОплат, "Сделка", Истина);
			ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(МассивФильтра);
			Фильтр.ИмяПоля			= "РасчетныйДокумент";
			Фильтр.ЗначениеПоля		= МассивФильтра;
			Фильтр.Условие			= "В";
	...
	// В ТаблицеПлатежей необходимо учесть суммы, которые уже есть в ДвиженияРегистров.ВзаиморасчетыРасход
	// ТаблицаПлатежей получается из регистра ПрочиеРасходы, движения в который по данному документу еще не попали
	Если ДвиженияРегистров <> Неопределено
		И ДвиженияРегистров.Свойство("ВзаиморасчетыРасход")
		И ЗначениеЗаполнено(ДвиженияРегистров.ВзаиморасчетыРасход) Тогда
		Для Каждого Платеж Из ТаблицаПлатежей Цикл
			Отбор = Новый Структура;
			Отбор.Вставить("СчетУчета",         Платеж.СчетУчета);
			Отбор.Вставить("РасчетныйДокумент", Платеж.РасчетныйДокумент);
			ПодготовленныеДвижения = ДвиженияРегистров.ВзаиморасчетыРасход.НайтиСтроки(Отбор);
			Для Каждого ПодготовленноеДвижение Из ПодготовленныеДвижения Цикл
				Платеж.Сумма = Мин(0, Платеж.Сумма + ПодготовленноеДвижение.Сумма);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого СтрокаПлатеж Из ТаблицаОплат Цикл
		...
		ИначеЕсли Получатель = "Взыскатель" Тогда
			СтруктураОтбора.Вставить("Контрагент",  СтрокаПлатеж.Контрагент);
		Иначе
		...
		ПриниматьУдержанныйНДФЛ = Получатель = "Налог"
			И СтрокаПлатеж.ДоговорКонтрагента = ПланыСчетов.Хозрасчетный.НДФЛ
			И ПриниматьУдержанныйНДФЛкРасходам(СтруктураШапкиДокумента.Дата);
		
		Если СуммаОсталосьПогасить > 0 И Не ЭтоФиксированныйВзносИП И Не ПриниматьУдержанныйНДФЛ Тогда
			...
			ИначеЕсли Получатель = "Взыскатель" Тогда
				ДвижениеВзиморасчетовПрочееУСН(ТаблицаДвиженияВзаиморасчетов, СтруктураШапкиДокумента.Организация, СтрокаПлатеж.Контрагент, СтрокаПлатеж.Сделка, ПланыСчетов.Хозрасчетный.РасчетыПоИсполнительнымДокументамРаботников, СуммаОсталосьПогасить);
			Иначе
		...
		Если ПриниматьУдержанныйНДФЛ Тогда
			Если ВидДвижения = ВидДвиженияНакопления.Приход Тогда
				КУДиР.Графа6 = КУДиР.Графа6 - СуммаОсталосьПогасить * Курс / Кратность;
			ИначеЕсли ВидДвижения = ВидДвиженияНакопления.Расход Тогда
				КУДиР.Графа4 = КУДиР.Графа4 - СуммаОсталосьПогасить * Курс / Кратность;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	// В ТаблицаРасходов необходимо учесть в расходах суммы, которые уже есть в ДвиженияРегистров.РасходыУСН
	// ТаблицаРасходов получается из регистра РасходыУСН, движения в который по данному документу еще не попали
	Если ДвиженияРегистров <> Неопределено
		И ДвиженияРегистров.Свойство("РасходыУСН")
		И ЗначениеЗаполнено(ДвиженияРегистров.РасходыУСН) Тогда
		Для Каждого Расход Из ТаблицаРасходов Цикл
			Отбор = Новый Структура;
			Отбор.Вставить("СчетУчета",         Расход.СчетУчета);
			Отбор.Вставить("РасчетныйДокумент", Расход.РасчетныйДокумент);
			Отбор.Вставить("ОтражениеВУСН",     Расход.ОтражениеВУСН);
			ПодготовленныеДвижения = ДвиженияРегистров.РасходыУСН.НайтиСтроки(Отбор);
			Для Каждого ПодготовленноеДвижение Из ПодготовленныеДвижения Цикл
				Расход.Сумма = Макс(0, Расход.Сумма - ПодготовленноеДвижение.Сумма);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	...
	СтруктураШапкиДокумента.Свойство("ДокументОплаты", ДокументОплаты);
	ТаблицыДвижений = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаДляСписания, , ВидОперации, Ложь, ДокументОплаты);
...
Процедура РасчетыСКонтрагентами_ЗачетАвансаПоставщикуУСН(СтруктураШапкиДокумента, ТаблицаАвансов, ТаблицаДвижений, ДвиженияРегистров, Договор, СуммаДокумента, ПриходОтражен = Ложь)
	...
	ИтоговаяСтруктура = Новый Структура("ТаблицаПриход, ТаблицаРасход, ТаблицаПринятых",
		ТаблицаДвижений.СкопироватьКолонки(),
		ТаблицаДвижений.СкопироватьКолонки(),
		ТаблицаДвижений.СкопироватьКолонки());
...
Процедура НачислениеНДСТаможенногоСоюза(СтруктураШапкиДокумента, ТаблицаПрихода, ДвиженияРегистров)
	...
	|	ВЫБОР
	|		КОГДА РасходыПриУСН.СтатусСписания = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.Принимаются)
	|		ИНАЧЕ РасходыПриУСН.СтатусСписания
	|	КОНЕЦ КАК СтатусСписания,
	...
	|	И РасходыПриУСН.СтатусСписания В (ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.Принимаются), ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.Распределяются), ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура РаспределениеНДСВключениеВСтоимость(СтруктураШапкиДокумента, ВключенныйВСтоимостьНДС, ДвиженияРегистров)
	
	Если Не ЗначениеЗаполнено(ВключенныйВСтоимостьНДС) Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаПриход = РегистрыНакопления.РасходыПриУСН.СоздатьНаборЗаписей().ВыгрузитьКолонки();
	
	МассивНоменклатуры = ОбщегоНазначения.ВыгрузитьКолонку(ВключенныйВСтоимостьНДС, "ЭлементРасхода", Истина);
	МассивПартий = ОбщегоНазначения.ВыгрузитьКолонку(ВключенныйВСтоимостьНДС, "Партия", Истина);
	
	ТаблицаБлокировок = ВключенныйВСтоимостьНДС.Скопировать(, "ЭлементРасхода, Партия");
	...
	ОтраженияВУСН = Новый Массив;
	ОтраженияВУСН.Добавить(Перечисления.ОтражениеВУСН.Принимаются);
	ОТраженияВУСН.Добавить(Перечисления.ОтражениеВУСН.Распределяются);
	
	ВидыРасходов = Новый Массив;
	ВидыРасходов.Добавить(Перечисления.ВидыРасходовУСН.Номенклатура);
	ВидыРасходов.Добавить(Перечисления.ВидыРасходовУСН.Услуги);
	...
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля      = "ОтражениеВУСН";
	Фильтр.ЗначениеПоля = ОтраженияВУСН;
	Фильтр.Условие      = "В";
	
	ГраницаОстатков = Новый Граница(СтруктураШапкиДокумента.Ссылка.МоментВремени(), ВидГраницы.Включая);
	
	ОстаткиРасходов = ВыполнитьЗапросПоРасходам(
		ГраницаОстатков, СтруктураШапкиДокумента.Организация, ТаблицаФильтров, ТаблицаБлокировок);
	
	ОстаткиРасходов.Индексы.Добавить("ЭлементРасхода, РасчетныйДокумент");
	
	СтруктураОтбораОстатков = Новый Структура("ЭлементРасхода, РасчетныйДокумент");
	
	Для Каждого СтрокаВключенныйВСтоимостьНДС Из ВключенныйВСтоимостьНДС Цикл
		
		СуммаВключенногоВСтоимостьНДС = СтрокаВключенныйВСтоимостьНДС.НДСУчитываетсяВCтоимости;

		СтруктураОтбораОстатков.ЭлементРасхода    = СтрокаВключенныйВСтоимостьНДС.ЭлементРасхода;
		СтруктураОтбораОстатков.РасчетныйДокумент = СтрокаВключенныйВСтоимостьНДС.Партия;

		СтрокиОстатков = ОстаткиРасходов.НайтиСтроки(СтруктураОтбораОстатков);
		
		Если СтрокиОстатков.Количество() >  0 Тогда
			
			СуммаНДСДляРаспределения = СуммаВключенногоВСтоимостьНДС;
			
			Для Каждого СтрокаОстатка Из СтрокиОстатков Цикл
				
				Если СуммаНДСДляРаспределения <= 0 Тогда
					Прервать;
				КонецЕсли;
				
				СкорректированнаяСуммаНДС = ?(СтрокаВключенныйВСтоимостьНДС.НДС = 0, 0,
					СтрокаОстатка.НДС / СтрокаВключенныйВСтоимостьНДС.НДС * СуммаВключенногоВСтоимостьНДС);
				
				СуммаНДСДляВключенияВСтоимость = Мин(СуммаНДСДляРаспределения, СкорректированнаяСуммаНДС);
				
				Если СуммаНДСДляВключенияВСтоимость = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				Если СтрокаОстатка.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеСписаноПринято Тогда
					
					// Расходы признаны. Включаем НДС в расходы.
					НоваяСтрокаПриход = ТаблицаПриход.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаПриход, СтрокаОстатка);
					НоваяСтрокаПриход.Количество = 0;
					НоваяСтрокаПриход.Сумма = СуммаНДСДляВключенияВСтоимость;
					НоваяСтрокаПриход.НДС = 0;
					
				КонецЕсли;
				
				СуммаНДСДляРаспределения = СуммаНДСДляРаспределения - СуммаНДСДляВключенияВСтоимость;
				
			КонецЦикла;
			
		Иначе
			
			// Расходы списаны полностью. Признаем в расходах сумму НДС
			НоваяСтрокаПриход = ТаблицаПриход.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПриход, СтрокаВключенныйВСтоимостьНДС);
			НоваяСтрокаПриход.Количество = 0;
			НоваяСтрокаПриход.Сумма = СуммаВключенногоВСтоимостьНДС;
			НоваяСтрокаПриход.НДС = 0;
	...
	ТаблицаПринятых = ТаблицаПриход.Скопировать();
	
	ИтоговаяСтруктура = Новый Структура("ТаблицаПриход, ТаблицаРасход, ТаблицаПринятых",
		ТаблицаПриход, ТаблицаРасход, ТаблицаПринятых);
...
Процедура КорректировкаРеализацииУСН(СтруктураШапкиДокумента, РеквизитыКорректировки, ТаблицаВозврат, ТаблицаРасходыУСНПоСделке, ДвиженияРегистровВх)
	...
	Для Каждого СтрокаКорректировкиКУДиР Из ТаблицаКорректировкиКУДиР Цикл
		Если СтрокаКорректировкиКУДиР.ИсключатьНДС И СтрокаКорректировкиКУДиР.НДС <> 0 Тогда
			СтрокаКорректировкиКУДиР.Сумма = СтрокаКорректировкиКУДиР.Сумма - СтрокаКорректировкиКУДиР.НДС;
			СтрокаКорректировкиКУДиР.НДС = 0;
		КонецЕсли;
	КонецЦикла;
...
Процедура РаспределениеРозничнойВыручкиУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияКУДиРПатент, Выручка, КУДиР, СтруктураДопПараметров, ДоговорДляОтложенногоПроведения = Неопределено)
	...
	Иначе // АТТ
		...
		Для Каждого СтрокаВыручка Из ТаблицаВыручка Цикл
			...
			Если СтрокаВыручка.Всего = 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			ИсключаемаяСуммаНДС = 0;
			Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН Тогда
				ИсключаемаяСуммаНДС = ИсключаемаяСуммаНДС(СтруктураШапкиДокумента,
					СтруктураШапкиДокумента.Ссылка,
					СуммаКоррДоходов);
				КУДиР.Графа5 = КУДиР.Графа5 - ИсключаемаяСуммаНДС;
				СтрокаРасшифровки.Графа5 = СтрокаРасшифровки.Графа5 - ИсключаемаяСуммаНДС;
			КонецЕсли;
			...
			Если СтрокаВыручка.ТорговыйСбор > 0 Тогда
				...
				КУДиР.ДоходТорговыйСбор = КУДиР.ДоходТорговыйСбор + СтрокаВыручка.ТорговыйСбор - ИсключаемаяСуммаНДС;
				СтрокаРасшифровки.ДоходТорговыйСбор = СтрокаРасшифровки.ДоходТорговыйСбор + СтрокаВыручка.ТорговыйСбор - ИсключаемаяСуммаНДС;
			КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаВзаиморасчетов Цикл
		...
		Если УчестьДоходыТорговыйСбор <> 0 Тогда
			...
			Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН Тогда
				ИсключаемаяСуммаНДС = ИсключаемаяСуммаНДС(СтруктураШапкиДокумента,
					СтруктураШапкиДокумента.Ссылка,
					УчестьДоходыТорговыйСбор);
				КУДиР.ДоходТорговыйСбор = КУДиР.ДоходТорговыйСбор - ИсключаемаяСуммаНДС;
				СтрокаРасшифровки.ДоходТорговыйСбор = СтрокаРасшифровки.ДоходТорговыйСбор - ИсключаемаяСуммаНДС;
			КонецЕсли;
		КонецЕсли;
...
Процедура РозничнаяВыручкаПродажаПодарочныхСертификатов(СтруктураШапкиДокумента, ПроданныеСертификаты, КУДиР, ДвиженияКУДиРПатент, ДоговорДляОтложенногоПроведения)
	...
	Для каждого Сертификат Из ПроданныеСертификаты Цикл
		...
		Если ТипОплаты = Перечисления.ТипыОплат.ПодарочныйСертификатСобственный Тогда
			...
			//  если ранее принимался в оплату продаж по ЕНВД, комиссии -
			...
			// Собственный сертификат при применении патентной системы:
			...
			Если Сертификат.СуммаБУ_Патент > 0 И ПатентРасчетов <> ПатентВыручки Тогда
	...
	Для Каждого СтрокаДохода Из ДоходыПоДоговорам Цикл
		...
		Если СтрокаДохода.СторноПатент > 0 И ЗначениеЗаполнено(ПатентВыручки) Тогда
			ЗаписьКУДиРПатент = ДвиженияКУДиРПатент.Добавить();
			ЗаписьКУДиРПатент.Патент = ПатентВыручки;
...
Процедура КорректировкаПоступленияУСН(СтруктураШапкиДокумента, РеквизитыКорректировки, ТаблицаКорректировки, ДвиженияРегистров)
		...
		Если СписатьСуммуВал <> 0 Или СписатьНДСВал <> 0 Тогда
			...
			Если СтрокаКорректировки.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.НеПринимаются
				...
				Если КоэффициентТекущийГод > 0 Тогда
					...
					// Если НДС принимается к вычету, то при отражении расхода он уже был исключен
					Если Не СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС 
						Или СтрокаКорректировки.СпособУчетаНДС <> Перечисления.СпособыУчетаНДС.ПринимаетсяКВычету Тогда
	...
	Если ТаблицаКорректировкиКУДиР_ЕНВД.Количество() > 0 Тогда
		КорректировкаКУДиР_РаспределениеРасходов(СтруктураШапкиДокумента, РеквизитыКорректировки, ТаблицаКорректировкиКУДиР_ЕНВД);
	КонецЕсли;
...
Процедура КорректировкаКУДиР_РаспределениеРасходов(СтруктураШапкиДокумента, ПараметрыКорректировки, ТаблицаПринятых)
...
Процедура ВыплатаЗарплаты(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ВыплатаПоДГПХ = Ложь)
	
	Если ТаблицаОплат.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если ПриниматьУдержанныйНДФЛкРасходам(СтруктураШапкиДокумента.Дата) Тогда
		ОтразитьВыплатуЗарплаты(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ВыплатаПоДГПХ);
	Иначе
		ОписаниеПолучателя = ?(ВыплатаПоДГПХ = Истина, "Договорник", "Работник");
		ОплатаПрочееУСН(СтруктураШапкиДокумента, ТаблицаОплат, ДвиженияРегистров, КУДиР, ОписаниеПолучателя);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОтразитьВыплатуЗарплаты(СтруктураШапкиДокумента, ТаблицаВыплат, ДвиженияРегистров, КУДиР, ВыплатаПоДГПХ)
	
	КоэффициентЗнакаВзаиморасчетов = -1;
	
	ДвиженияДокумента = СтруктураШапкиДокумента.Объект.Движения;
	
	ТаблицаУдержанныйНДФЛ = УчетЗарплаты.УдержанныйПриВыплатеНДФЛ(ДвиженияДокумента);
	...
	Если Не ЗначениеЗаполнено(ТаблицаУдержанныйНДФЛ) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме")
			...
			// Когда зарплата ведется во внешней программе и производится выплата по ДГПХ или депонированной зарплаты,
			// то в программе не фиксируются данные по оплаченным ведомостям.
			// В связи с этим ведомости необходимо получить напрямую из документа выплаты.
			МассивВедомостей = Новый Массив;
			МетаданныеДокумента = СтруктураШапкиДокумента.Ссылка.Метаданные();
			
			Если ВыплатаПоДГПХ Тогда
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ПлатежнаяВедомость", МетаданныеДокумента) Тогда
					Ведомость = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураШапкиДокумента.Ссылка, "ПлатежнаяВедомость");
					МассивВедомостей.Добавить(Ведомость);
				КонецЕсли;
			Иначе
				ТабличныеЧастиДокумента = МетаданныеДокумента.ТабличныеЧасти;
				Если ТабличныеЧастиДокумента.Найти("ВыплатаДепонентов") <> Неопределено Тогда
					Депоненты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураШапкиДокумента.Ссылка, "ВыплатаДепонентов");
					МассивВедомостей = Депоненты.Выгрузить().ВыгрузитьКолонку("Ведомость");
				ИначеЕсли ТабличныеЧастиДокумента.Найти("ПеречислениеДепонентов") <> Неопределено Тогда
					Депоненты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураШапкиДокумента.Ссылка, "ПеречислениеДепонентов");
					МассивВедомостей = Депоненты.Выгрузить().ВыгрузитьКолонку("Ведомость");
				КонецЕсли;
			КонецЕсли;
			
			УдержанныйНДФЛПоВедомостям = УчетЗарплаты.УдержанныйНДФЛПоДаннымВедомостейНаВыплатуЗарплаты(МассивВедомостей);
			
			Если ЗначениеЗаполнено(УдержанныйНДФЛПоВедомостям) Тогда
				Для Каждого СтрокаУдержанныйНДФЛПоВедомостям Из УдержанныйНДФЛПоВедомостям Цикл
					ЗаполнитьЗначенияСвойств(ТаблицаУдержанныйНДФЛ.Добавить(), СтрокаУдержанныйНДФЛПоВедомостям);
				КонецЦикла;
			КонецЕсли;
			
		ИначеЕсли ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровСредствамиБухгалтерии")
			И СтруктураШапкиДокумента.ВидОперацииУСН = "ВыплатаДепонентов" Тогда
			
			// Когда зарплата ведется в бухгалтерии и производится выплата депонированной зарплаты,
			// то при выплате нет возможности определить удержанный НДФЛ.
			// Поэтому удержанный НДФЛ получаем по данным депонирования зарплаты.
			УдержанныйНДФЛПоДепонентам = УчетЗарплаты.УдержанныйНДФЛПоДаннымДепонированияЗарплаты(ДвиженияДокумента);
			
			Если ЗначениеЗаполнено(УдержанныйНДФЛПоДепонентам) Тогда
				Для Каждого СтрокаУдержанныйНДФЛПоДепонентам Из УдержанныйНДФЛПоДепонентам Цикл
					ЗаполнитьЗначенияСвойств(ТаблицаУдержанныйНДФЛ.Добавить(), СтрокаУдержанныйНДФЛПоДепонентам);
				КонецЦикла;
			КонецЕсли;
	...
	ТаблицаВыплатСУчетомНДФЛ = РаспределитьУдержанныйНДФЛПоСпискуВыплат(ТаблицаВыплат, ТаблицаУдержанныйНДФЛ);
	
	Если ВыплатаПоДГПХ Тогда
		СчетУчетаЗарплаты = ПланыСчетов.Хозрасчетный.ПрочиеРасчетыСФизическимиЛицами;
	Иначе
		СчетУчетаЗарплаты = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда;
	КонецЕсли;
	...
	МассивРаботников = ТаблицаВыплат.ВыгрузитьКолонку("Работник");
	ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(МассивРаботников);
	...
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля      = "СчетУчета";
	Фильтр.ЗначениеПоля = СчетУчетаЗарплаты;
	Фильтр.Условие      = "=";
	
	Граница = СтруктураШапкиДокумента.Ссылка.МоментВремени();
	
	ТаблицаОстатковРасчетов = ВыполнитьЗапросПоВзаиморасчетамПрочееУСН(
		Граница, СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
	ТаблицаОстатковРасчетов.Индексы.Добавить("Контрагент");
	
	ТаблицаПрочихРасчетов = ТаблицаОстатковРасчетов.СкопироватьКолонки();
	
	Для Каждого Выплата Из ТаблицаВыплатСУчетомНДФЛ Цикл
		
		Сумма = Выплата.Сумма + Выплата.УдержаноНДФЛ;
		
		Если Сумма = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		КУДиР.Графа6 = КУДиР.Графа6 + Сумма;
		
		СуммаОсталосьПогасить = Сумма;
		ОтборОстатковРасчетов = Новый Структура;
		ОтборОстатковРасчетов.Вставить("Контрагент", Выплата.Работник);
		НайденныеСтрокиРасчетов = ТаблицаОстатковРасчетов.НайтиСтроки(ОтборОстатковРасчетов);
		
		Для Каждого СтрокаРасчетов Из НайденныеСтрокиРасчетов Цикл
			
			Если СуммаОсталосьПогасить <= 0 Тогда
				Прервать;
			КонецЕсли;
			
			СуммаРасчетов = СтрокаРасчетов.Сумма *КоэффициентЗнакаВзаиморасчетов;
			
			Если СуммаРасчетов <= 0 Тогда
				Продолжить;
			КонецЕсли;
			
			СуммаСписания = Мин(СуммаРасчетов, СуммаОсталосьПогасить);
			СуммаОсталосьПогасить = СуммаОсталосьПогасить - СуммаСписания;
			
			НоваяСтрокаПрочихРасчетов = ТаблицаПрочихРасчетов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПрочихРасчетов, СтрокаРасчетов);
			НоваяСтрокаПрочихРасчетов.Сумма = СуммаСписания;
			
			СтрокаРасчетов.Сумма = СтрокаРасчетов.Сумма - СуммаСписания * КоэффициентЗнакаВзаиморасчетов;
			
		КонецЦикла;
		
		// Отдельно не учитываем сумму удержанного НДФЛ.
		// В переходный период может возникнуть ситуация, когда начисление зарплаты сформировало движения без учета НДФЛ,
		// а при выплате зарплаты расход учитывается с учетом НДФЛ. Соответственно, остаток по расчетам будет меньше.
		// В этом случае суммы НДФЛ не откладываем, т.к. она была учтена при начислении зарплаты.
		Если СуммаОсталосьПогасить > 0
			И СуммаОсталосьПогасить <> Выплата.УдержаноНДФЛ Тогда
			ДвижениеВзиморасчетовПрочееУСН(
				ТаблицаПрочихРасчетов,
				СтруктураШапкиДокумента.Организация,
				Выплата.Работник,
				СтруктураШапкиДокумента.Ссылка,
				СчетУчетаЗарплаты,
				СуммаОсталосьПогасить);
		Иначе
			КУДиР.Графа6 = КУДиР.Графа6 - СуммаОсталосьПогасить;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ТаблицаПрочихРасчетов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаФильтров.Очистить();
	
	СтатусыРасходовУСН = Новый Массив;
	СтатусыРасходовУСН.Добавить(Перечисления.СтатусыРасходовУСН.НеСписаноНеОплачено);
	СтатусыРасходовУСН.Добавить(Перечисления.СтатусыРасходовУСН.НеОплачено);
	СтатусыРасходовУСН.Добавить(Перечисления.СтатусыРасходовУСН.НеОплаченоНеОплаченоПокупателем);
	
	МассивРасчетныхДокументов = ТаблицаПрочихРасчетов.ВыгрузитьКолонку("РасчетныйДокумент");
	ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(МассивРасчетныхДокументов);
	
	Если МассивРасчетныхДокументов.Количество() > 0 Тогда
		Фильтр = ТаблицаФильтров.Добавить();
		Фильтр.ИмяПоля = "РасчетныйДокумент";
		Фильтр.ЗначениеПоля = МассивРасчетныхДокументов;
		Фильтр.Условие = "В";
	...
	Фильтр = ТаблицаФильтров.Добавить();
	Фильтр.ИмяПоля      = "СтатусыОплатыРасходовУСН";
	Фильтр.ЗначениеПоля = СтатусыРасходовУСН;
	Фильтр.Условие      = "В";
	
	ТаблицаОстаткаРасходов = ВыполнитьЗапросПоРасходам(Граница, СтруктураШапкиДокумента.Организация, ТаблицаФильтров);
	ТаблицаОстаткаРасходов.Индексы.Добавить("РасчетныйДокумент, ЭлементРасхода");
	
	ТаблицаРасходов = ТаблицаОстаткаРасходов.СкопироватьКолонки();
	
	Для Каждого СтрокаПрочихРасчетов Из ТаблицаПрочихРасчетов Цикл
		
		Если СтрокаПрочихРасчетов.РасчетныйДокумент = СтруктураШапкиДокумента.Ссылка Тогда
			Продолжить;
		КонецЕсли;
		
		СуммаОсталосьПогасить = СтрокаПрочихРасчетов.Сумма;
		ОтборОстаткаРасходов = Новый Структура;
		ОтборОстаткаРасходов.Вставить("РасчетныйДокумент", СтрокаПрочихРасчетов.РасчетныйДокумент);
		ОтборОстаткаРасходов.Вставить("ЭлементРасхода", СтрокаПрочихРасчетов.Контрагент);
		НайденныеСтрокиРасходов = ТаблицаОстаткаРасходов.НайтиСтроки(ОтборОстаткаРасходов);
		
		Для Каждого СтрокаРахода Из НайденныеСтрокиРасходов Цикл
			
			Если СуммаОсталосьПогасить <= 0 Тогда
				Прервать;
			КонецЕсли;
			
			СуммаРасхода = СтрокаРахода.Сумма;
			
			Если СуммаРасхода <= 0 Тогда
				Продолжить;
			КонецЕсли;
			
			СуммаСписания = Мин(СуммаРасхода, СуммаОсталосьПогасить);
			СуммаОсталосьПогасить = СуммаОсталосьПогасить - СуммаСписания;
			КоэффициентРасхода = СуммаСписания / СуммаРасхода;
			
			НоваяСтрокаРасхода = ТаблицаРасходов.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаРасхода, СтрокаРахода);
			
			Если Не КоэффициентРасхода = 1 Тогда
				НоваяСтрокаРасхода.Сумма      = КоэффициентРасхода * НоваяСтрокаРасхода.Сумма;
				НоваяСтрокаРасхода.Количество = КоэффициентРасхода * НоваяСтрокаРасхода.Количество;
			КонецЕсли;
			
			СтрокаРахода.Сумма      = СтрокаРахода.Сумма - НоваяСтрокаРасхода.Сумма;
			СтрокаРахода.Количество = СтрокаРахода.Количество - НоваяСтрокаРасхода.Количество;
	...
	ДокументОплаты = Неопределено;
	СтруктураШапкиДокумента.Свойство("ДокументОплаты", ДокументОплаты);
	ТаблицаДвижений = ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, , , Ложь, ДокументОплаты);
	
	Если ТаблицаПрочихРасчетов.Колонки.Найти("ВидДвижения") = Неопределено Тогда
		ТаблицаПрочихРасчетов.Колонки.Добавить("ВидДвижения");
	КонецЕсли;
	ТаблицаПрочихРасчетов.ЗаполнитьЗначения(ВидДвиженияНакопления.Приход, "ВидДвижения");
	
	ДвиженияРегистров = СобратьТаблицыДвижений(ТаблицаДвижений, ТаблицаПрочихРасчетов, , ДвиженияРегистров);
	
КонецПроцедуры

Функция РаспределитьУдержанныйНДФЛПоСпискуВыплат(Знач ТаблицаВыплат, Знач ТаблицаУдержанныйНДФЛ)
	
	ТаблицаВыплат.Колонки.Добавить("УдержаноНДФЛ", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ОбщегоНазначенияБПВызовСервера.ПронумероватьТаблицу(ТаблицаВыплат);
	ТаблицаВыплат.Индексы.Добавить("НомерСтроки");
	
	ТаблицаУдержанныйНДФЛ.Свернуть("ФизЛицо", "Сумма");
	ТаблицаУдержанныйНДФЛ.Сортировать("ФизЛицо");
	
	Для Каждого УдержанныйНДФЛ Из ТаблицаУдержанныйНДФЛ Цикл
		
		ОтборВыплат = Новый Структура("Работник", УдержанныйНДФЛ.ФизЛицо);
		
		НайденныеВыплаты = ТаблицаВыплат.Скопировать(ОтборВыплат);
		НайденныеВыплаты.Сортировать("НомерСтроки");
		
		ОбщегоНазначенияБПВызовСервера.РаспределитьСуммуПоКолонкеТаблицы(
			УдержанныйНДФЛ.Сумма, НайденныеВыплаты, "УдержаноНДФЛ", "Сумма");
		
		Для Каждого Выплата Из НайденныеВыплаты Цикл
			СтрокаТаблицыВыплаты = ТаблицаВыплат.Найти(Выплата.НомерСтроки, "НомерСтроки");
			Если СтрокаТаблицыВыплаты <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(СтрокаТаблицыВыплаты, Выплата);
			КонецЕсли;
		КонецЦикла;
...
// Возвращает содержание операции сторнирования полученного ранее аванса, для записи книги учета доходов по патенту.
// 
// Параметры:
//  СуммаСторно - Число - Сумма сторнируемого аванса
//  КодЯзыка - Строка - Код языка, на котором формируется содержание
// 
// Возвращаемое значение:
//  Строка - Содержание операции книги учета доходов по патенту
Функция СодержаниеОперацииКУДПоПатентуСторнированиеАванса(СуммаСторно, КодЯзыка) Экспорт
	
	СодержаниеОперации = СтрШаблон(
		НСтр("ru = 'Аванс на сумму %1 руб. использован в деятельности, не подпадающей под действие патента.'", КодЯзыка),
		ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаСторно, , , " "));
	
	Возврат СодержаниеОперации;
	
КонецФункции

// Возвращает содержание операции получения оплаты от покупателя, для записи книги учета доходов по патенту.
// 
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - Контрагент, от которого получена оплата
//  Договор - СправочникСсылка.ДоговорыКонтрагентов - Договор с контрагентом, от которого получена оплата
//  Период - Дата - Дата записи книги учета доходов по патенту
//  КодЯзыка - Строка - Код языка, на котором формируется содержание
// 
// Возвращаемое значение:
//  Строка - Содержание операции книги учета доходов по патенту
Функция СодержаниеОперацииКУДПоПатентуОплатаОтПокупателя(Контрагент, Договор, Период, КодЯзыка) Экспорт
	
	СодержаниеОперации = СтрШаблон(
		НСтр("ru = 'Поступила оплата от покупателя ""%1"" по договору ""%2""'", КодЯзыка),
		НаименованиеКонтрагентаНаПечать(Контрагент, Период), Договор);
...
Функция ОписаниеСобытияДляКУДиР(СтруктураШапкиДокумента, ВидОперации, МетаданныеДокумента, ДоговорКонтрагента = Неопределено)
	...
	Если ВидОперации = "Поступление" Тогда
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
			...
			Если ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Права Тогда
				СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					"Приобретение прав у контрагента""%1"" по договору ""%2""",
						НаименованиеКонтрагентаНаПечать(ДокументСсылка.Контрагент, СтруктураШапкиДокумента.Дата),
						ДокументСсылка.ДоговорКонтрагента);
			ИначеЕсли ДокументСсылка.Услуги.Количество() = 0 Тогда
				СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	ИначеЕсли ВидОперации = "НачислениеЗП" Тогда
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.Премия") Тогда
			СтрокаСодержание = НСтр("ru = 'Начислена премия'");
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.Увольнение") Тогда
			СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Расчет при увольнении работника %1",
				НаименованиеФизлицаНаПечать(ДокументСсылка.ФизическоеЛицо, СтруктураШапкиДокумента.Дата));
		Иначе
			СтрокаСодержание = НСтр("ru = 'Перечислена заработная плата'");
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "УслугаКомиссионера"
		...
		Или ВидОперации = "ЗачетВознагражденияКомиссионераНДС"
		Или ВидОперации = "ОтражениеДоходаОтРеализацииКомиссионером" Тогда
		
		КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
		
		ШаблонСодержания = НСтр("ru = 'Получен отчет комиссионера ""%1"" по договору ""%2""; %3'", КодЯзыка);
		
		ОписаниеОперации = НСтр("ru = 'оказаны посреднические услуги в торговле'", КодЯзыка); // По умолчанию
		
		Если ВидОперации = "ЗачетВознагражденияКомиссионера" Тогда
			ОписаниеОперации = НСтр("ru = 'вознаграждение комиссионера зачтено из выручки от реализации'", КодЯзыка);
		ИначеЕсли ВидОперации = "ЗачетВознагражденияКомиссионераНДС" Тогда
			ОписаниеОперации = НСтр("ru = 'НДС с вознаграждения комиссионера зачтен из выручки от реализации'", КодЯзыка);
		ИначеЕсли ВидОперации = "ОтражениеДоходаОтРеализацииКомиссионером" Тогда
			ОписаниеОперации = НСтр("ru = 'реализованы товары'", КодЯзыка);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "СписаниеУКомиссионера" Тогда
		
		СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"Списание товаров по акту комиссионера ""%1""",
				НаименованиеКонтрагентаНаПечать(ДокументСсылка.Контрагент, СтруктураШапкиДокумента.Дата));
				
	ИначеЕсли ВидОперации = "ЗачетВыручкиКомиссионера" Тогда 
			
		ШаблонСодержания = "Оказаны услуги комиссионером ""%1"" по договору ""%2""; %3";
		
		СтрокаСодержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСодержания,
			НаименованиеКонтрагентаНаПечать(ДокументСсылка.Контрагент, СтруктураШапкиДокумента.Дата),
			СтруктураШапкиДокумента.ДоговорКонтрагента,
			"вознаграждение комиссионера зачтено из выручки от реализации");
...
Функция СобратьСодержаниеКУДиРПоРезультатамПризнанияРасходов(КУДиР, ВидОперации, Расход, ДеревоРасходов, СтруктураШапкиДокумента)
	...
	Если ВидОперации = "РаспределениеРасходов" Тогда
		...
		СовмещаемаяДеятельность = ?(СтруктураШапкиДокумента.Дата >= УчетЕНВДКлиентСервер.ДатаОтменыЕНВД(), "Патент", "ЕНВД");
		Пояснение = СтрШаблон(НСтр("ru = '%1 расходы по результатам распределения (УСН / %2)'"),
			НаправлениеРасходов, СовмещаемаяДеятельность);
	...
	ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Номенклатура Тогда
		...
		ИначеЕсли ВидОперации = "РаспределениеНДС" Тогда
			Пояснение =
				НСтр("ru = 'Учтен в стоимости товаров (материалов) распределенный НДС'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
	...
	ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Услуги Тогда
		...
		ИначеЕсли ВидОперации = "Поступление" И СтруктураШапкиДокумента.ВидДокумента <> "АвансовыйОтчет" Тогда
			Если СтруктураШапкиДокумента.ВидДокумента = "ПоступлениеТоваровУслуг" 
				И (СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером
				ИЛИ СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СТранспортнойКомпанией)
				И Расход.ОплатаИзВыручкиКомиссионера Тогда
				
				ДеталиСобытия = "вознаграждение комиссионера зачтено из выручки от реализации";
			Иначе
				ДеталиСобытия = "в счет предварительной оплаты";
			КонецЕсли; 
		ИначеЕсли ВидОперации = "УслугиФакторинга" Тогда
	...
	ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Зарплата Тогда
		...
		Если ВидОперации = "ВыплатаДГПХ" Тогда
			Пояснение = НаправлениеРасходов + " расходы на оплату по договорам ГПХ";
		Иначе
	...
	ИначеЕсли Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.Налоги Тогда
		...
		Если (ВидОперации = "НачислениеЗП" Или ВидОперации = "НачислениеИмущественныхНалогов")
			...
			Пояснение = ОписаниеЗачетаНалоговПриНачислении(Расход.КлючУникальности, СтруктураШапкиДокумента.Дата) + " ";
		ИначеЕсли ВидОперации = "НачислениеСтраховыхВзносовИП" Тогда
			// Для этих расходов событие полностью переопределяется
			ТекстСобытия = УчетСтраховыхВзносовИП.СодержаниеОперацииУплатыВзносов(Расход.СчетУчета, Расход.ВидСтраховогоВзноса,
				Расход.СтраховойПериод, СтруктураШапкиДокумента.Организация, Расход.ВзносыПодлежащиеУплате);
			ДополнитьСодержаниеКУДиР(КУДиРДляТекущейЗаписи, "Событие", ТекстСобытия, Истина);
		ИначеЕсли ВидОперации = "ЗачетАвансаПоЕдиномуНалоговомуСчету" Тогда
			Пояснение = "Зачет аванса по единому налоговому счету.";
		Иначе
...
Процедура ДополнитьСодержаниеКУДиРПояснениемПоИсключениюНДС(КУДиР, СуммаНДС)
	
	ПояснениеОперации = СтрШаблон(
		НСтр("ru = 'НДС в сумме %1 руб. исключен из доходов.'"),
		ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаНДС, , , " "));
...
Функция ОписаниеЗачетаНалоговПриНачислении(СчетУчета, ПериодУплаты)
	
	Описание = "";
	
	ВидНалога = РасчетыСБюджетом.ВидНалогаПоСчетуУчета(СчетУчета, , ПериодУплаты);
...
// Возвращает наименование контрагента для содержания КУДиР с учетом истории изменения наименования.
// 
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - Контрагент, наименование которого необходимо получить
//  ДатаСведений - Дата - Дата, на которую необходимо получить наименование контрагента
// 
// Возвращаемое значение:
...
Функция ВидОперацииДДС(СтруктураШапкиДокумента) Экспорт
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым) Тогда
		...
		ПредставлениеВида = "ВыплатыСамозанятым";
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПрочееПоступление
		Или ВидОперации = Перечисления.ВидыОперацийВыдачаДенежныхДокументов.ПрочаяВыдача
		Или ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхДокументов.ПоступлениеОтПоставщика
		Или ВидОперации = Перечисления.ВидыОперацийВыдачаДенежныхДокументов.ВозвратПоставщику
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ПрочийПриход
		Или ВидОперации = Перечисления.ВидыОперацийПКО.РасчетыПоКредитамИЗаймам
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратЗаймаКонтрагентом
		Или ВидОперации = Перечисления.ВидыОперацийПКО.ПолучениеЗайма
		...
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.КомиссияБанка
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДивидендов Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ОплатаПоставщику
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыдачаПодотчетномуЛицу
		...
		Или ВидОперации = Перечисления.ВидыОперацийВыдачаДенежныхДокументов.ВыдачаПодотчетномуЛицу Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗаработнойПлатыРаботнику Тогда

		ПредставлениеВида = "ВыплатаЗПРаботнику";
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникамПоДоговорамПодряда Тогда
		
		Если СтруктураШапкиДокумента.Свойство("ОплатаПоДГПХ") И СтруктураШапкиДокумента.ОплатаПоДГПХ Тогда
			ПредставлениеВида = "ВыплатаДГПХ";
		Иначе
			ПредставлениеВида = "ПрочееДДС";
		КонецЕсли;

	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаДепонентов
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДепонентов Тогда
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника Тогда
		...
		ПредставлениеВида = "ПеречислениеПоИсполнительномуЛисту";
	...
	Если ТаблицыРасходов.Свойство("ТаблицаПринятых") И ТаблицыРасходов.ТаблицаПринятых.Количество() > 0 Тогда
		...
		Для Каждого СтрокаПризнанногоРасхода Из ТаблицыРасходов.ТаблицаПринятых Цикл
			...
			Если НоваяСтрокаРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.ТаможенныеПлатежи
				...
				Если Не СтрокаПризнанногоРасхода.ИсключатьНДС Тогда
...
Функция ИзменениеСтатусовРасходов(СтруктураШапкиДокумента, ТаблицаРасходов, Сумма = 0, ВидОперации = "Оплата", КонтролироватьСумму = Истина, ДокументОплаты = Неопределено, ВходящаяСтруктура = Неопределено)
	...
	КонецЕсли;
	Если ТаблицаРасходов.Колонки.Найти("РасчетныйДокумент") = Неопределено Тогда
	...
	ЗаполнитьПризнакВыделятьНДСУСНДокумент(СтруктураШапкиДокумента, ТаблицаРасходов, ВыделятьНДСУСН);
	
	ТаблицаСписанныеПартииНДС = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураШапкиДокумента, "СписанныеПартииНДС");
	СписанныеПартииНДС = ?(ЗначениеЗаполнено(ТаблицаСписанныеПартииНДС), ТаблицаСписанныеПартииНДС.Скопировать(), Неопределено);
	
	ТаблицаСпособыУчетаНДСПартий = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураШапкиДокумента, "СпособыУчетаНДСПартий");
	СпособыУчетаНДСПартий = ?(ЗначениеЗаполнено(ТаблицаСпособыУчетаНДСПартий), ТаблицаСпособыУчетаНДСПартий.Скопировать(), Неопределено);
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		Если КонтролироватьСумму Тогда
			...
			Если Расход.Сумма = 0 Тогда
				Продолжить;
			КонецЕсли;
			Если Расход.ВыделятьНДСУСНДокумент И Не СтруктураШапкиДокумента.ПлательщикНДС
				И (Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ОС)
				И (Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НМА) Тогда
				СуммаКорректировки = Мин(Расход.Сумма + Расход.НДС, Сумма);
				Сумма = Макс(0, Сумма - СуммаКорректировки);
				КоэффициентРасхода = СуммаКорректировки / (Расход.Сумма + Расход.НДС);
			Иначе
				СуммаКорректировки = Мин(Расход.Сумма, Сумма);
				Сумма = Сумма - СуммаКорректировки;
				КоэффициентРасхода = СуммаКорректировки / Расход.Сумма;
			КонецЕсли;
		Иначе
			КоэффициентРасхода = 1;
		...
		// Необходимо распределить расходы по способам учета НДС
		РаспределятьРасходыПоСпособамУчетаНДС = ЗначениеЗаполнено(СпособыУчетаНДСПартий)
			И Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ТаможенныеПлатежи
			И Расход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НДС_ТС;
		
		ПараметрыИзмененияСтатусаРасхода = Новый Структура;
		ПараметрыИзмененияСтатусаРасхода.Вставить("СтрокаРасход", Расход);
		ПараметрыИзмененияСтатусаРасхода.Вставить("НовыйСтатусОплаты", НовыйСтатусОплаты);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ТаблицаКорректировкиРасход", ТаблицаКорректировкиРасход);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ТаблицаКорректировкиПриход", ТаблицаКорректировкиПриход);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ТаблицаПризнанныхРасходов", ТаблицаПризнанныхРасходов);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ВидОперации", ВидОперации);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ДокументОплаты", ДокументОплаты);
		ПараметрыИзмененияСтатусаРасхода.Вставить("КоэффициентРасхода", КоэффициентРасхода);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ПризнаватьРасход", ПризнаватьРасход);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ВалютаРеглУчета", ВалютаРеглУчета);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ТаблицаРасходов", ТаблицаРасходов);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ВыделятьНДСУСН", ВыделятьНДСУСН);
		ПараметрыИзмененияСтатусаРасхода.Вставить("ИсключатьНДС", СтруктураШапкиДокумента.ПрименяетсяОбщаяСтавкаНДС);
		
		Если РаспределятьРасходыПоСпособамУчетаНДС Тогда
			ОтборСпособУчетаНДС = Новый Структура("Номенклатура, Партия, СчетУчета");
			ОтборСпособУчетаНДС.Номенклатура = Расход.ЭлементРасхода;
			ОтборСпособУчетаНДС.Партия = Расход.РасчетныйДокумент;
			ОтборСпособУчетаНДС.СчетУчета = Расход.СчетУчета;
			
			НайденныеСтрокиСпособУчетаНДС = СпособыУчетаНДСПартий.НайтиСтроки(ОтборСпособУчетаНДС);
			СпособыУчетаНДСПоПартии = СпособыУчетаНДСПартий.Скопировать(НайденныеСтрокиСпособУчетаНДС);
			НДСВПартии = СпособыУчетаНДСПоПартии.Итог("НДС");
			НДСРасход = Расход.НДС * КоэффициентРасхода;
			
			Если НДСВПартии > 0 И НДСРасход > 0 Тогда
				Для Каждого СтрокаСпособаУчетаНДС Из НайденныеСтрокиСпособУчетаНДС Цикл
					Если НДСРасход <= 0 Тогда
						Прервать;
					КонецЕсли;
					
					Если СтрокаСпособаУчетаНДС.НДС = 0 Тогда
						Продолжить;
					КонецЕсли;
					
					ПараметрыИзмененияСтатусаРасхода.ИсключатьНДС =
						СтрокаСпособаУчетаНДС.СпособУчетаНДС <> Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
					ОбработатьИзменениеСтатусаРасхода(СтруктураШапкиДокумента, ПараметрыИзмененияСтатусаРасхода);
					НДСРасход = НДСРасход - СтрокаСпособаУчетаНДС.НДС * КоэффициентРасхода;
				КонецЦикла;
			Иначе
				ОбработатьИзменениеСтатусаРасхода(СтруктураШапкиДокумента, ПараметрыИзмененияСтатусаРасхода);
			КонецЕсли;
		...
		ОбработатьИзменениеСпособаУчетаНДС = ЗначениеЗаполнено(СписанныеПартииНДС)
			...
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.ПустаяСсылка()
			И Расход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.НеПринимаются
			И (ПризнаватьРасход Или Не ЗначениеЗаполнено(НовыйСтатусОплаты));
		...
		Если ОбработатьИзменениеСпособаУчетаНДС Тогда
			...
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ТаблицаПризнанныхРасходов", ТаблицаПризнанныхРасходов);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("СписанныеПартииНДС", СписанныеПартииНДС);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ЭтоРазукомплектация", Ложь);
			ПараметрыИзмененияСпособаУчетаНДС.Вставить("ЭтоПеремещение", Ложь);
			
			ОбработатьИзменениеСпособаУчетаНДСПартий(ПараметрыИзмененияСпособаУчетаНДС);
			
		КонецЕсли;
		
		Если КоэффициентРасхода = 1 Тогда
			Расход.Сумма = 0;
			Расход.Количество = 0;
			Расход.НДС = 0;
		Иначе
			Расход.Сумма = Расход.Сумма - КоэффициентРасхода * Расход.Сумма;
			Расход.Количество = Расход.Количество - КоэффициентРасхода * Расход.Количество;
			Расход.НДС = Расход.НДС - КоэффициентРасхода * Расход.НДС;
		КонецЕсли;
		
	КонецЦикла;
	
	Для Каждого СтрокаПризанныйРасход Из ТаблицаПризнанныхРасходов Цикл
		Если СтрокаПризанныйРасход.ИсключатьНДС И СтрокаПризанныйРасход.НДС <> 0 Тогда
			СтрокаПризанныйРасход.Сумма = СтрокаПризанныйРасход.Сумма - СтрокаПризанныйРасход.НДС;
			СтрокаПризанныйРасход.НДС = 0;
		КонецЕсли;
	КонецЦикла;
	...
	Если ВидОперации = "Оплата" Тогда
		...
		Для Каждого Строка Из ТаблицаПризнанныхРасходов Цикл
			...
			ДоговорКонтрагента = Строка.ДоговорКонтрагента;
			ЭтоВалютныйДоговорКонтрагента = ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
				И ДоговорКонтрагента.Валютный;
			Если Не ЭтоВалютныйДоговорКонтрагента Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаПриход = ТаблицаКорректировкиПриход.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаПриход, Строка);
			СтрокаПриход.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеОплачено;
			СтрокаРасход = ТаблицаКорректировкиРасход.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаРасход, Строка);
			СтрокаРасход.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеОплачено;
...
Процедура ПроверитьСторноРасходовПоСписанию(СтруктураШапкиДокумента, НомерСтрокиРасходов, ВыделятьНДСУСНДокумент,
	...
	Если (СтрокаКорректировкиПриход.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Распределяются) И
		...
		Если (ВыделятьНДСУСН И Не СтруктураШапкиДокумента.ПлательщикНДС) И (СтрокаКорректировкиПриход.НДС > 0)
			И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ОС)
			И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НМА)
			И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ТаможенныеПлатежи)
			И (СтрокаКорректировкиПриход.СтатусыОплатыРасходовУСН <> Перечисления.СтатусыРасходовУСН.НеОплачено)
			И(СтрокаКорректировкиПриход.СтатусыОплатыРасходовУСН <> Перечисления.СтатусыРасходовУСН.НеОплаченоНеОплаченоПокупателем) Тогда
...
Процедура ОбработатьИзменениеСтатусаРасхода(СтруктураШапкиДокумента, ПараметрыИзмененияСтатусаРасхода)
	
	Расход = ПараметрыИзмененияСтатусаРасхода.СтрокаРасход;
	НовыйСтатусОплаты = ПараметрыИзмененияСтатусаРасхода.НовыйСтатусОплаты;
	ТаблицаКорректировкиРасход = ПараметрыИзмененияСтатусаРасхода.ТаблицаКорректировкиРасход;
	ТаблицаКорректировкиПриход = ПараметрыИзмененияСтатусаРасхода.ТаблицаКорректировкиПриход;
	ТаблицаПризнанныхРасходов = ПараметрыИзмененияСтатусаРасхода.ТаблицаПризнанныхРасходов;
	ВидОперации = ПараметрыИзмененияСтатусаРасхода.ВидОперации;
	ДокументОплаты = ПараметрыИзмененияСтатусаРасхода.ДокументОплаты;
	КоэффициентРасхода = ПараметрыИзмененияСтатусаРасхода.КоэффициентРасхода;
	ПризнаватьРасход = ПараметрыИзмененияСтатусаРасхода.ПризнаватьРасход;
	ВалютаРеглУчета = ПараметрыИзмененияСтатусаРасхода.ВалютаРеглУчета;
	ТаблицаРасходов = ПараметрыИзмененияСтатусаРасхода.ТаблицаРасходов;
	ВыделятьНДСУСН = ПараметрыИзмененияСтатусаРасхода.ВыделятьНДСУСН;
	ИсключатьНДС = ПараметрыИзмененияСтатусаРасхода.ИсключатьНДС;
	...
	СтрокаКорректировкиРасход.ИсключатьНДС = ИсключатьНДС;
	СтрокаКорректировкиПриход.ИсключатьНДС = ИсключатьНДС;
	...
	Если Не КоэффициентРасхода = 1 Тогда
		...
		СтрокаКорректировкиРасход.Сумма         = КоэффициентРасхода * СтрокаКорректировкиРасход.Сумма;
		СтрокаКорректировкиРасход.Количество    = КоэффициентРасхода * СтрокаКорректировкиРасход.Количество;
		СтрокаКорректировкиРасход.НДС           = КоэффициентРасхода * СтрокаКорректировкиРасход.НДС;
		СтрокаКорректировкиПриход.Сумма         = КоэффициентРасхода * СтрокаКорректировкиПриход.Сумма;
		СтрокаКорректировкиПриход.Количество    = КоэффициентРасхода * СтрокаКорректировкиПриход.Количество;
		СтрокаКорректировкиПриход.НДС           = КоэффициентРасхода * СтрокаКорректировкиПриход.НДС;

	КонецЕсли;
		
	// Признание расхода в валюте или пересчет на дату оплаты по договору в у.е.
	// письмо Минфина от 25 апреля 2017 г. N 03-11-11/24828.
	Если (СтрокаКорректировкиРасход.Валюта <> ВалютаРеглУчета) И ТаблицаРасходов.Колонки.Найти("СтруктураКурса") <> Неопределено
		И ЗначениеЗаполнено(Расход.СтруктураКурса) И (ПризнаватьРасход Или ВидОперации = "Оплата") Тогда
			
		// Определим курс на дату оплаты
		Если ДокументОплаты <> Неопределено Тогда
			ДатаКурса = ДокументОплаты.Дата;
		Иначе
			ДатаКурса = СтруктураШапкиДокумента.Дата;
		КонецЕсли;

		СтруктураКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаКорректировкиРасход.Валюта, ДатаКурса);
		Курс = СтруктураКурса.Курс;
		Кратность = ?(СтруктураКурса.Кратность = 0, 1, СтруктураКурса.Кратность);
		...
		Если Курс <> 1 Тогда
			СтрокаКорректировкиПриход.Валюта = ?(Расход.СтруктураКурса.Свойство("ВалютаПолучателя"),
				Расход.СтруктураКурса.ВалютаПолучателя, ВалютаРеглУчета);
		КонецЕсли;

	КонецЕсли;

	// Признаем расходы в части входящего НДС, который был выделен
	Если (ВидОперации = "Оплата") И (СтрокаКорректировкиПриход.НДС > 0)
		И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ОС)
		И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НМА)
		И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.ТаможенныеПлатежи)
		И (СтрокаКорректировкиРасход.ВидРасхода <> Перечисления.ВидыРасходовУСН.НДС_ТС)
		И (СтрокаКорректировкиРасход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.ПустаяСсылка())
		И (СтрокаКорректировкиРасход.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.НеПринимаются) Тогда

		Если ВыделятьНДСУСН И Не СтруктураШапкиДокумента.ПлательщикНДС Тогда
			...
			Если Не Расход.ВыделятьНДСУСНДокумент Тогда
		...
		ИначеЕсли Не ВыделятьНДСУСН И Расход.ВыделятьНДСУСНДокумент Тогда
	...
	Если (ВидОперации = "Оплата")
		И (СтрокаКорректировкиПриход.СтатусыОплатыРасходовУСН = Перечисления.СтатусыРасходовУСН.НеСписаноПринято)
		И (СтрокаКорректировкиРасход.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются) Тогда

		СтрокаРасходов = ТаблицаПризнанныхРасходов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаРасходов, СтрокаКорректировкиПриход);
		СтрокаРасходов.ИсключатьНДС = ИсключатьНДС;
	...
	Если Не ТаблицаРасходов.Колонки.Найти("СтатусСписания") = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Расход.СтатусСписания) И Расход.ОтражениеВУСН <> НовыйСтатусРасхода Тогда
			
			Если Расход.Количество <> 0 И Расход.Количество > Расход.КоличествоПоСтатусуСписания Тогда

				КоэффициентРасходаКорректировка = Расход.КоличествоПоСтатусуСписания / Расход.Количество;
				СуммаКорректировки          = КоэффициентРасходаКорректировка * СтрокаКорректировкиПриход.Сумма;
				КоличествоКорректировки     = КоэффициентРасходаКорректировка * СтрокаКорректировкиПриход.Количество;
				НДСКорректировки            = КоэффициентРасходаКорректировка * СтрокаКорректировкиПриход.НДС;
				СтрокаКорректировкиПриход.Сумма      = СтрокаКорректировкиПриход.Сумма - СуммаКорректировки;
				СтрокаКорректировкиПриход.Количество = СтрокаКорректировкиПриход.Количество - КоличествоКорректировки;
				...
				СтрокаКорректировкиПриход.Сумма      = СуммаКорректировки;
				СтрокаКорректировкиПриход.Количество = КоличествоКорректировки;
			...
			ПроверитьСторноРасходовПоСписанию(СтруктураШапкиДокумента, НомерСтрокиРасходов,
				Расход.ВыделятьНДСУСНДокумент, ТаблицаКорректировкиРасход, ТаблицаКорректировкиПриход,
				ТаблицаПризнанныхРасходов, СтрокаКорректировкиРасход, СтрокаКорректировкиПриход);
			
		КонецЕсли;
		
	КонецЕсли;

	Если (НомерСтрокиРасходов > 0) И (СтрокаКорректировкиПриход.НДС > 0) И (Не ВыделятьНДСУСН)
		И Расход.ВыделятьНДСУСНДокумент И (Не ВидОперации = "Оплата") Тогда
		СтрокаКорректировкиПриход.НДС = 0;
	КонецЕсли;
...
Процедура ОбработатьИзменениеСпособаУчетаНДСПартий(ПараметрыИзмененияСпособаУчетаНДС)
	
	СтрокаРасход = ПараметрыИзмененияСпособаУчетаНДС.СтрокаРасход;
	ТаблицаКорректировкиПриход = ПараметрыИзмененияСпособаУчетаНДС.ТаблицаКорректировкиПриход;
	ТаблицаКорректировкиРасход = ПараметрыИзмененияСпособаУчетаНДС.ТаблицаКорректировкиРасход;
	ТаблицаПризнанныхРасходов  = ПараметрыИзмененияСпособаУчетаНДС.ТаблицаПризнанныхРасходов;
	СписанныеПартииНДС = ПараметрыИзмененияСпособаУчетаНДС.СписанныеПартииНДС;
	ЭтоРазукомплектация = ПараметрыИзмененияСпособаУчетаНДС.ЭтоРазукомплектация;
	ЭтоПеремещение = ПараметрыИзмененияСпособаУчетаНДС.ЭтоПеремещение;
	
	// Признаем расход при восстановлении НДС
	ИменаПолейОтбораНДСУчитываетсяВСтоимости =
		"Номенклатура, Партия, СпособУчетаНДСИзменился, СпособУчетаНДС, НовыйСпособУчетаНДС, ЕстьНДСПредъявленный";
	ОтборУчитываетсяВСтоимости = Новый Структура(ИменаПолейОтбораНДСУчитываетсяВСтоимости);
	ОтборУчитываетсяВСтоимости.Номенклатура = СтрокаРасход.ЭлементРасхода;
	ОтборУчитываетсяВСтоимости.СпособУчетаНДСИзменился = Истина;
	ОтборУчитываетсяВСтоимости.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПринимаетсяКВычету;
	ОтборУчитываетсяВСтоимости.НовыйСпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
	ОтборУчитываетсяВСтоимости.ЕстьНДСПредъявленный = Истина;
	Если ЭтоРазукомплектация Тогда
		ОтборУчитываетсяВСтоимости.Партия = СтрокаРасход.РасчетныйДокумент;
	Иначе
		ОтборУчитываетсяВСтоимости.Партия = СтрокаРасход.Партия;
	КонецЕсли;

	СписанныеПартииНДСУчитываетсяВСтоимости = СписанныеПартииНДС.НайтиСтроки(ОтборУчитываетсяВСтоимости);
	
	ОтразитьВРасходахИзменениеСпособаУчетаНДС(СписанныеПартииНДСУчитываетсяВСтоимости, СтрокаРасход,
		ТаблицаКорректировкиПриход, ТаблицаКорректировкиРасход, ТаблицаПризнанныхРасходов,
		ЭтоПеремещение, ЭтоРазукомплектация, Ложь);
	
	// Исключим НДС из расходов при исключении из стоимости
	ИменаПолейОтбораНДСИсключаетсяИзСтоимости =
		"Номенклатура, Партия, СпособУчетаНДСИзменился, СпособУчетаНДС, ЕстьНДСПредъявленный";
	ОтборИсключениеИзСтоимости = Новый Структура(ИменаПолейОтбораНДСИсключаетсяИзСтоимости);
	ОтборИсключениеИзСтоимости.Номенклатура = СтрокаРасход.ЭлементРасхода;
	ОтборИсключениеИзСтоимости.СпособУчетаНДСИзменился = Истина;
	ОтборИсключениеИзСтоимости.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
	ОтборИсключениеИзСтоимости.ЕстьНДСПредъявленный = Истина;
	Если ЭтоРазукомплектация Тогда
		ОтборИсключениеИзСтоимости.Партия = СтрокаРасход.РасчетныйДокумент;
	Иначе
		ОтборИсключениеИзСтоимости.Партия = СтрокаРасход.Партия;
	КонецЕсли;

	СписанныеПартииНДСИсключениеИзСтоимости = СписанныеПартииНДС.НайтиСтроки(ОтборИсключениеИзСтоимости);
	
	ОтразитьВРасходахИзменениеСпособаУчетаНДС(СписанныеПартииНДСИсключениеИзСтоимости, СтрокаРасход,
		ТаблицаКорректировкиПриход, ТаблицаКорректировкиРасход, ТаблицаПризнанныхРасходов,
		ЭтоПеремещение, ЭтоРазукомплектация, Истина);
	
	// Признаем расход при отмене включения НДС в распределение
	ИменаПолейОтбораНДСОтменаРаспределения =
		"Номенклатура, Партия, СпособУчетаНДСИзменился, СпособУчетаНДС, НовыйСпособУчетаНДС, ЕстьНДСПредъявленный";
	ОтборОтменаРаспределения = Новый Структура(ИменаПолейОтбораНДСОтменаРаспределения);
	ОтборОтменаРаспределения.Номенклатура = СтрокаРасход.ЭлементРасхода;
	ОтборОтменаРаспределения.СпособУчетаНДСИзменился = Истина;
	ОтборОтменаРаспределения.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Распределяется;
	ОтборОтменаРаспределения.НовыйСпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
	ОтборОтменаРаспределения.ЕстьНДСПредъявленный = Истина;
	Если ЭтоРазукомплектация Тогда
		ОтборОтменаРаспределения.Партия = СтрокаРасход.РасчетныйДокумент;
	Иначе
		ОтборОтменаРаспределения.Партия = СтрокаРасход.Партия;
	КонецЕсли;

	СписанныеПартииНДСОтменаРаспределения = СписанныеПартииНДС.НайтиСтроки(ОтборОтменаРаспределения);
	
	ОтразитьВРасходахИзменениеСпособаУчетаНДС(СписанныеПартииНДСОтменаРаспределения, СтрокаРасход,
		ТаблицаКорректировкиПриход, ТаблицаКорректировкиРасход, ТаблицаПризнанныхРасходов,
		ЭтоПеремещение, ЭтоРазукомплектация, Ложь);
	
КонецПроцедуры

Процедура ОтразитьВРасходахИзменениеСпособаУчетаНДС(ПартииНДССИзменениемСпособа, СтрокаРасход, ТаблицаКорректировкиПриход,
	ТаблицаКорректировкиРасход, ТаблицаПризнанныхРасходов, ЭтоПеремещение, ЭтоРазукомплектация, ЭтоИсключениеИзСтоимости)
	
	Если Не ЗначениеЗаполнено(ПартииНДССИзменениемСпособа) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СписаннаяПартияНДС Из ПартииНДССИзменениемСпособа Цикл

		Если СписаннаяПартияНДС.НДС <= 0 Тогда
			Продолжить;
		КонецЕсли;

		СтрокаКорректировкиНДСПриход = ТаблицаКорректировкиПриход.Добавить();
		СтрокаКорректировкиНДСРасход = ТаблицаКорректировкиРасход.Добавить();

		ЗаполнитьЗначенияСвойств(СтрокаКорректировкиНДСПриход, СтрокаРасход);
		ЗаполнитьЗначенияСвойств(СтрокаКорректировкиНДСРасход, СтрокаРасход);

		СтрокаКорректировкиНДСПриход.Количество = 0;
		СтрокаКорректировкиНДСПриход.Сумма = ?(ЭтоИсключениеИзСтоимости, -СписаннаяПартияНДС.НДС, СписаннаяПартияНДС.НДС);
		СтрокаКорректировкиНДСПриход.НДС = 0;

		СтрокаКорректировкиНДСРасход.Количество = 0;
		СтрокаКорректировкиНДСРасход.Сумма = ?(ЭтоИсключениеИзСтоимости, -СписаннаяПартияНДС.НДС, СписаннаяПартияНДС.НДС);
		СтрокаКорректировкиНДСРасход.НДС = 0;

		Если Не ЭтоПеремещение Тогда
			СтрокаКорректировкиНДСПриход.НДС = ?(ЭтоИсключениеИзСтоимости, -СписаннаяПартияНДС.НДС, СписаннаяПартияНДС.НДС);
			СтрокаКорректировкиНДСПриход.СтатусСписания = Перечисления.ОтражениеВУСН.ПустаяСсылка();
			СтрокаКорректировкиНДСРасход.СтатусСписания = Перечисления.ОтражениеВУСН.ПустаяСсылка();
		КонецЕсли;

		СтрокаПризнанныйРасходНДС = ТаблицаПризнанныхРасходов.Добавить();

		ЗаполнитьЗначенияСвойств(СтрокаПризнанныйРасходНДС, СтрокаРасход);

		СтрокаПризнанныйРасходНДС.Количество = 0;
		СтрокаПризнанныйРасходНДС.Сумма = ?(ЭтоИсключениеИзСтоимости, -СписаннаяПартияНДС.НДС, СписаннаяПартияНДС.НДС);
		СтрокаПризнанныйРасходНДС.НДС = 0;

		Если ЭтоРазукомплектация Тогда
			СтрокаКорректировкиНДСПриход.ЭлементРасхода = СтрокаРасход.НоменклатураПриход;
			СтрокаКорректировкиНДСПриход.Партия = СтрокаРасход.ПартияПриход;
			СтрокаКорректировкиНДСПриход.СчетУчета = СтрокаРасход.СчетПриход;

			СтрокаКорректировкиНДСРасход.ЭлементРасхода = СтрокаРасход.НоменклатураПриход;
			СтрокаКорректировкиНДСРасход.Партия = СтрокаРасход.ПартияПриход;
			СтрокаКорректировкиНДСРасход.СчетУчета = СтрокаРасход.СчетПриход;

			СтрокаПризнанныйРасходНДС.ЭлементРасхода = СтрокаРасход.НоменклатураПриход;
			СтрокаПризнанныйРасходНДС.Партия = СтрокаРасход.ПартияПриход;
			СтрокаПризнанныйРасходНДС.СчетУчета = СтрокаРасход.СчетПриход;
		КонецЕсли;

		СписаннаяПартияНДС.НДС = 0;
...
Функция ПодготовитьТаблицуЗачетаАванса(СтруктураШапкиДокумента, ТаблицаРасчетов)
	...
	ТаблицаЗачетАвансаУСН.Колонки.Добавить("ПроверятьНДССАванса", Новый ОписаниеТипов("Булево"));
	ТаблицаЗачетАвансаУСН.Колонки.Добавить("АвансНДС",            ТипСтандартнойСуммы);
	ТаблицаЗачетАвансаУСН.Колонки.Добавить("ВключитьНДС_УСН",     ТипСтандартнойСуммы);
	...
	Для Каждого СтрокаРасчетов Из ТаблицаРасчетов Цикл
		...
		ВедетсяУчетНДСНеплательщиковПоФЗ84 = ЗначениеЗаполнено(СтрокаЗачета.ДокументРасчетов)
			И ЗначениеЗаполнено(СтрокаЗачета.ДатаДокументаРасчетов)
			И Не СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН;
		СтрокаЗачета.ИсключатьНДСИзДоходов = ?(ВедетсяУчетНДСНеплательщиковПоФЗ84,
		...
		// Определим сумму НДС в авансе
		НеплательщикНДСНаДатуАванса = Не УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(
			СтруктураШапкиДокумента.Организация, СтрокаЗачета.ДатаДокументаРасчетов);
		Если НеплательщикНДСНаДатуАванса Тогда
			ИсключатьНДСИзДоходаПоАвансамБезНДС = ИсключатьНДСИзДоходаПоАвансамБезНДС(
				СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата);
		Иначе
			ИсключатьНДСИзДоходаПоАвансамБезНДС = Истина;
		КонецЕсли;

		ОпределятьНДССАванса = СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
			И ЗначениеЗаполнено(СтрокаЗачета.ДокументРасчетов)
			И ЭтоДокументПоступленияАвансаОтПокупателя(СтрокаЗачета.ДокументРасчетов)
			И ИсключатьНДСИзДоходаПоАвансамБезНДС;
		
		ПроверятьНДСДоНачалаПрименения = СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
			И ЗначениеЗаполнено(СтрокаЗачета.ДокументРасчетов)
			И ЭтоДокументРасчетовСКонтрагентомДоПримененияНДС(СтрокаЗачета.ДокументРасчетов);
		
		СтрокаЗачета.ПроверятьНДССАванса = ОпределятьНДССАванса Или ПроверятьНДСДоНачалаПрименения;
		
		Если ОпределятьНДССАванса Тогда
			
			РасшифровкаПлатежаАванса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				СтрокаЗачета.ДокументРасчетов, "РасшифровкаПлатежа");
			РасшифровкаАванса = РасшифровкаПлатежаАванса.Выгрузить();
			ПараметрыОтбора = Новый Структура("ДоговорКонтрагента", СтрокаЗачета.ДоговорКонтрагента);
			СтрокиРасшифровкиАвансаПоДоговору = РасшифровкаАванса.НайтиСтроки(ПараметрыОтбора);
			
			Если СтрокиРасшифровкиАвансаПоДоговору.Количество() > 0 Тогда
				СуммаАвансаСНДС = 0;
				СуммаНДСАванса = 0;
				
				СуммаВзаиморасчетовУСН = СтрокаЗачета.СуммаВзаиморасчетов
					- СтрокаЗачета.СуммаВзаиморасчетовЕНВД
					- СтрокаЗачета.СуммаВзаиморасчетовКомитента
					- СтрокаЗачета.СуммаВзаиморасчетовПатент;
				...
				СтрокаЗачета.АвансНДС = ?(СуммаАвансаСНДС = 0, 0, СуммаНДСАванса * СуммаВзаиморасчетовУСН / СуммаАвансаСНДС);
			Иначе
				СтрокаЗачета.ПроверятьНДССАванса = Ложь;
			КонецЕсли;
...
Функция АвансПодлежитПереквалификации(СтрокаАванса, СтруктураШапкиДокумента)
	...
	Если СтрокаАванса.СуммаВзаиморасчетовПатент > 0 И ЗначениеЗаполнено(СтрокаАванса.ПатентАванса) Тогда
		Если Не УчетПСН.ДеятельностьПоПатентуДействуетНаДату(СтрокаАванса.ПатентАванса, СтруктураШапкиДокумента.Дата) Тогда
			// Авансы, полученные на патенте, после окончания срока действия патента не переквалифицируются.
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	
	// Если на дату аванса ИП не применял патент, а в последующем перешел на систему налогооблажения "Только патент"
	// то доходы по авансу, полученные до применения "Только патент", не должны переквалифицироваться
	Если Не УчетнаяПолитика.ПрименяетсяУСНПатент(СтруктураШапкиДокумента.Организация, СтрокаАванса.ДатаДокументаРасчетов)
		И УчетнаяПолитика.ПрименяетсяТолькоУСНПатент(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	// Если на дату аванса у ИП была система налогообложения "Только ЕНВД", то аванс относится к ЕНВД.
...
Функция ЭтоОплатаЗаПериодБезПатентаПриПримененииТолькоПатента(Организация, ОплачиваемыйПериод, ДатаОплаты)

	Возврат Не УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, ОплачиваемыйПериод)
		И УчетнаяПолитика.ПрименяетсяТолькоУСНПатент(Организация, ДатаОплаты);

КонецФункции

Функция ПодготовитьТаблицуОплатыВнеоборотноогоАктиваДляУСН(ТаблицаОплатыВНА)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТаблицаОплатыВНА", ТаблицаОплатыВНА);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОплатыВНУДляУСН.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	ОплатыВНУДляУСН.Организация КАК Организация,
	|	ОплатыВНУДляУСН.СуммаОплаты КАК СуммаОплаты,
	|	ОплатыВНУДляУСН.РеквизитыДокументаОплаты КАК ДокументОплаты
	|ПОМЕСТИТЬ ТаблицаОплатыВНА
	|ИЗ
	|	&ТаблицаОплатыВНА КАК ОплатыВНУДляУСН
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	ДокументОплаты
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаОплаты.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	...
	|ИЗ
	|	ТаблицаОплатыВНА КАК ТаблицаОплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ТаблицаОплаты.Организация = ДанныеПервичныхДокументов.Организация
	|			И ТаблицаОплаты.ДокументОплаты = ДанныеПервичныхДокументов.Документ";
...
//
// Параметры:
//  ТаблицаРезультатов		 - ТаблицаЗначений - исходная таблица
//  ГруппировочныеКолонки	 - Строка - список колонок, по котором необходимо группировать
//  КолонкиИтогов			 - Строка - список колонок, по которым формируются итоги
// 
// Возвращаемое значение:
//  ТаблицаЗнанчений - исходная таблица, сгруппированная
//
Функция ТаблицуЗначенийВТаблицуСГруппировкой(ТаблицаРезультатов, знач ГруппировочныеКолонки= "",знач КолонкиИтогов = "") Экспорт
	
	ИтоговаяТаблица = ТаблицаРезультатов.СкопироватьКолонки();
	...
	Для Каждого СтрокаДерева из ИтоговаяТаблица Цикл
		...
		КонецЦикла;
		ПромежуточнаяТаблица = ТаблицаРезультатов.СкопироватьКолонки();
		...
		Для Каждого СтрокаТаблицы Из МассивПодчиненныхСтрок Цикл
			ЗаполнитьЗначенияСвойств(ПромежуточнаяТаблица.Добавить(), СтрокаТаблицы);
		КонецЦикла;
		СтрокаДерева.Строки = ПромежуточнаяТаблица;
	КонецЦикла;
...
Функция ВыполнитьЗапросПоРасходам(Граница, Организация, ТаблицаФильтров, ТаблицаБлокировок = Неопределено) Экспорт
	...
	ФильтрПартия = ТаблицаФильтров.Найти("Партия", "ИмяПоля");
	Если ФильтрПартия <> Неопределено Тогда
		ФильтрПартия.ЗначениеПоля = ДокументыКорректныхТипов(ФильтрПартия.ЗначениеПоля, "Партия");
	КонецЕсли;
	
	ФильтрРасчетныйДокумент = ТаблицаФильтров.Найти("РасчетныйДокумент", "ИмяПоля");
	Если ФильтрРасчетныйДокумент <> Неопределено Тогда
		ФильтрРасчетныйДокумент.ЗначениеПоля = ДокументыКорректныхТипов(ФильтрРасчетныйДокумент.ЗначениеПоля, "РасчетныйДокумент");
	КонецЕсли;
	...
	Текст =
	"ВЫБРАТЬ
	|	РасходыУСН.Организация КАК Организация,
	|	РасходыУСН.СтатусыОплатыРасходовУСН КАК СтатусыОплатыРасходовУСН,
	|	РасходыУСН.СтатусыОплатыРасходовУСН.Порядок КАК СтатусыОплатыРасходовУСНПорядок,
	|	РасходыУСН.Партия КАК Партия,
	|	РасходыУСН.ВидРасхода КАК ВидРасхода,
	|	РасходыУСН.ЭлементРасхода КАК ЭлементРасхода,
	|	РасходыУСН.СтатусыПартийУСН КАК СтатусыПартийУСН,
	|	РасходыУСН.СчетУчета КАК СчетУчета,
	|	РасходыУСН.РасчетныйДокумент КАК РасчетныйДокумент,
	|	РасходыУСН.Валюта КАК Валюта,
	|	РасходыУСН.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	РасходыУСН.ОтражениеВУСН КАК ОтражениеВУСН,
	...
	|ПОМЕСТИТЬ РасходыПриУСН
	|ИЗ
	|	РегистрНакопления.РасходыПриУСН.Остатки(
	|			&Граница,
	|			Организация = &Организация) КАК РасходыУСН
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Партия
	...
	|	РасходыУСН.НомерСтрокиДокумента КАК НомерСтрокиДокумента,
	|	РасходыУСН.РеквизитыДокументаОплаты КАК РеквизитыДокументаОплаты,
	|	РасходыУСН.Количество КАК Количество,
	|	РасходыУСН.Сумма КАК Сумма,
	|	РасходыУСН.НДС КАК НДС,
	|	РеквизитыПартий.ДатаРегистратора КАК ПартияДата,
	|	РеквизитыРасчетныхДокументов.ДатаРегистратора КАК РасчетныйДокументДата,
	|	РеквизитыОплаты.ДатаРегистратора КАК РеквизитыДокументаОплатыДата
	|ИЗ
	|	РасходыПриУСН КАК РасходыУСН
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК РеквизитыОплаты
	|		ПО РасходыУСН.Организация = РеквизитыОплаты.Организация
	|			И РасходыУСН.РеквизитыДокументаОплаты = РеквизитыОплаты.Документ
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Для каждого Фильтр Из ТаблицаФильтров Цикл
		...
		Условие = ?(СтрНайти(Фильтр.Условие, "В") <> 0, "%1 %2 (&%1)", "%1 %2 &%1");
		Условие = СтрШаблон(Условие, Фильтр.ИмяПоля, Фильтр.Условие);
		Если ПустаяСтрока(ТекстУсловия) Тогда // инициализируем переменную
			
			Если Фильтр.ИмяПоля = "РасчетныйДокумент" Или Фильтр.ИмяПоля = "Партия" Тогда // приоритет у некластерного индекса
	...
	КонецЦикла;
	Запрос.Текст = СтрЗаменить(Текст, "Организация = &Организация", ТекстУсловия);
...
Процедура ОбработатьТаблицуФильтров(ТаблицаФильтров)
	...
	Если ТаблицаФильтров.Колонки.Найти("ПорядокФильтра") = Неопределено Тогда
		ТаблицаФильтров.Колонки.Добавить("ПорядокФильтра", ОбщегоНазначения.ОписаниеТипаЧисло(3, 0, ДопустимыйЗнак.Неотрицательный));
	КонецЕсли;
	
	ПорядокИзмерений = Новый Массив;
	ПорядокИзмерений.Добавить("Организация");
	ПорядокИзмерений.Добавить("СтатусыОплатыРасходовУСН");
	ПорядокИзмерений.Добавить("СчетУчета");
	Если ТаблицаФильтров.Найти("Партия", "ИмяПоля") = Неопределено Тогда
		ПорядокИзмерений.Добавить("Партия");
	Иначе
		ПорядокИзмерений.Вставить(0, "Партия");
	КонецЕсли;
	ПорядокИзмерений.Добавить("ВидРасхода");
	ПорядокИзмерений.Добавить("ЭлементРасхода");
	ПорядокИзмерений.Добавить("СтатусыПартийУСН");
	Если ТаблицаФильтров.Найти("РасчетныйДокумент", "ИмяПоля") = Неопределено Тогда
		ПорядокИзмерений.Добавить("РасчетныйДокумент");
	Иначе
		ПорядокИзмерений.Вставить(0, "РасчетныйДокумент");
	КонецЕсли;
	ПорядокИзмерений.Добавить("Валюта");
	ПорядокИзмерений.Добавить("ДоговорКонтрагента");
	ПорядокИзмерений.Добавить("ОтражениеВУСН");
	ПорядокИзмерений.Добавить("НомерСтрокиДокумента");
	ПорядокИзмерений.Добавить("РеквизитыДокументаОплаты");
	
	Для Каждого Фильтр Из ТаблицаФильтров Цикл

		ПорядокФильтра = ПорядокИзмерений.Найти(Фильтр.ИмяПоля);
		Если ПорядокФильтра = Неопределено Тогда
			Фильтр.ПорядокФильтра = 999;
		Иначе
			Фильтр.ПорядокФильтра = ПорядокФильтра;
		КонецЕсли;
	...
	КонецЦикла;
	ТаблицаФильтров.Сортировать("ПорядокФильтра");
...
Функция СобратьТаблицыДвижений(СтруктураРасходов, ТаблицаВзаиморасчетов = Неопределено, ДопТаблицаРасходов = Неопределено, ДопСтруктураРасходов = Неопределено)
	...
	Иначе
		...
		// Случай, когда все входные данные Неопределено
		// Скопируем колонки из регистра РасходыУСН
		Если ТаблицаРезультат.Колонки.Количество() = 0 Тогда
			СкоприоватьКолонкиРегистраРасходыУСН(ТаблицаРезультат);
		КонецЕсли;
...
Функция ПолучитьНовыйСтатусОплаты(СтруктураШапкиДокумента, Расход, ТекСтатус, Событие = "Оплата", ПризнаватьРасход = Ложь)
	...
	Если Событие = "Оплата"
		...
		ИначеЕсли ТекСтатус = Перечисления.СтатусыРасходовУСН.НеОплачено Тогда
			...
			Если (Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.НМА)
				И (СтруктураШапкиДокумента.Дата >= Дата("20060101"))
				И Не ЭтоПриобретениеПрав(Расход) Тогда
				ПризнаватьРасход = Ложь;
			КонецЕсли;
		КонецЕсли;

	ИначеЕсли (Событие = "Списание") или (Событие = "ВозвратПоставщику") Или Событие = "КорректировкаПоступления" 
			Или Событие = "БезвозмезднаяПередача" Или Событие = "РасходыНаБилеты" Или Событие = "СписаниеУКомиссионера" Тогда
	...
	ИначеЕсли Событие = "РаспределениеРасходов" Тогда
...
Функция ЭтоПриобретениеПрав(Знач Расход)
	
	Возврат (Расход.ВидРасхода = Перечисления.ВидыРасходовУСН.НМА) И ТипЗнч(Расход.РасчетныйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг");
...
Функция ПолучитьКурсВалютыИзКэша(Валюта, ДатаКурса, КэшКурсовВалют) Экспорт
	
	Если КэшКурсовВалют = Неопределено Тогда
		КэшКурсовВалют = Новый Соответствие;
	КонецЕсли;
	
	КлючЗаписи = СтрШаблон("%1_%2",
		Строка(Валюта.УникальныйИдентификатор()),
		Формат(ДатаКурса, "ДФ=yyyyMMdd; ДП=00010101"));
...
Функция НоваяТаблицаОплатыВнеоборотногоАктива()
	
	ТаблицаОплатыОС = Новый ТаблицаЗначений;
	ТаблицаОплатыОС.Колонки.Добавить("ВнеоборотныйАктив", Новый ОписаниеТипов("СправочникСсылка.ОсновныеСредства, СправочникСсылка.НематериальныеАктивы"));
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Налоговый учет по УСН'", ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецФункции

Процедура ЗаполнитьПризнакВыделятьНДСУСНДокумент(СтруктураШапкиДокумента, ТаблицаРасходов, ВыделятьНДСУСН)
	Перем ДатаПереходаНаУСН;
	
	Если ТаблицаРасходов.Колонки.Найти("ВыделятьНДСУСНДокумент") = Неопределено Тогда
		ТаблицаРасходов.Колонки.Добавить("ВыделятьНДСУСНДокумент");
	КонецЕсли;
	
	ДокументыДляОпределенияДаты = Новый Массив;
	Для Каждого Расход Из ТаблицаРасходов Цикл
		Если ЗначениеЗаполнено(Расход.РеквизитыДокументаОплаты) Тогда
			ДокументыДляОпределенияДаты.Добавить(Расход.РеквизитыДокументаОплаты);
		ИначеЕсли ЗначениеЗаполнено(Расход.РасчетныйДокумент) Тогда
			ДокументыДляОпределенияДаты.Добавить(Расход.РасчетныйДокумент);
		КонецЕсли;
	КонецЦикла;
	ДатыДокументов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ДокументыДляОпределенияДаты, "Дата");
	
	СуществованияПолитик = Новый Соответствие;
	ПорядкиПризнанияРасходов = Новый Соответствие;
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		Если ЗначениеЗаполнено(Расход.РеквизитыДокументаОплаты) Тогда
			ДатаАнализируемогоДокумента = ДатыДокументов[Расход.РеквизитыДокументаОплаты];
			Отказ = (ДатаАнализируемогоДокумента = Неопределено); // если по "битой" ссылке не найден документ
			Если Отказ Тогда
				ДокументСсылка = Неопределено;
				ДокументМетаданные = ?(СтруктураШапкиДокумента.Свойство("Ссылка", ДокументСсылка), ДокументСсылка.Метаданные(), Неопределено);
				ТекстОшибки = СтрШаблон(НСтр("ru = 'В базе данных отсутствует документ оплаты, на который имеется ссылка
					|%1'"), Расход.РеквизитыДокументаОплаты);
				ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
					УровеньЖурналаРегистрации.Ошибка, ДокументМетаданные, ДокументСсылка, ТекстОшибки);
				ВызватьИсключение ТекстОшибки;
			Иначе
				ДатаАнализируемогоДокумента = КонецМесяца(ДатаАнализируемогоДокумента);
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(Расход.РасчетныйДокумент) Тогда
			ДатаАнализируемогоДокумента = ДатыДокументов[Расход.РасчетныйДокумент];
			Отказ = (ДатаАнализируемогоДокумента = Неопределено); // если по "битой" ссылке не найден документ
			Если Отказ Тогда
				ДокументСсылка = Неопределено;
				ДокументМетаданные = ?(СтруктураШапкиДокумента.Свойство("Ссылка", ДокументСсылка), ДокументСсылка.Метаданные(), Неопределено);
				ТекстОшибки = СтрШаблон(НСтр("ru = 'В базе данных отсутствует расчетный документ, на который имеется ссылка
					|%1'"), Расход.РасчетныйДокумент);
				ЗаписьЖурналаРегистрации(ИмяСобытияЖурналаРегистрации(),
					УровеньЖурналаРегистрации.Ошибка, ДокументМетаданные, ДокументСсылка, ТекстОшибки);
				ВызватьИсключение ТекстОшибки;
			Иначе
				ДатаАнализируемогоДокумента = КонецМесяца(ДатаАнализируемогоДокумента);
				Существование = СуществованияПолитик[ДатаАнализируемогоДокумента];
				Если Существование = Неопределено Тогда
					Существование = УчетнаяПолитика.Существует(СтруктураШапкиДокумента.Организация, ДатаАнализируемогоДокумента);
					СуществованияПолитик.Вставить(ДатаАнализируемогоДокумента, Существование);
				КонецЕсли;
				Отказ = (Не Существование И ДатаПереходаНаУСН < ДатаАнализируемогоДокумента)
					Или ДатаАнализируемогоДокумента > ДатаНачалаПримененияНДС;
			КонецЕсли;
		Иначе
			ДатаАнализируемогоДокумента = Неопределено;
			Отказ = Истина;
		КонецЕсли;
		
		Если Отказ Тогда
			Расход.ВыделятьНДСУСНДокумент = ВыделятьНДСУСН;
		Иначе
			ПорядокПризнания = ПорядкиПризнанияРасходов[ДатаАнализируемогоДокумента];
			Если ПорядокПризнания = Неопределено Тогда
				ПорядокПризнания = УчетнаяПолитика.ПорядокПризнанияРасходовПоНДС(СтруктураШапкиДокумента.Организация, ДатаАнализируемогоДокумента);
				Если ПорядокПризнания = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику
					И УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата) Тогда
					ПорядокПризнания = Перечисления.ПорядокПризнанияРасходовПоНДС.ВключатьВСтоимость;
				КонецЕсли;
				ПорядкиПризнанияРасходов.Вставить(ДатаАнализируемогоДокумента, ПорядокПризнания);
			КонецЕсли;
			Расход.ВыделятьНДСУСНДокумент = (ПорядокПризнания = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику);
		КонецЕсли;
...
Процедура СкоприоватьКолонкиРегистраРасходыУСН(ТаблицаРасходыУСН)
	НаборЗаписей = РегистрыНакопления.РасходыПриУСН.СоздатьНаборЗаписей();
	ТаблицаРасходыУСН = НаборЗаписей.ВыгрузитьКолонки();
КонецПроцедуры

Функция НачисленияПрочиеРасчеты(Организация, Граница)
	...
	|	-ПрочиеРасчетыОстатки.СуммаОстаток КАК Сумма
	|ИЗ
	|	РегистрНакопления.ПрочиеРасчеты.Остатки(&Граница, Организация = &Организация) КАК ПрочиеРасчетыОстатки
	|ГДЕ
	|	ПрочиеРасчетыОстатки.СуммаОстаток < 0";
...
Функция ПриниматьУдержанныйНДФЛкРасходам(Период)
	
	Возврат Период >= '2024-01-01';
	
КонецФункции

// Проверяет, необходимо ли отражать доходы УСН на дату отчета комиссионера.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация, для которой необходимо выполнить проверку
//  Период - Дата - Дата, на которую необходимо проверить
// 
// Возвращаемое значение:
//  Булево - Признак отражения дохода на дату отчета комиссионера
Функция ОтражатьДоходыНаДатуОтчетаКомиссионера(Организация, Период) Экспорт
	
	// При реализации товаров по агентским договорам
	// датой признания дохода является дата поступления денег на счет комиссионера
	// Письмо Минфина России от 01.09.2023 № 03-11-11/83988
	// Письмо ФНС России от 04.08.2017 № СД-4-3/15363@
	//
	// Но также пользователю предоставлена возможность самостоятельно указать порядок признания дохода
	Возврат УчетнаяПолитика.ПризнаватьДоходОтКомиссионераНаДатуОтчетаУСН(Организация, Период);
	
КонецФункции

// Удаляет из массива элементы, типы которых не входят в измерение
//
// Параметры:
//  МассивУсловий  - Массив - массив с документами
//  Измерение      - Строка - имя измерения регистра
//
// Возвращаемое значение:
//  Массив - массив без документов, которых нет в типе измерения
//
Функция ДокументыКорректныхТипов(МассивУсловий, Измерение)
	
	Если ТипЗнч(МассивУсловий) = Тип("Массив") Тогда
		МассивПроверенный = Новый Массив;
		Для Каждого ТекущийДокумент Из МассивУсловий Цикл
			Если Метаданные.РегистрыНакопления.РасходыПриУСН.Измерения[Измерение].Тип.СодержитТип(ТипЗнч(ТекущийДокумент)) Тогда
				МассивПроверенный.Добавить(ТекущийДокумент);
			КонецЕсли;
		КонецЦикла;
	Иначе
		Если Метаданные.РегистрыНакопления.РасходыПриУСН.Измерения[Измерение].Тип.СодержитТип(ТипЗнч(МассивУсловий)) Тогда
			МассивПроверенный = МассивУсловий;
		КонецЕсли;
	КонецЕсли;
	
	Возврат МассивПроверенный;
	
КонецФункции

Функция ИсключаемаяСуммаНДС(СтруктураШапкиДокумента, Ссылка, СуммаДокумента)
	
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
		"Товары, Возвраты, Предоплаты, ПодарочныеСертификаты, СуммаВключаетНДС, Дата");
	
	Товары = ЗначенияРеквизитов.Товары.Выгрузить();
	Возвраты = ЗначенияРеквизитов.Возвраты.Выгрузить();
	Предоплаты = ЗначенияРеквизитов.Предоплаты.Выгрузить();
	ПодарочныеСертификаты = ЗначенияРеквизитов.ПодарочныеСертификаты.Выгрузить();
	
	СтавкаНДС = ПрименяемаяСтавкаНДС(СтруктураШапкиДокумента.Организация, ЗначенияРеквизитов.Дата);
	
	СчетаТоваровКомитентов = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссию);
	
	СуммаПоРеализации = 0;
	СуммаНДСПоРеализации = 0;
	
	Для Каждого СтрокаТовар Из Товары Цикл
		Если СчетаТоваровКомитентов.Найти(СтрокаТовар.СчетУчета) = Неопределено Тогда
			СуммаПоРеализации = СуммаПоРеализации + СтрокаТовар.Сумма;
			СуммаНДСПоРеализации = СуммаНДСПоРеализации + СтрокаТовар.СуммаНДС;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаВозврат Из Возвраты Цикл
		Если СчетаТоваровКомитентов.Найти(СтрокаВозврат.СчетУчета) = Неопределено Тогда
			СуммаПоРеализации = СуммаПоРеализации - СтрокаВозврат.Сумма;
			СуммаНДСПоРеализации = СуммаНДСПоРеализации - СтрокаВозврат.СуммаНДС;
		КонецЕсли;
	КонецЦикла;
	
	СуммаПодарочныеСертификаты = ПодарочныеСертификаты.Итог("Сумма");
	РасчетнаяСуммаНДСПодарочныеСертификаты =
		УчетНДСКлиентСервер.РассчитатьСуммуНДС(СуммаПодарочныеСертификаты, Истина, СтавкаНДС);
	СуммаНДСПодарочныеСертификаты = Окр(РасчетнаяСуммаНДСПодарочныеСертификаты, 2);
	Если Не ЗначенияРеквизитов.СуммаВключаетНДС Тогда
		СуммаПодарочныеСертификаты = СуммаПодарочныеСертификаты - СуммаНДСПодарочныеСертификаты;
	КонецЕсли;
	
	СуммаПоРеализации = СуммаПоРеализации + СуммаПодарочныеСертификаты;
	СуммаНДСПоРеализации = СуммаНДСПоРеализации + СуммаНДСПодарочныеСертификаты;
	
	Для Каждого СтрокаПредоплаты Из Предоплаты Цикл
		СтавкаНДСПредоплаты = УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтрокаПредоплаты.СтавкаНДС);
		
		РасчетнаяСуммаНДС =
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(СтрокаПредоплаты.Сумма, Истина, СтавкаНДСПредоплаты);
		РасчетнаяСуммаНДСВозврат =
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(СтрокаПредоплаты.СуммаВозврат, Истина, СтавкаНДСПредоплаты);
		
		СуммаПоРеализации = СуммаПоРеализации + СтрокаПредоплаты.Сумма - СтрокаПредоплаты.СуммаВозврат;
		СуммаНДСПоРеализации = СуммаНДСПоРеализации + Окр(РасчетнаяСуммаНДС, 2) - Окр(РасчетнаяСуммаНДСВозврат, 2);
		Если Не ЗначенияРеквизитов.СуммаВключаетНДС Тогда
			СуммаПоРеализации = СуммаПоРеализации - Окр(РасчетнаяСуммаНДС, 2) + Окр(РасчетнаяСуммаНДСВозврат, 2);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначенияРеквизитов.СуммаВключаетНДС Тогда
		СуммаПоРеализации = СуммаПоРеализации + СуммаНДСПоРеализации;
	КонецЕсли;
	
	ИсключаемаяСуммаНДС = ?(СуммаПоРеализации = 0, 0, Окр(СуммаНДСПоРеализации * СуммаДокумента / СуммаПоРеализации, 2));
	
	Возврат ИсключаемаяСуммаНДС;
	
КонецФункции

Функция ПрименяемаяСтавкаНДС(Организация, Дата)
	...
	Если ЗначениеЗаполнено(СтавкаНалогообложенияПриУСН) Тогда
		СтавкаНДС = Перечисления.СтавкиНДС.СтавкаНДС(СтавкаНалогообложенияПриУСН, Дата);
	Иначе
...
Процедура ДобавитьСвойстваПоНДС(Параметры)
	
	Если Параметры.Свойство("ДокументПоступленияДата") Тогда
		Дата = Параметры.ДокументПоступленияДата;
	ИначеЕсли Параметры.Свойство("ДокументРеализацииДата") Тогда
		Дата = Параметры.ДокументРеализацииДата;
	Иначе
		Дата = Параметры.Дата;
	КонецЕсли;
	
	Если Не Параметры.Свойство("ПлательщикНДС") Тогда
		ПлательщикНДС = Дата >= УчетУСНКлиентСервер.ДатаНачалаПримененияНДС();
		Параметры.Вставить("ПлательщикНДС", ПлательщикНДС);
	КонецЕсли;
	
	Если Не Параметры.Свойство("ЭтоПлательщикНДСПрименяющийУСН") Тогда
		ЭтоПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Параметры.Организация, Дата);
		Параметры.Вставить("ЭтоПлательщикНДСПрименяющийУСН", ЭтоПлательщикНДСПрименяющийУСН);
	КонецЕсли;
	
	Если Параметры.ЭтоПлательщикНДСПрименяющийУСН Тогда
		СтавкаНДСПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(Параметры.Организация, Дата);
		Параметры.Вставить("СтавкаНДСПриУСН", Перечисления.СтавкиНДС.СтавкаНДС(СтавкаНДСПриУСН, Дата));
	КонецЕсли;
	
	Если Не Параметры.Свойство("ПрименяетсяОбщаяСтавкаНДС") Тогда
		ПрименяетсяОбщаяСтавкаНДС = УчетНДС.ПрименяетсяОбщаяСтавкаНДС(Параметры.Организация, Дата);
		Параметры.Вставить("ПрименяетсяОбщаяСтавкаНДС", ПрименяетсяОбщаяСтавкаНДС);
	КонецЕсли;
	
	Если Не Параметры.Свойство("ПрименяетсяПониженнаяСтавкаНДС") Тогда
		ПрименяетсяПониженнаяСтавкаНДС = УчетНДС.ПрименяетсяПониженнаяСтавкаНДС(Параметры.Организация, Дата);
		Параметры.Вставить("ПрименяетсяПониженнаяСтавкаНДС", ПрименяетсяПониженнаяСтавкаНДС);
	КонецЕсли;
	
	Если Не Параметры.Свойство("РаздельныйУчетНДС") Тогда
		РаздельныйУчетНДС = УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Параметры.Организация, Дата);
		Параметры.Вставить("РаздельныйУчетНДС", РаздельныйУчетНДС);
	КонецЕсли;
	
КонецПроцедуры

Функция НаименованиеКонтрагентаНаПечатьИзКэша(ДоговорКонтрагента, ДатаСведений, КэшНаименованиеКонтрагента)
	
	Если КэшНаименованиеКонтрагента = Неопределено Тогда
		КэшНаименованиеКонтрагента = Новый Соответствие;
	КонецЕсли;
	
	КлючСведений = СтрШаблон("%1_%2",
		Строка(ДоговорКонтрагента.УникальныйИдентификатор()),
		Формат(ДатаСведений, "ДФ=yyyyMMdd; ДП=00010101"));
	
	ИмяНаПечать = КэшНаименованиеКонтрагента[КлючСведений];
	Если ИмяНаПечать = Неопределено Тогда
		Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента, "Владелец");
		ИмяНаПечать = НаименованиеКонтрагентаНаПечать(Контрагент, ДатаСведений);
		
		КэшНаименованиеКонтрагента[КлючСведений] = ИмяНаПечать;
	КонецЕсли;
...
Функция ИсключаемаяСуммаНДСВозвраты(СтруктураШапкиДокумента, СтрокаПлатеж, СторноПринимаемыхДоходов)
	
	ИсключаемаяСуммаНДС = 0;
	ЭтоПлательщикНДСПрименяющийУСН = СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН;
	НужноИсключатьСуммуНДС = ЭтоПлательщикНДСПрименяющийУСН
		И СторноПринимаемыхДоходов <> 0
		И СторноПринимаемыхДоходов <> -СтрокаПлатеж.Разница
		И ЗначениеЗаполнено(СтрокаПлатеж.РасчетныйДокумент)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПлатеж.РасчетныйДокумент, "РасшифровкаПлатежа");
	Если Не НужноИсключатьСуммуНДС Тогда
		Возврат ИсключаемаяСуммаНДС;
	КонецЕсли;
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РасчетныйДокумент,
		"РасшифровкаПлатежа, ВалютаДокумента, Дата");
	РасшифровкаПлатежа = ЗначенияРеквизитов.РасшифровкаПлатежа.Выгрузить();
	Валюта = ЗначенияРеквизитов.ВалютаДокумента;
	ДатаРасчетногоДокумента = ЗначенияРеквизитов.Дата;
	
	ПараметрыОтбора = Новый Структура("ДоговорКонтрагента", ДоговорКонтрагента);
	РасшифровкаПлатежаСОтбором = РасшифровкаПлатежа.Скопировать(ПараметрыОтбора);
	
	Если РасшифровкаПлатежаСОтбором.Количество() > 0 Тогда
		
		СуммаНДС = 0;
		СуммаДокумента = 0;
		
		Для Каждого СтрокаРасшифровки Из РасшифровкаПлатежаСОтбором Цикл
			Если СтрокаРасшифровки.СпособПогашенияЗадолженности <> Перечисления.СпособыПогашенияЗадолженности.ПоДокументу
				Или Не ЗначениеЗаполнено(СтрокаРасшифровки.Сделка)
				Или ЭтоДокументПоступленияАвансаОтПокупателя(СтрокаРасшифровки.Сделка) Тогда
				СуммаНДС = СуммаНДС + СтрокаРасшифровки.СуммаНДС;
				СуммаДокумента = СуммаДокумента + СтрокаРасшифровки.СуммаПлатежа;
			КонецЕсли;
		КонецЦикла;
		
		Если Валюта <> ВалютаРеглУчета Тогда
			
			Курс = ?(РасчетыВУсловныхЕдиницах, 1, РасшифровкаПлатежаСОтбором[0].КурсВзаиморасчетов);
			Кратность = РасшифровкаПлатежаСОтбором[0].КратностьВзаиморасчетов;
			
			ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность");
			ПараметрыТекущегоКурса.Валюта = Валюта;
			ПараметрыТекущегоКурса.Курс = Курс;
			ПараметрыТекущегоКурса.Кратность = Кратность;
			
			ПараметрыНовогоКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаРеглУчета, ДатаРасчетногоДокумента);
			
			СуммаДокумента = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СуммаДокумента,
				ПараметрыТекущегоКурса,
				ПараметрыНовогоКурса);
			СуммаНДС = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СуммаНДС,
				ПараметрыТекущегоКурса,
				ПараметрыНовогоКурса);
			
		КонецЕсли;
		
		КоэффициентРасчетовПоНДС = ?(СуммаДокумента = 0, 0, СторноПринимаемыхДоходов / СуммаДокумента);
		ИсключаемаяСуммаНДС = Окр(КоэффициентРасчетовПоНДС * СуммаНДС, 2);
...
Функция ТаблицаНДСПоВозвратам(СтруктураШапкиДокумента, РасчетныеДокументы)
	
	Если Не СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
		Или Не ЗначениеЗаполнено(РасчетныеДокументы) Тогда
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НДСЗаписиКнигиПокупокОбороты.СчетФактура КАК ДокументРасчетов,
	...
	|				И СчетФактура В (&РасчетныеДокументы)) КАК НДСЗаписиКнигиПокупокОбороты";
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции

Функция ЭтоДокументПоступленияАвансаОтПокупателя(Документ)
	
	Возврат ТипЗнч(Документ) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет")
		Или ТипЗнч(Документ) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер")
		Или ТипЗнч(Документ) = Тип("ДокументСсылка.ОплатаПлатежнойКартой");
	
КонецФункции

Функция ЭтоДокументРасчетовСКонтрагентомДоПримененияНДС(Документ)
	
	Если ТипЗнч(Документ) <> Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДатаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "Дата");
	
	Возврат ДатаДокумента < УчетУСНКлиентСервер.ДатаНачалаПримененияНДС();
	
КонецФункции

Функция СпособыУчетаНДСПартийПоДаннымТаблицыСписания(СтруктураШапкиДокумента, ТаблицаДляСписания)
	
	СпособыУчетаНДСПартий = НоваяТаблицаСпособовУчетаНДСПартий();

	ОтборПоВидуРасхода = Новый Структура("ВидРасхода");

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.Номенклатура;
	ТаблицаДляСписанияПоНоменклатуре = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоНоменклатуре.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоНоменклатуре, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоНоменклатуре = СпособыУчетаНДСПартий(
			СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоНоменклатуре, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.Услуги;
	ТаблицаДляСписанияПоУслугам = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоУслугам.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоУслугам, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоУслугам = СпособыУчетаНДСУслуг(СтруктураШапкиДокумента.Организация, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоУслугам, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.ЛизинговыеПлатежи;
	ТаблицаДляСписанияПоЛизинговымПлатежам = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоЛизинговымПлатежам.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоЛизинговымПлатежам, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоЛизинговымПлатежам = СпособыУчетаНДСУслуг(СтруктураШапкиДокумента.Организация, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоЛизинговымПлатежам, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.РБП;
	ТаблицаДляСписанияПоРБП = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоРБП.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоРБП, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоРБП = СпособыУчетаНДСУслуг(СтруктураШапкиДокумента.Организация, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоРБП, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.ОС;
	ТаблицаДляСписанияПоОС = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоОС.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоОС, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоОС = СпособыУчетаНДСУслуг(СтруктураШапкиДокумента.Организация, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоОС, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.НМА;
	ТаблицаДляСписанияПоНМА = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоНМА.Количество() > 0 Тогда
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоНМА, "РасчетныйДокумент", Истина);

		СпособыУчетаНДСПоНМА = СпособыУчетаНДСУслуг(СтруктураШапкиДокумента.Организация, Партии);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоНМА, СпособыУчетаНДСПартий);
	КонецЕсли;

	ОтборПоВидуРасхода.ВидРасхода = Перечисления.ВидыРасходовУСН.ДопРасходы;
	ТаблицаДляСписанияПоДопРасходам = ТаблицаДляСписания.Скопировать(ОтборПоВидуРасхода);

	Если ТаблицаДляСписанияПоДопРасходам.Количество() > 0 Тогда
		ПартииДопРасходов = ОбщегоНазначения.ВыгрузитьКолонку(
			ТаблицаДляСписанияПоДопРасходам, "РасчетныйДокумент", Истина);
		Партии = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДляСписанияПоДопРасходам, "Партия", Истина);

		СпособыУчетаНДСПоДопРасходам = СпособыУчетаНДСПартийДополнительныхРасходов(
				СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата, Партии, ПартииДопРасходов);

		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СпособыУчетаНДСПоДопРасходам, СпособыУчетаНДСПартий);
	КонецЕсли;
	
	СпособыУчетаНДСПартий.Свернуть("СчетУчета, Номенклатура, Партия, СпособУчетаНДС", "НДС");
	
	Возврат СпособыУчетаНДСПартий;
	
КонецФункции

Функция СпособыУчетаНДСПартийПоДаннымПоступленияРасходов(ТаблицаПоступленияРасходов, ЭтоКорректировка = Ложь)
	
	СпособыУчетаНДСПартий = НоваяТаблицаСпособовУчетаНДСПартий();
	
	Если Не ЗначениеЗаполнено(ТаблицаПоступленияРасходов) Тогда
		Возврат СпособыУчетаНДСПартий;
	КонецЕсли;
	
	Для Каждого СтрокаПрихода Из ТаблицаПоступленияРасходов Цикл
		
		НоваяСтрокаСпособаУчетаНДС = СпособыУчетаНДСПартий.Добавить();
		НоваяСтрокаСпособаУчетаНДС.Номенклатура = СтрокаПрихода.ЭлементРасхода;
		НоваяСтрокаСпособаУчетаНДС.СчетУчета = СтрокаПрихода.СчетУчета;
		
		Если ЭтоКорректировка Тогда
			НоваяСтрокаСпособаУчетаНДС.Партия = СтрокаПрихода.Партия;
			НоваяСтрокаСпособаУчетаНДС.НДС = ?(СтрокаПрихода.НДС < 0, -СтрокаПрихода.НДС, СтрокаПрихода.НДС);
			НоваяСтрокаСпособаУчетаНДС.СпособУчетаНДС = СтрокаПрихода.СпособУчетаНДС;
		Иначе
			НоваяСтрокаСпособаУчетаНДС.Партия = СтрокаПрихода.РасчетныйДокумент;
			НоваяСтрокаСпособаУчетаНДС.НДС = СтрокаПрихода.НДС;
			
			Если СтрокаПрихода.ИсключатьНДС Тогда
				НоваяСтрокаСпособаУчетаНДС.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПринимаетсяКВычету;
			Иначе
				НоваяСтрокаСпособаУчетаНДС.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
			КонецЕсли;
		КонецЕсли;
...
Функция СпособыУчетаНДСПартий(Организация, Период, Партии)
	...
	|				Организация = &Организация
	|					И Партия В (&Партии)
	|					И СпособУчетаНДС <> ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределен)) КАК НДСРаздельныйУчетОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА НДСРаздельныйУчет.СпособУчетаНДСПриВыбытии = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПустаяСсылка)
	|				ТОГДА НДСРаздельныйУчет.СпособУчетаНДС
	|			ИНАЧЕ НДСРаздельныйУчет.СпособУчетаНДСПриВыбытии
	|		КОНЕЦ,
	|		НДСРаздельныйУчет.НДС
	|	ИЗ
	|		РегистрНакопления.НДСРаздельныйУчет КАК НДСРаздельныйУчет
	|	ГДЕ
	|		НДСРаздельныйУчет.Организация = &Организация
	|		И НДСРаздельныйУчет.Партия В(&Партии)
	|		И НДСРаздельныйУчет.Период <= &Период
	|		И НДСРаздельныйУчет.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		И НЕ НДСРаздельныйУчет.Регистратор ССЫЛКА Документ.ПеремещениеТоваров
	|		И НЕ(НДСРаздельныйУчет.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	|					И ВЫРАЗИТЬ(НДСРаздельныйУчет.Регистратор КАК Документ.РеализацияТоваровУслуг).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРеализацияТоваров.ПередачаТоваров))
	|		И НДСРаздельныйУчет.СпособУчетаНДС <> ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределяется)
	|		И НДСРаздельныйУчет.Активность) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.АналитикаУчетаЗатрат,
	|	ВложенныйЗапрос.Партия,
	|	ВложенныйЗапрос.СпособУчетаНДС
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РегистрАналитикаУчетаЗатрат.СчетЗатрат КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА РегистрАналитикаУчетаЗатрат.Субконто1 ССЫЛКА Справочник.Номенклатура
	|			ТОГДА РегистрАналитикаУчетаЗатрат.Субконто1
	|		КОГДА РегистрАналитикаУчетаЗатрат.Субконто2 ССЫЛКА Справочник.Номенклатура
	|			ТОГДА РегистрАналитикаУчетаЗатрат.Субконто2
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	КОНЕЦ КАК Номенклатура,
	|	ВТСпособыУчетаНДС.Партия КАК Партия,
	|	ВТСпособыУчетаНДС.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ВТСпособыУчетаНДС.НДС КАК НДС
	|ИЗ
	|	ВТСпособыУчетаНДС КАК ВТСпособыУчетаНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаЗатрат КАК РегистрАналитикаУчетаЗатрат
	|		ПО ВТСпособыУчетаНДС.АналитикаУчетаЗатрат = РегистрАналитикаУчетаЗатрат.КлючАналитики
	|
	|УПОРЯДОЧИТЬ ПО
	|	СпособУчетаНДС УБЫВ
	|;
	...
	|	НДСРаздельныйУчет.НДС КАК НДС,
	|	ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаНДС, 0) + ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаЕНВД, 0) + ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаБезНДС, 0) + ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаНДС0, 0) КАК ВыручкаВсего,
	|	ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаНДС, 0) + ЕСТЬNULL(БазаРаспределенияНДС.ВыручкаНДС0, 0) КАК ВыручкаСНДС
	|ИЗ
	|	РегистрНакопления.НДСРаздельныйУчет КАК НДСРаздельныйУчет
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.БазаРаспределенияНДС КАК БазаРаспределенияНДС
	|		ПО (НАЧАЛОПЕРИОДА(НДСРаздельныйУчет.Период, КВАРТАЛ) = БазаРаспределенияНДС.Период)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаЗатрат КАК РегистрАналитикаУчетаЗатрат
	|		ПО НДСРаздельныйУчет.АналитикаУчетаЗатрат = РегистрАналитикаУчетаЗатрат.КлючАналитики
	|ГДЕ
	|	НДСРаздельныйУчет.Организация = &Организация
	|	И НДСРаздельныйУчет.Партия В(&Партии)
	|	И НДСРаздельныйУчет.Период <= &Период
	|	И НДСРаздельныйУчет.СпособУчетаНДС = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределен)
	|	И НДСРаздельныйУчет.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И НДСРаздельныйУчет.Активность";
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	СпособыУчетаНДС = РезультатЗапроса[1].Выгрузить();
	РаспределенныйНДС = РезультатЗапроса[2].Выгрузить();
	
	Для Каждого СтрокаРаспределенныйНДС Из РаспределенныйНДС Цикл
		
		Если СтрокаРаспределенныйНДС.НДС <= 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НДСРаспределенный = СтрокаРаспределенныйНДС.НДС;
		НДСПринимаемыйКВычету = 0;
		НДСУчитываемыйВСтоимости = 0;
		
		// Если в базе распределения выручка равна 0, то весь НДС отнесем к деятельности с НДС
		Если СтрокаРаспределенныйНДС.ВыручкаВсего = 0 Тогда
			НДСПринимаемыйКВычету = НДСРаспределенный;
		Иначе
			НДСПринимаемыйКВычету = Окр(НДСРаспределенный * СтрокаРаспределенныйНДС.ВыручкаСНДС / СтрокаРаспределенныйНДС.ВыручкаВсего, 2);
			НДСУчитываемыйВСтоимости = НДСРаспределенный - НДСПринимаемыйКВычету;
		КонецЕсли;
		
		Если НДСПринимаемыйКВычету > 0 Тогда
			НоваяСтрокаСпособаУчетаНДС = СпособыУчетаНДС.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаСпособаУчетаНДС, СтрокаРаспределенныйНДС);
			НоваяСтрокаСпособаУчетаНДС.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПринимаетсяКВычету;
			НоваяСтрокаСпособаУчетаНДС.НДС = НДСПринимаемыйКВычету;
		КонецЕсли;
		
		Если НДСУчитываемыйВСтоимости > 0 Тогда
			НоваяСтрокаСпособаУчетаНДС = СпособыУчетаНДС.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаСпособаУчетаНДС, СтрокаРаспределенныйНДС);
			НоваяСтрокаСпособаУчетаНДС.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
			НоваяСтрокаСпособаУчетаНДС.НДС = НДСУчитываемыйВСтоимости;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат СпособыУчетаНДС;
	
КонецФункции

Функция СпособыУчетаНДСУслуг(Организация, РасчетныеДокументы)
	...
	|	РасходыПриУСН.ЭлементРасхода КАК Номенклатура,
	|	РасходыПриУСН.РасчетныйДокумент КАК Партия,
	|	ВЫБОР
	|		КОГДА РасходыПриУСН.ИсключатьНДС
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПринимаетсяКВычету)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.УчитываетсяВCтоимости)
	...
	|	И РасходыПриУСН.РасчетныйДокумент В (&РасчетныеДокументы)
	|	И (РасходыПриУСН.СтатусыОплатыРасходовУСН = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеОплачено)
	|			ИЛИ РасходыПриУСН.СтатусыОплатыРасходовУСН = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписаноНеОплачено))
...
Функция СпособыУчетаНДСПартийДополнительныхРасходов(Организация, Период, Партии, ПартииДополнильныхРасходов)
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Партии",      Партии);
	Запрос.УстановитьПараметр("ПартииДополнительныхРасходов", ПартииДополнильныхРасходов);
	...
	|ВЫБРАТЬ
	|	РегистрАналитикаУчетаЗатрат.СчетЗатрат КАК СчетУчета,
	|	РегистрАналитикаУчетаЗатрат.Субконто1 КАК Номенклатура,
	|	РегистрАналитикаУчетаНДС.СчетФактура КАК Партия,
	|	ВТСпособыУчетаНДС.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ВТСпособыУчетаНДС.НДС КАК НДС
	|ИЗ
	|	ВТСпособыУчетаНДС КАК ВТСпособыУчетаНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаЗатрат КАК РегистрАналитикаУчетаЗатрат
	|		ПО ВТСпособыУчетаНДС.АналитикаУчетаЗатрат = РегистрАналитикаУчетаЗатрат.КлючАналитики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНДС КАК РегистрАналитикаУчетаНДС
	|		ПО ВТСпособыУчетаНДС.АналитикаУчетаНДС = РегистрАналитикаУчетаНДС.КлючАналитики
	|ГДЕ
	|	РегистрАналитикаУчетаНДС.СчетФактура В(&ПартииДополнительныхРасходов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	СпособУчетаНДС УБЫВ";
...
Функция СпособыУчетаНДСПартийДополнительныхРасходовСписание(Организация, НачалоПериода, КонецПериода, Партии, ПартииДополнильныхРасходов)
...
Функция НоваяТаблицаСпособовУчетаНДСПартий()
	
	ТипЭлементаРасходов = Метаданные.РегистрыНакопления.РасходыПриУСН.Измерения.ЭлементРасхода.Тип;
	
	СпособыУчетаНДСПартий = Новый ТаблицаЗначений;
	СпособыУчетаНДСПартий.Колонки.Добавить("СчетУчета", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	СпособыУчетаНДСПартий.Колонки.Добавить("Номенклатура", ТипЭлементаРасходов);
	СпособыУчетаНДСПартий.Колонки.Добавить("Партия", Документы.ТипВсеСсылки());
	СпособыУчетаНДСПартий.Колонки.Добавить("СпособУчетаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СпособыУчетаНДС"));
	СпособыУчетаНДСПартий.Колонки.Добавить("НДС", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	
	Возврат СпособыУчетаНДСПартий;
	
КонецФункции

Процедура ИсключитьИзКУДиРНДС_ТС(ДвиженияРегистров)
	
	КУДиР = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДвиженияРегистров, "КУДиР");
	Если КУДиР = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Отбор = Новый Структура("ВидРасхода, ИсключатьНДС");
	Отбор.ВидРасхода = Перечисления.ВидыРасходовУСН.НДС_ТС;
	Отбор.ИсключатьНДС = Истина;
	
	СтрокиДляУдаления = КУДиР.НайтиСтроки(Отбор);
	Для Каждого СтрокаДляУдаления Из СтрокиДляУдаления Цикл
		КУДиР.Удалить(СтрокаДляУдаления);
	КонецЦикла;
	
КонецПроцедуры

// Проверяет необходимость признавать доход с процентов по вкладам.
// С 2026 года проценты по вкладам не являются доходом УСН для предпринимателей (425-ФЗ от 28.11.25).
//
// Параметры:
//  Организация         - СправочникСсылка.Организации - Организация для проверки
//  Дата                - Дата - Дата документа
//  СтатьяПрочихДоходов - СправочникСсылка.ПрочиеДоходыИРасходы - статья прочих доходов и расходов
// 
// Возвращаемое значение:
//  Булево - истина, если доход признается
//
Функция ПризнаватьДоходомПроцентыПоВкладам(Организация, Дата, СтатьяПрочихДоходов) Экспорт
	
	Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ДатаОтменыПризнания = Дата(2026, 01, 01);
	СтатьяДоходаПроценты =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.ПроцентыКПолучению");
	
	Если Не ЗначениеЗаполнено(СтатьяДоходаПроценты) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ПризнаватьДоход = Не (Дата >= ДатаОтменыПризнания И СтатьяПрочихДоходов = СтатьяДоходаПроценты);
	
	Возврат ПризнаватьДоход;
	
КонецФункции

// Проверяет, необходимо ли исключать из дохода УСН НДС по авансам, полученным в период освобождения от уплаты НДС.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация, для которой необходимо выполнить проверку
//  Период - Дата - Дата, на которую необходимо проверить
// 
// Возвращаемое значение:
//  Булево - Признак исключения
Функция ИсключатьНДСИзДоходаПоАвансамБезНДС(Организация, Период)
	
	Возврат УчетнаяПолитика.ИсключатьНДСИзДоходаПоАвансамБезНДС(Организация, Период);
