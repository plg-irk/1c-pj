...
// Смотреть одноименную функцию в общем модуле ПользователиКлиентСервер.
...
Функция ЭтоПолноправныйПользователь(Пользователь = Неопределено,
	...
	Если Не СвойстваПользователяИБ.УказанТекущийПользовательИБ Тогда
		...
		Если ПроверитьРольАдминистратораСистемы
		   И Не Роли.Содержит(РольАдминистратораСистемы(Истина)) Тогда
			Возврат Ложь;
		КонецЕсли;
...
//  ОбработатьИерархиюВторогоИТретьегоУровней - Булево - необязательный, признак обработки вложенности 
...
Функция СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	// +Рарус
	Если ПользовательИБ = Неопределено ИЛИ ПользовательИБ.Имя = "" Тогда // -РАРУС
	...
	Если НЕ ЗначениеЗаполнено(Пользователь) Тогда
		...
		Пользователь.ПодразделениеКомпании = Справочники.ПодразделенияКомпании.ОсновноеПодразделение;
		Пользователь.Организация           = Справочники.Организации          .ОсновнаяОрганизация;
		// -РАРУС
	...
	// +Рарус Иначе происходит двойная запись объекта
	Пользователь.ДополнительныеСвойства.Вставить("ПроверкаИзмененияОбъектовПараметровУчета", Ложь);
	// -Рарус
	...
	// если создан администратор
	Если ПользовательСоздан Тогда
		Администраторы = Справочники.ГруппыДоступа.Администраторы.ПолучитьОбъект();
		Администратор = Администраторы.Пользователи.Добавить();
		Администратор.Пользователь = Пользователь.Ссылка;
		Администраторы.ОбменДанными.Загрузка = Истина;
		Администраторы.Записать();
	КонецЕсли;
