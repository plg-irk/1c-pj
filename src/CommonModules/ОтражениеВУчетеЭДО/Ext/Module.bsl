		...
		Если ЕстьРаботаСНоменклатурой И ЗначениеЗаполнено(НоменклатураКонтрагента.ИдентификаторНоменклатурыСервиса) Тогда
			...
			Если ЗначениеЗаполнено(СтрокиДанныхСервиса) Тогда
				...
				ДополнительныеПараметры = 
					СопоставлениеНоменклатурыКонтрагентов.НовыеДополнительныеПараметрыПриЗаписиНоменклатурыКонтрагентов();
				ДополнительныеПараметры.ИспользоватьХарактеристики = ИспользоватьХарактеристики;
				Отказ = Ложь;
...
Функция ВключитьКонтрольСопоставленияНоменклатуры(Знач ЭлектронныйДокумент, Знач НоменклатураБезСопоставления) Экспорт
	...
	Попытка
		...
		Для Каждого НоменклатураКонтрагента Из НоменклатураБезСопоставления Цикл
			...
			СопоставлениеНоменклатурыКонтрагентов.СоздатьОбновитьНоменклатуруКонтрагента(
				НоменклатураКонтрагента,
				СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураКонтрагентаИнформационнойБазы(),
				Отказ);
...
Процедура ОтразитьДанныеФормализованногоЭлектронногоДокумента(ДанныеЭлектронногоДокумента, ДанныеДляОтражения, СпособОбработки, ДокументыУчета = Неопределено)
	...
	ИспользуетсяДокументУчета = Ложь;
	ЗаполнитьДокументыУчета = Истина;
	ДокументУчета = Неопределено;
	РезультатОтраженияВУчете = НовыйРезультатОтраженияВУчете();
	...
	ВерсияФормата = НовыйЭД.ВерсияФормата;
	
	ПараметрыДляОтраженияВУчете = НовыеПараметрыДляОтраженияВУчете();
	
	ПараметрыЭлектронногоДокумента = ПараметрыДляОтраженияВУчете.ПараметрыЭлектронногоДокумента;
	ПараметрыЭлектронногоДокумента.ТипДокумента = ДанныеЭлектронногоДокумента.ТипДокумента;
	ПараметрыЭлектронногоДокумента.ФорматДокумента = ВерсияФормата;
	ПараметрыЭлектронногоДокумента.ДанныеОсновногоФайла = ДанныеЭлектронногоДокумента.ДанныеОсновногоФайла;
	
	ПараметрыСпособаОбмена = ПараметрыДляОтраженияВУчете.ПараметрыСпособаОбмена;
	ПараметрыСпособаОбмена.СпособОбмена = ДанныеЭлектронногоДокумента.СпособОбмена;
	ПараметрыСпособаОбмена.ДанныеСпособаОбмена = ДанныеЭлектронногоДокумента.ДополнительныеДанные;
	
	ОбработчикиОтражения = ОбработчикиПриОтраженииЭлектронногоДокумента();
	ОбменСКонтрагентамиПереопределяемый.ПриОпределенииОбработчикаПриОтраженииЭлектронногоДокумента(ОбработчикиОтражения);
	...
	Попытка
		...
		Если ДанныеДляОтражения.ДанныеТЭДО <> Неопределено Тогда
			...
			ИспользуетсяДокументУчета = Истина;
			ТрансграничныйЭДО.ПодготовитьКОтражениюВУчете(
				ДанныеДляОтражения.ДанныеТЭДО, ДанныеЭлектронногоДокумента.Отправитель);
			ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаТЭДО(ДокументУчета,
				СпособОбработки, ДанныеДляОтражения.ДанныеТЭДО, РезультатОтраженияВУчете.ОписаниеОшибки);
		...
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.ПередачаТоваров.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаПередачаТоваров Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаПередачаТоваров(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьДокументПередачаРезультатовРабот(ДеревоЭД,
					ДокументУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
				
				ИспользуетсяДокументУчета = Истина;
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.ПередачаРабот.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаПередачаРезультатовРабот Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаПередачаРезультатовРабот(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьДокументПередачаРезультатовРабот(ДеревоЭД, ДокументУчета,
					СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
					
				ИспользуетсяДокументУчета = Истина;
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УПД.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2016 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2016(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйПередаточныйДокумент(ДеревоЭД,
					РезультатОтраженияВУчете.ДокументыУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УПД2019.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2019 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2019(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйПередаточныйДокумент_2019(ДеревоЭД,
					РезультатОтраженияВУчете.ДокументыУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_02 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_02(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйПередаточныйДокумент_5_02(ДеревоЭД,
					РезультатОтраженияВУчете.ДокументыУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УПД_5_03.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_03 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_03(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйПередаточныйДокумент_5_03(ДеревоЭД,
					РезультатОтраженияВУчете.ДокументыУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;
			
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2020 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2020(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУКД_2020(ДеревоЭД, РезультатОтраженияВУчете.ДокументыУчета,
					СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.УКД.ИнформацияПродавца Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2016 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2016(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьУниверсальныйКорректировочныйДокумент(ДеревоЭД,
					РезультатОтраженияВУчете.ДокументыУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
			КонецЕсли;
			
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.АктОРасхождениях.ИнформацияПокупателя Тогда
			
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаАктОРасхождениях_ФНС_2019 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
			
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаАктОРасхождениях_ФНС_2019(
				РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьАктОРасхождениях_ФНС_2019(ДеревоЭД, ДокументУчета,
					СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
				
				РезультатОтраженияВУчете.ДокументыУчета = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументУчета);
				
			КонецЕсли;
			
			ЗаполнитьАктОРасхожденияхПоДополнительнымСведениям(РезультатОтраженияВУчете, ДанныеЭлектронногоДокумента.ДанныеДополнительногоТитула);
			
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.АктСверкиВзаиморасчетов.ИнформацияОтправителя Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаАктСверкиВзаиморасчетов Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаАктСверкиВзаиморасчетов(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьАктСверкиВзаиморасчетов(ДеревоЭД, ДокументУчета,
					СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
				ИспользуетсяДокументУчета = Истина;
			КонецЕсли;

		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.СчетНаОплату101 Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаСчетНаОплату101 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаСчетНаОплату101(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьСчетНаОплату101(ДеревоЭД, ДокументУчета, СпособОбработки,
					РезультатОтраженияВУчете.ОписаниеОшибки);
				ИспользуетсяДокументУчета = Истина;
			КонецЕсли;
		ИначеЕсли НовыйЭД.ВерсияФормата = ПоддерживаемыеФорматы.ФНС.СчетНаОплату_5_01 Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаСчетНаОплату_5_01 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаСчетНаОплату_5_01(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
		...
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.АктПриемкиСтроительныхРаботУслуг.ИнформацияПодрядчика Тогда
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаАктОПриемкеСтроительныхРаботУслуг Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаАктОПриемкеСтроительныхРаботУслуг(
					РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
			Иначе
		...
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.ДоговорныйДокумент101 Тогда
			...
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДоговорногоДокумента101 Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);

				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДоговорногоДокумента101(
					РезультатОтраженияВУчете, КонструкторЭД, ПараметрыДляОтраженияВУчете);
			Иначе
		...
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ФНС.ДоговорныйДокументXML.ИнформацияОтправителя Тогда
			...
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДоговорногоДокументаXML Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);

				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДоговорногоДокументаXML(
					РезультатОтраженияВУчете, ОбъектXDTO, ПараметрыДляОтраженияВУчете);
			Иначе
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьДоговорнойДокумент101(КонструкторЭД,
					ДокументУчета, СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);
				ИспользуетсяДокументУчета = Истина;
		...
		ИначеЕсли ВерсияФормата = ПоддерживаемыеФорматы.ДокументПредприятия Тогда
			КонструкторЭД = НовыйЭД.ЗначениеРеквизита;
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаПредприятия Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);

				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаПредприятия(
					РезультатОтраженияВУчете, КонструкторЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ПараметрыОбработки = Новый Структура("СпособОбработки", СпособОбработки);
				ОбменСКонтрагентамиПереопределяемый.НайтиСоздатьДокументПредприятия(ДокументУчета, КонструкторЭД,
					ПараметрыОбработки, РезультатОтраженияВУчете.ОписаниеОшибки, ДанныеЭлектронногоДокумента.ДополнительныеДанные);
				ИспользуетсяДокументУчета = Истина;
			КонецЕсли;
		...
		ИначеЕсли НовыйЭД.ВидЭД = Перечисления.ТипыДокументовЭДО.КаталогТоваров
			...
			Если ДокументУчета = Неопределено Тогда
				СвойстваДокумента = ЭлектронныеДокументыЭДО.СвойстваДокумента(
					ДанныеЭлектронногоДокумента.ЭлектронныйДокумент,
					"ВидДокумента, Организация, Контрагент, ДоговорКонтрагента");
				ДокументУчета = НастройкиЭДО.СсылкаНаОбъектНастройкиЭДО(СвойстваДокумента.Организация,
					СвойстваДокумента.Контрагент, СвойстваДокумента.ДоговорКонтрагента);
			КонецЕсли;
		...
		ИначеЕсли ДанныеЭлектронногоДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.ЗапросКоммерческихПредложений Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует(
				...
				МодульЗапросКоммерческогоПредложенияОтКлиента = ОбщегоНазначения.ОбщийМодуль(
					"Документы.ЗапросКоммерческогоПредложенияОтКлиента");
				МодульЗапросКоммерческогоПредложенияОтКлиента.НайтиСоздатьЗапросКоммерческихПредложений(ДеревоЭД,
					ДокументУчета);
				ИспользуетсяДокументУчета = Истина;
			Иначе
				Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаЗапросКоммерческихПредложений Тогда
					ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
					ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
					
					ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаЗапросКоммерческихПредложений(
						РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
				Иначе
		...
		ИначеЕсли ДанныеЭлектронногоДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.КоммерческоеПредложение Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует(
				...
				МодульКоммерческоеПредложениеПоставщика = ОбщегоНазначения.ОбщийМодуль(
				...
				ИспользуетсяДокументУчета = Истина;
			Иначе
				Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаКоммерческоеПредложение Тогда
					ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
					ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
					
					ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаКоммерческоеПредложение(
						РезультатОтраженияВУчете, ДеревоЭД, ПараметрыДляОтраженияВУчете);
				Иначе
		...
		ИначеЕсли ДанныеЭлектронногоДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.МашиночитаемаяДоверенность Тогда

			МашиночитаемыеДоверенности.ОтразитьВУчете(ДанныеЭлектронногоДокумента, РезультатОтраженияВУчете.ДокументыУчета,
				СпособОбработки, РезультатОтраженияВУчете.ОписаниеОшибки);

		ИначеЕсли (МодульОбменСГИСЭПД <> Неопределено И МодульОбменСГИСЭПД.ЭтоДокументЭПД(
			ДанныеЭлектронногоДокумента.ТипДокумента)) Тогда
		...
		Иначе
			...
			Если ОбработчикиОтражения.ПриОтраженииВУчетеДокументаCML Тогда
				ПараметрыПоФорматуДокумента = ПараметрыДляОтраженияВУчете.ПараметрыПоФорматуДокумента;
				ПараметрыПоФорматуДокумента.Вставить("СпособОбработки", СпособОбработки);
				ПараметрыПоФорматуДокумента.Вставить("ДеревоСсылокНаОбъекты", ДеревоРазбора);
				
				ОбменСКонтрагентамиПереопределяемый.ПриОтраженииВУчетеДокументаCML(
					РезультатОтраженияВУчете, НовыйЭД, ПараметрыДляОтраженияВУчете);
			Иначе
				ПараметрыОбработки = ПараметрыОбработкиСохранитьДанныеОбъектаВБД(ДокументУчета, Истина, СпособОбработки);
				
				ОбменСКонтрагентамиПереопределяемый.СохранитьДанныеОбъектаВБД(НовыйЭД,
					ДеревоРазбора, ПараметрыОбработки, ДанныеЭлектронногоДокумента.ДополнительныеДанные,
					ДокументУчета, РезультатОтраженияВУчете.ОписаниеОшибки);
	...
	Если ИспользуетсяДокументУчета Тогда
		Если Не ЗначениеЗаполнено(ДокументУчета) Тогда
			ЗаполнитьДокументыУчета = Ложь;
		КонецЕсли;
		...
		КонецЕсли;
	Иначе
		ДокументыУчета = РезультатОтраженияВУчете.ДокументыУчета;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДокументыУчета) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатОтраженияВУчете.ОписаниеОшибки) Тогда
			ТекстЖурналаРегистрации = РезультатОтраженияВУчете.ОписаниеОшибки;
			ТекстСообщения = ТекстСообщения + Символы.ПС + РезультатОтраженияВУчете.ОписаниеОшибки;
		Иначе
...
// Отражает дополнительные сведения акта о расхождениях в прикладном решение акт.
// 
// Параметры:
//  РезультатОтраженияВУчете - см. НовыйРезультатОтраженияВУчете
//  ДанныеДополнительногоТитула - см. ОтражениеВУчетеЭДОКлиентСервер.НовыеДанныеДополнительногоТитулаДляОтраженияВУчете
//
Процедура ЗаполнитьАктОРасхожденияхПоДополнительнымСведениям(РезультатОтраженияВУчете, ДанныеДополнительногоТитула)
	...
	Если ЗначениеЗаполнено(ДанныеДополнительногоТитула.ОсновнойФайл.ИмяФайла) Тогда
		...
		ДанныеДокумента = ЭлектронныеДокументыЭДО.ДанныеДокументаДляЗагрузкиПросмотра(
					ПараметрыПолученияДополнительногоТитула);
		...
		ОбменСКонтрагентамиПереопределяемый.ЗаполнитьАктОРасхожденияхПоДополнительнымСведениям(
			ДанныеДокумента, РезультатОтраженияВУчете.ДокументыУчета);
	КонецЕсли;
...
// Возвращаемое значение:
//  Структура - параметры для отражения в учете электронного документа:
...
//   * ПараметрыПоФорматуДокумента - Структура - дополнительные параметры по формату документа для отражения в учете.
//      Указываются индивидуально для каждого формата ЭД в описание процедуры при отражении в учете.
Функция НовыеПараметрыДляОтраженияВУчете()
	
	Результат = Новый Структура;
	Результат.Вставить("ПараметрыЭлектронногоДокумента", НовыеПараметрыЭлектронногоДокумента());
	Результат.Вставить("ПараметрыСпособаОбмена", НовыеПараметрыСпособаОбмена());
	Результат.Вставить("ПараметрыПоФорматуДокумента", Новый Структура);
	
	Возврат Результат;
	
КонецФункции

#Область КонструкторыДляЗагрузкиЭД

// Описание конструктора результата отражения электронного документа в учете.
//
// Возвращаемое значение:
//  Структура - возвращаемые данные после отражения в учете электронного документа:
//   * ДокументыУчета - Массив из ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО - ссылки на созданные или найденные
//                      учетные документы.
//   * ОписаниеОшибки - Строка - описание ошибки при работе с учетным документом.
Функция НовыйРезультатОтраженияВУчете() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ДокументыУчета", Новый Массив);
...
// Описание конструктора параметров электронного документа при отражении документа в учете.
//
// Возвращаемое значение:
//  Структура - паар для заполнения электронного документа:
//  * ТипДокумента - ПеречислениеСсылка.ТипыДокументовЭДО - используется для определения типа пришедшего документа.
//  * ФорматДокумента - Строка - формат электронного документа, используется для определения свойств заполнения
//                       данных электронного документа, см. ОбменСКонтрагентами.ФорматыДокументов .
...
// Описание конструктора параметров способа обмена загрузки электронных документов в учет.
//
// Возвращаемое значение:
//  Структура - параметры способа обмена:
//  * СпособОбмена - ПеречислениеСсылка.СпособыОбменаЭД - способ обмена электронным документов.
//  * ДанныеСпособаОбмена - Неопределено, Произвольный - дополнительные данные для отражения в учете. Заполняются другими подсистемами
//     из состава библиотеки. Состав данных должен быть описан в документации соответствующей подсистемы.
//     Для способа обмена "Перечисления.СпособыОбменаЭД.Через1CShare" см ИнтеграцияShare.НовыеПараметрыДляЗагрузкиДокументаВОбъектУчета.
//     В случае отсутствия дополнительных данных передается Неопределено.
Функция НовыеПараметрыСпособаОбмена() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("СпособОбмена", Перечисления.СпособыОбменаЭД.ПустаяСсылка());
	Результат.Вставить("ДанныеСпособаОбмена", Неопределено);
...
// Возвращаемое значение:
//  Структура из КлючИЗначение - описание используемых процедур:
//   * Ключ - Строка - имя процедуры, поддержанная вместо устаревшей.
//   * Значение - Булево - признак использования процедуры.
//
Функция ОбработчикиПриОтраженииЭлектронногоДокумента()
	
	ОбработчикиОтражения = Новый Структура;
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2020", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоКорректировочногоДокумента_2016", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_02", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_5_03", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2019", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеУниверсальногоПередаточногоДокумента_2016", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаПередачаТоваров", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаПередачаРезультатовРабот", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаАктОРасхождениях_ФНС_2019", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаАктОПриемкеСтроительныхРаботУслуг", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаАктСверкиВзаиморасчетов", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаСчетНаОплату101", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаСчетНаОплату_5_01", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДоговорногоДокумента101", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДоговорногоДокументаXML", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаCML", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаЗапросКоммерческихПредложений", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаКоммерческоеПредложение", Ложь);
	ОбработчикиОтражения.Вставить("ПриОтраженииВУчетеДокументаПредприятия", Ложь);
	
	Возврат ОбработчикиОтражения;
