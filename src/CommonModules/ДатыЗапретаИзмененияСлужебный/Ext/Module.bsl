...
//   2. Процедура УстановитьРежимВыводаВПанеляхОтчетов позволяет настроить
//      режим группировки вариантов в панелях отчетов:
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, НастройкиОтчета, Истина/Ложь);
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, Метаданные.Отчеты.<ИмяОтчета>, Истина/Ложь);
//       ВариантыОтчетов.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, Метаданные.Подсистемы.<ИмяПодсистемы>, 
//                                                            Истина/Ложь);
...
Функция ДатаЗапретаПоОписанию(ОписаниеДатыЗапрета, ДатаЗапрета, НачалоДня = '00010101') Экспорт
	...
	ИначеЕсли ВариантДатыЗапрета = "ТекущаяДата" Тогда
		ТекущаяДатаЗапрета    = НачалоДня(НачалоДня) - КоличествоДнейРазрешения * Сутки;
		ПредыдущаяДатаЗапрета = НачалоДня(ТекущаяДатаЗапрета) - КоличествоДнейРазрешения * Сутки;;
	Иначе
		Возврат '00030303'; // Неизвестный формат.
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекущаяДатаЗапрета) Тогда
		Если НЕ ВариантДатыЗапрета = "ТекущаяДата" Тогда
...
Процедура ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(Источник,
		Отказ, ИсточникРегистр = Ложь, Замещение = Ложь, Удаление = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
...
Функция ПропуститьПроверкуДатЗапрета(Объект,
	...
	Если ТипЗнч(Объект) <> Тип("УдалениеОбъекта")
	    // +РАРУС
	    И НЕ ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект))
	    // -РАРУС
