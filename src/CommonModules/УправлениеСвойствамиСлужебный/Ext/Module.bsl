...
Функция ЗначенияСвойств(ДополнительныеСвойстваОбъекта, Наборы, ЭтоДополнительноеСведение) Экспорт
	...
	Если ДополнительныеСвойстваОбъекта.Количество() = 0 Тогда
		...
		Если СвойстваНеНайдены Тогда
			...
			// +РАРУС
			ОписаниеСвойств.Колонки.Добавить("ТолькоДляПечати");
			// -РАРУС
	...
	// +РАРУС
	|	СвойстваНаборов.Свойство.ТолькоДляПечати КАК ТолькоДляПечати,
	// -РАРУС
	...
	|	СвойстваНаборов.Свойство,
	// +РАРУС
	|	СвойстваНаборов.ТолькоДляПечати,
	// -РАРУС
	...
	// +РАРУС
	|	ВсеСвойства.Свойство.Заголовок КАК СвойствоЗаголовок,
	|	ВсеСвойства.Свойство.Подсказка КАК СвойствоПодсказка,
	|	ВсеСвойства.Свойство.МногострочноеПолеВвода КАК СвойствоМногострочноеПолеВвода,
	|	ВсеСвойства.ТолькоДляПечати,
	// -РАРУС
...
Функция ВидыСвойствНабора(Ссылка, УчитыватьПометкуУдаления = Истина) Экспорт
	...
	ВидыСвойствНабора.Вставить("ДополнительныеРеквизиты", УправлениеСвойствами.ИспользоватьДопРеквизиты(ПустаяСсылка));
	
	// Проверка использования дополнительных сведений.
	ВидыСвойствНабора.Вставить("ДополнительныеСведения", УправлениеСвойствами.ИспользоватьДопСведения(ПустаяСсылка));
...
//  
// +Рарус экспортная процедура, нужна для получение реквизитов для печати
Функция ПолучитьОсновнойНаборСвойствДляОбъекта(ВладелецСвойств) Экспорт // -РАРУС
	...
	// +РАРУС Удалена проверка является ли "ВладелецСвойств" группой
