...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	// +РАРУС //!!! Вынести в ОбщегоНазначенияКлиентПереопределяемый
	Если ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована") Тогда
		Если ПараметрыКлиента.РаботаСВнешнимиРесурсамиЗаблокирована Тогда
...
//  ТекстПредупреждения  - Строка - текст предупреждения, который будет показан пользователю.
//
...
//	ОбработчикЗавершения		- ОписаниеОповещения - Описание процедуры, которая будет вызвана
//														после завершения показа (со значением Неопределено).
//
...
//														Если указано Неопределено, то строки дерева развернуты не будут. Значение по умолчанию: "*".
...
Процедура ДействияПередНачаломРаботыСистемыПослеВосстановленияСвязиСГлавнымУзлом(Неопределен, Параметры) Экспорт
	...
	// +РАРУС
	Параметры.Вставить("ОбработкаПродолжения", Новый ОписаниеОповещения(
		"ПослеПроверкиЛегальностиПолученияОбновления", ЭтотОбъект, Параметры));
	// -РАРУС
...
// +РАРУС Только для внутреннего использования. Продолжение процедуры ПередНачаломРаботыСистемы.
Процедура ПослеПроверкиЛегальностиПолученияОбновления(Неопределен, Параметры) Экспорт
	...
	Параметры.Вставить("ОбработкаПродолжения", Новый ОписаниеОповещения(
		"ПослеПроверкиСистемыЗащиты", ЭтотОбъект, Параметры));
	
	Попытка
		
		// Выполним проверку доступности ключа
		
		ЛицензированиеСлужебныйКлиент.ПередНачаломРаботыСистемы(Параметры);
...
КонецПроцедуры // ПослеПроверкиЛегальностиПолученияОбновления() // -РАРУС


// +РАРУС Только для внутреннего использования. Продолжение процедуры ПередНачаломРаботыСистемы.
Процедура ПослеПроверкиСистемыЗащиты(Неопределен, Параметры) Экспорт
	...
	Попытка
		...
		// Выполним загрузку подготовленных данных переноса, после перехода на новую редакцию.
		ТехнологическаяПлатформаКлиент.ПроверитьНеобходимостьПереносаДанныхИзПредыдущейРедакции(Параметры);
