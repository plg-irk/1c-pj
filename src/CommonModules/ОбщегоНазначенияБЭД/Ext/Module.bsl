...
// Параметры:
//  ПредставлениеДаты - Строка - строка, которую необходимо привести к дате.
//                               Формат даты должен быть в виде "ДД.ММ.ГГГГ" или "ДД/ММ/ГГ" или "ДД-ММ-ГГ ЧЧ:ММ:CC"
//                               См. СтроковыеФункцииКлиентСервер.СтрокаВДату
//                               или иметь текстовый формат (22 апреля 2025, 22_апреля_2025).
//                               Также поддерживаются различные варианты написания месяца, учитывается транслитерация.
// 
// Возвращаемое значение:
// - Неопределено
// - Дата
Функция ДатаПоСтроковомуПредставлению(ПредставлениеДаты) Экспорт
	
	// Текстовый формат (22 апреля 2025, 22_апреля_2025)
	РегулярноеВыражение = "\d{1,2}[\s_]+(?:января|январь|янв|февраля|февраль|февр|марта|март|мар|апреля|апрель|апр|мая|май|"
		+ "июня|июнь|июля|июль|августа|август|авг|сентября|сентябрь|сен|октября|октябрь|окт|ноября|ноябрь|ноя|декабря|декабрь|дек|"
		+ "janvarja|janvarya|janv|fevralja|fevralya|fevr|marta|mart|mar|aprelja|aprelya|apr|maja|maya|may|ijunja|iyunya|ijun|iyun|"
		+ "ijulja|iyulya|ijul|iyul|avgusta|augusta|avg|aug|sentjabrja|sentyabrya|sent|oktjabrja|oktyabrya|okt|"
		+ "nojahrja|noyabrya|noj|noy|dekabrja|dekabrya|dek)[\s_]+\d{4}";
	РезультатПоискаДаты = СтрНайтиПоРегулярномуВыражению(ПредставлениеДаты, РегулярноеВыражение);
	
	Если РезультатПоискаДаты.НачальнаяПозиция = 0 Тогда
		Возврат СтроковыеФункцииКлиентСервер.СтрокаВДату(ПредставлениеДаты);
	КонецЕсли;
	
	НайденнаяДата = РезультатПоискаДаты.Значение;
	
	СимволМеждуЧастямиДаты = " ";
	Если СтрНайти(НайденнаяДата, "_") <> 0 Тогда
		СимволМеждуЧастямиДаты = "_";
	КонецЕсли;
	
	Части = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НайденнаяДата, СимволМеждуЧастямиДаты,
		Истина, Истина);
	...
	Месяцы.Вставить("janvarja", 1);
	Месяцы.Вставить("janvarya", 1);
	Месяцы.Вставить("janv", 1);

	Месяцы.Вставить("февраля", 2);
	Месяцы.Вставить("февраль", 2);
	Месяцы.Вставить("февр", 2);
	Месяцы.Вставить("fevralja", 2);
	Месяцы.Вставить("fevralya", 2);
	Месяцы.Вставить("fevr", 2);

	Месяцы.Вставить("марта", 3);
	Месяцы.Вставить("март", 3);
	Месяцы.Вставить("мар", 3);
	Месяцы.Вставить("marta", 3);
	Месяцы.Вставить("mart", 3);
	Месяцы.Вставить("mar", 3);

	Месяцы.Вставить("апреля", 4);
	Месяцы.Вставить("апрель", 4);
	Месяцы.Вставить("апр", 4);
	Месяцы.Вставить("aprelja", 4);
	Месяцы.Вставить("aprelya", 4);
	Месяцы.Вставить("apr", 4);

	Месяцы.Вставить("мая", 5);
	Месяцы.Вставить("май", 5);
	Месяцы.Вставить("maja", 5);
	Месяцы.Вставить("maya", 5);
	Месяцы.Вставить("may", 5);

	Месяцы.Вставить("июня", 6);
	Месяцы.Вставить("июнь", 6);
	Месяцы.Вставить("ijunja", 6);
	Месяцы.Вставить("iyunya", 6);
	Месяцы.Вставить("ijun", 6);
	Месяцы.Вставить("iyun", 6);

	Месяцы.Вставить("июля", 7);
	Месяцы.Вставить("июль", 7);
	Месяцы.Вставить("ijulja", 7);
	Месяцы.Вставить("iyulya", 7);
	Месяцы.Вставить("ijul", 7);
	Месяцы.Вставить("iyul", 7);

	Месяцы.Вставить("августа", 8);
	Месяцы.Вставить("август", 8);
	Месяцы.Вставить("авг", 8);
	Месяцы.Вставить("avgusta", 8);
	Месяцы.Вставить("augusta", 8);
	Месяцы.Вставить("avg", 8);
	Месяцы.Вставить("aug", 8);

	Месяцы.Вставить("сентября", 9);
	Месяцы.Вставить("сентябрь", 9);
	Месяцы.Вставить("сен", 9);
	Месяцы.Вставить("sentjabrja", 9);
	Месяцы.Вставить("sentyabrya", 9);
	Месяцы.Вставить("sent", 9);

	Месяцы.Вставить("октября", 10);
	Месяцы.Вставить("октябрь", 10);
	Месяцы.Вставить("окт", 10);
	Месяцы.Вставить("oktjabrja", 10);
	Месяцы.Вставить("oktyabrya", 10);
	Месяцы.Вставить("okt", 10);

	Месяцы.Вставить("ноября", 11);
	Месяцы.Вставить("ноябрь", 11);
	Месяцы.Вставить("ноя", 11);
	Месяцы.Вставить("nojahrja", 11);
	Месяцы.Вставить("noyabrya", 11);
	Месяцы.Вставить("noj", 11);
	Месяцы.Вставить("noy", 11);

	Месяцы.Вставить("декабря", 12);
	Месяцы.Вставить("декабрь", 12);
	Месяцы.Вставить("дек", 12);
	Месяцы.Вставить("dekabrja", 12);
	Месяцы.Вставить("dekabrya", 12);
	Месяцы.Вставить("dek", 12);
	
	ОписаниеТипаДата = ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата);
	
	МесяцЧисло = Месяцы[НРег(МесяцТекст)];
	Если МесяцЧисло <> Неопределено И День > 0 И День < 32 И Год > 0 И Год < 3999 Тогда
		ЗначениеДата = Формат(Год, "ЧЦ=4; ЧН=0001; ЧВН=; ЧГ=0")
			+ Формат(МесяцЧисло, "ЧЦ=2; ЧН=01; ЧВН=; ЧГ=0")
			+ Формат(День, "ЧЦ=2; ЧН=01; ЧВН=; ЧГ=0");
		
		Возврат ОписаниеТипаДата.ПривестиЗначение(ЗначениеДата);
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ЭтоРежимЦентрЭДО() Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ЦентрЭДО") Тогда
		МодульЦентрЭДО = ОбщегоНазначения.ОбщийМодуль("ЦентрЭДО");
		Возврат МодульЦентрЭДО.Использовать();
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция КоличествоОрганизаций() Экспорт
	...
	МодульОрганизации = ОбщегоНазначения.ОбщийМодуль("Справочники." + ИмяСправочникаОрганизации); // АПК:271 Модуль существует
...
Функция ЗначениеФункциональнойОпции(НаименованиеФО) Экспорт
	...
	СоответствиеФО.Вставить("ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО",
		"ИспользоватьПроверкуПоддерживаемыхАбонентамиФорматовЭДО");
