...
// Заполняет настройку обмена КаталогСбораИнформацииОбОбменах из параметров запуска.
Процедура УстановитьНастройкуКаталогСбораИнформацииОбОбменах(СтруктураНастроекОбмена)
	СтруктураНастроекОбмена.Вставить("КаталогСбораИнформацииОбОбменах", "");
	// Каталог сбора информации об обменах.
	ПараметрЗапускаКлиента = "";
	Попытка
		ПараметрЗапускаКлиента = ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПараметрЗапуска");
	Исключение
		// Параметр может быть не инициализирован - это не ошибка.
		Возврат;
	КонецПопытки;
	Если ЗначениеЗаполнено(ПараметрЗапускаКлиента) Тогда
		СтруктураПараметровЗапуска = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(ПараметрЗапускаКлиента);
		КаталогСбораИнформацииОбОбменах = "";
		Если СтруктураПараметровЗапуска.Свойство("КаталогСбораИнформацииОбОбменах", КаталогСбораИнформацииОбОбменах) Тогда
			СтруктураНастроекОбмена.КаталогСбораИнформацииОбОбменах = КаталогСбораИнформацииОбОбменах;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

// Создает копию файла сообщения обмена при загрузке в каталог КаталогСбораИнформацииОбОбменах.
Процедура СоздатьКопиюФайлаСообщенияОбмена(СтруктураНастроекОбмена, ИмяФайлаСообщенияОбмена)
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.КаталогСбораИнформацииОбОбменах) Тогда
		Возврат;
	КонецЕсли;
	// Имя вложенного каталога по настройкам обмена.
	ИмяПодкаталога1 = СтруктураНастроекОбмена.ИмяПланаОбмена + "_" 
					+ СтруктураНастроекОбмена.УзелИнформационнойБазыКод;
	ИмяПодкаталога2 = Формат(СтруктураНастроекОбмена.ДатаНачала,"ДЛФ=ДВ");
	ПолныйПутьКФайлу = СтруктураНастроекОбмена.КаталогСбораИнформацииОбОбменах + "\" + ИмяПодкаталога1  + "\"
						+ ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяПодкаталога2, "_");
	КраткоеИмяФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайлаСообщенияОбмена).Имя;
	ВложенныйКаталог = Новый Файл(ПолныйПутьКФайлу);
	Если НЕ ВложенныйКаталог.Существует() Тогда
		СоздатьКаталог(ПолныйПутьКФайлу);
	КонецЕсли;
	КопироватьФайл(ИмяФайлаСообщенияОбмена, ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(ПолныйПутьКФайлу, КраткоеИмяФайла));
КонецПроцедуры
...
Процедура ПрочитатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "", Знач ТолькоПараметры = Ложь)
	...
	СоздатьКопиюФайлаСообщенияОбмена(СтруктураНастроекОбмена, ИмяФайлаСообщенияОбмена);
	Если СтруктураНастроекОбмена.ЭтоОбменВРИБ Тогда // Обмен в РИБ
...
Функция ПолучитьСтруктуруНастроекОбменаДляУзлаИнформационнойБазы(
	...
	УстановитьНастройкуКаталогСбораИнформацииОбОбменах(СтруктураНастроекОбмена);
