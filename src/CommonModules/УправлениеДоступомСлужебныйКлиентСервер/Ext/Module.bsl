...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
	...
	// +Рарус
	Если ОписаниеВидаДоступа.Свойство("Профиль") Тогда
		Отбор = ОтборВТаблицахФормыРедактированияРазрешенныхЗначений(Форма, ОписаниеВидаДоступа.ВидДоступа,ОписаниеВидаДоступа.Профиль);
	Иначе
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЗначенияРедактируются Тогда
			...
			// +РАРУС
			Если ТекущиеДанные.Свойство("Профиль") Тогда
				Отбор = ОтборВТаблицахФормыРедактированияРазрешенныхЗначений(Форма, ТекущиеДанные.ВидДоступа,ТекущиеДанные.Профиль);
			Иначе
			...
			// +РАРУС
			ИначеЕсли Отбор.Свойство("ГруппаДоступа") И ОтборСтрок.Свойство("ГруппаДоступа") И ОтборСтрок.ГруппаДоступа <> Отбор.ГруппаДоступа Тогда
				ОбновитьОтборСтрок = Истина;
			
			ИначеЕсли Отбор.Свойство("Профиль") И ОтборСтрок.Профиль <> Отбор.Профиль Тогда
				ОбновитьОтборСтрок = Истина;
			// -РАРУС
	...
	// +РАРУС
	Если ТекущиеДанные <> Неопределено И ТекущиеДанные.Свойство("Предустановленный")Тогда
		Если Форма.ЭтоПрофильГруппДоступа Тогда
			Элементы.ЗначенияДоступа.Доступность = ТекущиеДанные.Предустановленный;	
		Иначе
			Элементы.ЗначенияДоступа.Доступность = НЕ ТекущиеДанные.Предустановленный;	
		КонецЕсли;
	Иначе
...
// Только для внутреннего использования.
// +РАРУС
Функция ОтборВТаблицахФормыРедактированияРазрешенныхЗначений(Форма, ВидДоступа = "БезОтбораПоВидуДоступа", Профиль = "БезОтбораПоПрофилю") Экспорт // -РАРУС
	...
	Если Структура.ТекущаяГруппаДоступа <> "РеквизитНеСуществует" Тогда
		...
		// +РАРУС
		Если ТипЗнч(Структура.ТекущаяГруппаДоступа) = Тип("СправочникСсылка.ПрофилиГруппДоступа") Тогда
			
			Отбор.Вставить("Профиль", Структура.ТекущаяГруппаДоступа);
		Иначе
			Если ЗначениеЗаполнено(Структура.ТекущаяГруппаДоступа) Тогда
		Отбор.Вставить("ГруппаДоступа", Структура.ТекущаяГруппаДоступа);
			Иначе
				Отбор.Вставить("ГруппаДоступа", Форма.ГруппаДоступаПользователя);
			КонецЕсли;			
	...
	// +РАРУС
	Если Профиль <> "БезОтбораПоПрофилю" Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервереФормыРедактированияРазрешенныхЗначений(
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		// +РАРУС
		Если СтрокаВидаДоступа.Свойство("Профиль") Тогда
			ОтборВидовДоступа.Вставить("Профиль", СтрокаВидаДоступа.Профиль);
		КонецЕсли;
		// -РАРУС
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если СтрокаЗначенияДоступа.Свойство("Профиль") Тогда
				ОтборЗначенийДоступа.Вставить("Профиль", СтрокаЗначенияДоступа.Профиль);
			КонецЕсли;
			// -Рарус
