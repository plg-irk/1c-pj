...
Процедура ОткрытьТекущиеДелаЭДО(ПараметрыЗапуска = Неопределено, ОповещениеОЗакрытии = Неопределено,
	ПараметрыОткрытия = Неопределено) Экспорт
	
	Если ПараметрыОткрытия = Неопределено Тогда
		ПараметрыОткрытия = ОбменСКонтрагентамиКлиентСервер.ПараметрыОткрытияФормыТекущиеДелаЭДО();
	КонецЕсли;
	
	ОценкаПроизводительностиКлиент.ЗамерВремени("ОбщийМодуль.ИнтерфейсДокументовЭДОКлиент.ОткрытьТекущиеДелаЭДО");
	
	ОткрытьФорму(ФормаТекущиеДелаПоЭДО(), ПараметрыЗапуска, ПараметрыОткрытия.Владелец, ПараметрыОткрытия.Уникальность,
		ПараметрыОткрытия.Окно, , ОповещениеОЗакрытии, ПараметрыОткрытия.РежимОткрытияОкна);
...
Процедура ИзменитьВариантИнтерфейсаПользователя(Легкий = Ложь, ТекущаяФорма = Неопределено, 
	...
	Если СохранятьНастройку Тогда
		ИнтерфейсДокументовЭДОВызовСервера.СохранитьНастройкуИспользованияЛегкогоВариантаИнтерфейсаДокументовЭДО(Легкий);
	КонецЕсли;
...
// Параметры:
//  Результат - см. ИнтерфейсДокументовЭДОКлиентСервер.НовыйИтогВыполненияДействийПоЭДО
...
Процедура ЗавершениеФормированияЭлектронныхДокументовСобытие(Результат, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
	Оповестить("ВнутреннийЭДО_ОбновлениеСостоянияФормыПечатиБСП", Результат, Источник);
КонецПроцедуры
...
// Возвращаемое значение:
//  Строка
Функция ИмяСобытияИзмененияНастроекТекущихДелЭДО() Экспорт
	Возврат "ИзменениеНастроекТекущихДелЭДО";
КонецФункции
...
// Показывает проблемы при обработке документов.
// 
// Параметры:
//  Оповещение - ОписаниеОповещения
...
Процедура ОформитьГруппуСостоянияИСтатусыМЧД(Форма, Подписана, Верна, Отозвана, ДатаОтзыва, СтатусВРеестреФНС, СведенияОРегистрации = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			Элементы.ГруппаОшибкаПроверкиПодписи.Видимость = Истина;
			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПротоколПроверкиМЧД")
				И Форма.ПротоколПроверкиМЧД <> Неопределено Тогда
				
				Протокол = Форма.ПротоколПроверкиМЧД.СвойстваПодписи.Протокол;
				Если Протокол <> Неопределено 
					И МашиночитаемыеДоверенностиКлиентСервер.СертификатОтозванИлиИстек(Протокол) Тогда
					
					Элементы.ДекорацияНеверна.Заголовок = НСтр("ru = 'Возможно, неверна'");
					Элементы.ГруппаСостоянияИСтатусы.ЦветФона = ЦветаФона.НедействительнаяМЧД;
					Элементы.ДекорацияВернаКартинка.Картинка = БиблиотекаКартинок.ИнформацияБЭД;
					Элементы.ГруппаОшибкаПроверкиПодписи.Видимость = Истина;
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Объект") 
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПроверенаВручную") 
				И Форма.ПроверенаВручную Тогда
				Элементы.ГруппаОшибкаПроверкиПодписи.Видимость = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Процедура НачатьОбработкуОшибокФормированияДокумента(ОповещениеОЗавершении, ОшибкиФормирования, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		...
		Если ОшибкиФормирования.Количество() = 1
			И ОшибкиФормирования[0].ОшибкаПодготовкиДанныхВИнтеграцииЦентраЭДО Тогда
			КонтекстДиагностики = ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики();
			ТекстОшибки = НСтр("ru = 'Не удалось подготовить данные предварительного просмотра в центре ЭДО'");
...
Процедура ОткрытьНовыйЭлектронныйДокументПоПодготовленнымДанным(ОбъектУчета, НастройкиФормирования, АдресДанных, 
	...
	ПараметрыФормы.Вставить("ИспользоватьЦентрЭДО", НастройкиФормирования.ИспользоватьЦентрЭДО);
...
Процедура СформироватьЭДИзФормыПечатиБСП(Форма, Параметры = Неопределено)
	...
	ДополнительныеПараметрыЗавершения = Новый Структура("Источник", Форма);
	ПараметрыОповещения.Вставить("ОповещениеУспешногоЗавершения",
		...
		ДополнительныеПараметрыЗавершения));
