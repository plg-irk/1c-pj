...
Функция ИдентификаторНоменклатурыКонтрагентаПоНатуральнымКлючам(Знач НоменклатураКонтрагента, 
	...
	Для Каждого ЭлементКлюча Из Ключ Цикл
		Ключ.Вставить(ЭлементКлюча.Ключ, СокрЛП(ЭлементКлюча.Значение));
	КонецЦикла;
...
Процедура СоздатьОбновитьНоменклатуруКонтрагента(ДанныеНоменклатурыКонтрагента,
	...
	Попытка
		...
		Если Не ПометитьНаУдаление Тогда
			...
			ЗаполнитьИспользоватьХарактеристики(ОбъектНоменклатураКонтрагента, ДополнительныеПараметры, НоменклатураИБ);
...
//   * ИспользоватьХарактеристики - Булево,Неопределено - признак использования характеристики в номенклатуре.
//
Функция НовыеДополнительныеПараметрыПриЗаписиНоменклатурыКонтрагентов() Экспорт
	...
	ДополнительныеПараметры.Вставить("ИспользоватьХарактеристики", Неопределено);
...
// Устарела. Следует использовать СоздатьОбновитьНоменклатуруКонтрагента.
...
Процедура УстановитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента, Знач НоменклатураИБ = Неопределено, 
	...
	Отказ = Ложь;
	СоздатьОбновитьНоменклатуруКонтрагента(НоменклатураКонтрагента, НоменклатураИБ, Отказ,, ДополнительныеПараметры);
...
Процедура УдалитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента) Экспорт
	...
	ДополнительныеПараметры.ДанныеРегистраНоменклатурыКонтрагентов = Ложь;
	Отказ = Ложь;
	СоздатьОбновитьНоменклатуруКонтрагента(НоменклатураКонтрагента,
		СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураИнформационнойБазы(),
		Отказ,
		"",
...
//
// Возвращаемое значение:
//  Массив из см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовоеСоответствиеНоменклатуры - соответствия 
...
Функция НайтиСоответствиеНоменклатуры(Знач Отбор = Неопределено, Знач ТолькоСопоставленные = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НоменклатураИБ  = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураИнформационнойБазы();
		Соответствие    = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовоеСоответствиеНоменклатуры(ДанныеБазы,
			НоменклатураИБ);
...
Процедура СоздатьВременнуюТаблицуСоответствияНоменклатуры(Знач ТаблицаОтбора, Знач ИмяВременнойТаблицы, МенеджерВременныхТаблиц) Экспорт
	...
	|		ИНАЧЕ НоменклатураКонтрагентовБЭД.НаименованиеБазовойЕдиницыИзмерения
	|	КОНЕЦ КАК ЕдиницаИзмерения,
	|	НоменклатураКонтрагентовБЭД.КодОКЕИБазовойЕдиницыИзмерения КАК ЕдиницаИзмеренияКод,
	...
	|	НоменклатураКонтрагентовБЭД.Штрихкод КАК ШтрихкодКомбинации,
	|	НоменклатураКонтрагентовБЭД.ДругиеШтрихкодыНоменклатурыСтрокой КАК ШтрихкодыНоменклатуры,
	...
	|	НоменклатураКонтрагентовБЭД.НаименованиеБазовойЕдиницыИзмерения КАК НаименованиеБазовойЕдиницаИзмерения,
	...
	|ИЗ
	|	Справочник.НоменклатураКонтрагентов КАК НоменклатураКонтрагентовБЭД
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втОтбор КАК втОтбор
	|		ПО НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры = втОтбор.Владелец
	|		И НоменклатураКонтрагентовБЭД.Идентификатор = втОтбор.Идентификатор
	|		И (втОтбор.Идентификатор <> """")
	|
	|ГДЕ
	|	НЕ НоменклатураКонтрагентовБЭД.ПометкаУдаления
	|	И НЕ НоменклатураКонтрагентовБЭД.Недействителен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры,
	...
	|		ИНАЧЕ НоменклатураКонтрагентовБЭД.НаименованиеБазовойЕдиницыИзмерения
	|	КОНЕЦ КАК ЕдиницаИзмерения,
	|	НоменклатураКонтрагентовБЭД.КодОКЕИБазовойЕдиницыИзмерения,
	...
	|	НоменклатураКонтрагентовБЭД.ДругиеШтрихкодыНоменклатурыСтрокой,
	|	НоменклатураКонтрагентовБЭД.Штрихкод,
	|	НоменклатураКонтрагентовБЭД.НаименованиеУпаковки КАК НаименованиеУпаковки,
	|	НоменклатураКонтрагентовБЭД.НаименованиеБазовойЕдиницыИзмерения КАК НаименованиеБазовойЕдиницаИзмерения,
	|	НоменклатураКонтрагентовБЭД.ТипНоменклатурыФНС КАК ТипНоменклатурыФНС,
...
Функция ИнициализироватьТекстЗапросаСоответствияНоменклатурыКонтрагентовБЭДПоИдентификаторуИВладельцу(Знач ИмяВременнойТаблицыОтбора = Неопределено) Экспорт
	...
	|	НоменклатураКонтрагентовБЭД.Наименование КАК Наименование,
	|	НоменклатураКонтрагентовБЭД.НаименованиеХарактеристики КАК НаименованиеХарактеристики,
	|	НоменклатураКонтрагентовБЭД.НаименованиеБазовойЕдиницыИзмерения КАК ЕдиницаИзмерения,
	|	НоменклатураКонтрагентовБЭД.КодОКЕИБазовойЕдиницыИзмерения КАК ЕдиницаИзмеренияКод,
	|	НоменклатураКонтрагентовБЭД.Артикул КАК Артикул,
	|	НоменклатураКонтрагентовБЭД.СтавкаНДС КАК СтавкаНДС,
	|	НоменклатураКонтрагентовБЭД.Штрихкод КАК Штрихкод,
	...
	|		ПО НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры = ТаблицаУникальныхОтборов.Владелец
...
#Область Версия_1_10_1

	Справочники.НоменклатураКонтрагентов.ПриДобавленииОбработчиковОбновления(Обработчики);
...
Процедура ЗаполнитьИспользоватьХарактеристики(ОбъектНоменклатураКонтрагента, ДополнительныеПараметры, НоменклатураИБ)
	
	Если ДополнительныеПараметры.ИспользоватьХарактеристики <> Неопределено Тогда
		ИспользоватьХарактеристики = ДополнительныеПараметры.ИспользоватьХарактеристики;
	...
	ОбъектНоменклатураКонтрагента.ИспользоватьХарактеристики = ИспользоватьХарактеристики;
...
Функция ТекстЗапросаДанныхНоменклатурыКонтрагента()
	...
	|	НЕ НоменклатураКонтрагентовБЭД.ПометкаУдаления
	|	И НЕ НоменклатураКонтрагентовБЭД.Недействителен
	|	И (НоменклатураКонтрагентовБЭД.ВладелецНоменклатуры, НоменклатураКонтрагентовБЭД.Идентификатор) В
	|		(ВЫБРАТЬ
	|			втОтбор.Владелец КАК Владелец,
	|			втОтбор.Идентификатор КАК Идентификатор
	|		ИЗ
...
Функция ТекстЗапросаКРегистру()
	...
	Возврат "ВЫБРАТЬ
	|	Сопоставление.ВладелецНоменклатуры КАК Владелец,
	...
	|	Сопоставление.НаименованиеБазовойЕдиницыИзмерения КАК ЕдиницаИзмерения,
	|	Сопоставление.КодОКЕИБазовойЕдиницыИзмерения КАК ЕдиницаИзмеренияКод,
	...
	|	Сопоставление.Штрихкод КАК ШтрихкодКомбинации,
	|	Сопоставление.ДругиеШтрихкодыНоменклатурыСтрокой КАК ШтрихкодыНоменклатуры,
	...
	|ИЗ
	|	Справочник.НоменклатураКонтрагентов КАК Сопоставление";
...
Функция ЗапросСоответствиеНоменклатуры(Отбор, ДобавитьИндексы, ДанныеНоменклатурыКонтрагентов)
	...
	Если Отбор.Свойство("Владелец") Тогда
		ТекстыУсловий.Добавить("Сопоставление.ВладелецНоменклатуры = &Владелец");
		Запрос.УстановитьПараметр("Владелец", Отбор.Владелец);
	КонецЕсли;
	
	ТекстыУсловий.Вставить(0, "НЕ Сопоставление.Недействителен");
	ТекстыУсловий.Вставить(0, "НЕ Сопоставление.ПометкаУдаления");
	...
	Если Отбор.Свойство("НоменклатураКонтрагента") Тогда
		...
		Иначе
			ТекстыОсновногоЗапроса.Добавить("ГДЕ");
		КонецЕсли;
		ТекстыОсновногоЗапроса.Добавить("	(Сопоставление.ВладелецНоменклатуры, Сопоставление.Идентификатор) В
		...
		ТекстыОсновногоЗапроса.Добавить("ВНУТРЕННЕЕ СОЕДИНЕНИЕ втТаблицаНоменклатурыКонтрагентов КАК Идентификаторы
			|		ПО Сопоставление.ВладелецНоменклатуры = Идентификаторы.Владелец
...
Процедура ОбновитьДополнитьДанныеСопоставления(Знач Соответствие, Знач ДанныеДокумента, Знач ИспользоватьХарактеристики)
	...
	Если ЕстьИзменения Тогда
		...
		ДополнительныеПараметры.ИспользоватьХарактеристики = ИспользоватьХарактеристики;
		Отказ = Ложь;
		СоздатьОбновитьНоменклатуруКонтрагента(ДанныеБазы, НоменклатураИБ, Отказ,, ДополнительныеПараметры);
	КонецЕсли;
...
//
// Параметры:
//  Соответствие - см. СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НовоеСоответствиеНоменклатуры.
...
Функция СоответствиеАктуально(Соответствие, ДанныеДокумента)
	...
	Если Результат Тогда
		Отказ = Ложь;
		СоздатьОбновитьНоменклатуруКонтрагента(ДанныеБазы, НоменклатураИБ, Отказ);
	КонецЕсли;
