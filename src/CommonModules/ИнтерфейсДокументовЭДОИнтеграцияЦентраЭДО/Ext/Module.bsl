...
//  ОбщийМодуль
Функция МенеджерДанныхИнтеграцииЦентраЭДО() Экспорт
	Возврат ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО;
КонецФункции
...
Функция ДанныеФайлаИнформацииОтправителяДокумента(ЭлектронныйДокумент, ИдентификаторФормы) Экспорт
	...
	РезультатВыполнения = ИнтеграцияЦентраЭДО.ВыполнитьМетодСервиса(ПараметрыВыполнения, ОписаниеМетода,
		КонтекстДиагностики); // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ДанныеФайлаИнформацииОтправителяДокумента
...
Процедура ЗаполнитьДокументПоПодготовленнымДанным(Форма, ПросмотрПодготовленныхДанных) Экспорт
	...
	Форма.Объект.ИспользоватьЦентрЭДО = Истина;
...
Функция ПодготовитьДанныеПредварительногоПросмотра(ОписанияДокументовОбъектаУчета, НастройкиФормирования, 
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотра", // См. ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотра
...
Функция РезультатПодготовкиДанныхПросмотраПослеДлительнойОперации(ДлительнаяОперация) Экспорт
	...
	Если ТипЗнч(РезультатДлительнойОперации) = Тип("Структура") Тогда
		...
		ДанныеПредварительногоПросмотра = РезультатДлительнойОперации.ДанныеПредварительногоПросмотра; //см. ВизуализацияЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотра
...
Функция ПодготовитьДанныеПредварительногоПросмотраПоФайлуXML(ВыбранныйФайл) Экспорт
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполненияВФоне,
		"ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотраПоФайлуXML", // См. ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотраПоФайлуXML
		ВыбранныйФайл);
...
Процедура ЗаполнитьДанныеКомпонентовПросмотраДокумента(ДанныеКомпонентов, ПараметрыЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеКомпонентов.ПанельСообщений) Тогда
		ЛегкийИнтерфейсДокументовЭДО.ЗаполнитьДанныеКомпонентаПанельСообщений(ДанныеКомпонентов.ПанельСообщений,
			 СвойстваДокумента);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеКомпонентовПросмотраДокументаАсинхронно(ДанныеКомпонентов, ПараметрыЗаполнения) Экспорт
	...
	Если ТребуетсяПолнаяСинхронизация Тогда
		КэшВизуализации.ТребуетсяОбновить = Истина;
		ОписаниеМетода = ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ОписаниеМетодаДанныеПолнойСинхронизацииДокументаЭДО(
			СостояниеСинхронизации);
		ИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаВПакет(ПакетМетодов, ОписаниеМетода, 0);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеКомпонентов.ПанельВизуализации)
		...
		Если ЭтоПредварительныйПросмотрДокумента Тогда
			...
			ИнтеграцияЦентраЭДО.ДобавитьОписаниеМетодаВПакет(ПакетМетодов, ОписаниеМетода, 1);
		Иначе
	...
	Если ЗначениеЗаполнено(ПакетМетодов) Тогда
		...
		Если Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда
			ВызватьИсключение НСтр("ru = 'Отсутствует учетная запись центра ЭДО'");
		КонецЕсли;
		...
		ДанныеСинхронизацииВФорматеСервиса = РезультатыПакета[0]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ДанныеПолнойСинхронизацииДокументаЭДО
		ВизуализацияВФорматеСервиса = РезультатыПакета[1]; // См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ВизуализацияДокументаЭДО
		
	КонецЕсли;
	
	ЭлектронныеДокументыЭДОИнтеграцияЦентраЭДО.ЗагрузитьПолныеДанныеДокументаЭДО(ДанныеСинхронизацииВФорматеСервиса,
	...
	Если ЗначениеЗаполнено(ДанныеКомпонентов.ПанельЗакладокВходящегоДокумента) Тогда
		...
		ЛегкийИнтерфейсДокументовЭДО.ЗаполнитьДанныеКомпонентаПанельЗакладокВходящегоДокумента(
			ДанныеКомпонентов.ПанельЗакладокВходящегоДокумента, ОписанияПодписей, СопроводительныеЗаписи, СвойстваДокумента);
	КонецЕсли;
	...
	Если КэшВизуализации.ТребуетсяОбновить И ЗначениеЗаполнено(ВизуализацияВФорматеСервиса) Тогда
		ВизуализацияДокумента = 
			ВизуализацияЭДОИнтеграцияЦентраЭДО.ВизуализацияДокументаИзФорматаСервиса(ВизуализацияВФорматеСервиса);
	...
	Если ЗначениеЗаполнено(ДанныеКомпонентов.ПанельВизуализации) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеКомпонентов.ПанельРеквизитовВходящегоДокументаДляРедактирования) Тогда
			ЛегкийИнтерфейсДокументовЭДО.ЗаполнитьДанныеКомпонентаПанельРеквизитовВходящегоДокументаДляРедактирования(
...
//
// Параметры:
//  ДанныеФайлаВФорматеСервиса - См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ДанныеФайлаИнформацииОтправителяДокумента
// 
// Возвращаемое значение:
//  См. ЛегкийИнтерфейсДокументовЭДО.НовыйРезультатПолученияДанныхФайлаИнформацииОтправителя
Функция ДанныеФайлаИнформацииОтправителяДокументаИзФорматаСервиса(ДанныеФайлаВФорматеСервиса)
	...
	Если Не ЗначениеЗаполнено(ДанныеФайлаВФорматеСервиса) Тогда
		ВидОперации = НСтр("ru = 'Получение файла электронного документа из центра ЭДО'");
		ТекстСообщения = НСтр("ru = 'Не удалось получить файл электронного документа из центра ЭДО'");
...
// 
// Возвращаемое значение:
//  См. ВизуализацияЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотра
Функция ДанныеПредварительногоПросмотра(ОписанияДокументовОбъектаУчета, НастройкиФормирования,
	ПараметрыОткрытия) Экспорт
	
	Возврат ВизуализацияЭДОИнтеграцияЦентраЭДО.ДанныеПредварительногоПросмотра(ОписанияДокументовОбъектаУчета,
...
//  ПредставлениеФайлаВФорматеСервиса - См. ПрограммныйИнтерфейсЦентраЭДОВерсия3.ПредставлениеФайлаДокументаЭДО
