...
Процедура Подписать(ОписаниеДанных, КонтекстДиагностики, Форма = Неопределено, Оповещение = Неопределено,
	...
	Если Не (ОписаниеДанных.Свойство("Операция") И
		ОписаниеДанных.Операция = СервисЭДОКлиент.ОперацияПодписаниеРегистрационныхДанных()) Тогда
		
		ПараметрыПодписи.РазрешитьОтправкуНаПодписание = Истина;
	КонецЕсли;

#Если Не МобильныйКлиент Тогда
	ЭлектроннаяПодписьКлиент.Подписать(ОписаниеДанных, Форма, СлужебноеОповещение, ПараметрыПодписи);
#Иначе
	МобильныйЭДОКлиент.Подписать(ОписаниеДанных, Форма, СлужебноеОповещение, ПараметрыПодписи);
#КонецЕсли
...
// 
Процедура ПроверитьСертификат(ОбработчикРезультатаПроверки, Сертификат, КонтекстДиагностики, Подпись = Неопределено,
...
#Иначе
	МобильныйЭДОКлиент.Расшифровать(ОписаниеДанных, Форма, СлужебноеОповещение);
#КонецЕсли
...
Функция НовыеПараметрыРасшифровкиНабораДанных() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("ПаролиСертификатов", НовыеПаролиСертификатов());
	Параметры.Вставить("КонтекстДиагностики", ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики());
	Параметры.Вставить("ЗаголовокДанных", "");
	Параметры.Вставить("ПредставленияДанных", Новый Соответствие);
	Возврат Параметры;
...
//  ПараметрыРасшифровки - см. НовыеПараметрыРасшифровкиНабораДанных
Процедура НачатьРасшифровкуНабораДанных(Оповещение, НаборДанныхДляРасшифровки, ПараметрыРасшифровки = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыРасшифровки) Тогда
		КонтекстРасшифровки.ПаролиСертификатов = ПараметрыРасшифровки.ПаролиСертификатов;
		КонтекстРасшифровки.КонтекстДиагностики = ПараметрыРасшифровки.КонтекстДиагностики;
		КонтекстРасшифровки.ЗаголовокДанных = ПараметрыРасшифровки.ЗаголовокДанных;
		КонтекстРасшифровки.ПредставленияДанных = ПараметрыРасшифровки.ПредставленияДанных;
	КонецЕсли;
...
//                    - Неопределено
Процедура ПроверитьПароль(Сертификат, Пароль, ОписаниеОповещения, ДанныеСертификата = Неопределено) Экспорт
	...
	ОписаниеДанных.Вставить("Данные",
		?(ДанныеСертификата = Неопределено, ПолучитьДвоичныеДанныеИзСтроки("123"), ДанныеСертификата));
	...
	Подписать(ОписаниеДанных, КонтекстДиагностики, , ОписаниеОповещения);
...
//                 * ДополнительныеПараметры - Произвольный - значение, которое было указано при создании
//                                 объекта ОписаниеОповещения.
//  ПрограммаКриптографии - СправочникСсылка.ПрограммыЭлектроннойПодписиИШифрования,
//  					  - Строка,
//  					  - Неопределено
Процедура НайтиСоздатьСертификатКриптографии(Отпечаток, Организация, Оповещение, ПрограммаКриптографии = Неопределено) Экспорт
	
	КриптографияБЭДСлужебныйКлиент.НайтиСоздатьСертификатКриптографии(Отпечаток, Организация, Оповещение, ПрограммаКриптографии)
...
// требующий подтверждение машиночитаемой доверенностью, при этом в базе отсутствует подходящая доверенность.
// 
// Возвращаемое значение:
//  Строка
...
// Это тип подписи CAdESBES.
// 
// Параметры:
// 	ТипПодписи - См. НастройкиЭДО.ТипПодписи
// 	
// Возвращаемое значение:
//  Булево
Функция ЭтоТипПодписиCAdESBES(ТипПодписи) Экспорт
	Возврат ТипПодписи = ПредопределенноеЗначение("Перечисление.ТипыПодписиКриптографии.БазоваяCAdESBES");
КонецФункции
...
// Возвращаемое значение:
//  Булево
Функция ЕстьПарольСертификата(ПаролиСертификатов, Сертификат)
