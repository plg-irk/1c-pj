...
Функция ЕстьРоль(Знач Роль, Знач СсылкаНаОбъект = Неопределено, Знач Пользователь = Неопределено) Экспорт
	...
	Если СсылкаНаОбъект = Неопределено ИЛИ НЕ ОграничиватьДоступНаУровнеЗаписей() Тогда
		...
		|		ПО ГруппыДоступаПрофили.Профиль = ПрофилиГруппДоступаРоли.Ссылка
		// -РАРУС
	...
	Для каждого Строка Из НаборыЗначенийДоступа Цикл
		...
		Если ВРег(Строка.ВидДоступа) = ВРег("ПравоЧтения")
		 ИЛИ ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
			...
			Если ТипЗнч(Строка.ЗначениеДоступа) <> Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") И 
			 ТипЗнч(Строка.ЗначениеДоступа) <> Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
	...
	|												КОГДА ТИПЗНАЧЕНИЯ(НаборыЗначений.ЗначениеДоступа) = ТИП(Справочник.ИдентификаторыОбъектовРасширений)
	|													ТОГДА ИСТИНА В
	|															(ВЫБРАТЬ ПЕРВЫЕ 1
	|																ИСТИНА
	|															ИЗ
...
// +РАРУС  Разрешение редактирования настроек доступа на уровне записи
// 
//
// Возвращаемое значение:
//  Булево - если Истина, тогда доступ ограничивается на уровне записей.
//
Функция НастраиватьДоступНаУровнеЗаписей() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПолучитьФункциональнуюОпцию("НастраиватьДоступНаУровнеЗаписей");
...
Функция ТаблицаНаборыЗначенийДоступа() Экспорт
	...
	Таблица.Колонки.Добавить("Уточнение",       Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений"));
...
//
// Параметры:
//  СсылкаИлиОбъект - СправочникСсылка, ДокументСсылка, Ссылка - ссылка или объект, 
...
Процедура ЗаменитьПраваВНастройкахПравОбъектов(ТаблицаПереименований) Экспорт
	...
	|ИМЕЮЩИЕ
	|	НастройкиПрав.НовоеИмя <> """"
	|	И КОЛИЧЕСТВО(НастройкиПрав.НовоеИмя) > 1
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
// Функция проверяет, разрешено ли значение доступа пользователя
Функция ПроверитьЗначениеДоступа(Пользователь, ВидДоступа, ЗначениеДоступа) Экспорт
	// инициализируем
	Разрешено = Ложь;
	
	// получаем группу пользователя
	ГруппаДоступаПользователя = ГруппаДоступаПользователя(Пользователь);
	...
	|	ЗначенияГруппДоступаПоУмолчанию.Профиль КАК Профиль,
	...
	|	ЕСТЬNULL(ЗначенияГруппДоступа.ЗначениеДоступа, &ТипЗначенийДоступа) КАК ЗначениеДоступа,
	|	ЕСТЬNULL(ЗначенияГруппДоступа.ЗначениеРазрешено, Ложь) КАК ЗначениеРазрешено
	|ИЗ
	...
	|		ПО ЗначенияГруппДоступаПоУмолчанию.ГруппаДоступа = ЗначенияГруппДоступа.ГруппаДоступа
	|			И ЗначенияГруппДоступаПоУмолчанию.Профиль = ЗначенияГруппДоступа.Профиль
	|			И (ТИПЗНАЧЕНИЯ(ЗначенияГруппДоступа.ЗначениеДоступа) = &ТипЗначенияДоступа ИЛИ ЗначенияГруппДоступа.ЗначениеДоступа ЕСТЬ NULL)
	|ГДЕ
	|	ЗначенияГруппДоступаПоУмолчанию.ГруппаДоступа = &ГруппаДоступа
	|	И ЗначенияГруппДоступаПоУмолчанию.ТипЗначенийДоступа = &ТипЗначенийДоступа";
	
	// ставим параметры
	Запрос.УстановитьПараметр("ГруппаДоступа", ГруппаДоступаПользователя);
	Запрос.УстановитьПараметр("ТипЗначенийДоступа", ВидДоступа);
	Запрос.УстановитьПараметр("ТипЗначенияДоступа", ТипЗнч(ВидДоступа));
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		// Перебираем выборку по правилу "Если разрешено для одного профиля то разрешено вообще".
		Пока Выборка.Следующий() Цикл
			// если заполнено значение то значит ищем сравнения с нашим значением
			Если ЗначениеЗаполнено(Выборка.ЗначениеДоступа) Тогда
				// если нашли нужное значение то взводим флаг элемента
				Если Выборка.ЗначениеДоступа = ЗначениеДоступа Тогда
					ЭлементНайден = Истина;
					// если нужное значение разрешено взводим флаг и выходим из цикла
					Если Выборка.ЗначениеРазрешено Тогда
						Разрешено = Истина;
						Прервать;
					КонецЕсли;
				КонецЕсли;
			Иначе
				// если значение доступа не заполнено то либо все запрещены, либо разрешены
				ЭлементНайден = Истина;
				
				// если все разрешены то взводим флаг и выходим из цикла
				Если Выборка.ВсеРазрешены Тогда
					Разрешено = Истина;
					Прервать;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьКодыТиповИТаблицуСтрокНабора(КодыТипов, КодыПеречислений, ТаблицаСтрокНабора)
	...
	ТаблицаСтрокНабора.Колонки.Добавить("Уточнение",           Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных, СправочникСсылка.ИдентификаторыОбъектовРасширений"));
