...
//  см. ВизуализацияЭДОСлужебный.ВизуализацияЭлектронногоДокумента
Функция ВизуализацияЭлектронногоДокумента(ДанныеЭлектронногоДокумента,
	...
	Возврат ВизуализацияЭДОСлужебный.ВизуализацияЭлектронногоДокумента(ДанныеЭлектронногоДокумента, 
		ДанныеЭлектронногоДокументаДляИзвлеченияПараметров, ПараметрыВизуализации, КонтекстДиагностики);
...
Функция ПреобразоватьФорматЭД(ПотокИсходногоXML, Параметры = Неопределено, КонтекстДиагностики = Неопределено) Экспорт
	...
	Иначе
		Кодировка = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "Кодировка");
	...
	ЗаписьИтоговогоXML.ОткрытьПоток(ПотокИтоговогоXML, Кодировка);
	
	Попытка
		ПотокФайлаXML = ПотокФайлаДляПреобразованияXSL(ПотокИсходногоXML);
		ЧтениеИсходногоXML = Новый ЧтениеXML;
		ЧтениеИсходногоXML.ОткрытьПоток(ПотокФайлаXML, , , Кодировка);
	...
	ИначеЕсли Параметры.ИтоговыйФормат = "ПреобразованиеСодержимого" Тогда 
		ПотокИтоговогоXML = ОбщегоНазначенияБЭД.ПотокЗаписиСОтредактированнымОбъявлениемXML(ПотокИтоговогоXML, 
			Кодировка);
		Результат = ПотокИтоговогоXML.ЗакрытьИПолучитьДвоичныеДанные();
...
// Выполняет преобразование содержимого файла. Например добавляет .00 для целых чисел в форматах ФНС
//
// Параметры:
//  ОписаниеФайла - См. РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла
Процедура ВыполнитьПреобразованиеСодержимого(ОписаниеФайла) Экспорт
	Попытка
		ДанныеФайла = ОписаниеФайла.ДвоичныеДанные;
		ПараметрыФайла = ПараметрыФайлаПроизвольногоДокумента(ДанныеФайла);
		Если ПараметрыФайла = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ПараметрыФайла.Вставить("ИтоговыйФормат", "ПреобразованиеСодержимого");
		ПараметрыФайла.Вставить("ВариантЗаполнения", "");
		ПараметрыФайла.Вставить("Кодировка", ОбщегоНазначенияБЭД.КодировкаИзОбъявленияXML(ДанныеФайла));
		ОтформатированныйФайл = ПреобразоватьФорматЭД(ДанныеФайла.ОткрытьПотокДляЧтения(), ПараметрыФайла);
		
		Если ОтформатированныйФайл = Неопределено Тогда
			Возврат;
		КонецЕсли;

		ОписаниеОтформатированногоФайла = РаботаСФайламиБЭДКлиентСервер.НовоеОписаниеФайла();
		ОписаниеОтформатированногоФайла.ДвоичныеДанные = ОтформатированныйФайл;
		ОписаниеОтформатированногоФайла.ИмяФайла = ОписаниеФайла.ИмяФайла;
		РезультатПроверки = ФорматыЭДО.ПроверитьДокумент(ОписаниеОтформатированногоФайла);
		
		Если РезультатПроверки.Ошибки Тогда
			ВызватьИсключение РезультатПроверки.ТекстОшибки;
		Иначе
			ОписаниеФайла.ДвоичныеДанные = ОтформатированныйФайл;
		КонецЕсли;
		
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось установить формат сумм по причине:'") + Символы.ПС
			+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
//  * ИтоговыйФормат - Строка
//  * ВерсияИтоговогоФормата - Массив Из Строка
// 
// Возвращаемое значение:
//  Дата
Функция ДатаОбновленияПравилаПреобразованияФормата(ПараметрыПравила) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПравилаПреобразованияФорматов.ДатаОбновления КАК ДатаОбновления
	|ИЗ
	|	РегистрСведений.ПравилаПреобразованияФорматов КАК ПравилаПреобразованияФорматов
	|ГДЕ
	|	ПравилаПреобразованияФорматов.ИсходныйФормат = &ИсходныйФормат
	...
	|	И ПравилаПреобразованияФорматов.ВерсияИтоговогоФормата В(&ВерсияИтоговогоФормата)";
...
//  * ВариантЗаполнения - Строка
//  * ИтоговыйФормат - Строка
//  * ВерсияИтоговогоФормата - См. ПоддерживаемаяВерсияИтоговогоФормата
Функция ПараметрыПравилаПреобразованияПроизвольногоДокумента() Экспорт
	Параметры = Новый Структура;
	Параметры.Вставить("ИсходныйФормат", "ПроизвольныйXML");
	Параметры.Вставить("ВариантЗаполнения", "");
	Параметры.Вставить("ИтоговыйФормат", "ПараметрыЭлектронногоДокумента");
	ВерсияИтоговогоФормата = ПоддерживаемаяВерсияИтоговогоФормата(Параметры.ИтоговыйФормат);
	Параметры.Вставить("ВерсияИтоговогоФормата", ВерсияИтоговогоФормата);
	Возврат Параметры;
КонецФункции
...
//  * ИтоговыйФормат - Строка
//  * ВерсияИтоговогоФормата - Массив Из Строка
Функция ПараметрыПравилаВизуализацииВТабличныйДокумент(ПараметрыДокумента) Экспорт
	ПараметрыПравила = Новый Структура;
	ПараметрыПравила.Вставить("ИсходныйФормат", ПараметрыДокумента.ИсходныйФормат);
	ВариантЗаполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыДокумента, "ВариантЗаполнения", ""); // Строка
	ПараметрыПравила.Вставить("ВариантЗаполнения", ВариантЗаполнения);
	ПараметрыПравила.Вставить("ИтоговыйФормат", "ТабличныйДокумент");
...
// Возвращаемое значение:
//  Строка
Функция ИдентификаторПравилаПреобразования(ПараметрыПравила) Экспорт
	КлючПравила = ПараметрыПравила.ИсходныйФормат + ПараметрыПравила.ВариантЗаполнения
		+ ПараметрыПравила.ИтоговыйФормат + ПараметрыПравила.ВерсияИтоговогоФормата;
	Возврат ОбщегоНазначения.КонтрольнаяСуммаСтрокой(КлючПравила);
КонецФункции
...
// * ИтоговыйФормат - Строка
// * ВерсияИтоговогоФормата - Строка
Функция НовыеПараметрыПравил() Экспорт
	ПараметрыПравила = Новый Структура;
	ПараметрыПравила.Вставить("ИсходныйФормат", "");
	ПараметрыПравила.Вставить("ВариантЗаполнения", "");
	ПараметрыПравила.Вставить("ИтоговыйФормат", "");
	ПараметрыПравила.Вставить("ВерсияИтоговогоФормата", "");
	Возврат ПараметрыПравила;
КонецФункции

// Параметры:
//  ИдентификаторыФорматов - Неопределено
//                         - Массив Из Строка
Процедура ОчиститьДатыОбновленияФорматовДляПреобразованияИПравилПреобразованияФорматов(ИдентификаторыФорматов = Неопределено) Экспорт
	...
	ЕстьОтборПоИдентификаторам = ЗначениеЗаполнено(ИдентификаторыФорматов);
	...
	Попытка
		...
		Константы.ДатаЗагрузкиОписанияФорматовДляПреобразования.Установить(ПустаяДата);
		
		НаборЗаписей = РегистрыСведений.ФорматыДляПреобразования.СоздатьНаборЗаписей();
		НаборЗаписей.Прочитать();
		Для Каждого Запись Из НаборЗаписей Цикл
			Если ЕстьОтборПоИдентификаторам
				И ИдентификаторыФорматов.Найти(Запись.ИсходныйФормат) <> Неопределено Тогда
				Запись.ДатаОбновленияПравил = ПустаяДата;
			ИначеЕсли Не ЕстьОтборПоИдентификаторам Тогда
				Запись.ДатаОбновленияПравил = ПустаяДата;
			Иначе
				Продолжить;
			КонецЕсли;
		...
		Для Каждого Запись Из НаборЗаписей Цикл
			Если ЕстьОтборПоИдентификаторам
				И ИдентификаторыФорматов.Найти(Запись.ИсходныйФормат) <> Неопределено Тогда
				Запись.ДатаОбновления = ПустаяДата;
			ИначеЕсли Не ЕстьОтборПоИдентификаторам Тогда
				Запись.ДатаОбновления = ПустаяДата;
			Иначе
				Продолжить;
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения =  НСтр("ru = 'Не удалось очистить даты обновления правил преобразования форматов по причине:'")
...
// Возвращаемое значение:
//  Строка
Функция ПривестиСтрокуКРегистру(ИмяПравила, Значение)
	
	Результат = Значение;
	
	Если ИмяПравила = "ВРег" Тогда
		Результат = ВРег(Результат);
	ИначеЕсли ИмяПравила = "НРег" Тогда
		Результат = НРег(Результат);
	ИначеЕсли ИмяПравила = "ТРег" Тогда
		Результат = ТРег(Результат);
	КонецЕсли;
...
// Возвращаемое значение:
//  Булево
Функция ЭтоФункцияПреобразованияСтрокиКРегистру(ИмяФункции)
	
	Возврат ИмяФункции = "ВРег" Или ИмяФункции = "НРег" Или ИмяФункции = "ТРег";
...
Функция КонвертироватьЗначениеПоИнструкции(Инструкция)
	...
	ИначеЕсли ЭтоФункцияПреобразованияСтрокиКРегистру(Инструкция.Правило) Тогда
		
		Для Каждого СвойствоИнструкции Из Инструкция Цикл
			Если СвойствоИнструкции.Ключ <> "Правило" Тогда
				Результат = ПривестиСтрокуКРегистру(Инструкция.Правило, СвойствоИнструкции.Значение);
			КонецЕсли;
		КонецЦикла;
...
Функция ПроверитьЭлектронныйДокументПоСхемеXML(ПотокИсходногоXML, ПараметрыФайлаXML, КонтекстДиагностики = Неопределено)
	...
	ПотокНормализованногоXML = РаботаСФайламиБЭД.НовыйПотокЧтенияНормализованногоXML(ПотокИсходногоXML);
	ПотокКонтрольногоXML = Новый ПотокВПамяти;
	
	Попытка
		ЧтениеИсходногоXML = Новый ЧтениеXML;
		ЧтениеИсходногоXML.ОткрытьПоток(ПотокНормализованногоXML);
...
Функция ПоддерживаемаяВерсияИтоговогоФормата(ИтоговыйФормат)
	...
	ИначеЕсли ИтоговыйФормат = "ТабличныйДокумент" Тогда
		ВерсииФормата.Добавить("1.0");
	ИначеЕсли ИтоговыйФормат = "ПреобразованиеСодержимого" Тогда
		ВерсииФормата.Добавить("1.0");
...
Функция СинхронизироватьОписаниеФорматовСПровайдером(Провайдер, КонтекстДиагностики)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		Если Не (ЧтениеXML.Имя = "fmt" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента) Тогда
			Продолжить;
		КонецЕсли;
	...
	ОтборПоПолюОбновлен = Новый Структура("Обновлен", Истина);
	ИзмененныеФорматы = ТаблицаФорматов.Скопировать(ОтборПоПолюОбновлен);
...
Процедура ОбновитьОписаниеФорматовДляПреобразования(Провайдер, ОбновляемоеПравило = Неопределено, КонтекстДиагностики = Неопределено)
	...
	ВариантЗаполнения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбновляемоеПравило, "ВариантЗаполнения", "");
	ИтоговыйФормат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбновляемоеПравило, "ИтоговыйФормат", "");
	
	ОписаниеРесурса = Новый Структура;
	ОписаниеРесурса.Вставить("СсылкаНаРесурс", "");
	ОписаниеРесурса.Вставить("ДатаОбновленияПравил", '00010101');
	ОписаниеРесурса.Вставить("ПроверяемыйФормат", ИтоговыйФормат);
	ОписаниеРесурса.Вставить("ПроверяемыйВариантЗаполнения", ВариантЗаполнения);
...
// * СсылкаНаРесурс - Строка 
// * ДатаОбновленияПравил - Дата
// * ПроверяемыйФормат - Строка - Имя формата к которому будет выполнено преобразование
// * ПроверяемыйВариантЗаполнения - Строка - Вариант заполнения итогового формата при преобразовании
//  КонтекстДиагностики - См. ОбработкаНеисправностейБЭД.НовыйКонтекстДиагностики
Процедура ОбновитьПравилоПреобразования(Провайдер, Формат, ОписаниеРесурса, КонтекстДиагностики)
	...
	УсловиеПроверятьИтоговыйФормат = ЗначениеЗаполнено(ОписаниеРесурса.ПроверяемыйФормат);
	МассивПроверяемыхВерсий = Новый Массив;
	Если УсловиеПроверятьИтоговыйФормат Тогда
		МассивПроверяемыхВерсий = ПоддерживаемаяВерсияИтоговогоФормата(ОписаниеРесурса.ПроверяемыйФормат);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ПроверятьИтоговыйФормат", УсловиеПроверятьИтоговыйФормат);
	Запрос.УстановитьПараметр("ПроверятьВариантЗаполнения", ЗначениеЗаполнено(ОписаниеРесурса.ПроверяемыйВариантЗаполнения));
	Запрос.УстановитьПараметр("ВариантЗаполнения", ОписаниеРесурса.ПроверяемыйВариантЗаполнения);
	Запрос.УстановитьПараметр("ИтоговыйФормат", ОписаниеРесурса.ПроверяемыйФормат);
	Запрос.УстановитьПараметр("МассивВерсийИтоговогоФормата", МассивПроверяемыхВерсий);
	
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ
	|	ПравилаПреобразованияФорматов.ВариантЗаполнения КАК ВариантЗаполнения,
	|	ПравилаПреобразованияФорматов.ИтоговыйФормат КАК ИтоговыйФормат,
	|	ПравилаПреобразованияФорматов.ВерсияИтоговогоФормата КАК ВерсияИтоговогоФормата,
	|	ПравилаПреобразованияФорматов.ДатаОбновления КАК ДатаОбновления,
	|	ЛОЖЬ КАК Обработано
	|ПОМЕСТИТЬ ПравилаФормата
	|ИЗ
	|	РегистрСведений.ПравилаПреобразованияФорматов КАК ПравилаПреобразованияФорматов
	|ГДЕ
	|	ПравилаПреобразованияФорматов.ИсходныйФормат = &ИсходныйФормат
	|	И ВЫБОР
	|		КОГДА &ПроверятьВариантЗаполнения
	|			ТОГДА ПравилаПреобразованияФорматов.ВариантЗаполнения = &ВариантЗаполнения
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|	И ВЫБОР
	|		КОГДА &ПроверятьИтоговыйФормат
	|			ТОГДА ПравилаПреобразованияФорматов.ИтоговыйФормат = &ИтоговыйФормат
	|			И ПравилаПреобразованияФорматов.ВерсияИтоговогоФормата В (&МассивВерсийИтоговогоФормата)
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ
	|;
	|
	...
	|	1 КАК Поле1
	|ИЗ
	|	ПравилаФормата КАК ПравилаФормата
	|ГДЕ
	|	ПравилаФормата.ДатаОбновления < &ДатаОбновленияФормата";
	УстановитьПривилегированныйРежим(Истина);
	...
	НаборЗаписей.Отбор.ИсходныйФормат.Установить(Формат);
	Пока ЧтениеXML.Прочитать() Цикл
		Если Не (ЧтениеXML.Имя = "rule" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента) Тогда
			Продолжить;
		КонецЕсли;
		
		ИтоговыйФорматИзXML = ЧтениеXML.ЗначениеАтрибута("out");
		// Если обновление идет по конкретному итоговому формату, то другие не загружаем.
		Если УсловиеПроверятьИтоговыйФормат И ИтоговыйФорматИзXML <> ОписаниеРесурса.ПроверяемыйФормат Тогда
			Продолжить;
		КонецЕсли;

		Отбор.ВариантЗаполнения = ЧтениеXML.ЗначениеАтрибута("type");
		Отбор.ИтоговыйФормат = ИтоговыйФорматИзXML;
