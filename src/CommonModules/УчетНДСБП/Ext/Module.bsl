...
// Возвращает признак того, что переданная система налогообложения по умолчанию предполагает
// использование НДС
//
// Параметры:
...
//  Булево
//
Функция ВыделятьНДСПоУмолчанию(СистемаНалогообложения) Экспорт
	
	Возврат Не ЗначениеЗаполнено(СистемаНалогообложения)
		Или СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая
		Или СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная;
...
Функция ТекстЗапросаИнформацияПоТаможеннымДекларациямПродажи()
	...
	|	1
	|ИЗ
	|	Документ.ПередачаТоваров.Товары КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|		ПО ТаблицаТоваров.Ссылка = СписокСчетовФактур.СчетФактура
	|ГДЕ
	|	ТаблицаТоваров.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПередачаТоваров.БезвозмезднаяПередача)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|			И ТаблицаТоваров.Ссылка = ТаблицаПокупатели.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|		ПО (ТаблицаПокупатели.СчетФактура.Ссылка ЕСТЬ NULL
	|				И ТаблицаПокупатели.Ссылка = СписокСчетовФактур.СчетФактура)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	1
	|ИЗ
	|	Документ.ОтражениеНачисленияНДС.ТоварыИУслуги КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|		ПО ТаблицаТоваров.Ссылка = СписокСчетовФактур.СчетФактура
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Процедура ЗаполнениеДанныхПоДоговорамДляПроверкиВРЭЦ(Параметры) Экспорт
	...
	РеквизитыДекларацииНДС = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.ДекларацияНДС,
		"ДатаНачала, ДатаОкончания, ДанныеОтчета");
	ВключатьОбособленныеПодразделения = Ложь;
	ГруппаОрганизаций = Новый СписокЗначений;
	Если РеквизитыДекларацииНДС.ДанныеОтчета.Получить().Свойство("ГруппаОрганизаций", ГруппаОрганизаций)
...
Функция ПолучитьПризнакВеденияУчетаТаможенныхДекларацийЭкспорт() Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ВедетсяУчетТаможенныхДекларацийЭкспорт");
...
Функция ТекстЗапросаПодготовкаСведенийРНПТЖурнал() Экспорт
	...
	|ГДЕ
	|	НЕ СчетФактураПолученныйДокументыОснования.ДокументОснование ССЫЛКА Документ.ОтчетКомитентуОПродажах
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОснование";
...
// Возвращаемое значение: 
//        ЗадолженностьПоСрокам - ТаблицаЗначений - таблица задолженности по счетам 68.32, 68.42.
//            * Контрагент       - СправочникСсылка.Контрагенты - аналитика задолженности по 68.32, 68.42.
//            * Договор          - СправочникСсылка.ДоговорыКонтрагентов - аналитика задолженности по 68.32, 68.42.
//            * ДокументРасчетов - см. вид субконто ДокументыРасчетовСКонтрагентами - аналитика задолженности по 68.32.
//                                 или см. вид субконто СФПолученные - аналитика задолженности по 68.42. 
...
Функция ЗадолженностьНалоговогоАгентаПоНДС(РеквизитыДокумента) Экспорт
	...
	Запрос.УстановитьПараметр("СчетУчета",                    РеквизитыДокумента.СчетУчета);
	Запрос.УстановитьПараметр("ОграничитьПериодОсмотра",      РеквизитыДокумента.ОграничитьПериодОсмотра);
	Запрос.УстановитьПараметр("ПериодОсмотра",                НачалоДня(РеквизитыДокумента.ДатаДокумента));
	Запрос.УстановитьПараметр("АнализироватьОборот",          НЕ ЗначениеЗаполнено(РеквизитыДокумента.ТекущийДокумент));
	Запрос.УстановитьПараметр("НачалоПериодаОборотов",        НачалоКвартала(РеквизитыДокумента.ДатаДокумента));
	
	ВидыСубконто = Новый Массив();
	ВидыСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	ВидыСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	
	Если РеквизитыДокумента.СчетУчета = ПланыСчетов.Хозрасчетный.НДСНалоговогоАгента Тогда
		...
		ТаблицаЗадолженностей = РаспределитьЗадолженностьПоСрокамУплаты(МассивЗадолженностей, РеквизитыДокумента);
	...
	Иначе
		...
		Запрос.Текст = ТекстЗапросаЗадолженностьПриВвозеТоваровИзСтранТаможенногоСоюза();
		ТаблицаЗадолженностей = Запрос.Выполнить().Выгрузить();
		
	КонецЕсли;
	
	Возврат ТаблицаЗадолженностей;
...
Функция ТекстЗапросаЗадолженностьНалоговогоАгента()
	...
	// - налог, подлежащий уплате в день начисления (нерезидент, эл. услуги) до 2023 года,
	...
	|ПОМЕСТИТЬ ОборотыТекущегоПериода
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериодаОборотов, &ДатаОстатков, Запись, СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНалоговогоАгента), &ВидыСубконто, СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет), , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|ГДЕ
	|	&АнализироватьОборот
	|;
	|
	...
	|
	|ВЫБРАТЬ
	|	ОборотыТекущегоПериода.Субконто1,
	|	ОборотыТекущегоПериода.Субконто2,
	|	ОборотыТекущегоПериода.Субконто3,
	|	ОборотыТекущегоПериода.СуммаОборот
	|ИЗ
	|	ОборотыТекущегоПериода КАК ОборотыТекущегоПериода
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Субконто2
	|;
	...
	|			ТОГДА НАЧАЛОПЕРИОДА(ДанныеПервичныхДокументов.Дата, ДЕНЬ)
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(ДанныеПервичныхДокументов.ДатаРегистратора, ДЕНЬ)
	|	КОНЕЦ КАК КонтрольныйСрок
	|ПОМЕСТИТЬ ВТ_Нерезидент
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Нерезидент.Контрагент КАК Контрагент,
	|	ВТ_Нерезидент.Договор КАК Договор,
	|	ВТ_Нерезидент.ДокументРасчетов КАК ДокументРасчетов,
	|	ВТ_Нерезидент.Сумма КАК Сумма,
	|	ВТ_Нерезидент.СрокУплаты КАК СрокУплаты
	|ИЗ
	|	ВТ_Нерезидент КАК ВТ_Нерезидент
	|ГДЕ
	|	ВТ_Нерезидент.КонтрольныйСрок < ДАТАВРЕМЯ(2023, 1, 1)
	|	И ВЫБОР
	|			КОГДА &ОграничитьПериодОсмотра
	|				ТОГДА ВТ_Нерезидент.КонтрольныйСрок <= НАЧАЛОПЕРИОДА(&ПериодОсмотра, ДЕНЬ)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Резидент.Контрагент КАК Контрагент,
	|	ВТ_Резидент.Договор КАК Договор,
	|	ВТ_Резидент.ДокументРасчетов КАК ДокументРасчетов,
	|	СчетФактура.СуммаНДСДокумента КАК ИсчисленоНДС,
	|	ВТ_Резидент.ОстатокНДС КАК ОстатокНДС,
	|	ВТ_Резидент.ПериодОплатыКонтрагенту КАК ПериодОплатыКонтрагенту
	|ИЗ
	|	ВТ_Резидент КАК ВТ_Резидент
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактура
	|		ПО (СчетФактура.ДокументОснование = ВТ_Резидент.ДокументРасчетов)
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ОграничитьПериодОсмотра
	|				ТОГДА ВТ_Резидент.КонтрольныйСрок <= НАЧАЛОПЕРИОДА(&ПериодОсмотра, ДЕНЬ)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_Нерезидент.Контрагент,
	|	ВТ_Нерезидент.Договор,
	|	ВТ_Нерезидент.ДокументРасчетов,
	|	СчетФактура.СуммаНДСДокумента,
	|	ВТ_Нерезидент.Сумма,
	|	НАЧАЛОПЕРИОДА(ВТ_Нерезидент.СрокУплаты, КВАРТАЛ)
	|ИЗ
	|	ВТ_Нерезидент КАК ВТ_Нерезидент
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактура
	|		ПО (СчетФактура.ДокументОснование = ВТ_Нерезидент.ДокументРасчетов)
	|ГДЕ
	|	ВТ_Нерезидент.КонтрольныйСрок >= ДАТАВРЕМЯ(2023, 1, 1)
	|	И ВЫБОР
	|			КОГДА &ОграничитьПериодОсмотра
...
Функция РаспределитьЗадолженностьПоСрокамУплаты(МассивЗадолженностей, РеквизитыДокумента)
	...
	ЗадолженностьПоСрокам = МассивЗадолженностей[7].Выгрузить();
	ЗадолженностьКРаспределениюПоСрокам = МассивЗадолженностей[8].Выгрузить();
	
	ТекущийКвартал = НачалоКвартала(РеквизитыДокумента.ДатаДокумента);
	
	// Распределим задолженность на суммы по срокам уплаты.
	Для Каждого СтрокаТаблицы Из ЗадолженностьКРаспределениюПоСрокам Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ДокументРасчетов) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ТекстЗапросаЗадолженностьПриВвозеТоваровИзСтранТаможенногоСоюза()
	...
	|	&ПериодОсмотра КАК СрокУплаты,
	|	ХозрасчетныйОстатки.Субконто1 КАК Контрагент,
	|	ХозрасчетныйОстатки.Субконто2 КАК Договор,
	|	ХозрасчетныйОстатки.Субконто3 КАК ДокументРасчетов
	|ПОМЕСТИТЬ ВТ_ОстаткиИОбороты
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаОстатков, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате), &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|				И Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате),
	|			,
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериодаОборотов, &ДатаОстатков, Запись, СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате), &ВидыСубконто, СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет), , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОборотыДтКт.СубконтоДт3,
	|	ХозрасчетныйОборотыДтКт.СчетДт,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ОстаткиИОбороты.СчетУчета КАК СчетУчета,
	|	СУММА(ВТ_ОстаткиИОбороты.Сумма) КАК Сумма,
	|	ВТ_ОстаткиИОбороты.СрокУплаты КАК СрокУплаты,
	|	ВТ_ОстаткиИОбороты.Контрагент КАК Контрагент,
	|	ВТ_ОстаткиИОбороты.Договор КАК Договор,
	|	ВТ_ОстаткиИОбороты.ДокументРасчетов КАК ДокументРасчетов
	|ИЗ
	|	ВТ_ОстаткиИОбороты КАК ВТ_ОстаткиИОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ОстаткиИОбороты.СрокУплаты,
	|	ВТ_ОстаткиИОбороты.СчетУчета,
	|	ВТ_ОстаткиИОбороты.Договор,
	|	ВТ_ОстаткиИОбороты.Контрагент,
	|	ВТ_ОстаткиИОбороты.ДокументРасчетов";
	
КонецФункции

Функция ЗадолженностьНалоговогоАгентаПоНДСПоОтдельнымВидамТоваров(РеквизитыДокумента) Экспорт
	
	Запрос = Новый Запрос;
	КонецОтчетногоКвартала = КонецКвартала(НачалоКвартала(РеквизитыДокумента.ДатаДокумента) - 1);

	Запрос.УстановитьПараметр("ДатаОстатков",
		Новый Граница(КонецДня(КонецОтчетногоКвартала), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("Организация",      РеквизитыДокумента.Организация);
	Запрос.УстановитьПараметр("СрокУплаты",       РеквизитыДокумента.ДатаДокумента);
	Запрос.УстановитьПараметр("СчетУчета",        ПланыСчетов.Хозрасчетный.НДСНалоговогоАгентаПоОтдельнымВидамТоваров);
	Запрос.УстановитьПараметр("ВидыСубконто",     ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет);
	Запрос.УстановитьПараметр("ЗначениеСубконто", Перечисления.ВидыПлатежейВГосБюджет.Налог);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ХозрасчетныйОстатки.Счет КАК СчетУчета,
	|	ХозрасчетныйОстатки.СуммаОстатокКт КАК Сумма,
	...
	|				И Субконто1 = &ЗначениеСубконто) КАК ХозрасчетныйОстатки";
...
Функция СрокУплатыНалогаВБюджет(МесяцУплаты)
	...
	День = 24 * 60 * 60;
	Если НачалоДня(МесяцУплаты) < '20221231' Тогда
		ДатаУплатыНалога = НачалоДня(МесяцУплаты) + 24 * День;
	Иначе
		// С 2023 года срок уплаты НДС - 28 число каждого из трех месяцев, следующих за окончанием квартала.
		ДатаУплатыНалога = НачалоДня(МесяцУплаты) + 27 * День;
	КонецЕсли;
...
Функция СписокСчетовАгентскогоНДС() Экспорт
	
	СписокСчетовАгентскогоНДС = Новый СписокЗначений;
	СписокСчетовАгентскогоНДС.Добавить(ПланыСчетов.Хозрасчетный.НДСНалоговогоАгента);
	СписокСчетовАгентскогоНДС.Добавить(ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате);
	
	Возврат СписокСчетовАгентскогоНДС;
	
КонецФункции

Функция СписокОписанийСчетовНДС() Экспорт
	
	СписокОписанийСчетовНДС = Новый СписокЗначений;
	СписокОписанийСчетовНДС.Добавить(ПланыСчетов.Хозрасчетный.НДС,
		НСтр("ru='Реализация товаров, работ, услуг'"));
	СписокОписанийСчетовНДС.Добавить(ПланыСчетов.Хозрасчетный.НДСНалоговогоАгента,
		НСтр("ru='Налоговый агент'"));
	СписокОписанийСчетовНДС.Добавить(ПланыСчетов.Хозрасчетный.НДСНалоговогоАгентаПоОтдельнымВидамТоваров,
		НСтр("ru='Налоговый агент по отдельным видам товаров'"));
	
	Возврат СписокОписанийСчетовНДС;
...
// Формирует движения документа по НДС для возврата товаров комиссионеру (корректировка отгрузки комиссионеру).
//
// Параметры:
//  ТаблицаТовары					 - ТаблицаЗначений - Таблица возвращаемых товаров, структуру см. в ПодготовитьПараметрыВозвратОтКомиссионера()
//  ТаблицаСписанныеТоварыБухУчет	 - ТаблицаЗначений - Списанные товары по партиям, см. таблицу СписаниеМатериалов из УчетТоваров.ПодготовитьТаблицыСписанныеТовары().
//  ТаблицаРеквизиты				 - ТаблицаЗначений - Таблица с реквизитами документа, должна содержать одну строку
//  Движения						 - НаборДвижений - Набор движений документа.
//  Отказ							 - Булево - Признак отказа в проведении.
//
Процедура СформироватьДвиженияВозвратОтКомиссионера(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Параметры = ПодготовитьПараметрыВозвратОтКомиссионера(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты);
...
Функция ПодготовитьПараметрыВозвратОтКомиссионера(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
...
Функция ВыделятьНДСприПоступленииТоваровУслуг(Организация, Дата) Экспорт
	...
	ПлательщикНДСНаУСНСВычетами = УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Организация, Дата);
	...
	ВыделятьНДС = СистемаНалогообложения <> Перечисления.СистемыНалогообложения.ОсобыйПорядок
		...
		И Не (СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН)
...
#Область ВыпускПродукцииПриРеализации

// ВЫПУСК ПРОДУКЦИИ ПРИ РЕАЛИЗАЦИИ

Процедура СформироватьДвиженияОприходованияИПродажиПродукции(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Параметры = ПодготовитьПараметрыОприходованияИПродажиПродукции(ТаблицаТовары, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	...
	УчетНДСПереопределяемый.СформироватьДвиженияОприходованияИПродажиПродукции(Параметры.Товары, Реквизиты, Движения, Отказ);
	
КонецПроцедуры

Функция ПодготовитьПараметрыОприходованияИПродажиПродукции(ТаблицаТовары, ТаблицаРеквизиты)
...
Процедура СформироватьДвиженияПересортицаТоваров(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Параметры = ПодготовитьПараметрыПересортицаТоваровНедостача(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	
	СформироватьПроводкиСписаниеГТД(Параметры.Товары, Реквизиты, Движения, Отказ);
	
	Параметры.СписанныеТоварыБухУчет.ЗаполнитьЗначения(ПланыСчетов.Хозрасчетный.ПрочиеРасходы, "КорСчетСписания");
	Параметры.СписанныеТоварыБухУчет.ЗаполнитьЗначения(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы, "ВидКорСубконто1");
	Параметры.СписанныеТоварыБухУчет.ЗаполнитьЗначения(КлассификаторыДоходовРасходов.ПредопределенныйЭлемент(
		Справочники.ПрочиеДоходыИРасходы, "ПересортицаТоваровСписание"), "КорСубконто1");
	
	Параметры.ТоварыОприходование.ЗагрузитьКолонку(Параметры.ТоварыОприходование.ВыгрузитьКолонку("НоменклатураОприходование"), "Номенклатура");
	Параметры.ТоварыОприходование.Колонки.Добавить("Партия", Документы.ТипВсеСсылки());
	Параметры.ТоварыОприходование.ЗаполнитьЗначения(Реквизиты.Регистратор, "Партия");
	...
	Если РаздельныйУчетНДС Тогда
		...
		УчетНДС.ЗаполнитьВидыЦенностей(Параметры.ТоварыОприходование, Неопределено, "СчетУчета");
		СписанныеПартииНДС = УчетНДСПереопределяемый.ПолучитьТаблицуСписанныеПартииНДС(
			Параметры.Товары, Параметры.СписанныеТоварыБухУчет, Реквизиты, Отказ);
		УчетНДСПереопределяемый.СформироватьДвиженияВыбытиеТоваров(
			СписанныеПартииНДС, Параметры.СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ);
		УчетНДСПереопределяемый.СформироватьДвиженияОприходованиеТоваров(
			Параметры.ТоварыОприходование, Реквизиты, Движения, Отказ);  
...
Функция ПодготовитьПараметрыПересортицаТоваровНедостача(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
	...
	+ "Подразделение"             // <Ссылка на справочник подразделений>
	;
	Параметры.Вставить("Реквизиты", ОбщегоНазначенияБПВызовСервера
	...
	// Для списания на расходы ранее принятого к вычету НДС счет и аналитика списания товара не используются
	Параметры.Реквизиты.Колонки.Добавить("СписыватьНДСнаКорСчетИАналитикуТовара", Новый ОписаниеТипов("Булево")); // Ложь
	// НДС, ранее принятый к вычету, при недостаче всегда относится на счет 91.02 "Прочие расходы"
	Параметры.Реквизиты.Колонки.Добавить("СчетСписанияНДС", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	Параметры.Реквизиты.ЗаполнитьЗначения(ПланыСчетов.Хозрасчетный.ПрочиеРасходы, "СчетСписанияНДС");
	Параметры.Реквизиты.Колонки.Добавить("СубконтоСписанияНДС1", Новый ОписаниеТипов("Неопределено"));
	Параметры.Реквизиты.Колонки.Добавить("СубконтоСписанияНДС2", Новый ОписаниеТипов("Неопределено"));
	Параметры.Реквизиты.Колонки.Добавить("СубконтоСписанияНДС3", Новый ОписаниеТипов("Неопределено"));
	...
	+ "НовыйСпособУчетаНДС"
	;
	Параметры.Вставить("Товары", ОбщегоНазначенияБПВызовСервера
		.ПолучитьТаблицуПараметровПроведения(ТаблицаТовары, СписокОбязательныхКолонок));
	
	// Подготовка таблица оприходованных товаров документа.
	СписокОбязательныхКолонок = ""
	+ "Номенклатура,"              // <СправочникСсылка.Номенклатура>
	...
	+ "Количество"                 // <Число,15,3>
	;
	Параметры.Вставить("ТоварыОприходование", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		Параметры.Товары, СписокОбязательныхКолонок));	
...
// Конструктор таблиц товаров для формирования движений по НДС при списании товаров на расходы.
// См. первый парметр СформироватьДвиженияСписаниеТоваровНаРасходы().
// 
// Возвращаемое значение:
//   - ТаблицаЗначений
//
Функция НовыйТаблицаТоварыДляСписанияНаРасходы() Экспорт
	
	ТоварыДляСписания = Новый ТаблицаЗначений;
	ТоварыДляСписания.Колонки.Добавить("ИмяСписка", ОбщегоНазначения.ОписаниеТипаСтрока(100));
	ТоварыДляСписания.Колонки.Добавить("СинонимСписка", ОбщегоНазначения.ОписаниеТипаСтрока(100));
	ТоварыДляСписания.Колонки.Добавить("НомерСтрокиДокумента", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0));
	...
	ТоварыДляСписания.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	ТоварыДляСписания.Колонки.Добавить("СтранаПроисхождения", Новый ОписаниеТипов("СправочникСсылка.СтраныМира"));
	ТоварыДляСписания.Колонки.Добавить("НомерГТД", Новый ОписаниеТипов("СправочникСсылка.НомераГТД"));
	ТоварыДляСписания.Колонки.Добавить("НовыйСпособУчетаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СпособыУчетаНДС"));
	Возврат ТоварыДляСписания;
	
КонецФункции

// Формирует движения документа по НДС для списания товаров на расходы.
//
// Параметры:
//  ТаблицаТовары					 - ТаблицаЗначений - Таблица товаров для списания, структуру см. в НовыйТаблицаТоварыДляСписанияНаРасходы()
//  ТаблицаСписанныеТоварыБухУчет	 - ТаблицаЗначений - Списанные товары по партиям, см. таблицу СписаниеМатериалов из УчетТоваров.ПодготовитьТаблицыСписанныеТовары().
//  ТаблицаРеквизиты				 - ТаблицаЗначений - Таблица с реквизитами документа, должна содержать одну строку
//  Движения						 - НаборДвижений - Набор движений документа.
//  Отказ							 - Булево - Признак отказа в проведении.
...
Процедура СформироватьДвиженияНДСПоПриобретеннымЦенностямСписаниеАвтоматическиВыпущеннойПродукции(Товары, Реквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Запись = Движения.НДСПоПриобретеннымЦенностям.ДобавитьРасход();
...
Процедура СформироватьПроводкиВключениеНДСВСтоимостьЗапасов(СтрокиТаблицыУменьшениеВычета, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ)
	...
	Для каждого СтрокаТаблицы Из СтрокиТаблицыУменьшениеВычета Цикл
		...
		ВалютаСчетаДт   = Константы.ВалютаРегламентированногоУчета.Получить();
		Для НомерСубконто = 1 По 3 Цикл
			
			Если СвойстваСчетаДт.Валютный 
				И ТипЗнч(СтрокаСписанияБухУчет["КорСубконто" + НомерСубконто]) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				ВалютаСчетаДт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСписанияБухУчет["КорСубконто" + НомерСубконто], "ВалютаВзаиморасчетов");
			КонецЕсли;
		...
		Если СвойстваСчетаДт.Валютный Тогда
			Проводка.ВалютаДт = ВалютаСчетаДт;
		КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из СтрокиВключениеВСтоимость Цикл
		...
		Если Реквизиты.СписыватьНДСнаКорСчетИАналитикуТовара Тогда
			...
			Для Ном = 1 По 3 Цикл
				...
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
					СтрокаСписанияБухУчет["ВидКорСубконто" + Ном], СтрокаСписанияБухУчет["КорСубконто" + Ном]);
				
				Если СвойстваСчетаДт.Валютный 
					И ТипЗнч(СтрокаСписанияБухУчет["КорСубконто" + Ном]) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
					ВалютаСчетаДт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСписанияБухУчет["КорСубконто" + Ном], "ВалютаВзаиморасчетов");
				КонецЕсли;
...
Функция ПодготовитьТаблицуКорректировкаНДСПоПартиямЗапасов(Реквизиты)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКорректировки Цикл 
		...
		СтрокаТаблицы.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
			СтрокаТаблицы.Стоимость, Истина, УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтрокаТаблицы.СтавкаНДС));
	КонецЦикла;
	...
	|	ВТТаблицаДокумента.СуммаНДС * (&Курс / &Кратность) КАК НДС,
...
Функция ПолучитьТабличнуюЧастьДокументаПоИмениСписка(ИмяСписка, НомерСтрокиДокумента)
	...
	ИначеЕсли ИмяСпискаВРег = "ПРАВА" Тогда
		ТабличнаяЧастьДокумента = Перечисления.ТабличныеЧастиДокументов.Права;
	...
	ИначеЕсли ИмяСпискаВРег = "ПРЕДМЕТЫАРЕНДЫ" Тогда
		ТабличнаяЧастьДокумента = Перечисления.ТабличныеЧастиДокументов.ПредметыАренды;
	ИначеЕсли ИмяСпискаВРег = "ТМЦ" Тогда
		ТабличнаяЧастьДокумента = Перечисления.ТабличныеЧастиДокументов.ТМЦ;
	Иначе
...
Процедура ВыполнитьДвиженияРублевыеСуммыДокументовВВалюте(ТаблицаДокумента, Реквизиты, НДСВключенВСтоимость, БезНДС, Движения)
	...
	Если ТаблицаДокумента.Колонки.Найти("ИдентификаторСтроки") <> Неопределено Тогда
	...
	Если ЕстьИдентификаторСтроки Тогда 
		Если БезНДС Тогда
			ТаблицаДокумента.Свернуть("ИмяСписка, НомерСтроки, ИдентификаторСтроки", "СуммаРуб");
		Иначе
			ТаблицаДокумента.Свернуть("ИмяСписка, НомерСтроки, ИдентификаторСтроки", "СуммаБУ, СуммаНДСРуб, СуммаБезНДСРуб");
		КонецЕсли;
	Иначе
		Если БезНДС Тогда
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
		...
		Движение.Организация				= Реквизиты.Организация;
		Движение.ИдентификаторСтроки		= ?(ЕстьИдентификаторСтроки, СтрокаТаблицы.ИдентификаторСтроки, "");
...
Функция ПодготовитьПараметрыРублевыеСуммыДокументовВВалюте(ТаблицаДокумента, ТаблицаРеквизиты)
	...
	Если ТаблицаДокумента <> Неопределено И ТаблицаДокумента.Колонки.Найти("ИдентификаторСтроки") <> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ", ИдентификаторСтроки";
	КонецЕсли;
...
Функция ПечатьСчетовФактур(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати, ПараметрыВывода) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		УправлениеПечатьюБП.ДобавитьПодписантовДокумента(ПараметрыВывода, ВыборкаСФ.СчетФактура,
			"РуководительСчетФактура", "Руководитель", ДанныеШапки);
		УправлениеПечатьюБП.ДобавитьПодписантовДокумента(ПараметрыВывода, ВыборкаСФ.СчетФактура,
			"ГлавныйБухгалтерСчетФактура", "ГлавныйБухгалтер", ДанныеШапки);
		...
		Если УправлениеПечатьюБП.ЭтоПечатнаяФормаЗаПоставщика(ВыборкаСФ.СчетФактура) Тогда
			УправлениеПечатьюБП.ОчиститьФаксимильнуюПодписьИПечатьИзМакета(ТабДокумент, ОбъектыПечати, ВыборкаСФ.СчетФактура);
		Иначе
			Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаСФ.СчетФактура, "Организация");
			УправлениеПечатьюБП.ДоработатьФаксимильнуюПодписьИПечать(ТабДокумент, ОбъектыПечати,
				Организация, ВыборкаСФ.СчетФактура);
		КонецЕсли;
...
Функция КППНаДату(Ссылка, ДатаСведений) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.Организации") Тогда
		Возврат Справочники.Организации.КППНаДату(Ссылка, ДатаСведений, Ложь);
...
Функция ПечатьКорректировочныхСчетовФактур(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати, ПараметрыВывода) Экспорт
...
Функция ПолучитьТаблицуПоступлениеТоваровУслуг(ДокументСсылка, Ошибка)
	...
	|	ПоступлениеТоваровУслугОборудование.НомерГТД КАК НомерГТД,
	...
	|	ПоступлениеТоваровУслугПрава.Количество,
	|	ПоступлениеТоваровУслугПрава.Цена,
	|	ПоступлениеТоваровУслугПрава.Сумма,
	|	ВЫБОР
	|		КОГДА ПоступлениеТоваровУслугПрава.Ссылка.СуммаВключаетНДС
	...
	|	NULL,
	|	NULL,
	|	ПоступлениеТоваровУслугПрава.Счет,
	|	ПоступлениеТоваровУслугПрава.СчетУчетаНДС,
	|	ПоступлениеТоваровУслугПрава.Счет,
	|	ПоступлениеТоваровУслугПрава.НомерСтроки,
	|	ЛОЖЬ
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.Права КАК ПоступлениеТоваровУслугПрава
	|ГДЕ
	|	ПоступлениеТоваровУслугПрава.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерТаблицы,
	|	НомерСтроки";
...
Функция ПолучитьТаблицуСчетФактураВыданный(ДокументСсылка, Ошибка)
	...
	Если РеквизитыДокумента.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАванс Тогда
		...
		НоваяСтрока.СтавкаНДС = Перечисления.СтавкиНДС.РасчетнаяСтавкаНДСПоОбычной(РеквизитыДокумента.СтавкаНДС);
	Иначе
...
Функция ПечатьСводныхСправок(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСводнымСправкам, ПараметрыПечати) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Если Реквизиты.Дата >= '20250101' Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_СводнаяСправка2025");
		Иначе
		...
		ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокТаблицы");
		ОбластьМакета.Параметры.ОсновнаяСтавкаНДС = Строка(УчетНДСКлиентСервер.ОбщаяСтавкаНДС(Реквизиты.Дата));
		ТабДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Итого");
		ОбластьМакета.Параметры.ИтогоВсего               = ТаблицаДокумента.Итог("Всего");
		ОбластьМакета.Параметры.ИтогоСуммаНДСОсновная    = ТаблицаДокумента.Итог("СуммаНДСОсновная");
		ОбластьМакета.Параметры.ИтогоСуммаБезНДСОсновная = ТаблицаДокумента.Итог("СуммаБезНДСОсновная");
		...
		Если Реквизиты.Дата >= '20250101' Тогда
			ОбластьМакета.Параметры.ИтогоСуммаНДС7    = ТаблицаДокумента.Итог("СуммаНДС7");
			ОбластьМакета.Параметры.ИтогоСуммаБезНДС7 = ТаблицаДокумента.Итог("СуммаБезНДС7");
			ОбластьМакета.Параметры.ИтогоСуммаНДС5    = ТаблицаДокумента.Итог("СуммаНДС5");
			ОбластьМакета.Параметры.ИтогоСуммаБезНДС5 = ТаблицаДокумента.Итог("СуммаБезНДС5");
		КонецЕсли;
...
Функция ПодготовитьДанныеДляПечатиСводныхСправок(Ссылка)
	...
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	|				ТОГДА НДСЗаписиКнигиПродаж.СуммаБезНДС
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаБезНДСОсновная,
	|	СУММА(ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	...
	|	СУММА(ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|				ТОГДА НДСЗаписиКнигиПродаж.СуммаБезНДС
	|			ИНАЧЕ 0
	...
	|		КОНЕЦ) КАК СуммаНДС7,
	|	СУММА(ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	|					ИЛИ НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|				ТОГДА НДСЗаписиКнигиПродаж.СуммаБезНДС
	|			ИНАЧЕ 0
	...
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ПриходныйКассовыйОрдер
	...
	|		КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОплатаПлатежнойКартой
...
#Область ЗаполнениеДекларацииПоНДС
...
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета) Экспорт
	...
	Если УчетНДСВызовСервера.КлассифицироватьНоменклатуруПоОперациям0(ДатаЗапроса, Организация) Тогда
		...
		Запрос.УстановитьПараметр("ДоступныеКодыОпераций", КодыОперацийРеестрНДСПриложение(ПараметрыОтчета));
		ТекстЗапросаРеестрНДСПриложениеКодыОпераций(Запрос);
		
	Иначе
		
		ТекстЗапросаРеестрНДСПриложениеКодыОперацийТаможенныхДеклараций(Запрос);
		
	КонецЕсли;
	
	Если ПараметрыОтчета.ИДОтчета = "РегламентированныйОтчетРеестрНДСПриложение5" Тогда
		Запрос.Текст = СтрШаблон("%1, КодТС",Запрос.Текст);
	КонецЕсли;
	
КонецПроцедуры

Функция КодыОперацийРеестрНДСПриложение (ПараметрыОтчета)
	
	МассивКодовОпераций = Новый Массив;
	
	Макет = Отчеты[ПараметрыОтчета.ИДОтчета].ПолучитьМакет(ПараметрыОтчета.СпискиВыбора);
...
Процедура ТекстЗапросаРеестрНДСПриложениеКодыОпераций(Запрос)
	...
	|			ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.РеализацияТоваровУслуг).ДоговорКонтрагента
	|		КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.РеализацияУслугПоПереработке
	|			ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.РеализацияУслугПоПереработке).ДоговорКонтрагента
	|		КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ПередачаОС
	|			ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.ПередачаОС).ДоговорКонтрагента
	|	КОНЕЦ КАК ДоговорКонтрагента
	|ПОМЕСТИТЬ ПодтвердженныеОтгрузки
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияТаможенныхДекларацийЭкспорт КАК СведенияТаможенныхДекларацийЭкспорт
	|		ПО НДСЗаписиКнигиПродаж.СчетФактура = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
	|ГДЕ
	...
	|	И НДСЗаписиКнигиПродаж.КодОперации0 В(&ДоступныеКодыОпераций)
	|	И (НДСЗаписиКнигиПродаж.НДС > 0
	|			ИЛИ НДСЗаписиКнигиПродаж.СуммаБезНДС > 0)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СведенияТаможенныхДекларацийЭкспорт.НомерТСД,
	|	СведенияТаможенныхДекларацийЭкспорт.ДатаТСД,
	|	ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.РеализацияОтгруженныхТоваров).ДоговорКонтрагента
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияТаможенныхДекларацийЭкспорт КАК СведенияТаможенныхДекларацийЭкспорт
	|		ПО НДСЗаписиКнигиПродаж.СчетФактура.ДокументОтгрузки = СведенияТаможенныхДекларацийЭкспорт.ДокументОтгрузки
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Организация В(&Организация)
	|	И НДСЗаписиКнигиПродаж.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НДСЗаписиКнигиПродаж.Событие = &Событие
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПодтвердженныеОтгрузки.ПодтвержденнаяНалоговаяБаза КАК ПодтвержденнаяНалоговаяБаза,
	|	ПодтвердженныеОтгрузки.НомерТаможеннойДекларации КАК НомерТаможеннойДекларации,
	|	ПодтвердженныеОтгрузки.Примечание КАК Примечание,
	|	ПодтвердженныеОтгрузки.КодОперации КАК КодОперации,
	|	ПодтвердженныеОтгрузки.КодТС КАК КодТС,
	|	ПодтвердженныеОтгрузки.ВидДокумента КАК ВидДокумента,
	|	ПодтвердженныеОтгрузки.НомерТСД КАК НомерТСД,
	|	ПодтвердженныеОтгрузки.ДатаТСД КАК ДатаТСД,
	...
	|ИЗ
	|	ПодтвердженныеОтгрузки КАК ПодтвердженныеОтгрузки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО ПодтвердженныеОтгрузки.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
	|ИТОГИ
	|	СУММА(ПодтвержденнаяНалоговаяБаза),
	|	МАКСИМУМ(Примечание),
...
Процедура ТекстЗапросаРеестрНДСПриложениеКодыОперацийТаможенныхДеклараций(Запрос)
	...
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.РеализацияТоваровУслуг).ДоговорКонтрагента
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияУслугПоПереработке
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.РеализацияУслугПоПереработке).ДоговорКонтрагента
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияОтгруженныхТоваров
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.РеализацияОтгруженныхТоваров).ДоговорКонтрагента
	|		КОГДА ПодтвержденныеОтгрузки.ДокументОтгрузки ССЫЛКА Документ.ПередачаОС
	|			ТОГДА ВЫРАЗИТЬ(ПодтвержденныеОтгрузки.ДокументОтгрузки КАК Документ.ПередачаОС).ДоговорКонтрагента
	|	КОНЕЦ КАК ДоговорКонтрагента
...
Функция ТекстЗапросаРаздел5КодыОпераций()
	...
	|	И НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
	...
	|	СУММА(ДляИтогов.НалоговаяБазаПоРанееПодтвержденнойНулевойСтавке) КАК НалоговаяБазаПоРанееПодтвержденнойНулевойСтавке,
	|	СУММА(ДляИтогов.ВычетНДСПоРанееПодтвержденнойНулевойСтавке) КАК ВычетНДСПоРанееПодтвержденнойНулевойСтавке,
	|	СУММА(ДляИтогов.НалоговаяБазаПоРанееНеПодтвержденнойНулевойСтавке) КАК НалоговаяБазаПоРанееНеПодтвержденнойНулевойСтавке,
...
Функция ТекстЗапросаРаздел5КодыОперацийТаможенныхДеклараций()
	...
	|	ЕСТЬNULL(СведенияТаможенныхДекларацийЭкспорт.КодОперации, &КодОперацииПоУмолчанию)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ НалоговаяБазаИВычет
	...
	|	НалоговаяБазаИВычетПоКодамОперацийДетальныеЗаписи.ПодтвержденнаяНалоговаяБаза КАК НалоговаяБазаПоРанееПодтвержденнойНулевойСтавке,
...
Функция ТекстЗапросаНаборДанныхНачислениеРеализация0(Запрос) Экспорт
	...
	ДатаЗапроса = Запрос.Параметры.КонецПериода;
	Организация = Запрос.Параметры.Организация;
	
	Если УчетНДСВызовСервера.КлассифицироватьНоменклатуруПоОперациям0(ДатаЗапроса,Организация) Тогда
		Возврат ТекстЗапросаНаборДанныхНачислениеРеализация0КодыОпераций();
	Иначе
		Возврат ТекстЗапросаНаборДанныхНачислениеРеализация0КодыОперацийТаможенныхДеклараций();
	КонецЕсли;
	
КонецФункции

Функция ТекстЗапросаНаборДанныхНачислениеРеализация0КодыОпераций()

	Возврат
	"ВЫБРАТЬ
	|	НДСРеализация0Остатки.СчетФактура КАК ДокументРегистратор,
	|	&КодОперацииПоУмолчанию КАК КодВидаОперации,
	...
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Организация В(&Организация)
	|	И НЕ НДСЗаписиКнигиПродаж.ВидЦенности В (&ВидыЦенностейАвансыВсе, &ВидыЦенностейНалоговыйАгент)
	|	И НДСЗаписиКнигиПродаж.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НЕ НДСЗаписиКнигиПродаж.ЗаписьДополнительногоЛиста
	|	И НДСЗаписиКнигиПродаж.Событие В (ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ПодтвержденаСтавка0), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НеПодтвержденаСтавка0))
	|	И НДСЗаписиКнигиПродаж.Активность
	|	И (НДСЗаписиКнигиПродаж.НДС > 0
	|			ИЛИ НДСЗаписиКнигиПродаж.СуммаБезНДС > 0)
	|
...
Функция ТекстЗапросаНаборДанныхНачислениеРеализация0КодыОперацийТаможенныхДеклараций()
	...
	|ГДЕ
	|	(КнигаПродаж.СуммаБезНДСОборот > 0
	|			ИЛИ КнигаПродаж.НДСОборот > 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ТаможенныеДекларацииПоЭкспорту.КодОперации, &КодОперацииПоУмолчанию) КАК КодВидаОперации
	|ИЗ
...
Функция ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0(Запрос) Экспорт
	...
	Если УчетНДСВызовСервера.КлассифицироватьНоменклатуруПоОперациям0(ДатаЗапроса,Организация) Тогда
		Возврат ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0КодыОпераций();
	Иначе
		Возврат ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0КодыОперацийТаможенныхДеклараций();
	КонецЕсли;
	
КонецФункции

Функция ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0КодыОпераций()
	...
	|	СоставКнигиПокупок.КодОперации0 КАК КодОперации,
	|	ПодтверждениеНулевойСтавкиНДССостав.СчетФактураВыданный КАК СчетФактураВыданныйНеПодтведжениеСтавки0
	...
	|		ПО (ПодтверждениеНулевойСтавкиНДССостав.ДокументОтгрузки = СоставКнигиПокупок.ДокументОтгрузки)
	|ГДЕ
	...
	|	И СоставКнигиПокупок.Событие В (ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычету0), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.НеПодтвержденаСтавка0), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахПодтвержденаСтавка0), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахНеПодтвержденаСтавка0), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПодтвержденаСтавка0))
	...
	|	СоставКнигиПокупок.КодОперации0,
	|	ПодтверждениеНулевойСтавкиНДССостав.СчетФактураВыданный
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК СоставКнигиПокупок
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПодтверждениеНулевойСтавкиНДС.Состав КАК ПодтверждениеНулевойСтавкиНДССостав
	|		ПО (ПодтверждениеНулевойСтавкиНДССостав.ДокументОтгрузки = СоставКнигиПокупок.ДокументОтгрузки)
	...
	|	СоставКнигиПокупок.ДокументОтгрузки КАК ДокументОтгрузки,
	|	СоставКнигиПокупок.КодОперации КАК КодВидаОперации,
	|	СоставКнигиПокупок.СтавкаНДС КАК СтавкаНДС,
	...
	|	СоставКнигиПокупок.ЗаписьДополнительногоЛиста КАК ЗаписьДополнительногоЛиста,
	|	СоставКнигиПокупок.НомерСтроки КАК НомерСтроки,
	|	СоставКнигиПокупок.СчетФактураВыданныйНеПодтведжениеСтавки0 КАК СчетФактураВыданныйНеПодтведжениеСтавки0
	|ПОМЕСТИТЬ ДляОпределенияСобытияПодтвержденияСтавки0
	|ИЗ
	...
	|	ДокументОтгрузки,
	|	КодВидаОперации,
	|	СчетФактураВыданныйНеПодтведжениеСтавки0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДокументРегистратор КАК ДокументРегистратор,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки КАК ДокументОтгрузки,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.КодВидаОперации КАК КодВидаОперации,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СтавкаНДС КАК СтавкаНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.Сумма КАК Сумма,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаНДС КАК СуммаНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ПризнакНаличияОшибок КАК ПризнакНаличияОшибок,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ПризнакОтраженияНДС КАК ПризнакОтраженияНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.КоличествоДокументов КАК КоличествоДокументов,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаВключенные КАК СуммаВключенные,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаНеВключенные КАК СуммаНеВключенные,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактура КАК СчетФактура,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.Событие КАК Событие,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ВидЦенности КАК ВидЦенности,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ЗаписьДополнительногоЛиста КАК ЗаписьДополнительногоЛиста,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.НомерСтроки КАК НомерСтроки,
	|	НДСЗаписиКнигиПродаж.Событие КАК СобытиеПодтвержденияСтавки0,
	|	ВЫБОР
	|		КОГДА НДСЗаписиКнигиПродаж.ДатаСобытия >= ДАТАВРЕМЯ(2024, 1, 1)
	|			ТОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.ДатаСобытия, КВАРТАЛ)
	|		ИНАЧЕ НАЧАЛОПЕРИОДА(ДанныеПервичныхДокументов.Дата, КВАРТАЛ)
	|	КОНЕЦ КАК НалоговыйПериод
	|ИЗ
	|	ДляОпределенияСобытияПодтвержденияСтавки0 КАК ДляОпределенияСобытияПодтвержденияСтавки0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ПО (НДСЗаписиКнигиПродаж.КодОперации0 = ДляОпределенияСобытияПодтвержденияСтавки0.КодВидаОперации)
	|			И (НДСЗаписиКнигиПродаж.СчетФактура = ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактураВыданныйНеПодтведжениеСтавки0)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО (ДанныеПервичныхДокументов.Документ = ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки)
	|ГДЕ
	|	НЕ НДСЗаписиКнигиПродаж.НесырьевыеТовары
	|	И НДСЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НеПодтвержденаСтавка0)
	|	И НДСЗаписиКнигиПродаж.Активность
	|	И ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактура.Проведен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДокументРегистратор,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.КодВидаОперации,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СтавкаНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.Сумма,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ПризнакНаличияОшибок,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ПризнакОтраженияНДС,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.КоличествоДокументов,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаВключенные,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СуммаНеВключенные,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.НомерВходящегоДокумента,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ДатаВходящегоДокумента,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактура,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.Событие,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ВидЦенности,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.ЗаписьДополнительногоЛиста,
	|	ДляОпределенияСобытияПодтвержденияСтавки0.НомерСтроки,
	|	НДСЗаписиКнигиПродаж.Событие,
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.ДатаСобытия, КВАРТАЛ)
	|ИЗ
	|	ДляОпределенияСобытияПодтвержденияСтавки0 КАК ДляОпределенияСобытияПодтвержденияСтавки0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ПО ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки = НДСЗаписиКнигиПродаж.СчетФактура
	|			И ДляОпределенияСобытияПодтвержденияСтавки0.КодВидаОперации = НДСЗаписиКнигиПродаж.КодОперации0
	|ГДЕ
	|	НЕ НДСЗаписиКнигиПродаж.НесырьевыеТовары
	|	И НДСЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ПодтвержденаСтавка0)
	|	И НДСЗаписиКнигиПродаж.Активность
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДляОпределенияСобытияПодтвержденияСтавки0";

КонецФункции

Функция ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0КодыОперацийТаможенныхДеклараций()
	...
	|	ВычетНДСПоСтавке0Предварительная.СчетФактураВыданныйНеПодтведжениеСтавки0 КАК СчетФактураВыданныйНеПодтведжениеСтавки0
	|ПОМЕСТИТЬ ДляОпределенияСобытияПодтвержденияСтавки0
	|ИЗ
	...
	|ГДЕ
	|	ВычетНДСПоСтавке0Предварительная.СчетФактура.Проведен
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОтгрузки,
	|	ВычетНДСПоСтавке0Предварительная.СчетФактураВыданныйНеПодтведжениеСтавки0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ДляОпределенияСобытияПодтвержденияСтавки0.НомерСтроки КАК НомерСтроки,
	|	НДСЗаписиКнигиПродаж.Событие КАК СобытиеПодтвержденияСтавки0,
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.ДатаСобытия, КВАРТАЛ) КАК НалоговыйПериод
	|ИЗ
	|	ДляОпределенияСобытияПодтвержденияСтавки0 КАК ДляОпределенияСобытияПодтвержденияСтавки0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ПО ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки = НДСЗаписиКнигиПродаж.СчетФактура
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ПодтвержденаСтавка0)
	|	И НДСЗаписиКнигиПродаж.Активность
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ДляОпределенияСобытияПодтвержденияСтавки0 КАК ДляОпределенияСобытияПодтвержденияСтавки0
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ПО ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактураВыданныйНеПодтведжениеСтавки0 = НДСЗаписиКнигиПродаж.СчетФактура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО (ДанныеПервичныхДокументов.Документ = ДляОпределенияСобытияПодтвержденияСтавки0.ДокументОтгрузки)
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.НеПодтвержденаСтавка0)
	|	И НДСЗаписиКнигиПродаж.Активность
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
