...
// Параметры:
//  Команда - См. УсловиеВидимости
//  ДанныеСтроки - ДанныеФормыСтруктура:
//  * Ссылка - ЛюбаяСсылка
//  РезультатыВычисленияМассовыхАлгоритмов - См. ВычислитьЗначенияМассовыхАлгоритмов
// 
// Возвращаемое значение:
//  Булево
Функция УсловиеВидимостиВычисленоМассовымАлгоритмом(Команда, ДанныеСтроки, РезультатыВычисленияМассовыхАлгоритмов) Экспорт
	Если Не ЗначениеЗаполнено(Команда.ИмяАлгоритмаМассовойПроверкиУсловия) Тогда
		Возврат Ложь;
	КонецЕсли;
	Если РезультатыВычисленияМассовыхАлгоритмов[Команда.ИмяАлгоритмаМассовойПроверкиУсловия][ДанныеСтроки.Ссылка] = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
КонецФункции
...
//  * Ключ - Строка - ИмяАлгоритмаМассовойПроверкиУсловия из См. ВычислитьЗначенияМассовыхАлгоритмов.КомандыСУсловиямиВидимости
//  * Значение - Соответствие Из КлючИЗначение:
//  ** Ключ - ЛюбаяСсылка - Ссылка из См. ВычислитьЗначенияМассовыхАлгоритмов.Строки
//  ** Значение - См. ВычислитьЗначениеАлгоритма 
Функция ВычислитьЗначенияМассовыхАлгоритмов(КомандыСУсловиямиВидимости, Строки) Экспорт
	ПараметрАлгоритма = Новый Массив;
	Для Каждого ОписаниеСтрокиДинамическогоСписка Из Строки Цикл
		ПараметрАлгоритма.Добавить(ОписаниеСтрокиДинамическогоСписка.Значение.Данные.Ссылка);
	КонецЦикла;
	РезультатыВычисленияМассовыхАлгоритмов = Новый Соответствие;
	Для Каждого Команда Из КомандыСУсловиямиВидимости Цикл
		Если РезультатыВычисленияМассовыхАлгоритмов[Команда.ИмяАлгоритмаМассовойПроверкиУсловия] = Неопределено Тогда
			РезультатыВычисленияМассовыхАлгоритмов[Команда.ИмяАлгоритмаМассовойПроверкиУсловия] = ВычислитьЗначениеАлгоритма(
				Команда.ИмяАлгоритмаМассовойПроверкиУсловия, ПараметрАлгоритма);
		КонецЕсли;
	КонецЦикла;
	Возврат РезультатыВычисленияМассовыхАлгоритмов;
КонецФункции

#Область КомандыЛегкогоИнтерфейса
	
// Размещает команды ЭДО легкого интерфейса на форме.
//
// Параметры:
//   ПараметрыРазмещенияКоманд - См. ПодключаемыеКомандыЭДО.ПараметрыРазместитьНаФормеКомандыЭДОЛегкогоИнтерфейса
//
Процедура РазместитьНаФормеКомандыЭДОЛегкогоИнтерфейса(ПараметрыРазмещенияКоманд) Экспорт

	Форма = ПараметрыРазмещенияКоманд.Форма;
	МестоРазмещенияКоманд = ПараметрыРазмещенияКоманд.МестоРазмещенияКоманд;

	КомандыЭДО = ПодключаемыеКомандыЭДОСлужебныйПовтИсп.КомандыЭДОЛегкогоИнтерфейса();

	ДобавитьКомандыЭДО(Форма, КомандыЭДО, МестоРазмещенияКоманд);
...
// Возвращает список команд ЭДО легкого интерфейса для указанной формы.
//
// Возвращаемое значение:
//  См. СоздатьКоллекциюКомандЭДО
//
Функция КомандыЭДОЛегкогоИнтерфейса() Экспорт
	
	КомандыЭДО = СоздатьКоллекциюКомандЭДО();
	
	СформироватьКомандыЭДОЛегкогоИнтерфейса(КомандыЭДО);
...
#КонецОбласти // КомандыЛегкогоИнтерфейса
...
//                             Пример "СчетЗаказ".
//                  - Массив Из Строка  - Список идентификаторов команд ЭДО.
//
...
//
//  * Отображение   - ОтображениеКнопки - (необязательный).
//  
//  * Подсказка     - Строка            - (необязательный) Подсказка, которая будет добавлена к элементу формы.
//                                        
...
//
//  * РежимИспользованияПараметра - РежимИспользованияПараметраКоманды - (необязательный).
//
//  * ДополнительныеПараметры - Структура - (необязательный).
//
//  * НеВыполнятьЗаписьВФорме - Булево
//
//  * Оповещение - Строка - (необязательный) Клиентский обработчик для передачи управления после выполнения команды.
//
...
//  * Отключена - Булево
//
//  * УправлениеВидимостью - Структура - (необязательный).
//
...
//  
//  * ПроверятьНаличиеУчетныхЗаписейЭДО - Булево
//
Функция СоздатьКоллекциюКомандЭДО() Экспорт
	...
	// описание
	Результат.Колонки.Добавить("Идентификатор", ОписаниеТипаСтрока);
	...
	Результат.Колонки.Добавить("Обработчик", ОписаниеТипаСтрока);
	
	// представление
	Результат.Колонки.Добавить("Порядок", ОписаниеТипаЧисло);
	...
	Результат.Колонки.Добавить("Подсказка", ОписаниеТипаСтрока);
	
	// Имена форм для размещения команд, разделитель - запятая.
	Результат.Колонки.Добавить("СписокФорм", ОписаниеТипаСтрока);
	Результат.Колонки.Добавить("МестоРазмещения", ОписаниеТипаСтрока);
	// Имена функциональных опций, влияющих на видимость команды, разделитель - запятая.
	Результат.Колонки.Добавить("ФункциональныеОпции", ОписаниеТипаСтрока);
	...
	Результат.Колонки.Добавить("НеВыполнятьЗаписьВФорме", ОписаниеТипаБулево);
	
	// для передачи управления в обработчик оповещения после выполнения команды
	Результат.Колонки.Добавить("Оповещение", ОписаниеТипаСтрока);
	
	// Для служебного использования.
	Результат.Колонки.Добавить("СкрытаФункциональнымиОпциями", ОписаниеТипаБулево);
	Результат.Колонки.Добавить("УникальныйИдентификатор", ОписаниеТипаСтрока);
	Результат.Колонки.Добавить("Отключена", ОписаниеТипаБулево);
	Результат.Колонки.Добавить("УправлениеВидимостью", Новый ОписаниеТипов("Структура"));
	
	Результат.Колонки.Добавить("ТолькоВоВсехДействиях", ОписаниеТипаБулево);
	Результат.Колонки.Добавить("Недоступна", ОписаниеТипаБулево);
	
	Результат.Колонки.Добавить("ИмяКомандыНаФорме", ОписаниеТипаСтрока);
	Результат.Колонки.Добавить("КнопкаПоУмолчанию", ОписаниеТипаБулево);
	
	Результат.Колонки.Добавить("ПроверятьНаличиеУчетныхЗаписейЭДО", ОписаниеТипаБулево);
...
//  ОписаниеКомандыЭДО - СтрокаТаблицыЗначений: См. СоздатьКоллекциюКомандЭДО
Процедура ДобавитьКомандуФормы(Форма, ОписаниеКомандыЭДО)
	...
	Если ЗначениеЗаполнено(ОписаниеКомандыЭДО.Подсказка) Тогда
		КомандаФормы.Подсказка = ОписаниеКомандыЭДО.Подсказка;
	КонецЕсли;
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения 
//  ОписаниеКомандыЭДО - См. СоздатьКоллекциюКомандЭДО 
//  МестоРазмещения - ГруппаФормы
//  Видимость - Булево
//  
Процедура ДобавитьКнопкуФормы(Форма, ОписаниеКомандыЭДО, МестоРазмещения, Видимость)
	...
	НовыйЭлемент.КнопкаПоУмолчанию = ОписаниеКомандыЭДО.КнопкаПоУмолчанию;
КонецПроцедуры
...
Функция УсловиеВидимости(ОписаниеКомандыЭДО)
	...
	УсловияВидимости.Вставить("ИмяАлгоритмаМассовойПроверкиУсловия", Неопределено);
...
// Формирование легких команд ЭДО.
//
// Параметры:
//  КомандыЭДО - См. СоздатьКоллекциюКомандЭДО
Процедура СформироватьКомандыЭДОЛегкогоИнтерфейса(КомандыЭДО)
	ПодключаемыеКомандыЭДОСобытия.ПриОпределенииСпискаКомандЭДОЛегкогоИнтерфейса(КомандыЭДО);
КонецПроцедуры
