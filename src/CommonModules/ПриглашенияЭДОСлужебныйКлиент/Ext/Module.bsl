...
Процедура ПослеВводаПричиныОтказаОтПриглашения(РезультатВвода, Контекст) Экспорт
	...
	ПриглашенияЧерезЦентрЭДО = Новый Массив;
	...
	Пока КоличествоПриглашений > 0 Цикл
		...
		Если Приглашение.ИспользоватьЦентрЭДО Тогда
			ПриглашенияЧерезЦентрЭДО.Добавить(Приглашение);
			Приглашения.Удалить(КоличествоПриглашений);
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ПриглашенияЧерезЦентрЭДО) Тогда
		ВыполнитьДействиеСПриглашениямиЧерезЦентрЭДО(ПриглашенияЧерезЦентрЭДО, Контекст);
	Иначе
		ВыполнитьДействиеСПриглашениямиЧерезЛокальныйЭДО(Контекст);
	КонецЕсли;
...
//  Приглашения - Массив из см. ПриглашенияЭДОКлиент.НовоеВходящееПриглашение
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ВыполнитьДействиеСПриглашениямиЧерезЦентрЭДО(Приглашения, Контекст)
	
	Оповещение = Новый ОписаниеОповещения("ОбработатьРезультатДействияСПриглашениямиЧерезЦентрЭДО",
		ЭтотОбъект, Контекст);
	
	Действие = Контекст.Действие;
	ДействияСПриглашениями = ДействияСПриглашениями();
	
	ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДОКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
		"ПриглашенияЭДОИнтеграцияЦентраЭДОКлиент");
	
	Если Действие = ДействияСПриглашениями.Отправить Тогда
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДОКлиент.ОтправитьПриглашения(Оповещение, Приглашения);
	ИначеЕсли Действие = ДействияСПриглашениями.Принять Тогда
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДОКлиент.ПринятьПриглашения(Оповещение, Приглашения);
	ИначеЕсли Действие = ДействияСПриглашениями.Отклонить
		ИЛИ Действие = ДействияСПриглашениями.Отозвать Тогда
		ОбщийМодульПриглашенияЭДОИнтеграцияЦентраЭДОКлиент.ОтклонитьПриглашения(Оповещение, Приглашения);
	Иначе
		ВыполнитьОбработкуОповещения(Оповещение);
	КонецЕсли;
...
// Параметры:
//  Результат - см. ИнтеграцияЦентраЭДОВызовСервера.РезультатДействияСПриглашениямиВФоне
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ОбработатьРезультатДействияСПриглашениямиЧерезЦентрЭДО(Результат, Контекст) Экспорт
...
// Параметры:
//  Контекст - см. НовыйКонтекстВыполненияДействияСПриглашениями
Процедура ВыполнитьДействиеСПриглашениямиЧерезЛокальныйЭДО(Контекст)
