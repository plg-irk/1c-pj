...
//
Функция НачатьПодключениеУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ДополнительныеПараметры) Экспорт
...
//
Функция НачатьОтключениеУстройства(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
...
//
Функция НачатьВыполнениеКоманды(Команда, ВходныеПараметры = Неопределено, ОбъектДрайвера, Параметры, ПараметрыПодключения) Экспорт
	...
	Если Команда = "ВыгрузитьТовары" Тогда
		...
		РезультатВыполнения = НачатьВыгрузкуТоваров(Параметры, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры, РасширеннаяВыгрузка);
...
//
Функция НачатьВыгрузкуТоваров(Параметры, Товары, ЧастичнаяВыгрузка, ВыходныеПараметры, РасширеннаяВыгрузка)
	...
	Если ЧастичнаяВыгрузка Тогда
		РезультатВыполнения = НачатьПроверкуОбработанностиФайла(ДополнительныеПараметры, Параметры.БазаТоваров); // Проверяем обработанность файла.
	Иначе
		РезультатВыполнения = ВыгрузкаТоваровЗавершение(Истина, ДополнительныеПараметры);
	КонецЕсли;
...
Функция НачатьПроверкуОбработанностиФайла(ДополнительныеПараметры, ИмяФайла) 
	...
	Если Файл.Существует() Тогда
		...
		МенеджерОфлайнОборудованияСервер.ПолучитьСодержаниеТекстовыхФайлов(ИмяФайла, РезультатЧтенияФайлов, КодировкаТекста.ANSI);
...
//
Функция НачатьОтчетЗагружен(Параметры, ВыходныеПараметры);
	...
	Если Файл.Существует() Тогда
		...
		РезультатВыполнения = ОтчетЗагруженЗавершение(РезультатЧтенияФайлов, ДополнительныеПараметры);
...
Функция ОтчетЗагруженЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
//
Функция НачатьОчисткуТоваровНаККМ(Параметры, ВыходныеПараметры, ОчисткаНастроек = Ложь)
...
//
Функция НачатьТестУстройства(Параметры, ВыходныеПараметры)
