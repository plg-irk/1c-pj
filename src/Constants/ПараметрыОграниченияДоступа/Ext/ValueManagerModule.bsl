...
Функция СвойстваВидовДоступа()
	...
	ЗаголовокОшибки =
		...
		+ Символы.ПС
		+ НСтр("ru = 'общего модуля УправлениеДоступомПереопределяемый.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Для каждого ВидДоступа Из ВидыДоступа Цикл
		...
		// +Рарус
		Если ВидДоступа.Имя = "РегламентированныйУчет" Тогда
			ПустаяСсылкаТипаЗначений = Истина;
		Иначе
...
Процедура ПроверитьТип(ВидДоступа, Тип, ВсеТипы, Параметры, ПроверкаТиповГрупп = Ложь, ПроверкаПересечения = Ложь)
	...
	// Проверка, что указан тип ссылки. // +Рарус
	Если НЕ ОбщегоНазначения.ЭтоСсылка(Тип) И НЕ ВидДоступа.Имя = "РегламентированныйУчет" Тогда  // -РАРУС
		Если ПроверкаТиповГрупп Тогда
			ОписаниеОшибки =
	...
	Если ВсеТипы.Получить(Тип) <> Неопределено И НЕ ВидДоступа.Имя = "ПодразделенияКомпании" Тогда // -РАРУС
