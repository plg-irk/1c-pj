...
&НаКлиенте
Функция ПроверитьДоступностьОбновленияКлиент()
...
Процедура ЗагрузитьКлассификаторССайтаАутентификация(Знач Аутентификация, Знач КодыРегионов)
	...
	Задание = ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, Аутентификация, УникальныйИдентификатор);
...
Процедура ПослеФоновойЗагрузкиССайта(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Ошибка" Тогда
		Если СтрНайти(Задание.КраткоеПредставлениеОшибки, "404 Not Found") > 0 ИЛИ СтрНайти(Задание.КраткоеПредставлениеОшибки, "401 Unauthorized") > 0 Тогда
			...
			ТекстОшибки = ТекстОшибки + СтрПолучитьСтроку(Задание.КраткоеПредставлениеОшибки, 1);
		Иначе
...
&НаСервереБезКонтекста
Функция ЗапуститьФоновуюЗагрузкуССайтаНаСервере(КодыРегионов, Аутентификация, УникальныйИдентификатор)
	
	ПараметрыВызоваСервера = Новый Массив;
	ПараметрыВызоваСервера.Добавить(КодыРегионов);
	ПараметрыВызоваСервера.Добавить(Аутентификация);
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если Позиция <= ПомещенныеФайлы.ВГраница() Тогда
		...
		ДанныеФайла.Вставить("Имя",      Описание.ПолноеИмя);
		ДанныеФайла.Вставить("Хранение", Описание.Хранение);
		
		ДополнительныеПараметры.Вставить("ДанныеФайла", ДанныеФайла);
		ДополнительныеПараметры.Вставить("ПомещенныеФайлы", ПомещенныеФайлы);
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПослеПолученияВремениИзменения", ЭтотОбъект, ДополнительныеПараметры);
	...
	Иначе // выход из цикла
		...
		Задание = ЗапуститьФоновуюЗагрузкуНаСервере(ДополнительныеПараметры.ПараметрыЗагрузки.КодыРегионов, ПомещенныеФайлы, Режим, УникальныйИдентификатор);
...
&НаСервереБезКонтекста
Функция ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим, Знач УникальныйИдентификатор)
...
Функция СохраненныеДанныеАутентификацииСайта()
	...
	Результат = АдресныйКлассификаторСлужебный.ПараметрыАутентификацииНаСайте();
...
Функция ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления()
	...
	Если Метаданные.РегистрыСведений.Найти("УдалитьАдресныйКлассификатор") <> Неопределено Тогда
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Очистка адресного классификатора в формате КЛАДР'");
	ФоновоеЗадание = ДлительныеОперации.ВыполнитьВФоне("АдресныйКлассификаторСлужебный.ОчиститьУстаревшиеАдресныеСведенияКЛАДР",
		Новый Массив, ПараметрыВыполнения);
		
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ТекстОшибки = НСтр("ru = 'Не удается загрузить адресные сведения.'");
		ТекстОшибки = ТекстОшибки + НСтр("ru = 'Техническая информация:'") + " " + Символы.ПС;
		ТекстОшибки = ТекстОшибки + ФоновоеЗадание.КраткоеПредставлениеОшибки;
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Очистка адресного классификатора в формате КЛАДР'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.РегистрыСведений.УдалитьАдресныйКлассификатор,, ТекстОшибки + Символы.ПС + ФоновоеЗадание.ПодробноеПредставлениеОшибки);
	КонецЕсли;
