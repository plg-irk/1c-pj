...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// +РАРУС
	Объект.Пользователь = ?(Объект.Пользователь.Пустая(), Пользователи.ТекущийПользователь(), Объект.Пользователь);
	Объект.Автор        = ?(Объект.Автор.Пустая(),        Пользователи.ТекущийПользователь(), Объект.Автор);
	// -РАРУС
	...
	Элементы.ПредметНапоминания.Видимость = Ложь;//ЗначениеЗаполнено(Объект.Источник);
	Элементы.НадписьПредмет.Видимость = Ложь;
	...
	// +РАРУС
	ЭтотОбъект.Пользователь = Объект.Пользователь;
	Элементы.ПодобратьПользователей.Видимость = (НЕ ЗначениеЗаполнено(Объект.ИсходныйКлючЗаписи));
	// -РАРУС
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТипЗнч(ЭтотОбъект.Пользователь)=Тип("СписокЗначений") И ЭтотОбъект.Пользователь.Количество() > 1 Тогда
		
		ЭтоПервыйЭлемент = Истина;
		
		Для Каждого ЭлементСписка Из ЭтотОбъект.Пользователь Цикл
			...
			Иначе
				...
				МенеджерЗаписиНапоминания.Пользователь                      = ЭлементСписка.Значение;
				МенеджерЗаписиНапоминания.Автор                             = ТекущийОбъект.Автор;
				МенеджерЗаписиНапоминания.ВремяСобытия                      = ТекущийОбъект.ВремяСобытия;
				МенеджерЗаписиНапоминания.Источник                          = ТекущийОбъект.Источник;
				
				МенеджерЗаписиНапоминания.СрокНапоминания                   = ТекущийОбъект.СрокНапоминания;
				
				МенеджерЗаписиНапоминания.Описание                          = ТекущийОбъект.Описание;
				МенеджерЗаписиНапоминания.СпособУстановкиВремениНапоминания = ТекущийОбъект.СпособУстановкиВремениНапоминания;
				МенеджерЗаписиНапоминания.ИнтервалВремениНапоминания        = ТекущийОбъект.ИнтервалВремениНапоминания;
				МенеджерЗаписиНапоминания.ИмяРеквизитаИсточника             = ТекущийОбъект.ИмяРеквизитаИсточника;
				МенеджерЗаписиНапоминания.Расписание                        = ТекущийОбъект.Расписание;
				МенеджерЗаписиНапоминания.ПредставлениеИсточника            = ТекущийОбъект.ПредставлениеИсточника;
...
КонецПроцедуры // ПослеЗаписиНаСервере() // -РАРУС
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение.Количество() > 1 Тогда
		...
		// Подобрали список пользователей, меняем тип реквизита формы на список и передаем туда пользователей.
		ЭтотОбъект.Пользователь = ВыбранноеЗначение;
		Объект.Пользователь = ЭтотОбъект.Пользователь[0].Значение;
		
	ИначеЕсли ВыбранноеЗначение.Количество() = 1 Тогда
		
		// Выбрали одного пользователя, сбрасываем тип реквизита формы на эл-т справочника и передаем туда пользователя.
		ЭтотОбъект.Пользователь = ВыбранноеЗначение[0].Значение;
		Объект.Пользователь = ЭтотОбъект.Пользователь;
		
	КонецЕсли;
	
КонецПроцедуры // ПользовательОбработкаВыбора() // -РАРУС
...
Процедура ПользовательПриИзменении(Элемент)
	...
	Если ТипЗнч(ЭтотОбъект.Пользователь)=Тип("СправочникСсылка.Пользователи") Тогда
		
		Объект.Пользователь = ЭтотОбъект.Пользователь;
		
	КонецЕсли;
	
КонецПроцедуры // ПользовательПриИзменении() // -РАРУС

// +РАРУС
&НаКлиенте
Процедура ПользовательНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Если ТипЗнч(ЭтотОбъект.Пользователь)=Тип("СписокЗначений") Тогда
		
		СтандартнаяОбработка = ЛОЖЬ;
		ПодобратьПользователей();
		
	КонецЕсли;
	
КонецПроцедуры // ПользовательНачалоВыбора() // -РАРУС
...
Процедура ПользовательОчистка(Элемент, СтандартнаяОбработка)
	...
	ЭтотОбъект.Пользователь = ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
	
КонецПроцедуры // ПользовательОчистка() // -РАРУС

// +РАРУС
&НаКлиенте
Процедура ПодобратьПользователей(Команда=Неопределено, ПараметрыФормы=Неопределено)
	
	Если ТипЗнч(ЭтотОбъект.Пользователь)=Тип("СписокЗначений") Тогда
		
		ПараметрыФормы = Новый Структура("ПользователиВыбранные", ЭтотОбъект.Пользователь);
		
	ИначеЕсли ЗначениеЗаполнено(ЭтотОбъект.Пользователь) Тогда
		
		ПользователиВыбранные = Новый СписокЗначений;
		ПользователиВыбранные.Добавить(ЭтотОбъект.Пользователь);
		
		ПараметрыФормы = Новый Структура("ПользователиВыбранные", ПользователиВыбранные);
		
	КонецЕсли;
	
	ОткрытьФорму("РегистрСведений.НапоминанияПользователя.Форма.ПодборПользователей", ПараметрыФормы, ЭтотОбъект, ЭтотОбъект,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры // ПодобратьПользователей() // +РАРУС
#КонецОбласти
...
Процедура ЗаполнитьСписокРеквизитовИсточника()
	...
	КонецЦикла;
	НапоминанияПользователяПереопределяемый.ПриЗаполненииСпискаРеквизитовИсточникаСДатамиДляНапоминания(Объект.Источник, МассивРеквизитов);
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Источник, СтрСоединить(МассивРеквизитов, ","));
