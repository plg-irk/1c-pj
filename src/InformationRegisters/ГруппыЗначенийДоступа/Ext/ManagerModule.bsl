...
Процедура ОбновитьДанныеРегистра(ЕстьИзменения = Неопределено) Экспорт
	...
	// +Рарус Необходимо исключение для разработанного ограничения (В иерархии подразделений).
	ОбновитьСтруктуруИерархииДоступаПодразделений(); // -РАРУС
...
Процедура ОбновитьГруппыИсполнителей(ГруппыИсполнителей = Неопределено,
	...
	|
	// +РАРУС Индексация по составному типу запрещена
	//|ИНДЕКСИРОВАТЬ ПО
	//|	ПустыеСсылкиГруппЗначений.ПустаяСсылка
	// -РАРУС
	|;
...
// +Рарус
// Обновляет группировки подразделений для проверки разрешенных значений
// по видам доступа подразделения компании
//
Процедура ОбновитьСтруктуруИерархииДоступаПодразделений() Экспорт

	УстановитьПривилегированныйРежим(Истина);
	
	МаксимальнаяДлинаПути = 1000;
	...
	Эпилог = "ВЫБРАТЬ НачалоДуги Предок, КонецДуги Потомок ИЗ ЗамыканияДлины#2 ГДЕ НачалоДуги <> КонецДуги ИТОГИ ПО Потомок";
	...
	НаборЗаписей = РегистрыСведений.ГруппыЗначенийДоступа.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ГруппаДанных.Установить(5);
	НаборЗаписей.Прочитать();
	НаборЗаписей.Очистить();
	Пока Выборка.Следующий() Цикл
		...
		Пока ДетальнаяВыборка.Следующий() Цикл
			...
			НоваяЗапись.ЗначениеДоступа = ДетальнаяВыборка.Потомок;
			НоваяЗапись.ГруппаДанных = 5;
			НоваяЗапись.ГруппаЗначенийДоступа = ДетальнаяВыборка.Предок;
		КонецЦикла;
		
		// Добавим связь предок - предок
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.ЗначениеДоступа = Выборка.Потомок;
		НоваяЗапись.ГруппаДанных = 5;
		НоваяЗапись.ГруппаЗначенийДоступа = Выборка.Потомок;
	КонецЦикла;
	НаборЗаписей.Записать(Истина);
	УстановитьПривилегированныйРежим(Ложь);
	
КонецПроцедуры // ОбновитьСтруктуруИерархииДоступаПодразделений() // -РАРУС
