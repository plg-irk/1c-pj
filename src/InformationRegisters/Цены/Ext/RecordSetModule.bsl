...
Перем ВыгружатьВОборудованиеПриИзменении Экспорт; // Булево. Истина-инвалидировать товары в оборудовании, Ложь-нет
Перем ПересчитатьСУчетомНДС Экспорт; // Булево. Истина-инвалидировать товары в оборудовании, Ложь-нет
...
Функция ПолучитьТаблицуТоваров()
	...
	|	ДокументТовары." + ИмяРеквизитаНоменклатура + ".Наименование КАК НоменклатураНаименование," + ?(НЕ ПересчитатьСУчетомНДС = Неопределено, "
	|	ДокументТовары." + ИмяРеквизитаНоменклатура + ".СтавкаНДС.Ставка КАК Ставка,", "") + "
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА ДокументТовары.Коэффициент = 0 ТОГДА
	|			ДокументТовары." + ИмяРеквизитаЦена + "
	|		ИНАЧЕ
	|			ДокументТовары." + ИмяРеквизитаЦена + " / ДокументТовары.Коэффициент
	|	КОНЕЦ) КАК НоваяЦена,";
...
Функция ПроверитьНаОдинаковыеЗаписи(ТаблицаТоваров)
	...
	Ошибки = НСтр("ru = 'Ошибка. Тип цен'") + " <" + ТипЦен + ">," + НСтр("ru = 'аналитика учета цен'") + " <" 
		+ АлгоритмПолученияЦены + "> - " + НСтр("ru = 'попытка назначения одинаковых цен для следующих позиций:'");
...
Функция УстановитьЦены() Экспорт
	...
	Если ТипЦен.Рассчитывается Тогда
		ВывестиСообщение(НСтр("ru = 'Невозможно установить расчетную цену'") + " 	" + ТипЦен,ДокументОбъект,,,Ложь);
		Возврат Истина;
	КонецЕсли;
	...
	СообщатьОбИзменении = ПравоПользователя("СообщатьОбИзмененииЦен", ДокументОбъект);
	...
	ЕстьВалюта = ЕстьРеквизит(ДокументОбъект, "ВалютаДокумента");
	Для Каждого СтрокаТовар Из РезультатЗапросаПоТоварам Цикл
		
		//Если СтрокаТовар.НоваяЦена=0 Тогда Продолжить; КонецЕсли; //TruA
		...
		Если ТипЦен.ВВалютеУчета Тогда
			...
			КурсВалютыТипаЦен = СтруктураКурса.Курс / ?(СтруктураКурса.Кратность = 0, 1, СтруктураКурса.Кратность);
		КонецЕсли;
		Если НЕ ПересчитатьСУчетомНДС = Неопределено Тогда
			Если ПересчитатьСУчетомНДС Тогда
				СтрокаТовар.НоваяЦена = СтрокаТовар.НоваяЦена*100/(100 + СтрокаТовар.Ставка);
			Иначе 
				СтрокаТовар.НоваяЦена = СтрокаТовар.НоваяЦена+((СтрокаТовар.Ставка/100)*СтрокаТовар.НоваяЦена);
			КонецЕсли;
		КонецЕсли;
		Если ЗначениеЗаполнено(ВалютаТипаЦен) Тогда
			...
			КонецЕсли;
				...
				ОбработаннаяНоменклатура.Добавить(НоваяЗапись.Номенклатура);
				...
				Если УчетЦенПоХарактеристикам Тогда
					...
					Если СтрокаТовар.УчетЦенТолькоВРазрезеДопПараметров И СтрокаТовар[ИмяРеквизитаХарактеристикаНоменклатуры].Пустая() Тогда
						...
						Исключение 
							СтрокаНомер = "";
						КонецПопытки;
						ВывестиСообщение(НСтр("ru = 'Невозможно установить цену для номенклатуры ""%1"", так как тип номенклатуры %2 предусматривает обязательный учет цен только в разрезе единиц измерения, при этом тип цен ""%3"" использует ведение аналитики учета цен ""По характеристике"".%4'"), ДокументОбъект, "Товары",,, СтрокаТовар.НоменклатураНаименование, СтрокаТовар[ИмяРеквизитаНоменклатура].ТипНоменклатуры, ТипЦен, СтрокаНомер);
						Продолжить;
				...
				ИначеЕсли УчетЦенПоЕдиницамИзмерения Тогда
					...
					Если СтрокаТовар.УчетЦенТолькоВРазрезеДопПараметров И СтрокаТовар[ИмяРеквизитаЕдиницаИзмерения].Пустая() Тогда
						...
						Исключение
							СтрокаНомер = "";
						КонецПопытки;
						ВывестиСообщение(НСтр("ru = 'Невозможно установить цену для номенклатуры ""%1"", так как не задана единица измерения номенклатуры.%2'"), ДокументОбъект, "Товары",,, СтрокаТовар.НоменклатураНаименование, СтрокаНомер);
						Продолжить;
					Иначе
				...
				Если СообщатьОбИзменении И НоваяЦена <> СтрокаТовар.ЦенаВБазе Тогда
					ВывестиСообщение(НСтр("ru = '[%1] Товар ""%2""; Старая цена: %3; Новая цена: %4; Тип цен: %5%6'"),
						,
						"Товары",
						,
						,
	...
	Если (НЕ ЗначениеЗаполнено(Контрагент)) И ВыгружатьВОборудованиеПриИзменении<>Ложь Тогда
		...
		Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
		                      ...
		                      |ГДЕ
		                      |	Оборудование.ОсновнойОтдел.Розничный
		                      |	И (Оборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли = &ТипЦен
		                      |			ИЛИ (Оборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.БазовыйТипЦен = &ТипЦен
		                      |				И Оборудование.ОсновнойОтдел.ТипЦенРозничнойТорговли.Рассчитывается))
		                      |	И Оборудование.ОсновнойОтдел.ПодразделениеКомпании В ИЕРАРХИИ(&ПодразделениеКомпании)");
		Запрос.УстановитьПараметр("ТипЦен", ТипЦен);
		Запрос.УстановитьПараметр("ПодразделениеКомпании", ПодразделениеКомпании);
		
		ВыгружатьВОборудованиеПриИзменении = (НЕ Запрос.Выполнить().Пустой());
	Конецесли;
	
	Если (НЕ ЗначениеЗаполнено(Контрагент)) И ВыгружатьВОборудованиеПриИзменении = Истина Тогда
		ОбменДаннымиСобытия.ЗарегистрироватьИзмененияРегистраТоварыВОборудовании(ОбработаннаяНоменклатура, , ТипЦен, ПодразделениеКомпании);
	КонецЕсли;
	...
	ВыгружатьВОборудованиеПриИзменении = Истина;
...
ВыгружатьВОборудованиеПриИзменении     = Истина;
ПересчитатьСУчетомНДС                    = Неопределено;
