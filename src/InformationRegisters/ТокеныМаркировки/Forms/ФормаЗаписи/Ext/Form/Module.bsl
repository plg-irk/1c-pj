///////////////////////////////////////////////////////////////////////////////
// Модуль формы записи регистра "Клиентские токены СУЗ"
//
///////////////////////////////////////////////////////////////////////////////
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	// Вызываем общий обработчик события
	Если НЕ УправлениеДиалогомРегистраКлиент.ПередЗакрытием(
			ЭтотОбъект,
			Отказ,
			ЗавершениеРаботы,
			ТекстПредупреждения,
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Вызываем общий обработчик события
	Если НЕ УправлениеДиалогомРегистраСервер.ПередЗаписьюНаСервере(
			ЭтотОбъект,
			Отказ,
			ТекущийОбъект,
			ПараметрыЗаписи) Тогда
		Возврат;
	КонецЕсли;
	
	//ВывестиСообщение(НСтр("ru = 'Запрещено изменять настройку токена'"),,,, Отказ);
...
&НаСервере
Процедура ТипТокенаПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ТипТокенаПриИзменении(Элемент)
	
	ТипТокенаПриИзмененииНаСервере();
...
Процедура УправлениеДиалогомНаСервере()
	...
	Элементы.ИдентификаторСоединения.Видимость = (Запись.ТипТокена = Перечисления.ТипыТокеновАвторизации.СУЗ);
	Элементы.СрокДействияВСеансе.Заголовок = СтрШаблон(НСтр("ru = 'Текущее время: %1'"), МестноеВремя(Запись.СрокДействия, ЧасовойПоясСеанса()));
...
//
// Параметры:
//  ИмяКоманды - Строка - Имя вызванной команды.
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
