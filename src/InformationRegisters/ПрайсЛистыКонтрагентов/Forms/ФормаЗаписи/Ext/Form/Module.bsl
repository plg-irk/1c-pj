// Модуль формы записи регистра "Прайс листы контрагентов"
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "ПрайсЛист" в контексте сервера.
//
// Параметры:
//  ПараметрыДействия - Структура - Набор параметров, использующихся при выполнения операции.
//
&НаСервере
Процедура ПрайсЛистПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	// Измени контрагента, если поменялся прайс лист
	Если ЗначениеЗаполнено(Запись.ПрайсЛист) Тогда
		Запись.Контрагент = Запись.ПрайсЛист.Владелец;
	КонецЕсли;
	
КонецПроцедуры // ПрайсЛистПриИзмененииНаСервере()

// Обработчик события возникающего на клиенте при изменении данных реквизита "ПрайсЛист".
//
// Параметры:
...
Процедура ПрайсЛистПриИзменении(Элемент)
	...
	// Обработаем событие в контексте сервера
	ПрайсЛистПриИзмененииНаСервере(ПараметрыДействия);
	
КонецПроцедуры // ПрайсЛистПриИзменении()
...
Процедура НоменклатураПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(Запись.Артикул) Тогда
	...
	Если НЕ ЗначениеЗаполнено(Запись.Производитель) Тогда
		Запись.Производитель = Номенклатура.Производитель;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Запись.НаименованиеИностранное) Тогда
		Запись.НаименованиеИностранное = Номенклатура.НаименованиеИностранное;
	КонецЕсли;
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Запись.Номенклатура) И (НЕ ЗначениеЗаполнено(Запись.Артикул) ИЛИ НЕ ЗначениеЗаполнено(Запись.НаименованиеИностранное) ИЛИ НЕ ЗначениеЗаполнено(Запись.Производитель) ИЛИ НЕ ЗначениеЗаполнено(Запись.Наименование))Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Запись.ПрайсЛист) И ЗначениеЗаполнено(Запись.ПрайсЛист.Владелец) Тогда
