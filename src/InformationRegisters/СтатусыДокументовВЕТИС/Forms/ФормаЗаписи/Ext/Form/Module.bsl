// Модуль формы записи регистра "Статусы документов ЕГАИС"
...
Процедура НастроитьПараметрыВыбораЭлементовФормы()
	...
	ЗаполнитьСтатусыОбработкиПоДокументу();
...
&НаСервере
Процедура СтатусОбработкиПриИзмененииНаСервере()
	
	Запись.ВидСтатуса = Запись.СтатусОбработки.ВидСтатуса;
	
КонецПроцедуры

&НаКлиенте
Процедура СтатусОбработкиПриИзменении(Элемент)
	СтатусОбработкиПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Функция ЗаполнитьСтатусыОбработкиПоДокументу()
	
	Результат = Ложь;
	
	УправлениеДиалогомСервер.УдалитьПараметрВыбора(Элементы.СтатусОбработки.ПараметрыВыбора, "Отбор.ВидДокумента");
	
	Если Запись.Документ = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ИмяДокумента = Запись.Документ.Метаданные().Имя;
	ВидДокумента = ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ."+ИмяДокумента);
	
	УправлениеДиалогомСервер.ОбновитьПараметрВыбора(Элементы.СтатусОбработки.ПараметрыВыбора, "Отбор.ВидДокумента", ВидДокумента);
