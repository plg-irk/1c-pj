...
&НаКлиенте
Перем ПользователиПродолжитьДобавление, ВыбранныйСпособУказанияДатыЗапрета;
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	// +рарус  перетянуто из бсп 2.4. Убран серверный вызов.
	//СтандартнаяОбработка = Ложь;
	//
...
Процедура ОписаниеДатыЗапретаПриИзменении(Элемент)
	...
	Если ОписаниеДатыЗапрета = "ТекущаяДата" Тогда
		РазрешитьИзменениеДанныхДоДатыЗапрета = Истина;
		КоличествоДнейРазрешения = ?(КоличествоДнейРазрешения = 0, 1, КоличествоДнейРазрешения);
	КонецЕсли;
...
Процедура РазрешитьИзменениеДанныхДоДатыЗапретаПриИзменении(Элемент)
	...
	Если РазрешитьИзменениеДанныхДоДатыЗапрета = Ложь и ОписаниеДатыЗапрета = "ТекущаяДата" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Режим запрета для ""Текущего дня"" требует включенной отсрочки",,РазрешитьИзменениеДанныхДоДатыЗапрета, РазрешитьИзменениеДанныхДоДатыЗапрета);
		КоличествоДнейРазрешения = ?(КоличествоДнейРазрешения = 0, 1, КоличествоДнейРазрешения);
		РазрешитьИзменениеДанныхДоДатыЗапрета = Истина;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура КоличествоДнейРазрешенияАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ПодключитьОбработчикОжидания("ОбработчикОжиданияКоличествоДнейРазрешенияАвтоПодбор", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикОжиданияКоличествоДнейРазрешенияАвтоПодбор()
...
Процедура ПользователиПередНачаломИзменения(Элемент, Отказ)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Представление) Тогда
			// +Рарус
			ПодключитьОбработчикОжидания("ПользователиПередНачаломИзмененияОбработчикОжидания", 0.1, Истина);  // -РАРУС
		КонецЕсли;
	КонецЕсли;
...
// +Рарус
&НаКлиенте
Процедура ПользователиПередНачаломИзмененияОбработчикОжидания()
	
	ТекущиеДанные = Элементы.Пользователи.ТекущиеДанные;
	ЗаблокироватьНаборЗаписейПользователя(ТекущиеДанные.Пользователь, Элементы.Пользователи.ТекущаяСтрока);
...
&НаКлиенте
Процедура ПользователиПолноеПредставлениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
Процедура ДатыЗапретаПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ЗафиксироватьЗаписьВЖурнале(ТекущиеДанные.Представление);
	
КонецПроцедуры

&НаСервере
Процедура ЗафиксироватьЗаписьВЖурнале(Раздел)
		
	ЗаписьЖурналаРегистрации("Изменение даты запрета", УровеньЖурналаРегистрации.Информация,Метаданные.РегистрыСведений.ДатыЗапретаИзменения,, "Изменена дата запрета для: " + Раздел);
...
Процедура ПрочитатьДанныеПользователя(Контекст, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	Если Контекст.СпособУказанияДатыЗапрета = "ОбщаяДата" Тогда
		...
		ДатыЗапретаИзмененияСлужебныйКлиентСервер.ОбщаяДатаЗапретаСОписаниемПриИзменении(Контекст, Истина);
