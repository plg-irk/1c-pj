...
Функция ГруппаАналоговНоменклатуры(Номенклатура)
...
Процедура УстановитьОтборСписка()
	...
	ЗначениеОтбора = ГруппаАналоговНоменклатуры(Номенклатура);
...
Процедура ПоместитьТекущуюНоменклатуруВГруппу(НовыйОбъект)
	...
	Если НовыйОбъект.ИдентификаторГруппы <> ГруппаАналоговНоменклатуры(Номенклатура) Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение =  Новый ПолеКомпоновкиДанных("Номенклатура");
...
&НаСервере
Процедура ПриоритетВверхНаСервере(КлючЗаписи, Приоритет)
	...
	|	И ГруппыАналогов.Приоритет < &Приоритет
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет УБЫВ";
	
	Запрос = Новый Запрос(Текст);
	Запрос.УстановитьПараметр("ИдентификаторГруппы", КлючЗаписи.ИдентификаторГруппы);
	...
	Иначе
		...
		ПервыйПриоритет = Макс(0, Приоритет-1);
		// повышаем приоритет текущего
		НоваяЗапись = РегистрыСведений.ГруппыАналогов.СоздатьМенеджерЗаписи(); 
		НоваяЗапись.Номенклатура        = КлючЗаписи.Номенклатура;
		НоваяЗапись.ИдентификаторГруппы = КлючЗаписи.ИдентификаторГруппы;
...
Процедура ПриоритетВверх(Команда)
	...
	Если ТекущиеДанные.Приоритет = 0 Тогда
		ВывестиСообщение(НСтр("ru = 'Замена имеет наивысшее значение приоритета.'"), Элементы.Список.ТекущаяСтрока);
		Возврат;
	КонецЕсли;
	
	ПриоритетВверхНаСервере(Элементы.Список.ТекущаяСтрока, Элементы.Список.ТекущиеДанные.Приоритет);
	
КонецПроцедуры

&НаСервере
Процедура ПриоритетВнизНаСервере(КлючЗаписи, Приоритет)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяЗапись.Номенклатура        = Выборка.Номенклатура;
		НоваяЗапись.Коэффициент         = Выборка.Коэффициент;
		НоваяЗапись.Приоритет           = Макс(0, ПервыйПриоритет-1);
		НоваяЗапись.Записать(Истина);
	КонецЦикла;
...
Процедура ПриоритетВниз(Команда)
	...
	ПриоритетВнизНаСервере(Элементы.Список.ТекущаяСтрока, Элементы.Список.ТекущиеДанные.Приоритет);
