...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2024Кв2")
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2024Кв2") Тогда
			...
			ВозможныеСтатусыНалогоплательщика = СтрРазделить("1,2,3,4,5,6,7,8,9,0", ",", Ложь);
	...
	Если НЕ (ОКТМОДлина = 8 ИЛИ ОКТМОДлина = 11) Тогда
		...
		СтатусПроверки = Ложь;
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы.ПроверкаКодовПоКлассификаторам") Тогда
			МодульПроверкаКодовПоКлассификаторамКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ПроверкаКодовПоКлассификаторамКлиентСервер");
			ПараметрыПроверки = МодульПроверкаКодовПоКлассификаторамКлиентСервер.НовыйПараметрыПроверки();
			ПараметрыПроверки.КодПоОКТМО.Объект = СтруктураРеквизитовФормы.Организация;
			ПараметрыПроверки.КодПоОКТМО.ОКТМО = ПараметрыВыгрузки.ОКТМО;
			МодульПроверкаКодовПоКлассификаторам = ОбщегоНазначения.ОбщийМодуль("ПроверкаКодовПоКлассификаторам");
			РезультатПроверки = МодульПроверкаКодовПоКлассификаторам.Проверить(ПараметрыПроверки);
			Если РезультатПроверки.КодПоОКТМО.Успешно = Ложь Тогда
				СообщитьВТС(ИмяФормы, РезультатВыгрузки, РезультатПроверки.КодПоОКТМО.ТекстОшибки, "Титульный", "ОКТМО");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношения_2024Кв2(СтруктураПараметров)
	...
	Если ДанныеКС.ГодовыеПроверки Тогда
...
#Область Проверки
	...
	Если Документирование ИЛИ ДанныеКС.ГодовыеПроверки Тогда
...
Функция ВыполнитьДокументированиеОбъект_2024Кв2() Экспорт
	...
	ДанныеКС.Вставить("Счетчик"			, 0);
	ДанныеКС.Вставить("МесОтчета"		, 12); 
	ДанныеКС.Вставить("ГодовыеПроверки"	, Истина); 
...
Процедура СформироватьДанныеОтчета_2024Кв2()
	...
	СтрокаЗначенийПериодов = ("51,52,53,83,84,86");
	ЭтоЛиквидацияВнутриГода = ЗначениеЗаполнено(ДанныеКС.ПериодОтчета) 
		И НЕ СтрНайти(СтрокаЗначенийПериодов, ДанныеКС.ПериодОтчета) = 0;
		
	ДанныеКС.Вставить("ГодовыеПроверки", ДанныеКС.МесОтчета = 12 ИЛИ ЭтоЛиквидацияВнутриГода);
...
Функция ПолучитьСтруктуруОтчета_2024Кв2(СтруктураДанных)
	...
	СтруктураОтчета.Вставить("ПериодОтчета"	, СтруктураОтчета.Раздел0.Период);
