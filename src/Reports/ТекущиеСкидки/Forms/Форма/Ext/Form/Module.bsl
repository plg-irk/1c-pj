...
// Обработчик события возникающего на клиенте при начале удаления
//
// Параметры:
//  Элемент	 - 	 - 
//  Отказ	 - 	 - 
...
//
&НаКлиенте
Процедура КолонкиПоляВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Поле = Элементы.КолонкиПоля.ТекущийЭлемент;
	
	Если Поле.Имя = "КолонкиПоляЗаголовок" Тогда
		
		СтандартнаяОбработка = Ложь;
		
	КонецЕсли;
	
КонецПроцедуры //КолонкиПоляВыбор()
...
// Обработчик события возникающего на клиенте при изменении поля использования.
//
&НаКлиенте
Процедура КолонкиПоляИспользованиеПриИзменении()
	...
	Для Каждого Строка Из ГруппировкиСтрок Цикл
		Если Строка.Заголовок = НСтр("ru = 'Период действия'") Тогда
			...
			Для Каждого Поле Из Поля Цикл
				Если Поле.Заголовок = Элементы.КолонкиПоля.ТекущиеДанные.Заголовок Тогда
					Поле.Использование = Элементы.КолонкиПоля.ТекущиеДанные.Использование;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			Прервать;
...
//
&НаКлиенте
Процедура КолонкиПоляПередУдалением(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ПроверитьЗначениеСсылочногоТипа(Ссылка) Тогда
		ОткрытьФорму(ПолучитьИмяСправочника(Ссылка)+".ФормаОбъекта", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
