...
//
&НаКлиенте
Процедура Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтервалов(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	
	Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтерваловНаСервере(РезультатЗакрытия, ДополнительныеПараметры);
	
КонецПроцедуры //Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтервалов()

// Обработчик события возникающего на сервере при выполнении оповещения данной формы о прекращении работы подчиненной.
//
// Параметры:
//  УчетнаяЗаписьНастроена       - Произвольный - Результат выполнения операции в подчиненной форме.
//  ДополнительныеПараметры - Произвольный - Значение, которое было указано при создании объекта описания оповещения.
//
&НаСервере
Процедура Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтерваловНаСервере(РезультатЗакрытия, ДополнительныеПараметры)
	
	Если НЕ РезультатЗакрытия = Неопределено Тогда
		Таблицы = ПолучитьИзВременногоХранилища(РезультатЗакрытия);
		ЗначениеИнтервалыДней = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИнтервалыДней");
		ЗначениеИнтервалыСумм = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИнтервалыСумм");
		ЗначениеИнтервалыДней.Значение = Новый ХранилищеЗначения(Таблицы.ИнтервалыДнейИнтервалДней);
		ЗначениеИнтервалыСумм.Значение = Новый ХранилищеЗначения(Таблицы.ИнтервалыСуммИнтервалСумм);
	КонецЕсли;
	
КонецПроцедуры //Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтерваловНаСервере()
...
// Обработчик события возникающего на клиенте при выполнении команды "Настройка интервалов".
//
// Параметры:
//   Команда   (КомандаФормы)     Команда, которая была вызвана.
//
&НаКлиенте
Процедура НастройкаИнтервалов(Команда)
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Таблицы", НастройкаИнтерваловНаСервере());
	ОписаниеОповещенияЗакрытияОкнаНастройкиИнтервалов = Новый ОписаниеОповещения("Подключаемый_ОповещениеЗакрытияОкнаНастройкиИнтервалов", ЭтотОбъект, ДополнительныеПараметры);
	ОткрытьФорму("Отчет.ВозрастЗадолженности.Форма.ФормаНастройкиИнтервалов", ДополнительныеПараметры, ЭтотОбъект,,,,ОписаниеОповещенияЗакрытияОкнаНастройкиИнтервалов, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

// Функция возвращает адрес временного хранилища, куда помещены таблицы отчета
// 
&НаСервере
Функция НастройкаИнтерваловНаСервере()
	
	ЗначениеПараметраИнтервалыДней = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИнтервалыДней").Значение.Получить();
	ЗначениеПараметраИнтервалыСумм = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИнтервалыСумм").Значение.Получить();
	
	Таблицы = Новый Структура;
	Таблицы.Вставить("ИнтервалыДнейИнтервалДней", ЗначениеПараметраИнтервалыДней);
	Таблицы.Вставить("ИнтервалыСуммИнтервалСумм", ЗначениеПараметраИнтервалыСумм);
