...
Процедура ВывестиОтчет(Ссылка)
	...
	|ПОМЕСТИТЬ ТаблицаПрофилей
	|ИЗ
	|	Справочник.ГруппыДоступа.ПрофилиДоступа КАК ГруппыДоступаПрофилиДоступа
	|ГДЕ
	|	НЕ ГруппыДоступаПрофилиДоступа.Ссылка.ПометкаУдаления
	|	И НЕ ГруппыДоступаПрофилиДоступа.Профиль.ПометкаУдаления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	ТаблицаПрофилей КАК ГруппыДоступа
	...
	|	ПРЕДСТАВЛЕНИЕ(ГруппыДоступаПользователя.Профиль) КАК ПредставлениеПрофиля
	|ИЗ
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		// Вывод групп доступа.
		ОписанияГруппДоступа = РезультатыЗапроса[2].Выгрузить(
		...
		РолиПоПрофилям = РезультатыЗапроса[4].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	НастройкиПрав = РезультатыЗапроса[5].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
