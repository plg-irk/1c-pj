...
//       Может использоваться для получения настроек варианта этого отчета
//       при помощи функции ВариантыОтчетов.ОписаниеВарианта().
...
// Функция добавляет поле итога в схему компоновки данных.
// Если параметр Выражение не указан, используется Сумма(ПутьКДанным).
...
Процедура НастройкиКДПоУмолчаниюТиповойСхемы(ОтчетОбъект, ПараметрыОтчета, НастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если НастройкиКД = Неопределено Тогда
		ДопСвойстваНастроек = ОтчетОбъект.КомпоновщикНастроек.Настройки.ДополнительныеСвойства;
	Иначе
		ДопСвойстваНастроек = НастройкиКД.ДополнительныеСвойства;
	КонецЕсли;
	
	Если Не ДопСвойстваНастроек.Свойство("ОтчетИнициализирован") Или (ДопСвойстваНастроек.Свойство("ОтчетИнициализирован")
		И Не ДопСвойстваНастроек.ОтчетИнициализирован) Тогда
