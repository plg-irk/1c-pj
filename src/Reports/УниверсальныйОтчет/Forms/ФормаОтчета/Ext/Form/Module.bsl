...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "ВыводитьСуммуВыделенныхЯчеек") = Ложь Тогда
	...
	РазрешитьДетальнуюНастройкуУниверсальныхОтчетов = ПравоПользователя("РазрешитьДетальнуюНастройкуУниверсальныхОтчетов",Неопределено);
	Элементы.РедактироватьСхему.Видимость = РазрешитьДетальнуюНастройкуУниверсальныхОтчетов;
	Элементы.ВосстановитьСтандартнуюСхему.Видимость = РазрешитьДетальнуюНастройкуУниверсальныхОтчетов;
	Элементы.ЗагрузитьСхемуИзФайла.Видимость = РазрешитьДетальнуюНастройкуУниверсальныхОтчетов;
	Элементы.ЗагрузитьСхемуИзФайла.Видимость = РазрешитьДетальнуюНастройкуУниверсальныхОтчетов;
...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("УправляемаяФорма") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			КонецЕсли;
		ИначеЕсли ТипЗнч(Результат) = Тип("ДанныеФормыСтруктура") Тогда
			УстанвитьДанныеФормы(Результат);
		КонецЕсли;
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	ПараметрыФормированияОтчета.Вставить("Заголовок",       ОтчетНаименованиеТекущегоВарианта);
	ПараметрыФормированияОтчета.Вставить("КлючОбъекта",     НастройкиОтчета.ПолноеИмя);
	ПараметрыФормированияОтчета.Вставить("Настройки",       Отчет.КомпоновщикНастроек.ПолучитьНастройки());
	ПараметрыФормированияОтчета.Вставить("ИмяОформлениеТаблицы", "");
	
	ПараметрыФормированияОтчета.Вставить("ОтчетСсылка",    НастройкиОтчета.ОтчетСсылка);
	ПараметрыФормированияОтчета.Вставить("ВариантСсылка",  НастройкиОтчета.ВариантСсылка);
	...
	Иначе
		...
		КонецЕсли;
		ПараметрыФормированияОтчета.Вставить("СхемаКомпоновки", ПолучитьИзВременногоХранилища(НастройкиОтчета.АдресСхемы));
...
Процедура ФоновоеЗаданиеЗагрузитьРезультат()
	...
	Если ЗначениеЗаполнено(РезультатФормирования.ИнформацияОбОшибке) Тогда
...
Процедура ЗаполнитьКомандыВыбораВариантов()
	...
	КонецЕсли;
	// +РАРУС
	ТаблицаВариантов.Свернуть("Ссылка, КлючВарианта, Наименование, Автор, ТипДоступа, ОбъектДоступа");
	...
	ДоступПоПользователю        = Перечисления.ТипыДоступаКВариантуОтчета.ПоПользователю;
	ДоступПоПодразделению       = Перечисления.ТипыДоступаКВариантуОтчета.ПоПодразделениюКомпании;
	ДоступПоГруппеПользователей = Перечисления.ТипыДоступаКВариантуОтчета.ПоГруппеПользователей;
	
	ПодразделениеПользователя = ПараметрыСеанса.ПодразделениеКомпании;
	ПользовательскиеГруппы    = Неопределено;
	// -РАРУС
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		// +РАРУС
		Если СтрокаТаблицы.ТипДоступа = ДоступПоПользователю
			И СтрокаТаблицы.ОбъектДоступа <> АвторизованныйПользователь Тогда
			Продолжить;
		ИначеЕсли СтрокаТаблицы.ТипДоступа = ДоступПоПодразделению
			И СтрокаТаблицы.ОбъектДоступа <> ПодразделениеПользователя Тогда
			Продолжить;
		ИначеЕсли СтрокаТаблицы.ТипДоступа = ДоступПоГруппеПользователей Тогда
			
			Если ПользовательскиеГруппы = Неопределено Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = "
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				...
				Запрос.УстановитьПараметр("Автор", АвторизованныйПользователь);
				ПользовательскиеГруппы = Запрос.Выполнить().Выгрузить();
			КонецЕсли;
			
			Если ПользовательскиеГруппы.Найти(СтрокаТаблицы.ОбъектДоступа, "ГруппаПользователей") = Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ОбновитьИнформациюОВариантеОтчета()
	...
	Если Выборка.Следующий() Тогда
		...
		ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
		Попытка
			НастройкиВарианта = Выборка.ВариантСсылка.Настройки.Получить();
			ОтчетОбъект.МакетФиксОбластьПерваяСтрока =          НастройкиВарианта.ДополнительныеСвойства.МакетФиксОбластьПерваяСтрока;
			ОтчетОбъект.МакетФиксОбластьПерваяКолонка = 		НастройкиВарианта.ДополнительныеСвойства.МакетФиксОбластьПерваяКолонка;
			ОтчетОбъект.МакетФиксОбластьПоследняяСтрока = 		НастройкиВарианта.ДополнительныеСвойства.МакетФиксОбластьПоследняяСтрока;
			ОтчетОбъект.МакетФиксОбластьПоследняяКолонка = 		НастройкиВарианта.ДополнительныеСвойства.МакетФиксОбластьПоследняяКолонка;
		Исключение
		КонецПопытки;
...
Процедура ДопНастройки(Команда)
	СтруктараПараметровДляНастроек = ПолучитьСтруктуруПараметровДляНастроек();
	ОткрытьФорму("Отчет.УниверсальныйОтчет.Форма.ФормаНастроек",СтруктараПараметровДляНастроек,ЭтаФорма);
КонецПроцедуры

&НаСервере
Функция ПолучитьСтруктуруПараметровДляНастроек()
	СтруктураНастроек = Новый Структура;
	
	Для каждого стрРеквизит из Метаданные.Отчеты.УниверсальныйОтчет.Реквизиты Цикл
		Попытка
			СтруктураНастроек.Вставить(стрРеквизит.Имя,Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства[стрРеквизит.Имя]);
		Исключение
		КонецПопытки;
	КонецЦикла;	
...
&НаСервере
Процедура УстанвитьДанныеФормы(Результат)
	ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	
	Для каждого стрРеквизит из Метаданные.Отчеты.УниверсальныйОтчет.Реквизиты Цикл
		Попытка
			ОтчетОбъект.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Вставить(стрРеквизит.Имя,Результат[стрРеквизит.Имя]);
		Исключение
		КонецПопытки;
	КонецЦикла;	
