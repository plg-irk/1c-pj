...
Процедура ОбработкаКомандыНаСервере(ПараметрКоманды, СтруктураПараметров)
	...
	// фильтры
	Если ТипЗнч(ПараметрКоманды)=Тип("ДокументСсылка.ЗаказПокупателя") ИЛИ ТипЗнч(ПараметрКоманды)=Тип("ДокументСсылка.ЗаказПоставщику") Тогда
...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	ОтчетыПлатформаКлиент.ОткрытьОтчет("Отчет.СостояниеЗаказовПоставщикам","ДляЗаказа",СтруктураПараметров.НастройкиВарианта,,, СтруктураПараметров.Отбор,, КонецДня(ОбщегоНазначенияКлиент.ДатаСеанса()),);
