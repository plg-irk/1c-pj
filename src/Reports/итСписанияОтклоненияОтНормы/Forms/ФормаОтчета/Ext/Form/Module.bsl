...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПрочитатьДанныеНормативаИШтрафа();
	
КонецПроцедуры

&НаСервере
Процедура ПрочитатьДанныеНормативаИШтрафа()
	
	УстановитьПривилегированныйРежим(Истина);
	
	Норматив = Константы.итНормативОтТоварооборота.Получить();
	Штраф    = Константы.итШтрафУправляющего.Получить();
...
Процедура ЗаполнитьКомандыВыбораВариантов()
	...
	Если НастройкиОтчета.Внешний Тогда // Добавить предопределенные варианты внешнего отчета в таблицу вариантов.
		...
		КонецЦикла;
	КонецЕсли;
	ТаблицаВариантов.Свернуть("Ссылка, КлючВарианта, Наименование, Автор");
