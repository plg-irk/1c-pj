...
Процедура Подключаемый_ОбработкаРезультатаОповещения(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Событие = "ПоляГруппировки" Тогда
		...
		ОтчетыПлатформаВызовСервера.ЗаполнитьНастройкиОтчета(ЭтотОбъект, ТекущиеДанные.ПоляГруппировки, РезультатЗакрытия.ПоляГруппировки.Элементы, Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляГруппировок, ТекущиеДанные);
	...
	ИначеЕсли ДополнительныеПараметры.Событие = "Выбор" Тогда
		...
		ОтчетыПлатформаВызовСервера.ЗаполнитьНастройкиОтчета(ЭтотОбъект, ТекущиеДанные.Выбор, РезультатЗакрытия.Выбор.Элементы, Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора, ТекущиеДанные);
		
	ИначеЕсли ДополнительныеПараметры.Событие = "Порядок" Тогда
		
		Если ТекущиеДанные = Неопределено Тогда
			ОтчетыПлатформаВызовСервера.ЗаполнитьНастройкиОтчета(ЭтотОбъект, Отчет.КомпоновщикНастроек.Настройки.Порядок, РезультатЗакрытия.Порядок.Элементы, Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляПорядка, ТекущиеДанные);
		Иначе
			ОтчетыПлатформаВызовСервера.ЗаполнитьНастройкиОтчета(ЭтотОбъект, ТекущиеДанные.Порядок, РезультатЗакрытия.Порядок.Элементы, Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляПорядка, ТекущиеДанные);
		КонецЕсли;
...
Функция ПолучениеВыгрузкиНастроекОтчета()
	...
	ПредставлениеФайла = НСтр("ru = 'Настройки отчета'") +" " 
		+ СтрЗаменить(ЭтотОбъект.Заголовок,НСтр("ru = 'Настройки отчета:'") + " ", "") + "'.xml";
...
КонецПроцедуры // ОтправитьНастройкиОтчетаПоПочтеНастройкаУчетнойЗаписиПредложена()
...
Процедура ОткрытьФормуЭксперта()
	...
	ОписаниеОповещенияИзмененияНастроек = Новый ОписаниеОповещения("Подключаемый_ОповещениеИзмененияНастроек", ЭтотОбъект.ВладелецФормы);
	
	ОткрытьФорму(КлючОбъекта+".ФормаВарианта", СтруктураПараметров, ЭтотОбъект.ВладелецФормы,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбновитьНастройкиСКД(ОтсекатьГруппировки = Ложь)
	...
	// Начало периода
	ДоступныйПараметр = НастройкиСКД.ПараметрыДанных.ДоступныеПараметры.НайтиПараметр(Новый ПараметрКомпоновкиДанных("НачалоПериода"));
...
Процедура Подключаемый_ВыбратьВариант(Команда)
	...
	Если РежимТолькоЭксперт Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru = 'Данный вариант отчета доступен только в режиме ""Эксперт"". 
...
Процедура ТаблицаВидДиаграммы(Команда)
	...
	Если СокрЛП(ТекущиеДанные.ТипДиаграммы) = "" Тогда
		...
		Элементы.ДекорацияТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ТекущиеДанные.ТипДиаграммы);
	...
	Если НайденныеСтроки.Количество()> 0 Тогда
		...
		Если СтрокиТаблицы.Количество()>0 Тогда
			...
			Если ТекущаяСтрока.ТипДиаграммы <> "" Тогда
				Элементы.ДекорацияТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ТекущаяСтрока.ТипДиаграммы);
			КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ КОМАНД ТАБЛИЦЫ ГРУППИРОВКИ СТРОК 
...
//////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ КОМАНД ТАБЛИЦЫ ГРУППИРОВКИ КОЛОНОК
...
//////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ КОМАНД ТАБЛИЦЫ ПОКАЗАТЕЛИ
...
//////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ КОМАНД ТАБЛИЦЫ ОТБОРЫ
...
Процедура ТипДиаграммыОтчетаПриИзменении(Элемент)
	...
	Элементы.ДекорацияТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(Элементы.СписокТаблиц.ТекущиеДанные.ТипДиаграммы);
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ГРУППИРОВКА СТРОКИ
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ГРУППИРОВКА КОЛОНОК
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ОТБОР
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ТАБЛИЦЫ ПОРЯДОК
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СтандартныеПодсистемы.ВариантыОтчетов.ИзменениеВарианта" Тогда
