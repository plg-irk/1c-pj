...
//
&НаКлиенте
Процедура Подключаемый_ОповещениеВыбораЗначенияОтбораРоль(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные.ПравоеЗначение = РезультатЗакрытия;
	ОтборПравоеЗначениеПриИзменении(Неопределено);
	
КонецПроцедуры //Подключаемый_ОповещениеВыбораЗначенияОтбораРоль()
...
//
&НаКлиенте
Процедура Подключаемый_ОповещениеВыбораЗначенияОтбораОбъект(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
...
КонецПроцедуры //Подключаемый_ОповещениеВыбораЗначенияОтбораОбъект()
...
&НаСервере
Процедура УстановитьУсловноеОформлениеОтбора()
	
	НовоеОформление = УсловноеОформление.Элементы.Добавить();
	
	ПолеКД = НовоеОформление.Поля.Элементы.Добавить();
	ПолеКД.Поле = Новый ПолеКомпоновкиДанных("ОтборПравоеЗначение");
	...
	ОтборПоРеквизиту.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Отбор.Заголовок");
	ОтборПоРеквизиту.ПравоеЗначение = НСтр("ru = 'Доступность права'");
	ОтборПоРеквизиту.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	
	ОтборПоРеквизиту = НовоеОформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборПоРеквизиту.Использование = Истина;
	ОтборПоРеквизиту.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Отбор.ПравоеЗначение");
	ОтборПоРеквизиту.ПравоеЗначение = 0;
	ОтборПоРеквизиту.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	
	НовоеОформление.Оформление.УстановитьЗначениеПараметра("Текст", "-");
	...
	ОтборПоРеквизиту.Использование = Истина;
	ОтборПоРеквизиту.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Отбор.ПравоеЗначение");
	ОтборПоРеквизиту.ПравоеЗначение = 1;
	ОтборПоРеквизиту.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура ОткрытьНастройкиОтчета(Команда)
	...
	Если НастройкиОтчета.Параметры.ЭкспертнаяНастройка Тогда
		ОткрытьФорму(КлючОбъекта+".Варианта", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОткрытьФорму(КлючОбъекта+".Настроек", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ОтборВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ОтборПравоеЗначение" Тогда
		...
		Если ТекущиеДанные.Заголовок = "Объект" Тогда
			Элементы.ОтборПравоеЗначение.КнопкаВыбора = Истина;
			Элементы.ОтборПравоеЗначение.РедактированиеТекста = Ложь;
		Иначе
			Элементы.ОтборПравоеЗначение.РедактированиеТекста = Истина;
		КонецЕсли;
...
Процедура ОтборПравоеЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОтборПоСписку(ТекущиеДанные.ВидСравнения) Тогда
		...
		Если ТекущиеДанные.Заголовок = "Роль" ИЛИ ТекущиеДанные.Заголовок = "Объект" Тогда
			СтруктураПараметров.Вставить("ВыборРоли", ТекущиеДанные.Заголовок = "Роль");
			ОткрытьФорму("Отчет.ОтчетПоПравамДоступа.Форма.ВыборЗначенияОтбораИзСписка", СтруктураПараметров, Элемент,,,, ОписаниеОповещенияВыбораЗначения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
		...
		КонецЕсли;
	ИначеЕсли ТекущиеДанные.Заголовок = "Роль" Тогда
		СтандартнаяОбработка = Ложь;
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВыборРоли", Истина);
		
		ОписаниеОповещенияВыбораЗначения = Новый ОписаниеОповещения("Подключаемый_ОповещениеВыбораЗначенияОтбораРоль", ЭтотОбъект);
	...
	ИначеЕсли ТекущиеДанные.Заголовок = "Объект" Тогда
		СтандартнаяОбработка = Ложь;
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВыборРоли", Ложь);
		
		ОписаниеОповещенияВыбораЗначения = Новый ОписаниеОповещения("Подключаемый_ОповещениеВыбораЗначенияОтбораОбъект", ЭтотОбъект);
		ОткрытьФорму("Отчет.ОтчетПоПравамДоступа.Форма.ФормаВыбораДанных", ПараметрыФормы, Элемент,,,, ОписаниеОповещенияВыбораЗначения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьУсловноеОформлениеОтбора();
...
&НаКлиенте
Процедура Подключаемый_ОбработкаВыбораПравДоступа(РезультатОповещения, ДополнительныеПараметры = Неопределено) Экспорт
	
	Если НЕ РезультатОповещения = Неопределено Тогда
		ПраваДоступа = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Права"));
		ПраваДоступа.Значение = РезультатОповещения;
	КонецЕсли;
...
Процедура ПараметрыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Права", Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("Права")).Значение);
	
	ОбработкаОповещения = Новый ОписаниеОповещения("Подключаемый_ОбработкаВыбораПравДоступа", ЭтотОбъект);
	
	ОткрытьФорму("Отчет.ОтчетПоПравамДоступа.Форма.ФормаВыбораПрав", ПараметрыФормы, ЭтотОбъект,,,, ОбработкаОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
