...
Процедура Подключаемый_ОповещениеВыбораТипаДиаграммы(ВыбранныйТипДиаграммы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныйТипДиаграммы <> Неопределено Тогда
		...
		Элементы.ДекорацияТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ВыбранныйТипДиаграммы);
...
Процедура ОбновитьТипДиаграммы()
	...
	Элементы.ГруппаТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ТипДиаграммыОтчета);
	Элементы.ДекорацияТипДиаграммы.Картинка = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ТипДиаграммыОтчета);
...
// Процедура выполняет изменение порядка сортировки данных отчета
//
&НаКлиенте
Процедура ПриИзмененииПорядкаСортировкиОтчета()
	
	ПараметрыПорядка = Отчет.КомпоновщикНастроек.Настройки.Порядок;
	ПараметрыПорядка.Элементы.Очистить();
	
	Если Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[0].Значение Тогда
		Отчет.НаправлениеСортировки = Элементы.НаправлениеСортировки.СписокВыбора[0].Значение;
		Элементы.НаправлениеСортировки.Доступность = Ложь;
		Возврат;
	Иначе
		Элементы.НаправлениеСортировки.Доступность = Истина;
	КонецЕсли;
	
	ЭлементПорядка = ПараметрыПорядка.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
	
	// По торговой наценке(ПоТорговойНаценке)
	Если Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[1].Значение Тогда
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("СуммаТорговойНаценкиОборот");
	// По сумме продаж со скидкой(ПоСуммеПродажСоСкидкой)
	ИначеЕсли Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[2].Значение Тогда
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("СуммаПродажСоСкидкойОборот");
	// По себестоимости(ПоСебестоимости)
	ИначеЕсли Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[3].Значение Тогда
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("СебестоимостьОборот");
	// По количеству(ПоКоличеству)
	ИначеЕсли Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[4].Значение Тогда
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("КоличествоОборот");
	// По сумме скидок(ПоСуммеСкидок)
	ИначеЕсли Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[5].Значение Тогда
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("СуммаСкидкиОборот");
	// По проценту скидок(ПоПроцентуСкидок)
	Иначе
		ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("ПроцентСкидкиОборот");
	КонецЕсли;
	
	// Направление сортировки (возрастание/убывание)
	Если Отчет.НаправлениеСортировки = Элементы.НаправлениеСортировки.СписокВыбора[0].Значение Тогда
...
// Функция выполняет определение типа номенклатуры
//
// Параметры:
//	Ссылка - СправочникСсылка.Номенклатура - Ссылка на номенклатуру, для которой выполняется проверка.
//	
// Возвращаемое значение:
//	Булево - Это номенклатура
//
&НаСервере
Функция ПроверитьНаПринадлежностьНоменклатуре(Ссылка)
...
// Функция выполняет определение типа подразделения
//
// Параметры:
//	Ссылка - СправочникСсылка.Номенклатура - Ссылка на номенклатуру, для которой выполняется проверка.
//	
...
//
&НаСервере
Функция ПроверитьНаПринадлежностьПодразделению(Ссылка)
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ПодразделенияКомпании") Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции // ПроверитьНаПринадлежностьПодразделению()

// Функция выполняет определение типа Покупателя
//
// Параметры:
//	Ссылка - СправочникСсылка.Номенклатура - Ссылка на номенклатуру, для которой выполняется проверка.
//	
...
//
&НаСервере
Функция ПроверитьНаПринадлежностьПокупателю(Ссылка)
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Контрагенты") Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции // ПроверитьНаПринадлежностьПокупателю()

// Функция выполняет определение реквизита ЭтоГруппа
//
// Параметры:
//	Ссылка - СправочникСсылка.Номенклатура - Ссылка на номенклатуру, для которой выполняется проверка.
//	
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Направление сортировки".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура НаправлениеСортировкиПриИзменении(Элемент)
	...
	ПриИзмененииПорядкаСортировкиОтчета();
	
КонецПроцедуры

// Обработчик события возникающего на клиенте при изменении данных реквизита "Режим сортировки".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
&НаКлиенте
Процедура РежимСортировкиПриИзменении(Элемент)
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ФОРМЫ ОНОСЯЩИХСЯ К ТАБЛИЧНОМУ ДОКУМЕНТУ
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ПроверитьНаПринадлежностьНоменклатуре(Ссылка) Тогда
		Если НЕ ПроверитьЭтоГруппа(Ссылка) Тогда
			ОткрытьФорму("Справочник.Номенклатура.ФормаОбъекта", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ОткрытьФорму("Справочник.Номенклатура.ФормаГруппы", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	ИначеЕсли ПроверитьНаПринадлежностьПодразделению(Ссылка) Тогда
		Если НЕ ПроверитьЭтоГруппа(Ссылка) Тогда
			ОткрытьФорму("Справочник.ПодразделенияКомпании.ФормаОбъекта", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ОткрытьФорму("Справочник.ПодразделенияКомпании.ФормаГруппы", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	ИначеЕсли ПроверитьНаПринадлежностьПокупателю(Ссылка) Тогда
		Если НЕ ПроверитьЭтоГруппа(Ссылка) Тогда
			ОткрытьФорму("Справочник.Контрагенты.ФормаОбъекта", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Иначе
			ОткрытьФорму("Справочник.Контрагенты.ФормаГруппы", Новый Структура("Ключ",Ссылка),ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ТекущийТипДиаграммы Из ТипДиаграммы Цикл
		...
		НовоеОформление.Картинка   = ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ИмяТипаДиаграммы);
		
		СписокВыбора.Добавить(ИмяТипаДиаграммы, СокрЛП(ТекущийТипДиаграммы),, ОтчетыПлатформаВызовСервера.ПолучитьКартинкуДиаграммы(ИмяТипаДиаграммы));
	...
	Отчет.РежимСортировки = Элементы.РежимСортировки.СписокВыбора[0].Значение;
	Отчет.НаправлениеСортировки = Элементы.НаправлениеСортировки.СписокВыбора[0].Значение;
	Элементы.НаправлениеСортировки.Доступность = Ложь;
