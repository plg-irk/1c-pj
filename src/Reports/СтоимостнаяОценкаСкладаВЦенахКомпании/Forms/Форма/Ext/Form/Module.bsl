...
// Процедура - Обработчик события возникающий при изменении типов цен.
//
// Параметры:
// Элемент - ТаблицаФормы - Элемент управления, в котором возникло данное событие.
//
&НаКлиенте
Процедура ТипыЦенПриИзменении(Элемент)
	ЗаполнитьПараметрТипыЦен();
КонецПроцедуры

// Процедура - Заполняет параметр СКД ТипыЦен, из текущей формы.
//
&НаСервере
Процедура ЗаполнитьПараметрТипыЦен()
	
	ПараметрыОтчета = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных;
	
	СписокТиповЦен = Новый СписокЗначений;
	Для каждого ТипЦен Из Отчет.ТипыЦен Цикл 
		Если ТипЦен.Использование И ТипЗнч(ТипЦен.ТипЦены) <> Тип("Строка") Тогда
			СписокТиповЦен.Добавить(ТипЦен.ТипЦены,,Ложь);
		ИначеЕсли ТипЦен.ТипЦены = "Количество" Тогда
			ПараметрыОтчета.УстановитьЗначениеПараметра("Количество", ТипЦен.Использование);
		ИначеЕсли ТипЦен.ТипЦены = "Себестоимость" Тогда
			ПараметрыОтчета.УстановитьЗначениеПараметра("Себестоимость", ТипЦен.Использование);
		ИначеЕсли ТипЦен.ТипЦены = НСтр("ru = 'Себестоимость без НДС'") Тогда
			ПараметрыОтчета.УстановитьЗначениеПараметра("СебестоимостьБезНДС", ТипЦен.Использование);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектНаСервере.ПриИзмененииНастроекФормы(ЭтаФорма);
...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	ПараметрыДанных.УстановитьЗначениеПараметра("Валюта", Константы.ВалютаРегламентированногоУчетаОрганизаций.Получить());
