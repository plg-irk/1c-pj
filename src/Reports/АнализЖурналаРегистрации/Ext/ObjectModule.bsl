...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			// Заполним поле текущим пользователем
			Пользователь = ПараметрыСеанса.ТекущийПользователь;
			НастройкиОтчета.ПараметрыДанных.Элементы.Найти("Пользователь").Значение = Пользователь;
			КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиОтчета);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не было заполнено значение поля Пользователь. Выбран автоматически текущий пользователь.'"), , );
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Если Не ЗначениеЗаполнено(ПользователиИГруппы) Тогда
			// Заполним список всеми пользователями
			ПользователиИГруппы = Новый СписокЗначений;
			Запрос = Новый Запрос;
			Запрос.Текст = 
				"ВЫБРАТЬ
			...
			Пока Выборка.Следующий() Цикл
				Если НЕ Отчеты.АнализЖурналаРегистрации.ИмяПользователяИБ(Выборка.Ссылка) = Неопределено Тогда
					ПользователиИГруппы.Добавить(Выборка.Ссылка,, Истина);
				КонецЕсли;
			КонецЦикла;
			НастройкиОтчета.ПараметрыДанных.Элементы.Найти("ПользователиИГруппы").Значение = ПользователиИГруппы;
			КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиОтчета);
			
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не было заполнено значение поля Пользователи. Выбраны автоматически все пользователи системы.'"), , );
		КонецЕсли;
