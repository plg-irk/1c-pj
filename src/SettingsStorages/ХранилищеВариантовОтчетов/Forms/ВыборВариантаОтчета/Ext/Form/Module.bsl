...
Процедура ЗаполнитьСписокВариантов()
	...
	|	КОНЕЦ КАК Описание,
	|	ВЫБОР
	|		КОГДА НЕ ВариантыОтчетов.Пользовательский И НЕ ВариантыОтчетов.ВидимостьПоУмолчаниюПереопределена
	...
	|	ВариантыОтчетов.ТипДоступа,
	|	ВариантыОтчетов.ОбъектДоступа,
	...
	|			ИЛИ ВариантыОтчетов.ПометкаУдаления = ЛОЖЬ)" + ?(ПоказыватьЛичныеВариантыОтчетовДругихАвторов, "", "
	|	И (ВариантыОтчетов.Автор = &ТекущийПользователь
	|			ИЛИ ВариантыОтчетов.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.БезОграничения)
	|			ИЛИ (ВариантыОтчетов.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоПользователю) И ВариантыОтчетов.ОбъектДоступа = &ТекущийПользователь)
	|			ИЛИ (ВариантыОтчетов.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоПодразделениюКомпании) И ВариантыОтчетов.ОбъектДоступа = &ПодразделениеКомпании)
	|			ИЛИ (ВариантыОтчетов.ТипДоступа = ЗНАЧЕНИЕ(Перечисление.ТипыДоступаКВариантуОтчета.ПоГруппеПользователей) И ВариантыОтчетов.ОбъектДоступа В 
	|		(ВЫБРАТЬ
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		КонецЕсли;
		// +РАРУС
		Вариант.КартинкаАвтора = ?(СтрокаТаблицы.ТипДоступа=Перечисления.ТипыДоступаКВариантуОтчета.БезОграничения, -1, 0);
		// -РАРУС
	КонецЦикла;
