...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Контекст.ПолныеПраваНаВарианты Тогда
		// +РАРУС
		Элементы.ПараметрыДоступа.ТолькоПросмотр = Истина;
		// -РАРУС
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	// +Рарус
	ВидПериодаПриИзменении(Неопределено) // -Рарус
КонецПроцедуры
...
Процедура ВидПериодаПриИзменении(Элемент)
	...
	Элементы.НачалоПериода.АвтоОтметкаНезаполненного =  Произвольный;
	Элементы.КонецПериода.АвтоОтметкаНезаполненного =  Произвольный;
...
Процедура ВыполнитьПакет(Результат, Пакет) Экспорт
	...
	Если Пакет.ПроверитьСтраницу1 = Истина Тогда
		...
		// Не указаны даты для произвольного периода
		Если ВидПериода = "Произвольный" Тогда
			Если Не ЗначениеЗаполнено(НачалоПериода) ИЛИ Не ЗначениеЗаполнено(КонецПериода)Тогда
				ТекстОшибки = НСтр("ru = 'Для произвольного периода должны быть заполнены поля ""Начало периода"" и ""Конец периода""'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "НачалоПериода");
				Возврат;
			КонецЕсли;
			Если НачалоПериода.Дата>КонецПериода.Дата Тогда
				ТекстОшибки = НСтр("ru = 'Начало периода не может быть позже конца периода'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ВариантНаименование");
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Если Пакет.ЗакрытьПослеЗаписи = Истина Тогда
		...
		Если ВладелецФормы <> Неопределено
			...
			ПараметрыВарианта.Вставить("ВариантНаименование", ВариантНаименование);
	...
	Если ИмяСобытия = "ПриСозданииНаСервере"
		...
		Если Вариант = Неопределено Тогда
			...
			//Если Не Форма.Контекст.ПолныеПраваНаВарианты Тогда
				// +РАРУС
				Форма.ВидПериода    = "Месяц";
				Форма.ТипДоступа    = ПредопределенноеЗначение("Перечисление.ТипыДоступаКВариантуОтчета.ПоПользователю");
				Форма.ОбъектДоступа = Форма.Контекст.ТекущийПользователь;
				// -РАРУС
			//КонецЕсли;
		Иначе
			...
			Если ПравоЗаписиВарианта Тогда
				...
				// +РАРУС
				Форма.ВидПериода    = Вариант.ВидПериода;
				Если Форма.ВидПериода = "Произвольный" Тогда
					Форма.НачалоПериода = Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[Вариант.НачалоПериода]);
					Форма.КонецПериода  = Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[Вариант.КонецПериода]);
				КонецЕсли;
				Форма.ТипДоступа    = Вариант.ТипДоступа;
				Форма.ОбъектДоступа = Вариант.ОбъектДоступа;
				// -РАРУС
		...
		// +РАРУС
		НастроитьОбъектДоступа(Форма);
		Произвольный = (Форма.ВидПериода = "Произвольный");
		...
		ДоступИзмененияПоПользователям = ИТ_СпециальныйМодульСервер.ПроверитьПравоНаСервере("итИзменениеВариантовОтчетаДляВсехПользователей");
		СтандартноеОтображение         = Истина;
		Если Не ДоступИзмененияПоПользователям Тогда
			Форма.Элементы.ПараметрыДоступа.Доступность = Ложь;
			Если Форма.ТипДоступа    <> ПредопределенноеЗначение("Перечисление.ТипыДоступаКВариантуОтчета.ПоПользователю") Тогда
				Форма.Элементы.Далее.Доступность 		= Ложь;
				Форма.Элементы.Сохранить.Доступность 	= Ложь;   
	            СтандартноеОтображение 			= Ложь;
			КонецЕсли;
		КонецЕсли;
	
		Если СтандартноеОтображение Тогда 
			Если БудетЗаписанНовый Тогда
				Форма.Элементы.ЧтоБудетДальше.ТекущаяСтраница   = Форма.Элементы.Новый;
				// +РАРУС
				//Форма.Элементы.СтраницыСбросить.ТекущаяСтраница = Форма.Элементы.ФлажокСброситьСкрыт;
...
Процедура ПроверитьИЗаписатьНаСервере(Пакет)
	...
	Если ВидПериода = "Произвольный" Тогда
		ВариантОбъект.НачалоПериода = ОтчетыПлатформаКлиентСервер.СоответствиеСтандартнойДатыНачала(НачалоПериода);
		ВариантОбъект.КонецПериода = ОтчетыПлатформаКлиентСервер.СоответствиеСтандартнойДатыНачала(КонецПериода);
	КонецЕсли;
...
Процедура ЗаполнитьСписокВариантов(ОбновитьФорму)
	...
	Если Контекст.ЭтоВнешний Тогда
		...
		// +РАРУС
		ДоступБезОграничения = ПредопределенноеЗначение("Перечисление.ТипыДоступаКВариантуОтчета.БезОграничения");
		ВидПериодаМесяц      = "Месяц";
		// -РАРУС
		...
		Для Каждого ВариантНастроекКД Из ОтчетОбъект.СхемаКомпоновкиДанных.ВариантыНастроек Цикл
			...
			// +РАРУС
			Вариант.ВидПериода   = ВидПериодаМесяц;
			Вариант.ТипДоступа   = ДоступБезОграничения;
			// -РАРУС
