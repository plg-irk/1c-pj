...
//
&НаСервере
Процедура УстановитьЗаголовокЭлемента() Экспорт
	
	// Получаем объект описания метаданных по переданному имени
	ОбъектМетаданных = УправлениеДиалогомСервер.ПолучитьМетаданныеОбъектаФормы(ЭтотОбъект);
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПредставлениеЭлемента = ?(ОбъектМетаданных.ОсновноеПредставление=Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСправочника.ВВидеКода, Объект.Код, Объект.Наименование);
	Иначе
		ПредставлениеЭлемента =  "создание";
	КонецЕсли;
	
	// Сформируем итоговое представление динамического списка
	ЭтотОбъект.Заголовок = ПредставлениеОбъекта + ": " + ПредставлениеЭлемента;
	
КонецПроцедуры // УстановитьЗаголовокЭлементаСправочника()

// заполняет виды предопределенных операций для поля ввода "путь к объекту"
//
&НаСервере
...
// Обработчик изменения реквизита "СпособРасчета" на сервере
//
&НаСервере
...
// Обработчик изменения реквизита "СпособРасчета" на клиенте
//
// Параметры:
//	Элемент - Элемент формы - Элемент формы вызвавший событие.
//
&НаКлиенте
...
// Обработчик начала выбора реквизита "ПутьКОбъекту" на клиенте
//
// Параметры:
//	Элемент              - Элемент формы   - Элемент формы вызвавший событие
//	ДанныеВыбора         - Список значений - Список данных выбора
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЕстьРеквизит(ЭтотОбъект, "ПараметрыЗапретаРедактированияРеквизитов") Тогда
